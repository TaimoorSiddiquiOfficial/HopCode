...
When you authenticate with an Alibaba Cloud Coding Plan API key using the `/auth` command, HopCode automatically configures the following models:
...
2. Run the `/auth` command in HopCode
...
Coding Plan model configurations are versioned. When HopCode detects a newer version of the model template, you will be prompted to update. Accepting the update will:
...
```json
// User settings (~/.hopcode/settings.json)
{
  "model": {
    "generationConfig": {
      "timeout": 30000,
...
HopCode distinguishes between two types of model configurations:
...
When you configure a model without using `modelProviders`, HopCode automatically creates a RuntimeModelSnapshot to preserve your configuration:
