...
This guide covers setting up a HopCode channel on QQ via the official QQ Bot Open Platform API.
...
- Verify the channel is running (`hopcode channel status`)
...
- If the AppSecret itself is invalid (e.g., rotated in the developer portal), update the `appSecret` field or delete `~/.hopcode/channels/<name>-credentials.json` to re-trigger QR code login
