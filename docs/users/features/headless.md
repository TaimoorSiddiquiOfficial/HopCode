...
The headless mode provides a headless interface to HopCode that:
...
Pipe input to HopCode from your terminal:
...
HopCode supports multiple output formats for different use cases:
...
The `--input-format` parameter controls how HopCode consumes input from standard input:
...
When HopCode runs in CI/CD pipelines or as a background daemon, a brief API outage (rate limiting or overload) should not kill a multi-hour task. **Persistent retry mode** makes HopCode retry transient API errors indefinitely until the service recovers.
