...
5. **Verify --experimental-lsp flag**: Make sure you're using the flag when starting HopCode
...
HopCode supports Claude Code-style `.lsp.json` configuration files in the language-keyed format defined in the [Claude Code plugins reference](https://code.claude.com/docs/en/plugins-reference#lsp-servers). If you're migrating from Claude Code, use the language-as-key layout in your configuration.
...
Claude Code LSP plugins can also supply `lspServers` in `plugin.json` (or a referenced `.lsp.json`). HopCode loads those configs when the extension is enabled, and they must use the same language-keyed format.
