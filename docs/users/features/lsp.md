...
LSP support in HopCode works by connecting to language servers that understand your code. Once you configure servers via `.lsp.json` (or extensions), HopCode can start them and use them to:
...
LSP is an experimental feature in HopCode. To enable it, use the `--experimental-lsp` command line flag:

```bash
hopcode --experimental-lsp
```

LSP servers are configuration-driven. You must define them in `.lsp.json` (or via extensions) for HopCode to start them.
...
HopCode exposes LSP functionality through the unified `lsp` tool. Here are the available operations:
...
1. **Verify `--experimental-lsp` flag**: Make sure you're using the flag when starting HopCode
...
LSP debug logs are automatically written to session log files in `~/.hopcode/debug/`. To check LSP-related entries:

```bash
# View the latest session log
grep '\[LSP\]' ~/.hopcode/debug/latest
...
Extensions can provide LSP server configurations through the `lspServers` field in their `plugin.json`. This can be either an inline object or a path to a `.lsp.json` file. HopCode loads these configs when the extension is enabled. The format is the same language-keyed layout used in project `.lsp.json` files.
...
Use the `--experimental-lsp` flag when starting HopCode:
...
Check the debug log for `[LSP]` entries (`grep '\[LSP\]' ~/.hopcode/debug/latest`), or verify the process directly with `ps aux | grep <server-name>`.
