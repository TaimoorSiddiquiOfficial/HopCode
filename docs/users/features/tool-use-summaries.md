...
HopCode can generate a short, git-commit-subject-style label after each tool batch completes, summarizing what the batch accomplished. The label appears inline: for a completed tool group in the main view it replaces the generic `Tool × N` header; when the group is force-expanded (in the `Ctrl+O` full-detail transcript, or for error / user-initiated batches) it appears as a dim `● <label>` line below the group.
...
After a tool batch finalizes, HopCode fires a fire-and-forget call to the configured fast model with:
...
`HOPCODE_EMIT_TOOL_USE_SUMMARIES` overrides the `experimental.emitToolUseSummaries` setting for the current session:

- `HOPCODE_EMIT_TOOL_USE_SUMMARIES=0` or `=false` — force off.
- `HOPCODE_EMIT_TOOL_USE_SUMMARIES=1` or `=true` — force on.
...
3. **Main-agent batches only.** The trigger lives in the main session's turn loop (`useHopCodeStream`), so:
...
- Disable the feature entirely with `experimental.emitToolUseSummaries: false` (or `HOPCODE_EMIT_TOOL_USE_SUMMARIES=0`).
...
If you do not want the extra cost, turn the feature off via `experimental.emitToolUseSummaries: false` or `HOPCODE_EMIT_TOOL_USE_SUMMARIES=0`.
