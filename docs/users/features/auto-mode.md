...
When you're in Auto Mode and the agent tries to run a tool, HopCode
...
1. **acceptEdits fast-path** — Edit / Write whose target path is inside
   ...
   **Exception:** writes to HopCode's own self-modification surfaces
   (`.hopcode/settings*.json`, `HOPCODE.md`, `AGENTS.md`, `HOPCODE.local.md`,
   configured context filenames, `.hopcode/rules/`, `.hopcode/commands/`,
   `.hopcode/agents/`, `.hopcode/skills/`, `.hopcode/hooks/`, `.mcp.json`) and
...
- `permissions.allow` rules with specific specifiers (e.g.
  ...
  into permission to rewrite HopCode settings, commands, hooks,
...
When you enter Auto Mode, HopCode temporarily removes these rules from
...
If you genuinely need these broad rules, use IZN mode instead.
  ...
  "permissions": {
    "autoMode": {
      "hints": {
        ...
        "softDeny": [
          "Editing HopCode settings unless I explicitly ask for the exact change",
...
the main agent. If you've configured HopCode to use a self-hosted
