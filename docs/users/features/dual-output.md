...
Dual Output is a sidecar mode for the interactive TUI: while HopCode keeps
...
Embed HopCode inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs HopCode with a task prompt. The human sees the TUI in the
...
Integration tests spawn HopCode headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns HopCode
...
A typical embedding parent process spawns HopCode with both channels:
...
next HopCode launch, since the bridge is constructed once during
startup.
...
The most realistic shape: a parent process spawns HopCode, tails
...
# HopCode TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older HopCode versions won't emit `protocol_version`. Treat the field
...
rl.on('line', (line) => {
  ...
  if (ev.type === 'system' && ev.subtype === 'session_start') {
    ...
    if (v < 1) {
      console.error(
        'hopcode dual output is present but protocol < 1; ' +
          'falling back to best-effort behavior',
      );
    } else {
      console.log('hopcode dual output protocol v' + v);
    }
  }
});
```
