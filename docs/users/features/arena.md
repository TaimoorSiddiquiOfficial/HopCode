...
1. **Worktree setup**: HopCode creates isolated Git worktrees for each agent at `~/.hopcode/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
- Ensure you have write access to the worktree base directory (`~/.hopcode/arena/` by default)
...
Agent Arena is one of several planned multi-agent modes in HopCode. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple HopCode sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
