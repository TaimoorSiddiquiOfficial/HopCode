...
With MCP servers connected, you can ask HopCode to:
...
before showing any UI), set `HOPCODE_CODE_LEGACY_MCP_BLOCKING=1` in your
...
Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from HopCode’s perspective).
