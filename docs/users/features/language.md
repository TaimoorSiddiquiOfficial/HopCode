...
On first startup, HopCode detects your system locale and sets the UI language automatically.
...
On first startup, if no `output-language.md` file exists, HopCode automatically creates one based on your system locale. For example:
...
> [!note]
>
> After changing the output language, restart HopCode for the change to take effect.
