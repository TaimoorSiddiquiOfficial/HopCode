...
This workflow acts as a central dispatcher for HopCode CLI, routing requests to the appropriate workflow based on the triggering event and the command provided in the comment. For a detailed guide on how to set up the dispatch workflow, go to the [HopCode Dispatch workflow documentation](./common-workflow).
...
### HopCode CLI Assistant

This type of action can be used to invoke a general-purpose, conversational HopCode AI assistant within the pull requests and issues to perform a wide range of tasks. For a detailed guide on how to set up the general-purpose HopCode CLI workflow, go to the [HopCode Assistant workflow documentation](./common-workflow).
...
- <a name="__input_use_pnpm"></a><a href="#user-content-__input_use_pnpm"><code>use*pnpm</code></a>: *(Optional, default: `false`)\_ Whether or not to use pnpm instead of npm to install hopcode-cli
...
- <a name="__output_summary"></a><a href="#user-content-__output_summary"><code>summary</code></a>: The summarized output from the HopCode CLI execution.

- <a name="__output_error"></a><a href="#user-content-__output_error"><code>error</code></a>: The error output from the HopCode CLI execution, if any.
...
| --------------------- | ------------------------------------------------------- | -------- | -------- | ------------------------- |
| `DEBUG`               | Enables debug logging for the HopCode CLI.              | Variable | No       | Never                     |
| `HOPCODE_CLI_VERSION` | Controls which version of the HopCode CLI is installed. | Variable | No       | Pinning the CLI version   |
...
This action requires authentication to the GitHub API and optionally to HopCode services.
...
The HopCode CLI can be extended with additional functionality through extensions.
