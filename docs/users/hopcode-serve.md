...
> **🚧 v0.16-alpha**: `hopcode serve` first ships to npm in v0.16-alpha as **text-only chat / coding** with **local-only deployment**. Image / file attachments on the prompt path, containerized deployment (Docker / k8s / nginx reverse-proxy), and remote / multi-daemon hardening land in a follow-up patch when an enterprise pilot is committed. See [v0.16-alpha known limits](#v016-alpha-known-limits) for the full deferred list.

> **Status:** Stage 1 (experimental). The protocol surface is locked at the §04 routes table from issue [#3803](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/3803). Stage 1.5 (`hopcode --serve` flag — TUI co-hosts the same HTTP server) and Stage 2 (in-process refactor + `mDNS`/OpenAPI/WebSocket/Prometheus polish) are immediately downstream.
>
...
- **Built-in Web Shell UI** — `hopcode serve` serves the browser-based Web Shell at its root (`http://127.0.0.1:4170/`) out of the box; run `hopcode serve --open` to launch it in your browser automatically. It is served on the same origin as the API, so no second port or reverse proxy is needed. Pass `--no-web` for an API-only daemon.
...
- **Experimental daemon-managed channels** — start with `hopcode serve --channel <name>`, or start without a channel and select one later with `qwen channel set`. Workers are separate processes owned by the daemon lifecycle. Their selection can be queried, replaced, reloaded, and stopped without restarting the daemon.
- **Remote runtime control** — change a session's approval mode (`POST /session/:id/approval-mode`), toggle a tool (`POST /workspace/tools/:name/enable`) or loaded skill (`POST /workspace/skills/:name/enable`) per workspace, scaffold an empty `HOPCODE.md` (`POST /workspace/init`, mechanical only — does NOT call the model; for AI-fill, follow up with `POST /session/:id/prompt`), restart a single MCP server with a budget pre-check (`POST /workspace/mcp/:server/restart`), or add/remove MCP servers at runtime without a daemon restart (`POST /workspace/mcp/servers`, `DELETE /workspace/mcp/servers/:name`). All strict-gated — configure `--token` first.
- **Session recap** ([#4175](https://github.com/QwenLM/hopcode/issues/4175) follow-up) — fetch a one-sentence "where did I leave off" summary of an active session (`POST /session/:id/recap`). Wraps core's `generateSessionRecap` as a side-query against the fast model; pollutes neither the main chat history nor the SSE stream. Non-strict gate (same posture as `/prompt`); SDK helper `client.recapSession(sessionId)`.
...
The first npm release of `hopcode serve` (v0.16-alpha) is intentionally narrow — text-only chat / coding for developers running the daemon on their own machine. The list below makes the deferred surface explicit so adopters can plan around it; everything here is on the v0.16.x patch roadmap or a near-term follow-up release.
...
- ✅ Local launch via `systemd` / `launchd` / `nohup &` / `tmux` — see [Local launch templates](./hopcode-serve-deploy-local.md)
- ✅ Bring-your-own bearer token via `HOPCODE_SERVER_TOKEN` env var ([Authentication](#authentication) for setup)
...
- ✅ Boot-time security gate (refuses non-loopback bind without a token, [PR 15 / #4236](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4236))
...
# → hopcode serve: bearer auth disabled (loopback default). Set HOPCODE_SERVER_TOKEN to enable.
```
...
```bash
# Start one configured channel under hopcode serve
hopcode serve --channel telegram

# Start several configured channels under daemon-owned workspace workers
hopcode serve --channel telegram --channel feishu
...
Each selected channel's `cwd` must resolve to a registered workspace, and channels are grouped by that owning workspace: a single-workspace daemon runs one worker (unchanged from before); a multi-workspace daemon (`--workspace` repeated) runs one worker per workspace that owns a selected channel, each bound to that workspace's cwd, `hopcode_daemon_workspace`, and env overlay. To host a channel in a non-primary workspace, define it in that workspace's own `.hopcode/settings.json` (no `cwd` needed) or set an explicit `cwd` equal to the workspace path; a channel defined only in user/system scope with no `cwd` is ambiguous across workspaces and causes a boot error. `--channel all` stays primary-only (it hosts the primary workspace's channels) and cannot be combined with named channels.
...
```bash
export HOPCODE_SERVER_TOKEN="$(openssl rand -hex 32)"
hopcode serve --hostname 0.0.0.0 --port 4170
# → boot refuses without HOPCODE_SERVER_TOKEN
```

Clients then send `Authorization: Bearer $HOPCODE_SERVER_TOKEN` on every request. `/health` is exempted **only on loopback binds** so k8s/Compose liveness probes inside the pod (where the daemon listens on `127.0.0.1`) don't need credentials. On non-loopback binds (`--hostname 0.0.0.0` etc.) `/health` requires the token like every other route — otherwise an attacker can probe arbitrary addresses to confirm the daemon's existence. Use `/capabilities` to verify your token is correct end-to-end (it always requires auth):
...
>
> ```bash
> hopcode serve --require-auth --token "$(openssl rand -hex 32)"
...
```bash
curl -H "Authorization: Bearer $HOPCODE_SERVER_TOKEN" http://your-host:4170/capabilities
...
# → hopcode serve listening on https://0.0.0.0:4170
```
...
| `--require-auth`                        | `false`         | Refuse to start without a bearer token, even on loopback. Hardens the `127.0.0.1` developer default for shared dev hosts / CI runners / multi-tenant workstations where any local user can hit the listener. Boots only with `--token` or `hopcode_server_token` set; gates `/health` behind the bearer too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `--channel <name\|all>`                 | —               | Experimental daemon-managed channel worker. Repeat the flag to select multiple configured channels, or pass `all` to start every configured channel. `all` cannot be combined with named channels. Selected channel `cwd` values must resolve to a registered workspace; a multi-workspace daemon runs one worker per owning workspace. The worker is owned by `hopcode serve`; stop the daemon to stop serve-managed channels.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
>
> ```sh
> hopcode serve --mcp-client-budget=10 --mcp-budget-mode=warn
> # later, after telemetry shows your real-world distribution:
> hopcode serve --mcp-client-budget=10 --mcp-budget-mode=enforce
> ```
>
...
>
> **Push events (issue [#4175](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4175) PR 14b).** SDK clients subscribed to `GET /session/:id/events` receive typed frames when budget thresholds cross — `mcp_budget_warning` (synthetic, fires once per upward 75% crossing with hysteresis re-arm at 37.5%, advertised via `mcp_guardrail_events`) and `mcp_child_refused_batch` (coalesced once per discovery pass under `enforce` mode; length-1 from `readResource` lazy-spawn refusal). The snapshot at `GET /workspace/mcp` is still the source-of-truth for state-after-reconnect; events are change-edges. Useful when dashboarding in real-time without polling.
...
> (issue [#4514](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4514) T2.9)
...
To handle multiple **users or security principals** (each with an independent token, quota, audit log, sandbox, or process fault boundary) or to scale beyond one process's reach (cold-start budget, FD count, RSS), spawn one daemon per principal behind an external orchestrator. Each such daemon may still host several workspaces for that principal. The orchestrator (multi-tenancy / OIDC / Quota / Audit / k8s) is **out of scope** for the hopcode project — see issue [#3803](https://github.com/QwenLM/hopcode/issues/3803) "External Reference Architecture" for the design pointers.
...
Stage 1's contract is sized for prototyping. Per [#3889 chiga0 downstream-consumer review](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/3889#issuecomment-4427875644), the following are **not** in Stage 1 — production-grade integrations need Stage 1.5+ before relying on them:
...
3. ~~**Client-initiated heartbeat path**~~ — shipped via [#4175](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4175) PR 9. `POST /session/:id/heartbeat` records last-seen timestamps on the daemon (capability tag `client_heartbeat`); SDK helpers are `DaemonClient.heartbeat()` / `DaemonSessionClient.heartbeat()`.
...
The full convergence roadmap is tracked on [#3803](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/3803).
...
### Session state is local-mutation-only (per [LaZzyMan review #4270256721](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/3889#pullrequestreview-4270256721))
...
#### Mode 1 — headless `hopcode serve` (this PR)
...
**Peer agents (Cursor / Continue / Claude Code / OpenCode / Gemini CLI) all do single-process multi-session.** hopcode matches them at the agent layer; the Stage 1 bridge in this PR makes the same architecture visible over HTTP.
...
> [!note]
>
> hopcode OAuth free tier was discontinued on 2026-04-15. The `hopcode-oauth`
...
`hopcode serve` appends diagnostic records across normal restarts at the stable
active path:

```
${hopcode_runtime_dir or ~/.qwen}/debug/daemon/daemon.log
```
...
```bash
tail -F ~/.hopcode/debug/daemon/daemon.log
```
...
`hopcode_runtime_dir` values when daemons need independent history. New daemon log
directories are private to the user (`0700`) and new files use `0600` on POSIX.
There is no age-based expiry.

### Disabling

Set `HOPCODE_DAEMON_LOG_FILE=0` (or `false`/`off`/`no`) to skip file logging entirely. Stderr output is unaffected.
...
## Runtime MCP server management (issue [#4514](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4514))
...
- **Setting up a long-running daemon?** [Local launch templates (systemd / launchd / nohup / tmux)](./hopcode-serve-deploy-local.md) for v0.16-alpha (local-only).
...
- **Tracking the roadmap?** Stage 1.5 / Stage 2 progress is tracked on issue [#3803](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/3803).
