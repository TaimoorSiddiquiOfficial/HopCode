...
| `HOPCODE_HOME` redirect                     | `getUserSettingsPath()` already resolves the path; watcher uses the resolved path                             |
...
Edit `~/.hopcode/settings.json` during a running session and observe debug log output for change events.
...
user restarts `hopcode`. Sensitive values (credentials) additionally should
