...
2. The shared `hopcode --acp` child process crashes (`channel.exited` handler)
...
| ----------------------------------------- | ------------------------- | -------------------------------------------------------------------------------- |
| `channelIdleTimeoutMs` + `startIdleTimer` | Channel (child process)   | Kills the `hopcode --acp` child when ALL sessions are gone                          |
...
**CLI surface** (`hopcode serve` flags):
  ...
  sessionReaper = setInterval(() => {
    ...
    for (const [id, entry] of byId) {
      ...
      bridgeImpl
        ...
        .catch((err) => {
          writeStderrLine(
            `hopcode serve: session reaper failed to close ${JSON.stringify(id)}: ${String(err)}`,
          );
        });
    }
...
- **stderr log**: `hopcode serve: reaping idle session "<id>" (idle for Nms)` on
  each reap, matching existing `hopcode serve:` prefix convention.
- **Telemetry event**: `session.close` with operation
  `hopcode.daemon.bridge.operation: 'session.close'` (reuses existing
