...
> **Status**: Proposed — implementation in [PR #5884](https://github.com/QwenLM/hopcode/pull/5884) (branch `codex/sessionless-daemon-remember`), not yet merged.
...
```json
{
  ...
  "result": {
    ...
    "filesTouched": ["~/.hopcode/memories/user/user.md"],
    "touchedScopes": ["user"]
  }
}
```
...
- **Write tools** (`write_file`, `edit`): only allowed within the project
  auto-memory root or user memory root (`~/.hopcode/memories`).
...
### TypeScript SDK (`@hoptrendy/sdk-typescript`)
