...
- `src/commands.ts`：Claude Code 内置 `/doctor`、`/release-notes` 等命令，HopCode 当前已实现 `/doctor`；本阶段不实现 `/release-notes`。
...
tmux new-session -d -s hopcode-slash-phase3 -x 200 -y 50 \
  "cd /tmp/hopcode-slash-phase3 && /Users/mochi/code/hopcode-test/dist/cli.js --approval-mode izn"
sleep 3
```
