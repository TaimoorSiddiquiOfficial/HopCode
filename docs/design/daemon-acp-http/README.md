...
The daemon (`hopcode serve`) today speaks a **bespoke REST + SSE** dialect to web/SDK
clients, while speaking **real ACP JSON-RPC over stdio** to the spawned `hopcode --acp`
...
**Decision: extension namespace = `_hopcode/…`** (single-underscore prefix, the
...
- `acp-bridge/src/spawnChannel.ts` spawns `hopcode --acp`, wraps stdin/stdout with
...
| `jsonRpc.ts` | JSON-RPC 2.0 parse/validate/serialize helpers; error codes (`-32600` etc.); `_hopcode/` namespace guard. |
...
**Northbound choice:** vendor-namespaced **`_hopcode/<area>/<verb>`** method names
...
- The REST surface carries features with no clean ACP mapping yet (workspace
  ...
  registry). Those degrade to `_hopcode/*` extensions on `/acp` but the REST surface stays
...
- Toggle (v1, shipped): on by default; **`HOPCODE_SERVE_ACP_HTTP=0`** disables the mount. A
...
2. Start daemon: `hopcode serve --listen 127.0.0.1:0 --token <t>` (or env token).
3. Run `node scripts/acp-http-smoke.mjs`:
   ...
   - `POST {_hopcode/session/set_model}` → assert model switch + `session/update`.
...
agent round-trip; `_hopcode/session/set_model`; method-not-found; `DELETE` teardown.

### Live daemon (real model)

Booted `hopcode serve --port 8767 --token … --workspace …` (bundle entry so the
spawned `hopcode --acp` child is self-contained) and ran `scripts/acp-http-smoke.mjs`:
...
**决策（采纳评审/owner 意见）**：不让 `/acp` transport 再去直连这些 route 级服务（那会复制 REST 的架构漂移、并使 transport 耦合翻倍）。**正确做法是先在 `@hopcode/acp-bridge` 的 `HttpAcpBridge` 上补齐这些能力**（如 `readWorkspaceFile`/`writeWorkspaceFile`/`globWorkspace`、`startDeviceFlow`/`pollDeviceFlow`、`listAgents`/`upsertAgent`/`deleteAgent`、`readMemory`/`writeMemory`），让 REST 与 `/acp` 都经由 bridge。届时 `/acp` 再加 `_hopcode/fs/*`、`_hopcode/auth/*`、`_hopcode/workspace/agent*`、`_hopcode/workspace/memory*`（文件读因无标准 ACP client→agent 方法，属合法厂商扩展）。
...
2. **Follow-up PR 2 — `/acp` 剩余对齐（依赖 PR 1）**：`_hopcode/fs/*`、`_hopcode/auth/*`、`_hopcode/workspace/agent*`、`_hopcode/workspace/memory*` → 完全等价 REST。
...
- **Namespace `_qwen.ai/` → `_hopcode/`**: ACP's only hard rule is the leading `_`; the `_zed.dev/` domain segment is convention-by-example, not a MUST. Since `hopcode` is distinctive, we use the shorter bare form. `_meta` key likewise `"hopcode"`. (Survey of real agents: Zed/gemini-cli mostly use `_meta`-on-standard-methods + ACP's own `unstable_*`; bare custom `_` methods are rare — our `_hopcode/*` are genuinely-new workspace/session ops with no standard equivalent, so a `_` method is the right tool.)
- **Why hand-rolled transport (not SDK-based)**: the TS SDK ships only `ndJsonStream` (stdio); RFD #721 HTTP is SDK Phase-3 (not implemented). The SDK `Connection` is single-duplex-stream; our transport is multi-stream (POSTs + connection-SSE + per-session-SSE) and needs outbound demux by sessionId — which our dispatcher already knows at routing time. A full SDK rewrite fights that model and wouldn't remove the bulk (bridge translation, SSE lifecycle, ownership, EventBus→JSON-RPC). **Pragmatic improvement (candidate follow-up): adopt the SDK's Zod schema validators + types for param validation while keeping the hand-rolled transport.** SDK clients using `extMethod('_hopcode/…')` interoperate with our handlers (identical wire shape).
