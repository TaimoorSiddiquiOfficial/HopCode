...
> | `/acp` northbound `hopcode/workspace/*` dispatch（§6）                          | ⏳ **deferred**                                                                                                                |
...
`hopcode/workspace/...`（与现有 `qwen/control/...` 区分）：

- `qwen/control/...` = daemon→child 转发命令（southbound，经 AcpSessionBridge）
- `hopcode/workspace/...` = daemon 本地工作区操作（northbound，终止于 DaemonWorkspaceService）
...
| --------------------------------- | ----------------------------------------------- | --------------------------------------------------- |
| `hopcode/workspace/fs/read`          | `GET /file?path=...`                            | `workspace.file.read(ctx, path)`                    |
| `hopcode/workspace/fs/readBytes`     | `GET /file/bytes?path=...`                      | `workspace.file.readBytes(ctx, path)`               |
| `hopcode/workspace/fs/write`         | `POST /file/write`                              | `workspace.file.write(ctx, path, content)`          |
| `hopcode/workspace/fs/edit`          | `POST /file/edit`                               | `workspace.file.edit(ctx, path, edits)`             |
| `hopcode/workspace/fs/glob`          | `GET /glob?pattern=...`                         | `workspace.file.glob(ctx, pattern)`                 |
| `hopcode/workspace/fs/list`          | `GET /list?path=...`                            | `workspace.file.list(ctx, path)`                    |
| `hopcode/workspace/fs/stat`          | `GET /stat?path=...`                            | `workspace.file.stat(ctx, path)`                    |
| `hopcode/workspace/auth/start`       | `POST /workspace/auth/device-flow`              | `workspace.auth.startFlow(ctx)`                     |
| `hopcode/workspace/auth/status`      | `GET /workspace/auth/status`                    | `workspace.auth.getAuthStatus(ctx)`                 |
| `hopcode/workspace/auth/flow`        | `GET /workspace/auth/device-flow/:id`           | `workspace.auth.getFlowStatus(ctx, flowId)`         |
| `hopcode/workspace/auth/cancel`      | `POST /workspace/auth/device-flow/:id` (cancel) | `workspace.auth.cancelFlow(ctx, flowId)`            |
| `hopcode/workspace/agents/list`      | `GET /workspace/agents`                         | `workspace.agents.list(ctx)`                        |
| `hopcode/workspace/agents/get`       | `GET /workspace/agents/:agentType`              | `workspace.agents.get(ctx, agentType)`              |
| `hopcode/workspace/agents/create`    | `POST /workspace/agents`                        | `workspace.agents.create(ctx, spec)`                |
| `hopcode/workspace/agents/update`    | `POST /workspace/agents/:agentType`             | `workspace.agents.update(ctx, agentType, spec)`     |
| `hopcode/workspace/agents/delete`    | `DELETE /workspace/agents/:agentType`           | `workspace.agents.delete(ctx, agentType)`           |
| `hopcode/workspace/memory/list`      | `GET /workspace/memory`                         | `workspace.memory.list(ctx)`                        |
| `hopcode/workspace/memory/read`      | `GET /workspace/memory/:key`                    | `workspace.memory.read(ctx, key)`                   |
| `hopcode/workspace/memory/write`     | `POST /workspace/memory`                        | `workspace.memory.write(ctx, key, content)`         |
| `hopcode/workspace/memory/delete`    | `DELETE /workspace/memory/:key`                 | `workspace.memory.delete(ctx, key)`                 |
| `hopcode/workspace/init`             | `POST /workspace/init`                          | `workspace.initWorkspace(ctx, opts)`                |
| `hopcode/workspace/tool/toggle`      | `POST /workspace/tool/toggle`                   | `workspace.setToolEnabled(ctx, toolName, enabled)`  |
| `hopcode/workspace/status/mcp`       | `GET /workspace/mcp`                            | `workspace.getMcpStatus()`                          |
| `hopcode/workspace/status/skills`    | `GET /workspace/skills`                         | `workspace.getSkillsStatus()`                       |
| `hopcode/workspace/status/providers` | `GET /workspace/providers`                      | `workspace.getProvidersStatus()`                    |
| `hopcode/workspace/status/env`       | `GET /workspace/env`                            | `workspace.getEnvStatus()`                          |
| `hopcode/workspace/status/preflight` | `GET /workspace/preflight`                      | `workspace.getPreflightStatus()`                    |
| `hopcode/workspace/mcp/restart`      | `POST /workspace/mcp/restart`                   | `workspace.restartMcpServer(ctx, serverName, opts)` |

Capabilities advertise 时在 `_meta.hopcode.methods` 中声明这些方法。
...
| `cli/src/serve/runHopCodeServe.ts`                               | 构造 `DaemonWorkspaceService`，注入 callback，传给 routes 和 /acp handler                                                                                                           |
...
- File read/write：REST `GET /file` vs `/acp` `hopcode/workspace/fs/read` → 同结果
- Agent CRUD：REST `POST /workspace/agents` vs `/acp` `hopcode/workspace/agents/create` → 同行为
...
1. `/acp` northbound 命名空间：`hopcode/workspace/...` vs 其他（如复用 `qwen/control/...`）
