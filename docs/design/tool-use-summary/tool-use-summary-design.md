...
After each tool batch completes, HopCode fires a short fast-model call that returns a git-commit-subject-style label summarizing the batch. The label shows as an inline dim `● <label>` line in full mode and replaces the generic `Tool × N` header in compact mode. Generation runs fire-and-forget in parallel with the next turn's API stream, so its ~1s latency is hidden behind main-model streaming.

| Dimension             | Claude Code                                                           | HopCode                                                                                    |
| --------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Trigger point         | `query.ts` — after a tool batch finalizes                             | `useHopCodeStream.ts` → `handleCompletedTools` — same lifecycle point                      |
| Generation model      | Haiku via `queryHaiku`                                                | Configured `fastModel` via the HopCode client                                              |
| Subagent behavior     | `!toolUseContext.agentId` — main session only                         | Implicit — subagents run through `agents/runtime/`, not `useHopCodeStream`                 |
...
## 3. HopCode Implementation

### 3.1 Flow

HopCode hooks the same lifecycle point (`useHopCodeStream.handleCompletedTools`) but renders on both sides of `ui.compactMode` so the feature is useful to CLI users without any SDK plumbing.
...
| Trigger             | `packages/cli/src/ui/hooks/useHopCodeStream.ts:handleCompletedTools`  | Fires fast-model call, addItem on resolve                                 |
...
1. `HOPCODE_EMIT_TOOL_USE_SUMMARIES=0|1|true|false` — env override, highest priority.
...
| Deviation                                                                | Why                                                                                                                                                                                     |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Settings layer in addition to env gate                                   | HopCode renders the label in the CLI; users need a persistent switch, not a per-shell env export.                                                                                       |
...
| Dedicated `cleanSummary` post-processing                                 | HopCode supports more heterogeneous providers than CC; some models prepend `Label:` or wrap in quotes. Normalizing at the boundary keeps the UI consistent.                             |
...
| Dual render paths (full-mode inline + compact-mode header)               | HopCode's default is `ui.compactMode: false`; without the inline full-mode render, the feature would be invisible to most users.                                                        |
