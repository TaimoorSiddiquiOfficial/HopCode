...
hopcode 目前仅有面向 Arena 多模型对比场景的内部 worktree 实现（`GitWorktreeService`），用户无法在普通会话中使用 worktree 隔离工作，AgentTool 也不支持为 subagent 创建隔离的 worktree 环境。
...
```
{git 仓库根}/.hopcode/worktrees/{slug}
```
...
Arena 的 worktree 路径由 `agents.arena.worktreeBaseDir` 控制，默认 `~/.hopcode/arena`（`ArenaManager.ts:125`），与通用路径完全独立，不做任何改动。
...
3. 调用 `GitWorktreeService` 创建 worktree，路径为 `.hopcode/worktrees/{slug}`
...
| CLI 启动标志   | `hopcode --worktree my-feature`                          | Phase D  |
...
- Agent 启动时创建临时 worktree（slug：`agent-{7hex}`，路径：`.hopcode/worktrees/agent-{7hex}`）
- Agent 结束后：无变更则自动删除；有变更则保留，将路径和分支返回在结果中
- 过期 worktree 自动清理：扫描 `.hopcode/worktrees/`，匹配 `agent-{7hex}` 模式，超过 30 天且无未推送提交则删除，fail-closed 策略
...
- review skill（`SKILL.md`）：review 使用独立机制（路径 `.hopcode/tmp/review-pr-<n>`，通过 `hopcode review fetch-pr` 命令创建），与通用 worktree 路径和机制完全不同，不存在混淆
...
- 分支：`worktree-pr-<N>`（沿用 hopcode 现有 `worktree-<slug>` 命名规则；不采用 claude-code 的 `pr-<N>` 直接命名，避免与本地 `pr-<N>` 分支冲突）
...
1. **`--worktree-keep-on-exit`？** claude-code 没有，hopcode 是否需要一个 CLI flag 让 Exit Dialog 默认选 keep？建议**先不加**，等用户反馈。
