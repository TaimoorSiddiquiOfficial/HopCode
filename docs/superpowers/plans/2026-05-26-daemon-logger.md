...
Run: `npm install && cd packages/cli && npx vitest run src/serve/runHopCodeServe.test.ts && cd ../acp-bridge && npx vitest run`
...
Returns a no-op logger when `HOPCODE_DAEMON_LOG_FILE` is disabled. No filesystem touch yet.
...
describe('initDaemonLogger opt-out', () => {
  ...
  for (const val of ['0', 'false', 'off', 'no', 'False', ' OFF ']) {
    it(`returns no-op logger when HOPCODE_DAEMON_LOG_FILE=${JSON.stringify(val)}`, () => {
...
function isOptedOut(): boolean {
  const raw = process.env['HOPCODE_DAEMON_LOG_FILE'];
  if (!raw) return false;
...
export function initDaemonLogger(opts: InitDaemonLoggerOptions): DaemonLogger {
  ...
  try {
    ...
    stderr(
      `hopcode serve: daemon log disabled — init failed: ${
  ...
  pending = pending.then(() =>
    nodeFs.promises.appendFile(logPath, line).catch((err) => {
      if (!degraded) {
        degraded = true;
        stderr(
          `hopcode serve: daemon log write failed — entering degraded mode: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
      }
 ...
 * runHopCodeServe to capture them in the daemon log file. The bridge
...
```ts
   ...
   * No-op when omitted. Set by cli `runHopCodeServe` from the daemon logger.
   */
...
describe('onDiagnosticLine', () => {
  ...
  afterEach(() => {
    if (originalDebug === undefined) delete process.env['HOPCODE_SERVE_DEBUG'];
    else process.env['HOPCODE_SERVE_DEBUG'] = originalDebug;
  });
  ...
  it('receives writeServeDebugLine output when HOPCODE_SERVE_DEBUG=1', async () => {
    ...
    // ... trigger code here ...
    expect(captured.some((e) => e.line.includes('hopcode serve debug: '))).toBe(
      true,
    );
    expect(
...
describe('createSpawnChannelFactory onDiagnosticLine', () => {
  it('returns a ChannelFactory that tees child stderr lines', async () => {
    ...
    // HOPCODE_CLI_ENTRY escape hatch to point at a Node one-liner.
    const here = path.dirname(fileURLToPath(import.meta.url));
    process.env['HOPCODE_CLI_ENTRY'] = path.join(
      here,
      'testutil',
      'stderrOnlyEntry.cjs',
    );
...
if (daemonLog) {
  ...
  writeStderrLine(
    `hopcode serve: ${ctx?.route ?? 'unknown route'}: ${
...
it('runHopCodeServe initializes daemon logger and writes boot banner + flushes on shutdown', async () => {
  const tmpRuntime = mkdtempSync(path.join(os.tmpdir(), 'serve-runtime-'));
  const originalRuntime = process.env['HOPCODE_RUNTIME_DIR'];
  process.env['HOPCODE_RUNTIME_DIR'] = tmpRuntime;
  try {
...
Run: `cd packages/cli && npx vitest run src/serve/runHopCodeServe.test.ts -t "daemon logger"`
Expected: fail.

- [ ] **Step 4: Wire in `runHopCodeServe`**

Edit `runHopCodeServe.ts`:
...
writeStderrLine(
  `hopcode serve: daemon log → ${daemonLog.getLogPath() || '(disabled)'}`,
);
```
...
4. Update the `createServeApp(...)` call (currently at `runHopCodeServe.ts:706`, signature is `createServeApp(opts, getPort, deps)`) to add `daemonLog` to the deps object:
...
5. Replace **lifecycle-only** `writeStderrLine(...)` calls (the ones inside `onSignal`, the `bridge.shutdown` error path, the server `error` listener, the device-flow dispose error, the "received signal, draining" line) with `daemonLog.warn(...)` / `daemonLog.error(..., err)` — daemonLog tees to stderr so operator-visible output is preserved. Do NOT touch:
   ...
   - The lone "hopcode serve: daemon log → ..." banner added in step 2 (avoid logging a line about itself).
...
Run: `cd packages/cli && npx vitest run src/serve/runHopCodeServe.test.ts`
...
```bash
git add packages/cli/src/serve/runHopCodeServe.ts packages/cli/src/serve/runHopCodeServe.test.ts
git commit -m "feat(serve): init daemonLogger in runHopCodeServe + flush on shutdown (#4548)"
```
...
Pick the most natural home — likely `docs/cli/serve.md`. If none exists for `hopcode serve`, create `docs/cli/serve-daemon-log.md`.
...
process's log, so `tail -f ~/.hopcode/debug/daemon/latest` will follow whichever
daemon is running.

The log captures lifecycle messages, route errors (with `route=` and
`sessionId=` context), ACP child stderr, and — when `HOPCODE_SERVE_DEBUG=1`
...
Set `HOPCODE_DAEMON_LOG_FILE=0` (or `false`/`off`/`no`) to skip file logging
entirely. Stderr output is unaffected.

### Relation to session debug logs

Session-scoped debug logs (`~/.hopcode/debug/<sessionId>.txt` and the
`~/.hopcode/debug/latest` symlink) are independent. The daemon log lives
...
```bash
cd /Users/jinye.djy/Projects/hopcode/.claude/worktrees/feat-support-daemon-logger
...
```bash
HOPCODE_RUNTIME_DIR=$(mktemp -d) node packages/cli/dist/index.js serve --port 0 --hostname 127.0.0.1 &
SERVE_PID=$!
sleep 1
ls $HOPCODE_RUNTIME_DIR/debug/daemon/
cat $HOPCODE_RUNTIME_DIR/debug/daemon/latest
kill -TERM $SERVE_PID
wait $SERVE_PID 2>/dev/null || true
cat $HOPCODE_RUNTIME_DIR/debug/daemon/latest  # should now contain shutdown line
```
...
## Summary
- Adds a per-process daemon file logger at `~/.hopcode/debug/daemon/serve-<pid>-<workspaceHash>.log` (configurable via `HOPCODE_RUNTIME_DIR`, opt-out via `HOPCODE_DAEMON_LOG_FILE=0`).
- Routes `runHopCodeServe` lifecycle messages, `sendBridgeError` route errors, `writeServeDebugLine` debug breadcrumbs, and ACP child stderr into the daemon log without removing existing stderr output.
