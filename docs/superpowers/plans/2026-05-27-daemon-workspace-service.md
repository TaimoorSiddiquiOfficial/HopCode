...
- Modify: `packages/cli/src/serve/runHopCodeServe.ts` (import paths)
...
## Task 9: Wire Service into runHopCodeServe + REST Routes
...
- [ ] **Step 1: Construct service in runHopCodeServe.ts**
...
In the /acp handler that routes JSON-RPC methods, add a switch/map for `hopcode/workspace/*`:

```ts
// Pattern (exact location depends on codebase structure):
case 'hopcode/workspace/fs/read': {
  const ctx = buildAcpRequestContext(connection, 'hopcode/workspace/fs/read');
  const { path } = params;
  return workspace.file.read(ctx, path);
}
case 'hopcode/workspace/fs/write': {
  const ctx = buildAcpRequestContext(connection, 'hopcode/workspace/fs/write');
...
Ensure `_meta.hopcode.methods` includes all `hopcode/workspace/*` methods in the `initialize` response.
...
```bash
git add -A
git commit -m "feat(serve): add /acp northbound workspace methods (27 hopcode/workspace/* endpoints)"
```
...
describe('REST ↔ /acp equivalence', () => {
  ...
  describe('file read', () => {
    it('returns same content via both transports', async () => {
      ...
      const acpRes = await acpCall(app, 'hopcode/workspace/fs/read', { path: 'README.md' });
  ...
  describe('trust gate rejection', () => {
    ...
    it('rejects invalid clientId via /acp (JSON-RPC error)', async () => {
      const res = await acpCall(app, 'hopcode/workspace/fs/write', { path: 'x.ts', content: 'y' });
