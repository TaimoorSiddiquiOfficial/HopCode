...
- A new logger initialized once per `runHopCodeServe` process.
- File at `${HOPCODE_RUNTIME_DIR or ~/.hopcode}/debug/daemon/<daemon-id>.log`, append mode.
- Tee of:
  - `runHopCodeServe.ts` lifecycle / shutdown / signal messages
  - `sendBridgeError` (`server.ts`) route errors
  - `bridge.ts` `writeServeDebugLine` (when `HOPCODE_SERVE_DEBUG` is set)
  - `spawnChannel.ts` ACP child stderr forwarding
- Opt-out via `HOPCODE_DAEMON_LOG_FILE=0|false|off|no`.
...
| `packages/cli/src/serve/runHopCodeServe.ts`                | changed       | Init logger at boot; replace lifecycle `writeStderrLine` with `daemonLog.*`; `await flush()` on shutdown; pass `onDiagnosticLine` into bridge |
...
Logger is created in `runHopCodeServe`, passed by closure to internal serve modules that need it (or by callback to `acp-bridge`). Rationale:
...
- Path: `Storage.getGlobalDebugDir() + '/daemon/<daemon-id>.log'`
  - Resolves to `${HOPCODE_RUNTIME_DIR or ~/.hopcode}/debug/daemon/<daemon-id>.log`.
...
- `latest` symlink: `~/.hopcode/debug/daemon/latest` → current process's log file. Updated on init using the existing `updateSymlink` helper (`packages/core/src/utils/symlink.ts`). Symlink failure is logged and ignored — does not degrade primary writes. Distinct from `${runtimeBaseDir}/debug/latest` (session-scoped) per non-goal.
...
export interface DaemonLogger {
  ...
  getDaemonId(): string;
  /** Drain pending appends. Called from runHopCodeServe shutdown handler. */
  flush(): Promise<void>;
}
...
If `process.env['HOPCODE_DAEMON_LOG_FILE']` is one of `0|false|off|no`, `initDaemonLogger` short-circuits to a no-op logger before any filesystem call.
...
- Level: `INFO` | `WARN` | `ERROR`. (No DEBUG initially — `HOPCODE_SERVE_DEBUG` flows in as `INFO` via `raw()`.)
- Tag: literal `DAEMON`.
...
```
runHopCodeServe(opts):
  ...
  daemonLog = initDaemonLogger({ boundWorkspace })
  writeStderrLine(`hopcode serve: daemon log → ${daemonLog.getLogPath()}`)
...
| Source                                                        | Today                                        | After                                                                                            |
| ------------------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `runHopCodeServe.ts` lifecycle / signals / config warnings       | `writeStderrLine(...)`                       | `daemonLog.info \| warn(...)` (stderr still happens — `daemonLog` tees)                          |
| `runHopCodeServe.ts` "listening on URL" (stdout)                 | `writeStdoutLine(...)`                       | unchanged — operator scripts parse stdout                                                        |
...
| `bridge.ts:writeServeDebugLine` (`HOPCODE_SERVE_DEBUG`)          | `writeStderrLine('hopcode serve debug: ...')`   | tee to `onDiagnosticLine(line, 'info')`                                                          |
...
| CLI usage / argparse errors (`runHopCodeServe` early validation) | `writeStderrLine(...)`                       | unchanged (logger may not exist yet)                                                             |
...
| Env var                                         | Behavior                                                                     |
| ----------------------------------------------- | ---------------------------------------------------------------------------- |
| `HOPCODE_DAEMON_LOG_FILE=0\|false\|off\|no`        | `initDaemonLogger` returns no-op; tee is a no-op; stderr unchanged           |
| `HOPCODE_DAEMON_LOG_FILE=<anything else>` or unset | Enabled (default)                                                            |
| `HOPCODE_RUNTIME_DIR=<path>`                       | Relocates `~/.hopcode` root, daemon log moves with it (existing semantics)      |
| `HOPCODE_SERVE_DEBUG=1`                            | Existing — `writeServeDebugLine` activates; lines now also tee to daemon log |

`HOPCODE_DAEMON_LOG_FILE` is intentionally separate from `HOPCODE_DEBUG_LOG_FILE` so disabling per-session debug logs doesn't take down the operator's daemon log (and vice versa).
...
- `HOPCODE_DAEMON_LOG_FILE=0` → no file created.
...
### `runHopCodeServe.test.ts` (extend)
...
- `onDiagnosticLine` callback invoked from `writeServeDebugLine` when `HOPCODE_SERVE_DEBUG=1` and from `spawnChannel` child stderr forwarder. Tests inject a capturing fake; no filesystem.

## 13. Documentation

- `docs/cli/serve.md` (or wherever serve is documented) gains a "Daemon log file" section covering: path, daemon-id format, `latest` symlink, `HOPCODE_DAEMON_LOG_FILE` opt-out, distinction from per-session `debug/<sessionId>.txt`.
...
- Pure-additive change. Rollback = revert the commit:
  ...
  - Revert `runHopCodeServe.ts` lifecycle / sendBridgeError / bridge / spawnChannel changes.
...
| Criterion                                                           | How met                                                                                           |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `hopcode serve` creates / appends daemon log without shell redirection | `initDaemonLogger` opens the file at boot                                                         |
...
- Should `latest` symlink go in `~/.hopcode/debug/daemon/latest` or `~/.hopcode/debug/daemon-latest`? Spec picks the former for directory tidiness.
- Should we offer JSON-line output as a future flag (e.g., `HOPCODE_DAEMON_LOG_FORMAT=json`)? Out of scope for this PR; structured export is what #2014 owns.
