...
This document describes the `todo_write` tool for HopCode.
...
## How to use `todo_write` with HopCode
...
The tool stores todo lists in your home directory (`~/.hopcode/todos/`) with session-specific files, so each coding session maintains its own task list.
