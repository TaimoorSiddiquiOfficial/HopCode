...
This document describes the `task` tool for HopCode.
...
You can view available subagents by using the `/agents` command in HopCode.
...
Subagents are configured through HopCode's agent configuration system. Use the `/agents` command to:
