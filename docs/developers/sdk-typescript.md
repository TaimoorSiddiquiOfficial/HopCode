...
- Node.js >= 20.0.0
- [HopCode](https://github.com/TaimoorSiddiquiOfficial/HopCode) >= 0.4.0 (stable) installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the HopCode executable. You should explicitly set the `pathToQwenExecutable` option to the full path of the `hopcode` binary.
...
Creates a new query session with the HopCode.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'qwen-max'`, `'qwen-plus'`, `'qwen-turbo'`). Takes precedence over `OPENAI_MODEL` and `HOPCODE_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                                  |
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the HopCode executable. Supports multiple formats: `'hopcode'` (native binary from PATH), `'/path/to/hopcode'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `HOPCODE_CLI_PATH` env var, `~/.volta/bin/hopcode`, `~/.npm-global/bin/hopcode`, `/usr/local/bin/hopcode`, `~/.local/bin/hopcode`, `~/node_modules/.bin/hopcode`, `~/.yarn/bin/hopcode`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the HopCode process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                                        |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in HopCode system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                                        |
...
| `authType`               | `'openai' \| 'qwen-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'qwen-oauth'` in SDK is not recommended as credentials are stored in `~/.hopcode` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                                           |
...
```typescript
const query = hopcode.query('Your prompt', {
  timeout: {
