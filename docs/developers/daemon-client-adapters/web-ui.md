...
Web chat and web terminal clients should consume `hopcode serve` through the
...
A separate local dev server is cross-origin from `hopcode serve`; it must either
...
- The native `hopcode` TUI remains direct and unchanged.
