...
`packages/cli/src/serve/` is the boot layer for `hopcode serve`. It translates CLI flags into `ServeOptions`, validates startup configuration, builds the Express app, wires middleware, registers routes, exposes daemon-host preflight/status providers, maintains the permission audit ring, and owns the two-phase graceful shutdown sequence. HTTP-facing work lives in this layer; ACP-facing work lives one layer below in `@hoptrendy/acp-bridge` (see [`03-acp-bridge.md`](./03-acp-bridge.md)).
...
| `serve/workspaceMemory.ts`                                   | `GET/POST /workspace/memory` (HOPCODE.md CRUD).                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `serve/httpAcpBridge.ts` -> `@hoptrendy/acp-bridge`
...
- Issues: [#3803](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/3803), [#4175](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4175)
