...
The first production consumer is **`packages/webui/src/daemon/`** ([#4328](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4328)). Its React `DaemonSessionProvider` and transcript adapter let the web UI connect directly to daemon HTTP+SSE instead of only rendering host `postMessage` traffic. CLI TUI, channel base, and VS Code IDE can reuse the same layer later; [`../daemon-ui/MIGRATION.md`](../daemon-ui/MIGRATION.md) documents the v2 incremental migration guide.
...
This landed in [#4328](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4328).
...
- **Bundle size**: the `ui/*` subpackage is exported as an ESM subpath through `@hoptrendy/sdk/daemon` and does not pull in React or DOM dependencies. React integration is only loaded when a web UI consumer uses `DaemonSessionProvider`.
...
- Context PRs: [#4328](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4328) (v1 transcript layer and web UI provider), [#4353](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4353) (v2 unified completeness follow-up)
