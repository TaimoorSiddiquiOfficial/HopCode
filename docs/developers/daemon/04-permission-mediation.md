...
| ------------------- | ---------------------------------------------------- | ---------------- | --------------------------------------------------------------------------------------------- |
| JSON-RPC message id | `_HOPCODE_perm_N` (string, monotonic per connection)    | Connection-local | Correlates the JSON-RPC request→response pair on the session stream.                          |
...
```json
{
  ...
  "params": {
    ...
    "_meta": { "hopcode": { "requestId": "<bridge-request-id>" } }
  }
}
```
...
- Issue: [#4175](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4175) F3 series.
