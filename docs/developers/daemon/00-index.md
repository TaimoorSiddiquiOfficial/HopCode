...
- **F1 / F2 / F3 / F4** - internal milestones tracked in [#4175](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues/4175). F1: bridge extraction and `BridgeFileSystem`. F2: workspace-scoped MCP transport pool. F3: multi-client permission mediation. F4: protocol completion and daemon client surfaces.
...
This doc set reflects the daemon mode surface currently merged into `main`, including the follow-up work from [#4412](https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4412). It intentionally describes current behavior instead of earlier F-series planning snapshots.
