...
There are several ways to run HopCode. The option you choose depends on how you intend to use it.

---

### 1. Standard installation (Recommended for typical users)

This is the recommended way for end-users to install HopCode. It involves downloading the HopCode package from the NPM registry.
...
For security and isolation, HopCode can be run inside a container. This is the default way that the CLI executes tools that might have side effects.

- **Directly from the Registry:**
  You can run the published sandbox image directly. This is useful for environments where you only have Docker and want to run the CLI.
  ```bash
  # Run the published sandbox image
  docker run --rm -it ghcr.io/hoptrendy/hopcode:0.18.10
  ```
...
### 3. Running from source (Recommended for HopCode contributors)
...
### 4. Running the latest HopCode commit from GitHub

You can run the most recently committed version of HopCode directly from the GitHub repository. This is useful for testing features still in development.
...
The Docker-based execution method is supported by the `hopcode-sandbox` container image. This image is published to a container registry and contains a pre-installed, global version of HopCode.
