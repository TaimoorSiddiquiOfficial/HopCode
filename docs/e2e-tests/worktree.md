...
```bash
<hopcode> "say hello" --approval-mode izn --output-format json 2>/dev/null \
...
```bash
<hopcode> "create a new git worktree using the enter_worktree tool" \
  --approval-mode izn --output-format json 2>/dev/null > /tmp/a2.json
...
```bash
<hopcode> "use the enter_worktree tool with name='my-feature' to create a worktree" \
...
```bash
<hopcode> "use enter_worktree with name='../../../etc' to create a worktree" \
  --approval-mode izn --output-format json 2>/dev/null \
...
```bash
<hopcode> "create a worktree named 'temp-keep' using enter_worktree, then immediately exit it with action='keep' using exit_worktree" \
  --approval-mode izn --output-format json 2>/dev/null > /tmp/b1.json
...
```bash
<hopcode> "create a worktree named 'temp-remove' using enter_worktree, then immediately exit it with action='remove' using exit_worktree" \
  --approval-mode izn --output-format json 2>/dev/null
...
```bash
tmux new-session -d -s wt-test-b3 -x 200 -y 50 "cd $TEST_DIR && <hopcode> --approval-mode izn"
sleep 3
...
```bash
SESSION_ID=$(<hopcode> "create a worktree named 'persist-test' using enter_worktree" \
  --approval-mode izn --output-format json 2>/dev/null \
...
```bash
<hopcode> "spawn an agent using the agent tool with isolation='worktree' to run 'echo hello'" \
  --approval-mode izn --output-format json 2>/dev/null \
...
<hopcode> "spawn an agent with isolation='worktree' to list files in the current directory using ls" \
  --approval-mode izn --output-format json 2>/dev/null
...
```bash
<hopcode> "spawn an agent with isolation='worktree' to write 'test content' to a new file called test.txt" \
  --approval-mode izn --output-format json 2>/dev/null > /tmp/d3.json
