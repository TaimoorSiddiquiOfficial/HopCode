# HopCode Runtime Memory Benchmark Report

Date: 2026-05-18

## Summary

This report records local memory benchmarks for HopCode runtime behavior. It
compares HopCode across models and compares HopCode with Claude Code on the
...
- HopCode process-tree RSS peak: about `852-1062 MiB` (`0.83-1.04 GiB`).
- Claude Code process-tree RSS peak: about `279-366 MiB` (`0.27-0.36 GiB`).
- HopCode was about `2.3x-3.6x` higher in the tested
...
| HopCode binary                                | PATH-resolved `qwen` binary                |
...
Process-tree RSS is used as the headline metric because HopCode launches a
...
1. HopCode PR review memory run.
2. HopCode model comparison run.
3. Strict HopCode vs Claude Code comparison with `pai/glm-5`.
4. HopCode vs Claude Code, two CLIs by two models.
5. HopCode vs Claude Code, five-case matrix.
...
The current data is strong enough to say that HopCode has a higher runtime
...
The leading explanation is a HopCode runtime/path difference rather than a
...
- the gap reproduces in small PR and code-navigation tasks, not only in large
  diff tasks;
- HopCode repeatedly sends or accounts for more tokens than Claude Code for
  similar work;
- HopCode's largest observed component is the child Node/Qwen worker process,
...
1. **Task-time runtime cost first**: HopCode likely initializes or retains
...
2. **Context/tool-result volume second**: HopCode appears to carry larger
...
- 2 CLIs: HopCode and Claude Code.
...
The same matrix also showed HopCode using more model-side tokens in every
tested case.
...
- HopCode used more total tokens than Claude Code in every matrix cell.
...
| -------------- | ----------- | --------------------: |
| `pai/glm-5`    | HopCode     |            1000.7 MiB |
| `pai/glm-5`    | Claude Code |             349.0 MiB |
| `qwen3.6-plus` | HopCode     |            1095.8 MiB |
...
4. Context/tool-result handling is still a likely contributor. HopCode used
...
- Claude Code and HopCode use different runtime implementations and protocol
...
current RSS comparison can be connected to internal HopCode state.
