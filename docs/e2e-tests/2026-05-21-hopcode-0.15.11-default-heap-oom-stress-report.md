# HopCode 0.15.11 默认 Heap OOM 压测报告

日期：2026-05-21

## 测试范围

本报告记录了针对 HopCode `0.15.11` 最新本地构建的一轮默认 heap 压测。
...
| 配置模式                    | 临时复制 `~/.hopcode`，并隔离 `HOPCODE_RUNTIME_DIR` |
...
本轮没有修改全局 hopcode 配置。原始 runtime artifacts 在：

- `.hopcode/runtime-bench/2026-05-20T13-51-58-731Z-oom-stress`
- `.hopcode/runtime-bench/2026-05-20T15-20-37-790Z-oom-amplified`
...
每个模型 worker 都复用同一个 session，不断 `--resume`。每一轮要求 HopCode：
...
5 月 18 日 maintainer benchmark 里，普通任务下 hopcode process-tree RSS 峰值大多在
`942.5 MiB` 到 `1006.6 MiB`。这些数据主要说明：当时 HopCode 的任务期
...
等于 HopCode 已经没有任何 large-output memory 风险。
...
4. 仍然存在一个独立的极端 large-output 问题：`2048 MiB` stdout 附近，HopCode
