# HopCode Runtime Memory Investigation Plan

Date: 2026-05-18

## Context

Local benchmarks show HopCode using substantially more process-tree RSS than
Claude Code for similar non-interactive CLI task shapes. The latest five-case
matrix found HopCode peaking around `0.83-1.04 GiB` while Claude Code stayed
...
- HopCode has been compared across multiple models.
- HopCode and Claude Code have been compared on the same task shapes where
...
- HopCode used more tokens than Claude Code in every tested matrix cell.
...
The current data points toward a HopCode runtime/path issue more than a model
provider issue.

The strongest current inference is:

> HopCode appears to carry a high non-interactive CLI task execution
...
problem is that HopCode initializes or retains heavier state for the same
...
1. **Process and module startup/execution cost**: HopCode may initialize more
   runtime, tools, UI/session infrastructure, or provider machinery than needed
   for non-interactive CLI tasks.
2. **History and context assembly**: HopCode may retain or construct larger
...
2. Add or extend local diagnostic output so HopCode can report:
...
history loading. The implementation should still follow HopCode's own
...
The intended claim is narrower: HopCode shows a consistent local RSS gap in
