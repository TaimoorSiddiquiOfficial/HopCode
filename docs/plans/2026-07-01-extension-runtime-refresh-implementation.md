...
HopCode should keep the existing user experience where extension UI
...
for external file changes that HopCode did not initiate, such as editing
...
For HopCode, automatic mutation refresh is the better default because it
...
- Do not replace HopCode's automatic extension mutation behavior with a
