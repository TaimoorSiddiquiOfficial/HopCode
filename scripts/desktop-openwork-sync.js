...
Commit-migrate changes between hopcode packages/desktop and OpenWork.

Modes:
  --mode auto      Refuse if direction is ambiguous
  --mode export    Apply hopcode packages/desktop commits to OpenWork
  --mode import    Apply OpenWork commits to hopcode packages/desktop
...
function parseArgs(argv) {
    ...
    let openworkRef = process.env.OPENWORK_REF?.trim() ||
        ...
        'main';
    let hopcodeBase = process.env.HOPCODE_BASE_REF?.trim() || 'HEAD';
...
async function shouldSkipSyncedCommit(cwd, commit, mode) {
    ...
    if (mode === 'import' &&
        (syncMode === 'export' || findTrailer(body, 'hopcode-Commit'))) {
        return 'already came from hopcode';
    }
    if (mode === 'export' &&
...
async function migrateCommits(params) {
    ...
    const targetTrailer = params.mode === 'import' ? 'OpenWork-Commit' : 'hopcode-Commit';
...
async function runExport(options) {
    ...
    const base = await resolveSourceBase(repoRoot, options.sourceBase, openworkRoot, options.openworkRef, 'hopcode-Commit');
    const pathspecs = exportPathspecs(options.overlayPaths);
    const commits = await getSourceCommits(repoRoot, base, source, pathspecs);
    if (commits.length === 0) {
        console.log('No hopcode source changes to export.');
        return;
    }
...
async function runImport(options) {
    ...
    await ensureCleanWorktree(openworkRoot, 'OpenWork checkout');
    await ensureCleanWorktree(repoRoot, 'hopcode checkout');
    const source = await revParse(openworkRoot, options.openworkRef);
    const base = await resolveSourceBase(openworkRoot, options.sourceBase, repoRoot, options.hopcodeBase, 'OpenWork-Commit');
    ...
    await switchTargetBranch(repoRoot, branch, options.hopcodeBase);
    const hopcodeBase = await revParse(repoRoot, options.hopcodeBase);
    const count = await migrateCommits({
        mode: 'import',
