...
async function testProviderConnectivity(provider) {
    ...
    const isLocalProvider = provider.id.includes('local') || provider.id.includes('localhost');
    if (isLocalProvider) {
        result.status = 'skipped';
        result.errorMessage =
...
function formatStatus(result) {
    const { provider, status, responseTime, statusCode, errorMessage, hasApiKey, } = result;
    let details = '';
    let statusIcon = '';
    switch (status) {
        case 'reachable':
...
function generateReport(results) {
    ...
    for (const status of statuses) {
        ...
        lines.push(`| ${status.status} | ${status.label} (\`${status.id}\`) | ${urlDisplay} | ${apiKeyStatus} | ${status.details} |`);
    }
    lines.push('');
    // Issues section
    ...
    lines.push('');
    const noApiKey = statuses.filter((s) => s.apiKeyConfigured === false && s.status !== '○');
    if (noApiKey.length > 0) {
        lines.push('### Missing API Keys');
        lines.push('');
        lines.push('The following providers are reachable but do not have API keys configured:');
        lines.push('');
        for (const provider of noApiKey) {
            lines.push(`- **${provider.label}**: Run \`hopcode auth ${provider.id}\` to configure`);
        }
        lines.push('');
    }
    return lines.join('\n');
...
async function runDiagnostics() {
    ...
    const unreachable = results.filter((r) => r.status === 'error' || r.status === 'unreachable').length;
    if (unreachable > 0) {
...
//# sourceMappingURL=diagnose-providers.js.map