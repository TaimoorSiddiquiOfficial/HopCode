...
#   curl -fsSL https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-qwen-standalone.bat -o %TEMP%\install-qwen.bat && %TEMP%\install-qwen.bat
#
...
function Test-WritableDirectory {
    ...
    $probe = Join-Path $Directory ('.hopcode-write-test-' + [IO.Path]::GetRandomFileName())
    try {
...
function Install-CurrentCmdPathShim {
    ...
    foreach ($candidate in $candidates) {
        ...
        $shim = "@echo off`r`nREM HopCode current-session shim. Generated by install-qwen-standalone.ps1.`r`ncall `"$QwenCommand`" %*`r`n"
...
$qwenDefaultInstallerUrl = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-qwen-standalone.bat'
$qwenDefaultChecksumsUrl = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/SHA256SUMS'
if ([string]::IsNullOrEmpty($env:HOPCODE_INSTALLER_BAT_URL)) {
    ...
    $qwenInstallerUrl = $env:HOPCODE_INSTALLER_BAT_URL
}
...
if ([string]::IsNullOrEmpty($env:HOPCODE_INSTALLER_CHECKSUMS_URL)) {
    ...
    $qwenChecksumsUrl = $env:HOPCODE_INSTALLER_CHECKSUMS_URL
}
...
try {
    ...
    Write-Error "Failed to download HopCode installer from ${qwenInstallerUrl}: $($_.Exception.Message)"
    exit 1
}
...
try {
    ...
    Write-Error "Failed to download HopCode installer checksums from ${qwenChecksumsUrl}: $($_.Exception.Message)"
    exit 1
}
...
$qwenPreviousParentPowerShell = $env:HOPCODE_INSTALLER_PARENT_POWERSHELL
try {
    ...
    if ($null -eq $qwenPreviousParentPowerShell) {
        ...
        $env:HOPCODE_INSTALLER_PARENT_POWERSHELL = $qwenPreviousParentPowerShell
    }
    Remove-Item -LiteralPath $qwenInstallerPath -Force -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $qwenChecksumsPath -Force -ErrorAction SilentlyContinue
}
