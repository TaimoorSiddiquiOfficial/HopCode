...
#   curl -fsSL https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-hopcode-standalone.bat -o %TEMP%\install-hopcode.bat && %TEMP%\install-hopcode.bat
#
...
function Install-CurrentCmdPathShim {
    param([string]$HopCodeCommand, [string]$PathValue)
    ...
    foreach ($candidate in $candidates) {
        ...
        $shim = "@echo off`r`nREM HopCode current-session shim. Generated by install-hopcode-standalone.ps1.`r`ncall `"$HopCodeCommand`" %*`r`n"
...
function Update-CurrentShell {
    ...
    if ($parentProcessName -ieq 'cmd.exe') {
        ...
        $shimPath = Install-CurrentCmdPathShim -HopCodeCommand $hopcodeCommandPath -PathValue $inheritedPath
...
$hopcodeDefaultInstallerUrl = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-hopcode-standalone.bat'
$hopcodeDefaultChecksumsUrl = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/SHA256SUMS'
