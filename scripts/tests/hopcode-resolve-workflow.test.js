...
describe('hopcode resolve workflow', () => {
  const workflow = readFileSync(
    path.join(repoRoot, '.github/workflows/hopcode-pr-review.yml'),
    'utf8',
  );
  ...
  it('uses the existing PR command workflow', () => {
    expect(
      existsSync(
        path.join(repoRoot, '.github/workflows/hopcode-fix-conflicts.yml'),
      ),
    ).toBe(false);
    ...
    expect(workflow).toContain(
      "startsWith(github.event.comment.body, '@hopcode /resolve')",
    );
  ...
  it('listens for /resolve comments', () => {
    ...
    expect(workflow).toContain("format('@hopcode /resolve{0}',");
  ...
  it('reports failure paths instead of falling through silently', () => {
    ...
    expect(workflow).toContain(
      'HopCode attempted to resolve merge conflicts but the run did not complete successfully.',
    );
  ...
  it('uses resolve naming for run artifacts', () => {
    expect(workflow).toContain('hopcode-resolve-');
    expect(workflow).toContain('/tmp/hopcode-resolve');
    expect(workflow).toContain('<!-- hopcode-resolve-result -->');
    expect(workflow).not.toContain('hopcode-fix-conflicts');
  });
  ...
  it('isolates review agent state per run', () => {
    ...
    expect(cleanStep).toContain('mkdir -p "$HOPCODE_HOME"');
  ...
  it('fetches the PR head into a collision-free local ref', () => {
    expect(resolveJob).toContain(
      'head_fetch_ref="refs/remotes/origin/hopcode-resolve/pr-${PR_NUMBER}/head"',
    );
    expect(resolveJob).toContain(
