...
const workflow = readFileSync('.github/workflows/hopcode-triage.yml', 'utf8');
...
describe('hopcode-triage tmux workflow', () => {
  ...
  it('installs the heavy tmux test harness only for runnable PRs', () => {
    ...
    expect(installStep).toContain(
      "npm install -g --registry=https://registry.npmjs.org '@hopcode/hopcode@latest'",
    );
    expect(installStep).toContain('hopcode --version');
    expect(installStep).toContain('tmux -V');
    expect(resolverStep).not.toContain('tmux');
    expect(resolverStep).not.toContain('npm install');
    expect(resolverStep).not.toContain('hopcode --version');
    expect(
