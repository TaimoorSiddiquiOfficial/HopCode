...
const DIST_ALLOWED_ENTRIES = new Set([
  ...
  'locales',
  'examples',
  // Web Shell SPA served at the daemon root by `hopcode serve` (index.html +
...
async function main() {
  ...
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'hopcode-standalone-'),
  );
...
function copyNativeAddon(packageRoot, target) {
  ...
  if (!hasNativePrebuild(prebuildSrc)) {
    if (process.env.HOPCODE_STANDALONE_REQUIRE_AUDIO_CAPTURE_PREBUILD === '1') {
      fail(
  ...
  const addonDest = path.join(modulesDir, '@hopcode', 'audio-capture');
