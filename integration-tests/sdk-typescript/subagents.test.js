...
import { query, isSDKAssistantMessage, } from '@hoptrendy/sdk';
    ...
    describe('Subagent Execution', () => {
        it('should delegate task to subagent when appropriate', async () => {
            const fileReaderAgent = {
                ...
                systemPrompt: 'Use read_file to read the requested file, then report its exact contents.',
                level: 'session',
                tools: ['read_file'],
            };
            const testFile = helper.getPath('test.txt');
            const q = query({
                prompt: `Use the agent tool to ask the file-reader subagent to read ${testFile}. ` +
