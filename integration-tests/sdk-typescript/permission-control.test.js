...
import { query, isSDKAssistantMessage, isSDKResultMessage, isSDKUserMessage, } from '@hoptrendy/sdk';
...
describe('Permission Control (E2E)', () => {
    ...
    describe('setPermissionMode API', () => {
        ...
        it('should change permission mode from izn to plan', async () => {
            ...
            try {
                ...
                await Promise.race([
                    firstResponsePromise,
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout waiting for first response')), 60000)),
                ]);
                expect(firstResponseReceived).toBe(true);
                ...
                await Promise.race([
                    secondResponsePromise,
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout waiting for second response')), 90000)),
                ]);
                expect(secondResponseReceived).toBe(true);
            }
            finally {
        ...
        it('should throw error when setPermissionMode is called on closed query', async () => {
            ...
            await q.close();
            await expect(q.setPermissionMode('izn')).rejects.toThrow('Query is closed');
    ...
    describe('ApprovalMode behavior tests', () => {
        ...
        describe('izn mode', () => {
            ...
            it('should not invoke canUseTool callback in izn mode', async () => {
                let callbackInvoked = false;
                const q = query({
                ...
                for (const mode of modes) {
                    const q = query({
                        ...
                        options: {
                            ...
                            permissionMode: mode,
                            cwd: testDir,
                            canUseTool: mode === 'izn' || mode === 'auto-edit'
                                ? undefined
