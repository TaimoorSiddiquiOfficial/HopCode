...
const SKIP =
  process.env['HOPCODE_BENCHMARK_ENABLED'] !== '1' ||
  SANDBOX_ENABLED ||
...
interface StartupBenchmarkSnapshot {
  ...
  runHopCodeServeToListen?: Percentiles;
}
...
async function runStartupIteration(
  ...
  const workspace = makeTempWorkspace(
    `${iteration}`,
    'hopcode-daemon-startup-benchmark',
  );
  ...
  () => {
    it(
      ...
      async () => {
        ...
        snapshot.runHopCodeServeToListen = percentiles(
          measuredRuns.map((run) => run.runHopCodeServeToListenMs),
        );
        ...
        expect(snapshot.runHopCodeServeToListen.p99).toBeLessThan(
    ...
    afterAll(() => {
      ...
      console.log(
        `  runHopCodeServe -> listening:     ${formatPercentiles(
          snapshot.runHopCodeServeToListen,
        )}`,
      );
...
function renderMarkdown(s: StartupBenchmarkSnapshot): string {
  ...
  lines.push(
    ...
    '## Scope',
    '',
    'This benchmark stops at the stdout listening line and does not create a daemon session. It records the preheat state visible at listen time, but preheat completion is not part of the primary metric. Use `hopcode-daemon-vs-cli-benchmark.test.ts` for first-session, memory, and prompt metrics.',
    '',
  );
  return lines.join('\n');
}
