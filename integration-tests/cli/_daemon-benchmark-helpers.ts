...
/**
 * Benchmark-only helpers extracted from `hopcode-daemon-vs-cli-benchmark.test.ts`.
 *
...
export async function measureCliStartupWithProfiler(opts?: {
  ...
  const perfDir = path.join(os.homedir(), '.hopcode', 'startup-perf');
  ...
  const result = await spawnCliWithTime(
    ['-p', 'x', '--output-format', 'text'],
    {
      cwd: opts?.cwd,
      env: {
        HOPCODE_CODE_PROFILE_STARTUP: '1',
        HOPCODE_CODE_PROFILE_STARTUP_OUTER: '1',
      },
    },
  );
