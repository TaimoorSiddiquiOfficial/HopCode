...
const PROMPT_ITERATIONS = Number(process.env['HOPCODE_BASELINE_PROMPT_ITERATIONS'] ?? (HEAVY ? 100 : 20));
const RSS_SAMPLE_INTERVAL_MS = Number(process.env['HOPCODE_BASELINE_RSS_SAMPLE_INTERVAL_MS'] ?? 100);
const RSS_SAMPLE_DURATION_MS = Number(process.env['HOPCODE_BASELINE_RSS_SAMPLE_DURATION_MS'] ??
    (HEAVY ? 15_000 : 5_000));
...
const HAS_PROMPT_LATENCY_CREDENTIAL = process.env['HOPCODE_BASELINE_ENABLE_PROMPT_LATENCY'] === '1' ||
    PROMPT_LATENCY_CREDENTIAL_ENV_KEYS.some((key) => Boolean(process.env[key])) ||
    Object.entries(process.env).some(([key, value]) => key.startsWith('HOPCODE_CUSTOM_API_KEY_') && Boolean(value));
const SKIP_PROMPT_LATENCY = process.env['HOPCODE_BASELINE_SKIP_PROMPT_LATENCY'] === '1' ||
        ...
        if (SKIP_PROMPT_LATENCY) {
            it('prompt latency skipped (no model credential env)', () => {
                snapshot.promptLatency = {
                    ...
                    totalMs: null,
                    skipped: true,
                    skipReason: 'No recognized model credential env var is set; prompt latency requires real model access. Set HOPCODE_BASELINE_ENABLE_PROMPT_LATENCY=1 to force-run with non-env auth.',
                };
...
//# sourceMappingURL=hopcode-serve-baseline.test.js.map