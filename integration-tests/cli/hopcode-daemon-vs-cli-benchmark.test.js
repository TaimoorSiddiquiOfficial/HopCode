 ...
 *
 * Compares the two hopcode execution modes across startup latency,
 ...
 *
 * Gated by HOPCODE_BENCHMARK_ENABLED=1 — does NOT run in the default CI
 * suite. POSIX only (uses `ps`, `pgrep`, `/usr/bin/time`).
 *
 * Outputs a JSON + Markdown snapshot to the integration test output
 * directory, similar to `hopcode-serve-baseline.test.ts`.
 */
...
const HAS_MODEL_KEY = PROMPT_CREDENTIAL_ENV_KEYS.some((k) => Boolean(process.env[k])) ||
    Object.entries(process.env).some(([k, v]) => k.startsWith('HOPCODE_CUSTOM_API_KEY_') && Boolean(v));
...
const snapshot = {
    ...
    notes: [
        'CLI cold start uses -p mode with HOPCODE_CODE_PROFILE_STARTUP=1 to ' +
...
// Tests
// ---------------------------------------------------------------------------
(SKIP ? describe.skip : describe)('daemon vs CLI benchmark (POSIX-only, HOPCODE_BENCHMARK_ENABLED=1)', { retry: 0 }, () => {
    // -----------------------------------------------------------------------
...
function renderMarkdown(s) {
    ...
    const fmtTree = (r) => r
        ...
        : 'n/a';
    const lines = [
        `# hopcode daemon vs CLI — performance benchmark`,
        ``,
...
//# sourceMappingURL=hopcode-daemon-vs-cli-benchmark.test.js.map