...
describe('sleep-interception', () => {
    ...
    it('should block sleep >= 2s and mention Monitor in guidance', async () => {
        ...
        const foundBlockedCall = await waitForShellCall((call) => call.args.includes('sleep 5') && !call.success);
        if (!foundBlockedCall) {
        ...
        const blockedCall = shellCalls().find((call) => call.args.includes('sleep 5') && !call.success);
    ...
    it('should allow sleep < 2s', async () => {
        ...
        const foundSuccessfulCall = await waitForShellCall((call) => call.args.includes('sleep 1') && call.success);
    ...
    it('should allow retrying blocked sleep with an intentional sleep comment', async () => {
        ...
        const foundIntentionalCall = await waitForShellCall((call) => call.args.includes('intentional-sleep') && call.success);
