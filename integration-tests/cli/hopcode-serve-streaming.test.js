 ...
 */
/**
 * `hopcode serve` daemon — streaming / multi-client / recovery integration.
 *
 ...
 *
 *   1. Real `hopcode --acp` child crash → daemon publishes `session_died`,
...
// note in hopcode-serve-routes.test.ts for full rationale.
...
//
// Container sandbox (HOPCODE_SANDBOX=docker/podman): the model side is a fake
// OpenAI server bound to the host's 127.0.0.1, but under the sandbox the
// daemon's `hopcode --acp` child runs inside the container and cannot reach the
...
// under any container sandbox, matching the existing hopcode-serve-baseline /
// acp-integration / cron-tools precedent.
const SKIP = process.platform === 'win32' ||
    Boolean(process.env['HOPCODE_SANDBOX'] &&
...
describeLLM('hopcode serve — child-crash recovery (real SIGKILL)', () => {
    it('publishes session_died after the hopcode --acp child is SIGKILL-ed', async () => {
        ...
        const childPids = execSync(`pgrep -P ${daemon.pid} -f "(hopcode.*--acp|cli\\.js.*--acp)"`, {
            encoding: 'utf8',
        })
            .trim()
            .split('\n')
...
describeLLM('hopcode serve — multi-client first-responder permission', () => {
    it('fans out permission_request to both subscribers; only one vote wins', async () => {
        ...
        const tmp = `/tmp/hopcode-serve-mc-${Date.now()}.txt`;
...
describeLLM('hopcode serve — Last-Event-ID resume', () => {
...
//# sourceMappingURL=hopcode-serve-streaming.test.js.map