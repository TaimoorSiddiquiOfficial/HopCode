 ...
 *
 * Useful env:
 *   HOPCODE_TUI_E2E_REPO=/path/to/hopcode
 *   HOPCODE_TUI_E2E_OUT=/tmp/hopcode-tui-subagent-flicker
 *   HOPCODE_TUI_E2E_MAX_CLEAR_PAIRS=10       (default: 10)
 *   HOPCODE_TUI_E2E_MAX_CLEAR_SCREEN=20      (default: 20)
 *   HOPCODE_TUI_E2E_MAX_ERASE_LINE=460       (default: 460 — separates fix from
 *                                          no-fix; reverting the fix raises
 *                                          this counter to ~469)
 *   HOPCODE_TUI_E2E_SUBAGENT_TOOL_CALLS=5
 */
...
function startSubagentFakeOpenAIServer(packageJsonPath, subagentToolCalls) {
    ...
    const verbose = process.env['HOPCODE_TUI_E2E_VERBOSE'] === '1' ||
...
async function main() {
    ...
    const repoRoot = resolve(process.env['HOPCODE_TUI_E2E_REPO'] ??
        process.env['HOPCODE_TUI_E2E_REPO'] ??
        defaultRepoRoot);
    const defaultOut = join(tmpdir(), 'hopcode-tui-subagent-flicker', basename(repoRoot));
    const outputDir = resolve(process.env['HOPCODE_TUI_E2E_OUT'] ??
        process.env['HOPCODE_TUI_E2E_OUT'] ??
        defaultOut);
    const maxClearPairs = envNumber('HOPCODE_TUI_E2E_MAX_CLEAR_PAIRS', envNumber('HOPCODE_TUI_E2E_MAX_CLEAR_PAIRS', 10));
    const maxClearScreen = envNumber('HOPCODE_TUI_E2E_MAX_CLEAR_SCREEN', envNumber('HOPCODE_TUI_E2E_MAX_CLEAR_SCREEN', 20));
    ...
    const maxEraseLine = envNumber('HOPCODE_TUI_E2E_MAX_ERASE_LINE', envNumber('HOPCODE_TUI_E2E_MAX_ERASE_LINE', 460));
    const subagentToolCalls = envNumber('HOPCODE_TUI_E2E_SUBAGENT_TOOL_CALLS', envNumber('HOPCODE_TUI_E2E_SUBAGENT_TOOL_CALLS', 5));
    ...
    const env = {
        ...
        // Intentionally NOT setting HOPCODE_SIMPLE so the agent tool stays in
    ...
    delete env['NO_COLOR'];
    delete env['HOPCODE_SIMPLE']; // HOPCODE_SIMPLE: '0'
