...
const IS_CONTAINER_SANDBOX = SANDBOX_MODE === 'docker' || SANDBOX_MODE === 'podman';
describe('Interactive protocol tag retry guard', () => {
    ...
    it.skipIf(process.platform === 'win32')('retries protocol leaks across SSE disconnect and completed streams', async () => {
        ...
        try {
            const isReady = await rig.poll(() => /IZN (模式|mode)/i.test(stripAnsi(rig._interactiveOutput)), 30000, 200);
