...
export class TestRig {
    ...
    setup(testName, options = {}) {
        ...
        writeFileSync(join(hopcodeDir, 'settings.json'), JSON.stringify(settings, null, 2));
    }
    ...
    async waitForTelemetryEvent(eventName, timeout) {
        ...
        return this.poll(() => {
            ...
            return logs.some((logData) => logData.attributes &&
                logData.attributes['event.name'] === `hopcode.${eventName}`);
        }, timeout, 100);
    }
                    ...
                    if (braceDepth === 0) {
                        ...
                        try {
                            ...
                            if (obj.body &&
                                ...
                                obj.attributes['event.name'] === 'hopcode.tool_call') {
                                logs.push({
            ...
            if (logData.scopeMetrics) {
                for (const scopeMetric of logData.scopeMetrics) {
                    for (const metric of scopeMetric.metrics) {
                        if (metric.descriptor.name === `hopcode.${metricName}`) {
                            return metric;
                        }
                    }
                }
