...
description: Sync hopcode packages/desktop with modelstudioai/openwork using commit-by-commit path migration, not subtree split or tree overwrite. Use when exporting hopcode desktop changes to OpenWork, importing OpenWork desktop changes into hopcode, preserving target-owned overlay files such as README.md, resolving sync conflicts, or preparing sync PR branches between the two repositories.
---

# OpenWork Desktop Sync

Use this skill to sync desktop changes between this hopcode repo and an
...
hopcode `packages/desktop` and the OpenWork repository root, then applies each
commit with `git apply -3`.

Commits that already came from the receiving repository are skipped by their
sync trailers. During import, hopcode-origin export commits are skipped;
...
Successful sync commits include trailers such as `hopcode-Commit` or
...
```bash
bun run desktop-openwork-sync --mode export --source-base <hopcode-ref>
bun run desktop-openwork-sync --mode import --source-base <openwork-ref>
```

## Modes

- `--mode export`: hopcode `packages/desktop` commits -> OpenWork.
- `--mode import`: OpenWork commits -> hopcode `packages/desktop`.
