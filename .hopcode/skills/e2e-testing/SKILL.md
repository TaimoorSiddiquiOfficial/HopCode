...
- **Reproducing bugs**: use the globally installed `hopcode` command — this matches
...
- **Runtime-only checks (fastest)**: `npm run dev -- "<prompt>" <flags>` — runs TS
  ...
  shipped artifact itself matters. (`<hopcode>` below can be `npm run dev --`.)

### Running against a real model

Headless auth comes from `~/.hopcode`. Force a known-good model with `--auth-type` +
`--model`:
...
`HOPCODE_RUNTIME_DIR=<dir>` redirects hopcode's runtime output — `tmp/`, `debug/`,
and `projects/<sanitized-cwd>/...` (chat recordings, auto-memory, history) —
into `<dir>` instead of `~/.hopcode`. Config (`settings.json`, OAuth tokens,
`commands/`) still reads from `~/.hopcode`, so real auth and provider config
...
```bash
HOPCODE_RUNTIME_DIR=/tmp/test-1/runtime <hopcode> "prompt" ...
```
...
Run the CLI non-interactively with JSON output (`<hopcode>` = `hopcode` or
