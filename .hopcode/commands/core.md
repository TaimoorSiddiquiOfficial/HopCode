...
!{find packages/core \( -path packages/cli/dist -o -path packages/core/dist -o -name node_modules \) -prune -o -type f \( -name "_.ts" -o -name "_.tsx" \) ! -name "_.test.ts" ! -name "_.test.tsx" ! -name "\*.d.ts" -exec echo "--- {} ---" \; -exec cat {} \;}

In addition to the code context, you MUST strictly adhere to the following development guidelines when writing code in packages/core.
