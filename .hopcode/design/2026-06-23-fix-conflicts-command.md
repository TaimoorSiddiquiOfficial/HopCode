...
Add a maintainer-triggered `@hopcode /resolve` command for pull requests that are blocked by merge conflicts with the default branch.

## Scope

The first version is intentionally conservative:

- The command only runs in `QwenLM/hopcode`.
...
6. HopCode runs without GitHub credentials, merges `origin/<base>`, resolves conflicts, verifies the result, commits, and writes a summary artifact.
