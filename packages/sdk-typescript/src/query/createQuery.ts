...
export function query({
  ...
  /**
   * The prompt to send to the HopCode Code CLI process.
  ...
  const pathToHopCodeExecutable = options.pathToHopCodeExecutable;
...
function validateOptions(options: QueryOptions): SpawnInfo | undefined {
  ...
  try {
    return prepareSpawnInfo(options.pathToHopCodeExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToHopCodeExecutable: ${errorMessage}`);
  }
}
