# hopcode-serve-bridge MCP Server

将 `hopcode serve` 的 HTTP API 封装为 MCP (Model Context Protocol) Server，方便任何支持 MCP 的客户端直接调用。

## 快速开始

### 1. 启动 hopcode serve daemon
...
HOPCODE_SERVER_TOKEN=<your-token> hopcode serve \
  --port 4170 \
...
```json
{
  "mcpServers": {
    "hopcode-serve-bridge": {
      "type": "stdio",
      "command": "node",
      "args": ["/path/to/hopcode/packages/sdk-typescript/dist/daemon-mcp/serve-bridge/bin.js"],
      "env": {
        "HOPCODE_DAEMON_URL": "http://127.0.0.1:4170",
        "HOPCODE_DAEMON_TOKEN": "<your-token>",
        "HOPCODE_WORKSPACE_CWD": "/path/to/your/project"
      }
    }
  }
}
