...
export function workspaceWriteTools(state: BridgeState): any[] {
  return [
    ...
    tool(
      ...
      {
        mode: z.enum(['plan', 'default', 'auto-edit', 'auto', 'izn']).describe('Approval mode.'),
        ...
        if (!state.allowGlobalScope) {
          ...
          if (dangerousModes.includes(args.mode)) {
            return formatToolError(
              `Approval modes '${dangerousModes.join("', '")}' are restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.`,
            );
          }
          if (args.persist) {
            return formatToolError(
              'Persisting approval mode changes is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.',
            );
          }
        }
    ...
    tool(
      ...
      handler(async (args) => {
        if (!state.allowGlobalScope) {
          return formatToolError(
            'Tool toggling is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.',
          );
        }
        return formatJsonResult(
    ...
    tool(
      'workspace_init',
      'Scaffold an empty HOPCODE.md at the workspace root. No LLM invocation.',
      {
        force: z.boolean().optional().describe('Overwrite existing HOPCODE.md if present.'),
      },
      handler(async (args) =>
        formatJsonResult(
    ...
    tool(
      ...
      handler(async (args) => {
        if (!state.allowGlobalScope) {
          return formatToolError(
            'MCP server restart is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.',
          );
        }
        return formatJsonResult(
    ...
    tool(
      ...
      'Read workspace memory (HOPCODE.md hierarchy).',
      {},
      handler(async () =>
    ...
    tool(
      ...
      'Write to workspace memory (HOPCODE.md). Supports append or replace mode.',
      {
      ...
      handler(async (args) => {
        if (args.scope === 'global' && !state.allowGlobalScope) {
          return formatToolError(
            'Global scope is disabled for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.',
          );
        }
        return formatJsonResult(
