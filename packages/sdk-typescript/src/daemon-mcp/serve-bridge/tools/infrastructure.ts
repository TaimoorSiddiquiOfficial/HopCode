...
export function infrastructureTools(state: BridgeState): any[] {
  return [
    tool(
      'health',
      'Check if the hopcode serve daemon is alive.',
      {},
      handler(async () => formatJsonResult(await state.client.health())),
    ),
    tool(
      'capabilities',
      'Get hopcode serve daemon capabilities including protocol versions, mode, features, model services, and workspace CWD.',
      {},
