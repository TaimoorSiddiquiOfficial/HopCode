...
export function workspaceWriteTools(state) {
    return [
        ...
        tool('session_set_approval_mode', 'Change the approval mode of a session (plan, default, auto-edit, auto, IZN).', {
            mode: z
            ...
            if (!state.allowGlobalScope) {
                const dangerousModes = ['izn', 'auto', 'auto-edit'];
                if (dangerousModes.includes(args.mode)) {
                    return formatToolError(`Approval modes '${dangerousModes.join("', '")}' are restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.`);
                }
                if (args.persist) {
                    return formatToolError('Persisting approval mode changes is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
                }
            }
        ...
        tool('workspace_tool_toggle', 'Enable or disable a tool in the workspace settings.', {
            ...
            if (!state.allowGlobalScope) {
                return formatToolError('Tool toggling is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
            return formatJsonResult(await state.client.setWorkspaceToolEnabled(args.tool_name, args.enabled));
        })),
        tool('workspace_init', 'Scaffold an empty HOPCODE.md at the workspace root. No LLM invocation.', {
            force: z
                .boolean()
                .optional()
        ...
        tool('workspace_mcp_restart', 'Restart a configured MCP server. Pre-checks budget before restarting.', {
            ...
            if (!state.allowGlobalScope) {
                return formatToolError('MCP server restart is restricted for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
            return formatJsonResult(await state.client.restartMcpServer(args.server_name));
        })),
        tool('workspace_memory_read', 'Read workspace memory (HOPCODE.md hierarchy).', {}, handler(async () => formatJsonResult(await state.client.workspaceMemory()))),
        tool('workspace_memory_write', 'Write to workspace memory (HOPCODE.md). Supports append or replace mode.', {
            ...
            if (args.scope === 'global' && !state.allowGlobalScope) {
                return formatToolError('Global scope is disabled for security. Set HOPCODE_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
...
async function handleAgentUpdate(state, args) {
    ...
    if (!hasField) {
        return formatToolError('At least one field to update must be provided (description, system_prompt, tools, disallowed_tools, or model).');
    }
