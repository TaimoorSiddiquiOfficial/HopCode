...
export class GitHubDeviceFlowAuth {
    ...
    clientSecret;
    baseUrl;
    constructor(clientId, clientSecret, hostname = 'github.com') {
     ...
     */
    async authenticateWithProgress(onUserCode, onToken, onError, progressCallback) {
        try {
            ...
            if (progressCallback) {
                progressCallback(`Enter code ${deviceResponse.user_code} at ${deviceResponse.verification_uri}`);
            }
 ...
 */
export function createGitHubDeviceFlowAuth(clientId, clientSecret, hostname) {
    return new GitHubDeviceFlowAuth(clientId, clientSecret, hostname);
}
//# sourceMappingURL=github-device-flow-auth.js.map