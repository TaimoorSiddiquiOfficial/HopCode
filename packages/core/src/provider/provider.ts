...
export namespace Provider {
  ...
  function loadBaseURL(model: Model, options: Record<string, any>) {
    ...
    return raw.replace(/\$\{([^}]+)\}/g, (match, key) => {
      const val =
        Env.get(String(key)) ?? vars?.[String(key) as keyof typeof vars];
      return val ?? match;
    });
  }
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    'cloudflare-ai-gateway': async (input) => {
      ...
      const apiToken = await (async () => {
        const envToken =
          Env.get('CLOUDFLARE_API_TOKEN') || Env.get('CF_AIG_TOKEN');
        if (envToken) return envToken;
        const auth = await Auth.get(input.id);
        if (auth?.type === 'api') return auth.key;
        return undefined;
      ...
      const { createUnified } = await import(
        'ai-gateway-provider/providers/unified'
      );
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      for (const item of priority) {
        if (providerID === 'amazon-bedrock') {
          ...
          const candidates = Object.keys(provider.models).filter((m) =>
            m.includes(item),
          );
          ...
          if (region) {
            ...
            if (regionPrefix === 'us' || regionPrefix === 'eu') {
              const regionalMatch = candidates.find((m) =>
                m.startsWith(`${regionPrefix}.`),
              );
          ...
          const unprefixed = candidates.find(
            (m) => !crossRegionPrefixes.some((p) => m.startsWith(p)),
          );
    ...
    const opencodeProvider = await state().then(
      (state) => state.providers['opencode'],
    );
