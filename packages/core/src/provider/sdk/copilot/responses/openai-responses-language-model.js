...
export class OpenAIResponsesLanguageModel {
    ...
    async getArgs({ maxOutputTokens, temperature, stopSequences, topP, topK, presencePenalty, frequencyPenalty, seed, prompt, providerOptions, tools, toolChoice, responseFormat, }) {
        ...
        const webSearchToolName = tools?.find((tool) => tool.type === 'provider-defined' &&
            (tool.id === 'openai.web_search' ||
                tool.id === 'openai.web_search_preview'))?.name;
            ...
            switch (part.type) {
                ...
                case 'message': {
                    for (const contentPart of part.content) {
                        if (options.providerOptions?.openai?.logprobs &&
            ...
            stream: response.pipeThrough(new TransformStream({
                ...
                transform(chunk, controller) {
                    ...
                    if (isResponseOutputItemAddedChunk(value)) {
                        ...
                        const activeItem = currentReasoningOutputIndex !== null
