...
(function (Provider) {
    ...
    function googleVertexVars(options) {
    ...
    function loadBaseURL(model, options) {
        ...
        return raw.replace(/\$\{([^}]+)\}/g, (match, key) => {
            const val = Env.get(String(key)) ?? vars?.[String(key)];
            return val ?? match;
        });
    }
    ...
    const CUSTOM_LOADERS = {
        ...
        async opencode(input) {
            const hasKey = await (async () => {
                const env = Env.all();
                if (input.env.some((item) => env[item]))
                    return true;
                if (await Auth.get(input.id))
                    return true;
        ...
        'amazon-bedrock': async () => {
            ...
            const containerCreds = Boolean(process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI ||
            ...
            const endpoint = providerConfig?.options?.['endpoint'] ??
        ...
        'google-vertex': async (provider) => {
            const project = provider.options?.project ??
                ...
                Env.get('GCP_PROJECT') ??
                Env.get('GCLOUD_PROJECT');
            const location = provider.options?.location ??
                Env.get('GOOGLE_CLOUD_LOCATION') ??
                Env.get('VERTEX_LOCATION') ??
                'us-central1';
        ...
        'google-vertex-anthropic': async () => {
            const project = Env.get('GOOGLE_CLOUD_PROJECT') ??
                Env.get('GCP_PROJECT') ??
                Env.get('GCLOUD_PROJECT');
            const location = Env.get('GOOGLE_CLOUD_LOCATION') ??
                Env.get('VERTEX_LOCATION') ??
                'global';
            const autoload = Boolean(project);
            if (!autoload)
        ...
        'cloudflare-ai-gateway': async (input) => {
            ...
            if (!apiToken) {
                throw new Error('CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. ' +
            ...
            return {
                autoload: true,
                async getModel(_sdk, modelID, _options) {
    ...
    function fromModelsDevProvider(provider) {
        return {
            id: provider.id,
            source: 'custom',
            name: provider.name,
    ...
    const state = Instance.state(async () => {
        ...
        try {
            const _ = __addDisposableResource(env_1, log.time('state'), false);
            const config = await Config.get();
            ...
            for (const plugin of await Plugin.list()) {
                ...
                if (auth) {
                    const options = await p.auth.loader(() => Auth.get(providerID), database[p.auth.provider]);
                    const opts = options ?? {};
                ...
                if (providerID === 'github-copilot') {
                    ...
                    if (!disabled.has(enterpriseProviderID)) {
                        ...
                        if (enterpriseAuth) {
                            const enterpriseOptions = await p.auth.loader(() => Auth.get(enterpriseProviderID), database[enterpriseProviderID]);
                            const opts = enterpriseOptions ?? {};
                            const patch = providers[enterpriseProviderID]
                                ? { options: opts }
                                : { source: 'custom', options: opts };
    ...
    async function getSDK(model) {
        try {
            ...
            try {
                const _ = __addDisposableResource(env_2, log.time('getSDK', {
                    providerID: model.providerID,
                }), false);
                const s = await state();
                const provider = s.providers[model.providerID];
                const options = { ...provider.options };
                if (model.providerID === 'google-vertex' &&
            ...
            throw new Provider.InitError({ providerID: model.providerID }, { cause: e });
        }
    }
    ...
    Provider.getModel = getModel;
    async function getLanguage(model) {
        const s = await state();
    ...
    Provider.getLanguage = getLanguage;
    async function closest(providerID, query) {
        const s = await state();
        const provider = s.providers[providerID];
        if (!provider)
            return undefined;
    ...
    Provider.closest = closest;
    async function getSmallModel(providerID) {
        ...
        if (provider) {
            ...
            for (const item of priority) {
                if (providerID === 'amazon-bedrock') {
                    ...
                    if (globalMatch)
                        return getModel(providerID, globalMatch);
                    ...
                    if (region) {
                        ...
                        if (regionPrefix === 'us' || regionPrefix === 'eu') {
                            ...
                            if (regionalMatch)
                                return getModel(providerID, regionalMatch);
                        }
                    }
                    const unprefixed = candidates.find((m) => !crossRegionPrefixes.some((p) => m.startsWith(p)));
                    if (unprefixed)
                        return getModel(providerID, unprefixed);
                }
                else {
    ...
    Provider.getSmallModel = getSmallModel;
    const priority = ['gpt-5', 'claude-sonnet-4', 'big-pickle', 'gemini-3-pro'];
    function sort(models) {
        return sortBy(models, [
            (model) => priority.findIndex((filter) => model.id.includes(filter)),
            'desc',
        ], [(model) => (model.id.includes('latest') ? 0 : 1), 'asc'], [(model) => model.id, 'desc']);
    }
    Provider.sort = sort;
    ...
    Provider.parseModel = parseModel;
    Provider.ModelNotFoundError = NamedError.create('ProviderModelNotFoundError', z.object({
        providerID: z.string(),
        modelID: z.string(),
        suggestions: z.array(z.string()).optional(),
    }));
    Provider.InitError = NamedError.create('ProviderInitError', z.object({
        providerID: z.string(),
    }));
})(Provider || (Provider = {}));
