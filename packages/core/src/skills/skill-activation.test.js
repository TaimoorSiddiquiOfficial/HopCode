...
function makeSkill(overrides) {
    return {
        ...
        filePath: overrides.filePath ?? '/proj/.hopcode/skills/test/SKILL.md',
        ...overrides,
    };
}
...
describe('extractToolFilePaths → SkillActivationRegistry integration', () => {
    ...
    it('activates a skill keyed on src/**/*.ts from glob({ path: "src", pattern: "**/*.ts" })', async () => {
        ...
        for (const c of candidates) {
            for (const n of await reg.matchAndConsume(c))
                activated.add(n);
        }
    ...
    it.skipIf(process.platform === 'win32')('activates skills when file is reached via symlinked directory', async () => {
        const fs = await import('node:fs/promises');
        const path = await import('node:path');
        const os = await import('node:os');
        const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'symlink-test-'));
    ...
    it.skipIf(process.platform === 'win32')('activates skills when project root itself is a symlink', async () => {
        const fs = await import('node:fs/promises');
        const path = await import('node:path');
        const os = await import('node:os');
        const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'symlink-test-'));
        ...
        await fs.writeFile(realFile, '// test');
        const reg = new SkillActivationRegistry([makeSkill({ name: 'ts-helper', paths: ['src/**/*.ts'] })], symlinkProject);
