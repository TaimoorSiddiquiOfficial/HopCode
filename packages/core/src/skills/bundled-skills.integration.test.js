...
describe('bundled SKILL.md files', () => {
    ...
    it('ships dataviz validator and references with the bundled skill', () => {
        ...
        expect(fs.existsSync(path.join(datavizDir, 'scripts', 'validate_palette.js'))).toBe(true);
        expect(fs.existsSync(path.join(datavizDir, 'references', 'palette.md'))).toBe(true);
        expect(fs.existsSync(path.join(datavizDir, 'references', 'choosing-a-form.md'))).toBe(true);
        expect(fs.existsSync(path.join(datavizDir, 'references', 'anti-patterns.md'))).toBe(true);
    });
});
