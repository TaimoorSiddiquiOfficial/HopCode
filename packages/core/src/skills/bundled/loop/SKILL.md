...
Use this when the user wants the loop to work a task list kept in a file (they say "work through my loop.md", "loop over the tasks in .hopcode/loop.md", or point at such a file). Tasks live in `.hopcode/loop.md` (project) or `~/.hopcode/loop.md` (home; project wins). Instead of a natural-language prompt, set the loop's `prompt` to a sentinel so each fire re-reads the file:
...
At each fire you receive either the full task list (first delivery, after the file changes, or after a compaction) or a short reminder to keep working the list established earlier. Work the tasks; in self-paced mode re-arm LoopWakeup with `<<loop.md-dynamic>>` only when continued follow-up is useful (same "don't re-arm if complete/blocked" rules as the prompt-only path). If `.hopcode/loop.md` is absent at fire time, the loop falls back to autonomous mode (it keeps working autonomously instead of no-op'ing); a recreated file is picked up on the next fire. Confirm to the user in plain language ("looping over your `.hopcode/loop.md` task list…"), not the raw sentinel.
