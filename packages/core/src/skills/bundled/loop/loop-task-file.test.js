...
describe('readLoopTaskFile', () => {
    ...
    const writeProject = (content) => fs
        ...
        .then(() => fs.writeFile(path.join(projectRoot, '.qwen', 'loop.md'), content));
    const writeHome = (content) => fs
        .mkdir(path.join(homeDir, '.qwen'), { recursive: true })
        .then(() => fs.writeFile(path.join(homeDir, '.qwen', 'loop.md'), content));
    ...
    it('does not read a project loop.md symlinked to an in-workspace file (exfiltration guard)', async () => {
        ...
        await fs.symlink(path.join('..', '.env'), path.join(projectRoot, '.qwen', 'loop.md'));
        await writeHome('user tasks');
        const result = await readLoopTaskFile({
            projectRoot,
            homeDir,
    ...
    it('does not falsely refuse a project loop.md when the workspace root is a filesystem root', async () => {
        ...
        const actual = await vi.importActual('node:fs/promises');
        vi.spyOn(fs, 'realpath').mockImplementation((p) => String(p) === projectRoot
    ...
    it('skips a FIFO/non-regular project loop.md before opening it (does not hang)', async () => {
        ...
        vi.spyOn(fs, 'lstat').mockImplementation(async (p) => String(p) === projectLoop ? fifoStat : actual.lstat(p));
    ...
    it('skips a home loop.md that resolves to a non-regular file (directory/FIFO)', async () => {
        ...
        vi.spyOn(fs, 'stat').mockImplementation(async (p) => String(p) === homeLoop ? dirStat : actual.stat(p));
        const openSpy = vi.mocked(fs.open);
        openSpy.mockClear();
        const result = await readLoopTaskFile({
            projectRoot,
    ...
    it('rethrows non-whitelisted fs errors (e.g. EACCES)', async () => {
        ...
        await expect(readLoopTaskFile({ projectRoot, homeDir, allowProjectFile: true })).rejects.toThrow(/EACCES/);
    });
    it('evicts the cached project-root realpath after a transient failure and retries on the next tick', async () => {
        ...
        realpathSpy.mockImplementation((p) => actual.realpath(p));
    ...
    it('returns a missing result when no task file exists', async () => {
        await expect(readLoopTaskFile({ projectRoot, homeDir, allowProjectFile: true })).resolves.toEqual({
            status: 'missing',
            checkedPaths: [
                path.join(projectRoot, '.qwen', 'loop.md'),
                path.join(homeDir, '.qwen', 'loop.md'),
    ...
    it('byte-caps task files above the cap and flags them truncated', async () => {
        ...
        expect(Buffer.byteLength(result.content, 'utf8')).toBe(LOOP_TASK_FILE_MAX_BYTES);
    ...
    it('truncates on a UTF-8 boundary without exceeding the cap or inserting a replacement char', async () => {
        ...
        expect(result.truncated).toBe(true);
        expect(Buffer.byteLength(result.content, 'utf8')).toBeLessThanOrEqual(LOOP_TASK_FILE_MAX_BYTES);
    ...
    it('skips a candidate that raises ENAMETOOLONG and falls through instead of throwing', async () => {
        ...
        vi.spyOn(fs, 'lstat').mockImplementation(async (p) => String(p) === projectLoop
    ...
    it('lets the original read error propagate when handle.close() also throws', async () => {
        ...
        const ebadf = Object.assign(new Error('EBADF: bad file descriptor, close'), { code: 'EBADF' });
        ...
        await expect(readLoopTaskFile({ projectRoot, homeDir, allowProjectFile: true })).rejects.toBe(eio);
        // The close was still attempted (we swallow its failure, not skip it).
        expect(close).toHaveBeenCalled();
    });
});
//# sourceMappingURL=loop-task-file.test.js.map