...
Use this skill to create HopCode extensions with the existing extension
...
HopCode hydrates path variables in manifest string fields before
...
HopCode discovers command resources recursively from `commands/**/*.md` and
...
After linking, tell the user to restart HopCode if the new extension is not
...
- If the extension is missing, inspect the link command output, confirm
  ...
  restart HopCode with `HOPCODE_DEBUG_LOG_FILE` set to a writable log path, then
  inspect that file.
