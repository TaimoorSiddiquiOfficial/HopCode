...
export class IdeClient {
    ...
    async connect() {
        if (!this.currentIde) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run HopCode in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
        ...
        if (this.workspaceRejectedPorts.size > 0) {
            this.setState(IDEConnectionStatus.Disconnected, `Found IDE companion extension, but its workspace does not match the current directory. Run HopCode from the workspace open in your IDE, or switch the IDE to this project.`, true);
            return;
        }
    ...
    static validateWorkspacePath(ideWorkspacePath, cwd) {
        ...
        if (!isWithinWorkspace) {
            return {
                isValid: false,
                error: `Directory mismatch. HopCode is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(', ')}`,
            };
        }
        return { isValid: true };
    }
    ...
    getPortFromEnv() {
        const port = process.env['HOPCODE_IDE_SERVER_PORT'];
        if (!port) {
            return undefined;
        }
    ...
    getStdioConfigFromEnv() {
        const command = process.env['HOPCODE_IDE_SERVER_STDIO_COMMAND'];
        if (!command) {
            return undefined;
        }
        const argsStr = process.env['HOPCODE_IDE_SERVER_STDIO_ARGS'];
        let args = [];
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('HOPCODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `hopcode-ide-server-${this.ideProcessInfo.pid}.json`);
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `hopcode-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return {
                    config: JSON.parse(portFileContents),
                    source: 'env-port',
