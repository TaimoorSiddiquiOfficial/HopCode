...
describe('IdeClient', () => {
    ...
    beforeEach(async () => {
        ...
        process.env['HOPCODE_IDE_WORKSPACE_PATH'] = '/test/workspace';
        delete process.env['HOPCODE_IDE_SERVER_PORT'];
        delete process.env['HOPCODE_IDE_SERVER_STDIO_COMMAND'];
        delete process.env['HOPCODE_IDE_SERVER_STDIO_ARGS'];
        // Mock dependencies
    ...
    describe('createProxyAwareFetch', () => {
        it('uses undici fetch with the proxy-aware dispatcher', async () => {
            ...
            const fetch = ideClient.createProxyAwareFetch('127.0.0.1');
            ...
            expect(mockUndiciFetch).toHaveBeenCalledWith('http://127.0.0.1:8080/mcp', expect.objectContaining({
                method: 'POST',
    ...
    describe('connect', () => {
        it('should connect using HTTP when port is provided in config file', async () => {
            process.env['HOPCODE_IDE_SERVER_PORT'] = '8080';
            const config = { port: '8080' };
            vi.mocked(fs.promises.readFile).mockResolvedValue(JSON.stringify(config));
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'ide', '8080.lock'), 'utf8');
        ...
        it('should connect using HTTP when port is provided in environment variables', async () => {
            ...
            process.env['HOPCODE_IDE_SERVER_PORT'] = '9090';
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
        ...
        it('should try a newer lock-file port when the configured port is stale', async () => {
            process.env['HOPCODE_IDE_SERVER_PORT'] = '1111';
            const primaryConfig = {
                port: '1111',
        ...
        it('should not retry raw env port when its lock belongs to another workspace', async () => {
            process.env['HOPCODE_CODE_IDE_SERVER_PORT'] = '1234';
            const envConfig = {
                port: '1234',
            ...
            vi.mocked(fs.promises.readFile).mockImplementation(async (filePath) => {
                const file = String(filePath);
                if (file === path.join('/home/test', '.hopcode', 'ide', '1234.lock')) {
                    return JSON.stringify(envConfig);
                }
                if (file === path.join('/tmp', 'hopcode-ide-server-12345.json')) {
                    throw new Error('not found');
                }
                if (file === path.join('/tmp', 'hopcode-ide-server-1234.json')) {
                    throw new Error('not found');
                }
                throw new Error(`unexpected path: ${file}`);
            });
            ...
            expect(ideClient.getConnectionStatus().details).toContain('workspace does not match');
            delete process.env['HOPCODE_CODE_IDE_SERVER_PORT'];
        });
        it('should skip explicit workspace mismatches when trying fallback ports', async () => {
            ...
            vi.mocked(fs.promises.readFile).mockImplementation(async (filePath) => {
                ...
                if (file === path.join('/home/test', '.hopcode', 'ide', '3333.lock')) {
                    return JSON.stringify(legacyConfig);
                }
                throw new Error(`unexpected path: ${file}`);
            });
            vi.mocked(fs.promises.readdir).mockResolvedValue(['2222.lock', '3333.lock']);
            vi.mocked(fs.promises.stat).mockImplementation(async (filePath) => {
                const now = Date.now();
                const file = String(filePath);
                return {
            ...
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(1, new URL('http://127.0.0.1:1111/mcp'), expect.any(Object));
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(2, new URL('http://127.0.0.1:3333/mcp'), expect.any(Object));
            expect(StreamableHTTPClientTransport).not.toHaveBeenCalledWith(new URL('http://127.0.0.1:2222/mcp'), expect.any(Object));
            expect(ideClient.getConnectionStatus().status).toBe(IDEConnectionStatus.Connected);
        });
        it('should connect using stdio when stdio config is in environment variables', async () => {
            vi.mocked(fs.promises.readFile).mockRejectedValue(new Error('File not found'));
            vi.mocked(fs.promises.readdir).mockResolvedValue([]);
            process.env['HOPCODE_IDE_SERVER_STDIO_COMMAND'] = 'env-cmd';
            process.env['HOPCODE_IDE_SERVER_STDIO_ARGS'] = '["--bar"]';
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
            expect(StdioClientTransport).toHaveBeenCalledWith({
                command: 'env-cmd',
        ...
        it('should report workspace mismatch when discovered locks belong to another workspace', async () => {
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['2222.lock']);
            vi.mocked(fs.promises.stat).mockResolvedValue({ mtimeMs: Date.now() });
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
            expect(StreamableHTTPClientTransport).not.toHaveBeenCalled();
            expect(ideClient.getConnectionStatus().status).toBe(IDEConnectionStatus.Disconnected);
    ...
    describe('validateWorkspacePath', () => {
        it('accepts JSON encoded multi-root workspace paths', () => {
            const result = IdeClient.validateWorkspacePath(JSON.stringify(['/test/other', '/test/workspace']), '/test/workspace/sub-dir');
            expect(result.isValid).toBe(true);
        });
        it('ignores relative workspace entries in IDE env parsing', () => {
            const result = IdeClient.validateWorkspacePath(JSON.stringify(['relative/path', '/test/workspace']), '/test/workspace/sub-dir');
            expect(result.isValid).toBe(true);
        });
        it('keeps delimiter encoded workspace paths working', () => {
            const result = IdeClient.validateWorkspacePath(['/test/other', '/test/workspace'].join(path.delimiter), '/test/workspace/sub-dir');
            expect(result.isValid).toBe(true);
        });
    });
    describe('getPortFromEnv', () => {
        ...
        it.each(['1', '12345', '65535'])('should return valid env port %s', async (port) => {
            process.env['HOPCODE_CODE_IDE_SERVER_PORT'] = port;
            const ideClient = await IdeClient.getInstance();
            const result = ideClient.getPortFromEnv();
    ...
    describe('getConnectionConfigFromFile', () => {
        it('should return config from the env port lock file if it exists', async () => {
            process.env['HOPCODE_IDE_SERVER_PORT'] = '1234';
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'ide', '1234.lock'), 'utf8');
            delete process.env['HOPCODE_IDE_SERVER_PORT'];
        });
        ...
        it('should fall back to scanned locks when the env port lock belongs to another workspace', async () => {
            ...
            vi.mocked(fs.promises.readFile).mockImplementation(async (filePath) => {
                ...
                if (file === path.join('/home/test', '.hopcode', 'ide', '5678.lock')) {
                    return JSON.stringify(matchingConfig);
                }
                throw new Error(`unexpected path: ${file}`);
            });
            vi.mocked(fs.promises.readdir).mockResolvedValue(['1234.lock', '5678.lock']);
            vi.mocked(fs.promises.stat).mockImplementation(async (filePath) => {
                const now = Date.now();
                const file = String(filePath);
                return {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'ide', '1234.lock'), 'utf8');
            expect(fs.promises.readdir).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'ide'));
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Ignoring IDE env lock file: workspace "/other/workspace" does not match cwd "/test/workspace/sub-dir".');
            delete process.env['HOPCODE_CODE_IDE_SERVER_PORT'];
        });
        ...
        it('should return legacy config when workspacePath is undefined', async () => {
            ...
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(config);
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'hopcode-ide-server-12345.json'), 'utf8');
            expect(fs.promises.readdir).not.toHaveBeenCalled();
        });
        ...
        it('should reject env-port legacy config from another workspace', async () => {
            ...
            const ideClient = await IdeClient.getInstance();
            const result = await ideClient.getConnectionConfigFromFile();
            const rejectedPorts = ideClient.workspaceRejectedPorts;
            expect(result).toBeUndefined();
            expect(rejectedPorts.has('9999')).toBe(true);
            expect(rejectedPorts.has('1234')).toBe(true);
            expect(fs.promises.readdir).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'ide'));
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Ignoring legacy IDE connection config: workspace "/other/workspace" does not match cwd "/test/workspace/sub-dir".');
            delete process.env['HOPCODE_CODE_IDE_SERVER_PORT'];
        });
        it.each(['../evil', '12345/../../etc', 'abc', ' 8080 ', '8080.0'])('should scan the lock directory when env port is invalid: %s', async (port) => {
            process.env['HOPCODE_CODE_IDE_SERVER_PORT'] = port;
            const ideDir = path.join('/home/test', '.hopcode', 'ide');
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['2345.lock']);
            vi.mocked(fs.promises.stat).mockResolvedValue({ mtimeMs: Date.now() });
            vi.mocked(fs.promises.readFile).mockClear();
            const ideClient = await IdeClient.getInstance();
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(config);
            expect(fs.promises.readFile).not.toHaveBeenCalledWith(path.join(ideDir, `${port}.lock`), 'utf8');
            expect(fs.promises.readFile).not.toHaveBeenCalledWith(path.join('/tmp', `hopcode-ide-server-${port}.json`), 'utf8');
        ...
        it('should fall back to scanned locks when the legacy config belongs to another workspace', async () => {
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['5678.lock']);
        ...
        it('should fall back to legacy port file when pid file is missing', async () => {
            process.env['HOPCODE_IDE_SERVER_PORT'] = '2222';
            const config2 = { port: '2222', workspacePath: '/test/workspace2' };
            vi.mocked(fs.promises.readFile)
                .mockRejectedValueOnce(new Error('not found')) // ~/.hopcode/ide/<port>.lock
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'hopcode-ide-server-12345.json'), 'utf8');
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'hopcode-ide-server-2222.json'), 'utf8');
            delete process.env['HOPCODE_IDE_SERVER_PORT'];
        });
        it('should fall back to legacy config when env lock file has invalid JSON', async () => {
            process.env['HOPCODE_IDE_SERVER_PORT'] = '3333';
    ...
    describe('getAllConnectionConfigs ENOENT guard', () => {
        it('returns empty array silently when readdir rejects with ENOENT', async () => {
            ...
            mockDebugLogger.debug.mockClear();
            const ideClient = await IdeClient.getInstance();
            const result = await ideClient.getAllConnectionConfigs('/some/test/dir');
            expect(result).toEqual([]);
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith('Failed to read IDE connection directory:', expect.any(Error));
        });
        it('returns empty array and logs debug when readdir rejects with other error', async () => {
            ...
            eperm.code = 'EPERM';
            vi.mocked(fs.promises.readdir).mockRejectedValue(eperm);
            mockDebugLogger.debug.mockClear();
            const ideClient = await IdeClient.getInstance();
            const result = await ideClient.getAllConnectionConfigs('/some/test/dir');
            expect(result).toEqual([]);
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Failed to read IDE connection directory:', expect.any(Error));
        });
    });
});
