...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    if (this.workspaceRejectedPorts.size > 0) {
      this.setState(
        IDEConnectionStatus.Disconnected,
        `Found IDE companion extension, but its workspace does not match the current directory. Run HopCode from the workspace open in your IDE, or switch the IDE to this project.`,
        true,
      );
      return;
    }
    ...
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          debugLogger.error(
            'HOPCODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
