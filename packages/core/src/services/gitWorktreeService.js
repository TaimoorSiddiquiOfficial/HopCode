 ...
 * `.hopcode/worktrees/.gitignore`) so it cannot leak into commits.
 */
export const WORKTREE_SESSION_FILE = '.hopcode-session';
...
export class GitWorktreeService {
     ...
     * from — otherwise running `hopcode` from a monorepo subdirectory would
     * scatter `.hopcode/worktrees/` under each subdirectory instead of
    ...
    // `<projectRoot>/.hopcode/worktrees/<slug>` rather than under the
    // session-scoped Arena baseDir.
    // ──────────────────────────────────────────────────────────────────────
    /**
     * Returns the directory holding all general-purpose worktrees for this
     * repo: `<projectRoot>/.hopcode/worktrees`.
     */
    getUserWorktreesDir() {
        return path.join(this.sourceRepoPath, '.hopcode', WORKTREES_DIR);
    }
    /**
    ...
    async isRegisteredLinkedWorktree(worktreePath) {
        ...
        try {
            ...
            const ourCommonDir = path.resolve(this.sourceRepoPath, (await this.git.raw(['rev-parse', '--git-common-dir'])).trim());
    ...
    /**
     * Creates a general-purpose worktree at `<projectRoot>/.hopcode/worktrees/<slug>`
    ...
    async createUserWorktree(slug, baseBranch, options) {
        ...
        try {
            ...
            // touches an existing user-managed `.hopcode/.gitignore`.
    ...
    async symlinkConfiguredDirectories(worktreePath, configured) {
        ...
        for (const raw of configured) {
            ...
            if (isWithinRoot(realSource, hopcodeDirAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — real source ${realSource} resolves inside .hopcode`);
                continue;
            }
    ...
    /**
     * Ensures `<projectRoot>/.hopcode/.gitignore` ignores the worktrees
    ...
    async ensureWorktreesGitignored() {
        try {
            ...
            try {
                await fs.writeFile(gitignorePath, `# Auto-generated by hopcode.\n${WORKTREES_DIR}/\n`, { encoding: 'utf8', flag: 'wx' });
            }
            catch (error) {
                if (isNodeError(error) && error.code === 'EEXIST') {
