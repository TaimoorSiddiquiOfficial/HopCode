...
import { SYSTEM_REMINDER_CLOSE, SYSTEM_REMINDER_OPEN, } from '../utils/environmentContext.js';
import { generateSessionRecap } from './sessionRecap.js';
const reminder = (body) => `${SYSTEM_REMINDER_OPEN}\n${body}\n${SYSTEM_REMINDER_CLOSE}`;
describe('generateSessionRecap', () => {
    it('strips startup and mid-session system reminders from recap input', async () => {
        ...
        let captured = null;
        const generateText = vi.fn(async (opts) => {
        ...
        const result = await generateSessionRecap(config, new AbortController().signal);
...
//# sourceMappingURL=sessionRecap.test.js.map