...
import { deserializeSnapshots, serializeSnapshot, } from './fileHistoryService.js';
vi.mock('node:path');
...
describe('ChatRecordingService', () => {
    ...
    describe('rewindRecording', () => {
        it('preserves a resumed user turn parent when rebuilding rewind boundaries', async () => {
            ...
            await chatRecordingService.flush();
            expect(jsonl.writeLine).toHaveBeenCalledTimes(1);
            const rewind = vi.mocked(jsonl.writeLine).mock.calls[0][1];
    ...
    describe('recordFileHistorySnapshot', () => {
        ...
        it('appends single-snapshot updates in order so resume can last-win', async () => {
            ...
            await chatRecordingService.flush();
            expect(jsonl.writeLine).toHaveBeenCalledTimes(2);
            const first = vi.mocked(jsonl.writeLine).mock.calls[0][1];
        ...
        it('re-records surviving snapshots after rewind on the active branch', async () => {
            ...
            const staleSnapshot = vi.mocked(jsonl.writeLine).mock
                .calls[0][1];
            const rewind = vi.mocked(jsonl.writeLine).mock.calls[1][1];
    ...
    describe('recordToolResult', () => {
        ...
        it('compacts large resultDisplay metadata before recording', async () => {
            ...
            const metadata = {
                ...
                status: 'success',
                responseParts: toolResultParts,
                resultDisplay: `head-${'x'.repeat(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS)}-tail`,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
            };
            chatRecordingService.recordToolResult(toolResultParts, metadata);
            await chatRecordingService.flush();
    ...
    describe('flush', () => {
        ...
        it('notifies once with the failed record session id', async () => {
            let rejectWrite;
            vi.mocked(jsonl.writeLine).mockReturnValueOnce(new Promise((_resolve, reject) => {
                rejectWrite = reject;
            }));
        ...
        it('isolates synchronous and asynchronous listener failures', async () => {
            ...
            try {
                ...
                vi.mocked(jsonl.writeLine).mockRejectedValueOnce(new Error('sync observer write failure'));
                syncFailure.recordUserMessage([{ text: 'first' }]);
                await expect(syncFailure.flush()).rejects.toThrow('sync observer write failure');
                const asyncFailure = new ChatRecordingService(mockConfig, async () => {
                    throw new Error('listener rejected');
                });
                vi.mocked(jsonl.writeLine).mockRejectedValueOnce(new Error('async observer write failure'));
                asyncFailure.recordUserMessage([{ text: 'second' }]);
                await expect(asyncFailure.flush()).rejects.toThrow('async observer write failure');
    ...
    describe('recordAttributionSnapshot', () => {
        ...
        it('stops queued normal writes when a strict artifact write fails', async () => {
            let rejectStrict;
            const strictWrite = new Promise((_resolve, reject) => {
                rejectStrict = reject;
            });
            vi.mocked(jsonl.writeLine)
            ...
            await expect(chatRecordingService.recordSessionArtifactSnapshot({
                v: 2,
                sessionId: 'test-session-id',
                sequence: 2,
        ...
        it('rejects strict artifact records after a previous strict write failed', async () => {
            ...
            vi.mocked(jsonl.writeLine)
                ...
                .mockResolvedValue(undefined);
            await expect(chatRecordingService.recordSessionArtifactEvent({
                v: 2,
                sessionId: 'test-session-id',
                sequence: 1,
                recordedAt: '2026-07-04T00:00:00.000Z',
        ...
        it('does not let anchor size estimation preempt a strict writer result', async () => {
            ...
            await expect(chatRecordingService.recordSessionArtifactEvent(payload)).resolves.toBeUndefined();
        ...
        it('keeps artifact journal records out of the active conversation chain', async () => {
            ...
            await chatRecordingService.flush();
            const before = vi.mocked(jsonl.writeLine).mock.calls[0][1];
            const artifact = vi.mocked(jsonl.writeLine).mock
                .calls[1][1];
            const after = vi.mocked(jsonl.writeLine).mock.calls[2][1];
