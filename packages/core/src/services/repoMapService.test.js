...
describe('RepoMapService', () => {
    ...
    describe('caching', () => {
        ...
        it('discovers imports and ranks files', async () => {
            ...
            await fs.writeFile(path.join(tmpDir, 'a.ts'), 'export const foo = 1;\n', 'utf8');
            await fs.writeFile(path.join(tmpDir, 'b.ts'), "import { foo } from './a';\nexport const bar = 2;\n", 'utf8');
            const service = new RepoMapService(tmpDir);
            const result = await service.getRelevantFiles();
        ...
        it('handles bare specifier imports (npm packages)', async () => {
            await fs.writeFile(path.join(tmpDir, 'c.ts'), "import glob from 'glob';\nexport const x = 1;\n", 'utf8');
            const service = new RepoMapService(tmpDir);
            const result = await service.getRelevantFiles();
    ...
    describe('seed-based personalization', () => {
        ...
        it('returns seed file info in result', async () => {
            await fs.writeFile(path.join(tmpDir, 'seed.ts'), "import { dep } from './dep';\nexport const seed = 1;\n", 'utf8');
            await fs.writeFile(path.join(tmpDir, 'dep.ts'), 'export const dep = 1;\n', 'utf8');
            const service = new RepoMapService(tmpDir);
...
//# sourceMappingURL=repoMapService.test.js.map