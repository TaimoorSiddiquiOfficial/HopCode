...
describe('MemoryDiagnosticsDumper', () => {
    ...
    it('writes diagnostics JSON on first dump', async () => {
        ...
        expect(result.filePath).toContain(path.join('/tmp/test-project', 'diagnostics') + path.sep);
        expect(result.filePath).toContain('memory-test-ses');
        expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('diagnostics'), { recursive: true });
        // Two-phase write: Phase 1 (minimal) + Phase 2 (full)
        expect(fs.writeFileSync).toHaveBeenCalledTimes(2);
        const phase1Content = JSON.parse(vi.mocked(fs.writeFileSync).mock.calls[0][1]);
        ...
        const phase2Content = JSON.parse(vi.mocked(fs.writeFileSync).mock.calls[1][1]);
    ...
    it('includes critical suggestion for critical pressure', async () => {
        ...
        const writtenContent = JSON.parse(vi.mocked(fs.writeFileSync).mock.calls[1][1]);
    ...
    it('includes recentSamples in both Phase 1 and Phase 2 payloads', async () => {
        ...
        await dumper.dump('hard', fakeSamples);
        const phase1 = JSON.parse(vi.mocked(fs.writeFileSync).mock.calls[0][1]);
        expect(phase1.recentSamples).toEqual(fakeSamples);
        expect(phase1.collectionComplete).toBe(false);
        const phase2 = JSON.parse(vi.mocked(fs.writeFileSync).mock.calls[1][1]);
        expect(phase2.recentSamples).toEqual(fakeSamples);
        expect(phase2.collectionComplete).toBe(true);
    });
});
//# sourceMappingURL=memoryDiagnosticsDumper.test.js.map