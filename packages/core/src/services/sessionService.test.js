...
import { SessionService, buildApiHistoryFromConversation, getResumePromptTokenCount, getResumeTokenCounts, } from './sessionService.js';
import { SESSION_TRANSCRIPT_MAX_INDEX_BYTES, SessionTranscriptTooLargeError, } from './session-transcript-reader.js';
import { SESSION_ARTIFACT_PERSISTENCE_VERSION, stableSessionArtifactId, } from './session-artifact-persistence.js';
...
describe('SessionService', () => {
    ...
    describe('listSessions', () => {
        ...
        it('should ignore archive directory when listing active sessions', async () => {
            ...
            expect(vi.mocked(jsonl.readLines).mock.calls[0][0]).not.toContain('/archive/');
        });
        it('should list archived sessions from archive directory only', async () => {
            readdirSyncSpy.mockImplementation((dir) => {
                if (dir.toString().endsWith('/chats/archive')) {
                    return [`${sessionIdB}.jsonl`];
                }
                return [`${sessionIdA}.jsonl`];
            });
            statSyncSpy.mockReturnValue({
                mtimeMs: Date.now(),
                isFile: () => true,
            ...
            expect(vi.mocked(jsonl.readLines).mock.calls[0][0]).toContain('/chats/archive/');
        });
        ...
        it('getSessionInfoCounts excludes sessions from other projects', async () => {
            readdirSyncSpy.mockImplementation((dir) => dir.toString().endsWith(`${path.sep}archive`)
                ? []
                : [`${sessionIdA}.jsonl`]);
            vi.mocked(jsonl.readLines).mockResolvedValue([
        ...
        it('marks counts truncated when a candidate session cannot be read', async () => {
            readdirSyncSpy.mockImplementation((dir) => dir.toString().endsWith(`${path.sep}archive`)
                ? []
                : [`${sessionIdA}.jsonl`, `${sessionIdB}.jsonl`]);
            vi.mocked(jsonl.readLines).mockImplementation(async (filePath) => {
                if (filePath.includes(sessionIdA))
                    return [recordA1];
    ...
    describe('loadSession', () => {
        ...
        it('reads archived sessions only through the explicit read-only method', async () => {
            ...
            expect(loaded?.conversation.messages).toHaveLength(2);
            expect(vi.mocked(jsonl.read)).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdB}.jsonl`));
        ...
        it('accepts an archived session exactly at the requested size limit', async () => {
            ...
            vi.mocked(jsonl.read).mockResolvedValue([recordB1, recordB2]);
            await expect(sessionService.loadArchivedSession(sessionIdB, {
                maxBytes: SESSION_TRANSCRIPT_MAX_INDEX_BYTES,
        ...
        it('rejects an archived session above the requested size limit', async () => {
            ...
            await expect(load).rejects.toEqual(new SessionTranscriptTooLargeError(sessionIdB, snapshotSize, SESSION_TRANSCRIPT_MAX_INDEX_BYTES));
            expect(vi.mocked(jsonl.read)).not.toHaveBeenCalled();
        });
        it('rejects invalid archived session ids before accessing storage', async () => {
            await expect(sessionService.loadArchivedSession('../outside', {
                maxBytes: SESSION_TRANSCRIPT_MAX_INDEX_BYTES,
        ...
        it('loads artifact side records attached to the active branch', async () => {
            ...
            const artifactId = stableSessionArtifactId(sessionIdB, 'url:https://example.com/report');
            ...
            const loaded = await sessionService.loadSession(sessionIdB);
            expect(loaded?.conversation.messages.map((record) => record.uuid)).toEqual(['b1', 'b2']);
        ...
        it('loads artifact side records after a tail-neutral title reanchor', async () => {
            ...
            const artifactId = stableSessionArtifactId(sessionIdB, 'url:https://example.com/reanchored-report');
            const titleRecord = {
                ...recordB1,
                uuid: 'title-reanchor',
                parentUuid: 'b1',
        ...
        it('loads chained artifact side records attached to the active branch', async () => {
            ...
            const artifactId = stableSessionArtifactId(sessionIdB, 'url:https://example.com/chained-report');
            const createRecord = {
                ...recordB1,
        ...
        it('does not load artifact side records from abandoned branches', async () => {
            ...
            const artifactId = stableSessionArtifactId(sessionIdB, 'url:https://example.com/abandoned-report');
            const artifactRecord = {
                ...recordB1,
        ...
        it('does not treat trailing artifact side records as the conversation leaf', async () => {
            ...
            const artifactId = stableSessionArtifactId(sessionIdB, 'url:https://example.com/trailing-report');
            const artifactRecord = {
                ...recordB2,
                uuid: 'artifact-tail',
                parentUuid: 'b2',
    ...
    describe('removeSession', () => {
        it('should remove session file', async () => {
            ...
            expect(rmSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`file-history/${sessionIdA}`), { recursive: true, force: true });
        });
        ...
        it('should remove archived session files and both worktree sidecars', async () => {
            ...
            existsSyncSpy.mockImplementation((filePath) => filePath.toString().endsWith(`${sessionIdA}.worktree.json`));
            const result = await sessionService.removeSession(sessionIdA);
            expect(result).toBe(true);
            expect(unlinkSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`));
            expect(unlinkSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.worktree.json`));
            expect(unlinkSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.worktree.json`));
        });
        it('should remove both JSONL files when active and archived copies conflict', async () => {
            vi.mocked(jsonl.readLines).mockResolvedValue([recordA1]);
            existsSyncSpy.mockImplementation((filePath) => filePath.toString().endsWith(`/chats/archive/${sessionIdA}.jsonl`));
            const result = await sessionService.removeSession(sessionIdA);
            expect(result).toBe(true);
            expect(unlinkSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.jsonl`));
            expect(unlinkSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`));
        });
    });
    describe('archiveSessions', () => {
        ...
        it('should move active sessions into the archive directory', async () => {
            ...
            expect(mkdirSyncSpy).toHaveBeenCalledWith(expect.stringContaining('/chats/archive'), { recursive: true });
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`));
        });
        it('should archive JSONL and warn when archiving worktree sidecar fails', async () => {
            ...
            expect(warnings[0]).toContain(`archiveSessions: failed to move worktree sidecar for ${sessionIdA}`);
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`));
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.worktree.json`), expect.stringContaining(`/chats/archive/${sessionIdA}.worktree.json`));
        });
        it('should not move worktree sidecar when archiving JSONL fails', async () => {
            mockActiveSessionOnly();
            mockActiveWorktreeSidecarOnly();
            const jsonlError = new Error(`EACCES: permission denied, rename '/tmp/runtime/chats/${sessionIdA}.jsonl' -> '/tmp/runtime/chats/archive/${sessionIdA}.jsonl'`);
            ...
            expect(result.errors[0]?.error.message).toBe('Failed to archive session file: EACCES');
            expect(result.errors[0]?.error.message).not.toContain('/tmp/runtime');
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`));
            expect(renameSyncSpy).not.toHaveBeenCalledWith(expect.stringContaining(`/chats/${sessionIdA}.worktree.json`), expect.stringContaining(`/chats/archive/${sessionIdA}.worktree.json`));
        });
    ...
    describe('unarchiveSessions', () => {
        ...
        it('should move archived sessions back to the active directory', async () => {
            ...
            expect(result.notFound).toEqual([]);
            expect(result.errors).toEqual([]);
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/${sessionIdA}.jsonl`));
        });
        ...
        it('should recreate active chats directory before moving archived sessions', async () => {
            ...
            const result = await sessionService.unarchiveSessions([sessionIdA]);
            expect(result.unarchived).toEqual([sessionIdA]);
            expect(mkdirSyncSpy).toHaveBeenCalledWith(expect.stringMatching(/\/chats$/), { recursive: true });
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/${sessionIdA}.jsonl`));
        });
        ...
        it('should unarchive JSONL and warn when worktree sidecar move fails', async () => {
            ...
            expect(result.errors).toEqual([]);
            expect(warnings).toHaveLength(1);
            expect(warnings[0]).toContain(`unarchiveSessions: failed to move worktree sidecar for ${sessionIdA}`);
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/${sessionIdA}.jsonl`));
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.worktree.json`), expect.stringContaining(`/chats/${sessionIdA}.worktree.json`));
        });
        it('should not move worktree sidecar when unarchiving JSONL fails', async () => {
            mockArchivedSessionOnly();
            mockArchivedWorktreeSidecarOnly();
            const jsonlError = new Error(`ENOSPC: no space left on device, rename '/tmp/runtime/chats/archive/${sessionIdA}.jsonl' -> '/tmp/runtime/chats/${sessionIdA}.jsonl'`);
            ...
            expect(result.unarchived).toEqual([]);
            expect(result.errors[0]?.sessionId).toBe(sessionIdA);
            expect(result.errors[0]?.error.message).toBe('Failed to unarchive session file: ENOSPC');
            expect(result.errors[0]?.error.message).not.toContain('/tmp/runtime');
            expect(renameSyncSpy).toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.jsonl`), expect.stringContaining(`/chats/${sessionIdA}.jsonl`));
            expect(renameSyncSpy).not.toHaveBeenCalledWith(expect.stringContaining(`/chats/archive/${sessionIdA}.worktree.json`), expect.stringContaining(`/chats/${sessionIdA}.worktree.json`));
        });
    ...
    describe('getSessionLocation', () => {
        it('should report conflict when active and archived files both exist', async () => {
            vi.mocked(jsonl.readLines).mockResolvedValue([recordA1]);
            await expect(sessionService.getSessionLocation(sessionIdA)).resolves.toBe('conflict');
        });
        it('should warn when reading a session head fails', async () => {
            ...
            vi.mocked(jsonl.readLines).mockRejectedValue(error);
            await expect(service.getSessionLocation(sessionIdA)).rejects.toThrow(error);
    ...
    describe('sessionExists', () => {
        ...
        it('should keep default existence checks active-only', async () => {
            ...
            await expect(sessionService.sessionExists(sessionIdA)).resolves.toBe(false);
            await expect(sessionService.sessionExistsInAnyState(sessionIdA)).resolves.toBe(true);
        });
    ...
    describe('getResumePromptTokenCount', () => {
        ...
        it('should return latest assistant usage without scanning further back', () => {
            ...
            expect(getResumeTokenCounts(makeConversation([compressionRecord, assistant]))).toEqual({ promptTokenCount: 450, outputTokenCount: 0 });
        });
        it('should prefer promptTokenCount over totalTokenCount when both are present', () => {
            ...
            expect(getResumeTokenCounts(makeConversation([compressionRecord, assistant]))).toEqual({ promptTokenCount: 200, outputTokenCount: 250 });
        });
        it('should restore disjoint candidate and thought output tokens when total is unavailable', () => {
            ...
            expect(getResumeTokenCounts(makeConversation([compressionRecord, assistant]))).toEqual({ promptTokenCount: 200, outputTokenCount: 100 });
        });
        it('should fall back to compression when latest assistant has zero usage', () => {
            ...
            expect(getResumeTokenCounts(makeConversation([compressionRecord, assistant]))).toEqual({ promptTokenCount: 300, outputTokenCount: 0 });
        });
    });
    ...
    describe('forkSession', () => {
        ...
        beforeEach(async () => {
            ...
            // `~/.hopcode` and makes the fork write outside the tmp sandbox.
        ...
        const seedSession = (sessionId, sessionCwd = cwd) => {
        ...
        it('copies artifact side records from the active branch', async () => {
            ...
            const oldArtifactId = stableSessionArtifactId(oldId, 'url:https://example.com/forked');
            const artifactRecord = {
                uuid: 'artifact-1',
                parentUuid: 'u1',
                sessionId: oldId,
            ...
            fs.writeFileSync(file, [lines[0], artifactRecord, lines[1]]
            ...
            expect(loaded?.conversation.messages.map((record) => record.uuid)).toEqual(['u1', 'u2']);
            expect(forkedLines.find((record) => record.uuid === 'artifact-1')).toMatchObject({
                parentUuid: 'u1',
            });
        ...
        it('does not copy artifact side records from abandoned branches', async () => {
            ...
            const { file, lines } = seedSession(oldId);
            const oldArtifactId = stableSessionArtifactId(oldId, 'url:https://example.com/abandoned-forked');
            const artifactRecord = {
                uuid: 'artifact-abandoned',
                parentUuid: 'u1',
                sessionId: oldId,
            ...
            fs.writeFileSync(file, [lines[0], artifactRecord, abandonedChild, lines[1]]
                .map((line) => JSON.stringify(line))
                .join('\n') + '\n');
            const result = await service.forkSession(oldId, newId);
            const loaded = await service.loadSession(newId);
            const forkedLines = fs
                ...
                .map((line) => JSON.parse(line));
            expect(result.copiedCount).toBe(2);
            expect(forkedLines.some((record) => record.uuid === 'artifact-abandoned')).toBe(false);
            expect(loaded?.conversation.messages.map((record) => record.uuid)).toEqual(['u1', 'u2']);
            expect(loaded?.artifactSnapshot).toBeUndefined();
        });
        it('does not treat trailing artifact side records as the fork leaf', async () => {
            ...
            fs.writeFileSync(file, [...lines, artifactRecord]
                .map((line) => JSON.stringify(line))
                .join('\n') + '\n');
            const result = await service.forkSession(oldId, newId);
            const loaded = await service.loadSession(newId);
        ...
        it('does not resurrect artifacts removed by later side records when forking', async () => {
            ...
            fs.writeFileSync(file, [lines[0], createRecord, removeRecord, lines[1]]
                .map((line) => JSON.stringify(line))
                .join('\n') + '\n');
            const result = await service.forkSession(oldId, newId);
            const loaded = await service.loadSession(newId);
            const forkedLines = fs
                ...
                .split('\n')
                .map((line) => JSON.parse(line));
            const forkedRemovePayload = forkedLines.find((record) => record.uuid === 'artifact-remove')?.systemPayload;
            expect(result.copiedCount).toBe(4);
            expect(loaded?.artifactSnapshot?.artifacts).toEqual([]);
            expect(loaded?.artifactSnapshot?.tombstonedIds).toContain(forkedArtifactId);
        ...
        it('preserves file history snapshots on the forked session', async () => {
            ...
            fs.writeFileSync(file, [...lines, snapshotRecord].map((l) => JSON.stringify(l)).join('\n') +
                '\n');
            await service.forkSession(oldId, newId);
            const loaded = await service.loadSession(newId);
            expect(loaded?.fileHistorySnapshots).toHaveLength(1);
            expect(loaded?.fileHistorySnapshots?.[0]?.promptId).toBe(`${newId}########0`);
        });
        it('removes copied file-history backups when deleting a fork', async () => {
            ...
            await service.forkSession(oldId, newId);
            expect(fs.existsSync(realPath.join(targetBackupDir, 'backup-a'))).toBe(true);
        ...
        it('forks only the active branch after rewind', async () => {
            ...
            const chatsDir = realPath.join(service['storage'].getProjectDir(), 'chats');
            fs.mkdirSync(chatsDir, { recursive: true });
            fs.writeFileSync(realPath.join(chatsDir, `${oldId}.jsonl`), [
                {
                    uuid: 'u1',
                    parentUuid: null,
                    sessionId: oldId,
            ...
            const loaded = await service.loadSession(newId);
            expect(result.copiedCount).toBe(3);
            expect(loaded?.conversation.messages.flatMap((message) => message.message?.parts?.map((part) => part.text) ?? [])).toEqual(['first', 'first reply']);
        });
        ...
        it('removes a partially written target when fork creation fails', async () => {
            ...
            seedSession(oldId);
            const targetPath = realPath.join(service['storage'].getProjectDir(), 'chats', `${newId}.jsonl`);
            ...
            await expect(service.forkSession(oldId, newId)).rejects.toThrow('disk full');
        ...
        it('drops creation metadata so the fork inherits no lineage or source', async () => {
            ...
            fs.writeFileSync(srcFile, lines.map((l) => JSON.stringify(l)).join('\n') + '\n');
            const result = await service.forkSession(oldId, newId);
            const written = fs
                .readFileSync(result.filePath, 'utf8')
                .trim()
                .split('\n')
                .map((l) => JSON.parse(l));
            expect(written.some((r) => r.type === 'system' && r.subtype === 'parent_session')).toBe(false);
            expect(written.some((r) => r.type === 'system' && r.subtype === 'session_source')).toBe(false);
    ...
    describe('listSessions parentSessionId round-trip', () => {
        ...
        beforeEach(async () => {
            ...
            vi.mocked(statSyncSpy).mockRestore?.();
            vi.mocked(unlinkSyncSpy).mockRestore?.();
            realTmpDir = fs.mkdtempSync(realPath.join(realOs.tmpdir(), 'parent-session-id-'));
            process.env['QWEN_RUNTIME_DIR'] = realTmpDir;
            cwd = process.cwd();
            service = new SessionService(cwd);
        });
        ...
        const parentSessionLine = (sessionId, parentSessionId) => ({
            uuid: 'u2',
            parentUuid: 'u1',
            sessionId,
            type: 'system',
        ...
        const sessionSourceLine = (sessionId) => ({
            uuid: 'u3',
            parentUuid: 'u2',
            sessionId,
            type: 'system',
        ...
        const writeSession = (sessionId, lines) => {
            const file = realPath.join(getChatsDir(), `${sessionId}.jsonl`);
            fs.writeFileSync(file, lines.map((l) => JSON.stringify(l)).join('\n') + '\n');
            return file;
        };
        const findItem = (items, sessionId) => items.find((item) => item.sessionId === sessionId);
