...
describe('GitWorktreeService.isRegisteredLinkedWorktree() (real git)', () => {
    ...
    it('returns false for a main tree whose .git is a FILE (separate-git-dir)', async () => {
        ...
        const base = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-linked-sep-')));
        tmpDirs.push(base);
    ...
    it('returns false (fail-closed) for a path that is not a git repository', async () => {
        ...
        const plain = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-linked-plain-')));
        tmpDirs.push(plain);
    ...
    it('matches a registered worktree through a symlinked input path', async () => {
        ...
        const linkParent = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-linked-symlink-lnk-')));
    ...
    it('returns false for a fake worktree carrying a copied .git file (not registered)', async () => {
        ...
        execFileSync('git', ['worktree', 'add', '-b', 'review-pr-1', realWt, 'HEAD'], { cwd: repo });
    ...
    it.skipIf(process.platform === 'win32')('is not fooled by a worktree path containing a newline that fakes a registry entry', async () => {
...
//# sourceMappingURL=gitWorktreeService.linked.integ.test.js.map