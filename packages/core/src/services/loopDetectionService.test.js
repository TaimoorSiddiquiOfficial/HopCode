...
import { DEFAULT_MAX_TOOL_CALLS_PER_TURN, LoopDetectionService, } from './loopDetectionService.js';
...
describe('LoopDetectionService', () => {
    ...
    const makeConfig = (cap = DEFAULT_MAX_TOOL_CALLS_PER_TURN, explicit = false) => ({
    ...
    describe('Tool Call Loop Detection', () => {
        ...
        it('resets the consecutive tool-call counter on retry', () => {
            ...
            expect(service.checkAlwaysOnSafeties({
        ...
        it('should expose the current consecutive tool-call count', () => {
            ...
            expect(service.getConsecutiveToolCallCount()).toBe(TOOL_CALL_LOOP_THRESHOLD - 1);
            expect(service.checkAlwaysOnSafeties(event)).toBe(true);
            expect(service.getConsecutiveToolCallCount()).toBe(TOOL_CALL_LOOP_THRESHOLD);
        });
        it('halts consecutive identical calls via the always-on guard', () => {
            ...
            expect(service.checkAlwaysOnSafeties(event)).toBe(true);
            expect(service.getLastLoopType()).toBe(LoopType.CONSECUTIVE_IDENTICAL_TOOL_CALLS);
        ...
        it('treats reordered argument fields as identical for the consecutive guard', () => {
            ...
            for (let i = 0; i < TOOL_CALL_LOOP_THRESHOLD; i++) {
                ...
                fired = service.checkAlwaysOnSafeties(createToolCallRequestEvent('stuck_tool', args));
                if (fired)
                    break;
            }
    ...
    describe('Shell Command Stagnation (Always-On Circuit Breaker)', () => {
        it('halts repeated git inspection command variants via the always-on guard', () => {
            ...
            for (const command of commands.slice(0, -1)) {
                expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('run_shell_command', {
                    command,
        ...
        it('resets the streak when a non-inspection tool call interrupts the run', () => {
            ...
            const gitInspect = (i) => service.checkAlwaysOnSafeties(createToolCallRequestEvent('run_shell_command', {
            ...
            expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('read_file', {
            ...
            expect(service.getLastLoopType()).not.toBe(LoopType.SHELL_COMMAND_STAGNATION);
        });
        ...
        it('honors an in-session disable for shell inspection stagnation', () => {
            ...
            expect(loggers.logLoopDetected).not.toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                loop_type: 'shell_command_stagnation',
            }));
        });
    ...
    describe('Turn Tool Call Cap', () => {
        ...
        it('does not fire at or below the soft cap', () => {
            ...
            for (let i = 0; i < SOFT_CAP; i++) {
                expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('any_tool', { i }))).toBe(false);
            }
        });
        ...
        it('fires when a stuck signal accumulates between the soft and hard cap', () => {
            ...
            for (let i = 0; i < GLOBAL_DUPLICATE_THRESHOLD * 2 && !fired; i++) {
                ...
                fired = service.checkAlwaysOnSafeties(createToolCallRequestEvent('any_tool', args));
            }
            expect(fired).toBe(true);
        ...
        it('fires on a stuck signal accumulated across Finished round-trips', () => {
            ...
            const step = (args) => {
                if (!fired)
                    fired = service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', args));
            };
        ...
        it('fires at the hard cap regardless of diversity', () => {
            ...
            for (let i = 0; i < HARD_CAP; i++) {
                expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { i }))).toBe(false);
            }
            expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { last: true }))).toBe(true);
            expect(service.getLastLoopType()).toBe(LoopType.TURN_TOOL_CALL_CAP);
        });
        it('fires at the soft cap when a stuck-repetition signal is present', () => {
            ...
            for (let i = 0; i < SOFT_CAP + GLOBAL_DUPLICATE_THRESHOLD && !fired; i++) {
                const isRepeat = i % 2 === 0;
            ...
            expect(fired).toBe(true);
            expect(loggers.logLoopDetected).toHaveBeenCalledTimes(1);
            expect(loggers.logLoopDetected).toHaveBeenCalledWith(capConfig, expect.objectContaining({ loop_type: 'turn_tool_call_cap' }));
            expect(service.getLastLoopType()).toBe(LoopType.TURN_TOOL_CALL_CAP);
        });
        it('allows diverse calls past the built-in default soft cap', () => {
            ...
            for (let i = 0; i < DEFAULT_MAX_TOOL_CALLS_PER_TURN + 1; i++) {
                expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { i }))).toBe(false);
            }
        });
        it('never fires when the cap is disabled (Config resolves <= 0 to Infinity)', () => {
            const svc = new LoopDetectionService(makeConfig(Number.POSITIVE_INFINITY));
            svc.reset('');
        ...
        it('rolls back a failed attempt on retry so its calls do not count', () => {
            ...
            for (let i = 0; i < HARD_CAP; i++) {
                expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { j: i }))).toBe(false);
            }
            expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { last: true }))).toBe(true);
            expect(loggers.logLoopDetected).toHaveBeenCalledTimes(1);
        });
        it('rolls back the stuck-repetition signal on retry', () => {
            ...
            for (let i = 0; i < 6; i++) {
                svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { stuck: true }));
        ...
        it('preserves committed round-trip counts when a later attempt retries', () => {
            ...
            for (let i = 0; i < 4; i++) {
                service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { k: i }));
            }
            service.checkAlwaysOnSafeties(retryEvent);
            ...
            for (let i = 0; i < HARD_CAP - 6; i++) {
                expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { m: i }))).toBe(false);
            }
            expect(service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { last: true }))).toBe(true);
        });
        it('still accumulates across committed round-trips to trip the cap', () => {
            ...
            for (let rt = 0; rt < 12 && !fired; rt++) {
                for (let i = 0; i < 15 && !fired; i++) {
                    fired = service.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { rt, i }));
                }
                if (!fired) {
                    service.checkAlwaysOnSafeties(finishedEvent);
                }
        ...
        it('treats an explicit value as a hard cap: cap of 2 halts call 3', () => {
            ...
            svc.reset('');
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 1 }))).toBe(false);
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 2 }))).toBe(false);
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 3 }))).toBe(true);
        ...
        it('the same value left at the default is adaptive, not a hard cap', () => {
            ...
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 1 }))).toBe(false);
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 2 }))).toBe(false);
            expect(svc.checkAlwaysOnSafeties(createToolCallRequestEvent('t', { a: 3 }))).toBe(false);
        });
    });
    describe('Global Tool Call Duplicate Detection', () => {
        ...
        it('should fire when same (tool, args) appears threshold times non-consecutively', () => {
            ...
            for (let i = 0; i < GLOBAL_DUPLICATE_THRESHOLD - 1; i++) {
                ...
                expect(service.addAndCheckHeuristicLoops(otherEvents[otherIdx % otherEvents.length])).toBe(false);
                otherIdx++;
            }
            ...
            expect(service.getLastLoopType()).toBe(LoopType.GLOBAL_TOOL_CALL_DUPLICATE);
        });
        it('should not fire for different (tool, args) pairs', () => {
            service.reset('');
            for (let i = 0; i < GLOBAL_DUPLICATE_THRESHOLD; i++) {
                const isLoop = service.addAndCheckHeuristicLoops(createToolCallRequestEvent('stuck_tool', { param: i }));
                expect(isLoop).toBe(false);
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
        });
        ...
        it('does not count a retried replay toward the global-duplicate threshold', () => {
            ...
            const retry = { type: GeminiEventType.Retry };
    ...
    describe('Alternating Tool Call Pattern Detection', () => {
        it('should fire for a clean ABABAB alternating pattern', () => {
            ...
            expect(service.getLastLoopType()).toBe(LoopType.ALTERNATING_TOOL_CALL_PATTERN);
        });
        it('should not fire when calls alternate but with varying keys', () => {
            ...
            for (let i = 0; i < totalCycles; i++) {
                expect(service.addAndCheckHeuristicLoops(createToolCallRequestEvent('tool_a', { param: i }))).toBe(false);
                expect(service.addAndCheckHeuristicLoops(createToolCallRequestEvent('tool_b', { param: i }))).toBe(false);
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
        });
