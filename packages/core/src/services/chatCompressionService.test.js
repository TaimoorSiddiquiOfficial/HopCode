...
import { ChatCompressionService, COMPACT_MAX_OUTPUT_TOKENS, computeThresholds, MAX_CONSECUTIVE_FAILURES, MAX_HOOK_INSTRUCTIONS_CHARS, } from './chatCompressionService.js';
...
describe('ChatCompressionService', () => {
    ...
    it('should return NOOP when consecutiveFailures has hit the breaker and not forced', async () => {
        ...
        // from "empty session".
        vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(120_000);
        const result = await service.compress(mockChat, {
            promptId: mockPromptId,
            force: false,
            model: mockModel,
    ...
    describe('screenshot-overflow trigger', () => {
        ...
        function historyWithToolImages(imageCount) {
        ...
        it('fires compaction when tool-image count reaches the threshold, even below the token threshold', async () => {
            ...
            expect(generateText).toHaveBeenCalledWith(expect.objectContaining({ stream: true }));
    ...
    it('manual /compress strips a trailing orphaned functionCall from the post-compact history', async () => {
        ...
        const lastIsOrphanFc = last.role === 'model' && (last.parts ?? []).some((p) => !!p.functionCall);
    ...
    it('silently ignores the deprecated chatCompression.contextPercentageThreshold = 0 (no longer disables compaction)', async () => {
        ...
        vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(110_000);
    ...
    it('should use estimated token count if usage metadata is missing', async () => {
        ...
        vi.mocked(mockChat.getHistory).mockReturnValue(history);
        vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(5_000);
        vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
            model: 'gemini-pro',
            contextWindowSize: 6_000,
        });
        const debug = vi.fn();
        mockConfig.getDebugLogger = () => ({
            warn: vi.fn(),
            debug,
        });
        const mockGenerateContent = vi.fn().mockResolvedValue({
        ...
        expect(result.newHistory).not.toBeNull();
        expect(result.newHistory[0].parts[0].text).toContain('Summary');
        expect(debug).toHaveBeenCalledWith(expect.stringContaining('usage metadata missing'));
        expect(debug).toHaveBeenCalledWith(expect.stringContaining('API-reported non-visible remainder (1000)'));
    });
    ...
    it('should reject cap-sized summaries even if usage metadata is missing', async () => {
        ...
        vi.mocked(mockChat.getHistory).mockReturnValue(history);
        vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(180_000);
        vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
            model: 'gemini-pro',
        ...
        expect(result.info.compressionStatus).toBe(CompressionStatus.COMPRESSION_FAILED_OUTPUT_TRUNCATED);
        expect(result.newHistory).toBeNull();
        expect(warn).toHaveBeenCalledWith(expect.stringContaining('local estimate'));
        expect(warn).toHaveBeenCalledWith(expect.stringContaining('COMPACT_MAX_OUTPUT_TOKENS'));
    });
...
describe('ChatCompressionService.compress cheap-gate uses estimated tokens', () => {
    ...
    it('NOOPs when neither originalTokenCount nor estimated total reaches threshold', async () => {
        const spy = vi
            ...
            .mockResolvedValue({ text: 's', usage: {} });
        const result = await new ChatCompressionService().compress(makeFakeChat(), {
            promptId: 'p',
            force: false,
            model: 'hopcode-test',
...
describe('computeThresholds', () => {
    ...
    describe('custom pct parameter', () => {
        ...
        it('clamps negative pct to 0', () => {
            expect(computeThresholds(32_000, -0.5)).toEqual(computeThresholds(32_000, 0));
        });
        it('clamps pct > 1 to 1', () => {
            expect(computeThresholds(32_000, 1.5)).toEqual(computeThresholds(32_000, 1));
        });
        it('NaN pct falls back to DEFAULT_PCT (via Number.isFinite check)', () => {
            expect(computeThresholds(32_000, NaN)).toEqual(computeThresholds(32_000));
        });
    });
});
describe('ChatCompressionService.compress — claude-code-style full-history compression', () => {
    ...
    it('sends the ENTIRE history to the summary side-query (no split)', async () => {
        ...
        const calledWith = runSideQuerySpy.mock.calls[0][1];
    ...
    it('includes a pending tool result in the summary side-query', async () => {
        ...
        const result = await new ChatCompressionService().compress(makeFakeChat(history), {
            promptId: 'p',
            force: true,
            model: 'qwen-vl',
            config: makeFakeConfig(),
        ...
        expect(calledWith.contents[2].parts[0].functionResponse?.id).toBe('tool-call-1');
        expect(result.newHistory
            ?.at(-1)
            ?.parts?.some((part) => part.functionCall?.id === 'tool-call-1')).toBe(true);
    });
    ...
    it('produces newHistory composed via composePostCompactHistory', async () => {
        ...
        const firstPart = result.newHistory[0].parts?.[0];
...
describe('ChatCompressionService.compress — single-turn computer-use regression', () => {
    ...
    it('preserves the user prompt verbatim in summary and restores 3 most recent screenshots', async () => {
        ...
        const inlineDataParts = flat.flatMap((c) => (c.parts ?? []).filter((p) => p.inlineData?.mimeType?.startsWith('image/')));
        expect(inlineDataParts.map((p) => p.inlineData.data)).toEqual(['s3', 's4', 's5']);
...
describe('ChatCompressionService.compress — customInstructions plumbing', () => {
    ...
    it('forwards customInstructions verbatim to firePreCompactEvent', async () => {
        ...
        expect(firePreCompactEvent).toHaveBeenCalledWith(PreCompactTrigger.Manual, 'focus auth', undefined);
    });
        ...
        const { mockChat, mockConfig } = setup({
            hookSystem: {
                firePreCompactEvent: vi
                    .fn()
                    .mockResolvedValue(makeHookOutput({ additionalContext: 'prefer Chinese summaries' })),
                firePostCompactEvent: vi.fn().mockResolvedValue(undefined),
            },
        });
        await new ChatCompressionService().compress(mockChat, {
    ...
    it('orders user instructions before hook additionalContext', async () => {
        ...
        const { mockChat, mockConfig } = setup({
            hookSystem: {
                firePreCompactEvent: vi
                    .fn()
                    .mockResolvedValue(makeHookOutput({ additionalContext: 'HOOK_TEXT' })),
                firePostCompactEvent: vi.fn().mockResolvedValue(undefined),
            },
        });
        await new ChatCompressionService().compress(mockChat, {
...
describe('ChatCompressionService.compress — plan-mode + subagent attachment wiring', () => {
    ...
    it('passes planModeActive=true when getApprovalMode() returns PLAN', async () => {
        ...
        const opts = composeSpy.mock.calls[0][2];
    ...
    it('fallback path still injects plan-mode reminder + subagent snapshot when composePostCompactHistory throws', async () => {
        ...
        vi.spyOn(postCompactModule, 'composePostCompactHistory').mockRejectedValue(new Error('EACCES: simulated restoration failure'));
        const { mockChat, mockConfig } = setupWithAppState({
            approvalMode: ApprovalMode.PLAN,
            backgroundTasks: [
                {
