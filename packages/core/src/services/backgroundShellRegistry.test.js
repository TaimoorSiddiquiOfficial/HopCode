 ...
 */
import { constants as fsConstants, mkdtempSync, rmSync, symlinkSync, writeFileSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { afterEach, describe, expect, it, vi } from 'vitest';
import { BackgroundShellRegistry, MAX_NOTIFICATION_OUTPUT_TAIL_BYTES, MAX_RETAINED_TERMINAL_SHELLS, } from './backgroundShellRegistry.js';
let tmpDirs = [];
afterEach(() => {
...
function makeOutputFile(content) {
...
describe('BackgroundShellRegistry', () => {
    ...
    describe('notifications', () => {
        it('emits one task-notification when a shell completes', () => {
            ...
            expect(modelText).toContain('<output-tail truncated="false">first line\nfinal result</output-tail>');
        ...
        it('truncates long commands for display, summary, and model XML', () => {
            ...
            expect(displayText).toBe(`Background shell "${displayCommand}" completed.`);
            expect(modelText).toContain(`<summary>Shell command "${displayCommand}" completed.</summary>`);
            expect(modelText).toContain(`<command truncated="true">${modelCommand}</command>`);
        ...
        it('escapes XML and strips display control characters on failure', () => {
            ...
            expect(modelText).toContain('<command>echo &quot;&lt;script&gt;&quot;</command>');
            expect(modelText).toContain('<cwd>/repo&amp;work</cwd>');
            expect(modelText).toContain('<result>bad &lt;thing&gt;[31m</result>');
            expect(modelText).toContain('<output-file>/tmp/out&amp;err.log</output-file>');
        });
        it('limits output-tail to the retained byte budget', () => {
            const reg = new BackgroundShellRegistry();
            const callback = vi.fn();
            const outputPath = makeOutputFile('prefix-' +
