...
import { getLockFilePath, releaseLock, tryAcquireLock, } from './cronTasksLock.js';
...
describe('cronTasksLock', () => {
    ...
    describe('getLockFilePath', () => {
        it('resolves to the per-project runtime dir, not the working tree', () => {
            ...
            expect(lock).toBe(path.join(Storage.getGlobalTempDir(), getProjectHash('/project'), 'scheduled_tasks.lock'));
    ...
    describe('lockId fencing', () => {
        it('does not treat a different lockId as an idempotent re-acquire', async () => {
            ...
            const content = JSON.parse(await fs.readFile(getLockFilePath(tmpDir), 'utf-8'));
    ...
    describe('tryAcquireLock', () => {
        ...
        it('fails when another live process holds the lock', async () => {
            ...
            await fs.writeFile(lockPath, JSON.stringify({ pid: process.pid, sessionId: 'other' }));
        ...
        it('takes over when lock owner PID is dead', async () => {
            ...
            await fs.writeFile(lockPath, JSON.stringify({ pid: 2147483647, sessionId: 'dead' }));
            expect(await tryAcquireLock(tmpDir, 'session-1')).toBe(true);
        ...
        it('does not displace a fresh lock created after the stale inspection', async () => {
            ...
            renameHook.current = async (src) => {
                ...
                await fs.writeFile(lockPath, JSON.stringify({ pid: process.pid, sessionId: 'winner' }));
            };
    ...
    describe('releaseLock', () => {
        ...
        it('does not delete lock owned by another session', async () => {
            const lockPath = getLockFilePath(tmpDir);
            await fs.mkdir(path.dirname(lockPath), { recursive: true });
            await fs.writeFile(lockPath, JSON.stringify({ pid: 1, sessionId: 'other' }));
            await releaseLock(tmpDir, 'session-1');
...
//# sourceMappingURL=cronTasksLock.test.js.map