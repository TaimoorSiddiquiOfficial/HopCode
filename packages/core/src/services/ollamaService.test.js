...
import { OllamaService, detectOllamaEndpoint, detectOllamaApiKey, isCloudEndpoint, DEFAULT_OLLAMA_ENDPOINT, DEFAULT_OLLAMA_CONFIG, } from './ollamaService.js';
// ── Mock fetch ────────────────────────────────────────────────────────────────
...
function mockTagsResponse(models) {
...
function mockEmbeddingResponse(embedding) {
...
describe('OllamaService', () => {
    ...
    describe('getAvailableModels', () => {
        ...
        it('throws on HTTP error', async () => {
            ...
            await expect(service.getAvailableModels()).rejects.toThrow('Failed to fetch models');
        });
        it('retries on network error', async () => {
            mockFetch
                .mockRejectedValueOnce(new Error('ECONNRESET'))
                .mockResolvedValueOnce(mockTagsResponse([{ name: 'llama3.2', size: 2e9 }]));
            const models = await service.getAvailableModels();
    ...
    describe('isModelAvailable', () => {
        it('finds exact model name', async () => {
            mockFetch.mockResolvedValueOnce(mockTagsResponse([{ name: 'llama3.2:latest', size: 2e9 }]));
    ...
    describe('chat', () => {
        ...
        it('sends a chat request and returns response', async () => {
            mockFetch.mockResolvedValueOnce(mockChatResponse({ role: 'assistant', content: 'Hi there!' }));
        ...
        it('throws on missing model', async () => {
            ...
            await expect(service.chat(request)).rejects.toThrow('model name is required');
        });
        it('throws on empty messages', async () => {
            ...
            await expect(service.chat(request)).rejects.toThrow('messages array is empty');
        });
        it('throws on missing messages', async () => {
            const request = { model: 'llama3.2' };
            await expect(service.chat(request)).rejects.toThrow('messages must be an array');
        });
        ...
        it('throws on non-string content', async () => {
            ...
            await expect(service.chat(request)).rejects.toThrow('content must be a string');
        });
        it('handles 404 model not found', async () => {
            mockFetch.mockResolvedValueOnce(mockResponse(404, 'model not found', 'Not Found'));
            await expect(service.chat(validRequest)).rejects.toThrow('Model not found');
        });
        it('handles 500 server error', async () => {
            mockFetch.mockResolvedValueOnce(mockResponse(500, 'Internal', 'Server Error'));
            await expect(service.chat(validRequest)).rejects.toThrow('Ollama server error');
        });
        it('handles 401 unauthorized for cloud', async () => {
            mockFetch.mockResolvedValueOnce(mockResponse(401, 'unauthorized', 'Unauthorized'));
            await expect(service.chat(validRequest)).rejects.toThrow('Authentication failed');
        });
    });
    // ── Chat Stream ─────────────────────────────────────────────────────────────
    describe('chatStream', () => {
        ...
        it('throws on HTTP error', async () => {
            mockFetch.mockResolvedValueOnce(mockResponse(500, 'Server error', 'Internal Server Error'));
            await expect(async () => {
    ...
    describe('generateEmbedding', () => {
        it('returns embedding array', async () => {
            ...
            const result = await service.generateEmbedding('nomic-embed-text', 'Hello');
            expect(result).toEqual(embedding);
        });
        it('throws on error response', async () => {
            mockFetch.mockResolvedValueOnce(mockResponse(500, 'Server error', 'Internal Server Error'));
            await expect(service.generateEmbedding('nomic-embed-text', 'Hello')).rejects.toThrow('Ollama embedding error');
        });
    });
    // ── Error Handling ─────────────────────────────────────────────────────────
    describe('error handling', () => {
        it('enhances ECONNREFUSED errors', async () => {
            ...
            await expect(service.getAvailableModels()).rejects.toThrow(/Troubleshooting/);
        });
