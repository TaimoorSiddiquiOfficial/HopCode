...
import { readWorktreeSession, writeWorktreeSession, clearWorktreeSession, restoreWorktreeContext, isSessionRuntimeActive, } from './worktreeSessionService.js';
...
describe('isSessionRuntimeActive', () => {
    ...
    it('lets active runtime status win over a dead status found in an earlier root', async () => {
        ...
        const worktreePath = path.join(repoRoot, '.hopcode', 'worktrees', 'feature');
        await fs.mkdir(worktreePath, { recursive: true });
        Storage.setRuntimeBaseDir(path.join(tmpDir, 'runtime'));
        await writeRuntimeStatus(new Storage(repoRoot).getRuntimeStatusPath('owner-session'), {
            sessionId: 'owner-session',
            workDir: repoRoot,
            pid: 2147483647,
        });
        await writeRuntimeStatus(new Storage(worktreePath).getRuntimeStatusPath('owner-session'), {
            sessionId: 'owner-session',
            workDir: worktreePath,
            pid: process.pid,
        });
        await expect(isSessionRuntimeActive('owner-session', [repoRoot, worktreePath])).resolves.toBe(true);
    });
    it('does not trust repo-contained dead runtime status as proof of inactivity', async () => {
        ...
        await writeRuntimeStatus(path.join(fakeRuntimeBase, 'projects', 'fake-project', 'chats', 'owner-session.runtime.json'), {
            sessionId: 'owner-session',
            workDir: repoRoot,
            pid: 2147483647,
        });
        await expect(isSessionRuntimeActive('owner-session', repoRoot)).resolves.toBe(true);
    });
});
describe('restoreWorktreeContext', () => {
    ...
    it('rejects and clears a sidecar whose worktreePath escapes the managed subtree', async () => {
        ...
        // under `<originalCwd>/.hopcode/worktrees/` must be treated as
        // untrusted, regardless of fs.stat success.
        const escape = {
            ...sample,
            originalCwd: tmpDir,
            worktreePath: tmpDir, // outside .hopcode/worktrees/
        };
    ...
    it('cleans up stale sidecar when worktree dir is gone', async () => {
        // sample.worktreePath points at /repo/.hopcode/... which does not exist.
        await writeWorktreeSession(filePath, sample);
        expect(await readWorktreeSession(filePath)).toEqual(sample);
        const result = await restoreWorktreeContext(filePath);
        expect(result.session).toBeNull();
