 ...
 * it drafts a SKILL.md candidate in .hopcode/evolve/skills/ and queues it as a
...
import { errorLogger, withinServiceBoundary, withinAgentBoundary, } from '../errors/index.js';
 ...
 */
export async function runEvolvePass(config, recentMessages) {
    turnsSinceLastReview++;
    if (turnsSinceLastReview < SKILL_REVIEW_INTERVAL)
        return;
    turnsSinceLastReview = 0;
    if (recentMessages.length < 4)
        return;
    const projectRoot = config.getTargetDir();
    const evolveSkillsDir = path.join(projectRoot, HOPCODE_DIR, EVOLVE_DIR_NAME, 'skills');
    const proposalsDir = getProposalsDir(projectRoot);
    // Create directories with proper error handling
    await withinServiceBoundary({ serviceName: 'filesystem-evolve-skills' }, async () => await fs.mkdir(evolveSkillsDir, { recursive: true })).catch((error) => {
    ...
    await withinServiceBoundary({ serviceName: 'filesystem-proposals' }, async () => await fs.mkdir(proposalsDir, { recursive: true })).catch((error) => {
    ...
    const systemPrompt = buildEvolvePrompt(recentText, existingSkills, evolveSkillsDir, proposalsDir);
    ...
    try {
        const agent = await AgentHeadless.create('evolve-skill-detector', config, promptConfig, { model: config.getModel() }, runConfig, toolConfig);
        const context = new ContextState();
        context.set('evolveSkillsDir', evolveSkillsDir);
        context.set('proposalsDir', proposalsDir);
        await withinAgentBoundary({ agentId: 'evolve-skill-detector' }, async () => await agent.execute(context));
...
async function getExistingSkillNames(projectRoot) {
...
function buildEvolvePrompt(recentText, existingSkills, evolveSkillsDir, proposalsDir) {
    const skillList = existingSkills.length > 0 ? existingSkills.join(', ') : '(none yet)';
...
**File 1** — Draft skill at ${evolveSkillsDir}/<slug>/SKILL.md
...
**File 2** — Proposal at ${proposalsDir}/skill-candidate-<slug>.md
\`\`\`markdown
A skill candidate was detected from recent conversation turns.

**Skill name:** /<slug>
**Draft location:** ${evolveSkillsDir}/<slug>/SKILL.md

To promote: copy the SKILL.md to .hopcode/skills/<slug>/SKILL.md
To dismiss: delete the draft file.
\`\`\`

## Recent conversation
${recentText}`;
}
//# sourceMappingURL=evolveService.js.map