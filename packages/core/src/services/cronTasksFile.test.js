...
import { addCronTask, appendCronRun, generateCronTaskId, getCronFilePath, MAX_TASK_RUNS, readCronTasks, removeCronTasks, updateCronTasks, writeCronTasks, } from './cronTasksFile.js';
import { Storage } from '../config/storage.js';
import { getProjectHash } from '../utils/paths.js';
/** Seeds the on-disk tasks file directly, creating its (now hashed) dir. */
async function seedTasksFile(projectRoot, raw) {
...
vi.mock('node:fs/promises', async (importOriginal) => {
    ...
    return {
        ...
        rename: (async (src, dst) => {
            if (renameHook.current)
                await renameHook.current(String(src));
...
describe('cronTasksFile', () => {
    ...
    describe('getCronFilePath', () => {
        it('resolves to the per-project runtime dir, not the working tree', () => {
            ...
            expect(file).toBe(path.join(Storage.getGlobalTempDir(), getProjectHash('/project'), 'scheduled_tasks.json'));
            // Crucially, not in the project working tree.
            expect(file.startsWith('/project')).toBe(false);
            expect(file).not.toContain(`${path.sep}.hopcode${path.sep}scheduled_tasks`);
        });
    });
    describe('readCronTasks', () => {
        ...
        it('throws for non-finite timestamps', async () => {
            ...
            const raw = (createdAt, lastFiredAt) => `[{"id":"t1","cron":"* * * * *","prompt":"p","recurring":true,` +
        ...
        it('rejects a task whose name is not a string', async () => {
            await seedTasksFile(tmpDir, JSON.stringify([{ ...makeTask(), name: 123 }]));
            await expect(readCronTasks(tmpDir)).rejects.toThrow(/Invalid task entry/);
        });
        it('rejects a task whose enabled is not a boolean', async () => {
            await seedTasksFile(tmpDir, JSON.stringify([{ ...makeTask(), enabled: 'yes' }]));
            await expect(readCronTasks(tmpDir)).rejects.toThrow(/Invalid task entry/);
        });
    ...
    describe('appendCronRun', () => {
        ...
        it('is pure — does not mutate the input array', () => {
            const input = [{ at: 1, kind: 'scheduled' }];
    ...
    describe('writeCronTasks', () => {
        ...
        it('replaces a symlink at the tasks path instead of writing through it', async () => {
            ...
            expect((await fs.lstat(getCronFilePath(tmpDir))).isSymbolicLink()).toBe(false);
    ...
    describe('removeCronTasks', () => {
        ...
        it('leaves no trace when nothing matches', async () => {
            ...
            await expect(fs.stat(path.dirname(getCronFilePath(tmpDir)))).rejects.toThrow();
        });
    });
    describe('updateCronTasks', () => {
        ...
        it('refuses to clobber a malformed file', async () => {
            ...
            await expect(updateCronTasks(tmpDir, (tasks) => tasks.filter(() => true))).rejects.toThrow(/Malformed JSON/);
        ...
        it('refuses to clobber a file with invalid task entries', async () => {
            ...
            await expect(updateCronTasks(tmpDir, (tasks) => [
                ...tasks,
        ...
        it('does not displace a fresh lock created after the stale inspection', async () => {
            ...
            await update;
            expect((await readCronTasks(tmpDir)).map((t) => t.id)).toContain('after-race');
        });
    });
    describe('generateCronTaskId', () => {
        ...
        it('is very unlikely to collide across calls', () => {
            const ids = new Set(Array.from({ length: 200 }, () => generateCronTaskId()));
...
//# sourceMappingURL=cronTasksFile.test.js.map