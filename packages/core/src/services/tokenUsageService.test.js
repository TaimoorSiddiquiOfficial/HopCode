...
import { __overrideNowForTesting, apiResponseEventToTokenUsageRecord, exportTokenUsageSummary, formatTokenUsageSummaryAsCsv, getTokenUsageFilePath, queryTokenUsage, recordTokenUsageFromApiResponse, recordTokenUsageFromApiResponseBestEffort, resetTokenUsageFailureLogging, } from './tokenUsageService.js';
describe('tokenUsageService', () => {
    let tempDir;
    let originalRuntimeDir;
    let originalDebugLogFileEnv;
    beforeEach(async () => {
        vi.useFakeTimers();
    ...
    function createEvent(model, promptId, usageData, options = {}) {
        const event = new ApiResponseEvent(options.responseId ?? `${promptId}-response`, model, options.durationMs ?? 100, promptId, options.authType ?? AuthType.USE_GEMINI, usageData, undefined, options.subagentName);
    ...
    it('maps an API response event to a privacy-preserving usage record', () => {
        ...
        const event = createEvent('qwen-model', 'prompt-1', {
            promptTokenCount: 10,
    ...
    it('uses current local date when API timestamps are missing or invalid', () => {
        ...
        const invalidTimestampRecord = apiResponseEventToTokenUsageRecord(config, invalidTimestampEvent);
        const missingTimestampRecord = apiResponseEventToTokenUsageRecord(config, missingTimestampEvent);
    ...
    it('persists API usage to monthly JSONL and aggregates daily totals', async () => {
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('model-a', 'prompt-1', {
            promptTokenCount: 10,
            candidatesTokenCount: 20,
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('model-b', 'prompt-2', {
            promptTokenCount: 7,
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('model-a', 'prompt-3', {
        ...
        const fileContent = await readFile(getTokenUsageFilePath('2026-05'), 'utf-8');
    ...
    it('swallows best-effort write errors and surfaces non-ENOENT failures', async () => {
        ...
        try {
            expect(() => recordTokenUsageFromApiResponseBestEffort(config, event)).not.toThrow();
            expect(writeSpy).toHaveBeenCalledTimes(1);
            await vi.waitFor(() => {
                expect(stderrSpy).toHaveBeenCalledWith('[token-usage] Write failed (ENOSPC):', 'disk full');
            });
        }
        finally {
    ...
    it('swallows synchronous best-effort record conversion errors', () => {
        ...
        try {
            ...
            expect(stderrSpy).toHaveBeenCalledWith('[token-usage] Write failed (EACCES):', 'session unavailable');
        }
        finally {
            writeSpy.mockRestore();
            stderrSpy.mockRestore();
    ...
    it('suppresses repeated console.error for same error code within 60s cooldown, re-fires after cooldown', async () => {
        ...
        try {
            ...
            expect(stderrSpy).toHaveBeenCalledWith('[token-usage] Write failed (EACCES):', 'permission denied');
        }
        finally {
            writeSpy.mockRestore();
        }
        ...
        try {
            ...
            expect(stderrSpy).toHaveBeenLastCalledWith('[token-usage] Write failed (ENOSPC):', 'disk full (1 similar suppressed since last log)');
        }
        finally {
            writeSpy.mockRestore();
        }
    ...
    it('aggregates monthly model, auth type, model/auth, and source groups', async () => {
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('model-a', 'prompt-2', {
            promptTokenCount: 4,
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('model-b', 'prompt-3', {
            promptTokenCount: 6,
    ...
    it('surfaces token usage read failures instead of returning zero totals', async () => {
        ...
        try {
            await expect(queryTokenUsage({ period: 'month', value: '2026-05' })).rejects.toThrow('permission denied');
    ...
    it('exports summaries as JSON and escaped CSV', async () => {
        ...
        await recordTokenUsageFromApiResponse(config, createEvent('=cmd|quoted', 'prompt-1', {
            promptTokenCount: 1,
            candidatesTokenCount: 2,
            totalTokenCount: 3,
        }, {
        ...
        const csv = formatTokenUsageSummaryAsCsv(await queryTokenUsage({ period: 'day', value: '2026-05-25' }));
        expect(csv).toContain('day,2026-05-25,total,total,,,,1,1,2,0,0,3,100');
        expect(csv).toContain("day,2026-05-25,model,'=cmd|quoted,'=cmd|quoted,,,1,1,2,0,0,3,100");
        expect(csv).toContain('day,2026-05-25,auth_type,"auth""quoted",,"auth""quoted",,1,1,2,0,0,3,100');
    });
    it('escapes formula-like CSV fields after leading whitespace', () => {
        ...
        expect(csv).toContain("day,2026-05-25,model,'=SUM(A1),'=SUM(A1),,,1,1,2,0,0,3,100");
        expect(csv).toContain("day,2026-05-25,model,' =SUM(A1),' =SUM(A1),,,1,1,2,0,0,3,100");
        expect(csv).toContain("day,2026-05-25,model,'+SUM(A1),'+SUM(A1),,,1,1,2,0,0,3,100");
        expect(csv).toContain("day,2026-05-25,model,' +SUM(A1),' +SUM(A1),,,1,1,2,0,0,3,100");
        expect(csv).toContain("day,2026-05-25,model,'-SUM(A1),'-SUM(A1),,,1,1,2,0,0,3,100");
    ...
    it('persists best-effort records asynchronously without blocking callers', async () => {
        ...
        recordTokenUsageFromApiResponseBestEffort(config, createEvent('model-a', 'prompt-1', {
            promptTokenCount: 1,
            candidatesTokenCount: 2,
            totalTokenCount: 3,
        }));
        const fileContent = await vi.waitFor(() => readFile(getTokenUsageFilePath('2026-05'), 'utf-8'));
        expect(fileContent).toContain('"model":"model-a"');
    });
    it('validates period values', async () => {
        await expect(queryTokenUsage({ period: 'day', value: '2026-05' })).rejects.toThrow('Expected YYYY-MM-DD');
        await expect(queryTokenUsage({ period: 'day', value: '2026-02-29' })).rejects.toThrow('Expected YYYY-MM-DD');
        await expect(queryTokenUsage({ period: 'day', value: '2024-02-29' })).resolves.toMatchObject({ value: '2024-02-29' });
        await expect(queryTokenUsage({ period: 'month', value: '2026-05-25' })).rejects.toThrow('Expected YYYY-MM');
        await expect(queryTokenUsage({ period: 'month', value: '2026-13' })).rejects.toThrow('Expected YYYY-MM');
        expect(() => getTokenUsageFilePath('2026-00')).toThrow('Expected YYYY-MM');
    });
});
//# sourceMappingURL=tokenUsageService.test.js.map