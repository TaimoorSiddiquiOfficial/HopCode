...
export const CRON_TASKS_DISPLAY_PATH = `~/.hopcode/tmp/<project-hash>/${TASKS_FILENAME}`;
...
export async function writeCronTasks(projectRoot, tasks) {
    const filePath = getCronFilePath(projectRoot);
 ...
 */
export async function updateCronTasks(projectRoot, mutate) {
    const filePath = getCronFilePath(projectRoot);
...
export async function removeCronTasks(projectRoot, ids) {
    ...
    // the update lock would mkdir .hopcode/ just to discover there is nothing
