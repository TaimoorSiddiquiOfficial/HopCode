...
describe('GitWorktreeService.createUserWorktree() — symlinkDirectories', () => {
    ...
    let repoRoot;
    beforeEach(async () => {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-symlinks-'));
    ...
    it('silently skips an existing destination (no overwrite)', async () => {
        ...
        await service.symlinkConfiguredDirectories(wt, ['node_modules']);
        // The preexisting file survived — no overwrite happened.
        const marker = await fs.readFile(path.join(wt, 'node_modules', 'preexisting'), 'utf8');
    ...
    it('works when the repo path itself contains a symlink boundary (round-7 self-inflicted regression guard)', async () => {
        ...
        const realDirRaw = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-realdir-'));
        const realDir = await fs.realpath(realDirRaw);
        const linkParentRaw = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-linkdir-'));
        ...
        try {
            ...
            expect(lst, 'symlinkDirectories entry was silently rejected — canonical vs lexical isWithinRoot mismatch').not.toBeNull();
    ...
    it('refuses sources whose realpath escapes the repo root or lands in .git/.hopcode (committed-symlink bypass)', async () => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-outside-'));
        ...
        try {
            ...
            for (const name of ['escape-to-git', 'escape-to-outside']) {
                ...
                const exists = await fs
                    ...
                    .then(() => true)
                    .catch(() => false);
                expect(exists, `realpath guard must refuse to create <wt>/${name} — committed symlink escape would chain through to a sensitive location`).toBe(false);
            }
    ...
    it('handles multiple entries — some present, some missing', async () => {
        ...
        expect(result.success).toBe(true);
        const wt = result.worktree.path;
        expect(await fs.readlink(path.join(wt, 'present-a'))).toBe(path.join(repoRoot, 'present-a'));
        expect(await fs.readlink(path.join(wt, 'present-b'))).toBe(path.join(repoRoot, 'present-b'));
    ...
    describe('Phase D-3: fetchPullRequestRef error messages', () => {
        ...
        it('handles "no such ref" when origin is reachable but the PR does not exist', async () => {
            ...
            const upstream = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-pr-no-such-ref-'));
            ...
            try {
                ...
                if (!res.success) {
                    ...
                    expect(res.error.toLowerCase()).toMatch(/pr.*not exist|origin.*unreachable/);
                }
            }
            finally {
...
//# sourceMappingURL=gitWorktreeService.symlinks.integ.test.js.map