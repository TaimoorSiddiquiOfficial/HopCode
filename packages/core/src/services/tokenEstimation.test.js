...
import { estimateContentTokens, estimatePromptTokens, getUsageOutputTokenCountForPromptEstimate, } from './tokenEstimation.js';
...
describe('estimatePromptTokens', () => {
    ...
    it('includes the previous turn candidate tokens in the steady-state estimate', () => {
        const userEst = estimateContentTokens([user]);
        expect(estimatePromptTokens(history, user, 5000, 1200)).toBe(5000 + 1200 + userEst);
    });
    it('keeps custom image-token estimates as the fifth argument', () => {
        ...
        expect(estimatePromptTokens(history, imageUser, 5000, 1200, 1600)).toBe(5000 + 1200 + 1600);
    });
...
//# sourceMappingURL=tokenEstimation.test.js.map