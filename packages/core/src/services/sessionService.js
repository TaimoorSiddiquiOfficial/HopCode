...
import { deserializeSnapshots, FILE_HISTORY_DIR, MAX_SNAPSHOTS, serializeSnapshot, } from './fileHistoryService.js';
...
import { isSessionArtifactRecord, rebuildSessionArtifactSnapshot, remapSessionArtifactPayloadForFork, } from './session-artifact-persistence.js';
...
async function copyFileHistoryBackups(sourceSessionId, targetSessionId) {
    const fsPromises = await import('node:fs/promises');
    const sourceDir = path.join(Storage.getGlobalHopCodeDir(), FILE_HISTORY_DIR, sourceSessionId);
    const targetDir = path.join(Storage.getGlobalHopCodeDir(), FILE_HISTORY_DIR, targetSessionId);
    let entries;
    try {
    ...
    await Promise.all(entries.map(async (name) => {
        ...
        try {
            ...
            try {
                ...
                debugLogger.warn(`copyFileHistoryBackups: failed to copy ${name}: ${copyErr}`);
            }
        }
    }));
}
...
export class SessionService {
    ...
    async sessionBelongsToCurrentProject(sessionId, recordCwd) {
        if (getProjectHash(recordCwd) === this.projectHash) {
            return true;
        }
        const status = await readRuntimeStatus(this.storage.getRuntimeStatusPath(sessionId));
    ...
    removeFileHistoryBackups(sessionId) {
        fs.rmSync(path.join(Storage.getGlobalhopcodeDir(), FILE_HISTORY_DIR, sessionId), { recursive: true, force: true });
    }
    ...
    async loadSessionFromState(sessionId, state, stats) {
        ...
        const cappedSnapshots = fileHistorySnapshots.length > MAX_SNAPSHOTS
        ...
        return {
            ...
            fileHistorySnapshots: cappedSnapshots.length > 0 ? cappedSnapshots : undefined,
    ...
    async forkSession(sourceSessionId, newSessionId) {
        ...
        const sourceRecords = includeActiveSideArtifactRecords(records, activeMessages).filter(
        ...
        const forked = sourceRecords.map((record) => {
            const isArtifactRecord = isSessionArtifactRecord(record);
            const systemPayload = remapSystemPayloadForFork(record, sourceSessionId, newSessionId, remappedArtifactIds);
            const next = {
                ...record,
        ...
        try {
            ...
            if (!targetComplete) {
                try {
                    ...
                    this.warn(`forkSession: failed to clean up incomplete target ${newSessionId}: ${cleanupError}`);
                }
                try {
                    this.removeFileHistoryBackups(newSessionId);
                }
                catch (cleanupError) {
                    this.warn(`forkSession: failed to clean up file history for incomplete target ${newSessionId}: ${cleanupError}`);
                }
            }
        }
...
function includeActiveSideArtifactRecords(records, activeRecords) {
    ...
    for (let index = records.length - 1; index >= 0; index--) {
        ...
        else if (!isSessionArtifactRecord(records[index]) &&
    ...
    for (let index = 0; index < records.length; index++) {
        ...
        const hasBlockingRecordBeforeNextActive = nextBlockingUuidByIndex.has(index);
...
function isTailNeutralSideRecord(record) {
