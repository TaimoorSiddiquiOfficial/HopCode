...
import { metricsToUsageRecord, aggregateUsage, loadUsageHistory, loadUsageHistoryWithLive, persistSessionUsage, } from './usageHistoryService.js';
...
describe('metricsToUsageRecord', () => {
    it('populates totalLatencyMs from sum of model api.totalLatencyMs', () => {
        ...
        const record = metricsToUsageRecord('session-1', '/project', 1000, 5000, metrics);
        expect(record.totalLatencyMs).toBe(3500); // 2000 + 1500
    });
    it('populates totalDurationMs for each tool in byName', () => {
        const metrics = makeMetrics();
        const record = metricsToUsageRecord('session-2', '/project', 1000, 6000, metrics);
    ...
    it('sets totalLatencyMs to 0 when no models present', () => {
        ...
        const record = metricsToUsageRecord('session-3', '/project', 0, 1000, metrics);
        expect(record.totalLatencyMs).toBe(0);
    });
    it('preserves existing fields correctly alongside new fields', () => {
        const metrics = makeMetrics();
        const record = metricsToUsageRecord('session-4', '/my/project', 1000, 4000, metrics);
...
describe('loadUsageHistory + persistSessionUsage (issue #4994 regression)', () => {
    ...
    function plantChatJsonl(sessionId, tokens) {
        ...
        const projDir = path.join(process.env['HOPCODE_HOME'], 'projects', 'repro-project');
        ...
        const records = [
            ...
            },
            {
                ...
                systemPayload: {
                    uiEvent: {
                        'event.name': 'hopcode.api_response',
                        'event.timestamp': mid,
                        response_id: 'r1',
                        model: 'qwen-max',
                        duration_ms: 1200,
        ...
        fs.writeFileSync(path.join(projDir, 'chats', `${sessionId}.jsonl`), records.map((r) => JSON.stringify(r)).join('\n') + '\n');
    }
    function makeLiveMetrics(tokens) {
        return {
            models: {
                'qwen-max': {
    ...
    it('read-side: dedups duplicate sessionId records already on disk (last-wins)', async () => {
        ...
        const usagePath = path.join(process.env['HOPCODE_HOME'], 'usage_record.jsonl');
        const rec = (totalTokens) => ({
            version: 1,
            sessionId,
            timestamp: Date.now(),
            startTime: Date.now() - 60000,
        ...
        fs.writeFileSync(usagePath, JSON.stringify(rec(1000)) + '\n' + JSON.stringify(rec(1600)) + '\n');
    ...
    it('read-only: persistRebuild:false rebuilds without writing usage_record.jsonl', async () => {
        ...
        plantChatJsonl(sessionId, 1600);
        const usagePath = path.join(process.env['QWEN_HOME'], 'usage_record.jsonl');
    ...
    function planted(sessionId) {
        return path.join(process.env['QWEN_HOME'], 'projects', 'repro-project', 'chats', `${sessionId}.jsonl`);
    }
    function seedPersisted(records) {
        fs.writeFileSync(path.join(process.env['QWEN_HOME'], 'usage_record.jsonl'), records.map((r) => JSON.stringify(r)).join('\n') + '\n');
    }
    function persistedRec(sessionId, totalTokens) {
        return {
            version: 1,
            sessionId,
            timestamp: Date.now(),
    ...
    it('withLive: unions a never-persisted (daemon) session with the persisted history', async () => {
        ...
        const lines = fs
            .readFileSync(path.join(process.env['QWEN_HOME'], 'usage_record.jsonl'), 'utf8')
            .trim()
            .split('\n');
        expect(lines).toHaveLength(1);
    });
    ...
    it('withLive: a corrupt usage_record.jsonl falls back to a full transcript replay', async () => {
        ...
        fs.writeFileSync(path.join(process.env['QWEN_HOME'], 'usage_record.jsonl'), '{ this is not valid json\nalso broken}\n');
        plantChatJsonl('sess-daemon', 1600);
        const merged = await loadUsageHistoryWithLive();
...
describe('aggregateUsage — skills', () => {
    function skillRecord(sessionId, skills) {
        return {
            version: 1,
            sessionId,
            timestamp: Date.now(),
...
//# sourceMappingURL=usageHistoryService.test.js.map