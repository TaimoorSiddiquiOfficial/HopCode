...
import { getStartupContextLength, isSystemReminderContent, stripSystemReminderBlocks, } from '../utils/environmentContext.js';
...
export async function tryGenerateSessionTitle(config, abortSignal) {
    try {
        ...
        // as a real user turn.
        const hasUserMessage = fullHistory.some((msg) => msg.role === 'user' && !isSystemReminderContent(msg));
        if (!hasUserMessage)
            return { ok: false, reason: 'empty_history' };
