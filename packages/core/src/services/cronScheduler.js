 ...
 * under the user runtime dir (~/.hopcode/tmp/<project-hash>/) and survive
 * restarts.
 */
...
export class CronScheduler {
    ...
    create(cronExpr, prompt, recurring) {
        ...
        if (process.env['HOPCODE_CODE_TEST_CRON_FAST'] === '1' && !job.durable) {
            const delayMs = Number(process.env['HOPCODE_CODE_TEST_CRON_DELAY_MS']) || 5000;
     ...
     */
    scheduleWakeup(delaySeconds, prompt) {
     ...
     * persisted under ~/.hopcode (per-project) so it survives restarts.
...
export function buildMissedCronNotification(missed) {
    ...
    const header = `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while HopCode was not running. ` +
