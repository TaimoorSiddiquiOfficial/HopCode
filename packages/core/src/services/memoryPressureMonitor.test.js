...
import { DEFAULT_PRESSURE_CONFIG, validateMemoryPressureConfig, } from './memoryPressureMonitor.js';
...
const { getMockOsTotalmem, setOsTotalmem, getMockCgroupFile, setCgroupMemoryMax, setCgroupV1MemoryLimit, getMockHeapSizeLimit, setHeapSizeLimit, } = vi.hoisted(() => {
    let totalmem = 16 * 1024 * 1024 * 1024; // 16 GB default
    let cgroupMemoryMax = 'max';
    let cgroupV1MemoryLimit;
    ...
    return {
        ...
        setCgroupMemoryMax: (v) => {
            cgroupMemoryMax = v;
        },
        setCgroupV1MemoryLimit: (v) => {
...
vi.mock('node:fs', async (importOriginal) => ({
    ...(await importOriginal()),
    readFileSync: (path) => getMockCgroupFile(path),
}));
vi.mock('node:v8', () => ({
    getHeapStatistics: () => ({
...
const { mockIsPerformanceMonitoringActive, mockRecordMemoryUsage, mockRecordCpuUsage, } = vi.hoisted(() => ({
...
function setMemUsage(rssBytes, heapUsedBytes = 256 * 1024 * 1024) {
    vi.spyOn(process, 'memoryUsage').mockReturnValue(createMemUsage(rssBytes, heapUsedBytes));
}
function createMemUsage(rssBytes, heapUsedBytes = 256 * 1024 * 1024) {
    return {
        rss: rssBytes,
...
async function drainCleanupMeasurement() {
...
describe('MemoryPressureMonitor', () => {
    ...
    describe('validateMemoryPressureConfig', () => {
        it('accepts valid config', () => {
            expect(() => validateMemoryPressureConfig({
    ...
    describe('getPressureLevel', () => {
        ...
        it('does not treat a zero effective memory limit as RSS pressure', () => {
            ...
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('Effective memory limit is not positive; RSS pressure checks are disabled');
        });
        it('returns normal when process memory usage cannot be read', () => {
            ...
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Failed to read memory usage for pressure check: memory API unavailable');
        });
        it('uses cgroup memory.max when available', () => {
            ...
            expect(monitor.getPressureLevel()).toBe('soft');
            expect(mockDebugLogger.info).toHaveBeenCalledWith('Using cgroup v2 memory limit: 2048 MiB');
        });
        ...
        it('uses cgroup v1 memory.limit_in_bytes when cgroup v2 is unavailable', () => {
            ...
            setMemUsage(1200 * 1024 * 1024); // 1.2/2 = 0.586 >= 0.50
            expect(monitor.getPressureLevel()).toBe('soft');
            expect(mockDebugLogger.info).toHaveBeenCalledWith('Using cgroup v1 memory limit: 2048 MiB');
        });
        it('ignores cgroup v1 unlimited sentinel values', () => {
            ...
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.warn).not.toHaveBeenCalledWith(expect.stringContaining('9223372036854771712'));
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Ignoring unlimited cgroup memory limit from ' +
        ...
        it('ignores malformed cgroup limits without partially parsing them', () => {
            ...
            setMemUsage(1200 * 1024 * 1024); // 1.2/16 = 0.073 < 0.50
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('Ignoring non-numeric cgroup memory limit from ' +
                '/sys/fs/cgroup/memory.max: 2048garbage');
            expect(mockDebugLogger.info).toHaveBeenCalledWith('Using host memory limit: 16384 MiB');
        });
        it('logs cgroup read failures before falling back to host memory', () => {
            ...
            setMemUsage(1200 * 1024 * 1024);
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Failed to read cgroup memory limit from /sys/fs/cgroup/memory.max: ' +
                'ENOENT');
            expect(mockDebugLogger.info).toHaveBeenCalledWith('Using host memory limit: 16384 MiB');
        });
        it('logs out-of-range cgroup limits distinctly', () => {
            ...
            setMemUsage(1200 * 1024 * 1024);
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('Ignoring out-of-range cgroup memory limit from ' +
        ...
        it('ignores safe cgroup limits above host total memory', () => {
            ...
            setMemUsage(1200 * 1024 * 1024); // 1.2/16 = 0.073 < 0.50
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Ignoring cgroup memory limit above host total from ' +
        ...
        it('ignores unrealistically small cgroup limits', () => {
            ...
            setMemUsage(1200 * 1024 * 1024); // 1.2/16 = 0.073 < 0.50
            expect(monitor.getPressureLevel()).toBe('normal');
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('Ignoring unrealistically small cgroup memory limit from ' +
                '/sys/fs/cgroup/memory.max: 1');
        });
    ...
    describe('performCheck with cleanup', () => {
        ...
        it('emits a diagnostic event after repeated ineffective cleanups', async () => {
            ...
            expect(cleanupIneffective).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('throttles diagnostic events for continued ineffective cleanup', async () => {
            ...
            expect(cleanupIneffective).toHaveBeenLastCalledWith(expect.objectContaining({
        ...
        it('measures a cleanup before running a queued escalation', async () => {
            ...
            await drainCleanupMeasurement();
            expect(clearSpy).toHaveBeenCalledTimes(1);
            expect(mockDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('Cleanup "light" completed; RSS delta 1073741824 bytes'));
        });
        it('resets ineffective cleanup count after an effective cleanup', async () => {
            ...
            vi.spyOn(process, 'memoryUsage').mockImplementation(() => createMemUsage(rss));
            const monitor = new MemoryPressureMonitor(createMockConfig({
                fileReadCache: { evictNotAccessedSince: evictSpy },
            }), { ...DEFAULT_PRESSURE_CONFIG, cleanupCooldownMs: 0 });
            const cleanupIneffective = vi.fn();
        ...
        it('records queued cleanup startup failures', async () => {
            ...
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Cleanup "aggressive" failed: queued RSS unavailable; ' +
        ...
        it('warns when explicit GC is requested but unavailable', async () => {
            ...
            await drainCleanupMeasurement();
            expect(clearSpy).toHaveBeenCalled();
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('trigger_gc requested but global.gc is not available; ' +
        ...
        it('runs explicit GC when requested and available', async () => {
            ...
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('global.gc() freed 0 bytes');
        });
        it('skips same-priority cleanup while another cleanup is in progress', () => {
            ...
            monitor.performCheck();
            expect(evictSpy).toHaveBeenCalledTimes(1);
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Cleanup already in progress, skipping');
        });
        it('counts cleanup step exceptions as failures', async () => {
            ...
            expect(cleanupFailed).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('records cleanup failures when RSS cannot be read', async () => {
            ...
            await drainCleanupMeasurement();
            expect(monitor.getConsecutiveFailures()).toBe(1);
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Failed to read RSS after cleanup failure: RSS unavailable');
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Cleanup "light" failed: cache failure; consecutive failures: 1');
        });
        it('throttles repeated cleanup failure events after the threshold', async () => {
            ...
            expect(cleanupFailed).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    describe('compact_history step', () => {
        ...
        it('compacts history and clears fileReadCache when meta is non-null', async () => {
            ...
            const blankedResponses = compacted.filter((entry) => entry.parts?.some((p) => p.functionResponse?.response?.['output'] ===
            ...
            expect(memoryResult?.functionResponse?.response?.['output']).toBe('content of f0');
        });
    ...
    describe('global.gc() safety guards', () => {
        it('should not include trigger_gc in soft or hard pressure tiers', () => {
            ...
            monitor.performCheck();
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('global.gc()'));
    ...
    describe('runtime sampling and telemetry', () => {
        ...
        it('reports memory and CPU metrics when performance monitoring is active', () => {
            ...
            expect(mockRecordMemoryUsage).toHaveBeenCalledWith(expect.anything(), rss, { memory_type: MemoryMetricType.RSS });
            expect(mockRecordMemoryUsage).toHaveBeenCalledWith(expect.anything(), heapUsed, { memory_type: MemoryMetricType.HEAP_USED });
            expect(mockRecordCpuUsage).toHaveBeenCalledTimes(1);
            expect(mockRecordCpuUsage).toHaveBeenCalledWith(expect.anything(), expect.any(Number), {});
        });
        ...
        it('logs first sampling failure at error level and subsequent ones at debug', () => {
            ...
            monitor.performCheck();
            expect(mockDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('Runtime sampling failed'));
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Runtime sampling failed'));
            mockDebugLogger.error.mockClear();
            mockDebugLogger.debug.mockClear();
            // Second failure: should log at debug level (not error)
            monitor.performCheck();
            expect(mockDebugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Runtime sampling failed'));
            expect(mockDebugLogger.error).not.toHaveBeenCalledWith(expect.stringContaining('Runtime sampling failed'));
        });
        it('passes runtime samples to the diagnostics dumper on hard pressure', async () => {
            ...
            expect(dumpSpy).toHaveBeenCalledWith('hard', expect.arrayContaining([expect.objectContaining({ rss })]));
        });
    });
});
