...
async function withoutGitPagerEnv(run) {
...
describe('ShellExecutionService', () => {
    ...
    describe('Successful Execution', () => {
        ...
        it('disposes PTY resources and resolves when final render throws', async () => {
            ...
            const { result } = await simulateExecution('render-fails-on-exit', (pty) => {
                pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null });
            }, {
        ...
        it('bounds buffered PTY output before building the final string', async () => {
            const { result } = await simulateExecution('large-output', (pty) => {
            ...
            expect(result.output).toContain('Output exceeded the maximum captured size');
        ...
        it('keeps PTY replay fallback bounded after the capture limit is exceeded', async () => {
            ...
            const { result } = await simulateExecution('large-output-replay-fallback', (pty) => {
                pty.onData.mock.calls[0][0]('12345678');
                pty.onData.mock.calls[0][0]('abcdefg');
                pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null });
            }, { ...shellExecutionConfig, maxBufferedOutputBytes: 10 });
        ...
        it('does not add a capture-limit notice at the exact PTY buffer boundary', async () => {
            const { result } = await simulateExecution('exact-output', (pty) => {
                pty.onData.mock.calls[0][0]('1234567890');
                pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null });
            }, { ...shellExecutionConfig, maxBufferedOutputBytes: 10 });
            expect(result.rawOutput.length).toBe(10);
            expect(result.output).toBe('1234567890');
            expect(result.output).not.toContain('Output exceeded the maximum captured size');
        });
    ...
    describe('Aborting Commands', () => {
        ...
        it('background-promote replay failure falls back to full decoded raw output', async () => {
            ...
            const { result } = await simulateExecution('long-running-output', (pty, ac) => {
    ...
    describe('Windows process cleanup (#5873)', () => {
        ...
        it('cancel on win32 tree-kills via taskkill, with a ptyProcess.kill fallback', async () => {
            ...
            expect(mockSpawnSync.mock.invocationCallOrder[0]).toBeLessThan(mockPtyProcess.kill.mock.invocationCallOrder[0]);
        });
        ...
        it('normal completion on win32 swallows a taskkill launch error (no crash)', async () => {
            ...
            expect(() => taskkillProc.emit('error', new Error('spawn taskkill ENOENT'))).not.toThrow();
        });
        ...
        it('win32 taskkill is invoked by absolute System32 path, not the bare name', async () => {
            ...
            const cmd = mockCpSpawn.mock.calls.find((c) => /taskkill/i.test(String(c[0])))?.[0];
        ...
        it('a late abort after a normal exit does NOT tree-kill (no cancel retro-flag)', async () => {
            mockPlatform.mockReturnValue('win32');
            const { result } = await simulateExecution('echo hi', (pty, abortController) => {
        ...
        it('win32 promoted shell reaps its lingering pwsh on natural exit (shell-pid-only)', async () => {
            mockPlatform.mockReturnValue('win32');
            const settleCalls = [];
            const { result } = await simulateExecution('long-running-command', (_pty, ac) => {
                ac.abort({
                    kind: 'background',
                    shellId: 'bg_5873_reap',
                });
            }, shellExecutionConfig, { postPromote: { onSettle: (info) => settleCalls.push(info) } });
            expect(result.promoted).toBe(true);
        ...
        it('win32 promoted shell skips the post-settle reap when the pty already exited', async () => {
            ...
            }, shellExecutionConfig, { postPromote: { onSettle: () => { } } });
    ...
    describe('Platform-Specific Behavior', () => {
        it('should use cmd.exe on Windows', async () => {
            ...
            expect(mockPtySpawn).toHaveBeenCalledWith('cmd.exe', `/d /s /c ${CHCP} 65001 >nul 2>nul & dir "foo bar"`, expect.any(Object));
            mockGetShellConfiguration.mockReturnValue({
                executable: 'bash',
                argsPrefix: ['-c'],
                shell: 'bash',
        ...
        it('should not apply UTF-8 prefix for Git Bash on Windows', async () => {
            ...
            await simulateExecution('echo hello', (pty) => pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null }));
            expect(mockPtySpawn).toHaveBeenCalledWith('bash.exe', ['-c', 'echo hello'], expect.any(Object));
            mockGetShellConfiguration.mockReturnValue({
                executable: 'bash',
                argsPrefix: ['-c'],
                shell: 'bash',
        ...
        it('does not inject Unix pager defaults into Windows pty env when unset', async () => {
            ...
            await simulateExecution('echo hello', (pty) => pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null }), shellExecutionConfigWithoutPager);
            const spawnOptions = mockPtySpawn.mock.calls[0][2];
...
describe('ShellExecutionService child_process fallback', () => {
    ...
    const simulateExecutionWithConfig = async (command, simulation, config, options = {}) => {
        const abortController = new AbortController();
        const handle = await ShellExecutionService.execute(command, '/test/dir', onOutputEventMock, abortController.signal, true, config, options);
        await new Promise((resolve) => process.nextTick(resolve));
        simulation(mockChildProcess, abortController);
    ...
    describe('Successful Execution', () => {
        ...
        it('bounds buffered child_process output before building the final string', async () => {
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute('large-output', '/test/dir', onOutputEventMock, abortController.signal, false, { ...shellExecutionConfig, maxBufferedOutputBytes: 10 });
            await new Promise((resolve) => process.nextTick(resolve));
            ...
            expect(onOutputEventMock).toHaveBeenCalledWith({
                type: 'data',
                chunk: expect.stringContaining('Output exceeded the maximum captured size'),
            });
        });
        it('does not add a capture-limit notice at the exact child_process buffer boundary', async () => {
            const { result } = await simulateExecutionWithConfig('exact-output', (cp) => {
        ...
        it.each([
            ...
            'abc',
            undefined,
        ])('falls back to the default capture limit for invalid maxBufferedOutputBytes: %s', async (configuredValue) => {
            const { result } = await simulateExecutionWithConfig('invalid-limit', (cp) => {
                cp.stdout?.emit('data', Buffer.from('1234567890abcde'));
                cp.emit('exit', 0, null);
                cp.emit('close', 0, null);
            }, {
                ...shellExecutionConfig,
                maxBufferedOutputBytes: configuredValue,
            });
        ...
        it('reports capture-limit notice for streaming child_process output', async () => {
            const { result } = await simulateExecutionWithConfig('streaming-large-output', (cp) => {
                cp.stdout?.emit('data', Buffer.from('abcdef'));
                cp.emit('exit', 0, null);
                cp.emit('close', 0, null);
            }, { ...shellExecutionConfig, maxBufferedOutputBytes: 1 }, { streamStdout: true });
            expect(onOutputEventMock).toHaveBeenCalledWith({
                type: 'data',
                chunk: 'abcdef',
            });
        ...
        it('emits only the capture-limit notice when stripped captured output is empty', async () => {
            const { result } = await simulateExecutionWithConfig('empty-captured-output', (cp) => {
                cp.stdout?.emit('data', Buffer.from('\nabc'));
                cp.emit('exit', 0, null);
                cp.emit('close', 0, null);
            }, { ...shellExecutionConfig, maxBufferedOutputBytes: 1 });
            expect(result.rawOutput.length).toBe(1);
            expect(result.output).toMatch(/^\[Output exceeded the maximum captured size/);
        });
        it('should handle commands with no output', async () => {
    ...
    describe('Platform-Specific Behavior', () => {
        it('should use cmd.exe with chcp 65001 UTF-8 prefix on Windows', async () => {
            ...
            expect(mockCpSpawn).toHaveBeenCalledWith('cmd.exe', ['/d', '/s', '/c', `${CHCP} 65001 >nul 2>nul & dir "foo bar"`], expect.objectContaining({
                detached: false,
                windowsHide: true,
        ...
        it('should not apply UTF-8 prefix for Git Bash on Windows via child_process', async () => {
            ...
            expect(mockCpSpawn).toHaveBeenCalledWith('bash.exe', ['-c', 'echo hello'], expect.any(Object));
            mockGetShellConfiguration.mockReturnValue({
                executable: 'bash',
                argsPrefix: ['-c'],
                shell: 'bash',
        ...
        it('does not inject Unix pager defaults into Windows child_process env when unset', async () => {
            ...
            await withoutGitPagerEnv(() => simulateExecutionWithConfig('echo hello', (cp) => cp.emit('exit', 0, null), shellExecutionConfigWithoutPager));
            const spawnOptions = mockCpSpawn.mock.calls[0][2];
            expect(spawnOptions.env['PAGER']).toBe('');
        ...
        it('preserves explicit pager configuration in Windows child_process env', async () => {
            ...
            await withoutGitPagerEnv(() => simulateExecution('echo hello', (cp) => cp.emit('exit', 0, null)));
            const spawnOptions = mockCpSpawn.mock.calls[0][2];
            expect(spawnOptions.env['PAGER']).toBe('cat');
            expect(spawnOptions.env['GIT_PAGER']).toBeUndefined();
            mockGetShellConfiguration.mockReturnValue({
...
describe('ShellExecutionService execution method selection', () => {
    ...
    ])('does not spawn through $label when the signal is already aborted', async ({ shouldUseNodePty }) => {
        const abortController = new AbortController();
        abortController.abort();
        const handle = await ShellExecutionService.execute('test command', '/test/dir', onOutputEventMock, abortController.signal, shouldUseNodePty, shellExecutionConfig);
        const result = await handle.result;
        expect(handle.pid).toBeUndefined();
    ...
    it.each(['resolve', 'reject'])('returns on abort while getPty is pending and ignores its late %s', async (settlement) => {
        let resolvePty;
        let rejectPty;
        mockGetPty.mockReturnValue(new Promise((resolve, reject) => {
            resolvePty = resolve;
            rejectPty = reject;
        }));
        const abortController = new AbortController();
        const removeAbortListener = vi.spyOn(abortController.signal, 'removeEventListener');
        const handlePromise = ShellExecutionService.execute('test command', '/test/dir', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
        abortController.abort();
