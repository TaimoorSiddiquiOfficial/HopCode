 ...
 */
function mockOk(title, modelUsed = 'hopcode-turbo') {
...
describe('ChatRecordingService - auto-title trigger', () => {
    ...
    it('does not trigger when HOPCODE_DISABLE_AUTO_TITLE is set', async () => {
        ...
        try {
            ...
            if (prev === undefined)
                delete process.env['HOPCODE_DISABLE_AUTO_TITLE'];
            else
                process.env['HOPCODE_DISABLE_AUTO_TITLE'] = prev;
        }
    });
    ...
    it('triggers in ACP (daemon) mode even though isInteractive is false', async () => {
        ...
        expect(record.systemPayload.customTitle).toBe('Fix login button');
    });
    ...
    it('lets an explicit auto rename cancel and outrank background auto-title', async () => {
        ...
        await flushMicrotasks();
        await expect(chatRecordingService.recordCustomTitle('User Auto Title', 'auto')).resolves.toBe(true);
        resolveLlm({ ok: true, title: 'Background Title', modelUsed: 'fast' });
        await flushMicrotasks();
        const titleRecords = vi
            .mocked(jsonl.writeLine)
            .mock.calls.map((call) => call[1])
            .filter((record) => record.type === 'system' && record.subtype === 'custom_title');
        expect(titleRecords).toHaveLength(1);
        expect(titleRecords[0]?.systemPayload).toMatchObject({
            customTitle: 'User Auto Title',
            titleSource: 'auto',
        });
        expect(chatRecordingService.getCurrentCustomTitle()).toBe('User Auto Title');
    });
    it('does not start background auto-title while an explicit title is pending', async () => {
        let resolveTitleWrite;
        vi.mocked(jsonl.writeLine).mockReturnValueOnce(new Promise((resolve) => {
            resolveTitleWrite = resolve;
        }));
        const explicit = chatRecordingService.recordCustomTitle('Pending Explicit', 'auto');
