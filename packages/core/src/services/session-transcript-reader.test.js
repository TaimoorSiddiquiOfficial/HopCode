...
import { encodeSessionTranscriptCursor, getSessionTranscriptIndexCacheStatsForTest, InvalidSessionTranscriptCursorError, SESSION_TRANSCRIPT_MAX_INDEX_BYTES, SESSION_TRANSCRIPT_MAX_LIMIT, resetSessionTranscriptIndexCacheForTest, setSessionTranscriptIndexCacheMaxBytesForTest, SessionTranscriptCursorCodec, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptReader, } from './session-transcript-reader.js';
describe('SessionTranscriptReader', () => {
    ...
    beforeEach(async () => {
        vi.clearAllMocks();
        runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-transcript-reader-'));
    ...
    async function writeRecords(records, targetSessionId = sessionId) {
        const chatsDir = path.join(new Storage(workspaceDir).getProjectDir(), 'chats');
        await fs.mkdir(chatsDir, { recursive: true });
        const filePath = path.join(chatsDir, `${targetSessionId}.jsonl`);
        await fs.writeFile(filePath, records.map((record) => JSON.stringify(record)).join('\n') + '\n', 'utf8');
        return filePath;
    }
    async function writeRawTranscript(content) {
        const chatsDir = path.join(new Storage(workspaceDir).getProjectDir(), 'chats');
        await fs.mkdir(chatsDir, { recursive: true });
    ...
    let recordSeq = 0;
    function record(uuid, parentUuid, text, targetSessionId = sessionId) {
        return {
            uuid,
            parentUuid,
    ...
    it('rejects an empty transcript snapshot', async () => {
        ...
        await expect(new SessionTranscriptReader(workspaceDir).readPage(sessionId)).rejects.toBeInstanceOf(SessionTranscriptSnapshotUnavailableError);
    });
    it('returns a single-record transcript without a continuation cursor', async () => {
        ...
        const page = await new SessionTranscriptReader(workspaceDir).readPage(sessionId);
    ...
    it.each([0, -1, SESSION_TRANSCRIPT_MAX_LIMIT + 1, 1.5])('rejects invalid page limit %s', async (limit) => {
        await expect(new SessionTranscriptReader(workspaceDir).readPage(sessionId, {
            limit,
        })).rejects.toBeInstanceOf(RangeError);
    });
    it.each([0, -1, 1.5])('rejects invalid page byte limit %s', async (maxBytes) => {
        await expect(new SessionTranscriptReader(workspaceDir).readPage(sessionId, {
            maxBytes,
        })).rejects.toBeInstanceOf(RangeError);
    });
    it('stops at a record boundary when the page byte budget is reached', async () => {
        ...
        const firstTwoBytes = Buffer.byteLength(JSON.stringify(records[0])) +
    ...
    it('rejects a single aggregate record over the page byte budget', async () => {
        ...
        const aggregateBytes = Buffer.byteLength(JSON.stringify(first)) +
    ...
    it('keeps backward pages within a normal user turn boundary', async () => {
        ...
        const page = await new SessionTranscriptReader(workspaceDir).readPage(sessionId, { beforeRecordId: 'u2', limit: 4 });
    ...
    it('keeps a long user turn complete when it exceeds the record limit', async () => {
        ...
        const page = await new SessionTranscriptReader(workspaceDir).readPage(sessionId, { beforeRecordId: 'u2', limit: 2 });
        expect(page.records.map((item) => item.uuid)).toEqual([
            'u1',
            'a-tool',
            't1',
    ...
    it('continues a frozen snapshot after new records are appended', async () => {
        ...
        const reader = new SessionTranscriptReader(workspaceDir);
        const first = await reader.readPage(sessionId, { limit: 2 });
        await fs.appendFile(filePath, JSON.stringify(record('a2', 'u2', 'late append')) + '\n', 'utf8');
        const second = await reader.readPage(sessionId, {
            cursor: encodeCursor(first.nextCursorState),
            limit: 2,
        });
    ...
    it('rejects cursors from another session before touching transcript files', async () => {
        ...
        await expect(reader.readPage(otherSessionId, {
            cursor: encodeCursor(first.nextCursorState),
        })).rejects.toBeInstanceOf(InvalidSessionTranscriptCursorError);
    });
    it('rejects malformed and unsupported-version cursors', async () => {
        const reader = new SessionTranscriptReader(workspaceDir);
        await expect(reader.readPage(sessionId, { cursor: 'not-a-cursor' })).rejects.toBeInstanceOf(InvalidSessionTranscriptCursorError);
        await writeRecords([
            record('u1', null, 'root'),
            record('a1', 'u1', 'assistant'),
        ]);
        ...
        await expect(reader.readPage(sessionId, { cursor: wrongVersion })).rejects.toBeInstanceOf(InvalidSessionTranscriptCursorError);
    });
    it('rejects a cursor after the frozen snapshot is truncated', async () => {
        ...
        await expect(reader.readPage(sessionId, {
            cursor: encodeCursor(first.nextCursorState),
    ...
    it('uses an injected in-memory codec without creating a cursor key file', async () => {
        ...
        key.fill(9);
        const sameOriginalKey = new SessionTranscriptCursorCodec(Buffer.alloc(32, 7));
        ...
        await expect(fs.stat(path.join(new Storage(workspaceDir).getProjectDir(), 'session-transcript-cursor-key'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it('rejects in-memory cursors signed with another key or tampered', () => {
        ...
        expect(() => second.decode(cursor)).toThrow(InvalidSessionTranscriptCursorError);
        expect(() => first.decode(`${cursor.slice(0, -1)}A`)).toThrow(InvalidSessionTranscriptCursorError);
    });
    it('rejects an invalid in-memory cursor key length', () => {
        expect(() => new SessionTranscriptCursorCodec(Buffer.alloc(31))).toThrow(/must be 32 bytes/);
    });
    it('warns and replaces a corrupt persisted cursor signing key', async () => {
        ...
        expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining('invalid cursor signing key'));
        const replacement = Buffer.from((await fs.readFile(keyPath, 'utf8')).trim(), 'base64url');
    ...
    it('rejects cursors signed for another workspace', async () => {
        ...
        await expect(otherReader.readPage(sessionId, { cursor })).rejects.toBeInstanceOf(InvalidSessionTranscriptCursorError);
    });
    it('does not duplicate same-uuid fragments parsed from one glued JSONL line', async () => {
        ...
        await writeRawTranscript(`${JSON.stringify(first)}${JSON.stringify(second)}\n` +
    ...
    it('skips non-ChatRecord JSON lines while indexing', async () => {
        await writeRawTranscript(`${JSON.stringify({ event: 'metadata' })}\n${JSON.stringify(record('u1', null, 'hello'))}\n`);
        const page = await new SessionTranscriptReader(workspaceDir).readPage(sessionId);
        expect(page.records.map((item) => item.uuid)).toEqual(['u1']);
    });
    it('raises snapshot-unavailable when a same-size in-place rewrite reuses a cached segment', async () => {
        ...
        await fs.writeFile(filePath, initial.replace('"uuid":"u1"', '"uuid":"x1"'), 'utf8');
        await fs.utimes(filePath, fixed, fixed);
        await expect(reader.readPage(sessionId)).rejects.toBeInstanceOf(SessionTranscriptSnapshotUnavailableError);
    });
    it('does not repeatedly copy pending bytes for one large record', async () => {
        const largeRecord = record('u1', null, 'x'.repeat(512 * 1024));
        const filePath = await writeRawTranscript(`${JSON.stringify(largeRecord)}\n`);
    ...
    it('does not evict cached indexes when a new index exceeds the byte budget alone', async () => {
        ...
        for (let i = 0; i < 20; i++) {
            const uuid = `large-${i}`;
            largeRecords.push(record(uuid, parentUuid, `large transcript ${i}`, largeSessionId));
            parentUuid = uuid;
        }
    ...
    it('evicts the least-recently-used index after 32 cached sessions', async () => {
        ...
        for (let index = 0; index < 33; index++) {
            const targetSessionId = `00000000-0000-0000-0000-${index
                .toString(16)
                .padStart(12, '0')}`;
            await writeRecords([record(`u${index}`, null, `record ${index}`, targetSessionId)], targetSessionId);
    ...
    it('rejects tampered cursor snapshots before cache lookup', async () => {
        ...
        const reader = new SessionTranscriptReader(workspaceDir);
        const first = await reader.readPage(sessionId, { limit: 1 });
        const decoded = JSON.parse(Buffer.from(encodeCursor(first.nextCursorState), 'base64url').toString('utf8'));
        const tampered = Buffer.from(JSON.stringify({
            ...decoded,
            snapshotSize: 1,
        }), 'utf8').toString('base64url');
        await expect(reader.readPage(sessionId, { cursor: tampered })).rejects.toBeInstanceOf(InvalidSessionTranscriptCursorError);
    });
    ...
    it('reflects an in-place same-size rewrite once the mtime advances', async () => {
        ...
        const reader = new SessionTranscriptReader(workspaceDir);
        expect((await reader.readPage(sessionId, { limit: 100 })).records.map((r) => r.uuid)).toEqual(['11111111', '22222222']);
        // Same inode, identical byte length (8-char uuids), new content + mtime.
        await fs.writeFile(filePath, [mk('33333333', null), mk('44444444', '33333333')]
            .map((r) => JSON.stringify(r))
            .join('\n') + '\n', 'utf8');
        const later = new Date(Date.now() + 60_000);
        await fs.utimes(filePath, later, later);
        expect((await reader.readPage(sessionId, { limit: 100 })).records.map((r) => r.uuid)).toEqual(['33333333', '44444444']);
    });
});
//# sourceMappingURL=session-transcript-reader.test.js.map