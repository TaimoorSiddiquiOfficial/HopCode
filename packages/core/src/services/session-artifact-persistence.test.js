...
import { SESSION_ARTIFACT_PERSISTENCE_VERSION, normalizeEventPayload, normalizeSnapshotPayload, rebuildSessionArtifactSnapshot, remapSessionArtifactPayloadForFork, stableSessionArtifactId, } from './session-artifact-persistence.js';
function artifact(sessionId, url, overrides = {}) {
...
describe('session artifact persistence records', () => {
    ...
    it('skips stale events at or before the latest snapshot sequence', () => {
        ...
        expect(snapshot?.warnings).toContain('skipped stale event sequence 9 at or before snapshot sequence 10');
    });
    ...
    it('warns when oversized metadata is stripped during restore', () => {
        ...
        expect(restored?.warnings).toEqual([
            `skipped oversized metadata for artifact ${stableSessionArtifactId('s1', 'url:https://example.com/metadata')}`,
        ]);
    ...
    it('filters prototype metadata keys during restore normalization', () => {
                ...
                changes: [
                    {
                        ...
                        artifact: artifact('s1', 'https://example.com/prototype', {
                            metadata: JSON.parse('{"__proto__":null,"constructor":"blocked","prototype":"blocked","safe":"ok"}'),
                        }),
                    },
                ],
            }),
        ...
        expect(Object.prototype.hasOwnProperty.call(metadata, 'constructor')).toBe(false);
        expect(Object.prototype.hasOwnProperty.call(metadata, 'prototype')).toBe(false);
    });
    ...
    it('remaps forked payloads to the new session without carrying pinned content', () => {
        ...
        }, 'source-session', 'forked-session');
        const forked = remapped.changes[0]?.artifact;
        expect(remapped.sessionId).toBe('forked-session');
        expect(forked).toMatchObject({
            id: stableSessionArtifactId('forked-session', 'url:https://example.com/report'),
            retention: 'restorable',
        });
    ...
    it('remaps forked tombstone changes when artifact metadata is present', () => {
        ...
        expect(remapped.changes).toEqual([
            expect.objectContaining({
                action: 'removed',
                artifactId: stableSessionArtifactId('forked-session', 'url:https://example.com/deleted'),
                reason: 'unpin_to_ephemeral',
            }),
        ]);
        expect(remapped.changes[0]?.artifact).toMatchObject({
            id: stableSessionArtifactId('forked-session', 'url:https://example.com/deleted'),
            retention: 'restorable',
        });
        expect(remapped.changes[0]?.artifact).not.toHaveProperty('restoreState');
        expect(remapped.changes[0]?.artifact).not.toHaveProperty('persistenceWarning');
    ...
    it('drops unsafe forked event artifact payloads but keeps safe identity tombstones', () => {
        const unsafe = artifact('source-session', 'https://example.com/deleted-unsafe-event', {
        ...
        expect(remapped.changes).toEqual([
            {
                action: 'removed',
                artifactId: stableSessionArtifactId('forked-session', 'url:https://example.com/deleted-unsafe-event'),
                reason: 'explicit',
            },
        ]);
    });
    it('drops forked event artifact payloads with encoded secret fragments', () => {
        const unsafe = artifact('source-session', 'https://example.com/report#access%5Ftoken=sk-abcdefghijkl');
        const remapped = remapSessionArtifactPayloadForFork({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: 'source-session',
            sequence: 6,
    ...
    it('remaps forked tombstone changes that omit artifact metadata', () => {
        ...
        const forkedId = stableSessionArtifactId('forked-session', 'url:https://example.com/deleted');
    ...
    it('remaps forked non-removal changes that omit artifact metadata', () => {
        const sourceId = stableSessionArtifactId('source-session', 'fork:source-session:metadata-less');
        const remapped = remapSessionArtifactPayloadForFork({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: 'source-session',
            sequence: 7,
        ...
        expect(remapped.changes).toEqual([
            {
                action: 'updated',
                artifactId: stableSessionArtifactId('forked-session', `fork:source-session:${sourceId}`),
            },
        ]);
    });
    it('reuses remapped ids across separate forked event payloads', () => {
        const source = artifact('source-session', 'https://example.com/deleted-later');
        const remappedIds = new Map();
        const forkedId = stableSessionArtifactId('forked-session', 'url:https://example.com/deleted-later');
        ...
        }, 'source-session', 'forked-session', remappedIds);
    ...
    it('remaps forked snapshot payloads and marker state', () => {
        ...
        const deleted = artifact('source-session', 'https://example.com/deleted-in-source');
        const sticky = artifact('source-session', 'https://example.com/ephemeral-in-source');
        const remapped = remapSessionArtifactPayloadForFork({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: 'source-session',
            sequence: 7,
        ...
        const forkedSourceId = stableSessionArtifactId('forked-session', 'url:https://example.com/snapshot');
        const forkedDeletedId = stableSessionArtifactId('forked-session', 'url:https://example.com/deleted-in-source');
        const forkedStickyId = stableSessionArtifactId('forked-session', 'url:https://example.com/ephemeral-in-source');
        expect(remapped.sessionId).toBe('forked-session');
    ...
    it('drops unsafe snapshot marker artifacts but keeps safe identity tombstones', () => {
        const unsafe = artifact('source-session', 'https://example.com/deleted-unsafe', {
            metadata: { apiKey: 'redacted' },
        });
        const safe = artifact('source-session', 'https://example.com/deleted-safe', {
            metadata: { label: 'safe' },
        });
        const remapped = remapSessionArtifactPayloadForFork({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
        ...
        const forkedSafeId = stableSessionArtifactId('forked-session', 'url:https://example.com/deleted-safe');
        const forkedUnsafeId = stableSessionArtifactId('forked-session', 'url:https://example.com/deleted-unsafe');
    ...
    it('falls back for snapshot marker ids without identity metadata', () => {
        ...
        expect(remapped.tombstonedIds).toEqual([
            stableSessionArtifactId('forked-session', 'fork:source-session:deleted-in-source'),
        ]);
        expect(remapped.stickyEphemeralIds).toEqual([
            stableSessionArtifactId('forked-session', 'fork:source-session:ephemeral-in-source'),
        ]);
        expect(remapped.markerArtifacts).toBeUndefined();
    });
    it('does not keep unremapped source marker artifacts in forked snapshots', () => {
        const staleMarker = artifact('source-session', 'https://example.com/stale-marker');
        const remapped = remapSessionArtifactPayloadForFork({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: 'source-session',
            sequence: 7,
    ...
    it('normalizes inbound snapshot payloads with bounded artifacts and sticky ids', () => {
        const warnings = [];
        const artifacts = Array.from({ length: 501 }, (_, index) => artifact('session-A', `https://example.com/${index}`));
        const markerArtifacts = Array.from({ length: 1001 }, (_, index) => artifact('session-A', `https://example.com/marker-${index}`));
        const snapshot = normalizeSnapshotPayload({
            ...
            artifacts,
            markerArtifacts,
            stickyEphemeralIds: Array.from({ length: 501 }, (_, index) => `sticky-${index}`),
        }, warnings);
        ...
        expect(warnings).toContain('snapshot marker artifact list truncated to 1000');
    });
    ...
    it('warns when inbound artifact payloads are malformed', () => {
        const warnings = [];
        expect(normalizeSnapshotPayload({ v: SESSION_ARTIFACT_PERSISTENCE_VERSION }, warnings)).toBeUndefined();
        expect(normalizeEventPayload({
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: 'session-A',
            sequence: 1,
            recordedAt: '2026-07-04T00:00:00.000Z',
            changes: [{ action: 'created' }],
        }, warnings)?.changes).toEqual([]);
        expect(warnings).toContain('skipped snapshot record without artifacts array');
...
//# sourceMappingURL=session-artifact-persistence.test.js.map