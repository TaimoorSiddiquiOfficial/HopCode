...
export function rebuildSessionArtifactSnapshot(records, fallbackSessionId) {
    ...
    const stickyEphemeralIds = new Set();
    const markerArtifacts = new Map();
    const warnings = [];
    let sequence = 0;
    ...
    for (const record of records) {
        ...
        if (payload.sequence <= lastSnapshotSequence) {
            warnings.push(`skipped stale event sequence ${payload.sequence} at or before snapshot sequence ${lastSnapshotSequence}`);
            continue;
        }
        sequence = Math.max(sequence, payload.sequence);
