...
function clearEnv() {
    delete process.env['HOPCODE_MC_KEEP_RECENT'];
}
...
function makeFileToolCall(id, filePath) {
    return {
        role: 'model',
        parts: [
            {
...
function makeFileToolResult(id, output) {
    return {
        role: 'user',
        parts: [
            {
...
function makeFileToolErrorResult(id, error) {
    return {
        role: 'user',
        parts: [
            {
...
describe('microcompactHistory', () => {
    ...
    it('preserves managed-memory reads while clearing ordinary reads', () => {
        ...
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe('durable testing guidance');
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.meta.toolsCleared).toBe(1);
    ...
    it('preserves managed-memory reads during size-based clearing', () => {
        ...
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe('durable guidance '.repeat(20));
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.meta.toolResultCharsBefore).toBeGreaterThan('durable guidance '.repeat(20).length);
    });
    ...
    it('does not charge protected memory against the recent-result budget', () => {
        ...
        expect(result.meta.toolsCleared).toBe(0);
        expect(result.meta.toolsKept).toBe(1);
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(ordinaryContent);
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(memoryContent);
    });
    ...
    it('does not preserve a read when a reused call id maps to mixed paths', () => {
        ...
        expect(result.history[2].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
    ...
    it('does not preserve error responses for managed-memory reads', () => {
        ...
        expect(result.meta).toBeUndefined();
        expect(result.history[1].parts[0].functionResponse.response['error']).toBe('ENOENT');
    });
    ...
    it('uses integer HOPCODE_MC_KEEP_RECENT values over settings', () => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = '3';
    ...
    it.each(['0', '-2'])('floors integer HOPCODE_MC_KEEP_RECENT=%s to 1', (envValue) => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = envValue;
        const history = [
            makeToolCall('read_file'),
            makeToolResult('read_file', 'old content'),
            makeToolCall('grep_search'),
    ...
    it('ignores fractional HOPCODE_MC_KEEP_RECENT values', () => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = '1.5';
        const history = [
    ...
    it('falls back to settings when HOPCODE_MC_KEEP_RECENT is fractional', () => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = '1.5';
        const history = Array.from({ length: 4 }).flatMap((_, i) => [
            makeToolCall('read_file'),
            makeToolResult('read_file', `content ${i}`),
    ...
    it('checks env integer syntax before numeric conversion', () => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = '9007199254740990.5';
        const history = [
            makeToolCall('read_file'),
            makeToolResult('read_file', 'old content'),
            makeToolCall('grep_search'),
    ...
    it('ignores unsafe integer HOPCODE_MC_KEEP_RECENT values', () => {
        process.env['HOPCODE_MC_KEEP_RECENT'] = '9007199254740992';
        const history = [
            makeToolCall('read_file'),
            makeToolResult('read_file', 'old content'),
            makeToolCall('grep_search'),
    ...
    it('uses the default keepRecent when settings are not a safe integer', () => {
        const history = Array.from({ length: 6 }).flatMap((_, i) => [
            makeToolCall('read_file'),
            makeToolResult('read_file', `content ${i}`),
        ]);
        const result = microcompactHistory(history, twoHoursAgo, {
    ...
    it('size-compacts old tool results even when the idle trigger has not fired', () => {
        const history = [];
        for (let i = 0; i < 167; i++) {
            history.push(makeToolCall('run_shell_command'), makeToolResult('run_shell_command', 'Y'.repeat(25_500)));
        }
        const result = microcompactHistory(history, Date.now(), {
        ...
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history.at(-1).parts[0].functionResponse.response['output']).toBe('Y'.repeat(25_500));
    });
    it('size-compacts old skill results and keeps the most recent result', () => {
        ...
        expect(result.meta.toolsKept).toBe(1);
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(recentSkillContent);
    });
    it('counts pending content as a virtual tail for size-triggered compaction', () => {
        const history = [];
        for (let i = 0; i < 4; i++) {
            history.push(makeToolCall('run_shell_command'), makeToolResult('run_shell_command', 'Y'.repeat(120_000)));
        }
        const result = microcompactHistory(history, Date.now(), {
            toolResultsThresholdMinutes: 60,
            toolResultsNumToKeep: 1,
    ...
    it('does not clear media or non-compactable tool results for size overages', () => {
        ...
        expect(result.history[2].parts[0].functionResponse.response['output']).toBe('answer'.repeat(50_000));
        expect(result.history[4].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
    });
    it('does not size-compact errors or already-cleared results', () => {
        ...
        expect(result.meta.triggerReason).toBe('size');
        expect(result.meta.toolsCleared).toBe(1);
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe('E'.repeat(500_000));
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history[5].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history.at(-1).parts[0].functionResponse.response['output']).toBe('C'.repeat(200_000));
    });
    it('treats a negative legacy idle threshold as disabling the size trigger when unset', () => {
        const history = [];
        for (let i = 0; i < 20; i++) {
            history.push(makeToolCall('run_shell_command'), makeToolResult('run_shell_command', 'X'.repeat(30_000)));
        }
        const result = microcompactHistory(history, Date.now(), {
...
describe('microcompactHistory — force option', () => {
    ...
    it('force: true works even when threshold is disabled (-1)', () => {
        ...
        const result = microcompactHistory(history, null, { toolResultsThresholdMinutes: -1, toolResultsNumToKeep: 1 }, { force: true });
        expect(result.meta).toBeDefined();
        expect(result.meta.toolsCleared).toBeGreaterThanOrEqual(1);
    });
