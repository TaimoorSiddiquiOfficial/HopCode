...
describe('ChatRecordingService - recordParentSession', () => {
    ...
    it('records immutable session source metadata', async () => {
        const first = await chatRecordingService.recordSessionSource('scheduled_task', 'task-123');
        const second = await chatRecordingService.recordSessionSource('scheduled_task', 'task-123');
        await chatRecordingService.flush();
    ...
    it('rejects a different session source after the first write', async () => {
        await expect(chatRecordingService.recordSessionSource('scheduled_task', 'task-123')).resolves.toBe(true);
        await expect(chatRecordingService.recordSessionSource('web_shell', 'window-1')).resolves.toBe(false);
        await chatRecordingService.flush();
        expect(jsonl.writeLine).toHaveBeenCalledOnce();
    });
    it('reports a session source write failure', async () => {
        ...
        await expect(chatRecordingService.recordSessionSource('scheduled_task', 'task-123')).resolves.toBe(false);
    ...
    it('reports failure without retrying after the recorder degrades', async () => {
        ...
        await expect(chatRecordingService.recordParentSession('parent-abc')).resolves.toBe(false);
        await expect(chatRecordingService.flush()).rejects.toBe(writeError);
        await expect(chatRecordingService.recordParentSession('parent-abc')).resolves.toBe(false);
        expect(jsonl.writeLine).toHaveBeenCalledOnce();
    });
});
//# sourceMappingURL=chatRecordingService.parentSession.test.js.map