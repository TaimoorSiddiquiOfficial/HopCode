...
describe('FileDiscoveryService', () => {
    ...
    describe('initialization', () => {
        ...
        it('should load .hopcodeignore patterns even when not in a git repo', async () => {
            await createTestFile('.hopcodeignore', 'secrets.txt');
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldHopCodeIgnoreFile('secrets.txt')).toBe(true);
            expect(service.shouldHopCodeIgnoreFile('src/index.js')).toBe(false);
        });
    });
    describe('filterFiles', () => {
        beforeEach(async () => {
            await fs.mkdir(path.join(projectRoot, '.git'));
            await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
            await createTestFile('.hopcodeignore', 'logs/');
        });
        it('should filter out git-ignored and hopcode-ignored files by default', () => {
            const files = [
                'src/index.ts',
        ...
        it('should not filter files when respectGitIgnore is false', () => {
            ...
            const filtered = service.filterFiles(files, {
                ...
                respectHopCodeIgnore: true, // still respect this one
            });
            expect(filtered).toEqual(['src/index.ts', 'node_modules/package/index.js', '.git/config'].map((f) => path.join(projectRoot, f)));
        });
        it('should not filter files when respectHopCodeIgnore is false', () => {
            const files = [
                'src/index.ts',
                'node_modules/package/index.js',
                'logs/latest.log',
    ...
    describe('shouldGitIgnoreFile & shouldHopCodeIgnoreFile', () => {
        beforeEach(async () => {
            await fs.mkdir(path.join(projectRoot, '.git'));
            await createTestFile('.gitignore', 'node_modules/');
            await createTestFile('.hopcodeignore', '*.log');
        });
        ...
        it('should return true for hopcode-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldHopCodeIgnoreFile(path.join(projectRoot, 'debug.log'))).toBe(true);
        });
        it('should return false for non-hopcode-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldHopCodeIgnoreFile(path.join(projectRoot, 'src/index.ts'))).toBe(false);
        });
    });
    describe('edge cases', () => {
