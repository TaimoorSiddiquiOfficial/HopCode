...
export interface WorktreeSession {
   ...
   * always lives under `<repoTopLevel>/.hopcode/worktrees/`. When the
...
function shouldSkipRuntimeStatusScanDir(name: string, parent: string): boolean {
  ...
  return name === 'worktrees' && path.basename(parent) === '.hopcode';
}
...
export async function restoreWorktreeContext(
  ...
  // `<originalCwd>/.hopcode/worktrees/`. Schema validation (readWorktreeSession)
