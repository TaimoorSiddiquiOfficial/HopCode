...
describe('loadUsageDashboard', () => {
     ...
     */
    function plantTranscript(sessionId, totalTokens) {
        const cwd = '/daemon/project';
        const ts = new Date().toISOString();
        const chatsDir = path.join(process.env['QWEN_HOME'], 'projects', 'daemon-project', 'chats');
        ...
        fs.writeFileSync(path.join(chatsDir, `${sessionId}.jsonl`), records.map((r) => JSON.stringify(r)).join('\n') + '\n');
    }
...
//# sourceMappingURL=usage-dashboard-service.test.js.map