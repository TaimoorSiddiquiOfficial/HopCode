...
function createHelpChild(output) {
    const child = new EventEmitter();
...
function createHarness(platform = 'linux', env = {}, noAskPasswordSupported = true) {
    const children = [];
    ...
    const spawn = vi.fn((command, args, _options) => {
...
describe('SleepInhibitor', () => {
    it('starts systemd-inhibit on linux and stops it after the final release', async () => {
        ...
        expect(spawn).toHaveBeenCalledWith('systemd-inhibit', [
            '--no-ask-password',
            '--what=sleep',
            '--who=HopCode',
            '--why=working',
    ...
    it('skips systemd-inhibit for headless SSH sessions on Linux', () => {
        ...
        expect(inhibitor.isRunning()).toBe(false);
        expect(inhibitor.getActiveCount()).toBe(2);
        expect(logger.debug).toHaveBeenCalledWith('Sleep inhibition skipped for headless SSH session.');
    ...
    it('starts systemd-inhibit for SSH sessions with a display server', async () => {
        ...
        await vi.waitFor(() => expect(spawn).toHaveBeenCalledTimes(2));
        expect(spawn).toHaveBeenCalledWith('systemd-inhibit', expect.arrayContaining(['--what=sleep']), expect.any(Object));
        handle.release();
    });
    ...
    it('forwards a curated environment instead of an empty env', async () => {
        ...
        await vi.waitFor(() => expect(spawn).toHaveBeenCalledTimes(2));
        const env = spawn.mock.calls[1][2].env;
    ...
    it('uses caffeinate on macOS', () => {
        ...
        const handle = inhibitor.acquire();
        expect(spawn).toHaveBeenCalledWith('caffeinate', ['-is'], expect.any(Object));
        handle.release();
    });
    it('uses a PowerShell SetThreadExecutionState helper on Windows', () => {
        const { inhibitor, spawn } = createHarness('win32');
        const handle = inhibitor.acquire();
        expect(spawn).toHaveBeenCalledWith('powershell.exe', expect.arrayContaining([
    ...
    it('fails open when spawning throws', () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith('Failed to spawn sleep inhibitor: missing command');
        expect(inhibitor.getActiveCount()).toBe(0);
    });
    it('handles async error events from the spawned child', async () => {
        ...
        expect(inhibitor.isRunning()).toBe(false);
        expect(logger.debug).toHaveBeenCalledWith('Failed to start sleep inhibitor: EPERM');
        handle.release();
    });
    it('restarts after an unexpected exit when acquired again', async () => {
        ...
        expect(inhibitor.isRunning()).toBe(false);
        expect(logger.debug).toHaveBeenCalledWith('Sleep inhibitor exited while active: code=1 signal=null');
    ...
    it('does not propagate when child.kill() throws during release', async () => {
        ...
        await vi.waitFor(() => expect(children).toHaveLength(1));
        expect(() => handle.release()).not.toThrow();
        expect(logger.warn).toHaveBeenCalledWith('Failed to stop sleep inhibitor: ESRCH');
        expect(inhibitor.getActiveCount()).toBe(0);
    });
    ...
    it('ignores a late error event from an already-replaced child', async () => {
        ...
        expect(logger.debug).not.toHaveBeenCalledWith('Failed to start sleep inhibitor: ESRCH');
        expect(inhibitor.isRunning()).toBe(true);
        first.release();
        second.release();
    });
    it('latches on an unsupported platform so it only checks once', () => {
        const { inhibitor, logger, spawn } = createHarness('freebsd');
        const first = inhibitor.acquire();
        const second = inhibitor.acquire();
        expect(spawn).not.toHaveBeenCalled();
        expect(inhibitor.isRunning()).toBe(false);
        expect(logger.debug.mock.calls.filter((call) => String(call[0]).includes('unsupported on platform'))).toHaveLength(1);
        first.release();
        second.release();
    });
    ...
    describe('exitedWhileActiveLogged dedup latch', () => {
        it('logs exit-while-active exactly once per active run', async () => {
            ...
            expect(logger.debug.mock.calls.filter((call) => String(call[0]).includes('Sleep inhibitor exited while active:'))).toHaveLength(1);
            first.release();
            second.release();
        });
        it('resets the latch after all handles are released', async () => {
            ...
            expect(logger.debug).toHaveBeenCalledWith('Sleep inhibitor exited while active: code=2 signal=null');
            second.release();
        });
    });
});
//# sourceMappingURL=sleepInhibitor.test.js.map