...
describe('compactionInputSlimming', () => {
    ...
    describe('resolveCompactionTuning', () => {
        it('returns defaults when nothing is set', () => {
            ...
            expect(t.enableScreenshotTrigger).toBe(DEFAULT_SCREENSHOT_TRIGGER_ENABLED);
            expect(t.screenshotTriggerThreshold).toBe(DEFAULT_SCREENSHOT_TRIGGER_THRESHOLD);
        });
        it('honors settings when env is unset', () => {
        ...
        it('env overrides settings for every knob', () => {
            process.env['HOPCODE_COMPACT_MAX_RECENT_FILES'] = '7';
            process.env['HOPCODE_COMPACT_MAX_RECENT_IMAGES'] = '9';
            process.env['HOPCODE_COMPACT_SCREENSHOT_TRIGGER'] = '0';
            process.env['HOPCODE_COMPACT_SCREENSHOT_THRESHOLD'] = '99';
            const t = resolveCompactionTuning({
        ...
        it('rejects fractional count-like env values', () => {
            const cases = [
                {
                    envKey: 'HOPCODE_COMPACT_MAX_RECENT_FILES',
                    value: '1.5',
                    settings: { maxRecentFilesToRetain: 4 },
                    get: (t) => t.maxRecentFiles,
                    expected: 4,
                },
                {
                    envKey: 'HOPCODE_COMPACT_MAX_RECENT_IMAGES',
                    value: '2.5',
                    settings: { maxRecentImagesToRetain: 5 },
                    get: (t) => t.maxRecentImages,
                    expected: 5,
                },
                {
                    envKey: 'HOPCODE_COMPACT_SCREENSHOT_THRESHOLD',
                    value: '9007199254740990.5',
                    settings: { screenshotTriggerThreshold: 6 },
                    get: (t) => t.screenshotTriggerThreshold,
                    expected: 6,
                },
            ];
        ...
        it('rejects unsafe integer count-like values', () => {
            process.env['HOPCODE_COMPACT_MAX_RECENT_FILES'] = String(Number.MAX_SAFE_INTEGER + 1);
        ...
        it('parses the boolean env both ways and ignores typos', () => {
            process.env['HOPCODE_COMPACT_SCREENSHOT_TRIGGER'] = 'false';
            expect(resolveCompactionTuning(undefined).enableScreenshotTrigger).toBe(false);
            process.env['HOPCODE_COMPACT_SCREENSHOT_TRIGGER'] = '1';
            expect(resolveCompactionTuning(undefined).enableScreenshotTrigger).toBe(true);
            // Unrecognized env string falls through to the settings value.
            process.env['HOPCODE_COMPACT_SCREENSHOT_TRIGGER'] = 'yes-please';
            expect(resolveCompactionTuning({ enableScreenshotTrigger: false })
                .enableScreenshotTrigger).toBe(false);
        });
        it('falls through invalid numeric env to settings, then defaults', () => {
            process.env['HOPCODE_COMPACT_SCREENSHOT_THRESHOLD'] = 'not-a-number';
            expect(resolveCompactionTuning({ screenshotTriggerThreshold: 33 })
                .screenshotTriggerThreshold).toBe(33);
            // Threshold has a min of 1, so 0 is rejected → default.
            process.env['HOPCODE_COMPACT_SCREENSHOT_THRESHOLD'] = '0';
            expect(resolveCompactionTuning(undefined).screenshotTriggerThreshold).toBe(DEFAULT_SCREENSHOT_TRIGGER_THRESHOLD);
        });
    });
    describe('estimatePartChars', () => {
        ...
        it('counts model-facing function response errors', () => {
            ...
            expect(estimatePartChars({
                functionResponse: {
                    name: 'shell',
                    response: { error },
                },
            }, 1600)).toBe(error.length + 64);
        });
    });
