...
describe('GitWorktreeService', () => {
    ...
    it('createWorktree should create a sanitized branch and worktree path', async () => {
        ...
        const expectedPath = path.join('/mock-hopcode', 'worktrees', 's1', 'worktrees', 'model-a');
    ...
    describe('parsePRReference', () => {
        ...
        it('recognises full GitHub PR URLs (any host)', () => {
            expect(GitWorktreeService.parsePRReference('https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4174')).toBe(4174);
            expect(GitWorktreeService.parsePRReference('http://gh.enterprise.example.com/team/repo/pull/9')).toBe(9);
        });
        it('tolerates trailing slash, query string, and fragment', () => {
            expect(GitWorktreeService.parsePRReference('https://github.com/o/r/pull/123/')).toBe(123);
            expect(GitWorktreeService.parsePRReference('https://github.com/o/r/pull/123?foo=bar')).toBe(123);
            expect(GitWorktreeService.parsePRReference('https://github.com/o/r/pull/123#discussion_r999')).toBe(123);
        });
        it('returns null for plain slugs and malformed inputs', () => {
            ...
            expect(GitWorktreeService.parsePRReference('https://example.com/')).toBeNull();
            expect(GitWorktreeService.parsePRReference('https://github.com/o/r/issues/123')).toBeNull();
            expect(GitWorktreeService.parsePRReference('')).toBeNull();
        });
        it('safely handles non-string input', () => {
            expect(GitWorktreeService.parsePRReference(undefined)).toBeNull();
            expect(GitWorktreeService.parsePRReference(null)).toBeNull();
        });
    });
});
