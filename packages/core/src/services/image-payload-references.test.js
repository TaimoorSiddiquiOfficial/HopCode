...
import { InMemoryImagePayloadStore, buildReattachParts, countAllInlineImages, prepareImagePayloadsForRequest, replaceImagePayloadsInPlace, } from './image-payload-references.js';
function toolImageTurn(data) {
    return {
        role: 'user',
        parts: [
            {
...
describe('prepareImagePayloadsForRequest', () => {
    it('replaces historical image positions with stable refs and reattaches only the most recent images', () => {
        ...
        expect(prepared.at(-1)?.parts?.[1]?.text).toContain('Recent images reattached');
    });
    it('reattaches an older image when the current request explicitly references its stable id', () => {
        ...
        const firstPass = prepareImagePayloadsForRequest([oldImage, { role: 'model', parts: [{ text: 'ok' }] }], {
            maxRecentImages: 0,
            store,
        });
    ...
    it('reattaches a stored image when only its stable reference remains in history', () => {
        const store = new InMemoryImagePayloadStore();
        const firstPass = prepareImagePayloadsForRequest([toolImageTurn('old-shot'), { role: 'model', parts: [{ text: 'ok' }] }], {
            maxRecentImages: 0,
            store,
        });
        const id = JSON.stringify(firstPass).match(/Image #([a-f0-9]{12})/)?.[1];
        expect(id).toBeDefined();
        const prepared = prepareImagePayloadsForRequest([{ role: 'user', parts: [{ text: `inspect Image #${id}` }] }], {
            maxRecentImages: 0,
            store,
        });
        expect(imageParts(prepared).map((part) => part.inlineData?.data)).toEqual([
...
describe('replaceImagePayloadsInPlace', () => {
    ...
    it('skips the specified content entry', () => {
        ...
        const contents = [toolImageTurn('old-shot'), current];
...
//# sourceMappingURL=image-payload-references.test.js.map