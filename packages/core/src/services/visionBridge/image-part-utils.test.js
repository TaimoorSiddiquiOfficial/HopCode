...
import { collectText, hasImageParts, isImagePart, normalizeParts, replaceImagesWithText, splitImageParts, isUsableImagePart, } from './image-part-utils.js';
const imagePart = {
...
describe('isImagePart', () => {
    ...
    it('rejects an image part with empty data', () => {
        expect(isImagePart({ inlineData: { mimeType: 'image/png', data: '' } })).toBe(false);
    });
...
describe('isUsableImagePart', () => {
    ...
    it('rejects empty data', () => {
        expect(isUsableImagePart({ inlineData: { mimeType: 'image/png', data: '' } })).toBe(false);
    });
    it('rejects oversized data', () => {
        const huge = 'a'.repeat(11 * 1024 * 1024);
        expect(isUsableImagePart({ inlineData: { mimeType: 'image/png', data: huge } })).toBe(false);
    });
});
describe('collectText', () => {
    it('joins and trims text from parts, ignoring non-text', () => {
        expect(collectText([textPart, imagePart, { text: 'world' }])).toBe('hello\nworld');
    });
...
describe('replaceImagesWithText', () => {
    it('replaces the image in place, keeping surrounding parts in order', () => {
        ...
        expect(replaceImagesWithText([{ text: 'Content from a.png:' }, imagePart, { text: 'trailer' }], 'TRANSCRIPT')).toEqual([
    ...
    it('collapses multiple images into the first slot and drops the rest', () => {
        expect(replaceImagesWithText(['p', imagePart, 'q', imagePart], 'T')).toEqual([{ text: 'p' }, { text: 'T' }, { text: 'q' }]);
    });
...
//# sourceMappingURL=image-part-utils.test.js.map