...
export class ChatCompressionService {
    async compress(chat, opts) {
        const { promptId, force, model, config, consecutiveFailures, originalTokenCount, trigger, signal, } = opts;
        ...
        const summaryResult = await runSideQuery(config, {
            ...
            // session. See https://github.com/QwenLM/hopcode/issues/5861.
            stream: true,
            ...
            maxAttempts: 1,
            systemInstruction: buildCompressionSystemPrompt(opts.customInstructions, hookExtraInstructions),
            contents: [
                ...slim.slimmedHistory,
                {
                    role: 'user',
        ...
        if (!isSummaryEmpty &&
            ...
            config
                .getDebugLogger()
                .warn(`[chat-compression] summary output reached the ` +
        ...
        if (!isSummaryEmpty) {
            ...
            if (typeof compressionInputTokenCount === 'number' &&
                ...
                if (estimatedOriginalVisibleTokenCount > 0 &&
                    ...
                    config
                        .getDebugLogger()
                        .debug(`[chat-compression] usage metadata missing; estimated ` +
