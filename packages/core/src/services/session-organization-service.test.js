...
import { GROUP_COLOR_OPTIONS, SessionOrganizationService, } from './session-organization-service.js';
describe('SessionOrganizationService', () => {
    ...
    it('creates, updates, and rejects duplicate group names case-insensitively', async () => {
        ...
        await expect(service.createGroup({ name: 'frontend', color: 'green' })).rejects.toMatchObject({ code: 'group_name_conflict' });
    ...
    it('rejects invalid group names and colors', async () => {
        await expect(service.createGroup({ name: 'Bad\tName', color: 'blue' })).rejects.toMatchObject({
        ...
        await expect(service.createGroup({ name: '\u200b', color: 'blue' })).rejects.toMatchObject({
            code: 'invalid_group_name',
            field: 'name',
        });
        await expect(service.createGroup({ name: 'Bad\u202eName', color: 'blue' })).rejects.toMatchObject({
        ...
        await expect(service.createGroup({ name: 'Feature', color: 'pink' })).rejects.toMatchObject({
            code: 'invalid_group_color',
            field: 'color',
        });
        await expect(service.createGroup({ name: 'Short Hex', color: '#abc' })).rejects.toMatchObject({
            code: 'invalid_group_color',
            field: 'color',
        });
    });
    ...
    it('rejects creating more than 200 groups', async () => {
        ...
        await fs.writeFile(service.getStorePath(), JSON.stringify({
            schemaVersion: 1,
        ...
        await expect(service.createGroup({ name: 'Overflow', color: 'red' })).rejects.toMatchObject({ code: 'group_limit_reached' });
    });
    it('keeps groups with unsupported stored colors by falling back and warning once', async () => {
        ...
        expect(snapshot.sessions.get(sessionIdA)).toEqual(expect.objectContaining({
            groupId: 'group-future',
        }));
    ...
    it('unpins a session and clears pinnedAt', async () => {
        ...
        const snapshot = await service.readSnapshot();
        expect(snapshot.sessions.get(sessionIdA)).toEqual(expect.objectContaining({ isPinned: false }));
    });
    it('treats an empty session organization update as a no-op', async () => {
        ...
        await expect(fs.readFile(service.getStorePath(), 'utf8')).resolves.toBe(storeBefore);
    });
    it('rejects unknown group updates and assignments', async () => {
        await expect(service.updateGroup('missing-group', { name: 'Missing' })).rejects.toMatchObject({ code: 'group_not_found', field: 'groupId' });
        await expect(service.updateSessionOrganization(sessionIdA, {
    ...
    it('deleting a group clears session references without losing pinned state', async () => {
        ...
        expect(snapshot.sessions.get(sessionIdA)).toEqual(expect.objectContaining({ groupId: null, isPinned: true }));
        expect(snapshot.sessions.get(sessionIdB)).toEqual(expect.objectContaining({ groupId: null, isPinned: false }));
    });
    it('assigns and clears a quick color grouping tag', async () => {
        const assigned = await service.updateSessionOrganization(sessionIdA, {
            color: 'green',
        });
        expect(assigned).toEqual(expect.objectContaining({ color: 'green', groupId: null }));
        const snapshot = await service.readSnapshot();
        expect(snapshot.sessions.get(sessionIdA)).toEqual(expect.objectContaining({ color: 'green' }));
    ...
    it('keeps color, group, and pin independent in the store', async () => {
        ...
        expect(withColor).toEqual(expect.objectContaining({
            color: 'red',
            groupId: group.id,
            isPinned: true,
        }));
    ...
    it('normalizes unknown stored session colors to null', async () => {
        ...
        const snapshot = await service.readSnapshot();
        expect(snapshot.sessions.get(sessionIdA)).toEqual(expect.objectContaining({ color: null }));
    });
    ...
    it('treats a malformed sidecar as empty for reads and refuses to overwrite it', async () => {
        ...
        expect(warnings).toHaveLength(1);
        await expect(service.createGroup({ name: 'Fixed', color: 'orange' })).rejects.toMatchObject({
            code: 'session_organization_store_unreadable',
        });
        await expect(service.createGroup({ name: 'Fixed Again', color: 'orange' })).rejects.toThrow('Delete the file to reset session organization');
        await expect(fs.readFile(service.getStorePath(), 'utf8')).resolves.toBe('{not-json');
    });
    it('includes schema version details in unreadable store warnings', async () => {
        ...
        expect(warnings).toEqual([
            expect.stringContaining('store schema is unsupported (found schemaVersion 2, expected 1)'),
        ]);
    });
...
//# sourceMappingURL=session-organization-service.test.js.map