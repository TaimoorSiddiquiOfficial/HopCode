...
vi.mock('../config/storage.js', () => ({
    Storage: { getGlobalHopCodeDir: mockStorageDir },
}));
...
describe('FileHistoryService', () => {
    ...
    describe('trackEdit', () => {
        it('records the updated latest snapshot after tracking a file', async () => {
            ...
            const recordSnapshot = vi.fn((snapshot) => {
                recordedSnapshots.push(structuredClone(snapshot));
            });
            const recordingService = new FileHistoryService('test-session', true, projectDir, recordSnapshot);
            ...
            expect(recorded.trackedFileBackups['a.txt']).toEqual(expect.objectContaining({
        ...
        it('records again when a second file is tracked in the same snapshot', async () => {
            ...
            expect(recorded.trackedFileBackups['b.txt']).toEqual(expect.objectContaining({
                backupFileName: expect.any(String),
                version: 1,
            }));
        });
        it('swallows recorder errors after tracking a file', async () => {
            ...
            expect(recordSnapshot).toHaveBeenCalledTimes(1);
            expect(recordingService.getSnapshots()[0].trackedFileBackups['a.txt']).toEqual(expect.objectContaining({
                backupFileName: expect.any(String),
                version: 1,
            }));
        });
        ...
        it('heals a failed entry on the next trackEdit attempt', async () => {
            ...
            service = new FileHistoryService('test-session', true, projectDir, recordSnapshot);
            const file = join(projectDir, 'a.txt');
            ...
            expect(recordSnapshot).toHaveBeenCalledTimes(2);
            expect(recordedSnapshots[1]).toEqual(expect.objectContaining({
                promptId: 'p2',
    ...
    describe('restoreFromSnapshots', () => {
        ...
        it('records failed markers when restored backup files are missing', async () => {
            ...
            const fresh = new FileHistoryService('test-session', true, projectDir, recordSnapshot);
            fresh.restoreFromSnapshots([
                {
                    promptId: 'p1',
                    trackedFileBackups: {
            ...
            expect(recordSnapshot).toHaveBeenCalledTimes(1);
            expect(recordedSnapshots[0].trackedFileBackups['a.txt']?.failed).toBe(true);
        });
