...
let mockCurrentSessionId = undefined;
...
describe('LogToSpanProcessor', () => {
    ...
    it('drops the oldest spans when the buffer exceeds the configured limit', async () => {
        ...
        try {
            ...
            expect(stderrWrite).toHaveBeenCalledWith(expect.stringContaining('dropped 1 oldest span(s) since last warning, 1 total'));
            await processor.forceFlush();
    ...
    it('falls back to the default buffer size for invalid configured limits', async () => {
        ...
        try {
            ...
            expect(stderrWrite).not.toHaveBeenCalledWith(expect.stringContaining('buffer exceeded max size'));
        }
        finally {
            stderrWrite.mockRestore();
        }
    ...
    it('reports total dropped spans across overflow warnings', async () => {
        ...
        try {
            ...
            expect(stderrWrite).toHaveBeenNthCalledWith(1, expect.stringContaining('dropped 1 oldest span(s) since last warning, 1 total'));
            expect(stderrWrite).toHaveBeenNthCalledWith(2, expect.stringContaining('dropped 1 oldest span(s) since last warning, 2 total'));
        }
        finally {
            dateNow.mockRestore();
            stderrWrite.mockRestore();
    ...
    it('sets ERROR status for truthy error attributes', async () => {
        ...
        expect(JSON.stringify(exportedSpans[0].status)).not.toContain('connection refused');
    });
    ...
    it('falls back to getCurrentSessionId when log record has no session.id', async () => {
        ...
        expect(exportedSpans[0].spanContext().traceId).toBe(deriveTraceId('session-from-context'));
    });
    it('prefers log record session.id over getCurrentSessionId', async () => {
        ...
        await processor.forceFlush();
        const { deriveTraceId } = await import('./trace-id-utils.js');
        expect(exportedSpans[0].spanContext().traceId).toBe(deriveTraceId('fresh-session'));
    });
    describe('bridge skip-list (#3731 Phase 3)', () => {
        it('skips hopcode.subagent_execution when native subagent span is active', async () => {
            mockIsInNativeSubagentSpan = true;
            const logRecord = {
                body: 'subagent started',
                hrTime: [2000, 0],
                attributes: {
                    'event.name': 'hopcode.subagent_execution',
        ...
        it('bridges subagent_execution when no native span is active (e.g. runForkedAgent)', async () => {
            ...
            await processor.forceFlush();
            expect(exportedSpans).toHaveLength(1);
            expect(exportedSpans[0].name).toBe('hopcode.subagent_execution');
        });
        it('still bridges other events normally (e.g. hopcode.tool_call)', async () => {
            const logRecord = {
                body: 'tool call',
                hrTime: [3000, 0],
                attributes: {
                    'event.name': 'hopcode.tool_call',
                    tool_name: 'read_file',
                },
            };
            processor.onEmit(logRecord);
            ...
            expect(exportedSpans).toHaveLength(1);
            expect(exportedSpans[0].name).toBe('hopcode.tool_call');
        });
    });
    describe('export failure diagnostics', () => {
        function makeFailingProcessor(error) {
        ...
        it('falls back to error.name when message is empty (HTTP/2 / stripped reason phrase)', async () => {
            ...
            try {
                ...
                expect(stderrWrite).toHaveBeenCalledWith('[LogToSpan] export failed: code=1 error="OTLPExporterError" httpCode=403 data="Forbidden: invalid license"\n');
            }
            finally {
                stderrWrite.mockRestore();
            }
        ...
        it('reports error="unknown" when result.error is missing', async () => {
            ...
            try {
                await flushOne(processor);
                expect(stderrWrite).toHaveBeenCalledWith('[LogToSpan] export failed: code=1 error="unknown"\n');
            }
            finally {
                stderrWrite.mockRestore();
            }
        ...
        it('routes diagnostics to an injected sink without touching stderr', async () => {
            ...
            const stderrWrite = vi
                ...
                .mockImplementation(() => true);
            const failingExporter = {
                export: vi.fn((_spans, cb) => cb({ code: 1, error: new Error('boom') })),
                shutdown: vi.fn().mockResolvedValue(undefined),
                forceFlush: vi.fn().mockResolvedValue(undefined),
            };
            ...
            try {
                await flushOne(processor);
                expect(sink).toHaveBeenCalledWith('[LogToSpan] export failed: code=1 error="boom"');
        ...
        it('routes buffer-overflow warnings through the injected sink', async () => {
            ...
            expect(sink).toHaveBeenCalledWith(expect.stringContaining('[LogToSpan] buffer exceeded max size'));
        });
        it('routes export timeout through the injected sink', async () => {
            ...
            try {
                ...
                await flushPromise;
                expect(sink).toHaveBeenCalledWith(expect.stringMatching(/^\[LogToSpan] export timeout after \d+ms \(\d+ span\(s\)\)$/));
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('routes export-threw (synchronous exporter exception) through the injected sink', async () => {
            ...
            await flushOne(processor);
            expect(sink).toHaveBeenCalledWith('[LogToSpan] export threw: error="exporter exploded synchronously"');
        });
        it('surfaces httpCode/data when a sync-thrown error carries OTLPExporterError fields', async () => {
            ...
            await flushOne(processor);
            expect(sink).toHaveBeenCalledWith('[LogToSpan] export threw: error="Bad Request" httpCode=400 data="malformed payload"');
        });
        ...
        it('handles non-Error throws (e.g. throw "string") in the export-threw path', async () => {
            ...
            await flushOne(processor);
            expect(sink).toHaveBeenCalledWith('[LogToSpan] export threw: error="raw string thrown"');
        });
        it('keeps processing exports after the sink throws', async () => {
            ...
            const exportFn = vi.fn((_spans, cb) => cb({ code: 1, error: new Error('boom') }));
            processor = new LogToSpanProcessor({
                export: exportFn,
                shutdown: vi.fn().mockResolvedValue(undefined),
                forceFlush: vi.fn().mockResolvedValue(undefined),
