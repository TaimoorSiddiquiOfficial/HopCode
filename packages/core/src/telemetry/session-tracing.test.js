...
vi.mock('@opentelemetry/api', async () => {
    ...
    function createMockSpan(name, opts, parentCtx) {
        const record = {
            name,
            kind: opts?.kind ?? 0,
    ...
    const mockTracer = {
        startSpan: (name, opts, parentCtx) => createMockSpan(name, opts, parentCtx),
    };
    return {
        ...actual,
        ...
        trace: {
            ...
            getSpan: (ctx) => typeof ctx === 'object' && ctx !== null && '__activeSpan' in ctx
                ? ctx.__activeSpan
                : undefined,
            wrapSpanContext: actual.trace.wrapSpanContext,
        },
        context: {
            active: () => mockState.activeOtelSpan
...
describe('session-tracing', () => {
    ...
    describe('interaction spans', () => {
        ...
        it('defaults to ROOT_CONTEXT when no parentContext is provided', async () => {
            await withInteractionSpan(createMockConfig({ sessionId: 's' }), { promptId: 'p', model: 'm', messageType: 'cron' }, async () => { });
        ...
        it('runs scoped interaction spans without mutating the global interaction context', async () => {
            ...
            const result = await withInteractionSpan(config, {
                promptId: 'prompt-scoped',
                model: 'test-model',
                messageType: 'acp_prompt',
                parentContext: { parent: 'daemon' },
            }, async () => 'done');
            ...
            expect(mockSpans[0].attributes['hopcode.message_type']).toBe('acp_prompt');
            expect(mockSpans[0].ended).toBe(true);
        ...
        it('marks the interaction span ERROR when getResultStatus returns "error"', async () => {
            const config = createMockConfig();
            await withInteractionSpan(config, { promptId: 'p-cron-err', model: 'm', messageType: 'cron' }, async () => 'done', () => 'error');
            const span = mockSpans.find((s) => s.name === 'hopcode.interaction');
            expect(span?.attributes['hopcode.turn_status']).toBe('error');
        ...
        it('keeps a thrown error message instead of the generic error-status message', async () => {
            const config = createMockConfig();
            await expect(withInteractionSpan(config, { promptId: 'p-throw', model: 'm', messageType: 'cron' }, async () => {
    ...
    describe('LLM request spans', () => {
        ...
        it('marks standalone when no interaction is active', () => {
            const span = startLLMRequestSpan('m', 'p');
            endLLMRequestSpan(span, { success: true });
            expect(mockSpans[0].attributes['llm_request.context']).toBe('standalone');
        });
    ...
    describe('LLM request spans — Phase 4a (timing decomposition + GenAI dual-emit)', () => {
        ...
        it('endLLMRequestSpan omits output_tokens_per_second when sampling_ms == 0', () => {
            ...
            expect(mockSpans[0].attributes['sampling_ms']).toBe(0);
            expect(mockSpans[0].attributes['output_tokens_per_second']).toBeUndefined();
        });
    ...
    describe('LLM request spans — Phase 4c (recordApiRequestBreakdown wiring)', () => {
        ...
        it('records all 3 phases when config + ttftMs + requestSetupMs are present', () => {
            ...
            expect(mockMetrics.recordApiRequestBreakdown).toHaveBeenCalledWith(config, 50, { model: 'test-model', phase: 'request_preparation' });
            expect(mockMetrics.recordApiRequestBreakdown).toHaveBeenCalledWith(config, 200, { model: 'test-model', phase: 'network_latency' });
            expect(mockMetrics.recordApiRequestBreakdown).toHaveBeenCalledWith(config, 800, { model: 'test-model', phase: 'response_processing' });
        });
        ...
        it('skips RESPONSE_PROCESSING when samplingMs is 0 (ttftMs == duration)', () => {
            ...
            const phases = calls.map((c) => c[2].phase);
    ...
    describe('session.id derives from the owning session, not the process-global (#4602 review)', () => {
        ...
        it('stamps a tool.execution span with the owning session id via the tool span context', () => {
            ...
            const exec = mockSpans.find((s) => s.name === 'hopcode.tool.execution');
        ...
        it('stamps a blocked-on-user span with the owning session id via the tool parent', () => {
            ...
            endToolSpan(toolSpan, { success: true });
            const blocked = mockSpans.find((s) => s.name === 'hopcode.tool.blocked_on_user');
        ...
        it('isolates concurrent sessions: each tool span carries its own session id', async () => {
            ...
            await withInteractionSpan(createMockConfig({ sessionId: 'session-A' }), { promptId: 'pa', model: 'm', messageType: 'acp_prompt' }, async () => {
                endToolSpan(startToolSpan('Read', { 'tool.call_id': 'a1' }), {
                    success: true,
                });
            });
            await withInteractionSpan(createMockConfig({ sessionId: 'session-B' }), { promptId: 'pb', model: 'm', messageType: 'acp_prompt' }, async () => {
                endToolSpan(startToolSpan('Write', { 'tool.call_id': 'b1' }), {
                    success: true,
                });
            });
            const readSpan = mockSpans.find((s) => s.attributes['tool.name'] === 'Read');
            const writeSpan = mockSpans.find((s) => s.attributes['tool.name'] === 'Write');
    ...
    describe('tool execution sub-spans', () => {
        ...
        it('cancelled: true keeps status UNSET while still recording attributes (#4302)', () => {
            ...
            expect(record?.attributes['error']).toBe('Tool execution cancelled by user');
        });
    ...
    describe('blocked_on_user spans (#3731 Phase 2)', () => {
        it('parents the blocked span under the explicitly-passed tool span', () => {
            ...
            const blockedRecord = mockSpans.find((s) => s.name === 'hopcode.tool.blocked_on_user');
        ...
        it('handles concurrent blocked spans without findLast confusion', () => {
            ...
            const recordA = mockSpans.find((s) => s.name === 'hopcode.tool.blocked_on_user' &&
                s.attributes['tool.call_id'] === 'a');
            const recordB = mockSpans.find((s) => s.name === 'hopcode.tool.blocked_on_user' &&
        ...
        it('falls back to resolveParentContext when the tool span was already ended', () => {
            ...
            endToolSpan(toolSpan, { success: true });
            const blockedSpan = startToolBlockedOnUserSpan(toolSpan);
            expect(mockSpans.find((s) => s.name === 'hopcode.tool.blocked_on_user')).toBeDefined();
            endToolBlockedOnUserSpan(blockedSpan, { decision: 'proceed_once' });
        });
    });
    describe('hook spans (#3731 Phase 2)', () => {
        ...
        it('records shouldStop/hasAdditionalContext on PostToolBatch', () => {
            ...
            expect(hookRecord?.attributes['post_batch_stop_reason']).toBe('policy halt');
            expect(hookRecord?.statuses).toHaveLength(0);
        });
    ...
    describe('toolContext ALS lifecycle', () => {
        it('runInToolSpanContext scopes toolContext via run(), not enterWith', () => {
            ...
            const insideRecord = mockSpans.find((s) => s.name === 'hopcode.tool.execution' &&
                s.parentContext?.['__parentSpan']);
            expect(insideRecord).toBeDefined();
            // Outside context: should NOT have tool parent
            const outsideRecord = mockSpans.filter((s) => s.name === 'hopcode.tool.execution');
            expect(outsideRecord).toHaveLength(2);
            const noParent = outsideRecord.find((s) => !s.parentContext?.['__parentSpan']);
        ...
        it('endToolSpan without metadata preserves pre-set status', () => {
            ...
            toolSpan.setStatus({ code: SpanStatusCode.ERROR, message: 'hook blocked' });
    ...
    describe('OTel error resilience — span.end() must run on attribute/status failure', () => {
        ...
        it('endToolExecutionSpan: end() runs when setAttributes throws', () => {
            ...
            const execRecord = mockSpans.find((s) => s.name === 'hopcode.tool.execution');
            mockState.throwOnSetAttributes = true;
            endToolExecutionSpan(execSpan, { success: true });
        ...
        it('endSubagentSpan: end() runs and activeSpans is cleared when setAttributes throws', () => {
            ...
            const record = mockSpans.find((s) => s.name === 'hopcode.subagent');
            mockState.throwOnSetAttributes = true;
    ...
    describe('TTL safety net (#4321 review)', () => {
        it('marks stale spans with ttl_expired + duration_ms before ending them', () => {
            ...
            expect(record.attributes['hopcode.span.duration_ms']).toBeGreaterThanOrEqual(31 * 60 * 1000 - 1000);
    ...
    describe('truncateSpanError (#4321 review)', () => {
        ...
        it('backs up one code unit when the cut would split a surrogate pair (#4321)', () => {
            ...
            const lastCharCode = lastBeforeSentinel.charCodeAt(lastBeforeSentinel.length - 1);
    ...
    describe('subagent spans (#3731 Phase 3)', () => {
        ...
        it('foreground invocation creates a child span (no root flag, no links)', () => {
            ...
            expect(record.attributes['gen_ai.provider.name']).toBe('hopcode');
            expect(record.attributes['gen_ai.conversation.id']).toBe('session-uuid');
            // Vendor concept attrs.
            expect(record.attributes['hopcode.subagent.invocation_kind']).toBe('foreground');
            expect(record.attributes['hopcode.subagent.is_built_in']).toBe(true);
            expect(record.attributes['hopcode.subagent.depth']).toBe(0);
            endSubagentSpan(span, { status: 'completed' });
        });
        it('fork invocation creates a linked-root span (root: true + Link to invoker)', () => {
            ...
            const span = startSubagentSpan({
                ...baseOpts,
                invocationKind: 'fork',
                invokerSpanContext: fakeInvokerSpanContext,
            });
            const record = mockSpans.find((s) => s.name === 'hopcode.subagent');
            ...
            expect(record.links[0].attributes?.['hopcode.link.kind']).toBe('invoker');
            expect(record.attributes['hopcode.subagent.invocation_kind']).toBe('fork');
            endSubagentSpan(span, { status: 'completed' });
        });
        it('background invocation is also linked-root', () => {
            ...
            expect(record.attributes['hopcode.subagent.invocation_kind']).toBe('background');
            endSubagentSpan(span, { status: 'completed' });
        });
        it('captures optional attrs: parentAgentId, invokingRequestId, modelOverride', () => {
            const span = startSubagentSpan({
                ...
                modelOverride: 'HopCode-7b',
                depth: 2,
            });
            const record = mockSpans.find((s) => s.name === 'hopcode.subagent');
            expect(record.attributes['hopcode.subagent.parent_agent_id']).toBe('parent-agent-456');
            expect(record.attributes['hopcode.subagent.invoking_request_id']).toBe('req-789');
            expect(record.attributes['gen_ai.request.model']).toBe('HopCode-7b');
            expect(record.attributes['hopcode.subagent.depth']).toBe(2);
            endSubagentSpan(span, { status: 'completed' });
        });
        it('endSubagentSpan: completed → SpanStatus OK + duration recorded', () => {
            ...
            expect(record.attributes['hopcode.subagent.status']).toBe('completed');
            expect(record.attributes['hopcode.subagent.duration_ms']).toBeGreaterThanOrEqual(0);
        });
        it('endSubagentSpan: failed → SpanStatus ERROR + exception.message + error.type', () => {
            ...
            expect(record.attributes['hopcode.subagent.status']).toBe('failed');
        });
        ...
        it.each(['cancelled', 'aborted'])('endSubagentSpan: %s → SpanStatus UNSET (Phase 2 cancellation convention)', (status) => {
            const span = startSubagentSpan({
                ...baseOpts,
                invocationKind: 'foreground',
            });
            ...
            expect(record.statuses).toHaveLength(0);
            expect(record.attributes['hopcode.subagent.status']).toBe(status);
        });
        ...
        it('returns NOOP_SPAN when SDK is uninitialized', () => {
            ...
            expect(mockSpans.find((s) => s.name === 'hopcode.subagent')).toBeUndefined();
        ...
        it('error message is truncated via truncateSpanError', () => {
            ...
            const record = mockSpans.find((s) => s.name === 'hopcode.subagent');
            const recorded = record.attributes['exception.message'];
        ...
        it('TTL: fork subagent at 30 min stays alive (4h window)', () => {
            ...
            expect(record.ended).toBe(true);
            expect(record.attributes['hopcode.span.ttl_expired']).toBe(true);
            expect(record.attributes['hopcode.subagent.status']).toBe('aborted');
            expect(record.attributes['hopcode.subagent.terminate_reason']).toBe('ttl_swept');
            // TTL sweep stamps the subagent-namespaced duration_ms key so
            // dashboards querying that namespace include swept spans (the
            // generic hopcode.span.duration_ms is asserted above).
            // wenshao @ #4410 DeepSeek 3292560017.
            expect(record.attributes['hopcode.subagent.duration_ms']).toBeGreaterThan(0);
        });
        ...
        describe('child span parenting (#4410 DeepSeek 3290820352)', () => {
            ...
            it('startToolSpan inside runInSubagentSpanContext parents under the subagent span', async () => {
                ...
                const parentSpan = toolRecord.parentContext?.__parentSpan;
                expect(parentSpan).toBe(subagentRecord);
                endSubagentSpan(subagentSpan, { status: 'completed' });
                endInteractionSpan('ok');
            });
            it('startHookSpan inside runInSubagentSpanContext (no inner tool) parents under the subagent span', async () => {
                ...
                const hookRecord = mockSpans.find((s) => s.name === 'hopcode.hook');
                expect(hookRecord).toBeDefined();
                const parentSpan = hookRecord.parentContext?.__parentSpan;
                expect(parentSpan).toBe(subagentRecord);
                endSubagentSpan(subagentSpan, { status: 'completed' });
                endInteractionSpan('ok');
            });
            it('startHookSpan OUTSIDE runInSubagentSpanContext but inside a tool context parents under the tool span (documented bg SubagentStart asymmetry)', async () => {
                ...
                const agentToolRecord = mockSpans.find((s) => s.name === 'hopcode.tool');
            ...
            it('nested subagent: innermost subagent shadows outer for child parenting', async () => {
                ...
                const innerRecord = mockSpans.find((s) => s.name === 'hopcode.subagent' &&
                    s.attributes['hopcode.subagent.id'] === 'inner');
            ...
            it('after runInSubagentSpanContext exits, child spans go back to interactionContext', async () => {
                ...
                const interactionRecord = mockSpans.find((s) => s.name === 'hopcode.interaction');
                const subagentSpan = startSubagentSpan({
                    ...baseOpts,
                    invocationKind: 'foreground',
                });
...
//# sourceMappingURL=session-tracing.test.js.map