...
export function initializeTelemetry(config) {
    ...
    if (attrWarnings.length > 0) {
        ...
        console.warn(`[hopcode telemetry] ${attrWarnings.length} resource attribute issue(s):`);
        ...
        if (otlpProtocol === 'http') {
            ...
            if (logsUrl) {
                ...
                logToSpanProcessor = new LogToSpanProcessor(new OTLPTraceExporterHttp({ url: tracesUrl }), {
                    ...
                    // disabled file logging via HOPCODE_DEBUG_LOG_FILE=0, debugLogger.warn
            ...
            // grpc ΓÇö per-signal endpoints are not supported with gRPC protocol.
            if (!parsedEndpoint) {
...
export async function shutdownTelemetry() {
    ...
    telemetryShutdownPromise = (async () => {
        ...
        let timedOut = false;
        try {
            // Wrap in Promise.resolve for safety ΓÇö auto-mocked shutdown()
            ...
            // timeout wins the race ΓÇö the process is exiting anyway.
