...
export function addSystemPromptAttributes(config, span, systemInstruction) {
    if (!areSensitiveSpanAttributesEnabled(config) || !systemInstruction)
        return;
    ...
    if (!seenHashes.has(hash)) {
        seenHashes.add(hash);
        const { content, truncated } = truncateContent(text, getMaxContentSize(config));
...
export function addToolSchemaAttributes(config, span, tools) {
    if (!areSensitiveSpanAttributesEnabled(config) || !tools?.length)
        return;
    ...
    for (const decl of declarations) {
        ...
        if (!seenHashes.has(hashKey)) {
            ...
            const { content, truncated } = truncateContent(declJson, getMaxContentSize(config));
...
export function addModelOutputAttributes(config, span, responseText, originalLength) {
    if (!areSensitiveSpanAttributesEnabled(config) || !responseText)
        return;
    const responseTextOriginalLength = originalLength ?? responseText.length;
    const { content, truncated } = truncateContent(responseText, getMaxContentSize(config), responseTextOriginalLength);
    span.setAttributes({
...
export function addToolInputAttributes(config, span, toolName, toolInput) {
    if (!areSensitiveSpanAttributesEnabled(config))
        return;
    const { content, truncated, originalLength } = truncatePrefixedContent(`[TOOL INPUT: ${toolName}]\n`, toolInput, getMaxContentSize(config));
    span.setAttributes({
        tool_input: content,
        ...(truncated && {
...
export function addToolResultAttributes(config, span, toolName, toolResult) {
    if (!areSensitiveSpanAttributesEnabled(config))
        return;
    const { content, truncated, originalLength } = truncatePrefixedContent(`[TOOL RESULT: ${toolName}]\n`, toolResult, getMaxContentSize(config));
    span.setAttributes({
        tool_result: content,
        ...(truncated && {
