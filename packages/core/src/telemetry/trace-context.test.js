...
import { getActiveSpanTraceContext, getSessionRootTraceContext, getTraceContext, formatTraceparent, setShellTracePropagation, isShellTracePropagationEnabled, ZERO_TRACE_ID, } from './trace-context.js';
...
vi.mock('@opentelemetry/api', () => ({
    ...
    isSpanContextValid: vi
        .fn()
        .mockImplementation((ctx) => ctx.traceId !== INVALID_TRACE && ctx.spanId !== INVALID_SPAN),
}));
vi.mock('./session-context.js', () => ({
    getSessionContext: vi.fn().mockReturnValue(undefined),
}));
function mockSpan(traceId, spanId, traceFlags) {
    return {
...
describe('trace-context', () => {
    ...
    describe('getActiveSpanTraceContext', () => {
        it('returns trace context from active span', () => {
            vi.mocked(trace.getActiveSpan).mockReturnValue(mockSpan('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'bbbbbbbbbbbbbbbb', 1));
        ...
        it('returns null for NOOP span with zero traceId', () => {
            vi.mocked(trace.getActiveSpan).mockReturnValue(mockSpan(ZERO_TRACE_ID, 'bbbbbbbbbbbbbbbb', 0));
        ...
        it('rejects span with valid traceId but zero spanId', () => {
            vi.mocked(trace.getActiveSpan).mockReturnValue(mockSpan('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', INVALID_SPAN, 1));
            expect(getActiveSpanTraceContext()).toBeNull();
        });
    });
    describe('getSessionRootTraceContext', () => {
        it('returns trace context from session root span', () => {
            ...
            vi.mocked(trace.getSpan).mockImplementation((ctx) => ctx === sessionCtx
        ...
        it('returns null when session span has zero traceId', () => {
            const sessionCtx = {};
            vi.mocked(getSessionContext).mockReturnValue(sessionCtx);
            vi.mocked(trace.getSpan).mockReturnValue(mockSpan(ZERO_TRACE_ID, 'dddddddddddddddd', 0));
            expect(getSessionRootTraceContext()).toBeNull();
        });
    });
    describe('getTraceContext', () => {
        it('prefers active span over session root', () => {
            ...
            const sessionCtx = {};
            vi.mocked(getSessionContext).mockReturnValue(sessionCtx);
            vi.mocked(trace.getSpan).mockReturnValue(mockSpan('cccccccccccccccccccccccccccccccc', 'dddddddddddddddd', 1));
