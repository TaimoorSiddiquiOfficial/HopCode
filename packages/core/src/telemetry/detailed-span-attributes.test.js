...
describe('detailed-span-attributes', () => {
    ...
    describe('truncateContent', () => {
        it('returns content as-is when under limit', () => {
            const result = truncateContent('hello', DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH);
        ...
        it('uses the default 1MiB limit when maxSize is omitted', () => {
            ...
            expect(result.content.length).toBe(DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH);
            expect(result.content).toContain('configured limit of 1048576 characters');
        });
        ...
        it.each([0, -1, Number.NaN, Number.POSITIVE_INFINITY])('throws when maxSize is invalid: %s', (maxSize) => {
    ...
    describe('addUserPromptAttributes', () => {
        ...
        it('sets truncation attributes for content over the default limit', () => {
            ...
            expect(String(span.attrs['new_context'])).toHaveLength(DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH);
    ...
    it('uses configured max length for all native sensitive span payloads', () => {
        ...
        expect(toolSchemaSpan.events[0].attributes['tool_definition_truncated']).toBe(true);
        expect(toolSchemaSpan.events[0].attributes['tool_definition_original_length']).toBe(JSON.stringify(toolDeclaration).length);
        ...
        expect(toolInputSpan.attrs['tool_input_original_length']).toBe('abcdef'.length);
        ...
        expect(toolResultSpan.attrs['tool_result_original_length']).toBe('abcdef'.length);
    });
    ...
    describe('addToolSchemaAttributes', () => {
        ...
        it('emits tool_schema events for first occurrence', () => {
            ...
            expect(span.events[0].attributes['tool_definition_original_length']).toBeUndefined();
            expect(span.events[0].attributes['tool_definition_truncated']).toBeUndefined();
        });
    ...
    describe('addModelOutputAttributes', () => {
        ...
        it('sets truncation attributes for output over the default limit', () => {
            ...
            expect(span.attrs['response.model_output_truncated']).toBe(true);
            expect(span.attrs['response.model_output_original_length']).toBe(1024 * 1024 + 1);
        });
    ...
    describe('addToolInputAttributes', () => {
        ...
        it('sets truncation attributes for input over the default limit', () => {
            const config = createMockConfig();
            const span = createMockSpan();
            const largeInput = 'i'.repeat(DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH + 1);
            addToolInputAttributes(config, span, 'Bash', largeInput);
            expect(span.attrs['tool_input_truncated']).toBe(true);
            expect(span.attrs['tool_input_original_length']).toBe(DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH + 1);
        });
        it('keeps tool_input within the configured limit when tool name is long', () => {
            ...
            expect(span.attrs['tool_input_truncated']).toBe(true);
            expect(span.attrs['tool_input_original_length']).toBe('{"command":"ls"}'.length);
        });
        it('no-ops when flag is disabled', () => {
            mockState.sensitiveEnabled = false;
            const config = createMockConfig();
    ...
    describe('addToolResultAttributes', () => {
        ...
        it('keeps tool_result within the configured limit when tool name is long', () => {
            ...
            expect(span.attrs['tool_result_truncated']).toBe(true);
            expect(span.attrs['tool_result_original_length']).toBe('file contents here'.length);
        });
    });
