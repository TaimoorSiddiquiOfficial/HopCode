...
describe('event loop lag metrics', () => {
    let registerDaemonEventLoopLagGauge;
    let registerAcpEventLoopLagGauge;
    beforeEach(async () => {
        vi.resetModules();
        vi.clearAllMocks();
    ...
    it('registers daemon and ACP event loop lag gauges', () => {
        ...
        expect(mockCreateObservableGaugeFn).toHaveBeenCalledWith('qwen-code.daemon.event_loop.lag', expect.objectContaining({ unit: 'ms' }));
        expect(mockCreateObservableGaugeFn).toHaveBeenCalledWith('qwen-code.acp.event_loop.lag', expect.objectContaining({ unit: 'ms' }));
    });
    ...
    it('swallows snapshot errors from observable callbacks', () => {
        ...
        expect(() => gaugeCallbacks[0]({ observe: vi.fn() })).not.toThrow();
    });
});
//# sourceMappingURL=event-loop-lag-metrics.test.js.map