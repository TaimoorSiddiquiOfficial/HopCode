...
import { EVENT_API_REQUEST, EVENT_API_RESPONSE, EVENT_CLI_CONFIG, EVENT_FLASH_FALLBACK, EVENT_TOOL_CALL, EVENT_USER_PROMPT, EVENT_MALFORMED_JSON_RESPONSE, EVENT_FILE_OPERATION, EVENT_RIPGREP_FALLBACK, EVENT_SKILL_LAUNCH, EVENT_EXTENSION_ENABLE, EVENT_EXTENSION_DISABLE, EVENT_EXTENSION_INSTALL, EVENT_EXTENSION_UNINSTALL, EVENT_TOOL_OUTPUT_TRUNCATED, EVENT_PROTOCOL_TAG_SANITIZED, } from './constants.js';
import { logApiRequest, logApiResponse, logStartSession, logUserPrompt, logToolCall, logFlashFallback, logChatCompression, logMalformedJsonResponse, logFileOperation, logRipgrepFallback, logSkillLaunch, logToolOutputTruncated, logExtensionEnable, logExtensionDisable, logExtensionInstallEvent, logExtensionUninstall, logHookCall, logApiError, logApiRetry, logProtocolTagSanitized, } from './loggers.js';
...
import { ApiRequestEvent, ApiResponseEvent, FlashFallbackEvent, StartSessionEvent, ToolCallEvent, UserPromptEvent, RipgrepFallbackEvent, SkillLaunchEvent, MalformedJsonResponseEvent, makeChatCompressionEvent, FileOperationEvent, ToolOutputTruncatedEvent, ExtensionEnableEvent, ExtensionDisableEvent, ExtensionInstallEvent, ExtensionUninstallEvent, HookCallEvent, ApiErrorEvent, ApiRetryEvent, ProtocolTagSanitizedEvent, } from './types.js';
...
describe('loggers', () => {
    ...
    describe('logChatCompression', () => {
        beforeEach(() => {
            ...
            vi.spyOn(HopCodeLogger.prototype, 'logChatCompressionEvent');
        });
        it('logs the chat compression event to HopCodeLogger', () => {
            ...
            expect(HopCodeLogger.prototype.logChatCompressionEvent).toHaveBeenCalledWith(event);
        });
    ...
    describe('logProtocolTagSanitized', () => {
        it('emits a privacy-safe handled event to QwenLogger and OpenTelemetry', () => {
            ...
            expect(QwenLogger.prototype.logProtocolTagSanitizedEvent).toHaveBeenCalledWith(event);
            ...
            expect(JSON.stringify(mockLogger.emit.mock.calls[0])).not.toMatch(/response_text|reasoning|tool_name|arguments/);
        });
    });
    ...
    describe('logUserPrompt', () => {
        ...
        it('should include the model attribute when set (e.g. inline override)', () => {
            const event = new UserPromptEvent(11, 'prompt-id-model', AuthType.USE_OPENAI, 'test-prompt', 'qwen-max');
            logUserPrompt(mockConfig, event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
                body: 'User prompt. Length: 11.',
                attributes: {
    ...
    describe('logApiResponse', () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(tokenUsageService, 'recordTokenUsageFromApiResponseBestEffort').mockImplementation(() => undefined);
        });
        it('should log an API response with all fields', () => {
            ...
            expect(tokenUsageService.recordTokenUsageFromApiResponseBestEffort).toHaveBeenCalledWith(mockConfig, event);
        });
        it.each([
            'prompt_suggestion',
            'forked_query',
            ...
            const event = new ApiResponseEvent('test-response-id', 'test-model', 100, promptId, AuthType.USE_GEMINI, {
                promptTokenCount: 1,
                candidatesTokenCount: 2,
            });
            logApiResponse(mockConfig, event);
            expect(tokenUsageService.recordTokenUsageFromApiResponseBestEffort).not.toHaveBeenCalled();
        });
        it('does not record token usage when usage statistics are disabled', () => {
            ...
            const event = new ApiResponseEvent('test-response-id', 'test-model', 100, 'prompt-id-1', AuthType.USE_GEMINI, {
                promptTokenCount: 1,
                candidatesTokenCount: 2,
            });
    ...
    describe('logApiResponse skips chatRecordingService for internal prompt IDs', () => {
        it.each([
            ...
            'speculation',
            'side-query:session-title',
        ])('should not record to chatRecordingService when prompt_id is %s', (promptId) => {
    ...
    describe('logRipgrepFallback', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logRipgrepFallbackEvent');
        });
        it('should log ripgrep fallback event', () => {
            ...
            expect(HopCodeLogger.prototype.logRipgrepFallbackEvent).toHaveBeenCalled();
    ...
    describe('logSkillLaunch', () => {
        ...
        it('forwards the event to HopCodeLogger and emits an OTLP record', () => {
            ...
            expect(HopCodeLogger.prototype.logSkillLaunchEvent).toHaveBeenCalledWith(event);
            const emittedEvent = mockLogger.emit.mock.calls[0][0];
            expect(emittedEvent.body).toBe('Skill launch: test-skill. Success: true.');
            expect(emittedEvent.attributes).toEqual(expect.objectContaining({
                'session.id': 'test-session-id',
    ...
    describe('logMalformedJsonResponse', () => {
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logMalformedJsonResponseEvent');
        });
        it('logs the event to Clearcut and OTEL', () => {
            ...
            expect(HopCodeLogger.prototype.logMalformedJsonResponseEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionInstall', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logExtensionInstallEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension install event', () => {
            ...
            expect(HopCodeLogger.prototype.logExtensionInstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionUninstall', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logExtensionUninstallEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension uninstall event', () => {
            ...
            expect(HopCodeLogger.prototype.logExtensionUninstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionEnable', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logExtensionEnableEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension enable event', () => {
            ...
            expect(HopCodeLogger.prototype.logExtensionEnableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionDisable', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger.prototype, 'logExtensionDisableEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension disable event', () => {
            ...
            expect(HopCodeLogger.prototype.logExtensionDisableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logHookCall', () => {
        ...
        beforeEach(() => {
            vi.spyOn(HopCodeLogger, 'getInstance').mockReturnValue(mockHopCodeLogger);
        ...
        it('should handle when HopCodeLogger is not available', () => {
            vi.spyOn(HopCodeLogger, 'getInstance').mockReturnValue(undefined);
        ...
        it('should pass the exact event object to HopCodeLogger', () => {
