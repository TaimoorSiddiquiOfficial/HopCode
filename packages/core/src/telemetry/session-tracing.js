...
import { SERVICE_NAME, SPAN_HOOK, SPAN_INTERACTION, SPAN_LLM_REQUEST, SPAN_SUBAGENT, SPAN_TOOL, SPAN_TOOL_BLOCKED_ON_USER, SPAN_TOOL_EXECUTION, } from './constants.js';
...
/**
 * ALS for the active `hopcode.subagent` span. Child LLM/tool/hook spans
...
function ttlFor(ctx) {
    if (ctx.type === 'subagent') {
        const kind = ctx.attributes['hopcode.subagent.invocation_kind'];
...
function sweepStaleSpans(now) {
    for (const [spanId, weakRef] of activeSpans) {
        ...
        if (now - ctx.startTime < ttlFor(ctx))
            continue;
        if (!ctx.ended) {
            ctx.ended = true;
                ...
                ctx.span.setAttributes({
                    ...
                    ...(ctx.type === 'subagent'
                        ? {
                            ...
                            // only get the generic hopcode.span.duration_ms
                            // above). wenshao @ #4410.
                            'hopcode.subagent.duration_ms': ageMs,
                        }
                        : {}),
                });
            }
...
export function truncateSpanError(s) {
    if (s.length <= SPAN_ERROR_MAX_CHARS)
        return s;
...
export async function withInteractionSpan(config, options, fn, getResultStatus) {
    ...
    return await otelContext.with(activeContext, async () => interactionContext.run(spanContextObj, async () => {
        ...
        try {
            ...
            span.setStatus({
                code: SpanStatusCode.ERROR,
                message: truncateSpanError(error instanceof Error ? error.message : String(error)),
            });
            errorStatusSet = true;
            throw error;
        }
            ...
            if (!spanContextObj.ended) {
                ...
                span.setAttributes({
                    'interaction.duration_ms': duration,
                    'hopcode.turn_status': terminalStatus,
                });
...
// --- LLM Request Spans ---
export function startLLMRequestSpan(model, promptId) {
    if (!isTelemetrySdkInitialized()) {
        return NOOP_SPAN;
    }
    ...
    const span = getTracer().startSpan(SPAN_LLM_REQUEST, { kind: SpanKind.INTERNAL, attributes }, ctx);
    const spanId = getSpanId(span);
    const spanContextObj = {
        span,
        startTime: Date.now(),
    ...
    try {
        ...
        try {
            if (metadata?.config && metadata.success) {
                const model = String(spanCtx.attributes['hopcode.model'] ?? '');
                if (metadata.requestSetupMs !== undefined) {
...
// --- Tool Spans ---
export function startToolSpan(toolName, attrs) {
    if (!isTelemetrySdkInitialized()) {
        return NOOP_SPAN;
    }
    ...
    const span = getTracer().startSpan(SPAN_TOOL, { kind: SpanKind.INTERNAL, attributes }, ctx);
    const spanId = getSpanId(span);
    const spanContextObj = {
        span,
        startTime: Date.now(),
...
export function startToolExecutionSpan() {
    ...
    const span = getTracer().startSpan(SPAN_TOOL_EXECUTION, {
        kind: SpanKind.INTERNAL,
...
export function startSubagentSpan(opts) {
    ...
    const attributes = {
        ...
        // Vendor (hopcode-specific). Dual-emit id/name so dashboards already
        // querying spec keys still work.
        'hopcode.subagent.id': opts.agentId,
        'hopcode.subagent.name': opts.subagentName,
        'hopcode.subagent.invocation_kind': opts.invocationKind,
        'hopcode.subagent.is_built_in': opts.isBuiltIn,
        'hopcode.subagent.depth': opts.depth,
    };
...
export function endSubagentSpan(span, metadata) {
    ...
    try {
        ...
        const endAttributes = {
            ...
            'hopcode.subagent.duration_ms': duration,
            'hopcode.subagent.status': metadata.status,
        };
        if (metadata.terminateReason !== undefined) {
            endAttributes['hopcode.subagent.terminate_reason'] =
                metadata.terminateReason;
        }
        if (metadata.resultSummaryPresent !== undefined) {
            endAttributes['hopcode.subagent.result_summary_present'] =
