...
export class HopCodeLogger {
    ...
    enqueueLogEvent(event) {
        try {
            ...
            if (wasAtCapacity) {
                this.debugLogger.debug(`HopCodeLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`);
            }
        }
        catch (error) {
    ...
    async flushToRum() {
        if (this.isFlushInProgress) {
            this.debugLogger.debug('HopCodeLogger: Flush already in progress, marking pending flush.');
    ...
    requeueFailedEvents(eventsToSend) {
        ...
        if (eventsToSend.length > MAX_RETRY_EVENTS) {
            this.debugLogger.warn(`HopCodeLogger: Dropping ${eventsToSend.length - MAX_RETRY_EVENTS} events due to retry queue limit. Total events: ${eventsToSend.length}, keeping: ${MAX_RETRY_EVENTS}`);
        }
        ...
        this.debugLogger.debug(`HopCodeLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`);
    }
}
...
//# sourceMappingURL=hopcode-logger.js.map