...
import { HopCodeLogger, TEST_ONLY } from './hopcode-logger.js';
import { AuthType } from '../../core/contentGenerator.js';
import { StartSessionEvent, EndSessionEvent, IdeConnectionEvent, KittySequenceOverflowEvent, IdeConnectionType, HookCallEvent, SkillLaunchEvent, ProtocolTagSanitizedEvent, } from '../types.js';
...
const makeFakeConfig = (overrides = {}) => {
    const defaults = {
        ...
        getAuthType: () => AuthType.HOPCODE_OAUTH,
        getMcpServers: () => ({}),
        getModel: () => 'test-model',
...
describe('HopCodeLogger', () => {
    ...
    beforeEach(() => {
        ...
        HopCodeLogger.instance = undefined;
    });
    afterEach(() => {
        vi.useRealTimers();
        vi.restoreAllMocks();
    ...
    describe('getInstance', () => {
        it('returns undefined when usage statistics are disabled', () => {
            ...
            const logger = HopCodeLogger.getInstance(config);
            expect(logger).toBeUndefined();
        });
        it('returns an instance when usage statistics are enabled', () => {
            const logger = HopCodeLogger.getInstance(mockConfig);
            expect(logger).toBeInstanceOf(HopCodeLogger);
        });
        it('is a singleton', () => {
            const logger1 = HopCodeLogger.getInstance(mockConfig);
            const logger2 = HopCodeLogger.getInstance(mockConfig);
    ...
    describe('logHookCallEvent', () => {
        ...
        it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
            ...
            const logger = HopCodeLogger.getInstance(configWithLogPrompts);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
        ...
        it('should not include error when telemetry log prompts disabled', () => {
            ...
            HopCodeLogger.instance =
                undefined;
            const logger = HopCodeLogger.getInstance(configWithoutLogPrompts);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
        ...
        it('should sanitize hook name to remove sensitive information', () => {
            ...
            const event = new HookCallEvent('PreToolUse', 'command', '/home/user/.hopcode/hooks/check-secrets.sh --api-key=secret123', { tool_name: 'read_file' }, 100, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
...
//# sourceMappingURL=hopcode-logger.test.js.map