  ...
  describe('initializeDaemonMetrics', () => {
    it('creates counters and histograms', () => {
      ...
      expect(mockCreateCounterFn).toHaveBeenCalledWith(
        'hopcode.daemon.http.request.count',
        expect.objectContaining({ description: expect.any(String) }),
      );
      expect(mockCreateHistogramFn).toHaveBeenCalledWith(
        'hopcode.daemon.http.request.duration',
        expect.objectContaining({ unit: 'ms' }),
      );
      expect(mockCreateCounterFn).toHaveBeenCalledWith(
        'hopcode.daemon.session.lifecycle',
        expect.any(Object),
      );
      expect(mockCreateCounterFn).toHaveBeenCalledWith(
        'hopcode.daemon.channel.lifecycle',
        expect.any(Object),
      );
      expect(mockCreateHistogramFn).toHaveBeenCalledWith(
        'hopcode.daemon.prompt.queue_wait',
        expect.objectContaining({ unit: 'ms' }),
      );
      expect(mockCreateHistogramFn).toHaveBeenCalledWith(
        'hopcode.daemon.prompt.duration',
        expect.objectContaining({ unit: 'ms' }),
      );
      expect(mockCreateCounterFn).toHaveBeenCalledWith(
        'hopcode.daemon.bridge.error.count',
        expect.any(Object),
      );
      expect(mockCreateCounterFn).toHaveBeenCalledWith(
        'hopcode.daemon.cancel.count',
        expect.any(Object),
      );
      expect(mockCreateHistogramFn).toHaveBeenCalledWith(
  ...
  describe('registerDaemonGaugeCallbacks', () => {
    it('creates 3 observable gauges', () => {
      ...
      expect(mockCreateObservableGaugeFn).toHaveBeenCalledWith(
        'hopcode.daemon.session.active',
        expect.any(Object),
      );
      expect(mockCreateObservableGaugeFn).toHaveBeenCalledWith(
        'hopcode.daemon.sse.active',
        expect.any(Object),
      );
      expect(mockCreateObservableGaugeFn).toHaveBeenCalledWith(
        'hopcode.daemon.process.heap_used',
