...
import { SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT, TelemetryTarget, } from './index.js';
describe('telemetry/config helpers', () => {
    ...
    describe('resolveTelemetrySettings', () => {
        ...
        it('defaults sensitiveSpanAttributeMaxLength to 1MiB', async () => {
            const resolved = await resolveTelemetrySettings({});
            const resolvedMaxLength = resolved.sensitiveSpanAttributeMaxLength;
        ...
        it('parses sensitiveSpanAttributeMaxLength from settings and env', async () => {
            ...
            const resolvedFromEnv = await resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: '131072',
                },
                settings: { sensitiveSpanAttributeMaxLength: 65_536 },
            });
        ...
        it('accepts sensitiveSpanAttributeMaxLength at the configured maximum', async () => {
            const resolvedFromSettings = await resolveTelemetrySettings({
                settings: {
                    sensitiveSpanAttributeMaxLength: SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT,
                },
            });
            expect(resolvedFromSettings.sensitiveSpanAttributeMaxLength).toBe(SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT);
            const resolvedFromEnv = await resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: String(SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT),
                },
            });
            expect(resolvedFromEnv.sensitiveSpanAttributeMaxLength).toBe(SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT);
        });
        it('rejects invalid sensitiveSpanAttributeMaxLength settings', async () => {
            await expect(resolveTelemetrySettings({
            ...
            await expect(resolveTelemetrySettings({
                settings: {
                    sensitiveSpanAttributeMaxLength: SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT + 1,
                },
        ...
        it.each([
            ['string', '1024'],
            ['boolean', true],
        ])('rejects non-number sensitiveSpanAttributeMaxLength settings (%s)', async (_type, value) => {
            const settings = {
                sensitiveSpanAttributeMaxLength: value,
            };
            await expect(resolveTelemetrySettings({ settings })).rejects.toThrow(/sensitiveSpanAttributeMaxLength/i);
        });
        it('rejects invalid sensitive span max length env values', async () => {
            ...
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got ''/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: '   ',
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got ' {3}'/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: 'abc',
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got 'abc'/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: '1e3',
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got '1e3'/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: '0',
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got '0'/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: '9007199254740992',
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*got '9007199254740992'/);
            await expect(resolveTelemetrySettings({
                env: {
                    HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH: String(SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT + 1),
                },
            })).rejects.toThrow(/HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH.*104857600/);
        });
        ...
        it('throws on unknown protocol values', async () => {
            const env = { HOPCODE_TELEMETRY_OTLP_PROTOCOL: 'unknown' };
        ...
        it('throws on unknown target values', async () => {
            const env = { HOPCODE_TELEMETRY_TARGET: 'unknown' };
        ...
        it('HOPCODE_ env vars take precedence over OTEL_ vars for per-signal endpoints', async () => {
            const env = {
                HOPCODE_TELEMETRY_OTLP_TRACES_ENDPOINT: 'http://hopcode-traces:4318/v1/traces',
                OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: 'http://otel-traces:4318/v1/traces',
            };
            const resolved = await resolveTelemetrySettings({ env });
            expect(resolved.otlpTracesEndpoint).toBe('http://hopcode-traces:4318/v1/traces');
        });
    ...
    describe('resolveTelemetrySettings — resource attributes', () => {
        ...
        it('exposes resourceAttributeWarnings when input has issues', async () => {
            ...
            expect(resolved.resourceAttributeWarnings.length).toBeGreaterThanOrEqual(4);
        });
