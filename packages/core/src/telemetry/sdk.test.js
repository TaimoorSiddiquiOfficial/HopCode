...
import { initializeTelemetry, isTelemetrySdkInitialized, shutdownTelemetry, resolveHttpOtlpUrl, refreshSessionContext, } from './sdk.js';
...
import { resetDebugLoggingState, setDebugLogSession, } from '../utils/debugLogger.js';
function escapeRegExp(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function expectOtelDebugLogLine(level, message) {
    return expect.stringMatching(new RegExp(`\\[${level}\\] \\[OTEL\\]( \\[trace_id=[0-9a-f]{32} span_id=[0-9a-f]{16}\\])? ${escapeRegExp(message)}`));
}
...
vi.mock('./tracer.js', () => ({
    createSessionRootContext: vi.fn((id) => ({ __sessionId: id })),
}));
...
describe('Telemetry SDK', () => {
    ...
    it('should route OpenTelemetry diagnostics to debug log instead of console output', async () => {
        ...
        try {
            ...
            expect(appendFileSpy).toHaveBeenCalledWith(expect.stringContaining('11111111-2222-4333-8444-555555555555'), expectOtelDebugLogLine('ERROR', '{"message":"Error: PeriodicExportingMetricReader: metrics export failed (error Error: connect ECONNREFUSED)"}'), 'utf8');
            expect(appendFileSpy).toHaveBeenCalledWith(expect.stringContaining('11111111-2222-4333-8444-555555555555'), expectOtelDebugLogLine('ERROR', 'A different OpenTelemetry diagnostic'), 'utf8');
            expect(appendFileSpy).toHaveBeenCalledWith(expect.stringContaining('11111111-2222-4333-8444-555555555555'), expectOtelDebugLogLine('WARN', 'An OpenTelemetry warning'), 'utf8');
        }
        finally {
    ...
    it('should use per-signal overrides without base endpoint', () => {
        ...
        expect(LogToSpanProcessor).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ includeSensitiveSpanAttributes: false }));
        expect(NodeSDK.prototype.start).toHaveBeenCalled();
    });
    it('passes sensitive span attribute config to the log-to-span bridge', () => {
        ...
        vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('');
        vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue('http://traces-host/token/api/otlp/traces');
        vi.spyOn(mockConfig, 'getTelemetryIncludeSensitiveSpanAttributes').mockReturnValue(true);
        initializeTelemetry(mockConfig);
        expect(LogToSpanProcessor).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ includeSensitiveSpanAttributes: true }));
    });
    it('in interactive mode, routes log-to-span diagnostics through the OTEL debug logger to avoid TUI pollution', async () => {
        ...
        try {
            ...
            await vi.waitFor(() => {
                expect(appendFileSpy).toHaveBeenCalledWith(expect.stringContaining('log-to-span-sink-test'), expectOtelDebugLogLine('WARN', '[LogToSpan] sink wiring smoke test'), 'utf8');
            });
        }
        finally {
            if (previousDebugLogFileEnv === undefined) {
    ...
    it('in non-interactive mode, leaves diagnostics on the default stderr sink so CI/scripts see export failures', async () => {
        ...
        const { LogToSpanProcessor: RealProcessor } = await vi.importActual('./log-to-span-processor.js');
        const failingExporter = {
            export: (_spans, cb) => cb({ code: 1, error: new Error('boom') }),
            shutdown: () => Promise.resolve(),
            forceFlush: () => Promise.resolve(),
        };
        const realProcessor = new RealProcessor(failingExporter, { flushIntervalMs: 60000 });
        ...
        try {
            ...
            expect(stderrWrite).toHaveBeenCalledWith('[LogToSpan] export failed: code=1 error="boom"\n');
        }
        finally {
    ...
    describe('Resource attributes', () => {
        function getResourceAttributes() {
            const constructorCall = vi.mocked(NodeSDK).mock.calls[0][0];
            return constructorCall.resource.attributes;
        }
        ...
        it('emits a console summary when resource-attribute warnings are present', () => {
            const consoleWarnSpy = vi
                ...
                .mockImplementation(() => { });
            try {
                vi.spyOn(mockConfig, 'getTelemetryResourceAttributeWarnings').mockReturnValue([
                ...
                expect(consoleWarnSpy.mock.calls.some((c) => String(c[0]).includes('reserved key'))).toBe(true);
            }
            finally {
                consoleWarnSpy.mockRestore();
            }
    ...
    describe('Outbound trace-context propagation gate', () => {
        ...
        it('installs a no-op TextMapPropagator by default (propagateTraceContext=false)', () => {
            ...
            initializeTelemetry(mockConfig);
            const propagator = getTextMapPropagator();
            ...
            const carrier = { existing: 'h' };
            expect(() => propagator.inject({}, carrier, {})).not.toThrow();
            expect(carrier).toEqual({ existing: 'h' });
        });
        it('uses the SDK default propagator when propagateTraceContext=true (operator opt-in)', () => {
            vi.spyOn(mockConfig, 'getOutboundCorrelationPropagateTraceContext').mockReturnValue(true);
            initializeTelemetry(mockConfig);
    ...
    describe('Instrumentations', () => {
        function getInstrumentations() {
            const constructorCall = vi.mocked(NodeSDK).mock.calls[0][0];
            return (constructorCall.instrumentations ?? []);
        }
        it('registers both HttpInstrumentation and UndiciInstrumentation', () => {
            ...
            expect(instrumentations.some((i) => i instanceof UndiciInstrumentation)).toBe(true);
        });
        it('UndiciInstrumentation receives ignoreRequestHook that skips configured OTLP endpoints', () => {
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('http://collector.example.com:4318');
            initializeTelemetry(mockConfig);
            const config = vi.mocked(UndiciInstrumentation).mock.calls[0][0];
            // Configured OTLP endpoint must be skipped to avoid feedback loops.
            expect(config.ignoreRequestHook({
        ...
        it('ignoreRequestHook is a pure no-op when no OTLP endpoint is configured', () => {
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('');
            vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue(undefined);
            vi.spyOn(mockConfig, 'getTelemetryOtlpLogsEndpoint').mockReturnValue(undefined);
            vi.spyOn(mockConfig, 'getTelemetryOtlpMetricsEndpoint').mockReturnValue(undefined);
        ...
        it('ignoreRequestHook handles per-signal endpoint configuration', () => {
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('');
            vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue('http://traces.example.com:4318/v1/traces');
            vi.spyOn(mockConfig, 'getTelemetryOtlpLogsEndpoint').mockReturnValue('http://logs.example.com:4318/v1/logs');
            initializeTelemetry(mockConfig);
            const config = vi.mocked(UndiciInstrumentation).mock.calls[0][0];
        ...
        it('ignoreRequestHook strips #fragment from configured endpoint', () => {
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('http://collector.example.com:4318/v1/traces#anchor');
            initializeTelemetry(mockConfig);
            const config = vi.mocked(UndiciInstrumentation).mock.calls[0][0];
            expect(config.ignoreRequestHook({
                origin: 'http://collector.example.com:4318',
        ...
        it('ignoreRequestHook does NOT bleed across hostname boundary (otlp vs otlp.evil)', () => {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('https://otlp.example.com');
            initializeTelemetry(mockConfig);
            const config = vi.mocked(UndiciInstrumentation).mock.calls[0][0];
            expect(config.ignoreRequestHook({
        ...
        it('ignoreRequestHook does NOT bleed across path-segment boundary (/v1 vs /v1foo)', () => {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('http://collector.example.com:4318/v1');
            initializeTelemetry(mockConfig);
            const config = vi.mocked(UndiciInstrumentation).mock.calls[0][0];
            expect(config.ignoreRequestHook({
                origin: 'http://collector.example.com:4318',
        ...
        it('normalizeOtlpPrefix rejects unparseable URLs entirely (no dangerous "http" fallback)', () => {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('not-a-valid-url');
            vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue(undefined);
            vi.spyOn(mockConfig, 'getTelemetryOtlpLogsEndpoint').mockReturnValue(undefined);
            vi.spyOn(mockConfig, 'getTelemetryOtlpMetricsEndpoint').mockReturnValue(undefined);
            initializeTelemetry(mockConfig);
        ...
        it('HttpInstrumentation also receives ignoreOutgoingRequestHook for OTLP exporter', () => {
            ...
            expect(httpInstrumentationConfig.ignoreOutgoingRequestHook({
                protocol: 'http:',
        ...
        it('matches default-port requests against a portless prefix (URL.origin parity)', () => {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('http://collector.example.com');
            initializeTelemetry(mockConfig);
            const httpInstrumentationConfig = vi.mocked(HttpInstrumentation).mock
                .calls[0][0];
        ...
        it('fails open when req.protocol is missing (no silent HTTPS guard bypass)', () => {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
            vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('https://collector.example.com:4318');
            initializeTelemetry(mockConfig);
            const httpInstrumentationConfig = vi.mocked(HttpInstrumentation).mock
                .calls[0][0];
            expect(httpInstrumentationConfig.ignoreOutgoingRequestHook({
