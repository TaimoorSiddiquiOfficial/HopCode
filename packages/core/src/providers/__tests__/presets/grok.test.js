...
import { ALL_PROVIDERS, THIRD_PARTY_PROVIDERS, findProviderByCredentials, findProviderById, getAllProviderBaseUrls, } from '../../all-providers.js';
...
describe('grokProvider', () => {
    ...
    it('is found by its env key + base URL credentials', () => {
        expect(findProviderByCredentials('https://api.x.ai/v1', 'XAI_API_KEY')?.id).toBe('grok');
        // Wrong base URL for the right key must not match.
        expect(findProviderByCredentials('https://wrong.example.com/v1', 'XAI_API_KEY')).toBeUndefined();
    });
