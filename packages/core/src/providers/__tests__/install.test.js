...
import { applyProviderInstallPlan, buildInstallPlan, customProvider, generateCustomEnvKey, ProviderInstallError, } from '../index.js';
...
describe('applyProviderInstallPlan', () => {
    ...
    it('persists env, auth selection, selected model, and merged model providers', async () => {
        ...
        expect(syncAuthState).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'new-model', undefined);
    ...
    it('prints a shadowing warning when an env key changes', async () => {
        ...
        expect(consoleError).toHaveBeenCalledWith(expect.stringContaining('SHADOW_KEY is also set'));
    ...
    it('preserves existing custom provider models and selects the installed endpoint', async () => {
        ...
        expect(syncAuthState).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'model-b', baseUrl);
    });
