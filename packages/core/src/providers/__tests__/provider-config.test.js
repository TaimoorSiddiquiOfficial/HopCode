...
import { AuthType, buildInstallPlan, buildProviderTemplate, computeModelListVersion, findExistingProviderModels, findProviderByCredentials, getAllProviderBaseUrls, getDefaultModelIds, resolveBaseUrl, shouldShowStep, providerMatchesCredentials, } from '@hoptrendy/hopcode-core';
...
describe('findExistingProviderModels', () => {
    ...
    it('returns undefined when no saved models are owned by the provider', () => {
        expect(findExistingProviderModels(config, {
    ...
    it('returns undefined when ownership cannot be resolved (function envKey)', () => {
        ...
        expect(findExistingProviderModels(customConfig, {
...
describe('resolveBaseUrl edge cases', () => {
    ...
    it('matches BaseUrlOption trailing-slash variants', () => {
        ...
        expect(resolveBaseUrlSrc(config, 'https://b.com/v1/')).toBe('https://b.com/v1');
        expect(resolveBaseUrlSrc(config, 'https://a.com/v1///')).toBe('https://a.com/v1');
        expect(resolveBaseUrlSrc(config, 'https://c.com/v1')).toBe('https://c.com/v1/');
    });
});
describe('providerMatchesCredentials with function envKey (custom provider)', () => {
    ...
    it('matches a custom-style provider whose envKey is a function deriving from baseUrl', () => {
        const derivedFor = (_protocol, baseUrl) => `HOPCODE_CUSTOM_${Buffer.from(baseUrl).toString('hex').slice(0, 8)}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
    ...
    it('does not match when the derived key differs from the supplied envKey', () => {
        const derivedFor = (_protocol, baseUrl) => `HOPCODE_CUSTOM_${baseUrl.length}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
            baseUrl: undefined,
    ...
    it('iterates protocolOptions and matches when any one derives the env key', () => {
        ...
        const derivedFor = (protocol, baseUrl) => `HOPCODE_CUSTOM_${protocol.toUpperCase()}_${baseUrl.length}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
            baseUrl: undefined,
...
describe('customHeaders in ProviderConfig', () => {
    ...
    it('merges customHeaders into generationConfig for editable unknown models', () => {
        ...
        expect(plan.modelProviders?.[0]?.models[0]?.generationConfig?.customHeaders).toEqual({ 'X-Custom': 'val' });
    });
    it('merges customHeaders for custom-provider models (no predefined list)', () => {
        ...
        expect(plan.modelProviders?.[0]?.models[0]?.generationConfig?.customHeaders).toEqual({ Authorization: 'Bearer test' });
    });
    it('does not add generationConfig when customHeaders is absent', () => {
        ...
        expect(plan.modelProviders?.[0]?.models[0]?.generationConfig).toBeUndefined();
    });
