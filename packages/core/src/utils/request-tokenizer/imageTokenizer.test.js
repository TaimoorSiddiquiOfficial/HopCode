...
describe('ImageTokenizer', () => {
    ...
    describe('WebP dimension extraction', () => {
        function buildWebp(format, totalLength) {
        ...
        function buildVp8lWebp(width, height, signature = 0x2f) {
        ...
        it('should extract dimensions from a short VP8L lossless WebP', async () => {
            ...
            const metadata = await tokenizer.extractImageMetadata(buf.toString('base64'), 'image/webp');
        ...
        it('should still reject short VP8 and VP8X WebP files', async () => {
            ...
            for (const format of formats) {
                const metadata = await tokenizer.extractImageMetadata(buildWebp(format, 26).toString('base64'), 'image/webp');
                expect(metadata.width).toBe(512);
                expect(metadata.height).toBe(512);
            }
        });
    ...
    describe('different image formats', () => {
        ...
        it('should extract dimensions from GIF images', async () => {
            ...
            const metadata = await tokenizer.extractImageMetadata(buf.toString('base64'), 'image/gif');
    ...
    describe('TIFF dimension extraction', () => {
        ...
        function buildShortTiff(byteOrder, width, height) {
            const buf = Buffer.alloc(38);
            const le = byteOrder === 'II';
            buf.write(byteOrder, 0, 'ascii');
            const w16 = (off, v) => le ? buf.writeUInt16LE(v, off) : buf.writeUInt16BE(v, off);
            const w32 = (off, v) => le ? buf.writeUInt32LE(v, off) : buf.writeUInt32BE(v, off);
            w16(2, 42); // magic
