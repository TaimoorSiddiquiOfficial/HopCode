...
describe('createConcurrencyLimiter', () => {
    it('throws on a non-positive-integer limit', () => {
        ...
        expect(() => createConcurrencyLimiter(Number.NaN)).toThrow(/positive integer/i);
    });
    describe('run', () => {
        ...
        it('never runs more than `limit` thunks concurrently', async () => {
            ...
            await Promise.all(Array.from({ length: 12 }, mk).map((t) => limiter.run(t)));
    ...
    describe('abort', () => {
        ...
        it('rejects queued jobs promptly when the signal aborts, even if in-flight never settles', async () => {
            ...
            await expect(Promise.race([
                queued,
                new Promise((_, rej) => setTimeout(() => rej(new Error('hung')), 200)),
...
//# sourceMappingURL=concurrencyLimiter.test.js.map