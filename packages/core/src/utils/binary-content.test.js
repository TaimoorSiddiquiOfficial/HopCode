...
import { extensionForMimeType, formatByteSize, isBinaryContentType, looksLikeText, persistBinaryContent, sniffFileKind, } from './binary-content.js';
...
describe('persistBinaryContent', () => {
    ...
    it('writes raw bytes with a mime-derived extension', async () => {
        ...
        const result = await persistBinaryContent(bytes, 'pdf', dir, 'webfetch-test-1');
        expect(result).toEqual({
            filepath: path.join(dir, 'webfetch-test-1.pdf'),
            size: bytes.length,
            ext: 'pdf',
        });
        expect(fs.readFileSync(path.join(dir, 'webfetch-test-1.pdf'))).toEqual(bytes);
        // Owner-only: fetched content can be sensitive.
        expect(fs.statSync(path.join(dir, 'webfetch-test-1.pdf')).mode & 0o777).toBe(0o600);
    });
    it('creates the target directory when missing', async () => {
        const nested = path.join(dir, 'does', 'not', 'exist');
        const result = await persistBinaryContent(Buffer.from('x'), 'bin', nested, 'id');
        expect('filepath' in result && result.filepath).toBe(path.join(nested, 'id.bin'));
    });
    it('returns an error instead of throwing on unwritable paths', async () => {
        ...
        const result = await persistBinaryContent(Buffer.from('x'), 'bin', filePath, // dir path collides with an existing file
        'id');
...
describe('sniffFileKind', () => {
    ...
    it('refines ZIP magic to office extensions via Content-Disposition', () => {
        const kind = sniffFileKind(ZIP, 'application/octet-stream', 'attachment; filename="report.xlsx"', URL);
        expect(kind.extension).toBe('xlsx');
        expect(kind.magicMatched).toBe(true);
    });
    it('refines ZIP magic to office extensions via URL path', () => {
        const kind = sniffFileKind(ZIP, 'application/octet-stream', '', 'https://example.com/deck.pptx');
    ...
    it('refines ZIP magic via the declared Content-Type when names are generic', () => {
        const kind = sniffFileKind(ZIP, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', '', URL);
        expect(kind.extension).toBe('xlsx');
    });
    it('reports how the extension was determined', () => {
        expect(sniffFileKind(TEXT, '', 'attachment; filename="fw.bin"', URL)
            .extensionSource).toBe('name');
        expect(sniffFileKind(TEXT, 'application/pdf', '', URL).extensionSource).toBe('mime');
    ...
    it('refines ZIP magic to jar via filename or Content-Type', () => {
        const byName = sniffFileKind(ZIP, 'application/octet-stream', 'attachment; filename="library.jar"', URL);
    ...
    it.each([
        ...
        const kind = sniffFileKind(bytes, '', '', URL);
        expect(kind.extension).toBe(ext);
        expect(kind.mimeType).toBe(mime);
        expect(kind.magicMatched).toBe(true);
    });
    it('uses RFC 5987 filename* when present', () => {
        const kind = sniffFileKind(TEXT, 'application/octet-stream', "attachment; filename*=UTF-8''r13031cp.pdf", URL);
        ...
        expect(sniffFileKind(TEXT, '', "attachment; filename*=UTF-8'en'report.xlsx", URL)
            .extension).toBe('xlsx');
    });
    it('recognizes archive extensions from headerless URLs', () => {
        const kind = sniffFileKind(TEXT, '', '', 'https://example.com/backup/archive.tar');
        expect(kind.extension).toBe('tar');
        expect(kind.extensionSource).toBe('name');
    });
    it('uses the URL extension when headers say nothing', () => {
        const kind = sniffFileKind(TEXT, 'application/octet-stream', '', 'https://example.com/audio/track.mp3?sig=abc');
        expect(kind.extension).toBe('mp3');
    });
    it('falls back to the content-type map, then bin', () => {
        expect(sniffFileKind(TEXT, 'application/pdf', '', URL).extension).toBe('pdf');
        expect(sniffFileKind(TEXT, 'who/knows', '', URL).extension).toBe('bin');
    });
    it('ignores unknown extensions in filenames', () => {
        const kind = sniffFileKind(TEXT, '', 'attachment; filename="script.exe"', URL);
        expect(kind.extension).toBe('bin');
    });
});
//# sourceMappingURL=binary-content.test.js.map