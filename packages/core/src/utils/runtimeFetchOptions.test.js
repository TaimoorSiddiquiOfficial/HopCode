...
vi.mock('undici', () => {
    ...
    class MockEnvHttpProxyAgent {
        ...
        constructor(options) {
            this.options = options;
            const httpProxy = options.httpProxy || '';
...
import { buildRuntimeFetchOptions, extractHostnameFromProxyUrl, getOrCreateSharedDispatcher, isTlsVerificationDisabled, redactProxyCredentials, redactProxyError, resetDispatcherCache, } from './runtimeFetchOptions.js';
describe('buildRuntimeFetchOptions (node runtime)', () => {
    ...
    it('returns Agent with disabled timeouts for OpenAI when no proxy is set', () => {
        ...
        expect(result.fetch).toBe(mockUndiciFetch);
    });
...
describe('TLS verification opt-out (insecure)', () => {
    ...
    const getDispatcherOptions = (result) => result.fetchOptions?.dispatcher?.options;
    ...
    it('configures TLS opt-out for direct and proxied requests', () => {
        process.env['QWEN_TLS_INSECURE'] = '1';
        const dispatcher = getOrCreateSharedDispatcher('http://proxy.local');
    ...
    describe('isTlsVerificationDisabled', () => {
        it.each(['1', 'true', 'TRUE', 'Yes', 'on', '  1  '])('treats QWEN_TLS_INSECURE=%j as enabled', (value) => {
            process.env['QWEN_TLS_INSECURE'] = value;
            expect(isTlsVerificationDisabled()).toBe(true);
        });
        it.each(['0', 'false', 'no', 'off', '', 'enabled'])('treats QWEN_TLS_INSECURE=%j as disabled', (value) => {
            process.env['QWEN_TLS_INSECURE'] = value;
