...
import { prepareTranscriptRecords, } from './transcript-records.js';
...
describe('prepareTranscriptRecords', () => {
    ...
    it('stops at a missing parent and reports a history gap', () => {
        ...
        expect(prepared.diagnostics).toContainEqual(expect.objectContaining({
            code: 'history_gap',
    ...
    it('reports cycles and conflicting duplicate parents', () => {
        ...
        expect(prepared.diagnostics.map((item) => item.code)).toEqual(expect.arrayContaining(['parent_cycle', 'conflicting_parent_uuid']));
    });
    ...
    it('accepts session source metadata as a known record subtype', () => {
        ...
        expect(prepared.diagnostics).not.toContainEqual(expect.objectContaining({
            code: 'unknown_record_or_part',
            recordId: 'source',
            path: 'subtype',
        }));
    ...
    it('rejects mixed sessions and an explicit artifact leaf', () => {
        expect(() => prepareTranscriptRecords([
            record('a', null),
        ...
        ], { leafUuid: 'artifact' })).toThrowError(expect.objectContaining({
            code: 'leaf_not_found',
        }));
    });
});
//# sourceMappingURL=transcript-records.test.js.map