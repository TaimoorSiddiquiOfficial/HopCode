...
describe('stripRuntimeSnapshotPrefix', () => {
    it('returns bare model IDs unchanged', () => {
        expect(stripRuntimeSnapshotPrefix('qwen3.6-27b-autoround')).toBe('qwen3.6-27b-autoround');
    });
    it('strips a single runtime snapshot prefix', () => {
        expect(stripRuntimeSnapshotPrefix('$runtime|openai|qwen3.6-27b-autoround')).toBe('qwen3.6-27b-autoround');
    });
    it('strips nested runtime snapshot prefixes (corruption self-heal)', () => {
        expect(stripRuntimeSnapshotPrefix('$runtime|openai|$runtime|openai|qwen3.6-27b-autoround')).toBe('qwen3.6-27b-autoround');
    });
    it('returns the input unchanged for a malformed prefix with no model ID', () => {
        expect(stripRuntimeSnapshotPrefix('$runtime|openai|')).toBe('$runtime|openai|');
