...
import { parsePDFPageRange, isPdftotextAvailable, getPDFPageCount, extractPDFText, resetPdftotextCache, shouldRequirePDFPageRange, estimatePDFTextOutputTokens, buildLargePDFGuidance, buildPDFTextTooLargeGuidance, renderPDFPagesToImages, resetPdftoppmCache, PDF_RENDER_UNAVAILABLE_MESSAGE, } from './pdf.js';
...
describe('pdf utilities', () => {
    ...
    describe('PDF budget policy helpers', () => {
        ...
        it('builds exact page-range guidance for pdfinfo-backed and heuristic counts', () => {
            expect(buildLargePDFGuidance('paper.pdf', {
                required: true,
                effectivePageCount: 42,
                hadPdfInfo: true,
            })).toBe("PDF \"paper.pdf\" has 42 pages, which is too many to read at once. Use the 'pages' parameter to read a specific page range such as '1-5'. Maximum 20 pages per request.");
            expect(buildLargePDFGuidance('scan.pdf', {
                required: true,
                effectivePageCount: 21,
                hadPdfInfo: false,
            })).toBe("PDF \"scan.pdf\" appears to have about 21 pages, which is too many to read at once. Use the 'pages' parameter to read a specific page range such as '1-5'. Maximum 20 pages per request.");
        });
        it('builds exact dense-text guidance for range and single-page reads', () => {
            const defaultRangeGuidance = "PDF text extracted from \"paper.pdf\" is too large to return safely (12345 estimated tokens; limit 12000). Use the 'pages' parameter with a narrower range, for example '1-2' or a single page.";
            const fivePageRangeGuidance = "PDF text extracted from \"paper.pdf\" is too large to return safely (12345 estimated tokens; limit 12000). Use the 'pages' parameter with fewer pages, for example '1-3' or a single page.";
            const twoPageRangeGuidance = "PDF text extracted from \"paper.pdf\" is too large to return safely (12345 estimated tokens; limit 12000). Use the 'pages' parameter with a single page, for example '1'.";
            const singlePageGuidance = 'PDF text extracted from "paper.pdf" is too large to return safely (12345 estimated tokens; limit 12000). The selected page exceeds the output limit. Use a native PDF-capable model, split the page content externally, or extract a smaller section with another tool.';
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345)).toBe(defaultRangeGuidance);
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345, '1-5')).toBe(fivePageRangeGuidance);
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345, '1-2')).toBe(twoPageRangeGuidance);
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345, '1')).toBe(singlePageGuidance);
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345, '1-1')).toBe(singlePageGuidance);
            expect(buildPDFTextTooLargeGuidance('paper.pdf', 12_345, '1 - 1')).toBe(singlePageGuidance);
        });
    });
    ...
    describe('renderPDFPagesToImages', () => {
        ...
        const mockAvailable = () => mockExecResult({
            stdout: '',
            stderr: 'pdftoppm version 24.02.0',
            code: 0,
        });
        const asEntries = (names) => names;
        it('renders pages to base64 JPEG images, numerically sorted', async () => {
            ...
            expect(renderCall[1]).toEqual(expect.arrayContaining(['-jpeg', '-scale-to']));
        });
