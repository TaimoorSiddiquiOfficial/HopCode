...
import { FetchError, fetchWithPolicy, formatFetchErrorForUser, isPermittedRedirect, isPrivateHost, } from './fetch.js';
function makeTlsError() {
...
describe('fetchWithPolicy', () => {
    ...
    it('wraps a malformed Location header in a FetchError', async () => {
        ...
        await expect(fetchWithPolicy('https://example.com/bad-redirect', opts)).rejects.toThrow(/malformed Location header/);
    });
    ...
    it('errors after exceeding the redirect hop limit', async () => {
        ...
        await expect(fetchWithPolicy('https://example.com/start', opts)).rejects.toThrow(/Too many redirects/);
    });
    it('rejects oversized responses via Content-Length before reading', async () => {
        stubFetch(() => new Response('irrelevant', {
            status: 200,
            headers: { 'content-length': '999999' },
        }));
        await expect(fetchWithPolicy('https://example.com/big', opts)).rejects.toThrow(/Response too large/);
    });
    it('rejects oversized responses while streaming when no Content-Length', async () => {
        ...
        await expect(fetchWithPolicy('https://example.com/big-stream', opts)).rejects.toThrow(/exceeded the 1000-byte limit while streaming/);
    });
    it('propagates caller aborts', async () => {
        const controller = new AbortController();
        controller.abort(new Error('user cancelled'));
        stubFetch(() => new Response('never', { status: 200 }));
        await expect(fetchWithPolicy('https://example.com/x', {
            ...opts,
...
describe('fetchWithPolicy retry', () => {
    ...
    it.each(['ECONNRESET', 'EAI_AGAIN'])('retries once on transient network errors (%s)', async (code) => {
        let calls = 0;
        globalThis.fetch = vi.fn(async () => {
            calls++;
            if (calls === 1) {
    ...
    it('does not retry non-transient network errors', async () => {
        ...
        await expect(fetchWithPolicy('https://example.com/tls', opts)).rejects.toThrow('fetch failed');
        expect(calls).toBe(1);
    });
});
describe('fetchWithPolicy retry abort handling', () => {
    ...
    it('propagates caller abort over the original 403 during the retry window', async () => {
        ...
        await expect(fetchWithPolicy('https://example.com/blocked', {
            ...opts,
            signal: controller.signal,
        })).rejects.toThrow('user cancelled');
        expect(calls).toBe(1);
