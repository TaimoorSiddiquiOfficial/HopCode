...
import { isWithinRoot, isBinaryFile, detectFileType, processSingleFileContent, detectBOM, decodeBufferWithEncodingInfo, readFileWithLineAndLimit, readFileWithEncoding, readFileWithEncodingInfo, detectFileEncoding, fileExists, } from './fileUtils.js';
import { iconvEncode } from './iconvHelper.js';
import { LargeNonUtf8TextError } from './read-text-range.js';
import { StandardFileSystemService } from '../services/fileSystemService.js';
import { ToolErrorType } from '../tools/tool-error.js';
import { PDF_MAX_PAGES_PER_READ, renderPDFPagesToImages, resetPdftotextCache, } from './pdf.js';
...
describe('fileUtils', () => {
    ...
    describe('detectFileType', () => {
        ...
        it('uses content detection for text-looking .dat files', async () => {
            ...
            actualNodeFs.writeFileSync(filePath, '<?php\nfunction handleRequest() {\n  return true;\n}\n');
            try {
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should surface messages from plain object text read errors', async () => {
            ...
            vi.spyOn(fsService, 'readTextFile').mockRejectedValueOnce({
                code: -32603,
                message: 'path escapes workspace: /root/.qwen/skills/dataworks-di-data-processor/instructions/interaction_norms.md',
                data: {
        ...
        it('keeps image inline when preserveUnsupportedImage is true', async () => {
            ...
            const result = await processSingleFileContent(testImageFilePath, mockConfigNoImage, { preserveUnsupportedImage: true });
        ...
        it('still strips audio when preserveUnsupportedImage is true', async () => {
            ...
            const result = await processSingleFileContent(testAudioPath, mockConfigNoAudio, { preserveUnsupportedImage: true });
            expect(typeof result.llmContent).toBe('string');
        ...
        it('rejects large full-PDF text fallback before extracting text', async () => {
            actualNodeFs.writeFileSync(testPdfFilePath, Buffer.alloc(2 * 1024 * 1024));
            mockMimeGetType.mockReturnValue('application/pdf');
            mockExecResult({
                stdout: 'Pages:          42\n',
                stderr: '',
        ...
        it('returns a reference instead of an error for large @-attached PDFs', async () => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, { largePdfBehavior: 'reference' });
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toContain("Use the 'pages' parameter");
        ...
        it('keeps explicit pages reads on the pdftotext path', async () => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, { pages: '1' });
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toBe('page one text');
            expect(mockExecFile.mock.calls.some((call) => call[0] === 'pdftotext')).toBe(true);
        });
        it.each([
            ['abc', 'Invalid pages parameter'],
            ['1-', 'Open-ended page ranges'],
            ['1-21', 'Pages range exceeds maximum of 20'],
        ])('rejects unsafe internal pages value %s before extracting text', async (pages, expectedMessage) => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, { pages });
        ...
        it('rejects dense no-pages PDF extraction after exact page count allows full reads', async () => {
            ...
            expect(String(result.llmContent).length).toBeLessThan(1000);
            expect(mockExecFile.mock.calls.some((call) => call[0] === 'pdfinfo')).toBe(true);
        });
        ...
        it('rejects dense page-range PDF extraction for @ attachments', async () => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, { pages: '1-5', largePdfBehavior: 'reference' });
            expect(result.errorType).toBe(ToolErrorType.FILE_TOO_LARGE);
            expect(result.returnDisplay).toContain('PDF text too large');
            expect(String(result.llmContent).length).toBeLessThan(1000);
            expect(result.stats).toBeDefined();
        ...
        it('should skip the 10MB size gate when extracting PDF text by pages', async () => {
            ...
            try {
                ...
                const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, { pages: '1-5' });
        ...
        describe('PDF image rendering (vision fallback)', () => {
            ...
            const fakeImage = (data) => ({ data, mimeType: 'image/jpeg' });
            it('renders the requested page range when text overflows', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, visionConfig, { pages: '1-2' });
                expect(result.error).toBeUndefined();
            ...
            it('renders the whole document (up to the ceiling) for a no-pages overflow', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, visionConfig);
                expect(Array.isArray(result.llmContent)).toBe(true);
            ...
            it('flags truncation (never drops pages silently)', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, visionConfig, { pages: '1-5' });
                const parts = result.llmContent;
                expect(parts.some((p) => typeof p.text === 'string' && /omitted/.test(p.text))).toBe(true);
            });
            it('notes the page ceiling when a no-pages render fills it (page count unknown)', async () => {
                ...
                mockRender.mockResolvedValue({
                    success: true,
                    images: Array.from({ length: PDF_MAX_PAGES_PER_READ }, (_, i) => fakeImage(`P${i + 1}`)),
                    bytesTruncated: false,
                });
                const result = await processSingleFileContent(testPdfFilePath, visionConfig);
                const parts = result.llmContent;
                expect(parts.filter((p) => p.inlineData).length).toBe(PDF_MAX_PAGES_PER_READ);
                expect(parts.some((p) => typeof p.text === 'string' && /per-read maximum/.test(p.text))).toBe(true);
            });
            it('falls back to text guidance when the renderer is unavailable', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, visionConfig, { pages: '1' });
                expect(result.errorType).toBe(ToolErrorType.FILE_TOO_LARGE);
                expect(result.llmContent).toContain('too large to return safely');
            });
        ...
        describe('PDF vision-bridge rendering (text-only model)', () => {
            ...
            it('renders up to VISION_BRIDGE_MAX_IMAGES pages for a scanned @ PDF', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { preserveUnsupportedImage: true, largePdfBehavior: 'reference' });
                expect(Array.isArray(result.llmContent)).toBe(true);
                expect(mockRender).toHaveBeenCalledWith(testPdfFilePath, {
                    firstPage: 1,
                    lastPage: 2,
            ...
            it('notes how many pages were rendered when more remain', async () => {
                ...
                expect(parts.some((p) => typeof p.text === 'string' &&
                    /pages 5-10 were not included/.test(p.text))).toBe(true);
            });
            it('notes truncation when the render fills the cap and the page count is unknown', async () => {
                ...
                mockRender.mockResolvedValue({
                    success: true,
                    images: Array.from({ length: VISION_BRIDGE_MAX_IMAGES }, (_, i) => fakeImage(`B${i + 1}`)),
                    bytesTruncated: false,
                });
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { preserveUnsupportedImage: true, largePdfBehavior: 'reference' });
                const parts = result.llmContent;
                expect(parts.filter((p) => p.inlineData).length).toBe(VISION_BRIDGE_MAX_IMAGES);
                // No exact count is known, so no "of N", but truncation is still noted.
                const note = parts.find((p) => typeof p.text === 'string' && /later pages may remain/.test(p.text));
            ...
            it('renders from the requested start page and records the remaining range', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { pages: '20-25', preparePdfForVisionBridge: true });
                expect(mockRender).toHaveBeenCalledWith(testPdfFilePath, {
                    firstPage: 20,
                    lastPage: 23,
                });
                ...
                expect(parts
                    ...
                    .map((part) => part.inlineData?.displayName)).toEqual([
            ...
            it('clips an explicit range to the actual PDF and does not invent remaining pages', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { pages: '4-8', preparePdfForVisionBridge: true });
                expect(mockRender).toHaveBeenCalledWith(testPdfFilePath, {
                    firstPage: 4,
                    lastPage: 6,
                });
            ...
            it('marks continuation as possible when an unknown PDF fills the render cap', async () => {
                ...
                expect(JSON.stringify(result.llmContent)).toContain('additional requested pages may exist from page 24 through page 25');
            });
            ...
            it('prepares a candidate when an explicit single page still overflows', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { pages: '20', preparePdfForVisionBridge: true });
                expect(mockRender).toHaveBeenCalledWith(testPdfFilePath, {
                    firstPage: 20,
                    lastPage: 20,
                });
            ...
            it('renders an actual one-page @ PDF when its text overflows', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig, { preserveUnsupportedImage: true, largePdfBehavior: 'reference' });
                expect(result.error).toBeUndefined();
                expect(result.llmContent.filter((part) => part.inlineData)).toHaveLength(1);
                expect(result.pdfVisionBridgeCandidate).toBeUndefined();
            });
            it('records unrendered requested pages when the byte budget truncates images', async () => {
                ...
                expect(JSON.stringify(result.llmContent)).toContain('pages 22-25 were not included');
            });
            ...
            it('does not bridge explicit page overflow for a native PDF model', async () => {
                ...
                const result = await processSingleFileContent(testPdfFilePath, nativePdfConfig, { pages: '20', preparePdfForVisionBridge: true });
                expect(result.errorType).toBe(ToolErrorType.FILE_TOO_LARGE);
                expect(result.pdfVisionBridgeCandidate).toBeUndefined();
                expect(mockRender).not.toHaveBeenCalled();
            });
            ...
            it('does not render without the bridge flag (scanned stays an error)', async () => {
                ...
                mockExecResult({ stdout: '', stderr: 'pdftotext version', code: 0 });
                mockExecResult({ stdout: '   ', stderr: '', code: 0 });
                const result = await processSingleFileContent(testPdfFilePath, bridgeConfig);
                expect(result.errorType).toBe(ToolErrorType.READ_CONTENT_FAILURE);
                expect(result.llmContent).toContain('Cannot extract text from PDF');
                expect(mockRender).not.toHaveBeenCalled();
            });
            it('does not preserve ordinary images with the PDF-only bridge flag', async () => {
                ...
                mockMimeGetType.mockReturnValue('image/png');
                const result = await processSingleFileContent(testImageFilePath, bridgeConfig, { preparePdfForVisionBridge: true });
                expect(result.llmContent).toContain('Unsupported image file');
                expect(Array.isArray(result.llmContent)).toBe(false);
            });
        });
  ...
  `;
        ...
        it('should paginate text files correctly (offset and limit)', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, { offset: 5, limit: 5 }); // Read lines 6-10
        ...
        it('should preserve legacy positional pagination arguments', async () => {
            const lines = Array.from({ length: 20 }, (_, i) => `Line ${i + 1}`);
            actualNodeFs.writeFileSync(testTextFilePath, lines.join('\n'));
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 5, 5);
        ...
        it('should identify truncation when reading the end of a file', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, { offset: 10, limit: 10 });
        ...
        it('should handle limit exceeding file length', async () => {
            ...
            actualNodeFs.writeFileSync(testTextFilePath, lines.join('\n'));
            const result = await processSingleFileContent(testTextFilePath, mockConfig, { offset: 0, limit: 10 });
        ...
        it('should stream explicit offset reads for large text files', async () => {
            actualNodeFs.writeFileSync(testTextFilePath, `skip\n${'line\n'.repeat(3 * 1024 * 1024)}`);
        ...
        it('should propagate large non-UTF-8 errors through bounded reads', async () => {
            ...
            const gbkChunk = Buffer.concat(Array.from({ length: 1024 }, () => gbkLine));
            const repeatCount = Math.ceil((11 * 1024 * 1024) / gbkChunk.length);
            actualNodeFs.writeFileSync(testTextFilePath, Buffer.concat(Array.from({ length: repeatCount }, () => gbkChunk)));
            await expect(readFileWithLineAndLimit({
                path: testTextFilePath,
                limit: 10,
        ...
        it('should propagate aborts before large unbounded full reads', async () => {
            actualNodeFs.writeFileSync(testTextFilePath, 'x'.repeat(11 * 1024 * 1024));
            const controller = new AbortController();
            controller.abort();
            await expect(readFileWithLineAndLimit({
                path: testTextFilePath,
        ...
        it('should truncate when line count exceeds the limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, { offset: 0, limit: 5 });
            expect(result.isTruncated).toBe(true);
        ...
        it('should truncate when a line length exceeds the character limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, { offset: 0, limit: 11 });
            expect(result.isTruncated).toBe(true);
        ...
        it('should read large text files through bounded truncation instead of the 10MB gate', async () => {
            const lines = Array.from({ length: 65_000 }, (_, index) => `Line ${index + 1} ${'x'.repeat(180)}`);
            actualNodeFs.writeFileSync(testTextFilePath, lines.join('\n'));
            const result = await processSingleFileContent(testTextFilePath, mockConfig);
            expect(result.error).toBeUndefined();
            ...
            expect(result.isTruncated).toBe(true);
            expect(result.originalLineCount).toBeGreaterThanOrEqual(result.linesShown?.[1] ?? 1);
        ...
        it('should stream large text files when line truncation is disabled', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, noLineLimitConfig);
            expect(result.error).toBeUndefined();
            expect(typeof result.llmContent).toBe('string');
            expect(result.llmContent).toContain('... [truncated]');
            expect(result.returnDisplay).toBe('Read lines 1-1 of at least 1 from test.txt (truncated)');
            expect(result.isTruncated).toBe(true);
            expect(result.originalLineCountExact).toBe(false);
        });
        it('should mark byte truncation metadata without character truncation', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, byteTruncatedConfig);
            expect(typeof result.llmContent).toBe('string');
        ...
        it('should use selected range as a lower bound when large file metadata is missing', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, missingMetadataConfig, { offset: 9, limit: 2 });
            expect(result.originalLineCount).toBe(11);
            expect(result.originalLineCountExact).toBe(false);
            expect(result.returnDisplay).toBe('Read lines 10-11 of at least 11 from test.txt');
        });
        it('should preserve disabled output truncation for large text files', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, noCharacterLimitConfig);
            expect(typeof result.llmContent).toBe('string');
            const llmContent = result.llmContent;
        ...
        it('should still return an error if an inline media file exceeds 10MB', async () => {
            mockMimeGetType.mockReturnValue('image/png');
            actualNodeFs.writeFileSync(testImageFilePath, Buffer.alloc(11 * 1024 * 1024));
            const result = await processSingleFileContent(testImageFilePath, mockConfig);
