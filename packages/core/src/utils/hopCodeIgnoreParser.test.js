/**
 * @license
 * Copyright 2025 HopCode Team
 * SPDX-License-Identifier: Apache-2.0
 */
import { vi, describe, it, expect, beforeEach, afterEach } from 'vitest';
import { HopCodeIgnoreParser } from './hopCodeIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';
describe('HopCodeIgnoreParser', () => {
    let projectRoot;
    async function createTestFile(filePath, content = '') {
        const fullPath = path.join(projectRoot, filePath);
        await fs.mkdir(path.dirname(fullPath), { recursive: true });
        await fs.writeFile(fullPath, content);
    }
    beforeEach(async () => {
        projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcodeignore-test-'));
    });
    afterEach(async () => {
        await fs.rm(projectRoot, { recursive: true, force: true });
        vi.restoreAllMocks();
    });
    describe('when .hopcodeignore exists', () => {
        beforeEach(async () => {
            await createTestFile('.hopcodeignore', 'ignored.txt\n# A comment\n/ignored_dir/\n');
            await createTestFile('ignored.txt', 'ignored');
            await createTestFile('not_ignored.txt', 'not ignored');
            await createTestFile(path.join('ignored_dir', 'file.txt'), 'in ignored dir');
            await createTestFile(path.join('subdir', 'not_ignored.txt'), 'not ignored');
        });
        it('should ignore files specified in .hopcodeignore', () => {
            const parser = new HopCodeIgnoreParser(projectRoot);
            expect(parser.getPatterns()).toEqual(['ignored.txt', '/ignored_dir/']);
            expect(parser.isIgnored('ignored.txt')).toBe(true);
            expect(parser.getIgnoreFileNameForPath('ignored.txt')).toBe('.hopcodeignore');
            expect(parser.isIgnored('not_ignored.txt')).toBe(false);
            expect(parser.getIgnoreFileNameForPath('not_ignored.txt')).toBe(undefined);
            expect(parser.isIgnored(path.join('ignored_dir', 'file.txt'))).toBe(true);
            expect(parser.isIgnored(path.join('subdir', 'not_ignored.txt'))).toBe(false);
        });
        it('should still evaluate files whose names start with two dots', async () => {
            await createTestFile('.hopcodeignore', '..secret.log');
            const parser = new HopCodeIgnoreParser(projectRoot);
            expect(parser.isIgnored('..secret.log')).toBe(true);
        });
        it('should not evaluate paths outside the project root', () => {
            const parser = new HopCodeIgnoreParser(projectRoot);
            expect(parser.isIgnored(path.join('..', '..secret.log'))).toBe(false);
        });
    });
    describe('when .hopcodeignore does not exist', () => {
        it('should not load any patterns and not ignore any files', () => {
            const parser = new HopCodeIgnoreParser(projectRoot);
            expect(parser.getPatterns()).toEqual([]);
            expect(parser.isIgnored('any_file.txt')).toBe(false);
        });
    });
});
//# sourceMappingURL=hopCodeIgnoreParser.test.js.map