...
import { isSubpath, HOPCODE_DIR } from './paths.js';
...
async function getGeminiMdFilePathsInternalForEachDir(dir, userHomePath, fileService, extensionContextFilePaths = [], folderTrust, implicitDiscoveryEnabled = true) {
    ...
    for (const geminiMdFilename of geminiMdFilenames) {
        ...
        if (isHomeDirectory) {
            // For home directory, only check for HOPCODE.md directly in the home directory
            ...
            while (currentDir && currentDir !== path.dirname(currentDir)) {
                if (currentDir === globalHopcodeDir ||
...
function createMemoryTypeClassifier(userHomePath, foundRoot, extensionContextFilePaths) {
    const resolvedHome = path.resolve(userHomePath);
    const globalHopcodeDir = path.resolve(Storage.getGlobalHopCodeDir());
    ...
    return (filePath) => {
        ...
        if (resolvedPath.startsWith(`${globalHopcodeDir}${path.sep}`)) {
            return 'user';
        }
        if (resolvedRoot &&
            resolvedPath === path.join(resolvedRoot, HOPCODE_DIR, LOCAL_CONTEXT_FILENAME)) {
            return 'local';
        }
...
export async function loadServerHierarchicalMemory(currentWorkingDirectory, includeDirectoriesToReadGemini, fileService, extensionContextFilePaths = [], folderTrust, importFormat = 'tree', contextRuleExcludes = [], options = {}) {
    ...
    // Resolve project root once — needed both for the HOPCODE.local.md slot
    ...
    // `<projectRoot>/.hopcode/HOPCODE.local.md`. Loaded after all hierarchical
    // HOPCODE.md / AGENTS.md files so local instructions can supplement or
    ...
    if (implicitDiscoveryEnabled && folderTrust && foundRoot) {
        const localContextPath = path.join(foundRoot, HOPCODE_DIR, LOCAL_CONTEXT_FILENAME);
        try {
    ...
    if (filePaths.length > 0) {
        ...
        // Only count files that match configured memory filenames (e.g., HOPCODE.md),
    ...
    // Load path-based context rules from .hopcode/rules/ directories.
    ...
    if (!memoryContent && filePaths.length === 0 && ruleCount === 0) {
        logger.debug('No HOPCODE.md files or rules found.');
    }
    return {
        memoryContent,
        fileCount,
