...
import { compactStringForHistory, compactStringForRecording, compactToolResultDisplayForHistory, compactToolResultDisplayForRecording, MAX_RETAINED_AGENT_FIELD_CHARS, MAX_RETAINED_ANSI_OUTPUT_LINES, MAX_RETAINED_FILE_CONTENT_CHARS, MAX_RETAINED_FILE_DIFF_CHARS, MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS, } from './toolResultDisplayCompaction.js';
...
describe('toolResultDisplayCompaction', () => {
    ...
    it('keeps head and tail when compacting long strings', () => {
        const value = `start-${'x'.repeat(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS)}-end`;
        const compacted = compactStringForHistory(value);
        expect(compacted.length).toBeLessThanOrEqual(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS);
    ...
    it('does not split surrogate pairs at compaction boundaries', () => {
        ...
        const value = `${'h'.repeat(8)}${emoji}${'m'.repeat(183)}${emoji}${'t'.repeat(5)}`;
    ...
    it('drops subagent display fields that are not rendered in CLI history', () => {
        const nestedDisplay = `nested-${'x'.repeat(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS)}-done`;
        const display = {
            type: 'task_execution',
        ...
        expect(compacted.taskPrompt.length).toBeLessThanOrEqual(MAX_RETAINED_AGENT_FIELD_CHARS);
    ...
    it('compacts file diffs through the history display path', () => {
        const display = {
            ...
            originalContent: `old-${'o'.repeat(MAX_RETAINED_FILE_CONTENT_CHARS)}-done`,
        ...
        expect(compacted.fileDiff.length).toBeLessThanOrEqual(MAX_RETAINED_FILE_DIFF_CHARS);
        expect(compacted.originalContent?.length).toBeLessThanOrEqual(MAX_RETAINED_FILE_CONTENT_CHARS);
        expect(compacted.newContent.length).toBeLessThanOrEqual(MAX_RETAINED_FILE_CONTENT_CHARS);
        expect(compacted.truncatedForSession).toBe(true);
        expect(compacted.fileDiffLength).toBe(display.fileDiff.length);
        expect(compacted.originalContentLength).toBe(display.originalContent?.length);
    ...
    it('compacts ansi output tokens under the retained line limit', () => {
        const display = {
            ...
            ansiOutput: [
                [
                    {
                        text: `line-${'x'.repeat(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS)}-done`,
                        bold: false,
                        italic: false,
                        underline: false,
                        dim: false,
    ...
    it('bounds long ansi output and keeps the tail lines', () => {
        const display = {
            ansiOutput: Array.from({ length: MAX_RETAINED_ANSI_OUTPUT_LINES + 5 }, (_, index) => [
                {
                    text: `line-${index}`,
                    bold: false,
                    italic: false,
        ...
        expect(compacted.ansiOutput.at(-1)?.[0].text).toBe(`line-${MAX_RETAINED_ANSI_OUTPUT_LINES + 4}`);
    });
    it('uses saved session wording when compacting recording ansi output', () => {
        ...
        expect(compacted.ansiOutput[0][0].text).toContain('terminal lines omitted from saved session preview');
        expect(compacted.ansiOutput[0][0].text).not.toContain('CLI history display');
    });
    it('compacts todo, plan, and MCP progress displays', () => {
        ...
        const compactedProgress = compactToolResultDisplayForHistory(progressDisplay);
...
//# sourceMappingURL=toolResultDisplayCompaction.test.js.map