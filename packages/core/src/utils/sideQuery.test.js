...
describe('runSideQuery', () => {
    ...
    describe('JSON mode (schema present)', () => {
        ...
        it('adds the configured output language to JSON side queries', async () => {
            ...
            try {
                ...
                vi.mocked(mockConfig.getOutputLanguageFilePath).mockReturnValue(outputLanguagePath);
                vi.mocked(mockBaseLlmClient.generateJson).mockResolvedValue({
                    title: '测试标题',
                });
                await runSideQuery(mockConfig, {
                ...
                expect(callArg.systemInstruction).toContain('请始终用中文回答用户可见文本。');
            }
            finally {
    ...
    describe('text mode (no schema)', () => {
        ...
        it('forwards stream:true to generateText when opted in', async () => {
            ...
            expect(mockBaseLlmClient.generateText).toHaveBeenCalledWith(expect.objectContaining({ stream: true }));
        });
        ...
        it('adds the configured output language to text side queries', async () => {
            ...
            try {
                ...
                const callArg = vi.mocked(mockBaseLlmClient.generateText).mock
                    .calls[0][0];
                expect(callArg.systemInstruction).toContain('Summarize the tool batch.');
