...
const HOPCODE_IGNORE_FILE_NAME = '.hopcodeignore';
const debugLogger = createDebugLogger('HOPCODE_IGNORE');
export const DEFAULT_HOPCODE_CUSTOM_IGNORE_FILE_NAMES = [
    '.agentignore',
    '.aiignore',
];
export function normalizeHopCodeCustomIgnoreFileNames(ignoreFileNames = DEFAULT_HOPCODE_CUSTOM_IGNORE_FILE_NAMES) {
    const normalized = [];
    const seen = new Set();
...
function getCustomIgnoreFileNameSkipReason(candidate) {
    ...
    if (candidate === HOPCODE_IGNORE_FILE_NAME) {
        return '.hopcodeignore is always included';
    }
...
export function getHopCodeIgnoreFileNames(customIgnoreFileNames) {
    return [
        HOPCODE_IGNORE_FILE_NAME,
        ...normalizeHopCodeCustomIgnoreFileNames(customIgnoreFileNames),
    ];
}
export function formatHopCodeIgnoreFileNames(customIgnoreFileNames) {
    return getHopCodeIgnoreFileNames(customIgnoreFileNames).join(', ');
}
export class HopCodeIgnoreParser {
    projectRoot;
    patterns = [];
    ignoreFileNames;
    sourceIgnorers = [];
    constructor(projectRoot, customIgnoreFileNames) {
        ...
        this.ignoreFileNames = getHopCodeIgnoreFileNames(customIgnoreFileNames);
        this.loadPatterns();
    }
    loadPatterns() {
...
//# sourceMappingURL=hopCodeIgnoreParser.js.map