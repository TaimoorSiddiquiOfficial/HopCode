...
import { escapePath, formatDisplayPath, resolvePath, validatePath, resolveAndValidatePath, unescapePath, isSubpath, shortenPath, tildeifyPath, expandHomeDir, getProjectHash, _resetValidatePathCacheForTest, } from './paths.js';
...
describe('formatDisplayPath', () => {
    ...
    it('resolves relative input against the root before formatting', () => {
        expect(formatDisplayPath(path.join('src', 'app'), root)).toBe(path.join('src', 'app'));
    ...
    it('shortens the home directory to ~ for paths outside the project', () => {
        ...
        expect(formatDisplayPath(target, root)).toBe(`~${path.sep}elsewhere${path.sep}file.txt`);
    });
    it('does not tildeify project-internal paths when the project is under home', () => {
        ...
        expect(formatDisplayPath(target, homeRoot)).toBe(path.join('src', 'main.ts'));
    });
    it('expands a tilde-prefixed input like other tool paths', () => {
        expect(formatDisplayPath(path.join('~', 'data'), root)).toBe(`~${path.sep}data`);
    });
    it('compresses overlong paths with shortenPath semantics', () => {
        const target = path.join(root, 'very', 'deeply', 'nested', 'directory', 'structure', 'file.ts');
...
describe('expandHomeDir', () => {
    ...
    it('should preserve trailing separators in Windows-style tilde paths', () => {
        expect(expandHomeDir('~\\documents\\')).toBe(path.normalize(path.join(homeDir, 'documents') + path.sep));
    });
    it('should handle mixed separators in Windows-style tilde paths', () => {
        expect(expandHomeDir('~\\foo/bar\\baz')).toBe(path.join(homeDir, 'foo', 'bar', 'baz'));
    });
    it('should preserve legacy POSIX tilde path semantics', () => {
        expect(expandHomeDir('~/foo\\bar')).toBe(path.normalize(path.join(homeDir, 'foo\\bar')));
    });
    ...
    it('should expand %USERPROFILE%/path with forward-slash separator', () => {
        expect(expandHomeDir('%USERPROFILE%/documents')).toBe(path.join(homeDir, 'documents'));
    });
    it('should preserve trailing separators for %USERPROFILE% paths', () => {
        expect(expandHomeDir('%USERPROFILE%/')).toBe(path.normalize(homeDir + path.sep));
        expect(expandHomeDir('%USERPROFILE%\\documents\\')).toBe(path.normalize(path.join(homeDir, 'documents') + path.sep));
    });
    it('should preserve legacy %USERPROFILE% prefix semantics without a separator', () => {
        expect(expandHomeDir('%USERPROFILE%foo')).toBe(path.normalize(`${homeDir}foo`));
    });
