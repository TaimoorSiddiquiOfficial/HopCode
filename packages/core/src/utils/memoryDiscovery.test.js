...
import { setGeminiMdFilename, DEFAULT_CONTEXT_FILENAME, LOCAL_CONTEXT_FILENAME, } from '../memory/const.js';
...
describe('loadServerHierarchicalMemory', () => {
    ...
    it('should skip implicit global, project, and rule discovery in explicit-only mode', async () => {
        await createTestFile(path.join(homedir, HOPCODE_DIR, DEFAULT_CONTEXT_FILENAME), 'global context');
        ...
        await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'rules', 'baseline.md'), 'project rule');
    ...
    it('should load only the global context file if present and others are not (default filename)', async () => {
        const defaultContextFile = await createTestFile(path.join(homedir, HOPCODE_DIR, DEFAULT_CONTEXT_FILENAME), 'default context content');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should load only the global custom context file if present and filename is changed', async () => {
        ...
        const customContextFile = await createTestFile(path.join(homedir, HOPCODE_DIR, customFilename), 'custom context content');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should load extension context file paths', async () => {
        const extensionFilePath = await createTestFile(path.join(testRootDir, 'extensions/ext1/HOPCODE.md'), 'Extension memory content');
    ...
    it('notifies when startup instruction files are loaded', async () => {
        const globalFile = await createTestFile(path.join(homedir, HOPCODE_DIR, DEFAULT_CONTEXT_FILENAME), 'global context');
        const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context');
        const extensionFile = await createTestFile(path.join(testRootDir, 'extensions/ext1/HOPCODE.md'), 'extension context');
        const notifications = [];
        await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [extensionFile], DEFAULT_FOLDER_TRUST, 'tree', [], {
    ...
    it('uses refresh load reason for explicit memory refreshes', async () => {
        const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context');
        const notifications = [];
        await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            loadReason: 'refresh',
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
            },
    ...
    it('classifies home-directory project files as project memory', async () => {
        ...
        const globalFile = await createTestFile(path.join(homedir, HOPCODE_DIR, DEFAULT_CONTEXT_FILENAME), 'global context');
        const projectFile = await createTestFile(path.join(homedir, DEFAULT_CONTEXT_FILENAME), 'home project context');
        const notifications = [];
        await loadServerHierarchicalMemory(homedir, [], new FileDiscoveryService(homedir), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
            },
        });
    ...
    it('notifies when imported instruction files are loaded', async () => {
        ...
        const importedFile = await createTestFile(path.join(projectRoot, 'included.md'), 'included content');
        const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context @./included.md');
        const notifications = [];
        await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
        ...
        expect(notifications.findIndex((item) => item.filePath === projectFile)).toBeGreaterThan(notifications.findIndex((item) => item.filePath === importedFile));
    });
    it('inherits memory type from the importing instruction file', async () => {
        const importedFile = await createTestFile(path.join(homedir, 'rules', 'personal.md'), 'personal included content');
        const userFile = await createTestFile(path.join(homedir, DEFAULT_CONTEXT_FILENAME), 'user context @./rules/personal.md');
        const notifications = [];
        await loadServerHierarchicalMemory(homedir, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
            },
        });
    ...
    it('inherits memory type from the root instruction file for nested imports', async () => {
        const nestedFile = await createTestFile(path.join(homedir, 'rules', 'nested.md'), 'nested included content');
        const importedFile = await createTestFile(path.join(homedir, 'rules', 'personal.md'), 'personal included content @./nested.md');
        const userFile = await createTestFile(path.join(homedir, DEFAULT_CONTEXT_FILENAME), 'user context @./rules/personal.md');
        const notifications = [];
        await loadServerHierarchicalMemory(homedir, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
    ...
    it('reports the root trigger and immediate parent for nested imports', async () => {
        await createEmptyDir(path.join(projectRoot, '.git'));
        const grandchildFile = await createTestFile(path.join(projectRoot, 'grandchild.md'), 'grandchild content');
        const childFile = await createTestFile(path.join(projectRoot, 'child.md'), 'child content @./grandchild.md');
        const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context @./child.md');
        const notifications = [];
        await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
    ...
    it('classifies extension-owned imports as extension memory', async () => {
        ...
        const importedFile = await createTestFile(path.join(extensionDir, 'included.md'), 'extension included content');
        const extensionFile = await createTestFile(path.join(extensionDir, DEFAULT_CONTEXT_FILENAME), 'extension context @./included.md');
        const notifications = [];
        await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [extensionFile], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: (notification) => {
                notifications.push(notification);
    ...
    it('still loads memory when instruction load notification fails', async () => {
        const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
            onInstructionsLoaded: () => {
                throw new Error('hook failed');
            },
        });
        expect(result.fileCount).toBe(1);
        expect(result.memoryContent).toContain(`--- Context from: ${path.relative(cwd, projectFile)} ---\nproject context`);
        expect(mockLogger.warn).toHaveBeenCalledWith(`InstructionsLoaded notification failed for ${projectFile}: hook failed`);
    });
    ...
    describe('HOPCODE.local.md (project-local context file)', () => {
        ...
        it('loads .hopcode/HOPCODE.local.md from project root when present', async () => {
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'local context content');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(1);
            expect(result.memoryContent).toContain(`--- Context from: ${path.relative(cwd, localFile)} ---\nlocal context content`);
        });
        it('notifies when HOPCODE.local.md is loaded', async () => {
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, LOCAL_CONTEXT_FILENAME), 'local context content');
            const notifications = [];
            await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], {
                onInstructionsLoaded: (notification) => {
                    notifications.push(notification);
        ...
        it('orders HOPCODE.local.md after the project-root HOPCODE.md', async () => {
            const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'shared project context');
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'local override');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(2);
            const projectIdx = result.memoryContent.indexOf(path.relative(cwd, projectFile));
            const localIdx = result.memoryContent.indexOf(path.relative(cwd, localFile));
            expect(projectIdx).toBeGreaterThanOrEqual(0);
            expect(localIdx).toBeGreaterThan(projectIdx);
        });
        it('orders HOPCODE.local.md after upward-traversed CWD HOPCODE.md', async () => {
            const projectFile = await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project root memory');
            const cwdFile = await createTestFile(path.join(cwd, DEFAULT_CONTEXT_FILENAME), 'cwd memory');
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'local memory');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        ...
        it('silently ignores absent .hopcode/HOPCODE.local.md', async () => {
            await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project content');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(1);
        ...
        it('does not load HOPCODE.local.md from untrusted workspaces', async () => {
            await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'local content');
            const { fileCount, memoryContent } = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], false);
        ...
        it('does not search .hopcode/HOPCODE.local.md in CWD subdirectories', async () => {
            // A `.hopcode/HOPCODE.local.md` placed inside a nested directory (not the
            // project root) must NOT be picked up — the slot is single, fixed,
            // and lives at <projectRoot>/.hopcode/HOPCODE.local.md.
            await createTestFile(path.join(cwd, HOPCODE_DIR, 'HOPCODE.local.md'), 'misplaced local content');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(0);
        ...
        it('loads HOPCODE.local.md even when no project HOPCODE.md exists', async () => {
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'standalone local');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(1);
            expect(result.memoryContent).toContain(`--- Context from: ${path.relative(cwd, localFile)} ---\nstandalone local`);
        });
        it('loads HOPCODE.local.md when project root is marked by a .git FILE (worktree / submodule layout)', async () => {
            ...
            await fsPromises.writeFile(path.join(projectRoot, '.git'), 'gitdir: /elsewhere/worktrees/feature/.git\n');
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'worktree local');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(1);
            expect(result.memoryContent).toContain(`--- Context from: ${path.relative(cwd, localFile)} ---\nworktree local`);
        });
        it('skips HOPCODE.local.md when no project root can be found (no .git ancestor)', async () => {
            ...
            await createTestFile(path.join(cwd, HOPCODE_DIR, 'HOPCODE.local.md'), 'cwd-anchored local that must not load');
            await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'projectRoot-anchored local that must not load either');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(0);
            expect(result.memoryContent).not.toContain('cwd-anchored local that must not load');
            expect(result.memoryContent).not.toContain('projectRoot-anchored local that must not load either');
        });
        it('skips HOPCODE.local.md when cwd === homedir without .git (avoids global-dir collision)', async () => {
            ...
            await createTestFile(path.join(homedir, HOPCODE_DIR, 'HOPCODE.local.md'), 'do not promote this to project-local');
            const result = await loadServerHierarchicalMemory(homedir, // cwd === homedir
            [], new FileDiscoveryService(homedir), [], DEFAULT_FOLDER_TRUST);
            ...
            expect(result.memoryContent).not.toContain('do not promote this to project-local');
        });
        it('dedupes when an extension registers the local slot path explicitly', async () => {
            ...
            // (HOPCODE.md / AGENTS.md) and never produces a `HOPCODE.local.md` path,
            ...
            const localFile = await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'HOPCODE.local.md'), 'slot content only once');
            const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [localFile], // extension explicitly registers the slot path
            DEFAULT_FOLDER_TRUST);
            expect(result.fileCount).toBe(1);
            const occurrences = (result.memoryContent.match(/slot content only once/g) ?? []).length;
