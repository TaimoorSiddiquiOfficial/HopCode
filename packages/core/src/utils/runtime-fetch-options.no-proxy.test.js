...
import { getOrCreateSharedDispatcher, resetDispatcherCache, } from './runtimeFetchOptions.js';
describe('shared proxy dispatcher NO_PROXY behavior', () => {
    ...
    it.each(['NO_PROXY', 'no_proxy'])('bypasses the explicit proxy when %s matches the target', async (noProxyKey) => {
        ...
        try {
            ...
            const response = await undiciFetch(`http://127.0.0.1:${originPort}/v1/chat/completions`, {
                dispatcher,
...
//# sourceMappingURL=runtime-fetch-options.no-proxy.test.js.map