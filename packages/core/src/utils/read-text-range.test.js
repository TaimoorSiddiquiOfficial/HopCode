...
describe('readTextRange', () => {
    ...
    function largeUtf8Lines(lineCount) {
        return Array.from({ length: lineCount }, (_, index) => `line-${index + 1} ${'x'.repeat(180)}`).join('\n');
    }
    it('preserves split newline semantics on the fast path', async () => {
        const emptyPath = await writeFile('empty.txt', '');
        await expect(readTextRange({
            path: emptyPath,
            offset: 0,
            limit: 10,
            maxOutputBytes: 100,
    ...
    it('preserves CRLF content and line-ending metadata for large files', async () => {
        const content = Array.from({ length: 65_000 }, (_, index) => `line-${index + 1} ${'x'.repeat(180)}`).join('\r\n');
    ...
    it('strips UTF-8 BOM from large file content and reports BOM metadata', async () => {
        ...
        const filePath = await writeFile('bom.log', Buffer.concat([Buffer.from([0xef, 0xbb, 0xbf]), Buffer.from(body)]));
        const result = await readTextRange({
            path: filePath,
            offset: 0,
            limit: 1,
    ...
    it('rejects large non-UTF-8 files with a targeted error', async () => {
        ...
        const filePath = await writeFile('gbk.log', Buffer.concat(Array.from({ length: repeatCount }, () => gbkLine)));
        await expect(readTextRange({
            path: filePath,
            offset: 0,
            limit: 10,
    ...
    it('bounds selected output for a large single-line file', async () => {
        const filePath = await writeFile('single-line.log', 'x'.repeat(11 * 1024 * 1024));
        const result = await readTextRange({
            path: filePath,
            offset: 0,
            limit: 1,
...
//# sourceMappingURL=read-text-range.test.js.map