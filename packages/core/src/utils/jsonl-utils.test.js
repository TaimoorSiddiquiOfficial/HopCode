...
import { _recoverObjectsFromLine, _resetEnsuredDirsCacheForTest, countLines, parseLineTolerant, read, readLines, write, writeLine, writeLineSync, } from './jsonl-utils.js';
let tmpRoot;
beforeAll(() => {
...
describe('read() / readLines() with malformed lines', () => {
    ...
    it('can rethrow non-ENOENT read errors for user-visible callers', async () => {
        ...
        try {
            await expect(read(file, { throwOnNonEnoentError: true })).rejects.toThrow('permission denied');
        }
        finally {
            spy.mockRestore();
        }
    ...
    it('still returns [] for missing files when rethrowing read errors', async () => {
        await expect(read(path.join(tmpRoot, 'does-not-exist.jsonl'), {
...
describe('writeLine / writeLineSync / write', () => {
    it('writeLine round-trips through read() with flush:true appended records', async () => {
        const file = path.join(tmpRoot, `wl-${Math.random().toString(36).slice(2)}.jsonl`);
        ...
        expect(raw).toBe('{"kind":"a","n":1}\n{"kind":"b","n":2}\n{"kind":"c","n":3}\n');
    });
    it('writeLineSync appends well-formed records with trailing newlines', () => {
        const file = path.join(tmpRoot, `wls-${Math.random().toString(36).slice(2)}.jsonl`);
        writeLineSync(file, { sync: true, i: 0 });
        writeLineSync(file, { sync: true, i: 1 });
        expect(fs.readFileSync(file, 'utf8')).toBe('{"sync":true,"i":0}\n{"sync":true,"i":1}\n');
    });
    it('write() full-file replaces existing content via atomic write', async () => {
        const file = path.join(tmpRoot, `wf-${Math.random().toString(36).slice(2)}.jsonl`);
