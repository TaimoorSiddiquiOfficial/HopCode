...
export function escapeShellArg(arg, shell) {
    ...
    switch (shell) {
        ...
        case 'cmd':
            // Escape cmd.exe meta-characters: double-quote internal quotes and
            // escape % (environment variable), ^ (escape char), & | < > (pipes/redirects),
            // and ! (delayed expansion) within the quoted value.
            return `"${arg
                .replace(/"/g, '""')
                .replace(/%/g, '%%')
                .replace(/\^/g, '^^')
...
const HOPCODE_PROCESS_PATTERN = /(^|[^a-z0-9_-])qwen(?:-code)?(?:\.exe)?(?=$|[^a-z0-9_-])/i;
...
export const SHELL_SELF_KILL_REJECTION = 'Blocked: this command may terminate the running hopcode process because it targets all node/hopcode processes. Use task_stop for managed background shells, or kill a specific PID instead.';
...
function matchesQwenProcessPattern(value) {
    return HOPCODE_PROCESS_PATTERN.test(value);
}
 ...
 */
export const COMMAND_SUBSTITUTION_WARNING = 'Contains command substitution ($(...), backticks, <(...), or >(...)).';
/**
 ...
 */
export async function checkCommandPermissions(command, config, sessionAllowlist, shellType) {
    ...
    if (shellType === 'powershell') {
        ...
        const { evaluatePowerShellCommand } = await import('../security/powershell-security.js');
        const psResult = evaluatePowerShellCommand(command, psConfig);
        if (psResult.isHardDenial || !psResult.allowed) {
            return {
                allAllowed: false,
                disallowedCommands: [command],
                blockReason: psResult.reason ?? 'PowerShell command blocked by security policy.',
