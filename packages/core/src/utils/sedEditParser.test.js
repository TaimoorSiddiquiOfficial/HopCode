...
describe('sedEditParser', () => {
    ...
    it('rejects macOS empty suffix after the long in-place flag', () => {
        expect(parseSedEditCommand("sed --in-place '' 's/foo/bar/' file.txt")).toBeNull();
    });
    ...
    it('parses macOS empty suffix and extended regex flags', () => {
        expect(parseSedEditCommand("sed -i '' -E 's/foo|bar/baz/g' src/a.ts")).toEqual({
            filePath: 'src/a.ts',
            pattern: 'foo|bar',
            replacement: 'baz',
            flags: 'g',
    ...
    it('parses safe combined in-place and extended regex flags', () => {
        ...
        expect(parseSedEditCommand("sed -iE 's/foo|bar/baz/g' src/a.ts")).toBeNull();
    });
    it('parses expression flag forms', () => {
        ...
        expect(parseSedEditCommand("sed -i --expression 's/foo/bar/' file.txt")).toEqual({
            filePath: 'file.txt',
            pattern: 'foo',
            replacement: 'bar',
            flags: '',
    ...
    it('rejects command chains, globs, multiple files, and unsafe flags', () => {
        expect(parseSedEditCommand("sed -i 's/foo/bar/' a.ts && echo done")).toBeNull();
    ...
    it('rejects replacement backrefs without matching capture groups', () => {
        ...
        expect(parseSedEditCommand("sed -E -i 's/(a)(b)/\\1\\3/g' file.txt")).toBeNull();
        expect(parseSedEditCommand("sed -E -i 's/(a)(b)/\\2\\1/g' file.txt")).not.toBeNull();
    });
    ...
    it('rejects quantified alternation groups before simulated edits', () => {
        expect(parseSedEditCommand("sed -E -i 's/(a|aa)*b/X/g' file.txt")).toBeNull();
    });
    it('rejects POSIX bracket expressions before simulated edits', () => {
        expect(parseSedEditCommand("sed -i 's/[[:space:]]*$//' file.txt")).toBeNull();
        expect(parseSedEditCommand("sed -i 's/[[:digit:]]/X/g' file.txt")).toBeNull();
    });
    ...
    it('supports multi-digit numeric occurrences', () => {
        ...
        expect(sedInfo).not.toBeNull();
        expect(applySedSubstitution('x x x x x x x x x x x', sedInfo)).toBe('x x x x x x x x x y x');
    });
    it('supports global substitutions from a numeric occurrence', () => {
        const sedInfo = parseSedEditCommand("sed -i 's/foo/bar/2g' file.txt");
        expect(sedInfo).not.toBeNull();
        expect(applySedSubstitution('foo foo foo foo', sedInfo)).toBe('foo bar bar bar');
    });
...
//# sourceMappingURL=sedEditParser.test.js.map