...
import { fetchGitDiff, fetchGitDiffHunks, fetchGitDiffHunksForFile, getGitWorkingTreeStatus, MAX_DIFF_SIZE_BYTES, MAX_FILES, MAX_LINES_PER_FILE, parseDeletedFromNameStatus, parseGitDiff, parseGitNumstat, parseShortstat, parseStatusBranchLine, parseStatusEntries, resolveGitDir, } from './gitDiff.js';
const execFileAsync = promisify(execFile);
async function git(cwd, ...args) {
    await execFileAsync('git', args, { cwd });
}
async function makeRepo() {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-gitdiff-test-'));
...
describe('fetchGitDiff', () => {
    ...
    it('returns null when not in a git repo', async () => {
        const plain = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-plain-'));
        try {
...
describe('fetchGitDiffHunksForFile', () => {
    ...
    it('reports truncation for an untracked file past the line cap', async () => {
        ...
        await git(repo, 'add', '.');
        await git(repo, 'commit', '-q', '-m', 'init');
        const body = Array.from({ length: MAX_LINES_PER_FILE + 5 }, (_, i) => `line-${i}`).join('\n');
    ...
    it.skipIf(process.platform === 'win32')('returns null for an untracked FIFO without hanging', async () => {
    ...
    it('accepts an absolute path inside the repo and rejects one outside it', async () => {
        ...
        await git(repo, 'commit', '-q', '-m', 'init');
        await fs.writeFile(path.join(repo, 'a.txt'), 'one\nTWO\n');
        const result = await fetchGitDiffHunksForFile(repo, path.join(repo, 'a.txt'));
        expect(result).not.toBeNull();
        expect(result.hunks[0].lines.some((l) => l === '+TWO')).toBe(true);
    ...
    it('accepts a literal `..foo` filename at the root (not a traversal)', async () => {
        ...
        await git(repo, 'commit', '-q', '-m', 'init');
        await fs.writeFile(path.join(repo, '..foo'), 'one\nTWO\n');
        const result = await fetchGitDiffHunksForFile(repo, path.join(repo, '..foo'));
        expect(result).not.toBeNull();
        expect(result.hunks[0].lines.some((l) => l === '+TWO')).toBe(true);
    });
...
describe('resolveGitDir', () => {
    it('returns the .git directory for a regular repo', async () => {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-gitdir-'));
        try {
    ...
    it('follows the gitdir pointer for linked worktrees', async () => {
        const main = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-gitmain-'));
        try {
...
describe('parseStatusBranchLine', () => {
    it('parses branch with upstream and ahead/behind', () => {
        expect(parseStatusBranchLine('## main...origin/main [ahead 2, behind 1]')).toEqual({
            branch: 'main',
            detached: false,
            hasUpstream: true,
            ahead: 2,
    ...
    it('parses ahead-only and behind-only brackets', () => {
        expect(parseStatusBranchLine('## main...origin/main [ahead 3]')).toMatchObject({ ahead: 3, behind: 0 });
        expect(parseStatusBranchLine('## main...origin/main [behind 5]')).toMatchObject({ ahead: 0, behind: 5 });
    });
...
describe('getGitWorkingTreeStatus', () => {
    ...
    it('returns null when not in a git repo', async () => {
        const plain = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-status-plain-'));
        try {
    ...
    it('reports ahead/behind against an upstream', async () => {
        const remote = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-status-remote-'));
        const other = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-status-clone-'));
        try {
