...
describe('retryWithBackoff', () => {
    ...
    it('passes extra retry error codes into retry diagnostics', async () => {
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith(expect.stringContaining('Attempt 1 failed'), expect.objectContaining({
            kind: 'provider',
            diagnosis: 'retryable',
            reason: 'rate-limit',
        }), error);
    ...
    describe('shouldRetryOnContent', () => {
        it('retries on invalid content then returns the valid result', async () => {
            ...
            const promise = retryWithBackoff(fn, {
                maxAttempts: 5,
                initialDelayMs: 10,
                shouldRetryOnContent: (content) => content.text === 'bad',
            });
            await vi.runAllTimersAsync();
    ...
    describe('HopCode OAuth 429 error handling', () => {
        ...
        it('should throw immediately for HopCode OAuth with insufficient_quota message', async () => {
            ...
            await expect(promise).rejects.toThrow(/HopCode OAuth free tier has been discontinued/);
...
describe('isUnattendedMode', () => {
    ...
    beforeEach(() => {
        ...
        delete process.env['HOPCODE_UNATTENDED_RETRY'];
    });
    afterAll(() => {
        process.env = originalEnv;
    });
    it('should return true when HOPCODE_UNATTENDED_RETRY=1', () => {
        process.env['HOPCODE_UNATTENDED_RETRY'] = '1';
        expect(isUnattendedMode()).toBe(true);
    });
    it('should return true when HOPCODE_UNATTENDED_RETRY=true', () => {
        process.env['HOPCODE_UNATTENDED_RETRY'] = 'true';
        expect(isUnattendedMode()).toBe(true);
    });
    ...
    it('should return false for non-matching values', () => {
        process.env['HOPCODE_UNATTENDED_RETRY'] = '0';
        expect(isUnattendedMode()).toBe(false);
        process.env['HOPCODE_UNATTENDED_RETRY'] = 'false';
        expect(isUnattendedMode()).toBe(false);
    ...
    it('should use strict matching consistent with parseBooleanEnvFlag', () => {
        ...
        process.env['HOPCODE_UNATTENDED_RETRY'] = 'yes';
        expect(isUnattendedMode()).toBe(false);
    });
});
describe('retryWithBackoff - persistent mode', () => {
    ...
    it('should NOT retry indefinitely for fail-fast quota 429s in persistent mode', async () => {
        ...
        // eslint-disable-next-line vitest/valid-expect
        const assertionPromise = expect(promise).rejects.toThrow('Allocated quota exceeded');
        await vi.runAllTimersAsync();
        await assertionPromise;
        expect(fn).toHaveBeenCalledTimes(3);
    });
...
describe('retryWithBackoff - Retry-After handling in normal mode', () => {
    ...
    it('logs a 503 Retry-After retry at error level, 429 at warn level', async () => {
        ...
        const p503 = retryWithBackoff(vi.fn().mockRejectedValueOnce(e503).mockResolvedValue('ok'), { maxAttempts: 2, initialDelayMs: 10, maxDelayMs: 100 });
        await vi.runAllTimersAsync();
        await p503;
        expect(debugLoggerMock.error).toHaveBeenCalledWith(expect.stringContaining('Retrying after explicit delay'), expect.anything(), e503);
        debugLoggerMock.error.mockClear();
        const e429 = Object.assign(new Error('Rate limited'), {
            status: 429,
            headers: { 'retry-after': '1' },
        });
        const p429 = retryWithBackoff(vi.fn().mockRejectedValueOnce(e429).mockResolvedValue('ok'), { maxAttempts: 2, initialDelayMs: 10, maxDelayMs: 100 });
        await vi.runAllTimersAsync();
        await p429;
        // 429 throttling stays at warn, never error.
        expect(debugLoggerMock.error).not.toHaveBeenCalled();
        expect(debugLoggerMock.warn).toHaveBeenCalledWith(expect.stringContaining('Retrying after explicit delay'), expect.anything(), e429);
    });
...
describe('retryWithBackoff — Phase 4b retry context (ALS)', () => {
    ...
    it('exposes retryContext.requestSetupMs / retryTotalDelayMs (== 0 for attempt 1, > 0 for retries)', async () => {
        ...
        expect(snapshots[2].totalDelayMs).toBeGreaterThan(snapshots[1].totalDelayMs);
    });
    ...
    it('onRetry callback fires once per failed attempt with correct args', async () => {
        ...
        expect(first.error.message).toContain('attempt 1');
    ...
    it('shouldRetryOnError returns false mid-loop: onRetry not called for the giveup', async () => {
        ...
        const caught = retryWithBackoff(fn, {
            ...
            initialDelayMs: 1,
            maxDelayMs: 5,
            shouldRetryOnError: (e) => e.status === 500 || e.status === 429,
            onRetry,
        }).catch((e) => e);
        await vi.runAllTimersAsync();
    ...
    it('persistent mode (status=429): onRetry fires with correct attempt + delayMs from persistent backoff', async () => {
        ...
        const fn = vi.fn(async () => {
            n++;
            if (n <= 2) {
                const err = new Error(`rate limited #${n}`);
                err.status = 429;
                throw err;
            }
            return 'ok';
