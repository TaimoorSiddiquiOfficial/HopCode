...
import { truncateAndSaveToFile, truncateToolOutput, truncateLlmContent, TOOL_OUTPUT_TRUNCATED_PREFIX, } from './truncation.js';
...
describe('truncateAndSaveToFile', () => {
    ...
    it('should truncate content by lines when line limit is the binding constraint', async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(path.join(projectTempDir, `${fileName}.output`), content, { mode: 0o600 });
    ...
    describe('keep direction', () => {
        ...
        it("keep='head' retains only the beginning", async () => {
            const result = await truncateAndSaveToFile(makeContent(), 'f', '/tmp', THRESHOLD, TRUNCATE_LINES, 'head');
        ...
        it("keep='tail' retains only the end", async () => {
            const result = await truncateAndSaveToFile(makeContent(), 'f', '/tmp', THRESHOLD, TRUNCATE_LINES, 'tail');
        ...
        it("keep='both' (default) retains both ends", async () => {
            const result = await truncateAndSaveToFile(makeContent(), 'f', '/tmp', THRESHOLD, TRUNCATE_LINES);
            expect(result.content).toContain('FIRST_UNIQUE_LINE');
            expect(result.content).toContain('LAST_UNIQUE_LINE');
        });
        it("keep='tail' does not leak a whole line when the per-line tail budget rounds to zero", async () => {
            ...
            const content = 'H'.repeat(50_000) + '\n' + 'B'.repeat(60_000) + '\n' + 'C'.repeat(58);
            const result = await truncateAndSaveToFile(content, 'f', '/tmp', 100, TRUNCATE_LINES, 'tail');
            expect(result.outputFile).toBeDefined();
    ...
    describe('token-aware fallback', () => {
        ...
        it('returns original content when truncation would not save space', async () => {
            ...
            const result = await truncateAndSaveToFile(content, 'f', '/tmp', 50, 1000);
            expect(result).toEqual({ content });
            expect(result.outputFile).toBeUndefined();
            expect(mockWriteFile).not.toHaveBeenCalled();
        });
        it('still truncates when the wrapped output is genuinely smaller', async () => {
            ...
            const content = 'a'.repeat(2_000_000);
            const result = await truncateAndSaveToFile(content, 'f', '/tmp', THRESHOLD, TRUNCATE_LINES);
            expect(result.outputFile).toBeDefined();
...
describe('truncateToolOutput', () => {
    ...
    it('passes promptId into the telemetry event', async () => {
        const content = 'a'.repeat(200_000);
        await truncateToolOutput(mockConfig, 'shell', content, undefined, 'prompt-123');
...
describe('truncateLlmContent', () => {
    ...
    it('truncates a large string and returns an outputFile', async () => {
        const result = await truncateLlmContent(mockConfig, 'shell', 'a'.repeat(200_000));
    ...
    it('is idempotent when content is already truncated', async () => {
        const already = 'Tool output was too large and has been truncated\nThe full output...';
    ...
    it('truncates text parts but preserves media parts in Part[]', async () => {
        ...
        expect(textPart?.text).toContain('Tool output was too large and has been truncated');
        expect(result.outputFile).toBeDefined();
    });
