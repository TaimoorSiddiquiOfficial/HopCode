  ...
  const hintLines = [
    ...
    ...(shouldShowTlsHint
      ? isTlsVerificationDisabled()
        ? [
            '- TLS verification is already disabled (`--insecure` / `hopcode_tls_insecure`), so this is likely a network or protocol issue rather than a certificate trust problem.',
          ]
        : [
            '- If your network uses a corporate TLS inspection CA, set `NODE_EXTRA_CA_CERTS` to your CA bundle.',
            '- For a trusted self-signed endpoint, pass `--insecure` (or set `hopcode_tls_insecure=1`) to skip certificate verification.',
          ]
      : []),
  ];
