...
describe('getRetryAfterDelayMs', () => {
    ...
    it('should ignore malformed and nullish Retry-After header shapes', () => {
        ...
        expect(getRetryAfterDelayMs({ response: { headers: undefined } })).toBeNull();
    });
    it('should cap oversized numeric Retry-After at the setTimeout ceiling', () => {
        ...
        expect(getRetryAfterDelayMs({ headers: { 'retry-after': '2592000' } })).toBe(2_147_483_647);
    });
    it('should reject non-RFC numeric Retry-After shapes', () => {
        ...
        expect(getRetryAfterDelayMs({ headers: { 'retry-after': '0x10' } })).toBeNull();
        expect(getRetryAfterDelayMs({ headers: { 'retry-after': '1e3' } })).toBeNull();
    });
    it('should honor fractional Retry-After seconds', () => {
        expect(getRetryAfterDelayMs({ headers: { 'retry-after': '1.5' } })).toBe(1500);
    });
