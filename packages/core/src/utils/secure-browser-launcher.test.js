...
describe('secure-browser-launcher', () => {
    ...
    describe('URL validation', () => {
        ...
        it('should allow file URLs only when explicitly requested with an allow-list', async () => {
            ...
            await expect(openBrowserSecurely('file:///tmp/report.html')).rejects.toThrow('Unsafe protocol');
            await expect(openBrowserSecurely(fileUrl, {
                allowFile: true,
            ...
            expect(mockExecFile).toHaveBeenCalledWith('xdg-open', [fileUrl], expect.any(Object));
        });
        it('should restrict file URLs to the caller allow-list when provided', async () => {
            ...
            await expect(openBrowserSecurely(pathToFileURL(otherPath).href, {
                allowFile: true,
    ...
    describe('Platform-specific behavior', () => {
        ...
        it('should prefer BROWSER when it is configured', async () => {
            ...
            await openBrowserSecurely('https://example.com');
            expect(mockSpawn).toHaveBeenCalledWith('firefox', ['--new-tab', 'https://example.com'], expect.objectContaining({
                detached: true,
                stdio: 'ignore',
            }));
        ...
        it('should substitute the BROWSER placeholder instead of appending the URL', async () => {
            ...
            await openBrowserSecurely('https://example.com');
            expect(mockSpawn).toHaveBeenCalledWith('firefox', ['--new-tab', 'https://example.com'], expect.any(Object));
        });
        it('should substitute BROWSER placeholders literally', async () => {
            ...
            await openBrowserSecurely(url);
            expect(mockSpawn).toHaveBeenCalledWith('firefox', ['--new-tab', url], expect.any(Object));
        });
        it('should parse quoted arguments inside BROWSER tokens', async () => {
            setPlatform('linux');
            vi.stubEnv('DISPLAY', ':1');
            vi.stubEnv('BROWSER', 'chromium --user-data-dir="/tmp/my profile" --new-tab %s');
            await openBrowserSecurely('https://example.com');
            expect(mockSpawn).toHaveBeenCalledWith('chromium', ['--user-data-dir=/tmp/my profile', '--new-tab', 'https://example.com'], expect.any(Object));
        });
        it('should parse quoted command paths in BROWSER', async () => {
            ...
            await openBrowserSecurely('https://example.com');
            expect(mockSpawn).toHaveBeenCalledWith('/opt/my browser/chromium', ['--new-tab', 'https://example.com'], expect.any(Object));
            expect(mockExecFile).not.toHaveBeenCalled();
        });
        it('should let an explicit BROWSER override headless Linux detection', async () => {
            ...
            await openBrowserSecurely('https://example.com');
            expect(mockSpawn).toHaveBeenCalledWith('firefox', ['https://example.com'], expect.any(Object));
            expect(mockExecFile).not.toHaveBeenCalled();
        });
        ...
        it('should still skip blocklisted BROWSER values in headless Linux', async () => {
            ...
            expect(mockSpawn).not.toHaveBeenCalled();
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Please open this URL manually'));
        ...
        it('should fall back to the platform opener for invalid BROWSER values', async () => {
            ...
            expect(mockSpawn).not.toHaveBeenCalled();
            expect(mockExecFile).toHaveBeenCalledWith('open', ['https://example.com'], expect.any(Object));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid BROWSER environment variable'));
            consoleSpy.mockRestore();
        });
        it('should fall back to the platform opener when explicit BROWSER launch fails', async () => {
            ...
            expect(mockSpawn).toHaveBeenCalledWith('firefox', ['--new-tab', 'https://example.com'], expect.any(Object));
            expect(mockExecFile).toHaveBeenCalledWith('xdg-open', ['https://example.com'], expect.any(Object));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to open BROWSER command firefox'));
            consoleSpy.mockRestore();
        });
