...
import { buildShellExecWarnings, checkArgumentSafety, checkCommandPermissions, COMMAND_SUBSTITUTION_WARNING, detectSelfKillCommand, escapeShellArg, getCommandRoot, getCommandRoots, getShellConfiguration, hasNonFinalTopLevelBackgroundOperator, hasUnsafeMonitorBackgroundOperator, isCommandAllowed, isCommandNeedsPermission, normalizeMonitorCommand, stripTrailingBackgroundAmp, stripShellWrapper, } from './shell-utils.js';
...
describe('getCommandRoot — parameter expansion in command position', () => {
    ...
    it('skips leading env assignments before the expansion, like the plain path', () => {
        process.env[NAME] = '/repo/scripts/dev.js';
        expect(getCommandRoot(`FOO=1 "\${${NAME}:-qwen}" review foo`)).toBe('dev.js');
    });
    it('feeds getCommandRoots, so the shell tool no longer hard-refuses the skill form', () => {
        expect(getCommandRoots(`"\${${NAME}:-qwen}" review fetch-pr 7 --out x.json && echo done`)).toEqual(['qwen', 'echo']);
    });
});
describe('getCommandRoots', () => {
    ...
    it('should skip leading env var assignments', async () => {
        expect(getCommandRoots('PYTHONPATH=/Users/jinjing/.hopcode/skills/scripts python3 -c "print(1)"')).toEqual(['python3']);
    });
...
describe('stripShellWrapper', () => {
    ...
    it('should preserve the full unquoted command after cmd.exe /c', async () => {
        expect(stripShellWrapper('cmd.exe /c taskkill /F /IM node.exe')).toEqual('taskkill /F /IM node.exe');
    });
    it('should preserve the full unquoted command after PowerShell -Command', async () => {
        expect(stripShellWrapper('powershell -Command taskkill /F /IM node.exe')).toEqual('taskkill /F /IM node.exe');
    });
...
describe('detectSelfKillCommand', () => {
    it('detects broad Windows taskkill patterns that target hopcode hosts', () => {
        expect(detectSelfKillCommand('taskkill /F /IM node.exe 2>nul')).toBe(true);
        expect(detectSelfKillCommand('taskkill /FI "IMAGENAME eq hopcode.exe" /F')).toBe(true);
    });
    it('detects broad Unix killall and pkill patterns', () => {
        ...
        expect(detectSelfKillCommand('pkill -f hopcode')).toBe(true);
    ...
    it('detects self-kill commands in chains and execution prefixes', () => {
        ...
        expect(detectSelfKillCommand('false || taskkill /F /IM node.exe')).toBe(true);
        expect(detectSelfKillCommand('sudo killall node')).toBe(true);
        expect(detectSelfKillCommand('env FOO=bar pkill -f hopcode')).toBe(true);
    ...
    it('detects kill commands using pgrep selectors for qwen-code hosts', () => {
        ...
        expect(detectSelfKillCommand('pgrep node | xargs -I {} kill -9 {}')).toBe(true);
    });
    ...
    it('detects taskkill through Windows shell wrappers', () => {
        expect(detectSelfKillCommand('powershell -Command "taskkill /F /IM node.exe"')).toBe(true);
        expect(detectSelfKillCommand('pwsh -NoProfile -Command "taskkill /F /IM node.exe"')).toBe(true);
        expect(detectSelfKillCommand('powershell -Command taskkill /F /IM node.exe')).toBe(true);
        expect(detectSelfKillCommand('powershell -ExecutionPolicy Bypass -Command "taskkill /F /IM node.exe"')).toBe(true);
        expect(detectSelfKillCommand('cmd.exe /c taskkill /F /IM node.exe')).toBe(true);
    });
    it('allows targeted process kills and unrelated process patterns', () => {
        ...
        expect(detectSelfKillCommand('kill -9 $(pgrep -f "node server.js")')).toBe(false);
...
describe('buildShellExecWarnings', () => {
    it('returns undefined when neither stripped nor raw command has substitution', () => {
        expect(buildShellExecWarnings('npm install', 'npm install')).toBeUndefined();
    });
    it('returns the substitution warning when the stripped command has $()', () => {
        const result = buildShellExecWarnings('echo $(cat secret)', 'echo $(cat secret)');
    ...
    it('returns the warning for process substitution <(...)', () => {
        expect(buildShellExecWarnings('diff <(ls /a) <(ls /b)', 'diff <(ls /a) <(ls /b)')).toEqual([COMMAND_SUBSTITUTION_WARNING]);
    });
});
