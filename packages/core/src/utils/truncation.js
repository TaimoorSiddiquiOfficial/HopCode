 ...
 */
export async function truncateAndSaveToFile(content, fileName, projectTempDir, threshold, truncateLines, keep = 'both') {
...
export async function truncateToolOutput(config, toolName, content, limits, promptId) {
    ...
    const result = await truncateAndSaveToFile(content, fileName, config.storage.getProjectTempDir(), threshold, lines, keep);
