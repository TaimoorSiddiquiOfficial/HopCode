...
describe('shellPagerEnv', () => {
    ...
    it('clears inherited git pager values when requested without an effective pager', () => {
        expect(getShellPagerEnv(undefined, {
    ...
    it('treats an empty pager as an explicit request to disable pager env values', () => {
        expect(getShellPagerEnv('', { includeGitPager: true, platform: 'linux' })).toEqual({
            PAGER: '',
            GIT_PAGER: '',
        });
    });
    it('preserves explicit pager configuration on Windows', () => {
        expect(getShellPagerEnv('more', { includeGitPager: true, platform: 'win32' })).toEqual({
            PAGER: 'more',
            GIT_PAGER: 'more',
        });
    });
});
//# sourceMappingURL=shell-pager-env.test.js.map