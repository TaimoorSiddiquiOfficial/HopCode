...
describe('runForkedAgent (AgentHeadless path) bound-tool isolation', () => {
    ...
    function captureAgentHeadlessConfig() {
        const captured = { config: undefined, promptConfig: undefined };
        const spy = vi
            .spyOn(AgentHeadless, 'create')
            .mockImplementation(async (_name, config, promptConfig, ..._rest) => {
    ...
    it('strips internal tags from completed finalText', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parent.toolRegistry = parentRegistry;
        const createSpy = vi.spyOn(AgentHeadless, 'create').mockImplementation(async () => ({
            execute: vi.fn().mockResolvedValue(undefined),
            getTerminateMode: vi.fn().mockReturnValue(AgentTerminateMode.GOAL),
            getFinalText: vi
                .fn()
                .mockReturnValue('<analysis>scratch</analysis><summary>done</summary>'),
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        }));
        try {
            const result = await runForkedAgent({
    ...
    it.each([AgentTerminateMode.MAX_TURNS, AgentTerminateMode.LOOP_DETECTED])('reports %s as failed', async (terminateMode) => {
        const parent = new ConfigImpl(baseParams);
        const parentRegistry = await parent.createToolRegistry(undefined, {
            skipDiscovery: true,
        });
    ...
    it('reports filesWritten from successful mutating tool results only', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parent.toolRegistry = parentRegistry;
        const spy = vi.spyOn(AgentHeadless, 'create').mockImplementation(async (_name, _config, _promptConfig, _modelConfig, _runConfig, _toolConfig, eventEmitter) => ({
