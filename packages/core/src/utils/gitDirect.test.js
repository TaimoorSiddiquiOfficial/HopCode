...
import { isValidRefName, isValidGitSha, readGitHead, resolveBranchName, watchRepoBranch, clearGitDirCache, } from './gitDirect.js';
const watchMock = fs.watch;
const tmpRoots = [];
async function makeRepo(headContent, opts = {}) {
...
describe('readGitHead', () => {
    ...
    it.skipIf(process.platform === 'win32')('refuses a symlinked HEAD (would follow out of the repo)', async () => {
        const repo = await makeRepo('ref: refs/heads/main\n');
...
describe('resolveBranchName', () => {
    ...
    it('reads through a worktree gitdir pointer file', async () => {
        ...
        await fsp.writeFile(path.join(realGitDir, 'HEAD'), 'ref: refs/heads/feature\n');
    ...
    it.skipIf(process.platform === 'win32')('refuses a symlinked commondir', async () => {
        const main = await makeRepo('ref: refs/heads/main\n');
        const realGitDir = path.join(main, '.git', 'worktrees', 'wt1');
        await fsp.mkdir(realGitDir, { recursive: true });
        await fsp.writeFile(path.join(realGitDir, 'HEAD'), 'ref: refs/heads/feature\n');
    ...
    it('rejects a fake .git directory with only a HEAD (no object store)', async () => {
        const dir = await makeBareDir();
        await fsp.mkdir(path.join(dir, '.git', 'logs'), { recursive: true });
        await fsp.writeFile(path.join(dir, '.git', 'HEAD'), 'ref: refs/heads/FAKE-DOTGIT\n');
...
describe('watchRepoBranch', () => {
    ...
    function installWatchMock() {
        ...
        let errorHandler;
        const close = vi.fn();
        watchMock.mockImplementation((_p, l) => {
            listener = l;
            return {
                close,
                on: (event, handler) => {
        ...
        return {
            close,
            fire: (eventType) => listener?.(eventType),
    ...
    it('shares one watcher across subscribers and tears down on last unsubscribe', async () => {
        ...
        expect(watchMock).toHaveBeenCalledWith(path.join(repo, '.git', 'logs', 'HEAD'), expect.any(Function));
        w.fire('change');
    ...
    it.skipIf(process.platform === 'win32')('refuses a symlinked reflog (no out-of-repo watch)', async () => {
        installWatchMock();
        const repo = await makeRepo('ref: refs/heads/main\n', {
            withReflog: false,
        });
...
//# sourceMappingURL=gitDirect.test.js.map