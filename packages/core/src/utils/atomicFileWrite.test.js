...
import { atomicWriteFile, atomicWriteFileSync, atomicWriteJSON, renameWithRetry, renameWithRetrySync, } from './atomicFileWrite.js';
...
describe('atomicWriteFile', () => {
    ...
    it.skipIf(process.platform === 'win32')('should use atomic rename when ownership matches (inode changes)', async () => {
    ...
    it.skipIf(process.platform === 'win32' || typeof process.geteuid !== 'function')('should fall back to in-place write when atomic rename would change ownership', async () => {
    ...
    it.skipIf(process.platform === 'win32' || typeof process.geteuid !== 'function')('should skip in-place fallback for non-regular files and use atomic replace', async () => {
    ...
    it.skipIf(process.platform === 'win32' || typeof process.geteuid !== 'function')('should write via in-place fallback through a resolved symlink when ownership differs', async () => {
    ...
    it.skipIf(process.platform === 'win32' ||
        ...
        process.geteuid() === 0)('should surface EACCES when in-place fallback hits an unwritable file', async () => {
        ...
        process.geteuid = () => realStat.uid + 1;
        try {
            await expect(atomicWriteFile(filePath, 'updated')).rejects.toThrow(/EACCES/);
        }
        finally {
            process.geteuid = realGeteuid;
...
describe('atomicWriteFileSync', () => {
    let tmpDir;
    beforeEach(async () => {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'atomic-write-sync-test-'));
    });
    afterEach(async () => {
        await fs.rm(tmpDir, { recursive: true, force: true });
    });
    ...
    it.skipIf(process.platform === 'win32')('should preserve existing file permissions', () => {
        const filePath = path.join(tmpDir, 'test.txt');
    ...
    it.skipIf(process.platform === 'win32')('should apply explicit mode option for new files', () => {
        const filePath = path.join(tmpDir, 'secret.txt');
    ...
    it('should write through a broken symlink without replacing it', () => {
        ...
        expect(fsSync.readlinkSync(linkFile)).toBe(realFile);
        expect(fsSync.readFileSync(realFile, 'utf-8')).toBe('created via broken symlink');
    });
...
describe('forceMode option', () => {
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFile: forceMode tightens an over-permissive existing file', async () => {
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFile: without forceMode, existing 0o644 is preserved', async () => {
        const filePath = path.join(tmpDir, 'creds.json');
        await fs.writeFile(filePath, 'old');
        await fs.chmod(filePath, 0o644);
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFileSync: forceMode tightens an over-permissive existing file', () => {
        const filePath = path.join(tmpDir, 'creds.json');
    ...
    it.skipIf(process.platform === 'win32')('forceMode without mode preserves existing permissions (does not drop to umask)', async () => {
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFileSync: forceMode without mode preserves existing permissions', () => {
        const filePath = path.join(tmpDir, 'file.txt');
        fsSync.writeFileSync(filePath, 'old');
        fsSync.chmodSync(filePath, 0o600);
...
describe('renameWithRetry (async, dependency-injected rename)', () => {
    ...
    it('gives up after retries exhausted', async () => {
        ...
        await expect(renameWithRetry('s', 'd', 2, 1, mockRename)).rejects.toThrow(/EPERM/);
    ...
    it('does not retry on non-retryable errors (ENOSPC)', async () => {
        ...
        await expect(renameWithRetry('s', 'd', 3, 1, mockRename)).rejects.toThrow(/ENOSPC/);
...
describe('renameWithRetrySync (dependency-injected rename)', () => {
    ...
    it('gives up after retries exhausted', () => {
        ...
        expect(() => renameWithRetrySync('s', 'd', 2, 1, mockRename)).toThrow(/EPERM/);
        expect(attempts).toBe(3);
    });
    it('does not retry on non-retryable errors (EINVAL)', () => {
        ...
        expect(() => renameWithRetrySync('s', 'd', 3, 1, mockRename)).toThrow(/EINVAL/);
        expect(attempts).toBe(1);
    });
});
describe('EXDEV fallback (async + sync)', () => {
    ...
    it('atomicWriteFileSync: falls back to direct write on EXDEV, cleans up tmp', () => {
        ...
        expect(fsSync.readFileSync(filePath, 'utf-8')).toBe('sync-fallback-payload');
    ...
    it('atomicWriteFile: non-EXDEV rename failure propagates (no fallback)', async () => {
        ...
        await expect(atomicWriteFile(filePath, 'data', undefined, { rename: eioRename })).rejects.toThrow(/atomicWriteFile\(.*eio\.txt.*\):.*EIO/);
    ...
    it('atomicWriteFileSync: non-EXDEV rename failure propagates and cleans up tmp', () => {
        ...
        expect(() => atomicWriteFileSync(filePath, 'data', undefined, { rename: eioRename })).toThrow(/atomicWriteFileSync\(.*eio-sync\.txt.*\):.*EIO/);
        expect(fsSync.readdirSync(tmpDir)).toEqual([]);
    });
    it('atomicWriteFile: annotates errors whose message contains the target path (startsWith guard, not includes)', async () => {
        ...
        const renameWithPathInMsg = async () => {
            const e = new Error(`EIO: i/o error, rename '${filePath}.abc123.tmp'`);
            e.code = 'EIO';
            throw e;
        };
        await expect(atomicWriteFile(filePath, 'data', undefined, {
    ...
    it('atomicWriteFile: EXDEV fallback write failure is annotated with target + fn name', async () => {
        ...
        let writeCalls = 0;
        const failingWrite = async (p, d, opts) => {
            writeCalls++;
            if (writeCalls === 1) {
                // Actually write the tmp file so subsequent cleanup works.
                await fs.writeFile(p, d, opts);
                return;
            }
            const e = new Error(`ENOSPC: no space left on device, open '${filePath}'`);
            e.code = 'ENOSPC';
            throw e;
        };
        let caught;
        ...
        expect(caught.message).toMatch(/atomicWriteFile\(.*exdev-write-fail\.txt.*\):.*ENOSPC/);
    });
    it('atomicWriteFileSync: EXDEV fallback write failure is annotated with sync fn name', () => {
        ...
        let writeCalls = 0;
        const failingWrite = (p, d, opts) => {
            writeCalls++;
            if (writeCalls === 1) {
                fsSync.writeFileSync(p, d, opts);
                return;
            }
            const e = new Error(`ENOSPC: no space left on device, open '${filePath}'`);
            e.code = 'ENOSPC';
        ...
        expect(caught?.code).toBe('ENOSPC');
        expect(caught.message).toMatch(/atomicWriteFileSync\(.*exdev-sync-write-fail\.txt.*\):.*ENOSPC/);
    });
});
...
describe('noFollow option — symlink protection', () => {
    ...
    it('atomicWriteFile: noFollow EXDEV fallback also refuses to follow symlinks', async () => {
        ...
        await atomicWriteFile(link, 'NEW', { noFollow: true }, { rename: exdevRename });
        expect(fsSync.lstatSync(link).isSymbolicLink()).toBe(false);
        expect(await fs.readFile(link, 'utf-8')).toBe('NEW');
    ...
    it('atomicWriteFileSync: noFollow EXDEV fallback also refuses to follow symlinks', () => {
        ...
        atomicWriteFileSync(link, 'NEW', { noFollow: true }, { rename: exdevRename });
        expect(fsSync.lstatSync(link).isSymbolicLink()).toBe(false);
        expect(fsSync.readFileSync(link, 'utf-8')).toBe('NEW');
        expect(fsSync.readFileSync(real, 'utf-8')).toBe('ORIGINAL');
    });
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFile: noFollow EXDEV fallback creates a new file when target does not exist', async () => {
        ...
        await atomicWriteFile(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, chmod: chmodSpy });
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFileSync: noFollow EXDEV fallback creates a new file when target does not exist', () => {
        ...
        atomicWriteFileSync(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, chmod: chmodSpy });
        expect(fsSync.lstatSync(target).isSymbolicLink()).toBe(false);
    ...
    it.each(['ENOSYS', 'ENOTSUP'])('atomicWriteFile: noFollow EXDEV swallows fchmod %s (FAT/exFAT)', async (chmodCode) => {
        ...
        const fchmod = async () => {
            const e = new Error(chmodCode);
            e.code = chmodCode;
            throw e;
        };
        await atomicWriteFile(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, fchmod });
        expect(await fs.readFile(target, 'utf-8')).toBe('NEW');
    });
    it('atomicWriteFile: noFollow EXDEV propagates fchmod EPERM and removes the orphan', async () => {
        ...
        await expect(atomicWriteFile(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, fchmod })).rejects.toThrow(/EPERM/);
    ...
    it.each(['ENOSYS', 'ENOTSUP'])('atomicWriteFileSync: noFollow EXDEV swallows fchmod %s (FAT/exFAT)', (chmodCode) => {
        ...
        atomicWriteFileSync(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, fchmod });
        expect(fsSync.readFileSync(target, 'utf-8')).toBe('NEW');
    });
    it('atomicWriteFileSync: noFollow EXDEV propagates fchmod EPERM and removes the orphan', () => {
        ...
        expect(() => atomicWriteFileSync(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, fchmod })).toThrow(/EPERM/);
        expect(fsSync.existsSync(target)).toBe(false);
    });
    ...
    it('atomicWriteFile: noFollow EXDEV pre-open unlink propagates non-ENOENT errors', async () => {
        ...
        await expect(atomicWriteFile(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, unlink: unlink })).rejects.toThrow(/EACCES/);
    });
    it('atomicWriteFileSync: noFollow EXDEV pre-open unlink propagates non-ENOENT errors', () => {
        ...
        expect(() => atomicWriteFileSync(target, 'NEW', { noFollow: true, mode: 0o600 }, { rename: exdevRename, unlink: unlink })).toThrow(/EACCES/);
    });
    ...
    it.skipIf(process.platform === 'win32')('atomicWriteFile: annotates resolveSymlinkChain ELOOP failures with the logical filePath', async () => {
        ...
        await expect(atomicWriteFile(linkA, 'X')).rejects.toThrow(new RegExp(`atomicWriteFile\\(.*${path.basename(linkA)}.*\\):`));
    });
    it.skipIf(process.platform === 'win32')('atomicWriteFileSync: annotates resolveSymlinkChainSync ELOOP failures with the logical filePath', () => {
        const linkA = path.join(tmpDir, 'loop-a-sync');
        const linkB = path.join(tmpDir, 'loop-b-sync');
        fsSync.symlinkSync(linkB, linkA);
        fsSync.symlinkSync(linkA, linkB);
        expect(() => atomicWriteFileSync(linkA, 'X')).toThrow(new RegExp(`atomicWriteFileSync\\(.*${path.basename(linkA)}.*\\):`));
    });
    ...
    // narrowed catch.
    it.each(['ENOSYS', 'ENOTSUP'])('atomicWriteFile: tryChmod swallows %s (FAT/exFAT — non-noFollow EXDEV path)', async (chmodCode) => {
        ...
        await atomicWriteFile(target, 'NEW', { mode: 0o600 }, { rename: exdevRename, chmod });
        expect(await fs.readFile(target, 'utf-8')).toBe('NEW');
    });
    it('atomicWriteFile: tryChmod propagates EPERM (non-noFollow EXDEV path)', async () => {
        ...
        await expect(atomicWriteFile(target, 'NEW', { mode: 0o600 }, { rename: exdevRename, chmod })).rejects.toThrow(/EPERM/);
    });
    it.each(['ENOSYS', 'ENOTSUP'])('atomicWriteFileSync: tryChmodSync swallows %s (FAT/exFAT — non-noFollow EXDEV path)', (chmodCode) => {
        ...
        atomicWriteFileSync(target, 'NEW', { mode: 0o600 }, { rename: exdevRename, chmod });
        expect(fsSync.readFileSync(target, 'utf-8')).toBe('NEW');
    });
    it('atomicWriteFileSync: tryChmodSync propagates EPERM (non-noFollow EXDEV path)', () => {
        ...
        expect(() => atomicWriteFileSync(target, 'NEW', { mode: 0o600 }, { rename: exdevRename, chmod })).toThrow(/EPERM/);
    });
    ...
    it('atomicWriteFile: orphan cleanup on fchmod failure goes through unlinkImpl', async () => {
        ...
        await expect(atomicWriteFile(target, 'NEW', { noFollow: true, mode: 0o600 }, {
            rename: exdevRename,
            fchmod,
            unlink: unlinkSpy,
        })).rejects.toThrow(/EPERM/);
        const orphanCleanupCalls = unlinkSpy.mock.calls.filter(([p]) => p === target);
    ...
    it('atomicWriteFileSync: orphan cleanup on fchmod failure goes through unlinkImpl', () => {
        ...
        expect(() => atomicWriteFileSync(target, 'NEW', { noFollow: true, mode: 0o600 }, {
            rename: exdevRename,
            fchmod,
            unlink: unlinkSpy,
        })).toThrow(/EPERM/);
    ...
    it('atomicWriteFile: noFollow EXDEV opens the target with O_EXCL (no-clobber create)', async () => {
        ...
        await atomicWriteFile(target, 'NEW', { noFollow: true }, { rename: exdevRename, open: openSpy });
    ...
    it('atomicWriteFileSync: noFollow EXDEV opens the target with O_EXCL (no-clobber create)', () => {
        ...
        atomicWriteFileSync(target, 'NEW', { noFollow: true }, { rename: exdevRename, open: openSpy });
        expect(openSpy).toHaveBeenCalledTimes(1);
        const flags = openSpy.mock.calls[0][1];
        expect(flags & fsSync.constants.O_EXCL).toBe(fsSync.constants.O_EXCL);
        expect(flags & fsSync.constants.O_CREAT).toBe(fsSync.constants.O_CREAT);
