...
import { sessionIdContext, registerSessionProjectDir, unregisterSessionProjectDir, } from './sessionIdContext.js';
...
describe('getShellContextEnvVars', () => {
    ...
    afterEach(() => {
        if (originalSessionId !== undefined) {
            process.env['HOPCODE_CODE_SESSION_ID'] = originalSessionId;
        }
        else {
    ...
    it('returns empty strings for agent/prompt when no context is available', () => {
        ...
        expect(env).toEqual({
            HOPCODE_CODE_AGENT_ID: '',
            HOPCODE_CODE_PROMPT_ID: '',
        });
    });
    it('returns HOPCODE_CODE_SESSION_ID when set in process.env', () => {
        process.env['HOPCODE_CODE_SESSION_ID'] = 'test-session-123';
        const env = getShellContextEnvVars();
        expect(env['HOPCODE_CODE_SESSION_ID']).toBe('test-session-123');
    });
    it('returns HOPCODE_CODE_AGENT_ID when called within agent context', async () => {
        const env = await runWithAgentContext('my-agent-42', async () => getShellContextEnvVars());
        expect(env['HOPCODE_CODE_AGENT_ID']).toBe('my-agent-42');
    });
    it('returns HOPCODE_CODE_PROMPT_ID when called within prompt context', () => {
        const env = promptIdContext.run('prompt-abc', () => getShellContextEnvVars());
        expect(env['HOPCODE_CODE_PROMPT_ID']).toBe('prompt-abc');
    });
    it('returns all vars when all contexts are active', async () => {
        process.env['HOPCODE_CODE_SESSION_ID'] = 'sess-uuid';
        const env = await runWithAgentContext('agent-xyz', async () => promptIdContext.run('prompt-456', () => getShellContextEnvVars()));
        expect(env).toEqual({
            HOPCODE_CODE_SESSION_ID: 'sess-uuid',
            HOPCODE_CODE_AGENT_ID: 'agent-xyz',
            HOPCODE_CODE_PROMPT_ID: 'prompt-456',
        });
    });
    describe('project dir is per-session, not per-process', () => {
        ...
        it('drops a session entry on unregister — no daemon leak', () => {
            ...
            expect(sessionIdContext.run('sess-X', () => getShellContextEnvVars())['QWEN_CODE_PROJECT_DIR']).toBe('/proj/X');
            unregisterSessionProjectDir('sess-X');
            delete process.env['QWEN_CODE_PROJECT_DIR'];
            expect(sessionIdContext.run('sess-X', () => getShellContextEnvVars())['QWEN_CODE_PROJECT_DIR']).toBeUndefined();
        });
        it('falls back to the env var for the single-session CLI', () => {
            process.env['QWEN_CODE_PROJECT_DIR'] = '/proj/only';
            expect(getShellContextEnvVars()['QWEN_CODE_PROJECT_DIR']).toBe('/proj/only');
        });
    });
    describe('session ID from AsyncLocalStorage (daemon multi-session)', () => {
        it('prefers sessionIdContext over process.env', () => {
            ...
            process.env['HOPCODE_CODE_SESSION_ID'] = 'stale-first-session';
            const env = sessionIdContext.run('current-session', () => getShellContextEnvVars());
            expect(env['HOPCODE_CODE_SESSION_ID']).toBe('current-session');
        });
        it('falls back to process.env outside any session context (single-session CLI)', () => {
            process.env['HOPCODE_CODE_SESSION_ID'] = 'cli-session';
            const env = getShellContextEnvVars();
            expect(env['HOPCODE_CODE_SESSION_ID']).toBe('cli-session');
        });
        it('isolates concurrent sessions in the same process', async () => {
            ...
            expect(envSeenByA['HOPCODE_CODE_SESSION_ID']).toBe('session-A');
            expect(envSeenByB['HOPCODE_CODE_SESSION_ID']).toBe('session-B');
        });
    });
    it('sets empty string for agent/prompt to override inherited env', () => {
        // Simulates a nested hopcode process where parent injected these
        const env = getShellContextEnvVars();
        expect(env['HOPCODE_CODE_AGENT_ID']).toBe('');
        expect(env['HOPCODE_CODE_PROMPT_ID']).toBe('');
    ...
    describe('TRACEPARENT injection', () => {
        ...
        it('injects TRACEPARENT when propagation is enabled and context exists', () => {
            ...
            vi.mocked(formatTraceparent).mockReturnValue('00-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-bbbbbbbbbbbbbbbb-01');
            const env = getShellContextEnvVars();
...
//# sourceMappingURL=shellContextEnv.test.js.map