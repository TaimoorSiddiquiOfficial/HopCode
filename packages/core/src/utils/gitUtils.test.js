...
describe('getRecentGitStatus', () => {
    ...
    it('uses two git commands with piped stderr and timeout', async () => {
        ...
        expect(execSyncSpy).toHaveBeenNthCalledWith(1, 'git --no-optional-locks status --short --branch', expect.objectContaining({
            cwd: process.cwd(),
            encoding: 'utf8',
        ...
        expect(execSyncSpy).toHaveBeenNthCalledWith(2, 'git --no-optional-locks log --oneline -n 5', expect.objectContaining({
            cwd: process.cwd(),
            encoding: 'utf8',
            stdio: ['pipe', 'pipe', 'pipe'],
            timeout: 5000,
    ...
    it('returns null when status output has no branch header', () => {
        ...
        expect(mockWarn).toHaveBeenCalledWith('Failed to get recent git status for system prompt:', expect.objectContaining({
