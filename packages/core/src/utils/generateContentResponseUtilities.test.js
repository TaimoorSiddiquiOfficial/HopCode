...
import { getResponseTextFromParts, getFunctionCalls, getFunctionCallsFromParts, getFunctionCallsAsJson, getFunctionCallsFromPartsAsJson, getStructuredResponse, getStructuredResponseFromParts, getToolResponseDisplayText, TOOL_SUCCEEDED_OUTPUT, } from './generateContentResponseUtilities.js';
...
describe('generateContentResponseUtilities', () => {
    ...
    describe('getToolResponseDisplayText', () => {
        ...
        it('skips the non-informative "Tool execution succeeded." placeholder', () => {
            expect(getToolResponseDisplayText([frPart(TOOL_SUCCEEDED_OUTPUT)])).toBeUndefined();
        });
        it('emits <media: mime> placeholders for nested inline/file data', () => {
            ...
            expect(getToolResponseDisplayText(parts)).toBe('<media: image/png>\n<media: application/pdf>');
        });
        it('sanitizes control chars and angle brackets in media placeholders', () => {
            ...
            expect(getToolResponseDisplayText(parts)).toBe('<media: image/png[31mb>\n<media: file:///xscript>');
        });
        it('concatenates output and nested media, keeping nested text', () => {
            ...
            expect(getToolResponseDisplayText(parts)).toBe('main output\nnested note\n<media: image/jpeg>');
        });
        it('keeps text from a plain (non-functionResponse) part', () => {
            expect(getToolResponseDisplayText([{ text: 'plain text' }])).toBe('plain text');
        });
