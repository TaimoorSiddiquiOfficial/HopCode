    ...
    return {
        ...
        setup(pluginBuild) {
            pluginBuild.onResolve({ filter: /\.wasm\?binary$/ }, (args) => {
                ...
                const localRequire = createRequire(path.resolve(args.resolveDir || repoRoot, '_dummy_.js'));
                return {
...
function staticClosure(metafile, entry) {
    ...
    while (pending.length > 0) {
        ...
        if (!output)
            throw new Error(`Missing metafile output: ${outputPath}`);
...
function expectDeferredInput(metafile, closure, inputFragment) {
    const owningOutputs = Object.entries(metafile.outputs).filter(([, output]) => Object.keys(output.inputs).some((input) => input.replaceAll('\\', '/').includes(inputFragment)));
    expect(owningOutputs.length).toBeGreaterThan(0);
    expect(owningOutputs.every(([outputPath]) => !closure.has(outputPath))).toBe(true);
}
describe('shellAstParser lazy runtime', () => {
    ...
    it('latches a language load failure', async () => {
        ...
        const parser = await import('./shellAstParser.js');
        expect(await parser.classifyShellCommandSafety('git status')).toBe('unknown');
        ...
        await expect(parser.initParser()).rejects.toThrow('tree-sitter WASM failed to initialise');
    ...
    it('keeps the packaged runtime deferred and parses from emitted chunks', async () => {
        ...
        writeFileSync(entryPath, `export { _resetParser, classifyShellCommandSafety, isShellCommandReadOnlyAST, parseShellCommand } from ${JSON.stringify(path.join(repoRoot, 'packages/core/src/utils/shellAstParser.ts'))};\n`);
        ...
        writeFileSync(path.join(tempDir, 'dist/package.json'), JSON.stringify({ type: 'module' }));
        ...
        /* @vite-ignore */ `${pathToFileURL(path.join(tempDir, 'dist/entry.js')).href}?test=${Date.now()}`));
        ...
        treeA.delete();
        treeB.delete();
        expect(await packagedParser.isShellCommandReadOnlyAST('git status')).toBe(true);
        expect(await packagedParser.isShellCommandReadOnlyAST('rm -rf temp')).toBe(false);
        expect(await packagedParser.classifyShellCommandSafety('rm -rf temp')).toBe('write');
        await packagedParser.classifyShellCommandSafety('case x in x) rm target;; esac');
        expect(await packagedParser.classifyShellCommandSafety('git status')).toBe('read-only');
...
//# sourceMappingURL=shell-ast-parser-lazy.test.js.map