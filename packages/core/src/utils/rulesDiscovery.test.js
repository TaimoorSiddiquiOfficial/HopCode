...
Use hooks.`);
        ...
        it('skips project rules when folder is untrusted', async () => {
            await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'rules', 'r.md'), 'Untrusted.');
            const result = await loadRules(projectRoot, false);
            expect(result.ruleCount).toBe(0);
        });
        it('loads global rules even when folder is untrusted', async () => {
            await createTestFile(path.join(homedir, HOPCODE_DIR, 'rules', 'g.md'), 'Global.');
            const result = await loadRules(projectRoot, false);
            expect(result.ruleCount).toBe(1);
            expect(result.content).toContain('Global.');
        });
        it('does not duplicate rules when projectRoot equals homedir', async () => {
            await createTestFile(path.join(homedir, HOPCODE_DIR, 'rules', 's.md'), 'Shared.');
        ...
        it('formats rules with source markers', async () => {
            await createTestFile(path.join(projectRoot, HOPCODE_DIR, 'rules', 'test.md'), 'Content.');
            const result = await loadRules(projectRoot, true);
            expect(result.content).toContain(`--- Rule from: ${HOPCODE_DIR}/rules/test.md ---`);
        });
        it('reads global rules from HOPCODE_HOME when set', async () => {
            ...
            try {
                await createTestFile(path.join(customHopcodeHome, 'rules', 'fromCustomHome.md'), 'CustomHome rule.');
                // A stale rule in the legacy ~/.hopcode/rules location should NOT be
                // loaded once HOPCODE_HOME points elsewhere.
                await createTestFile(path.join(homedir, HOPCODE_DIR, 'rules', 'fromLegacyHome.md'), 'LegacyHome rule.');
                const result = await loadRules(projectRoot, true);
    ...
    describe('ConditionalRulesRegistry', () => {
        ...
        it('returns undefined when no patterns match', async () => {
            const reg = new ConditionalRulesRegistry([rule('/r/fe.md', ['src/**/*.tsx'], 'Use hooks.')], '/project');
            expect(await reg.matchAndConsume('/project/lib/utils.py')).toBeUndefined();
        });
        ...
        it('activates on dotfiles when glob covers them (dot: true semantics)', async () => {
            ...
            expect(await reg.matchAndConsume('/project/.github/workflows/ci.yml')).toContain('YAML rule.');
        });
        it('rejects Windows cross-drive paths (shared cross-drive guard)', async () => {
            ...
            expect(await reg.matchAndConsume('/totally/other/place/file.ts')).toBeUndefined();
        });
        it.skipIf(process.platform === 'win32')('should match shell-escaped file paths after unescaping', async () => {
            ...
            const result = await reg.matchAndConsume(path.resolve('/project', normalizedPath));
            expect(result).toContain('Use hooks.');
        });
        it.skipIf(process.platform === 'win32')('activates rules when file is reached via symlinked directory', async () => {
            ...
            const reg = new ConditionalRulesRegistry([rule('/r/ts.md', ['src/**/*.ts'], 'Strict.')], projectRoot);
        ...
        it.skipIf(process.platform === 'win32')('activates rules when project root itself is a symlink', async () => {
            ...
            const realFile = path.join(srcDir, 'foo.ts');
            await fsPromises.writeFile(realFile, '// test');
            const reg = new ConditionalRulesRegistry([rule('/r/ts.md', ['src/**/*.ts'], 'Strict.')], symlinkProject);
            const result = await reg.matchAndConsume(path.join(symlinkProject, 'src', 'foo.ts'));
            expect(result).toContain('Strict.');
        });
    });
});
