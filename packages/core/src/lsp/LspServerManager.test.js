...
function createReconcileManager() {
    ...
    const privateView = manager;
    vi.spyOn(privateView, 'startServer').mockImplementation(async (_name, handle) => {
        handle.status = 'READY';
    });
    vi.spyOn(privateView, 'stopServer').mockImplementation(async (_name, handle) => {
        handle.status = 'NOT_STARTED';
    });
    return { manager, privateView };
}
function createTrustedManager() {
    return new LspServerManager({
...
function pathToRootUri(rootPath) {
...
describe('LspServerManager', () => {
    ...
    describe('reconcileServerConfigs', () => {
        it('starts added servers', async () => {
            ...
            expect(debugLoggerMock.info).toHaveBeenCalledWith('Reconciling LSP server configs: desired=clangd');
            expect(debugLoggerMock.info).toHaveBeenCalledWith('LSP reconcile result: added=clangd, removed=<none>, restarted=<none>, unchanged=<none>, failed=<none>');
        });
        it('removes missing servers', async () => {
            ...
            expect(debugLoggerMock.info).toHaveBeenCalledWith('LSP reconcile result: added=<none>, removed=clangd, restarted=<none>, unchanged=<none>, failed=<none>');
        });
        it('restarts changed servers and preserves unchanged handles', async () => {
            ...
            expect(debugLoggerMock.info).toHaveBeenCalledWith('LSP reconcile result: added=<none>, removed=<none>, restarted=clangd, unchanged=pyright, failed=<none>');
        });
        ...
        it('serializes concurrent reconcile calls', async () => {
            const { manager, privateView } = createReconcileManager();
            const order = [];
            vi.mocked(privateView.startServer).mockImplementation(async (name, handle) => {
            ...
            expect(manager.getHandles().get('clangd')?.config.command).toBe('clangd-next');
        });
        it('aborts a server startup before removing it', async () => {
            ...
            let resolveStartup;
            handle.startingPromise = new Promise((resolve) => {
        ...
        it('stopAll waits for an active reconcile before clearing handles', async () => {
            const { manager, privateView } = createReconcileManager();
            let resolveStart;
            vi.mocked(privateView.startServer).mockImplementation(async (_name, handle) => {
        ...
        it('serializes stopAll with later reconcile calls', async () => {
            ...
            const order = [];
            manager.setServerConfigs([serverConfig]);
            vi.mocked(privateView.stopServer).mockImplementation(async (_name, handle) => {
                order.push('stop');
                await new Promise((resolve) => setTimeout(resolve, 10));
                handle.status = 'NOT_STARTED';
            });
    ...
    it('logs process diagnostics when startup fails after connection creation', async () => {
        ...
        expect(manager.getHandles().get('clangd')?.processDiagnostics).toBe(processDiagnostics);
    ...
    it('logs when workspace trust check blocks startup', async () => {
        const manager = createTrustedManager();
        vi.spyOn(manager, 'checkWorkspaceTrust').mockResolvedValue(false);
        manager.setServerConfigs([serverConfig]);
        await manager.startAll();
        expect(manager.getHandles().get('clangd')?.status).toBe('FAILED');
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('Workspace trust check failed, not starting LSP server clangd');
    });
    it('does not probe command existence for unsafe command paths', async () => {
        const manager = createTrustedManager();
        vi.spyOn(manager, 'checkWorkspaceTrust').mockResolvedValue(true);
        const isPathSafe = vi
            .spyOn(manager, 'isPathSafe')
            .mockReturnValue(false);
        const commandExists = vi.spyOn(manager, 'commandExists');
        ...
        expect(manager.getHandles().get('clangd')?.status).toBe('FAILED');
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('LSP server clangd command path is unsafe: ../../outside/payload');
    });
    it('retries the same config after initial command lookup failure', async () => {
        const manager = createTrustedManager();
        vi.spyOn(manager, 'checkWorkspaceTrust').mockResolvedValue(true);
        vi.spyOn(manager, 'isPathSafe').mockReturnValue(true);
        vi.spyOn(manager, 'commandExists')
        ...
        vi.spyOn(manager, 'initializeLspServer').mockResolvedValue(undefined);
        manager.setServerConfigs([serverConfig]);
        await manager.startAll();
        const result = await manager.reconcileServerConfigs([serverConfig]);
        expect(result.restarted).toEqual(['clangd']);
    ...
    it('passes LSP config env through command probe filtering', async () => {
        ...
        await manager.startAll();
        expect(commandExists).toHaveBeenCalledWith('clangd', { PATH: '/tmp/fake-bin', SAFE_VALUE: '1' }, '/workspace');
    });
    it('retries the same config after a crash restart failure', async () => {
        const manager = createTrustedManager();
        let exitHandler;
        const process = createMockProcess();
        process.once = vi.fn((event, handler) => {
            if (event === 'exit') {
                exitHandler = handler;
            }
            return process;
    ...
    it('does not restart a crashed server while stopping all servers', async () => {
        ...
        const createLspConnection = vi
            .spyOn(manager, 'createLspConnection')
            .mockResolvedValue({
            connection: createMockConnection(),
            process: process,
        });
    ...
    it('retries the same config after a crash without restartOnCrash', async () => {
        ...
        expect(exitHandler).toBeDefined();
        exitHandler?.(1);
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('LSP server clangd exited but restartOnCrash is disabled');
        const result = await manager.reconcileServerConfigs([serverConfig]);
        expect(result.restarted).toEqual(['clangd']);
    ...
    it('logs when a crashed server has zero restart attempts configured', async () => {
        ...
        expect(exitHandler).toBeDefined();
        exitHandler?.(1);
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('LSP server clangd exited but maxRestarts is 0');
        expect(manager.getHandles().get('clangd')?.status).toBe('FAILED');
    });
    ...
    it('does not use LSP config PATH when probing command existence', () => {
        const manager = createTrustedManager();
        const env = manager.buildCommandProbeEnv({
            PATH: '/tmp/fake-bin',
    ...
    it('ignores reset errors while queueing a crash restart', async () => {
        ...
        vi.spyOn(manager, 'isPathSafe').mockReturnValue(true);
        vi.spyOn(manager, 'commandExists').mockResolvedValue(true);
        vi.spyOn(manager, 'createLspConnection')
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('Error closing LSP connection during reset:', expect.any(Error));
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('Error killing LSP process during reset:', expect.any(Error));
        expect(manager.getHandles().get('clangd')?.status).toBe('READY');
    });
    ...
    it('cancels an in-flight socket startup retry when stopped', async () => {
        ...
        vi.spyOn(manager, 'isPathSafe').mockReturnValue(true);
        vi.spyOn(manager, 'commandExists').mockResolvedValue(true);
        vi.spyOn(LspConnectionFactory, 'createSocketConnection').mockReturnValue(new Promise(() => { }));
        manager.setServerConfigs([
            {
                ...serverConfig,
    ...
    it('cancels socket command spawn wait when startup is aborted', async () => {
        ...
        const childProcess = {
            exitCode: null,
            kill: vi.fn(),
            once: vi.fn((_event, _handler) => childProcess),
            off: vi.fn((_event, _handler) => childProcess),
        };
        const privateView = manager;
        const wait = privateView.waitForSocketProcessSpawn(childProcess, controller.signal);
        controller.abort();
        await expect(wait).rejects.toThrow('LSP server startup cancelled');
        expect(childProcess.kill).toHaveBeenCalledOnce();
        expect(childProcess.off).toHaveBeenCalledWith('spawn', expect.any(Function));
        expect(childProcess.off).toHaveBeenCalledWith('error', expect.any(Function));
    });
    it('cleans up a socket connection that resolves after startup abort wins', async () => {
        ...
        const wait = privateView.raceStartupAbort(connectionPromise, controller.signal, (value) => value.connection.end());
        controller.abort();
        await expect(wait).rejects.toThrow('LSP server startup cancelled');
    ...
    it('observes a startup promise that rejects after abort wins', async () => {
        ...
        const wait = privateView.raceStartupAbort(startupPromise, controller.signal);
        controller.abort();
        await expect(wait).rejects.toThrow('LSP server startup cancelled');
    ...
    it('cancels a startup that is waiting for protocol initialization', async () => {
        ...
        vi.spyOn(manager, 'initializeLspServer').mockReturnValue(new Promise(() => { }));
        manager.setServerConfigs([serverConfig]);
        const startAll = manager.startAll();
        await vi.waitFor(() => {
    ...
    it('fails socket startup early when the child exits before connect', async () => {
        ...
        expect(handle?.error?.message).toContain('LSP server exited before socket connection was ready');
    ...
    it('does not crash-restart a server that exits during protocol initialization', async () => {
        ...
        vi.spyOn(manager, 'initializeLspServer').mockReturnValue(new Promise((resolve) => {
    ...
    it('logs and continues when killing an owned process throws', async () => {
        ...
        expect(process.kill).toHaveBeenCalledOnce();
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('Error killing LSP server clangd process:', killError);
        expect(manager.getHandles().size).toBe(0);
    });
    ...
    it('unrefs and clears command probe timeout when the command errors first', async () => {
        ...
        const commandExists = manager.commandExists('__qwen_lsp_missing_command__');
