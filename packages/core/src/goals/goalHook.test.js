...
import { abortGoalForStopHookCap, createGoalStopHookCallback, GOAL_HOOK_TIMEOUT_MS, GOAL_JUDGE_TIMEOUT_MS, MAX_GOAL_ITERATIONS, registerGoalHook, unregisterGoalHook, } from './goalHook.js';
const judgeMock = vi.hoisted(() => vi.fn());
vi.mock('./goalJudge.js', () => ({
    judgeGoal: judgeMock,
}));
function makeGoalConfig(opts = {}) {
    return {
...
describe('createGoalStopHookCallback', () => {
    ...
    ])('defers evaluation while a %s is running', async (_name, opts) => {
...
describe('registerGoalHook / unregisterGoalHook', () => {
    ...
    ])('ignores an unusable initialSetAt (%s) and starts the clock now', (_label, make) => {
