...
export function formatEvidence(bundle) {
    const sections = [];
    ...
    if (bundle.resolutionSummary) {
        sections.push(`## Resolution Summary (model's response to previous findings)\n${escapeUntrustedDelimiter(bundle.resolutionSummary)}`);
    }
 ...
 */
export async function runGateAgent(config, bundle, 
...
_parentSignal) {
    ...
    const { config: planConfig, cleanup } = await createApprovalModeOverride(config, ApprovalMode.PLAN);
    ...
    const timeoutId = setTimeout(() => {
        debugLogger.warn(`[runGateAgent] Gate agent timed out after ${GATE_AGENT_TIMEOUT_MS}ms`);
        gateAbortController.abort(new Error('Gate agent timeout'));
    }, GATE_AGENT_TIMEOUT_MS);
    let disposeSubagent;
    try {
        const subagentManager = config.getSubagentManager();
        const { subagent, dispose } = await subagentManager.createAgentHeadless(subagentConfig, planConfig);
        ...
        if (terminateMode !== AgentTerminateMode.GOAL ||
            ...
            throw new Error(`Gate agent terminated with mode=${terminateMode} and no usable output`);
        }
        ...
        if (disposeSubagent) {
            try {
                ...
                debugLogger.warn(`[runGateAgent] Failed to dispose subagent: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        cleanup();
...
export function parseGateAgentResult(raw) {
    ...
    if (!validDecisions.has(obj['decision'])) {
        throw new Error(`Gate agent returned invalid decision: ${String(obj['decision'])}`);
    }
...
//# sourceMappingURL=gateReviewAgents.js.map