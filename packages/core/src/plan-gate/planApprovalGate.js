 ...
 */
import { CAPPED_REVIEW_LIMIT, MAX_AGENT_RETRIES, CAP_ESCALATION_LABELS, } from './types.js';
 ...
 */
export async function runPlanApprovalGate(config, bundle, signal) {
    ...
    if (result.decision === 'pass') {
        ...
        debugLogger.warn(`Gate agent returned 'pass' with ${findings.length} finding(s); treating as blocked`);
    }
    ...
    const hasBlocking = findings.some((f) => f.severity === 'P1' || f.severity === 'P2');
    if (atCap) {
        if (!hasBlocking) {
        ...
        return {
            kind: 'cap_escalation',
            blockingFindings: findings.filter((f) => f.severity === 'P1' || f.severity === 'P2'),
        };
    }
...
async function runAgentWithRetry(config, bundle, signal) {
    ...
    if (signal.aborted) {
        debugLogger.warn('Gate agent skipped: parent signal already aborted at entry');
        return null;
    }
    for (let attempt = 1; attempt <= MAX_AGENT_RETRIES; attempt++) {
        try {
            ...
            debugLogger.warn(`Gate agent attempt ${attempt}/${MAX_AGENT_RETRIES} failed: ${msg}`);
            if (attempt === MAX_AGENT_RETRIES) {
                debugLogger.error(`Gate agent exhausted all ${MAX_AGENT_RETRIES} retries`);
                return null;
            }
            ...
            try {
                ...
                debugLogger.warn(`Gate agent retry loop cancelled during backoff delay (attempt ${attempt}/${MAX_AGENT_RETRIES})`);
                return null;
            }
        }
    }
 ...
 */
export function assignFindingIds(result) {
...
export function formatBlockedResponse(decision) {
    const lines = [
...
export function formatNeedsUserResponse(decision) {
    const lines = [
...
export function formatCapEscalationResponse(decision) {
    const lines = [
    ...
    lines.push('\nProvide these options (the UI automatically provides a free-text "Other" input):', `1. "${CAP_ESCALATION_LABELS.CONTINUE}" — keep iterating with the gate (uncapped)`, `2. "${CAP_ESCALATION_LABELS.APPROVE}" — user override, skip the gate and execute`);
    return lines.join('\n');
}
export function formatApprovedNotes(findings) {
    if (findings.length === 0)
        return '';
...
//# sourceMappingURL=planApprovalGate.js.map