...
describe('matchesPathPattern', () => {
    ...
    it('matches a path after resolving parent-directory traversal', () => {
        withTempRoot((root) => {
            ...
            expect(matchesPathPattern(`/${path.basename(protectedDir)}/**`, `${nestedDir}${path.sep}..${path.sep}..${path.sep}protected${path.sep}new.txt`, root, root, 'canonical')).toBe(true);
        });
    });
    it('matches the canonical target of a symlinked path', () => {
        withTempRoot((root) => {
            ...
            expect(matchesPathPattern('/protected/**', path.join(link, 'existing.txt'), root, root)).toBe(false);
            expect(matchesPathPattern('/protected/**', path.join(link, 'existing.txt'), root, root, 'canonical')).toBe(true);
        });
    });
    it('canonicalizes through a file that causes ENOTDIR', () => {
        withTempRoot((root) => {
            ...
            fs.symlinkSync(protectedDir, link, 'dir');
            expect(matchesPathPattern('/protected/**', path.join(link, 'config.json', 'nested.txt'), root, root, 'canonical')).toBe(true);
        });
    });
    it('canonicalizes a symlinked project root in restrictive rules', () => {
        withTempRoot((root) => {
            ...
            expect(matchesPathPattern('/protected/**', path.join(protectedDir, 'file.txt'), linkedRoot, linkedRoot, 'canonical')).toBe(true);
        });
    });
    it('canonicalizes the nearest existing ancestor for a new path', () => {
        withTempRoot((root) => {
            ...
            fs.mkdirSync(protectedDir);
            fs.symlinkSync(protectedDir, link, 'dir');
            expect(matchesPathPattern('/protected/**', path.join(link, 'new', 'file.txt'), root, root, 'canonical')).toBe(true);
        });
    });
    it('matches the target of a dangling symlink', () => {
        withTempRoot((root) => {
            ...
            expect(matchesPathPattern('/protected/**', link, root, root, 'canonical')).toBe(true);
        });
    });
    it('preserves traversal semantics in a dangling symlink target', () => {
        withTempRoot((root) => {
            ...
            fs.symlinkSync(path.join(outsideDir, 'dir'), path.join(projectDir, 'inner'), 'dir');
            const link = path.join(projectDir, 'link.txt');
            fs.symlinkSync(`inner${path.sep}..${path.sep}safe${path.sep}new.txt`, link);
            expect(matchesPathPattern('/outside/safe/**', link, root, root, 'canonical')).toBe(true);
            expect(matchesPathPattern('/project/safe/**', link, root, root, 'canonical')).toBe(false);
        });
    });
    it('resolves parent traversal after following a directory symlink', () => {
        withTempRoot((root) => {
            ...
            expect(matchesPathPattern('/outside/safe/**', filePath, root, root, 'canonical')).toBe(true);
            expect(matchesPathPattern('/project/safe/**', filePath, root, root, 'canonical')).toBe(false);
        });
    });
    it('preserves matching against the lexical symlink path', () => {
        withTempRoot((root) => {
            ...
            fs.mkdirSync(protectedDir);
            fs.symlinkSync(protectedDir, link, 'dir');
            expect(matchesPathPattern('/link/**', path.join(link, 'new.txt'), root, root)).toBe(true);
        });
    });
});
...
describe('matchesRule', () => {
    ...
    it('matches a legacy dotted MCP server rule against provider-safe names', () => {
        ...
        expect(matchesRule(rule, normalizeToolNameForProvider('mcp__zybio.db__query_uniprot'))).toBe(true);
    ...
    it('matchesRule matches tool with param matcher', async () => {
        ...
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, undefined, {
            model: 'opus',
        })).toBe(true);
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, undefined, {
            model: 'sonnet',
    ...
    it('matchesRule handles mixed specifier and param matchers', async () => {
        ...
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, 'coder', { model: 'opus' })).toBe(true);
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, 'coder', { model: 'sonnet' })).toBe(false); // param mismatch
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, 'explore', { model: 'opus' })).toBe(false); // specifier mismatch
    });
...
describe('PermissionManager', () => {
    ...
    describe('basic rule evaluation', () => {
        ...
        it('matches a legacy truncated MCP permission alias', async () => {
            ...
            const pm2 = new PermissionManager(makeConfig({ permissionsAllow: [legacyName] }));
            pm2.initialize();
        ...
        it('honors legacy MCP wildcard deny rules for provider-safe names', async () => {
            ...
            const providerSafeName = normalizeToolNameForProvider(legacyName);
            const pm2 = new PermissionManager(makeConfig({ permissionsDeny: ['mcp__server__literature.*'] }));
            pm2.initialize();
            expect(await pm2.evaluate({
                toolName: providerSafeName,
            })).toBe('deny');
    ...
    describe('file path evaluation', () => {
        ...
        it('canonicalizes restrictive rules without widening allow rules', async () => {
            ...
            try {
                ...
                const denyManager = new PermissionManager(makeConfig({
                    permissionsDeny: ['Edit(/protected/**)'],
                    projectRoot: root,
                    cwd: root,
                }));
                denyManager.initialize();
                expect(await denyManager.evaluate({ toolName: 'edit', filePath })).toBe('deny');
                const allowManager = new PermissionManager(makeConfig({
                    permissionsAllow: ['Edit(/protected/**)'],
                    projectRoot: root,
                    cwd: root,
                }));
                allowManager.initialize();
                expect(await allowManager.evaluate({ toolName: 'edit', filePath })).toBe('default');
            }
            finally {
                fs.rmSync(root, { recursive: true, force: true });
            }
...
describe('buildHumanReadableRuleLabel', () => {
    ...
    it('converts Read with absolute path specifier', () => {
        const label = buildHumanReadableRuleLabel([
            'Read(//Users/mochi/.hopcode/**)',
        ]);
        expect(label).toBe('read files in /Users/mochi/.hopcode/');
    });
...
describe('PermissionManager.findMatchingDenyRule', () => {
    ...
    it('matches a deny rule through a symlinked path', () => {
        ...
        try {
            ...
            pm.initialize();
            expect(pm.findMatchingDenyRule({
                toolName: 'edit',
                filePath: path.join(link, 'file.txt'),
            })).toBe('Edit(/protected/**)');
        }
...
describe('PermissionManager — compound shell write attribution', () => {
    it('deny rule matches a write after `cd` into a subdir', async () => {
        const pm = new PermissionManager(makeConfig({
            permissionsDeny: ['WriteFileTool(.hopcode/settings.json)'],
            cwd: '/repo',
            projectRoot: '/repo',
        }));
        pm.initialize();
    ...
    it('allow rule on the same shell command does NOT downgrade a virtual-op deny', async () => {
        ...
        // .hopcode/settings.json escalates the verdict. Allow + virtual-op deny
    ...
    it('hasRelevantRules sees protected writes after `cd` before compound recursion', () => {
        const pm = new PermissionManager(makeConfig({
            permissionsDeny: ['Write(.hopcode/settings.json)'],
            cwd: '/repo',
            projectRoot: '/repo',
        }));
        pm.initialize();
    ...
    it('hasMatchingAskRule sees writes after `cd` into a subdir', () => {
        const pm = new PermissionManager(makeConfig({
            permissionsAsk: ['WriteFileTool(.hopcode/settings.json)'],
            cwd: '/repo',
            projectRoot: '/repo',
        }));
        pm.initialize();
...
describe('matchesRule — param matcher type guards', () => {
    ...
    it('accepts number param values via coercion', () => {
        const rule = parseRule('Agent(count:42)');
        expect(matchesRule(rule, 'agent', undefined, undefined, undefined, undefined, undefined, { count: 42 })).toBe(true);
    });
});
