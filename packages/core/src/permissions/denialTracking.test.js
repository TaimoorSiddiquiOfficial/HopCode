...
import { AUTO_MODE_DENIAL_LIMITS, createDenialState, formatDenialStateLog, isApproveOutcome, isDenialFallbackReason, recordAllow, recordBlock, recordFallbackApprove, recordUnavailable, resetDenialState, shouldFallback, } from './denialTracking.js';
const FRESH = {
    consecutiveBlock: 0,
...
describe('formatDenialStateLog', () => {
    it('formats every denial counter in a stable order', () => {
        expect(formatDenialStateLog({
            consecutiveBlock: 1,
            consecutiveUnavailable: 2,
            totalBlock: 3,
            totalUnavailable: 4,
        })).toBe('consecutiveBlock=1, consecutiveUnavailable=2, totalBlock=3, totalUnavailable=4');
    });
});
