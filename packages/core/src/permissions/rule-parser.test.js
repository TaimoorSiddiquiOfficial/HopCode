...
import { resolveToolName, getSpecifierKind, toolMatchesRuleToolName, parseRule, parseRules, splitCompoundCommand, matchesCommandPattern, resolvePathPattern, matchesPathPattern, matchesDomainPattern, matchesMcpPattern, matchesRule, buildPermissionRules, buildHumanReadableRuleLabel, getRuleDisplayName, } from '../permissions/rule-parser.js';
// ---------------------------------------------------------------------------
// resolveToolName
// ---------------------------------------------------------------------------
describe('resolveToolName', () => {
    ...
    it('passes through unknown tool names unchanged', () => {
        expect(resolveToolName('mcp__github__list_repos')).toBe('mcp__github__list_repos');
    });
...
describe('splitCompoundCommand', () => {
    ...
    it('splits on multiple operators', () => {
        expect(splitCompoundCommand('git add . && git commit -m "fix" || echo fail')).toEqual(['git add .', 'git commit -m "fix"', 'echo fail']);
    });
...
describe('matchesCommandPattern', () => {
    ...
    it('matches wildcard in middle', () => {
        expect(matchesCommandPattern('git * branch', 'git checkout branch')).toBe(true);
    });
    ...
    it('strips multiple leading variable assignments', () => {
        expect(matchesCommandPattern('git *', 'FOO=bar BAZ=qux git push')).toBe(true);
    });
});
// ---------------------------------------------------------------------------
// resolvePathPattern
...
describe('resolvePathPattern', () => {
    ...
    it('resolves // prefix to absolute path', () => {
        expect(resolvePathPattern('//etc/passwd', projectRoot, cwd)).toBe('/etc/passwd');
    });
...
describe('matchesPathPattern', () => {
    ...
    it('matches exact path', () => {
        expect(matchesPathPattern('./src/main.ts', '/home/user/project/src/main.ts', projectRoot, cwd)).toBe(true);
    });
    it('rejects non-matching path', () => {
        expect(matchesPathPattern('./src/*.ts', '/home/user/project/tests/main.test.ts', projectRoot, cwd)).toBe(false);
    });
    it('matches with ** recursive glob', () => {
        expect(matchesPathPattern('/src/**/*.ts', '/home/user/project/src/utils/deep/nested/file.ts', projectRoot, cwd)).toBe(true);
    });
    it('matches dotfiles with dot:true', () => {
        expect(matchesPathPattern('./.env', '/home/user/project/.env', projectRoot, cwd)).toBe(true);
    });
    it('rejects non-matching directory', () => {
        expect(matchesPathPattern('/src/**/*.ts', '/home/user/project/docs/readme.md', projectRoot, cwd)).toBe(false);
    });
});
// ---------------------------------------------------------------------------
...
describe('matchesMcpPattern', () => {
    it('matches exact tool name', () => {
        expect(matchesMcpPattern('mcp__puppeteer__navigate', 'mcp__puppeteer__navigate')).toBe(true);
    });
    it('matches wildcard suffix', () => {
        expect(matchesMcpPattern('mcp__puppeteer__*', 'mcp__puppeteer__navigate')).toBe(true);
    });
    it('matches server-level pattern (2 parts to 3+ parts)', () => {
        expect(matchesMcpPattern('mcp__puppeteer', 'mcp__puppeteer__navigate')).toBe(true);
    });
    ...
    it('rejects non-matching server', () => {
        expect(matchesMcpPattern('mcp__puppeteer__*', 'mcp__filesystem__read')).toBe(false);
    });
    it('matches prefix with wildcard', () => {
        expect(matchesMcpPattern('mcp__chrome__use_*', 'mcp__chrome__use_page')).toBe(true);
        expect(matchesMcpPattern('mcp__chrome__use_*', 'mcp__chrome__navigate')).toBe(false);
    });
});
// ---------------------------------------------------------------------------
// matchesRule (unified)
...
describe('matchesRule', () => {
    ...
    it('never matches invalid rules', () => {
        const rule = parseRule('Bash(git *');
        expect(matchesRule(rule, 'run_shell_command', 'git status', undefined, undefined, pathCtx)).toBe(false);
    });
    ...
    it('matches path specifier', () => {
        ...
        expect(matchesRule(rule, 'read_file', undefined, '/home/user/project/src/main.ts', undefined, pathCtx)).toBe(true);
    });
    it('matches domain specifier', () => {
        const rule = parseRule('WebFetch(domain:github.com)');
        expect(matchesRule(rule, 'web_fetch', undefined, undefined, 'github.com', pathCtx)).toBe(true);
    });
    it('matches literal specifier for Skill', () => {
        const rule = parseRule('Skill(frontend-design)');
        expect(matchesRule(rule, 'skill', undefined, undefined, undefined, pathCtx, 'frontend-design')).toBe(true);
    });
    it('rejects literal specifier when value does not match', () => {
        const rule = parseRule('Skill(frontend-design)');
        expect(matchesRule(rule, 'skill', undefined, undefined, undefined, pathCtx, 'backend-design')).toBe(false);
    });
...
describe('getRuleDisplayName', () => {
    ...
    it('falls back to canonical name for unmapped tools', () => {
        expect(getRuleDisplayName('mcp__github__list_repos')).toBe('mcp__github__list_repos');
    });
});
