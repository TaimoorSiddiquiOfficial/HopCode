...
describe('isAutoModeProtectedWritePath', () => {
    ...
    it('caches normalized HOPCODE_HOME prefixes per configured home', () => {
        ...
        try {
            ...
            expect(isAutoModeProtectedWritePath(settingsPath)).toBe(true);
            expect(isAutoModeProtectedWritePath(settingsPath)).toBe(true);
            expect(realpathSpy.mock.calls.filter(([arg]) => arg === tmpRoot)).toHaveLength(1);
        }
        finally {
...
describe('passesAcceptEditsFastPath', () => {
    ...
    it('rejects HopCode self-modification paths even inside cwd', () => {
        ...
        for (const filePath of protectedPaths) {
            expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.WRITE_FILE, filePath }), config)).toBe(false);
        }
    });
    ...
    it('rejects symlinks that resolve to protected self-modification paths', () => {
        ...
        try {
            ...
            expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.WRITE_FILE, filePath: link }), cfg)).toBe(false);
        }
        finally {
            fs.rmSync(tmpRoot, { recursive: true, force: true });
        }
...
describe('shouldForceAutoModeReviewForAllow', () => {
    it('returns true for Edit/Write targeting protected self-modification paths', () => {
        expect(shouldForceAutoModeReviewForAllow(ctx({
            toolName: ToolNames.EDIT,
    ...
    it('returns false for ordinary edits and non-edit tools', () => {
        expect(shouldForceAutoModeReviewForAllow(ctx({ toolName: ToolNames.EDIT, filePath: '/repo/src/index.ts' }))).toBe(false);
        expect(shouldForceAutoModeReviewForAllow(ctx({
...
describe('applyAutoModeDecision — blocked reason mapping', () => {
    ...
    it('maps classifier policy blocks to classifier_blocked', () => {
        ...
        const result = applyAutoModeDecision({
            ...
            stage: 'fast',
            durationMs: 10,
        }, { setAutoModeDenialState }, denialState);
    ...
    it('passes through fallback reason without mutating denial state', () => {
        const setAutoModeDenialState = vi.fn();
        const result = applyAutoModeDecision({ via: 'fallback', reason: 'consecutive_block' }, { setAutoModeDenialState }, denialState);
...
describe('PermissionDenied hook gating', () => {
    ...
    it('fires only for classifier blocks that produce a blocked outcome', () => {
        expect(shouldFirePermissionDeniedForAutoMode(classifierBlock, {
            kind: 'blocked',
            errorMessage: 'blocked',
            reason: 'classifier_blocked',
        })).toBe(true);
        expect(shouldFirePermissionDeniedForAutoMode({ ...classifierBlock, shouldBlock: false }, { kind: 'approved' })).toBe(false);
        expect(shouldFirePermissionDeniedForAutoMode({ via: 'fallback', reason: 'safety_check' }, { kind: 'fallback', reason: 'safety_check' })).toBe(false);
    });
    it('maps classifier blocks to stable PermissionDenied reasons', () => {
        expect(getAutoModePermissionDeniedReason(classifierBlock)).toBe('classifier_blocked');
...
describe('shouldRunAutoModeForCall', () => {
    ...
    it('excludes ENTER_PLAN_MODE even under AUTO — plan entries are always allowed without classification', () => {
        expect(shouldRunAutoModeForCall(ApprovalMode.AUTO, ToolNames.ENTER_PLAN_MODE)).toBe(false);
    });
...
describe('shouldClassifyAllShellForAutoMode', () => {
    function configWithClassifyAllShell(enabled) {
        return {
            getAutoModeSettings: () => ({ classifyAllShell: enabled }),
        };
    }
    it('returns true for Shell when classifyAllShell is enabled', () => {
        expect(shouldClassifyAllShellForAutoMode(ToolNames.SHELL, configWithClassifyAllShell(true))).toBe(true);
    });
    it('returns true for Monitor when classifyAllShell is enabled', () => {
        expect(shouldClassifyAllShellForAutoMode(ToolNames.MONITOR, configWithClassifyAllShell(true))).toBe(true);
    });
    it('returns false for Shell when classifyAllShell is disabled', () => {
        expect(shouldClassifyAllShellForAutoMode(ToolNames.SHELL, configWithClassifyAllShell(false))).toBe(false);
    });
    it('returns false for non-shell tools even when classifyAllShell is enabled', () => {
        for (const tool of [
            ...
            expect(shouldClassifyAllShellForAutoMode(tool, configWithClassifyAllShell(true))).toBe(false);
        }
    });
    it('returns false when classifyAllShell is undefined (default)', () => {
        ...
        expect(shouldClassifyAllShellForAutoMode(ToolNames.SHELL, config)).toBe(false);
    });
});
