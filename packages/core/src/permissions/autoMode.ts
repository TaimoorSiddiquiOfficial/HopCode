...
let hopcodeHomePrefixesCacheKey: string | undefined;
let hopcodeHomePrefixesCache: string[] | undefined;

function getNormalizedhopcodeHomePrefixes(): string[] {
  const hopcodeHome = process.env['HOPCODE_HOME'];
  if (!hopcodeHome) return [];
  if (
  ...
  const candidates = new Set<string>([path.resolve(hopcodeHome)]);
...
function matcheshopcodeHomeSurface(normalizedPath: string): boolean {
  for (const normalizedhopcodeHome of getNormalizedhopcodeHomePrefixes()) {
    const hopcodeHomePrefix = `${normalizedhopcodeHome}/`;
    if (!normalizedPath.startsWith(hopcodeHomePrefix)) continue;
