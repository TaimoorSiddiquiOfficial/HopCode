...
import { buildClassifierSystemPrompt, BUILTIN_ALLOW, BUILTIN_DENY, BUILTIN_ENVIRONMENT, BUILTIN_HARD_DENY, BUILTIN_SOFT_DENY, STAGE1_SUFFIX, STAGE2_SUFFIX, } from './system-prompt.js';
...
describe('buildClassifierSystemPrompt', () => {
    ...
    it('defines self-modification as a built-in SOFT BLOCK category', () => {
        expect(BUILTIN_SOFT_DENY.some((entry) => entry.includes('Self-modification'))).toBe(true);
        const prompt = buildClassifierSystemPrompt(makeConfig({}));
        expect(prompt).toContain('Self-modification');
        expect(prompt).toContain('.hopcode/settings');
        expect(prompt).toContain('HOPCODE.local.md');
        expect(prompt).toContain('.hopcode/rules/');
    ...
    it('defines AUTO-mode bypass and data exfiltration as built-in HARD BLOCK categories', () => {
        expect(BUILTIN_HARD_DENY.some((entry) => entry.includes('AUTO-mode bypass'))).toBe(true);
        expect(BUILTIN_HARD_DENY.some((entry) => entry.includes('Data exfiltration'))).toBe(true);
        const prompt = buildClassifierSystemPrompt(makeConfig({}));
    ...
    it('renders legacy `hints.deny` under the User SOFT BLOCK section', () => {
        ...
        const prompt = buildClassifierSystemPrompt(makeConfig({ hints: { deny: ['Legacy deny hint'] } }));
    ...
    it('renders `hints.hardDeny` under the User HARD BLOCK section', () => {
        const prompt = buildClassifierSystemPrompt(makeConfig({ hints: { hardDeny: ['Never touch production billing'] } }));
    ...
    it('renders `hints.softDeny` before legacy `hints.deny` in the User SOFT BLOCK section', () => {
        ...
        expect(prompt.indexOf('Modern soft entry')).toBeLessThan(prompt.indexOf('Legacy entry'));
    });
