...
import { isDestructiveCommand, userMentionsDiscard, extractLastUserPrompt, registerSessionCommit, clearSessionCommits, } from './destructive-commands.js';
// ─── userMentionsDiscard ───────────────────────────────────────────────────
...
describe('isDestructiveCommand — git patterns', () => {
    ...
    it('allows git reset --hard when user mentions discard', () => {
        const result = isDestructiveCommand('git reset --hard', 'discard all local changes and reset');
        expect(result).toBeNull();
    });
    it('allows git clean -fd when user mentions wipe', () => {
        const result = isDestructiveCommand('git clean -fd', 'wipe the working tree clean');
        expect(result).toBeNull();
    });
    it('allows git stash drop when user mentions discard', () => {
        const result = isDestructiveCommand('git stash drop', 'discard all stashes');
        expect(result).toBeNull();
    });
...
describe('isDestructiveCommand — shell indirection', () => {
    ...
    it('blocks bash -c "git reset --hard"', () => {
        const result = isDestructiveCommand('bash -c "git reset --hard"', 'fix something');
        expect(result).not.toBeNull();
        expect(result.blocked).toBe(true);
    });
    it("blocks sh -c 'git clean -fd'", () => {
        const result = isDestructiveCommand("sh -c 'git clean -fd'", 'remove untracked files');
        expect(result).not.toBeNull();
        expect(result.blocked).toBe(true);
    });
    ...
    it('allows bash -c with safe commands', () => {
        const result = isDestructiveCommand('bash -c "git status && git log"', 'check status');
        expect(result).toBeNull();
    });
});
...
describe('isDestructiveCommand — IaC patterns', () => {
    it('blocks terraform destroy', () => {
        const result = isDestructiveCommand('terraform destroy', 'update infrastructure');
        expect(result).not.toBeNull();
        expect(result.blocked).toBe(true);
        expect(result.reason).toContain('terraform');
    });
    it('blocks pulumi destroy', () => {
        const result = isDestructiveCommand('pulumi destroy', 'update infrastructure');
        expect(result).not.toBeNull();
        expect(result.blocked).toBe(true);
    });
    ...
    it('allows terraform destroy when user explicitly requests it', () => {
        const result = isDestructiveCommand('terraform destroy', 'terraform destroy the staging stack');
        expect(result).toBeNull();
    });
...
describe('isDestructiveCommand — git commit --amend', () => {
    ...
    it('blocks git commit --amend when no session commits registered', () => {
        const result = isDestructiveCommand('git commit --amend --no-edit', 'amend the commit');
        expect(result).not.toBeNull();
        expect(result.blocked).toBe(true);
        expect(result.reason).toContain('amend');
    });
    it('allows git commit (without --amend)', () => {
        const result = isDestructiveCommand('git commit -m "fix"', 'commit changes');
        expect(result).toBeNull();
    });
});
...
//# sourceMappingURL=destructive-commands.test.js.map