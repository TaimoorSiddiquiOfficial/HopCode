...
import { classifyAction, sanitizeClassifierReason, STAGE1_TIMEOUT_MS, STAGE2_TIMEOUT_MS, } from './classifier.js';
function makeConfig(autoModeSettings = {}) {
    return {
...
describe('classifier configuration', () => {
    ...
    it('falls back when configured stage timeouts are too low', async () => {
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith(`Classifier timeout 1ms below 1000ms floor, using default ${STAGE1_TIMEOUT_MS}ms`);
        expect(debugLoggerMock.warn).toHaveBeenCalledWith(`Classifier timeout 999ms below 1000ms floor, using default ${STAGE2_TIMEOUT_MS}ms`);
    });
    ...
    it('enables API thinking only for stage 2 when configured', async () => {
        ...
        const stage1 = runSideQueryMock.mock.calls[0]?.[1];
        const stage2 = runSideQueryMock.mock.calls[1]?.[1];
