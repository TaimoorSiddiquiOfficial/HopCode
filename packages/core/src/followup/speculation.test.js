...
import { abortSpeculation, ensureToolResultPairing, startSpeculation, } from './speculation.js';
...
vi.mock('../utils/forkedAgent.js', () => ({
    ...
    runWithForkedChatModel: vi.fn(async (_config, model, callback) => callback(model)),
}));
afterEach(() => {
    vi.clearAllMocks();
});
...
describe.each([
...
])('generatePipelinedSuggestion preserveTools — $scenario', ({ fastModel, expectedPreserveTools }) => {
    it(`passes preserveTools: ${String(expectedPreserveTools)} to runForkedAgent`, async () => {
        ...
        expect(forkedAgentMocks.runForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ preserveTools: expectedPreserveTools }));
        await abortSpeculation(state);
    });
});
