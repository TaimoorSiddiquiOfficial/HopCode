...
import { generatePromptSuggestion, getFilterReason, shouldFilterSuggestion, } from './suggestionGenerator.js';
...
describe('generatePromptSuggestion', () => {
    ...
    it('passes preserveTools: true for Anthropic prompt-cache sharing', async () => {
        ...
        await generatePromptSuggestion(config, conversationHistory, new AbortController().signal, { enableCacheSharing: true });
        expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ preserveTools: true }));
    });
    it('passes preserveTools: false when fast model differs from cache-safe model', async () => {
        ...
        await generatePromptSuggestion(config, conversationHistory, new AbortController().signal, { enableCacheSharing: true });
        expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ preserveTools: false }));
    });
});
describe('shouldFilterSuggestion', () => {
    ...
    it('does not filter abbreviations as multiple sentences', () => {
        ...
        expect(shouldFilterSuggestion("Let's start with the Weeds vs. Wildflowers audit.")).toBe(false);
        ...
        expect(shouldFilterSuggestion('Review options etc. Then commit')).toBe(false);
