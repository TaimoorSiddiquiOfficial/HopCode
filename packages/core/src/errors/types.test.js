...
import { HopCodeError, ErrorCategory, isHopCodeError, toHopCodeError, UnknownError, FilesystemError, FilesystemErrors, NetworkError, NetworkErrors, ProviderError, ProviderErrors, ToolError, ToolErrors, ConfigError, ConfigErrors, InputError, CancellationError, } from './types.js';
...
describe('FilesystemError', () => {
    ...
    it('should accept recovery hint', () => {
        const error = new FilesystemError('PERMISSION_DENIED', 'Permission denied', { filePath: '/test.txt', operation: 'write' }, undefined, 'Check file permissions');
...
describe('NetworkError', () => {
    ...
    it('should accept custom retryable flag', () => {
        const error = new NetworkError('DNS_FAILURE', 'DNS failed', {}, undefined, false);
        expect(error.isRetryable).toBe(false);
    });
});
...
describe('ToolErrors factory', () => {
    ...
    it('should create invalidParams error with details', () => {
        const error = ToolErrors.invalidParams('edit', 'Missing required field: path');
...
describe('ConfigErrors factory', () => {
    ...
    it('should create invalidConfig error', () => {
        const error = ConfigErrors.invalidConfig('timeout', 'Must be positive number');
