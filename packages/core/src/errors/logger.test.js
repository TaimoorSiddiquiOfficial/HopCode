...
import { ErrorLogger, LogLevel, ConsoleSink, FileSink, errorLogger, } from './logger.js';
...
describe('ConsoleSink', () => {
    ...
    it('should format console output with category and code', () => {
        ...
        sink.log(record, LogLevel.ERROR);
        expect(consoleErrorSpy.mock.calls[0][0]).toMatch(/\[FILESYSTEM\].*PERMISSION_DENIED.*Permission denied/);
        consoleErrorSpy.mockRestore();
    });
    ...
    it('should include file path in console output', () => {
        ...
        sink.log(record, LogLevel.ERROR);
        expect(consoleErrorSpy.mock.calls[0][0]).toContain('(file: /test/file.txt)');
        consoleErrorSpy.mockRestore();
    });
...
describe('ErrorLogger', () => {
    ...
    it('should enrich records with session context', () => {
        ...
        const call = sink.log.mock.calls[0];
        const record = call[0];
