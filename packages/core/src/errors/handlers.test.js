...
import { HopCodeError, ErrorCategory, NetworkErrors, ProviderErrors, ConfigErrors, NetworkError, } from './types.js';
...
describe('withRetry', () => {
    ...
    it('uses custom shouldRetry predicate', async () => {
        const customError = new HopCodeError('CUSTOM', ErrorCategory.INTERNAL, 'Custom error');
        const operation = vi
            .fn()
        ...
        const result = withRetry(operation, {
            maxRetries: 3,
            initialDelayMs: 100,
            shouldRetry: (error) => error instanceof HopCodeError && error.code === 'CUSTOM',
        });
        await vi.runAllTimersAsync();
        expect(await result).toBe('success');
        expect(operation).toHaveBeenCalledTimes(2);
...
describe('withFallback', () => {
    ...
    it('uses custom shouldFallback predicate', async () => {
        const customError = new HopCodeError('CUSTOM', ErrorCategory.TOOL, 'Tool failed');
        const operation = vi.fn().mockRejectedValue(customError);
        const fallback = 'fallback-value';
        const result = withFallback(operation, {
            fallback,
            shouldFallback: (error) => error instanceof HopCodeError && error.category === ErrorCategory.TOOL,
        });
    ...
    it('logs fallback usage by default', async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith('Using fallback value', expect.objectContaining({
...
describe('withinErrorBoundary', () => {
    ...
    it('wraps error in HopCodeError', async () => {
        ...
        await expect(result).rejects.toThrow('Tool execution failed: original error');
    ...
    it('uses custom shouldCatch predicate', async () => {
        ...
        const context = { operation: 'test-operation' };
        const result = withinErrorBoundary(operation, context, {
            shouldCatch: (error) => error instanceof HopCodeError && error.code === 'CUSTOM',
            onError,
        });
