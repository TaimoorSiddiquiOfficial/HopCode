...
// OAuth Endpoints
const HOPCODE_OAUTH_BASE_URL = 'https://chat.hopcode.ai';
const HOPCODE_OAUTH_DEVICE_CODE_ENDPOINT = `${HOPCODE_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const HOPCODE_OAUTH_TOKEN_ENDPOINT = `${HOPCODE_OAUTH_BASE_URL}/api/v1/oauth2/token`;
// OAuth Client Configuration
const HOPCODE_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';
 ...
 * on `instanceof HopCodeOAuthPollError` + read fields directly.
 */
export class HopCodeOAuthPollError extends Error {
    status;
    oauthError;
    description;
    constructor(opts) {
        super(`Device token poll failed: ${opts.oauthError ?? 'Unknown error'} - ${opts.description ?? '(no description)'}`);
        this.name = 'HopCodeOAuthPollError';
 ...
 */
export class HopCodeOAuth2Client {
    credentials = {};
    sharedManager;
    constructor() {
    ...
    async requestDeviceAuthorization(options, fetchOpts) {
        ...
        const response = await fetch(HOPCODE_OAUTH_DEVICE_CODE_ENDPOINT, {
            method: 'POST',
            headers: {
    ...
    async pollDeviceToken(options, fetchOpts) {
        const bodyData = {
            grant_type: HOPCODE_OAUTH_GRANT_TYPE,
            client_id: HOPCODE_OAUTH_CLIENT_ID,
        ...
        if (!response.ok) {
            ...
            // For other errors, throw a typed `HopCodeOAuthPollError` so
    ...
    async refreshAccessToken() {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`HopCode OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`);
            }
            // Check if the response indicates success
...
export var HopCodeOAuth2Event;
(function (HopCodeOAuth2Event) {
    HopCodeOAuth2Event["AuthUri"] = "auth-uri";
    HopCodeOAuth2Event["AuthProgress"] = "auth-progress";
    HopCodeOAuth2Event["AuthCancel"] = "auth-cancel";
})(HopCodeOAuth2Event || (HopCodeOAuth2Event = {}));
/**
...
export async function getHopCodeOAuthClient(config, options) {
    ...
    try {
        ...
        if (options?.requireCachedCredentials) {
            throw new Error('HopCode OAuth credentials expired. Please use /auth to re-authenticate with hopcode-oauth.');
        }
        ...
        if (!result.success) {
            ...
            if (result.reason === 'timeout') {
                HopCodeOAuth2Events.emit(HopCodeOAuth2Event.AuthProgress, 'timeout', 'Authentication timed out. Please try again or select a different authentication method.');
            }
...
async function authWithHopCodeDeviceFlow(client, config) {
...
export async function cacheHopCodeCredentials(credentials, opts) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (process.platform !== 'win32') {
                    throw new Error(`cacheHopCodeCredentials: refusing to publish credentials — chmod 0o${HOPCODE_CREDENTIAL_FILE_MODE.toString(8)} on temp file failed: ${chmodErr instanceof Error ? chmodErr.message : String(chmodErr)}`);
                }
                ...
                debugLogger.warn(`cacheHopCodeCredentials: chmod 0o${HOPCODE_CREDENTIAL_FILE_MODE.toString(8)} on Windows temp file ${tempPath} failed; relying on NTFS ACL: ${chmodErr instanceof Error ? chmodErr.message : String(chmodErr)}`);
            }
        ...
        try {
            ...
            debugLogger.warn(`cacheHopCodeCredentials: SharedTokenManager.clearCache failed; in-process callers may serve stale credentials until the next mtime poll: ${clearErr instanceof Error ? clearErr.message : String(clearErr)}`);
        }
    }
    catch (error) {
 ...
 */
export async function clearHopCodeCredentials() {
    try {
        const filePath = getHopCodeCachedCredentialPath();
        ...
        debugLogger.warn('Warning: Failed to clear cached HopCode credentials:', error);
    }
    finally {
...
function getHopCodeCachedCredentialPath() {
...
//# sourceMappingURL=hopCodeOAuth2.js.map