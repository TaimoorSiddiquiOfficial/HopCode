...
import { isErrorResponse, CredentialsClearRequiredError, } from './hopCodeOAuth2.js';
...
export class SharedTokenManager {
     ...
     */
    async getValidCredentials(hopcodeClient, forceRefresh = false) {
        try {
            ...
            if (!currentRefreshPromise) {
                ...
                currentRefreshPromise = this.performTokenRefresh(hopcodeClient, forceRefresh);
     ...
     */
    async checkAndReloadIfNeeded(hopcodeClient) {
     ...
     */
    async performFileCheck(hopcodeClient, checkTime) {
     ...
     */
    async forceFileCheck(hopcodeClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async reloadCredentialsFromFile(hopcodeClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async performTokenRefresh(hopcodeClient, forceRefresh = false) {
