...
import { generateCodeChallenge, generateCodeVerifier, generatePKCEPair, isDeviceAuthorizationSuccess, isDeviceTokenPending, isDeviceTokenSuccess, isErrorResponse, hopCodeOAuth2Events, HopCodeOAuth2Event, HopCodeOAuth2Client, } from './hopCodeOAuth2.js';
...
describe('HopCodeOAuth2Client', () => {
    ...
    describe('refreshAccessToken error handling', () => {
        ...
        it('should time out hung refresh token requests', async () => {
            vi.useFakeTimers();
            try {
                vi.mocked(global.fetch).mockImplementation((_url, init) => new Promise((_, reject) => {
                    ...
                    signal.addEventListener('abort', () => {
                        reject(Object.assign(new DOMException('The operation was aborted', 'AbortError'), { cause: signal.reason }));
                    }, { once: true });
                }));
                ...
                expect(error.message).toContain('Token refresh timeout: The operation was aborted (cause: Operation timed out)');
        ...
        it('should preserve non-timeout network errors and show the token endpoint', async () => {
            ...
            expect(error.message).toContain('https://chat.hopcode.ai/api/v1/oauth2/token');
        });
        it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
            ...
            await expect(client.refreshAccessToken()).rejects.toThrow('HopCode OAuth refresh returned invalid JSON:');
        });
...
describe('getHopCodeOAuthClient', () => {
    ...
    it('should load cached credentials if available', async () => {
        ...
        const client = await import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig));
    ...
    it('should handle cached credentials refresh failure', async () => {
        ...
        await expect(import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig))).rejects.toThrow('Device authorization flow failed');
        SharedTokenManager.getInstance = originalGetInstance;
    });
    it('should not start device flow when requireCachedCredentials is true', async () => {
        ...
        await expect(import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig, {
            requireCachedCredentials: true,
        }))).rejects.toThrow('HopCode OAuth credentials expired. Please use /auth to re-authenticate with hopcode-oauth.');
    ...
    it('should include troubleshooting hints when device auth fetch fails', async () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith(HopCodeOAuth2Event.AuthProgress, 'error', expect.stringContaining('NODE_EXTRA_CA_CERTS'));
        emitSpy.mockRestore();
        SharedTokenManager.getInstance = originalGetInstance;
    });
});
...
describe('getHopCodeOAuthClient - Enhanced Error Scenarios', () => {
    ...
    it('should handle different authentication failure reasons - timeout', async () => {
        ...
        await expect(import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig))).rejects.toThrow('Authorization timeout, please restart the process.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
    it('should handle authentication failure reason - rate limit', async () => {
        ...
        await expect(import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig))).rejects.toThrow('Too many requests. The server is rate limiting our requests. Please select a different authentication method or try again later.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
...
describe('authWithHopCodeDeviceFlow - Comprehensive Testing', () => {
    ...
    it('should handle 401 error during token polling', async () => {
        ...
        await expect(import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig))).rejects.toThrow('Device code expired or invalid, please restart the authorization process.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
...
describe('Browser Launch and Error Handling', () => {
    ...
    it('should handle browser launch failure gracefully', async () => {
        ...
        const originalGetInstance = SharedTokenManager.getInstance;
        SharedTokenManager.getInstance = vi.fn().mockReturnValue(mockTokenManager);
        mockOpenBrowserSecurely.mockRejectedValue(new Error('Browser launch failed'));
        const mockAuthResponse = {
            ok: true,
            json: async () => ({
                device_code: 'test-device-code',
        ...
        const client = await import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig));
        expect(client).toBeInstanceOf(Object);
        expect(mockOpenBrowserSecurely).toHaveBeenCalledWith('https://chat.hopcode.ai/device?code=TEST123');
        SharedTokenManager.getInstance = originalGetInstance;
    });
    ...
    it('should handle TokenManagerError types correctly in getHopCodeOAuthClient', async () => {
        ...
        for (const errorInfo of tokenErrors) {
            ...
            global.fetch = vi
                ...
                .mockResolvedValue(mockTokenResponse);
            try {
                await import('./hopCodeOAuth2.js').then((module) => module.getHopCodeOAuthClient(mockConfig));
            }
            catch {
...
//# sourceMappingURL=hopCodeOAuth2.test.js.map