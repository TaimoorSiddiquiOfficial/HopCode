...
vi.mock('./sharedTokenManager.js', () => ({
    SharedTokenManager: class {
        ...
        async getValidCredentials(hopcodeClient) {
                ...
                if (isAuthError) {
                    ...
                    try {
                        ...
                        if (refreshResult && !('error' in refreshResult)) {
                            ...
                            throw new Error('Failed to obtain valid HopCode access token. Please re-authenticate.');
                        }
                    }
                    catch {
                        throw new Error('Failed to obtain valid HopCode access token. Please re-authenticate.');
...
describe('HopCodeContentGenerator', () => {
    ...
    beforeEach(() => {
        ...
        HopCodeContentGenerator = new HopCodeGeneratorClass(mockHopCodeClient, contentGeneratorConfig, mockConfig);
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    describe('Core Content Generation Methods', () => {
        it('should generate content with valid token', async () => {
            ...
            vi.mocked(mockHopCodeClient.getCredentials).mockReturnValue(mockCredentials);
            const request = {
                model: 'hopcode-turbo',
                contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
            };
            const result = await HopCodeContentGenerator.generateContent(request, 'test-prompt-id');
        ...
        it('should generate content stream with valid token', async () => {
            ...
            const stream = await HopCodeContentGenerator.generateContentStream(request, 'test-prompt-id');
            const chunks = [];
    ...
    describe('Token Management and Refresh Logic', () => {
        ...
        it('should handle token refresh failure', async () => {
            ...
            mockTokenManager.setMockError(new Error('Failed to obtain valid HopCode access token. Please re-authenticate.'));
            const request = {
                model: 'hopcode-turbo',
                contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
            };
            await expect(HopCodeContentGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow('Failed to obtain valid HopCode access token. Please re-authenticate.');
            // Clean up
    ...
    describe('Endpoint URL Normalization', () => {
        it('should use default endpoint when no custom endpoint provided', async () => {
            ...
            const parentPrototype = Object.getPrototypeOf(Object.getPrototypeOf(HopCodeContentGenerator));
    ...
    describe('Client State Management', () => {
        it('should set dynamic credentials during operations', async () => {
            const client = HopCodeContentGenerator.pipeline.client;
            vi.mocked(mockHopCodeClient.getAccessToken).mockResolvedValue({
                token: 'temp-token',
            });
            vi.mocked(mockHopCodeClient.getCredentials).mockReturnValue({
    ...
    describe('Error Handling and Retry Logic', () => {
        it('should retry once on authentication errors', async () => {
            ...
            vi.mocked(mockHopCodeClient.getAccessToken).mockImplementation(async () => {
        ...
        it('should not retry non-authentication errors', async () => {
            ...
            await expect(HopCodeContentGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow('Network timeout');
        ...
        it('should handle error response from token refresh', async () => {
            vi.mocked(mockHopCodeClient.getAccessToken).mockRejectedValue(new Error('Token expired'));
            vi.mocked(mockHopCodeClient.refreshAccessToken).mockResolvedValue({
                error: 'invalid_grant',
            ...
            await expect(HopCodeContentGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow('Failed to obtain valid HopCode access token');
        });
    });
    describe('Token State Management', () => {
        it('should cache and return current token', () => {
            ...
            HopCodeContentGenerator.currentToken = 'cached-token';
        ...
        it('should clear token on clearToken()', () => {
            ...
            const hopcodeInstance = HopCodeContentGenerator;
        ...
        it('should handle concurrent token refresh requests', async () => {
            ...
            vi.mocked(mockHopCodeClient.getCredentials).mockReturnValue(mockCredentials);
            vi.mocked(mockHopCodeClient.refreshAccessToken).mockImplementation(async () => {
                refreshCallCount++;
    ...
    describe('Error Logging Suppression', () => {
        it('should suppress logging for authentication errors', () => {
            ...
            authErrors.forEach((error) => {
                const shouldSuppress = HopCodeContentGenerator.shouldSuppressErrorLogging(error, {});
    ...
    describe('SharedTokenManager Integration', () => {
        it('should use SharedTokenManager to get valid credentials', async () => {
            ...
            const newGenerator = new HopCodeGeneratorClass(mockHopCodeClient, { model: 'hopcode-turbo', authType: AuthType.HOPCODE_OAUTH }, mockConfig);
            const request = {
                model: 'hopcode-turbo',
                contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
            };
            await newGenerator.generateContent(request, 'test-prompt-id');
            expect(mockTokenManager.getValidCredentials).toHaveBeenCalledWith(mockHopCodeClient);
            // Restore original
        ...
        it('should handle SharedTokenManager errors gracefully', async () => {
            ...
            await expect(newGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow('Failed to obtain valid HopCode access token');
            SharedTokenManager.getInstance = originalGetInstance;
        });
    ...
    describe('getCurrentEndpoint Method', () => {
        it('should handle URLs with custom ports', () => {
            ...
            endpoints.forEach(({ input, expected }) => {
                ...
                const generator = HopCodeContentGenerator;
    ...
    describe('Stream Error Handling', () => {
        it('should set credentials when stream generation fails', async () => {
            ...
            try {
                await HopCodeContentGenerator.generateContentStream(request, 'test-prompt-id');
            }
            catch (error) {
        ...
        it('should set credentials for successful streams', async () => {
            ...
            vi.mocked(mockHopCodeClient.getCredentials).mockReturnValue(streamCredentials);
    ...
    describe('Constructor and Initialization', () => {
        it('should initialize with configured base URL when provided', () => {
            const generator = new HopCodeGeneratorClass(mockHopCodeClient, {
                model: 'hopcode-turbo',
                authType: AuthType.HOPCODE_OAUTH,
                baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
                apiKey: 'test-key',
        ...
        it('should get SharedTokenManager instance', () => {
            const generator = new HopCodeGeneratorClass(mockHopCodeClient, { model: 'hopcode-turbo', authType: AuthType.HOPCODE_OAUTH }, mockConfig);
    ...
    describe('Edge Cases and Error Conditions', () => {
        ...
        it('should handle method types with token failure (except countTokens)', async () => {
            ...
            await expect(newGenerator.generateContent(generateRequest, 'test-id')).rejects.toThrow('Failed to obtain valid HopCode access token');
            await expect(newGenerator.generateContentStream(generateRequest, 'test-id')).rejects.toThrow('Failed to obtain valid HopCode access token');
            await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow('Failed to obtain valid HopCode access token');
...
//# sourceMappingURL=hopCodeContentGenerator.test.js.map