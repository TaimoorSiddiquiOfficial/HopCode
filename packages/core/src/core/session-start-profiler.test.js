...
import { SESSION_START_PROFILE_ENV, createSessionStartProfiler, } from './session-start-profiler.js';
...
describe('session-start-profiler', () => {
    ...
    it('is a no-op when disabled', async () => {
        ...
        await expect(profiler.time('tool_registry_warm', async () => 'ok')).resolves.toBe('ok');
        const existingPromise = Promise.resolve('same-promise');
        expect(profiler.time('same_promise', () => existingPromise)).toBe(existingPromise);
        const disabledError = new Error('disabled failure');
        await expect(profiler.time('disabled_error', () => {
            throw disabledError;
    ...
    it.each(['true', '0', ''])('stays disabled when env var is set to %j', (envValue) => {
    ...
    it('records sync and async stages when enabled', async () => {
        ...
        await expect(profiler.time('initial_chat_history', async () => 'history')).resolves.toBe('history');
        expect(profiler.timeSync('system_instruction', () => 'system')).toBe('system');
        profiler.finish({
            ok: true,
        ...
        expect(debugLoggerMock.debug).toHaveBeenCalledWith('session-start-profiler enabled', { source: 'resume' });
    });
    ...
    it('accumulates repeated stage durations', () => {
        ...
        expect(profiler.timeSync('system_instruction', () => 'first')).toBe('first');
        expect(profiler.timeSync('system_instruction', () => 'second')).toBe('second');
        profiler.finish({ ok: true });
        expect(records[0]).toMatchObject({
            ok: true,
            totalMs: 20,
    ...
    it('rethrows stage errors and preserves the failed stage', async () => {
        ...
        await expect(profiler.time('set_tools', async () => {
            throw error;
        })).rejects.toBe(error);
    ...
    it('preserves the first failed stage', async () => {
        ...
        await expect(profiler.time('stage_a', async () => {
            throw new Error('stage a failed');
        })).rejects.toThrow('stage a failed');
        await expect(profiler.time('stage_b', async () => {
    ...
    it('rethrows sync stage errors and preserves the failed stage', () => {
        ...
        expect(() => profiler.timeSync('system_instruction', () => {
            throw error;
        })).toThrow(error);
        profiler.finish({ ok: false });
        expect(records[0]).toMatchObject({
    ...
    it('does not throw when the output writer fails', () => {
        ...
        expect(debugLoggerMock.debug).toHaveBeenCalledWith('session-start-profiler write failed', { name: 'Error', message: 'disk full', code: 'ENOSPC' });
    });
    ...
    itNoSymlink('does not write through a symlinked JSONL file', async () => {
        ...
        try {
            ...
            expect(debugLoggerMock.debug).toHaveBeenCalledWith('session-start-profiler write failed', expect.objectContaining({ name: 'Error' }));
        }
        finally {
            await rm(runtimeDir, { recursive: true, force: true });
        }
    });
    itNoSymlink('does not write through a symlinked profile directory', async () => {
        const runtimeDir = await mkdtemp(join(tmpdir(), 'session-start-profiler-'));
        vi.stubEnv('QWEN_RUNTIME_DIR', runtimeDir);
        vi.stubEnv(SESSION_START_PROFILE_ENV, '1');
        try {
...
//# sourceMappingURL=session-start-profiler.test.js.map