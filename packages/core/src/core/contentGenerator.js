...
(function (AuthType) {
    ...
    AuthType["HOPCODE_OAUTH"] = "hopcode-oauth";
...
export async function createContentGenerator(generatorConfig, config, isInitialAuth) {
    ...
    try {
        if (authType === AuthType.USE_OPENAI) {
            ...
            const { getHopCodeOAuthClient: getHopCodeOAuthClient } = await import('../hopcode/hopCodeOAuth2.js');
            const { HopCodeContentGenerator } = await import('../hopcode/hopCodeContentGenerator.js');
            try {
                const hopcodeClient = await getHopCodeOAuthClient(config, isInitialAuth ? { requireCachedCredentials: true } : undefined);
                baseGenerator = new HopCodeContentGenerator(hopcodeClient, generatorConfig, config);
            }
            catch (error) {
        ...
        if (moduleNotFoundError) {
            throw new Error(`HopCode was updated in the background and needs to be restarted.\n` +
                `Please exit and restart HopCode to use the '${authType}' provider.`, { cause: moduleNotFoundError });
        }
        throw error;
    }
