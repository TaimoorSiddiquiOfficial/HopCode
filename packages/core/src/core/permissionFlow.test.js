...
describe('evaluatePermissionFlow', () => {
    ...
    it('passes invocation permission aliases to the permission manager', async () => {
        ...
        await evaluatePermissionFlow(mockConfig({
            getPermissionManager: vi.fn().mockReturnValue(mockPm),
        }), invocation, 'mcp__server__provider_safe_name', {});
        expect(mockPm.hasRelevantRules).toHaveBeenCalledWith(expect.objectContaining({ toolAliases: [legacyName] }));
    });
    it('forces interaction even when PM allows the tool', async () => {
        ...
        const result = await evaluatePermissionFlow(mockConfig({ getPermissionManager: vi.fn().mockReturnValue(mockPm) }), invocation, ToolNames.EXIT_PLAN_MODE, { plan: 'Plan' });
        expect(result.finalPermission).toBe('ask');
    ...
    it('preserves an intrinsic deny for an interaction-required tool', async () => {
        ...
        const result = await evaluatePermissionFlow(mockConfig(), invocation, ToolNames.EXIT_PLAN_MODE, { plan: 'Plan' });
        expect(result.finalPermission).toBe('deny');
    });
...
describe('needsConfirmation', () => {
    it('should return false for IZN mode non-ask_user_question tools', () => {
        ...
        expect(needsConfirmation('default', ApprovalMode.IZN, 'read_file')).toBe(false);
    });
    it('should return true for ask_user_question in IZN mode', () => {
        expect(needsConfirmation('ask', ApprovalMode.IZN, ToolNames.ASK_USER_QUESTION)).toBe(true);
    });
    it('requires confirmation in YOLO when the invocation requires interaction', () => {
        expect(needsConfirmation('ask', ApprovalMode.YOLO, 'shell', true)).toBe(true);
    });
    it('never requests confirmation for a hard deny', () => {
        expect(needsConfirmation('deny', ApprovalMode.YOLO, 'shell', true)).toBe(false);
    });
...
describe('getEffectivePermissionForConfirmation', () => {
    ...
    it('preserves ordinary permission decisions', () => {
        ...
        expect(getEffectivePermissionForConfirmation('default', true)).toBe('default');
...
describe('isPlanModeBlocked', () => {
    ...
    it('should not block enter_plan_mode tool', () => {
        expect(isPlanModeBlocked(true, false, false, mockConfirmationDetails('exec'), true)).toBe(false);
    });
