...
import { evaluatePermissionRules, injectPermissionRulesIfMissing, persistPermissionOutcome, buildPermissionCheckContext, } from './permission-helpers.js';
...
describe('injectPermissionRulesIfMissing', () => {
    ...
    it('injects rules with path context for file tools', () => {
        ...
        expect(details.permissionRules.some((r) => r.includes('/project/src'))).toBe(true);
    });
    it('injects rules with domain context for web_fetch', () => {
        ...
        expect(details.permissionRules).toBeDefined();
        expect(details.permissionRules.some((r) => r.includes('example.com'))).toBe(true);
    });
});
// ---------------------------------------------------------------------------
...
describe('persistPermissionOutcome', () => {
    ...
    it('persists for ProceedAlwaysProject scope', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, mockDetails, mockPersistFn, mockPm);
        expect(mockPersistFn).toHaveBeenCalledWith('project', 'allow', 'Bash(echo *)');
        expect(mockPm?.addPersistentRule).toHaveBeenCalledWith('Bash(echo *)', 'allow');
    });
    it('persists for ProceedAlwaysUser scope', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysUser, mockDetails, mockPersistFn, mockPm);
    ...
    it('persists with payload.rules as fallback', async () => {
        ...
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, emptyDetails, mockPersistFn, mockPm, mockPayload);
        // Falls back to payload.permissionRules
        expect(mockPersistFn).toHaveBeenCalledWith('project', 'allow', 'Bash(echo *)');
    });
    it('prefers details.rules over payload.rules when both present', async () => {
        const detailsWithRules = {
            type: 'exec',
            title: 'Test',
            command: 'echo hello',
            permissionRules: ['Bash(echo details)'],
        };
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, detailsWithRules, mockPersistFn, mockPm, mockPayload);
        expect(mockPersistFn).toHaveBeenCalledWith('project', 'allow', 'Bash(echo details)');
    });
    it('skips persistence for ProceedOnce (not a persist scope)', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedOnce, mockDetails, mockPersistFn, mockPm);
    ...
    it('skips persistence for Cancel outcome', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.Cancel, mockDetails, mockPersistFn, mockPm);
        expect(mockPersistFn).not.toHaveBeenCalled();
    });
    it('skips persistence when no persistFn provided', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, mockDetails, undefined, // no persistFn
        mockPm);
        // Should still update in-memory PM
        expect(mockPm?.addPersistentRule).toHaveBeenCalledWith('Bash(echo *)', 'allow');
    });
    it('skips persistence when no PM provided', async () => {
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, mockDetails, mockPersistFn, null);
        // Should still persist to disk
        expect(mockPersistFn).toHaveBeenCalledWith('project', 'allow', 'Bash(echo *)');
    });
    it('handles empty permissionRules array', async () => {
        const noRulesDetails = {
            type: 'exec',
            title: 'Test',
            command: 'echo hello',
            permissionRules: [],
        };
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, noRulesDetails, mockPersistFn, mockPm);
        expect(mockPersistFn).not.toHaveBeenCalled();
        expect(mockPm?.addPersistentRule).not.toHaveBeenCalled();
    });
    it('handles both details and payload being empty', async () => {
        ...
        await persistPermissionOutcome(ToolConfirmationOutcome.ProceedAlwaysProject, noRulesDetails, mockPersistFn, mockPm, noRulesPayload);
        expect(mockPersistFn).not.toHaveBeenCalled();
    });
    ...
    it('uses server_name as the literal specifier for read_mcp_resource', () => {
        ...
        expect(buildPermissionCheckContext('read_mcp_resource', { server_name: 'asys-mcp', uri: 'asight://x.md' }, '/project')).toMatchObject({
