...
describe('EnhancedErrorHandler', () => {
    ...
    describe('handle method', () => {
        ...
        it('logs structured API diagnostics without request contents', () => {
            const apiError = Object.assign(new Error('event:error\n:HTTP_STATUS/429\ndata:{"request_id":"req-123","code":"Throttling.AllocationQuota","message":"Allocated quota exceeded"}'), { type: 'rate_limit_error' });
            expect(() => {
                errorHandler.handle(apiError, mockContext, mockRequest);
            }).toThrow(apiError);
            expect(debugLoggerSpy.error).toHaveBeenCalledWith('OpenAI API Error:', expect.any(String), {
                durationMs: 5000,
            ...
            expect(JSON.stringify(debugLoggerSpy.error.mock.calls[0])).not.toContain('test prompt');
        });
        it('prefers top-level request ids before parsed provider details', () => {
            ...
            expect(debugLoggerSpy.error).toHaveBeenCalledWith('OpenAI API Error:', expect.any(String), expect.objectContaining({
                requestId: 'req-top-level',
                statusCode: 500,
            }));
        });
