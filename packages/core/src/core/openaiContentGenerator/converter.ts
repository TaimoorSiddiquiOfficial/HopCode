...
export class OpenAIContentConverter {
   ...
   * Concurrent streams (e.g. two subagents running in parallel) raced on
   * that shared state: each stream's stream-start `reset()` wiped the
   * other's partial tool-call buffers, chunks from different streams landed
   * at the same `index=0` bucket, and `getCompletedToolCalls()` returned
   * interleaved corrupt JSON that surfaced upstream as `NO_RESPONSE_TEXT`
   * (issue #3516). The context is stream-local and GC'd when the stream ends.
   */
   ...
   * @param chunk - The OpenAI stream chunk to convert
   * @param ctx - Per-stream context holding the tool-call parser. Callers must
   *   obtain it from `createStreamContext()` at the start of the stream so that
   *   concurrent streams each use independent parser state.
   */
  convertOpenAIChunkToGemini(
    ...
    if (choice) {
      ...
      if (choice.finish_reason) {
        ...
        // ConverterStreamContext has stream-local lifetime — no manual
        // parser cleanup needed. The context is discarded (GC'd) when the
        // stream finishes.
      }
