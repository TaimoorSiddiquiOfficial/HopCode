...
import { DEFAULT_TIMEOUT, DEFAULT_MAX_RETRIES, DISABLED_REQUEST_TIMEOUT_MS, } from '../constants.js';
...
describe('DashScopeOpenAICompatibleProvider', () => {
    ...
    describe('buildHeaders', () => {
        ...
        it('should handle unknown CLI version', () => {
            ...
            const headers = provider.buildHeaders();
            expect(headers['User-Agent']).toBe(`HopCode/unknown (${process.platform}; ${process.arch})`);
            expect(headers['X-DashScope-UserAgent']).toBe(`HopCode/unknown (${process.platform}; ${process.arch})`);
        });
    });
    ...
    describe('buildRequest', () => {
        ...
        it('sends enable_thinking:true on a qwen model when a reasoning effort is set', () => {
            ...
            const result = generator.buildRequest({ ...baseRequest }, 'test-prompt-id');
        ...
        it('strips the pipeline-injected nested reasoning when enable_thinking is added on a qwen model', () => {
            ...
            const result = generator.buildRequest(requestWithReasoning, 'test-prompt-id');
            expect(result['enable_thinking']).toBe(true);
        ...
        it('does not send enable_thinking for a non-qwen wire model even with effort set', () => {
            ...
            const result = generator.buildRequest({ ...baseRequest, model: 'glm-4.6' }, 'test-prompt-id');
            expect(result['enable_thinking']).toBeUndefined();
        });
        ...
        it('should preserve all original request parameters', () => {
            ...
            expect(result.model).toBe('hopcode-max');
        ...
        describe('glm array-drop fix (plain-text flatten)', () => {
            ...
            it.each(functionCallingCases)('should keep cache control and array content for a glm request that $name', ({ extraMessages, tools, userIndex }) => {
                const request = {
                    model: 'glm-5.2',
                    stream: false,
                    messages: [
