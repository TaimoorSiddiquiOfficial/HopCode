 ...
 * including `0` to disable) > the `HOPCODE_STREAM_IDLE_TIMEOUT_MS` env deployment
...
function resolveStreamIdleTimeoutMs(config) {
    ...
    if (typeof fromConfig === 'number') {
        ...
        console.warn(`[hopcode] Ignoring out-of-range streamIdleTimeoutMs=${fromConfig} ` +
            `(expected an integer in (-∞, ${MAX_STREAM_IDLE_TIMEOUT_MS}]); ` +
            `falling back to ${HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV}/default.`);
    }
    ...
    const raw = process.env[HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV];
    ...
    if (trimmed) {
        ...
        // eslint-disable-next-line no-console
        console.warn(`[hopcode] Ignoring invalid ${HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV}="${raw}" ` +
            ...
            const timeout = new Promise((_resolve, reject) => {
                timer = setTimeout(() => {
                    if (parentSignal?.aborted) {
                        ...
                        abortRequest();
                        reject(new StreamInactivityTimeoutError(idleMs, chunksReceived, Date.now() - streamStartedAt));
                    }
                }, idleMs);
                timer.unref?.();
            });
...
export class ContentGenerationPipeline {
    ...
    async executeStream(request, userPromptId) {
        return this.executeWithErrorHandling(request, userPromptId, true, async (openaiRequest, context) => {
            ...
            const guarded = idleMs > 0
                ? withStreamInactivityTimeout(stream, idleMs, () => perRequestAc.abort(), parentSignal)
                : stream;
            ...
            const innerStream = this.processStreamWithLogging(guarded, context, request, userPromptId);
