...
describe('OpenAIContentConverter', () => {
    ...
    function isOpenAISplitMediaMessage(message) {
        return (message.role === 'user' &&
            Array.isArray(message.content) &&
            message.content.some((part) => {
                const typedPart = part;
                return (typedPart.type === 'image_url' ||
    ...
    describe('stream-local parser state', () => {
        const streamChunk = (id, delta, finishReason = null) => ({
            id,
            created: 1,
            model: 'test',
            choices: [{ index: 0, delta, finish_reason: finishReason }],
        });
        const emitReasoning = (stream) => converter.convertOpenAIChunkToGemini(streamChunk('reasoning', { reasoning_content: 'Let me check.' }), stream);
        const emitToolCall = (stream, content, toolArguments = '{}') => converter.convertOpenAIChunkToGemini(streamChunk('tool-call', {
            content,
            tool_calls: [
                {
                    index: 0,
        ...
        const finishStream = (stream, finishReason = 'tool_calls') => converter.convertOpenAIChunkToGemini(streamChunk('finish', {}, finishReason), stream);
        ...
        it('emits no-argument tool calls that stream an empty arguments string', () => {
            ...
            const fn = result.candidates?.[0]?.content?.parts?.find((p) => p.functionCall)?.functionCall;
        ...
        it('ignores a phantom slot beside a valid tool call', () => {
            ...
            converter.convertOpenAIChunkToGemini(streamChunk('open', {
                tool_calls: [
                    {
                        index: 0,
                        id: 'call_edit',
            ...
            const result = converter.convertOpenAIChunkToGemini(streamChunk('finish', {}, 'tool_calls'), stream);
        ...
        it('rejects a tool-call finish without a completed named call', () => {
            const stream = withStreamParser();
            converter.convertOpenAIChunkToGemini(streamChunk('open', { tool_calls: [{ index: 0, function: {} }] }), stream);
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('finish', {}, 'tool_calls'), stream)).toThrowError(expect.objectContaining({ type: 'MALFORMED_TOOL_CALL' }));
        });
        it('rejects a tool call that never provides a function name', () => {
            const stream = withStreamParser();
            const partial = converter.convertOpenAIChunkToGemini(streamChunk('open', {
                content: 'discard me',
                tool_calls: [
                    {
                        index: 0,
            ...
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('finish', {}, 'stop'), stream)).toThrowError(expect.objectContaining({ type: 'MALFORMED_TOOL_CALL' }));
        });
        it('rejects a protocol-tag recovery with a whitespace-only function name', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            converter.convertOpenAIChunkToGemini(streamChunk('tool-call', {
                content: '</think>',
                tool_calls: [
                    {
                        index: 0,
            ...
            expect(() => finishStream(stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
            expect(stream.protocolTagSanitized).toBeUndefined();
        });
        it('rejects the recorded cross-channel thinking-tag leak', () => {
            const stream = withStreamParser();
            const reasoning = converter.convertOpenAIChunkToGemini(streamChunk('reasoning', {
                reasoning_content: 'Let me check<think>',
            }), stream);
            expect(reasoning.candidates?.[0]?.content?.parts).toEqual([]);
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('content', { content: 'the result\n</think>\n' }), stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
        });
        it('rejects the recorded content-only nested thinking-tag leak', () => {
            ...
            const opening = converter.convertOpenAIChunkToGemini(streamChunk('opening', { content: '<think>\n\n' }), stream);
            const repeatedOpening = converter.convertOpenAIChunkToGemini(streamChunk('repeated-opening', { content: '</think><thi' }), stream);
            expect(opening.candidates?.[0]?.content?.parts).toEqual([]);
            expect(repeatedOpening.candidates?.[0]?.content?.parts).toEqual([]);
            const nestedOpening = converter.convertOpenAIChunkToGemini(streamChunk('nested-opening', { content: 'nk>9<think>-3' }), stream);
        ...
        it.each([
            ...
            const parts = chunks.flatMap((content, index) => {
                const response = converter.convertOpenAIChunkToGemini(streamChunk(`literal-${index}`, { content }, index === chunks.length - 1 ? 'stop' : null), stream);
        ...
        it('releases a long undecided prefix before the stream finishes', () => {
            ...
            const response = converter.convertOpenAIChunkToGemini(streamChunk('literal', { content: text }), stream);
            const continuation = converter.convertOpenAIChunkToGemini(streamChunk('continuation', { content: 'literal' }, 'stop'), stream);
        ...
        it('preserves a leak-shaped literal without provider provenance', () => {
            ...
            const response = converter.convertOpenAIChunkToGemini(streamChunk('literal', { content: text }, 'stop'), stream);
            expect(response.candidates?.[0]?.content?.parts).toEqual([{ text }]);
        });
        it.each([
            [
            ...
            stream.responseParsingOptions = { contentOnlyThinkingTagLeaks: true };
            const parts = chunks.flatMap((content, index) => {
                const response = converter.convertOpenAIChunkToGemini(streamChunk(`balanced-${index}`, { content }, index === chunks.length - 1 ? 'stop' : null), stream);
                return response.candidates?.[0]?.content?.parts ?? [];
            });
            expect(parts.map((part) => part.text).join('')).toBe(chunks.join(''));
        });
        it('fails closed when a suspicious prefix exceeds the buffer limit', () => {
            ...
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('long-leak', { content }), stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
        });
        it.each([
            ...
            const stream = withStreamParser();
            stream.responseParsingOptions = { contentOnlyThinkingTagLeaks: true };
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('variant', { content }, 'stop'), stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
        });
        it('rejects closing-tag recovery after a tag leaked in reasoning', () => {
            const stream = withStreamParser();
            converter.convertOpenAIChunkToGemini(streamChunk('reasoning', {
                reasoning_content: 'Let me check<think>',
            }), stream);
        ...
        it.each([
            ['think', '\n</think>\n\n'],
            ['thinking', ' </thinking> '],
        ])('sanitizes a standalone closing %s tag with complete tool calls', (tagName, tag) => {
            const stream = withStreamParser();
        ...
        it('sanitizes a standalone closing thinking tag split across chunks', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            const firstHalf = converter.convertOpenAIChunkToGemini(streamChunk('tag-start', { content: '\n</thi' }), stream);
        ...
        it('sanitizes a standalone closing thinking tag with multiple tool calls', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            converter.convertOpenAIChunkToGemini(streamChunk('tool-calls', {
                content: '</think>',
                tool_calls: [
                    {
                        index: 0,
        ...
        it.each([
        ...
        ])('rejects a protocol-tag recovery when a new id relabels %s nameless arguments', (_state, oldArguments, newArguments) => {
            const stream = withStreamParser();
            emitReasoning(stream);
            converter.convertOpenAIChunkToGemini(streamChunk('old-arguments', {
                content: '</think>',
                tool_calls: [
                    {
                        index: 0,
            ...
            converter.convertOpenAIChunkToGemini(streamChunk('new-name', {
                tool_calls: [
                    {
                        index: 0,
                        id: 'call_new',
        ...
        it('buffers leading whitespace before a split standalone closing tag', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            const whitespace = converter.convertOpenAIChunkToGemini(streamChunk('whitespace', { content: '\n' }), stream);
        ...
        it('buffers long whitespace without treating it as a protocol tag', () => {
            ...
            emitReasoning(stream);
            const pending = converter.convertOpenAIChunkToGemini(streamChunk('whitespace', { content: whitespace }), stream);
        ...
        it('ignores an exact cumulative replay of a deferred closing tag', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            converter.convertOpenAIChunkToGemini(streamChunk('tag', { content: '</THINK>' }), stream);
            const finish = converter.convertOpenAIChunkToGemini(streamChunk('finish', {
                content: '</THINK>',
                tool_calls: [
                    {
                        index: 0,
        ...
        it('ignores cumulative replays of an incomplete closing tag', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            const first = converter.convertOpenAIChunkToGemini(streamChunk('tag-prefix', { content: '</thi' }), stream);
            const replay = converter.convertOpenAIChunkToGemini(streamChunk('tag-prefix-replay', { content: '</thi' }), stream);
            const tag = emitToolCall(stream, '</think>');
            const finish = finishStream(stream);
        ...
        it('rejects an invalid tool-call index on a stop finish', () => {
            const stream = withStreamParser();
            converter.convertOpenAIChunkToGemini(streamChunk('invalid-tool-call', {
                tool_calls: [
                    {
            ...
            expect(() => finishStream(stream, 'stop')).toThrowError(expect.objectContaining({ type: 'MALFORMED_TOOL_CALL' }));
        });
        it('rejects valid tool calls accompanied by an invalid index', () => {
            const stream = withStreamParser();
            converter.convertOpenAIChunkToGemini(streamChunk('mixed-tool-calls', {
                tool_calls: [
                    {
                        index: 0,
                        id: 'call_read',
            ...
            expect(() => finishStream(stream)).toThrowError(expect.objectContaining({ type: 'MALFORMED_TOOL_CALL' }));
        });
        it('releases a split tag-like prefix when it becomes ordinary text', () => {
            const stream = withStreamParser();
            converter.convertOpenAIChunkToGemini(streamChunk('reasoning', { reasoning_content: 'Explain the syntax.' }), stream);
            const prefix = converter.convertOpenAIChunkToGemini(streamChunk('prefix', { content: '</thi' }), stream);
            const suffix = converter.convertOpenAIChunkToGemini(streamChunk('suffix', { content: 'ng is not a tag.' }), stream);
        ...
        it('does not accumulate whitespace after a complete closing tag candidate', () => {
            ...
            for (let i = 0; i < 1_000; i++) {
                converter.convertOpenAIChunkToGemini(streamChunk(`whitespace-${i}`, { content: ' ' }), stream);
            }
        ...
        it('rejects a standalone closing thinking tag without a complete tool call', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            const leakedTag = converter.convertOpenAIChunkToGemini(streamChunk('content', { content: '</think>' }), stream);
            expect(leakedTag.candidates?.[0]?.content?.parts).toEqual([]);
            expect(() => finishStream(stream, 'stop')).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
            expect(stream.protocolTagSanitized).toBeUndefined();
        });
        it.each(['stop', 'length', 'content_filter', 'unknown'])('rejects recovery when the stream finishes with %s', (finishReason) => {
            const stream = withStreamParser();
            emitReasoning(stream);
            emitToolCall(stream, '</think>');
            expect(() => finishStream(stream, finishReason)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
            expect(stream.protocolTagSanitized).toBeUndefined();
        });
        it('rejects visible content after a deferred closing thinking tag', () => {
            const stream = withStreamParser();
            emitReasoning(stream);
            converter.convertOpenAIChunkToGemini(streamChunk('content', { content: '</think>' }), stream);
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('content-after-tag', { content: 'unexpected' }), stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
            expect(stream.protocolTagSanitized).toBeUndefined();
        });
        it.each(['{"path":', '{bad}', 'null', '[]', '42', '   '])('rejects a standalone closing thinking tag with unsafe tool arguments %s', (toolArguments) => {
            const stream = withStreamParser();
            emitReasoning(stream);
        ...
        it('rejects a closing tag split after a visible line break', () => {
            ...
            converter.convertOpenAIChunkToGemini(streamChunk('content', { content: 'the result\n' }), stream);
            converter.convertOpenAIChunkToGemini(streamChunk('blank-lines', { content: '\n'.repeat(256) }), stream);
            expect(() => converter.convertOpenAIChunkToGemini(streamChunk('closing-tag', { content: '</think>\n' }), stream)).toThrowError(expect.objectContaining({ type: 'PROTOCOL_TAG_LEAK' }));
        });
        it('preserves a split literal closing tag after ordinary reasoning', () => {
            const stream = withStreamParser();
            const reasoning = converter.convertOpenAIChunkToGemini(streamChunk('reasoning', { reasoning_content: 'Explain the syntax.' }), stream);
            const prefix = converter.convertOpenAIChunkToGemini(streamChunk('prefix', { content: 'Use ' }), stream);
            const closingTag = converter.convertOpenAIChunkToGemini(streamChunk('closing-tag', { content: '</think> to close the tag.' }), stream);
            expect(reasoning.candidates?.[0]?.content?.parts).toEqual([
        ...
        it('releases inline thinking-tag references in both channels', () => {
            ...
            const content = converter.convertOpenAIChunkToGemini(streamChunk('content', { content: 'Use </think> to close the tag.' }), stream);
            const finish = converter.convertOpenAIChunkToGemini(streamChunk('finish', {}, 'stop'), stream);
            expect(reasoning.candidates?.[0]?.content?.parts).toEqual([]);
    ...
    describe('convertGeminiRequestToOpenAI', () => {
        ...
        it('should keep embedded media as content parts when string tool content is requested but splitToolMedia is false', () => {
            ...
            expect(toolContent.find((p) => p.type === 'text')?.text).toBe('screenshot');
            expect(toolContent.find((p) => p.type === 'image_url')?.image_url?.url).toBe('data:image/png;base64,aaa');
        });
        ...
        it('should drop tool responses that are not adjacent to their assistant tool call', () => {
            ...
            const messages = converter.convertGeminiRequestToOpenAI(request, requestContext);
            const assistantWithCallA = messages.find((message) => message.role === 'assistant' &&
                'tool_calls' in message &&
                Array.isArray(message.tool_calls) &&
                message.tool_calls.some((toolCall) => toolCall.id === 'call_a'));
            expect(assistantWithCallA?.tool_calls?.map((toolCall) => toolCall.id)).toEqual(['call_a']);
            const toolCallIds = messages
                .filter((message) => message.role === 'tool' && 'tool_call_id' in message)
        ...
        it('should drop later assistant tool calls that reuse a previous surviving id', () => {
            ...
            const messages = converter.convertGeminiRequestToOpenAI(request, requestContext);
            const assistantToolCallIds = messages.flatMap((message) => hasOpenAIToolCalls(message)
        ...
        it('should drop duplicate tool call IDs within a single assistant message', () => {
            ...
            expect(assistant?.tool_calls?.[0].function.arguments).toBe(JSON.stringify({ file_path: 'a.ts' }));
        });
        it('should keep only the first adjacent tool response and its split media for a surviving id', () => {
            ...
            const toolMessages = messages.filter((message) => message.role === 'tool' && 'tool_call_id' in message);
        ...
        it('should clean after merging consecutive assistant turns', () => {
            ...
            expect(messages[0].tool_calls?.map((toolCall) => toolCall.id)).toEqual(['call_a']);
        ...
        it('should merge assistant turns created by orphan cleanup', () => {
            ...
            expect(messages
                .filter((message) => message.role === 'tool' && 'tool_call_id' in message)
                .map((message) => message.tool_call_id)).toEqual(['call_a', 'call_b']);
        });
    ...
    describe('convertOpenAIResponseToGemini', () => {
        ...
        it('keeps the estimated prompt/completion split summing to total tokens', () => {
            ...
            expect((usage?.promptTokenCount ?? 0) + (usage?.candidatesTokenCount ?? 0)).toBe(5);
        });
    });
    describe('OpenAI -> Gemini reasoning content', () => {
        it('should convert reasoning_content to a thought part for non-streaming responses', () => {
            ...
            expect(isOpenAIReasoningThoughtPart(parts?.[0])).toBe(true);
    ...
    describe('OpenAI -> Gemini tagged thinking content', () => {
        ...
        it('should flush buffered reasoning_content when tagged streaming content has no thinking tags', () => {
            ...
            expect(isOpenAIReasoningThoughtPart(finalParts?.[0])).toBe(true);
        });
    ...
    describe('mergeConsecutiveAssistantMessages', () => {
        it('should preserve reasoning_content from every merged assistant turn', () => {
            ...
            expect(messages[0].reasoning_content).toBe('First reasoning.Second reasoning.');
        });
