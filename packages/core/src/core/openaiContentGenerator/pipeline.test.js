...
import { ContentGenerationPipeline, NonSSEResponseError, StreamContentError, StreamInactivityTimeoutError, } from './pipeline.js';
import { OpenAIContentConverter } from './converter.js';
import { openaiRequestCaptureContext } from './requestCaptureContext.js';
import { StreamingToolCallParser } from './streamingToolCallParser.js';
import { AuthType } from '../contentGenerator.js';
import { DEFAULT_STREAM_IDLE_TIMEOUT_MS, MAX_STREAM_IDLE_TIMEOUT_MS, HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, } from './constants.js';
...
describe('ContentGenerationPipeline', () => {
    ...
    describe('execute', () => {
        ...
        it('emits enable_thinking:false when baseUrl is unset (DashScope default)', async () => {
            ...
            delete mockContentGeneratorConfig.baseUrl;
            mockConfig = {
                ...mockConfig,
                contentGeneratorConfig: mockContentGeneratorConfig,
            };
        ...
        it('should pass abort signal to OpenAI client when provided', async () => {
            ...
            const call = mockClient.chat.completions.create.mock.calls[0];
        ...
        it('should propagate parent abort to SDK child signal', async () => {
            ...
            let capturedSignal;
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockClient.chat.completions.create.mockImplementation((_req, opts) => {
    ...
    describe('executeStream', () => {
        ...
        it('rejects an unresolved thinking-tag candidate at clean stream EOF', async () => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockConverter.convertOpenAIChunkToGemini.mockImplementation((_chunk, context) => {
        ...
        it('logs only the accepted finish after duplicate and empty trailing chunks', async () => {
            ...
            const chunks = ['finish-1', 'finish-2', 'trailing-empty'].map((id) => ({
                id,
            ...
            const makeFinishResponse = (responseId, callId) => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockConverter.convertOpenAIChunkToGemini.mockImplementation((chunk, context) => {
            ...
            expect(logProtocolTagSanitized).toHaveBeenCalledWith(mockCliConfig, expect.objectContaining({
        ...
        it('does not attribute sanitization from a discarded duplicate finish', async () => {
            ...
            const chunks = ['finish-1', 'finish-2', 'usage'].map((id) => ({
                id,
                choices: [{ delta: {}, finish_reason: null }],
            }));
            const mockStream = {
            ...
            const makeFinishResponse = (responseId) => {
                const response = new GenerateContentResponse();
                response.responseId = responseId;
                response.candidates = [
                    {
        ...
        it('rejects visible content after a sanitized finish', async () => {
            ...
            const chunks = ['finish', 'trailing-content'].map((id) => ({
                id,
                choices: [{ delta: {}, finish_reason: null }],
            }));
            const mockStream = {
        ...
        it.each(['transport error', 'explicit abort'])('handles a pending closing tag on %s', async (termination) => {
            const abortController = new AbortController();
            const streamError = new Error(termination === 'explicit abort' ? 'Aborted' : 'socket reset');
        ...
        it('should preserve an otherwise empty response with tool preparation metadata', async () => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockConverter.convertOpenAIChunkToGemini.mockReturnValue(preparationResponse);
            mockClient.chat.completions.create.mockResolvedValue(mockStream);
            const resultGenerator = await pipeline.executeStream(request, 'test-prompt-id');
            const results = [];
            for await (const result of resultGenerator) {
        ...
        it('should throw NonSSEResponseError when response has non-SSE content-type', async () => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockClient.chat.completions.create.mockReturnValue(mockApiPromise);
            let thrownError;
            try {
        ...
        it('should abort child signal after stream is fully consumed', async () => {
            ...
            mockClient.chat.completions.create.mockResolvedValue(mockStream);
            const resultGenerator = await pipeline.executeStream(request, 'test-id');
            const sdkSignal = mockClient.chat.completions.create.mock
                .calls[0][1]?.signal;
        ...
        it('should abort child signal when SDK create() throws', async () => {
            ...
            await expect(pipeline.executeStream(request, 'test-id')).rejects.toThrow();
            expect(capturedSignal.aborted).toBe(true);
        });
    ...
    describe('stream inactivity timeout', () => {
        ...
        function gatedStream() {
            ...
            return {
                push(chunk) {
                    if (resolveNext)
                        deliver({ done: false, value: chunk });
                    else
                        buffered.push(chunk);
                },
                error(error) {
        ...
        function streamingRequest(signal) {
            return {
                model: 'test-model',
                contents: [{ parts: [{ text: 'Hi' }], role: 'user' }],
        ...
        function buildPipeline(streamIdleTimeoutMs) {
            mockContentGeneratorConfig = {
                ...mockContentGeneratorConfig,
        ...
        beforeEach(() => {
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockConverter.convertOpenAIChunkToGemini.mockImplementation(() => {
            ...
            // Clean baseline: ignore any ambient HOPCODE_STREAM_IDLE_TIMEOUT_MS from the
            ...
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, undefined);
            vi.useFakeTimers();
        });
        afterEach(() => {
            vi.useRealTimers();
        ...
        it('aborts and throws ETIMEDOUT when the stream is silent past the idle timeout', async () => {
            ...
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
            const p = buildPipeline(1000);
            const gen = await p.executeStream(streamingRequest(new AbortController().signal), 'id');
            const consume = (async () => {
                for await (const _ of gen) {
                    /* drain */
                }
            })();
            const captured = consume.catch((e) => e);
            await vi.advanceTimersByTimeAsync(1000);
            const err = await captured;
            expect(err).toBeInstanceOf(StreamInactivityTimeoutError);
            expect(err.message).toBe('No stream activity for 1000ms after 0 chunks ' +
            ...
            expect(err.chunksReceived).toBe(0);
            expect(err.streamLifetimeMs).toBe(1000);
        ...
        it('swallows the orphaned SDK next() rejection after an idle timeout', async () => {
            ...
            mockClient.chat.completions.create.mockResolvedValue(stream);
            const unhandled = [];
        ...
        it('propagates mid-stream errors without converting them to ETIMEDOUT', async () => {
            ...
            expect(results).toHaveLength(1);
            expect(mockErrorHandler.handle).toHaveBeenCalledWith(networkError, expect.anything(), expect.anything());
            await vi.advanceTimersByTimeAsync(5000);
        });
        ...
        it('honors HOPCODE_STREAM_IDLE_TIMEOUT_MS when no explicit config is set', async () => {
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '3000');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('lets an explicit streamIdleTimeoutMs config take precedence over the env', async () => {
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '1000');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('ignores a malformed HOPCODE_STREAM_IDLE_TIMEOUT_MS and falls back to the default', async () => {
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, 'not-a-number');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('ignores an oversized HOPCODE_STREAM_IDLE_TIMEOUT_MS (beyond the timer ceiling)', async () => {
            ...
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '9999999999');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('rejects a non-decimal HOPCODE_STREAM_IDLE_TIMEOUT_MS (hex/scientific) and uses the default', async () => {
            // Number('0x10') === 16; a strict decimal-integer check must reject it so
            // a typo can't silently become a 16ms timeout.
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '0x10');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('falls back from an invalid config to the env value (config→env cascade)', async () => {
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '4000');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
        ...
        it('disables the watchdog when HOPCODE_STREAM_IDLE_TIMEOUT_MS=0', async () => {
            vi.stubEnv(HOPCODE_STREAM_IDLE_TIMEOUT_MS_ENV, '0');
            const gated = gatedStream(); // silent
            mockClient.chat.completions.create.mockResolvedValue(gated.stream);
