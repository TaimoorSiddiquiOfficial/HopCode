...
describe('hasUserVisibleContent', () => {
    ...
    it('returns true for functionCall part', () => {
        expect(hasUserVisibleContent(chunkWithParts([{ functionCall: { name: 'read', args: {} } }]))).toBe(true);
    });
    it('returns true for inlineData part', () => {
        expect(hasUserVisibleContent(chunkWithParts([
    ...
    it('returns true for thought / reasoning part with thought: true', () => {
        expect(hasUserVisibleContent(chunkWithParts([{ thought: true }]))).toBe(true);
    });
    it('returns false for thought: false (explicit non-thought part)', () => {
        ...
        expect(hasUserVisibleContent(chunkWithParts([{ thought: false }]))).toBe(false);
    });
    ...
    it('returns true when thought: true coexists with empty text', () => {
        ...
        expect(hasUserVisibleContent(chunkWithParts([{ text: '', thought: true }]))).toBe(true);
    });
    it('returns true when any part is user-visible (mixed)', () => {
        expect(hasUserVisibleContent(chunkWithParts([{ text: '' }, { text: 'hi' }]))).toBe(true);
    });
    ...
    it('handles parts that are non-objects defensively', () => {
        expect(hasUserVisibleContent(chunkWithParts([null, undefined, 'string', 42, { text: 'real' }]))).toBe(true);
        expect(hasUserVisibleContent(chunkWithParts([null, undefined, 'x']))).toBe(false);
    });
});
//# sourceMappingURL=streamContentDetection.test.js.map