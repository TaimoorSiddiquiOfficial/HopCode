...
import { startLLMRequestSpan, endLLMRequestSpan, addSystemPromptAttributes, addToolSchemaAttributes, addModelOutputAttributes, areSensitiveSpanAttributesEnabled, } from '../../telemetry/index.js';
...
export class LoggingContentGenerator {
    ...
    async generateContent(req, userPromptId) {
        ...
        try {
            ...
            const response = await context.with(spanContext, async () => {
                ...
                const shouldCollectSensitiveSpanAttributes = !isInternal && this.shouldCollectSensitiveSpanAttributes();
                const modelOutput = shouldCollectSensitiveSpanAttributes
                    ? this.extractResponseTextForSensitiveSpan(result, this.config.getTelemetrySensitiveSpanAttributeMaxLength())
                    : undefined;
    ...
    async generateContentStream(req, userPromptId) {
        ...
        return context.with(spanContext, () => this.loggingStreamWrapper(stream, startTime, userPromptId, req.model, resolvedRequest, llmSpan, spanContext, req.config?.abortSignal, retrySnapshot));
    }
    startCaptureSession() {
        let captured;
    ...
    retrySnapshot) {
        ...
        try {
            ...
            const shouldCollectSensitiveSpanAttributes = !isInternal &&
                span !== undefined &&
                this.shouldCollectSensitiveSpanAttributes();
            const streamModelOutput = shouldCollectSensitiveSpanAttributes
                ? this.extractResponseTextForSensitiveSpan(consolidatedResponse, this.config.getTelemetrySensitiveSpanAttributeMaxLength())
                : undefined;
    ...
    extractResponseTextForSensitiveSpan(response, maxLength) {
        let text = '';
    ...
    safelyAddModelOutputAttributes(span, modelOutput) {
        try {
            addModelOutputAttributes(this.config, span, modelOutput?.text, modelOutput?.originalLength);
        }
        catch (error) {
