...
import { addModelOutputAttributes, isTelemetrySdkInitialized, } from '../../telemetry/index.js';
...
vi.mock('../../telemetry/index.js', () => {
    ...
    return {
        startLLMRequestSpan: vi.fn((model, promptId) => createSpan('hopcode.llm_request', {
            model,
            prompt_id: promptId,
        })),
        ...
        areSensitiveSpanAttributesEnabled: vi.fn((config) => isTelemetrySdkInitialized() &&
...
const createConfig = (overrides = {}) => {
    ...
    return {
        ...
        getTelemetryIncludeSensitiveSpanAttributes: () => Boolean(configContent['includeSensitiveSpanAttributes']),
        getTelemetrySensitiveSpanAttributeMaxLength: () => configContent['sensitiveSpanAttributeMaxLength'] ??
            1024 * 1024,
    };
};
...
const getStreamSpanRecord = () => {
    const spanRecord = loggingSpanRecords.find((record) => record.name === 'hopcode.llm_request');
    if (!spanRecord) {
...
describe('LoggingContentGenerator', () => {
    ...
    it('captures ttftMs on the first user-visible stream chunk (Phase 4a)', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-ttft');
        for await (const _ of stream) {
            // consume
        }
        const spanRecord = getStreamSpanRecord();
        const meta = spanRecord.endMetadata;
    ...
    it('forwards cachedInputTokens from usageMetadata to endLLMRequestSpan (Phase 4a)', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-cache');
        for await (const _ of stream) {
            // consume
        }
        const spanRecord = getStreamSpanRecord();
    ...
    it('leaves ttftMs undefined when stream yields no user-visible chunks (Phase 4a)', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-no-content');
        for await (const _ of stream) {
            // consume
        }
        const spanRecord = getStreamSpanRecord();
    ...
    it('forwards cachedInputTokens to endLLMRequestSpan on non-stream success (Phase 4a)', async () => {
        const generateFn = vi.fn().mockResolvedValue(createResponse('resp-cache', 'test-model', [{ text: 'ok' }], {
            promptTokenCount: 100,
    ...
    it('passes uncapped response text to sensitive model output attributes', async () => {
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-long', 'test-model', [{ text: longText }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig({ includeSensitiveSpanAttributes: true }), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
        });
        ...
        await generator.generateContent(request, 'prompt-long');
        expect(vi.mocked(addModelOutputAttributes).mock.calls.at(-1)?.[2]).toBe(longText);
        const [, responseEvent] = vi.mocked(logApiResponse).mock.calls[0];
        expect(responseEvent.response_text).toHaveLength(MAX_RESPONSE_TEXT_LENGTH);
        expect(responseEvent.response_text).toBe(`${longText.slice(0, MAX_RESPONSE_TEXT_LENGTH - RESPONSE_TEXT_TRUNCATION_SUFFIX.length)}${RESPONSE_TEXT_TRUNCATION_SUFFIX}`);
    });
    it('excludes thought parts from sensitive model output attributes', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-thought', 'test-model', [
            { text: 'visible' },
    ...
    it('accumulates sensitive model output length across multiple parts', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-multipart', 'test-model', [
            { text: 'a'.repeat(100) },
            { text: 'b'.repeat(100) },
        ])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig({
    ...
    it('does not fail generateContent when sensitive span output attributes fail', async () => {
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-safe', 'test-model', [{ text: 'visible' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig({ includeSensitiveSpanAttributes: true }), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
    ...
    it('skips sensitive model output attributes when the sensitive flag is disabled', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-safe', 'test-model', [{ text: 'secret' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
    ...
    it('passes uncapped stream response text to sensitive model output attributes', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-long');
        for await (const _ of stream) {
            // consume
        }
        expect(vi.mocked(addModelOutputAttributes).mock.calls.at(-1)?.[2]).toBe(longText);
    ...
    it('does not fail generateContentStream when sensitive span output attributes fail', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-safe');
        const responses = [];
    ...
    it('logs streaming responses and consolidates tool calls', async () => {
        ...
        const usageResponse = createResponse('resp-usage', 'model-stream', [], usage2);
    ...
    it('does not retain every metadata-only streaming response for logging', async () => {
        ...
        const consolidateSpy = vi.spyOn(generator, 'consolidateGeminiResponsesForLogging');
    ...
    it('uses string tool result content in reconstructed OpenAI logs when configured', async () => {
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-tool-log', 'test-model', [{ text: 'ok' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: true,
        ...
        const [openaiRequest] = openaiLoggerInstance.logInteraction.mock
            .calls[0];
        const toolMessage = openaiRequest.messages.find((message) => message.role === 'tool');
...
describe('LoggingContentGenerator — Phase 4b retry context propagation', () => {
    ...
    it('non-stream: forwards retryContext.attempt/requestSetupMs/retryTotalDelayMs to endLLMRequestSpan', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn().mockResolvedValue(createResponse('r-1', 'test-model', [{ text: 'ok' }], {
            promptTokenCount: 10,
            candidatesTokenCount: 5,
            totalTokenCount: 15,
        })), vi.fn());
        ...
        await retryContext.run({ attempt: 3, requestSetupMs: 1200, retryTotalDelayMs: 1000 }, async () => {
    ...
    it('non-stream: defaults attempt=1 and leaves setup/delay undefined when no retry context (direct call / warmup)', async () => {
        const wrapped = createWrappedGenerator(vi.fn().mockResolvedValue(createResponse('r-2', 'test-model', [{ text: 'ok' }], {
            promptTokenCount: 10,
            candidatesTokenCount: 5,
            totalTokenCount: 15,
        })), vi.fn());
    ...
    it('stream: snapshots retry context in synchronous prelude and forwards through stream wrapper finally block', async () => {
        ...
        await retryContext.run({ attempt: 2, requestSetupMs: 500, retryTotalDelayMs: 400 }, async () => {
            const stream = await generator.generateContentStream(request, 'prompt-retry-stream');
            for await (const _ of stream) {
                // consume
            }
        });
    ...
    it('stream idle-timeout path: retrySnapshot propagates to the setTimeout-fired endLLMRequestSpan (R2 #8)', async () => {
        ...
        let releaseStream;
        const streamBlocker = new Promise((r) => {
            releaseStream = r;
        });
        ...
        await retryContext.run({ attempt: 4, requestSetupMs: 3000, retryTotalDelayMs: 2500 }, async () => {
            const stream = await generator.generateContentStream(request, 'prompt-idle-timeout');
        ...
        const records = loggingSpanRecords.filter((r) => r.name === 'hopcode.llm_request' && r.endMetadata !== undefined);
        const timeoutRecord = records.find((r) => r.endMetadata?.error === 'Stream span timed out (idle)');
