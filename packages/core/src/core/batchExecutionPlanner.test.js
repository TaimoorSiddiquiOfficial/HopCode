...
import { isConcurrencySafe, partitionToolCalls, BatchExecutionPlanner, } from './batchExecutionPlanner.js';
import { ToolNames } from '../tools/tool-names.js';
import { Kind, BaseDeclarativeTool } from '../index.js';
function makeScheduledCall(overrides = {}) {
    const tool = overrides.tool ??
        new BaseDeclarativeTool('mock_tool', 'mock', 'A mock tool', Kind.Read, {}, async () => { });
    return {
        status: 'scheduled',
        request: {
...
describe('isConcurrencySafe', () => {
    ...
    it('returns true for read-kind tools', () => {
        const tool = new BaseDeclarativeTool('read_file', 'read', 'Reads a file', Kind.Read, {}, async () => { });
        const call = makeScheduledCall({ tool });
        expect(isConcurrencySafe(call)).toBe(true);
    });
    it('returns false for edit-kind tools', () => {
        const tool = new BaseDeclarativeTool('edit', 'edit', 'Edits a file', Kind.Edit, {}, async () => { });
        const call = makeScheduledCall({ tool });
        expect(isConcurrencySafe(call)).toBe(false);
    });
    it('returns false for execute-kind tools with no command', () => {
        const tool = new BaseDeclarativeTool('run_shell_command', 'execute', 'Runs a command', Kind.Execute, {}, async () => { });
...
describe('partitionToolCalls', () => {
    it('groups consecutive safe tools into one batch', () => {
        const readTool = new BaseDeclarativeTool('read_file', 'read', '', Kind.Read, {}, async () => { });
        const calls = [
    ...
    it('splits unsafe tools into individual sequential batches', () => {
        const editTool = new BaseDeclarativeTool('edit', 'edit', '', Kind.Edit, {}, async () => { });
        const calls = [
    ...
    it('mixes safe and unsafe: Read,Read,Edit,Read', () => {
        const readTool = new BaseDeclarativeTool('read', 'read', '', Kind.Read, {}, async () => { });
        const editTool = new BaseDeclarativeTool('edit', 'edit', '', Kind.Edit, {}, async () => { });
        const calls = [
...
//# sourceMappingURL=batchExecutionPlanner.test.js.map