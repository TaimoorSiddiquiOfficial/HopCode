...
import { QURAN_GUIDED_AGENT_PROMPT, getQuranGuidedBehavior, } from '@hoptrendy/quran-guidance';
...
export function getCoreSystemPrompt(userMemory, model, appendInstruction, interactionMode = 'interactive') {
    ...
    const basePrompt = systemMdEnabled
        ...
        : `
You are HopCode, ${interaction.role} developed by Alibaba Group, specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
...
`.trim();
    // if HOPCODE_WRITE_SYSTEM_MD is set (and not 0|false), write base system prompt to file
    const writeSystemMdResolution = resolvePathFromEnv(process.env['HOPCODE_WRITE_SYSTEM_MD']);
...
function getToolCallExamples(model) {
    ...
    if (toolCallStyle) {
        switch (toolCallStyle.toLowerCase()) {
            ...
            default:
                debugLogger.warn(`Unknown HOPCODE_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`);
                break;
        }
    }
...
const INSIGHT_PROMPTS = {
    analysis: `Analyze this HopCode session and extract structured facets.

CRITICAL GUIDELINES:

1. **goal_categories**: Count ONLY what the USER explicitly asked for.
   - DO NOT count HopCode's autonomous codebase exploration
   - DO NOT count work HopCode decided to do on its own
...
4. If very short or just warmup, use warmup_minimal for goal_category`,
    impressive_workflows: `Analyze this HopCode usage data and identify what's working well for this user. Use second person ("you").
...
Include 3 impressive workflows.`,
    project_areas: `Analyze this HopCode usage data and identify project areas.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "areas": [
    {"name": "Area name", "session_count": N, "description": "2-3 sentences about what was worked on and how HopCode was used."}
  ]
}

Include 4-5 areas. Skip internal QC operations.`,
    future_opportunities: `Analyze this HopCode usage data and identify future opportunities.
...
Include 3 opportunities. Think BIG - autonomous workflows, parallel agents, iterating against tests.`,
    friction_points: `Analyze this HopCode usage data and identify friction points for this user. Use second person ("you").
...
Include 3 friction categories with 2 examples each.`,
    memorable_moment: `Analyze this HopCode usage data and find a memorable moment.
...
Find something genuinely interesting or amusing from the session summaries.`,
    improvements: `Analyze this HopCode usage data and suggest improvements.

## QC FEATURES REFERENCE (pick from these for features_to_try):
1. **MCP Servers**: Connect HopCode to external tools, databases, and APIs via Model Context Protocol.
   - How to use: Run \`hopcode mcp add --transport http <server-name> <http-url>\`
   - Good for: database queries, Slack integration, GitHub issue lookup, connecting to internal APIs
   - Example: "To connect to GitHub, run \`hopcode mcp add --header "Authorization: Bearer your_github_mcp_pat" --transport http github https://api.githubcopilot.com/mcp/\` and set the AUTHORIZATION header with your PAT. Then you can ask HopCode to query issues, PRs, or repos."
...
IMPORTANT for features_to_try: Pick 2-3 from the QC FEATURES REFERENCE above. Include 2-3 items for each category.`,
    interaction_style: `Analyze this HopCode usage data and describe the user's interaction style.
...
`,
    at_a_glance: `You're writing an "At a Glance" summary for a HopCode usage insights report for HopCode users. The goal is to help them understand their usage and improve how they can use HopCode better, especially as models improve.

Use this 4-part structure:

1. **What's working** - What is the user's unique style of interacting with HopCode and what are some impactful things they've done? You can include one or two details, but keep it high level since things might not be fresh in the user's memory. Don't be fluffy or overly complimentary. Also, don't focus on the tool calls they use.
 ...
 */
export function getQuranGuidanceSection() {
 ...
 */
export function getQuranGuidancePerTurnReminder(userMessage, agentContext, taskType) {
    try {
