...
import { MessageDisplayDispatcher, MESSAGE_DISPLAY_DRAIN_TIMEOUT_MS, } from './message-display-dispatcher.js';
...
function createControlledBus() {
    ...
    const releases = [];
    const request = vi.fn((message) => new Promise((resolve) => {
...
function createDispatcher(bus, opts = {}) {
    return new MessageDisplayDispatcher(bus, opts.signal ?? new AbortController().signal, opts.warn ?? (() => { }), 0);
}
...
describe('MessageDisplayDispatcher', () => {
    ...
    it('logs a failed delivery with the message_id and still delivers the final flush', async () => {
        ...
        await Promise.resolve();
        await dispatcher.finish();
        expect(warn).toHaveBeenCalledWith(`MessageDisplay hook failed [${dispatcher.messageId}]: Error: hook process failed`);
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(`MessageDisplay hook failed [${dispatcher.messageId}]: Error: hook process failed`);
        expect(sent).toHaveLength(2);
    ...
    it('gives up waiting on drain after the timeout and warns, while delivery keeps running in the background', async () => {
        ...
        await finished;
        expect(finishResolved).toBe(true);
        expect(warn).toHaveBeenCalledWith(expect.stringContaining(`MessageDisplay hook [${dispatcher.messageId}] still running after ${MESSAGE_DISPLAY_DRAIN_TIMEOUT_MS}ms`));
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining(`MessageDisplay hook [${dispatcher.messageId}] still running after ${MESSAGE_DISPLAY_DRAIN_TIMEOUT_MS}ms`));
    });
...
//# sourceMappingURL=message-display-dispatcher.test.js.map