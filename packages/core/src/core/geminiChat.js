...
import { resolveCompactionTuning, resolveSlimmingConfig, slimCompactionInput, } from '../services/compactionInputSlimming.js';
...
import { collectToolCallIdsFromHistory, normalizeModelToolCallIds, reserveModelToolCallId, } from './toolCallIdUtils.js';
...
function isToolCallPreparationOnly(response) {
    if (getToolCallPreparations(response).length === 0)
        return false;
    const hasCandidateOutput = response.candidates?.some((candidate) => Boolean(candidate.finishReason) ||
 ...
 */
const SESSION_START_CONTEXT_SENTINEL_START = '<hopcode:session-start-context hidden="true">';
...
export class GeminiChat {
    ...
    getRequestHistory(currentUserContent) {
        ...
        if (countAllInlineImages(curatedHistory) >= imagePayloadThreshold) {
            const skipEntry = currentUserContent
                ...
                : undefined;
            const replaced = replaceImagePayloadsInPlace(curatedHistory, this.imagePayloadStore, skipEntry);
     ...
     */
    seedResumeTokenCounts(promptTokenCount, outputTokenCount) {
    ...
    async sendMessageStream(model, params, prompt_id) {
        ...
        const parsedEnvMaxTokensForClamp = parsePositiveIntegerEnvValue(process.env['HOPCODE_CODE_MAX_OUTPUT_TOKENS']);
        ...
        try {
            ...
            if (exactRoute || (isHardTier && !shouldForceFromHard)) {
                ...
                compressionInfo = await this.tryCompress(prompt_id, model, shouldForceFromHard, params.config?.abortSignal, {
        ...
        return (async function* () {
            const sleepInhibitorHandle = acquireSleepInhibitor(self.config, 'HopCode is streaming a model response');
            try {
                ...
                const parsedEnvMaxTokens = parsePositiveIntegerEnvValue(process.env['HOPCODE_CODE_MAX_OUTPUT_TOKENS']);
                ...
                for (;;) {
                    ...
                    try {
                        ...
                        const stream = await self.makeApiCallAndProcessStream(model, requestContents, params, prompt_id, requestOverrides);
                ...
                const streamWithInvalidStreamRetries = async function* (buildAttempt, retryEvent = {
                    ...
                    for (;;) {
                        ...
                        try {
                            const stream = await self.makeApiCallAndProcessStream(model, attemptState.requestContents, attemptState.params, prompt_id, requestOverrides);
                            for await (const chunk of stream) {
                                yield { type: StreamEventType.CHUNK, value: chunk };
                            }
                            return;
                ...
                if (lastError === null &&
                    ...
                    while (recoveryFinishReason === FinishReason.MAX_TOKENS &&
                        ...
                        self.history.pop();
                        const walkRecoveryEstimate = estimateContentTokens(recoveryContents, recoveryImageTokenEstimate) + FIRST_SEND_CLAMP_OVERHEAD_PAD;
                        ...
                        if (isFallbackEligible(currentErrorClassification)) {
                            ...
                            for (const fallbackModelId of fallbackModels) {
                                ...
                                try {
                                    ...
                                    for await (const event of self.makeFallbackStream(resolvedFallbackModel, fallbackRequestContents, params, prompt_id, fallbackGenerator, fallbackRetryAuthType, fallbackRetryErrorCodes)) {
     ...
     * the retry wiring.
     */
    async makeApiCallAndProcessStream(model, requestContents, params, prompt_id, overrides) {
    ...
    async *makeFallbackStream(model, requestContents, params, prompt_id, contentGenerator, retryAuthType, retryErrorCodes) {
        const stream = await this.makeApiCallAndProcessStream(model, requestContents, params, prompt_id, { contentGenerator, retryAuthType, retryErrorCodes });
        for await (const chunk of stream) {
            yield { type: StreamEventType.CHUNK, value: chunk };
        }
    }
