...
import { generateToolUseId, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, appendAdditionalContext, } from './toolHookTriggers.js';
import { convertToFunctionResponse, extractToolFilePaths, } from './coreToolScheduler.js';
// ── Helpers moved from coreToolScheduler.ts ──────────────────────────
function createErrorResponse(request, error, errorType) {
    return {
...
export class ToolExecutionHandler {
    ...
    async execute(toolCall, signal) {
        ...
        const toolInput = toolCall.request.args;
        ...
        if (hooksEnabled && messageBus) {
            ...
            if (!preHookResult.shouldProceed) {
                ...
                const errorResponse = createErrorResponse(toolCall.request, new Error(blockMessage), ToolErrorType.EXECUTION_DENIED);
        ...
        const liveOutputCallback = toolCall.tool.canUpdateOutput
            ? (outputChunk) => {
                ...
                this.deps.updateToolCalls((calls) => calls.map((tc) => tc.request.callId === callId && tc.status === 'executing'
        ...
        try {
            ...
            if (signal.aborted) {
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, 'User cancelled tool execution.', true, this.deps.getApprovalMode());
                    ...
                    this.deps.onStatus(callId, 'cancelled', 'User cancelled tool execution.');
                }
                return;
            }
            if (toolResult.error === undefined) {
                ...
                if (hooksEnabled && messageBus) {
                    ...
                    if (postHookResult.shouldStop) {
                        ...
                        const errorResponse = createErrorResponse(toolCall.request, new Error(stopMessage), ToolErrorType.EXECUTION_DENIED);
                        this.deps.onStatus(callId, 'error', errorResponse);
                        return;
                    }
                }
                ...
                if (candidatePaths.length > 0) {
                    ...
                    if (activatedSkills && activatedSkills.length > 0) {
                        const hasSkillTool = !!this.deps.toolRegistry.getTool(ToolNames.SKILL);
                        if (hasSkillTool) {
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, toolResult.error.message, false, this.deps.getApprovalMode());
                    if (failureHookResult.additionalContext) {
                        errorMessage += `\n\n${failureHookResult.additionalContext}`;
                    }
                }
                const error = new Error(errorMessage);
                const errorResponse = createErrorResponse(toolCall.request, error, toolResult.error.type);
                this.deps.onStatus(callId, 'error', errorResponse);
            }
        }
            ...
            if (signal.aborted) {
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, false, this.deps.getApprovalMode());
                    if (failureHookResult.additionalContext) {
                        exceptionErrorMessage += `\n\n${failureHookResult.additionalContext}`;
                    }
                }
                this.deps.onStatus(callId, 'error', createErrorResponse(toolCall.request, executionError instanceof Error
...
//# sourceMappingURL=toolExecutionHandler.js.map