...
import { ApprovalMode, BaseDeclarativeTool, BaseToolInvocation, Kind, ToolConfirmationOutcome, DEFAULT_TRUNCATE_TOOL_OUTPUT_LINES, DEFAULT_TRUNCATE_TOOL_OUTPUT_THRESHOLD, MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS, ToolErrorType, } from '../index.js';
import * as path from 'node:path';
import { pathToFileURL } from 'node:url';
import { SkillTool } from '../tools/skill.js';
import { StructuredToolError } from '../tools/priorReadEnforcement.js';
import { ToolNames, ToolNamesMigration } from '../tools/tool-names.js';
import { CoreToolScheduler, convertToFunctionErrorResponse, convertToFunctionResponse, extractToolFilePaths, isToolCallConcurrencySafe, } from './coreToolScheduler.js';
...
import { getRuntimeContentGenerator, runWithAgentContext, } from '../agents/runtime/agent-context.js';
...
describe('CoreToolScheduler', () => {
    ...
    it('does not reset total denial counters for unrelated AUTO approvals', async () => {
        const { internals, toolCall, setAutoModeDenialState } = createSchedulerForDenialTrackingApprovalTest();
        await internals._handleConfirmationResponseInner('call-1', toolCall, vi.fn().mockResolvedValue(undefined), ToolConfirmationOutcome.ProceedOnce, new AbortController().signal);
    ...
    it('does not reset denial counters after cancelling a denialTracking fallback prompt', async () => {
        const { internals, toolCall, setAutoModeDenialState } = createSchedulerForDenialTrackingApprovalTest();
        internals.autoModeFallbackCallIds.add('call-1');
        await internals._handleConfirmationResponseInner('call-1', toolCall, vi.fn().mockResolvedValue(undefined), ToolConfirmationOutcome.Cancel, new AbortController().signal);
        expect(setAutoModeDenialState).not.toHaveBeenCalled();
    });
    ...
    function createSchedulerForLegacyToolTests(options) {
        ...
        const scheduler = new CoreToolScheduler({
            config: {
                ...
                getToolOutputBatchBudget: () => options.toolOutputBatchBudget ?? Number.POSITIVE_INFINITY,
                ...
                getAutoModeDenialState: () => options.autoModeDenialState ?? {
                    consecutiveBlock: 0,
                    consecutiveUnavailable: 0,
                    totalBlock: 0,
                    totalUnavailable: 0,
    ...
    it('keeps interaction-required tools awaiting approval despite YOLO and an allow hook', async () => {
        ...
        const messageBus = {
            request: vi.fn().mockImplementation(async (request) => ({
                ...
                success: true,
                output: request.eventName === 'PermissionRequest'
                    ? {
                        hookSpecificOutput: {
                            decision: {
                                behavior: 'allow',
        ...
        const waiting = onToolCallsUpdate.mock.calls
            .flatMap((call) => call[0])
            .find((call) => call.status === 'awaiting_approval');
        ...
        await waiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce);
    ...
    it('does not let AUTO_EDIT approve an interaction-required info tool', async () => {
        ...
        expect(onToolCallsUpdate.mock.calls
            .flatMap((call) => call[0])
            .some((call) => call.status === 'awaiting_approval')).toBe(true);
        expect(execute).not.toHaveBeenCalled();
    });
    it('does not auto-approve an interaction-required sibling after ProceedAlways', async () => {
        ...
        const siblingPermission = vi.fn(async () => siblingWouldOtherwiseAllow ? 'allow' : 'ask');
        ...
        const firstWaiting = onToolCallsUpdate.mock.calls
            .flatMap((call) => call[0])
            .find((call) => call.request.callId === 'ordinary-confirmation' &&
            call.status === 'awaiting_approval');
        await firstWaiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedAlways);
        expect(siblingWouldOtherwiseAllow).toBe(true);
        expect(siblingPermission).toHaveBeenCalledTimes(2);
        const latestCalls = onToolCallsUpdate.mock.calls.at(-1)?.[0];
        expect(latestCalls.find((call) => call.request.callId === 'interaction-required-sibling')?.status).toBe('awaiting_approval');
    ...
    it('aborts and fails a tool call that exceeds the execution timeout', async () => {
        ...
        try {
            ...
            const execute = vi.fn((_params, signal) => new Promise((resolve) => {
            ...
            const { scheduler, onAllToolCallsComplete } = createSchedulerForLegacyToolTests({ toolsByName });
            await scheduler.schedule([
                {
                    callId: 'timeout-1',
                    name: 'read_file',
            ...
            if (completedCall.status === 'error') {
                expect(completedCall.response.errorType).toBe(ToolErrorType.EXECUTION_TIMEOUT);
    ...
    it('keeps a tool-produced timeout as an error after a later parent abort', async () => {
        ...
        const messageBus = {
            request: vi
                .fn()
                .mockImplementation(async (request) => request.eventName === 'PostToolUseFailure'
                ? {
                    type: MessageBusType.HOOK_EXECUTION_RESPONSE,
        ...
        if (completedCall.status === 'error') {
            ...
            expect(completedCall.response.responseParts[0].functionResponse?.response).toEqual({
                error: 'Command timed out.\npartial output\n\ninspect the partial output',
            });
            expect(completedCall.response.responseParts[0].functionResponse?.response).not.toHaveProperty('output');
        }
    ...
    it('forwards a parent signal abort to the timeout controller', async () => {
        ...
        try {
            const parentController = new AbortController();
            let toolSawAbort = false;
            const execute = vi.fn((_params, signal) => new Promise((_resolve, reject) => {
                signal?.addEventListener('abort', () => {
                    toolSawAbort = true;
    ...
    it('propagates a tool rejection even when timeout is active', async () => {
        ...
        try {
            ...
            if (completedCall.status === 'error') {
                ...
                expect(completedCall.response.errorType).not.toBe(ToolErrorType.EXECUTION_TIMEOUT);
    ...
    it('executes only the first request for duplicate callIds in one batch', async () => {
        ...
        expect(execute).toHaveBeenCalledWith(expect.objectContaining({ file_path: 'a.ts' }));
        const completedCalls = onAllToolCallsComplete.mock
            .calls[0][0];
    ...
    it('does not dedupe requests with empty callIds in one batch', async () => {
        ...
        expect(execute).toHaveBeenNthCalledWith(1, expect.objectContaining({ file_path: 'a.ts' }));
        expect(execute).toHaveBeenNthCalledWith(2, expect.objectContaining({ file_path: 'b.ts' }));
        const completedCalls = onAllToolCallsComplete.mock
            .calls[0][0];
        expect(completedCalls).toHaveLength(2);
    });
    function outputOfFirstCall(onAllToolCallsComplete) {
    ...
    it('truncates oversized model-facing string output before recording results', async () => {
        ...
        expect(output).toContain('Tool output was too large and has been truncated');
    ...
    it('applies the per-tool budget for a tool invoked via a legacy alias', async () => {
        ...
        expect(outputOfFirstCall(onAllToolCallsComplete)).toContain('Tool output was too large and has been truncated');
    });
    ...
    it('offloads the largest tool outputs when a batch exceeds the budget', async () => {
        ...
        const outputOf = (name) => {
            ...
            return c && 'response' in c
                ? (c.response.responseParts[0]?.functionResponse?.response?.['output'] ?? '')
                : '';
        };
        // Largest output is offloaded to disk (recoverable pointer).
        expect(outputOf('bigBatchTool')).toContain('Tool output was too large and has been truncated');
    ...
    it('offloads timeout error detail while preserving failure metadata', async () => {
        ...
        const calls = onAllToolCallsComplete.mock.calls[0][0];
        const big = calls.find((call) => call.request.name === 'bigTimeoutTool');
        const small = calls.find((call) => call.request.name === 'smallTimeoutTool');
        expect(big?.status).toBe('error');
        expect(small?.status).toBe('error');
        if (big?.status === 'error' && small?.status === 'error') {
            const bigResponse = big.response.responseParts[0].functionResponse?.response;
            expect(bigResponse?.['error']).toContain('Tool output was too large and has been truncated');
            ...
            expect(small.response.responseParts[0].functionResponse?.response).toEqual({ error: 'b'.repeat(3000) });
        }
    });
    it('preserves PostToolBatch additionalContext in the offload preview tail', async () => {
        ...
        expect(bigOutput).toContain('Tool output was too large and has been truncated');
    ...
    it('applies a tool-declared maxOutputChars instead of the global threshold', async () => {
        ...
        expect(outputOf('tinyTool')).toContain('Tool output was too large and has been truncated');
    ...
    it('exempts a self-managed (Infinity maxOutputChars) tool from the line cap', async () => {
        ...
        const output = outputOfFirstCall(onAllToolCallsComplete);
        expect(output).not.toContain('Tool output was too large and has been truncated');
    ...
    it('applies canonical legacy tool names to the deny-list fallback', async () => {
        ...
        if (completedCall.status === 'error') {
            ...
            expect(completedCall.response.error?.message).toBe('HopCode requires permission to use edit, but that permission was declined.');
        }
        expect(execute).not.toHaveBeenCalled();
    ...
    it('fires PermissionDenied hooks for AUTO classifier blocks', async () => {
        ...
        expect(hookSystem.firePermissionDeniedEvent).toHaveBeenCalledWith(ToolNames.SHELL, { command: 'rm -rf /tmp/example' }, 'auto-denied', 'classifier_blocked', abortController.signal, 'auto-denied');
        expect(execute).not.toHaveBeenCalled();
        const completedCalls = onAllToolCallsComplete.mock
            .calls[0][0];
    ...
    it('fires PermissionDenied hooks for AUTO classifier unavailable blocks', async () => {
        ...
        expect(hookSystem.firePermissionDeniedEvent).toHaveBeenCalledWith(ToolNames.SHELL, { command: 'rm -rf /tmp/example' }, 'auto-unavailable', 'classifier_unavailable', abortController.signal, 'auto-unavailable');
        expect(execute).not.toHaveBeenCalled();
    });
    ...
    it('fires PostToolBatch once after a resolved tool batch before completion callback', async () => {
        ...
        const messageBus = {
            request: vi
                ...
                return {
                    ...
                    correlationId: `${request.eventName}-hook`,
                    success: true,
                    output: request.eventName === 'PostToolBatch'
                        ? {
                            hookSpecificOutput: {
                                hookEventName: 'PostToolBatch',
        ...
        const batchRequests = messageBus.request.mock.calls.filter(([request]) => request.eventName === 'PostToolBatch');
        expect(batchRequests).toHaveLength(1);
        expect(batchRequests[0][0]).toEqual(expect.objectContaining({
        ...
        expect(callOrder.indexOf('PostToolBatch')).toBeLessThan(callOrder.indexOf('complete'));
        const completionCalls = onAllToolCallsComplete.mock
            .calls;
        const completedCalls = completionCalls[0]?.[0];
        const lastCompletedCall = completedCalls?.at(-1);
        const lastCompletedResponse = lastCompletedCall && 'response' in lastCompletedCall
            ? lastCompletedCall.response
            : undefined;
        const lastResponse = lastCompletedResponse?.responseParts.at(-1);
        expect(lastResponse?.functionResponse?.response?.['output']).toContain('batch context');
        ...
        expect(scheduler.callIdToPostToolBatchSignal.size).toBe(0);
    });
    it('includes failed tool responses in PostToolBatch payloads', async () => {
        ...
        const batchRequest = messageBus.request.mock.calls.find(([request]) => request.eventName === 'PostToolBatch')?.[0];
    ...
    it('queues new tool calls while a PostToolBatch hook is still running', async () => {
        ...
        let resolveBatchHookStarted;
        const batchHookStarted = new Promise((resolve) => {
            resolveBatchHookStarted = resolve;
        });
        let releaseBatchHook;
        const batchHookRelease = new Promise((resolve) => {
    ...
    it('applies PostToolBatch stop decisions and preserves additional context', async () => {
        ...
        if (lastCompletedCall?.status === 'error') {
            expect(lastCompletedCall.response.errorType).toBe(ToolErrorType.EXECUTION_DENIED);
            expect(lastCompletedCall.response.error?.message).toContain('halt');
            const lastResponse = lastCompletedCall.response.responseParts.at(-1)?.functionResponse
                ?.response;
            expect(lastResponse?.['error']).toContain('halt');
            expect(lastResponse?.['error']).toContain('batch context');
            expect(lastCompletedCall.response.contentLength).toBe('halt'.length + 'batch context'.length + 2);
            expect(lastCompletedCall.outcome).toBeUndefined();
        }
        expect(debugLoggerInfoSpy).toHaveBeenCalledWith('PostToolBatch hook stopped batch (2 calls): halt');
        const batchHookSpan = toolSpanRecords.findLast((record) => record.name === 'hook' &&
        ...
        const messageBus = {
            request: vi.fn().mockImplementation(async (request) => ({
                ...
                output: request.eventName === 'PostToolBatch'
                    ? undefined
                    : { decision: 'allow' },
                error: request.eventName === 'PostToolBatch'
    ...
    describe('MCP tool-not-found messaging', () => {
        const makeScheduler = (opts) => {
            ...
            const mockConfig = {
                ...
                getMcpServerUnavailableReason: (name) => {
                    if ((opts.removed ?? []).includes(name))
                        return 'removed';
                    if (!(name in (opts.mcpServers ?? {})))
                        return undefined;
        ...
        it('names a server removed this session (precise, branch B)', () => {
            ...
            const msg = scheduler.getMcpToolUnavailableMessage('mcp__pangu-server__pangu_search');
        ...
        it('identifies a removed server whose name required normalization', () => {
            ...
            const registeredName = normalizeToolNameForProvider('mcp__zybio.db__literature.search_pubmed');
            // @ts-expect-error accessing private method
        ...
        it('reports an MCP tool with no configured server (branch A)', () => {
            ...
            // @ts-expect-error accessing private method
            const msg = scheduler.getMcpToolUnavailableMessage('mcp__ghost__do_thing');
            expect(msg).toContain('no MCP server providing it is currently configured');
        });
        it('reports a configured server that lacks the tool', () => {
            ...
            // @ts-expect-error accessing private method
            const msg = scheduler.getMcpToolUnavailableMessage('mcp__pangu-server__missing_tool');
        ...
        it('explains a not-allowed server with the allow-list recovery action', () => {
            ...
            // @ts-expect-error accessing private method
            const msg = scheduler.getMcpToolUnavailableMessage('mcp__pangu-server__search');
            expect(msg).toContain('"pangu-server"');
        ...
        it('getToolNotFoundMessage routes MCP names to the MCP branch, others to Levenshtein', async () => {
            ...
            // @ts-expect-error accessing private method
            const mcpMsg = await scheduler.getToolNotFoundMessage('mcp__pangu-server__pangu_search');
    ...
    describe('excluded tools handling', () => {
        it('should return permission error for excluded tools instead of "not found" message', async () => {
            ...
            if (completedCall.status === 'error') {
                ...
                expect(errorMessage).toBe('HopCode requires permission to use write_file, but that permission was declined.');
...
describe('convertToFunctionErrorResponse', () => {
    ...
    it('moves converted text to error and removes output', () => {
        const [part] = convertToFunctionErrorResponse(toolName, callId, 'timeout detail\npartial output', 'timeout summary');
    ...
    it('uses the fallback for empty converted content', () => {
        const [part] = convertToFunctionErrorResponse(toolName, callId, '', 'timeout summary');
        expect(part.functionResponse?.response).toEqual({
            error: 'timeout summary',
        });
    });
    it.each([[], {}])('uses the fallback instead of the success placeholder for %j', (content) => {
        const [part] = convertToFunctionErrorResponse(toolName, callId, content, 'actual failure');
        expect(part.functionResponse?.response).toEqual({
    ...
    it('prefers an existing error and preserves media and response fields', () => {
        ...
        const [part] = convertToFunctionErrorResponse(toolName, callId, content, 'fallback');
        expect(part.functionResponse).toEqual({
            id: callId,
            name: toolName,
...
describe('CoreToolScheduler IZN mode', () => {
    const runLongDisplayTool = async (longDisplay, isInteractive) => {
        ...
        expect(completedCall.status).toBe('success');
        if (completedCall.status === 'success') {
            return completedCall.response.resultDisplay;
        }
        return undefined;
    };
    it('compacts completed resultDisplay before retaining interactive scheduler state', async () => {
        ...
        expect(retainedDisplay?.length).toBeLessThanOrEqual(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS);
    ...
    it('preserves completed resultDisplay in non-interactive scheduler responses', async () => {
        const longDisplay = `head-${'x'.repeat(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS)}-tail`;
        await expect(runLongDisplayTool(longDisplay, false)).resolves.toBe(longDisplay);
    });
...
describe('CoreToolScheduler cancellation during executing with live output', () => {
    ...
    it('compacts live output only before retaining it in scheduler state', async () => {
        ...
        const liveOutputUpdate = onToolCallsUpdate.mock.calls
            .map((call) => call[0][0])
            .find((call) => call.status === 'executing' && call.liveOutput !== undefined);
        const retainedOutput = liveOutputUpdate?.liveOutput;
        expect(retainedOutput.length).toBeLessThanOrEqual(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS);
    ...
    it('forwards shell heartbeats without replacing liveOutput', async () => {
        ...
        class HeartbeatTool extends BaseDeclarativeTool {
            constructor() {
                super('heartbeat-tool', 'Heartbeat Tool', 'Emits a heartbeat and waits for abort', Kind.Other, {
                    type: 'object',
                    properties: { id: { type: 'string' } },
                    required: ['id'],
                }, true, true);
        ...
        const liveOutputs = onToolCallsUpdate.mock.calls
            .map((call) => call[0][0])
            .filter((call) => call.status === 'executing' && call.liveOutput !== undefined)
            .map((call) => call.liveOutput);
        expect(liveOutputs).toContain('real output');
        expect(liveOutputs.some((out) => typeof out === 'object' &&
            out !== null &&
            out.type === 'shell_progress')).toBe(false);
        abortController.abort();
        await schedulePromise;
    });
});
describe('CoreToolScheduler request queueing', () => {
    it('should queue a request if another is running', async () => {
        ...
        const mockConfig = {
            ...
            getUsageStatisticsEnabled: () => true,
            getDebugMode: () => false,
            getApprovalMode: () => ApprovalMode.IZN, // getApprovalMode: () => ApprovalMode.IZN, // Use IZN to avoid confirmation prompts
            getPermissionsAllow: () => [],
            getContentGeneratorConfig: () => ({
                model: 'test-model',
                authType: 'gemini',
        ...
        const schedulePromise2 = scheduler.schedule([request2], abortController.signal, runtimeView);
    ...
    function createPendingProtectedWriteHarness(options) {
        ...
        const setAutoModeDenialState = vi.fn((next) => {
            denialState = next;
        });
        const hookSystem = {
            firePermissionDeniedEvent: vi.fn().mockResolvedValue(undefined),
    ...
    it('runs AUTO classifier for pending L4 allow that writes protected paths', async () => {
        runSideQueryMock.mockResolvedValueOnce({ shouldBlock: false });
        const { scheduler, permissionManager, setAutoModeDenialState, onToolCallsUpdate, } = createPendingProtectedWriteHarness();
        await scheduler.autoApproveCompatiblePendingTools(new AbortController().signal, 'approved-sibling');
    ...
    it('fires PermissionDenied hooks for pending AUTO classifier blocks', async () => {
        ...
        const { scheduler, onToolCallsUpdate, hookSystem } = createPendingProtectedWriteHarness({ disableHooks: false });
        await scheduler.autoApproveCompatiblePendingTools(new AbortController().signal, 'approved-sibling');
        expect(hookSystem.firePermissionDeniedEvent).toHaveBeenCalledWith(ToolNames.SHELL, { command: "echo '{}' > .hopcode/settings.json" }, 'pending-protected-write', 'classifier_blocked', expect.any(AbortSignal), 'pending-protected-write');
    ...
    it('continues pending AUTO block handling when PermissionDenied hook fails', async () => {
        ...
        const { scheduler, onToolCallsUpdate, hookSystem } = createPendingProtectedWriteHarness({ disableHooks: false });
        hookSystem.firePermissionDeniedEvent.mockRejectedValueOnce(new Error('hook failed'));
        await scheduler.autoApproveCompatiblePendingTools(new AbortController().signal, 'approved-sibling');
        expect(hookSystem.firePermissionDeniedEvent).toHaveBeenCalled();
        const statuses = onToolCallsUpdate.mock.calls
            .flatMap((call) => call[0])
    ...
    it('keeps pending protected writes awaiting approval during AUTO fallback', async () => {
        ...
        await scheduler.autoApproveCompatiblePendingTools(new AbortController().signal, 'approved-sibling');
        expect(hookSystem.firePermissionDeniedEvent).not.toHaveBeenCalled();
        const toolCalls = scheduler.toolCalls;
        expect(toolCalls[0]?.status).toBe('awaiting_approval');
        expect(scheduler.autoModeFallbackCallIds.has('pending-protected-write')).toBe(true);
    });
});
...
describe('CoreToolScheduler plan mode with ask_user_question', () => {
    ...
    function createPlanModeScheduler(tool, onAllToolCallsComplete, onToolCallsUpdate, options = {}) {
        const mockToolRegistry = {
            getTool: () => tool,
            ensureTool: async () => tool,
            getToolByName: () => tool,
    ...
    it('should block non-ask_user_question tools that need confirmation in plan mode', async () => {
        ...
        if (completedCalls[0].status === 'error') {
            ...
            expect(completedCalls[0].response.errorType).toBe(ToolErrorType.EXECUTION_DENIED);
        }
    });
    it.each([
        {
            ...
            run: (schedule) => runWithAgentContext('agent-1', schedule),
        },
        {
            label: 'teammates',
            promptId: 'prompt-plan-teammate-blocked',
            run: (schedule) => runWithTeammateIdentity({
                agentId: 'agent@test',
                agentName: 'agent',
                teamName: 'test',
                isTeamLead: false,
    ...
    ])('should tell $label to return the plan directly when plan mode blocks a tool', async ({ promptId, run, schedulerOptions }) => {
        const editTool = new MockTool({
            name: 'write_file',
            getDefaultPermission: MOCK_TOOL_GET_DEFAULT_PERMISSION,
            getConfirmationDetails: MOCK_TOOL_GET_CONFIRMATION_DETAILS,
        ...
        const onAllToolCallsComplete = vi.fn();
        const onToolCallsUpdate = vi.fn();
        const scheduler = createPlanModeScheduler(editTool, onAllToolCallsComplete, onToolCallsUpdate, schedulerOptions);
        const abortController = new AbortController();
        const request = {
            callId: '1',
            name: 'write_file',
    ...
    it('blocks a plan-required teammate before confirmation is requested', async () => {
        ...
        const onAllToolCallsComplete = vi.fn();
        const onToolCallsUpdate = vi.fn();
        const scheduler = createPlanModeScheduler(tool, onAllToolCallsComplete, onToolCallsUpdate, { avoidPermissionPrompts: true });
        ...
        if (completedCalls[0].status === 'error') {
            ...
            expect(response).toContain('send_message is not available while this plan-required teammate is waiting for leader approval');
    ...
    it('blocks a plan-required teammate even when permission rules allow the tool', async () => {
        ...
        const onAllToolCallsComplete = vi.fn();
        const onToolCallsUpdate = vi.fn();
        const scheduler = createPlanModeScheduler(tool, onAllToolCallsComplete, onToolCallsUpdate, {
    ...
    it('blocks trusted MCP-like default-allow tools before leader approval', async () => {
        ...
        if (completedCalls[0].status === 'error') {
            const response = JSON.stringify(completedCalls[0].response.responseParts);
            expect(response).toContain('mcp__trusted__write_record is not available while this plan-required teammate is waiting for leader approval');
        }
        const statuses = onToolCallsUpdate.mock.calls
            .flatMap((call) => call[0])
            .map((call) => call.status);
...
describe('CoreToolScheduler telemetry spans', () => {
    ...
    function buildScheduler(options) {
        ...
        const lookup = (name) => (name ? toolsByName.get(name) : undefined) ?? tools[0];
        const mockToolRegistry = {
            getTool: (n) => lookup(n),
            ensureTool: async (n) => lookup(n),
            getFunctionDeclarations: () => [],
            tools: new Map(),
            discovery: {},
            registerTool: () => { },
            getToolByName: (n) => lookup(n),
            getToolByDisplayName: (n) => lookup(n),
            getTools: () => tools,
            discoverTools: async () => { },
            getAllTools: () => tools,
            getToolsByServer: () => [],
        ...
        const mockConfig = {
            ...
            getExperimentalZedIntegration: () => options.experimentalZedIntegration ?? false,
            getShouldAvoidPermissionPrompts: () => options.shouldAvoidPermissionPrompts ?? false,
            getTelemetryIncludeSensitiveSpanAttributes: () => options.includeSensitiveSpanAttributes ?? false,
            getTelemetrySensitiveSpanAttributeMaxLength: () => options.sensitiveSpanAttributeMaxLength ?? 1024 * 1024,
        };
        const onAllToolCallsComplete = vi.fn();
        const onToolCallsUpdate = vi.fn();
        const scheduler = new CoreToolScheduler({
    ...
    it('acquires the sleep inhibitor around actual tool execution', async () => {
        ...
        expect(mockAcquireSleepInhibitor).toHaveBeenCalledWith(expect.any(Object), 'HopCode is executing tool mockTool');
    ...
    it('does not fail tool execution when sensitive tool span attributes fail', async () => {
        ...
        expect(completedCalls[0].status).toBe('success');
        expect(spanRecord.ended).toBe(true);
        expect(debugLoggerWarnSpy).toHaveBeenCalledWith('Failed to add tool input span attributes:', expect.any(TypeError));
        expect(debugLoggerWarnSpy).toHaveBeenCalledWith('Failed to add tool result span attributes:', expect.any(TypeError));
    });
    ...
    function preToolUseCallCount(messageBus) {
        return messageBus.request.mock.calls.filter((call) => call[0]?.eventName === 'PreToolUse').length;
    }
    it('bounces a PreToolUse ask to awaiting_approval with an info confirmation', async () => {
        ...
        const waiting = (await waitForStatus(onToolCallsUpdate, 'awaiting_approval'));
        expect(waiting.confirmationDetails.type).toBe('info');
        // The hook re-evaluates on every call, so "always allow" is hidden.
        expect(waiting.confirmationDetails
            .hideAlwaysAllow).toBe(true);
        expect(waiting.confirmationDetails.prompt).toContain('confirm deploy 38111');
    ...
    it('executes the tool exactly once when the user approves an ask (no re-ask loop)', async () => {
        ...
        const completed = onAllToolCallsComplete.mock.calls.at(-1)?.[0];
    ...
    it('does not double-unescape path args across an ask bounce', async () => {
        ...
        const completed = onAllToolCallsComplete.mock.calls.at(-1)?.[0];
        expect(completed[0].status).toBe('success');
        expect(completed[0].request.args['file_path']).toBe(unescapePath(rawPath));
    });
    it('approving a bounced ask runs the tool even while a sibling is still executing', async () => {
        ...
        const messageBus = {
            request: vi
                ...
                correlationId: 'pre-hook',
                success: true,
                output: req.eventName === 'PreToolUse' && req.input?.tool_name === 'toolA'
                    ? { decision: 'ask', reason: 'confirm A' }
                    : {},
            })),
        };
        const { scheduler, onAllToolCallsComplete, onToolCallsUpdate } = buildScheduler({ tools, messageBus, disableHooks: false });
        ...
        expect(completed.find((c) => c.request.callId === 'a')?.status).toBe('success');
        expect(completed.find((c) => c.request.callId === 'b')?.status).toBe('success');
    });
    ...
    it("a sibling's ProceedAlways must not auto-approve a bounced ask", async () => {
        ...
        const messageBus = {
            request: vi
                ...
                correlationId: 'pre-hook',
                success: true,
                output: req.eventName === 'PreToolUse'
        ...
        const toolBWaiting = onToolCallsUpdate.mock.calls
            .flatMap((c) => c[0])
            .find((tc) => tc.request.callId === 'b' && tc.status === 'awaiting_approval');
        ...
        await toolBWaiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedAlways);
    ...
    it('ignores ModifyWithEditor for a bounced ask info confirmation', async () => {
        ...
        const tool = Object.assign(new MockTool({
            name: 'mockTool',
            kind: Kind.Edit,
            execute,
        }), { getModifyContext });
        ...
        const waiting = (await waitForStatus(onToolCallsUpdate, 'awaiting_approval'));
        await expect(waiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ModifyWithEditor)).resolves.toBeUndefined();
    ...
    it('pauses later unsafe batches while a bounced ask awaits approval', async () => {
        ...
        const waiting = onToolCallsUpdate.mock.calls
            .flatMap((c) => c[0])
            .find((tc) => tc.request.callId === 'a' && tc.status === 'awaiting_approval');
        expect(waiting).toBeDefined();
        await waiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce);
        await vi.waitFor(() => {
            expect(onAllToolCallsComplete).toHaveBeenCalled();
    ...
    it('abort drain cancels scheduled siblings behind a bounced ask', async () => {
        ...
        const abortController = new AbortController();
        const { scheduler, onAllToolCallsComplete, onToolCallsUpdate } = buildScheduler({
            tools,
            messageBus,
            disableHooks: false,
        });
    ...
    it('cleans bounce markers when post-ask re-execution fails before body runs', async () => {
        ...
        await expect(waiting.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce)).rejects.toThrow('context failed before callback');
        expect(scheduler
            .bouncedAwaitingApproval.size).toBe(0);
        expect(scheduler
            .bouncedToolUseId.size).toBe(0);
    ...
    it('hook span records shouldProceed=false / blockType=denied when pre-hook blocks (#3731 Phase 2)', async () => {
        ...
        await runSingleTool({ messageBus, disableHooks: false });
        const preToolUseSpan = getHookSpans().find((span) => span.attributes['hook_event'] === 'PreToolUse');
    ...
    it('preserves PostToolUseFailure artifacts on user-abort cancellations', async () => {
        ...
        const messageBus = {
            request: vi.fn(async (req) => ({
                ...
                success: true,
                output: req.eventName === 'PostToolUseFailure'
                    ? {
                        hookSpecificOutput: {
                            artifacts: [
                                {
    ...
    it('preserves live output when hook artifacts are attached to user-abort cancellations', async () => {
        ...
        expect(completedCall.status).toBe('cancelled');
        if (completedCall.status === 'cancelled') {
            expect(completedCall.response.resultDisplay).toBe('live output before abort');
            expect(completedCall.response.artifacts).toEqual([
                {
                    title: 'Cancel report',
                    workspacePath: 'reports/cancel.html',
     ...
     */
    function buildApprovalScheduler(overrides, tool = new MockEditTool()) {
        const mockToolRegistry = {
            getTool: () => tool,
            ensureTool: async () => tool,
            getFunctionDeclarations: () => [],
    ...
    it('keeps the exact runtime through manual approval', async () => {
        ...
        ], new AbortController().signal, runtimeView);
        const awaitingCall = (await waitForStatus(onToolCallsUpdate, 'awaiting_approval'));
    ...
    it('ModifyWithEditor !editorType stamps modify_with_editor_unavailable on tool span (#4321)', async () => {
        ...
        const toolSpan = toolSpanRecords.find((r) => r.name === 'tool.mockEditTool');
        expect(toolSpan?.spanAttributes['hopcode.tool.modify_with_editor_unavailable']).toBe(true);
...
describe('Fire hook functions integration', () => {
    ...
    describe('Concurrent tool execution', () => {
        ...
        it('ignores malformed HOPCODE_CODE_MAX_TOOL_CONCURRENCY values', async () => {
            process.env['HOPCODE_CODE_MAX_TOOL_CONCURRENCY'] = '2abc';
            const executionLog = [];
            let release;
            ...
            try {
                await vi.waitFor(() => {
                    expect(executionLog.filter((e) => e.startsWith('start:'))).toHaveLength(3);
                });
            }
            finally {
                release();
        ...
        describe('isToolCallConcurrencySafe', () => {
            it('treats agent tools as safe regardless of resolved kind', () => {
                expect(isToolCallConcurrencySafe(ToolNames.AGENT, undefined, {})).toBe(true);
                expect(isToolCallConcurrencySafe(ToolNames.AGENT, Kind.Other, {})).toBe(true);
            });
            it('treats pure-read kinds as safe', () => {
                expect(isToolCallConcurrencySafe('read_file', Kind.Read, {})).toBe(true);
            ...
            it('treats a read-only shell command as safe and a mutating one as unsafe', () => {
                expect(isToolCallConcurrencySafe('shell', Kind.Execute, {
                    command: 'git status',
                })).toBe(true);
                expect(isToolCallConcurrencySafe('shell', Kind.Execute, {
                    command: 'rm -rf build',
            ...
            it('treats a shell call with a non-string command as unsafe (fail-closed)', () => {
                expect(isToolCallConcurrencySafe('shell', Kind.Execute, {})).toBe(false);
                expect(isToolCallConcurrencySafe('shell', Kind.Execute, { command: 42 })).toBe(false);
            });
            it('treats an unresolved (undefined) kind on a non-agent tool as unsafe', () => {
                expect(isToolCallConcurrencySafe('mystery_tool', undefined, {})).toBe(false);
            });
        });
        ...
        it('should run read-only shell commands concurrently and non-read-only sequentially', async () => {
            ...
            const wrapperStart = executionLog.indexOf("shell:start:bash -c 'git status'");
            ...
            const sortStart = executionLog.indexOf('shell:start:sort -o output input');
...
describe('CoreToolScheduler validation retry loop detection', () => {
    ...
    function makeRequest(callId, name, args, wasOutputTruncated = false) {
        const request = {
            callId,
            name,
            args,
    ...
    it('should inject RETRY LOOP DETECTED directive after 3 consecutive validation failures', async () => {
        ...
        await scheduler.schedule([makeRequest('c1', 'strictStringTool', { value: {} })], new AbortController().signal);
        ...
        await scheduler.schedule([makeRequest('c2', 'strictStringTool', { value: {} })], new AbortController().signal);
        msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
        // Turn 3: same bad params — should trigger directive
        await scheduler.schedule([makeRequest('c3', 'strictStringTool', { value: {} })], new AbortController().signal);
        msg = getLastErrorMessage(onToolCallsUpdate);
    ...
    it('should keep retry counts stable when truncation guidance is toggled', async () => {
        const tool = new StrictStringTool();
        const { scheduler, onToolCallsUpdate } = createSchedulerWithTool(tool);
        await scheduler.schedule([makeRequest('c1', 'strictStringTool', { value: {} }, true)], new AbortController().signal);
        let msg = getLastErrorMessage(onToolCallsUpdate);
        ...
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
        await scheduler.schedule([makeRequest('c3', 'strictStringTool', { value: {} }, true)], new AbortController().signal);
        msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).not.toContain('previous response was truncated');
        expect(msg).toContain(RETRY_LOOP_STOP_DIRECTIVE);
    });
    it('should reset retry counter when a different tool is called', async () => {
        ...
        await scheduler.schedule([makeRequest('c4', 'strictStringTool', { value: {} })], new AbortController().signal);
    ...
    it('should reset retry counter after a successful invocation of the same tool', async () => {
        ...
        await scheduler.schedule([makeRequest('c4', 'strictStringTool', { value: {} })], new AbortController().signal);
        await scheduler.schedule([makeRequest('c5', 'strictStringTool', { value: {} })], new AbortController().signal);
        const msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).toBeDefined();
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
    });
    it('should isolate retry counters per-tool across batches', async () => {
        ...
        await scheduler.schedule([makeRequest('a1', StrictStringTool.Name, { value: {} })], new AbortController().signal);
        await scheduler.schedule([makeRequest('a2', StrictStringTool.Name, { value: {} })], new AbortController().signal);
        // Now a batch for tool B only — tool A's counter must be pruned because
        // A is not present in this batch.
        await scheduler.schedule([makeRequest('b1', StrictToolAlt.Name, { other: {} })], new AbortController().signal);
        ...
        // should be emitted.
        await scheduler.schedule([makeRequest('a3', StrictStringTool.Name, { value: {} })], new AbortController().signal);
        const msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).toBeDefined();
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
    });
        ...
        const mockConfig = {
            ...
            getSkillManager: () => {
                ...
                return {
                    ...
                    listSkills: vi.fn().mockResolvedValue(names.map((n) => ({
                        name: n,
                        description: `Description of ${n}`,
                        level: 'project',
                        filePath: `/p/.hopcode/skills/${n}/SKILL.md`,
                        body: '',
                    }))),
                ...
                listSkills: vi.fn().mockResolvedValue([
                    {
                        ...
                        level: 'project',
                        filePath: '/p/.hopcode/skills/tsx-helper/SKILL.md',
                        body: '',
                    },
                ]),
                isSkillActive: vi.fn().mockReturnValue(true),
...
describe('CoreToolScheduler prompt_id propagation', () => {
    ...
    class PromptIdAwareTool extends BaseDeclarativeTool {
        lastBuiltInvocation;
        constructor() {
            super('promptIdAwareTool', 'promptIdAwareTool', 'A tool that captures prompt_id via setPromptId', Kind.Read, {});
        }
        createInvocation(params) {
    ...
    it('passes request.prompt_id to invocation.setPromptId via buildInvocation', async () => {
        ...
        expect(tool.lastBuiltInvocation?.capturedPromptId).toBe('expected-prompt-id-xyz');
    });
    it('buildInvocation calls setPromptId when promptId is provided (covers both setArgs and schedule call sites)', () => {
        ...
        const invocation = scheduler.buildInvocation(tool, {}, 'call-direct', 'expected-via-setArgs-path');
    ...
    it('buildInvocation does not throw when promptId is omitted', () => {
        ...
        const invocation = scheduler.buildInvocation(tool, {}, 'call-omitted');
    ...
    it('is a no-op when invocation does not expose setPromptId', async () => {
        ...
        ], abortController.signal)).resolves.not.toThrow();
        await vi.waitFor(() => {
            expect(onAllToolCallsComplete).toHaveBeenCalled();
        });
    });
