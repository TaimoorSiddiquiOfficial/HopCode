...
export class Logger {
  private hopcodeDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.hopcodeDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.hopcodeDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.hopcodeDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.hopcodeDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.hopcodeDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.hopcodeDir) {
      this.debugLogger.error(
