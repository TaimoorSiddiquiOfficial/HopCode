...
import { normalize, tokenLimit, knownTokenLimit, clampOutputTokensToWindow, outputClampMargin, defaultOutputCeiling, reconcileMaxTokens, DEFAULT_TOKEN_LIMIT, DEFAULT_OUTPUT_TOKEN_LIMIT, ESCALATED_MAX_TOKENS, MIN_CLAMPED_OUTPUT_TOKENS, OUTPUT_TOKEN_CEILING, } from './tokenLimits.js';
describe('normalize', () => {
...
describe('clampOutputTokensToWindow', () => {
    ...
    it('floors at MIN_CLAMPED_OUTPUT_TOKENS when no room is left', () => {
        ...
        expect(clampOutputTokensToWindow(32_000, 40_000, 39_000)).toBe(MIN_CLAMPED_OUTPUT_TOKENS);
        expect(clampOutputTokensToWindow(32_000, 40_000, 60_000)).toBe(MIN_CLAMPED_OUTPUT_TOKENS);
    });
...
describe('defaultOutputCeiling', () => {
    ...
    it('uses the default output limit for an unknown model', () => {
        expect(defaultOutputCeiling('some-unknown-model')).toBe(Math.min(DEFAULT_OUTPUT_TOKEN_LIMIT, OUTPUT_TOKEN_CEILING));
    });
});
