...
import { DEFAULT_TIMEOUT, DISABLED_REQUEST_TIMEOUT_MS, } from '../openaiContentGenerator/constants.js';
...
describe('AnthropicContentGenerator', () => {
    ...
    it('disables the request timeout when configured to 0', async () => {
        ...
        expect(anthropicState.constructorOptions?.['timeout']).toBe(DISABLED_REQUEST_TIMEOUT_MS);
    });
    it('falls back to the default request timeout when unset', async () => {
        ...
        expect(anthropicState.constructorOptions?.['timeout']).toBe(DEFAULT_TIMEOUT);
    });
    ...
    describe('per-request anthropic-beta header', () => {
        ...
        it('keeps customHeaders + User-Agent in defaultHeaders while sending computed anthropic-beta per-request', async () => {
            ...
            const defaultHeaders = (anthropicState.constructorOptions?.['defaultHeaders'] || {});
            expect(defaultHeaders['User-Agent']).toContain('HopCode/1.2.3');
    ...
    describe('generateContent', () => {
        ...
        it('does not leak abort listeners onto the caller signal across non-streaming requests', async () => {
            ...
            anthropicState.createImpl.mockImplementation((_req, opts) => {
        ...
        it('keeps temperature on Opus 4.7 (still accepted)', async () => {
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ temperature: 0.7 }));
        });
        ...
        it('keeps temperature on unknown/unversioned model id', async () => {
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ temperature: 0.3 }));
        });
        it("clamps effort: 'xhigh' to 'max' on Opus 4.6 (has max, lacks xhigh)", async () => {
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ output_config: { effort: 'max' } }));
        });
        ...
        it("passes effort: 'xhigh' through on a reseller-prefixed Opus 4.7 (bedrock/…)", async () => {
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ output_config: { effort: 'xhigh' } }));
        });
        ...
        describe('adaptive thinking (Claude 4.6+ models)', () => {
            ...
            it('honors explicit reasoning.budget_tokens on models that still accept manual thinking (e.g. claude-opus-4-6)', async () => {
                ...
                expect(await thinkingFor('claude-opus-4-6', {
                    effort: 'medium',
        ...
        describe('output token limits', () => {
            ...
            it('uses model default when max_tokens is not explicitly configured', async () => {
                ...
                const [anthropicRequest] = anthropicState.lastCreateArgs;
                expect(anthropicRequest).toEqual(expect.objectContaining({ max_tokens: 64000 }));
            });
            ...
            it('respects a valid HOPCODE_CODE_MAX_OUTPUT_TOKENS value', async () => {
                ...
                const [anthropicRequest] = anthropicState.lastCreateArgs;
                expect(anthropicRequest).toEqual(expect.objectContaining({ max_tokens: 9000 }));
            });
    ...
    describe('Anthropic-compatible proxy thinking history', () => {
        ...
        async function sendWithBaseUrl(baseUrl, contents = unsignedThinkingConversation) {
            const { AnthropicContentGenerator } = await importGenerator();
            anthropicState.createImpl.mockResolvedValue({
                id: 'msg-1',
                model: 'claude-opus-4-6',
            ...
            return anthropicState.lastCreateArgs?.[0];
        }
        it('drops unsigned thinking for Claude 4.6 through a non-native proxy', async () => {
            const request = await sendWithBaseUrl('https://internal-proxy.example/anthropic');
        ...
        it('fails before sending an unsigned tool-use turn through a proxy', async () => {
            ...
            await expect(sendWithBaseUrl('https://internal-proxy.example/anthropic', toolUseConversation)).rejects.toThrow('proxy omitted the thinking signature');
    ...
    describe('generateContentStream', () => {
        it('emits tool preparation metadata before the complete function call', async () => {
            const { AnthropicContentGenerator } = await importGenerator();
            const { getToolCallPreparations } = await import('../tool-call-preparation.js');
            let stopEventReached = false;
            anthropicState.createImpl.mockResolvedValue((async function* toolUseStream() {
                yield {
                    type: 'message_start',
                    message: {
                        id: 'msg-1',
            ...
            const chunks = [];
            let stopReachedWhenFunctionCallEmitted;
        ...
        it('emits preparations before both function calls in a multi-tool stream', async () => {
            const { AnthropicContentGenerator } = await importGenerator();
            const { getToolCallPreparations } = await import('../tool-call-preparation.js');
            anthropicState.createImpl.mockResolvedValue((async function* multiToolStream() {
                yield {
                    type: 'content_block_start',
                    index: 0,
                    content_block: {
            ...
            const preparations = chunks.flatMap((chunk, index) => getToolCallPreparations(chunk).map((preparation) => ({
                ...preparation,
                index,
            })));
            const functionCalls = chunks.flatMap((chunk, index) => (chunk.functionCalls ?? []).map((functionCall) => ({
                ...functionCall,
                index,
            })));
        ...
        it.each([
        ...
        ])('does not emit tool preparation metadata when $label', async ({ contentBlock }) => {
            const { AnthropicContentGenerator } = await importGenerator();
            const { getToolCallPreparations } = await import('../tool-call-preparation.js');
            anthropicState.createImpl.mockResolvedValue((async function* toolUseStream() {
                yield {
            ...
            expect(chunks.every((chunk) => getToolCallPreparations(chunk).length === 0)).toBe(true);
        });
        ...
        it('falls back to non-streaming when the stream is empty and surfaces provider errors', async () => {
            ...
            const [fallbackRequest] = anthropicState.createImpl.mock
                .calls[1];
            expect(streamingRequest).toEqual(expect.objectContaining({ stream: true }));
    ...
    describe('tool_choice mapping from Gemini toolConfig', () => {
        async function sendWithToolConfig(mode, hasTools = true) {
            const { AnthropicContentGenerator } = await importGenerator();
            anthropicState.createImpl.mockResolvedValue({
                id: 'msg-1',
                model: 'claude-opus-4-6',
