...
import { ChatCompressionService, MAX_CONSECUTIVE_FAILURES, } from '../services/chatCompressionService.js';
import { estimateContentTokens, estimatePromptTokens, } from '../services/tokenEstimation.js';
...
const { mockLogContentRetry, mockLogContentRetryFailure, mockLogProtocolTagSanitized, } = vi.hoisted(() => ({
...
describe('GeminiChat', async () => {
    ...
    describe('system instruction helpers', () => {
        ...
        it('does not strip legitimate content that only resembles the old plain-text marker', () => {
            ...
            expect(isolatedChat['generationConfig'].systemInstruction).toContain('<hopcode:session-start-context hidden="true">\nSessionStart additional context:\nCtx1\n</hopcode:session-start-context>');
        });
    });
    describe('sendMessageStream', () => {
        it('releases the sleep inhibitor after the stream is consumed', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test message' }, 'prompt-id-sleep-inhibitor');
            for await (const _ of stream) {
                /* consume stream */
            }
            expect(mockAcquireSleepInhibitor).toHaveBeenCalledWith(mockConfig, 'HopCode is streaming a model response');
        ...
        it('increments the user-content push counter once per surviving send', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, 'prompt-id-push-count');
            for await (const _ of stream) {
                /* consume stream */
            }
        ...
        it('releases the sleep inhibitor when the stream errors', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'fail' }, 'prompt-id-stream-error');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume stream */
                }
        ...
        it('synthesizes a functionResponse for a dangling tool_use before sending', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(ackStream);
            const stream = await chat.sendMessageStream('test-model', { message: 'next user prompt after a stream-error-mid-tool_use' }, 'prompt-send-repair');
            for await (const _ of stream) {
                /* drain */
            }
            const history = chat.getHistory();
            ...
            expect(fr?.functionResponse?.response?.error).toMatch(/interrupted/i);
            // The user's own text part is still present.
            expect(userTurn.parts.some((p) => p.text === 'next user prompt after a stream-error-mid-tool_use')).toBe(true);
            ...
            expect(userTurn.parts[0].functionResponse?.id).toBe('call_dangling_for_send');
        });
        it('does NOT synthesize when the user supplies a matching tool_result', async () => {
            ...
            expect(frParts[0].functionResponse?.response?.output).toBe('real-tool-output');
        });
        ...
        it('uses the normalized function call ID for preparation metadata', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'second request' }, 'prompt-normalized-preparation-id');
            const events = [];
            for await (const event of stream)
                events.push(event);
            ...
            const functionCall = events.find((event) => event.type === StreamEventType.CHUNK &&
                event.value.functionCalls?.length);
            expect(preparation?.callId).toBe('call-1__qwen_dup_2');
            expect(functionCall?.type === StreamEventType.CHUNK
                ? functionCall.value.functionCalls?.[0]?.id
                : undefined).toBe(preparation?.callId);
        });
        it('persists partial assistant turn when stream throws after a tool_use chunk', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(streamThatThrowsAfterToolCall);
            const stream = await chat.sendMessageStream('test-model', { message: 'open /tmp/x.txt please' }, 'prompt-weak-network-tool');
            await expect((async () => {
                for await (const _ of stream) {
                    /* drain */
                }
            ...
            expect(functionCallPart?.functionCall?.id).toBe('call_00_CeJrKJB0PSmXUZTCWHET7332');
        ...
        it('preserves thinking parts alongside tool_use when stream throws mid-tool', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(streamWithThinkingAndTool);
            const stream = await chat.sendMessageStream('test-model', { message: 'read /tmp/a.txt' }, 'prompt-thinking-tool-weak-network');
            await expect((async () => {
                for await (const _ of stream) {
                    /* drain */
                }
        ...
        it('does NOT persist partial assistant turn when stream throws before any tool_use chunk', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(streamThatThrowsAfterText);
            const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, 'prompt-weak-network-text');
            await expect((async () => {
                for await (const _ of stream) {
                    /* drain */
                }
        ...
        it('should retry semantically empty responses after a tool result', async () => {
            ...
            try {
                ...
                const chatWithRecording = new GeminiChat(mockConfig, config, [], {
                    recordAssistantTurn,
                ...
                const stream = await chatWithRecording.sendMessageStream('test-model', {
                    message: [
                        {
                            functionResponse: {
                                id: 'call_read_file',
                ...
                expect(events.some((event) => event.type === StreamEventType.RETRY)).toBe(true);
                const lastRetryIndex = events.findLastIndex((event) => event.type === StreamEventType.RETRY);
                const finishIndex = events.findIndex((event) => event.type === StreamEventType.CHUNK &&
                    Boolean(event.value.candidates?.[0]?.finishReason));
                expect(finishIndex).toBeGreaterThan(lastRetryIndex);
                expect(events
                    .slice(lastRetryIndex + 1)
                    .some((event) => event.type === StreamEventType.CHUNK &&
                    event.value.candidates?.[0]?.content?.parts?.some((part) => part.text === '(empty content)'))).toBe(false);
                expect(mockLogContentRetry).toHaveBeenCalledTimes(3);
                expect(mockLogContentRetry).toHaveBeenLastCalledWith(mockConfig, expect.objectContaining({
        ...
        it('should surface the last empty tool result continuation after retry exhaustion', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streamResponse(stopResponse([{ thought: true, text: 'I should keep working.' }])));
                const stream = await chat.sendMessageStream('test-model', {
                    message: [
                        {
                            functionResponse: {
                ...
                const resultPromise = collecting.then(() => {
                    ...
                    expect(error).toMatchObject({
                        message: 'Model stream ended after a tool result without visible progress.',
        ...
        it('should not retry tool result continuations that make another tool call', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(streamResponse(stopResponse([
                {
                    functionCall: {
                        id: 'call_list_files',
                        name: 'list_files',
            ...
            expect(events.some((event) => event.type === StreamEventType.RETRY)).toBe(false);
            expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                event.value.candidates?.[0]?.content?.parts?.some((part) => part.functionCall?.id === 'call_list_files'))).toBe(true);
        });
        it('should escalate thought-only MAX_TOKENS responses after a tool result', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => responses.shift());
            const stream = await chat.sendMessageStream('gemini-pro', {
                message: [
                    {
                        functionResponse: {
                            id: 'call_read_file',
            ...
            const calls = vi.mocked(mockContentGenerator.generateContentStream).mock
                .calls;
            expect(calls[1][0].config?.maxOutputTokens).toBeGreaterThan(calls[0][0].config?.maxOutputTokens ?? 0);
            expect(events.some((event) => event.type === StreamEventType.RETRY &&
                event.maxOutputTokensEscalated !== undefined)).toBe(true);
        ...
        it('should not escalate thought-only MAX_TOKENS responses when max tokens are user-set', async () => {
            ...
            try {
                ...
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(events.some((event) => event.type === StreamEventType.RETRY &&
                    event.maxOutputTokensEscalated !== undefined)).toBe(false);
                expect(mockLogContentRetry).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
        ...
        it('keeps historical image refs stable and reattaches only recent image bytes', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(response);
            const stream = await chat.sendMessageStream('test-model', { message: 'continue' }, 'prompt-id-image-refs');
            for await (const _ of stream) {
                // consume stream
            }
            ...
            expect(serialized).toMatch(/\[Image #[a-f0-9]{12}: image\/png, \d+ bytes\]/);
        ...
        it('coalesces startup reminders with the first user prompt for provider requests', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(response);
            const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, 'prompt-id-startup-coalesce');
            for await (const _ of stream) {
                // consume stream
            }
            const request = vi.mocked(mockContentGenerator.generateContentStream).mock
        ...
        it.each([
            ...
            ['undefined', undefined],
            ['string', '42'],
        ])('coerces hostile-provider %s promptTokenCount so the compaction gate is not poisoned', async (_label, badValue) => {
            const response = (async function* () {
                yield {
                    candidates: [
                        {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(response);
            const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, `prompt-id-hostile-${_label}`);
            for await (const _ of stream) {
                // consume stream
            }
            ...
            if (badValue == null) {
                ...
                const tokenWarn = warnCalls.find((args) => typeof args[0] === 'string' &&
                    (args[0].includes('promptTokenCount') ||
                        args[0].includes('totalTokenCount')));
                expect(tokenWarn).toBeUndefined();
            }
            else {
                const promptWarn = warnCalls.find((args) => typeof args[0] === 'string' &&
                    args[0].includes('hostile promptTokenCount'));
                const totalWarn = warnCalls.find((args) => typeof args[0] === 'string' &&
        ...
        it('sanitizes a standalone closing thinking tag without retrying valid tool calls', async () => {
            ...
            const provider = {
                buildClient: () => ({ chat: { completions: { create } } }),
                buildRequest: (request) => request,
                buildHeaders: () => ({}),
                getDefaultGenerationConfig: () => ({}),
            };
            const generator = new OpenAIContentGenerator({ model: 'test-model', authType: AuthType.USE_OPENAI }, mockConfig, provider);
            vi.mocked(mockConfig.getContentGenerator).mockReturnValue(generator);
            vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
                model: 'test-model',
                authType: AuthType.USE_OPENAI,
            });
            const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-sanitized-protocol-tag');
            const events = [];
            for await (const event of stream)
                events.push(event);
            const parts = events.flatMap((event) => event.type === StreamEventType.CHUNK
                ? (event.value.candidates?.[0]?.content?.parts ?? [])
                : []);
            expect(create).toHaveBeenCalledTimes(1);
            expect(mockLogContentRetry).not.toHaveBeenCalled();
            expect(mockLogProtocolTagSanitized).toHaveBeenCalledTimes(1);
            expect(mockLogProtocolTagSanitized).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                model: 'test-model',
            ...
            expect(JSON.stringify(chatWithRecording.getHistory())).not.toContain('</think>');
            expect(recordAssistantTurn).toHaveBeenCalledTimes(1);
            expect(JSON.stringify(recordAssistantTurn.mock.calls[0]?.[0].message)).not.toContain('</think>');
        });
        it('falls back to coerced totalTokenCount when promptTokenCount is hostile', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(response);
            const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, 'prompt-id-hostile-fallback');
            for await (const _ of stream) {
                // consume stream
            }
            expect(chat.getLastPromptTokenCount()).toBe(73);
            expect(uiTelemetryService.setLastPromptTokenCount).toHaveBeenCalledWith(73);
        });
    ...
    describe('auto-compression integration', () => {
        function makeStreamResponse(text = 'ok', usageMetadata) {
            return (async function* () {
                yield {
                    candidates: [
                        {
        ...
        it('forwards the pending user message to the compression cheap-gate', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: userMessageText }, 'prompt-id-first-turn');
            ...
            expect(passedOpts.pendingUserMessage?.parts?.some((part) => part.text === userMessageText)).toBe(true);
        });
        it('triggers compaction end-to-end through the real ChatCompressionService when lastPromptTokenCount === 0 and inherited history is large (R3.4)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('done'));
            const stream = await chat.sendMessageStream('test-model', { message: 'follow-up after restore' }, 'prompt-r3-4');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            const compressed = events.find((e) => e.type === StreamEventType.COMPRESSED);
            expect(compressed).toBeDefined();
            expect(compressed
                .info.compressionStatus).toBe(CompressionStatus.COMPRESSED);
    ...
    describe('sendMessageStream hard-tier rescue', () => {
        ...
        it('forces compaction with force=true when estimated tokens cross hard threshold', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('after rescue'));
            ...
            const stream = await chatWithRecording.sendMessageStream('test-model', { message: userMessage }, 'prompt-id-hard-rescue-forces');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(1);
            ...
            expect(passedOpts.pendingUserMessage).toBeDefined();
            expect(passedOpts.pendingUserMessage?.role).toBe('user');
            expect(passedOpts.pendingUserMessage?.parts?.some((part) => part.text === userMessage)).toBe(true);
            expect(recordChatCompression).toHaveBeenCalledTimes(1);
            const recordPayload = recordChatCompression.mock.calls[0][0];
            expect(recordPayload.info).toEqual(expect.objectContaining({
                compressionStatus: CompressionStatus.COMPRESSED,
                newTokenCount: 40_000,
            }));
        ...
        it('rejects before request serialization when oversized resumed history cannot be compressed', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValue(new Error('Invalid string length'));
            await expect(chat.sendMessageStream('test-model', { message: 'continue' }, 'prompt-id-oversized-resume-guard')).rejects.toThrow(/compression status: COMPRESSION_FAILED_EMPTY_SUMMARY/i);
            expect(compressSpy).toHaveBeenCalledTimes(1);
        ...
        it('rejects before request serialization and restores history when hard-rescue compression is still oversized', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValue(new Error('Invalid string length'));
            await expect(chatWithRecording.sendMessageStream('test-model', { message: 'continue' }, 'prompt-id-oversized-after-compression')).rejects.toThrow(/compression status: COMPRESSED/i);
            expect(mockContentGenerator.generateContentStream).not.toHaveBeenCalled();
            expect(recordChatCompression).not.toHaveBeenCalled();
            expect(chatWithRecording.getLastPromptTokenCount()).toBe(176_999);
            expect(chatWithRecording.getHistory()[0].parts?.[0].text).toBe(originalHistory[0].parts?.[0].text);
        });
        it('rejects when compressed history is below hard but the pending user message pushes it over', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('should not send'));
            await expect(chatWithRecording.sendMessageStream('test-model', { message: 'x'.repeat(8_000) }, 'prompt-id-oversized-after-compression-and-user')).rejects.toThrow(/Estimated prompt tokens: 178000; hard limit: 177000/i);
            expect(mockContentGenerator.generateContentStream).not.toHaveBeenCalled();
            expect(recordChatCompression).not.toHaveBeenCalled();
        ...
        it('does not treat the image token estimate as output tokens after hard-rescue compression', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('sent after compression'));
            const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'x'.repeat(3_000) }, 'prompt-id-hard-rescue-image-estimate-slot');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(1);
        ...
        it('includes previous response output tokens in the hard-tier estimate', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                .mockResolvedValueOnce(makeStreamResponse('first', {
            ...
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime the token counters' }, 'prompt-prime-candidates');
            for await (const _ of firstStream) {
                /* consume */
            }
            const rescueStream = await chat.sendMessageStream('test-model', { message: 'small follow-up' }, 'prompt-hard-rescue-candidates');
            ...
            expect(compressSpy.mock.calls[1][1].force).toBe(true);
            expect(compressSpy.mock.calls[1][1].precomputedEffectiveTokens).toBeGreaterThanOrEqual(177_000);
        });
        it('does not double-count output tokens when prompt count falls back to total token count', async () => {
            ...
            chat.setLastPromptTokenCount(50_000);
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime fallback token counters' }, 'prompt-prime-total-token-fallback');
            for await (const _ of firstStream) {
                /* consume */
            }
            const secondStream = await chat.sendMessageStream('test-model', { message: 'small follow-up' }, 'prompt-total-token-fallback-follow-up');
            for await (const _ of secondStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
            expect(compressSpy.mock.calls[1][1].force).toBe(false);
            expect(compressSpy.mock.calls[1][1].precomputedEffectiveTokens).toBeLessThan(177_000);
        });
        it('includes previous response thought tokens in the hard-tier estimate', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('after rescue'));
            chat.setLastPromptTokenCount(50_000);
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime thought token counters' }, 'prompt-prime-thought-tokens');
            for await (const _ of firstStream) {
                /* consume */
            }
            const rescueStream = await chat.sendMessageStream('test-model', { message: 'small follow-up' }, 'prompt-hard-rescue-thought-tokens');
            for await (const _ of rescueStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('includes disjoint candidate and thought tokens when total token count is unavailable', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('after rescue'));
            chat.setLastPromptTokenCount(50_000);
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime disjoint output token counters' }, 'prompt-prime-disjoint-output-tokens');
            for await (const _ of firstStream) {
                /* consume */
            }
            const rescueStream = await chat.sendMessageStream('test-model', { message: 'small follow-up' }, 'prompt-hard-rescue-disjoint-output-tokens');
            for await (const _ of rescueStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('does not double-count OpenAI-compatible reasoning tokens already included in candidates', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('second'));
            chat.setLastPromptTokenCount(50_000);
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime OpenAI-compatible reasoning token counters' }, 'prompt-prime-openai-reasoning-tokens');
            for await (const _ of firstStream) {
                /* consume */
            }
            const secondStream = await chat.sendMessageStream('test-model', { message: 'small follow-up' }, 'prompt-openai-reasoning-follow-up');
            for await (const _ of secondStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('resets previous response output tokens when seeding last prompt tokens externally', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('second'));
            const firstStream = await chat.sendMessageStream('test-model', { message: 'collect candidates' }, 'prompt-collect-candidates');
            for await (const _ of firstStream) {
                /* consume */
            }
            chat.setLastPromptTokenCount(176_000);
            const secondStream = await chat.sendMessageStream('test-model', { message: 'seeded follow-up' }, 'prompt-seeded-after-candidates');
            for await (const _ of secondStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('resets previous response output tokens after successful compression', async () => {
            ...
            chat.setLastPromptTokenCount(50_000);
            const firstStream = await chat.sendMessageStream('test-model', { message: 'prime output tokens' }, 'prompt-prime-compression-reset');
            for await (const _ of firstStream) {
                /* consume */
            }
            const rescueStream = await chat.sendMessageStream('test-model', { message: 'trigger compression' }, 'prompt-compression-reset-rescue');
            for await (const _ of rescueStream) {
                /* consume */
            }
            const followUpStream = await chat.sendMessageStream('test-model', { message: 'after compression reset' }, 'prompt-after-compression-reset');
            for await (const _ of followUpStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(3);
            expect(compressSpy.mock.calls[1][1].force).toBe(true);
            expect(compressSpy.mock.calls[2][1].precomputedEffectiveTokens).toBeLessThan(100_000);
        });
        it('stops pre-send hard-rescue after repeated failed hard-tier compactions', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => makeStreamResponse('after failed rescue'));
            chat.setLastPromptTokenCount(176_999);
            for (let i = 0; i < MAX_CONSECUTIVE_FAILURES; i++) {
                await expect(chat.sendMessageStream('test-model', { message: `hard-rescue-${i}` }, `prompt-hard-rescue-bound-${i}`)).rejects.toThrow(/compression status: COMPRESSION_FAILED_EMPTY_SUMMARY/i);
            }
            const stream = await chat.sendMessageStream('test-model', { message: 'send after bounded hard-rescue failures' }, 'prompt-hard-rescue-after-failures');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(MAX_CONSECUTIVE_FAILURES);
            expect(compressSpy.mock.calls.map(([, opts]) => opts.force)).toEqual(Array(MAX_CONSECUTIVE_FAILURES).fill(true));
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(1);
            expect(mockDebugLoggerWarn).toHaveBeenCalledWith(expect.stringContaining('hard-tier rescue skipped'));
            expect(mockDebugLoggerWarn).toHaveBeenCalledWith(expect.stringContaining('prompt_id=prompt-hard-rescue-after-failures'));
        });
        it('falls back to reactive overflow recovery after the hard-rescue bound is exhausted', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                .mockRejectedValueOnce(new Error('prompt is too long: 180000 tokens > 128000 maximum'))
                .mockResolvedValueOnce(makeStreamResponse('after reactive fallback'));
            chat.setLastPromptTokenCount(176_999);
            for (let i = 0; i < MAX_CONSECUTIVE_FAILURES; i++) {
                await expect(chat.sendMessageStream('test-model', { message: `failed-hard-rescue-${i}` }, `prompt-hard-rescue-before-reactive-${i}`)).rejects.toThrow(/compression status: COMPRESSION_FAILED_EMPTY_SUMMARY/i);
            }
            const stream = await chat.sendMessageStream('test-model', { message: 'send after hard-rescue bound' }, 'prompt-hard-rescue-reactive-fallback');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(MAX_CONSECUTIVE_FAILURES + 1);
            expect(compressSpy.mock.calls
                .slice(0, MAX_CONSECUTIVE_FAILURES)
                .map(([, opts]) => opts.force)).toEqual(Array(MAX_CONSECUTIVE_FAILURES).fill(true));
            expect(compressSpy.mock.calls[MAX_CONSECUTIVE_FAILURES][1].force).toBe(true);
            expect(compressSpy.mock.calls[MAX_CONSECUTIVE_FAILURES][1].originalTokenCount).toBe(180_000);
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
        });
        it('does not count thrown hard-rescue attempts toward the retry bound', async () => {
            ...
            for (let i = 0; i < MAX_CONSECUTIVE_FAILURES + 1; i++) {
                await expect(chat.sendMessageStream('test-model', { message: `throwing-hard-rescue-${i}` }, `prompt-hard-rescue-throw-${i}`)).rejects.toThrow(compressionError);
            }
            expect(compressSpy).toHaveBeenCalledTimes(MAX_CONSECUTIVE_FAILURES + 1);
            expect(mockContentGenerator.generateContentStream).not.toHaveBeenCalled();
        });
        it('stops hard-rescue after repeated NOOP results are still oversized', async () => {
            ...
            chat.setLastPromptTokenCount(176_999);
            for (let i = 0; i < MAX_CONSECUTIVE_FAILURES; i++) {
                await expect(chat.sendMessageStream('test-model', { message: `noop-hard-rescue-${i}` }, `prompt-hard-rescue-noop-${i}`)).rejects.toThrow(/compression status: NOOP/i);
            }
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('after bounded noop hard-rescue'));
            const stream = await chat.sendMessageStream('test-model', { message: 'send after bounded noop hard-rescue' }, 'prompt-hard-rescue-after-noop-bound');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(MAX_CONSECUTIVE_FAILURES);
            ...
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(1);
            expect(mockDebugLoggerWarn).toHaveBeenCalledWith(expect.stringContaining('hard-tier rescue skipped'));
            expect(mockDebugLoggerWarn).toHaveBeenCalledWith(expect.stringContaining('prompt_id=prompt-hard-rescue-after-noop-bound'));
        });
        it('does not replace token counters when usage reports zero prompt tokens', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('zero prompt count', {
                promptTokenCount: 0,
                totalTokenCount: 5000,
            }));
            chat.setLastPromptTokenCount(123_456);
            const stream = await chat.sendMessageStream('test-model', { message: 'zero prompt count should not reseed' }, 'prompt-zero-count-no-reseed');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('ignores previous response output tokens when the prompt token count is zero', () => {
            ...
            expect(estimatePromptTokens(history, userMessage, 0, 9999)).toBe(estimateContentTokens([...history, userMessage]));
        });
        it('forwards latched consecutiveFailures into hard-rescue (no pre-call reset); success recovers via the post-call branch', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => makeStreamResponse());
            chat.setLastPromptTokenCount(50_000);
            for (let i = 0; i < MAX_CONSECUTIVE_FAILURES; i++) {
                const s = await chat.sendMessageStream('test-model', { message: `latch-${i}` }, `prompt-latch-${i}`);
            ...
            expect(compressSpy.mock.calls.at(-1)[1].consecutiveFailures).toBe(MAX_CONSECUTIVE_FAILURES - 1);
            ...
            chat.setLastPromptTokenCount(176_999);
            const rescueStream = await chat.sendMessageStream('test-model', { message: 'rescue me' }, 'prompt-hard-rescue-no-prereset');
            for await (const _ of rescueStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(1);
            ...
            expect(compressSpy.mock.calls[0][1].consecutiveFailures).toBe(MAX_CONSECUTIVE_FAILURES);
            ...
            chat.setLastPromptTokenCount(50_000);
            const followUpStream = await chat.sendMessageStream('test-model', { message: 'after recovery' }, 'prompt-hard-rescue-after-recovery');
            for await (const _ of followUpStream) {
                /* consume */
            }
            expect(compressSpy.mock.calls[0][1].consecutiveFailures).toBe(0);
        ...
        it('does not force when tokens are below hard threshold (normal auto path)', async () => {
            ...
            chat.setLastPromptTokenCount(50_000);
            const stream = await chat.sendMessageStream('test-model', { message: 'small message' }, 'prompt-id-hard-rescue-below');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(1);
        ...
        it('gates thresholds on the full window and clamps maxOutputTokens to the room left (issue #5950)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('clamped response'));
            const chatInstance = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
            chatInstance.setLastPromptTokenCount(170_000);
            const stream = await chatInstance.sendMessageStream('claude-sonnet-4-6', { message: 'hi' }, 'prompt-window-clamp-taper');
            for await (const _ of stream) {
                /* consume */
            }
            ...
            const requestConfig = vi.mocked(mockContentGenerator.generateContentStream).mock.calls[0][0].config;
            expect(requestConfig.maxOutputTokens).toBe(19_999);
            expect(170_000 + requestConfig.maxOutputTokens).toBeLessThanOrEqual(200_000);
        });
        it('sends the default ceiling when the window has room (unknown model → 32K)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('roomy response'));
            const chatInstance = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
            chatInstance.setLastPromptTokenCount(50_000);
            const stream = await chatInstance.sendMessageStream('test-model', { message: 'hi' }, 'prompt-window-clamp-ceiling');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('uses QWEN_CODE_MAX_OUTPUT_TOKENS as the ceiling when set', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('env ceiling response'));
                const chatInstance = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
                chatInstance.setLastPromptTokenCount(50_000);
                const stream = await chatInstance.sendMessageStream('test-model', { message: 'hi' }, 'prompt-window-clamp-env-ceiling');
                for await (const _ of stream) {
                    /* consume */
                }
                const requestConfig = vi.mocked(mockContentGenerator.generateContentStream).mock.calls[0][0].config;
        ...
        it('pads the first-send clamp estimate for unseen system/tool overhead', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('first send'));
            const chatInstance = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
            // lastPromptTokenCount deliberately left at 0 (fresh session).
            const stream = await chatInstance.sendMessageStream('test-model', { message: 'hi' }, 'prompt-first-send-clamp-pad');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('keeps a sane input budget on small custom windows (issue #6144)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('normal response'));
            const chatInstance = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
            chatInstance.setLastPromptTokenCount(6_276);
            const stream = await chatInstance.sendMessageStream('qwen3coder-64k', { message: 'hi' }, 'prompt-small-window-clamp');
            for await (const _ of stream) {
    ...
    describe('sendMessageStream with retries', () => {
        ...
        it('should fail after all retries on persistent invalid content and report metrics', async () => {
            ...
            try {
                ...
                expect(mockLogContentRetryFailure).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                    total_attempts: 5,
        ...
        it('should recover after four consecutive invalid streams', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-four-invalid-streams');
                ...
                for (const [index, retryDelayMs] of [
                    ...
                    expect(mockLogContentRetry).toHaveBeenNthCalledWith(index + 1, mockConfig, expect.objectContaining({
        ...
        ])('should retry $name through the OpenAI pipeline', async (testCase) => {
            vi.useFakeTimers();
            try {
                const create = vi.fn().mockImplementation(async () => (async function* () {
                    yield {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, `prompt-id-${testCase.errorType.toLowerCase()}-budget`);
                await expectStreamExhaustion(stream);
                expect(create).toHaveBeenCalledTimes(testCase.retryCount + 1);
                expect(mockLogContentRetry).toHaveBeenCalledTimes(testCase.retryCount);
                expect(mockLogContentRetryFailure).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
        ...
        it('keeps invalid stream retry budgets independent across error types', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-mixed-invalid-streams');
                const events = await collectStreamWithFakeTimers(stream, 15_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(4);
                expect(mockLogContentRetry).toHaveBeenCalledTimes(3);
                expect(mockLogContentRetry).toHaveBeenLastCalledWith(mockConfig, expect.objectContaining({
        ...
        it('surfaces an abort fired during the invalid-stream retry delay without retrying again', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streamResponse(stopResponse([])));
                const stream = await chat.sendMessageStream('test-model', { message: 'test', config: { abortSignal: abortController.signal } }, 'prompt-id-invalid-stream-abort-delay');
        ...
        it('should retry usage-only empty streams without recording failed attempts', async () => {
            ...
            try {
                ...
                const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-empty-usage-retry');
                const events = await collectStreamWithFakeTimers(stream);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(mockLogContentRetry).toHaveBeenCalledTimes(1);
                expect(recordAssistantTurn).toHaveBeenCalledTimes(1);
        ...
        it('rolls back the partial assistant turn when a retryable error fires after a tool_use chunk', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-rollback-on-retry');
                const iterator = stream[Symbol.asyncIterator]();
                ...
                expect(history.some((h) => h.parts?.some((p) => p.functionCall))).toBe(false);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('rolls back the partial assistant turn when an InvalidStreamError fires after a tool_use chunk on the transient-stream retry budget', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream)
                    .mockResolvedValueOnce(failingStream)
                    .mockResolvedValueOnce(successStream);
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-rollback-transient');
                const iterator = stream[Symbol.asyncIterator]();
                ...
                expect(history[0].role).toBe('user');
                expect(history[1].role).toBe('model');
                expect(history[1].parts.find((p) => p.text)?.text).toBe('Recovered on retry');
                expect(history.some((h) => h.parts?.some((p) => p.functionCall))).toBe(false);
            }
            finally {
                vi.useRealTimers();
        ...
        it('does not enter the fallback chain in unattended retry mode', async () => {
            ...
            try {
                ...
                const capacityError = Object.assign(new StreamContentError('{"error":{"code":"429","message":"Throttling: TPM(1/1)"}}'), { status: 429 });
                vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValueOnce(capacityError);
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-unattended-no-fallback');
                await expect((async () => {
                    for await (const _ of stream) {
                        /* consume */
                    }
        ...
        it('uses one exact image route across retries and filters history for the next target', async () => {
            const capacityError = Object.assign(new Error('temporarily unavailable'), {
                status: 503,
            });
            ...
            const routeSelector = 'openai:vision-agent\0https://vision.example.com/v1';
            ...
            const stream = await chat.sendMessageStream(selector, {
                message: [
                    { text: 'inspect' },
                    {
                        inlineData: {
            ...
            expect(mockContentGenerator.generateContentStream).not.toHaveBeenCalled();
            const routeRequest = JSON.stringify(routeGenerateContentStream.mock.calls.at(-1)?.[0]);
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValueOnce((async function* () {
                yield {
                    candidates: [
                        {
            ...
            const primaryStream = await chat.sendMessageStream('test-model', {
                message: [
            ...
            const primaryRequest = JSON.stringify(vi.mocked(mockContentGenerator.generateContentStream).mock
                .calls[0]?.[0]);
        ...
        it('tries the next fallback when a fallback emits only preparation metadata', async () => {
            ...
            const makeFallbackGenerator = (generateContentStream) => ({
                generateContent: vi.fn(),
            ...
            const resolveForModel = vi
                .fn()
                .mockResolvedValueOnce({
                contentGenerator: makeFallbackGenerator(fallbackAGenerateContentStream),
                ...
                .mockResolvedValueOnce({
                contentGenerator: makeFallbackGenerator(fallbackBGenerateContentStream),
            ...
            fallbackAGenerateContentStream.mockResolvedValueOnce((async function* () {
                yield preparationResponse;
                throw capacityError;
            })());
            fallbackBGenerateContentStream.mockResolvedValueOnce((async function* () {
                yield {
                    candidates: [
                        {
            ...
            expect(events.filter((event) => event.type === StreamEventType.MODEL_FALLBACK)).toHaveLength(2);
            expect(events.filter((event) => event.type === StreamEventType.MODEL_FALLBACK)).toEqual([
                {
            ...
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(1);
            expect(JSON.stringify(fallbackAGenerateContentStream.mock.calls[0]?.[0])).not.toContain('fallback-image');
        ...
        it('skips a fallback alias that resolves to the current model', async () => {
            ...
            const resolveForModel = vi
                .fn()
                .mockResolvedValueOnce({
                contentGenerator: makeFallbackGenerator(duplicateGenerateContentStream),
                retryAuthType: AuthType.USE_GEMINI,
                retryErrorCodes: undefined,
                model: 'resolved-primary-model',
            })
            ...
            const stream = await chat.sendMessageStream('requested-primary-model', { message: 'test' }, 'prompt-skip-resolved-duplicate-fallback');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
        ...
        it('skips an unresolvable fallback model and tries the next fallback', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-skip-bad-fallback');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            ...
            expect(events.filter((event) => event.type === StreamEventType.MODEL_FALLBACK)).toHaveLength(1);
            expect(fallbackBGenerateContentStream).toHaveBeenCalledTimes(1);
            expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                event.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                    'fallback-b ok')).toBe(true);
        ...
        it('does not try the next fallback after a fallback emits output', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-fallback-output-then-error');
            const events = [];
            await expect((async () => {
                for await (const event of stream) {
                    events.push(event);
            ...
            expect(history[0].role).toBe('user');
            expect(history.some((entry) => entry.parts?.some((part) => part.text === 'fallback-a partial'))).toBe(false);
        });
        it('surfaces an abort raised while resolving a fallback model', async () => {
            ...
            const capacityError = Object.assign(new StreamContentError('{"error":{"code":"429","message":"Throttling: TPM(1/1)"}}'), { status: 429 });
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValueOnce(capacityError);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-fallback-resolve-abort');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        it('surfaces an abort raised by a fallback stream without trying later fallbacks', async () => {
            ...
            const capacityError = Object.assign(new StreamContentError('{"error":{"code":"429","message":"Throttling: TPM(1/1)"}}'), { status: 429 });
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValueOnce(capacityError);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-fallback-stream-abort');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        it('does not fallback on non-eligible primary auth errors', async () => {
            ...
            const authError = Object.assign(new StreamContentError('{"error":{"code":"401","message":"Unauthorized"}}'), { status: 401 });
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValueOnce(authError);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-primary-auth-no-fallback');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        it('preserves primary partial tool calls when fallback is skipped after output', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-skipped-fallback-failure');
                const collecting = (async () => {
                    for await (const _ of stream) {
                        /* consume */
                    }
                })();
                const resultPromise = await expect(collecting).rejects.toBe(capacityError);
                await vi.advanceTimersByTimeAsync(0);
                await resultPromise;
                expect(resolveForModel).not.toHaveBeenCalled();
                const history = chat.getHistory();
        ...
        it('retries retryable transport stream errors and succeeds on a later attempt', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-retry');
                const events = await collectStreamWithFakeTimers(stream, 5_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(1);
                expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                    event.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                        'Recovered after transport retry')).toBe(true);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('stops retrying retryable transport stream errors after the retry budget is exhausted', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(() => Promise.resolve((async function* () {
                    throw transportError;
                    yield {};
                })()));
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-retry-exhausted');
                const events = [];
                ...
                expect(caughtError.message).toContain('terminated');
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(3);
                expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(2);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('does not retry retryable transport stream errors after yielding a chunk', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-no-retry-after-chunk');
            const events = [];
            await expect(async () => {
                for await (const event of stream) {
                    events.push(event);
            ...
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(1);
            expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(0);
            expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                event.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                    'Partial response before socket close')).toBe(true);
        });
        it('retries a transport stream error after yielding only tool preparation metadata', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-after-preparation');
                const events = await collectStreamWithFakeTimers(stream, 5_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(1);
            }
        ...
        it('falls back after yielding only tool preparation metadata', async () => {
            ...
            const fallbackGenerateContentStream = vi.fn().mockResolvedValue((async function* () {
                yield {
                    candidates: [
                        {
            ...
            const capacityError = Object.assign(new StreamContentError('{"error":{"code":"429","message":"Throttling"}}'), { status: 429 });
            const preparationResponse = {
                candidates: [{ content: { parts: [] } }],
            };
            setToolCallPreparations(preparationResponse, [
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-fallback-after-preparation');
            const events = [];
            for await (const event of stream)
                events.push(event);
        ...
        it.each([...RETRYABLE_STREAM_TRANSPORT_CODES])('retries a pre-first-chunk transport error carrying code %s', async (transportCode) => {
            vi.useFakeTimers();
            try {
                const transportError = Object.assign(new TypeError('terminated'), {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, `prompt-transport-${transportCode}`);
                const events = await collectStreamWithFakeTimers(stream, 5_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(1);
                expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                    event.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                        `Recovered from ${transportCode}`)).toBe(true);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('retries a transport error whose code is on the error itself (no cause)', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-direct-code');
                const events = await collectStreamWithFakeTimers(stream, 5_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
                expect(events.filter((event) => event.type === StreamEventType.RETRY)).toHaveLength(1);
            }
        ...
        it('does not retry a transport error that carries an HTTP 4xx status', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-4xx-no-retry');
            const events = [];
            await expect(async () => {
                for await (const event of stream) {
                    events.push(event);
        ...
        it('does not retry a transport code outside the stream allow-list', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-transport-not-allowlisted');
            const events = [];
            await expect(async () => {
                for await (const event of stream) {
                    events.push(event);
        ...
        it('surfaces an abort fired during the transport retry delay without retrying again', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test', config: { abortSignal: abortController.signal } }, 'prompt-transport-abort-delay');
                const iterator = stream[Symbol.asyncIterator]();
        ...
        it('rolls back the chat-recording entry too when the retry succeeds', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream)
                    .mockResolvedValueOnce(failingStream)
                    .mockResolvedValueOnce(successStream);
                const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-recording-rollback');
                const iterator = stream[Symbol.asyncIterator]();
                for (;;) {
                    const next = iterator.next();
                    await vi.advanceTimersByTimeAsync(60_000);
        ...
        it('flushes the chat-recording entry on the unretryable break path (kept partial → durable JSONL)', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValueOnce(failingStream);
                const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-recording-flush-on-break');
                const iterator = stream[Symbol.asyncIterator]();
                await expect((async () => {
                    for (;;) {
                ...
                expect(lastModelTurn?.parts?.some((p) => p.functionCall?.id === 'call_unretryable_kept')).toBe(true);
                ...
                const recordedMessage = recordAssistantTurn.mock.calls[0][0]
                    ?.message;
                expect(recordedMessage.some((p) => p.functionCall?.id === 'call_unretryable_kept')).toBe(true);
            }
            finally {
                vi.useRealTimers();
            }
    ...
    it('discards a completed protocol-tagged response and retries before persistence', async () => {
                ...
                candidates: [
                    {
                        content: {
                            parts: [
                                {
                                    text: 'lysis>failed scratch</analysis>' +
        ...
        const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-protocol-leak');
        const events = [];
        for await (const event of stream)
            events.push(event);
        expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
        ...
        expect(chatWithRecording.getLastModelMessageText()).toBe('Successful final response');
        expect(recordAssistantTurn).toHaveBeenCalledTimes(1);
        expect(recordAssistantTurn.mock.calls[0]?.[0].message).toEqual([
    ...
    it('does not reject normal HTML or protocol tag names in prose', async () => {
        const response = '<details><summary>Title</summary></details> ' +
        ...
        const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-protocol-literal');
        const events = [];
        for await (const event of stream)
            events.push(event);
        expect(events.some((event) => event.type === StreamEventType.RETRY)).toBe(false);
    ...
    it('retries a protocol-tagged turn even when the leaked attempt also contains a tool call', async () => {
        ...
        try {
            ...
            const stream = await chatWithRecording.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-protocol-leak-tool-call');
            const events = [];
            const iterator = stream[Symbol.asyncIterator]();
            for (;;) {
                const next = iterator.next();
            ...
            expect(history.some((turn) => turn.parts?.some((part) => part.functionCall))).toBe(false);
            expect(recordAssistantTurn).toHaveBeenCalledTimes(1);
            expect(recordAssistantTurn.mock.calls[0]?.[0].message).toEqual([
                { text: 'Successful final response' },
            ]);
    ...
    describe('repairOrphanedToolUseTurns', () => {
        ...
        it('handles parallel tool_use turns with only some responses present', () => {
            ...
            expect(history[2].parts[0].functionResponse?.response?.output).toBe('a-content');
        });
        ...
        it('repairs multiple non-adjacent dangling tool_uses across history', () => {
            ...
            expect(history[1].parts.some((p) => p.functionResponse?.id === 'early_orphan')).toBe(true);
        ...
        it('uses caller-provided reason text', () => {
            ...
            expect(fr?.response?.error).toBe('custom reason');
        });
        it('hoists the real functionResponse from a non-adjacent later user turn into the adjacent one', () => {
            ...
            expect(history[2].parts[0].functionResponse?.id).toBe('call_nonadjacent_real');
        ...
        it('synthesizes missing fr AND hoists real fr in a parallel tool_use mismatch', () => {
            ...
            expect(adjacentParts[0].functionResponse?.response
                ?.error).toBeDefined();
        ...
        it('drops duplicate functionResponse entries for the same callId across user turns', () => {
            ...
            const trailingHasDup = history
                .slice(3)
                .some((entry) => (entry.parts ?? []).some((part) => part.functionResponse?.id === 'cid_dup'));
        ...
        it('drops duplicate fr even when the canonical copy is already in the adjacent turn', () => {
            ...
            const allFrIds = history
                .slice(2)
                .flatMap((entry) => (entry.parts ?? []).map((p) => p.functionResponse?.id))
    ...
    describe('output token recovery', () => {
        ...
        it('re-clamps maxOutputTokens on each recovery send as the prompt grows (window invariant)', async () => {
            ...
            const stream = await chat.sendMessageStream('claude-sonnet-4-6', { message: 'hi' }, 'prompt-recovery-reclamp');
            for await (const _ of stream) {
                /* consume */
            }
            ...
            const calls = vi.mocked(mockContentGenerator.generateContentStream).mock
                .calls;
            const initialConfig = calls[0][0].config;
            const recoveryConfig = calls[1][0].config;
        ...
        it('re-clamps recovery sends even when an intermediate response omits usage metadata', async () => {
            ...
            const stream = await chat.sendMessageStream('claude-sonnet-4-6', { message: 'hi' }, 'prompt-recovery-stale-usage');
            for await (const _ of stream) {
                /* consume */
            }
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(3);
            const calls = vi.mocked(mockContentGenerator.generateContentStream).mock
                .calls;
            const recovery1Config = calls[1][0].config;
            const recovery2Config = calls[2][0].config;
        ...
        it('should enter recovery loop when escalated response is also truncated', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long essay' }, 'prompt-recovery');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
        ...
        it('retries protocol-tag leaks during max-tokens escalation', async () => {
            ...
            try {
                            ...
                            candidates: [
                                {
                                    content: {
                                        parts: [
                                            {
                                                text: 'lysis>discard escalated attempt</analysis>' +
                ...
                let callIndex = 0;
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
                const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long essay' }, 'prompt-escalation-protocol-retry');
                const events = [];
                const iterator = stream[Symbol.asyncIterator]();
                for (;;) {
                    const next = iterator.next();
                ...
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(3);
                expect(events.filter((event) => event.type === StreamEventType.RETRY).length).toBe(2);
                expect(chat.getLastModelMessageText()).toBe('Hello world');
                expect(JSON.stringify(chat.getHistory()).includes('DISCARD_ESCALATED_ATTEMPT')).toBe(false);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('preserves current user image bytes during output recovery', async () => {
            ...
            const recoveryRequest = vi.mocked(mockContentGenerator.generateContentStream).mock.calls[2]?.[0];
        ...
        it('should skip no-op escalation and recover directly for high-output models', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-direct-recovery');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            ...
            expect(retries[0].isContinuation).toBe(true);
            expect(retries[0]
                .maxOutputTokensEscalated).toBeUndefined();
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(2);
            const history = chat.getHistory();
        ...
        it('retries protocol-tag leaks during direct output recovery', async () => {
            ...
            try {
                            ...
                            candidates: [
                                {
                                    content: {
                                        parts: [
                                            {
                                                text: 'lysis>discard recovery attempt</analysis>' +
                ...
                let callIndex = 0;
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
                const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-direct-recovery-protocol-retry');
                const events = [];
                const iterator = stream[Symbol.asyncIterator]();
                for (;;) {
                    const next = iterator.next();
                ...
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(3);
                const retries = events.filter((event) => event.type === StreamEventType.RETRY);
                expect(retries).toHaveLength(2);
                expect(retries.map((event) => event.isContinuation)).toEqual([true, true]);
                expect(chat.getLastModelMessageText()).toBe('Hello world');
                expect(chat.getHistory()).toEqual([
        ...
        it('should coalesce overlapping recovery continuation text', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long essay' }, 'prompt-recovery-overlap');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should coalesce recovery text that replays a previous tail anchor', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long mermaid answer' }, 'prompt-recovery-contained-replay');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve prose continuation that coincidentally repeats an opener phrase', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write something' }, 'prompt-recovery-prose-opener');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should not strip prose that coincides with a far-earlier substring of the previous turn', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write something' }, 'prompt-recovery-far-prose');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve continuation when its structural prefix appears mid-paragraph in the previous tail (line-boundary rejection)', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write something with a heading' }, 'prompt-recovery-line-boundary-reject');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve prose continuation that opens with a single-cell pipe expression matching mid-tail', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'continue the derivation' }, 'prompt-recovery-single-cell-pipe-prose');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should insert a newline separator when the replayed prefix ends with newline but previous tail does not', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a structured answer' }, 'prompt-recovery-newline-normalization');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should drop continuation entirely when it exactly replays the previous tail', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write something' }, 'prompt-recovery-full-overlap');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should leave continuation untouched when the previous turn has no plain text', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write something' }, 'prompt-recovery-thought-only');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should dedup recovery continuation when the continuation begins with a thought part', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long essay' }, 'prompt-recovery-thinking-continuation');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should keep the recovery thought before the merged text part (thought-signature provenance)', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long essay' }, 'prompt-recovery-thinking-continuation-order');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve a coincidental 2-character CJK overlap (byte floor insufficient for CJK)', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: '帮我分析数据' }, 'prompt-recovery-cjk-floor');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should dedup a replayed structural prefix even when the continuation has leading whitespace', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a long markdown answer' }, 'prompt-recovery-leading-whitespace');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should truncate the previous_response_suffix to the trailing 1200 chars when the previous turn is longer', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a very long answer' }, 'prompt-recovery-tail-truncation');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
        ...
        it('should neutralize a literal previous_response_suffix delimiter inside the tail so the recovery prompt structure stays intact', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a response that contains my delimiter' }, 'prompt-recovery-delimiter-collision');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
        ...
        it('should neutralize a literal opening previous_response_suffix delimiter inside the tail', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a response that contains my opening delimiter' }, 'prompt-recovery-delimiter-collision-open');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
        ...
        it('should skip recovery when truncated turn has a functionCall', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'write a file' }, 'prompt-recovery-skip');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
        ...
        it('keeps protocol tag leak budget during output continuation', async () => {
            ...
            try {
                ...
                let callIndex = 0;
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
                const stream = await chat.sendMessageStream('gemini-pro', { message: 'essay' }, 'prompt-recovery-protocol-leak-budget');
        ...
        it('keeps continuation retry budgets independent across error types', async () => {
            ...
            try {
                ...
                let callIndex = 0;
                vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
                const stream = await chat.sendMessageStream('gemini-pro', { message: 'essay' }, 'prompt-recovery-mixed-invalid-streams');
                const events = await collectStreamWithFakeTimers(stream, 15_000);
                expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(6);
                expect(mockLogContentRetry).toHaveBeenCalledTimes(3);
                expect(mockLogContentRetry).toHaveBeenLastCalledWith(mockConfig, expect.objectContaining({
                    attempt_number: 0,
                    error_type: 'PROTOCOL_TAG_LEAK',
        ...
        it('should cap recovery attempts at MAX_OUTPUT_RECOVERY_ATTEMPTS (3)', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'infinite loop test' }, 'prompt-recovery-cap');
            // Consume
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('should pop dangling recovery message and emit STOP chunk when recovery throws', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'recovery fails' }, 'prompt-recovery-fail');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
        ...
        it('should pop both the partial model turn AND the recovery user message when recovery throws after a functionCall', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'recovery throws after functionCall' }, 'prompt-recovery-fc-throw');
            ...
            const stillHasPartialFc = history.some((entry) => (entry.parts ?? []).some((part) => part.functionCall?.id === 'call_recovery_throw'));
        ...
        it('should stop recovery mid-loop when a later iteration emits functionCall', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'mixed recovery' }, 'prompt-recovery-mixed');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('should coalesce successful recovery iterations into the preceding model turn', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-pro', { message: 'essay' }, 'prompt-recovery-coalesce');
            for await (const _ of stream) {
                /* consume */
            }
            const history = chat.getHistory();
        ...
        it('rolls back an escalated partial tool call when the stream fails', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chatWithRecording.sendMessageStream('gemini-pro', { message: 'kick off' }, 'prompt-escalation-flush');
            ...
            const recordedHasPartial = recordAssistantTurn.mock.calls.some((call) => {
                ...
                return message?.some((p) => p.functionCall?.id === 'call_escalation_throw');
            });
    ...
    describe('compression failure circuit breaker', () => {
        ...
        it('tolerates MAX_CONSECUTIVE_FAILURES - 1 failures and increments the counter each time', async () => {
            ...
            expect(compressSpy.mock.calls[MAX_CONSECUTIVE_FAILURES][1].consecutiveFailures).toBe(MAX_CONSECUTIVE_FAILURES);
        });
