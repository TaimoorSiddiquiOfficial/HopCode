...
import { getCoreSystemPrompt, getCustomSystemPrompt, getPlanModeSystemReminder, resolvePathFromEnv, getCompressionPrompt, resolveInteractionMode, } from './prompts.js';
...
describe('Core System Prompt (prompts.ts)', () => {
    beforeEach(() => {
        vi.resetAllMocks();
        vi.stubEnv('HOPCODE_SYSTEM_MD', undefined);
        vi.stubEnv('HOPCODE_WRITE_SYSTEM_MD', undefined);
    });
    ...
    it('instructs the model not to bypass denied tool calls through equivalent paths', () => {
        ...
        expect(prompt).toContain('genuinely safer alternative that does not accomplish the denied action');
        expect(prompt).toContain('request explicit approval only when the current interaction mode can receive it');
    });
    it.each([
        [
            'interactive',
    ...
    ])('aligns the system prompt with %s mode', (mode, role, questionGuidance) => {
        vi.stubEnv('SANDBOX', undefined);
    ...
    it('does not tell headless runs to wait for user input', () => {
        ...
        const prompt = getCoreSystemPrompt(undefined, undefined, undefined, 'headless');
        ...
        expect(prompt).not.toContain('ask for clarification or confirmation where needed');
        expect(prompt).not.toMatch(/Use 'ask_user_question' when you need/);
        expect(prompt.lastIndexOf('This is a non-interactive, single-turn run')).toBeGreaterThan(prompt.lastIndexOf('# Examples'));
    });
    it('instructs the model to preserve unrelated existing work', () => {
        vi.stubEnv('SANDBOX', undefined);
        vi.mocked(isGitRepository).mockReturnValue(true);
        const prompt = getCoreSystemPrompt();
        expect(prompt).toContain('Treat existing or unexpected changes as user-owned');
        expect(prompt).toContain('Do not modify, stage, commit, or revert unrelated changes');
        expect(prompt).toContain('Stage only paths that belong to the requested change');
        expect(prompt).toContain('Do not use broad staging commands such as `git add -A` when unrelated changes are present');
    });
    it('does not tell the model to enter plan mode without user opt-in', () => {
        vi.stubEnv('SANDBOX', undefined);
        const prompt = getCoreSystemPrompt();
        expect(prompt).toContain('Do not enter plan mode or call enter_plan_mode on your own');
        expect(prompt).toContain('Use plan mode only when the user explicitly asks you to switch to plan mode');
        expect(prompt).not.toContain('When the work requires a shared plan before execution, enter plan mode');
    });
    it('uses todos selectively and keeps plans outcome-oriented', () => {
        ...
        expect(prompt).toContain('rather than one item per error, file, command, or minor edit');
    ...
    it('adapts final response detail to the request', () => {
        vi.stubEnv('SANDBOX', undefined);
        const prompt = getCoreSystemPrompt();
        expect(prompt).toContain('Final responses should be concise by default, but their shape and depth must match the request');
        expect(prompt).toContain('For code reviews, explanations, investigations, or substantial changes');
        expect(prompt).toContain('complex findings may require several paragraphs or sections');
    ...
    describe('HOPCODE_SYSTEM_MD environment variable', () => {
        it('should use default prompt when HOPCODE_SYSTEM_MD is "false"', () => {
            vi.stubEnv('HOPCODE_SYSTEM_MD', 'false');
            const prompt = getCoreSystemPrompt();
            expect(fs.readFileSync).not.toHaveBeenCalled();
            expect(prompt).not.toContain('custom system prompt');
        });
        it('should use default prompt when HOPCODE_SYSTEM_MD is "0"', () => {
            vi.stubEnv('HOPCODE_SYSTEM_MD', '0');
            const prompt = getCoreSystemPrompt();
            expect(fs.readFileSync).not.toHaveBeenCalled();
            expect(prompt).not.toContain('custom system prompt');
        });
        it('should throw error if HOPCODE_SYSTEM_MD points to a non-existent file', () => {
            const customPath = '/non/existent/path/system.md';
            vi.stubEnv('HOPCODE_SYSTEM_MD', customPath);
        ...
        it('should read from custom path when HOPCODE_SYSTEM_MD provides one, preserving case', () => {
        ...
        it('should expand tilde in custom path when HOPCODE_SYSTEM_MD is set', () => {
    ...
    describe('HOPCODE_WRITE_SYSTEM_MD environment variable', () => {
        it('should not write to file when HOPCODE_WRITE_SYSTEM_MD is "false"', () => {
            vi.stubEnv('HOPCODE_WRITE_SYSTEM_MD', 'false');
            getCoreSystemPrompt();
            expect(fs.writeFileSync).not.toHaveBeenCalled();
        });
        it('should not write to file when HOPCODE_WRITE_SYSTEM_MD is "0"', () => {
            vi.stubEnv('HOPCODE_WRITE_SYSTEM_MD', '0');
            getCoreSystemPrompt();
            expect(fs.writeFileSync).not.toHaveBeenCalled();
        });
        ...
        it('should write to custom path when HOPCODE_WRITE_SYSTEM_MD provides one', () => {
            const customPath = path.resolve('/custom/path/system.md');
            vi.stubEnv('HOPCODE_WRITE_SYSTEM_MD', customPath);
            getCoreSystemPrompt();
            expect(fs.writeFileSync).toHaveBeenCalledWith(customPath, expect.any(String));
        });
        it('should expand tilde in custom path when HOPCODE_WRITE_SYSTEM_MD is set', () => {
            const homeDir = '/Users/test';
            vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
            const customPath = '~/custom/system.md';
            const expectedPath = path.join(homeDir, 'custom/system.md');
        ...
        it('should expand tilde in custom path when HOPCODE_WRITE_SYSTEM_MD is just ~', () => {
            const homeDir = '/Users/test';
            vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
            const customPath = '~';
...
describe('Model-specific tool call formats', () => {
    ...
    it('should use JSON format for qwen-vl model', () => {
        vi.mocked(isGitRepository).mockReturnValue(false);
        const prompt = getCoreSystemPrompt(undefined, 'hopcode-vl-max');
    ...
    it('should preserve model-specific formats with sandbox environment', () => {
        ...
        vi.mocked(isGitRepository).mockReturnValue(false);
        const prompt = getCoreSystemPrompt(undefined, 'hopcode-vl-plus');
        // Should contain JSON-style tool calls
        expect(prompt).toContain('{"name": "run_shell_command"');
    ...
    it('should use native Gemma 4 format for gemma4 models', () => {
        ...
        const prompt = getCoreSystemPrompt(undefined, 'unsloth/gemma-4-26B-A4B-it-qat');
        // Should contain Gemma native token boundaries and quotes
        expect(prompt).toContain('<|tool_call>call:run_shell_command');
        expect(prompt).toContain('{command:<|"|>node server.js<|"|>,is_background:true}<tool_call|>');
...
describe('New Applications workflow deferred to skill', () => {
    ...
    it('system prompt does not contain the full New Applications workflow', () => {
        vi.mocked(isGitRepository).mockReturnValue(false);
        const prompt = getCoreSystemPrompt();
        expect(prompt).not.toContain('Autonomously implement and deliver a visually appealing');
...
describe('getCompressionPrompt', () => {
    ...
    it('does NOT include the resume trailer in the prompt body', () => {
        ...
        const prompt = getCompressionPrompt();
        expect(prompt).not.toMatch(/resume.*directly|continue the conversation from where it left off/i);
    });
});
describe('resolveInteractionMode', () => {
    ...
    it("resolves a stream-json session to 'acp' so the model may still ask questions", () => {
        ...
        expect(resolveInteractionMode(makeConfig({ inputFormat: InputFormat.STREAM_JSON }))).toBe('acp');
    });
    it("resolves an interactive text session to 'interactive'", () => {
        expect(resolveInteractionMode(makeConfig({ inputFormat: InputFormat.TEXT, interactive: true }))).toBe('interactive');
    });
    it("resolves a non-interactive text session to 'headless'", () => {
        expect(resolveInteractionMode(makeConfig({ inputFormat: InputFormat.TEXT, interactive: false }))).toBe('headless');
    });
    it("prefers 'acp' over 'interactive' for a stream-json session (ACP precedence)", () => {
        expect(resolveInteractionMode(makeConfig({ inputFormat: InputFormat.STREAM_JSON, interactive: true }))).toBe('acp');
    });
