...
import { getArenaSystemReminder, getCoreSystemPrompt, getCustomSystemPrompt, getPlanModeSystemReminder, resolveInteractionMode, } from './prompts.js';
...
export class GeminiClient {
    ...
    async initialize(sessionStartSource) {
        ...
        if (resumedSessionData) {
            ...
            if (resumeTokenCounts) {
                chat.seedResumeTokenCounts(resumeTokenCounts.promptTokenCount, resumeTokenCounts.outputTokenCount);
            }
            else {
    ...
    getMainSessionSystemInstruction() {
        ...
        if (overrideSystemPrompt) {
            const base = getCustomSystemPrompt(overrideSystemPrompt, userMemory, appendSystemPrompt);
            return gitStatus ? base + '\n\n' + gitStatus : base;
        }
        const base = getCoreSystemPrompt(userMemory, this.config.getModel(), appendSystemPrompt, resolveInteractionMode(this.config));
        return gitStatus ? base + '\n\n' + gitStatus : base;
    }
     ...
     */
    resolveDeferredToolsForReminder() {
        const toolRegistry = this.config.getToolRegistry();
    ...
    async tryCompressChat(prompt_id, force = false, signal, customInstructions) {
        ...
        const info = await this.getChat().tryCompress(prompt_id, model, force, signal, customInstructions ? { customInstructions } : undefined);
