...
async function* mockTextStream(chunks, usage) {
...
describe('BaseLlmClient', () => {
    ...
    describe('generateText - streaming', () => {
        it('routes through generateContentStream, concatenates deltas, trims once, and captures final-chunk usage', async () => {
            ...
            mockGenerateContentStream.mockImplementation(async () => mockTextStream(['  Hello', ', ', 'world  '], usage));
        ...
        it('drops thought parts and tolerates a stream that omits usage', async () => {
            async function* streamWithThought() {
            ...
            mockGenerateContentStream.mockImplementation(async () => streamWithThought());
            const result = await client.generateText({
                contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
                model: 'test-model',
                abortSignal: abortController.signal,
        ...
        it('does not stream when stream is omitted (non-streaming path, still trimmed)', async () => {
            mockGenerateContent.mockResolvedValue(createMockTextResponse('  plain  '));
            const result = await client.generateText({
                contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
                model: 'test-model',
                abortSignal: abortController.signal,
        ...
        it('surfaces an abort that fires mid-stream and skips error reporting', async () => {
            ...
            mockGenerateContentStream.mockImplementation(async () => abortingStream());
        ...
        it('returns an empty result for a stream that yields no chunks', async () => {
            ...
            mockGenerateContentStream.mockImplementation(async () => mockTextStream([]));
            const result = await client.generateText({
                contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
                model: 'test-model',
                abortSignal: abortController.signal,
    ...
    describe('per-model resolution', () => {
        ...
        beforeEach(() => {
            ...
            crossProviderConfig = {
                ...
                getAllConfiguredModels: vi.fn((authTypes) => authTypes?.includes(AuthType.HOPCODE_OAUTH)
                    ? []
                    : [
                        {
                            id: fastModel,
        ...
        it('does not confuse a qualified cross-provider namesake with the primary', async () => {
            ...
            getResolvedModel.mockImplementation((authType, model) => authType === AuthType.USE_ANTHROPIC && model === 'shared-model'
                ? {
                    id: 'shared-model',
                    authType: AuthType.USE_ANTHROPIC,
                    baseUrl: '',
                ...
                : undefined);
            const resolved = await new BaseLlmClient(mockContentGenerator, crossProviderConfig).resolveForModel('anthropic:shared-model', { failClosed: true });
            expect(resolved.contentGenerator).toBe(fastContentGenerator);
        ...
        it('keeps explicit vision capability on the resolved generator config', async () => {
            getResolvedModel.mockImplementation((authType, model) => authType === AuthType.USE_ANTHROPIC && model === fastModel
                ? {
                    id: fastModel,
                    authType: AuthType.USE_ANTHROPIC,
                    baseUrl: 'https://api.anthropic.com',
            ...
            const resolved = await new BaseLlmClient(mockContentGenerator, crossProviderConfig).resolveForModel(fastModel, { failClosed: true });
            expect(resolved.contentGeneratorConfig.modalities?.image).toBe(true);
            expect(mockCreateContentGenerator).toHaveBeenCalledWith(expect.objectContaining({ modalities: { image: true } }), crossProviderConfig);
        });
        it('resolves same-id model selectors by baseUrl when provided', async () => {
            const selectedBaseUrl = 'https://token-plan.example.com/v1';
            getResolvedModel.mockImplementation((authType, model, baseUrl) => {
            ...
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            const resolved = await c.resolveForModel(`openai:qwen3.7-plus\0${selectedBaseUrl}`);
            expect(resolved.contentGenerator).toBe(fastContentGenerator);
            expect(getResolvedModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'qwen3.7-plus', selectedBaseUrl);
            expect(mockBuildAgentContentGeneratorConfig).toHaveBeenCalledWith(crossProviderConfig, 'qwen3.7-plus', expect.objectContaining({
                authType: AuthType.USE_OPENAI,
                baseUrl: selectedBaseUrl,
            }));
        });
        it('threads baseUrl through bare model registry lookups', async () => {
            ...
            expect(getResolvedModel).toHaveBeenCalledWith(AuthType.QWEN_OAUTH, 'qwen3.7-plus', selectedBaseUrl);
        });
        ...
        it('fails closed (throws) for an unregistered model when failClosed is set', async () => {
            ...
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            await expect(c.resolveForModel('ghost-model', { failClosed: true })).rejects.toThrow(/not registered/i);
            expect(mockCreateContentGenerator).not.toHaveBeenCalled();
        });
        it('fails closed when the requested baseUrl does not match any registered model', async () => {
            ...
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            await expect(c.resolveForModel('openai:real-model\0https://wrong-url.example.com', {
                failClosed: true,
            })).rejects.toThrow('Model "openai:real-model" at baseUrl "https://wrong-url.example.com" is not registered');
            expect(mockCreateContentGenerator).not.toHaveBeenCalled();
        });
        it('fails closed (throws) when generator creation fails and failClosed is set', async () => {
            getResolvedModel.mockImplementation((authType, model) => authType === AuthType.USE_ANTHROPIC && model === fastModel
                ...
                : undefined);
            mockCreateContentGenerator.mockRejectedValue(new Error('missing credential'));
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            await expect(c.resolveForModel(fastModel, { failClosed: true })).rejects.toThrow(/missing credential/i);
        });
        ...
        it('streams through a per-model generator resolved by model (compression path)', async () => {
            ...
            fastGenerateContentStream.mockImplementation(async () => mockTextStream(['fast ', 'stream'], usage));
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            ...
            expect(fastGenerateContentStream).toHaveBeenCalledWith(expect.objectContaining({
                model: fastModel,
                contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
                config: expect.objectContaining({
                    abortSignal: abortController.signal,
