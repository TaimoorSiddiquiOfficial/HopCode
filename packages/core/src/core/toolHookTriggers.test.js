...
import { generateToolUseId, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, firePostToolBatchHook, fireNotificationHook, appendAdditionalContext, firePermissionRequestHook, } from './toolHookTriggers.js';
...
describe('toolHookTriggers', () => {
    ...
    describe('firePostToolBatchHook', () => {
        ...
        it('should send resolved tool calls and return additional context', async () => {
            ...
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                ...
                signal: undefined,
            }, MessageBusType.HOOK_EXECUTION_RESPONSE, 15_000, undefined);
            expect(result).toEqual({
                shouldStop: false,
                additionalContext: 'batch note',
            });
        ...
        it('should return hookError when hook execution fails without an error message', async () => {
            ...
            expect(result.shouldStop).toBe(false);
            expect(result.hookError).toMatch(/success: false/);
            expect(debugLoggerWarnSpy).toHaveBeenCalledWith(expect.stringContaining('PostToolBatch hook returned failure'));
        });
        ...
        it('should return hookError when messageBus.request throws', async () => {
            const mockMessageBus = createMockMessageBus();
            mockMessageBus.request.mockRejectedValue(new Error('bus timeout'));
            const result = await firePostToolBatchHook(mockMessageBus, []);
            expect(result.shouldStop).toBe(false);
    ...
    describe('fireNotificationHook', () => {
        ...
        it('should send correct parameters to MessageBus for permission_prompt', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'HopCode needs your permission to use Bash', NotificationType.PermissionPrompt, 'Permission needed');
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
        ...
        it('should send correct parameters to MessageBus for idle_prompt', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'HopCode is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input');
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
    ...
    describe('firePermissionRequestHook', () => {
        ...
        it('should handle different permission modes', async () => {
            ...
            const result2 = await firePermissionRequestHook(mockMessageBus, 'test-tool', {}, 'izn');
