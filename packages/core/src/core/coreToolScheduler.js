...
import { truncateLlmContent, truncateToolOutput, TOOL_OUTPUT_TRUNCATED_PREFIX, } from '../utils/truncation.js';
...
import { applyAutoModeDecision, evaluateAutoMode, getAutoModePermissionDeniedReason, shouldClassifyAllShellForAutoMode, shouldForceAutoModeReviewForAllow, shouldFirePermissionDeniedForAutoMode, shouldRunAutoModeForCall, } from '../permissions/autoMode.js';
...
function batchResponseOutputSize(call) {
    let size = 0;
...
const createErrorResponse = (request, error, errorType, artifacts, resultDisplay) => ({
...
const createCancelledResponse = (request, reason, artifacts) => {
...
function withPostToolBatchStop(completedCalls, stopReason) {
    ...
    calls[calls.length - 1] = {
        ...
        tool: lastCall.tool,
        response: createErrorResponse(lastCall.request, new Error(stopReason), ToolErrorType.EXECUTION_DENIED),
...
export class CoreToolScheduler {
    ...
    setArgsInternal(targetCallId, args) {
        this.toolCalls = this.toolCalls.map((call) => {
            ...
            const invocationOrError = this.buildInvocation(call.tool, args, targetCallId, call.request.prompt_id);
    ...
    async _schedule(request, signal, runtimeView) {
        ...
        try {
            ...
            const requestsToProcess = dedupeRequestsByCallId(Array.isArray(request) ? request : [request]);
            ...
            for (const reqInfo of requestsToProcess) {
                ...
                if (reqInfo.wasOutputTruncated && toolInstance.kind === Kind.Edit) {
                    const count = this.recordRetryableToolError(reqInfo.name, TRUNCATION_EDIT_REJECTION);
                ...
                const invocationOrError = this.buildInvocation(toolInstance, reqInfo.args, reqInfo.callId, reqInfo.prompt_id);
                if (invocationOrError instanceof Error) {
                    ...
                    const count = this.recordRetryableToolError(reqInfo.name, invocationOrError.message);
            ...
            for (const toolCall of newToolCalls) {
                ...
                try {
                    ...
                    if (isPlanRequiredTeammateAwaitingApproval(this.config) &&
                        ...
                        this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(message), ToolErrorType.EXECUTION_DENIED));
                    ...
                    if (!needsConfirmation(confirmationPermission, approvalMode, canonicalName, requiresUserInteraction)) {
                        this.setToolCallOutcome(reqInfo.callId, ToolConfirmationOutcome.ProceedAlways);
                        this.setStatusInternal(reqInfo.callId, 'scheduled');
                    }
                    else {
                        ...
                        if (isPlanModeBlocked(isPlanMode, isExitPlanModeTool, isAskUserQuestionTool, confirmationDetails, isEnterPlanModeTool)) {
                            ...
                            this.setStatusInternal(reqInfo.callId, 'error', {
                                ...createErrorResponse(reqInfo, planModeError, ToolErrorType.EXECUTION_DENIED),
                        ...
                        if (hooksEnabled && messageBus) {
                            fireNotificationHook(messageBus, `HopCode needs your permission to use ${reqInfo.name}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
    ...
    async runConcurrently(calls, signal) {
        const maxConcurrency = parsePositiveIntegerEnv(process.env['HOPCODE_CODE_MAX_TOOL_CONCURRENCY'], 10);
    ...
    bounceToAwaitingApprovalForAsk(scheduledCall, reason, toolSpan, signal) {
        ...
        if (!this.config.getDisableAllHooks() && messageBus) {
            fireNotificationHook(messageBus, `HopCode needs your permission to use ${toolName}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
                debugLogger.warn(`Permission prompt notification hook failed: ${error instanceof Error ? error.message : String(error)}`);
            });
        }
    }
    safelyAddToolInputAttributes(span, toolName, toolInput) {
        try {
    ...
    safelyAddToolResultAttributes(span, toolName, toolResult) {
        try {
    ...
    async _executeToolCallBody(scheduledCall, signal, span) {
        ...
        if (this.config.getTelemetryIncludeSensitiveSpanAttributes?.()) {
            this.safelyAddToolInputAttributes(span, toolName, safeJsonStringify(toolInput) ?? '{}');
        }
        ...
        if (hooksEnabled && messageBus && !isPostAskReexecution) {
            ...
            if (!preHookResult.shouldProceed) {
                ...
                this.safelyAddToolResultAttributes(span, toolName, `BLOCKED: ${blockMessage}`);
        ...
        // are skipped.
        const sleepInhibitorHandle = acquireSleepInhibitor(this.config, `HopCode is executing tool ${canonicalName}`);
        ...
        try {
            ...
            const toolExecutionTimeoutMs = Math.min(parsePositiveIntegerEnv(process.env['HOPCODE_CODE_TOOL_EXECUTION_TIMEOUT_MS'], 0), 2_147_483_647);
            ...
            if (invocation instanceof ShellToolInvocation) {
                ...
                promise = invocation.execute(execSignal, liveOutputCallback, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback, canPromoteForegroundShell);
            }
            else {
                promise = invocation.execute(execSignal, liveOutputCallback, shellExecutionConfig);
            }
            let toolResult;
            ...
            if (aborted) {
                ...
                this.safelyAddToolResultAttributes(span, toolName, `CANCELLED: ${cancelMessage}`);
                this.setStatusInternal(callId, 'cancelled', failureHookArtifacts && failureHookArtifacts.length > 0
                    ? createCancelledResponse(scheduledCall.request, cancelMessage, failureHookArtifacts)
                    : cancelMessage);
                setToolSpanCancelled(span);
            ...
            if (toolResult.error === undefined) {
                ...
                if (hooksEnabled && messageBus) {
                    ...
                    if (postHookResult.shouldStop) {
                        ...
                        this.safelyAddToolResultAttributes(span, toolName, `STOPPED: ${stopMessage}`);
                        this.setStatusInternal(callId, 'error', errorResponse);
                ...
                if (this.config.getTelemetryIncludeSensitiveSpanAttributes?.()) {
                    this.safelyAddToolResultAttributes(span, toolName, typeof content === 'string'
                        ? content
                ...
                if (isTimeout) {
                    ...
                    let responseParts = convertToFunctionErrorResponse(toolName, callId, timeoutContent, operationalErrorMessage);
                    ...
                    this.safelyAddToolResultAttributes(span, toolName, `ERROR: ${operationalErrorMessage}`);
                    const artifacts = [
                        ...(toolResult.artifacts ?? []),
                ...
                this.safelyAddToolResultAttributes(span, toolName, `ERROR: ${errorMessage}`);
                const error = new Error(errorMessage);
                const errorResponse = createErrorResponse(scheduledCall.request, error, toolResult.error.type, failureHookArtifacts, typeof toolResult.returnDisplay === 'string'
                    ? undefined
            ...
            if (aborted) {
                ...
                this.safelyAddToolResultAttributes(span, toolName, `EXCEPTION: ${exceptionErrorMessage}`);
                this.setStatusInternal(callId, 'error', createErrorResponse(scheduledCall.request, executionError instanceof Error
                    ...
                    : new Error(String(executionError)), ToolErrorType.UNHANDLED_EXCEPTION, failureHookArtifacts));
    ...
    async checkAndNotifyCompletion() {
        ...
        if (this.toolCalls.length > 0 && allCallsAreTerminal) {
            ...
            let messageBus;
            try {
                const shouldFirePostToolBatch = !this.config.getDisableAllHooks() &&
            ...
            try {
                if (messageBus) {
                    ...
                    const permissionMode = this.config.getApprovalMode();
                    const batchHookResult = await this.withHookSpan({ hookEvent: 'PostToolBatch', toolName: 'batch' }, () => firePostToolBatchHook(messageBus, batchToolCalls, permissionMode, batchSignal), (r) => r.hookError
                        ? {
                            success: false,
                            error: r.hookError,
                            shouldStop: false,
                    ...
                    if (batchHookResult.shouldStop) {
                        ...
                        completedCalls = withPostToolBatchStop(completedCalls, batchHookResult.stopReason ||
    ...
    async maybePersistLargeToolResult(callId, toolName, content) {
        if (GATE_EXEMPT_TOOLS.has(toolName))
            return content;
    ...
    async applyBatchOutputBudget(completedCalls) {
        ...
        if (offloaded > 0) {
            debugLogger.info(`Batch output budget (${budget} chars): offloaded ${offloaded} largest result(s) to disk.`);
        }
        if (total > budget) {
            ...
            debugLogger.warn(`Batch output budget (${budget} chars) still exceeded after offloading ${offloaded}: ${total} chars across ${completedCalls.length} result(s).`);
        }
        return result;
    }
    /**
    ...
    async offloadCallOutput(call) {
        ...
        let truncated;
        try {
            truncated = await truncateToolOutput(this.config, call.request.name, content, { threshold: BATCH_OFFLOAD_PREVIEW_CHARS }, call.request.prompt_id);
        }
        catch {
                ...
                if (finalPermission === 'allow' && forceAutoReviewForAllow) {
                    ...
                    switch (outcome.kind) {
                        case 'approved':
                            ...
                            break;
                        case 'blocked': {
                            this.setStatusInternal(pendingTool.request.callId, 'error', createErrorResponse(pendingTool.request, new Error(outcome.errorMessage), ToolErrorType.EXECUTION_DENIED));
                        ...
                        case 'fallback':
                            if (fallback.fallback) {
                                ...
                                debugLogger.warn(`Auto mode fallback for pending tool (${fallback.reason}): consecutiveBlock=${denialState.consecutiveBlock}, consecutiveUnavailable=${denialState.consecutiveUnavailable}`);
                            }
                            break;
                        default: {
                            const _exhaustive = outcome;
