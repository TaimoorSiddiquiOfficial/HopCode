...
export function clampOutputTokensToWindow(outputCeiling, contextWindowSize, promptTokens) {
    ...
    // HOPCODE_CODE_MAX_OUTPUT_TOKENS=2000 on a capacity-constrained backend) must
...
export function normalize(model) {
    ...
    if (!s.match(/^(?:qwen|hopcode)-(?:plus|flash|vl-max)-latest$/) &&
        !s.match(/^kimi-k2-\d{4}$/)) {
        // Regex breakdown:
...
const OUTPUT_PATTERNS = [
    ...
    // Anthropic Claude
    [/^claude-opus-4-(?:6|7|8)/, 128_000], // Opus 4.6-4.8: 128K
