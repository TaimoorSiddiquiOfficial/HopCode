...
vi.mock('./session-start-profiler.js', () => ({
    createSessionStartProfiler: sessionStartProfilerMocks.createSessionStartProfiler,
}));
...
import { buildChangedAgentsReminder, buildChangedMcpToolsReminder, buildChangedSkillsReminder, getInitialChatHistory, } from '../utils/environmentContext.js';
...
import { clearCacheSafeParams, getCacheSafeParams, } from '../utils/forkedAgent.js';
...
vi.mock('../utils/environmentContext', async (importOriginal) => {
    ...
    return {
        ...
        buildChangedMcpToolsReminder: vi.fn((tools, removedToolNames) => tools.length === 0 && removedToolNames.length === 0
            ? null
            : `<system-reminder>\nchanged mcp: added=${tools.map((tool) => tool.name).join(', ')} removed=${removedToolNames.join(', ')}\n</system-reminder>`),
        buildChangedSkillsReminder: vi.fn((entries, removedNames) => entries.length === 0 && removedNames.length === 0
            ? null
            : `<system-reminder>\nchanged skills: added=${entries.map((entry) => entry.name).join(', ')} removed=${removedNames.join(', ')}\n</system-reminder>`),
        buildChangedAgentsReminder: vi.fn((addedAgents, removedAgentNames) => addedAgents.length === 0 && removedAgentNames.length === 0
            ? null
...
vi.mock('../utils/debugLogger.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        createDebugLogger: (namespace) => namespace === 'CLIENT'
            ? mockClientDebugLogger
            : actual.createDebugLogger(namespace),
    };
});
vi.mock('../services/microcompaction/microcompact.js', async (importOriginal) => {
    const actual = await importOriginal();
    return {
        ...actual,
...
describe('Gemini Client (client.ts)', () => {
    ...
    beforeEach(async () => {
        ...
        sessionStartProfilerMocks.createSessionStartProfiler.mockImplementation(() => {
            const profiler = {
                time: vi.fn(async (_stage, fn) => fn()),
                timeSync: vi.fn((_stage, fn) => fn()),
                finish: vi.fn(),
            };
    ...
    describe('initialize', () => {
        ...
        it('seeds resumed chat with previous response output token count', async () => {
            const seedResumeTokenCountsSpy = vi.spyOn(GeminiChat.prototype, 'seedResumeTokenCounts');
            vi.mocked(mockConfig.getResumedSessionData).mockReturnValue({
                conversation: {
                    sessionId: 'resumed-session-id',
                    projectHash: 'project-hash',
    ...
    describe('startChat — session start profiling', () => {
        ...
        it('passes startup, resume, and clear sources to the profiler', async () => {
            ...
            expect(sessionStartProfilerMocks.createSessionStartProfiler.mock.calls.map(([source]) => source)).toEqual([
            ...
            expect(sessionStartProfilerMocks.profilers[1].finish).toHaveBeenCalledWith(expect.objectContaining({ extraHistoryLength: 1 }));
        ...
        it('finalizes successful startChat profiles with bounded counts', async () => {
            ...
            expect(profiler.finish).toHaveBeenCalledWith(expect.objectContaining({
                ok: true,
        ...
        it('records non-zero snapshot and deferred reminder counts', async () => {
            const toolRegistry = vi.mocked(mockConfig.getToolRegistry)();
            toolRegistry.getDeferredToolSummary.mockReturnValue([
                { name: 'cron_create', description: 'schedule' },
            ]);
            toolRegistry.getTool.mockImplementation((name) => name === ToolNames.TOOL_SEARCH ? {} : null);
        ...
        it('does not record context apply stage without SessionStart context', async () => {
            await client.startChat();
            const profiler = sessionStartProfilerMocks.profilers.at(-1);
            expect(profiler.timeSync.mock.calls.map(([stage]) => stage)).not.toContain('session_start_context_apply');
        });
        it('finalizes failed startChat profiles without changing the thrown error', async () => {
            vi.mocked(getInitialChatHistory).mockRejectedValueOnce(new Error('history failed'));
            await expect(client.startChat()).rejects.toThrow('Failed to initialize chat: history failed');
            const profiler = sessionStartProfilerMocks.profilers.at(-1);
            expect(profiler.finish).toHaveBeenCalledWith(expect.objectContaining({
                ok: false,
                extraHistoryLength: 0,
        ...
        it('finalizes failed startChat profiles for first-stage warm errors', async () => {
            ...
            await expect(client.startChat()).rejects.toThrow('Failed to initialize chat: warm failed');
            const profiler = sessionStartProfilerMocks.profilers.at(-1);
            expect(profiler.time.mock.calls.map(([stage]) => stage)).toContain('tool_registry_warm');
            expect(profiler.finish).toHaveBeenCalledWith(expect.objectContaining({
                ok: false,
                extraHistoryLength: 0,
                historyLength: 0,
        ...
        it('finalizes failed startChat profiles for sync stage errors', async () => {
            vi.spyOn(client, 'getMainSessionSystemInstruction').mockImplementationOnce(() => {
                throw new Error('system instruction failed');
            });
            await expect(client.startChat()).rejects.toThrow('Failed to initialize chat: system instruction failed');
            const profiler = sessionStartProfilerMocks.profilers.at(-1);
            expect(profiler.timeSync.mock.calls.map(([stage]) => stage)).toContain('system_instruction');
            expect(profiler.finish).toHaveBeenCalledWith(expect.objectContaining({
                ok: false,
                extraHistoryLength: 0,
                historyLength: 1,
        ...
        it('finalizes failed startChat profiles with partial counts', async () => {
            ...
            vi.spyOn(client, 'setTools').mockRejectedValueOnce(new Error('set tools failed'));
            await expect(client.startChat()).rejects.toThrow('Failed to initialize chat: set tools failed');
            const profiler = sessionStartProfilerMocks.profilers.at(-1);
            expect(profiler.finish).toHaveBeenCalledWith(expect.objectContaining({
                ok: false,
                extraHistoryLength: 0,
    ...
    describe('startChat — repair orphan tool_use on resume', () => {
        it('synthesizes a functionResponse for a transcript ending in a dangling model[functionCall]', async () => {
            ...
            const danglingIdx = history.findIndex((h) => h.role === 'model' &&
        ...
        it('is a no-op when the resumed transcript has no dangling tool_use', async () => {
            ...
            const hasAnyFunctionResponse = history.some((h) => h.parts?.some((p) => p.functionResponse));
    ...
    describe('setTools — progressive MCP reminders', () => {
        ...
        async function runTurn(type = SendMessageType.UserQuery) {
            mockTurnRunFn.mockReturnValue((async function* () {
                yield { type: GeminiEventType.Content, value: 'response' };
            })());
            const stream = client.sendMessageStream([{ text: 'hello' }], new AbortController().signal, `prompt-${type}`, { type });
        ...
        it('omits already-revealed deferred tools from added reminders', async () => {
            ...
            expect(addHistorySpy).not.toHaveBeenCalled();
            await runTurn();
            expect(buildChangedMcpToolsReminder).toHaveBeenCalledWith([{ name: 'mcp__server__beta', description: 'b', serverName: 'server' }], []);
        ...
        it('announces removed MCP deferred tools after disconnect', async () => {
            ...
            await client.setTools();
            await runTurn();
            expect(buildChangedMcpToolsReminder).toHaveBeenCalledWith([], ['mcp__gone__do']);
            expect(addHistorySpy).toHaveBeenCalledWith({
                role: 'user',
                parts: [
                    {
        ...
        it('keeps queued MCP changes when the reminder builder returns null', () => {
            ...
            expect(priv.pendingRemovedMcpToolNames).toEqual(new Set(['mcp__gone__do']));
        });
        it('eagerly reveals every deferred tool when ToolSearch is unavailable', async () => {
            ...
            reg.revealDeferredTool.mockClear();
            const addHistorySpy = vi.spyOn(client.getChat(), 'addHistory');
            const setSystemInstructionSpy = vi.spyOn(client.getChat(), 'setSystemInstruction');
            vi.spyOn(client.getChat(), 'setTools').mockImplementation(() => { });
            vi.mocked(getCoreSystemPrompt).mockClear();
            await client.setTools();
        ...
        it('preserves SessionStart additionalContext because setTools does not rewrite the system instruction', async () => {
            ...
            vi.mocked(mockConfig.getHookSystem).mockReturnValue(hookSystem);
            await client.startChat(undefined, SessionStartSource.Startup);
            const systemInstructionBefore = client.getChat()['generationConfig'].systemInstruction;
            const setSystemInstructionSpy = vi.spyOn(client.getChat(), 'setSystemInstruction');
            await client.setTools();
            expect(setSystemInstructionSpy).not.toHaveBeenCalled();
            expect(client.getChat()['generationConfig'].systemInstruction).toBe(systemInstructionBefore);
            expect(systemInstructionBefore).toContain('SessionStart additional context:\nHookCtx');
        });
    });
    describe('addHistory', () => {
    ...
    describe('resetChat', () => {
        it('refreshes the live system instruction after the working directory changes', async () => {
            ...
            await client.startChat();
            expect(client.getChat()['generationConfig'].systemInstruction).toContain('Git snapshot A');
            await client.addWorkingDirectoryChangedContext('/test/project/root', '/test/other/root');
            const systemInstruction = client.getChat()['generationConfig']
                .systemInstruction;
    ...
    describe('history mutation invalidates FileReadCache', () => {
        ...
        it('restores stripped retry entries when session token limit skips send', async () => {
            ...
            vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(101);
            const events = await fromAsync(client.sendMessageStream([{ text: 'retry me' }], new AbortController().signal, 'prompt-retry-limit', { type: SendMessageType.Retry }));
    ...
    describe('thinking block idle cleanup and latch', () => {
        ...
        it('seeds Hook microcompaction checkpoint on user turns', async () => {
            ...
            const before = Date.now();
            const gen = client.sendMessageStream([{ text: 'Hello' }], new AbortController().signal, 'prompt-hook-seed', { type: SendMessageType.UserQuery });
            for await (const _ of gen) {
                /* drain */
            }
            expect(client['lastHookMicrocompactionTimestamp']).toBeGreaterThanOrEqual(before);
        });
    });
    describe('microcompaction FileReadCache invalidation', () => {
        ...
        async function makeReadFileResponses(count, outputLength) {
            const out = [];
            const paths = [];
        ...
        it('does not abort the turn when microcompaction cleanup fails', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            const events = [];
            const stream = client.sendMessageStream([{ text: 'hi' }], new AbortController().signal, 'prompt-mc-error-boundary', { type: SendMessageType.UserQuery });
        ...
        it('microcompacts old tool results on Hook continuations', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook', { type: SendMessageType.Hook });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).toHaveBeenCalled();
            expect(clear).not.toHaveBeenCalled();
            expect(markReadEvictedFromHistory).toHaveBeenCalledTimes(1);
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('[TIME-BASED MC]'));
            expect(client['lastHookMicrocompactionTimestamp']).toBeGreaterThan(Date.now() - 60_000);
        });
        it('does not abort Hook continuations when microcompaction cleanup fails', async () => {
            ...
            const events = [];
            const stream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook-error-boundary', { type: SendMessageType.Hook });
            for await (const event of stream) {
                events.push(event);
            }
            expect(events).toEqual([
            ...
            expect(mockClientDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('microcompactHistory failed: hook cache disarm failed'));
        ...
        it('skips the next Hook microcompaction after one just ran', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now();
            client['lastHookMicrocompactionTimestamp'] = Date.now() - 90 * 60_000;
            const firstStream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook-fire', { type: SendMessageType.Hook });
            ...
            const secondStream = client.sendMessageStream([{ text: 'continue goal again' }], new AbortController().signal, 'prompt-mc-hook-skip', { type: SendMessageType.Hook });
            for await (const _ of secondStream) {
                /* drain */
            }
            expect(client['lastHookMicrocompactionTimestamp']).toBe(checkpointAfterFire);
        ...
        it('initializes Hook microcompaction from the last API completion timestamp', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            client['lastHookMicrocompactionTimestamp'] = null;
            const stream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook-init', { type: SendMessageType.Hook });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).toHaveBeenCalled();
        ...
        it('does not microcompact Hook continuations when the checkpoint is recent', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            client['lastHookMicrocompactionTimestamp'] = Date.now();
            const stream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook-recent', { type: SendMessageType.Hook });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).not.toHaveBeenCalled();
        ...
        it('seeds Hook microcompaction checkpoint to now when no API call completed', async () => {
            ...
            client['lastHookMicrocompactionTimestamp'] = null;
            const before = Date.now();
            const stream = client.sendMessageStream([{ text: 'continue goal' }], new AbortController().signal, 'prompt-mc-hook-no-api-completion', { type: SendMessageType.Hook });
            for await (const _ of stream) {
                /* drain */
            }
            expect(client['lastHookMicrocompactionTimestamp']).toBeGreaterThanOrEqual(before);
        ...
        it('invalidates only the path when an evicted path cannot be stat’d', async () => {
            ...
            // recovered path.
            const { clear, markReadEvictedFromHistory, invalidateByPath } = mockFileReadCacheStub();
            const history = [];
            for (let i = 0; i < 6; i++) {
            ...
            expect(invalidateByPath).toHaveBeenCalledWith(join(mcTmpDir, 'ghost-0.ts'));
            expect(clear).not.toHaveBeenCalled();
            expect(markReadEvictedFromHistory).not.toHaveBeenCalled();
        });
        ...
        it('runs microcompaction on SendMessageType.Hook', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            const stream = client.sendMessageStream([{ text: 'goal continuation' }], new AbortController().signal, 'prompt-hook-test', { type: SendMessageType.Hook });
            for await (const _ of stream) {
                /* drain */
            }
        ...
        it('does not run idle microcompaction on SendMessageType.ToolResult', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            const stream = client.sendMessageStream([{ text: 'tool result' }], new AbortController().signal, 'prompt-toolresult-test', { type: SendMessageType.ToolResult });
            for await (const _ of stream) {
                /* drain */
            }
        ...
        it('runs size-only microcompaction on SendMessageType.ToolResult with pending content counted', async () => {
            ...
            ], new AbortController().signal, 'prompt-toolresult-size-budget', { type: SendMessageType.ToolResult });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).toHaveBeenCalled();
            const compacted = setHistory.mock.calls[0][0];
            expect(compacted[1].parts[0].functionResponse.response['output']).toBe('[Old tool result content cleared]');
            expect(clear).not.toHaveBeenCalled();
            expect(markReadEvictedFromHistory).toHaveBeenCalledTimes(1);
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('[TOOL-RESULT MC] tool result chars 530000 > 500000'));
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('history now 360000 (+50000 pending)'));
        });
        it('logs size overages when protected results leave nothing to clear', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'hi' }], new AbortController().signal, 'prompt-size-overage-all-protected', { type: SendMessageType.UserQuery });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).not.toHaveBeenCalled();
            expect(clear).not.toHaveBeenCalled();
            expect(markReadEvictedFromHistory).not.toHaveBeenCalled();
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('[TOOL-RESULT MC] tool result chars 800000 > 500000'));
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('cleared 0 tool result(s)'));
            expect(mockClientDebugLogger.info).toHaveBeenCalledWith(expect.stringContaining('history now 800000'));
        });
        it('runs microcompaction on SendMessageType.Cron', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            const stream = client.sendMessageStream([{ text: 'cron job' }], new AbortController().signal, 'prompt-cron-test', { type: SendMessageType.Cron });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).toHaveBeenCalled();
        ...
        it('does not reset the Hook checkpoint when Cron skips microcompaction', async () => {
            ...
            const checkpoint = Date.now() - 90 * 60_000;
            client['lastHookMicrocompactionTimestamp'] = checkpoint;
            const stream = client.sendMessageStream([{ text: 'cron job' }], new AbortController().signal, 'prompt-cron-hook-checkpoint', { type: SendMessageType.Cron });
            for await (const _ of stream) {
                /* drain */
            }
            expect(client['lastHookMicrocompactionTimestamp']).toBe(checkpoint);
        ...
        it('does not run microcompaction on SendMessageType.Retry', async () => {
            ...
            client['lastApiCompletionTimestamp'] = Date.now() - 90 * 60_000;
            const stream = client.sendMessageStream([{ text: 'retry' }], new AbortController().signal, 'prompt-retry-test', { type: SendMessageType.Retry });
            for await (const _ of stream) {
                /* drain */
            }
            expect(setHistory).not.toHaveBeenCalled();
        ...
        it('continues sendMessage when microcompactHistory throws', async () => {
            ...
            mockClientDebugLogger.error.mockClear();
            const stream = client.sendMessageStream([{ text: 'cron job' }], new AbortController().signal, 'prompt-mc-error-test', { type: SendMessageType.Cron });
            for await (const _ of stream) {
                /* drain */
            }
            expect(mockClientDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('microcompactHistory failed: compaction boom'));
            expect(setHistory).not.toHaveBeenCalled();
        });
    });
    ...
    describe('tryCompressChat (delegation)', () => {
        ...
        it('forwards prompt id, model, force, and signal to chat.tryCompress', async () => {
            ...
            expect(tryCompress).toHaveBeenCalledWith('p1', 'the-model', true, signal, undefined);
        });
        it('forwards customInstructions through the options bag when supplied', async () => {
            ...
            expect(tryCompress).toHaveBeenCalledWith('p1', 'the-model', true, undefined, { customInstructions: 'focus on auth bug' });
        });
        ...
        it('re-prepends the startup prelude after an auto-compaction ChatCompressed event', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'hi' }], new AbortController().signal, 'prompt-auto-restore', { type: SendMessageType.UserQuery });
            for await (const _ of stream) {
                /* drain */
            }
    ...
    describe('sendMessageStream', () => {
        it('filters unsupported media from the shared history snapshot', async () => {
            ...
            for await (const _ of client.sendMessageStream([{ text: 'next turn' }], new AbortController().signal, 'prompt-cache-media')) {
                /* drain */
            }
...
\`\`\``;
        ...
        it('should halt via the always-on turn cap before the skipLoopDetection gate', async () => {
            ...
            vi.spyOn(loopDetector, 'getLastLoopType').mockReturnValue(LoopType.TURN_TOOL_CALL_CAP);
            ...
            mockTurnRunFn.mockImplementation(async function* () {
                this.pendingToolCalls.push({ name: 'read_file', args: { path: 'a.ts' } }, { name: 'read_file', args: { path: 'b.ts' } });
                yield { type: 'content', value: 'looping' };
            });
            const stream = client.sendMessageStream([{ text: 'trigger the cap' }], new AbortController().signal, 'prompt-id-cap', { type: SendMessageType.UserQuery });
            const events = [];
            ...
            const returnedTurn = result.value;
            ...
            const loopEvent = events.find((e) => e.type === GeminiEventType.LoopDetected);
        ...
        it('always-on consecutive halt clears all pending calls (uniform with the turn cap)', async () => {
            ...
            client['chat'] = mockChat;
            const stream = client.sendMessageStream([{ text: 'mix distinct then repeat' }], new AbortController().signal, 'prompt-id-splice-mixed');
            const events = [];
            let result = await stream.next();
            while (!result.done) {
                events.push(result.value);
        ...
        it('should inject the current date on every UserQuery turn', async () => {
            ...
            client['chat'] = mockChat;
            const stream = client.sendMessageStream([{ text: 'What day is it?' }], new AbortController().signal, 'prompt-id-date-inject');
            for await (const _ of stream) {
                // consume stream
            }
        ...
        it('uses the subagent plan reminder when a subagent inherits PLAN mode', async () => {
            ...
            vi.mocked(getPlanModeSystemReminder).mockReturnValue('<system-reminder>return plan to caller</system-reminder>');
            const mockStream = (async function* () {
            ...
            await runWithAgentContext('agent-1', async () => {
                const stream = client.sendMessageStream([{ text: 'Plan this change' }], new AbortController().signal, 'prompt-id-subagent-plan-reminder');
                for await (const _ of stream) {
                    // consume stream
                }
            });
        ...
        it('uses the subagent plan reminder when SDK mode is active', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'Plan this change' }], new AbortController().signal, 'prompt-id-sdk-plan-reminder');
            for await (const _ of stream) {
                // consume stream
            }
            expect(getPlanModeSystemReminder).toHaveBeenCalledWith(true);
        ...
        it('uses the main-session plan reminder outside subagent and SDK mode', async () => {
            vi.mocked(mockConfig.getApprovalMode).mockReturnValue(ApprovalMode.PLAN);
            vi.mocked(mockConfig.getSdkMode).mockReturnValue(false);
            vi.mocked(getPlanModeSystemReminder).mockReturnValue('<system-reminder>call exit_plan_mode</system-reminder>');
            const mockStream = (async function* () {
                yield { type: 'content', value: 'Plan ready' };
            })();
            mockTurnRunFn.mockReturnValue(mockStream);
            ...
            const stream = client.sendMessageStream([{ text: 'Plan this change' }], new AbortController().signal, 'prompt-id-main-plan-reminder');
            for await (const _ of stream) {
                // consume stream
            }
        ...
        it('should not inject duplicate date on the same day', async () => {
            ...
            const stream1 = client.sendMessageStream([{ text: 'First question' }], new AbortController().signal, 'prompt-id-date-first');
            for await (const _ of stream1) {
                // consume stream
            }
            expect(mockTurnRunFn).toHaveBeenLastCalledWith('test-model', [
                expect.stringMatching(/^<system-reminder>\nThe current date is:.*June 5, 2026/),
                'First question',
            ], expect.any(AbortSignal));
            ...
            const stream2 = client.sendMessageStream([{ text: 'Second question' }], new AbortController().signal, 'prompt-id-date-second');
            for await (const _ of stream2) {
                // consume stream
            }
        ...
        it('should re-inject date when session spans midnight', async () => {
            ...
            const stream1 = client.sendMessageStream([{ text: 'Day one' }], new AbortController().signal, 'prompt-id-date-day-one');
            for await (const _ of stream1) {
                // consume stream
            }
            expect(mockTurnRunFn).toHaveBeenLastCalledWith('test-model', [
            ...
            const stream2 = client.sendMessageStream([{ text: 'Day two' }], new AbortController().signal, 'prompt-id-date-day-two');
            for await (const _ of stream2) {
                // consume stream
            }
            // New date should be injected with June 5
            const secondCall = mockTurnRunFn.mock.calls[1];
            expect(secondCall[1][0]).toMatch(/^<system-reminder>\nThe current date is:.*June 5, 2026/);
        });
        it('should not inject date on Cron turns', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'cron-task' }], new AbortController().signal, 'prompt-id-cron', { type: SendMessageType.Cron });
            for await (const _ of stream) {
                // consume stream
            }
            ...
            expect(cronRequest).not.toContain('<system-reminder>\nThe current date is:');
            ...
            mockTurnRunFn.mockReturnValue(mockStream2);
            const stream2 = client.sendMessageStream([{ text: 'User question' }], new AbortController().signal, 'prompt-id-cron-user');
            for await (const _ of stream2) {
                // consume stream
            }
            expect(mockTurnRunFn).toHaveBeenLastCalledWith('test-model', [
        ...
        describe('autoSkill: scheduleSkillReview via runManagedAutoMemoryBackgroundTasks', () => {
            ...
            it('should pass confirmBeforePersist from getAutoSkillConfirmEnabled', async () => {
                vi.spyOn(client['config'], 'getAutoSkillConfirmEnabled').mockReturnValue(true);
                mockMemoryManager.scheduleSkillReview.mockReturnValue({
                    status: 'skipped',
                    skippedReason: 'below_threshold',
                });
                await fromAsync(client.sendMessageStream([{ text: 'a query' }], new AbortController().signal, 'prompt-id-autoskill-confirm'));
                expect(mockMemoryManager.scheduleSkillReview).toHaveBeenCalledWith(expect.objectContaining({ confirmBeforePersist: true }));
            });
        });
  ...
  - /path/to/recent/file2.ts`;
        ...
        describe('Editor context delta', () => {
            ...
            it.each(testCases)('$description', async ({ previousActiveFile, currentActiveFile, shouldSendContext, }) => {
                ...
                if (shouldSendContext) {
                    ...
                    expect(getLastTurnRequestText()).not.toContain("Here is a summary of changes in the user's current editor context");
                }
            });
        ...
        describe('IDE context with pending tool calls', () => {
            ...
            it('keeps an empty full IDE snapshot unsent until context text is available', async () => {
                ...
                expect(getLastTurnRequestText()).not.toContain("Here is the user's current editor context");
                expect(client['lastSentIdeContext']).toBeUndefined();
                expect(client['forceFullIdeContext']).toBe(true);
                vi.mocked(ideContextStore.get).mockReturnValue({
                    workspaceState: {
        ...
        it('hard-stops identical tool calls even when skipLoopDetection is true (always-on guard)', async () => {
            ...
            client['chat'] = mockChat;
            const events = await fromAsync(client.sendMessageStream([{ text: 'repeat a tool' }], new AbortController().signal, 'prompt-id-skip-loop-identical'));
        ...
        it('hard-stops identical tool calls when loop detection is enabled', async () => {
            ...
            client['chat'] = mockChat;
            const events = await fromAsync(client.sendMessageStream([{ text: 'repeat a tool' }], new AbortController().signal, 'prompt-id-loop-identical'));
            expect(events.at(-1)).toEqual({
                type: GeminiEventType.LoopDetected,
                value: { loopType: LoopType.CONSECUTIVE_IDENTICAL_TOOL_CALLS },
            });
        ...
        describe('retry sendMessageType', () => {
            ...
            it('restores stripped retry entries when the retry stream throws before its first event', async () => {
                ...
                await expect(fromAsync(client.sendMessageStream([{ text: 'retry me' }], new AbortController().signal, 'prompt-retry-pre-event-failure', { type: SendMessageType.Retry }))).rejects.toThrow('retry failed before first event');
            ...
            it('does not re-add stripped retry entries when the chat already pushed them before failing', async () => {
                ...
                await expect(fromAsync(client.sendMessageStream([{ text: 'retry me' }], new AbortController().signal, 'prompt-retry-post-push-failure', { type: SendMessageType.Retry }))).rejects.toThrow('retry failed after push, before first event');
            ...
            it('does not re-add stripped retry entries when auto-compression shrank history below the pre-send length after the push', async () => {
                ...
                await expect(fromAsync(client.sendMessageStream([{ text: 'retry me' }], new AbortController().signal, 'prompt-retry-compression-shrink', { type: SendMessageType.Retry }))).rejects.toThrow('failed after compression+push, before first event');
        ...
        describe('hooks fast-path optimization', () => {
            ...
            it('should skip messageBus.request for MessageDisplay when hasHooksForEvent returns false', async () => {
                ...
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockReturnValue(false);
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-hooks-message-display-off');
                for await (const _ of stream) {
                    // consume stream
                }
                expect(mockMessageBus.request).not.toHaveBeenCalled();
            ...
            it('fires MessageDisplay with the cumulative streamed text, exactly once, when is_final on turn end', async () => {
                ...
                vi.mocked(mockConfig.getDisableAllHooks).mockReturnValue(false);
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockImplementation((event) => event === 'MessageDisplay');
                mockTurnRunFn.mockReturnValue((async function* () {
                    yield { type: GeminiEventType.Content, value: 'Hello, ' };
                    yield { type: GeminiEventType.Content, value: 'world.' };
                })());
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display');
                for await (const _ of stream) {
                    // consume stream
                }
            ...
            it('fires a debounced mid-stream flush once the debounce window elapses, then a separate final flush', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display-debounced');
                ...
                expect(finalCall[0].input.message_id).toBe(midStreamCall[0].input.message_id);
            });
            it('logs and swallows a rejected MessageDisplay hook request', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display-rejected');
                for await (const _ of stream) {
                    // consume stream
                }
                // The log line carries the message_id so a failure can be correlated
                // to its turn when debug logging is enabled.
                expect(debugLogger.warn).toHaveBeenCalledWith(expect.stringMatching(/^MessageDisplay hook failed \[[0-9a-f-]{36}\]: Error: hook process failed$/));
                ...
                expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringMatching(/^MessageDisplay hook failed/));
            ...
            it('does not end the turn until the final MessageDisplay payload has been delivered', async () => {
                ...
                let releaseHook;
                mockMessageBus.request.mockImplementation(() => new Promise((resolve) => {
                    releaseHook = () => resolve({});
                }));
                mockTurnRunFn.mockReturnValue((async function* () {
                    yield { type: GeminiEventType.Content, value: 'Hello, world.' };
                })());
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display-drain');
                let turnEnded = false;
                const consumed = (async () => {
                    for await (const _ of stream) {
                        // consume stream
            ...
            it('fires the final MessageDisplay flush when the always-on loop-detection safety trips mid-stream', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'trigger the always-on safety' }], new AbortController().signal, 'prompt-message-display-always-on-loop');
                for await (const _ of stream) {
                    // consume stream
                }
                ...
                const finalCall = mockMessageBus.request.mock.calls.find(([request]) => request.eventName === 'MessageDisplay' && request.input?.is_final);
            ...
            it('fires the final MessageDisplay flush when heuristic loop detection trips mid-stream', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'trigger a heuristic loop' }], new AbortController().signal, 'prompt-message-display-heuristic-loop');
                for await (const _ of stream) {
                    // consume stream
                }
                const finalCall = mockMessageBus.request.mock.calls.find(([request]) => request.eventName === 'MessageDisplay' && request.input?.is_final);
            ...
            it('fires the final MessageDisplay flush when the turn stream yields an Error event', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display-error');
                for await (const _ of stream) {
                    // consume stream
                }
                const finalCall = mockMessageBus.request.mock.calls.find(([request]) => request.eventName === 'MessageDisplay' && request.input?.is_final);
            ...
            it('suppresses the final MessageDisplay flush when the signal is aborted before the stream ends', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'Hi' }], controller.signal, 'prompt-message-display-aborted');
                for await (const _ of stream) {
                    // consume stream
                }
                const messageDisplayCalls = mockMessageBus.request.mock.calls.filter(([request]) => request.eventName === 'MessageDisplay');
            ...
            it('suppresses the final MessageDisplay flush for a tool-call-only turn with no Content events', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-message-display-tool-only');
                for await (const _ of stream) {
                    // consume stream
                }
                const messageDisplayCalls = mockMessageBus.request.mock.calls.filter(([request]) => request.eventName === 'MessageDisplay');
            ...
            it('gives a blocking Stop hook continuation a fresh per-turn tool-call budget', async () => {
                ...
                const events = await fromAsync(client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-stop-hook-budget'));
                ...
                expect(events).not.toContainEqual(expect.objectContaining({ type: GeminiEventType.LoopDetected }));
            });
            ...
            it('does not run UserPromptSubmit hooks for same-turn steer input', async () => {
                ...
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockImplementation((event) => event === 'UserPromptSubmit');
                const stream = client.sendMessageStream([{ text: 'focus on error handling' }], new AbortController().signal, 'prompt-steer', { type: SendMessageType.Steer });
                for await (const _ of stream) {
                    // consume stream
                }
                expect(mockMessageBus.request).not.toHaveBeenCalled();
            ...
            it('consumes steer input before running Stop hooks', async () => {
                ...
                await fromAsync(client.sendMessageStream([{ text: 'start the analysis' }], new AbortController().signal, 'prompt-steer-before-stop', { type: SendMessageType.UserQuery, getSteerInput }));
                expect(mockTurnRunFn).toHaveBeenCalledTimes(2);
                expect(getLastTurnRequestText()).toContain('focus on error handling');
                expect(getSteerInput.mock.invocationCallOrder[0]).toBeLessThan(mockMessageBus.request.mock.invocationCallOrder[0]);
            });
            it('consumes input queued during a blocking Stop hook before its continuation', async () => {
                ...
                const getSteerInput = vi
                    ...
                    .mockResolvedValue(undefined);
                await fromAsync(client.sendMessageStream([{ text: 'start the analysis' }], new AbortController().signal, 'prompt-steer-during-stop', { type: SendMessageType.UserQuery, getSteerInput }));
                expect(mockTurnRunFn).toHaveBeenCalledTimes(2);
            ...
            it('uses input queued during next-speaker classification for the continuation', async () => {
                ...
                const getSteerInput = vi
                    ...
                    .mockResolvedValue(undefined);
                await fromAsync(client.sendMessageStream([{ text: 'start the analysis' }], new AbortController().signal, 'prompt-steer-during-next-speaker', { type: SendMessageType.UserQuery, getSteerInput }));
                expect(mockTurnRunFn).toHaveBeenCalledTimes(2);
            ...
            it('does not drain steer input without another model-turn budget', async () => {
                ...
                await fromAsync(client.sendMessageStream([{ text: 'start the analysis' }], new AbortController().signal, 'prompt-steer-no-budget', { type: SendMessageType.UserQuery, getSteerInput }, 1));
            ...
            it('restores steer input when the continuation fails before history accepts it', async () => {
                ...
                await expect(fromAsync(client.sendMessageStream([{ text: 'start the analysis' }], new AbortController().signal, 'prompt-steer-restore', { type: SendMessageType.UserQuery, getSteerInput }))).rejects.toThrow('setup failed before history push');
            ...
            it('settles an attached ToolResult steer only after history accepts it', async () => {
                ...
                const accept = vi.fn();
                const restore = vi.fn();
                await fromAsync(client.sendMessageStream([{ text: 'tool result plus steer' }], new AbortController().signal, 'prompt-attached-steer-accept', {
            ...
            it('restores an attached ToolResult steer when history never accepts it', async () => {
                ...
                const accept = vi.fn();
                const restore = vi.fn();
                await expect(fromAsync(client.sendMessageStream([{ text: 'tool result plus steer' }], new AbortController().signal, 'prompt-attached-steer-restore', {
                    type: SendMessageType.ToolResult,
                    steerInput: {
                        parts: [{ text: 'steer' }],
                        accept,
            ...
            it('restores an attached ToolResult steer when UserPromptSubmit blocks it', async () => {
                ...
                const accept = vi.fn();
                const restore = vi.fn();
                await fromAsync(client.sendMessageStream([{ text: 'tool result plus steer' }], new AbortController().signal, 'prompt-attached-steer-blocked', {
                    type: SendMessageType.ToolResult,
                    steerInput: {
                        parts: [{ text: 'steer' }],
                        accept,
        ...
        describe('file history snapshot persistence', () => {
            ...
            async function collectStream(messageType, promptId = 'prompt-uq') {
                const stream = client.sendMessageStream([{ text: 'user' }], new AbortController().signal, promptId, { type: messageType });
                const chunks = [];
            ...
            it('calls makeSnapshot for UserQuery turns', async () => {
                ...
                expect(mockFileHistoryService.makeSnapshot).toHaveBeenCalledWith('prompt-file-history');
            });
            ...
            it('swallows makeSnapshot rejection and still yields content', async () => {
                mockFileHistoryService.makeSnapshot.mockRejectedValueOnce(new Error('snapshot failed'));
    ...
    describe('generateContent', () => {
        it('filters unsupported media for the resolved target model', async () => {
            ...
            await client.generateContent(contents, {}, new AbortController().signal, 'test-model');
        ...
        it('should call generateContent with the correct parameters', async () => {
            ...
            await client.generateContent(contents, generationConfig, abortSignal, DEFAULT_HOPCODE_FLASH_MODEL);
        ...
        it('forwards configured retryErrorCodes to retryWithBackoff', async () => {
            ...
            await client.generateContent([{ role: 'user', parts: [{ text: 'hi' }] }], {}, new AbortController().signal, client['config'].getModel());
            expect(retryWithBackoff).toHaveBeenCalledWith(expect.any(Function), expect.objectContaining({ extraRetryErrorCodes: [4999] }));
        });
        it('should use current model from config for content generation', async () => {
            ...
            await client.generateContent(contents, {}, new AbortController().signal, DEFAULT_HOPCODE_FLASH_MODEL);
        ...
        it('should prefer the current prompt id context for stateless requests', async () => {
            ...
            await promptIdContext.run('btw-prompt-id', async () => {
                await client.generateContent(contents, {}, abortSignal, DEFAULT_HOPCODE_FLASH_MODEL);
            });
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                model: DEFAULT_HOPCODE_FLASH_MODEL,
                contents,
        ...
        it('should prefer an explicit prompt id override over the current context', async () => {
            ...
            await promptIdContext.run('context-prompt-id', async () => {
                await client.generateContent(contents, {}, abortSignal, DEFAULT_HOPCODE_FLASH_MODEL, 'override-prompt-id');
            });
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                model: DEFAULT_HOPCODE_FLASH_MODEL,
                contents,
        ...
        it('should append config appendSystemPrompt to the core system prompt', async () => {
            ...
            await client.generateContent(contents, {}, abortSignal, DEFAULT_HOPCODE_FLASH_MODEL);
            expect(getCoreSystemPrompt).toHaveBeenCalledWith('', 'test-model', 'Be extra concise.', 'headless');
        });
        it.each([
            ['interactive', true, false],
            ['acp', false, true],
            ['headless', false, false],
        ])('should pass %s mode to the core system prompt', async (mode, interactive, acp) => {
            const contents = [{ role: 'user', parts: [{ text: 'hello' }] }];
            const abortSignal = new AbortController().signal;
            vi.mocked(getCoreSystemPrompt).mockClear();
            vi.mocked(client['config'].isInteractive).mockReturnValue(interactive);
            vi.mocked(client['config'].getExperimentalZedIntegration).mockReturnValue(acp);
            await client.generateContent(contents, {}, abortSignal, DEFAULT_QWEN_FLASH_MODEL);
            expect(getCoreSystemPrompt).toHaveBeenCalledWith('', 'test-model', undefined, mode);
        });
        ...
        it('sets a generic span status when content generation fails', async () => {
            ...
            await expect(client.generateContent(contents, {}, abortSignal, DEFAULT_HOPCODE_FLASH_MODEL)).rejects.toThrow('raw upstream 500 with sensitive details');
        });
        it('propagates error when content generation is aborted', async () => {
            ...
            await expect(client.generateContent(contents, {}, abortController.signal, DEFAULT_HOPCODE_FLASH_MODEL)).rejects.toThrow('raw abort reason with sensitive details');
        });
    ...
    describe('drainSkillAndCommandReminders', () => {
        const makeEntries = (names, level = 'project') => names.map((name) => ({ name, description: `desc-${name}`, level }));
        const mockSkillManager = {
            listSkills: vi.fn().mockResolvedValue([]),
            getActivatedSkillNames: vi.fn().mockReturnValue(new Set()),
        };
        const mockChat = {
            addHistory: vi.fn(),
            getHistory: vi.fn().mockReturnValue([]),
        ...
        beforeEach(() => {
            mockSkillManager.getActivatedSkillNames.mockReturnValue(new Set());
            vi.mocked(mockConfig.getSkillManager).mockReturnValue(mockSkillManager);
            const toolReg = mockConfig.getToolRegistry();
            vi.mocked(toolReg.getTool).mockImplementation((name) => name === ToolNames.SKILL ? {} : undefined);
            priv().chat = mockChat;
        ...
        it('first drain with snapshot seed emits nothing for seeded entries', async () => {
            ...
            priv().seedSkillReminderDedupFromSnapshot(makeEntries(['skill-a', 'skill-b']));
            vi.mocked(collectAvailableSkillEntries).mockResolvedValue({
                availableSkills: [],
                pendingConditionalSkillNames: new Set(),
                modelInvocableCommands: [],
        ...
        it('path-activated skill is announced by drain (no suppression based on shared activation set)', async () => {
            mockSkillManager.getActivatedSkillNames.mockReturnValue(new Set(['skill-a']));
            // Seed from snapshot
            priv().seedSkillReminderDedupFromSnapshot(makeEntries(['skill-existing']));
        ...
        it('returns early and logs when collectAvailableSkillEntries throws', async () => {
            vi.mocked(collectAvailableSkillEntries).mockRejectedValue(new Error('load failed'));
            await drain();
            expect(priv().skillRemindersInitialized).toBe(false);
            expect(mockChat.addHistory).not.toHaveBeenCalled();
        });
        it('command entries use cmd: key prefix and are not suppressed by activatedConditional', async () => {
            mockSkillManager.getActivatedSkillNames.mockReturnValue(new Set(['mcp-prompt-a']));
        ...
        it('inline-announced skills consumed from config are not re-announced by drain', async () => {
            ...
            vi.mocked(mockConfig.consumeInlineAnnouncedSkillKeys).mockReturnValue(new Set(['skill:skill-inline']));
            ...
            expect(priv().announcedSkillReminderKeys.has('skill:skill-inline')).toBe(true);
            expect(mockChat.addHistory).not.toHaveBeenCalled();
        });
    ...
    describe('#5147 shutdown gate', () => {
        ...
        it('skips background memory tasks after shutdown is requested', () => {
            ...
            client.getHistoryShallow = () => [];
            const runBgTasks = client.runManagedAutoMemoryBackgroundTasks.bind(client);
    ...
    describe('drainAgentReminders', () => {
        ...
        beforeEach(() => {
            const toolReg = mockConfig.getToolRegistry();
            vi.mocked(toolReg.getTool).mockImplementation((name) => name === ToolNames.AGENT ? {} : undefined);
        ...
        it('seeds current agents on first drain without emitting a reminder', async () => {
            ...
            expect(priv().announcedAgentReminderNames).toEqual(new Set(['seed-agent']));
            expect(buildChangedAgentsReminder).not.toHaveBeenCalled();
            expect(addHistorySpy).not.toHaveBeenCalled();
        });
        it('returns early when listing agents fails', async () => {
            ...
            const addHistorySpy = vi.spyOn(client.getChat(), 'addHistory');
            await priv().drainAgentReminders();
            expect(priv().announcedAgentReminderNames).toEqual(new Set(['old-agent']));
            expect(buildChangedAgentsReminder).not.toHaveBeenCalled();
            expect(addHistorySpy).not.toHaveBeenCalled();
        });
        ...
        it('announces added-only agents', async () => {
            ...
            const addHistorySpy = vi.spyOn(client.getChat(), 'addHistory');
            await priv().drainAgentReminders();
            expect(buildChangedAgentsReminder).toHaveBeenCalledWith([{ name: 'new-agent', description: 'New agent' }], []);
            expect(addHistorySpy).toHaveBeenCalled();
            expect(priv().announcedAgentReminderNames).toEqual(new Set(['old-agent', 'new-agent']));
        });
        it('announces removed-only agents', async () => {
            ...
            const addHistorySpy = vi.spyOn(client.getChat(), 'addHistory');
            await priv().drainAgentReminders();
            expect(buildChangedAgentsReminder).toHaveBeenCalledWith([], ['old-agent']);
            expect(addHistorySpy).toHaveBeenCalled();
            expect(priv().announcedAgentReminderNames).toEqual(new Set(['stay-agent']));
        });
        it('announces added and removed agents', async () => {
            ...
            const addHistorySpy = vi.spyOn(client.getChat(), 'addHistory');
            await priv().drainAgentReminders();
            expect(buildChangedAgentsReminder).toHaveBeenCalledWith([{ name: 'new-agent', description: 'New agent' }], ['old-agent']);
            expect(addHistorySpy).toHaveBeenCalledWith({
                role: 'user',
                parts: [
                    {
        ...
        it('keeps agent reminder state unchanged if history append fails', async () => {
            ...
            await expect(priv().drainAgentReminders()).rejects.toThrow('history failed');
            expect(priv().announcedAgentReminderNames).toEqual(new Set(['old-agent']));
        });
    });
});
function makeMockConfigForShutdown(mgr) {
    return {
