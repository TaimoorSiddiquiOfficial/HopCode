 ...
 */
export function buildIznClarificationMessage(gateResult) {
    const lines = [
        '<system-reminder>',
    ...
    if (gateResult.impactScope.length > 0) {
        lines.push('', '## Impact Analysis (investigate before acting)', '', ...gateResult.impactScope.map((item, i) => `${i + 1}. ${item}`));
    }
    if (gateResult.intentQuestions.length > 0) {
        lines.push('', '## Clarify Intent (ask the user)', '', ...gateResult.intentQuestions.map((q, i) => `${i + 1}. ${q}`));
    }
    lines.push('', 'After completing verification and receiving confirmed intent, retry the tool call.', '', 'If you cannot verify safety: do NOT retry. Use ask_user_question to revert to consultation.', '</system-reminder>');
    return lines.join('\n');
}
export class IznGateHandler {
     ...
     */
    check(params) {
        const commandStr = typeof params.toolArgs.command === 'string'
...
//# sourceMappingURL=iznGateHandler.js.map