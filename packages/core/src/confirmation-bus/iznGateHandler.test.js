...
import { IznGateHandler, buildIznClarificationMessage, } from './iznGateHandler.js';
...
const mockCheckIznGate = checkIznGate;
const mockReportIznScope = reportIznScope;
...
describe('IznGateHandler', () => {
    ...
    describe('check', () => {
        ...
        it('blocks and returns clarification when the gate does not pass', () => {
            ...
            if (!result.allowed) {
                ...
                expect(result.clarificationMessage).toContain('Self-Verification Steps');
            }
        });
        ...
        it('different command hashes produce different bypass behavior', () => {
            ...
            const result = handler.check(makeParams({ toolArgs: { command: 'rm b' } }));
        ...
        it('handles tool without command arg (empty hash)', () => {
            ...
            const result = handler.check(makeParams({ toolName: 'write_file', toolArgs: {} }));
            expect(result.allowed).toBe(false);
        });
    });
    ...
    describe('clearVerifiedHashes', () => {
        it('clears stale verified hashes so they cannot auto-approve across turns', () => {
            ...
            const result = handler.check(makeParams({ toolArgs: { command: 'rm a' } }));
            expect(result.allowed).toBe(false);
            expect(mockCheckIznGate).toHaveBeenCalledTimes(2);
        });
...
//# sourceMappingURL=iznGateHandler.test.js.map