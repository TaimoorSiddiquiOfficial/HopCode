...
import { HookEventName, HookType, HooksConfigSource, MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH, } from './types.js';
// Hoisted mock
...
describe('HookRunner', () => {
    ...
    describe('executeHooksParallel', () => {
        ...
        it('should chain UserPromptExpansion additional context into the next hook input', async () => {
            const firstProcess = createMockProcess(0, JSON.stringify({
                hookSpecificOutput: {
            ...
            await hookRunner.executeHooksSequential(hookConfigs, HookEventName.UserPromptExpansion, input);
            const secondInputJson = secondProcess.stdin.write.mock.calls[0]?.[0];
            expect(typeof secondInputJson).toBe('string');
            const secondInput = JSON.parse(secondInputJson);
        ...
        it('should preserve raw UserPromptSubmit additional context when chaining', async () => {
            ...
            await hookRunner.executeHooksSequential(hookConfigs, HookEventName.UserPromptSubmit, input);
            const secondInputJson = secondProcess.stdin.write.mock.calls[0]?.[0];
            expect(typeof secondInputJson).toBe('string');
            const secondInput = JSON.parse(secondInputJson);
        ...
        it('should truncate UserPromptExpansion context before sanitizing it for chaining', async () => {
            ...
            expect(chainedContext).toHaveLength(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH);
        });
    });
