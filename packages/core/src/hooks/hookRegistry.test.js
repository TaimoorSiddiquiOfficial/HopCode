...
describe('HookRegistry', () => {
    ...
    describe('addAgentHooks — per-agent frontmatter ephemeral entries', () => {
        ...
        it('coexists with session/user hooks of the same identity', async () => {
            ...
            expect(after.some((e) => e.source === HooksConfigSource.User && e.agentScope === undefined)).toBe(true);
            expect(after.some((e) => e.source === HooksConfigSource.Session &&
                e.agentScope === 'agent:test:def')).toBe(true);
        });
        ...
        it('restores all previous hooks when configured hooks reload fails', async () => {
            ...
            await expect(registry.reloadConfiguredHooks()).rejects.toThrow('reload failed');
