...
import { MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH, createHookOutput, DefaultHookOutput, UserPromptExpansionHookOutput, HookEventName, isToolArtifactLike, } from './types.js';
...
describe('UserPromptExpansionHookOutput.getAdditionalContext', () => {
    it('returns undefined when hookSpecificOutput is absent', () => {
        expect(new UserPromptExpansionHookOutput().getAdditionalContext()).toBeUndefined();
    });
    it('returns undefined when additionalContext is absent', () => {
        expect(new UserPromptExpansionHookOutput({
            hookSpecificOutput: {},
        }).getAdditionalContext()).toBeUndefined();
    });
    ...
    it('preserves empty-string semantics', () => {
        ...
        }).getAdditionalContext()).toBe('');
    });
    it('escapes ampersands and angle brackets before capping the result', () => {
        const output = new UserPromptExpansionHookOutput({
            hookSpecificOutput: {
                additionalContext: `a&b<${'x'.repeat(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH)}`,
            },
        });
        const result = output.getAdditionalContext();
        expect(result).toHaveLength(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH);
    ...
    it('does not leave a partial entity after truncation', () => {
        const output = new UserPromptExpansionHookOutput({
            hookSpecificOutput: {
                additionalContext: 'x'.repeat(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH - 1) +
                    '<',
            },
        });
        const result = output.getAdditionalContext();
        expect(result).toBe('x'.repeat(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH - 1));
    });
    it('does not leave a partial ampersand entity after truncation', () => {
        const output = new UserPromptExpansionHookOutput({
            hookSpecificOutput: {
                additionalContext: 'x'.repeat(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH - 2) +
                    '&',
            },
        });
        const result = output.getAdditionalContext();
        expect(result).toBe('x'.repeat(MAX_USER_PROMPT_EXPANSION_ADDITIONAL_CONTEXT_LENGTH - 2));
    });
});
