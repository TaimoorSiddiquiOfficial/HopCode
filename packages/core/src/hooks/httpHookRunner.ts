...
/**
 * Maximum output length (10,000 characters as per HopCode Code spec)
 */
...
export class HttpHookRunner {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        // Per HopCode Code spec: Non-2xx status is a non-blocking error
        ...
        if (
          ...
          // Timeout or abort is a non-blocking error per HopCode Code spec
          debugLogger.warn(
        ...
        // Connection failure is a non-blocking error per HopCode Code spec
        debugLogger.warn(
   ...
   * Per HopCode Code spec: output is capped at 10,000 characters
   */
  ...
  private normalizeOutput(
    ...
    if ('systemMessage' in json && typeof json['systemMessage'] === 'string') {
      // Apply output length limit per HopCode Code spec
