...
describe('HookAggregator', () => {
    ...
    describe('mergeWithOrLogic - PreToolUse', () => {
        ...
        it('should preserve PostToolBatch stop decisions across multiple hooks', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PostToolBatch);
            const hookOutput = createHookOutput(HookEventName.PostToolBatch, result.finalOutput ?? {});
    ...
    describe('mergeSimple (default case)', () => {
        ...
        it('falls through to mergeSimple/DefaultHookOutput for MessageDisplay (no control-effect merge logic)', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.MessageDisplay);
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('a\nb');
    ...
    describe('terminalSequence merging', () => {
        ...
        it('preserves terminalSequence in simple merge events', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.SessionStart);
