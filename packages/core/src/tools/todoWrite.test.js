...
describe('TodoWriteTool', () => {
    ...
    describe('execute', () => {
        it('should create new todos file when none exists', async () => {
            ...
            expect(mockAtomicWrite).toHaveBeenCalledWith(expect.stringContaining('test-session-123.json'), expect.stringContaining('"todos"'), { encoding: 'utf-8' });
        });
        it('should replace todos with new ones', async () => {
            ...
            expect(mockAtomicWrite).toHaveBeenCalledWith(expect.stringContaining('test-session-123.json'), expect.stringMatching(/"Updated Task"/), { encoding: 'utf-8' });
        });
        ...
        it('should dispatch post-write completion hooks sequentially in list order for a batched completion', async () => {
            ...
            const mockHookSystem = {
                ...
                fireTodoCompletedEvent: vi
                    .fn()
                    .mockImplementation(async (id, _content, _previousStatus, _todos, phase) => {
                    if (phase === HookPhase.PostWrite) {
                        activePostWrite++;
                        maxConcurrentPostWrite = Math.max(maxConcurrentPostWrite, activePostWrite);
    ...
    describe('tool properties', () => {
        ...
        it('should describe selective, outcome-oriented task tracking', () => {
            ...
            expect(tool.description).toContain('Do not use it for simple or single-step work');
            expect(tool.description).toContain('unless the user explicitly requests a todo list');
            expect(tool.description).toContain('short and outcome-oriented');
            expect(tool.description).toContain('Do not create a separate todo for every error, file, command, or minor edit');
            expect(tool.description).not.toContain('After receiving new instructions');
...
describe('TodoWriteTool – runtime output directory', () => {
    ...
    it('should write todos to custom runtime dir when setRuntimeBaseDir is set', async () => {
        ...
        const writePath = mockAtomicWrite.mock.calls[0]?.[0];
        expect(writePath).toContain(path.join(customRuntimeDir, 'todos'));
        expect(writePath).toContain('runtime-session.json');
    });
    it('should write todos to env var dir when HOPCODE_RUNTIME_DIR is set', async () => {
