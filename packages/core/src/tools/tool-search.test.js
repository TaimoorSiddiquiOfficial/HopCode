...
describe('ToolSearchTool', () => {
    ...
    it('select: exit_plan_mode remains inspectable in the main session', async () => {
        ...
        const result = await tool
            ...
            .execute(new AbortController().signal);
        expect(String(result.llmContent)).toContain(`"name":"${ToolNames.EXIT_PLAN_MODE}"`);
        expect(registry.isDeferredToolRevealed(ToolNames.EXIT_PLAN_MODE)).toBe(false);
        expect(setToolsSpy).not.toHaveBeenCalled();
    });
    it.each([
        {
    ...
    ])('select: rejects $toolName inside subagent-like context without revealing or syncing tools', async ({ toolName, shouldDefer, alwaysLoad }) => {
        registry.registerTool(new MockTool({
            name: toolName,
            shouldDefer,
            alwaysLoad,
        ...
        const contextCases = [
            ...
            {
                run: (callback) => runWithTeammateIdentity({
                    agentId: 'agent@test',
                    agentName: 'agent',
                    teamName: 'test',
                    isTeamLead: false,
        ...
        for (const { run } of contextCases) {
            const result = await run(() => tool
                ...
                .execute(new AbortController().signal));
            expect(String(result.llmContent)).toContain('not available inside subagents');
    ...
    it('select: loads allowed tools while rejecting plan lifecycle tools inside subagent context', async () => {
        ...
        const tool = new ToolSearchTool(config);
        const result = await runWithAgentContext('agent-1', () => tool
            .build({
            query: `select:${ToolNames.READ_FILE},${ToolNames.ENTER_PLAN_MODE}`,
        })
            .execute(new AbortController().signal));
        expect(String(result.llmContent)).toContain(`"name":"${ToolNames.READ_FILE}"`);
        expect(String(result.llmContent)).not.toContain(`"name":"${ToolNames.ENTER_PLAN_MODE}"`);
        expect(String(result.llmContent)).toContain('not available inside subagents');
    ...
    it('select: lets plan-required teammates inspect exit_plan_mode but not enter_plan_mode', async () => {
        ...
        expect(String(result.llmContent)).toContain(`"name":"${ToolNames.EXIT_PLAN_MODE}"`);
        expect(String(result.llmContent)).not.toContain(`"name":"${ToolNames.ENTER_PLAN_MODE}"`);
        expect(String(result.llmContent)).toContain(`${ToolNames.ENTER_PLAN_MODE} is not available`);
        expect(result.error).toBeUndefined();
        expect(result.returnDisplay).toBe('Loaded 1 tool(s), 1 unavailable');
    });
    ...
    it('excludes visibleTools from keyword-search candidates', async () => {
        ...
        visibleRegistry.registerTool(new MockTool({
            name: 'web_fetch',
            shouldDefer: true,
    ...
    it('select: for a visibleTool does NOT trigger reveal/setTools', async () => {
        ...
        const visibleRegistry = new ToolRegistry(visibleConfig);
        visibleRegistry.registerTool(new MockTool({ name: 'web_fetch', shouldDefer: true }));
        vi.spyOn(visibleConfig, 'getToolRegistry').mockReturnValue(visibleRegistry);
    ...
    it('select: mixed visible+non-visible only reveals the hidden ones', async () => {
        ...
        const visibleRegistry = new ToolRegistry(visibleConfig);
        visibleRegistry.registerTool(new MockTool({ name: 'web_fetch', shouldDefer: true }));
        visibleRegistry.registerTool(new MockTool({ name: 'cron_create', shouldDefer: true }));
        vi.spyOn(visibleConfig, 'getToolRegistry').mockReturnValue(visibleRegistry);
        const mockSetTools = vi.fn().mockResolvedValue(undefined);
        vi.spyOn(visibleConfig, 'getGeminiClient').mockReturnValue({
            setTools: mockSetTools,
