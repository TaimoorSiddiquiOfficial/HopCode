...
function makeDurableTask(overrides) {
    return {
        id: 'durable01',
        cron: '0 */2 * * *',
        prompt: 'check deploy',
...
describe('CronListTool', () => {
    ...
    it('lists pending wakeups', async () => {
        ...
        try {
            ...
            const result = await invocation.execute(new AbortController().signal);
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toContain(new Date(2025, 0, 15, 10, 35, 0).toISOString());
            expect(result.llmContent).not.toContain('@wakeup');
            expect(result.llmContent).toContain(`[session-only]: ${longPrompt.slice(0, 57)}...`);
    ...
    it('lists durable jobs from the tasks file without the scheduler loading them', async () => {
        ...
        const result = await invocation.execute(new AbortController().signal);
        expect(result.error).toBeUndefined();
        expect(result.llmContent).toContain('durable01 — 0 */2 * * * (recurring) [durable]: check deploy');
    ...
    it('surfaces a durable task name and its disabled status', async () => {
        ...
        expect(result.llmContent).toContain('off01 — 0 */2 * * * (recurring) [durable, disabled]: Weekly digest: check deploy');
        expect(result.returnDisplay).toContain('[durable, disabled]: Weekly digest');
    });
    it('reports a legacy condition task as disabled while a normal task stays enabled', async () => {
        ...
        expect(result.llmContent).toContain('legacy01 — 0 */2 * * * (recurring) [durable, disabled]: guarded');
        // ...while an ordinary durable task stays enabled (no disabled marker).
        expect(result.llmContent).toContain('normal01 — 0 */2 * * * (recurring) [durable]: runs fine');
    });
    ...
    it('surfaces a corrupted tasks file as an error instead of an empty list', async () => {
        ...
        expect(result.llmContent).not.toContain('No active cron jobs or loop wakeups');
    });
});
