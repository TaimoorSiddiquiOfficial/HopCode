...
describe('SendMessageTool — team mode', () => {
    ...
    it('sends a message via TeamManager', async () => {
        ...
        expect(sendMessage).toHaveBeenCalledWith('alice', 'hello', 'leader', undefined);
    });
    ...
    it('validates required params', () => {
        ...
        expect(() => tool.build({ to: 'alice' })).toThrow();
    });
});
describe('SendMessageTool — background-task mode', () => {
    ...
    let reviveCompletedBackgroundAgent;
    beforeEach(() => {
    ...
    it('revives a completed task with the message as the next instruction', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'now refactor the helper' }, new AbortController().signal);
        expect(reviveCompletedBackgroundAgent).toHaveBeenCalledWith('agent-1', 'now refactor the helper');
    ...
    it('returns error when a completed task cannot be revived', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'try again' }, new AbortController().signal);
        expect(result.error?.type).toBe(ToolErrorType.SEND_MESSAGE_NOT_RUNNING);
