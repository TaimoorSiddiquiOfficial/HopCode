...
class WriteFileToolInvocation extends BaseToolInvocation {
    ...
    async execute(_abortSignal) {
        ...
        try {
            ...
            const artifactReminder = buildRecordArtifactReminder(this.config, file_path);
            if (artifactReminder) {
...
function buildRecordArtifactReminder(config, filePath) {
    ...
    return (`If this file is a reusable user-facing artifact, call ` +
...
export class WriteFileTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(WriteFileTool.Name, ToolDisplayNames.WRITE_FILE, `Writes content to a specified file in the local filesystem. The file_path argument MUST be an absolute path. Always construct it by combining the project root with the file's relative path (e.g. project root '/path/to/project/' + relative 'foo/bar.txt' = '/path/to/project/foo/bar.txt'). If the user provides a relative path, resolve it against the project root first.
