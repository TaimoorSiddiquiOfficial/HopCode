...
vi.mock('../services/visionBridge/vision-bridge-service.js', async (importOriginal) => {
...
describe('ReadFileTool', () => {
    ...
    describe('build', () => {
        ...
        it('should throw error if offset is negative', () => {
            ...
            expect(() => tool.build(params)).toThrow('Offset must be a non-negative integer');
        });
        ...
        it.each([0, -1])('should throw error if limit is not positive (%s)', (limit) => {
            const params = {
                file_path: path.join(tempRootDir, 'test.txt'),
                limit,
            };
            expect(() => tool.build(params)).toThrow('Limit must be a positive integer');
        });
        it.each([0.5, 1.5])('should throw error if limit is fractional (%s)', (limit) => {
            const params = {
                file_path: path.join(tempRootDir, 'test.txt'),
                limit,
            };
    ...
    describe('getDefaultPermission', () => {
        ...
        it('should return allow for paths within the user extensions directory', async () => {
            const params = {
                file_path: path.join(Storage.getUserExtensionsDir(), 'my-ext', 'index.js'),
            };
            const invocation = tool.build(params);
            const permission = await invocation.getDefaultPermission();
            expect(permission).toBe('allow');
    ...
    describe('execute', () => {
        ...
        it('should read and truncate a text file larger than 10MB', async () => {
            ...
            expect(result.returnDisplay).toBe('Read lines 1-1 of at least 1 from largefile.txt (truncated)');
            expect(result.llmContent).toContain('Showing lines 1-1 of at least 1 total lines');
        ...
        it('should propagate an aborted signal before reading', async () => {
            ...
            controller.abort();
            await expect(invocation.execute(controller.signal)).rejects.toThrow(/abort/i);
        });
        ...
        describe('PDF vision bridge fallback', () => {
            ...
            async function readCandidate(signalOverride = abortSignal) {
            ...
            it('replaces candidate images with an untrusted transcription before returning', async () => {
                ...
                expect(JSON.stringify(result.llmContent)).toContain('Untrusted transcription');
                expect(JSON.stringify(result.llmContent)).toContain('Pages 24-25 exist but were not transcribed');
                expect(JSON.stringify(result.llmContent)).not.toContain('pages 24-25 were not included');
                const display = bridgeDisplay(result);
                expect(display.summary).toContain('transcribed PDF pages 20-23; remaining pages 24-25');
                expect(display.notice).toContain('qwen3-vl-plus');
                expect(display.notice).toContain('dashscope.aliyuncs.com');
                expect(visionBridgeMocks.runVisionBridge).toHaveBeenCalledWith(expect.objectContaining({
                    sourceContext: {
            ...
            it('does not present unknown continuation pages as certain', async () => {
                ...
                const result = await readCandidate();
                const display = bridgeDisplay(result);
                expect(display.summary).toContain('additional pages may exist from page 24 through page 25');
            ...
            it.each([
                ...
                const display = bridgeDisplay(result);
                expect(display.summary).toContain('rendered PDF pages 20-23; remaining pages 24-25');
                expect(display.notice).toContain('dashscope.aliyuncs.com');
            });
            it('restores the PDF error for an unusable successful bridge result', async () => {
                ...
                expect(result.error?.type).toBe(ToolErrorType.READ_CONTENT_FAILURE);
                expect(JSON.stringify(result.llmContent)).not.toContain('inlineData');
                expect(bridgeDisplay(result).notice).toContain('dashscope.aliyuncs.com');
            });
            it.each([
                [
                    'inlineData',
            ...
            ])('fails closed when a successful bridge result still contains %s', async (mediaKey, mediaPart) => {
                visionBridgeMocks.runVisionBridge.mockResolvedValue({
                    applied: true,
                    status: 'ok',
                    parts: [mediaPart],
            ...
            it('restores the PDF error when the bridge omits a rendered page', async () => {
                ...
                expect(result.llmContent).toContain('Cannot extract text from PDF');
                expect(bridgeDisplay(result).notice).toContain('dashscope.aliyuncs.com');
                expect(bridgeDisplay(result).notice).toContain('transcription was discarded');
                expect(bridgeDisplay(result).notice).not.toContain('vision model request failed');
                expect(JSON.stringify(result.llmContent)).not.toContain('inlineData');
            });
            it('restores the PDF error when the bridge throws before replacement', async () => {
                visionBridgeMocks.runVisionBridge.mockRejectedValue(new Error('network failure'));
                const result = await readCandidate();
                expect(result.error?.type).toBe(ToolErrorType.READ_CONTENT_FAILURE);
                expect(result.error?.message).toBe('No extractable text layer.');
                expect(result.llmContent).toContain('Cannot extract text from PDF');
                expect(JSON.stringify(result.llmContent)).not.toContain('inlineData');
                expect(bridgeDisplay(result).notice).toContain('failed before producing a transcription');
            });
            it('propagates cancellation instead of restoring a PDF error', async () => {
                ...
                await expect(readCandidate(controller.signal)).rejects.toThrow(/abort/i);
            });
        ...
        describe('with FileReadCache', () => {
            ...
            it('records an auto-memory read in the cache so a follow-up Edit can pass enforcement', async () => {
                ...
                // read. Put a file under .hopcode/<auto-memory>/ via
                // HOPCODE_MEMORY_LOCAL=1 and assert recordRead happened.
                const previousLocal = process.env['HOPCODE_MEMORY_LOCAL'];
                ...
                try {
                    ...
                    if (previousLocal === undefined) {
                        ...
                        process.env['HOPCODE_MEMORY_LOCAL'] = previousLocal;
                    }
                }
            });
        ...
        describe('with .hopcodeignore', () => {
            beforeEach(async () => {
                await fsp.writeFile(path.join(tempRootDir, '.hopcodeignore'), ['foo.*', 'ignored/'].join('\n'));
            });
            ...
            it('should throw error if path is ignored by .agentignore or .aiignore', async () => {
                await fsp.writeFile(path.join(tempRootDir, '.agentignore'), 'agent-secret.txt\n');
                await fsp.writeFile(path.join(tempRootDir, '.aiignore'), 'ai-secret.txt\n');
                ...
                expect(() => tool.build({ file_path: agentIgnoredFilePath })).toThrow(/\.agentignore/);
                expect(() => tool.build({ file_path: aiIgnoredFilePath })).toThrow(/\.aiignore/);
            });
            it('should throw error using configured custom ignore file display', async () => {
                await fsp.writeFile(path.join(tempRootDir, '.cursorignore'), 'cursor-secret.txt\n');
                const customConfig = {
                    getFileService: () => new FileDiscoveryService(tempRootDir, ['.cursorignore']),
                    getFileSystemService: () => new StandardFileSystemService(),
                    getTargetDir: () => tempRootDir,
                    getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir),
                    storage: {
                ...
                await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
                expect(() => customTool.build({ file_path: ignoredFilePath })).toThrow(/\.cursorignore/);
            });
