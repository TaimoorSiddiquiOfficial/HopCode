...
describe('LoopWakeupTool', () => {
    ...
    it('documents the fallback-heartbeat semantics for monitor/background work', () => {
        ...
        const params = tool.schema.parametersJsonSchema;
    ...
    it('shows the clamped delay in the permission description', () => {
        ...
        expect(invocation.getDescription()).toBe('60s (requested 5s): continue loop');
    });
    ...
    it('rejects scheduling when the scheduler is disabled', async () => {
        ...
        const result = await invocation.execute(new AbortController().signal);
        expect(result.error?.message).toBe('Loop wakeups are disabled for the rest of this session ' +
    ...
    it('reports the clamp when the requested delay is out of range', async () => {
        ...
        expect(result.llmContent).toContain('Requested 5s was clamped to the [60, 3600] s range.');
    });
    ...
    it('projects scheduling details into AUTO classifier input', () => {
        expect(tool.toAutoClassifierInput({
            delaySeconds: 300,
            prompt: 'continue loop',
            reason: 'CI is still running',
        })).toEqual({
    ...
    it('surfaces a scheduler failure as a structured tool error', async () => {
        ...
        const result = await invocation.execute(new AbortController().signal);
        expect(result.error?.message).toBe('scheduler boom (cause: clock unavailable)');
        expect(result.llmContent).toContain('Error scheduling loop wakeup:');
    });
});
//# sourceMappingURL=loop-wakeup.test.js.map