...
let capturedBackend = null;
vi.mock('../agents/backends/InProcessBackend.js', async () => {
    const { FakeBackend: FB } = await import('../agents/team/test-utils/fake-backend.js');
    return {
        InProcessBackend: class MockInProcessBackend extends FB {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            constructor(_config) {
                super();
                capturedBackend = this;
            }
        },
    };
});
...
function makeConfig() {
    ...
    return {
        ...
        setTeamManager: vi.fn((m) => {
            teamManager = m;
        }),
        getTeamContext: () => teamContext,
        setTeamContext: vi.fn((c) => {
            teamContext = c;
        }),
    };
}
...
describe('Team lifecycle E2E', () => {
    it('full lifecycle: create → tasks → messages → list → update → delete', async () => {
        ...
        const teamConfigPath = path.join(tmpDir, 'teams', 'lifecycle', 'config.json');
        ...
        const display = listResult.returnDisplay;
        ...
        const alice = backend.getAgent(aliceId);
        const bob = backend.getAgent(bobId);
        ...
        expect(alice
            .getReceivedMessages()
            .some((m) => m.includes('review the auth'))).toBe(true);
        ...
        const deleteDisplay = deleteResult.returnDisplay;
    ...
    it('team file tracks spawned members', async () => {
        ...
        const names = teamFile.members.map((m) => m.name);
    ...
    it('injects prompt addendum into spawned teammates', async () => {
        ...
        const prompt = spawnConfig?.inProcess?.runtimeConfig?.promptConfig?.systemPrompt ?? '';
