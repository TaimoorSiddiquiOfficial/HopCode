...
import { WorkflowOrchestrator, WorkflowExecutionError, createProductionDispatch, } from '../../agents/runtime/workflow-orchestrator.js';
import { WorkflowBudgetImpl, MAX_TOKENS_PER_WORKFLOW_ENV, } from '../../agents/runtime/workflow-budget.js';
...
class WorkflowToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput, _shellExecutionConfig) {
        ...
        // P5: per-run token tracker. Reads `HOPCODE_MAX_TOKENS_PER_WORKFLOW`
        ...
        const dispatch = this.toolOptions.dispatch ??
        ...
        if (this.params.scriptPath && this.params.script === undefined) {
            const loaded = await resolveSavedWorkflowScript({ scriptPath: this.params.scriptPath }, this.config);
        ...
        const runId = this.params.resumeFromRunId ?? `wf_${randomBytes(8).toString('hex')}`;
        ...
        const registryEntry = registry?.register({
            ...
            // to disk and saved to `.hopcode/workflows/<name>.js` from the dialog.
        ...
        try {
            const outcome = await orchestrator.run({
                ...
                // against the saved-workflow scripts in `.hopcode/workflows/`.
                resolveSavedWorkflow: (ref) => resolveSavedWorkflowScript(ref, this.config),
            ...
            const usageBanner = resolveUsageBanner(this.config, registry, budget.total);
            ...
            const phases = err instanceof WorkflowExecutionError ? err.phases : undefined;
            ...
            if (registryEntry) {
                if (meta && !registryEntry.meta)
                    registryEntry.meta = meta;
            }
            if (logs)
                registry?.setRecentLogs(runId, logs);
            if (signal.aborted) {
            ...
            if (registryEntry && registryEntry.status !== 'running') {
                ...
                try {
                    logWorkflowRun(this.config, new WorkflowRunEvent({
                        ...
                        duration_ms: (registryEntry.endTime ?? registryEntry.startTime) -
...
export class WorkflowTool extends BaseDeclarativeTool {
    ...
    constructor(config, toolOptions = {}) {
        super(ToolNames.WORKFLOW, ToolDisplayNames.WORKFLOW, 'Execute a workflow script that orchestrates subagents. ' +
            ...
            'spawned agents.', Kind.Other, WORKFLOW_PARAM_SCHEMA, 
