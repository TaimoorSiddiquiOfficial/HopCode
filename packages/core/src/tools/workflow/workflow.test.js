...
describe('WorkflowTool', () => {
    ...
    it('rejects build() when script is missing', () => {
        const tool = new WorkflowTool(fakeConfig());
        expect(() => tool.build({})).toThrow(/script/);
    });
    ...
    it('rejects build() when both script and scriptPath are given', () => {
        const tool = new WorkflowTool(fakeConfig());
        expect(() => tool.build({ script: 'return 1', scriptPath: '/x/y.js' })).toThrow(/exactly one/);
    });
    it('rejects build() when resumeFromRunId is not a wf_<hex> id (path-traversal guard)', () => {
        const tool = new WorkflowTool(fakeConfig());
        expect(() => tool.build({ script: 'return 1', resumeFromRunId: '../../etc/evil' })).toThrow(/resumeFromRunId/);
    ...
    it('execute() runs parallel() end-to-end and returns the revived array', async () => {
        ...
        const result = await invocation.execute(new AbortController().signal);
        expect(result.error).toBeUndefined();
        const llmText = result.llmContent[0].text;
    ...
    it('execute() runs agent({schema}) end-to-end and returns the revived object', async () => {
        ...
        const invocation = tool.build({
            script: 'const r = await agent("hello", { schema: { type: "object", properties: { extracted: { type: "string" } } } }); return r;',
        });
        const result = await invocation.execute(new AbortController().signal);
        expect(result.error).toBeUndefined();
        const llmText = result.llmContent[0].text;
    ...
    // PR #4947 R2 T8 (hopcode-ci-bot): pipeline() through WorkflowTool
    ...
    it('R7: dialog-cancel race during run — logs accumulated before cancel survive', async () => {
        ...
        const dispatch = async (prompt) => new Promise((_resolve, reject) => {
      ...
      `,
        ...
        for (let i = 0; i < 200; i++) {
            if (registry.list().length > 0 && dispatchInflight)
                break;
        ...
        expect(final.recentLogs.some((l) => l.includes('before agent dispatch'))).toBe(true);
    });
    ...
    it('P5 T7: prepends the usage banner on the first run only', async () => {
        ...
        expect(first.returnDisplay).toMatch(/Workflows have no per-run token cap|Workflow token cap is/);
        expect(first.returnDisplay).toMatch(/skipWorkflowUsageWarning/);
        // Second invocation: latch already flipped on the registry.
        const second = await tool
            .build({ script: 'return 2' })
            .execute(new AbortController().signal);
        expect(second.returnDisplay).not.toMatch(/skipWorkflowUsageWarning/);
    ...
    it('P5 T7: suppressed by skipWorkflowUsageWarning setting', async () => {
        ...
        const result = await tool
            .build({ script: 'return 1' })
            .execute(new AbortController().signal);
        expect(result.returnDisplay).not.toMatch(/skipWorkflowUsageWarning/);
    ...
    it('P5 T7 R1: failure path does NOT emit banner or consume the latch', async () => {
        ...
        const failed = await tool
            ...
            .execute(new AbortController().signal);
        expect(failed.returnDisplay).not.toMatch(/skipWorkflowUsageWarning/);
        expect(failed.returnDisplay).toMatch(/Workflow failed: /);
    ...
    it('P5 T7 R1: failed-then-succeeded → banner appears on the SUCCESS run', async () => {
        ...
        const success = await tool
            .build({ script: 'return 1' })
            .execute(new AbortController().signal);
        expect(success.returnDisplay).toMatch(/skipWorkflowUsageWarning/);
    ...
    it('P5 R1 #10: capped banner shape (`total !== null`) — was untested', async () => {
        ...
        const originalEnv = process.env['HOPCODE_MAX_TOKENS_PER_WORKFLOW'];
        process.env['HOPCODE_MAX_TOKENS_PER_WORKFLOW'] = '50000';
        try {
            const tool = new WorkflowTool(config, { dispatch: async () => 'ok' });
            const result = await tool
                .build({ script: 'return 1' })
            ...
            if (originalEnv === undefined) {
                delete process.env['HOPCODE_MAX_TOKENS_PER_WORKFLOW'];
            }
            else {
                process.env['HOPCODE_MAX_TOKENS_PER_WORKFLOW'] = originalEnv;
            }
        }
    });
});
