...
describe('RipGrepTool', () => {
    ...
    describe('validateToolParams', () => {
        ...
        it('should return null for a positive integer limit', () => {
            const params = { pattern: 'hello', limit: 2 };
            expect(grepTool.validateToolParams(params)).toBeNull();
        });
        it.each([
            ...
            const params = { pattern: 'hello', limit };
    ...
    describe('execute', () => {
        ...
        it('should pass .hopcodeignore to ripgrep when respected', async () => {
            await fs.writeFile(path.join(tempRootDir, '.hopcodeignore'), 'ignored.txt\n');
            runRipgrep.mockResolvedValue({
                stdout: '',
                truncated: false,
                error: undefined,
        ...
        it('should pass useBuiltinRipgrep setting to ripgrep execution', async () => {
            ...
            await invocation.execute(abortSignal);
            expect(runRipgrep).toHaveBeenCalledWith(expect.any(Array), abortSignal, false);
        });
    });
    describe('multi-directory workspace', () => {
        ...
        it('should load .hopcodeignore from each workspace directory', async () => {
            const secondDir = await fs.mkdtemp(path.join(os.tmpdir(), 'grep-tool-second-'));
            await fs.writeFile(path.join(secondDir, '.hopcodeignore'), 'ignored.txt\n');
            await fs.writeFile(path.join(tempRootDir, '.hopcodeignore'), 'other-ignored.txt\n');
            const multiDirConfig = {
                ...mockConfig,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [secondDir]),
            };
            ...
            const invocation = multiDirGrepTool.build(params);
            await invocation.execute(abortSignal);
            // Verify both .hopcodeignore files were passed
        ...
        it('should pass .agentignore and .aiignore to ripgrep when respected', async () => {
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), 'agent-secret.txt\n');
            await fs.writeFile(path.join(tempRootDir, '.aiignore'), 'ai-secret.txt\n');
            runRipgrep.mockResolvedValue({
                stdout: '',
                truncated: false,
                error: undefined,
        ...
        it('should pass non-qwen ignore files unchanged so ripgrep preserves negations', async () => {
            ...
            runRipgrep.mockImplementation(async (rgArgs) => {
                const ignoreFileArgs = rgArgs.filter((a, i) => i > 0 && rgArgs[i - 1] === '--ignore-file');
                expect(ignoreFileArgs).toContain(hopcodeignorePath);
                expect(ignoreFileArgs).toContain(agentIgnorePath);
                expect(ignoreFileArgs.indexOf(agentIgnorePath)).toBeLessThan(ignoreFileArgs.indexOf(hopcodeignorePath));
        ...
        it('should preserve negation semantics within the same non-qwen ignore file', async () => {
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), '*.env\n!allowed.env\n');
        ...
        it('should post-filter matches unignored by a custom nested .hopcodeignore', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, '.hopcodeignore'), '*.env\n');
            await fs.writeFile(path.join(tempRootDir, 'nested', '.hopcodeignore'), '!*.env\n');
            await fs.writeFile(path.join(tempRootDir, 'secret.env'), 'API_KEY=1');
            await fs.writeFile(path.join(tempRootDir, 'visible.txt'), 'API_KEY=2');
            Object.assign(mockConfig, {
                getFileFilteringOptions: () => ({
        ...
        it('should pass configured custom ignore files to ripgrep', async () => {
            await fs.writeFile(path.join(tempRootDir, '.cursorignore'), 'cursor-secret.txt\n');
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), 'agent-secret.txt\n');
            Object.assign(mockConfig, {
                getFileFilteringOptions: () => ({
                    respectGitIgnore: true,
            ...
            expect(ignoreFileArgs).not.toContain(path.join(tempRootDir, '.agentignore'));
        });
        it('should resolve ignore files from the workspace root for subdirectory searches', async () => {
            await fs.writeFile(path.join(tempRootDir, '.cursorignore'), 'cursor-secret.txt\n');
            await fs.writeFile(path.join(tempRootDir, 'sub', '.cursorignore'), 'sub-secret.txt\n');
            Object.assign(mockConfig, {
                getFileFilteringOptions: () => ({
                    respectGitIgnore: true,
                    respecthopcodeignore: true,
            ...
            const ignoreFileArgs = rgArgs.filter((a, i) => i > 0 && rgArgs[i - 1] === '--ignore-file');
            expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.cursorignore'));
            expect(ignoreFileArgs).not.toContain(path.join(tempRootDir, 'sub', '.cursorignore'));
        });
        it('should not load ignore files from relative external search paths', async () => {
            const testCwd = await fs.mkdtemp(path.join(os.tmpdir(), 'grep-tool-cwd-'));
            const outsideDir = path.join(testCwd, 'outside');
            const originalCwd = process.cwd();
            try {
                await fs.mkdir(outsideDir);
                await fs.writeFile(path.join(outsideDir, '.cursorignore'), 'cursor-secret.txt\n');
                Object.assign(mockConfig, {
                    getFileFilteringOptions: () => ({
                        respectGitIgnore: true,
                        respecthopcodeignore: true,
        ...
        it('should cache resolved relative result paths across filtering and result metadata', async () => {
            ...
            const fileAProbeCount = existsSyncSpy.mock.calls.filter(([candidate]) => String(candidate) === fileAPath).length;
    ...
    describe('getDescription', () => {
        ...
        it('should generate correct description with pattern and path', async () => {
            ...
            const invocation = grepTool.build(params);
            expect(invocation.getDescription()).toBe(`'testPattern' in ${path.join('src', 'app')}`);
        });
        ...
        it('should generate correct description with pattern, glob, and path', async () => {
            ...
            const invocation = grepTool.build(params);
            expect(invocation.getDescription()).toBe(`'testPattern' in ${path.join('src', 'app')} (filter: '*.ts')`);
        });
        ...
        it('should keep paths outside the project absolute (never project-relative)', () => {
            ...
            const invocation = grepTool.build(params);
            expect(invocation.getDescription()).toBe(`'testPattern' in ${tildeifyPath(outside)}`);
        });
    });
