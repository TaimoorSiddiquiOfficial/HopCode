...
describe('LSTool', () => {
    ...
    beforeEach(async () => {
        ...
        const userSkillsBase = path.join(os.homedir(), '.hopcode', 'skills');
        mockConfig = {
    ...
    describe('execute', () => {
        ...
        it('should respect agent and ai ignore patterns', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), 'agent-secret.log');
        ...
        it('should respect configured custom qwen ignore files', async () => {
            await fs.writeFile(path.join(tempRootDir, 'file1.txt'), 'content1');
            await fs.writeFile(path.join(tempRootDir, 'cursor-secret.log'), 'content');
            await fs.writeFile(path.join(tempRootDir, 'agent-secret.log'), 'content');
            await fs.writeFile(path.join(tempRootDir, '.cursorignore'), 'cursor-secret.log');
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), 'agent-secret.log');
            const customConfig = {
                ...mockConfig,
