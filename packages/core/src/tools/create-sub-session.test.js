...
import { CreateSubSessionTool, MAX_SUB_SESSION_PROMPT_CHARS, } from './create-sub-session.js';
function makeConfig(spawner) {
    return {
...
describe('CreateSubSessionTool', () => {
    it('has the correct name', () => {
        expect(new CreateSubSessionTool(makeConfig()).name).toBe('create_sub_session');
    });
    ...
    it('returns as soon as the turn is cancelled, without waiting for the spawn', async () => {
        ...
        let rejectSpawn;
        const spawner = vi.fn(() => new Promise((_, reject) => {
            rejectSpawn = reject;
        }));
    ...
    it('rejects a prompt over the character limit before reaching the spawner', async () => {
        ...
        const tool = new CreateSubSessionTool(makeConfig(spawner));
        expect(() => tool.build({ prompt: 'x'.repeat(MAX_SUB_SESSION_PROMPT_CHARS + 1) })).toThrow(new RegExp(`${MAX_SUB_SESSION_PROMPT_CHARS}`));
        expect(spawner).not.toHaveBeenCalled();
        // The boundary itself is accepted.
        expect(() => tool.build({ prompt: 'x'.repeat(MAX_SUB_SESSION_PROMPT_CHARS) })).not.toThrow();
    });
});
