...
describe('WriteFileTool', () => {
    ...
    describe('shouldConfirmExecute', () => {
        ...
        it('auto-allows private memory writes but proposes team memory writes', async () => {
            const prev = process.env['HOPCODE_CODE_MEMORY_LOCAL'];
            process.env['HOPCODE_CODE_MEMORY_LOCAL'] = '1';
            clearAutoMemoryRootCache();
            try {
                const privatePath = path.join(rootDir, '.hopcode', 'memory', 'user', 'x.md');
                const teamPath = path.join(rootDir, '.hopcode', 'team-memory', 'feedback', 'x.md');
                expect(await tool
                    .build({ file_path: privatePath, content: 'c' })
                    .getDefaultPermission()).toBe('allow');
                expect(await tool
                ...
                if (prev === undefined) {
                    ...
                    process.env['HOPCODE_CODE_MEMORY_LOCAL'] = prev;
                }
                clearAutoMemoryRootCache();
            }
        });
        it('blocks writing a secret to a team-memory path', () => {
            const params = {
                file_path: path.join(rootDir, '.hopcode', 'team-memory', 'feedback.md'),
                content: `token = ghp_${'a'.repeat(36)}`,
            };
            expect(() => tool.build(params)).toThrow(/shared with all repository collaborators/i);
        });
        it('blocks a secret added to team-memory content before execute', async () => {
            const filePath = path.join(rootDir, '.hopcode', 'team-memory', 'feedback.md');
            ...
            expect(JSON.stringify(result)).toMatch(/shared with all repository collaborators/i);
            // The blocked write must carry an `error` field so the framework
            // treats it as a failure, not a silent success.
            expect(result.error?.type).toBe(ToolErrorType.INVALID_TOOL_PARAMS);
            expect(result.error?.message).toMatch(/shared with all repository collaborators/i);
    ...
    describe('execute', () => {
        ...
        it('reminds the model to record artifact-like workspace files', async () => {
            ...
            expect(result.llmContent).toContain('workspacePath "reports/weather.html"');
        ...
        it('reminds for case-insensitive artifact extensions', async () => {
            ...
            const result = await tool.build(params).execute(abortSignal);
            expect(result.llmContent).toContain('record_artifact');
            expect(result.llmContent).toContain('workspacePath "reports/dashboard.HTML"');
        });
    ...
    describe('specific error types for write failures', () => {
        ...
        it('should include cause details for non-Node write errors', async () => {
            ...
            vi.spyOn(fsService, 'writeTextFile').mockRejectedValueOnce(new TypeError('fetch failed', { cause }));
            const params = { file_path: filePath, content };
            const invocation = tool.build(params);
            const result = await invocation.execute(abortSignal);
            expect(result.error?.type).toBe(ToolErrorType.FILE_WRITE_FAILURE);
            expect(result.llmContent).toContain('Error writing to file: fetch failed (cause: ECONNREFUSED)');
            expect(result.returnDisplay).toContain('Error writing to file: fetch failed (cause: ECONNREFUSED)');
        });
        it('should surface plain object write error messages without object stringification', async () => {
            ...
            const result = await invocation.execute(abortSignal);
            expect(result.error?.type).toBe(ToolErrorType.FILE_WRITE_FAILURE);
            expect(result.llmContent).toContain('Error writing to file: Plain object write error');
