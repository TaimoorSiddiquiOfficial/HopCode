...
describe('McpClientManager', () => {
    ...
    it('reports status from its own client instance', async () => {
        ...
        expect(manager.getServerStatus('missing')).toBe(MCPServerStatus.DISCONNECTED);
    });
    it('routes discovery through the pool when one is injected (F2 commit 4)', async () => {
        ...
        expect(acquireSpy).toHaveBeenCalledWith('srv', {}, 'sid-1', expect.anything(), expect.anything(), expect.anything());
    ...
    it('swallows BudgetExhaustedError from pool.acquire and logs at debug (F2 commit 6 W23)', async () => {
        ...
        const acquireSpy = vi.fn().mockImplementation((name) => {
            if (name === 'srvB') {
    ...
    it('pool path skips a gated server pending approval — no acquire, no spawn (#4615, sub-task 3)', async () => {
        ...
        const mockConfig = {
            ...
            getSessionId: () => 'sid-1',
            isMcpServerDisabled: () => false,
            isMcpServerPendingApproval: (name) => name === 'gated',
        };
        const manager = mkManager({
            config: mockConfig,
            options: { pool: fakePool },
        ...
        expect(acquireSpy).toHaveBeenCalledTimes(1);
        expect(acquireSpy).toHaveBeenCalledWith('ok', {}, 'sid-1', expect.anything(), expect.anything(), expect.anything());
        expect(McpClient).not.toHaveBeenCalled();
    });
    ...
    it('stop() awaits in-flight pool discovery before releasing pool connections (W94/W108/W112)', async () => {
        ...
        const events = [];
        const acquireSpy = vi.fn().mockImplementation(async (name) => {
    ...
    it('stop() proceeds when injected discoveryInFlight rejects (W94/W108/W112/W116 rejection path)', async () => {
        ...
        manager.discoveryInFlight = rejected;
    ...
    it('stop() proceeds when discoveryInFlight exceeds the 5s grace cap (W108/W116 timeout path)', async () => {
        ...
        try {
            ...
            manager.discoveryInFlight = new Promise(() => {
                /* never resolves */
            });
            const stopPromise = manager.stop();
            ...
            expect(manager.stopTimedOut).toBe(true);
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('discovery resets stopTimedOut so manager remains usable after a timed-out shutdown (W118)', async () => {
        ...
        await manager.discoverAllMcpTools(mockConfig);
        expect(manager.stopTimedOut).toBe(false);
        // Connection MUST have been tracked (not silently released by the
        // sticky-flag guard).
        expect(manager.pooledConnections.has('srv')).toBe(true);
    });
    ...
    it('readResource self-heals when pooled handle is dead (R24 T19)', async () => {
        ...
        let mockedStatus = MCPServerStatus.CONNECTED;
        const acquireSpy = vi.fn().mockResolvedValue({
            release: vi.fn(),
            on: vi.fn(),
            id: 'srv::abc',
        ...
        await expect(manager.readResource('srv', 'mcp://srv/doc')).rejects.toThrow(/pool entry disconnected; retry after discovery/);
        // Handle evicted — confirms self-heal cleanup ran.
        const pooledMap = manager.pooledConnections;
    ...
    it('serializes concurrent discovery passes via mutex (F2 commit 6 W6)', async () => {
        ...
        let resolveAcquire;
        const blockedAcquire = new Promise((resolve) => {
    ...
    it('returns instructions from connected clients', async () => {
        vi.mocked(McpClient).mockImplementation((name) => ({
            ...
            getInstructions: vi
                .fn()
                .mockReturnValue(name === 'with-instructions' ? 'Use concise replies.' : undefined),
        }));
        const mockConfig = {
            isTrustedFolder: () => true,
            getMcpServers: () => ({
        ...
        await manager.discoverAllMcpTools(mockConfig);
        expect(manager.getServerInstructions()).toEqual(new Map([['with-instructions', 'Use concise replies.']]));
    });
    ...
    it('should not connect a project server that is pending approval (#4615)', async () => {
        ...
        const mockConfig = {
            ...
            getDebugMode: () => false,
            isMcpServerDisabled: () => false,
            isMcpServerPendingApproval: (name) => name === 'pending-server',
        };
        const manager = new McpClientManager(mockConfig, {});
        await manager.discoverAllMcpTools(mockConfig);
    ...
    describe('discovery timeout process cleanup', () => {
        function makeTimedOutClient(rootPid) {
            return {
        ...
        async function runTimedOutDiscovery(mockedMcpClient, serverConfig) {
            vi.mocked(McpClient).mockReturnValue(mockedMcpClient);
            const config = {
                isTrustedFolder: () => true,
        ...
        it('still disconnects when descendant enumeration fails', async () => {
            ...
            vi.mocked(listDescendantPids).mockClear();
            vi.mocked(sigtermPids).mockClear();
            vi.mocked(listDescendantPids).mockRejectedValueOnce(new Error('process table unavailable'));
            await runTimedOutDiscovery(mockedMcpClient, {
                command: 'node',
                args: [],
                discoveryTimeoutMs: 100,
...
describe('McpClientManager — PR 14 guardrails', () => {
    ...
    it('discoverMcpToolsForServerInternal rejects pending-approval servers', async () => {
        ...
        const config = configWithServers({ a: { command: 'node' } }, {
            isMcpServerPendingApproval: ((name) => name === 'a'),
        });
    ...
    it('readBudgetFromEnv emits stderr warning on invalid budget value (wenshao R7 #6 line 191)', async () => {
        ...
        try {
            ...
            expect(calls.some((s) => s.includes('ignoring invalid HOPCODE_SERVE_MCP_CLIENT_BUDGET') &&
    ...
    it('readBudgetFromEnv rejects non-decimal budget values (hex / scientific / float)', async () => {
        ...
        try {
            for (const bad of ['0x10', '1e2', '1.0']) {
                process.env['HOPCODE_SERVE_MCP_CLIENT_BUDGET'] = bad;
            ...
            process.env['HOPCODE_SERVE_MCP_CLIENT_BUDGET'] = '16';
    ...
    it('readResource rejects existing-but-now-pending servers', async () => {
        ...
        let pending = false;
        const config = configWithServers({ a: { command: 'node' } }, {
            isMcpServerPendingApproval: ((name) => name === 'a' && pending),
        });
        const manager = new McpClientManager(config, {});
        await manager.discoverAllMcpTools(config);
        pending = true;
        await expect(manager.readResource('a', 'file:///x')).rejects.toThrow(/'a' is pending approval/);
    });
    ...
    it('readBudgetFromEnv emits stderr breadcrumb on enforce-no-budget downgrade (wenshao R9 #7)', async () => {
        ...
        try {
            ...
            expect(manager.getMcpBudgetMode()).toBe('off');
            const calls = writeSpy.mock.calls.map((c) => String(c[0]));
            expect(calls.some((s) => s.includes('HOPCODE_SERVE_MCP_BUDGET_MODE=enforce') &&
...
describe('McpClientManager — addRuntimeMcpServer / removeRuntimeMcpServer (T2.8)', () => {
    ...
    it('case 1: happy fresh add → replaced=false, correct transport and toolCount', async () => {
        ...
        const result = await manager.addRuntimeMcpServer('my-server', { command: 'echo', args: ['hello'] }, 'client-1');
        ...
        expect(acquireSpy).toHaveBeenCalledTimes(1);
        expect(config.addRuntimeMcpServer).toHaveBeenCalledWith('my-server', expect.objectContaining({ command: 'echo' }));
    });
    it('case 2: budget enforce + at-cap + new name → throws McpBudgetWouldExceedError', async () => {
        ...
        const config = mkRuntimeConfig();
        const manager = mkManager({ config, options: { pool: fakePool } });
        await expect(manager.addRuntimeMcpServer('new-server', { command: 'node', args: ['server.js'] }, 'client-2')).rejects.toThrow(McpBudgetWouldExceedError);
    ...
    it('case 3: budget warn + at-cap + new name → skipped with budget_warning_only', async () => {
        ...
        const config = mkRuntimeConfig();
        const manager = mkManager({ config, options: { pool: fakePool } });
        const result = await manager.addRuntimeMcpServer('warn-server', { command: 'node', args: ['server.js'] }, 'client-3');
        expect(result).toEqual({
            name: 'warn-server',
            skipped: true,
    ...
    it('case 4: replace same name + same fingerprint → replaced=true, pool.acquire NOT re-called', async () => {
        ...
        const result = await manager.addRuntimeMcpServer('dup-srv', serverConfig, 'client-4');
    ...
    it('case 5: shadows settings → shadowedSettings=true', async () => {
        ...
        const manager = mkManager({ config, options: { pool: fakePool } });
        const result = await manager.addRuntimeMcpServer('shadow-srv', { command: 'new-cmd', args: [] }, 'client-5');
        expect(result).toMatchObject({
            name: 'shadow-srv',
    ...
    it('case 5b: ifAbsent skips before replacing a different runtime server', async () => {
        ...
        const manager = mkManager({ config, options: { pool: fakePool } });
        const result = await manager.addRuntimeMcpServer('runtime-srv', {
            command: 'new-cmd',
    ...
    it('case 1: removes runtime entry → removed=true, wasShadowingSettings=false', async () => {
        ...
        const manager = mkManager({ config, options: { pool: fakePool } });
        // Add then remove
        await manager.addRuntimeMcpServer('rm-srv', { command: 'echo' }, 'client-6');
    ...
    it('case 2: removes shadow over settings → wasShadowingSettings=true', async () => {
        ...
        await manager.addRuntimeMcpServer('shadow-rm', { command: 'runtime-cmd' }, 'client-7');
        const result = await manager.removeRuntimeMcpServer('shadow-rm', 'client-7');
        expect(result).toMatchObject({
            name: 'shadow-rm',
            removed: true,
    ...
    it('throws InvalidMcpConfigError for config with unknown transport', async () => {
        ...
        const config = mkRuntimeConfig();
        const manager = mkManager({ config });
        await expect(manager.addRuntimeMcpServer('bad-cfg', {}, 'client-9')).rejects.toThrow(InvalidMcpConfigError);
    });
    it('throws McpServerSpawnFailedError when pool.acquire rejects', async () => {
        ...
        const config = mkRuntimeConfig({ runtimeRemoveSpy: removeSpy });
        const manager = mkManager({ config, options: { pool: fakePool } });
        await expect(manager.addRuntimeMcpServer('fail-srv', { command: 'bad-binary' }, 'client-10')).rejects.toThrow(McpServerSpawnFailedError);
