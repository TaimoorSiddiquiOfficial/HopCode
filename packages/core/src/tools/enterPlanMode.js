...
const enterPlanModeToolSchemaData = {
    ...
    parametersJsonSchema: {
        ...
        properties: {
            userRequested: {
                type: 'boolean',
                description: 'Set to true ONLY when the user explicitly asked for plan mode in this turn, or explicitly confirmed they want it. Leave unset (or false) when you are deciding to plan on your own without the user asking. In IZN mode, an explicit user request will not take effect unless this is true.',
            },
        },
...
class EnterPlanModeToolInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        ...
        if (this.config.getApprovalMode() === ApprovalMode.IZN &&
            ...
            debugLogger.info('Blocked model-initiated plan entry from IZN (userRequested=%s)', this.params.userRequested);
            return {
                llmContent: 'Plan mode was not entered: the session is in IZN mode, which the user explicitly chose for low-friction execution. Continue investigating and presenting your plan in the current mode without switching. If the user explicitly asked for plan mode in this turn, retry this tool call with userRequested: true.',
