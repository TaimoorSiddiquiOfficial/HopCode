...
describe('isTransientNetworkError', () => {
    ...
    it('returns true for "status code 502"', () => {
        expect(isTransientNetworkError(new Error('Request failed with status code 502'))).toBe(true);
    });
    ...
    it('returns true for "HTTP/1.1 504"', () => {
        expect(isTransientNetworkError(new Error('HTTP/1.1 504 Gateway Timeout'))).toBe(true);
    });
    ...
    it('returns true for Streamable HTTP connection error', () => {
        expect(isTransientNetworkError(new Error('Streamable HTTP connection failed'))).toBe(true);
    });
    ...
    it('returns true for error with ECONNRESET in a longer message', () => {
        expect(isTransientNetworkError(new Error('read ECONNRESET at TCPReadWrap.afterCall'))).toBe(true);
    });
    it('returns false for message containing 502 as non-status number', () => {
        expect(isTransientNetworkError(new Error('processed 502 items successfully'))).toBe(false);
    });
    it('returns false for message containing 503 as non-status number', () => {
        expect(isTransientNetworkError(new Error('timeout after 503ms'))).toBe(false);
    });
});
describe('retryWithBackoff', () => {
    it('returns the result on first successful attempt', async () => {
        const result = await retryWithBackoff(() => Promise.resolve('success'), 'test-label');
    ...
    it('retries up to maxRetries and then throws', async () => {
        ...
        await expect(retryWithBackoff(fn, 'test-exhaust', { maxRetries: 2 })).rejects.toThrow('ECONNRESET');
    ...
    it('does not retry on permanent errors (401)', async () => {
        ...
        await expect(retryWithBackoff(fn, 'test-permanent')).rejects.toThrow('401 Unauthorized');
    ...
    it('does not retry on method-not-found (-32601)', async () => {
        ...
        await expect(retryWithBackoff(fn, 'test-method-not-found')).rejects.toMatchObject({ code: -32601 });
        expect(fn).toHaveBeenCalledTimes(1);
    });
    ...
    it('respects custom maxRetries=1 (single retry)', async () => {
        ...
        await expect(retryWithBackoff(fn, 'test-single-retry', { maxRetries: 1 })).rejects.toThrow('ECONNRESET');
        expect(fn).toHaveBeenCalledTimes(2);
    });
    ...
    it('rejects immediately when signal is already aborted', async () => {
        ...
        await expect(retryWithBackoff(fn, 'test-pre-aborted', {
            maxRetries: 2,
            baseDelayMs: 10000,
            signal: controller.signal,
...
//# sourceMappingURL=mcp-retry.test.js.map