...
import { readWorktreeSession, writeWorktreeSession, } from '../services/worktreeSessionService.js';
...
describe('ExitWorktreeTool — WorktreeSession sidecar cleanup', () => {
    ...
    it('removes a re-attached worktree when the old marker owner is inactive', async () => {
        ...
        const wtPath = new GitWorktreeService(repoRoot).getUserWorktreePath('reattached-stale');
        const currentSessionId = 'new-session';
        const currentSessionService = new SessionService(wtPath);
        const currentSessionPath = currentSessionService.getWorktreeSessionPath(currentSessionId);
        await writeRuntimeStatus(new Storage(wtPath).getRuntimeStatusPath('old-session'), {
            sessionId: 'old-session',
            workDir: wtPath,
            pid: 2147483647,
        });
    ...
    it('removes a stale-owned worktree when launched from inside it without a sidecar', async () => {
        ...
        const wtPath = new GitWorktreeService(repoRoot).getUserWorktreePath('cwd-stale');
        ...
        if (details.type === 'exec') {
            ...
            expect(details.command).not.toContain(path.join(nestedCwd, '.hopcode', 'worktrees', 'cwd-stale'));
        }
    ...
    it('refuses a re-attached remove when the marker owner runtime is active', async () => {
        ...
        const wtPath = new GitWorktreeService(repoRoot).getUserWorktreePath('reattached-active');
        await writeRuntimeStatus(new Storage(wtPath).getRuntimeStatusPath('old-session'), {
            sessionId: 'old-session',
            workDir: wtPath,
            pid: process.pid,
        ...
        await writeWorktreeSession(currentSessionService.getWorktreeSessionPath(currentSessionId), {
        ...
        const result = await new ExitWorktreeTool(exitConfig)
            ...
            .execute(new AbortController().signal);
        expect(result.error?.message).toMatch(/different session.*owner=old-session/i);
    ...
    it('refuses a re-attached remove when the owner is active under a repo-subdir relative runtime dir', async () => {
        ...
        const wtPath = new GitWorktreeService(repoRoot).getUserWorktreePath('reattached-relative-active');
        const packageDir = path.join(repoRoot, 'packages', 'app');
        await fs.mkdir(packageDir, { recursive: true });
        Storage.setRuntimeBaseDir('.hopcode', packageDir);
        await writeRuntimeStatus(new Storage(packageDir).getRuntimeStatusPath('old-session'), {
            sessionId: 'old-session',
            workDir: packageDir,
            pid: process.pid,
        });
        Storage.setRuntimeBaseDir('.hopcode', wtPath);
        const currentSessionId = 'new-session';
        const currentSessionService = new SessionService(wtPath);
        const originalHeadCommit = execFileSync('git', ['rev-parse', 'HEAD'], {
            cwd: repoRoot,
