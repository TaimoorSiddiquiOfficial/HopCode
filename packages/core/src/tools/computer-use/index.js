 ...
 * approval mode. In IZN the scheduler auto-approves the tool call and skips
...
export async function registerComputerUseTools(registerLazy, config) {
    for (const upstreamName of COMPUTER_USE_TOOL_NAMES) {
        ...
        await registerLazy(hopcodeName, async () => new ComputerUseTool(upstreamName, schema, config));
    }
}
