...
import { ComputerUseTool, buildLlmContent, buildDisplayText, coerceTypes, isHighRiskCall, } from './tool.js';
...
function makeFakeClient(callToolImpl) {
...
describe('ComputerUseTool', () => {
    ...
    it('marks itself as deferred', () => {
        const tool = new ComputerUseTool('list_apps', COMPUTER_USE_SCHEMAS.list_apps);
    ...
    it('resolves the configured maxImageDimension and forwards it to the client', async () => {
        ...
        try {
            ...
            const tool = new ComputerUseTool('list_apps', COMPUTER_USE_SCHEMAS.list_apps, config);
...
describe('coerceTypes', () => {
    ...
    it('does not coerce non-numeric string fields like app', () => {
        const result = coerceTypes({ app: 'com.apple.stocks', element_index: 5 }, schema);
...
describe('ComputerUseTool.build() coercion integration', () => {
    ...
    it('build() forwards the coerced integer element_index to the client', async () => {
        ...
        expect(fake.callTool).toHaveBeenCalledWith('click', expect.objectContaining({ element_index: 11 }));
    });
...
vi.mock('./install-state.js', async (importOriginal) => {
    ...
    return {
        ...real,
        isPackageSpecApproved: vi.fn(async (_home, spec) => real.isPackageSpecApproved(mockHome, spec)),
        saveInstallState: vi.fn(async (_home, state) => real.saveInstallState(mockHome, state)),
        loadInstallState: vi.fn(async (_home) => real.loadInstallState(mockHome)),
    };
});
describe('ComputerUseInvocation confirmation pathway', () => {
    ...
    // approval. With HOPCODE_COMPUTER_USE_AUTO_APPROVE unset, the bootstrap fallback
    // used to refuse and surface "install declined by user":
    //   - IZN       → needsConfirmation() returns false, dialog never built.
    ...
    it('flags destructive / sensitive tools as high-risk', () => {
        ...
        expect(isHighRiskCall('page', { action: 'enable_javascript_apple_events' })).toBe(true);
    });
...
describe('buildLlmContent', () => {
    ...
    it('returns Part[] when content includes an image part', () => {
        ...
        expect(parts.some((p) => p.text?.includes('image') && p.text.includes('image/png'))).toBe(true);
    ...
    it('forwards structuredContent (real window_ids) the terse text omits', () => {
        ...
        const content = [{ type: 'text', text: 'Found 11 window(s).' }];
        const structured = {
            windows: [
        ...
        expect(result).toContain('is_on_screen');
    });
    it('strips tree_markdown from structuredContent (already in the text)', () => {
        const content = [
            { type: 'text', text: 'window_id=358 pid=717 elements=457' },
        ];
        const structured = {
            window_id: 358,
            element_count: 457,
...
describe('execute() image content forwarding', () => {
    ...
    it('llmContent is Part[] containing inlineData when MCP returns an image', async () => {
        ...
        ComputerUseClient.setSharedForTest(fake);
        const tool = new ComputerUseTool('get_window_state', COMPUTER_USE_SCHEMAS.get_window_state);
