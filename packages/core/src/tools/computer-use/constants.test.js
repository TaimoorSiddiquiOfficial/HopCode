...
import { CUA_DRIVER_VERSION, approvalKey, binaryPath, resolveAssetTarget, resolveAssetUrls, resolveChecksumUrls, resolveMaxImageDimension, } from './constants.js';
...
describe('resolveAssetTarget', () => {
    it('maps darwin/arm64 to the .app-bearing tarball, spawning the in-bundle binary', () => {
        ...
        expect(t.asset).toBe(`cua-driver-rs-${CUA_DRIVER_VERSION}-darwin-arm64.tar.gz`);
    ...
    it('maps darwin/x64 to the x86_64 tarball', () => {
        expect(resolveAssetTarget('darwin', 'x64').asset).toBe(`cua-driver-rs-${CUA_DRIVER_VERSION}-darwin-x86_64.tar.gz`);
    });
    it('maps linux/x64 to the -binary tarball whose lone cua-driver sits at the archive root', () => {
        ...
        expect(t.asset).toBe(`cua-driver-rs-${CUA_DRIVER_VERSION}-linux-x86_64-binary.tar.gz`);
    ...
    it('maps win32/x64 to the .zip with .exe binary', () => {
        ...
        expect(t.asset).toBe(`cua-driver-rs-${CUA_DRIVER_VERSION}-windows-x86_64.zip`);
    ...
    it('throws on unsupported platforms / arches', () => {
        ...
        expect(() => resolveAssetTarget('aix', 'x64')).toThrow(/unsupported/i);
    });
});
describe('resolveAssetUrls', () => {
    it('orders sources OSS-first, GitHub-fallback by default', () => {
        ...
        expect(urls[0]).toContain('hopcode-assets.oss-cn-hangzhou.aliyuncs.com/computer-use');
    ...
    it('prepends HOPCODE_COMPUTER_USE_DOWNLOAD_HOST as the first source', () => {
        ...
        expect(urls[0]).toBe(`https://mirror.internal/cua-driver-rs/v${CUA_DRIVER_VERSION}/a.tar.gz`);
    });
...
describe('binaryPath', () => {
    it('resolves to the in-bundle binary under ~/.hopcode/computer-use/...', () => {
        ...
        expect(p).toBe(join('/home/u', '.hopcode', 'computer-use', `cua-driver-rs-${CUA_DRIVER_VERSION}`, `cua-driver-rs-${CUA_DRIVER_VERSION}-darwin-arm64`, 'CuaDriver.app', 'Contents', 'MacOS', 'cua-driver'));
    });
    it('resolves the Linux binary at the version-dir root (no wrapper dir)', () => {
        const p = binaryPath('/home/u', 'linux', 'x64');
        expect(p).toBe(join('/home/u', '.hopcode', 'computer-use', `cua-driver-rs-${CUA_DRIVER_VERSION}`, 'cua-driver'));
    });
});
...
describe('resolveMaxImageDimension', () => {
    ...
    it('treats 0 as an explicit "no resize" override (full resolution)', () => {
        ...
        expect(resolveMaxImageDimension(undefined, {
    ...
    it('lets the env var override the setting', () => {
        expect(resolveMaxImageDimension(1024, {
    ...
    it('rejects a non-integer / non-finite setting value (no override)', () => {
        ...
        expect(resolveMaxImageDimension(Number.POSITIVE_INFINITY, {})).toBeUndefined();
    });
