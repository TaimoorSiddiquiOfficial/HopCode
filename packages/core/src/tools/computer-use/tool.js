...
const INSTALL_REASON = 'This downloads the Computer Use driver (~20MB, signed + notarized) into ~/.hopcode/computer-use/ the first time. ' +
...
class ComputerUseInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput) {
        ...
        // written): IZN (needsConfirmation() returns false), AUTO_EDIT
...
export class ComputerUseTool extends BaseDeclarativeTool {
    ...
    constructor(upstreamName, schema, config) {
        ...
        super(hopcodeName, hopcodeName, // displayName == name; no MCP branding in UI
