...
describe('runBootstrap', () => {
  ...
  beforeEach(() => {
    ...
    deps = {
      homeDir: tmpHome,
      packageSpec: '@hopcode/open-computer-use@^0.3.0',
      platform: 'darwin',
  ...
  it('starts the client when binary is approved + permissions ok', async () => {
    ...
    await saveInstallState(tmpHome, {
      approvedPackageSpec: '@hopcode/open-computer-use@^0.3.0',
  ...
  it('auto-approves install under IZN (ctx.autoApproveInstall) without prompting', async () => {
    // First use (no install state). In IZN mode the scheduler bypasses
    // the confirmation dialog, so its onConfirm never records approval.
    // promptInstallApproval is set to REFUSE here to prove the IZN path
  ...
  it('persists approval on success', async () => {
    ...
    expect(state?.approvedPackageSpec).toBe(
      '@hopcode/open-computer-use@^0.3.0',
    );
  });
