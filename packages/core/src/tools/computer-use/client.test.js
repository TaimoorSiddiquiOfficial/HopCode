...
import { ComputerUseClient, DEFAULT_COMPUTER_USE_IDLE_TIMEOUT_MS, isTransportClosedError, MAX_COMPUTER_USE_IDLE_TIMEOUT_MS, } from './client.js';
...
describe('applyRuntimeConfig (set_config on connect)', () => {
    const invokeApply = (c, inner, progress) => c.applyRuntimeConfig(inner, progress);
    ...
    it('never aborts startup when set_config fails — warns via progress and swallows', async () => {
        ...
        expect(progress).toHaveBeenCalledWith(expect.stringContaining('max_image_dimension=800'));
    });
});
// ---------------------------------------------------------------------------
// Idle shutdown
...
describe('idle shutdown', () => {
    ...
    it.each([NaN, Infinity, -Infinity])('treats %p as invalid and falls back to the default', async (value) => {
        vi.useFakeTimers();
        const c = new ComputerUseClient({
            binary: '/fake/cua-driver',
            idleTimeoutMs: value,
...
describe('isTransportClosedError', () => {
    ...
    it('matches SDK JSON-RPC wrapping: "MCP error -32000: Connection closed"', () => {
        expect(isTransportClosedError(new Error('MCP error -32000: Connection closed'))).toBe(true);
    });
    ...
    it('matches the daemon-restart error (Screen Recording grant → daemon restart)', () => {
        ...
        expect(isTransportClosedError(new Error('MCP error -32603: daemon transport error forwarding `list_windows`: connect to /Users/x/Library/Caches/cua-driver/cua-driver.sock: Connection refused (os error 61)'))).toBe(true);
    });
    it('matches a bare "Connection refused" / "os error 61"', () => {
        expect(isTransportClosedError(new Error('Connection refused'))).toBe(true);
        expect(isTransportClosedError(new Error('connect failed: os error 61'))).toBe(true);
    });
    ...
    it('does NOT match unrelated upstream tool errors', () => {
        expect(isTransportClosedError(new Error('Tool execution failed'))).toBe(false);
    });
    it('does NOT match element_index errors', () => {
        expect(isTransportClosedError(new Error('element_index out of range'))).toBe(false);
    });
...
class ReconnectTestClient extends ComputerUseClient {
    ...
    installInner() {
        this.client = { callTool: () => this.runNextBehavior() };
    }
    async start(_onProgress) {
        this.startCalled++;
        this.installInner();
    }
    async stop() {
    ...
    runNextBehavior() {
        ...
        if (!b)
            throw new Error(`No behavior defined for call index ${idx}`);
        return b();
    }
}
function makeClient() {
...
describe('callTool reconnect path', () => {
    ...
    it('reconnects on the daemon-restart "Connection refused" error', async () => {
        ...
        c.behaviors = [
            async () => {
                throw new Error('MCP error -32603: daemon transport error forwarding `list_windows`: connect to /Users/x/Library/Caches/cua-driver/cua-driver.sock: Connection refused (os error 61)');
            },
            async () => successResult,
        ];
    ...
    it('does NOT reconnect on non-transport errors (e.g. upstream tool validation)', async () => {
        ...
        await expect(c.callTool('get_app_state', {})).rejects.toThrow('Tool execution failed');
        expect(c.stopCalled).toBe(0);
        expect(c.startCalled).toBe(0);
        expect(c.callCount).toBe(1);
    });
    it('does NOT reconnect on element_index errors from upstream', async () => {
        ...
        await expect(c.callTool('click_element', { element_index: 99 })).rejects.toThrow('element_index out of range');
        expect(c.stopCalled).toBe(0);
        expect(c.startCalled).toBe(0);
    });
    it('re-throws when retry also fails (no infinite reconnect loop)', async () => {
        ...
        await expect(c.callTool('get_app_state', {})).rejects.toThrow('Still failing after reconnect');
    ...
    it('succeeds on a later attempt after backing off past an earlier transport failure', async () => {
        ...
        expect(await c.callTool('get_window_state', { pid: 1 })).toBe(successResult);
    ...
    it('rethrows immediately when a retry hits a non-transport error (no further attempts)', async () => {
        ...
        await expect(c.callTool('click', { element_index: 9 })).rejects.toThrow('element_index out of range');
