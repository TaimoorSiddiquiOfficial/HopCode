...
import { parseChecksums, findInstalled, ensureInstalled, } from './downloader.js';
...
describe('parseChecksums', () => {
    it('parses sha256sum lines into a filename -> hash map', () => {
        ...
        expect(map.get('cua-driver-rs-0.5.2-darwin-arm64.tar.gz')).toBe('a'.repeat(64));
...
describe('ensureInstalled on Windows (.zip extraction)', () => {
    ...
    it('downloads + verifies + unzips, resolving cua-driver.exe under the wrapper dir', async () => {
        ...
        const unzipImpl = async (_zip, dest) => {
            unzipCalled = true;
            const wrapper = join(dest, `cua-driver-rs-${CUA_DRIVER_VERSION}-windows-x86_64`);
