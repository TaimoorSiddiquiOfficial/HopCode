 ...
 *      Requires @hopcode/open-computer-use >= 0.2.2 for permission-status.
 */
...
export interface BootstrapContext {
   ...
   * dialog (IZN / AUTO_EDIT / AUTO): in those modes the dialog's onConfirm
 ...
 * window-free command.
 *
 * Requires `@hopcode/open-computer-use@>=0.2.2`. On older pinned packages
...
export async function runBootstrap(
  ...
  if (!approved) {
    if (ctx.autoApproveInstall) {
      // An auto-approve mode (IZN / AUTO_EDIT / AUTO) already approved the
