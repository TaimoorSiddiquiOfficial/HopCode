...
import { runBootstrap, parsePermissionsStatus, } from './bootstrap.js';
...
describe('runBootstrap', () => {
    ...
    beforeEach(() => {
        ...
        deps = {
            ...
            probePermissions: vi.fn(async () => 'ok'),
    ...
    it('starts the proxy directly when already granted (no panes opened)', async () => {
        ...
        await runBootstrap(client, { signal: new AbortController().signal }, deps);
    ...
    it('throws and does NOT download when user declines install', async () => {
        ...
        const client = makeFakeClient();
        await expect(runBootstrap(client, { signal: new AbortController().signal }, deps)).rejects.toThrow(/declined/i);
    ...
    it('auto-approves the install (no prompt) and persists state when autoApproveInstall is set', async () => {
        ...
        const client = makeFakeClient();
        await runBootstrap(client, { signal: new AbortController().signal, autoApproveInstall: true }, deps);
    ...
    it('guides one permission at a time: Accessibility pane, then Screen Recording pane', async () => {
        ...
        const client = makeFakeClient();
        await runBootstrap(client, { signal: new AbortController().signal }, deps);
        const panes = deps.openPermissionPane.mock
            .calls;
    ...
    it('relaunches the status daemon when status reads unknown (e.g. SR restart)', async () => {
        ...
        deps.probePermissions = vi.fn(async () => {
            n++;
            if (n === 1)
                return 'unknown'; // daemon coming up / restarted
            if (n === 2)
                return 'accessibility';
            return 'ok';
        });
    ...
    it('times out (and tears down the daemon) if permissions never arrive', async () => {
        ...
        deps.probePermissions = vi.fn(async () => 'accessibility');
        deps.pollTimeoutMs = 30;
        const client = makeFakeClient();
        await expect(runBootstrap(client, { signal: new AbortController().signal }, deps)).rejects.toThrow(/timed out/i);
...
describe('parsePermissionsStatus', () => {
    it("returns 'ok' when both grants are true", () => {
        expect(parsePermissionsStatus('{"accessibility":true,"screen_recording":true}')).toBe('ok');
    });
    it("returns 'accessibility' when accessibility is false", () => {
        expect(parsePermissionsStatus('{"accessibility":false,"screen_recording":true}')).toBe('accessibility');
    });
    it("returns 'screenRecording' when only screen recording is false", () => {
        expect(parsePermissionsStatus('{"accessibility":true,"screen_recording":false}')).toBe('screenRecording');
    });
