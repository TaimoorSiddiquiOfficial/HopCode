...
export function createStreamableHttpCompatibilityFetch(mcpServerName, fetchFn = globalThis.fetch.bind(globalThis), mcpServerConfig) {
    return async (input, init) => {
        ...
        if (!isStreamableHttpGetSseRequest(init) ||
...
function createMcpStreamableHttpFetch(mcpServerName, mcpServerConfig) {
    return createStreamableHttpCompatibilityFetch(mcpServerName, globalThis.fetch.bind(globalThis), mcpServerConfig);
}
/**
...
export class McpClient {
    ...
    constructor(serverName, serverConfig, toolRegistry, promptRegistry, workspaceContext, debugMode, sendSdkMcpMessage) {
        ...
        this.client = new Client({
            name: `hopcode-cli-mcp-client-${this.serverName}`,
            version: '0.0.1',
        });
    }
    /**
    ...
    try {
        ...
        for (const funcDecl of tool.functionDeclarations) {
            try {
                ...
                mcpTimeout, cliConfig?.getMcpToolIdleTimeoutMs?.(), annotationsMap.get(funcDecl.name), mcpServerConfig.alwaysLoadTools === true));
            }
            catch (error) {
 ...
 * silently invisible in hopcode (no `/`-menu entry) while lenient
