...
describe('EnterWorktreeTool.execute', () => {
    ...
    it('refuses nested invocation from inside a worktree', async () => {
        ...
        const cwd = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'hopcode-nested-'));
        // Build a path that contains the nested-marker substring.
        const nested = pathMod.join(cwd, '.hopcode', 'worktrees', 'inner');
    ...
    it('fails cleanly when cwd is not a git repository', async () => {
        ...
        const pathMod = await import('node:path');
        const os = await import('node:os');
        const cwd = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'hopcode-no-git-'));
        const cfg = {
            getTargetDir: () => cwd,
            getSessionId: () => 'mock',
        };
...
describe('session marker round-trip', () => {
    it('write then read returns the same session id', async () => {
        ...
        const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-session-'));
        try {
...
describe('GitWorktreeService.getUserWorktreesDir / getUserWorktreePath', () => {
    it('uses .hopcode/worktrees under the project root', () => {
        ...
        // separator is `\`, not `/`.
        expect(service.getUserWorktreesDir()).toBe(path.join(root, '.hopcode', 'worktrees'));
        expect(service.getUserWorktreePath('feat-x')).toBe(path.join(root, '.hopcode', 'worktrees', 'feat-x'));
    });
});
