...
import { canonicalOAuth, connectionIdOf, fingerprint, isPoolable, mcpTransportOf, parseConnectionId, POOLED_TRANSPORTS_DEFAULT, } from './mcp-pool-key.js';
describe('mcp-pool-key', () => {
    describe('fingerprint', () => {
        ...
        it('diverges on any byte change in env value (critical for credential isolation)', () => {
            const a = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://api.example.com', { Authorization: 'Bearer tokenA' });
            const b = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://api.example.com', { Authorization: 'Bearer tokenB' });
            expect(fingerprint(a)).not.toBe(fingerprint(b));
        });
        it('diverges on header-key permutation only via value (keys are sorted)', () => {
            const a = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x', { 'X-A': '1', 'X-B': '2' });
            const b = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x', { 'X-B': '2', 'X-A': '1' });
            expect(fingerprint(a)).toBe(fingerprint(b));
        });
        it('SAME key when includeTools/excludeTools/trust/description differ (per-session filters excluded)', () => {
            const a = new MCPServerConfig('node', ['s.js'], undefined, undefined, undefined, undefined, undefined, undefined, undefined, 
            /*trust*/ false, 
            /*description*/ 'A', ['onlyA'], ['notA']);
            const b = new MCPServerConfig('node', ['s.js'], undefined, undefined, undefined, undefined, undefined, undefined, undefined, 
            /*trust*/ true, 
            /*description*/ 'B', undefined, undefined);
            expect(fingerprint(a)).toBe(fingerprint(b));
        });
    ...
    describe('canonicalOAuth (V21-9)', () => {
        ...
        ])('distinguishes fingerprints on %s (W88 — pre-fix collided)', (_field, diff) => {
            const base = {
                enabled: true,
                clientId: 'id',
    ...
    describe('mcpTransportOf', () => {
        ...
        it('classifies sdk via isSdkMcpServerConfig', () => {
            const cfg = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, 'sdk');
            expect(mcpTransportOf(cfg)).toBe('sdk');
        });
        it('classifies http when httpUrl set', () => {
            expect(mcpTransportOf(new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://api.x.com'))).toBe('http');
        });
    ...
    describe('isPoolable + POOLED_TRANSPORTS_DEFAULT', () => {
        it('stdio is poolable by default', () => {
            expect(isPoolable(new MCPServerConfig('node'), POOLED_TRANSPORTS_DEFAULT)).toBe(true);
        });
        it('http is NOT poolable by default (V21 C8 / opt-in)', () => {
            const cfg = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x');
            expect(isPoolable(cfg, POOLED_TRANSPORTS_DEFAULT)).toBe(false);
        });
        it('http IS poolable when operator opts in via pooledTransports', () => {
            const cfg = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x');
            expect(isPoolable(cfg, new Set(['stdio', 'websocket', 'http']))).toBe(true);
        });
        it('SDK MCP is never poolable (always bypass)', () => {
            const cfg = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, 'sdk');
            // Even with sdk in pooledTransports, isPoolable returns false.
            expect(isPoolable(cfg, new Set(['stdio', 'websocket', 'sdk']))).toBe(false);
        });
    });
...
//# sourceMappingURL=mcp-pool-key.test.js.map