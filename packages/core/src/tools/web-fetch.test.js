...
import { WebFetchTool, clearWebFetchCache, rewriteGitHubBlobUrl, } from './web-fetch.js';
...
describe('WebFetchTool', () => {
    ...
    describe('execute', () => {
        ...
        it.each(['HTTPS://example.com', 'Http://example.com'])('should accept uppercase http url schemes: %s', (url) => {
            const tool = new WebFetchTool(mockConfig);
            expect(() => tool.build({ url, prompt: 'summarize this' })).not.toThrow();
        });
        it.each([
            [
                'ftp://example.com',
        ...
        ])('should reject invalid or unsupported urls: %s', (url, expectedError) => {
            const tool = new WebFetchTool(mockConfig);
            expect(() => tool.build({ url, prompt: 'summarize this' })).toThrow(expectedError);
        });
        it.each([
            ...
            const tool = new WebFetchTool(mockConfig);
            expect(() => tool.build({ url, prompt: 'summarize this' })).toThrow("The 'url' must not include credentials.");
        });
        it('should return WEB_FETCH_FALLBACK_FAILED on fetch failure', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockRejectedValue(new Error('fetch failed'));
            const tool = new WebFetchTool(mockConfig);
        ...
        it('should return an error result for non-2xx statuses', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({ status: 404, statusText: 'Not Found' }));
            const tool = new WebFetchTool(mockConfig);
    ...
    describe('request headers', () => {
        ...
        ])('should map format=%s to the Accept header', async (format, accept) => {
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockResolvedValue(okResponse());
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
        ...
        it('should upgrade http to https for public hosts', async () => {
            ...
            await invocation.execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('https://example.com/page', expect.anything());
        });
        it.each([
        ...
        it('should upgrade public IPv6 literal hosts (bracketed, dot-free)', async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith('https://[2606:4700:4700::1111]/x', expect.anything());
        });
        ...
        it('should NOT upgrade explicit non-default http ports', async () => {
            ...
            await tool
                ...
                .execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('http://example.com:8080/api', expect.anything());
        });
        it('should NOT upgrade http for localhost or private hosts', async () => {
            ...
            await invocation.execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('http://localhost:3000/api', expect.anything());
        });
    });
    describe('content handling', () => {
        it('should convert HTML to markdown preserving link hrefs', async () => {
            let receivedContent = '';
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({
                body: Buffer.from('<html><body><p>See <a href="/docs/alpha-guide">the alpha guide</a>.</p></body></html>'),
            }));
        ...
        it('should convert the full HTML body before truncating, with an explicit marker', async () => {
            ...
            let receivedContent = '';
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({ body: Buffer.from(html) }));
            mockGenerateContent.mockImplementation((options) => {
                receivedContent = options.contents[0].parts[0].text;
                return Promise.resolve({ text: 'Processed', usage: undefined });
            });
            ...
            await invocation.execute(new AbortController().signal);
            expect(receivedContent).toContain('[Content truncated: showing first 100,000 of');
        ...
        it('should keep content past 100k of raw HTML when the text itself fits', async () => {
            ...
            const item = '<li class="item-row"><span class="meta-label">entry</span> <a href="/files/f.txt">f</a></li>';
        ...
        it('should drop images (incl. data URIs) but keep anchor hrefs', async () => {
            let receivedContent = '';
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({
                body: Buffer.from('<html><body><img src="data:image/png;base64,AAAABBBBCCCC" alt="hero">' +
        ...
        it('should strip script/style/noscript content before conversion', async () => {
            let receivedContent = '';
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({
                body: Buffer.from('<html><head><style>.hydration{color:red}</style><script>window.__BLOB__="HYDRATION-GARBAGE";</script></head>' +
    ...
    describe('result shape', () => {
        it('should prefix llmContent with a metadata header and set a summary returnDisplay', async () => {
            ...
            expect(result.returnDisplay).toMatch(/^Received 28 bytes \(200 OK\) from example\.com$/);
        });
        it('should note the final URL when redirects were followed', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({ finalUrl: 'https://example.com/moved-here' }));
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
            const tool = new WebFetchTool(mockConfig);
            const invocation = tool.build({
            ...
            const result = await invocation.execute(new AbortController().signal);
            expect(result.llmContent).toContain('URL: https://example.com/old (final: https://example.com/moved-here)');
        });
        it('should replace an empty side-query response with an explicit note', async () => {
            ...
            expect(result.llmContent).toContain('The processing model returned no content');
        });
    });
    ...
    describe('binary content', () => {
        ...
        it('should treat a headerless .svg as text, not binary', async () => {
            ...
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({
                contentType: '',
                body: Buffer.from('<svg xmlns="http://www.w3.org/2000/svg"><title>SVG-TITLE-TEXT</title></svg>'),
        ...
        it('should refuse to persist when the session disk budget is exhausted', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({ contentType: 'application/pdf', body: pdfBytes }));
            const cappedConfig = {
                ...mockConfig,
        ...
        it('should pass the abort signal to PDF extraction', async () => {
            ...
            expect(mockExtractPDFText).toHaveBeenCalledWith(expect.stringMatching(/\.pdf$/), { signal: controller.signal });
        });
        it('should persist PDFs, extract their text, and summarize it', async () => {
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockResolvedValue(okResponse({ contentType: 'application/pdf', body: pdfBytes }));
            mockExtractPDFText.mockResolvedValue({
                success: true,
            ...
            expect(result.llmContent).toMatch(/\[Binary content \(application\/pdf, .+\) saved to .+webfetch-.+\.pdf\. Use read_file to examine it \(reads PDFs natively; pass pages for large files\)\.\]/);
            const savedPath = result.llmContent.match(/saved to (\S+\.pdf)\./)?.[1];
        ...
        it('should skip the side-query when PDF extraction fails, with no mojibake', async () => {
            ...
            expect(mockGenerateContent).not.toHaveBeenCalled();
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toContain('No text could be extracted from this binary content');
            expect(result.llmContent).not.toContain('�');
        ...
        it('should not suggest read_file for binaries it cannot display', async () => {
            ...
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({ contentType: 'application/octet-stream', body: junk }));
            const tool = new WebFetchTool(mockConfig);
            const invocation = tool.build({
    ...
    describe('preapproved list — markdown passthrough only', () => {
        it('should gate the raw passthrough on the final URL, not the requested one', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockResolvedValue(okResponse({
                ...
                finalUrl: 'https://raw.githubusercontent.com/QwenLM/qwen-code/main/README.md',
            }));
            const tool = new WebFetchTool(mockConfig);
            const invocation = tool.build({
    ...
    describe('cache', () => {
        ...
        it('should fall back to the original http URL on connection-level https failures', async () => {
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockRejectedValueOnce(new fetchUtils.FetchError('connect ECONNREFUSED', 'ECONNREFUSED'))
                .mockResolvedValueOnce(okResponse({ finalUrl: 'http://wiki.corp.example.com/page' }));
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
            const result = await new WebFetchTool(mockConfig)
                .build({ url: 'http://wiki.corp.example.com/page', prompt: 'read' })
                .execute(new AbortController().signal);
            expect(result.error).toBeUndefined();
            expect(fetchSpy).toHaveBeenNthCalledWith(1, 'https://wiki.corp.example.com/page', expect.anything());
            expect(fetchSpy).toHaveBeenNthCalledWith(2, 'http://wiki.corp.example.com/page', expect.anything());
        });
        it('should fall back to http when the TLS handshake is reset', async () => {
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockRejectedValueOnce(new fetchUtils.FetchError('socket hang up', 'ECONNRESET'))
                .mockResolvedValueOnce(okResponse({ finalUrl: 'http://legacy.example.com/page' }));
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
            const result = await new WebFetchTool(mockConfig)
                .build({ url: 'http://legacy.example.com/page', prompt: 'read' })
                .execute(new AbortController().signal);
            expect(result.error).toBeUndefined();
            expect(fetchSpy).toHaveBeenNthCalledWith(2, 'http://legacy.example.com/page', expect.anything());
        });
        it('should fall back to http for curated-list hosts like any other', async () => {
            ...
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockRejectedValueOnce(new fetchUtils.FetchError('socket hang up', 'ECONNRESET'))
                .mockResolvedValueOnce(okResponse({ finalUrl: 'http://react.dev/learn' }));
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
            const result = await new WebFetchTool(mockConfig)
                .build({ url: 'http://react.dev/learn', prompt: 'read' })
                .execute(new AbortController().signal);
            expect(result.error).toBeUndefined();
            expect(fetchSpy).toHaveBeenNthCalledWith(1, 'https://react.dev/learn', expect.anything());
            expect(fetchSpy).toHaveBeenNthCalledWith(2, 'http://react.dev/learn', expect.anything());
        });
        it('should not fall back to http when the caller asked for https', async () => {
            vi.spyOn(fetchUtils, 'fetchWithPolicy').mockRejectedValue(new fetchUtils.FetchError('connect ECONNREFUSED', 'ECONNREFUSED'));
            const result = await new WebFetchTool(mockConfig)
        ...
        it('should NOT cache an http-fallback response under the https key', async () => {
            ...
            const fetchSpy = vi
                ...
                .mockResolvedValueOnce(okResponse({ finalUrl: 'https://legacy.example.com/page' }));
            mockGenerateContent.mockResolvedValue({ text: 'Summary' });
            const tool = new WebFetchTool(mockConfig);
            await tool
                .build({ url: 'http://legacy.example.com/page', prompt: 'read' })
            ...
            expect(fetchSpy).toHaveBeenNthCalledWith(3, 'https://legacy.example.com/page', expect.anything());
        });
        ...
        it('should not cache error responses', async () => {
            const fetchSpy = vi
                .spyOn(fetchUtils, 'fetchWithPolicy')
                .mockResolvedValueOnce(okResponse({ status: 500, statusText: 'Internal Server Error' }))
            ...
            expect(firstResult.error?.type).toBe(ToolErrorType.WEB_FETCH_FALLBACK_FAILED);
            const second = tool.build({
                url: 'https://example.com',
                prompt: 'summarize',
            });
    ...
    describe('getConfirmationDetails', () => {
        ...
        it('should normalize github blob urls to the raw destination in params and confirmation', async () => {
            ...
            expect(invocation.params.url).toBe('https://raw.githubusercontent.com/google/gemini-react/main/README.md');
            const confirmationDetails = await invocation.getConfirmationDetails(new AbortController().signal);
            expect(confirmationDetails).toEqual({
                type: 'info',
                title: 'Confirm Web Fetch',
                prompt: 'Fetch content from https://raw.githubusercontent.com/google/gemini-react/main/README.md and process with: summarize the README',
                urls: [
...
describe('rewriteGitHubBlobUrl', () => {
    it('rewrites github.com blob URLs to the raw host', () => {
        expect(rewriteGitHubBlobUrl('https://github.com/owner/repo/blob/main/README.md')).toBe('https://raw.githubusercontent.com/owner/repo/main/README.md');
    });
    it('rewrites www.github.com blob URLs too', () => {
        expect(rewriteGitHubBlobUrl('https://www.github.com/owner/repo/blob/main/f.ts')).toBe('https://raw.githubusercontent.com/owner/repo/main/f.ts');
    });
    it('only removes the /blob/ path segment, not later occurrences', () => {
        expect(rewriteGitHubBlobUrl('https://github.com/owner/repo/blob/main/docs/blob/x.md')).toBe('https://raw.githubusercontent.com/owner/repo/main/docs/blob/x.md');
    });
