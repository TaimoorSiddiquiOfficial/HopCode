...
import { MCPServerStatus, removeMCPServerStatus, updateMCPServerStatus, } from './mcp-client.js';
...
describe('generateValidName', () => {
    ...
    it('should normalize dotted MCP names for strict providers', () => {
        const normalized = generateValidName('mcp__zybio__literature.search_pubmed');
        expect(normalized).toMatch(/^[A-Za-z][A-Za-z0-9_-]*$/);
        expect(normalized).not.toContain('.');
        expect(normalized).toBe(generateValidName('mcp__zybio__literature.search_pubmed'));
        expect(generateValidName(normalized)).toBe(normalized);
    });
    it('should not collide after replacing unsupported characters', () => {
        expect(generateValidName('mcp__zybio__literature.search_pubmed')).not.toBe(generateValidName('mcp__zybio__literature_search_pubmed'));
    });
...
describe('DiscoveredMCPTool', () => {
    ...
    describe('getDefaultPermission and getConfirmationDetails', () => {
        ...
        it('should use the registered provider-safe name in permission rules', async () => {
            const dottedTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, 'literature.search_pubmed', baseDescription, inputSchema);
    ...
    describe('getDefaultPermission with folder trust', () => {
        ...
        it.each([
            ...
            const annotatedTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, testCase.trust, undefined, mockConfig(testCase.isTrustedFolder), undefined, undefined, undefined, { readOnlyHint: testCase.readOnlyHint });
    ...
    describe('auto-reconnect on connection error', () => {
        it('should attempt reconnect and retry on connection error', async () => {
            ...
            const newTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, reconnectServerToolName, baseDescription, inputSchema, undefined, undefined, undefined, newMockMcpClient);
            ...
            const reconnectTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, reconnectServerToolName, baseDescription, inputSchema, undefined, undefined, mockConfig, mockMcpClient);
        ...
        it('should not retry on non-connection errors', async () => {
            ...
            const retryTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, undefined, retryClient);
            const discoverToolsForServer = vi.fn().mockResolvedValue(undefined);
        ...
        it('should not retry aborted calls even when the server is disconnected', async () => {
            ...
            mockMcpClient.callTool.mockRejectedValue(abortError);
            const reconnectTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, mockConfig, mockMcpClient);
            const invocation = reconnectTool.build(params);
            await expect(invocation.execute(new AbortController().signal)).rejects.toThrow('The operation was aborted');
            expect(mockMcpClient.callTool).toHaveBeenCalledTimes(1);
            expect(discoverToolsForServer).not.toHaveBeenCalled();
            expect(ensureTool).not.toHaveBeenCalled();
            expect(retryClient.callTool).not.toHaveBeenCalled();
    ...
    describe('MCP Tool Idle Timeout', () => {
        it('should abort when MCP server does not respond within idle timeout', async () => {
            ...
            const mockMcpClient = {
                callTool: vi.fn().mockImplementation((_params, _schema, options) => new Promise((_resolve, reject) => {
                    // Simulate SDK behavior: reject when signal is aborted
                    options?.signal?.addEventListener('abort', () => {
                        const error = new Error(options?.signal?.reason
            ...
            const tool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, true, undefined, undefined, mockMcpClient, undefined, idleTimeoutMs);
            ...
            await expect(executePromise).rejects.toThrow(/did not respond within.*idle timeout/);
        ...
        it('should not apply idle timeout when set to 0 or undefined', async () => {
            ...
            const tool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, true, undefined, undefined, mockMcpClient, undefined, undefined);
            const invocation = tool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toBeDefined();
