...
import { addMCPStatusChangeListener, attemptAutomaticMcpOAuth, connectToMcpServer, createStreamableHttpCompatibilityFetch, createTransport, discoverPrompts, discoverResources, getAllMCPServerStatuses, getMcpOAuthDialogInstruction, getMCPServerStatus, hasNetworkTransport, isEnabled, listMcpPrompts, listMcpResources, MCPServerStatus, McpClient, mcpServerRequiresOAuth, discoverTools, populateMcpServerCommand, probeMcpServerForOAuth, removeMCPServerStatus, removeMCPStatusChangeListener, updateMCPServerStatus, } from './mcp-client.js';
...
describe('mcp-client', () => {
    ...
    describe('getMcpOAuthDialogInstruction', () => {
        it('builds an authenticate instruction for the named MCP server', () => {
            expect(getMcpOAuthDialogInstruction('authenticate', 'foo')).toBe("In interactive HopCode sessions, open the /mcp dialog to authenticate with MCP server 'foo'. For headless or SDK usage, configure MCP OAuth with qwen mcp add --oauth-* or settings.json, then authenticate once in an interactive session before connecting.");
        });
        it('builds a re-authenticate instruction for the named MCP server', () => {
            expect(getMcpOAuthDialogInstruction('re-authenticate', 'foo')).toBe("In interactive HopCode sessions, open the /mcp dialog to re-authenticate with MCP server 'foo'. For headless or SDK usage, configure MCP OAuth with qwen mcp add --oauth-* or settings.json, then re-authenticate once in an interactive session before connecting.");
        });
    });
    describe('connectToMcpServer', () => {
        ...
        function setupHttpOAuthRetry(connectError) {
            const connect = vi
                .fn()
        ...
        it('reports rejected stored OAuth tokens for SSE servers', async () => {
            ...
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow("Stored OAuth token for SSE server 'sse-server' was rejected. " +
        ...
        it('reports unusable stored OAuth tokens for SSE servers', async () => {
            ...
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow("Stored OAuth tokens for SSE server 'sse-server' are expired or could not be refreshed. " +
                getMcpOAuthDialogInstruction('re-authenticate', 'sse-server'));
        });
        it('reports unusable SSE OAuth tokens when token validation fails', async () => {
            ...
            const oauthMessage = "Stored OAuth tokens for SSE server 'sse-server' are expired or could not be refreshed. " +
                getMcpOAuthDialogInstruction('re-authenticate', 'sse-server');
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow(oauthMessage);
            expect(mockDebugLogger.error).toHaveBeenCalledWith("Failed to validate stored OAuth token for SSE server 'sse-server': Token store unavailable");
        ...
        it('logs unusable SSE OAuth tokens when stored credentials disappear', async () => {
            ...
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow("Stored OAuth tokens for SSE server 'sse-server' are expired or could not be refreshed. " +
                getMcpOAuthDialogInstruction('re-authenticate', 'sse-server'));
            expect(mockDebugLogger.warn).toHaveBeenCalledWith("Stored OAuth tokens for SSE server 'sse-server' are expired or could not be refreshed. " +
                getMcpOAuthDialogInstruction('re-authenticate', 'sse-server'));
        });
        it('reports SSE OAuth guidance when credentials fail to re-read after 401', async () => {
            ...
            const oauthMessage = "Stored OAuth tokens for SSE server 'sse-server' are expired or could not be refreshed. " +
                getMcpOAuthDialogInstruction('re-authenticate', 'sse-server');
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow(oauthMessage);
            expect(mockDebugLogger.error).toHaveBeenCalledWith("Failed to re-read stored OAuth credentials for SSE server 'sse-server' after 401: Corrupt token file");
            expect(mockDebugLogger.warn).toHaveBeenCalledWith(oauthMessage);
        });
        it('reports missing OAuth configuration for SSE servers without stored credentials', async () => {
            ...
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).rejects.toThrow("401 error received for SSE server 'sse-server' without OAuth configuration. " +
        ...
        it('continues connecting when the SSE OAuth credential pre-read fails', async () => {
            ...
            await expect(connectToMcpServer('sse-server', { url: 'http://test-server/sse' }, false, workspaceContext)).resolves.toBeDefined();
            expect(connect).toHaveBeenCalledOnce();
            expect(mockDebugLogger.warn).toHaveBeenCalledWith("Failed to pre-read stored OAuth credentials for SSE server 'sse-server': Corrupt token file");
        });
        it('reports OAuth guidance when automatic OAuth handling fails', async () => {
            ...
            const oauthMessage = "Failed to handle automatic OAuth for server 'http-server'. " +
                getMcpOAuthDialogInstruction('authenticate', 'http-server');
            await expect(connectToMcpServer('http-server', { httpUrl: 'http://test-server/mcp' }, false, workspaceContext)).rejects.toThrow(oauthMessage);
        ...
        it('retries HTTP connections after base-url OAuth discovery without www-authenticate', async () => {
            const { authenticate, connect, discoverOAuthConfig, getValidToken, workspaceContext, } = setupHttpOAuthRetry(new Error('Streamable HTTP error: Error POSTing to endpoint: {"error":"unauthorized"}'));
            vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 401 }));
            await expect(connectToMcpServer('http-server', { httpUrl: 'http://test-server/mcp' }, false, workspaceContext)).resolves.toBeDefined();
            expect(discoverOAuthConfig).toHaveBeenCalledWith('http://test-server');
            expect(authenticate).toHaveBeenCalledWith('http-server', {
                enabled: true,
                authorizationUrl: 'https://auth.example/authorize',
                tokenUrl: 'https://auth.example/token',
                scopes: ['mcp.read'],
            ...
            const oauthTransport = connect.mock.calls[1][0];
        ...
        it('falls back to base-url OAuth discovery when www-authenticate lacks resource metadata', async () => {
            const { authenticate, connect, discoverOAuthConfig, workspaceContext } = setupHttpOAuthRetry(new Error('HTTP 401 Unauthorized\nwww-authenticate: Bearer realm="example"'));
            await expect(connectToMcpServer('http-server', { httpUrl: 'http://test-server/mcp' }, false, workspaceContext)).resolves.toBeDefined();
            expect(discoverOAuthConfig).toHaveBeenCalledWith('http://test-server');
            expect(authenticate).toHaveBeenCalledWith('http-server', {
                enabled: true,
        ...
        it('reports OAuth guidance when post-discovery transport creation fails', async () => {
            ...
            const { connect, discoverOAuthConfig, getValidToken, workspaceContext } = setupHttpOAuthRetry(new Error('HTTP 401 Unauthorized'));
            vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 401 }));
            getValidToken.mockImplementation(async () => {
                config.httpUrl = 'not a valid URL';
                return 'access-token';
            });
            const oauthMessage = "Failed to create OAuth transport for server 'http-server'. " +
                getMcpOAuthDialogInstruction('authenticate', 'http-server');
            await expect(connectToMcpServer('http-server', config, false, workspaceContext)).rejects.toThrow(oauthMessage);
            expect(discoverOAuthConfig).toHaveBeenCalledWith('http://test-server');
            expect(connect).toHaveBeenCalledTimes(1);
            expect(mockDebugLogger.error).toHaveBeenCalledWith(oauthMessage);
        });
        it('reports OAuth guidance when post-discovery token lookup fails', async () => {
            const { getValidToken, workspaceContext } = setupHttpOAuthRetry(new Error('HTTP 401 Unauthorized'));
            vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 401 }));
            getValidToken.mockResolvedValue(null);
            const oauthMessage = "Failed to get OAuth token for server 'http-server'. " +
                getMcpOAuthDialogInstruction('authenticate', 'http-server');
            await expect(connectToMcpServer('http-server', { httpUrl: 'http://test-server/mcp' }, false, workspaceContext)).rejects.toThrow(oauthMessage);
            expect(mockDebugLogger.error).toHaveBeenCalledWith(oauthMessage);
        });
        it('reports OAuth guidance when post-discovery credentials are unavailable', async () => {
            const { getCredentials, workspaceContext } = setupHttpOAuthRetry(new Error('HTTP 401 Unauthorized'));
            vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 401 }));
            getCredentials.mockReset();
            getCredentials.mockResolvedValue(null);
            const oauthMessage = "Failed to get stored credentials for server 'http-server'. " +
                getMcpOAuthDialogInstruction('authenticate', 'http-server');
            await expect(connectToMcpServer('http-server', { httpUrl: 'http://test-server/mcp' }, false, workspaceContext)).rejects.toThrow(oauthMessage);
            expect(mockDebugLogger.error).toHaveBeenCalledWith(oauthMessage);
        });
        it('wraps OAuth discovery errors with remediation guidance', async () => {
            ...
            vi.spyOn(OAuthUtils, 'discoverOAuthConfig').mockRejectedValue(new Error('Discovery timed out'));
            const workspaceContext = {
                getDirectories: vi.fn().mockReturnValue([]),
                onDirectoriesChanged: vi.fn().mockReturnValue(vi.fn()),
            };
            const oauthMessage = "OAuth discovery failed for server 'http-server'. " +
                getMcpOAuthDialogInstruction('authenticate', 'http-server') +
    ...
    describe('McpClient', () => {
        it('recovers HTTP connections when the SDK omits the 401 status', async () => {
            const connect = vi
                .fn()
                .mockRejectedValueOnce(new Error('Streamable HTTP error: Error POSTing to endpoint: {"error":"unauthorized"}'))
                .mockResolvedValueOnce(undefined);
            vi.mocked(ClientLib.Client).mockReturnValue({
                connect,
                registerCapabilities: vi.fn(),
            ...
            await expect(attemptAutomaticMcpOAuth(serverName, serverConfig, false)).resolves.toBe(false);
            expect(authenticate).not.toHaveBeenCalled();
            await expect(Promise.all([
        ...
        it('preserves a captured OAuth challenge when the SDK error only reports 401', async () => {
            ...
            vi.mocked(ClientLib.Client).mockReturnValue({
                connect: vi.fn().mockImplementation(async (transport) => {
                    const transportFetch = transport._fetch;
            ...
            const authenticate = vi.fn().mockResolvedValue(undefined);
            vi.mocked(MCPOAuthProvider).mockImplementation(() => ({ authenticate }));
            const discoverOAuthConfig = vi
                .spyOn(OAuthUtils, 'discoverOAuthConfig')
                .mockResolvedValue({
                authorizationUrl: 'https://auth.example/authorize',
            ...
            await expect(attemptAutomaticMcpOAuth(serverName, serverConfig, true)).resolves.toBe(true);
        ...
        it('does not classify a non-401 HTTP failure as OAuth', async () => {
            ...
            const client = new McpClient(serverName, { httpUrl: 'https://example.com/mcp' }, {}, {}, {
                getDirectories: vi.fn().mockReturnValue([]),
            }, false);
            mcpServerRequiresOAuth.set(serverName, true);
            await expect(client.connect()).rejects.toThrow('HTTP 403 Forbidden');
            await expect(probeMcpServerForOAuth(serverName, {
                httpUrl: 'https://example.com/mcp',
            })).resolves.toBe(false);
            expect(mcpServerRequiresOAuth.has(serverName)).toBe(false);
        });
        it('serializes browser OAuth across servers and isolates requirements by URL', async () => {
            ...
            const authenticate = vi.fn().mockImplementation(async () => {
                ...
                maxActiveAuthentications = Math.max(maxActiveAuthentications, activeAuthentications);
                await new Promise((resolve) => setTimeout(resolve, 10));
                activeAuthentications -= 1;
            });
            vi.mocked(MCPOAuthProvider).mockImplementation(() => ({ authenticate }));
            await expect(attemptAutomaticMcpOAuth('queued-first', { httpUrl: 'https://other.example/mcp' }, true)).resolves.toBe(false);
            await expect(Promise.all([
        ...
        it('clears only the matching URL from the name-level OAuth marker', async () => {
            ...
            const firstClient = new McpClient(serverName, firstConfig, ...registries, false);
            const secondClient = new McpClient(serverName, secondConfig, ...registries, false);
        ...
        it('readResource does not gate on the resources capability (lenient read)', async () => {
            ...
            vi.mocked(ClientLib.Client).mockReturnValue(mockedClient);
            vi.spyOn(SdkClientStdioLib, 'StdioClientTransport').mockReturnValue({});
            const client = new McpClient('srv', { command: 'test-command' }, {}, {}, {}, false);
            await client.connect();
            const result = await client.readResource('res://doc');
            expect(mockedClient.request).toHaveBeenCalledWith({ method: 'resources/read', params: { uri: 'res://doc' } }, expect.anything(), undefined);
            expect(result.contents[0].text).toBe('BODY');
        });
        ...
        it('should handle errors when discovering prompts', async () => {
            ...
            await client.connect();
            await expect(client.discover(cfgWithResources())).rejects.toThrow('No prompts, tools, or resources found on the server.');
        });
        ...
        it('discoverAndReturn returns tools and prompts WITHOUT registering them', async () => {
            ...
            const client = new McpClient('pure-server', { command: 'test-command' }, toolRegistry, promptRegistry, {}, false);
            await client.connect();
        ...
        it('marks discovered tools alwaysLoad when the MCP server config requests it', async () => {
            ...
            const tools = await discoverTools('chrome-devtools', {
                command: 'test-command',
                alwaysLoadTools: true,
            }, mockedClient, cfgWithResources());
        ...
        it('discoverAndReturn with { applyConfigFilters: false } ignores config filters and trust for shared pool snapshots', async () => {
            ...
            const client = new McpClient('pure-filter-server', new MCPServerConfig('test-command', undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true, undefined, ['allowed'], ['filtered_out']), toolRegistry, {}, {}, false);
            await client.connect();
            ...
            expect(snapshot.tools.every((tool) => tool.trust === undefined)).toBe(true);
            expect(toolRegistry.registerTool).not.toHaveBeenCalled();
        });
        it('discoverAndReturn (default) applies config filters and trust — legacy non-pool callers (R23 T1)', async () => {
            ...
            const client = new McpClient('legacy-filter-server', new MCPServerConfig('test-command', undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true, undefined, ['allowed'], ['filtered_out']), toolRegistry, {}, {}, false);
            await client.connect();
        ...
        it('discoverAndReturn throws "No prompts or tools found" when both empty', async () => {
            ...
            const client = new McpClient('empty-server', { command: 'test-command' }, toolRegistry, promptRegistry, {}, false);
            await client.connect();
            await expect(client.discoverAndReturn(cfgWithResources())).rejects.toThrow('No prompts, tools, or resources found on the server.');
        ...
        it('discoverAndReturn throws when called before connect()', async () => {
            const client = new McpClient('unconnected-server', { command: 'test-command' }, {}, {}, {}, false);
            // Status starts DISCONNECTED; discoverAndReturn must reject without
            // hitting the network.
            await expect(client.discoverAndReturn(cfgWithResources())).rejects.toThrow('Client is not connected.');
        });
        it('discover() delegates to discoverAndReturn and registers both tools and prompts', async () => {
            ...
            const client = new McpClient('compat-server', { command: 'test-command' }, toolRegistry, promptRegistry, {}, false);
            await client.connect();
            await client.discover(cfgWithResources());
        ...
        it('discover() registers discovered resources into the Config ResourceRegistry', async () => {
            ...
            const mockedClient = {
                ...
                request: vi
                    .fn()
                    .mockImplementation((req) => req.method === 'resources/list'
            ...
            const client = new McpClient('srv', { command: 'test-command' }, { registerTool: vi.fn() }, { registerPrompt: vi.fn() }, {}, false);
            await client.connect();
            ...
            expect(registerResource).toHaveBeenCalledWith(expect.objectContaining({ uri: 'res://a', serverName: 'srv' }));
        });
    ...
    describe('listMcpResources', () => {
        it('returns enriched DiscoveredMCPResource[] with serverName', async () => {
            ...
            expect(result[0]['invoke']).toBeUndefined();
        });
        ...
        it('does NOT swallow an unrelated error that merely contains "method not found" (case-sensitive)', async () => {
            ...
            const mockClient = {
                ...
                request: vi
                    .fn()
                    .mockRejectedValue(new Error('Error in method not found handler: null pointer')),
            };
    ...
    describe('discoverResources wrapper', () => {
        it('registers discovered resources into the supplied ResourceRegistry', async () => {
            ...
            const out = await discoverResources('wrapper-server', mockClient, resourceRegistry);
    ...
    describe('discoverPrompts wrapper (backward compat after F2-1 split)', () => {
        it('still registers prompts into the supplied PromptRegistry', async () => {
            ...
            const out = await discoverPrompts('wrapper-server', mockClient, promptRegistry);
            ...
            expect(out[0]['serverName']).toBeUndefined();
            expect(out[0]['invoke']).toBeUndefined();
        });
    });
    ...
    describe('createTransport', () => {
        describe('should connect via httpUrl', () => {
            ...
            it('captures OAuth challenges from the initial HTTP handshake', async () => {
                const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, {
                    status: 401,
                    headers: {
                        'www-authenticate': 'Bearer resource_metadata="https://test-server/.well-known/oauth-protected-resource"',
                    },
                }));
                const serverName = 'handshake-oauth-server';
                const serverConfig = { httpUrl: 'https://test-server/mcp' };
                const transport = await createTransport(serverName, serverConfig, false);
                const transportFetch = transport._fetch;
                ...
                expect(mcpServerRequiresOAuth.get(serverName)).toBe(true);
                await expect(probeMcpServerForOAuth(serverName, serverConfig)).resolves.toBe(true);
                expect(fetchSpy).toHaveBeenCalledTimes(1);
            });
            it('treats 400 from optional GET SSE stream as unsupported', async () => {
                ...
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('spring-ai', fetchFn);
            ...
            it('omits response body diagnostics when the fallback body is empty', async () => {
                ...
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('empty-body', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
                    headers: { Accept: 'text/event-stream' },
                });
                expect(response.status).toBe(405);
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.not.stringContaining('Response body:'));
            });
            it('truncates Streamable HTTP GET SSE error body diagnostics', async () => {
                const fetchFn = vi
                    .fn()
                    .mockResolvedValue(new Response('x'.repeat(1024), { status: 400 }));
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('large-error-body', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
                    headers: { Accept: 'text/event-stream' },
                });
                expect(response.status).toBe(405);
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`Response body: ${JSON.stringify(`${'x'.repeat(512)}...`)}`));
                expect(mockDebugLogger.warn).not.toHaveBeenCalledWith(expect.stringContaining('x'.repeat(600)));
            });
            ...
            it('does not hide Streamable HTTP GET SSE server errors', async () => {
                ...
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('server-error', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
                    headers: { Accept: 'text/event-stream' },
                });
            ...
            it('does not rewrite the SDK-native GET SSE unsupported sentinel', async () => {
                ...
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('native-unsupported', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
                    headers: { Accept: 'text/event-stream' },
                });
            ...
            it('does not rewrite resumable GET SSE errors with Last-Event-ID', async () => {
                const fetchFn = vi
                    .fn()
                    .mockResolvedValue(new Response('{"error":"invalid cursor"}', { status: 400 }));
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('resume-error', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
                    headers: {
            ...
            it('does not rewrite non-SSE GET responses', async () => {
                ...
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('plain-get', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'GET',
            ...
            it('does not rewrite POST responses', async () => {
                const fetchFn = vi
                    .fn()
                    .mockResolvedValue(new Response('bad request', { status: 400 }));
                const fetchWithFallback = createStreamableHttpCompatibilityFetch('post-test', fetchFn);
                const response = await fetchWithFallback('http://test-server/mcp', {
                    method: 'POST',
        ...
        describe('authenticated Streamable HTTP compatibility fetch', () => {
            ...
            it('throws the /mcp instruction when OAuth has no valid token', async () => {
                ...
                await expect(createTransport('oauth-test-server', {
                    httpUrl: 'http://test-server',
                    oauth: {
                        enabled: true,
                        clientId: 'client-id',
                    },
                }, false)).rejects.toThrow(getMcpOAuthDialogInstruction('authenticate', 'oauth-test-server'));
            ...
            it('warns when stored OAuth credentials cannot produce a token', async () => {
                ...
                const transport = await createTransport('oauth-test-server', {
                    httpUrl: 'http://test-server',
                }, false);
                expect(transport).toBeInstanceOf(StreamableHTTPClientTransport);
                ...
                expect(mockDebugLogger.warn).toHaveBeenCalledWith("Stored OAuth credentials exist for server 'oauth-test-server' but no valid token could be obtained. Transport will be created without authentication; expect a 401. " +
                    getMcpOAuthDialogInstruction('re-authenticate', 'oauth-test-server'));
            });
            ...
            it('wires the compatibility fetch for service account httpUrl transports', async () => {
                const transport = await createTransport('service-account-test-server', {
                    httpUrl: 'http://test-server',
                    authProviderType: AuthProviderType.SERVICE_ACCOUNT_IMPERSONATION,
                    targetAudience: 'client.apps.googleusercontent.com',
                    targetServiceAccount: 'service-account@example-project.iam.gserviceaccount.com',
                }, false);
                expect(transport).toBeInstanceOf(StreamableHTTPClientTransport);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                expect(transport._fetch).toEqual(expect.any(Function));
