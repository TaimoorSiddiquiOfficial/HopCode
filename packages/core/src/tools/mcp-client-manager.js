...
function readBudgetFromEnv() {
    ...
    if (rawMode === 'enforce' || rawMode === 'warn' || rawMode === 'off') {
        ...
        if (rawMode !== undefined && rawMode !== '') {
            ...
            // typo is visible.
            process.stderr.write(`hopcode serve: ignoring invalid HOPCODE_SERVE_MCP_BUDGET_MODE=` +
    ...
    if ((budgetMode === 'enforce' || budgetMode === 'warn') &&
        ...
        process.stderr.write(`hopcode serve: HOPCODE_SERVE_MCP_BUDGET_MODE=${budgetMode} requires ` +
...
export class McpClientManager {
    ...
    constructor(config, toolRegistry, options = {}) {
        ...
        if ((resolvedMode === 'enforce' || resolvedMode === 'warn') &&
            ...
            process.stderr.write(`hopcode serve: McpClientManager constructed with budgetMode=${resolvedMode} ` +
    ...
    refuseAndLog(serverName, serverConfig) {
        ...
        process.stderr.write(`hopcode serve: MCP server '${serverName}' refused (budget exhausted, ` +
    ...
    async runDiscoverAllMcpToolsViaPool(cliConfig) {
        ...
        try {
            ...
            const acquirePromises = Object.entries(servers).map(async ([name, config]) => {
                ...
                // constructed in `HopCodeAgent` ctor without it. Routing SDK
                ...
                if (this.pooledConnections.has(name))
                    return;
                try {
