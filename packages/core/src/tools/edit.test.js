...
describe('EditTool', () => {
    ...
    describe('team memory', () => {
        const teamFile = () => path.join(rootDir, '.hopcode', 'team-memory', 'feedback', 'x.md');
        it('blocks a secret written to a team-memory path via new_string', () => {
            ...
            expect(tool.validateToolParams(params)).toMatch(/shared with all repository collaborators/i);
        });
        ...
        it('reports the pre-existing-secret message and leaves the file untouched', async () => {
            ...
            const result = await tool
                ...
                .execute(new AbortController().signal);
            expect(JSON.stringify(result)).toMatch(/secret already exists in the current file content/i);
    ...
    describe('getConfirmationDetails', () => {
        ...
        it('should surface plain object read errors without object stringification', async () => {
            ...
            expect(err.message).toContain('Error preparing edit: Plain object read error');
    ...
    describe('Error Scenarios', () => {
        ...
        it('should return EDIT_PREPARATION_FAILURE with plain object read error messages', async () => {
            ...
            expect(result.llmContent).toContain('Error preparing edit: Plain object read error');
        ...
        it('should surface plain object write error messages without object stringification', async () => {
            ...
            const result = await invocation.execute(new AbortController().signal);
            expect(result.error?.type).toBe(ToolErrorType.FILE_WRITE_FAILURE);
            expect(result.llmContent).toContain('Error executing edit: Plain object edit error');
            expect(result.llmContent).not.toContain('[object Object]');
        });
    });
