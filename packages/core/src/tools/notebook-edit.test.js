...
describe('NotebookEditTool', () => {
    ...
    it('blocks writing a secret into a team-memory notebook', async () => {
        const teamDir = path.join(tempDir, '.hopcode', 'team-memory');
    ...
    it('blocks a secret in a sibling cell at execute time (full-notebook backstop)', async () => {
        ...
        expect(result.llmContent).toMatch(/shared with all repository collaborators/i);
    ...
    it('returns FILE_CHANGED_SINCE_READ when a notebook disappears after content read', async () => {
        ...
        vi.spyOn(fileSystemService, 'readTextFile').mockImplementation(async (args) => {
    ...
    it('rejects hopcodeignored notebooks during validation', () => {
        fs.writeFileSync(path.join(tempDir, '.hopcodeignore'), '*.ipynb\n', 'utf-8');
