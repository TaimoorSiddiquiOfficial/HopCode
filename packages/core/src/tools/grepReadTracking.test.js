...
describe('recordGrepResultFileReads', () => {
    ...
    function mockConfig(cache = fileReadCache, disabled = false) {
        return {
    ...
    it('does nothing when the config has no cache', async () => {
        const filePath = path.join(tempRootDir, 'result.ts');
        await fs.writeFile(filePath, 'match');
        await expect(recordGrepResultFileReads(mockConfig(undefined), [filePath])).resolves.toBeUndefined();
    });
    ...
    it('ignores result paths that disappear before stat', async () => {
        await expect(recordGrepResultFileReads(mockConfig(), [
...
//# sourceMappingURL=grepReadTracking.test.js.map