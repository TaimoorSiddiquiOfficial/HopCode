...
import { FORK_AGENT, FORK_DEFAULT_MAX_TURNS, FORK_SUBAGENT_TYPE, FORK_PLACEHOLDER_RESULT, buildForkedMessages, buildChildMessage, buildPinnedWorktreeNotice, buildWorktreeNotice, isForkSubagentEnabled, runInForkContext, } from './fork-subagent.js';
 ...
 *   user-facing reason.
 */
async function resolveExternalWorktreeDir(config, workingDir) {
    ...
    if (relToRepo.startsWith('..') || path.isAbsolute(relToRepo)) {
        return {
            error: `working_dir "${resolvedPath}" resolves outside this repository ` +
    ...
    if (!(await wtService.isRegisteredLinkedWorktree(resolvedPath))) {
        ...
        return {
            error: `working_dir "${resolvedPath}" is not a registered linked worktree of ` +
...
export function resolveSubagentApprovalMode(parentApprovalMode, agentApprovalMode, isTrustedFolder) {
    ...
    if (parentApprovalMode === ApprovalMode.IZN ||
 ...
 */
export const TOOL_REGISTRY_REBUILT = Symbol.for('hopcode:tool-registry-rebuilt');
/**
...
function capturePersistedCliFlags(config, resolvedApprovalMode, modelOverride) {
    return {
        ...
        const initialSchema = {
            ...
            properties: {
                ...
                run_in_background: {
                    type: 'boolean',
                    default: true,
                    description: 'Defaults to true for top-level one-shot agents. Set to false to run in the foreground and return the result inline. Nested agents run in the foreground. Caller-owned working_dir launches default to foreground and cannot run in the background.',
                },
                ...
                isolation: {
                    type: 'string',
                    enum: ['worktree'],
                    description: "Isolation mode. 'worktree' creates a temporary git worktree under <projectRoot>/.hopcode/worktrees/agent-<7hex> so the agent works on an isolated copy of the repo. The worktree is auto-removed if the agent makes no changes; otherwise the worktree path and branch are returned in the result.",
                },
                working_dir: {
                    type: 'string',
...
class AgentToolInvocation extends BaseToolInvocation {
    ...
    /**
     * Wrap a subagent body in `hopcode.subagent` span lifecycle.
     *
    ...
    async execute(signal, updateOutput) {
        ...
        try {
            ...
            if (isForkRequested && !isFork) {
                debugLogger.debug(`[AgentTool] Fork request downgraded to a regular sub-agent (${isTopLevelSession()
            ...
            // child's results.
            const backgroundRequested = this.params.run_in_background ??
            ...
            if (shouldRunInBackground) {
                ...
                subagentModelId = resolveModelId(subagentConfig.model, buildModelIdContext(this.config))?.modelId;
            ...
            if (isFork) {
                ...
                const result = await this.subagentManager.createAgentHeadless(subagentConfig, agentConfig, { eventEmitter: this.eventEmitter });
            ...
            if (shouldRunInBackground) {
                ...
                const shouldBubble = Boolean(subagentConfig.approvalMode === BUBBLE_APPROVAL_MODE &&
                ...
                if (isFork) {
                    ...
                    const bgResult = await this.subagentManager.createAgentHeadless(subagentConfig, bgConfig, { eventEmitter: bgEventEmitter });
                ...
                writeAgentMeta(metaPath, {
                    ...
                    persistedCliFlags: capturePersistedCliFlags(this.config, resolvedApprovalMode, bgSubagent.getCore().modelConfig.model),
                    subagentName: subagentConfig.name,
                    agentColor: subagentConfig.color,
                    resumeCount: 0,
                ...
                // spawn nested forks.
                const bgBody = async (recordSpanOutcome) => {
                    try {
                        ...
                        recordSpanOutcome(deriveSubagentOutcomeMetadata({
                            terminateMode,
                        ...
                        recordSpanOutcome(deriveSubagentExceptionMetadata(error, bgAbortController.signal.aborted));
                ...
                // hopcode.subagent span (#3731 Phase 3) — background is
                ...
                const framedBgBody = () => this.runWithSubagentSpan(this.buildSubagentSpanSpec(hookOpts, subagentConfig, isFork ? 'fork' : 'background'), 
                ...
                bgAbortController.signal, (recordOutcome) => runWithAgentContext(hookOpts.agentId, () => bgBody(recordOutcome)));
                ...
                const bgPromise = isFork
                    ...
                    : framedBgBody();
                bgPromise.catch((err) => debugLogger.warn(`[Agent] background subagent ${hookOpts.agentId} body raised unexpected rejection: ${err instanceof Error ? err.message : String(err)}`));
            ...
            if (isFork) {
                ...
                // Wrap fork body in hopcode.subagent span (#3731 Phase 3). Forks
            ...
            // Wrap in hopcode.subagent span (#3731 Phase 3). Foreground
            // invocations are child spans of the AGENT tool's `hopcode.tool`
