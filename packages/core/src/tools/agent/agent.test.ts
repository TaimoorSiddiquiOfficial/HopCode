  ...
  describe('AgentToolInvocation', () => {
    ...
    it('passes custom ignore files into worktree isolation file service', async () => {
      ...
      try {
        ...
        expect(
          agentConfig.getFileService().gethopcodeignoreFileNamesDisplay(),
        ).toBe('.hopcodeignore, .cursorignore');
        expect(
          agentConfig.getFileService().shouldhopcodeignoreFile('secret.txt'),
        ).toBe(true);
        expect(
          agentConfig
            .getFileService()
            .gethopcodeignoreFileDisplayForPath('secret.txt'),
        ).toBe('.cursorignore');
      } finally {
