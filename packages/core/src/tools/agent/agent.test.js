...
import { buildChildMessage, FORK_AGENT, FORK_DEFAULT_MAX_TURNS, runInForkContext, } from './fork-subagent.js';
...
vi.mock('../../telemetry/index.js', async (importOriginal) => {
    ...
    return {
        ...
        runInSubagentSpanContext: (_span, fn) => fn(),
    };
});
...
describe('AgentTool', () => {
    ...
    describe('initialization', () => {
        ...
        it('omits fork discipline but keeps "Writing the prompt" when non-interactive', async () => {
            ...
            expect(nonInteractiveTool.description).not.toContain('Writing a fork prompt');
            ...
            expect(nonInteractiveTool.description).toContain('Never delegate understanding');
        });
        ...
        it('requires bounded delegation and verification of subagent results', async () => {
            ...
            expect(tool.description).toContain('Do not duplicate work between the parent and subagents');
            expect(tool.description).toContain('disjoint write scopes');
            expect(tool.description).toContain("Treat the agent's output as evidence, not as automatically correct");
            expect(tool.description).not.toContain("The agent's outputs should generally be trusted");
            expect(tool.description).not.toContain('Launch multiple agents concurrently whenever possible');
        });
    });
    describe('schema generation', () => {
        ...
        it('declares the background default and foreground opt-out', () => {
            const properties = agentTool.schema.parametersJsonSchema;
            expect(properties.properties.run_in_background.default).toBe(true);
            expect(properties.properties.run_in_background.description).toContain('Set to false');
        });
        ...
        it('exposes plan_mode_required only when teams are enabled', async () => {
            ...
            const schema = teamAgentTool.schema;
            const parameters = schema.parametersJsonSchema;
            expect(parameters.properties.plan_mode_required?.description).toContain('named teammate');
            ...
            const ordinaryParameters = ordinarySchema.parametersJsonSchema;
    ...
    describe('isolation — round-7 parent-dirty guard', () => {
        it('refuses isolation when parent has uncommitted edits', async () => {
            ...
            const repo = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'hopcode-iso-dirty-'));
            try {
        ...
        it('would allow isolation when parent is clean (sanity)', async () => {
            ...
            const os = await import('node:os');
            const { execFileSync } = await import('node:child_process');
            const repo = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'hopcode-iso-clean-'));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], {
                    cwd: repo,
    ...
    describe('execution with an unknown subagent', () => {
        it('reports not-found with the available list when the agent is missing on disk', async () => {
            ...
            expect(mockSubagentManager.loadSubagent).toHaveBeenCalledWith('non-existent');
            expect(result.llmContent).toBe('Subagent "non-existent" not found. Available subagents: file-search, code-review');
        });
        it('still reports not-found when listing available subagents fails', async () => {
            vi.mocked(mockSubagentManager.loadSubagent).mockResolvedValue(null);
            vi.mocked(mockSubagentManager.listSubagents).mockRejectedValue(new Error('fs error'));
            const invocation = agentTool.build({
                description: 'Use missing agent',
                prompt: 'Do work',
                subagent_type: 'non-existent',
    ...
    describe('team routing', () => {
        ...
        it('passes plan_mode_required through to TeamManager for named teammates', async () => {
            ...
            expect(spawnTeammate).toHaveBeenCalledWith(expect.objectContaining({
                name: 'planner',
        ...
        it('rejects plan_mode_required direct execution from a subagent context', async () => {
            ...
            const result = await runWithAgentContext('child-agent', () => invocation.execute(new AbortController().signal));
        ...
        it('rejects working_dir when a named teammate would spawn (worktree pin would be silently ignored)', async () => {
            ...
            const result = await invocation.execute(new AbortController().signal);
            expect(partToString(result.llmContent)).toMatch(/not supported for a named teammate/i);
            expect(spawnTeammate).not.toHaveBeenCalled();
        });
    });
    describe('nesting depth guard', () => {
        it('rejects a spawn that would exceed maxSubagentDepth', async () => {
            ...
            const result = await runWithAgentContext('sub-1', () => invocation.execute(new AbortController().signal));
        ...
        it('blocks a teammate from spawning any sub-agent', async () => {
            ...
            const result = await runWithTeammateIdentity({
                ...
                teamName: 'demo',
                isTeamLead: false,
            }, () => runWithAgentContext('teammate-1', () => invocation.execute(new AbortController().signal)));
        ...
        it('blocks a fork child from spawning any sub-agent', async () => {
            ...
            const result = await runInForkContext(() => invocation.execute(new AbortController().signal));
            expect(result.llmContent).toContain('Cannot spawn sub-agents from within a fork');
            expect(mockSubagentManager.loadSubagent).not.toHaveBeenCalled();
            // Same failure-path contract as the depth guard above: the model-facing
            // message keeps the actionable instruction.
            expect(result.error?.message).toContain('Cannot spawn sub-agents from within a fork');
        ...
        it('allows nesting while depth remains under the cap', async () => {
            ...
            await runWithAgentContext('sub-1', () => runWithAgentContext('sub-2', () => invocation.execute(new AbortController().signal)));
            expect(mockSubagentManager.loadSubagent).toHaveBeenCalledWith('file-search');
        });
        it('ignores a teammate name from a nested sub-agent and spawns a regular sub-agent', async () => {
            ...
            await runWithAgentContext('sub-1', () => invocation.execute(new AbortController().signal));
            expect(spawnTeammate).not.toHaveBeenCalled();
            expect(mockSubagentManager.loadSubagent).toHaveBeenCalledWith('file-search');
        });
    ...
    describe('AgentToolInvocation', () => {
        ...
        it('rejects working_dir when the resolved subagent config runs in the background', async () => {
            ...
            const invocation = agentTool.createInvocation({
                description: 'Review',
                prompt: 'Review the diff',
                subagent_type: 'file-search',
                working_dir: '.qwen/tmp/review-pr-1',
        ...
        it('allows working_dir for a background:true subagent that downgrades to foreground when nested', async () => {
            ...
            const nonRepo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-nested-')));
            try {
        ...
        it('preserves diagnostic tags from failed subagent result', async () => {
            ...
            vi.mocked(mockAgent.getTerminateMode).mockReturnValue(AgentTerminateMode.ERROR);
            const params = {
                description: 'Search files',
                prompt: 'Find all TypeScript files',
                subagent_type: 'file-search',
        ...
        it('explains successful subagents with no model-visible output', async () => {
            vi.mocked(mockAgent.getFinalText).mockReturnValue('<analysis>scratch only</analysis>');
            const params = {
                description: 'Search files',
                prompt: 'Find all TypeScript files',
                subagent_type: 'file-search',
            ...
            const invocation = agentTool.createInvocation(params);
            const result = await invocation.execute();
            expect(partToString(result.llmContent)).toBe('(subagent produced no model-visible output)');
        });
        it('passes custom ignore files into worktree isolation file service', async () => {
            ...
            try {
                ...
                const createCall = vi.mocked(mockSubagentManager.createAgentHeadless)
                    .mock.calls[0];
                const agentConfig = createCall[1];
                expect(agentConfig.getProjectRoot()).not.toBe(repo);
                expect(agentConfig.getFileService().gethopcodeignoreFileNamesDisplay()).toBe('.hopcodeignore, .cursorignore');
                expect(agentConfig.getFileService().shouldhopcodeignoreFile('secret.txt')).toBe(true);
                expect(agentConfig
                    .getFileService()
                    .gethopcodeignoreFileDisplayForPath('secret.txt')).toBe('.cursorignore');
            }
            finally {
        ...
        it('pins the sub-agent to a caller-owned worktree via working_dir and leaves it in place', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                ...
                expect(partToString(result.llmContent)).not.toContain('[worktree preserved');
                // A pinned worktree gets the narrow notice, not the isolation notice
                // (which tells the agent to translate the parent's paths).
                expect(mockContextState.set).toHaveBeenCalledWith('task_prompt', expect.stringContaining('Your working directory is'));
                expect(mockContextState.set).not.toHaveBeenCalledWith('task_prompt', expect.stringContaining('translate it to the corresponding path'));
            }
            finally {
                fs.rmSync(repo, { recursive: true, force: true });
                vi.useFakeTimers();
        ...
        it('keeps a working_dir launch in the foreground when the flag is omitted', async () => {
            ...
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-fg-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
                ...
                const wt = path.join(repo, '.qwen', 'tmp', 'review-pr-1');
                fs.mkdirSync(path.dirname(wt), { recursive: true });
                execFileSync('git', ['worktree', 'add', '-b', 'review-pr-1', wt, 'HEAD'], { cwd: repo });
                vi.mocked(config.getProjectRoot).mockReturnValue(repo);
                vi.mocked(config.getTargetDir).mockReturnValue(repo);
                vi.mocked(config.getCwd).mockReturnValue(repo);
                vi.mocked(config.getWorkingDir).mockReturnValue(repo);
                ...
                expect(partToString(result.llmContent)).not.toContain('Background agent launched');
                expect(vi.mocked(mockSubagentManager.createAgentHeadless)).toHaveBeenCalled();
            }
            finally {
                fs.rmSync(repo, { recursive: true, force: true });
                vi.useFakeTimers();
        ...
        it('rejects working_dir that is not a registered worktree of this repo', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-bad-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
                ...
                const result = await invocation.execute();
                expect(partToString(result.llmContent)).toMatch(/not a registered linked worktree/i);
                expect(mockSubagentManager.createAgentHeadless).not.toHaveBeenCalled();
            }
            finally {
                fs.rmSync(repo, { recursive: true, force: true });
        ...
        it('resolves a repo-relative working_dir against the parent cwd (the /review production form)', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-rel-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
        ...
        it('rejects working_dir pointing at the repository main working tree', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-main-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
        ...
        it('rejects working_dir when the parent directory is not a git repository', async () => {
            vi.useRealTimers();
            const nonRepo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-nogit-')));
            try {
                vi.mocked(config.getProjectRoot).mockReturnValue(nonRepo);
                vi.mocked(config.getTargetDir).mockReturnValue(nonRepo);
                vi.mocked(config.getCwd).mockReturnValue(nonRepo);
                ...
                expect(partToString(result.llmContent)).toMatch(/not a git repository/i);
                expect(mockSubagentManager.createAgentHeadless).not.toHaveBeenCalled();
            }
            finally {
                fs.rmSync(nonRepo, { recursive: true, force: true });
        ...
        it('accepts a registered worktree in detached HEAD state (no branch)', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-detached-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
        ...
        it('leaves the caller-owned worktree in place when the sub-agent fails', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-failpath-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
                ...
                vi.mocked(mockAgent.execute).mockRejectedValue(new Error('subagent boom'));
                const invocation = agentTool.createInvocation({
                    description: 'Review',
                    prompt: 'Review the diff',
                    subagent_type: 'file-search',
        ...
        it('re-anchors validation at the repo root when launched from a monorepo subdirectory', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-mono-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
        ...
        it('resolves a repo-relative working_dir against the subdirectory cwd, not the repo root (monorepo)', async () => {
            vi.useRealTimers();
            const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-agent-wd-monorel-')));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], { cwd: repo });
                execFileSync('git', ['config', 'user.name', 't'], { cwd: repo });
        ...
        describe('hopcode.subagent span outcome (#4410 wenshao)', () => {
            beforeEach(() => {
            ...
            it('GOAL terminateMode → status="completed" + resultSummaryPresent', async () => {
                vi.mocked(mockAgent.getTerminateMode).mockReturnValue(AgentTerminateMode.GOAL);
            ...
            it('MAX_TURNS terminateMode → status="failed" + error/errorType populated', async () => {
                vi.mocked(mockAgent.getTerminateMode).mockReturnValue(AgentTerminateMode.MAX_TURNS);
                await runForegroundOnce();
                const meta = lastEndMeta();
                expect(meta.status).toBe('failed');
            ...
            it('SHUTDOWN terminateMode → status="cancelled" + terminateReason="subagent_shutdown"', async () => {
                ...
                vi.mocked(mockAgent.getTerminateMode).mockReturnValue(AgentTerminateMode.SHUTDOWN);
                await runForegroundOnce();
                const meta = lastEndMeta();
                expect(meta.status).toBe('cancelled');
            ...
            it('subagent.execute throws → status="failed" + errorType=Error', async () => {
                vi.mocked(mockAgent.execute).mockRejectedValue(new Error('catastrophic boom'));
                await runForegroundOnce();
                const meta = lastEndMeta();
                expect(meta.status).toBe('failed');
            ...
            it('fallback: body that skips recordOutcome → status="failed" + wiring-bug terminateReason', async () => {
                ...
                invocation.runSubagentWithHooks = vi.fn().mockResolvedValue(undefined);
                await invocation.execute();
                const meta = lastEndMeta();
                expect(meta.status).toBe('failed');
                expect(meta.terminateReason).toBe('wiring_bug_record_outcome_not_called');
            ...
            it('throw + aborted signal → status="aborted" + terminateReason="signal_aborted"', async () => {
                ...
                vi.mocked(mockAgent.execute).mockRejectedValue(new Error('boom mid-cancel'));
                const params = {
                    description: 'Search files',
                    prompt: 'Find all TypeScript files',
                    subagent_type: 'file-search',
    ...
    describe('Fork dispatch (subagent_type: "fork")', () => {
        ...
        it('does not require a commit unless the directive asks for one', () => {
            ...
            expect(childMessage).toContain('Do NOT create a commit unless the directive explicitly asks you to');
            expect(childMessage).toContain('Verification: <checks performed and their outcome');
            expect(childMessage).not.toContain('commit your changes before reporting');
        });
        it('forks in interactive mode', async () => {
            const mockLoadedSubagent = {
            ...
            vi.mocked(mockSubagentManager.loadSubagent).mockResolvedValue(mockLoadedSubagent);
            const params = {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(mockSubagentManager.loadSubagent).not.toHaveBeenCalledWith('general-purpose');
        ...
        it('reserves a background slot with the resolved parent model when fork runs in background', async () => {
            ...
            expect(stubRegistry.tryReserveBackgroundSlot).toHaveBeenCalledWith('parent-model');
            expect(stubRegistry.register).toHaveBeenCalledWith(expect.objectContaining({
                model: 'parent-model',
                isBackgrounded: true,
                subagentType: 'fork',
    ...
    describe('SubagentStart hook integration', () => {
        ...
        it('should inject hook_context as empty string when additionalContext is undefined', async () => {
            ...
            expect(mockContextState.set).not.toHaveBeenCalledWith('hook_context', expect.stringMatching(/.+/));
        });
    ...
    describe('IDE diff-tab confirmation clears pendingConfirmation', () => {
        ...
        it('preserves subagent tool protocol payloads in non-interactive mode', async () => {
            vi.mocked(config.isInteractive).mockReturnValue(false);
            const responseParts = [{ text: 'raw protocol result' }];
            const snapshots = [];
            ...
            const resultSnapshot = snapshots.find((snapshot) => snapshot.toolCalls?.some((toolCall) => toolCall.callId === 'call-read-1' && toolCall.status === 'success'));
            const toolCall = resultSnapshot?.toolCalls?.find((entry) => entry.callId === 'call-read-1');
    ...
    describe('Agent-level background: true', () => {
        ...
        it('should run in background when agent definition has background: true', async () => {
            ...
            expect(writeMetaSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
        ...
        it('stores sanitized background results in the registry', async () => {
            vi.mocked(mockAgent.getFinalText).mockReturnValue('<analysis>scratch</analysis><summary>visible</summary>');
            const invocation = agentTool.createInvocation({
                description: 'Start monitor',
                prompt: 'Watch for changes',
                subagent_type: 'monitor',
            ...
            await invocation.execute();
            await vi.runAllTimersAsync();
            expect(mockRegistry.complete).toHaveBeenCalledWith(expect.any(String), 'visible', expect.any(Object));
        });
        it('stores a fallback for background results with no model-visible text', async () => {
            ...
            await invocation.execute();
            await vi.runAllTimersAsync();
            expect(mockRegistry.complete).toHaveBeenCalledWith(expect.any(String), '(subagent produced no model-visible output)', expect.any(Object));
        });
        ...
        it('runs a top-level subagent in the background when the flag is omitted', async () => {
            ...
            vi.mocked(mockSubagentManager.loadSubagent).mockResolvedValue(defaultSubagent);
            const invocation = agentTool.createInvocation({
                description: 'Search files',
                prompt: 'Find all TypeScript files',
                subagent_type: 'file-search',
            ...
            const result = await invocation.execute();
            expect(partToString(result.llmContent)).toContain('Background agent launched');
            expect(mockRegistry.register).toHaveBeenCalled();
        });
        it('keeps a named-teammate launch in the foreground when no team is active and the flag is omitted', async () => {
            ...
            expect(partToString(result.llmContent)).not.toContain('Background agent launched');
            expect(mockRegistry.register).toHaveBeenCalledWith(expect.objectContaining({ isBackgrounded: false }));
        });
        ...
        it('lets an explicit run_in_background: false override a config with background: true', async () => {
            ...
            const explicitBackgroundConfig = {
                ...bgSubagent,
                name: 'file-search',
            };
            vi.mocked(mockSubagentManager.loadSubagent).mockResolvedValue(explicitBackgroundConfig);
            const invocation = agentTool.createInvocation({
                description: 'Search files',
                prompt: 'Find all TypeScript files',
                subagent_type: 'file-search',
        ...
        it('downgrades a background request from a nested sub-agent to an awaited foreground run', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            const result = await runWithAgentContext('sub-1', () => invocation.execute());
            const llmText = partToString(result.llmContent);
        ...
        it('returns registry registration errors to the model without launching the background body', async () => {
            const errorMessage = 'Cannot start background agent: maximum concurrent background agents ' +
            ...
            try {
                ...
                expect(result.returnDisplay.status).toBe('failed');
        ...
        it('fires SubagentStop when the final background register check fails after SubagentStart', async () => {
            ...
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenCalledWith(expect.stringContaining('monitor-'), 'monitor', expect.stringMatching(/subagents[\\/]test-session-id[\\/]agent-monitor-.*\.jsonl$/), 'Monitor done', false, PermissionMode.AutoEdit, undefined);
            expect(mockAgent.execute).not.toHaveBeenCalled();
        });
        it('waits for a background slot before hooks and subagent setup', async () => {
            ...
            mockRegistry.waitForBackgroundSlot.mockReturnValue(new Promise((resolve) => {
                releaseSlot = resolve;
            }));
            const mockHookSystem = {
                fireSubagentStartEvent: vi.fn().mockResolvedValue(undefined),
            ...
            expect(mockRegistry.tryReserveBackgroundSlot).toHaveBeenCalledWith('parent-model');
            expect(mockRegistry.waitForBackgroundSlot).toHaveBeenCalledWith(undefined, 'parent-model');
            expect(mockHookSystem.fireSubagentStartEvent).not.toHaveBeenCalled();
            expect(mockSubagentManager.createAgentHeadless).not.toHaveBeenCalled();
            expect(mockRegistry.register).not.toHaveBeenCalled();
            expect(updates.some((update) => update.terminateReason ===
                'Waiting for a sub-agent slot (1 already queued).')).toBe(true);
            releaseSlot?.(slotReservation);
            const result = await executePromise;
            expect(partToString(result.llmContent)).toContain('Background agent launched');
            expect(mockRegistry.register).toHaveBeenCalledWith(expect.objectContaining({ status: 'running' }), expect.objectContaining({ slotReservation }));
        });
        it('passes the sidechain transcript path to SubagentStop hooks for fresh background agents', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            const expectedTranscriptPrefix = path.join('/tmp/hopcode-test', 'subagents', 'test-session-id', 'agent-monitor-');
            await vi.waitFor(() => {
        ...
        it('keeps bubble-mode background agents on auto-deny in non-interactive mode', async () => {
            ...
            const createCalls = vi.mocked(mockSubagentManager.createAgentHeadless)
                .mock.calls;
            const createdConfig = createCalls[createCalls.length - 1][1];
        ...
        it('forwards the scheduler-provided callId as toolUseId on the registry entry', async () => {
            ...
            await invocation.execute();
            expect(mockRegistry.register).toHaveBeenCalledWith(expect.objectContaining({ toolUseId: 'call-xyz-789' }), expect.objectContaining({
...
describe('resolveSubagentApprovalMode', () => {
    it('should return izn when parent is izn, regardless of agent config', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.IZN, 'plan', true)).toBe(PermissionMode.Izn);
        expect(resolveSubagentApprovalMode(ApprovalMode.IZN, undefined, false)).toBe(PermissionMode.Izn);
    });
    ...
    it('should respect agent-declared mode when parent is default and folder is trusted', () => {
        ...
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'izn', true)).toBe(PermissionMode.Izn);
    });
    it('should block privileged agent-declared modes in untrusted folders', () => {
        ...
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'izn', false)).toBe(PermissionMode.Default);
    });
    ...
    it('should resolve the subagent-only "bubble" mode to Default (confirmation required)', () => {
        ...
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'bubble', true)).toBe(PermissionMode.Default);
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'bubble', false)).toBe(PermissionMode.Default);
    });
    it('should let a permissive parent win over a "bubble" subagent mode', () => {
        ...
        expect(resolveSubagentApprovalMode(ApprovalMode.IZN, 'bubble', true)).toBe(PermissionMode.Izn);
        expect(resolveSubagentApprovalMode(ApprovalMode.AUTO_EDIT, 'bubble', true)).toBe(PermissionMode.AutoEdit);
    });
});
