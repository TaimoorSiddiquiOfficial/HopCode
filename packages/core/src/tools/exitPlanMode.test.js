...
describe('ExitPlanModeTool', () => {
    ...
    it('exposes the plan schema without the removed gate fields', () => {
        ...
        expect(tool.schema.parametersJsonSchema.properties).not.toHaveProperty('resolutionSummary');
    });
    it.each([undefined, '', '  \n', 123])('rejects an invalid plan (%j)', (plan) => {
        expect(tool.validateToolParams({ plan })).toBe('Parameter "plan" must be a non-empty string.');
    });
    ...
    it('denies outside plan mode without constructing a misleading prompt', async () => {
        ...
        await expect(invocation.getConfirmationDetails(new AbortController().signal)).rejects.toThrow('outside plan mode');
    });
    it.each([
    ...
    ])('records %s and changes mode only during execute', async (outcome, targetMode) => {
    ...
    it('executes the snapshot belonging to the confirmation that was approved', async () => {
        ...
        const invocation = tool.build(params);
        const firstConfirmation = await invocation.getConfirmationDetails(new AbortController().signal);
    ...
    it.each([
        ...
        const invocation = tool.build({ plan: 'Plan' });
        const confirmation = await invocation.getConfirmationDetails(new AbortController().signal);
        await expect(confirmation.onConfirm(outcome)).rejects.toThrow('Invalid plan approval outcome');
        expect(approvalMode).toBe(ApprovalMode.PLAN);
        expect(config.setApprovalMode).not.toHaveBeenCalled();
    });
    it('keeps plan mode when aborted after approval', async () => {
        ...
        const invocation = tool.build({ plan: 'Plan' });
        const confirmation = await invocation.getConfirmationDetails(controller.signal);
    ...
    it('retains the subagent lifecycle policy', async () => {
        ...
        await expect(runWithAgentContext('agent-1', async () => invocation.requiresUserInteraction?.())).resolves.toBe(false);
        await expect(runWithAgentContext('agent-1', () => invocation.getDefaultPermission())).resolves.toBe('allow');
        const result = await runWithAgentContext('agent-1', () => invocation.execute(new AbortController().signal));
    ...
    it('rechecks the revision after asynchronous teammate leader approval', async () => {
        let resolveDecision;
        const requestPlanApproval = vi.fn(() => new Promise((resolve) => {
