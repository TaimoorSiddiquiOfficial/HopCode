...
class BrowserToolInvocation extends BaseToolInvocation {
    ...
    getDescription() {
        ...
        switch (this.params.action) {
            ...
            case 'click':
                ...
                break;
            case 'fill':
                parts.push(`fill ${this.params.selector ?? '(no selector)'} with "${this.params.value ?? ''}"`);
                break;
            case 'screenshot':
    ...
    async _executeAction() {
        ...
        const { session, id, isNew } = await getOrCreateSession(sessionId, headless);
        switch (action) {
            case 'navigate': {
                if (!url) {
                    return {
                ...
                let snapshotText = '';
                try {
                    snapshotText = await pageOf(session).evaluate(() => document.body ? document.body.innerText.slice(0, 5000) : '');
                }
                catch {
            ...
            case 'fill': {
                if (!selector || value === undefined) {
                    return {
                        llmContent: 'Error: selector and value parameters are required for fill.',
                        returnDisplay: 'Error: selector and value parameters are required for fill.',
                    };
                }
            ...
            case 'screenshot': {
                ...
                const screenshotPath = path.join(tmpDir, `hopcode-browser-${Date.now()}.png`);
            ...
            case 'snapshot': {
                const text = await pageOf(session).evaluate(() => document.body ? document.body.innerText : '');
                if (!text.trim()) {
                    return {
...
export class BrowserTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.BROWSER;
    constructor(config) {
        super(BrowserTool.Name, ToolDisplayNames.BROWSER, 'Automate a headless web browser via Playwright. ' +
            ...
            'The first navigate call returns a sessionId -- pass it in subsequent calls to reuse the same browser session.', Kind.Fetch, {
            type: 'object',
            properties: {
                action: {
                    type: 'string',
                    description: 'The action to perform: navigate, click, fill, screenshot, snapshot, wait, or close.',
                ...
                sessionId: {
                    type: 'string',
                    description: 'Session ID from a previous navigate call. Omit for a new session.',
                },
                headed: {
                    type: 'boolean',
                    description: 'Run browser in headed (visible) mode. Default: false (headless).',
                },
            },
            required: ['action'],
        });
