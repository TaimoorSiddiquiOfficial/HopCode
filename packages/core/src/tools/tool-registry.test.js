...
describe('ToolRegistry', () => {
    ...
    describe('registerTool', () => {
        ...
        it('honors a legacy dotted disabled MCP tool name', () => {
            ...
            const registry = new ToolRegistry(disabledConfig);
            const mcpTool = new DiscoveredMCPTool({}, 'zybio', 'literature.search_pubmed', 'description', {});
        ...
        it('honors a legacy truncated disabled MCP tool name', () => {
            ...
            const registry = new ToolRegistry(disabledConfig);
            const mcpTool = new DiscoveredMCPTool({}, 'server', 'x'.repeat(80), 'description', {});
            expect(mcpTool.name).not.toBe(legacyName);
            registry.registerTool(mcpTool);
            expect(registry.getTool(mcpTool.name)).toBeUndefined();
        });
    ...
    describe('deferred tool filtering', () => {
        ...
        it('includes deferred tools when includeDeferred is true', () => {
            ...
            toolRegistry.registerTool(new MockTool({ name: 'hidden-a', shouldDefer: true }));
        ...
        it('sorts MCP declarations deterministically regardless of registration order', () => {
            ...
            registryA.registerTool(new DiscoveredMCPTool(mcpCallable, 'github', 'search_issues', 'Search GitHub issues', {}));
            registryA.registerTool(new DiscoveredMCPTool(mcpCallable, 'filesystem', 'read_tree', 'Read filesystem tree', {}));
            registryB.registerTool(new DiscoveredMCPTool(mcpCallable, 'filesystem', 'read_tree', 'Read filesystem tree', {}));
            registryB.registerTool(new DiscoveredMCPTool(mcpCallable, 'github', 'search_issues', 'Search GitHub issues', {}));
        ...
        it('getDeferredToolSummary includes MCP server names', () => {
            const mcpCallable = {};
            toolRegistry.registerTool(new DiscoveredMCPTool(mcpCallable, 'schedule-server', 'cron_list', 'list scheduled jobs', {}));
        ...
        it('includes deferred tools listed in visibleTools in function declarations', () => {
            ...
            registry.registerTool(new MockTool({ name: 'should-appear', shouldDefer: true }));
            registry.registerTool(new MockTool({ name: 'still-hidden', shouldDefer: true }));
        ...
        it('excludes visibleTools items from deferred tool summary', () => {
            ...
            const registry = new ToolRegistry(visibleConfig);
            registry.registerTool(new MockTool({ name: 'alpha', description: 'a', shouldDefer: true }));
            registry.registerTool(new MockTool({ name: 'beta', description: 'b', shouldDefer: true }));
        ...
        it('disabledTools takes priority over visibleTools', () => {
            ...
            const registry = new ToolRegistry(config);
            registry.registerTool(new MockTool({ name: 'contested', shouldDefer: true }));
            const names = registry.getFunctionDeclarations().map((d) => d.name);
        ...
        it('visible tool survives clearRevealedDeferredTools', () => {
            ...
            const registry = new ToolRegistry(visibleConfig);
            registry.registerTool(new MockTool({ name: 'web_fetch', shouldDefer: true }));
            registry.registerTool(new MockTool({ name: 'monitor', shouldDefer: true }));
            // Both start visible (web_fetch via visibleTools, monitor is hidden)
            expect(registry.getFunctionDeclarations().map((d) => d.name)).toContain('web_fetch');
