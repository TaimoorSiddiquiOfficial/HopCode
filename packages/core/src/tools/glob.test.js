...
describe('GlobTool', () => {
    ...
    const mockGlobStreamResults = (prefix, count, extension = 'streamlimit') => {
        ...
        async function* streamEntries() {
            for (let index = 0; index < count; index++) {
                ...
                yield {
                    fullpath: () => path.join(tempRootDir, `${prefix}${fileNumber}.${extension}`),
        ...
        vi.mocked(glob.globStream).mockReturnValueOnce(stream);
    ...
    describe('validateToolParams', () => {
        ...
        it('should return error if path is provided but is not a string', () => {
            const params = {
                pattern: '*.ts',
                path: {},
            }; // Force incorrect type (object, not coercible)
    ...
    describe('multi-directory workspace', () => {
        ...
        it('should not scan later workspace directories after hitting the collection limit', async () => {
            ...
            try {
                ...
                const result = await invocation.execute(abortSignal);
                const llmContent = partListUnionToString(result.llmContent);
                expect(vi.mocked(glob.globStream).mock.calls.length).toBe(globStreamCallsBefore + 1);
    ...
    describe('ignore file handling', () => {
        ...
        it('should respect .hopcodeignore files by default', async () => {
            await fs.writeFile(path.join(tempRootDir, '.hopcodeignore'), '*.hopcodeignored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.hopcodeignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.notignored.txt'), 'not ignored content');
        ...
        it('should respect .agentignore and .aiignore files by default', async () => {
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), '*.agentignored.txt');
            await fs.writeFile(path.join(tempRootDir, '.aiignore'), '*.aiignored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.agentignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.aiignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'c.notignored.txt'), 'not ignored content');
            const params = { pattern: '*.txt' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
        ...
        it('should respect configured custom qwen ignore files', async () => {
            await fs.writeFile(path.join(tempRootDir, '.cursorignore'), '*.cursorignored.txt');
            await fs.writeFile(path.join(tempRootDir, '.agentignore'), '*.agentignored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.cursorignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.agentignored.txt'), 'not ignored by this config');
            await fs.writeFile(path.join(tempRootDir, 'c.notignored.txt'), 'not ignored content');
            const customConfig = {
                ...mockConfig,
        ...
        it('does not over-ignore nested dirs for a root-anchored gitignore pattern', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, 'src', 'dist', 'nested.keep'), 'x');
        ...
        it('prunes a gitignored directory (e.g. node_modules) during traversal', async () => {
            await fs.writeFile(path.join(tempRootDir, '.gitignore'), 'node_modules/\n');
            await fs.mkdir(path.join(tempRootDir, 'node_modules', 'pkg'), {
                recursive: true,
            });
            await fs.writeFile(path.join(tempRootDir, 'node_modules', 'pkg', 'dep.keep'), 'x');
        ...
        it('does not prune entries outside the project root during traversal', async () => {
            ...
            const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'glob-external-'));
            try {
        ...
        it('honors gitignore negation re-inclusion during traversal', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, '.gitignore'), 'build/**\n!build/keep.keep\n');
    ...
    describe('file count truncation', () => {
        it('stops collecting glob results at a bounded scan limit', async () => {
            ...
            const params = { pattern: '*.streamlimit' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
            const llmContent = partListUnionToString(result.llmContent);
    ...
    describe('getDescription', () => {
        ...
        it('should show project-internal paths relative to the project root', () => {
            const params = { pattern: '*.ts', path: 'sub' };
            const invocation = globTool.build(params);
        ...
        it('should keep paths outside the project absolute (never project-relative)', () => {
            ...
            const params = { pattern: '*.ts', path: outside };
            const invocation = globTool.build(params);
            expect(invocation.getDescription()).toBe(`'*.ts' in ${tildeifyPath(outside)}`);
        });
    });
