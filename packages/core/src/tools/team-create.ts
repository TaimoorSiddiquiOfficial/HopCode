      ...
      if (isNodeError(err) && err.code === 'EEXIST') {
        ...
        if (!reclaimed) {
          const msg =
            ...
            `owned by a live hopcode session. Pick a different ` +
...
- **Custom agents** defined in \`.hopcode/agents/\` may have their own tool restrictions. Check their descriptions to understand what they can and cannot do.
...
This creates:
- A team file at \`~/.hopcode/teams/{team-name}/config.json\`
- A corresponding task list directory at \`~/.hopcode/tasks/{team-name}/\`
...
- **Team config location**: \`~/.hopcode/teams/{team-name}/config.json\`
...
\`\`\`
Use the Read tool to read ~/.hopcode/teams/{team-name}/config.json
\`\`\`

## Task List Coordination

Teams share a task list that all teammates can access at \`~/.hopcode/tasks/{team-name}/\`.
