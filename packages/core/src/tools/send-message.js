...
export class SendMessageTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(SendMessageTool.Name, ToolDisplayNames.SEND_MESSAGE, 'Send a message to a teammate (use "to") or to a running background task (use "task_id"). ' +
            ...
            'Your text output is NOT visible to other agents — use this tool to communicate.', Kind.Other, {
            type: 'object',
            properties: {
                to: {
                    type: 'string',
                ...
                task_id: {
                    type: 'string',
                    description: 'The ID of the background task (from the launch response, a recovered paused task, or a completed task to revive).',
                },
                message: {
                    type: 'string',
