...
import { HostPublisher, tokenizeCommand, } from './host-publisher.js';
...
describe('HostPublisher', () => {
    ...
    it('uploads via the command and returns the templated url', async () => {
        ...
        const pub = new HostPublisher({
            uploadCommand: 'aws s3 cp {file} s3://bkt/{key} --content-type text/html',
            urlTemplate: 'https://bkt.example.com/{key}',
        }, run);
        const res = await pub.publish(input);
        expect(res.id).toBe('deadbeef');
        expect(res.url).toBe('https://bkt.example.com/artifacts/deadbeef/index.html');
    ...
    it('removes the temp file after a successful upload', async () => {
        let filePath = '';
        const run = async (_c, args) => {
            filePath = args.find((a) => a.endsWith('.html'));
        };
        await new HostPublisher({ uploadCommand: 'up {file} {key}', urlTemplate: 'https://h/{key}' }, run).publish(input);
    ...
    it('uses a unique temp file for each upload of the same artifact', async () => {
        ...
        const pub = new HostPublisher({ uploadCommand: 'up {file} {key}', urlTemplate: 'https://h/{key}' }, run);
    ...
    it('rejects urlTemplate without the key placeholder', async () => {
        ...
        await expect(new HostPublisher({ uploadCommand: 'up {file} {key}', urlTemplate: 'https://h/static' }, run).publish(input)).rejects.toThrow(/\{key\}/i);
        expect(run).not.toHaveBeenCalled();
    });
    it('rejects urlTemplate with the file placeholder', async () => {
        const run = vi.fn(async () => { });
        await expect(new HostPublisher({
            uploadCommand: 'up {file} {key}',
            urlTemplate: 'https://h/{key}?src={file}',
        }, run).publish(input)).rejects.toThrow(/\{file\}/i);
        expect(run).not.toHaveBeenCalled();
    });
    ...
    it.each([
        ...
        const run = vi.fn(async () => { });
        await expect(new HostPublisher(config, run).publish(input)).rejects.toThrow(re);
        expect(run).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=host-publisher.test.js.map