...
import { OssPublisher, signOssPut, ossCredentialsFromEnv, } from './oss-publisher.js';
const creds = {
    accessKeyId: 'AKID',
...
describe('ossCredentialsFromEnv', () => {
    ...
    it('returns undefined when missing', () => {
        expect(ossCredentialsFromEnv({})).toBeUndefined();
    });
});
describe('OssPublisher', () => {
    it('PUTs to the virtual-hosted URL with signed headers and returns the public url', async () => {
        let captured;
        const httpPut = async (url, headers, body) => {
            captured = { url, headers, body };
        };
        const pub = new OssPublisher({ bucket: 'bkt', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { httpPut, credentials: () => creds, now: fixedNow });
        const res = await pub.publish(input);
        const expectedUrl = 'https://bkt.oss-cn-hangzhou.aliyuncs.com/artifacts/cafe1234/index.html';
    ...
    it('uses publicBaseUrl for the returned url when set', async () => {
        ...
        }, { httpPut, credentials: () => creds, now: fixedNow }).publish(input);
        expect(res.url).toBe('https://cdn.example.com/artifacts/cafe1234/index.html');
    });
    it('rejects publicBaseUrl without a URL scheme', async () => {
        const httpPut = vi.fn(async () => { });
        await expect(new OssPublisher({
            bucket: 'bkt',
            endpoint: 'oss-cn-hangzhou.aliyuncs.com',
            publicBaseUrl: 'cdn.example.com',
        }, { httpPut, credentials: () => creds, now: fixedNow }).publish(input)).rejects.toThrow(/publicBaseUrl/i);
    ...
    it('uses a timeout signal for default uploads', async () => {
        ...
        try {
            await new OssPublisher({ bucket: 'bkt', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { credentials: () => creds, now: fixedNow }).publish(input);
        }
        finally {
            vi.unstubAllGlobals();
        }
    ...
    it('hides OSS error response bodies from thrown errors', async () => {
        vi.stubGlobal('fetch', vi.fn(async () => new Response('secret AccessKeyId', { status: 403 })));
        try {
            await expect(new OssPublisher({ bucket: 'bkt', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { credentials: () => creds, now: fixedNow }).publish(input)).rejects.toThrow('OSS upload failed: 403');
            await expect(new OssPublisher({ bucket: 'bkt', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { credentials: () => creds, now: fixedNow }).publish(input)).rejects.not.toThrow('AccessKeyId');
        }
        finally {
            vi.unstubAllGlobals();
        }
    ...
    it('honors a custom keyPrefix', async () => {
        let url = '';
        const httpPut = async (u) => {
            url = u;
        };
        await new OssPublisher({ bucket: 'bkt', endpoint: 'e.aliyuncs.com', keyPrefix: '/pages/' }, { httpPut, credentials: () => creds, now: fixedNow }).publish(input);
    ...
    it('rejects invalid keyPrefix values', async () => {
        ...
        await expect(new OssPublisher({
            ...
            endpoint: 'oss-cn-hangzhou.aliyuncs.com',
            keyPrefix: '/',
        }, { httpPut, credentials: () => creds, now: fixedNow }).publish(input)).rejects.toThrow(/keyPrefix/i);
        await expect(new OssPublisher({
            bucket: 'bkt',
            endpoint: 'oss-cn-hangzhou.aliyuncs.com',
    ...
    it('rejects when bucket/endpoint/credentials are missing', async () => {
        const httpPut = vi.fn(async () => { });
        await expect(new OssPublisher({ bucket: '', endpoint: 'e' }, { httpPut, credentials: () => creds }).publish(input)).rejects.toThrow(/bucket/i);
        await expect(new OssPublisher({ bucket: 'b', endpoint: '' }, { httpPut, credentials: () => creds }).publish(input)).rejects.toThrow(/endpoint/i);
        await expect(new OssPublisher({ bucket: 'b', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { httpPut, credentials: () => undefined }).publish(input)).rejects.toThrow(/credentials/i);
        expect(httpPut).not.toHaveBeenCalled();
    });
    it('rejects non-Aliyun OSS endpoints before signing credentials', async () => {
        const httpPut = vi.fn(async () => { });
        await expect(new OssPublisher({ bucket: 'b', endpoint: 'evil.example.com' }, { httpPut, credentials: () => creds }).publish(input)).rejects.toThrow(/aliyun oss endpoint/i);
        expect(httpPut).not.toHaveBeenCalled();
    });
    it('propagates upload failure', async () => {
        ...
        await expect(new OssPublisher({ bucket: 'b', endpoint: 'oss-cn-hangzhou.aliyuncs.com' }, { httpPut, credentials: () => creds, now: fixedNow }).publish(input)).rejects.toThrow(/403/);
    });
});
//# sourceMappingURL=oss-publisher.test.js.map