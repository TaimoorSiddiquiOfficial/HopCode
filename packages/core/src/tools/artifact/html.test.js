...
import { MAX_ARTIFACT_BYTES, sanitizeArtifactTitle, validateSelfContained, wrapArtifactHtml, byteLength, } from './html.js';
describe('artifact html helpers', () => {
    ...
    describe('validateSelfContained', () => {
        ...
        it('accepts external hyperlinks', () => {
            const ok = `<a href="https://github.com/QwenLM/hopcode/pull/1">PR</a>`;
            expect(validateSelfContained(ok)).toBeNull();
        });
    ...
    describe('wrapArtifactHtml', () => {
        ...
        it('uses the default title when none given', () => {
            expect(wrapArtifactHtml('<p>x</p>', undefined)).toContain('<title>Artifact</title>');
        });
    });
    describe('byteLength', () => {
