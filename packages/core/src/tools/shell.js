...
import { getShellAbortReasonKind, ShellExecutionService, } from '../services/shellExecutionService.js';
...
function findAttributableCommitSegment(command) {
    ...
    for (const sub of splitCommands(command)) {
        ...
        if (start < 0) {
            ...
            // when HOPCODE_DEBUG_LOG_FILE is set.
...
export class ShellToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback, canPromoteForegroundShell) {
        ...
        // HOPCODE_DEBUG_LOG_FILE entry the user has likely never set up.
    ...
    addAttributionToPR(command) {
        ...
        // skip in `HOPCODE_DEBUG_LOG_FILE`. Inline-body rewriting is the
...
- Try to maintain your current working directory throughout the session by using absolute paths and avoiding usage of \`cd\`. You may use \`cd\` if the User explicitly requests it.
  ...
  </bad-example>**Background vs Foreground Execution:**
