...
vi.mock('../services/shellExecutionService.js', () => ({
    ...
    getShellAbortReasonKind: (reason) => typeof reason === 'object' &&
        reason !== null &&
        'kind' in reason &&
...
describe('ShellTool', () => {
    ...
    describe('build', () => {
        ...
        it('should explain rejected intentional sleep comments', async () => {
            ...
            const overCapError = shellTool.validateToolParams({
                command: 'sleep 601s # intentional-sleep: wait for MCP rate limit reset',
                is_background: false,
            });
        ...
        it('should reject broad kill commands that can terminate hopcode', async () => {
            for (const command of [
                ...
                expect(() => shellTool.build({
                    command,
                    is_background: false,
                })).toThrow('Blocked: this command may terminate the running hopcode process');
            }
        });
        it('should allow targeted process kills', async () => {
            expect(shellTool.validateToolParams({
        ...
        it('should reject sibling-prefix directories outside the workspace', async () => {
            ...
            mockConfig.getWorkspaceContext.mockReturnValue(workspaceContext);
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/tmp/project-other',
                is_background: false,
            })).toThrow("Directory '/tmp/project-other' is not within any of the registered workspace directories.");
            expect(workspaceContext.isPathWithinWorkspace).toHaveBeenCalledWith('/tmp/project-other');
        });
        it('should throw an error for a directory within the user skills directory', async () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.hopcode/skills/my-skill',
                is_background: false,
        ...
        it('should throw an error for the user skills directory itself', async () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.hopcode/skills',
                is_background: false,
            })).toThrow('Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.');
        });
        it('should resolve directory path before checking user skills directory', async () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.hopcode/skills/../skills/my-skill',
                is_background: false,
            })).toThrow('Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.');
        });
    ...
    describe('execute', () => {
        ...
        describe('simulated sed edit', () => {
            ...
            const confirmSedEdit = async (invocation) => {
                const details = await invocation.getConfirmationDetails(mockAbortSignal);
            ...
            it('falls back to shell execution when sed has no prepared preview', async () => {
                ...
                await vi.waitFor(() => expect(mockShellExecutionService).toHaveBeenCalled());
            ...
            it('applies a qualifying sed -i command without spawning a shell after preview', async () => {
                ...
                expect(mockDebugLogger.debug).toHaveBeenCalledWith('executing simulated sed edit', { command: "sed -i 's/foo/bar/g' file.txt" });
                expect(mockFileHistoryService.trackEdit).toHaveBeenCalledWith(expectedSedFilePath);
            ...
            it.each([
                ...
                mockFileSystemService.writeTextFile.mockRejectedValue(Object.assign(new Error(code), { code }));
                const invocation = shellTool.build({
                    command: "sed -i 's/foo/bar/' file.txt",
                    directory: '/test/dir',
                    is_background: false,
                ...
                expect(mockShellExecutionService).not.toHaveBeenCalled();
                expect(result.error?.type).toBe(type);
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining('sed edit write failed after file history backup was recorded'));
            });
            it('continues applying a simulated sed edit when file history tracking fails', async () => {
                ...
                mockFileHistoryService.trackEdit.mockRejectedValue(new Error('backup failed'));
                const invocation = shellTool.build({
                    command: "sed -i 's/foo/bar/' file.txt",
                    directory: '/test/dir',
                    is_background: false,
                ...
                const result = await invocation.execute(mockAbortSignal);
                expect(mockShellExecutionService).not.toHaveBeenCalled();
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining('file history trackEdit failed for sed edit'));
                expect(mockFileSystemService.writeTextFile).toHaveBeenCalledWith({
                    path: expectedSedFilePath,
                    content: 'bar\n',
                    _meta: { bom: false, encoding: 'utf-8', lineEnding: 'lf' },
            ...
            it('logs non-fatal sed attribution and read-cache failures', async () => {
                ...
                vi.spyOn(CommitAttributionService.getInstance(), 'recordEdit').mockImplementation(() => {
                ...
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining('commit attribution recordEdit failed for sed edit'));
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining('file read cache recordWrite failed for sed edit'));
                expect(result.llmContent).toContain('sed edit applied');
            });
            it('applies confirmed inline modifications to a simulated sed edit', async () => {
                ...
                const recordEditSpy = vi.spyOn(CommitAttributionService.getInstance(), 'recordEdit');
                const invocation = shellTool.build({
                    command: "sed -i 's/foo/bar/' file.txt",
                    directory: '/test/dir',
                    is_background: false,
            ...
            it('does not write when sed execution is cancelled after reading', async () => {
                ...
                const details = await invocation.getConfirmationDetails(abortController.signal);
                expect(details.type).toBe('edit');
                if (details.type !== 'edit') {
                    throw new Error('expected edit confirmation');
                }
            ...
            it('awaits an in-flight sed write after cancellation starts', async () => {
                ...
                mockFileSystemService.writeTextFile.mockReturnValue(new Promise((resolve) => {
                ...
                await vi.waitFor(() => expect(mockFileSystemService.writeTextFile).toHaveBeenCalled());
                let settled = false;
            ...
            it('rejects simulated sed edits when the file was not read first', async () => {
                ...
                await expect(invocation.getConfirmationDetails(mockAbortSignal)).rejects.toMatchObject({
            ...
            it('reports timeout when a prepared sed edit times out before execution', async () => {
                ...
                try {
                    ...
                    expect(mockFileHistoryService.trackEdit).not.toHaveBeenCalled();
                    expect(mockFileSystemService.writeTextFile).not.toHaveBeenCalled();
                    expect(result.llmContent).toBe('Command timed out after 5000ms before it could complete. There was no output before it timed out.');
                    expect(result.returnDisplay).toBe('Command timed out after 5000ms before it could complete. There was no output before it timed out.');
            ...
            it('falls back to shell execution for background sed commands', async () => {
                ...
                expect(mockFileSystemService.readTextFile).not.toHaveBeenCalled();
                expect(mockFileSystemService.writeTextFile).not.toHaveBeenCalled();
                expect(mockShellExecutionService).toHaveBeenCalledWith("sed -i 's/foo/bar/' file.txt", '/test/dir', expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
            ...
            it('falls back to shell execution when sed preview cannot read the file', async () => {
                mockFileSystemService.readTextFile.mockRejectedValue(new Error('not text'));
                const invocation = shellTool.build({
                    command: "sed -i 's/foo/bar/' file.txt",
                    directory: '/test/dir',
                    is_background: false,
                ...
                expect(details.warnings).toContain('Sed edit preview unavailable; showing raw shell command confirmation.');
        ...
        describe('Silent-command heartbeat', () => {
            ...
            const heartbeats = () => updateOutputMock.mock.calls
                .map(([arg]) => arg)
                .filter((arg) => typeof arg === 'object' &&
                arg !== null &&
                arg.type === 'shell_progress');
            const settle = async () => {
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
                    output: '',
            ...
            it('is disabled by heartbeatIntervalMs: 0', async () => {
                mockConfig.getShellHeartbeatIntervalMs.mockReturnValue(0);
                const invocation = shellTool.build({
                    command: 'quiet-soak-test',
                    is_background: false,
                });
            ...
            it('honours a configured interval', async () => {
                mockConfig.getShellHeartbeatIntervalMs.mockReturnValue(5_000);
                const invocation = shellTool.build({
                    command: 'quiet-soak-test',
                    is_background: false,
                });
            ...
            it('stops on abort before the process settles', async () => {
                ...
                const promise = invocation.execute(abortController.signal, updateOutputMock);
                await vi.advanceTimersByTimeAsync(10_000);
                expect(heartbeats()).toHaveLength(1);
                abortController.abort();
                await vi.advanceTimersByTimeAsync(30_000);
        ...
        describe('Streaming to `updateOutput`', () => {
            ...
            it('should warn shortly before a foreground command times out', async () => {
                ...
                const promise = invocation.execute(mockAbortSignal, updateOutputMock, undefined, undefined, setPromoteAbortController, () => true);
                await Promise.resolve();
                ...
                expect(updateOutputMock).toHaveBeenCalledOnce();
                expect(updateOutputMock).toHaveBeenLastCalledWith(expect.stringContaining('about to time out'));
                expect(updateOutputMock).toHaveBeenLastCalledWith(expect.stringContaining('Ctrl+B'));
                mockShellOutputCallback({ type: 'data', chunk: 'still running' });
                await vi.advanceTimersByTimeAsync(OUTPUT_UPDATE_INTERVAL_MS + 1);
                expect(updateOutputMock).toHaveBeenCalledTimes(2);
                expect(updateOutputMock).toHaveBeenLastCalledWith(expect.stringContaining('still running'));
                expect(updateOutputMock).toHaveBeenLastCalledWith(expect.stringContaining('Ctrl+B'));
                resolveExecutionPromise({
            ...
            it('should append the foreground timeout warning to ANSI output', async () => {
                ...
                await vi.advanceTimersByTimeAsync(5_000);
                expect(updateOutputMock).toHaveBeenCalledTimes(2);
                const warningFrame = updateOutputMock.mock.calls.at(-1)?.[0];
            ...
            it('should not show the Ctrl+B warning when promotion is ambiguous', async () => {
                ...
                const promise = invocation.execute(mockAbortSignal, updateOutputMock, undefined, undefined, setPromoteAbortController, () => false);
                await Promise.resolve();
                expect(setPromoteAbortController).toHaveBeenCalledOnce();
                await vi.advanceTimersByTimeAsync(5_000);
                expect(updateOutputMock).not.toHaveBeenCalled();
            ...
            it('should suppress a stale Ctrl+B warning if promotion becomes ambiguous before the timer fires', async () => {
                ...
                const promise = invocation.execute(mockAbortSignal, updateOutputMock, undefined, undefined, setPromoteAbortController, () => canPromote);
                await Promise.resolve();
                expect(setPromoteAbortController).toHaveBeenCalledOnce();
                await vi.advanceTimersByTimeAsync(4_999);
                canPromote = false;
            ...
            it('schedules the warning against the real timeout clock', async () => {
                mockShellExecutionService.mockImplementationOnce((_cmd, _cwd, callback) => {
                    mockShellOutputCallback = callback;
            ...
            it('should not show the Ctrl+B warning outside interactive mode', async () => {
                const interactiveMock = mockConfig;
                ...
                const promise = invocation.execute(mockAbortSignal, updateOutputMock, undefined, undefined, vi.fn(), () => true);
                await Promise.resolve();
                await vi.advanceTimersByTimeAsync(5_000);
                expect(updateOutputMock).not.toHaveBeenCalled();
                resolveExecutionPromise({
            ...
            it('should not arm the near-timeout warning after an abort that happens before the shell handle is ready', async () => {
                ...
                const promise = invocation.execute(ac.signal, updateOutputMock, undefined, undefined, setPromoteAbortController, () => true);
                ac.abort();
                resolveShellHandle({
                    pid: 12345,
                    result: new Promise((resolve) => {
        ...
        describe('long-running foreground hint', () => {
            ...
            it('truncates shell output char-only so the line cap cannot undercut the char budget', async () => {
                ...
                try {
                    ...
                    expect(spy).toHaveBeenCalledWith(expect.anything(), ShellTool.Name, expect.any(String), expect.objectContaining({ lines: Number.POSITIVE_INFINITY }));
                }
                finally {
                    spy.mockRestore();
                }
        ...
        describe('addCoAuthorToGitCommit', () => {
            it('should add co-author to git commit with double quotes', async () => {
                ...
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: HopCode <hopcode@hopcode.com>'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should add Co-authored-by trailer to the LAST -m when multiple are present', async () => {
                ...
                expect(observed).toMatch(/-m\s+"Body line 1\s+Co-authored-by: HopCode <hopcode@hopcode\.com>"/s);
 ...
 spanning multiple lines"`;
        ...
        describe('addAttributionToPR', () => {
            ...
            // warning to surface the skip when HOPCODE_DEBUG_LOG_FILE is set.
            it.each([
            ...
            it('should append attribution to `gh pr new --body "..."` (alias form)', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Generated with HopCode'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should NOT match -b in earlier non-gh segments of a compound', async () => {
                ...
                expect(observed).toMatch(/gh pr create --title "x" --body "summary[\s\S]*Generated with HopCode"/);
            });
            ...
            it('should pick the OUTER --body when an inner -b appears in body text', async () => {
                ...
                expect(cmd).toMatch(/--body "docs mention -b 'flag' here[\s\S]*Generated with HopCode"/);
                expect(cmd).not.toMatch(/-b 'flag[\s\S]*Generated with HopCode[\s\S]*' here"/);
            });
            ...
            it('should target the LAST --body when gh pr create has multiple', async () => {
                ...
                const calls = mockShellExecutionService.mock.calls;
                const cmd = calls[calls.length - 1]?.[0];
                expect(cmd).toMatch(/--body "ignored" --body "real summary[\s\S]*Generated with HopCode/);
                // The trailer must NOT be inside the first --body.
                expect(cmd).not.toMatch(/--body "ignored[\s\S]*Generated with HopCode[\s\S]*" --body/);
            });
            ...
            it('should NOT rewrite when gh pr create appears only inside quoted text', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.not.stringContaining('Generated with HopCode'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it("should match the full body across '\\\\'' apostrophe escapes", async () => {
                ...
                // middle of it.
                expect(observed).toMatch(/don'\\''t break me[\s\S]*Generated with HopCode/);
            });
        });
    ...
    describe('timeout parameter', () => {
        ...
        it('keeps the first timeout after a later user cancellation', async () => {
            ...
            expect(result.returnDisplay).toContain('Command timed out after 5000ms before it could complete.');
        ...
        it('returns a structured timeout when the command produced no output', async () => {
            ...
            try {
                ...
                const result = await promise;
                expect(result.llmContent).toContain('There was no output before it timed out.');
                expect(result.returnDisplay).toContain('There was no output before it timed out.');
                expect(result.error?.type).toBe(ToolErrorType.EXECUTION_TIMEOUT);
            }
            finally {
                vi.stubGlobal('AbortSignal', originalAbortSignal);
...
describe('detectBlockedSleepPattern', () => {
    ...
    it('allows standalone sleep with an intentional sleep comment', () => {
        expect(detectBlockedSleepPattern('sleep 5 # intentional-sleep: wait for MCP rate limit reset')).toBeNull();
        expect(detectBlockedSleepPattern('sleep 2s # intentional-sleep: deliberate rate limit backoff')).toBeNull();
        expect(detectBlockedSleepPattern('sleep 10m # intentional-sleep: wait for MCP rate limit reset')).toBeNull();
    });
    it('requires a meaningful intentional sleep reason', () => {
        expect(detectBlockedSleepPattern('sleep 5 # intentional-sleep:')).toBe('standalone sleep 5');
        expect(detectBlockedSleepPattern('sleep 5 # intentional-sleep: wait')).toBe('standalone sleep 5');
    ...
    it('blocks intentional sleep comments above the duration cap', () => {
        expect(detectBlockedSleepPattern('sleep 601s # intentional-sleep: wait for MCP rate limit reset')).toBe('standalone sleep 601s');
    });
    it('does not allow intentional sleep comments on leading sleep chains', () => {
        expect(detectBlockedSleepPattern('sleep 5 && echo ok # intentional-sleep: wait for rate limit reset')).toBe('sleep 5 followed by: echo ok');
    });
    it('does not allow intentional sleep comments to hide newline commands', () => {
        expect(detectBlockedSleepPattern('sleep 5 # intentional-sleep: wait for rate limit reset\necho ok')).toBe('sleep 5 followed by: echo ok');
    });
