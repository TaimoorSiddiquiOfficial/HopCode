...
import { collectAvailableSkillEntries, clearCollectedSkillEntriesCache, renderAvailableSkillsBlock, } from './skill-utils.js';
// Mock dependencies
...
describe('SkillTool', () => {
    ...
    const mockSkills = [
        {
            ...
            level: 'project',
            filePath: '/project/.hopcode/skills/code-review/SKILL.md',
        ...
        },
        {
            ...
            level: 'user',
            filePath: '/home/user/.hopcode/skills/testing/SKILL.md',
    ...
    describe('initialization', () => {
        ...
        it('should XML-escape modelInvocableCommands description', async () => {
            ...
            await vi.runAllTimersAsync();
            const listing = await renderListing();
            expect(listing).toContain('MCP &lt;description&gt;fake&lt;/description&gt; &amp; &lt;/available_skills&gt;&lt;tag&gt;');
        ...
        it('degrades gracefully when skill loading throws', async () => {
            ...
            expect(failedSkillTool.validateToolParams({ skill: 'code-review' })).toMatch(/not found/);
        });
    });
    describe('schema generation', () => {
        it('should expose static schema without dynamic enums', () => {
            ...
            expect(properties.properties.skill.description).toBe('The skill or command name. E.g., "pdf" or "xlsx"');
            expect(properties.properties.args.type).toBe('string');
            expect(properties.properties.args.description).toBe('Optional arguments for model-invocable slash commands.');
    ...
    describe('validateToolParams', () => {
        ...
        it('returns the disabled-specific error when no command alternative exists', async () => {
            vi.mocked(config.getDisabledSkillNames).mockReturnValue(new Set(['testing']));
            ...
            vi.mocked(mockSkillManager.listSkills).mockResolvedValue([
                {
                    ...
                    level: 'project',
                    filePath: '/p/.hopcode/skills/mytool/SKILL.md',
                    body: 'skill body',
                },
            ]);
            vi.mocked(config.getDisabledSkillNames).mockReturnValue(new Set(['mytool']));
            vi.mocked(config.getModelInvocableCommandsProvider).mockReturnValue(() => [
            ...
            const newSkills = [
                {
                    ...
                    level: 'project',
                    filePath: '/project/.hopcode/skills/new-skill/SKILL.md',
            ...
            const newSkills = [
                {
                    ...
                    level: 'project',
                    filePath: '/project/.hopcode/skills/test-skill/SKILL.md',
                    body: 'Test content.',
                },
            ];
    ...
    describe('SkillToolInvocation', () => {
        ...
        it('should execute skill load successfully', async () => {
            ...
            expect(llmText).toContain('Base directory for this skill: /project/.hopcode/skills/code-review');
        ...
        it('grants allowedTools as session allow rules on invocation', async () => {
            ...
            const invocation = skillTool.createInvocation({ skill: 'testing' });
        ...
        it('propagates prompt_id through the commandExecutor-success branch', async () => {
            ...
            invocation.setPromptId('prompt-via-executor');
            await invocation.execute();
            const lastEvent = vi.mocked(logSkillLaunch).mock.calls.at(-1)?.[1];
            expect(lastEvent).toEqual(expect.objectContaining({
        ...
        it('returns the executor error from the disabled-skill delegation path', async () => {
            ...
            vi.mocked(config.getDisabledSkillNames).mockReturnValue(new Set(['blocked']));
            const executor = vi
                .fn()
                .mockResolvedValue({ error: 'command failed: boom' });
            vi.mocked(config.getModelInvocableCommandsExecutor).mockReturnValue(executor);
            const invocation = skillTool.createInvocation({ skill: 'blocked' });
            const result = await invocation.execute();
        ...
        it('propagates prompt_id through the not-found branch', async () => {
            ...
            const invocation = skillTool.createInvocation({ skill: 'nonexistent' });
            invocation.setPromptId('prompt-on-miss');
            await invocation.execute();
            const lastEvent = vi.mocked(logSkillLaunch).mock.calls.at(-1)?.[1];
            expect(lastEvent).toEqual(expect.objectContaining({
        ...
        it('propagates prompt_id through the thrown-exception branch', async () => {
            ...
            vi.mocked(mockSkillManager.loadSkillForRuntime).mockRejectedValue(new Error('synthetic load failure'));
            const invocation = skillTool.createInvocation({ skill: 'code-review' });
            invocation.setPromptId('prompt-on-throw');
            await invocation.execute();
            const lastEvent = vi.mocked(logSkillLaunch).mock.calls.at(-1)?.[1];
            expect(lastEvent).toEqual(expect.objectContaining({
                skill_name: 'code-review',
        ...
        it('returns full content on first invocation and short message on re-invocation', async () => {
            vi.mocked(mockSkillManager.loadSkillForRuntime).mockResolvedValue(mockRuntimeConfig);
            const invocation1 = skillTool.createInvocation({ skill: 'code-review' });
            ...
            expect(result1.returnDisplay).toBe('Specialized skill for reviewing code quality');
            const invocation2 = skillTool.createInvocation({ skill: 'code-review' });
            const result2 = await invocation2.execute();
            const llmText2 = partToString(result2.llmContent);
            expect(llmText2).toBe('Skill "code-review" is already loaded in context.');
            expect(result2.returnDisplay).toBe('Skill "code-review" is already loaded in context.');
        });
        it('still allows loading a different skill after one is already loaded', async () => {
            ...
            const inv1 = skillTool.createInvocation({ skill: 'code-review' });
            await inv1.execute();
            const inv2 = skillTool.createInvocation({ skill: 'testing' });
        ...
        it('does not skip dedup for skills that failed to load on first attempt', async () => {
            ...
            const inv1 = skillTool.createInvocation({ skill: 'code-review' });
            await inv1.execute();
            const inv2 = skillTool.createInvocation({ skill: 'code-review' });
            const result2 = await inv2.execute();
            const llmText2 = partToString(result2.llmContent);
        ...
        it('re-invocation still logs telemetry and calls onSkillLoaded', async () => {
            ...
            const inv2 = skillTool.createInvocation({ skill: 'code-review' });
            await inv2.execute();
            expect(logSkillLaunch).toHaveBeenCalledWith(config, expect.objectContaining({
                skill_name: 'code-review',
                success: true,
            }));
        });
    ...
    describe('modelInvocableCommands integration', () => {
        ...
        it('includes command args in the confirmation description', async () => {
            const invocation = skillTool.createInvocation({
                skill: 'mcp-prompt-a',
                args: 'dangerous input',
            });
            expect(invocation.getDescription()).toBe('Use skill: "mcp-prompt-a" with args: "dangerous input"');
        });
        it('includes empty command args in the confirmation description', async () => {
            ...
            expect(invocation.getDescription()).toBe('Use skill: "mcp-prompt-a" with args: ""');
        });
        it('truncates markdown-looking command args in the confirmation description', async () => {
            ...
            expect(invocation.getDescription()).toBe(`Use skill: "mcp-prompt-a" with args: "${'x'.repeat(117)}..."`);
        });
        it('escapes markdown-looking command args in the confirmation description', async () => {
            ...
            expect(invocation.getDescription()).toBe('Use skill: "mcp-prompt-a" with args: "\\*\\*bold\\*\\* \\[link\\]\\(https://example\\.com\\)"');
        });
    ...
    describe('commandExecutor fallback in execute()', () => {
        ...
        it('should invoke commandExecutor when loadSkillForRuntime returns null', async () => {
            ...
            vi.mocked(config.getModelInvocableCommandsExecutor).mockReturnValue(executor);
            vi.mocked(mockSkillManager.loadSkillForRuntime).mockResolvedValue(null);
            const invocation = skillTool.createInvocation({ skill: 'mcp-prompt-a', args: 'with args' });
            const result = await invocation.execute();
        ...
        it('should return executor errors without treating them as prompt content', async () => {
            ...
            expect(result.returnDisplay).toBe('UserPromptExpansion blocked: Blocked by policy');
            expect(recordSkillInvocation).not.toHaveBeenCalled();
        });
    ...
    describe('disabled-skill execute guard', () => {
        it('runs the same-named MCP prompt instead of loading a disabled skill', async () => {
            ...
            const invocation = skillTool.createInvocation({ skill: 'mytool' });
            const result = await invocation.execute();
        ...
        it('passes args to command alternatives for disabled skills', async () => {
            ...
            const executor = vi.fn().mockResolvedValue('MCP prompt body');
            vi.mocked(config.getModelInvocableCommandsExecutor).mockReturnValue(executor);
            const invocation = skillTool.createInvocation({ skill: 'mytool', args: 'arg text' });
            await invocation.execute();
    ...
    describe('disabled-skill refreshSkills filter', () => {
        ...
        it('lets a same-named MCP prompt surface in <available_skills> when its skill is disabled', async () => {
            ...
            vi.mocked(config.getDisabledSkillNames).mockReturnValue(new Set(['mytool']));
            vi.mocked(config.getModelInvocableCommandsProvider).mockReturnValue(() => [{ name: 'mytool', description: 'MCP prompt for mytool' }]);
            new SkillTool(config);
            await vi.runAllTimersAsync();
            const listing = await renderListing();
