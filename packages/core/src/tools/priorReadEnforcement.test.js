...
describe('checkPriorRead', () => {
    ...
    it('rejects when file disappears after read (post-read ENOENT)', async () => {
        vi.spyOn(fs.promises, 'stat').mockRejectedValue(Object.assign(new Error('ENOENT'), { code: 'ENOENT' }));
        const result = await checkPriorRead(cache, '/tmp/disappeared.txt', 'editing', { expectExisting: true });
    ...
    it('rejects when stat fails with EACCES (fail-closed)', async () => {
        vi.spyOn(fs.promises, 'stat').mockRejectedValue(Object.assign(new Error('EACCES'), { code: 'EACCES' }));
    ...
    it('rejects when stat fails with EBUSY (fail-closed)', async () => {
        vi.spyOn(fs.promises, 'stat').mockRejectedValue(Object.assign(new Error('EBUSY'), { code: 'EBUSY' }));
    ...
    it('rejects when stat fails with unknown error code (fail-closed)', async () => {
        vi.spyOn(fs.promises, 'stat').mockRejectedValue(Object.assign(new Error('unknown'), {}));
    ...
    it('rejects directories with TARGET_IS_DIRECTORY', async () => {
        vi.spyOn(fs.promises, 'stat').mockResolvedValue(mockStats({ isDirectory: () => true, isFile: () => false }));
    ...
    it('rejects FIFO with EDIT_REQUIRES_PRIOR_READ', async () => {
        vi.spyOn(fs.promises, 'stat').mockResolvedValue(mockStats({ isFile: () => false, isFIFO: () => true }));
    ...
    it('rejects socket with EDIT_REQUIRES_PRIOR_READ', async () => {
        vi.spyOn(fs.promises, 'stat').mockResolvedValue(mockStats({ isFile: () => false, isSocket: () => true }));
    ...
    it('rejects when cache state is unknown (never read)', async () => {
        ...
        if (!result.ok) {
            expect(result.type).toBe(ToolErrorType.EDIT_REQUIRES_PRIOR_READ);
            expect(result.rawMessage).toContain('has not been read');
            expect(result.rawMessage).toContain('a partial read with offset / limit is fine');
        }
    });
    ...
    it('uses "overwriting" in EACCES displayMessage for overwriting verb', async () => {
        vi.spyOn(fs.promises, 'stat').mockRejectedValue(Object.assign(new Error('EACCES'), { code: 'EACCES' }));
        const result = await checkPriorRead(cache, '/tmp/restricted', 'overwriting');
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.displayMessage).toContain('overwriting');
        }
...
describe('StructuredToolError', () => {
    it('has the correct name', () => {
        const err = new StructuredToolError('test message', ToolErrorType.EDIT_REQUIRES_PRIOR_READ);
    ...
    it('carries errorType for scheduler dispatch', () => {
        const err = new StructuredToolError('file changed', ToolErrorType.FILE_CHANGED_SINCE_READ);
        expect(err.errorType).toBe(ToolErrorType.FILE_CHANGED_SINCE_READ);
    });
});
//# sourceMappingURL=priorReadEnforcement.test.js.map