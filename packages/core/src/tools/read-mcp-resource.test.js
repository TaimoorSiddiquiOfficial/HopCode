...
import { MAX_MCP_RESOURCE_BLOB_CHARS, MAX_MCP_RESOURCE_TEXT_CHARS, } from './mcp-resource-content.js';
const inlineCount = (llmContent) => Array.isArray(llmContent)
    ? llmContent.filter((p) => 'inlineData' in p).length
    : 0; // empty read → llmContent is the diagnostic string, not parts
function configWith(readMcpResource, opts = {}) {
    return {
...
describe('ReadMcpResourceTool', () => {
    it('reads an MCP resource and returns framed content parts', async () => {
        ...
        expect(readMcpResource).toHaveBeenCalledWith('asys-mcp-http', 'asight://skills/analyze_interconnect_desync.md', { signal });
        ...
        expect(texts.join('')).toContain('--- Content from MCP resource asys-mcp-http:asight://skills/analyze_interconnect_desync.md [');
        // returnDisplay mirrors the summary so a success card never hides what was
        // actually injected ('resource body' is 13 chars).
        expect(result.returnDisplay).toBe('Read resource asys-mcp-http:asight://skills/analyze_interconnect_desync.md — Injected 13 chars');
    });
    ...
    it('reports no readable content when the resource has none', async () => {
        ...
        expect(result.returnDisplay).toBe('Read resource asys-mcp-http:asight://empty — (no readable content)');
    });
    it('asks for confirmation by default (untrusted server)', async () => {
        const tool = new ReadMcpResourceTool(configWith(vi.fn(), { mcpServers: { srv: {} }, trustedFolder: true }));
    ...
    it('surfaces a server-scoped ReadMcpResource rule in the confirmation', async () => {
        const tool = new ReadMcpResourceTool(configWith(vi.fn(), { mcpServers: { srv: {} } }));
        const inv = tool.build({ server_name: 'srv', uri: 'x://a' });
    ...
    it('exposes the read target to the AUTO-mode classifier', () => {
        const tool = new ReadMcpResourceTool(configWith(vi.fn()));
        expect(tool.toAutoClassifierInput({ server_name: 'srv', uri: 'x://a' })).toEqual({ server_name: 'srv', uri: 'x://a' });
    });
    ...
    it.each([
        ...
        await expect(invocation.execute(new AbortController().signal)).rejects.toThrow(message);
    });
});
