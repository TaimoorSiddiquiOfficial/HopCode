...
import { z } from 'zod';
import { ClientMcpRegistrar, } from './client-mcp-registrar.js';
...
function buildCannedServer(sink) {
    const server = new McpServer({
        name: 'chrome-tools',
        version: '0.0.1',
    });
    server.tool('chrome_read_page', 'Read the current page text', { selector: z.string().optional() }, async ({ selector }) => ({
        content: [
            {
                type: 'text',
...
describe('ClientMcpRegistrar reverse channel', () => {
    it('round-trips initialize / tools/list / tools/call through SdkControlClientTransport', async () => {
        ...
        const jsonrpcIdToFrameId = new Map();
        // Built before the registrar so its sendFrame can target it directly.
        const canned = buildCannedServer((message) => {
            ...
            const frameId = replyId !== undefined ? jsonrpcIdToFrameId.get(replyId) : undefined;
    ...
    it('rejects sends for unregistered servers', async () => {
        ...
        await expect(registrar.sendSdkMcpMessage('ghost', {
            jsonrpc: '2.0',
            id: 1,
            method: 'tools/list',
    ...
    it('times out a silent client', async () => {
        ...
        await expect(registrar.sendSdkMcpMessage('slow', {
            jsonrpc: '2.0',
            id: 1,
            method: 'tools/list',
...
//# sourceMappingURL=client-mcp-registrar.test.js.map