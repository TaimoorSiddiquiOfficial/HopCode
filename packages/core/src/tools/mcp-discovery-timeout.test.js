...
describe('discoveryTimeoutFor', () => {
    ...
    it('httpUrl config defaults to 5s (remote)', () => {
        expect(discoveryTimeoutFor(mkCfg({ httpUrl: 'https://api.x' }))).toBe(5_000);
    });
    ...
    it('honors per-server discoveryTimeoutMs override', () => {
        expect(discoveryTimeoutFor(mkCfg({ command: 'node', discoveryTimeoutMs: 12_345 }))).toBe(12_345);
    });
    it('clamps override above MAX (300s)', () => {
        expect(discoveryTimeoutFor(mkCfg({ command: 'node', discoveryTimeoutMs: 600_000 }))).toBe(300_000);
    });
    it('clamps override below MIN (100ms)', () => {
        expect(discoveryTimeoutFor(mkCfg({ command: 'node', discoveryTimeoutMs: 50 }))).toBe(100);
    });
    it('falls through to default when override is NaN', () => {
        expect(discoveryTimeoutFor(mkCfg({ command: 'node', discoveryTimeoutMs: Number.NaN }))).toBe(30_000);
    });
    it('falls through to default when override is Infinity', () => {
        expect(discoveryTimeoutFor(mkCfg({
            command: 'node',
...
describe('runWithTimeout', () => {
    ...
    it('rejects with timeout error when timer fires before task', async () => {
        ...
        expect(err.message).toMatch(/Timed out after 100ms: test-label/);
    ...
    it('propagates task rejection (clears timer)', async () => {
        ...
        await expect(runWithTimeout(task, 1_000, 'test')).rejects.toThrow('task failed');
    });
...
//# sourceMappingURL=mcp-discovery-timeout.test.js.map