...
function mkPoolOptions(overrides = {}) {
    return {
        workspaceContext: {},
        debugMode: false,
...
describe('McpTransportPool', () => {
    ...
    describe('acquire / release lifecycle', () => {
        it('3 sessions acquiring same key share 1 entry (1 connect call)', async () => {
            ...
            const c1 = await pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            const r2 = mkSessionRegistries();
            const c2 = await pool.acquire('srv', cfg, 's2', r2.tools, r2.prompts, r2.resources);
            const r3 = mkSessionRegistries();
            const c3 = await pool.acquire('srv', cfg, 's3', r3.tools, r3.prompts, r3.resources);
        ...
        it('resources discovered via the pool reach the session resource registry', async () => {
            ...
            expect(r.resources.registerResource).toHaveBeenCalledWith(expect.objectContaining({ uri: 'file:///doc.txt', serverName: 'srv' }));
        });
        it('different env between two sessions creates 2 distinct entries (credential isolation)', async () => {
            const mocked = mockMcpSuccess();
            const cfgA = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://api.x', { Authorization: 'tokenA' });
            const cfgB = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://api.x', { Authorization: 'tokenB' });
            const r1 = mkSessionRegistries();
            const r2 = mkSessionRegistries();
            ...
            const pool2 = new McpTransportPool(cliConfig, mkPoolOptions({
            ...
            const cA = await pool2.acquire('srv', cfgA, 's1', r1.tools, r1.prompts, r1.resources);
            const cB = await pool2.acquire('srv', cfgB, 's2', r2.tools, r2.prompts, r2.resources);
        ...
        it('release brings refs to 0 → starts drain timer; new acquire within drain cancels', async () => {
            ...
            pool.release(`srv::${'a'.repeat(16)}`, 'unknown'); // unknown id no-op
        ...
        it('release brings refs to 0 + drain timer expires → entry closed', async () => {
            mockMcpSuccess();
            const pool = new McpTransportPool(cliConfig, mkPoolOptions({ drainDelayMs: 100 }));
            const cfg = new MCPServerConfig('node');
            const r1 = mkSessionRegistries();
            await pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            pool.releaseSession('s1');
        ...
        it('applies session-level includeTools/excludeTools to unpooled tools (W81/W87)', async () => {
            ...
            const cfg = new MCPServerConfig('node', undefined, // args
            undefined, // env
            undefined, // cwd
            undefined, // url
            undefined, // httpUrl
            ...
            const r = mkSessionRegistries();
            await pool.acquire('srv', cfg, 's1', r.tools, r.prompts, r.resources);
            const registerTool = r.tools.registerTool;
            ...
            const registeredNames = registerTool.mock.calls.map((args) => args[0].name);
        ...
        it('cancels in-flight unpooled acquire when releaseSession races the connect/discover window (W77)', async () => {
            ...
            const cfg = new MCPServerConfig('node');
            const r = mkSessionRegistries();
            const acquirePromise = pool.acquire('srv', cfg, 's1', r.tools, r.prompts, r.resources);
            ...
            releaseConnect();
            await expect(acquirePromise).rejects.toThrow(/draining or unpooled.*was cancelled/);
            ...
            expect(r.tools.removeMcpToolsByServer).toHaveBeenCalledWith('srv');
        });
    });
    describe('pooled in-flight acquire (W90)', () => {
        it('transitions zombie entry to failed when transport silently drops, evicting from pool (W120)', async () => {
            ...
            const { updateMCPServerStatus, MCPServerStatus } = await import('./mcp-client.js');
            mockMcpSuccess({ toolNames: ['t1'] });
            const pool = new McpTransportPool(cliConfig, mkPoolOptions());
            const cfg = new MCPServerConfig('node');
            const r1 = mkSessionRegistries();
            const conn1 = await pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            ...
            const r2 = mkSessionRegistries();
            const conn2 = await pool.acquire('srv', cfg, 's2', r2.tools, r2.prompts, r2.resources);
            ...
            const entriesAfter = pool.entries;
        ...
        it('catches silent transport drop during drain window (W131)', async () => {
            ...
            const { updateMCPServerStatus, MCPServerStatus } = await import('./mcp-client.js');
            mockMcpSuccess({ toolNames: ['t1'] });
            const pool = new McpTransportPool(cliConfig, mkPoolOptions({ drainDelayMs: 1_000 }));
            const cfg = new MCPServerConfig('node');
            const r1 = mkSessionRegistries();
            await pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            ...
            const r2 = mkSessionRegistries();
            const conn2 = await pool.acquire('srv', cfg, 's-listen', r2.tools, r2.prompts, r2.resources);
            conn2.on('event', (e) => {
                if (e.kind === 'failed')
                    failedEventReceived = true;
            });
        ...
        it('PoolEntry.attach rejects on terminal-state entry (W90 contract — direct probe; W125 made the pool fast-path self-heal so we exercise the guard at the entry level)', async () => {
            ...
            const view = new SessionMcpView(mkSessionRegistries().tools, mkSessionRegistries().prompts, mkSessionRegistries().resources, 's2', 'srv', cfg);
            expect(() => entry.attach('s2', view)).toThrow(/Cannot attach to PoolEntry/);
        });
        ...
        it('rolls back early reverse-index insertion when spawnInFlight rejects', async () => {
            ...
            const r1 = mkSessionRegistries();
            const r2 = mkSessionRegistries();
            const first = pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            // Yield so 's1' enters spawnInFlight; 's2' joins via in-flight.
            await Promise.resolve();
            const second = pool.acquire('srv', cfg, 's2', r2.tools, r2.prompts, r2.resources);
            ...
            const r3 = mkSessionRegistries();
            const c = await pool.acquire('srv', cfg, 's3', r3.tools, r3.prompts, r3.resources);
        ...
        it('falls back to synthetic-only marker when no upstream onerror was captured (W133-a fallback)', async () => {
            ...
            expect(failedEvent?.lastError).toBe('transport disconnected (silent transport drop)');
        });
        it('emits structured warn log when silent-drop sweep throws on pid discovery (W134)', async () => {
            ...
            const r1 = mkSessionRegistries();
            await pool.acquire('srv', cfg, 's1', r1.tools, r1.prompts, r1.resources);
            mocked.onerror?.(new Error('upstream EPIPE'));
            ...
            await vi.waitFor(() => {
                const calls = debugMock.warn.mock.calls;
                const obs = calls.find((c) => typeof c[0] === 'string' &&
            ...
            const warnCalls = debugMock.warn.mock.calls;
            const obsCall = warnCalls.find((c) => typeof c[0] === 'string' &&
                c[0].includes('silent-drop sweep observability'));
        ...
        it('emits structured warn log when silent-drop sweep partially signals descendants (W134 partial-signal)', async () => {
            ...
            const { listDescendantPids, sigtermPids } = await import('./pid-descendants.js');
            const { createDebugLogger } = await import('../utils/debugLogger.js');
            const debugMock = createDebugLogger('McpPool:Entry');
            // The stub is a singleton across tests; clear prior call history
            // so we only observe THIS test's warn invocations.
    ...
    describe('spawnInFlight dedupe', () => {
        it('5 concurrent acquires for same key → 1 spawn', async () => {
            ...
            const results = await Promise.all(regs.map((r, i) => pool.acquire('srv', cfg, `s${i}`, r.tools, r.prompts, r.resources)));
            expect(mocked.connect).toHaveBeenCalledTimes(1);
    ...
    describe('restartByName (§13)', () => {
        ...
        it('restartByName with entryIndex filters to a single entry', async () => {
            mockMcpSuccess();
            const pool = new McpTransportPool(cliConfig, mkPoolOptions({
                pooledTransports: new Set(['stdio', 'http']),
            }));
            const cfgA = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x', { Authorization: 'A' });
            const cfgB = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, 'https://x', { Authorization: 'B' });
        ...
        it('restart fans out updated tool snapshot to attached subscribers (F2 commit 5 R3 / W40)', async () => {
            ...
            const initialRemoveCalls = r.tools.removeMcpToolsByServer.mock.calls.length;
            const initialResourceRemoveCalls = r.resources.removeResourcesByServer.mock.calls.length;
            ...
            const finalRemoveCalls = r.tools.removeMcpToolsByServer.mock.calls.length;
            const finalResourceRemoveCalls = r.resources.removeResourcesByServer.mock.calls.length;
            expect(finalRemoveCalls).toBeGreaterThan(initialRemoveCalls);
            expect(finalResourceRemoveCalls).toBeGreaterThan(initialResourceRemoveCalls);
        });
        it('a transiently-empty resources/list on restart keeps the snapshot for new sessions', async () => {
            ...
            expect(r2.resources.registerResource).toHaveBeenCalledWith(expect.objectContaining({ uri: 'res://keep', serverName: 'srv' }));
        });
    });
    ...
    describe('workspace budget integration (F2 commit 6)', () => {
        it('refuses acquire past cap under enforce mode and records the refusal', async () => {
            ...
            await expect(pool.acquire('srvC', cfgC, 's1', r.tools, r.prompts, r.resources)).rejects.toThrow(/budget exhausted/i);
        ...
        it('releases the slot when the only entry for a name closes', async () => {
            ...
            const pool = new McpTransportPool(cliConfig, mkPoolOptions({ budget, drainDelayMs: 1 }));
            const r = mkSessionRegistries();
            const cfg = new MCPServerConfig('node');
            const conn = await pool.acquire('srvA', cfg, 's1', r.tools, r.prompts, r.resources);
        ...
        it('preserves slot when entry closes during a same-name in-flight spawn (R1 race fix)', async () => {
            ...
            const connA = await pool.acquire('srvA', cfgA, 's1', r.tools, r.prompts, r.resources);
            ...
            const acquireB = pool.acquire('srvA', cfgB, 's1', r.tools, r.prompts, r.resources);
        ...
        it("does NOT phantom-release when 'already_held' spawn fails (R24 T17)", async () => {
            ...
            await pool.acquire('srvA', cfgA, 's1', r1.tools, r1.prompts, r1.resources);
            expect(budget.getReservedSlots()).toEqual(['srvA']);
            // B: same name, different fingerprint → tryReserve →
            // 'already_held'. Spawn throws.
            const releaseSpy = vi.spyOn(budget, 'release');
            const r2 = mkSessionRegistries();
            await expect(pool.acquire('srvA', cfgB, 's2', r2.tools, r2.prompts, r2.resources)).rejects.toThrow(/B spawn boom/);
        ...
        it('rolls back the slot reservation on spawn failure', async () => {
            ...
            vi.mocked(ClientLib.Client).mockReturnValue(failingClient);
            vi.spyOn(SdkClientStdioLib, 'StdioClientTransport').mockReturnValue({
                close: vi.fn().mockResolvedValue(undefined),
            });
            const { WorkspaceMcpBudget } = await import('./mcp-workspace-budget.js');
            ...
            const r = mkSessionRegistries();
            const cfg = new MCPServerConfig('node');
            await expect(pool.acquire('srvA', cfg, 's1', r.tools, r.prompts, r.resources)).rejects.toThrow();
...
//# sourceMappingURL=mcp-transport-pool.test.js.map