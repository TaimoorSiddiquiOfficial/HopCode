...
describe('GrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('normalizes CRLF fallback grep output without dropping result paths', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput('crlf.txt:1:hello world\r\n', tempRootDir);
        ...
        it('parses plain grep output for paths containing colons', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`${filePath}:1:hello: world\n`, tempRootDir);
            expect(matches[0]).toMatchObject({
        ...
        it('parses git grep -z output for paths containing colons', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`${filePath}\0${12}\0hello: world\n`, tempRootDir);
            expect(matches[0]).toMatchObject({
                absoluteFilePath: path.join(tempRootDir, filePath),
                filePath,
                line: 'hello: world',
        ...
        it('parses multiple git grep -z matches', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`first.txt\0${1}\0hello world\nsecond.txt\0${2}\0world again\n`, tempRootDir);
        ...
        it('parses git grep -z output without a trailing newline', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`tail.txt\0${3}\0world at eof`, tempRootDir);
        ...
        it('skips unframed binary notices in git grep -z output', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`Binary file binary.bin matches\n${filePath}\0${7}\0hello world\n`, tempRootDir);
            expect(matches).toHaveLength(1);
            expect(matches[0]).toMatchObject({
                absoluteFilePath: path.join(tempRootDir, filePath),
                filePath,
        ...
        it('parses system grep --null output for paths containing colons', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`${filePath}\0${12}:hello: world\n`, tempRootDir);
            expect(matches[0]).toMatchObject({
                absoluteFilePath: path.join(tempRootDir, filePath),
                filePath,
                line: 'hello: world',
        ...
        it('parses multiple system grep --null matches', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`first.txt\0${1}:hello world\nsecond.txt\0${2}:world again\n`, tempRootDir);
            expect(matches).toHaveLength(2);
            expect(matches[0]).toMatchObject({
                absoluteFilePath: path.join(tempRootDir, 'first.txt'),
                filePath: 'first.txt',
        ...
        it('parses system grep --null output without a trailing newline', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`tail.txt\0${3}:world at eof`, tempRootDir);
            expect(matches).toHaveLength(1);
            expect(matches[0]).toMatchObject({
                absoluteFilePath: path.join(tempRootDir, 'tail.txt'),
                filePath: 'tail.txt',
        ...
        it('skips malformed system grep --null records and keeps following matches', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`broken.txt\0missing-separator\nvalid.txt\0${4}:world after malformed\n`, tempRootDir);
            expect(matches).toHaveLength(1);
            expect(matches[0]).toMatchObject({
    ...
    describe('getDescription', () => {
        ...
        it('should generate correct description with pattern and glob', () => {
            ...
            const invocation = grepTool.build(params);
            expect(invocation.getDescription()).toBe("'testPattern' in . (filter: '*.ts')");
        });
        ...
        it('should keep paths outside the project absolute (never project-relative)', () => {
            ...
            const params = { pattern: 'testPattern', path: outside };
            const invocation = grepTool.build(params);
