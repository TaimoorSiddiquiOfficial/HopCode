...
import { MAX_MCP_RESOURCE_BLOB_CHARS, MAX_MCP_RESOURCE_TEXT_CHARS, formatMcpResourceContents, summarizeMcpResource, } from './mcp-resource-content.js';
const textOf = (parts) => parts.map((p) => p.text ?? '').join('');
const noncesOf = (joined) => [...joined.matchAll(/\[([0-9a-f]{8})\]/g)].map((m) => m[1]);
describe('formatMcpResourceContents', () => {
    it('frames text content with nonce-attributed delimiters', () => {
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://a', text: 'hello' }] }, 'srv:x://a');
    ...
    it('caps text at MAX_MCP_RESOURCE_TEXT_CHARS and flags truncation', () => {
        ...
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://a', text: big }] }, 'srv:x://a');
    ...
    it('does not flag truncation when text is exactly at the cap', () => {
        ...
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://a', text: exact }] }, 'srv');
        expect(out.truncated).toBe(false);
        expect(out.textChars).toBe(MAX_MCP_RESOURCE_TEXT_CHARS);
    ...
    it('skips blobs once the cumulative cap is exceeded', () => {
        ...
        expect(out.truncated).toBe(true);
        const inline = out.parts.filter((p) => 'inlineData' in p);
    ...
    it('defaults to application/octet-stream when a blob has no mimeType', () => {
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://b', blob: 'aGk=' }] }, 'srv');
        const inline = out.parts.find((p) => 'inlineData' in p);
    ...
    it('silently skips content items that are neither text nor blob', () => {
        ...
        // against.
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://link' }] }, 'srv');
        expect(out.parts).toEqual([]);
        expect(out.truncated).toBe(false);
        expect(summarizeMcpResource(out)).toBe('(no readable content)');
    });
    ...
    it('reports blobChars for injected blobs', () => {
        const out = formatMcpResourceContents({ contents: [{ uri: 'x://b', blob: 'aGk=', mimeType: 'image/png' }] }, 'srv');
...
//# sourceMappingURL=mcp-resource-content.test.js.map