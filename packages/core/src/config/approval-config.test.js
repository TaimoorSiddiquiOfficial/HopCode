...
describe('ApprovalConfig', () => {
    ...
    describe('setApprovalMode', () => {
        ...
        it('throws when setting IZN in untrusted folder', () => {
            const config = new ApprovalConfig({});
            expect(() => config.setApprovalMode(ApprovalMode.IZN, false)).toThrow('Cannot enable privileged approval modes');
        });
        it('throws when setting AUTO_EDIT in untrusted folder', () => {
            const config = new ApprovalConfig({});
            expect(() => config.setApprovalMode(ApprovalMode.AUTO_EDIT, false)).toThrow('Cannot enable privileged approval modes');
        });
        it('allows DEFAULT mode even in untrusted folder', () => {
            const config = new ApprovalConfig({
                approvalMode: ApprovalMode.AUTO_EDIT,
            });
            expect(() => config.setApprovalMode(ApprovalMode.DEFAULT, false)).not.toThrow();
            expect(config.getApprovalMode()).toBe(ApprovalMode.DEFAULT);
        });
        it('allows PLAN mode even in untrusted folder', () => {
            const config = new ApprovalConfig({});
            expect(() => config.setApprovalMode(ApprovalMode.PLAN, false)).not.toThrow();
...
//# sourceMappingURL=approval-config.test.js.map