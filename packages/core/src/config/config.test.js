...
import { Config, ApprovalMode, APPROVAL_MODES, APPROVAL_MODE_INFO, MCPServerConfig, TrustGateError, matchesServerPattern, matchesAnyServerPattern, } from './config.js';
...
import { DEFAULT_TELEMETRY_TARGET, DEFAULT_OTLP_ENDPOINT, SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT, QwenLogger, initializeTelemetry, isTelemetrySdkInitialized, shutdownTelemetry, refreshSessionContext, } from '../telemetry/index.js';
...
vi.mock('../hooks/index.js', () => {
    ...
    return {
        ...
        createInstructionsLoadedCallback: (getHookSystem) => async (notification) => {
            await getHookSystem()?.fireInstructionsLoadedEvent?.(notification.filePath, notification.memoryType, notification.loadReason, {
...
describe('matchesServerPattern', () => {
    ...
    it('* matches any sequence including empty', () => {
        expect(matchesServerPattern('puppeteer', '*puppeteer*')).toBe(true);
        expect(matchesServerPattern('my-puppeteer-server', '*puppeteer*')).toBe(true);
...
describe('matchesAnyServerPattern', () => {
    ...
    it('matches if any pattern matches', () => {
        expect(matchesAnyServerPattern('puppeteer', ['playwright', '*puppeteer*'])).toBe(true);
        expect(matchesAnyServerPattern('chrome', ['playwright', '*puppeteer*'])).toBe(false);
    });
    it('works with mixed exact and glob patterns', () => {
        expect(matchesAnyServerPattern('playwright', ['playwright', '*puppeteer*'])).toBe(true);
        expect(matchesAnyServerPattern('my-puppeteer', ['playwright', '*puppeteer*'])).toBe(true);
    });
});
describe('Server Config (config.ts)', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(HopCodeLogger.prototype, 'logStartSessionEvent').mockImplementation(async () => undefined);
    ...
    describe('getVisionBridgeTimeoutMs', () => {
        ...
        it('accepts the maximum supported integer timeout', () => {
            ...
            }).getVisionBridgeTimeoutMs()).toBe(2_147_483_647);
        });
    });
    describe('getShellDefaultTimeoutMs', () => {
        ...
        it('accepts the maximum supported integer timeout', () => {
            ...
            }).getShellDefaultTimeoutMs()).toBe(2_147_483_647);
        });
    });
    ...
    describe('getTeamMemoryEnabled', () => {
        ...
        it('is off by default and follows the enableTeamMemory setting', () => {
            ...
            }).getTeamMemoryEnabled()).toBe(true);
        });
        it('HOPCODE_CODE_MEMORY_TEAM overrides the setting', () => {
            ...
            expect(new Config({
                ...baseParams,
                enableTeamMemory: true,
            }).getTeamMemoryEnabled()).toBe(false);
        });
    ...
    describe('getCronRecurringMaxAgeDays', () => {
        ...
        it('defaults to 7 days and follows the setting', () => {
            ...
            }).getCronRecurringMaxAgeDays()).toBe(30);
        });
        it('maps 0 to Infinity (no expiry)', () => {
            ...
            }).getCronRecurringMaxAgeDays()).toBe(Infinity);
        });
        it('QWEN_CODE_CRON_MAX_AGE_DAYS overrides the setting', () => {
            ...
            expect(new Config({
                ...baseParams,
                cronRecurringMaxAgeDays: 30,
            }).getCronRecurringMaxAgeDays()).toBe(90);
            process.env['QWEN_CODE_CRON_MAX_AGE_DAYS'] = '0';
            expect(new Config(baseParams).getCronRecurringMaxAgeDays()).toBe(Infinity);
        });
        it('falls back to the default on invalid values', () => {
            ...
            }).getCronRecurringMaxAgeDays()).toBe(7);
        });
        it('warns on the console once at construction for an invalid value', () => {
            ...
            try {
                ...
                expect(config.getCronRecurringMaxAgeDays()).toBe(7);
                const cronWarnings = warnSpy.mock.calls.filter((call) => String(call[0]).includes('QWEN_CODE_CRON_MAX_AGE_DAYS'));
    ...
    describe('getTeamMemorySyncEnabled', () => {
        ...
        it('is off by default and follows the enableTeamMemorySync setting', () => {
            ...
            }).getTeamMemorySyncEnabled()).toBe(true);
        });
        it('HOPCODE_CODE_MEMORY_TEAM_SYNC overrides the setting', () => {
            ...
            expect(new Config({
                ...baseParams,
                enableTeamMemorySync: true,
            }).getTeamMemorySyncEnabled()).toBe(false);
        });
    ...
    describe('getDefaultVisionBridgeModel', () => {
        ...
        const stubProvider = (config, models) => {
            vi.spyOn(config, 'getModel').mockReturnValue('text-primary');
            vi.spyOn(config, 'getContentGeneratorConfig').mockReturnValue({
                authType: AuthType.USE_OPENAI,
                baseUrl: 'https://primary.example.com',
            });
            vi.spyOn(config, 'getAllConfiguredModels').mockReturnValue(models);
        };
        ...
        it.each([false, true])('honors an exact visionModel route with ignored fast-only namesakes (reversed=%s)', (reversed) => {
        ...
        it('falls back to auto-select on a visionModel with no selector before the baseUrl delimiter', () => {
            ...
            expect(warn).toHaveBeenCalledWith(expect.stringContaining("'\\0https://example.com/v1'"));
        });
    ...
    describe('getModelFallbacks', () => {
        ...
        it('accepts an array of model IDs', () => {
            ...
            }).getModelFallbacks()).toEqual(['qwen-plus', 'qwen-turbo']);
        });
        it('caps at 3 entries', () => {
            ...
            }).getModelFallbacks()).toEqual(['model-a', 'model-b', 'model-c']);
        });
    ...
    it('wires file history snapshot updates to chat recording', async () => {
        ...
        const recordFileHistorySnapshot = vi.fn((snapshot) => {
        ...
        try {
            ...
            expect(recordedSnapshots[0].trackedFileBackups['a.txt']).toEqual(expect.objectContaining({
    ...
    it('drops stale file history callbacks after session switch', async () => {
        ...
        const storageDir = await mkdtemp(path.join(os.tmpdir(), 'hopcode-storage-'));
        const config = new Config({
            ...baseParams,
            cwd: projectDir,
            fileCheckpointingEnabled: true,
    ...
    describe('MCP hot-reload (sub-task 3)', () => {
        const srvA = { command: 'a' };
        const srvB = { command: 'b' };
        ...
        it('reinitializeMcpServers is a safe no-op before initialize()', async () => {
            ...
            await expect(config.reinitializeMcpServers({ a: srvA })).resolves.toBeUndefined();
        ...
        it('classifies excluded / pending / removed servers with the right reason', async () => {
            ...
            expect(config.getMcpServerUnavailableReason('c')).toBe('pending_approval');
        ...
        it('reinitializeMcpServers replaces config then drives incremental reconcile', async () => {
            ...
            await config.initialize();
            const manager = config.getToolRegistry().__mcpManagerMock;
        ...
        it('coalesces a reconcile request that arrives mid-flight into one extra pass', async () => {
            ...
            let release;
            manager.discoverAllMcpToolsIncremental.mockImplementationOnce(() => new Promise((resolve) => {
                release = resolve;
            }));
        ...
        it('rethrows a failed reconcile and resets the in-progress guard so the next call still runs', async () => {
            ...
            manager.discoverAllMcpToolsIncremental.mockRejectedValueOnce(new Error('reconcile boom'));
            await expect(config.reinitializeMcpServers({ b: srvB })).rejects.toThrow('reconcile boom');
        ...
        it('clears the coalesce flag when a reconcile throws, so the next call runs exactly one pass', async () => {
            ...
            let reject;
            manager.discoverAllMcpToolsIncremental.mockImplementationOnce(() => new Promise((_, rej) => {
                reject = rej;
            }));
            const first = config.reinitializeMcpServers({ b: srvB });
            const second = config.reinitializeMcpServers({ a: srvA, b: srvB });
        ...
        it('getMcpServerUnavailableReason classifies by glob match', async () => {
            ...
            expect(config.getMcpServerUnavailableReason('puppeteer')).toBe('not_allowed');
    ...
    describe('MemoryPressure configuration environment', () => {
        ...
        function mockMemoryRatio(rssRatio, heapUsedBytes = 0) {
        ...
        it('applies valid memory pressure env overrides', async () => {
            ...
            expect(config.getMemoryPressureMonitor()?.getPressureLevel()).toBe('soft');
        });
        it('falls back to defaults and warns on strict env parse failures', async () => {
            ...
            await config.initialize({ skipGeminiInitialization: true });
            mockMemoryRatio(0.35);
            expect(config.getMemoryPressureMonitor()?.getPressureLevel()).toBe('normal');
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid memory pressure config'));
        });
        it('falls back to defaults and warns on invalid threshold ordering', async () => {
            ...
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('softPressureRatio must be < hardPressureRatio'));
        });
        it.each(['NaN', 'Infinity', '0'])('falls back to defaults for invalid soft threshold %s', async (value) => {
            const stderrSpy = mockStderrWrite();
    ...
    describe('startNewSession', () => {
        ...
        it('flushes the outgoing chat recording service when switching sessions', () => {
            ...
            config.chatRecordingService = { finalize, flush };
            config.startNewSession();
    ...
    describe('chat recording failure listeners', () => {
        const notify = (config, event) => {
            config.notifyChatRecordingFailure(event);
        };
    ...
    describe('initialize', () => {
        ...
        it('should skip hook, skill, and file checkpointing side effects when requested', async () => {
            ...
            await expect(config.initialize({
        ...
        it('registers loop_wakeup when cron is enabled', async () => {
            ...
            await config.initialize();
            const registeredNames = ToolRegistry.prototype.registerFactory.mock.calls.map((call) => call[0]);
        ...
        it('honors HOPCODE_CODE_LEGACY_MCP_BLOCKING=1 by running MCP discovery inline', async () => {
            const originalLegacy = process.env['HOPCODE_CODE_LEGACY_MCP_BLOCKING'];
            ...
            try {
                ...
                if (originalLegacy === undefined) {
                    ...
                    process.env['HOPCODE_CODE_LEGACY_MCP_BLOCKING'] = originalLegacy;
                }
            }
        });
        ...
        it('isMcpServerDisabled consults extension preferences only for the contributing extension', () => {
            ...
            vi.spyOn(manager, 'getDisabledMcpServers').mockImplementation((extensionName) => extensionName === 'my-ext' ? ['bar', 'foo'] : []);
    ...
    describe('refreshAuth', () => {
        ...
        it('re-applies the reasoning effort on a full-refresh model switch that wiped modelsConfig', async () => {
            ...
            const genConfig = config.modelsConfig.getGenerationConfig();
            ...
            await config.handleModelChange(authType, true);
    ...
    it('refreshHierarchicalMemory should not load team memory from untrusted workspaces', async () => {
        ...
        vi.mocked(rebuildTeamAutoMemoryIndex).mockResolvedValue('# Team Memory\n\n- [Shared](shared.md)');
        await config.refreshHierarchicalMemory();
    ...
    it('refreshHierarchicalMemory must not sync when the team-root safety check rejects', async () => {
        ...
        try {
            ...
            vi.mocked(rebuildTeamAutoMemoryIndex).mockRejectedValueOnce(new TeamMemoryRootSecurityError('Refusing to write team memory index: /tmp/.hopcode/team-memory is a ' +
    ...
    it('still syncs when the team-index rebuild fails for an OPERATIONAL reason', async () => {
        ...
        try {
            ...
            const operationalError = Object.assign(new Error('EACCES: permission denied, lstat'), {
                code: 'EACCES',
            });
            vi.mocked(rebuildTeamAutoMemoryIndex).mockRejectedValueOnce(operationalError);
            await config.refreshHierarchicalMemory();
    ...
    it('syncs when the rebuild succeeds and sync is enabled (positive gate)', async () => {
        ...
        try {
            ...
            vi.mocked(rebuildTeamAutoMemoryIndex).mockResolvedValueOnce('# Team Memory\n\n- [Shared](shared.md)');
            await config.refreshHierarchicalMemory();
            expect(rebuildTeamAutoMemoryIndex).toHaveBeenCalledTimes(1);
            expect(syncTeamMemory).toHaveBeenCalledTimes(1);
        }
    ...
    it('refreshHierarchicalMemory surfaces a one-time warning when team memory is not git-shareable', async () => {
        ...
        vi.mocked(getTeamMemoryShareabilityWarning).mockReturnValue('Team memory is enabled, but /tmp/.hopcode/team-memory is git-ignored');
        await config.refreshHierarchicalMemory();
        ...
        expect(config.getWarnings()).toContainEqual(expect.stringContaining('is git-ignored'));
    });
    it('refreshHierarchicalMemory should include appended auto-memory in the context warning estimate', async () => {
        ...
        vi.mocked(readAutoMemoryIndex).mockResolvedValueOnce('# Managed Auto-Memory Index\n\n' + 'remember this '.repeat(80));
        await config.refreshHierarchicalMemory();
        expect(config.getWarnings()).toContainEqual(expect.stringContaining('Loaded HOPCODE.md/context instructions'));
    });
    it('refreshHierarchicalMemory should warn when always-loaded context is large for the model window', async () => {
        ...
        await config.refreshHierarchicalMemory();
        expect(config.getWarnings()).toContainEqual(expect.stringContaining('Loaded HOPCODE.md/context instructions'));
        expect(config.getWarnings()).toContainEqual(expect.stringContaining("model's 1,000 token context window"));
        expect(config.getWarnings()).toContainEqual(expect.stringContaining('more than 15%'));
    });
    ...
    it('getWarnings should use the model token limit when no contextWindowSize is configured', () => {
        ...
        expect(config.getWarnings()).toContainEqual(expect.stringContaining(`model's ${DEFAULT_TOKEN_LIMIT.toLocaleString()} token context window`));
    });
    it('refreshHierarchicalMemory should not warn for small always-loaded context', async () => {
        ...
        expect(config
            .getWarnings()
            .some((warning) => warning.includes('Loaded HOPCODE.md/context instructions'))).toBe(false);
    });
    it('relocateWorkingDirectory should update the session working roots', async () => {
        ...
        expect(loadServerHierarchicalMemory).toHaveBeenCalledWith(newDir, expect.any(Array), expect.any(Object), expect.any(Array), expect.any(Boolean), expect.any(String), expect.any(Array), expect.any(Object));
    ...
    it('relocateWorkingDirectory should continue after recording flush fails', async () => {
        ...
        config.chatRecordingService = { finalize, flush, resetStoragePaths };
        const chdirSpy = vi.spyOn(process, 'chdir').mockImplementation(() => {
            // Keep the test process in its original directory.
        });
        const cwdSpy = vi.spyOn(process, 'cwd').mockReturnValue(newDir);
    ...
    it('relocateWorkingDirectory should move current session artifacts to the new workspace', async () => {
        ...
        const oldRuntimeStatusPath = path.join(oldChatsDir, `${sessionId}.runtime.json`);
        const oldWorktreeSessionPath = path.join(oldChatsDir, `${sessionId}.worktree.json`);
        const newTranscriptPath = path.join(newChatsDir, `${sessionId}.jsonl`);
        const newRuntimeStatusPath = path.join(newChatsDir, `${sessionId}.runtime.json`);
        const newWorktreeSessionPath = path.join(newChatsDir, `${sessionId}.worktree.json`);
        const chdirSpy = vi.spyOn(process, 'chdir').mockImplementation(() => {
            // Keep the test process in its original directory.
        });
        const cwdSpy = vi.spyOn(process, 'cwd').mockReturnValue(newDir);
        ...
        expect(fs.renameSync).toHaveBeenCalledWith(oldTranscriptPath, newTranscriptPath);
        expect(fs.renameSync).toHaveBeenCalledWith(oldRuntimeStatusPath, newRuntimeStatusPath);
        expect(fs.renameSync).toHaveBeenCalledWith(oldWorktreeSessionPath, newWorktreeSessionPath);
    ...
    it('relocateWorkingDirectory should reject and roll back when session artifact migration fails', async () => {
        ...
        await expect(config.relocateWorkingDirectory(newDir)).rejects.toThrow(moveError);
        expect(fs.renameSync).toHaveBeenCalledWith(oldTranscriptPath, newTranscriptPath);
        expect(fs.renameSync).toHaveBeenCalledWith(newTranscriptPath, oldTranscriptPath);
        expect(chdirSpy).toHaveBeenCalledWith(newDir);
    ...
    it('relocateWorkingDirectory should remove a partial EXDEV copy when source cleanup fails', async () => {
        ...
        await expect(config.relocateWorkingDirectory(newDir)).rejects.toThrow(cleanupError);
        expect(fs.copyFileSync).toHaveBeenCalledWith(oldRuntimeStatusPath, newRuntimeStatusPath);
    ...
    it('relocateWorkingDirectory should reject and roll back when the final cwd differs from the expected path', async () => {
        ...
        const cwdSpy = vi
            ...
            .mockReturnValueOnce(oldDir)
            .mockReturnValue(newDir);
        await expect(config.relocateWorkingDirectory(newDir, expectedDir)).rejects.toThrow(`Changed directory to ${newDir}, expected ${expectedDir}.`);
        expect(chdirSpy).toHaveBeenCalledWith(newDir);
        expect(chdirSpy).toHaveBeenCalledWith(oldDir);
        expect(config.getTargetDir()).toBe(oldDir);
        expect(config.storage.getProjectRoot()).toBe(oldDir);
    ...
    it('relocateWorkingDirectory should reject before mutating config when include directories are stale', async () => {
        ...
        vi.mocked(fs.existsSync).mockImplementation((pathToCheck) => pathToCheck !== staleIncludeDir);
        await expect(config.relocateWorkingDirectory(newDir)).rejects.toThrow(`Directory does not exist: ${staleIncludeDir}`);
    ...
    it('relocateWorkingDirectory should return memory refresh failures after moving', async () => {
        ...
        const cwdSpy = vi.spyOn(process, 'cwd').mockReturnValue(newDir);
        vi.mocked(loadServerHierarchicalMemory).mockRejectedValueOnce(new Error('memory failed'));
    ...
    it('refreshHierarchicalMemory should fire InstructionsLoaded hooks from memory notifications', async () => {
        ...
        await options?.onInstructionsLoaded?.({
            filePath: '/tmp/project/HOPCODE.md',
            memoryType: 'project',
            loadReason: 'include',
            triggerFilePath: '/tmp/project/AGENTS.md',
            parentFilePath: '/tmp/project/AGENTS.md',
        });
        expect(fireInstructionsLoadedEvent).toHaveBeenCalledWith('/tmp/project/HOPCODE.md', 'project', 'include', {
            triggerFilePath: '/tmp/project/AGENTS.md',
            parentFilePath: '/tmp/project/AGENTS.md',
        });
    });
    ...
    it('should set custom file filtering settings when provided', () => {
        ...
        expect(config.getFileService().gethopcodeignoreFileNamesDisplay()).toBe('.hopcodeignore, .cursorignore');
    });
    ...
    describe('Telemetry Settings', () => {
        ...
        it('should default sensitiveSpanAttributeMaxLength to 1MiB', () => {
            ...
            expect(configWithTelemetry.getTelemetrySensitiveSpanAttributeMaxLength()).toBe(1024 * 1024);
            const paramsWithoutTelemetry = { ...baseParams };
            delete paramsWithoutTelemetry.telemetry;
            const configWithoutTelemetry = new Config(paramsWithoutTelemetry);
            expect(configWithoutTelemetry.getTelemetrySensitiveSpanAttributeMaxLength()).toBe(1024 * 1024);
        });
        it('should reject invalid sensitiveSpanAttributeMaxLength values', () => {
            for (const [value, label] of [
                ...
                })).toThrow(new RegExp(`Invalid telemetry\\.sensitiveSpanAttributeMaxLength.*got ${label}`));
            }
        });
    ...
    describe('OutboundCorrelation Configuration', () => {
        ...
        ])('propagateTraceContext resolves to $expected when $label', ({ outboundCorrelation, expected }) => {
            const config = new Config({ ...baseParams, outboundCorrelation });
            expect(config.getOutboundCorrelationPropagateTraceContext()).toBe(expected);
        });
    });
    ...
    describe('getMaxToolCallsPerTurn', () => {
        it('should return the default cap when unset', () => {
            const config = new Config(baseParams);
            expect(config.getMaxToolCallsPerTurn()).toBe(DEFAULT_MAX_TOOL_CALLS_PER_TURN);
        });
        ...
        it('tracks whether maxToolCallsPerTurn was explicitly set', () => {
            expect(new Config({ ...baseParams }).isMaxToolCallsPerTurnExplicit()).toBe(false);
            expect(new Config({
                ...baseParams,
                maxToolCallsPerTurn: 42,
            }).isMaxToolCallsPerTurnExplicit()).toBe(true);
        });
        it.each([0, -1])('should return infinity (cap disabled) when set to %d', (capValue) => {
            const config = new Config({
                ...baseParams,
        ...
        it.each([0.5, Number.NaN, Number.POSITIVE_INFINITY])('should reject an invalid maxToolCallsPerTurn value: %s', (capValue) => {
            expect(() => new Config({ ...baseParams, maxToolCallsPerTurn: capValue })).toThrow(/maxToolCallsPerTurn: must be an integer/);
        });
    });
    describe('getMaxSessionTurns', () => {
        ...
        it.each([0.5, Number.NaN, Number.POSITIVE_INFINITY])('should reject an invalid value: %s', (maxSessionTurns) => {
            expect(() => new Config({ ...baseParams, maxSessionTurns })).toThrow(/maxSessionTurns: must be an integer/);
        });
    });
    describe('getClearContextOnIdle', () => {
        ...
        it('should use a custom cumulative tool result threshold if provided', () => {
            ...
            expect(config.getClearContextOnIdle().toolResultsTotalCharsThreshold).toBe(123_456);
        });
...
describe('setApprovalMode with folder trust', () => {
    ...
    it('should throw a TrustGateError when setting IZN mode in an untrusted folder', () => {
        ...
        const config = new Config(baseParams);
        vi.spyOn(config, 'isTrustedFolder').mockReturnValue(false);
        expect(() => config.setApprovalMode(ApprovalMode.IZN)).toThrow(TrustGateError);
        expect(() => config.setApprovalMode(ApprovalMode.IZN)).toThrow('Cannot enable privileged approval modes in an untrusted folder.');
    });
    it('should throw a TrustGateError when setting AUTO_EDIT mode in an untrusted folder', () => {
        const config = new Config(baseParams);
        vi.spyOn(config, 'isTrustedFolder').mockReturnValue(false);
        expect(() => config.setApprovalMode(ApprovalMode.AUTO_EDIT)).toThrow(TrustGateError);
    ...
    describe('prePlanMode tracking', () => {
        ...
        it('does not partially apply plan exit bookkeeping when transition work fails', () => {
            ...
            expect(() => config.setApprovalMode(ApprovalMode.AUTO)).toThrow('strip failed');
...
describe('Model Switching and Config Updates', () => {
    ...
    it('should update contextWindowSize when switching models with hot-update', async () => {
        ...
        await config.handleModelChange(AuthType.HOPCODE_OAUTH, false);
    ...
    describe('Config runtime MCP overlay', () => {
        it('addRuntimeMcpServer does not mutate this.mcpServers', () => {
            ...
            config.initialized = true;
            config.addRuntimeMcpServer('runtime-server', new MCPServerConfig('cmd-b'));
            const settingsLayer = config.mcpServers;
    ...
    describe('MessageDisplay dispatch through the hook execution bridge', () => {
        it('extracts message_id/displayed_text/is_final from the request input and forwards them positionally', async () => {
            ...
            expect(fireMessageDisplayEvent).toHaveBeenCalledWith('msg-123', 'Hello, world', true, undefined);
        ...
        it('defaults missing fields (empty message_id/text, is_final false) rather than throwing', async () => {
            ...
            expect(fireMessageDisplayEvent).toHaveBeenCalledWith('', '', false, undefined);
            expect(response.success).toBe(true);
        });
    });
});
