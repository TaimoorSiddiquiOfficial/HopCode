 ...
 * `process.env['HOPCODE_SESSION_ID']`, preventing throwaway instances
...
describe('Config sessionEnvClaimed guard', () => {
    let originalEnv;
    beforeEach(() => {
        originalEnv = process.env['HOPCODE_SESSION_ID'];
        delete process.env['HOPCODE_SESSION_ID'];
    ...
    afterEach(() => {
        if (originalEnv !== undefined) {
            process.env['HOPCODE_SESSION_ID'] = originalEnv;
        }
        else {
            delete process.env['HOPCODE_SESSION_ID'];
        }
    ...
    it('first Config sets process.env HOPCODE_SESSION_ID to its sessionId', async () => {
        const { Config } = await import('./config.js');
        const config = new Config({ ...baseParams });
        expect(process.env['HOPCODE_SESSION_ID']).toBe(config.getSessionId());
    });
    it('subsequent Config does not overwrite the env var set by the first', async () => {
        ...
        expect(process.env['HOPCODE_SESSION_ID']).toBe(firstSessionId);
        expect(process.env['HOPCODE_SESSION_ID']).not.toBe(secondConfig.getSessionId());
    });
    it('startNewSession updates env var to the new session ID', async () => {
        const { Config } = await import('./config.js');
        const config = new Config({ ...baseParams });
        const originalSessionId = config.getSessionId();
        expect(process.env['HOPCODE_SESSION_ID']).toBe(originalSessionId);
        // Simulate /clear or session switch
        config.startNewSession('new-session-uuid-123');
        expect(process.env['HOPCODE_SESSION_ID']).toBe('new-session-uuid-123');
        expect(process.env['HOPCODE_SESSION_ID']).not.toBe(originalSessionId);
    });
});
//# sourceMappingURL=config-session-env.test.js.map