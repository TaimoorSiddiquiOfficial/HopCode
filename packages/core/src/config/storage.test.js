...
describe('Storage – getPlansDir', () => {
    ...
    it('defaults to ~/.hopcode/plans when plansDirectory is not configured', () => {
        expect(Storage.getPlansDir(projectRoot)).toBe(path.join(Storage.getGlobalHopCodeDir(), 'plans'));
    });
    ...
    it('allows project subdirectories whose names start with two dots', () => {
        expect(Storage.getPlansDir(projectRoot, './..plans')).toBe(path.join(projectRoot, '..plans'));
    });
...
describe('Storage – runtime path methods use getRuntimeBaseDir', () => {
    ...
    it('getGlobalIdeDir is anchored to the global HopCode dir, not runtime base dir', () => {
        ...
        expect(Storage.getGlobalIdeDir()).toBe(path.join(Storage.getGlobalHopCodeDir(), 'ide'));
    });
...
describe('Storage – config paths remain at ~/.hopcode regardless of runtime dir', () => {
    ...
    it('getGlobalSettingsPath still uses ~/.hopcode', () => {
        expect(Storage.getGlobalSettingsPath()).toBe(path.join(globalHopcodeDir, 'settings.json'));
    });
    it('getInstallationIdPath still uses ~/.hopcode', () => {
        expect(Storage.getInstallationIdPath()).toBe(path.join(globalHopcodeDir, 'installation_id'));
    });
    it('getGoogleAccountsPath still uses ~/.hopcode', () => {
        expect(Storage.getGoogleAccountsPath()).toBe(path.join(globalHopcodeDir, 'google_accounts.json'));
    });
    it('getMcpOAuthTokensPath still uses ~/.hopcode', () => {
        expect(Storage.getMcpOAuthTokensPath()).toBe(path.join(globalHopcodeDir, 'mcp-oauth-tokens.json'));
    });
    it('getOAuthCredsPath still uses ~/.hopcode', () => {
        expect(Storage.getOAuthCredsPath()).toBe(path.join(globalHopcodeDir, 'oauth_creds.json'));
    });
    it('getUserCommandsDir still uses ~/.hopcode', () => {
        expect(Storage.getUserCommandsDir()).toBe(path.join(globalHopcodeDir, 'commands'));
    });
    it('getGlobalMemoryFilePath still uses ~/.hopcode', () => {
        expect(Storage.getGlobalMemoryFilePath()).toBe(path.join(globalHopcodeDir, 'memory.md'));
    });
    ...
    it('getUserSkillsDirs still includes ~/.hopcode/skills', () => {
        ...
        expect(skillsDirs.some((dir) => dir === path.join(globalHopcodeDir, 'skills'))).toBe(true);
    });
});
describe('Storage – HOPCODE_HOME env var', () => {
    ...
    it('project-level paths are NOT affected by HOPCODE_HOME', () => {
        ...
        expect(storage.getWorkspaceSettingsPath()).toBe(path.join(projectDir, '.hopcode', 'settings.json'));
        expect(storage.getProjectCommandsDir()).toBe(path.join(projectDir, '.hopcode', 'commands'));
    });
