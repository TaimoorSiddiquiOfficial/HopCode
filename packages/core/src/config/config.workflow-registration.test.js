...
describe('Workflow anti-recursion guard', () => {
    it('ToolNames.WORKFLOW is in EXCLUDED_TOOLS_FOR_SUBAGENTS', async () => {
        const { EXCLUDED_TOOLS_FOR_SUBAGENTS } = await import('../agents/runtime/agent-core.js');
    ...
    it('artifact tools are in EXCLUDED_TOOLS_FOR_SUBAGENTS', async () => {
        ...
        expect(EXCLUDED_TOOLS_FOR_SUBAGENTS.has(ToolNames.RECORD_ARTIFACT)).toBe(true);
    });
});
//# sourceMappingURL=config.workflow-registration.test.js.map