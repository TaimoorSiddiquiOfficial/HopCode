...
describe('resolveNetworkTarget', () => {
    ...
    it('requires credential-free HTTPS for public targets', async () => {
        await expect(resolveNetworkTarget('http://example.com/archive', 'public')).rejects.toThrow('must use HTTPS');
        await expect(resolveNetworkTarget('https://user:secret@example.com/archive', 'public')).rejects.toThrow('must not use credentials');
    });
    it.each([
        ...
        await expect(resolveNetworkTarget(url, 'public')).rejects.toThrow('resolved to a blocked address');
    });
    it('rejects a DNS answer set containing a private address', async () => {
        ...
        await expect(resolveNetworkTarget('https://packages.example/archive', 'public')).rejects.toThrow('resolved to a blocked address');
    });
    it('pins the validated address for the connection', async () => {
        vi.spyOn(dns, 'lookup').mockResolvedValue([
            { address: '8.8.8.8', family: 4 },
        ]);
        const target = await resolveNetworkTarget('https://packages.example:8443/archive', 'public');
    ...
    it('stops waiting for DNS when the caller aborts', async () => {
        ...
        const target = resolveNetworkTarget('https://packages.example/archive', 'public', controller.signal);
        controller.abort(reason);
        await expect(target).rejects.toBe(reason);
    });
    it('allows public IPv6 targets', async () => {
        const target = await resolveNetworkTarget('https://[2606:4700:4700::1111]/archive', 'public');
        expect(target.curlResolve).toBe('[2606:4700:4700::1111]:443:[2606:4700:4700::1111]');
    });
});
//# sourceMappingURL=network-policy.test.js.map