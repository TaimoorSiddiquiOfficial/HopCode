...
import { convertClaudeToHopCodeConfig, convertClaudeAgentConfig, mergeClaudeConfigs, isClaudePluginConfig, convertClaudePluginPackage, convertClaudePluginStandalone, normalizeClaudeMcpServer, } from './claude-converter.js';
...
describe('convertClaudePluginPackage', () => {
    ...
    it('throws when a marketplace source is a symlink resolving outside the marketplace dir', async () => {
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        await expect(convertClaudePluginPackage(pluginSourceDir, 'evil')).rejects.toThrow(/resolves through a symlink outside the marketplace/);
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
    it('treats uppercase HTTPS marketplace plugin sources as URLs', async () => {
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        vi.mocked(downloadFromGitHubRelease).mockResolvedValue(undefined);
    ...
    it('does not fall back to cloning when conversion is aborted', async () => {
        ...
        await expect(convertClaudePluginPackage(pluginSourceDir, 'remote', undefined, controller.signal)).rejects.toBe(reason);
        expect(cloneFromGit).not.toHaveBeenCalled();
    });
    ...
    it('should populate commands/skills/agents when marketplace references the whole folder (deep-wiki shape)', async () => {
        ...
        fs.writeFileSync(path.join(subSkillDir, 'SKILL.md'), '# wiki-skill', 'utf-8');
        // agents/ with file entries referenced individually
        const agentsDir = path.join(pluginSourceDir, 'agents');
        fs.mkdirSync(agentsDir, { recursive: true });
        fs.writeFileSync(path.join(agentsDir, 'wiki-architect.md'), '---\nname: wiki-architect\ndescription: Architect\n---\nbody', 'utf-8');
        fs.writeFileSync(path.join(agentsDir, 'wiki-writer.md'), '---\nname: wiki-writer\ndescription: Writer\n---\nbody', 'utf-8');
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        const result = await convertClaudePluginPackage(pluginSourceDir, 'deep-wiki');
        ...
        expect(fs.existsSync(path.join(convertedSkills, 'wiki-skill', 'SKILL.md'))).toBe(true);
    ...
    it('should populate resources when marketplace references whole folder with trailing slash variants', async () => {
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        const result = await convertClaudePluginPackage(pluginSourceDir, 'no-slash');
        const convertedCommands = path.join(result.convertedDir, 'commands');
    ...
    it('throws when marketplace.json itself is a symlink resolving outside the plugin', async () => {
        ...
        await expect(convertClaudePluginPackage(pluginSourceDir, 'evil')).rejects.toThrow(/resolves through a symlink outside the plugin/);
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
    it('throws in strict mode when plugin.json is a symlink escaping the plugin', async () => {
        ...
        fs.writeFileSync(secretFile, JSON.stringify({ name: 'leaked', version: '9.9.9' }), 'utf-8');
        ...
        await expect(convertClaudePluginPackage(pluginSourceDir, 'evil')).rejects.toThrow(/Strict mode requires a trusted plugin\.json/);
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
...
describe('convertClaudePluginStandalone', () => {
    ...
    it('converts a repo with root .claude-plugin/plugin.json, .mcp.json and skills', async () => {
        ...
        fs.mkdirSync(skillDir, { recursive: true });
        fs.writeFileSync(path.join(skillDir, 'SKILL.md'), '# best practices', 'utf-8');
        ...
        expect(fs.existsSync(path.join(result.convertedDir, 'qwen-extension.json'))).toBe(true);
        ...
        const mcp = result.config.mcpServers?.['clickhouse'];
        ...
        expect(fs.existsSync(path.join(result.convertedDir, 'skills', 'best-practices', 'SKILL.md'))).toBe(true);
    ...
    it('throws when there is no .claude-plugin/plugin.json', async () => {
        await expect(convertClaudePluginStandalone(testDir)).rejects.toThrow(/Plugin configuration not found/);
    });
    it('ignores an absolute mcpServers path so it cannot read out-of-tree files', async () => {
        ...
        fs.writeFileSync(secretFile, JSON.stringify({ leaked: { command: 'cat', args: ['/etc/passwd'] } }), 'utf-8');
        const pluginDir = path.join(testDir, '.claude-plugin');
        fs.mkdirSync(pluginDir, { recursive: true });
        fs.writeFileSync(path.join(pluginDir, 'plugin.json'), JSON.stringify({
            name: 'evil',
    ...
    it('throws when plugin.json is a symlink resolving outside the plugin', async () => {
        ...
        await expect(convertClaudePluginStandalone(testDir)).rejects.toThrow(/resolves through a symlink outside/);
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
    ...
    it('does not copy a symlink whose target escapes the plugin directory', async () => {
        ...
        const pluginDir = path.join(testDir, '.claude-plugin');
        fs.mkdirSync(pluginDir, { recursive: true });
        fs.writeFileSync(path.join(pluginDir, 'plugin.json'), JSON.stringify({ name: 'evil', version: '1.0.0' }), 'utf-8');
        ...
        expect(fs.existsSync(path.join(result.convertedDir, 'skills', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(result.convertedDir, 'skills', 'leak.txt'))).toBe(false);
        fs.rmSync(result.convertedDir, { recursive: true, force: true });
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
    it('skips a .mcp.json that has no mcpServers object instead of misparsing it', async () => {
        const pluginDir = path.join(testDir, '.claude-plugin');
        fs.mkdirSync(pluginDir, { recursive: true });
        fs.writeFileSync(path.join(pluginDir, 'plugin.json'), JSON.stringify({ name: 'no-servers', version: '1.0.0' }), 'utf-8');
        // No `mcpServers` key — the whole object must not be treated as the map.
        fs.writeFileSync(path.join(testDir, '.mcp.json'), JSON.stringify({ name: 'foo', other: 'bar' }), 'utf-8');
        const result = await convertClaudePluginStandalone(testDir);
        expect(result.config.mcpServers).toBeUndefined();
        expect(result.config.mcpServers?.['name']).toBeUndefined();
        fs.rmSync(result.convertedDir, { recursive: true, force: true });
    });
    ...
    it('throws a clear error when plugin.json parses to null', async () => {
        ...
        await expect(convertClaudePluginStandalone(testDir)).rejects.toThrow(/Invalid plugin configuration/);
    });
});
...
describe('convertClaudePluginPackage — git-subdir source', () => {
    ...
    const writeMarketplace = (source) => {
        const mp = path.join(extDir, '.claude-plugin');
        fs.mkdirSync(mp, { recursive: true });
        fs.writeFileSync(path.join(mp, 'marketplace.json'), JSON.stringify({
            name: 'm',
    ...
    it('clones, pins to the sha over the ref, and returns the subdirectory', async () => {
        vi.mocked(cloneFromGit).mockImplementation(async (_meta, dir) => {
            const sub = path.join(dir, 'packages', 'plugin');
            fs.mkdirSync(path.join(sub, '.claude-plugin'), { recursive: true });
            fs.writeFileSync(path.join(sub, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'p', version: '1.0.0' }), 'utf-8');
        });
        writeMarketplace({
            source: 'git-subdir',
        ...
        const meta = vi.mocked(cloneFromGit).mock.calls[0][0];
    ...
    it('rejects a subdirectory that escapes the repository root', async () => {
        vi.mocked(cloneFromGit).mockResolvedValue(undefined);
        writeMarketplace({
            source: 'git-subdir',
            url: 'https://example.com/repo.git',
            path: '../../etc',
        });
        await expect(convertClaudePluginPackage(extDir, 'p')).rejects.toThrow(/escapes the repository root/);
    });
    it('rejects an absolute subdirectory path', async () => {
        ...
        await expect(convertClaudePluginPackage(extDir, 'p')).rejects.toThrow(/Invalid plugin subdirectory/);
    });
    it('rejects a missing subdirectory', async () => {
        ...
        await expect(convertClaudePluginPackage(extDir, 'p')).rejects.toThrow(/not found/);
    });
    it('rejects a subdirectory that is a symlink escaping the clone', async () => {
        ...
        await expect(convertClaudePluginPackage(extDir, 'p')).rejects.toThrow(/resolves through a symlink/);
        fs.rmSync(secretDir, { recursive: true, force: true });
    });
});
describe('convertClaudePluginPackage — string URL source', () => {
    ...
    it('treats an uppercase HTTPS:// source as a URL download, not a local path', async () => {
        ...
        vi.mocked(downloadFromGitHubRelease).mockImplementation(async (_meta, dir) => {
            fs.mkdirSync(path.join(dir, '.claude-plugin'), {
                recursive: true,
            });
            fs.writeFileSync(path.join(dir, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'p', version: '1.0.0' }), 'utf-8');
...
describe('normalizeClaudeMcpServer', () => {
    ...
    const norm = (raw) => normalizeClaudeMcpServer(raw);
