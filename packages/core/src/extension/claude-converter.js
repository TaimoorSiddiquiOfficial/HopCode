...
export function convertClaudeAgentConfig(claudeAgent) {
    ...
    if (claudeAgent.permissionMode) {
        ...
        const mapped = claudeToHopCodeMode[claudeAgent.permissionMode] ??
...
async function convertAgentFiles(agentsDir) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const systemPrompt = hopcodeAgent['systemPrompt'] || body.trim();
            const newContent = `---
${newYaml}
---
 ...
 */
export function convertClaudeToHopCodeConfig(claudeConfig) {
...
export async function convertClaudePluginPackage(extensionDir, pluginName, networkPolicy, signal) {
    ...
    const pluginSource = await resolvePluginSource(marketplacePlugin, extensionDir, pluginDir, networkPolicy, signal);
 ...
 */
function resolvePluginRelativeFile(pluginSource, relativePath) {
 ...
 * conversion paths.
 */
async function buildQwenExtensionFromPlugin(pluginSource, mergedConfig) {
    ...
    try {
        ...
        fs.writeFileSync(hopcodeConfigPath, JSON.stringify(hopcodeConfig, null, 2), 'utf-8');
        return {
