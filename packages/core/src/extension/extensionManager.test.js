...
import { ExtensionManager, ExtensionUpdateState, SettingScope, validateName, getExtensionId, hashValue, } from './extensionManager.js';
...
const EXTENSIONS_DIRECTORY_NAME = path.join(HOPCODE_DIR, 'extensions');
...
describe('extension tests', () => {
    ...
    beforeEach(() => {
        ...
        tempWorkspaceDir = fs.mkdtempSync(path.join(tempHomeDir, 'hopcode-test-workspace-'));
    ...
    describe('installExtension', () => {
        function writeExtractedExtension(destination, name) {
            fs.mkdirSync(destination, { recursive: true });
            fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name, version: '1.0.0' }));
        }
        it('installs and uninstalls within an injected extension store root', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
            ...
            const installed = await manager.installExtension({ type: 'local', source: archivePath }, async () => { });
            expect(installed.path).toBe(path.join(customExtensionsDir, 'custom-root'));
        ...
        it('commits workspace initial activation with the installed artifact', async () => {
            ...
            const extension = await manager.installExtension({ type: 'local', source: archivePath }, () => Promise.resolve(), undefined, tempWorkspaceDir, undefined, { scope: 'workspace', workspacePath: tempWorkspaceDir });
            const activation = await manager.getExtensionActivation(extension.id, tempWorkspaceDir);
        ...
        it('prepares without mutating the store and commits exactly once', async () => {
            ...
            expect(fs.existsSync(path.join(userExtensionsDir, 'prepared-ext'))).toBe(false);
            expect((await manager.getExtensionStoreSnapshot()).generation).toBe(before.generation);
            ...
            await expect(manager.commitPreparedExtension(prepared)).rejects.toMatchObject({ code: 'prepared_extension_consumed' });
        ...
        it('signals the durable commit before runtime refresh completes', async () => {
            ...
            vi.spyOn(manager, 'refreshTools').mockImplementation(async () => await refreshBlocked);
            const prepared = await manager.prepareExtensionInstall({
                installMetadata: { type: 'local', source: archivePath },
                initialActivation: { scope: 'user' },
                requestConsent: async () => { },
        ...
        it('fully validates the staged extension before commit', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({
            ...
            const manager = createExtensionManager();
            const before = await manager.getExtensionStoreSnapshot();
            await expect(manager.prepareExtensionInstall({
                installMetadata: { type: 'local', source: archivePath },
                initialActivation: { scope: 'user' },
                requestConsent: async () => { },
            })).rejects.toThrow();
            expect(await manager.getExtensionStoreSnapshot()).toEqual(before);
            expect(fs.existsSync(path.join(userExtensionsDir, 'invalid-context'))).toBe(false);
        });
        it('commits a fully validated extension without an explicit version', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'default-version' }));
            });
            const manager = createExtensionManager();
            const prepared = await manager.prepareExtensionInstall({
                installMetadata: { type: 'local', source: archivePath },
        ...
        it('uses the installed path for Claude plugin root replacement', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                ...
                fs.writeFileSync(path.join(pluginDirectory, 'plugin.json'), JSON.stringify({ name: 'claude-ext', version: '1.0.0' }));
                fs.mkdirSync(path.join(destination, 'hooks'));
                fs.writeFileSync(path.join(destination, 'README.md'), '${CLAUDE_PLUGIN_ROOT}/scripts/setup.sh');
            });
            const manager = createExtensionManager();
            const prepared = await manager.prepareExtensionInstall({
                installMetadata: {
            ...
            try {
                ...
                expect(fs.readFileSync(path.join(prepared.destinationDirectory, 'README.md'), 'utf8')).toBe(path.join(prepared.destinationDirectory, 'scripts', 'setup.sh'));
            }
            finally {
                await manager.disposePreparedExtension(prepared);
            }
        ...
        it('does not report a temp cleanup warning when an immediate retry succeeds', async () => {
            ...
            await expect(manager.disposePreparedExtension(prepared)).resolves.toBeUndefined();
        ...
        it('signals the durable commit before deferred settings finish', async () => {
            ...
            let finishSettings;
            const settingsBlocked = new Promise((resolve) => {
            ...
            expect(onCommitted.mock.invocationCallOrder[0]).toBeLessThan(commitSettings.mock.invocationCallOrder[0]);
            finishSettings();
        ...
        it('surfaces committed runtime refresh warnings after install reloads', async () => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            vi.spyOn(manager, 'refreshTools').mockRejectedValueOnce(new Error('runtime stale'));
            await expect(manager.installExtension({ type: 'local', source: archivePath }, async () => { })).rejects.toMatchObject({
        ...
        it('records error telemetry when a prepared install commit fails', async () => {
            ...
            vi.spyOn(ExtensionStore.prototype, 'commitArtifact').mockRejectedValueOnce(new Error('disk full'));
            mockLogExtensionInstallEvent.mockClear();
            await expect(manager.commitPreparedExtension(prepared)).rejects.toThrow('disk full');
            expect(mockLogExtensionInstallEvent).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
        ...
        it('rejects forged prepared handles without deleting their paths', async () => {
            ...
            await expect(manager.commitPreparedExtension(forged)).rejects.toMatchObject({ code: 'invalid_prepared_extension' });
            await expect(manager.disposePreparedExtension(forged)).rejects.toMatchObject({ code: 'invalid_prepared_extension' });
        ...
        it('should install an extension from a local archive', async () => {
            ...
            expect(mockExtractArchiveFile).toHaveBeenCalledWith(archivePath, expect.any(String), undefined);
        ...
        it('should not reuse a dirty tempDir when falling back from GitHub release to git clone', async () => {
            ...
            vi.spyOn(ExtensionStorage, 'createTmpDir').mockImplementation(async () => fs.mkdtempSync(path.join(tempHomeDir, 'tracked-extension-')));
            const { downloadFromGitHubRelease } = await import('./github.js');
            const downloadMock = vi.mocked(downloadFromGitHubRelease);
            downloadMock.mockImplementation(async (_meta, destination) => {
            ...
            mockGit.clone.mockImplementation(async (_url, _target) => {
            ...
            const extension = await manager.installExtension({
                ...
                type: 'git',
            }, async () => { }, undefined, undefined, undefined, { scope: 'user' }, controller.signal);
        ...
        it('should clean up converted temp dir for local archive installs', async () => {
            ...
            vi.spyOn(ExtensionStorage, 'createTmpDir').mockImplementation(async () => {
                const tempDir = fs.mkdtempSync(path.join(tempHomeDir, 'tracked-extension-'));
                tempDirs.push(tempDir);
                return tempDir;
            });
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, 'gemini-extension.json'), JSON.stringify({
            ...
            expect(fs.existsSync(path.join(userExtensionsDir, 'gemini-archive-extension', EXTENSIONS_CONFIG_FILENAME))).toBe(true);
        });
        it('should install an extension from an archive URL', async () => {
            mockDownloadFromArchiveUrl.mockImplementation(async (_metadata, destination) => {
            ...
            expect(mockDownloadFromArchiveUrl).toHaveBeenCalledWith(expect.objectContaining({
                source: 'https://example.com/archive-extension.zip',
                type: 'archive-url',
            }), expect.any(String), controller.signal);
        ...
        it('forces the manager network policy onto remote operations', async () => {
            ...
            expect(mockDownloadFromArchiveUrl).toHaveBeenCalledWith(expect.objectContaining({ networkPolicy: 'public' }), expect.any(String), undefined);
        });
        it('should clean up the temp dir when archive URL download fails', async () => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            await expect(manager.installExtension({
                source: 'https://example.com/archive-extension.zip',
                type: 'archive-url',
        ...
        it('should clean up the temp dir when local archive extraction fails', async () => {
            ...
            await expect(manager.installExtension({
                source: archivePath,
                type: 'local',
            }, async () => { })).rejects.toThrow('extract failed');
            expect(tempDir).toBeDefined();
            expect(fs.existsSync(tempDir)).toBe(false);
        });
    });
    describe('uninstallExtension', () => {
        ...
        it('returns a committed warning when uninstall runtime refresh fails', async () => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            vi.spyOn(manager, 'refreshTools').mockRejectedValue(new Error('refresh failed'));
            const result = await manager.uninstallExtension('my-extension', false);
            expect(result.warnings).toEqual([
                {
                    code: 'extension_runtime_refresh_failed',
    ...
    describe('loadExtension', () => {
        it('uses the injected extension store root for discovery', async () => {
            ...
            expect(manager.getLoadedExtensions()[0]?.path).toBe(path.join(customExtensionsDir, 'custom-root-extension'));
        });
        ...
        describe('command discovery', () => {
            it('should discover .md command files', async () => {
                ...
                expect(ext?.commands).toEqual(expect.arrayContaining(['greet', 'farewell']));
            ...
            it('should discover .toml command files', async () => {
                ...
                const commandsDir = path.join(extDir, 'commands');
                fs.mkdirSync(commandsDir, { recursive: true });
                fs.writeFileSync(path.join(commandsDir, 'caveman.toml'), 'prompt = "Talk like caveman"\ndescription = "Caveman mode"');
                const manager = createExtensionManager();
                await manager.refreshCache();
                const extensions = manager.getLoadedExtensions();
                const ext = extensions.find((e) => e.config.name === 'toml-commands-ext');
            ...
            it('should discover both .md and .toml command files', async () => {
                ...
                await manager.refreshCache();
                const extensions = manager.getLoadedExtensions();
                const ext = extensions.find((e) => e.config.name === 'mixed-commands-ext');
                expect(ext?.commands).toEqual(expect.arrayContaining(['greet', 'caveman']));
                expect(ext?.commands).toHaveLength(2);
            });
            it('should list both entries when .md and .toml exist for same command name', async () => {
                ...
                fs.mkdirSync(commandsDir, { recursive: true });
                fs.writeFileSync(path.join(commandsDir, 'greet.md'), 'Hello!');
                fs.writeFileSync(path.join(commandsDir, 'greet.toml'), 'prompt = "Hello!"');
                const manager = createExtensionManager();
                await manager.refreshCache();
                const extensions = manager.getLoadedExtensions();
                const ext = extensions.find((e) => e.config.name === 'dedup-commands-ext');
            ...
            it('should discover nested .toml command files with colon-separated names', async () => {
                ...
                fs.writeFileSync(path.join(nestedDir, 'intensity.toml'), 'prompt = "Switch intensity"');
                const manager = createExtensionManager();
                await manager.refreshCache();
                const extensions = manager.getLoadedExtensions();
            ...
            it('should replace colons in path segments with underscores', async () => {
                if (process.platform !== 'linux')
                    return; // colons forbidden in filenames on macOS/Windows
                const extDir = createExtension({
                    extensionsDir: userExtensionsDir,
                    name: 'colon-name-ext',
                    version: '1.0.0',
    ...
    describe('enableExtension / disableExtension', () => {
        it('applies V2 default and workspace activation to loaded extensions', async () => {
            ...
            await manager.setExtensionWorkspaceActivation(extension.id, tempWorkspaceDir, 'enabled');
            expect(manager.getLoadedExtensions()[0]?.isActive).toBe(true);
            await manager.clearExtensionWorkspaceActivation(extension.id, tempWorkspaceDir);
            expect(manager.getLoadedExtensions()[0]?.isActive).toBe(false);
        });
        it('refreshes runtime tools after V2 activation changes', async () => {
            ...
            await manager.setExtensionWorkspaceActivation(extension.id, tempWorkspaceDir, 'disabled');
            await manager.clearExtensionWorkspaceActivation(extension.id, tempWorkspaceDir);
        ...
        it('returns a committed warning when activation runtime refresh fails', async () => {
            ...
            const extension = manager.getLoadedExtensions()[0];
            vi.spyOn(manager, 'refreshTools').mockRejectedValue(new Error('refresh failed'));
            const result = await manager.setExtensionDefaultActivation(extension.id, 'disabled');
            expect(result.warnings).toEqual([
                {
                    code: 'extension_runtime_refresh_failed',
                    error: 'refresh failed',
            ...
            expect(result.extensions[extension.id]?.defaultActivation).toBe('disabled');
        });
        it('derives activation from the supplied store snapshot', async () => {
            ...
            await manager.refreshCache();
            const extension = manager.getLoadedExtensions()[0];
            const disabledSnapshot = await manager.setExtensionDefaultActivation(extension.id, 'disabled');
            await manager.setExtensionDefaultActivation(extension.id, 'enabled');
            expect(manager.getExtensionActivationFromSnapshot(extension.id, disabledSnapshot, tempWorkspaceDir)).toMatchObject({ effective: 'disabled', source: 'default' });
            await expect(manager.getExtensionActivation(extension.id, tempWorkspaceDir)).resolves.toMatchObject({ effective: 'enabled', source: 'default' });
        });
        it('changes activation scope in one policy mutation', async () => {
            ...
            await manager.refreshCache();
            const extension = manager.getLoadedExtensions()[0];
            const workspaceSnapshot = await manager.setExtensionActivationScope(extension.id, {
                scope: 'workspace',
                workspacePath: tempWorkspaceDir,
            });
        ...
        it('keeps the V2 state in sync after a legacy scope mutation', async () => {
            ...
            await manager.refreshCache();
            const extension = manager.getLoadedExtensions()[0];
            await manager.disableExtension(extension.name, SettingScope.Workspace, tempWorkspaceDir);
            const activation = await manager.getExtensionActivation(extension.id, tempWorkspaceDir);
            expect(activation).toMatchObject({
                effective: 'disabled',
        ...
        it('keeps other workspace overrides during a legacy workspace mutation', async () => {
            ...
            await manager.setExtensionWorkspaceActivation(extension.id, otherWorkspace, 'enabled');
            await manager.disableExtension(extension.name, SettingScope.Workspace, tempWorkspaceDir);
        ...
        it('clears only child workspace overrides during a legacy user mutation', async () => {
            ...
            await manager.setExtensionWorkspaceActivation(extension.id, tempWorkspaceDir, 'enabled');
            await manager.setExtensionWorkspaceActivation(extension.id, outsideWorkspace, 'disabled');
        ...
        it('should not emit mutation lifecycle events when validation fails', async () => {
            ...
            const events = [];
            manager.addMutationListener((event) => events.push(event));
            await expect(manager.disableExtension('missing-extension', SettingScope.User)).rejects.toThrow('Extension with name missing-extension does not exist.');
            await expect(manager.enableExtension('missing-extension', SettingScope.User)).rejects.toThrow('Extension with name missing-extension does not exist.');
            await expect(manager.addSource('   ')).rejects.toThrow('Marketplace source cannot be empty.');
            expect(events).toEqual([]);
        });
    ...
    describe('preference-only operations', () => {
        it('should not emit mutation lifecycle events for preference changes', () => {
            ...
            fs.writeFileSync(path.join(userExtensionsDir, 'marketplaces.json'), JSON.stringify([
                {
                    name: 'marketplace',
                    source: 'owner/repo',
                    type: 'github',
    ...
    describe('updateExtension', () => {
        ...
        it('rejects a stale direct update after the artifact changes', async () => {
            ...
            fs.writeFileSync(archivePath, 'archive');
            const writeExtension = (destination, version) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'my-extension', version }));
            };
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            const metadata = { type: 'local', source: archivePath };
            const installed = await manager.installExtension(metadata, async () => { });
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                ...
                await concurrentStore.commitArtifact({
                    ...
                    expectedArtifactGeneration: before.extensions[installed.id].artifactGeneration,
                });
            });
            await expect(manager.installExtension(metadata, async () => { }, undefined, tempWorkspaceDir, installed.config)).rejects.toMatchObject({ code: 'extension_conflict' });
            expect(JSON.parse(fs.readFileSync(path.join(installed.path, EXTENSIONS_CONFIG_FILENAME), 'utf8'))).toMatchObject({ version: 'concurrent' });
        });
        it('marks a direct update reload failure as already committed', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'my-extension', version: '2.0.0' }));
            });
            const manager = createExtensionManager();
            await manager.refreshCache();
            const extension = manager.getLoadedExtensions()[0];
            ...
            await expect(manager.installExtension({ type: 'local', source: archivePath }, async () => { }, undefined, tempWorkspaceDir, extension.config)).rejects.toMatchObject({
                code: 'extension_committed_with_warnings',
                committed: true,
            });
            expect(JSON.parse(fs.readFileSync(path.join(extensionPath, EXTENSIONS_CONFIG_FILENAME), 'utf8'))).toMatchObject({ version: '2.0.0' });
        });
        it('rejects an invalid staged extension before commit', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'my-extension', version: '1.0.0' }));
            });
            const manager = createExtensionManager();
            const prepared = await manager.prepareExtensionInstall({
                installMetadata: { type: 'local', source: archivePath },
            ...
            fs.writeFileSync(path.join(prepared.stagingDirectory, EXTENSIONS_CONFIG_FILENAME), '{ invalid json');
            const before = await manager.getExtensionStoreSnapshot();
            try {
                await expect(manager.commitPreparedExtension(prepared)).rejects.toThrow('Failed to load extension config');
            }
            finally {
                await manager.disposePreparedExtension(prepared);
            }
        ...
        it('rejects staged identity changes before commit', async () => {
            ...
            mockExtractArchiveFile.mockImplementation(async (_source, destination) => {
                fs.mkdirSync(destination, { recursive: true });
                fs.writeFileSync(path.join(destination, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'original-name', version: '1.0.0' }));
            });
            const manager = createExtensionManager();
            const prepared = await manager.prepareExtensionInstall({
                installMetadata: { type: 'local', source: archivePath },
            ...
            fs.writeFileSync(path.join(prepared.stagingDirectory, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name: 'changed-name', version: '1.0.0' }));
            const before = await manager.getExtensionStoreSnapshot();
            try {
                await expect(manager.commitPreparedExtension(prepared)).rejects.toThrow('Prepared extension identity changed before commit.');
            }
            finally {
                await manager.disposePreparedExtension(prepared);
            }
        ...
        it('reports a committed update reload failure as needing restart', async () => {
            ...
            await expect(manager.updateExtension(extension, ExtensionUpdateState.UPDATE_AVAILABLE, callback)).resolves.toEqual({
                name: 'my-extension',
            ...
            expect(callback).toHaveBeenLastCalledWith('my-extension', ExtensionUpdateState.UPDATED_NEEDS_RESTART);
            expect(manager.getLoadedExtensions()).toEqual([]);
        });
        it('reports a committed update runtime warning as needing restart', async () => {
            ...
            vi.spyOn(manager, 'refreshTools').mockRejectedValueOnce(new Error('runtime stale'));
            const callback = vi.fn();
            await manager.updateExtension(extension, ExtensionUpdateState.UPDATE_AVAILABLE, callback);
            expect(callback).toHaveBeenLastCalledWith('my-extension', ExtensionUpdateState.UPDATED_NEEDS_RESTART);
        });
        it('surfaces a committed settings compatibility warning distinctly', async () => {
            ...
            const extension = manager.getLoadedExtensions()[0];
            const internals = manager;
            const prepared = await internals.prepareExtensionUpdateFromState(extension);
            Object.defineProperty(prepared, 'commitSettings', {
                value: vi.fn().mockRejectedValue(new Error('legacy sync unavailable')),
            });
            vi.spyOn(internals, 'prepareExtensionUpdateFromState').mockResolvedValueOnce(prepared);
            const callback = vi.fn();
            await expect(manager.updateExtension(extension, ExtensionUpdateState.UPDATE_AVAILABLE, callback)).resolves.toMatchObject({
                warnings: [
                    {
                        code: 'extension_settings_legacy_sync_failed',
            ...
            expect(callback).toHaveBeenLastCalledWith('my-extension', ExtensionUpdateState.UPDATED_WITH_WARNINGS);
        });
        it('should end mutation lifecycle events when temp directory creation fails', async () => {
            ...
            vi.spyOn(ExtensionStorage, 'createTmpDir').mockRejectedValueOnce(new Error('disk full'));
            await expect(manager.updateExtension(extension, ExtensionUpdateState.UPDATE_AVAILABLE, callback)).rejects.toThrow('disk full');
            expect(events).toEqual([
                { id: 1, phase: 'start', operation: 'updateExtension' },
                { id: 1, phase: 'end', operation: 'updateExtension' },
            ]);
            expect(callback).toHaveBeenCalledWith('my-extension', ExtensionUpdateState.ERROR);
        });
    });
    describe('performWorkspaceExtensionMigration', () => {
        ...
        it('reports a committed extension that could not be reloaded', async () => {
            const manager = createExtensionManager();
            vi.spyOn(manager, 'installExtension').mockRejectedValueOnce(Object.assign(new Error('committed with warnings'), {
                code: 'extension_committed_with_warnings',
                committed: true,
            ...
            await expect(manager.performWorkspaceExtensionMigration([extension], async () => { })).resolves.toEqual(['migration-extension']);
        });
        it('does not retry a committed extension for recoverable warnings', async () => {
            ...
            await expect(manager.performWorkspaceExtensionMigration([extension], async () => { })).resolves.toEqual([]);
        });
    });
    describe('validateExtensionOverrides', () => {
        ...
        it('should mark all extensions as inactive when "none" is provided', async () => {
            ...
            const extensions = manager.getLoadedExtensions();
            expect(extensions.every((e) => !e.isActive)).toBe(true);
            await expect(manager.getExtensionActivation(extensions[0].id)).resolves.toMatchObject({
                effective: 'disabled',
                source: 'cli_override',
            });
        });
    ...
    describe('loadExtensionConfig', () => {
        ...
        describe('refreshTools', () => {
            ...
            it('refreshTools should call all refresh methods', async () => {
                ...
                expect(mockReinitializeMcpServers).toHaveBeenCalledWith(mockSettingsMcpServers);
