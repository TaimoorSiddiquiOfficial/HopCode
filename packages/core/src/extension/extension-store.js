...
export class ExtensionStore {
            ...
            if (existing.legacyProjectionHash !== projectionHash(legacy)) {
                if (!(await this.legacyProjectionIsNewerThanState())) {
                    ...
                    for (const identity of identities.values()) {
                        ...
                        const { rules, activationChanged } = this.importLegacyProjection(legacy[identity.name]?.overrides ?? [], existingPolicy);
                        if (existingPolicy) {
    ...
    async commitArtifact(input) {
        ...
        return await this.withLock(async () => {
            ...
            const destinationExists = await this.pathExists(input.destinationDirectory);
            ...
            try {
                if (destinationExists) {
                    await renameWithRetry(input.destinationDirectory, backupDirectory, 3, 50);
                }
                if (input.operation !== 'uninstall') {
                    await renameWithRetry(input.stagingDirectory, input.destinationDirectory, 3, 50);
                }
    ...
    async readLegacyProjection() {
        try {
            return JSON.parse(await fsp.readFile(this.enablementPath, 'utf8'));
        }
        catch (error) {
            if (error.code === 'ENOENT')
                return {};
