...
import { parseInstallSource, loadMarketplaceConfigFromSource, } from './marketplace.js';
...
vi.mock('./github.js', () => ({
    isSupportedArchiveUrl: vi.fn((url) => {
        try {
...
describe('parseInstallSource', () => {
    ...
    describe('HTTPS URL parsing', () => {
        ...
        it('should parse supported archive URLs as archive-url installs', async () => {
            vi.mocked(fs.stat).mockRejectedValueOnce(new Error('ENOENT'));
            const result = await parseInstallSource('https://example.com/releases/extension.tar.gz');
            expect(result.source).toBe('https://example.com/releases/extension.tar.gz');
            expect(result.type).toBe('archive-url');
            expect(result.pluginName).toBeUndefined();
        });
        it('should parse supported archive URLs with plugin name', async () => {
            vi.mocked(fs.stat).mockRejectedValueOnce(new Error('ENOENT'));
            const result = await parseInstallSource('https://example.com/releases/extension.zip:my-plugin');
    ...
    describe('loadMarketplaceConfigFromSource', () => {
        it('fetches direct HTTP marketplace JSON with the HTTP client', async () => {
            ...
            const result = await loadMarketplaceConfigFromSource('http://example.com/marketplace.json');
            expect(result).toEqual(cfg);
            expect(http.get).toHaveBeenCalledWith('http://example.com/marketplace.json', {
        ...
        it('resolves a marketplace from a git@ SSH source', async () => {
            ...
            const result = await loadMarketplaceConfigFromSource('git@github.com:owner/repo.git');
            expect(result).toEqual(cfg);
        });
        it('resolves a marketplace from an uppercase HTTPS GitHub source', async () => {
            ...
            const result = await loadMarketplaceConfigFromSource('HTTPS://github.com/owner/repo');
            expect(result).toEqual(cfg);
        });
        it('resolves a direct JSON marketplace from an uppercase HTTPS source', async () => {
            ...
            const result = await loadMarketplaceConfigFromSource('HTTPS://example.com/marketplace.json');
            expect(result).toEqual(cfg);
        });
        it('resolves a direct JSON marketplace from an uppercase HTTP source', async () => {
            ...
            const result = await loadMarketplaceConfigFromSource('HTTP://example.com/marketplace.json');
            expect(result).toEqual(cfg);
            expect(https.get).not.toHaveBeenCalledWith('HTTP://example.com/marketplace.json', expect.anything(), expect.anything());
        });
        ...
        it('aborts and returns null when the response body exceeds the size cap', async () => {
            ...
            vi.mocked(https.get).mockImplementation((_url, _options, callback) => {
                ...
                process.nextTick(() => handlers['data']?.(Buffer.alloc(11 * 1024 * 1024)));
                return { on: vi.fn(), setTimeout: vi.fn(), destroy };
            });
            const result = await loadMarketplaceConfigFromSource('https://example.com/marketplace.json');
        ...
        it('aborts and returns null when the wall-clock deadline elapses', async () => {
            ...
            try {
                ...
                const promise = loadMarketplaceConfigFromSource('https://example.com/marketplace.json');
        ...
        it('does not start a request when DNS outlives the deadline', async () => {
            ...
            try {
                ...
                const promise = loadMarketplaceConfigFromSource('https://packages.example/marketplace.json', 'public');
