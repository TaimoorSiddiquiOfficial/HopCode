...
describe('extractZipArchive', () => {
    ...
    it('rejects entries that escape the destination', async () => {
        ...
        mockOpen.mockImplementation((_file, _options, callback) => callback(null, zipFile));
        await expect(extractZipArchive(path.join(tempDir, 'archive.zip'), destination)).rejects.toThrow('Out of bound path');
        await expect(fs.stat(path.join(tempDir, 'outside.txt'))).rejects.toThrow();
    });
    it.runIf(process.platform !== 'win32')('does not create directories through an existing symbolic link', async () => {
        tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'zip-extraction-'));
        const destination = path.join(tempDir, 'destination');
        ...
        zipFile.close = vi.fn(() => zipFile.emit('close'));
        mockOpen.mockImplementation((_file, _options, callback) => callback(null, zipFile));
        await expect(extractZipArchive(path.join(tempDir, 'archive.zip'), destination)).rejects.toThrow('Refusing to extract through non-directory path');
    ...
    it('sanitizes and bounds entry names in errors', async () => {
        ...
        zipFile.close = vi.fn(() => zipFile.emit('close'));
        mockOpen.mockImplementation((_file, _options, callback) => callback(null, zipFile));
        const error = await extractZipArchive(path.join(tempDir, 'archive.zip'), destination).then(() => undefined, (reason) => reason);
...
//# sourceMappingURL=zip-extraction.test.js.map