...
describe('extensionSettings', () => {
    ...
    describe('maybePromptForSettings', () => {
        ...
        it('defers adding sensitive settings until commit', async () => {
            ...
            const keychain = new KeychainTokenStorage('Qwen Code Extensions test-ext 12345');
            const commit = await maybePromptForSettings(config, '12345', mockRequestSetting, undefined, undefined, path.join(tempWorkspaceDir, 'staged.env'), true);
            expect(await keychain.getSecret('API_KEY')).toBeNull();
            expect(await getScopedEnvContents(config, '12345', ExtensionSettingScope.USER)).toEqual({});
            fs.renameSync(path.join(tempWorkspaceDir, '.qwen-extension-settings.json'), path.join(extensionDir, '.qwen-extension-settings.json'));
            expect(await getScopedEnvContents(config, '12345', ExtensionSettingScope.USER)).toEqual({ API_KEY: 'mock-API_KEY' });
            await commit?.commit();
        ...
        it('isolates concurrent prepared sensitive settings snapshots', async () => {
            ...
            await maybePromptForSettings(config, '12345', async () => 'first-secret', undefined, undefined, path.join(firstDir, '.env'), true);
            await maybePromptForSettings(config, '12345', async () => 'second-secret', undefined, undefined, path.join(secondDir, '.env'), true);
            const firstSelector = JSON.parse(fs.readFileSync(path.join(firstDir, '.qwen-extension-settings.json'), 'utf8'));
            const secondSelector = JSON.parse(fs.readFileSync(path.join(secondDir, '.qwen-extension-settings.json'), 'utf8'));
        ...
        it('discards an uncommitted sensitive settings snapshot', async () => {
            ...
            const prepared = await maybePromptForSettings(config, '12345', async () => 'temporary-secret', undefined, undefined, path.join(stagingDir, '.env'), true);
            const selector = JSON.parse(fs.readFileSync(path.join(stagingDir, '.qwen-extension-settings.json'), 'utf8'));
            const storage = mockKeychainData['Qwen Code Extensions test-ext 12345'];
        ...
        it('deletes the previous sensitive settings snapshot after commit', async () => {
            ...
            await maybePromptForSettings(config, '12345', async () => 'old-secret', undefined, undefined, path.join(extensionDir, '.env'), true);
            const oldSelector = JSON.parse(fs.readFileSync(path.join(extensionDir, '.qwen-extension-settings.json'), 'utf8'));
            const storage = mockKeychainData['Qwen Code Extensions test-ext 12345'];
            storage[`${oldSelector.bundleKey}:override:API_KEY`] = 'old-override';
            const stagingDir = path.join(tempWorkspaceDir, 'replacement');
            fs.mkdirSync(stagingDir);
            const prepared = await maybePromptForSettings({ ...config, version: '2.0.0' }, '12345', async () => 'new-secret', config, { API_KEY: 'old-secret' }, path.join(stagingDir, '.env'), true);
            const newSelector = JSON.parse(fs.readFileSync(path.join(stagingDir, '.qwen-extension-settings.json'), 'utf8'));
            fs.copyFileSync(path.join(stagingDir, '.qwen-extension-settings.json'), path.join(extensionDir, '.qwen-extension-settings.json'));
            await prepared?.commit();
            expect(storage[oldSelector.bundleKey]).toBeUndefined();
            expect(storage[`${oldSelector.bundleKey}:override:API_KEY`]).toBeUndefined();
            expect(JSON.parse(storage[newSelector.bundleKey])).toEqual({
                API_KEY: 'old-secret',
            });
            await expect(getScopedEnvContents(config, '12345', ExtensionSettingScope.USER)).resolves.toEqual({ API_KEY: 'old-secret' });
        });
        it('does not fall back to stale legacy secrets when a selected bundle is missing', async () => {
            ...
            await maybePromptForSettings(config, '12345', async () => 'new-secret', undefined, undefined, path.join(extensionDir, '.env'), true);
            const selector = JSON.parse(fs.readFileSync(path.join(extensionDir, '.qwen-extension-settings.json'), 'utf8'));
            const storage = mockKeychainData['Qwen Code Extensions test-ext 12345'];
            storage['API_KEY'] = 'stale-secret';
            delete storage[selector.bundleKey];
            await expect(getScopedEnvContents(config, '12345', ExtensionSettingScope.USER)).rejects.toThrow('Stored extension settings bundle is missing.');
        });
        it('defers clearing sensitive settings until commit', async () => {
            ...
            const commit = await maybePromptForSettings({ name: 'test-ext', version: '2.0.0', settings: [] }, '12345', mockRequestSetting, previousConfig, { API_KEY: 'old-secret' }, path.join(tempWorkspaceDir, 'staged.env'), true);
        ...
        it('rejects invalid environment variable names before prompting', async () => {
            ...
            await expect(maybePromptForSettings(config, '12345', mockRequestSetting, undefined, undefined)).rejects.toThrow('Extension setting "envVar" must be a valid environment variable name.');
            expect(mockRequestSetting).not.toHaveBeenCalled();
        });
        it('rejects invalid previous environment variable names before mutation', async () => {
            ...
            await expect(maybePromptForSettings(config, '12345', mockRequestSetting, previousConfig, { OLD_KEY: 'previous' })).rejects.toThrow('Extension setting "envVar" must be a valid environment variable name.');
            expect(mockRequestSetting).not.toHaveBeenCalled();
        ...
        it('should only prompt for new settings', async () => {
            ...
            expect(await fsPromises.readFile(symlinkTarget, 'utf-8')).toBe('ORIGINAL');
        });
        it('should clear settings if new config has no settings', async () => {
            ...
            const userKeychain = new KeychainTokenStorage(`HopCode Extensions test-ext 12345`);
            ...
            expect(await fsPromises.readFile(symlinkTarget, 'utf-8')).toBe('VAR1=previous-VAR1');
    ...
    describe('getScopedEnvContents', () => {
        ...
        it('should return combined contents from workspace .env and keychain for WORKSPACE scope', async () => {
            ...
            const workspaceKeychain = new KeychainTokenStorage(`HopCode Extensions test-ext 12345 ${tempWorkspaceDir}`);
    ...
    describe('getEnvContents (merged)', () => {
        ...
        it('should merge user and workspace settings, with workspace taking precedence', async () => {
            ...
            const userEnvPath = path.join(extensionDir, EXTENSION_SETTINGS_FILENAME);
            await fsPromises.writeFile(userEnvPath, 'VAR1=user-value1\nVAR3=user-value3');
            const userKeychain = new KeychainTokenStorage(`HopCode Extensions test-ext ${extensionId}`);
            await userKeychain.setSecret('VAR2', 'user-secret2');
            // Workspace settings
            const workspaceEnvPath = path.join(tempWorkspaceDir, EXTENSION_SETTINGS_FILENAME);
            await fsPromises.writeFile(workspaceEnvPath, 'VAR1=workspace-value1');
            const workspaceKeychain = new KeychainTokenStorage(`HopCode Extensions test-ext ${extensionId} ${tempWorkspaceDir}`);
    ...
    describe('updateSetting', () => {
        ...
        it('should update a non-sensitive setting in USER scope', async () => {
            ...
            expect(fs.lstatSync(expectedEnvPath).isSymbolicLink()).toBe(false);
            expect(await fsPromises.readFile(symlinkTarget, 'utf-8')).toBe('VAR1=value1\n');
        });
        ...
        it('synchronizes legacy sensitive settings through the current backend', async () => {
            const previousStorageOverride = process.env['QWEN_CODE_FORCE_FILE_STORAGE'];
            process.env['QWEN_CODE_FORCE_FILE_STORAGE'] = 'true';
            try {
                await maybePromptForSettings(config, '12345', async () => 'initial-value2', undefined, undefined, path.join(extensionDir, '.env'));
            }
            finally {
            ...
            await updateSetting(config, '12345', 'VAR2', async () => 'new-value2', ExtensionSettingScope.USER);
            await fsPromises.rm(path.join(extensionDir, '.qwen-extension-settings.json'));
            await expect(getScopedEnvContents(config, '12345', ExtensionSettingScope.USER)).resolves.toEqual({
                VAR1: 'initial-value2',
                VAR2: 'new-value2',
            });
        });
        ...
        it('surfaces authoritative sensitive setting write failures', async () => {
            ...
            await expect(updateSetting(config, '12345', 'VAR2', mockRequestSetting, ExtensionSettingScope.USER)).rejects.toThrow('write failed');
        });
        it('does not lose concurrent user-scope sensitive setting updates', async () => {
            ...
            await maybePromptForSettings(sensitiveConfig, '12345', async (setting) => `initial-${setting.envVar}`, undefined, undefined, path.join(extensionDir, '.env'), true);
            await Promise.all([
                updateSetting(sensitiveConfig, '12345', 'VAR2', async () => 'updated-VAR2', ExtensionSettingScope.USER),
                updateSetting(sensitiveConfig, '12345', 'VAR3', async () => 'updated-VAR3', ExtensionSettingScope.USER),
            ]);
            await expect(getScopedEnvContents(sensitiveConfig, '12345', ExtensionSettingScope.USER)).resolves.toEqual({
                VAR2: 'updated-VAR2',
                VAR3: 'updated-VAR3',
            });
        });
