...
import { convertGeminiOrClaudeExtension, SUPPORTED_EXTENSION_MANIFESTS, } from './extension-converter.js';
...
function restrictGitEnvironment(git, networkPolicy) {
 ...
 */
export async function cloneFromGit(installMetadata, destination, signal) {
    ...
    if (installMetadata?.type === 'local') {
        ...
        try {
            ...
            if (isSupportedArchivePath(installMetadata.source)) {
                ...
                signal?.throwIfAborted();
                const converted = await convertGeminiOrClaudeExtension(tempDir, installMetadata.pluginName, installMetadata.networkPolicy, signal);
...
function hasSupportedExtensionSourceManifest(rootPath) {
    return SUPPORTED_EXTENSION_MANIFESTS.some((manifestPath) => fs.existsSync(path.join(rootPath, manifestPath)));
}
