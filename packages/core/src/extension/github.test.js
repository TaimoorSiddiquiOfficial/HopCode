...
import { checkForExtensionUpdate, cloneFromGit, downloadFromArchiveUrl, downloadFromGitHubRelease, extractArchiveFile, extractFile, findReleaseAsset, isSupportedArchivePath, isSupportedArchiveUrl, parseGitHubRepoForReleases, } from './github.js';
...
describe('git extension helpers', () => {
    ...
    function createResponse(responseBody, statusCode = 200, headers = {}) {
    ...
    function callResponseCallback(_options, callback, response) {
    ...
    async function createZipBuffer(tempDir, entries) {
    ...
    describe('cloneFromGit', () => {
        ...
        it('should clone, fetch and checkout a repo', async () => {
            ...
            expect(mockGit.fetch).toHaveBeenCalledWith('http://my-repo.com', 'my-ref');
        ...
        it('pins public HTTPS Git traffic and disables redirects and proxies', async () => {
            ...
            expect(mockGit.env).toHaveBeenCalledWith(expect.objectContaining({
                GIT_CONFIG_NOSYSTEM: '1',
                GIT_CONFIG_GLOBAL: expect.any(String),
            }));
            expect(mockGit.env.mock.calls[0]?.[0]).not.toHaveProperty('GIT_CONFIG_COUNT');
        ...
        it('rejects SSH Git traffic under the public network policy', async () => {
            await expect(cloneFromGit({
                source: 'git@github.com:owner/repo.git',
                type: 'git',
                networkPolicy: 'public',
            }, '/dest')).rejects.toThrow('must use HTTPS');
        ...
        it('should redact URL credentials in clone failures', async () => {
            ...
            expect(message).toContain('https://***REDACTED***@my-repo.com/org/repo.git');
        ...
        it('should redact URL credentials in clone failure causes', async () => {
            ...
            const destination = '/dest';
            mockGit.clone.mockRejectedValue(new Error("fatal: Authentication failed for 'https://user:token@my-repo.com/org/repo.git'"));
            let message = '';
            try {
                await cloneFromGit(installMetadata, destination);
            }
        ...
        it('should preserve clone failure cause diagnostics while redacting its message', async () => {
            ...
            const destination = '/dest';
            const gitError = Object.assign(new Error("fatal: Authentication failed for 'https://user:token@my-repo.com/org/repo.git'"), {
                code: 'ENOTFOUND',
                task: { commands: ['clone'] },
            });
            ...
            expect(cause.message).toContain('https://***REDACTED***@my-repo.com/org/repo.git');
            expect(cause.message).not.toContain('user');
            expect(cause.code).toBe('ENOTFOUND');
            expect(cause.task).toEqual({
                commands: ['clone'],
            });
        });
        ...
        it('preserves abort errors raised after a git operation', async () => {
            ...
            await expect(cloneFromGit(installMetadata, '/dest', controller.signal)).rejects.toBe(reason);
        });
        it('preserves a git failure when the signal aborts as a side effect', async () => {
            ...
            await expect(cloneFromGit({ source: 'http://my-repo.com', type: 'git' }, '/dest', controller.signal)).rejects.toThrow('Failed to clone Git repository from http://my-repo.com authentication failed');
        });
    });
    describe('checkForExtensionUpdate', () => {
        ...
        it('should convert a local Gemini archive before checking for updates', async () => {
            const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'local-archive-update-test-'));
            try {
                ...
                const mockManager = {
                    loadExtensionConfig: vi.fn(({ extensionDir }) => {
                        expect(fsSync.existsSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME))).toBe(true);
                        return JSON.parse(fsSync.readFileSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8'));
                    }),
                };
                const result = await checkForExtensionUpdate(extension, mockManager);
                expect(result).toBe(ExtensionUpdateState.UPDATE_AVAILABLE);
        ...
        it('should propagate an abort observed after extracting a local archive', async () => {
            const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'local-archive-abort-test-'));
            try {
                const archivePath = path.join(tempDir, 'qwen-extension.zip');
                const archive = await createZipBuffer(tempDir, [
                    {
                ...
                await expect(checkForExtensionUpdate(extension, mockManager, signal)).rejects.toBe(abortError);
        ...
        it('should clean up a converted local archive when aborted after conversion', async () => {
            const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'local-converted-archive-abort-test-'));
            ...
            try {
                ...
                await expect(checkForExtensionUpdate(extension, {}, signal)).rejects.toBe(abortError);
        ...
        it('should return UPDATE_AVAILABLE for archive URL extension with different version', async () => {
            const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'archive-url-update-test-'));
            try {
                const archive = await createZipBuffer(tempDir, [
                    {
                        name: EXTENSIONS_CONFIG_FILENAME,
    ...
    describe('downloadFromGitHubRelease', () => {
        let tempDir;
        beforeEach(async () => {
            tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'github-release-archive-test-'));
        });
        afterEach(async () => {
            await fs.rm(tempDir, { recursive: true, force: true });
        });
        it('preserves the abort reason for release metadata response errors', async () => {
            ...
            await expect(downloadFromGitHubRelease({ source: 'owner/repo', type: 'github-release' }, tempDir, controller.signal)).rejects.toBe(abortReason);
        });
        ...
        it('times out release metadata requests', async () => {
            ...
            try {
                const download = downloadFromGitHubRelease({ source: 'owner/repo', type: 'github-release' }, tempDir);
                const outcome = download.catch((error) => error);
        ...
        it('rejects invalid release metadata JSON', async () => {
            ...
            await expect(downloadFromGitHubRelease({ source: 'owner/repo', type: 'github-release' }, tempDir)).rejects.toBeInstanceOf(SyntaxError);
        });
        it('should explain when a release archive is missing an extension manifest', async () => {
            ...
            mockHttpsResponses(JSON.stringify({
                assets: [
                    {
                        name: 'extension.zip',
            ...
            await expect(downloadFromGitHubRelease({
                source: 'owner/repo',
                type: 'git',
            }, tempDir)).rejects.toThrow('Extension archive is missing a supported extension manifest.');
        });
        it('should download and extract an archive URL', async () => {
            ...
            await expect(fs.readFile(path.join(tempDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('archive-extension');
        });
        it.each([307, 308])('should follow %i redirects with relative locations', async (statusCode) => {
            const archive = await createZipBuffer(tempDir, [
                {
                    name: EXTENSIONS_CONFIG_FILENAME,
                    content: JSON.stringify({
            ...
            expect(mockHttpsGet.mock.calls[1][0].toString()).toBe('https://example.com/download/extension.zip');
        });
        it('should reject malformed redirect locations without throwing', async () => {
            ...
            await expect(downloadFromArchiveUrl({
                source: 'https://example.com/releases/extension.zip',
                type: 'archive-url',
            }, tempDir)).rejects.toThrow('Invalid redirect URL:');
        ...
        it('should drain non-200 archive URL responses before rejecting', async () => {
            ...
            await expect(downloadFromArchiveUrl({
                source: 'https://example.com/releases/extension.zip',
                type: 'archive-url',
            }, tempDir)).rejects.toThrow('Request failed with status code 404');
            expect(resumeSpy).toHaveBeenCalled();
        });
        it('should time out archive URL downloads', async () => {
            ...
            timeoutCallback?.();
            await expect(download).rejects.toThrow('Timed out downloading extension archive');
            expect(request.destroy).toHaveBeenCalled();
        });
        it('does not start an archive request when DNS outlives the deadline', async () => {
            ...
            try {
                const outcome = downloadFromArchiveUrl({
                    ...
                    type: 'archive-url',
                    networkPolicy: 'public',
                }, tempDir).catch((error) => error);
                await vi.advanceTimersByTimeAsync(120_000);
                await expect(outcome).resolves.toMatchObject({
                    message: 'Failed to download archive from https://packages.example/extension.zip: Timed out downloading extension archive',
                });
        ...
        it('should reject oversized archive URL downloads', async () => {
            ...
            dataHandler?.({ length: 101 * 1024 * 1024 });
            await expect(download).rejects.toThrow('Extension archive download exceeded maximum size');
        ...
        it('should not include the GitHub token for archive URL downloads', async () => {
            ...
            const requestOptions = mockHttpsGet.mock.calls[0][1];
        ...
        it('should not forward the GitHub token to cross-host redirects', async () => {
            ...
            mockHttpsGet
                .mockImplementationOnce(((_url, _options, callback) => {
                const response = createResponse(JSON.stringify({
                    assets: [
                        {
                            name: 'extension.zip',
            ...
            const originalDownloadOptions = mockHttpsGet.mock.calls[1][1];
            const redirectedDownloadOptions = mockHttpsGet.mock.calls[2][1];
        ...
        it('should reject same-host scheme downgrade redirects before sending a token', async () => {
            ...
            mockHttpsGet
                ...
                .mockImplementationOnce(((_url, options, callback) => {
                const response = createResponse(undefined, 302, {
                    location: 'http://github.com/owner/repo/releases/download/v1.0.0/extension.zip',
                });
                callResponseCallback(options, callback, response);
                return createRequestMock();
            }));
            try {
                await expect(downloadFromGitHubRelease({ source: 'owner/repo', type: 'github-release' }, tempDir)).rejects.toThrow('Unsupported download URL protocol: http:');
            }
            finally {
                if (originalToken === undefined) {
                    delete process.env['GITHUB_TOKEN'];
        ...
        it('should stop following redirect loops', async () => {
            ...
            await expect(downloadFromArchiveUrl({
                source: 'https://example.com/extension.zip',
                type: 'archive-url',
            }, tempDir)).rejects.toThrow('Too many redirects while downloading extension archive');
        });
        it('should reject redirects without a location and clear the timeout', async () => {
            ...
            try {
                await expect(downloadFromArchiveUrl({
                    source: 'https://example.com/extension.zip',
                    type: 'archive-url',
                }, tempDir)).rejects.toThrow('Redirect response missing location header');
                expect(resumeSpy).toHaveBeenCalled();
                expect(vi.getTimerCount()).toBe(0);
            }
            finally {
        ...
        it('should reject when an archive URL response stream errors', async () => {
            ...
            await expect(downloadFromArchiveUrl({
                source: 'https://example.com/extension.zip',
                type: 'archive-url',
            }, tempDir)).rejects.toThrow('Failed to download archive from https://example.com/extension.zip: connection lost');
        });
        it('should explain when an archive URL cannot be extracted', async () => {
            ...
            await expect(downloadFromArchiveUrl({
                source: 'https://example.com/extension.zip',
                type: 'archive-url',
            }, tempDir)).rejects.toThrow('Extension archive could not be extracted. Make sure it is a valid .zip or .tar.gz file.');
        });
        it('should explain when a local archive is missing an extension manifest', async () => {
            ...
            await expect(extractArchiveFile(invalidArchivePath, tempDir)).rejects.toThrow('Extension archive is missing a supported extension manifest.');
        });
        it('should extract and flatten a tar.gz archive with a wrapped extension directory', async () => {
            ...
            await fs.writeFile(path.join(wrappedDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({
            ...
            await expect(fs.readFile(path.join(tempDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('tar-wrapped-extension');
        });
        it('should flatten wrapped archives when the archive file is in the destination', async () => {
            ...
            await fs.writeFile(archivePath, archive);
            await extractArchiveFile(archivePath, tempDir);
            await expect(fs.readFile(path.join(tempDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('wrapped-with-readme-extension');
            await expect(fs.readFile(path.join(tempDir, 'README.md'), 'utf-8')).resolves.toBe('readme');
        ...
        it('should not flatten when the archive root already has a manifest', async () => {
            ...
            await fs.writeFile(archivePath, archive);
            await extractArchiveFile(archivePath, tempDir);
            await expect(fs.readFile(path.join(tempDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('root-extension');
            await expect(fs.readFile(path.join(tempDir, 'wrapped', EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('wrapped-extension');
        });
        it('should reject flattening when wrapper contents collide with root files', async () => {
            ...
            await fs.rm(archiveBuildDir, { recursive: true, force: true });
            await fs.writeFile(archivePath, archive);
            await expect(extractArchiveFile(archivePath, tempDir)).rejects.toThrow('Extension archive cannot be flattened because "README.md" exists at both the archive root and inside "wrapped".');
            await expect(fs.readFile(path.join(tempDir, 'README.md'), 'utf-8')).resolves.toBe('root readme');
            await expect(fs.readFile(path.join(tempDir, 'wrapped', EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('wrapped-extension');
        });
        it('should not flatten archives with multiple top-level entries', async () => {
            ...
            await fs.writeFile(archivePath, archive);
            await expect(extractArchiveFile(archivePath, tempDir)).rejects.toThrow('Extension archive is missing a supported extension manifest.');
            await expect(fs.readFile(path.join(tempDir, 'wrapped', EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('wrapped-extension');
        });
        it('should not flatten archives without a top-level directory', async () => {
            ...
            await fs.writeFile(archivePath, archive);
            await extractArchiveFile(archivePath, tempDir);
            await expect(fs.readFile(path.join(tempDir, EXTENSIONS_CONFIG_FILENAME), 'utf-8')).resolves.toContain('files-only-extension');
        });
        it('should not flatten a top-level directory without a supported manifest', async () => {
            ...
            await fs.writeFile(archivePath, archive);
            await expect(extractArchiveFile(archivePath, tempDir)).rejects.toThrow('Extension archive is missing a supported extension manifest.');
            await expect(fs.readFile(path.join(tempDir, 'wrapped', 'README.md'), 'utf-8')).resolves.toBe('not an extension');
        });
        it('should identify supported archive paths and URLs', () => {
            ...
            expect(isSupportedArchiveUrl('https://example.com/extension.zip')).toBe(true);
            expect(isSupportedArchiveUrl('http://example.com/extension.zip')).toBe(false);
            expect(isSupportedArchiveUrl('https://example.com/extension.tar.gz')).toBe(true);
            expect(isSupportedArchiveUrl('git@github.com:owner/repo.git')).toBe(false);
        });
    });
    ...
    describe('parseGitHubRepoForReleases', () => {
        ...
        it('should only strip a trailing .git, not an embedded one', () => {
            const { repo } = parseGitHubRepoForReleases('owner/my.gitignore-tools.git');
        ...
        it('should redact URL credentials in invalid source errors', () => {
            ...
            expect(message).toContain('https://***REDACTED***@example.com/owner/repo.git');
            expect(message).not.toContain('user');
            expect(message).not.toContain('token');
        });
    ...
    describe('extractFile', () => {
        ...
        async function waitForFileData(filePath) {
        ...
        it('should cancel while extracting a tar archive', async () => {
            ...
            const extraction = extractFile(archivePath, extractionDest, controller.signal);
            try {
            ...
            expect(await waitForStableFileSize(extractedFilePath)).toBeLessThan(content.length);
        });
        it.skipIf(process.platform === 'win32')('should reject symlink entries in tar archives', async () => {
            ...
            await expect(extractFile(archivePath, extractionDest)).rejects.toThrow('Tar archive contains unsupported link entry: escape-link');
            await expect(fs.lstat(path.join(extractionDest, 'escape-link'))).rejects.toThrow();
        });
        ...
        it('should reject symlink entries in zip archives', async () => {
            ...
            await archive.finalize();
            await streamFinished;
            await expect(extractFile(archivePath, extractionDest)).rejects.toThrow('Zip archive contains unsupported symbolic link entry: escape-link');
            await expect(fs.lstat(path.join(extractionDest, 'escape-link'))).rejects.toThrow();
        });
        it.skipIf(process.platform === 'win32')('should reject zip extraction through an existing symlink', async () => {
            ...
            await archive.finalize();
            await streamFinished;
            await expect(extractFile(archivePath, extractionDest)).rejects.toThrow('Refusing to extract through non-directory path');
            await expect(fs.lstat(path.join(outsideDir, 'file.txt'))).rejects.toThrow();
        });
