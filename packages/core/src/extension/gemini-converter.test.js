...
import { convertGeminiToHopCodeConfig, isGeminiExtensionConfig, } from './gemini-converter.js';
// Mock fs module
vi.mock('node:fs', async (importOriginal) => {
    ...
    return {
        ...
        realpathSync: vi.fn((p) => nodePath.resolve(p)),
    };
});
describe('convertGeminiToHopCodeConfig', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    afterEach(() => {
    ...
    it('should convert basic Gemini config from directory', () => {
        ...
        const result = convertGeminiToHopCodeConfig(mockDir);
    ...
    it('should throw error for missing name', () => {
        ...
        expect(() => convertGeminiToHopCodeConfig(mockDir)).toThrow('Gemini extension config must have name and version fields');
    });
    ...
    it('throws when gemini-extension.json resolves through a symlink outside the extension', () => {
        ...
        vi.mocked(fs.realpathSync).mockReturnValueOnce('/outside/extension/gemini-extension.json');
        expect(() => convertGeminiToQwenConfig(mockDir)).toThrow(/resolves through a symlink outside the extension/);
    });
});
