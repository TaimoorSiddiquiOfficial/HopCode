...
import { ExtensionConflictError, ExtensionStore, ExtensionStoreCorruptError, } from './extension-store.js';
describe('ExtensionStore', () => {
    ...
    const makeStore = () => new ExtensionStore({ extensionsDir, storeDir, enablementPath });
    const readQuarantinedJournal = async (journal) => {
        const prefix = `${path.basename(journal)}.corrupt-`;
        const quarantined = (await fsp.readdir(path.dirname(journal))).find((name) => name.startsWith(prefix));
        expect(quarantined).toBeDefined();
        return await fsp.readFile(path.join(path.dirname(journal), quarantined), 'utf8');
    };
    it('imports V1 rules without materializing workspace overrides', async () => {
        await fsp.writeFile(enablementPath, JSON.stringify({
        ...
        expect(store.getActivation(snapshot, 'a'.repeat(64), 'demo', '/work/disabled')).toMatchObject({ effective: 'disabled', source: 'legacy_path_rule' });
        expect(store.getActivation(snapshot, 'a'.repeat(64), 'demo', '/work/enabled')).toMatchObject({ effective: 'enabled', source: 'legacy_path_rule' });
    });
    it('preserves exact workspace overrides when the global default changes', async () => {
        ...
        await store.setWorkspaceActivation({ id, name: 'demo' }, '/workspace/a', 'enabled');
        const snapshot = await store.setDefaultActivation({ id, name: 'demo' }, 'disabled');
        expect(snapshot.generation).toBe(2);
        expect(snapshot.extensions[id]?.workspaceOverrides).toEqual({
            '/workspace/a': 'enabled',
        });
        expect(store.getActivation(snapshot, id, 'demo', '/workspace/a')).toMatchObject({ effective: 'enabled', source: 'workspace_override' });
    });
    it('uses an inherit mask when clearing an override matched by a legacy rule', async () => {
        await fsp.writeFile(enablementPath, JSON.stringify({ demo: { overrides: ['!/workspace/*'] } }));
        const store = makeStore();
        const id = 'c'.repeat(64);
        await store.ensureInitialized([{ id, name: 'demo' }]);
        const snapshot = await store.clearWorkspaceActivation({ id, name: 'demo' }, '/workspace/a');
        expect(snapshot.extensions[id]?.workspaceOverrides).toEqual({
            '/workspace/a': 'inherit',
        });
    ...
    it('serializes writes from independent store instances without losing updates', async () => {
        ...
        await Promise.all([
            first.setWorkspaceActivation({ id, name: 'demo' }, '/workspace/a', 'enabled'),
            second.setWorkspaceActivation({ id, name: 'demo' }, '/workspace/b', 'disabled'),
        ]);
    ...
    it('preserves a committed result when lock release reports an error', async () => {
        ...
        try {
            await expect(store.setDefaultActivation(identity, 'disabled')).resolves.toMatchObject({ generation: 1 });
        }
        finally {
            lockSpy.mockRestore();
        }
    ...
    it('serializes mutations from two Node processes sharing QWEN_HOME', async () => {
        ...
        const runChild = async (workspacePath, activation) => {
            const source = `
      ...
      `;
            await new Promise((resolve, reject) => {
                const child = spawn(process.execPath, ['--import', 'tsx', '--input-type=module', '--eval', source], { cwd: process.cwd(), stdio: ['ignore', 'ignore', 'pipe'] });
                let stderr = '';
    ...
    it.runIf(process.platform !== 'win32')('uses one workspace key for symlink and real paths', async () => {
        const store = makeStore();
        ...
        await store.ensureInitialized([{ id, name: 'demo' }]);
        const snapshot = await store.setWorkspaceActivation({ id, name: 'demo' }, linkedWorkspace, 'disabled');
        expect(snapshot.extensions[id]?.workspaceOverrides).toEqual({
            [fs.realpathSync.native(realWorkspace)]: 'disabled',
        });
        expect(store.getActivation(snapshot, id, 'demo', realWorkspace)).toMatchObject({
            effective: 'disabled',
            source: 'workspace_override',
        });
    });
    it.runIf(process.platform !== 'win32')('matches legacy rules against symlink and canonical workspace paths', async () => {
        ...
        expect(store.getActivation(snapshot, identity.id, identity.name, linkedWorkspace)).toMatchObject({
            effective: 'disabled',
            source: 'legacy_path_rule',
        });
        snapshot = await store.setWorkspaceActivation(identity, linkedWorkspace, 'enabled');
        expect(store.getActivation(snapshot, identity.id, identity.name, linkedWorkspace)).toMatchObject({
            effective: 'enabled',
            source: 'workspace_override',
        });
        snapshot = await store.clearWorkspaceActivation(identity, linkedWorkspace);
        expect(store.getActivation(snapshot, identity.id, identity.name, linkedWorkspace)).toMatchObject({
            effective: 'enabled',
            source: 'default',
        });
    ...
    it('writes a V1 projection after every policy mutation', async () => {
        ...
        await store.setWorkspaceActivation({ id, name: 'demo' }, '/workspace/a', 'enabled');
        const projection = JSON.parse(await fsp.readFile(enablementPath, 'utf8'));
    ...
    it('repairs an older V1 projection without changing generation', async () => {
        ...
        await store.ensureInitialized([{ id, name: 'demo' }]);
        const changed = await store.setDefaultActivation({ id, name: 'demo' }, 'disabled');
    ...
    it('fails closed when state and a different V1 projection have equal mtimes', async () => {
        ...
        await expect(store.ensureInitialized([{ id, name: 'demo' }])).rejects.toBeInstanceOf(ExtensionStoreCorruptError);
        expect(JSON.parse(await fsp.readFile(enablementPath, 'utf8'))).toEqual({});
    });
    it('keeps V2 reads available when an older V1 projection cannot be repaired', async () => {
        ...
        const projectionAgeSpy = vi
            .spyOn(store, 'legacyProjectionIsNewerThanState')
    ...
    it('repairs an unchanged newer V1 projection without changing generation', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 10));
        await fsp.writeFile(enablementPath, JSON.stringify({ stale: { overrides: ['/workspace/unused'] } }));
        const repaired = await store.ensureInitialized([{ id, name: 'demo' }]);
    ...
    it('merges newly discovered extensions while repairing an older V1 projection', async () => {
        ...
        await fsp.writeFile(enablementPath, JSON.stringify({ stale: { overrides: ['!/workspace/*'] } }));
    ...
    it('preserves artifact generation across a sequential downgrade write', async () => {
        ...
        expect(imported.extensions[identity.id]?.artifactGeneration).toBe(installed.extensions[identity.id]?.artifactGeneration);
    ...
    it('preserves V2 activation policy across a sequential downgrade write', async () => {
        ...
        await store.setWorkspaceActivation(identity, '/workspace/enabled', 'enabled');
        await new Promise((resolve) => setTimeout(resolve, 10));
        await fsp.writeFile(enablementPath, JSON.stringify({ demo: { overrides: ['!/workspace/legacy/*'] } }));
        const imported = await store.ensureInitialized([identity]);
        expect(imported.extensions[identity.id]).toMatchObject({
    ...
    it('fails closed when the V2 state is corrupt', async () => {
        ...
        const store = makeStore();
        await expect(store.readSnapshot()).rejects.toBeInstanceOf(ExtensionStoreCorruptError);
    ...
    it('commits an installed artifact and its initial activation together', async () => {
        ...
        await expect(fsp.readFile(path.join(extensionsDir, 'demo', 'qwen-extension.json'), 'utf8')).resolves.toBe('{}');
    ...
    it('preserves the original error when rollback also fails', async () => {
        ...
        vi.spyOn(internals, 'writeSnapshotUnlocked').mockRejectedValueOnce(primaryError);
    ...
    it('changes artifact generation only for artifact commits', async () => {
        ...
        expect(activated.extensions[identity.id]?.artifactGeneration).toBe(installed.generation);
        ...
        expect(updated.extensions[identity.id]?.artifactGeneration).toBe(updated.generation);
    });
    it('does not recreate activation policy after uninstall', async () => {
        ...
        await expect(store.setDefaultActivation(identity, 'disabled')).rejects.toMatchObject({ code: 'extension_conflict' });
        await expect(store.readSnapshot()).resolves.toMatchObject({
            extensions: {},
        });
    });
    it('rejects installing a renamed extension with an existing id', async () => {
        ...
        await expect(fsp.readFile(path.join(extensionsDir, original.name, 'version'), 'utf8')).resolves.toBe('one');
    ...
    it('rejects a stale prepared update without replacing the artifact', async () => {
        ...
        await expect(fsp.readFile(path.join(destination, 'version'), 'utf8')).resolves.toBe('two');
    });
    it('rebases prepared updates for different artifacts', async () => {
        ...
        await store.commitArtifact({
            ...
            expectedArtifactGeneration: firstInstalled.extensions[first.id].artifactGeneration,
        });
        await store.commitArtifact({
            operation: 'update',
            identity: second,
            ...
            expectedArtifactGeneration: secondInstalled.extensions[second.id].artifactGeneration,
        });
        await expect(fsp.readFile(path.join(extensionsDir, first.name, 'version'), 'utf8')).resolves.toBe('first-updated');
        await expect(fsp.readFile(path.join(extensionsDir, second.name, 'version'), 'utf8')).resolves.toBe('second-updated');
    });
    it('replaces stale policy state when its artifact is absent', async () => {
        ...
        await expect(fsp.readFile(path.join(destination, 'version'), 'utf8')).resolves.toBe('new artifact');
    });
    ...
    it('atomically replaces an artifact while preserving activation policy', async () => {
        ...
        expect(await fsp.readFile(path.join(destination, 'version'), 'utf8')).toBe('new');
    ...
    it('allows uninstalling an extension from a snapshot with duplicate names', async () => {
        ...
        await fsp.writeFile(path.join(storeDir, 'state.json'), JSON.stringify(snapshot));
    ...
    it('rolls back an artifact-swapped transaction before the commit point', async () => {
        ...
        const journal = path.join(storeDir, 'transactions', `${transactionId}.json`);
        await fsp.mkdir(destination);
        ...
        await fsp.writeFile(journal, JSON.stringify({
            version: 1,
            transactionId,
            operation: 'update',
            phase: 'artifact_swapped',
            destinationDirectory: destination,
            stagingDirectory: path.join(storeDir, 'staging', 'recover-before-commit'),
        ...
        await store.ensureInitialized([identity]);
        expect(await fsp.readFile(path.join(destination, 'version'), 'utf8')).toBe('old');
    ...
    it.each([
        ...
        if (scenario.destinationVersion) {
            await fsp.mkdir(destination);
            await fsp.writeFile(path.join(destination, 'version'), scenario.destinationVersion);
        }
        ...
        if (scenario.expectedDestinationVersion) {
            await expect(fsp.readFile(path.join(destination, 'version'), 'utf8')).resolves.toBe(scenario.expectedDestinationVersion);
        }
        else {
            expect(fs.existsSync(destination)).toBe(false);
        }
    ...
    it('keeps an artifact when state reached the target generation before the journal phase', async () => {
        ...
        await fsp.mkdir(backup);
        await fsp.writeFile(path.join(backup, 'version'), 'old');
        await fsp.writeFile(path.join(storeDir, 'state.json'), JSON.stringify(targetSnapshot));
        await fsp.writeFile(journal, JSON.stringify({
            version: 1,
            transactionId,
            operation: 'update',
    ...
    it('finishes cleanup after a committed transaction', async () => {
        ...
        await store.ensureInitialized([identity]);
        const targetSnapshot = await store.setDefaultActivation(identity, 'disabled');
        ...
        await fsp.writeFile(journal, JSON.stringify({
            ...
            destinationDirectory: destination,
            stagingDirectory: path.join(storeDir, 'staging', 'recover-after-commit'),
            backupDirectory: backup,
            previousGeneration: 0,
            targetGeneration: 1,
            targetSnapshot,
    ...
    it('keeps committed cleanup failures from blocking store operations', async () => {
        ...
        try {
            ...
            await expect(store.setDefaultActivation(identity, 'enabled')).resolves.toMatchObject({ generation: 2 });
    ...
    it.each(['destination', 'backup', 'staging', 'transaction-id'])('quarantines a journal with a hostile %s path', async (kind) => {
        const store = makeStore();
        ...
        await fsp.writeFile(journal, JSON.stringify({
            version: 1,
            transactionId: kind === 'transaction-id' ? 'different-id' : transactionId,
            operation: 'update',
            phase: 'artifact_swapped',
            destinationDirectory: kind === 'destination'
                ? outside
                : path.join(extensionsDir, identity.name),
            stagingDirectory: kind === 'staging'
                ? outside
                : path.join(storeDir, 'staging', transactionId),
            backupDirectory: kind === 'backup'
                ? outside
    ...
    it.each(['corrupt', 'missing'])('recovers committed state from a journal when state.json is %s', async (stateCondition) => {
        const store = makeStore();
        ...
        await fsp.writeFile(journal, JSON.stringify({
            ...
            phase: 'state_committed',
            destinationDirectory: destination,
            stagingDirectory: path.join(storeDir, 'staging', 'recover-corrupt-commit'),
            backupDirectory: backup,
            previousGeneration: 0,
            targetGeneration: 1,
            targetSnapshot,
        ...
        expect(recovered.generation).toBe(1);
        expect(recovered.extensions[identity.id]?.defaultActivation).toBe('disabled');
        expect(fs.existsSync(journal)).toBe(false);
    });
    it('rolls back an artifact-swapped transaction when current state is corrupt', async () => {
        ...
        const recovered = await store.readSnapshot();
        expect(recovered.generation).toBe(0);
        expect(recovered.extensions[identity.id]?.defaultActivation).toBe('enabled');
        await expect(fsp.readFile(path.join(destination, 'version'), 'utf8')).resolves.toBe('old');
        expect(fs.existsSync(journal)).toBe(false);
    });
    it.each(['corrupt', 'missing'])('recovers state and projection from state.previous.json when state.json is %s', async (stateCondition) => {
        const store = makeStore();
        ...
        await fsp.writeFile(enablementPath, JSON.stringify({ demo: { overrides: ['!/*'] } }));
        const recovered = await store.ensureInitialized([identity]);
        expect(recovered.generation).toBe(0);
        expect(recovered.extensions[identity.id]?.defaultActivation).toBe('enabled');
        expect(JSON.parse(await fsp.readFile(enablementPath, 'utf8'))).toEqual({});
    ...
    it('fails closed when current and previous state are corrupt', async () => {
        ...
        await store.setDefaultActivation(identity, 'disabled');
        await fsp.writeFile(path.join(storeDir, 'state.json'), '{broken');
        await fsp.writeFile(path.join(storeDir, 'state.previous.json'), '{also-broken');
        await expect(store.ensureInitialized([identity])).rejects.toBeInstanceOf(ExtensionStoreCorruptError);
    });
});
//# sourceMappingURL=extension-store.test.js.map