...
describe('ExtensionStorage', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(Storage.getUserExtensionsDir).mockReturnValue(path.join(mockHomeDir, '.hopcode', 'extensions'));
    ...
    it('should return the correct extension directory', () => {
        const expectedDir = path.join(mockHomeDir, '.hopcode', 'extensions', extensionName);
        expect(storage.getExtensionDir()).toBe(expectedDir);
    });
    it('should return the correct config path', () => {
        const expectedPath = path.join(mockHomeDir, '.hopcode', 'extensions', extensionName, EXTENSIONS_CONFIG_FILENAME);
        expect(storage.getConfigPath()).toBe(expectedPath);
    });
    it('should return the correct env file path', () => {
        const expectedPath = path.join(mockHomeDir, '.hopcode', 'extensions', extensionName, EXTENSION_SETTINGS_FILENAME);
        expect(storage.getEnvFilePath()).toBe(expectedPath);
    });
    it('should return the correct user extensions directory', () => {
        const expectedDir = path.join(mockHomeDir, '.hopcode', 'extensions');
    ...
    it('should create a temporary directory', async () => {
        ...
        expect(fs.promises.mkdtemp).toHaveBeenCalledWith(path.join('/tmp', 'hopcode-extension'));
