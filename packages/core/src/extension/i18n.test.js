...
import { resolveLocalizableString, resolveExtensionConfigLocale, } from './i18n.js';
...
describe('resolveExtensionConfigLocale', () => {
    ...
    it('should resolve locale map description', () => {
        ...
        expect(resolveExtensionConfigLocale(raw, 'zh').description).toBe('中文描述');
        expect(resolveExtensionConfigLocale(raw, 'en').description).toBe('English desc');
    });
    it('should resolve locale map displayName', () => {
        ...
        expect(resolveExtensionConfigLocale(raw, 'zh').displayName).toBe('我的扩展');
        expect(resolveExtensionConfigLocale(raw, 'en').displayName).toBe('My Extension');
    });
