...
export const EXTENSIONS_DIRECTORY_NAME = path.join(HOPCODE_DIR, 'extensions');
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            try {
                ...
                // Replace references to ".claude" directory with ".hopcode" in markdown files
                ...
                const updatedMdContent = syntaxUpdatedContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.hopcode$2');
    ...
    try {
        ...
        for (const file of scriptFiles) {
            ...
            try {
                ...
                // Replace references to ".claude" directory with ".hopcode" in shell scripts
                // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
                // Avoid matching URLs, comments, or string literals containing .claude
                const finalScriptContent = adaptedScriptContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.hopcode$2');
                // Only write if content was actually changed
                if (finalScriptContent !== content) {
                    fs.writeFileSync(filePath, finalScriptContent, 'utf8');
                    debugLogger.debug(`Updated transcript format and replaced .claude with .hopcode in shell script: ${filePath}`);
                }
            }
            catch (error) {
