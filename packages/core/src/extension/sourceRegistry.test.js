...
import { SourceRegistryStore, parseExtensionSourceType, discoverPlugins, } from './sourceRegistry.js';
...
describe('SourceRegistryStore', () => {
    ...
    it('quarantines a corrupt registry (parse error) to a .corrupted sibling', () => {
        ...
        expect(fs.readFileSync(`${filePath}.corrupted`, 'utf-8')).toBe('[ not valid json');
...
describe('discoverPlugins', () => {
    ...
    const config = (name, plugins) => ({
        name,
        owner: { name: 'o', email: 'e' },
        plugins,
    });
    it('flattens plugins across sources and marks installed ones', async () => {
        vi.mocked(loadMarketplaceConfigFromSource).mockImplementation(async (source) => {
    ...
    it('passes the public network policy to marketplace loading', async () => {
        vi.mocked(loadMarketplaceConfigFromSource).mockResolvedValue(config('Skills', []));
        await discoverPlugins([{ name: 'Skills', source: 'anthropics/skills', type: 'github' }], new Set(), 'public');
        expect(loadMarketplaceConfigFromSource).toHaveBeenCalledWith('anthropics/skills', 'public');
    });
    it('surfaces declared components and lastUpdated for the detail view', async () => {
        ...
        const [plugin] = await discoverPlugins([{ name: 'Skills', source: 'anthropics/skills', type: 'github' }], new Set());
    ...
    it('strips ANSI/control sequences from untrusted display fields', async () => {
        ...
        const ESC = '\x1b';
        const BEL = '\x07';
        vi.mocked(loadMarketplaceConfigFromSource).mockResolvedValue(config(`Hostile${ESC}[2K`, [
            {
    ...
    it('derives install source from per-plugin source for http sources', async () => {
        vi.mocked(loadMarketplaceConfigFromSource).mockResolvedValue(config('Remote', [
            {
                name: 'gh-plugin',
                version: '1.0.0',
        ...
        const discovered = await discoverPlugins([{ name: 'Remote', source: 'https://x/m.json', type: 'http' }], new Set());
        expect(discovered.find((p) => p.name === 'gh-plugin').installSource).toBe('someone/repo:gh-plugin');
        expect(discovered.find((p) => p.name === 'url-plugin').installSource).toBe('https://example.com/p.tgz');
    });
    it('rejects local-path sources from a remote (http) marketplace', async () => {
        ...
        const discovered = await discoverPlugins([{ name: 'Remote', source: 'https://x/m.json', type: 'http' }], new Set());
        const src = (name) => discovered.find((p) => p.name === name).installSource;
...
//# sourceMappingURL=sourceRegistry.test.js.map