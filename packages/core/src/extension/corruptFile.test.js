...
describe('quarantineCorruptFile', () => {
    ...
    it('renames the file aside to ${path}.corrupted, preserving its bytes', () => {
        ...
        expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('moved aside'));
    });
    it('does not throw when the file cannot be moved aside', () => {
        ...
        expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('could not be moved aside'));
    });
});
//# sourceMappingURL=corruptFile.test.js.map