...
import { parseNpmPackageSource, isScopedNpmPackage, resolveNpmRegistry, checkNpmUpdate, downloadFromNpmRegistry, } from './npm.js';
...
describe('parseNpmPackageSource', () => {
    ...
    it('should redact URL credentials in invalid source errors', () => {
        ...
        expect(message).toContain('https://***REDACTED***@example.com/some-package');
...
function mockNpmRegistryStatus(statusCode) {
    const response = {
        statusCode,
        headers: {},
        on: vi.fn(),
...
function npmMetadataResponse(tarballUrl) {
    return {
        statusCode: 200,
        headers: {},
        on: vi.fn((event, handler) => {
...
function mockNpmDownload(tarballUrl, tarballBytes) {
    let requestCount = 0;
    vi.mocked(https.get).mockImplementation((_url, _options, callback) => {
        requestCount += 1;
        const mockRes = requestCount === 1
            ? {
                statusCode: 200,
                headers: {},
                on: vi.fn((event, handler) => {
...
describe('downloadFromNpmRegistry', () => {
    ...
    it('redacts credentialed registry URLs in metadata request errors', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            source: '@scope/pkg',
            type: 'npm',
            registryUrl: 'https://user:token@registry.example.com',
        }, '/tmp/hopcode-extension')).rejects.toThrow('npm registry request failed with status 404: https://***REDACTED***@registry.example.com/@scope%2fpkg');
    ...
    it('rejects npm metadata response stream errors', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toBe(responseError);
    });
    it('destroys npm metadata responses that exceed the size limit', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('npm package metadata exceeded maximum size');
    ...
    it('destroys non-200 npm tarball responses before rejecting', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Failed to download npm tarball: status 503');
    ...
    it('preserves the original reason for a pre-aborted npm download', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension', controller.signal)).rejects.toBe(reason);
    ...
    it('uses the HTTPS client for uppercase HTTPS tarball URLs', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'HTTPS://registry.example.com',
        }, '/tmp/qwen-extension')).resolves.toEqual({ version: '1.0.0', type: 'npm' });
    ...
    it('rejects a public-policy tarball URL targeting the private network', async () => {
        ...
        }, '/tmp/qwen-extension')).rejects.toThrow('must use HTTPS');
    ...
    it('resolves relative npm metadata redirects', async () => {
        ...
        expect(vi.mocked(https.get).mock.calls[1]?.[0]).toBe('https://registry.example.com/redirected-metadata');
    });
    it('rejects an invalid npm metadata redirect from an async response', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Invalid npm redirect URL: http://[');
        expect(tar.t).not.toHaveBeenCalled();
    });
    it('stops following npm metadata redirect loops', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Too many redirects while fetching npm package metadata');
    ...
    it('resolves relative npm tarball redirects', async () => {
        ...
        expect(vi.mocked(https.get).mock.calls[2]?.[0]).toBe('https://registry.example.com/pkg-final.tgz');
    });
    it('stops following npm tarball redirect loops', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Too many redirects while downloading npm package');
    ...
    it('preserves the original abort reason during a redirected npm download', async () => {
        ...
        const reason = new Error('download cancelled');
        let requestCount = 0;
        let finalRequestError;
        vi.mocked(https.get).mockImplementation((_url, _options, callback) => {
            requestCount += 1;
            if (typeof callback === 'function') {
                if (requestCount === 1) {
                    callback(npmMetadataResponse('https://registry.example.com/pkg.tgz'));
                }
        ...
        const outcome = downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension', controller.signal);
        await vi.waitFor(() => expect(requestCount).toBe(3));
        expect((vi.mocked(https.get).mock.calls[2]?.[1])
            .signal?.aborted).toBe(false);
        controller.abort(reason);
    ...
    it.each(['SymbolicLink', 'Link'])('rejects npm tarballs containing %s entries before extraction', async (type) => {
        mockNpmDownload('https://registry.example.com/pkg.tgz');
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Tar archive contains unsupported link entry: package/escape');
        expect(tar.x).not.toHaveBeenCalled();
    });
    it('sanitizes and bounds rejected tar entry paths', async () => {
        ...
        expect(message).toHaveLength('Tar archive contains unsupported link entry: '.length + 200);
    ...
    it('rejects tar links whose sanitized path is empty', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Tar archive contains unsupported link entry: <sanitized empty path>');
        expect(tar.x).not.toHaveBeenCalled();
    });
    it('reports every rejected tar link', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('Tar archive contains 2 unsupported link entries: package/first-link, package/second-link');
        expect(tar.x).not.toHaveBeenCalled();
    });
    it('bounds rejected tar link collection', async () => {
        ...
        await expect(downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('more than 100 unsupported link entries');
        expect(tar.x).not.toHaveBeenCalled();
    });
    ...
    it('rejects npm tarballs larger than 100 MB', async () => {
        mockNpmDownload('https://registry.example.com/pkg.tgz', 100 * 1024 * 1024 + 1);
        await expect(downloadFromNpmRegistry({
            source: '@scope/pkg',
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension')).rejects.toThrow('npm extension archive download exceeded maximum size of 104857600 bytes');
        expect(tar.t).not.toHaveBeenCalled();
    });
    it('times out a stalled npm tarball download', async () => {
        ...
        const outcome = downloadFromNpmRegistry({
            ...
            type: 'npm',
            registryUrl: 'https://registry.example.com',
        }, '/tmp/qwen-extension').catch((error) => error);
    ...
    it('does not start a tarball request when DNS outlives the deadline', async () => {
        ...
        vi.mocked(https.get).mockImplementation((_url, _options, callback) => {
            if (typeof callback === 'function') {
                callback(npmMetadataResponse('https://cdn.example.com/pkg.tgz'));
            }
            return { on: vi.fn().mockReturnThis(), destroy: vi.fn() };
        });
        const outcome = downloadFromNpmRegistry({
