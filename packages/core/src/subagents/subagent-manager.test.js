...
describe('SubagentManager', () => {
    ...
    beforeEach(() => {
        ...
        mockParseYaml.mockImplementation((yamlString) => {
            ...
            if (yamlString.includes('background:') ||
                ...
                if (bgMatch)
                    result['background'] = bgMatch[1] === 'true';
                if (approvalMatch)
                    result['approvalMode'] = approvalMatch[1];
                return result;
            }
    ...
    const validConfig = {
        ...
        level: 'project',
        filePath: '/test/project/.hopcode/agents/test-agent.md',
    };
    const validMarkdown = `---
name: test-agent
description: A test subagent
...
You are a helpful assistant.
`;
            const config = manager.parseSubagentContent(markdownWithCSV, validConfig.filePath, 'project');
...
You are a helpful assistant.
`;
            const config = manager.parseSubagentContent(markdownWithSingle, validConfig.filePath, 'project');
...
`;
        ...
        it('should determine level from file path', () => {
            const projectPath = '/test/project/.hopcode/agents/test-agent.md';
            const userPath = '/home/user/.hopcode/agents/test-agent.md';
...
`;
        ...
        it('should not warn when filename matches subagent name', () => {
            ...
            const matchingPath = '/test/project/.hopcode/agents/test-agent.md';
...
You are a bubbler.
`;
            const config = manager.parseSubagentContent(md, validConfig.filePath, 'project');
...
You are weird.
`;
            expect(() => manager.parseSubagentContent(md, validConfig.filePath, 'project')).toThrow(/Invalid "approvalMode"/);
        });
        it('should round-trip approvalMode: bubble through serialize', () => {
            ...
            const reparsed = manager.parseSubagentContent(serialized, validConfig.filePath, 'project');
    ...
    describe('loadSubagent', () => {
        it('applies the configured model only to the built-in Explore agent', async () => {
            ...
            const configuredManager = new SubagentManager(makeFakeConfig({
                agents: { builtin: { exploreModel: 'fast' } },
            }));
            expect((await configuredManager.loadSubagent('Explore'))?.model).toBe('fast');
            expect((await configuredManager.loadSubagent('Explore', 'builtin'))?.model).toBe('fast');
            const builtins = await configuredManager.listSubagents({
                level: 'builtin',
                force: true,
            });
            expect(builtins.find((agent) => agent.name === 'Explore')?.model).toBe('fast');
        });
        ...
        ])('resolves the built-in Explore model setting "$exploreModel"', async ({ exploreModel, expectedModel }) => {
            const configuredManager = new SubagentManager(makeFakeConfig({
        ...
        it('should load subagent from project level first', async () => {
            ...
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents/test-agent.md'), 'utf8');
        });
        it('should fall back to user level if project level fails', async () => {
            ...
            expect(config).toBeDefined();
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/home/user/.hopcode/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/home/user/.hopcode/agents/test-agent.md'), 'utf8');
        });
...
You are another assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.hopcode/agents/wrong-filename.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/home/user/.hopcode/agents/user-agent.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.hopcode/agents/misnamed-file.md'));
        });
    });
    ...
    describe('deleteSubagent', () => {
        it('should delete subagent from specified level', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents/test-agent.md'));
        });
        it('should delete from both levels if no level specified', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents/test-agent.md'));
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/home/user/.hopcode/agents/test-agent.md'));
        });
        it('should throw error if subagent not found', async () => {
            vi.mocked(fs.readdir).mockRejectedValue(new Error('Directory not found'));
...
You are a helpful assistant.`;
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents/wrong-name.md'));
        });
...
Target content`,
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.hopcode/agents/target-file.md'));
        });
    });
...
System prompt 3`);
    ...
    describe('safe mode', () => {
        ...
        it('listSubagents returns only builtin subagents', async () => {
            ...
            vi.mocked(fs.readdir)
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                .mockResolvedValue(['evil-agent.md']);
    ...
    describe('Runtime Configuration Methods', () => {
        describe('convertToRuntimeConfig', () => {
            ...
            it('should populate runConfig.max_turns from top-level maxTurns', async () => {
                const cfg = { ...validConfig, maxTurns: 42 };
        ...
        describe('createAgentHeadless — caller-driven dispose contract', () => {
            ...
            it('dispose is a safe no-op when neither hooks nor mcpServers are declared', async () => {
                const result = await manager.createAgentHeadless(baseConfig, mockConfig);
                expect(typeof result.dispose).toBe('function');
            ...
            it('runs cleanup when AgentHeadless.create throws — caller never gets dispose', async () => {
                ...
                mockAgentHeadlessCreate.mockRejectedValueOnce(new Error('synthetic constructor failure'));
                await expect(manager.createAgentHeadless({
                    ...baseConfig,
                    hooks: {
                        PreToolUse: [
                            {
                ...
                }, mockConfig)).rejects.toThrow(/synthetic constructor failure/);
                expect(unregisterSpy).toHaveBeenCalledTimes(1);
            });
        });
    });
