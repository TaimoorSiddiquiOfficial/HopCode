...
describe('BuiltinAgentRegistry', () => {
    describe('getBuiltinAgents', () => {
        ...
        it('should include general-purpose agent', () => {
            ...
            expect(generalAgent?.systemPrompt).toContain('general-purpose subagent working for a parent agent');
            expect(generalAgent?.systemPrompt).toContain('Preserve unrelated user changes');
            expect(generalAgent?.systemPrompt).toContain('Verify factual claims before reporting');
            expect(generalAgent?.systemPrompt).toContain('run the smallest relevant checks');
            expect(generalAgent?.systemPrompt).toContain('Report uncertainty or blockers');
        });
        ...
        it('keeps the Explore agent read-only without banning shell pipelines', () => {
            ...
            expect(exploreAgent?.systemPrompt).toContain('pipelines are allowed when every command is read-only');
