...
import { BACKGROUND_AGENT_CONCURRENCY_ENV, BackgroundTaskRegistry, DEFAULT_MAX_CONCURRENT_BACKGROUND_AGENTS, MAX_CONCURRENT_BACKGROUND_AGENTS, MAX_RETAINED_TERMINAL_AGENTS, resolveMaxConcurrentBackgroundAgents, } from './background-tasks.js';
...
function makeApproval(callId, respond = vi.fn(async () => { })) {
    return {
        callId,
        name: 'Shell',
...
describe('BackgroundTaskRegistry', () => {
    ...
    describe('background concurrency limit', () => {
        it('resolves the default and env override for the background agent cap', () => {
            expect(resolveMaxConcurrentBackgroundAgents({})).toBe(DEFAULT_MAX_CONCURRENT_BACKGROUND_AGENTS);
        ...
        it('rejects new running background agents once the cap is reached', () => {
            ...
            expect(() => registry.register(makeRegistration('bg-3'))).toThrow('Cannot start background agent: maximum concurrent background agents ' +
        ...
        it('allows replacing the same running background agent at the cap', () => {
            ...
            registry.register(makeRegistration('bg-1'));
            expect(() => registry.register(makeRegistration('bg-1', {
        ...
        it('does not count foreground agents toward the background cap', () => {
            ...
            registry.register(makeRegistration('fg-1', {
                isBackgrounded: false,
            }));
            registry.register(makeRegistration('bg-1'));
        ...
        it('does not count paused or terminal entries toward the cap', () => {
            ...
            registry.register(makeRegistration('paused-1', {
                status: 'paused',
            }));
            registry.register(makeRegistration('bg-1'));
            expect(() => registry.register(makeRegistration('bg-2'))).toThrow('maximum concurrent background agents (1) reached');
        ...
        it('queues waiters until a background slot is released', async () => {
            ...
            registry.register(makeRegistration('bg-1'));
            const reservationPromise = registry.waitForBackgroundSlot(new AbortController().signal);
        ...
        it('throws immediately when the slot wait signal is already aborted', async () => {
            ...
            await expect(registry.waitForBackgroundSlot(abortController.signal)).rejects.toThrow('Agent launch cancelled while waiting for a background slot.');
            expect(registry.getQueuedCount()).toBe(0);
        });
        it('resolves immediately when a background slot is available', async () => {
            ...
            registry.register(makeRegistration('bg-1'));
            const reservation = await registry.waitForBackgroundSlot(new AbortController().signal);
        ...
        it('releases a reserved slot and drains the wait queue', async () => {
            ...
            expect(reservation).toBeDefined();
            const waiterPromise = registry.waitForBackgroundSlot(new AbortController().signal);
            expect(registry.getQueuedCount()).toBe(1);
        ...
        it('reserves a drained slot until registration consumes it', async () => {
            ...
            registry.register(makeRegistration('bg-1'));
            const first = registry.waitForBackgroundSlot(new AbortController().signal);
            const second = registry.waitForBackgroundSlot(new AbortController().signal);
            ...
            expect(registry.getQueuedCount()).toBe(1);
            expect(() => registry.register(makeRegistration('racer'))).toThrow('maximum concurrent background agents (1) reached');
            registry.register(makeRegistration('bg-2'), {
        ...
        it('removes an aborted waiter from the queue', async () => {
            ...
            registry.register(makeRegistration('bg-1'));
            const abortController = new AbortController();
            const reservation = registry.waitForBackgroundSlot(abortController.signal);
            abortController.abort();
            await expect(reservation).rejects.toThrow('Agent launch cancelled while waiting for a background slot.');
            expect(registry.getQueuedCount()).toBe(0);
        });
        it('rejects queued waiters on reset', async () => {
            ...
            registry.register(makeRegistration('bg-1'));
            const reservation = registry.waitForBackgroundSlot(new AbortController().signal);
            registry.reset();
            await expect(reservation).rejects.toThrow('Agent launch cancelled while waiting for a background slot.');
            expect(registry.getQueuedCount()).toBe(0);
        });
        it('reports when reset invalidates a drained slot reservation', async () => {
            ...
            const reservation = await reservationPromise;
            registry.reset();
            expect(() => registry.register(makeRegistration('bg-2'), {
                slotReservation: reservation,
    ...
    describe('per-model background concurrency limit', () => {
        it('caps a single model while leaving room for others', () => {
            ...
            expect(() => registry.register(makeRegistration('bg-2', { model: 'weak-model' }))).toThrow('Cannot start background agent: maximum concurrent background agents ' +
        ...
        it('lets a model without a per-model cap use the global limit', () => {
            ...
            expect(() => registry.register(makeRegistration('bg-3', { model: 'uncapped-model' }))).toThrow('maximum concurrent background agents (2) reached');
        });
        it('enforces the global cap even when the per-model cap has room', () => {
            ...
            expect(() => registry.register(makeRegistration('bg-2', { model: 'weak-model' }))).toThrow('maximum concurrent background agents (1) reached');
        });
        ...
        it('drains a different-model waiter while a capped-model waiter stays queued', async () => {
            ...
            const weakWaiter = registry.waitForBackgroundSlot(new AbortController().signal, 'weak-model');
            const strongWaiter = registry.waitForBackgroundSlot(new AbortController().signal, 'strong-model');
        ...
        it('ignores malformed per-model cap values', () => {
            ...
            registry.register(makeRegistration('bg-negative', { model: 'bad-negative' }));
            ...
            expect(() => registry.register(makeRegistration('bg-good-2', { model: 'good' }))).toThrow('for model "good" (1) reached');
        });
        it('accepts a ReadonlyMap for the per-model caps', () => {
            ...
            registry.register(makeRegistration('bg-1', { model: 'weak-model' }));
            expect(() => registry.register(makeRegistration('bg-2', { model: 'weak-model' }))).toThrow('for model "weak-model" (1) reached');
        });
    });
    ...
    describe('permission bubbling (pending approvals)', () => {
        it('parks an approval and surfaces it on the entry', () => {
            ...
            expect(registry.get('bg-appr-1')?.pendingApprovals?.[0].callId).toBe('c1');
        ...
        it('refuses to park for an unknown or terminal entry', () => {
            ...
            expect(registry.addPendingApproval('bg-appr-2', makeApproval('c1'))).toBe(false);
            expect(registry.addPendingApproval('missing', makeApproval('c1'))).toBe(false);
        });
        it('ignores a duplicate callId', () => {
            ...
            expect(registry.addPendingApproval('bg-appr-3', makeApproval('c1'))).toBe(false);
        ...
        it('resolves a parked approval via its respond callback and removes it', async () => {
            ...
            const resolved = await registry.resolvePendingApproval('bg-appr-4', 'c1', ToolConfirmationOutcome.ProceedOnce);
        ...
        ])('downgrades persistent approval outcome %s to one-time approval', async (outcome) => {
            const respond = vi.fn(async () => { });
            registry.register(makeRegistration(`bg-appr-${outcome}`));
            registry.addPendingApproval(`bg-appr-${outcome}`, makeApproval('c1', respond));
            const resolved = await registry.resolvePendingApproval(`bg-appr-${outcome}`, 'c1', outcome);
            expect(resolved).toBe(true);
            expect(respond).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, undefined);
        });
        it('returns false when resolving a non-parked call', async () => {
            registry.register(makeRegistration('bg-appr-5'));
            expect(await registry.resolvePendingApproval('bg-appr-5', 'nope', ToolConfirmationOutcome.Cancel)).toBe(false);
        });
        ...
        it('fails the agent before aborting when a parked approval respond() rejects', async () => {
            ...
            const ok = await registry.resolvePendingApproval('bg-appr-retry', 'c1', ToolConfirmationOutcome.ProceedOnce);
            expect(ok).toBe(false);
            expect(registry.getPendingApprovals('bg-appr-retry')).toHaveLength(0);
            expect(registry.get('bg-appr-retry')?.status).toBe('failed');
            expect(registry.get('bg-appr-retry')?.error).toBe('Failed to resolve background approval: c1');
            expect(abortController.signal.aborted).toBe(true);
        ...
        it('cancel() rejects parked approvals before the abort-driven clear (production ordering)', () => {
            ...
            const abortController = new AbortController();
            registry.register(makeRegistration('bg-appr-cancel', { abortController }));
