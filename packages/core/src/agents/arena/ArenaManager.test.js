...
describe('ArenaManager', () => {
    ...
    describe('chat history forwarding', () => {
        ...
        it('builds the in-process worker system prompt in headless interaction mode', async () => {
            ...
            for (const call of mockBackend.spawnAgent.mock.calls) {
                const spawnConfig = call[0];
                const systemPrompt = spawnConfig.inProcess?.runtimeConfig?.promptConfig?.systemPrompt;
                expect(systemPrompt).toContain('This is a non-interactive, single-turn run');
            }
        });
    });
