...
export class ArenaManager {
    ...
    constructor(config, callbacks = {}) {
        ...
        this.arenaBaseDir =
            arenaSettings?.worktreeBaseDir ??
                path.join(Storage.getGlobalHopCodeDir(), 'arena');
    ...
    buildAgentSpawnConfig(agent) {
        ...
        if (model.apiKey) {
            env['HOPCODE_API_KEY'] = model.apiKey;
        }
        if (model.baseUrl) {
            env['HOPCODE_BASE_URL'] = model.baseUrl;
        }
        const spawnConfig = {
            agentId,
            ...
            inProcess: {
                ...
                runtimeConfig: {
                    promptConfig: {
                        systemPrompt: getCoreSystemPrompt(this.config.getUserMemory(), model.modelId, undefined, 'headless'),
                    },
