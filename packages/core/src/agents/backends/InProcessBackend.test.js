...
const runReasoningLoopMock = vi.hoisted(() => vi.fn().mockResolvedValue({
    text: 'Done',
    terminateMode: null,
    turnsUsed: 1,
}), AuthType, {
...
describe('InProcessBackend', () => {
    ...
    it('routes owned monitor notifications into the agent message queue', async () => {
        ...
        const monitorRegistry = mockConfig.getMonitorRegistry();
        const localBackend = new InProcessBackend(mockConfig);
        await localBackend.init();
        await localBackend.spawnAgent(createSpawnConfig('agent-1'));
        expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith('agent-1', expect.any(Function));
        ...
        expect(monitorRegistry.cancelRunningForOwner).toHaveBeenCalledWith('agent-1', { notify: false });
        expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith('agent-1', undefined);
    });
    ...
    it('restores approval override cleanup when per-agent setup fails', async () => {
        ...
        parentConfig.createToolRegistry.mockRejectedValueOnce(new Error('registry boom'));
        parentConfig.getPermissionManager.mockReturnValue({
            restoreDangerousRules,
            stripDangerousRulesForAutoMode,
        });
        const localBackend = new InProcessBackend(parentConfig);
        await localBackend.init();
        const config = createSpawnConfig('agent-1');
        config.inProcess.approvalMode = ApprovalMode.AUTO;
        await expect(localBackend.spawnAgent(config)).rejects.toThrow('registry boom');
    ...
    it('uses a per-agent approval mode without mutating the parent config', async () => {
        const parentConfig = createMockConfig();
        const backendWithParentMode = new InProcessBackend(parentConfig);
    ...
    it('uses a teammate-scoped plan file path in per-agent config', async () => {
        ...
        const { runtimeContext } = destructureAgentCoreCall(lastCall);
        const agentContext = runtimeContext;
        expect(agentContext.getPlanFilePath()).toBe('/tmp/plans/test-session-agent-1.md');
        expect(agentContext.getPlanFilePath()).not.toBe(parentConfig.getPlanFilePath());
    });
    it('keeps Config approval-mode safety checks on per-agent config', async () => {
        const parentConfig = createMockConfig();
        parentConfig.isTrustedFolder.mockReturnValue(false);
        const backendWithUntrustedParent = new InProcessBackend(parentConfig);
        ...
        const { runtimeContext } = destructureAgentCoreCall(lastCall);
        const agentContext = runtimeContext;
        expect(() => agentContext.setApprovalMode(ApprovalMode.AUTO_EDIT)).toThrow('Cannot enable privileged approval modes in an untrusted folder.');
    });
    ...
    it('should pass parent custom ignore files to per-agent file service', async () => {
        ...
        expect(agentContext.getFileService().gethopcodeignoreFileNamesDisplay()).toBe('.hopcodeignore, .cursorignore');
    });
    ...
    it('should fire exit callback with code 1 when start() throws', async () => {
        ...
        const failingBackend = new InProcessBackend(parentConfig);
        ...
        await expect(failingBackend.spawnAgent(createSpawnConfig('agent-fail'))).resolves.toBeUndefined();
        ...
        expect(failingBackend.agentApprovalCleanups.size).toBe(0);
        expect(failingBackend.agentRegistries.size).toBe(0);
    });
