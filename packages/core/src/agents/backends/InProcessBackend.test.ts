...
// liveOutputs, shellPids, pushMessage, etc.) â€” otherwise agent lifecycle
...
describe('InProcessBackend', () => {
  ...
  it('stopAgent on a non-existent id is a no-op (no throw, Map untouched)', async () => {
    ...
    // throwing â€” matches the behavior of `agents.get` returning undefined
  ...
  it('cleanup disposes all remaining registries (covers the in-flight shutdown path)', async () => {
    ...
    // call â€” the shared `createMockConfig` returns the same singleton
