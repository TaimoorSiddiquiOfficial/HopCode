...
import { createTask, getTask, updateTask, deleteTask, listTasks, resetTaskList, blockTask, claimTask, releaseOwnedTask, unassignTeammateTasks, getAgentStatuses, onTasksUpdated, notifyTasksUpdated, TaskOwnershipError, RECIPROCAL_CALLER, } from './tasks.js';
...
describe('tasks', () => {
    ...
    describe('getTask', () => {
        ...
        it('rejects non-numeric task IDs to prevent path traversal', async () => {
            await expect(getTask('team', '../../etc/passwd')).rejects.toThrow('Invalid task ID');
            await expect(getTask('team', '../../settings')).rejects.toThrow('Invalid task ID');
        ...
        it('rejects non-numeric task IDs from updateTask and deleteTask', async () => {
            await expect(updateTask('team', '../../oops', { status: 'completed' })).rejects.toThrow('Invalid task ID');
            await expect(deleteTask('team', '../../oops')).rejects.toThrow('Invalid task ID');
        });
    });
    // ─── updateTask ────────────────────────────────────────────
    describe('updateTask', () => {
        ...
        it('persists completion even when unblocking one dependent fails', async () => {
            ...
            const corruptPath = path.join(tmpDir, 'tasks', 'team', `${corrupt.id}.json`);
        ...
        it('returns undefined for nonexistent task', async () => {
            expect(await updateTask('team', '999', {
                status: 'completed',
            })).toBeUndefined();
        });
        it('rejects a teammate caller from clobbering a different owner', async () => {
            ...
            await expect(updateTask('team', task.id, { status: 'in_progress', owner: 'bob' }, { callerName: 'bob' })).rejects.toBeInstanceOf(TaskOwnershipError);
        ...
        it('serializes concurrent teammate claims under the lock', async () => {
            ...
            const aliceClaim = updateTask('team', task.id, { status: 'in_progress', owner: 'alice' }, { callerName: 'alice' });
            const bobClaim = updateTask('team', task.id, { status: 'in_progress', owner: 'bob' }, { callerName: 'bob' });
            const results = await Promise.allSettled([aliceClaim, bobClaim]);
            const fulfilled = results.filter((r) => r.status === 'fulfilled');
            const rejected = results.filter((r) => r.status === 'rejected');
            expect(fulfilled).toHaveLength(1);
            expect(rejected).toHaveLength(1);
            expect(rejected[0].reason).toBeInstanceOf(TaskOwnershipError);
        ...
        it('lets the existing owner change their own task', async () => {
            ...
            await updateTask('team', task.id, { status: 'in_progress', owner: 'alice' }, { callerName: 'alice' });
            const updated = await updateTask('team', task.id, { status: 'completed' }, { callerName: 'alice' });
            expect(updated?.status).toBe('completed');
        });
        it('lets the reciprocal sentinel bypass the ownership guard', async () => {
            ...
            const updated = await updateTask('team', task.id, { addBlockedBy: [other.id] }, { callerName: RECIPROCAL_CALLER });
            expect(updated?.blockedBy).toContain(other.id);
            // A real non-owner caller is still blocked.
            await expect(updateTask('team', task.id, { addBlockedBy: [other.id] }, { callerName: 'bob' })).rejects.toBeInstanceOf(TaskOwnershipError);
        });
    });
    // ─── deleteTask ────────────────────────────────────────────
    describe('deleteTask', () => {
        ...
        it("rejects a teammate caller from deleting another owner's task", async () => {
            ...
            await expect(deleteTask('team', task.id, { callerName: 'bob' })).rejects.toBeInstanceOf(TaskOwnershipError);
        ...
        it('lets the owner, the leader, and any teammate (unowned) delete', async () => {
            ...
            expect(await deleteTask('team', owned.id, { callerName: 'alice' })).toBe(true);
            ...
            expect(await deleteTask('team', unowned.id, { callerName: 'bob' })).toBe(true);
        });
    });
    // ─── listTasks ─────────────────────────────────────────────
    describe('listTasks', () => {
    ...
    describe('concurrent operations', () => {
        it('only one claimTask wins under concurrency', async () => {
            ...
            const results = await Promise.all(Array.from({ length: 5 }, (_, i) => claimTask('team', task.id, `worker-${i}`)));
