 ...
 *
 * All file operations target `~/.hopcode/teams/{team-name}/config.json`.
...
export function getTeamsRootDir() {
    return path.join(Storage.getGlobalHopCodeDir(), TEAMS_DIR);
}
/**
...
export function getTasksDir(teamName) {
    return path.join(Storage.getGlobalHopCodeDir(), TASKS_DIR, teamName);
}
// ─── Name helpers ───────────────────────────────────────────
/**
