...
import { sanitizeName, formatAgentId, generateUniqueTeammateName, assignTeammateColor, clearTeammateColors, setMemberActive, findMemberById, findMemberByName, classifyShutdownResponse, readTeamFile, writeTeamFile, deleteTeamDirs, tryReclaimStaleTeam, getTeamDir, getTeamFilePath, getInboxesDir, getTasksDir, } from './teamHelpers.js';
...
function makeTeamFile(overrides) {
    return {
        name: 'test-team',
        createdAt: Date.now(),
...
describe('generateUniqueTeammateName', () => {
    ...
    it('throws on duplicate name', () => {
        ...
        expect(() => generateUniqueTeammateName('Worker', members)).toThrow('A teammate named "worker" already exists');
    });
    it('throws on duplicate even with multiple members', () => {
        ...
        expect(() => generateUniqueTeammateName('Worker', members)).toThrow('already exists');
    });
    ...
    it('rejects names that sanitize to empty string', () => {
        expect(() => generateUniqueTeammateName('@@@', [])).toThrow('sanitizes to an empty string');
        expect(() => generateUniqueTeammateName('', [])).toThrow('sanitizes to an empty string');
    });
    it('rejects the reserved leader name', () => {
        expect(() => generateUniqueTeammateName('leader', [])).toThrow('reserved for the team leader');
        expect(() => generateUniqueTeammateName('LEADER', [])).toThrow('reserved for the team leader');
    });
});
describe('assignTeammateColor', () => {
    ...
    it('wraps around when all colors taken', () => {
        const members = TEAMMATE_COLORS.map((color, i) => makeMember({ name: `m${i}`, color }));
...
describe('findMemberByName', () => {
    it('finds member by name (case-insensitive)', () => {
        const members = [makeMember({ name: 'worker' })];
        expect(findMemberByName(members, 'Worker')?.agentId).toBe('worker@test-team');
    });
    it('finds member when lookup uses unsanitized human name', () => {
        ...
        expect(findMemberByName(members, 'QA Tester')?.agentId).toBe('qa-tester@test-team');
    });
...
describe('classifyShutdownResponse', () => {
    it('classifies a reply that leads with the approve token', () => {
        expect(classifyShutdownResponse('shutdown_approved')).toBe('shutdown_approved');
        // Verbose lead-in form an exact-string match would miss.
        expect(classifyShutdownResponse('shutdown_approved, work finished')).toBe('shutdown_approved');
        expect(classifyShutdownResponse('  shutdown_approved\nbye')).toBe('shutdown_approved');
    });
    it('classifies a reply that leads with the reject token', () => {
        expect(classifyShutdownResponse('shutdown_rejected: still mid-task')).toBe('shutdown_rejected');
    });
    it('does not classify a mid-prose mention of the token', () => {
        // The false-abort bug: a token mentioned mid-report is not a
        // response.
        expect(classifyShutdownResponse('I reviewed the shutdown_approved handler and it looks correct.')).toBeUndefined();
        expect(classifyShutdownResponse('I will send shutdown_approved when done.')).toBeUndefined();
    });
...
describe('file I/O', () => {
    ...
    beforeEach(async () => {
        ...
        Storage.__setMockGlobalDir(tmpDir);
    });
    afterEach(async () => {
    ...
    describe('path helpers', () => {
        ...
        it('getTeamFilePath returns correct path', () => {
            expect(getTeamFilePath('my-team')).toBe(path.join(tmpDir, 'teams', 'my-team', 'config.json'));
        });
        it('getInboxesDir returns correct path', () => {
            expect(getInboxesDir('my-team')).toBe(path.join(tmpDir, 'teams', 'my-team', 'inboxes'));
        });
        it('getTasksDir returns correct path', () => {
            expect(getTasksDir('my-team')).toBe(path.join(tmpDir, 'tasks', 'my-team'));
        });
    });
...
//# sourceMappingURL=teamHelpers.test.js.map