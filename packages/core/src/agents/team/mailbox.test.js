...
import { getInboxPath, readInbox, writeMessage, consumeUnread, clearInbox, clearAllInboxes, sendStructuredMessage, disposeInboxLocks, } from './mailbox.js';
...
describe('mailbox', () => {
    ...
    it('returns correct inbox path', () => {
        ...
        expect(p).toBe(path.join(tmpDir, 'teams', 'my-team', 'inboxes', 'worker.json'));
    });
    // ─── readInbox ─────────────────────────────────────────────
    ...
    it('compacts aged read messages but keeps recent and unread ones', async () => {
        ...
        await writeMessage('team', 'leader', makeMessage({ text: 'aged-read', read: true, timestamp: old }));
        await writeMessage('team', 'leader', makeMessage({ text: 'recent-read', read: true, timestamp: recent }));
        await writeMessage('team', 'leader', makeMessage({ text: 'aged-unread', read: false, timestamp: old }));
    ...
    it('only consumes messages of matching type', async () => {
        ...
        await writeMessage('team', 'worker', makeMessage({ text: 'task', type: 'task_assignment' }));
    ...
    it('writeMessage quarantines a corrupt inbox and keeps delivering', async () => {
        ...
        const quarantined = entries.find((e) => e.startsWith('worker.json.corrupt-'));
    ...
    it('handles concurrent writes without corruption', async () => {
        const count = 10;
        const promises = Array.from({ length: count }, (_, i) => writeMessage('team', 'worker', makeMessage({ text: `msg-${i}` })));
    ...
    it('lockless readInbox never sees a partial file mid-write', async () => {
        ...
        const writes = Array.from({ length: writeCount }, (_, i) => writeMessage('team', 'worker', makeMessage({ text: `w-${i}` })));
        const reads = Array.from({ length: readCount }, () => readInbox('team', 'worker'));
