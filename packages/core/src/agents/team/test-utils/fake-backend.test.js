...
function spawnConfig(agentId, agentName) {
    return {
        agentId,
        command: '',
        args: [],
...
describe('FakeBackend', () => {
    ...
    it('throws on duplicate agentId', async () => {
        const backend = new FakeBackend();
        await backend.spawnAgent(spawnConfig('agent-1'));
        await expect(backend.spawnAgent(spawnConfig('agent-1'))).rejects.toThrow('already exists');
    });
...
//# sourceMappingURL=fake-backend.test.js.map