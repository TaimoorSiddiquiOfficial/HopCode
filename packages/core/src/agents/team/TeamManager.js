...
import { formatAgentId, generateUniqueTeammateName, assignTeammateColor, writeTeamFile, findMemberByName, classifyShutdownResponse, } from './teamHelpers.js';
...
export class TeamManager {
    ...
    async spawnTeammate(config) {
        ...
        try {
            ...
            const addendum = buildTeammatePromptAddendum(name, this.teamFile.name, LEADER_NAME, { planModeRequired: config.planModeRequired });
     ...
     */
    async sendMessage(toName, message, from, summary) {
        ...
        if (toName.toLowerCase() === LEADER_NAME ||
            ...
            const sender = from
                ...
                : undefined;
            const shutdownResponse = sender && this._shutdownPending.has(sender.name)
    ...
    async tryAutoClaimTask(agentId, agentName, pending) {
        ...
        for (const task of pendingTasks) {
            ...
            if (claimed) {
                ...
                const taskPrompt = `You have been assigned task #${claimed.id}.\n\n` +
                    `${open}\n` +
