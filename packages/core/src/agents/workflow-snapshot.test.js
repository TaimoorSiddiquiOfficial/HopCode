...
import { toSnapshot, writeWorkflowSnapshot, listWorkflowSnapshots, MAX_RETAINED_SNAPSHOTS, } from './workflow-snapshot.js';
...
describe('writeWorkflowSnapshot + listWorkflowSnapshots', () => {
    ...
    it('lists newest-first by startTime', async () => {
        const config = fakeConfig(projectDir);
        await writeWorkflowSnapshot(config, task({ runId: 'wf_old', startTime: 1_000 }));
        await writeWorkflowSnapshot(config, task({ runId: 'wf_new', startTime: 9_000 }));
        const list = await listWorkflowSnapshots(config);
    ...
    it('does not recursively delete via a crafted snapshot filename (path traversal)', async () => {
        ...
        for (let i = 0; i < MAX_RETAINED_SNAPSHOTS; i++) {
            await writeWorkflowSnapshot(config, task({ runId: `wf_${i.toString(16)}`, startTime: 10_000 + i }));
        }
        ...
        await writeWorkflowSnapshot(config, task({ runId: 'wf_ff', startTime: 99_999 }));
        // The guard spared both the parent canary and the non-run sibling dir.
        await expect(fs.access(canary)).resolves.toBeUndefined();
        await expect(fs.access(path.join(dir, 'notarun', 'keep.txt'))).resolves.toBeUndefined();
    });
});
//# sourceMappingURL=workflow-snapshot.test.js.map