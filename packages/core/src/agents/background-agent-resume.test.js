...
import { getAgentJsonlPath, getAgentMetaPath, readAgentMeta, writeAgentMeta, } from './agent-transcript.js';
...
import { FORK_DEFAULT_MAX_TURNS, FORK_SUBAGENT_TYPE, buildChildMessage, } from '../tools/agent/fork-subagent.js';
describe('BackgroundAgentResumeService', () => {
    ...
    function createService(options = {}) {
        ...
        const hookSystem = options.hookSystem !== undefined
            ? options.hookSystem
            : {
    ...
    it('stores a fallback when resumed output has no model-visible text', async () => {
        ...
        expect(registry.get(agentId)?.result).toBe('(subagent produced no model-visible output)');
    });
    ...
    it('keeps a paused agent paused when resume cannot claim a background slot', async () => {
        ...
        expect(registry.get(agentId)?.error).toContain('maximum concurrent background agents (1) reached');
    ...
    ])('normalizes persisted maxSubagentDepth $persisted to $expected on resume', async ({ persisted, expected }) => {
    ...
    ])('restores persisted launch depth $persisted as $expected on resume', async ({ persisted, expected }) => {
    ...
    it('revives a completed background agent from its transcript and bumps resumeCount', async () => {
        ...
        const revived = await service.reviveCompletedBackgroundAgent(agentId, 'now write the summary');
        ...
        expect(fs.statSync(sessionDir).mtime.getTime()).toBeGreaterThan(oldSessionMtime.getTime());
    });
    it('does not revive non-completed or transcript-less entries', async () => {
        ...
        await expect(service.reviveCompletedBackgroundAgent('still-running', 'go')).resolves.toBeUndefined();
        ...
        await expect(service.reviveCompletedBackgroundAgent('completed-bare', 'go')).resolves.toBeUndefined();
        ...
        await expect(service.reviveCompletedBackgroundAgent('failed-agent', 'go')).resolves.toBeUndefined();
        // Unknown id → not revivable.
        await expect(service.reviveCompletedBackgroundAgent('nope', 'go')).resolves.toBeUndefined();
        expect(subagentManager.createAgentHeadless).not.toHaveBeenCalled();
    });
    it('does not mutate a completed entry when revive preflight fails', async () => {
        ...
        await expect(service.reviveCompletedBackgroundAgent(missingMetaAgentId, 'go')).resolves.toBeUndefined();
        await expect(service.reviveCompletedBackgroundAgent(missingOutputAgentId, 'go')).resolves.toBeUndefined();
        await expect(service.reviveCompletedBackgroundAgent(corruptOutputAgentId, 'go')).resolves.toBeUndefined();
    ...
    it('restores the completed entry when revive setup fails after the state flip', async () => {
        ...
        const { service, subagentManager } = createService();
        subagentManager.createAgentHeadless.mockRejectedValue(new Error('setup failed'));
        await expect(service.reviveCompletedBackgroundAgent(agentId, 'keep going')).resolves.toBeUndefined();
    ...
    it('does not revive when the background concurrency cap is full', async () => {
        ...
        const { service, subagentManager } = createService();
        const revived = await service.reviveCompletedBackgroundAgent(agentId, 'keep going');
