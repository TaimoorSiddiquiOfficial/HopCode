...
import { attachStallWatchdog, resolveStallMs, runStallResilient, DEFAULT_STALL_MS, MAX_STALL_ATTEMPTS, MAX_WORKFLOW_STALL_MS_ENV, } from './workflow-stall.js';
describe('resolveStallMs', () => {
    ...
    it('falls back to env seconds when no per-call override', () => {
        expect(resolveStallMs(undefined, { [MAX_WORKFLOW_STALL_MS_ENV]: '30' })).toBe(30_000);
    });
    it('env 0 disables', () => {
        expect(resolveStallMs(undefined, { [MAX_WORKFLOW_STALL_MS_ENV]: '0' })).toBe(0);
    });
    it.each(['0x10', '1e3', '1.0', '2.5', '0x0'])('ignores malformed env seconds %j', (value) => {
        expect(resolveStallMs(undefined, { [MAX_WORKFLOW_STALL_MS_ENV]: value })).toBe(DEFAULT_STALL_MS);
    });
...
describe('attachStallWatchdog', () => {
    ...
    it('resets the timer on a progress event', () => {
        ...
        emitter.emit(AgentEventType.STREAM_TEXT, {}); // activity → reset
        vi.advanceTimersByTime(800);
    ...
    it('suspends the timer while a tool is in flight', () => {
        ...
        const controller = new AbortController();
        const wd = attachStallWatchdog(emitter, controller, 1000);
        emitter.emit(AgentEventType.TOOL_CALL, {}); // tool starts
        vi.advanceTimersByTime(5000); // long tool — must NOT count as stall
        expect(wd.stalled()).toBe(false);
        emitter.emit(AgentEventType.TOOL_RESULT, {}); // tool done → re-arm
...
describe('runStallResilient', () => {
    it('returns the result on success (no stall, no retry)', async () => {
        let calls = 0;
        const result = await runStallResilient(async () => {
            calls += 1;
            return 'ok';
        }, { stallMs: 1000 });
    ...
    it('retries on stall up to MAX_STALL_ATTEMPTS then abandons', async () => {
        ...
        const attemptFn = async (signal, emitter) => {
            calls += 1;
    ...
    it('stallMs=0 runs a single raw attempt with the parent signal', async () => {
        ...
        let capturedSignal;
        let calls = 0;
        await runStallResilient(async (signal) => {
            calls += 1;
            capturedSignal = signal;
            return 'ok';
...
//# sourceMappingURL=workflow-stall.test.js.map