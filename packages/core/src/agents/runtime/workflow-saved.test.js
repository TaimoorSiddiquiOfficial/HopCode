...
import { listSavedWorkflows, resolveSavedWorkflowScript, saveWorkflowScript, validateWorkflowName, WORKFLOW_NAME_PATTERN, } from './workflow-saved.js';
/**
 * Build a Config whose `.storage` points at `projectDir`, and point the
 * user scope (`~/.hopcode`) at `userHome` via the HOPCODE_HOME env override so
 * tests never touch the real home directory.
 */
function fakeConfig(projectDir) {
    return { storage: new Storage(projectDir) };
}
async function writeWorkflow(dir, name, body) {
...
describe('workflow-saved', () => {
    ...
    beforeEach(async () => {
        ...
        // Storage.getGlobalhopcodeDir() reads HOPCODE_HOME, else ~/.hopcode. Point it at
        // `<userHome>/.hopcode` so the user scope is sandboxed.
        process.env['HOPCODE_HOME'] = path.join(userHome, '.hopcode');
    });
    afterEach(async () => {
    ...
    describe('resolveSavedWorkflowScript — by name', () => {
        it('resolves a project-scope workflow', async () => {
            await writeWorkflow(new Storage(projectDir).getProjectWorkflowsDir(), 'foo', `return 'project-foo';`);
            const resolved = await resolveSavedWorkflowScript('foo', fakeConfig(projectDir));
        ...
        it('resolves a user-scope workflow when project lacks it', async () => {
            await writeWorkflow(Storage.getUserWorkflowsDir(), 'bar', `return 'user-bar';`);
            const resolved = await resolveSavedWorkflowScript('bar', fakeConfig(projectDir));
            expect(resolved.script).toBe(`return 'user-bar';`);
        });
        it('project scope wins over user scope for the same name', async () => {
            await writeWorkflow(new Storage(projectDir).getProjectWorkflowsDir(), 'dup', `return 'PROJECT';`);
            await writeWorkflow(Storage.getUserWorkflowsDir(), 'dup', `return 'USER';`);
            const resolved = await resolveSavedWorkflowScript('dup', fakeConfig(projectDir));
            expect(resolved.script).toBe(`return 'PROJECT';`);
        });
        it('throws with available names on a miss', async () => {
            await writeWorkflow(new Storage(projectDir).getProjectWorkflowsDir(), 'alpha', `return 1;`);
            await expect(resolveSavedWorkflowScript('missing', fakeConfig(projectDir))).rejects.toThrow(/no workflow with that name. Available: alpha/);
        });
        it('throws "(none)" when no saved workflows exist', async () => {
            await expect(resolveSavedWorkflowScript('missing', fakeConfig(projectDir))).rejects.toThrow(/Available: \(none\)/);
        });
    });
    describe('resolveSavedWorkflowScript — by {scriptPath}', () => {
        it('reads a script path inside a saved-workflow dir', async () => {
            ...
            const resolved = await resolveSavedWorkflowScript({ scriptPath: p }, fakeConfig(projectDir));
        ...
        it('throws a clear error for a missing path under a saved dir', async () => {
            const dir = new Storage(projectDir).getProjectWorkflowsDir();
            await fs.mkdir(dir, { recursive: true });
            await expect(resolveSavedWorkflowScript({ scriptPath: path.join(dir, 'nope.js') }, fakeConfig(projectDir))).rejects.toThrow(/scriptPath/);
        });
        it('rejects an empty scriptPath', async () => {
            await expect(resolveSavedWorkflowScript({ scriptPath: '' }, fakeConfig(projectDir))).rejects.toThrow(/workflow name \(string\) or \{scriptPath/);
        });
        ...
        it('refuses a scriptPath outside the saved-workflow directories', async () => {
            ...
            await expect(resolveSavedWorkflowScript({ scriptPath: outside }, fakeConfig(projectDir))).rejects.toThrow(/outside the saved-workflow directories/);
        });
    });
    ...
    describe('resolveSavedWorkflowScript — name traversal', () => {
        it('rejects a traversal name before any path join', async () => {
            await expect(resolveSavedWorkflowScript('../../outside', fakeConfig(projectDir))).rejects.toThrow(/Invalid workflow name|lower-case/);
        });
    });
    describe('listSavedWorkflows', () => {
        it('merges both scopes, project shadows user, sorted by name', async () => {
            await writeWorkflow(new Storage(projectDir).getProjectWorkflowsDir(), 'zeta', `return 1;`);
            await writeWorkflow(new Storage(projectDir).getProjectWorkflowsDir(), 'shared', `return 'P';`);
            await writeWorkflow(Storage.getUserWorkflowsDir(), 'alpha', `return 1;`);
            await writeWorkflow(Storage.getUserWorkflowsDir(), 'shared', `return 'U';`);
    ...
    describe('saveWorkflowScript', () => {
        ...
        it('writes a user-scope workflow under the sandboxed HOPCODE_HOME', async () => {
            const config = fakeConfig(projectDir);
            const result = await saveWorkflowScript(config, {
                name: 'user-flow',
                scope: 'user',
            ...
            if (result.status !== 'saved')
                throw new Error('expected saved');
            expect(result.path).toContain(path.join(userHome, '.hopcode'));
        ...
        it('overwrites when overwrite:true', async () => {
            ...
            if (result.status !== 'saved')
                throw new Error('expected saved');
            expect(await fs.readFile(result.path, 'utf8')).toBe('return "replacement";');
        });
    });
    ...
    describe('security — symlinked root workflow dir', () => {
        ...
        beforeEach(async () => {
            ...
            await fs.writeFile(path.join(external, 'leak.js'), `return 'EXFILTRATED';`, 'utf8');
            // Make `<projectDir>/.hopcode/workflows` a symlink to that external dir.
        ...
        it("workflow('leak') is refused, not read, through a symlinked root", async () => {
            await expect(resolveSavedWorkflowScript('leak', fakeConfig(projectDir))).rejects.toThrow(/no workflow with that name/);
        });
        it('{scriptPath} into a symlinked root is refused', async () => {
            const p = path.join(projectWorkflowsDir, 'leak.js');
            await expect(resolveSavedWorkflowScript({ scriptPath: p }, fakeConfig(projectDir))).rejects.toThrow(/outside the saved-workflow directories/);
        });
        it('save into a symlinked root is refused (no write-through)', async () => {
            await expect(saveWorkflowScript(fakeConfig(projectDir), {
                name: 'planted',
                scope: 'project',
                script: 'return 1;',
            })).rejects.toThrow(/symlinked saved-workflow director/i);
            // Nothing was written through the link.
            await expect(fs.access(path.join(external, 'planted.js'))).rejects.toThrow();
        });
    });
});
//# sourceMappingURL=workflow-saved.test.js.map