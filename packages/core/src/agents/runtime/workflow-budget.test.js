...
import { WorkflowBudgetImpl, WorkflowBudgetExceededError, resolveMaxTokensPerWorkflow, MAX_TOKENS_PER_WORKFLOW_ENV, HARD_MAX_TOKENS_CEILING, } from './workflow-budget.js';
describe('resolveMaxTokensPerWorkflow', () => {
    ...
    it('returns null when env is empty / whitespace', () => {
        expect(resolveMaxTokensPerWorkflow({ [MAX_TOKENS_PER_WORKFLOW_ENV]: '' })).toBeNull();
        expect(resolveMaxTokensPerWorkflow({ [MAX_TOKENS_PER_WORKFLOW_ENV]: '   ' })).toBeNull();
    });
    it('parses a positive integer env value', () => {
        expect(resolveMaxTokensPerWorkflow({
    ...
    it('returns null on hex / scientific / non-decimal-integer overrides', () => {
        ...
        for (const raw of ['0x2BF20', '1e6', '5.0']) {
            expect(resolveMaxTokensPerWorkflow({ [MAX_TOKENS_PER_WORKFLOW_ENV]: raw })).toBeNull();
        }
    });
...
//# sourceMappingURL=workflow-budget.test.js.map