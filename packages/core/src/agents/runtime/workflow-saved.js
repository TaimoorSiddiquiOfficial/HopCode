 ...
 * `.hopcode/workflows/<name>.js` (project) or `~/.hopcode/workflows/<name>.js`
 ...
 * `.hopcode/workflows -> /outside` link into the allowed boundary — letting discovery
...
async function readWorkflowFileSecurely(filePath, config) {
    ...
    const dirs = (await Promise.all(getSavedWorkflowDirs(config).map(async ({ dir }) => {
        ...
        // `.hopcode/workflows -> /outside` link into the allowed boundary, so a
...
export async function saveWorkflowScript(config, opts) {
    ...
    // Refuse to write through a symlinked root (e.g. `.hopcode/workflows -> /outside`):
