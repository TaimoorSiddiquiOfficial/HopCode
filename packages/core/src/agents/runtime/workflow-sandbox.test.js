...
import { stripExportMeta, extractAndStripMeta, createWorkflowSandbox, } from './workflow-sandbox.js';
...
return 1`;
    ...
    it('throws on unbalanced meta braces (does not silently delete script)', () => {
        expect(() => stripExportMeta(`export const meta = { name: 'x'`)).toThrow(/unbalanced/i);
    });
    it('throws on meta with unterminated string', () => {
        expect(() => stripExportMeta(`export const meta = { name: 'foo }\nreturn 1`)).toThrow(/unbalanced/i);
    });
...
describe('extractAndStripMeta', () => {
    ...
    it('throws upstream-verbatim error when name is missing', () => {
        ...
        expect(() => extractAndStripMeta(src)).toThrow(/^meta\.name must be a non-empty string$/);
    });
    it('throws upstream-verbatim error when description is missing', () => {
        const src = `export const meta = { name: 'x' }\nreturn 1`;
        expect(() => extractAndStripMeta(src)).toThrow(/^meta\.description must be a non-empty string$/);
    });
    ...
    it('throws when a phase is missing its title', () => {
        ...
        expect(() => extractAndStripMeta(src)).toThrow(/phases\[\]\.title must be a non-empty string/);
    });
    ...
    it('meta source cannot reach the host process / require / fs', () => {
        ...
        expect(() => extractAndStripMeta(src1)).toThrow(/failed to evaluate meta object literal/);
        const src2 = `export const meta = { name: 'x', description: require('fs').readFileSync('/etc/passwd', 'utf8') }\nreturn 1`;
        expect(() => extractAndStripMeta(src2)).toThrow(/failed to evaluate meta object literal/);
    });
    ...
    it('throws when meta value is a Promise (dynamic import) — no unhandled rejection crash', () => {
        ...
        expect(() => extractAndStripMeta(src)).toThrow(/meta values must not be Promises/);
    });
    ...
    it('consecutive identical phase titles dedup at the sandbox layer', async () => {
        ...
        await sandbox.run(`phase('X'); phase('X'); phase('Y'); phase('X'); return 1`);
...
describe('createWorkflowSandbox', () => {
    ...
    it('Math.random() throws inside sandbox', async () => {
        ...
        await expect(sandbox.run(`return Math.random()`)).rejects.toThrow(/Math\.random/);
    });
    ...
    it('getMeta() returns the parsed meta when present', async () => {
        ...
        const result = await sandbox.run(`export const meta = { name: 'unit', description: 'unit-test workflow', phases: [{ title: 'one' }] }\nreturn 'done'`);
    ...
    it('getMeta() failure on malformed meta propagates as the run rejection', async () => {
        ...
        await expect(sandbox.run(`export const meta = { name: 'x' }\nreturn 1`)).rejects.toThrow(/^meta\.description must be a non-empty string$/);
    });
});
...
describe('createWorkflowSandbox security', () => {
    ...
    it('agent() rejects unknown opts (typo guard)', async () => {
        ...
        await expect(sandbox.run(`return agent("hi", { scema: { type: 'object' } });`)).rejects.toThrow(/scema.*unknown option/);
    });
    ...
    it('synchronous infinite loop is aborted by vm timeout', async () => {
        ...
        await expect(sandbox.run(`while(true){}`)).rejects.toThrow(/Script execution timed out/i);
    ...
    it('agent({schema}) is passed through to dispatch in P3', async () => {
        ...
        const result = await sandbox.run(`return await agent("hi", { schema: { type: "object", properties: { ok: { type: "boolean" } } } });`);
        expect(seen).toHaveLength(1);
        expect(seen[0].opts.schema).toBeDefined();
    ...
    it('phase() caps at MAX_PHASE_ENTRIES with a truncation marker', async () => {
        ...
        await sandbox.run(`for (let i = 0; i < 10100; i++) phase("p"+i); return 0;`);
    ...
    it('new Date() throws inside sandbox', async () => {
        ...
        await expect(sandbox.run(`return new Date()`)).rejects.toThrow(/unavailable in workflow scripts/i);
    });
    it('Date() (bare call) throws inside sandbox', async () => {
        ...
        await expect(sandbox.run(`return Date()`)).rejects.toThrow(/unavailable in workflow scripts/i);
    });
    it('Date.UTC() throws inside sandbox', async () => {
        ...
        await expect(sandbox.run(`return Date.UTC(2026, 0, 1)`)).rejects.toThrow(/unavailable in workflow scripts/i);
    });
    ...
    `);
        expect(result).not.toMatch(/object|darwin|linux|win32/i);
        expect(['no-ctor', 'no-inner-ctor', 'undefined', 'threw']).toContain(result);
    });
    ...
    it('rejects an async never-resolving Promise via wall-clock timeout', async () => {
        ...
        await expect(sandbox.run(`return new Promise(() => {});`)).rejects.toThrow(/timed out after 100 ms wall clock/);
    });
    ...
    it('HOPCODE_CODE_MAX_WORKFLOW_SECONDS env var sets the wall-clock cap', async () => {
        process.env['HOPCODE_CODE_MAX_WORKFLOW_SECONDS'] = '0.1';
        try {
            const sandbox = createWorkflowSandbox({
                args: undefined,
                dispatch: async () => 'ignored',
            ...
            delete process.env['HOPCODE_CODE_MAX_WORKFLOW_SECONDS'];
        }
    });
    ...
    it('parallel() throws an availability error rather than ReferenceError when not injected', async () => {
        ...
        await expect(sandbox.run(`return parallel([() => agent("a")]);`)).rejects.toThrow(/parallel\(\) is unavailable/);
    });
    it('pipeline() throws an availability error rather than ReferenceError when not injected', async () => {
        ...
        await expect(sandbox.run(`return pipeline([1, 2], x => x, x => x);`)).rejects.toThrow(/pipeline\(\) is unavailable/);
    });
    it('workflow() throws "unavailable" when no workflow impl is injected (bare sandbox / nesting limit)', async () => {
        ...
        await expect(sandbox.run(`return workflow('child', { foo: 1 });`)).rejects.toThrow(/workflow\(\) is unavailable here/);
    });
    it('workflow() resolves through the injected host impl (single result revived)', async () => {
        ...
        const result = await sandbox.run(`return await workflow('child', { foo: 1 });`);
        expect(result).toEqual({
            echoedRef: 'child',
            echoedArgs: { foo: 1 },
        });
    ...
    it('budget.spent() / .remaining() throw with clear P1-unsupported errors', async () => {
        ...
        await expect(sandbox.run(`return budget.spent();`)).rejects.toThrow(/budget\.spent.*not supported in P1/);
        await expect(sandbox.run(`return budget.remaining();`)).rejects.toThrow(/budget\.remaining.*not supported in P1/);
    });
    ...
    it('Date.parse() throws inside sandbox', async () => {
        ...
        await expect(sandbox.run(`return Date.parse("2026-01-01")`)).rejects.toThrow(/unavailable in workflow scripts/i);
    });
    ...
    it('agent() rejects unknown isolation mode with clear error', async () => {
        ...
        await expect(sandbox.run(`return agent("hi", { isolation: "not-a-real-mode" });`)).rejects.toThrow(/unknown isolation mode/);
    });
    it('agent({isolation:"worktree"}) is passed through to dispatch in P3', async () => {
        ...
        const result = await sandbox.run(`return await agent("x", { isolation: "worktree" });`);
        expect(result).toBe('done');
        expect(seen[0].opts.isolation).toBe('worktree');
    });
    it('agent({isolation:"remote"}) is passed through to dispatch in P3', async () => {
        ...
        expect(seen[0].opts.isolation).toBe('remote');
    });
    it('agent({model}) is passed through to dispatch in P3', async () => {
        ...
        expect(seen[0].opts.model).toBe('qwen3-max');
    });
    it('agent({agentType}) is passed through to dispatch in P3', async () => {
        ...
        expect(seen[0].opts.agentType).toBe('Explore');
    });
    ...
    it('agent() object return that cannot serialize collapses to null', async () => {
        ...
        const result = await sandbox.run(`return await agent("x", { schema: { type: "object" } });`);
        expect(result).toBeNull();
    });
...
describe('createWorkflowSandbox primitives', () => {
    ...
    it('agent() invokes dispatch and resolves with its return value', async () => {
        ...
        const result = await sandbox.run(`const a = await agent("write hello", { label: "h1" });
       return a;`);
...
//# sourceMappingURL=workflow-sandbox.test.js.map