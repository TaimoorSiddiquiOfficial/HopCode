...
import { ContextState, AgentHeadless, templateString, } from './agent-headless.js';
...
vi.mock('../../core/contentGenerator.js', async (importOriginal) => {
    ...
    const { DEFAULT_HOPCODE_MODEL } = await import('../../config/models.js');
    return {
        ...actual,
...
describe('subagent.ts', () => {
    ...
    describe('templateString', () => {
        it('should replace valid identifier placeholders', () => {
            ...
            const result = templateString('Hello ${name}, your task is ${task}.', context);
        ...
        it('should treat ${1} and ${2} as literal text', () => {
            const context = new ContextState();
            const result = templateString('Use {0} and {1}, not ${0} or ${1}.', context);
            expect(result).toBe('Use {0} and {1}, not ${0} or ${1}.');
        });
        it('should still throw for missing valid identifier placeholders', () => {
            const context = new ContextState();
            context.set('name', 'Agent');
            expect(() => templateString('Hello ${name}, missing ${missing}.', context)).toThrow('Missing context values for the following keys: missing');
        });
        it('should handle mixed numeric and identifier placeholders', () => {
            ...
            expect(() => templateString('${var} and ${0} and ${_private}', context)).toThrow('Missing context values for the following keys: _private');
        });
        it('should handle ${0} alongside valid placeholders without error', () => {
            const context = new ContextState();
            context.set('name', 'Agent');
            const result = templateString('Hello ${name}. Do not write ${0} or ${1}.', context);
            expect(result).toBe('Hello Agent. Do not write ${0} or ${1}.');
        });
    });
    describe('AgentHeadless', () => {
        let mockSendMessageStream;
        let mockGetHistoryFunctionResponseIds;
        ...
        beforeEach(async () => {
            ...
            mockGetHistoryFunctionResponseIds = vi.fn(() => new Set());
        ...
        describe('execute - Execution and Tool Use', () => {
            ...
            it('should ignore duplicate provider tool-call ids across rounds', async () => {
                ...
                expect(toolCallEvents[1].callId).toMatch(/^call_1__hopcode_dup_2:duplicate:/);
                expect(toolResultEvents[1].callId).toBe(toolCallEvents[1].callId);
                expect(toolResultEvents[1].error).toContain('Duplicate provider tool call id "call_1"');
                const thirdCallArgs = mockSendMessageStream.mock.calls[2][1];
                const parts = thirdCallArgs.message;
                expect(parts[0].functionResponse?.id).toBe('call_1__hopcode_dup_2');
                expect(parts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "call_1"');
                expect(scope.getTerminateMode()).toBe(AgentTerminateMode.GOAL);
            });
            ...
            it('should stop consecutive identical tool calls with fresh ids', async () => {
                ...
                const toolConfig = { tools: ['list_directory'] };
                ...
                const listDirectoryInvocation = {
                    ...
                    getDefaultPermission: vi.fn().mockResolvedValue('allow'),
                    execute: vi.fn().mockResolvedValue({
                        llmContent: 'Error: ENOENT: no such file or directory, scandir ' +
                            missingPath,
                ...
                vi.mocked(config.getToolRegistry().getTool).mockImplementation((name) => name === 'list_directory' ? listDirectoryTool : undefined);
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, toolConfig);
                await scope.execute(new ContextState());
            ...
            it('should ignore duplicate provider tool-call ids already present in chat history', async () => {
                ...
                mockSendMessageStream.mockImplementation(createMockStream([[duplicateNormalizedPart.functionCall], 'stop']));
                const listFilesInvocation = {
                    params: { path: '.' },
                    getDescription: vi.fn().mockReturnValue('List files'),
                    toolLocations: vi.fn().mockReturnValue([]),
                ...
                expect(toolCallEvents[0].callId).toMatch(/^call_1__hopcode_dup_2:duplicate:/);
                expect(toolResultEvents[0].callId).toBe(toolCallEvents[0].callId);
                expect(toolResultEvents[0].error).toContain('Duplicate provider tool call id "call_1"');
                const secondCallArgs = mockSendMessageStream.mock.calls[1][1];
                const parts = secondCallArgs.message;
                expect(parts[0].functionResponse?.id).toBe('call_1__hopcode_dup_2');
                expect(parts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "call_1"');
            ...
            it('should execute only the first duplicate functionCall id in one model turn', async () => {
                ...
                expect(parts
                    ...
                    .filter((id) => Boolean(id))).toEqual(['dup_id_0001']);
            });
            it('should report unauthorized tool names before duplicate provider ids', async () => {
                ...
                expect(parts[0].functionResponse?.response?.['error']).toContain('Tool "write_file" not found');
                expect(parts[0].functionResponse?.response?.['error']).not.toContain('Duplicate provider tool call id');
                expect(scope.getTerminateMode()).toBe(AgentTerminateMode.GOAL);
            });
        });
        ...
        describe('execute - Tool Restriction Enforcement (Issue #1121)', () => {
            ...
            it('keeps automatic max token escalation warm for the next agent round', async () => {
                ...
                await scope.execute(new ContextState());
                expect(mockSendMessageStream).toHaveBeenCalledTimes(2);
                expect(mockSendMessageStream.mock.calls[0][1].config.maxOutputTokens).toBeUndefined();
                expect(mockSendMessageStream.mock.calls[1][1].config.maxOutputTokens).toBe(65_536);
            });
        });
    });
});
