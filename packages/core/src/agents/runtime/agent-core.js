...
import { dedupeToolCallsById, getProviderToolCallId, } from '../../core/toolCallIdUtils.js';
...
export class AgentCore {
        ...
        while (true) {
            ...
            try {
                ...
                const responseStream = await chat.sendMessageStream(this.modelConfig.model ||
                    ...
                    DEFAULT_HOPCODE_MODEL, messageParams, promptId);
                ...
                if (functionCalls.length > 0) {
                    const toolCallResult = await this.processFunctionCalls(functionCalls, roundAbortController, promptId, turnCounter, toolsList, currentResponseId, wasOutputTruncated, handledProviderToolCallIds, duplicateProviderToolCallResponseIds);
     ...
     */
    async processFunctionCalls(functionCalls, abortController, promptId, currentRound, toolsList, responseId, wasOutputTruncated = false, handledProviderToolCallIds = new Set(), duplicateProviderToolCallResponseIds = new Set()) {
        ...
        const repeatedDuplicateCall = findRepeatedDuplicateProviderToolCall(uniqueFunctionCalls, (fc) => getProviderToolCallId(fc) ?? fc.id, handledProviderToolCallIds, duplicateProviderToolCallResponseIds);
