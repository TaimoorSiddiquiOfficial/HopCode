...
export const MAX_WORKFLOW_CONCURRENCY_ENV = 'HOPCODE_CODE_MAX_WORKFLOW_CONCURRENCY';
/**
...
onTokens) {
    return async (prompt, opts) => {
        ...
        return runStallResilient((attemptSignal, emitter) => runSingleDispatch(config, prompt, opts, attemptSignal, emitter, onTokens), {
            stallMs,
            signal,
...
async function runSingleDispatch(config, prompt, opts, attemptSignal, emitter, onTokens) {
    ...
    if (opts.agentType === undefined &&
        ...
        const subagent = await AgentHeadless.create(opts.label ?? 'workflow-agent', config, {
    ...
    return runOverridePath(config, ctx, opts, attemptSignal, workflowAgentId, onTokens, emitter);
}
/**
...
emitter) {
    ...
    if (opts.isolation === 'worktree') {
        ...
        effectiveContext = createWorktreeConfigOverride(config, worktreeIsolation.path);
    }
...
export class WorkflowOrchestrator {
    ...
    async run(req) {
        ...
        const countedDispatch = (prompt, opts) => {
            ...
            // (HOPCODE_MAX_TOKENS_PER_WORKFLOW), fail-fast at fire time if the
            ...
            if (budget && budget.total !== null && budget.remaining() <= 0) {
                ...
                return Promise.reject(new WorkflowBudgetExceededError(runId, budget.total, budget.spent()));
            }
            // P5 R3 (wenshao #7): agent-count cap runs AFTER the budget gate.
            // See the reordering rationale at the top of countedDispatch.
            agentCount += 1;
            if (agentCount > maxAgents) {
                return Promise.reject(new Error(`Workflow exceeded the maximum of ${maxAgents} agent() calls per run.`));
            }
...
function makePipelineImpl(signal) {
    return (items, ...stages) => {
        ...
        const chains = items.map((item, idx) => () => runPipelineChain(item, idx, stages));
