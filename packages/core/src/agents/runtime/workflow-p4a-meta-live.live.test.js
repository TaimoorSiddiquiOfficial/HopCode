...
import { WorkflowOrchestrator, WorkflowExecutionError, } from './workflow-orchestrator.js';
...
const liveDispatch = async (prompt, _opts) => {
    const res = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a workflow subagent. Return the final answer as plain text only — no preamble, no markdown, no quotes. Keep the answer terse (≤ 1 sentence).',
                },
...
describeOrSkip('P4a real-LLM E2E (DashScope qwen3-coder-plus)', () => {
    it('A: meta declaration parsed + agent call returns real LLM text', async () => {
        ...
        const metaPhases = outcome.meta.phases;
        ...
        expect(outcome.result.answer.toLowerCase()).toMatch(/paris/);
    }, 60_000);
    ...
    it('E: real-world parallel() fan-out — meta.phases titles match executed phases', async () => {
        ...
        const declaredTitles = outcome.meta.phases.map((p) => p.title);
        expect(declaredTitles).toEqual(outcome.phases);
        const verdicts = outcome.result.verdicts;
    ...
    it('F: real-world pipeline() multi-stage — meta.phases declares both, both run', async () => {
        ...
        const tap = async (prompt, opts) => {
            if (prompt.includes('Translate'))
                translateCalls++;
            else if (prompt.includes('uppercase'))
                upperCalls++;
            return liveDispatch(prompt, opts);
        };
        const orch = new WorkflowOrchestrator(tap);
...
//# sourceMappingURL=workflow-p4a-meta-live.live.test.js.map