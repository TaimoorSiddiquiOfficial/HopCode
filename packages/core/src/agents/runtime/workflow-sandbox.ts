...
export interface WorkflowAgentOpts {
   ...
   * override `HOPCODE_CODE_WORKFLOW_STALL_SECONDS`). `0` disables the watchdog
   * for this call.
   */
  stallMs?: number;
...
export interface WorkflowOrchestratorEmitter {
   ...
   * (`HOPCODE_MAX_TOKENS_PER_WORKFLOW` unset). Caller (the
...
export interface SandboxOptions {
   ...
   * (by name from `.hopcode/workflows/<name>.js`, or by `{scriptPath}`) and runs
   ...
   *
   * Defaults to 30 minutes, override via `HOPCODE_CODE_MAX_WORKFLOW_SECONDS`
...
function resolveMaxWallClockMs(opts: SandboxOptions): number {
  ...
  const envSec = Number(process.env['HOPCODE_CODE_MAX_WORKFLOW_SECONDS']);
          ...
          reject(
            new Error(
              `Workflow execution timed out after ${maxWallClockMs} ms wall clock. ` +
                'Override via SandboxOptions.maxWallClockMs or HOPCODE_CODE_MAX_WORKFLOW_SECONDS env var.',
            ),
          );
        }, maxWallClockMs);
