...
import { getCurrentAgentDepth, getCurrentAgentId, getRuntimeContentGenerator, runWithAgentContext, runWithRuntimeContentGenerator, } from './agent-context.js';
import { subagentNameContext } from '../../utils/subagentNameContext.js';
import { runInForkContext } from '../../tools/agent/fork-subagent.js';
import { ToolNames } from '../../tools/tool-names.js';
import { getAgentName, getTeammateContext, isTeammate, runWithTeammateIdentity, } from '../team/identity.js';
describe('AgentCore.runInAgentFrames', () => {
    ...
    it('restores the nesting depth for deferred-approval continuations', async () => {
        ...
        await runWithAgentContext('lvl1', () => runWithAgentContext('lvl2', () => runWithAgentContext('lvl3', async () => {
            const inheritedAgentId = getCurrentAgentId();
            const inheritedAgentDepth = getCurrentAgentDepth();
            expect(inheritedAgentDepth).toBe(2);
            respondClosure = () => core.runInAgentFrames(onConfirm, undefined, inheritedAgentId ?? undefined, undefined, inheritedAgentDepth);
        })));
    ...
    it('restores the teammate identity for deferred-approval continuations', async () => {
        ...
        let respondClosure;
        let observedAgentName;
        let observedIsTeammate;
        const onConfirm = async () => {
        ...
        await runWithTeammateIdentity(teammateIdentity, async () => {
            ...
            respondClosure = () => core.runInAgentFrames(onConfirm, undefined, undefined, inherited);
        });
...
describe('AgentCore.prepareTools', () => {
    ...
    function buildAgentForTools(toolConfig, fnDeclarations, maxSubagentDepth = 5) {
        const debugSpy = vi.fn();
        const getFunctionDeclarationsSpy = vi.fn().mockReturnValue(fnDeclarations);
        const getFunctionDeclarationsFilteredSpy = vi.fn((names) => fnDeclarations.filter((d) => d.name && names.includes(d.name)));
        const config = {
    ...
    it('does not re-enable plan lifecycle tools via explicit tool names', async () => {
        const fnDecls = [
            { name: ToolNames.READ_FILE, description: 'read' },
            {
                name: ToolNames.ENTER_PLAN_MODE,
                description: 'enter plan mode',
            },
        ...
        const { core, debugSpy, getFunctionDeclarationsFilteredSpy } = buildAgentForTools({
            tools: [
                ToolNames.READ_FILE,
        ...
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered "${ToolNames.ENTER_PLAN_MODE}" from explicit subagent tool list`);
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered "${ToolNames.EXIT_PLAN_MODE}" from explicit subagent tool list`);
    });
    it('filters inline declarations using the full subagent exclusion floor', async () => {
        ...
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered inline declaration "${ToolNames.SEND_MESSAGE}" from subagent tool list`);
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered inline declaration "${ToolNames.TASK_UPDATE}" from subagent tool list`);
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered inline declaration "${ToolNames.ENTER_PLAN_MODE}" from subagent tool list`);
        expect(debugSpy).toHaveBeenCalledWith(`[prepareTools] Filtered inline declaration "${ToolNames.EXIT_PLAN_MODE}" from subagent tool list`);
    });
    ...
    it.each([ToolNames.ENTER_PLAN_MODE, ToolNames.EXIT_PLAN_MODE])('returns a dedicated message when filtered %s is called directly', async (toolName) => {
        const { core } = buildAgentForTools(undefined, []);
        const result = await runWithAgentContext('test-subagent', () => core.runInAgentFrames(() => core.processFunctionCalls([
            {
                name: toolName,
                args: { plan: 'Plan from filtered tool' },
                id: 'call-1',
            },
        ], new AbortController(), 'prompt-filtered-plan-tool', 1, [{ name: ToolNames.READ_FILE }])));
    ...
    it('nesting: excludes AgentTool at the leaf depth', async () => {
        ...
        const tools = await runWithAgentContext('lvl1', () => runWithAgentContext('lvl2', () => core.prepareTools()));
    ...
    it('nesting: fork execution contexts never receive the AgentTool', async () => {
        ...
        const { core } = buildAgentForTools({ tools: ['*'] }, nestingDecls(), 5);
        const tools = await runInForkContext(() => runWithAgentContext('lvl1', () => core.prepareTools()));
        const names = tools.map((t) => t.name);
        expect(names).not.toContain(ToolNames.AGENT);
        expect(names).toContain('read_file');
    });
    it('nesting: teammates never receive the AgentTool regardless of depth', async () => {
        ...
        const tools = await runWithTeammateIdentity(identity, () => runWithAgentContext('lvl1', () => core.prepareTools()));
        expect(tools.map((t) => t.name)).not.toContain(ToolNames.AGENT);
    });
    it('nesting: explicit tools list includes AgentTool only when nesting is allowed', async () => {
        const allowed = buildAgentForTools({ tools: ['read_file', ToolNames.AGENT] }, nestingDecls(), 5);
        const allowedTools = await runWithAgentContext('lvl1', () => allowed.core.prepareTools());
        expect(allowedTools.map((t) => t.name)).toContain(ToolNames.AGENT);
        const denied = buildAgentForTools({ tools: ['read_file', ToolNames.AGENT] }, nestingDecls(), 1);
        const deniedTools = await runWithAgentContext('lvl1', () => denied.core.prepareTools());
