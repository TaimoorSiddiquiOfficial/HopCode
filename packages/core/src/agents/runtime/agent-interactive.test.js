...
import { getCurrentAgentDepth, getCurrentAgentId, runWithAgentContext, } from './agent-context.js';
...
describe('AgentInteractive', () => {
    ...
    it('pins the construction-time depth when built inside a sub-agent frame', async () => {
        ...
        const agent = await runWithAgentContext('parent-agent', async () => new AgentInteractive(createConfig({ initialTask: 'go' }), core));
        await agent.start(context);
        await vi.waitFor(() => {
            expect(agent.getStatus()).toBe('idle');
        });
    ...
    it('processes a message enqueued synchronously during the IDLE status emit', async () => {
        ...
        const processed = [];
        core.runReasoningLoop.mockImplementation((_chat, initialMessages) => {
