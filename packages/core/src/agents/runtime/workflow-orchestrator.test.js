...
import { WorkflowOrchestrator, WorkflowExecutionError, createProductionDispatch, DEFAULT_MAX_AGENTS_PER_RUN, resolveMaxAgentsPerRun, resolveConcurrencyLimit, } from './workflow-orchestrator.js';
...
const { created, nextFinalText, nextTerminateMode, nextOutputTokens, nextExecuteThrow, } = vi.hoisted(() => ({
    created: [],
...
const worktreeStubs = vi.hoisted(() => {
    const makeStub = () => ({
        ...
        createUserWorktree: vi.fn(async (slug, _base, _options) => ({
            success: true,
            worktree: {
                path: `/fake/repo/.hopcode/worktrees/${slug}`,
                branch: `worktree-${slug}`,
            },
        })),
        removeUserWorktree: vi.fn(async () => ({ success: true })),
    });
    return { makeStub, instances: [] };
});
...
vi.mock('./agent-headless.js', () => ({
    AgentHeadless: {
        create: async (name, _runtimeContext, promptConfig, _modelConfig, runConfig, toolConfig, 
        ...
        _eventEmitter, _hooks, _runtimeView) => ({
            execute: async (ctx, signal) => {
                ...
                if (!promptConfig.systemPrompt?.includes('subagent spawned by a workflow')) {
                    throw new Error('orchestrator did not pass workflow subagent system prompt');
                }
            ...
            getFinalText: () => nextFinalText.value ??
...
describe('WorkflowOrchestrator', () => {
    it('runs a script with injected mock dispatch and returns the script value', async () => {
        const orchestrator = new WorkflowOrchestrator(async (prompt) => `mock:${prompt}`);
    ...
    it('emitter callbacks fire in expected order with expected args', async () => {
        ...
        const emitter = {
            ...
            agentDispatched: (label) => events.push({ kind: 'dispatched', payload: label }),
            agentCompleted: (label, error) => events.push({ kind: 'completed', payload: { label, error } }),
    ...
    it('P5 T4: budgetUpdated does NOT fire when no budget is passed', async () => {
        ...
        const budgetUpdates = [];
        const emitter = {
            budgetUpdated: (spent) => budgetUpdates.push(spent),
        };
        await orchestrator.run({
    ...
    it('P-nested: workflow(name) resolves via injected resolver and returns nested result', async () => {
        const orchestrator = new WorkflowOrchestrator(async (prompt) => `agent:${prompt}`);
        const resolveSavedWorkflow = async (ref) => {
    ...
    it('P-nested: nested agents share the parent agent-count cap', async () => {
        ...
        process.env['HOPCODE_CODE_MAX_WORKFLOW_AGENTS'] = '2';
        try {
            let dispatchCalls = 0;
            const orchestrator = new WorkflowOrchestrator(async () => {
                dispatchCalls += 1;
            ...
            if (prev === undefined)
                ...
                process.env['HOPCODE_CODE_MAX_WORKFLOW_AGENTS'] = prev;
        }
    });
    ...
    it('P6: a normal run journals a started+result per agent() call', async () => {
        ...
        const orchestrator = new WorkflowOrchestrator(async (prompt) => `r:${prompt}`);
        await orchestrator.run({
    ...
    it('P6: first miss runs live and the suffix goes live (first-miss invalidates suffix)', async () => {
        ...
        const outcome = await orch2.run({
            script: `const a = await agent('a');
               ...
               return [a, b, c].join('|');`,
            args: undefined,
            journal: { append: () => Promise.resolve() },
            resumeReplay: buildReplay(entries),
        });
    ...
    it('P6: cached dispatches do NOT consume the agent-count cap', async () => {
        ...
        try {
            ...
            process.env['HOPCODE_CODE_MAX_WORKFLOW_AGENTS'] = '10';
...
describe('createProductionDispatch', () => {
    ...
    it('strips internal tags from fast-path final text', async () => {
        ...
        const dispatch = createProductionDispatch(fakeConfig());
        await expect(dispatch('hello', { label: 'h1' })).resolves.toBe('clean result');
    });
    ...
    it.each([['CANCELLED'], ['MAX_TURNS'], ['TIMEOUT'], ['ERROR']])('throws when subagent terminate mode is %s', async (mode) => {
        nextTerminateMode.value = mode;
        const dispatch = createProductionDispatch(fakeConfig());
        await expect(dispatch('hello', { label: 'h1' })).rejects.toThrow(new RegExp(`workflow-agent-[0-9a-f]{16} did not complete \\(terminate mode: ${mode}\\)\\.`));
    });
    ...
    it('R1 #3: records tokens on GOAL success', async () => {
        ...
        const reports = [];
        const dispatch = createProductionDispatch(fakeConfig(), undefined, (tokens, opts) => reports.push({ tokens, label: opts.label }));
        await dispatch('q1', { label: 'a' });
        expect(reports).toEqual([{ tokens: 1234, label: 'a' }]);
    });
    it.each(['CANCELLED', 'MAX_TURNS', 'TIMEOUT', 'ERROR'])('R1 #3: records tokens on %s failure path (still throws)', async (mode) => {
        nextTerminateMode.value = mode;
        nextOutputTokens.value = 777;
        const reports = [];
        const dispatch = createProductionDispatch(fakeConfig(), undefined, (tokens) => reports.push(tokens));
        await expect(dispatch('q1', { label: 'doomed' })).rejects.toThrow(new RegExp(`terminate mode: ${mode}`));
    ...
    it('R1 #1 + #3: onTokens is undefined ⇒ no crash', async () => {
        ...
        const dispatch = createProductionDispatch(fakeConfig());
        await expect(dispatch('q1', { label: 'x' })).resolves.toBe('headless-said:q1');
    });
    ...
    it('R3 #6: records tokens when subagent.execute() THROWS (the real production ERROR path)', async () => {
        ...
        const reports = [];
        const dispatch = createProductionDispatch(fakeConfig(), undefined, (tokens) => reports.push(tokens));
        await expect(dispatch('q1', { label: 'thrown' })).rejects.toThrow(/reasoning-loop boom/);
...
describe('WorkflowOrchestrator P2 — parallel() / pipeline() / caps', () => {
    ...
    describe('pipeline()', () => {
        ...
        it('is staggered with no inter-stage barrier (item A reaches stage 2 while item B is still in stage 1)', async () => {
            ...
            try {
                let releaseItem1Stage1 = () => { };
                const item1Stage1Gate = new Promise((resolve) => {
                    releaseItem1Stage1 = resolve;
                });
                let item0ReachedStage2 = false;
                const orchestrator = new WorkflowOrchestrator(async (prompt) => {
                    if (prompt === 's1-0')
                        return 'ok'; // item 0's stage 1: fast
                    if (prompt === 's1-1') {
            ...
            (prev, item) => agent("s2-" + item),
                ...
                if (envPrev === undefined)
                    ...
                    process.env['HOPCODE_CODE_MAX_WORKFLOW_CONCURRENCY'] = envPrev;
            }
        }, 10_000);
    });
    describe('1000-agent cap', () => {
        it('the 1001st sequential agent() call throws the cap error', async () => {
          ...
          return "done";`,
                args: undefined,
            })).rejects.toThrow(new RegExp(`${DEFAULT_MAX_AGENTS_PER_RUN} agent\\(\\) calls per run`));
        });
        it('the cap counts agents launched via parallel() — a fan-out cannot bypass it', async () => {
          ...
          Array.from({ length: ${DEFAULT_MAX_AGENTS_PER_RUN + 1} }, () => () => agent("x"))
            ...
            const arr = outcome.result;
            // Exactly 1000 dispatches succeed; the one over the cap becomes null.
            expect(arr.filter((v) => v === 'ok')).toHaveLength(DEFAULT_MAX_AGENTS_PER_RUN);
        ...
        it('a parallel() inside a pipeline() stage does not deadlock at concurrency=1', async () => {
            ...
            (prev, item) => parallel([() => agent("a" + item)]),
                ...
                if (prev === undefined)
                    ...
                    process.env['HOPCODE_CODE_MAX_WORKFLOW_CONCURRENCY'] = prev;
            }
        }, 15_000);
    ...
    describe('env-overridable caps', () => {
        it('resolveMaxAgentsPerRun defaults to 1000 and honors a valid override', () => {
            ...
            expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: '50' })).toBe(50);
        });
        it('resolveMaxAgentsPerRun rejects a non-integer / <1 override and falls back', () => {
            expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: '0' })).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
            expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: 'abc' })).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
            expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: '2.5' })).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
        });
        it('resolveMaxAgentsPerRun rejects hex / scientific / non-decimal-integer overrides', () => {
            ...
            for (const raw of ['0x10', '1e3', '1.0']) {
                expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: raw })).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
            }
        });
        it('resolveConcurrencyLimit treats hex / scientific overrides as invalid (cpu default)', () => {
            ...
            for (const raw of ['0x10', '1e2']) {
                expect(resolveConcurrencyLimit({ HOPCODE_CODE_MAX_WORKFLOW_CONCURRENCY: raw })).toBe(cpuDefault);
            }
        });
        ...
        it('resolveMaxAgentsPerRun clamps an over-ceiling override to the hard maximum', () => {
            expect(resolveMaxAgentsPerRun({
                HOPCODE_CODE_MAX_WORKFLOW_AGENTS: '999999999',
            })).toBe(10_000);
            // Just under the ceiling is preserved.
            expect(resolveMaxAgentsPerRun({ HOPCODE_CODE_MAX_WORKFLOW_AGENTS: '9999' })).toBe(9999);
        });
        it('resolveConcurrencyLimit honors a valid override and clamps the cpu default to [1,16]', () => {
            expect(resolveConcurrencyLimit({ HOPCODE_CODE_MAX_WORKFLOW_CONCURRENCY: '4' })).toBe(4);
...
describe('WorkflowOrchestrator P3 — agentType / model / isolation / schema', () => {
    ...
    function fakeConfigWithMgr(opts) {
        ...
        const cfg = {
            ...
            getSubagentManager: () => ({
                ...
                createAgentHeadless: async (subagentConfig, runtimeContext, options) => {
                    const call = {
                    ...
                    calls.push(call);
                    const outcome = await opts.onCreate(call, options?.eventEmitter);
    ...
    it('agentType resolves SubagentConfig and routes through createAgentHeadless', async () => {
        ...
        expect(calls[0].config.disallowedTools).toEqual(expect.arrayContaining([
            'send_message',
            'monitor',
            'enter_plan_mode',
            'exit_plan_mode',
    ...
    it('strips internal tags from override-path final text', async () => {
        const { config } = fakeConfigWithMgr({
            ...
            onCreate: async () => ({
                finalText: '<analysis>scratch</analysis><summary>clean result</summary>',
                terminateMode: 'GOAL',
            }),
        });
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('find foo', { agentType: 'Explore' })).resolves.toBe('clean result');
    });
    it('agentType not found throws upstream-aligned error', async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('whatever', { agentType: 'NotARealAgent' })).rejects.toThrow(/^agent\(\{agentType\}\): agent type 'NotARealAgent' not found\.$/);
    });
    ...
    it("isolation:'remote' throws upstream-aligned 'not available' error", async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'remote' })).rejects.toThrow(/agent\(\{isolation:'remote'\}\) is not available in this build\./);
    });
    ...
    it('R1 #1: schema-mode SUCCESS records tokens via onTokens (was missing before fix)', async () => {
        ...
        const reports = [];
        const dispatch = createProductionDispatch(config, undefined, (tokens) => reports.push(tokens));
        const result = await dispatch('extract', {
            schema: { type: 'object', properties: { ok: { type: 'boolean' } } },
        });
        expect(result).toEqual({ ok: true, value: 42 });
    ...
    it('R3 #6: override-path records tokens when execute() THROWS (sibling of fast path)', async () => {
        ...
        const reports = [];
        const dispatch = createProductionDispatch(config, undefined, (tokens) => reports.push(tokens));
        await expect(dispatch('q1', { label: 'thrown', schema: { type: 'object' } })).rejects.toThrow(/override-path boom/);
    ...
    it('schema-mode: 3 failed structured_output calls → upstream-aligned terminal error', async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('extract', {
            schema: { type: 'object' },
        })).rejects.toThrow(/subagent completed without calling StructuredOutput \(after 2 in-conversation nudges\)\./);
    });
    ...
    it('schema-mode: subagent never calls structured_output → "no validation attempt" terminal', async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('extract', { schema: { type: 'object' } })).rejects.toThrow(/subagent completed without calling structured_output \(no validation attempt — model produced plain-text content\)\./);
    });
    ...
    it('schema-mode: caller abort takes priority over terminal "no structured_output" error', async () => {
        ...
        await expect(dispatch('extract', { schema: { type: 'object' } })).rejects.toThrow(/aborted/i);
    });
    ...
    it('override path always calls dispose() even when terminateMode is non-GOAL', async () => {
        ...
        const dispatch = createProductionDispatch(helper.config);
        await expect(dispatch('hi', { model: 'qwen3-max' })).rejects.toThrow(/terminate mode: ERROR/);
        expect(helper.disposed).toBeGreaterThanOrEqual(1);
    });
    ...
    it('override path: dispose() still runs in finally when execute throws', async () => {
        ...
        const dispatch = createProductionDispatch(helper.config);
        await expect(dispatch('extract', { schema: { type: 'object' } })).rejects.toThrow(/simulated subagent failure/);
        expect(helper.disposed).toBeGreaterThanOrEqual(1);
    });
    ...
    it("isolation:'worktree' refuses when parent cwd is already inside a worktree", async () => {
        ...
        config.getTargetDir = () => '/some/repo/.hopcode/worktrees/agent-existing/inner';
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'worktree' })).rejects.toThrow(/already inside a worktree/);
    });
    it("isolation:'worktree' refuses when git is not available", async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'worktree' })).rejects.toThrow(/git binary missing/);
    });
    it("isolation:'worktree' refuses when cwd is not a git repository", async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'worktree' })).rejects.toThrow(/not a git repository/);
    });
    it("isolation:'worktree' refuses when parent working tree is dirty", async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'worktree' })).rejects.toThrow(/uncommitted changes/);
    });
    it("isolation:'worktree' surfaces createUserWorktree failure", async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', { isolation: 'worktree' })).rejects.toThrow(/simulated worktree create failure/);
    });
    ...
    it("isolation:'worktree' cleanup: removeUserWorktree failure preserves path+branch", async () => {
        ...
        expect(String(result)).toMatch(/\[worktree preserved:.*\(branch worktree-agent-deadbe1\)\]/);
    });
    ...
    it("isolation:'worktree' + schema setup throws → worktree is still cleaned up", async () => {
        ...
        vi.mocked(GitWorktreeService).mockImplementation(() => {
            ...
            stub.removeUserWorktree = vi.fn(async (slug) => {
        ...
        config.createToolRegistry = async () => {
            throw new Error('simulated registry rebuild failure');
        };
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('hi', {
            isolation: 'worktree',
            schema: { type: 'object' },
    ...
    it('schema-mode + agentType restricted tools: structured_output appended to allowlist', async () => {
        ...
        const tools = calls[0].config.tools ?? [];
        expect(tools).toContain('structured_output');
        // The original agentType tools survive — not replaced.
        expect(tools).toEqual(expect.arrayContaining(['Read', 'Grep', 'Glob', 'structured_output']));
    });
    ...
    it('schema-mode + agentType: systemPrompt appends schema instructions (persona preserved)', async () => {
        ...
        const sp = calls[0].config.systemPrompt ?? '';
    ...
    it('schema-mode: parent-abort listener is removed after each call (no accumulation)', async () => {
        ...
        sharedSignal.addEventListener = ((type, ...rest) => {
            if (type === 'abort')
                liveListeners += 1;
            return origAdd(type, ...rest);
        });
        sharedSignal.removeEventListener = ((type, ...rest) => {
            if (type === 'abort')
                liveListeners -= 1;
    ...
    it.each(['TIMEOUT', 'MAX_TURNS', 'ERROR'])('schema-mode + terminateMode=%s → "did not complete" terminal, not "after 2 nudges"', async (mode) => {
        const { config } = fakeConfigWithMgr({
            onCreate: async () => ({
                finalText: '',
                terminateMode: mode,
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('extract', { schema: { type: 'object' } })).rejects.toThrow(new RegExp(`workflow-agent-[0-9a-f]{16} did not complete \\(terminate mode: ${mode}\\)\\.`));
    });
    ...
    it('schema-mode: 3 failed structured_output calls → upstream-verbatim "after 2 nudges"', async () => {
        ...
        const dispatch = createProductionDispatch(config);
        await expect(dispatch('extract', { schema: { type: 'object' } })).rejects.toThrow(/subagent completed without calling StructuredOutput \(after 2 in-conversation nudges\)\./);
    });
});
