...
import { canonicalizeAgentOpts, deriveAgentKey, deriveArgsSeed, buildReplay, WorkflowJournal, JOURNAL_KEY_VERSION, } from './workflow-journal.js';
...
describe('deriveAgentKey', () => {
    ...
    it('changes when an opt changes', () => {
        expect(deriveAgentKey('', 'x', { model: 'm1' })).not.toBe(deriveAgentKey('', 'x', { model: 'm2' }));
    });
    it('does NOT change when only a cosmetic opt (label) changes', () => {
        expect(deriveAgentKey('', 'x', { label: 'a' })).toBe(deriveAgentKey('', 'x', { label: 'b' }));
    });
    it('changes when the prefix hash changes (chaining)', () => {
        expect(deriveAgentKey('prefA', 'x', {})).not.toBe(deriveAgentKey('prefB', 'x', {}));
    });
});
...
//# sourceMappingURL=workflow-journal.test.js.map