 ...
 */
export type ModelGenerationConfig = Pick<ContentGeneratorConfig, 'samplingParams' | 'timeout' | 'maxRetries' | 'retryErrorCodes' | 'enableCacheControl' | 'forceGlobalCacheScope' | 'schemaCompliance' | 'reasoning' | 'customHeaders' | 'extra_body' | 'contextWindowSize' | 'modalities' | 'splitToolMedia' | 'toolResultContentFormat'>;
/**
 * Model configuration for a single model within an authType
 */
export interface ModelConfig {
    /** Unique model ID within authType (e.g., "hopcoder", "gpt-4-turbo") */
    id: string;
