...
import { ModelRegistry, HOPCODE_OAUTH_MODELS, modelRegistryKey, resolveProviderProtocol, } from './modelRegistry.js';
...
describe('ModelRegistry', () => {
    describe('initialization', () => {
        ...
        it('should initialize with empty config', () => {
            const registry = new ModelRegistry();
            expect(registry.getModelsForAuthType(AuthType.HOPCODE_OAUTH).length).toBe(HOPCODE_OAUTH_MODELS.length);
    ...
    describe('getModelsForAuthType', () => {
        ...
        it('should return AvailableModel format with correct fields', () => {
            ...
            expect(models.find((m) => m.id === 'gpt-3.5-turbo')?.registryBaseUrl).toBeUndefined();
        });
    });
    describe('getModel', () => {
        let registry;
        ...
        it('matches a plain registry key by its resolved default baseUrl', () => {
            ...
            const unkeyed = registry.getModel(AuthType.USE_OPENAI, 'default-endpoint-model');
            expect(unkeyed?.baseUrl).toBeTruthy();
            expect(registry.getModel(AuthType.USE_OPENAI, 'default-endpoint-model', unkeyed?.baseUrl)).toBe(unkeyed);
            expect(registry.getModel(AuthType.USE_OPENAI, 'default-endpoint-model', 'https://wrong.example.com')).toBeUndefined();
        });
    });
    ...
    describe('getDefaultModelForAuthType', () => {
        it('should return coder-model for hopcode-oauth', () => {
            const registry = new ModelRegistry();
            const defaultModel = registry.getDefaultModelForAuthType(AuthType.HOPCODE_OAUTH);
    ...
    describe('reloadModels', () => {
        ...
        it('should preserve hard-coded hopcode-oauth models after reload', () => {
            ...
            expect(registry.getModelsForAuthType(AuthType.HOPCODE_OAUTH).length).toBe(HOPCODE_OAUTH_MODELS.length);
            expect(registry.getModel(AuthType.HOPCODE_OAUTH, 'coder-model')).toBeDefined();
        });
...
describe('resolveProviderProtocol', () => {
    ...
    it('returns undefined for an unknown id with no mapping (typo guard)', () => {
        ...
        expect(resolveProviderProtocol('typo-key', { other: 'openai' })).toBe(undefined);
    });
    it('ignores an explicit mapping to an unknown protocol', () => {
        const map = { idealab: 'not-a-protocol' };
        expect(resolveProviderProtocol('idealab', map)).toBeUndefined();
    });
});
describe('providerProtocol mapping (custom provider ids)', () => {
    it('registers a custom provider under its mapped protocol', () => {
        ...
        expect(registry.getModel(AuthType.USE_OPENAI, 'qwen3.7-max', 'https://idealab.example/v1')).toBeDefined();
    });
    ...
    it('still skips a custom provider id with no mapping (backward compatible)', () => {
        ...
        expect(registry.getModelsForAuthType(AuthType.USE_OPENAI).map((m) => m.id)).toEqual(['gpt-4o']);
        // idealab had no providerProtocol entry, so its models are not registered.
        expect(registry.getModel(AuthType.USE_OPENAI, 'qwen3.7-max')).toBeUndefined();
    });
    it('warns clearly when providerProtocol maps to an unknown protocol', () => {
        ...
        expect(registry.getModelsForAuthType(AuthType.USE_OPENAI)).toEqual([]);
        expect(debugLoggerWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Provider "idealab" maps to "opneai" via providerProtocol'));
        expect(debugLoggerWarnSpy).toHaveBeenCalledWith(expect.not.stringContaining('has no providerProtocol mapping'));
    });
    it('routes a custom provider to a non-openai protocol when mapped', () => {
        const registry = new ModelRegistry({
            'my-vertex': [{ id: 'gemini-2.5-pro' }],
        }, { 'my-vertex': 'gemini' });
        expect(registry.getModelsForAuthType(AuthType.USE_GEMINI).map((m) => m.id)).toEqual(['gemini-2.5-pro']);
        expect(registry.getModelsForAuthType(AuthType.USE_OPENAI)).toEqual([]);
    });
    it('persists the mapping across reloadModels when none is supplied', () => {
        const registry = new ModelRegistry({ idealab: [{ id: 'qwen3.7-max' }] }, { idealab: 'openai' });
        ...
        expect(registry
            .getModelsForAuthType(AuthType.USE_OPENAI)
            .map((m) => m.id)
            .sort()).toEqual(['qwen3.7-coder', 'qwen3.7-max']);
    });
    it('updates the mapping when reloadModels supplies a new one', () => {
        const registry = new ModelRegistry({ idealab: [{ id: 'qwen3.7-max' }] }, { idealab: 'openai' });
        registry.reloadModels({ idealab: [{ id: 'qwen3.7-max' }] }, { idealab: 'gemini' });
        expect(registry.getModelsForAuthType(AuthType.USE_OPENAI)).toEqual([]);
        expect(registry.getModelsForAuthType(AuthType.USE_GEMINI).map((m) => m.id)).toEqual(['qwen3.7-max']);
    });
    ...
    it('passing an empty providerProtocol to reloadModels REPLACES (clears) the map', () => {
        ...
        registry.reloadModels({ idealab: [{ id: 'qwen3.7-max' }] }, {});
        expect(registry.getModelsForAuthType(AuthType.USE_OPENAI)).toEqual([]);
    });
    it('silently ignores a custom provider explicitly mapped to hopcode-oauth', () => {
        const registry = new ModelRegistry({
            'my-alias': [{ id: 'secret-model' }],
        }, { 'my-alias': 'hopcode-oauth' });
        // Hard-coded HOPCODE_OAUTH bucket untouched; the aliased model is not added.
        expect(registry.getModelsForAuthType(AuthType.HOPCODE_OAUTH)).toHaveLength(HOPCODE_OAUTH_MODELS.length);
        expect(registry.getModel(AuthType.HOPCODE_OAUTH, 'secret-model')).toBeUndefined();
    });
