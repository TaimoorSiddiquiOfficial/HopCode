...
import { AUTH_ENV_MAPPINGS, DEFAULT_MODELS, HOPCODE_OAUTH_ALLOWED_MODELS, MODEL_GENERATION_CONFIG_FIELDS, setGenerationConfigField, } from './constants.js';
 ...
 */
function resolveHopCodeOAuthConfig(input, warnings) {
    const { cli, settings, proxy, modelProvider } = input;
    const sources = {};
    // HopCode OAuth only allows specific models
    const allowedModels = new Set(HOPCODE_OAUTH_ALLOWED_MODELS);
    ...
    if (requestedModel && allowedModels.has(requestedModel)) {
        ...
        if (requestedModel) {
            ...
            const extraMessage = isVisionModel
                ...
                : '';
            warnings.push(`Warning: Unsupported HopCode OAuth model '${requestedModel}', falling back to '${DEFAULT_HOPCODE_MODEL}'.${extraMessage}`);
        }
        resolvedModel = DEFAULT_HOPCODE_MODEL;
        modelSource = defaultSource(`fallback to '${DEFAULT_HOPCODE_MODEL}'`);
    }
    ...
    const generationConfig = resolveGenerationConfig(settings?.generationConfig, modelProvider?.generationConfig, AuthType.HOPCODE_OAUTH, resolvedModel, sources);
    // ---- Env override: HOPCODE_API_TIMEOUT_MS ----
