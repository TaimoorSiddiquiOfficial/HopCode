...
describe('ModelsConfig', () => {
    ...
    it('should preserve an existing apiKey when switching between models with the same provider credentials', async () => {
        ...
        expect(modelsConfig.getGenerationConfigSources()['apiKey']?.kind).toBe('programmatic');
    });
    ...
    it.each([
        { kind: 'cli', detail: '--base-url' },
        { kind: 'env', envKey: 'OPENAI_BASE_URL' },
        { kind: 'settings', settingsPath: 'model.baseUrl' },
    ])('should preserve $kind baseUrl during same-model auth refresh', (baseUrlSource) => {
        const modelProvidersConfig = {
            openai: [
                {
        ...
        modelsConfig.syncAfterAuthRefresh(AuthType.USE_OPENAI, 'shared-base-model');
        const gc = currentGenerationConfig(modelsConfig);
    ...
    it('should always force HopCode OAuth apiKey placeholder when applying model defaults', async () => {
    ...
    it('should apply HopCode OAuth apiKey placeholder during syncAfterAuthRefresh for fresh users', () => {
        ...
        modelsConfig.syncAfterAuthRefresh(AuthType.HOPCODE_OAUTH, modelsConfig.getModel());
        const gc = currentGenerationConfig(modelsConfig);
    ...
    it('should use explicit provider baseUrl when syncing after provider install', () => {
        ...
        modelsConfig.syncAfterAuthRefresh(AuthType.USE_OPENAI, 'shared-model', 'https://new.example.com/v1');
    ...
    it('recomputes raw model modalities instead of carrying provider multimodal defaults', async () => {
        ...
        expect(modelsConfig.getGenerationConfig().contextWindowSize).not.toBe(12345);
        expect(modelsConfig.getGenerationConfigSources()['contextWindowSize']).toEqual({
            kind: 'computed',
            detail: 'auto-detected from model',
        });
    });
    ...
    it('refreshes model-derived modalities when hot-switching to the default hopcode-oauth model', async () => {
        // Start on hopcode-oauth with a text-only model so modalities are empty.
        const modelsConfig = new ModelsConfig({
            initialAuthType: AuthType.HOPCODE_OAUTH,
            generationConfig: {
        ...
        // Hot-update to coder-model (DEFAULT_HOPCODE_MODEL), which accepts images.
    ...
    it('rolls back raw model state when owner refresh fails', async () => {
        ...
        await expect(modelsConfig.setModel('qwen3.7-max')).rejects.toThrow('refresh failed');
    ...
    describe('getAllConfiguredModels', () => {
        it('should return all models across all authTypes and put hopcode-oauth first', () => {
            ...
            const firstNonHopCodeIndex = allModels.findIndex((m) => m.authType !== AuthType.HOPCODE_OAUTH);
            expect(firstNonHopCodeIndex).toBeGreaterThan(0);
            expect(allModels
                .slice(0, firstNonHopCodeIndex)
                .every((m) => m.authType === AuthType.HOPCODE_OAUTH)).toBe(true);
            expect(allModels
                .slice(firstNonHopCodeIndex)
                .every((m) => m.authType !== AuthType.HOPCODE_OAUTH)).toBe(true);
            // Should include hopcode-oauth models (hard-coded)
            const hopcodeModels = allModels.filter((m) => m.authType === AuthType.HOPCODE_OAUTH);
        ...
        it('should support filtering by authTypes and still put hopcode-oauth first when included', () => {
            ...
            const firstNonHopCodeIndex = withHopCode.findIndex((m) => m.authType !== AuthType.HOPCODE_OAUTH);
            expect(firstNonHopCodeIndex).toBeGreaterThan(0);
            expect(withHopCode
                .slice(0, firstNonHopCodeIndex)
                .every((m) => m.authType === AuthType.HOPCODE_OAUTH)).toBe(true);
        ...
        it('should include an active runtime model whose authType has no registry models', () => {
            ...
            const allModels = modelsConfig.getAllConfiguredModels();
            const openaiModel = allModels.find((m) => m.authType === AuthType.USE_OPENAI);
            expect(openaiModel).toBeDefined();
            expect(openaiModel?.id).toBe('my-openai-model');
            expect(openaiModel?.isRuntimeModel).toBe(true);
            // hopcode-oauth registry models still come first and are still listed.
            expect(allModels.some((m) => m.authType === AuthType.HOPCODE_OAUTH)).toBe(true);
        });
        it('should not inject the runtime model when an explicit authType filter excludes it', () => {
            ...
            expect(qwenOnly.every((m) => m.authType === AuthType.HOPCODE_OAUTH)).toBe(true);
        });
    });
    ...
    describe('reloadModelProvidersConfig', () => {
        ...
        it('should handle reload with empty config', async () => {
            ...
            expect(modelsConfig
                ...
                .filter((m) => m.authType !== 'hopcode-oauth').length).toBeGreaterThan(0);
    ...
    describe('getModelDisplayName', () => {
        ...
        it('should disambiguate duplicate model ids by current baseUrl', async () => {
            ...
            expect(modelsConfig.getModelDisplayName('qwen3.7-max')).toBe('[Idealab] qwen3.7-max');
        ...
        it('tracks implicit and explicit registry routes with the same effective URL', async () => {
            ...
            expect(modelsConfig.getGenerationConfig().apiKeyEnvKey).toBe('IMPLICIT_KEY');
            await modelsConfig.switchModel(AuthType.USE_OPENAI, 'shared', {
                baseUrl: defaultBaseUrl,
            });
        ...
        it('should return raw modelId when model is not found in registry', () => {
            ...
            expect(modelsConfig.getModelDisplayName('unknown-model')).toBe('unknown-model');
        });
        it('should return raw modelId when model.name equals model.id', () => {
            ...
            expect(modelsConfig.getModelDisplayName('coder-model')).toBe('coder-model');
        });
    });
    describe('providerProtocolConfig wiring', () => {
        it('threads providerProtocolConfig into the registry so custom ids resolve', () => {
            ...
            expect(modelsConfig
                ...
                .map((m) => m.id)).toContain('qwen3.7-max');
        });
        it('skips a custom id when no providerProtocolConfig is supplied', () => {
            ...
            expect(modelsConfig.getAvailableModelsForAuthType(AuthType.USE_OPENAI)).toEqual([]);
        });
