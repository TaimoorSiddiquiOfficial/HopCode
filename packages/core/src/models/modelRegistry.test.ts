...
describe('ModelRegistry', () => {
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded hopcode-oauth models after reload', () => {
      ...
      expect(
        registry.getModel(AuthType.HOPCODE_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
...
describe('providerProtocol mapping (custom provider ids)', () => {
  ...
  it('silently ignores a custom provider explicitly mapped to hopcode-oauth', () => {
    ...
    expect(registry.getModelsForAuthType(AuthType.HOPCODE_OAUTH)).toHaveLength(
      HOPCODE_OAUTH_MODELS.length,
    );
    expect(
      registry.getModel(AuthType.HOPCODE_OAUTH, 'secret-model'),
    ).toBeUndefined();
  });
