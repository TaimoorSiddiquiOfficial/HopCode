...
import { HOPCODE_OAUTH_MODELS } from './constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';
const debugLogger = createDebugLogger('MODEL_REGISTRY');
export { HOPCODE_OAUTH_MODELS } from './constants.js';
/**
...
export class ModelRegistry {
    ...
    getDefaultBaseUrl(authType) {
        switch (authType) {
            case AuthType.HOPCODE_OAUTH:
                return 'DYNAMIC_HOPCODE_OAUTH_BASE_URL';
    ...
    registerProvidersConfig(modelProvidersConfig) {
        ...
        for (const [providerId, models] of Object.entries(modelProvidersConfig)) {
            ...
            // hopcode-oauth uses hard-coded models and cannot be overridden
    ...
    getDefaultModelForAuthType(authType) {
        if (authType === AuthType.HOPCODE_OAUTH) {
            return this.getModel(authType, DEFAULT_HOPCODE_MODEL);
        }
        const models = this.modelsByAuthType.get(authType);
    ...
    validateModelConfig(config, authType) {
        ...
        if (config.fastOnly && config.voiceOnly) {
            debugLogger.warn(`Model "${config.id}" in authType "${authType}" has both fastOnly and voiceOnly set. It will be unreachable in all model selectors.`);
        }
    }
    /**
