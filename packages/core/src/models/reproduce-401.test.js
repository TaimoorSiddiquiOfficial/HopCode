...
describe('Persistent 401 reproduction — issue #5979 / #6129 / #6283', () => {
    ...
    it('loses apiKey when switching to model with different envKey', async () => {
        ...
        await modelsConfig.switchModel(AuthType.USE_OPENAI, 'qwen3-coder');
        expect(modelsConfig.getGenerationConfig().apiKey).toBe('sk-dashscope-valid');
        ...
        expect(debugLoggerDebug).toHaveBeenCalledWith(expect.stringContaining(`process.env["${ENV_KEY_TOKEN_PLAN}"] is not set`));
    });
    /**
    ...
    it('treats empty-string env var as unset (apiKey stays undefined at model layer)', async () => {
        ...
        expect(gc.apiKey).toBeUndefined();
        expect(debugLoggerDebug).toHaveBeenCalledWith(expect.stringContaining(`process.env["${ENV_KEY_DASHSCOPE}"] is empty string`));
    });
});
//# sourceMappingURL=reproduce-401.test.js.map