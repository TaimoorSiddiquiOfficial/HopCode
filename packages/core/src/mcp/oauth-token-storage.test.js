...
import { HOPCODE_DIR } from '../utils/paths.js';
...
describe('MCPOAuthTokenStorage', () => {
    let MCPOAuthTokenStorage;
    let FORCE_ENCRYPTED_FILE_ENV_VAR;
    let stderrWriteSpy;
    let tokenStorage;
    async function loadStorageModule() {
        vi.resetModules();
        ({ FORCE_ENCRYPTED_FILE_ENV_VAR } = await import('./token-storage/index.js'));
    ...
    describe('with encrypted flag false', () => {
        ...
        describe('getAllCredentials', () => {
            ...
            it('should load tokens from file successfully', async () => {
                ...
                expect(fs.readFile).toHaveBeenCalledWith(path.join('/mock/home', HOPCODE_DIR, 'mcp-oauth-tokens.json'), 'utf-8');
            });
        ...
        describe('saveToken', () => {
            it('should warn once when writing plaintext tokens', async () => {
                ...
                expect(mockDebugLogger.warn).toHaveBeenCalledWith(expect.stringContaining(FORCE_ENCRYPTED_FILE_ENV_VAR));
                expect(stderrWriteSpy).toHaveBeenCalledTimes(1);
                expect(stderrWriteSpy).toHaveBeenCalledWith(expect.stringContaining(FORCE_ENCRYPTED_FILE_ENV_VAR));
            });
            it('should save token with restricted permissions', async () => {
                ...
                expect(fs.mkdir).toHaveBeenCalledWith(path.join('/mock/home', HOPCODE_DIR), { recursive: true });
                expect(atomicWriteFile).toHaveBeenCalledWith(path.join('/mock/home', HOPCODE_DIR, 'mcp-oauth-tokens.json'), expect.stringContaining('test-server'), { mode: 0o600, forceMode: true, noFollow: true });
            });
            ...
            it('should not fail token writes when stderr warning output fails', async () => {
                ...
                await expect(tokenStorage.saveToken('test-server', mockToken)).resolves.toBeUndefined();
                expect(stderrWriteSpy).toHaveBeenCalledTimes(1);
                expect(mockDebugLogger.error).not.toHaveBeenCalled();
            });
        });
        ...
        describe('deleteCredentials', () => {
            ...
            it('should remove token file when no tokens remain', async () => {
                ...
                expect(fs.unlink).toHaveBeenCalledWith(path.join('/mock/home', HOPCODE_DIR, 'mcp-oauth-tokens.json'));
