...
describe('MCPOAuthProvider', () => {
    ...
    describe('authenticate', () => {
        ...
        it('should preserve expires_in=0 as an immediate expiry', async () => {
            ...
            const tokenStorage = new MCPOAuthTokenStorage();
            expect(tokenStorage.saveToken).toHaveBeenCalledWith('test-server', expect.objectContaining({ expiresAt: 1_700_000_000_000 }), 'test-client-id', 'https://auth.example.com/token', undefined);
        });
    ...
    describe('refreshAccessToken', () => {
        ...
        it('should reject malformed JSON expires_in values', async () => {
            ...
            const authProvider = new MCPOAuthProvider();
            await expect(authProvider.refreshAccessToken(mockConfig, 'refresh_token', 'https://auth.example.com/token')).rejects.toThrow('Invalid expires_in value');
        });
    ...
    describe('Authorization URL building', () => {
        ...
        // Scenario: user runs `hopcode mcp add --transport http yuque https://mcp.alibaba-inc.com/yuque/mcp`
