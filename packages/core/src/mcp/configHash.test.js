...
describe('hashMcpServerConfig', () => {
    ...
    it('is deterministic for identical configs', () => {
        ...
        expect(hashMcpServerConfig(config)).toBe(hashMcpServerConfig({ ...config }));
    });
    ...
    describe('provenance / cosmetic fields are excluded', () => {
        ...
        it.each(['scope', 'extensionName', 'description'])('ignores changes to %s', (field) => {
    ...
    describe('behavioral changes change the hash', () => {
        ...
        it('command', () => {
            expect(hashMcpServerConfig({ ...base, command: 'python' })).not.toBe(baseHash);
        });
        it('args value', () => {
            expect(hashMcpServerConfig({ ...base, args: ['other.js'] })).not.toBe(baseHash);
        });
        ...
        it('env value', () => {
            expect(hashMcpServerConfig({ ...base, env: { TOKEN: 'xyz' } })).not.toBe(baseHash);
        });
        it('trust flag', () => {
            expect(hashMcpServerConfig({ ...base, trust: true })).not.toBe(baseHash);
        });
        it('remote url', () => {
            expect(hashMcpServerConfig({ url: 'https://a.example' })).not.toBe(hashMcpServerConfig({ url: 'https://b.example' }));
        });
        it('headers', () => {
            expect(hashMcpServerConfig({
                httpUrl: 'https://a.example',
                headers: { Authorization: 'one' },
            })).not.toBe(hashMcpServerConfig({
                httpUrl: 'https://a.example',
