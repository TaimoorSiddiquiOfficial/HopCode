...
describe('FileTokenStorage', () => {
    ...
    describe('setCredentials', () => {
        it('should create token file when saving credentials for the first time', async () => {
            ...
            expect(mockFs.mkdir).toHaveBeenCalledWith(path.join('/home/test', '.hopcode'), { recursive: true, mode: 0o700 });
        ...
        it('should propagate atomicWriteFile failures', async () => {
            ...
            mockFs.readFile.mockResolvedValue(encryptedData);
            mockFs.mkdir.mockResolvedValue(undefined);
            vi.mocked(atomicWriteFile).mockRejectedValueOnce(Object.assign(new Error('ENOSPC: no space left on device'), {
                code: 'ENOSPC',
            }));
            await expect(storage.setCredentials({
                serverName: 'test-server',
    ...
    describe('deleteCredentials', () => {
        ...
        it('should delete file when last credential is removed', async () => {
            ...
            expect(mockFs.unlink).toHaveBeenCalledWith(path.join('/home/test', '.hopcode', 'mcp-oauth-tokens-v2.json'));
        });
        ...
        it('should propagate atomicWriteFile failures', async () => {
            ...
            const encryptedData = storage['encrypt'](JSON.stringify({ server1: credentials1, server2: credentials2 }));
            mockFs.readFile.mockResolvedValue(encryptedData);
            vi.mocked(atomicWriteFile).mockRejectedValueOnce(Object.assign(new Error('EROFS: read-only file system'), {
                code: 'EROFS',
            }));
            await expect(storage.deleteCredentials('server1')).rejects.toThrow(/EROFS/);
        });
    });
    ...
    describe('secret storage', () => {
        const secretFilePath = path.join('/home/test', '.hopcode', 'extension-secrets-v1.json');
        beforeEach(() => {
            mockFs.mkdir.mockResolvedValue(undefined);
            vi.mocked(atomicWriteFile).mockResolvedValue(undefined);
        });
        ...
        it('persists a secret encrypted under the service name', async () => {
            ...
            const [writtenPath, writtenData, writeOptions] = vi.mocked(atomicWriteFile).mock.calls[0];
            ...
            expect(JSON.parse(storage['decrypt'](writtenData))).toEqual({
                'test-storage': { API_KEY: 'sk-123' },
            });
        });
        it('reads back a stored secret and ignores other keys and services', async () => {
            const encrypted = storage['encrypt'](JSON.stringify({
                'test-storage': { API_KEY: 'sk-123' },
        ...
        it('deletes a secret and drops the now-empty service bucket', async () => {
            const encrypted = storage['encrypt'](JSON.stringify({ 'test-storage': { API_KEY: 'sk-123' } }));
            mockFs.readFile.mockResolvedValue(encrypted);
