...
import { CHANNEL_MEMORY_ID_RE, MAX_CHANNEL_MEMORY_ENTRIES, MAX_CHANNEL_MEMORY_ENTRY_CODE_POINTS, createChannelMemoryEntry, normalizeChannelMemoryText, parseChannelMemoryDocument, parseLegacyChannelMemory, renderChannelMemoryRecall, serializeChannelMemoryDocument, } from './channel-memory-document.js';
describe('channel memory document', () => {
    it('normalizes channel memory text', () => {
        expect(normalizeChannelMemoryText('  USE\u00a0staging  ')).toBe('use staging');
    });
    it('rejects unsupported document versions', () => {
        expect(() => parseChannelMemoryDocument('{"version":2,"entries":[]}')).toThrow('Unsupported channel memory version');
    });
    it('rejects entries with invalid ids', () => {
        expect(() => parseChannelMemoryDocument(JSON.stringify({
            version: 1,
            entries: [
                { id: 'bad', text: 'x' },
    ...
    it('validates the complete version-1 document shape', () => {
        const document = parseChannelMemoryDocument(JSON.stringify({
            version: 1,
    ...
    it.each([
        ...
        expect(() => parseChannelMemoryDocument(JSON.stringify(value))).toThrow('Invalid channel memory');
    });
    it('rejects duplicate JSON object keys', () => {
        expect(() => parseChannelMemoryDocument('{"version":1,"entries":[],"entries":[]}')).toThrow('Invalid channel memory document');
    });
    it.each([
    ...
    it('rejects documents exceeding the entry limit', () => {
        const entries = Array.from({ length: MAX_CHANNEL_MEMORY_ENTRIES + 1 }, (_, index) => ({
            id: `m-${index.toString(16).padStart(12, '0')}`,
            text: 'x',
        }));
        expect(() => parseChannelMemoryDocument(JSON.stringify({ version: 1, entries }))).toThrow('maximum number of entries');
    });
    it('counts astral Unicode text by code point', () => {
        const astralCharacter = '\u{1f600}';
        const acceptedText = astralCharacter.repeat(MAX_CHANNEL_MEMORY_ENTRY_CODE_POINTS);
        expect(parseChannelMemoryDocument(JSON.stringify({
            version: 1,
            entries: [{ id: 'm-123456789abc', text: acceptedText }],
        })).entries[0].text).toBe(acceptedText);
        expect(() => parseChannelMemoryDocument(JSON.stringify({
            version: 1,
            entries: [
                {
                    id: 'm-123456789abc',
                    text: astralCharacter.repeat(MAX_CHANNEL_MEMORY_ENTRY_CODE_POINTS + 1),
                },
            ],
        }))).toThrow('Invalid channel memory entry');
    });
    it('converts legacy lines with stable ids and a migration hash', () => {
        ...
        expect(first.entries.every((entry) => CHANNEL_MEMORY_ID_RE.test(entry.id))).toBe(true);
    ...
    it('preserves surrounding whitespace in legacy entry text', () => {
        const document = parseLegacyChannelMemory(Buffer.from('  Keep surrounding whitespace  \n'));
    ...
    it('creates a timestamped channel memory entry', () => {
        expect(createChannelMemoryEntry({
            text: ' Use staging ',
            createdBy: 'alice',
    ...
    it('renders recall text without entry metadata', () => {
        expect(renderChannelMemoryRecall([
            {
                id: 'm-abcdef012345',
                text: 'Use staging',
                createdBy: 'alice',
    ...
    it('does not silently discard unknown keys during serialization', () => {
        ...
        expect(() => serializeChannelMemoryDocument(document)).toThrow('Invalid channel memory document');
    });
});
//# sourceMappingURL=channel-memory-document.test.js.map