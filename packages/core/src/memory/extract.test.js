...
describe('auto-memory extraction', () => {
    ...
    describe('rebuild failure isolation (asymmetric)', () => {
        ...
        async function readCursor() {
            return JSON.parse(await fs.readFile(getAutoMemoryExtractCursorPath(projectRoot), 'utf-8'));
        }
        it('project-scope rebuild failure bubbles up so the cursor is NOT advanced (retry on next session)', async () => {
            ...
            vi.mocked(rebuildManagedAutoMemoryIndex).mockRejectedValueOnce(new Error('EACCES: project memory index write failed'));
            await expect(runAutoMemoryExtract({
                projectRoot,
                sessionId: 'session-1',
                config: mockConfig,
        ...
        it('user-scope rebuild failure is logged and swallowed; project rebuild + cursor advance still happen', async () => {
            // User-level memory is best-effort: a read-only `~/.hopcode/memories/`
            ...
            vi.mocked(rebuildUserAutoMemoryIndex).mockRejectedValueOnce(new Error('EACCES: user memory index write failed'));
            await expect(runAutoMemoryExtract({
                projectRoot,
                sessionId: 'session-1',
                config: mockConfig,
    ...
    describe('#5147 OOM regression', () => {
        ...
        it('only processes unread messages via cursor-first ordering', async () => {
            ...
            history.push({ role: 'user', parts: [{ text: 'new user question?' }] }, { role: 'model', parts: [{ text: 'new assistant answer.' }] });
            ...
            expect(vi.mocked(runAutoMemoryExtractionByAgent).mock.calls.length).toBe(agentCallsBefore + 1);
        ...
        it('skips extract when cursor is already up to date', async () => {
            ...
            expect(vi.mocked(runAutoMemoryExtractionByAgent).mock.calls.length).toBe(agentCallsBefore);
        ...
        it('avoids full-history regex replace when most messages are already processed', async () => {
            ...
            history.push({ role: 'user', parts: [{ text: 'final question?'.repeat(50) }] }, { role: 'model', parts: [{ text: 'final answer.'.repeat(50) }] });
            const agentCallsBefore = vi.mocked(runAutoMemoryExtractionByAgent).mock
                .calls.length;
