...
import { getAutoMemoryRoot, getUserAutoMemoryRoot, clearAutoMemoryRootCache, } from './paths.js';
...
describe('dreamAgentPlanner', () => {
    ...
    it('returns project-scoped session transcript directory', () => {
        ...
        expect(getTranscriptDir(projectRoot)).not.toContain(`${path.sep}.hopcode${path.sep}tmp${path.sep}`);
    });
    ...
    it('returns the forked agent result', async () => {
        const mockResult = {
            ...
            filesTouched: [
                path.join(projectRoot, '.hopcode', 'memory', 'user', 'prefers-vim.md'),
            ],
        };
