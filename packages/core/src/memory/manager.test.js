...
describe('MemoryManager', () => {
    ...
    describe('scheduleSkillReview() confirmBeforePersist', () => {
        ...
        beforeEach(async () => {
            ...
            projectRoot = path.join(tempDir, 'project');
            await fs.mkdir(projectRoot, { recursive: true });
            skillFilePath = path.join(projectRoot, '.hopcode', 'skills', 'auto-skill-foo', 'SKILL.md');
        ...
        it('stages the skill and records pendingSkills when confirmBeforePersist is true', async () => {
            ...
            const pendingSkills = record.metadata?.['pendingSkills'];
    ...
    describe('scheduleExtract #5147', () => {
        ...
        it('supersedes trailing queue without leaking old history refs', async () => {
            ...
            const trailingPromise = new Promise((r) => {
                resolveTrailing = r;
            });
            ...
            expect(runAutoMemoryExtract).toHaveBeenLastCalledWith(expect.objectContaining({
                history: [
                    { role: 'user', parts: [{ text: 'third history' }] },
                    { role: 'model', parts: [{ text: 'third response' }] },
                ],
    ...
    describe('appendToUserMemory', () => {
        it('forwards options to buildManagedAutoMemoryPrompt', () => {
            ...
            const condensed = mgr.appendToUserMemory('', '/project/.qwen/memory', null);
            // With forceFullProtocol → full verbose path
            const full = mgr.appendToUserMemory('', '/project/.qwen/memory', null, undefined, undefined, { forceFullProtocol: true });
