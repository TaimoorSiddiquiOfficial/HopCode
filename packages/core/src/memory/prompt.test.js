...
import { appendManagedAutoMemoryToUserMemory, buildManagedAutoMemoryPrompt, CONDENSED_DO_NOT_SAVE_SECTION, CONDENSED_TEAM_GUIDANCE, CONDENSED_TYPES_SECTION, CONDENSED_WHEN_TO_ACCESS_SECTION, MAX_MANAGED_AUTO_MEMORY_INDEX_LINES, } from './prompt.js';
describe('managed auto-memory prompt helpers', () => {
    ...
    it('embeds the current MEMORY.md index content', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', '- [User Memory](user/terse.md) � User prefers terse responses.');
    ...
    it('warns extraction not to save MCP tool schemas or failed calls', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory');
        expect(prompt).toContain('MCP tool names, parameter schemas, field mappings, guessed tool-call formats, or raw failed tool-call transcripts');
    ...
    it('appends managed auto-memory after existing hierarchical memory', () => {
        const result = appendManagedAutoMemoryToUserMemory('--- Context from: HOPCODE.md ---\nProject rules', '/tmp/project/.hopcode/memory', '- [Project Memory](project/release-freeze.md) � Release freeze starts Friday.');
        expect(result).toContain('Project rules');
        expect(result).toContain('\n\n---\n\n');
        expect(result).toContain('# auto memory');
    });
    it('returns only managed auto-memory when hierarchical memory is empty', () => {
        const result = appendManagedAutoMemoryToUserMemory('   ', '/tmp/project/.hopcode/memory', '- [Reference](reference/grafana.md) � Grafana dashboard link.');
        expect(result).toContain('# auto memory');
        expect(result.startsWith('# auto memory')).toBe(true);
    });
    it('adds a shared team tier when a team section is provided', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', '- [Project](project/x.md) — note.', { memoryDir: '/home/u/.hopcode/memories', indexContent: null }, {
            memoryDir: '/tmp/project/.hopcode/team-memory',
        ...
        expect(prompt).toContain('/tmp/project/.hopcode/team-memory');
        ...
        expect(prompt).toContain('## /tmp/project/.hopcode/team-memory/MEMORY.md');
        ...
        expect(prompt).toContain('PROJECT memory (this project only, private to you)');
    ...
    it('renders a two-tier project+team prompt when no user section is given', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', '- [Project](project/x.md) — note.', undefined, {
            memoryDir: '/tmp/project/.hopcode/team-memory',
            indexContent: '- [Convention](feedback/tests.md) — use real DBs.',
        });
        ...
        expect(prompt.indexOf('## /tmp/project/.hopcode/memory/MEMORY.md')).toBeLessThan(prompt.indexOf('## /tmp/project/.hopcode/team-memory/MEMORY.md'));
    });
    it('omits the team tier when no team section is provided', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', null, {
    ...
    it('truncates oversized managed auto-memory index content', () => {
        ...
        const result = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', oversizedIndex);
    ...
    it('condensed prompt with empty indexes is significantly shorter than full', () => {
        ...
        const full = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', undefined, undefined, undefined, { forceFullProtocol: true });
    ...
    it('emits full prompt with forceFullProtocol even when all indexes are empty', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, undefined, undefined, { forceFullProtocol: true });
        expect(prompt).toContain('## Types of memory');
        expect(prompt).toContain('## What NOT to save in memory');
        expect(prompt).toContain('## When to access memories');
        expect(prompt).toContain('## Before recommending from memory');
    ...
    it('emits condensed prompt for multi-tier setup when all indexes are empty', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, { memoryDir: '/home/u/.qwen/memories', indexContent: null });
    ...
    it('emits condensed prompt for three-tier setup with team section when all indexes are empty', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, { memoryDir: '/home/u/.qwen/memories', indexContent: null }, { memoryDir: '/tmp/project/.qwen/team-memory', indexContent: null });
        expect(prompt).toContain('three persistent, file-based memory directories');
        expect(prompt).toContain('TEAM memory');
    ...
    it('appendManagedAutoMemoryToUserMemory passes through options', () => {
        const withOptions = appendManagedAutoMemoryToUserMemory('', '/tmp/project/.qwen/memory', null, undefined, undefined, { forceFullProtocol: true });
        const without = appendManagedAutoMemoryToUserMemory('', '/tmp/project/.qwen/memory', null);
    ...
    it('emits full prompt when only userSection has content (project index empty)', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, {
    ...
    it('emits condensed prompt for project+team two-tier without userSection (all empty)', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, undefined, { memoryDir: '/tmp/project/.qwen/team-memory', indexContent: null });
    ...
    it('condensed prompt includes read-path behavioral guidance', () => {
        ...
        expect(prompt).toContain('MUST access memory when the user explicitly asks');
    ...
    it('condensed multi-tier prompt includes cross-directory duplicate check', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, { memoryDir: '/home/u/.qwen/memories', indexContent: null });
        expect(prompt).toContain('check if there is an existing memory in any of your memory directories');
    });
    ...
    it('emits condensed prompt when forceFullProtocol is explicitly false', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', null, undefined, undefined, { forceFullProtocol: false });
    ...
    it('condensed do-not-save splits git history and debugging solutions into separate bullets', () => {
        ...
        expect(joined).toContain('- Git history, recent changes, or who-changed-what');
    ...
    it('condensed prompt includes persistence guidance', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory');
        expect(prompt).toContain('Use plans and tasks for in-conversation work; reserve memory for durable cross-conversation knowledge');
    });
});
