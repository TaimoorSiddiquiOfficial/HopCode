...
import { rebuildTeamAutoMemoryIndex, TeamMemoryRootSecurityError, } from './indexer.js';
import { clearAutoMemoryRootCache, getTeamAutoMemoryIndexPath, getTeamAutoMemoryRoot, TEAM_AUTO_MEMORY_DIRNAME, } from './paths.js';
describe('rebuildTeamAutoMemoryIndex', () => {
    let projectRoot;
    const writeMemory = (rel, name, description) => {
    ...
    it('generates the index from saved files and writes MEMORY.md', async () => {
        ...
        expect(fs.readFileSync(getTeamAutoMemoryIndexPath(projectRoot), 'utf-8')).toBe(content);
    ...
    it('collapses entries with the same description into one line', async () => {
        ...
        writeMemory('alice/feedback/db.md', 'Use real DB', 'Integration tests hit a real database.');
        writeMemory('bob/feedback/db.md', 'Real database', 'integration tests hit a real database');
        const content = (await rebuildTeamAutoMemoryIndex(projectRoot)) ?? '';
    ...
    it('refuses to write through a symlinked team root (no write outside the repo)', async () => {
        // A committed `.hopcode/team-memory -> /elsewhere` symlink would otherwise
        ...
        try {
            ...
            await expect(rebuildTeamAutoMemoryIndex(projectRoot)).rejects.toBeInstanceOf(TeamMemoryRootSecurityError);
            await expect(rebuildTeamAutoMemoryIndex(projectRoot)).rejects.toThrow(/symlink/);
    ...
    it('rejects a PARENT-component symlink that escapes the repo (no write outside)', async () => {
        // `.hopcode` itself is a symlink to an outside dir, with a real `team-memory`
        ...
        try {
            ...
            const teamRoot = getTeamAutoMemoryRoot(projectRoot);
            const hopcodeDir = path.dirname(teamRoot); // <repo>/.hopcode
            fs.symlinkSync(outside, hopcodeDir, 'dir');
            ...
            await expect(rebuildTeamAutoMemoryIndex(projectRoot)).rejects.toBeInstanceOf(TeamMemoryRootSecurityError);
            await expect(rebuildTeamAutoMemoryIndex(projectRoot)).rejects.toThrow(/outside the repository/);
            // Nothing was scanned or written into the escaped target.
            expect(fs.existsSync(path.join(outside, TEAM_AUTO_MEMORY_DIRNAME, 'MEMORY.md'))).toBe(false);
        }
        finally {
            fs.rmSync(outside, { recursive: true, force: true });
        }
    ...
    it('replaces a symlinked MEMORY.md instead of writing through it', async () => {
        ...
        try {
            ...
            expect(fs.readFileSync(target, 'utf-8')).toBe('SENTINEL — must not be overwritten');
...
//# sourceMappingURL=team-index.test.js.map