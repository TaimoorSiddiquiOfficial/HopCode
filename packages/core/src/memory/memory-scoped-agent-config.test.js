...
import { clearAutoMemoryRootCache, getAutoMemoryRoot, getUserAutoMemoryRoot, } from './paths.js';
describe('createMemoryScopedAgentConfig', () => {
    ...
    function permissionManager(config) {
        ...
        if (!pm)
            throw new Error('missing permission manager');
        return pm;
    }
    it('restricts reads to memory paths only when requested', async () => {
        const unrestricted = permissionManager(createMemoryScopedAgentConfig({}, projectRoot));
        await expect(unrestricted.evaluate({
            toolName: ToolNames.READ_FILE,
            filePath: path.join(projectRoot, 'transcripts', 'latest.jsonl'),
        })).resolves.toBe('default');
        const restricted = permissionManager(createMemoryScopedAgentConfig({}, projectRoot, {
            restrictReadsToMemoryPaths: true,
        }));
        await expect(restricted.evaluate({
            toolName: ToolNames.READ_FILE,
            filePath: path.join(projectRoot, 'transcripts', 'latest.jsonl'),
        })).resolves.toBe('deny');
        await expect(restricted.evaluate({
    ...
    it('can keep writes project-memory-only for the dream agent', async () => {
        const pm = permissionManager(createMemoryScopedAgentConfig({}, projectRoot, {
            includeUserMemory: false,
        }));
        await expect(pm.evaluate({
    ...
    it('allows creating new nested topic files inside memory roots', async () => {
        ...
        await expect(pm.evaluate({
            toolName: ToolNames.WRITE_FILE,
            filePath: path.join(getAutoMemoryRoot(projectRoot), 'project', 'new-topic', 'fact.md'),
        })).resolves.toBe('allow');
        await expect(pm.evaluate({
            toolName: ToolNames.EDIT,
            filePath: path.join(getUserAutoMemoryRoot(), 'user', 'new-topic', 'fact.md'),
        })).resolves.toBe('allow');
    });
    it('allows memory paths with dot-prefixed names inside memory roots', async () => {
        ...
        await expect(pm.evaluate({
            toolName: ToolNames.WRITE_FILE,
            filePath: path.join(getAutoMemoryRoot(projectRoot), '..topic', 'fact.md'),
        })).resolves.toBe('allow');
    });
    ...
    it('denies dangling symlink leaves inside memory roots', async () => {
        const outsideDir = path.join(tempDir, 'outside');
        await fs.mkdir(outsideDir, { recursive: true });
        const link = path.join(getAutoMemoryRoot(projectRoot), 'project', 'link.md');
    ...
    it('allows only read-only shell commands when shell is enabled', async () => {
        const disabled = permissionManager(createMemoryScopedAgentConfig({}, projectRoot));
        await expect(disabled.isToolEnabled(ToolNames.SHELL)).resolves.toBe(false);
        await expect(disabled.evaluate({
            toolName: ToolNames.SHELL,
            command: 'ls',
        })).resolves.toBe('deny');
        const enabled = permissionManager(createMemoryScopedAgentConfig({}, projectRoot, {
            allowShell: true,
        }));
        await expect(enabled.isToolEnabled(ToolNames.SHELL)).resolves.toBe(true);
        await expect(enabled.evaluate({
            toolName: ToolNames.SHELL,
            command: 'ls -la',
        })).resolves.toBe('allow');
        await expect(enabled.evaluate({
    ...
    it('lets base deny rules override scoped allows', async () => {
        ...
        const pm = permissionManager(createMemoryScopedAgentConfig({
    ...
    it('can bypass base ask rules for scoped memory writes only', async () => {
        ...
        const pm = permissionManager(createMemoryScopedAgentConfig({
            getPermissionManager: () => basePm,
        }, projectRoot, { bypassBaseAskForScopedPaths: true }));
        await expect(pm.evaluate({
            toolName: ToolNames.WRITE_FILE,
            filePath: path.join(getUserAutoMemoryRoot(), 'user', 'a.md'),
        })).resolves.toBe('allow');
...
//# sourceMappingURL=memory-scoped-agent-config.test.js.map