...
function makeWorkingClone(bareRemote, label) {
...
function writeTeamMemory(repo, rel, body) {
...
describe('syncTeamMemory', () => {
    ...
    function freshRemoteAndClone(label) {
        const bareParent = fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-sync-bare-'));
    ...
    it('commits local team memory and pushes it to the remote', async () => {
        ...
        expect(fs.existsSync(path.join(getTeamAutoMemoryRoot(verify), 'feedback/use-real-db.md'))).toBe(true);
    }, 30_000);
    it('attributes the commit to opts.author when provided', async () => {
        ...
        expect(git(repo, 'log', '-1', '--format=%an <%ae>').trim()).toBe('bob <bob@team.dev>');
    }, 30_000);
    it('fast-forward-pulls team memory another collaborator pushed', async () => {
        ...
        git(bob, 'add', '--', '.hopcode/team-memory');
        ...
        expect(fs.existsSync(path.join(getTeamAutoMemoryRoot(repo), 'reference/grafana.md'))).toBe(true);
    }, 30_000);
    it('reconciles a second writer instead of diverging (commit lands on top)', async () => {
        ...
        expect(fs.existsSync(path.join(teamDir, 'reference/grafana.md'))).toBe(true);
        expect(fs.existsSync(path.join(teamDir, 'feedback/use-real-db.md'))).toBe(true);
    }, 30_000);
...
//# sourceMappingURL=team-memory-sync.test.js.map