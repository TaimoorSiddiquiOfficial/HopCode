...
describe('auto-memory topic scanning', () => {
    ...
    it('parses a CRLF (Windows checkout) topic document', () => {
        ...
        const parsed = parseAutoMemoryTopicDocument('/tmp/crlf.md', [
            '---',
            'type: project',
            'name: CRLF Memory',
    ...
    it('survives an unreadable file instead of dropping the whole index', async () => {
        const goodPath = getAutoMemoryFilePath(projectRoot, path.join('feedback', 'good.md'));
        ...
        await fs.mkdir(getAutoMemoryFilePath(projectRoot, path.join('feedback', 'broken.md')), { recursive: true });
        const docs = await scanAutoMemoryTopicDocuments(projectRoot);
        expect(docs.find((d) => d.relativePath === 'feedback/good.md')).toBeTruthy();
        expect(docs.some((d) => d.relativePath === 'feedback/broken.md')).toBe(false);
    });
});
