...
function isActiveToolUsageMemory(doc, recentTools) {
    ...
    const haystack = [doc.title, doc.description, normalizeBody(doc.body)]
        .join(' ')
        .toLowerCase();
    const namesActiveTool = recentTools.some((toolName) => toolAliases(toolName).some((alias) => haystack.includes(alias)));
    if (!namesActiveTool) {
        return false;
    }
...
export async function resolveRelevantAutoMemoryPromptForQuery(projectRoot, query, options = {}) {
    ...
    // `~/.hopcode/memories/` must not cancel the project-level scan, otherwise
    ...
    const heuristicDocs = docs.filter((doc) => !isActiveToolUsageMemory(doc, options.recentTools ?? []));
    const selectedDocs = selectRelevantAutoMemoryDocuments(query, heuristicDocs, limit);
