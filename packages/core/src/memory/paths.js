...
import { HOPCODE_DIR, resolvePath, sanitizeCwd } from '../utils/paths.js';
...
/**
 * Directory name (under the repo's `.hopcode/`) for the team auto-memory layer —
...
export function getMemoryBaseDir() {
    if (process.env['HOPCODE_CODE_MEMORY_BASE_DIR']) {
        return resolvePath(undefined, process.env['HOPCODE_CODE_MEMORY_BASE_DIR']);
    }
...
export function getAutoMemoryRoot(projectRoot) {
    ...
    if (useLocalMemory) {
        ...
        result = path.join(memoryBaseDir, 'projects', sanitizeCwd(gitRoot), AUTO_MEMORY_DIRNAME);
    }
 ...
 * Lives at `${getMemoryBaseDir()}/memories/` — typically `~/.hopcode/memories/`.
...
/**
 * Returns the team auto-memory root: `<gitRoot>/.hopcode/team-memory/`.
...
export function getTeamAutoMemoryRoot(projectRoot) {
    ...
    const result = path.join(root, HOPCODE_DIR, TEAM_AUTO_MEMORY_DIRNAME);
 ...
 * `decoy.md -> .hopcode/team-memory/leak.md` (target absent) so the path classifies
