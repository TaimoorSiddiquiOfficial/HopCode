...
const activeToolDocs = [
    {
        ...
        description: 'article-list-query parameter schema and failed tool-call attempts',
...
describe('auto-memory relevant recall', () => {
    ...
    it('keeps active tool schemas out of heuristic fallback', async () => {
        ...
        vi.mocked(selectRelevantAutoMemoryDocumentsByModel).mockRejectedValue(new Error('selector failed'));
        const result = await resolveRelevantAutoMemoryPromptForQuery('/tmp/project', 'read the ATA article with article-list-query', {
            config: {},
            recentTools: ['mcp__ata__article-list-query'],
        });
        expect(result.strategy).toBe('heuristic');
        expect(result.selectedDocs.map((doc) => doc.filePath)).not.toContain('/tmp/ata-tool.md');
        expect(result.selectedDocs.map((doc) => doc.filePath)).toContain('/tmp/ata-gotcha.md');
        expect(result.selectedDocs.map((doc) => doc.filePath)).toContain('/tmp/ata-owner.md');
    });
});
