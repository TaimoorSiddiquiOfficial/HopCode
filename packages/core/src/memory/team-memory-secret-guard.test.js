...
describe('checkTeamMemorySecrets', () => {
    ...
    it('blocks a secret written to a team path', () => {
        const msg = checkTeamMemorySecrets(teamFile, `token=${secret}`, projectRoot);
        expect(msg).toMatch(/team memory is shared with all repository collaborators/i);
        expect(msg).toContain('GitHub PAT');
        expect(msg).not.toContain(secret);
    });
    it('allows clean content on a team path', () => {
        expect(checkTeamMemorySecrets(teamFile, 'Use real DBs in integration tests.', projectRoot)).toBeNull();
    });
    it('ignores secrets written outside the team directory', () => {
        expect(checkTeamMemorySecrets(outsideFile, `token=${secret}`, projectRoot)).toBeNull();
    });
    it('blocks secrets written through a symlink into team memory', () => {
        ...
        expect(checkTeamMemorySecrets(path.join(alias, 'leak.md'), `token=${secret}`, projectRoot)).toMatch(/team memory is shared/i);
    });
});
//# sourceMappingURL=team-memory-secret-guard.test.js.map