...
import { assertRealProjectSkillPath, getProjectSkillsRoot, isProjectSkillPath, SKILL_FILE_NAME, } from '../skills/skill-paths.js';
...
export const SKILL_REVIEW_SYSTEM_PROMPT = [
    ...
    `- When creating a new skill, its directory MUST use the \`${AUTO_SKILL_DIR_PREFIX}\` prefix (e.g. \`.hopcode/skills/${AUTO_SKILL_DIR_PREFIX}<name>/SKILL.md\`) so the project's .gitignore keeps auto-generated skills out of version control. Keep the frontmatter \`name:\` as the natural \`<name>\` without the prefix.`,
...
export async function buildTaskPrompt(projectRoot) {
    ...
    return [
        ...
        `New skills you create MUST live at \`.hopcode/skills/${AUTO_SKILL_DIR_PREFIX}<name>/SKILL.md\` — the \`${AUTO_SKILL_DIR_PREFIX}\` directory prefix is mandatory so the project's .gitignore keeps auto-generated skills out of version control. Keep the frontmatter \`name:\` as the natural \`<name>\` (no prefix). The frontmatter MUST include 'source: auto-skill':`,
        '',
        '---',
        'name: <skill-name>',
