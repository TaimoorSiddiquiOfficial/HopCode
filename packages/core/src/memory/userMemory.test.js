...
import { AUTO_MEMORY_INDEX_FILENAME, USER_AUTO_MEMORY_DIRNAME, clearAutoMemoryRootCache, getAutoMemoryRoot, getUserAutoMemoryIndexPath, getUserAutoMemoryRoot, getUserAutoMemoryTopicPath, isAnyAutoMemPath, isAutoMemPath, isUserAutoMemPath, } from './paths.js';
import { ensureUserAutoMemoryScaffold, readUserAutoMemoryIndex, } from './store.js';
import { scanUserAutoMemoryTopicDocuments } from './scan.js';
import { rebuildUserAutoMemoryIndex } from './indexer.js';
import { appendManagedAutoMemoryToUserMemory, buildManagedAutoMemoryPrompt, } from './prompt.js';
describe('user-level auto-memory', () => {
    ...
    beforeEach(async () => {
        ...
        previousBaseDir = process.env['HOPCODE_CODE_MEMORY_BASE_DIR'];
        process.env['HOPCODE_CODE_MEMORY_BASE_DIR'] = tempDir;
    ...
    describe('paths', () => {
        it('places user memory at {HOPCODE_CODE_MEMORY_BASE_DIR}/memories', () => {
            expect(getUserAutoMemoryRoot()).toBe(path.join(tempDir, USER_AUTO_MEMORY_DIRNAME));
            expect(getUserAutoMemoryIndexPath()).toBe(path.join(tempDir, USER_AUTO_MEMORY_DIRNAME, AUTO_MEMORY_INDEX_FILENAME));
            expect(getUserAutoMemoryTopicPath('user')).toBe(path.join(tempDir, USER_AUTO_MEMORY_DIRNAME, 'user.md'));
        });
        it('isUserAutoMemPath accepts paths under the user root, rejects others', () => {
            const userRoot = getUserAutoMemoryRoot();
            expect(isUserAutoMemPath(path.join(userRoot, 'user', 'role.md'))).toBe(true);
            ...
            expect(isUserAutoMemPath(path.join(userRoot, '..', 'escape.md'))).toBe(false);
        });
        it('isAnyAutoMemPath accepts paths in either the project or the user root', () => {
            ...
            const userRoot = getUserAutoMemoryRoot();
            expect(isAnyAutoMemPath(path.join(projectMemoryRoot, 'feedback', 'x.md'), projectRoot)).toBe(true);
            expect(isAnyAutoMemPath(path.join(userRoot, 'user', 'y.md'), projectRoot)).toBe(true);
            expect(isAnyAutoMemPath(path.join(tempDir, 'outside.md'), projectRoot)).toBe(false);
            // Symmetric check: project-only helper should reject the user path
            expect(isAutoMemPath(path.join(userRoot, 'user', 'y.md'), projectRoot)).toBe(false);
        });
    });
    describe('scaffold + index', () => {
        it('ensureUserAutoMemoryScaffold creates root dir + empty MEMORY.md, idempotent', async () => {
            ...
            await expect(fs.readFile(getUserAutoMemoryIndexPath(), 'utf-8')).resolves.toBe('');
            ...
            await ensureUserAutoMemoryScaffold();
            await expect(fs.readFile(getUserAutoMemoryIndexPath(), 'utf-8')).resolves.toBe(customIndex);
            // Unlike per-project scaffold, no meta.json / extract-cursor.json
            await expect(fs.access(path.join(getUserAutoMemoryRoot(), 'meta.json'))).rejects.toThrow();
            await expect(fs.access(path.join(getUserAutoMemoryRoot(), 'extract-cursor.json'))).rejects.toThrow();
        });
        ...
        it('readUserAutoMemoryIndex reads existing content', async () => {
            await ensureUserAutoMemoryScaffold();
            await fs.writeFile(getUserAutoMemoryIndexPath(), '- [Role](user/role.md) — User is a Go engineer.\n', 'utf-8');
            await expect(readUserAutoMemoryIndex()).resolves.toBe('- [Role](user/role.md) — User is a Go engineer.\n');
        });
    });
    describe('scan + rebuild', () => {
        async function writeUserMemoryDoc(type, name, description, body) {
        ...
        it('scanUserAutoMemoryTopicDocuments returns documents written under the user root', async () => {
            await ensureUserAutoMemoryScaffold();
            await writeUserMemoryDoc('user', 'role', 'User is a Go engineer.', 'User has been writing Go for 10 years.');
        ...
        it('rebuildUserAutoMemoryIndex writes MEMORY.md from the user docs', async () => {
            await ensureUserAutoMemoryScaffold();
            await writeUserMemoryDoc('user', 'role', 'User is a Go engineer.', 'User has been writing Go for 10 years.');
            await writeUserMemoryDoc('feedback', 'terse', 'User prefers terse responses.', 'Skip end-of-turn summaries.');
            ...
            await expect(fs.readFile(getUserAutoMemoryIndexPath(), 'utf-8')).resolves.toBe(index);
        });
    });
    describe('system prompt rendering', () => {
        it('renders both index sections when a user section is provided', () => {
            const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.hopcode/memory', '- [Release](project/release.md) — Release Friday.', {
            ...
            expect(prompt).toContain('- [Role](user/role.md) — User is a Go engineer.');
            expect(prompt).toContain('- [Release](project/release.md) — Release Friday.');
            // Scope guidance is surfaced for every type
            expect(prompt).toContain('<scope>always user (cross-project)</scope>');
            expect(prompt).toContain('<scope>always project (this-project-only)</scope>');
        ...
        it('renders user section FIRST (background) then project section (more specific)', () => {
            ...
            const projectIdx = prompt.indexOf('## /tmp/project/.hopcode/memory/MEMORY.md');
        ...
        it('appendManagedAutoMemoryToUserMemory passes the user section through', () => {
            const result = appendManagedAutoMemoryToUserMemory('Project rules from HOPCODE.md', '/tmp/project/.hopcode/memory', '- [Release](project/release.md) — Release Friday.', {
                memoryDir: '/tmp/global/memories',
                indexContent: '- [Role](user/role.md) — User is a Go engineer.',
            });
            expect(result).toContain('Project rules from HOPCODE.md');
            expect(result).toContain('USER memory');
            expect(result).toContain('/tmp/global/memories');
        });
    });
});
//# sourceMappingURL=userMemory.test.js.map