...
describe('selectRelevantAutoMemoryDocumentsByModel', () => {
    ...
    it('tells the selector not to recall active tool schemas or failed calls', async () => {
        ...
        await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'read the ATA article', docs, 2, ['mcp__ata__article-list-query']);
        const options = vi.mocked(runSideQuery).mock.calls[0][1];
        expect(options.systemInstruction).toContain('parameter schemas, field mappings, guessed call formats, or failed-call transcripts');
        expect(options.systemInstruction).toContain('known gotchas, warnings, or confirmed workarounds');
        expect(JSON.stringify(options.contents)).toContain('Recently used tools: mcp__ata__article-list-query');
    });
    ...
    it('throws when selector returns unknown file paths', async () => {
        ...
        await expect(selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check memory', docs, 2)).rejects.toThrow('Recall selector returned unknown file path');
    });
    it('distinguishes docs with identical relativePath across scopes', async () => {
        ...
        const dualScopeDocs = [
            {
                type: 'user',
                filePath: '/hopcode/projects/proj/memory/user/role.md',
            ...
            {
                type: 'user',
                filePath: '/hopcode/memories/user/role.md',
                relativePath: 'user/role.md',
                filename: 'role.md',
        ...
        vi.mocked(runSideQuery).mockResolvedValue({
            selected_memories: [
                '/hopcode/projects/proj/memory/user/role.md',
                '/hopcode/memories/user/role.md',
            ],
        });
        const result = await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'who is the user', dualScopeDocs, 5, []);
