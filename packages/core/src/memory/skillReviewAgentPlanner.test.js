...
import { AUTO_SKILL_DIR_PREFIX, buildTaskPrompt, createSkillScopedAgentConfig, DEFAULT_AUTO_SKILL_TIMEOUT_MS, listExistingSkillDirNames, runSkillReviewByAgent, SKILL_REVIEW_SYSTEM_PROMPT, } from './skillReviewAgentPlanner.js';
import { ToolNames } from '../tools/tool-names.js';
import { runForkedAgent } from '../utils/forkedAgent.js';
vi.mock('../utils/forkedAgent.js', () => ({
    runForkedAgent: vi.fn(),
}));
function makeMinimalConfig(projectRoot) {
    return {
 ...
 */
function scopedPm(projectRoot) {
    const scoped = createSkillScopedAgentConfig(makeMinimalConfig(projectRoot), projectRoot);
    const pm = scoped.getPermissionManager();
    if (!pm) {
        throw new Error('createSkillScopedAgentConfig must install a PermissionManager');
    }
    return pm;
}
async function writeSkillFile(projectRoot, skillName, content) {
...
describe('skillReviewAgentPlanner — write_file collision deny (#4437)', () => {
    ...
    it('allows write_file to a fresh path that does not yet exist', async () => {
        const fresh = path.join(projectRoot, '.hopcode', 'skills', 'brand-new', 'SKILL.md');
        const pm = scopedPm(projectRoot);
        const decision = await pm.evaluate({
            toolName: ToolNames.WRITE_FILE,
            filePath: fresh,
    ...
    it('denies write_file to a non-SKILL.md path inside the skills root', async () => {
        ...
        // hard guard for that.
        const aux = path.join(projectRoot, '.hopcode', 'skills', 'my-skill', 'NOTES.md');
    ...
    it('denies write_file when the target traverses a symlink outside the skills root', async () => {
        ...
        const skillsRoot = path.join(projectRoot, '.hopcode', 'skills');
    ...
    it('denies write_file when the target path is a directory, not a file', async () => {
        ...
        const dirAsFile = path.join(projectRoot, '.hopcode', 'skills', 'is-a-directory', 'SKILL.md');
...
describe('listExistingSkillDirNames', () => {
    ...
    it('skips directories without SKILL.md so half-built dirs do not reserve names', async () => {
        ...
        await fs.mkdir(path.join(projectRoot, '.hopcode', 'skills', 'empty'), {
            recursive: true,
        });
...
describe('buildTaskPrompt', () => {
    ...
    it('instructs the agent to use the auto-skill- directory prefix (#4837)', async () => {
        // The `.gitignore` re-ignores `.hopcode/skills/auto-skill-*/`, so new
        ...
        expect(prompt).toContain(`.hopcode/skills/${AUTO_SKILL_DIR_PREFIX}<name>/`);
...
describe('SKILL_REVIEW_SYSTEM_PROMPT', () => {
    it('requires the auto-skill- directory prefix for new skills (#4837)', () => {
        ...
        expect(SKILL_REVIEW_SYSTEM_PROMPT).toContain(`.hopcode/skills/${AUTO_SKILL_DIR_PREFIX}<name>/`);
...
describe('runSkillReviewByAgent timeout wiring', () => {
    let tempDir;
    let projectRoot;
    function makeConfig(timeoutMinutes) {
        return {
            getProjectRoot: () => projectRoot,
            getPermissionManager: () => undefined,
    ...
    it('uses the configured memory agent timeout when no timeoutMs param is passed', async () => {
        ...
        expect(runForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ maxTimeMinutes: 30 }));
    });
    it('lets an explicit timeoutMs param override the configured value', async () => {
        ...
        expect(runForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ maxTimeMinutes: 1 }));
    });
...
//# sourceMappingURL=skillReviewAgentPlanner.test.js.map