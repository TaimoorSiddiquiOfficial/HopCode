...
async function makeSkill(root, name, body = 'hi') {
    const dir = path.join(root, '.hopcode', 'skills', `auto-skill-${name}`);
...
describe('pendingSkills', () => {
    ...
    it('moves auto-skill dirs out of skills root into pending root', async () => {
        ...
        await expect(fs.access(pending[0].stagedManifestPath)).resolves.toBeUndefined();
        expect(pending[0].stagedManifestPath.startsWith(getPendingSkillsRoot(root))).toBe(true);
    });
    it('does NOT stage a pre-existing skill the agent edited in place', async () => {
        const file = await makeSkill(root, 'epsilon');
        const pending = await stageSkillDirs([file], root, new Set(['auto-skill-epsilon']));
    ...
    it('stages new skills while leaving pre-existing edited ones in place', async () => {
        ...
        const pending = await stageSkillDirs([newFile, editedFile], root, new Set(['auto-skill-old-one']));
    ...
    it('parses an empty description as empty, not the next YAML line', async () => {
        const dir = path.join(root, '.hopcode', 'skills', 'auto-skill-empty');
        await fs.mkdir(dir, { recursive: true });
        await fs.writeFile(path.join(dir, 'SKILL.md'), '---\ndescription:\nsource: auto-skill\n---\nbody\n', 'utf-8');
    ...
    it('strips surrounding quotes from a quoted description', async () => {
        const dir = path.join(root, '.hopcode', 'skills', 'auto-skill-quoted');
        await fs.mkdir(dir, { recursive: true });
    ...
    it('ignores touched paths whose skill dir no longer exists (edited existing skill)', async () => {
        const pending = await stageSkillDirs([path.join(root, '.hopcode', 'skills', 'auto-skill-x', 'SKILL.md')], root);
        expect(pending).toHaveLength(0);
    });
});
//# sourceMappingURL=pending-skills.test.js.map