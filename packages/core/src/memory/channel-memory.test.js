...
import { addChannelMemoryEntries, appendChannelMemory, CHANNEL_MEMORY_FILE_NAME, clearChannelMemory, getChannelMemoryFilePath, getChannelMemoryRevision, getLegacyChannelMemoryFilePath, listChannelMemoryEntries, MAX_CHANNEL_MEMORY_BYTES, readChannelMemory, removeChannelMemoryEntries, updateChannelMemoryEntry, } from './channel-memory.js';
import { parseChannelMemoryDocument, parseLegacyChannelMemory, serializeChannelMemoryDocument, } from './channel-memory-document.js';
const fsFailure = vi.hoisted(() => ({
    tempSync: false,
    tempBytesAtSync: 0,
    rename: false,
    legacyUnlinkPath: undefined,
    readErrorPath: undefined,
    readRace: undefined,
    legacyAppendAfterRename: undefined,
}));
...
describe('channel memory', () => {
    ...
    async function expectCredentialRejection(write) {
        let error;
        try {
            await write();
        }
        ...
        expect(error instanceof Error ? error.message : undefined).toBe('Channel memory cannot store detected credentials');
    }
    it('uses JSON for canonical storage and Markdown for legacy storage', () => {
        const filePath = getChannelMemoryFilePath(target);
        const legacyPath = getLegacyChannelMemoryFilePath(target);
        expect(filePath.startsWith(qwenHome + path.sep)).toBe(true);
        expect(filePath.endsWith(path.join('', CHANNEL_MEMORY_FILE_NAME))).toBe(true);
    ...
    it.each(['.', '..'])('does not use exact %s as the channel directory segment', (channelName) => {
        const filePath = getChannelMemoryFilePath({
            channelName,
            chatId: 'chat-1',
        });
    ...
    it('uses different paths for colliding sanitized channel names and threads', () => {
        expect(getChannelMemoryFilePath({ channelName: 'ops/alerts', chatId: 'chat-1' })).not.toBe(getChannelMemoryFilePath({ channelName: 'ops alerts', chatId: 'chat-1' }));
        expect(getChannelMemoryFilePath({ ...target, threadId: 'thread-1' })).not.toBe(getChannelMemoryFilePath({ ...target, threadId: 'thread-2' }));
    });
    ...
    it('changes revision when canonical channel memory changes', async () => {
        ...
        writeJson(serializeChannelMemoryDocument({
            version: 1,
    ...
    it('migrates legacy content on add and cleans up the legacy file after commit', async () => {
        const legacyPath = writeLegacy('Use staging\n');
        const result = await addChannelMemoryEntries(target, ['Run tests'], 'alice');
    ...
    it('does not delete legacy bytes changed after canonical commit', async () => {
        ...
        await expect(addChannelMemoryEntries(target, ['Run tests'])).resolves.toMatchObject({ changed: true });
    ...
    it('skips normalized duplicate additions and returns their existing IDs', async () => {
        const first = await addChannelMemoryEntries(target, ['Use staging'], 'alice');
        const duplicate = await addChannelMemoryEntries(target, [' use   STAGING '], 'alice');
    ...
    it('rejects credential-bearing batches before creating canonical storage', async () => {
        ...
        await expectCredentialRejection(() => addChannelMemoryEntries(target, ['Use staging', credentialText]));
        expect(fs.existsSync(getChannelMemoryFilePath(target))).toBe(false);
    });
    it('rejects credential-bearing batches without changing canonical storage', async () => {
        ...
        await expectCredentialRejection(() => addChannelMemoryEntries(target, ['Run tests', credentialText]));
    ...
    it('updates only text and updatedAt while preserving identity metadata', async () => {
        const [entry] = (await addChannelMemoryEntries(target, ['Use staging'], 'alice')).added;
    ...
    it('rejects credential-bearing replacements without changing the entry', async () => {
        ...
        await expectCredentialRejection(() => updateChannelMemoryEntry(target, { id: entry.id, text: credentialText }));
        expect(fs.readFileSync(filePath)).toEqual(before);
    ...
    it('allows manually seeded credential entries to be removed and cleared', async () => {
        ...
        await expect(removeChannelMemoryEntries(target, { ids: [entry.id] })).resolves.toMatchObject({ changed: true, removed: [entry] });
    ...
    it('rejects updates that duplicate another entry after normalization', async () => {
        const [first, second] = (await addChannelMemoryEntries(target, ['Use staging', 'Run tests'])).added;
        await expect(updateChannelMemoryEntry(target, {
            id: second.id,
            text: ' use   STAGING ',
    ...
    it('rejects remove CAS when any expected entry was deleted', async () => {
        ...
        await expect(removeChannelMemoryEntries(target, {
    ...
    it('removes requested IDs once and ignores missing IDs', async () => {
        ...
        expect(result.changed).toBe(true);
        await expect(removeChannelMemoryEntries(target, { ids: ['m-111111111111'] })).resolves.toEqual({
            changed: false,
            filePath: getChannelMemoryFilePath(target),
            removed: [],
        });
    ...
    it('clears entries while preserving migration metadata', async () => {
        const legacyPath = writeLegacy('Use staging\n');
        await addChannelMemoryEntries(target, ['Run tests']);
        const before = parseChannelMemoryDocument(fs.readFileSync(getChannelMemoryFilePath(target), 'utf8'));
        await expect(clearChannelMemory(target)).resolves.toEqual({
            changed: true,
            filePath: getChannelMemoryFilePath(target),
        });
        expect(fs.existsSync(legacyPath)).toBe(false);
        expect(parseChannelMemoryDocument(fs.readFileSync(getChannelMemoryFilePath(target), 'utf8'))).toEqual({ version: 1, migration: before.migration, entries: [] });
    });
    ...
    it('rejects additions beyond request, entry, and text limits', async () => {
        await expect(addChannelMemoryEntries(target, Array.from({ length: 11 }, () => 'entry'))).rejects.toThrow();
        await expect(addChannelMemoryEntries(target, ['a'.repeat(2_001)])).rejects.toThrow('Invalid channel memory entry');
        for (let index = 0; index < 50; index++) {
            await addChannelMemoryEntries(target, Array.from({ length: 10 }, (_, offset) => `entry ${index * 10 + offset}`));
        }
        await expect(addChannelMemoryEntries(target, ['too many'])).rejects.toThrow('Channel memory exceeds maximum number of entries');
    });
    it('rejects oversized serialized JSON without creating canonical storage', async () => {
        await expect(addChannelMemoryEntries(target, ['entry'], 'x'.repeat(MAX_CHANNEL_MEMORY_BYTES))).rejects.toThrow('Channel memory exceeds maximum size');
        expect(fs.existsSync(getChannelMemoryFilePath(target))).toBe(false);
    });
    it('fails closed for malformed JSON and unsupported JSON versions', async () => {
        writeLegacy('Use staging\n');
        writeJson('{');
        await expect(listChannelMemoryEntries(target)).rejects.toThrow('Invalid channel memory document');
        await expect(addChannelMemoryEntries(target, ['Run tests'])).rejects.toThrow('Invalid channel memory document');
        expect(fs.readFileSync(getLegacyChannelMemoryFilePath(target), 'utf8')).toBe('Use staging\n');
        writeJson('{"version":2,"entries":[]}');
        await expect(listChannelMemoryEntries(target)).rejects.toThrow('Unsupported channel memory version');
    });
    it('rejects unknown JSON keys without modifying either source', async () => {
        const legacyPath = writeLegacy('Use staging\n');
        const legacyBefore = fs.readFileSync(legacyPath);
        const filePath = writeJson(JSON.stringify({
            version: 1,
            migration: {
        ...
        const readResult = await readChannelMemory(target).then(() => 'fulfilled', () => 'rejected');
    ...
    it('rejects invalid UTF-8 JSON without modifying either source', async () => {
        ...
        fs.writeFileSync(filePath, Buffer.concat([
            Buffer.from(`{"version":1,"migration":{"legacySha256":"${migrationHash}"},"entries":[{"id":"m-111111111111","text":"`),
            Buffer.from([0xff]),
            Buffer.from('"}]}'),
        ]));
        const canonicalBefore = fs.readFileSync(filePath);
        await expect(readChannelMemory(target)).rejects.toThrow();
        await expect(addChannelMemoryEntries(target, ['Run tests'])).rejects.toThrow();
        expect(fs.readFileSync(filePath)).toEqual(canonicalBefore);
        expect(fs.readFileSync(legacyPath)).toEqual(legacyBefore);
    });
    ...
    it('rejects non-missing read errors without modifying either source', async () => {
        ...
        const filePath = writeJson(serializeChannelMemoryDocument(parseLegacyChannelMemory(legacy)));
        const canonicalBefore = fs.readFileSync(filePath);
        const legacyBefore = fs.readFileSync(legacyPath);
    ...
    it('rejects divergent or unhashed dual files', async () => {
        ...
        await expect(listChannelMemoryEntries(target)).rejects.toThrow('Channel memory migration conflict');
        writeJson(serializeChannelMemoryDocument({ version: 1, entries: [] }));
        await expect(addChannelMemoryEntries(target, ['Run tests'])).rejects.toThrow('Channel memory migration conflict');
    });
    it('cleans a written temp file and recovers lock and queue after sync failure', async () => {
        ...
        await expect(addChannelMemoryEntries(target, ['Run tests'])).rejects.toThrow('temp sync failed');
        expect(fsFailure.tempBytesAtSync).toBeGreaterThan(0);
        expect(fs.readdirSync(directory).filter((name) => name.endsWith('.tmp'))).toEqual([]);
        expect(fs.existsSync(getChannelMemoryFilePath(target))).toBe(false);
        expect(fs.readFileSync(legacyPath, 'utf8')).toBe('Use staging\n');
        fsFailure.tempSync = false;
        await expect(addChannelMemoryEntries(target, ['after failure'])).resolves.toMatchObject({
            changed: true,
        });
        await expect(readChannelMemory(target)).resolves.toBe('Use staging\nafter failure\n');
    });
    it('preserves the previous JSON when atomic rename fails', async () => {
        ...
        await expect(addChannelMemoryEntries(target, ['Run tests'])).rejects.toThrow('rename failed');
    ...
    it('reports a committed migration when legacy cleanup fails and retries later', async () => {
        ...
        await expect(addChannelMemoryEntries(target, ['Run tests'])).resolves.toMatchObject({
            changed: true,
    ...
    it('serializes concurrent additions without losing entries', async () => {
        const additions = await Promise.all(Array.from({ length: 20 }, (_, index) => addChannelMemoryEntries(target, [`entry ${index}`])));
        const entries = await listChannelMemoryEntries(target);
        expect(entries).toHaveLength(20);
        expect(new Set(entries.map((entry) => entry.id)).size).toBe(20);
        expect(new Set(additions.flatMap((result) => result.added.map((entry) => entry.text)))).toEqual(new Set(Array.from({ length: 20 }, (_, index) => `entry ${index}`)));
        expect(() => parseChannelMemoryDocument(fs.readFileSync(getChannelMemoryFilePath(target), 'utf8'))).not.toThrow();
    });
    ...
    it('serializes clear racing additions and first migration racing another add', async () => {
        ...
        await Promise.all([
            ...
            ...Array.from({ length: 10 }, (_, index) => addChannelMemoryEntries(target, [`entry ${index}`])),
        ]);
        const entries = await listChannelMemoryEntries(target);
        expect(entries.map((entry) => entry.text)).toEqual(Array.from({ length: 10 }, (_, index) => `entry ${index}`));
        expect(new Set(entries.map((entry) => entry.id)).size).toBe(entries.length);
        expect(() => parseChannelMemoryDocument(fs.readFileSync(getChannelMemoryFilePath(target), 'utf8'))).not.toThrow();
    });
});
//# sourceMappingURL=channel-memory.test.js.map