...
import { buildBareRememberPrompt, buildManagedRememberPrompt, runManagedRememberByAgent, } from './remember.js';
...
function createConfig(projectRoot, managed = true, overrides = {}) {
    return {
...
describe('remember memory helper', () => {
    ...
    it('builds the same managed and bare prompts used by /remember', () => {
        const managed = buildManagedRememberPrompt('  prefers focused tests  ', projectRoot);
        expect(managed).toContain('Please save the following to your memory system.');
        ...
        const wrapped = buildManagedRememberPrompt('  hidden context  ', projectRoot, { wrapUserContent: true });
        expect(wrapped).toContain('<user-content>\nhidden context\n</user-content>');
        const bare = buildBareRememberPrompt('  appends to qwen  ');
        expect(bare).toBe('Please save the following fact to memory (e.g. append to QWEN.md in the project root):\n\nappends to qwen');
    });
    it('runs clean context with managed-memory tools only', async () => {
        ...
        const params = vi.mocked(runForkedAgent).mock.calls[0]?.[0];
        ...
        const pm = params.config.getPermissionManager();
    ...
    it('keeps the built-in 5-minute default when no timeout is configured', async () => {
        ...
        expect(runForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ maxTimeMinutes: 5 }));
    });
    ...
    it('rebuilds touched project indexes and best-effort user indexes', async () => {
        ...
        vi.mocked(rebuildUserAutoMemoryIndex).mockRejectedValue(new Error('user index unavailable'));
        vi.mocked(runForkedAgent).mockResolvedValue({
            status: 'completed',
    ...
    it('rejects when managed memory is unavailable', async () => {
        await expect(runManagedRememberByAgent({
...
//# sourceMappingURL=remember.test.js.map