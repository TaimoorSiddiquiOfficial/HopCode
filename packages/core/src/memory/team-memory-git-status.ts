 ...
 * directory: a `!.hopcode/team-memory/` re-include is a directory-form negation
 ...
 * e.g. a `.hopcode/` directory-form ignore, which git never descends into, so a
...
export function getTeamMemoryShareabilityWarning(
  ...
  // (e.g. `.hopcode/team-memory/*.md` + `!.hopcode/team-memory/MEMORY.md`) pass while
  ...
  if (indexIgnored || topicIgnored) {
    ...
    return (
      ...
      `memories are not shared. If your .gitignore excludes '.hopcode/' ` +
      `(directory form), change it to '.hopcode/*' and re-include ` +
      `'.hopcode/team-memory/' (and its contents, e.g. '!.hopcode/team-memory/**').`
    );
  }
  return null;
}
