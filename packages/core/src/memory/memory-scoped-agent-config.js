...
import { getAutoMemoryRoot, getAutoMemoryTrustedAnchor, getUserAutoMemoryRoot, } from './paths.js';
 ...
 * tracked `.hopcode -> /outside` under `HOPCODE_CODE_MEMORY_LOCAL` — which would
...
async function evaluateScopedDecision(ctx, projectRoot, opts) {
    switch (ctx.toolName) {
...
function getScopedDenyRule(ctx, projectRoot, opts) {
...
export function createMemoryScopedAgentConfig(config, projectRoot, options = {}) {
    ...
    const scopedPm = {
        ...
        async evaluate(ctx) {
            const scopedDecision = await evaluateScopedDecision(ctx, projectRoot, opts);
            if (!basePm) {
