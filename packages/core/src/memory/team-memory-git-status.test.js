...
function initRepo(label) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), `qwen-gitstatus-${label}-`));
...
describe('getTeamMemoryShareabilityWarning', () => {
    ...
    it('warns when a directory-form .gitignore swallows the team dir', () => {
        ...
        cleanup.push(repo);
        // The common pitfall: ignoring `.hopcode/` (directory form) makes git skip the
        // folder entirely, so memories are silently never tracked.
        fs.writeFileSync(path.join(repo, '.gitignore'), '.hopcode/\n');
        const warning = getTeamMemoryShareabilityWarning(repo);
        expect(warning).toContain('git-ignored');
    });
    it('returns null when .hopcode/* re-includes the team dir', () => {
    ...
    it('warns when topic files are ignored even though the index is re-included', () => {
        ...
        fs.writeFileSync(path.join(repo, '.gitignore'), '.hopcode/team-memory/*.md\n!.hopcode/team-memory/MEMORY.md\n');
        const warning = getTeamMemoryShareabilityWarning(repo);
        expect(warning).toContain('git-ignored');
    });
});
//# sourceMappingURL=team-memory-git-status.test.js.map