...
describe('scanForSecrets', () => {
    it('returns no matches for clean content', () => {
        expect(scanForSecrets('Integration tests must hit a real database, not mocks.')).toEqual([]);
    });
    ...
    it('detects all current OpenAI key formats', () => {
        for (const sample of [
            ...
            expect(scanForSecrets(sample).map((m) => m.ruleId)).toContain('openai-api-key');
        }
    });
    it('detects a secret followed by punctuation delimiters', () => {
        const key = `sk-ant-api03-${'a'.repeat(30)}`;
        for (const delim of ['.', ',', ')', '}']) {
            expect(scanForSecrets(`key=${key}${delim}`).map((m) => m.ruleId)).toContain('anthropic-api-key');
        }
    });
    it('detects an AWS access key whose suffix contains digits 0/1/8/9', () => {
        ...
        expect(scanForSecrets(`AKIA0918ABCDEFGH9012`).map((m) => m.ruleId)).toContain('aws-access-token');
    });
    it('detects a HuggingFace token containing digits', () => {
        expect(scanForSecrets(`hf_${'a1b2'.repeat(9)}`).map((m) => m.ruleId)).toContain('huggingface-access-token');
    });
    ...
    it('handles many unmatched private-key markers without a match', () => {
        const payload = Array.from({ length: 4000 }, () => '-----BEGIN PRIVATE KEY-----').join('\n');
    ...
    it('reports readable labels (acronyms and providers expanded)', () => {
        ...
        expect(scanForSecrets('AKIAIOSFODNN7EXAMPLE')[0].label).toBe('AWS Access Token');
        expect(scanForSecrets(`LTAI${'a'.repeat(16)}`)[0].label).toBe('Alibaba Cloud Access Key');
    });
...
//# sourceMappingURL=secret-scanner.test.js.map