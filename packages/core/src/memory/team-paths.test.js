...
import { clearAutoMemoryRootCache, getAutoMemoryRoot, getTeamAutoMemoryIndexPath, getTeamAutoMemoryRoot, getUserAutoMemoryRoot, isAnyAutoMemPath, isManagedMemoryPath, isTeamAutoMemPath, TEAM_AUTO_MEMORY_DIRNAME, } from './paths.js';
describe('team auto-memory paths', () => {
    ...
    it('anchors the team root inside the repo .hopcode directory', () => {
        expect(getTeamAutoMemoryRoot(projectRoot)).toBe(path.join(projectRoot, '.hopcode', TEAM_AUTO_MEMORY_DIRNAME));
    });
    it('uses the current linked worktree root instead of the common git root', () => {
        ...
        const worktree = fs.mkdtempSync(path.join(os.tmpdir(), 'hopcode-wt-'));
        try {
            ...
            expect(getTeamAutoMemoryRoot(worktree)).toBe(path.join(worktree, '.hopcode', TEAM_AUTO_MEMORY_DIRNAME));
        }
        finally {
    ...
    it('places MEMORY.md at the team root', () => {
        expect(getTeamAutoMemoryIndexPath(projectRoot)).toBe(path.join(projectRoot, '.hopcode', TEAM_AUTO_MEMORY_DIRNAME, 'MEMORY.md'));
    });
    it('contains paths inside the team root and rejects escapes', () => {
        ...
        expect(isTeamAutoMemPath(path.join(root, 'feedback/x.md'), projectRoot)).toBe(true);
        expect(isTeamAutoMemPath(path.join(root, '..', 'escape.md'), projectRoot)).toBe(false);
    });
    it('excludes team paths from isAnyAutoMemPath while isTeamAutoMemPath includes them', () => {
        ...
        const teamFile = path.join(getTeamAutoMemoryRoot(projectRoot), 'feedback', 'shared.md');
    ...
    it('classifies all managed roots for read retention and rejects symlink escapes', () => {
        ...
        clearAutoMemoryRootCache();
        try {
            const projectMemory = path.join(getAutoMemoryRoot(projectRoot), 'project', 'context.md');
            const userMemory = path.join(getUserAutoMemoryRoot(), 'feedback', 'testing.md');
            const teamMemory = path.join(getTeamAutoMemoryRoot(projectRoot), 'reference', 'architecture.md');
            ...
            expect(isManagedMemoryPath(path.join(projectRoot, 'src', 'main.ts'), projectRoot)).toBe(false);
    ...
    it('recognizes a first-ever write before the team-memory dir exists', () => {
        const root = getTeamAutoMemoryRoot(projectRoot);
        // Normal first-write state: nothing under .hopcode has been created yet, so
        ...
        expect(isTeamAutoMemPath(path.join(root, 'feedback', 'new.md'), projectRoot)).toBe(true);
    });
    it('recognizes new files under a symlinked team-memory directory', () => {
        ...
        expect(isTeamAutoMemPath(path.join(alias, 'leak.md'), projectRoot)).toBe(true);
    });
    ...
    it('clearAutoMemoryRootCache invalidates the team root cache', () => {
        ...
        try {
            ...
            expect(getTeamAutoMemoryRoot(nested)).toBe(path.join(nested, '.hopcode', TEAM_AUTO_MEMORY_DIRNAME));
            fs.mkdirSync(path.join(fresh, '.git'));
            clearAutoMemoryRootCache();
            // After clearing, it must re-resolve to the new git root.
            expect(getTeamAutoMemoryRoot(nested)).toBe(path.join(fresh, '.hopcode', TEAM_AUTO_MEMORY_DIRNAME));
        }
        finally {
...
//# sourceMappingURL=team-paths.test.js.map