...
describe('runAutoMemoryExtractionByAgent', () => {
    ...
    it('passes 0 through to disable the time limit', async () => {
        ...
        await runAutoMemoryExtractionByAgent(mockConfig, '/tmp');
        expect(runForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ maxTimeMinutes: 0 }));
    });
    ...
    it('uses a scoped config that allows shell and denies outside writes', async () => {
        ...
        expect(permissionManager.findMatchingDenyRule({
            toolName: ToolNames.WRITE_FILE,
            filePath: '/tmp/outside.md',
        })).toBe('ManagedAutoMemory(write_file: only within /tmp/user-memory or /tmp/auto-memory)');
        expect(await permissionManager.evaluate({
            toolName: ToolNames.WRITE_FILE,
            filePath: '/tmp/outside.md',
        })).toBe('deny');
    });
    ...
    it('attributes user-rooted writes to the user scope (not project)', async () => {
        ...
        const result = await runAutoMemoryExtractionByAgent(mockConfig, '/tmp');
        expect(result.touchedTopics).toEqual(expect.arrayContaining(['user', 'feedback']));
    ...
    it('classifies file paths when the root is backslash-native (Windows) but agent reports forward slashes', async () => {
        ...
        vi.mocked(getAutoMemoryRoot).mockReturnValue('C:\\Users\\foo\\.hopcode\\projects\\proj\\memory');
        vi.mocked(getUserAutoMemoryRoot).mockReturnValue('C:\\Users\\foo\\.hopcode\\memories');
        vi.mocked(runForkedAgent).mockResolvedValue({
            status: 'completed',
            finalText: '',
            filesTouched: [
        ...
        try {
            const result = await runAutoMemoryExtractionByAgent(mockConfig, '/tmp');
            expect(result.touchedTopics).toEqual(expect.arrayContaining(['project', 'user']));
            expect(result.touchedProjectScope).toBe(true);
            expect(result.touchedUserScope).toBe(true);
        }
        finally {
    ...
    it('reports both scopes when the agent writes to both roots in one run', async () => {
        ...
        const result = await runAutoMemoryExtractionByAgent(mockConfig, '/tmp');
        expect(result.touchedTopics).toEqual(expect.arrayContaining(['user', 'project']));
        expect(result.touchedProjectScope).toBe(true);
        expect(result.touchedUserScope).toBe(true);
    });
});
