...
import { didWriteManagedMemory, refreshMemoryAfterManagedWrite, refreshMemoryInstruction, } from './refresh.js';
vi.mock('./indexer.js', () => ({
    rebuildManagedAutoMemoryIndex: vi.fn(),
    rebuildUserAutoMemoryIndex: vi.fn(),
}));
function createConfig(projectRoot, managed = true) {
    return {
...
describe('managed memory refresh helper', () => {
        ...
        expect(didWriteManagedMemory([
            {
                toolName: 'write_file',
                args: {
                    file_path: path.join(projectRoot, '.qwen', 'team-memory', 'shared.md'),
                },
                status: 'success',
            },
        ], projectRoot)).toBe(false);
    ...
    it('rebuilds touched indexes before refreshing the live instruction', async () => {
        ...
        await expect(refreshMemoryAfterManagedWrite(config, [
            {
                toolName: 'write_file',
        ...
        expect(config.getGeminiClient().refreshSystemInstruction).toHaveBeenCalledTimes(1);
        expect(vi.mocked(rebuildManagedAutoMemoryIndex).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(config.refreshHierarchicalMemory).mock.invocationCallOrder[0]);
    });
    it('keeps refreshing when index rebuild fails', async () => {
        vi.mocked(rebuildManagedAutoMemoryIndex).mockRejectedValueOnce(new Error('index failed'));
        const config = createConfig(projectRoot);
        await expect(refreshMemoryAfterManagedWrite(config, [
            {
                toolName: 'write_file',
    ...
    it('keeps refreshing the system instruction when hierarchical refresh fails', async () => {
        const config = createConfig(projectRoot);
        vi.mocked(config.refreshHierarchicalMemory).mockRejectedValueOnce(new Error('hierarchical refresh failed'));
