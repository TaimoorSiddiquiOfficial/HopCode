...
import { AuthType, OutputFormat } from '@hoptrendy/hopcode-core';
...
describe('validateNonInterActiveAuth', () => {
    ...
    it('exits if currentAuthType does not match enforcedAuthType', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('The configured auth type is hopcode-oauth, but the current auth type is openai. Please re-authenticate with the correct type.');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    describe('JSON output mode', () => {
        ...
        it('emits error result and exits when enforced auth mismatches current auth', async () => {
            ...
            expect(emitResultMock).toHaveBeenCalledWith({
                isError: true,
                errorMessage: expect.stringContaining('The configured auth type is hopcode-oauth, but the current auth type is openai.'),
                durationMs: 0,
                apiDurationMs: 0,
                numTurns: 0,
                usage: undefined,
