...
function createOAuthCallbackServer(parsedUrl, expectedState, port, timeoutMs) {
    let server;
    let timeout;
    let settled = false;
    const close = async () => {
    ...
    server = createServer((req, res) => {
        ...
        res.end('<html><body><h1>OpenRouter authentication complete.</h1><p>You can return to HopCode.</p></body></html>');
...
export async function startOAuthCallbackListenerWithRetry(callbackUrl = OPENROUTER_OAUTH_CALLBACK_URL, timeoutMs = OPENROUTER_OAUTH_TIMEOUT_MS, expectedState, maxRetries = OPENROUTER_OAUTH_CALLBACK_PORT_RETRIES) {
    const parsedUrl = new URL(callbackUrl);
    if (parsedUrl.protocol !== 'http:') {
        throw new Error('Only http localhost callback URLs are currently supported.');
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        const listener = createOAuthCallbackServer(parsedUrl, expectedState, port, timeoutMs);
        try {
            await listener.ready;
            return listener;
        }
        catch (error) {
            const isAddrInUse = error instanceof Error &&
                'code' in error &&
    ...
    throw new Error(`Could not find an available port (tried ${basePort}–${basePort + maxRetries}).`);
}
...
const OPENROUTER_RECOMMENDED_MODEL_LIMIT = OPENROUTER_RECOMMENDED_FREE_MODEL_IDS.length;
...
function getOpenRouterRecommendedFreeModelPriority(modelId) {
    const normalizedId = modelId.toLowerCase();
    const matchedIndex = OPENROUTER_RECOMMENDED_FREE_MODEL_IDS.findIndex((recommendedId) => recommendedId === normalizedId);
...
function compareOpenRouterModels(a, b) {
    const recommendedFreeDiff = getOpenRouterRecommendedFreeModelPriority(a.id) -
...
export function selectRecommendedOpenRouterModels(models, limit = OPENROUTER_RECOMMENDED_MODEL_LIMIT) {
    ...
    for (const recommendedId of OPENROUTER_RECOMMENDED_FREE_MODEL_IDS) {
        addRecommendedModel(recommended, sorted.find((model) => model.id.toLowerCase() === recommendedId &&
            isOpenRouterFreeConfig(model)), selectedIds, limit);
    }
...
export async function runOpenRouterOAuthLogin(callbackUrl = OPENROUTER_OAUTH_CALLBACK_URL, deps = {}) {
    ...
    const listener = await startListener(preSession.callbackUrl, OPENROUTER_OAUTH_TIMEOUT_MS, preSession.state);
    const portChanged = listener.port !==
    ...
    if (deps.session?.authorizationUrl && !portChanged) {
        ...
        const challenge = deps.session != null
            ? new URL(deps.session.authorizationUrl).searchParams.get('code_challenge')
