...
describe('openrouterOAuth', () => {
    ...
    it('passes the session state to the OAuth callback listener', async () => {
        ...
        await runOpenRouterOAuthLogin('http://localhost:3000/openrouter/callback', {
            ...
            session: {
                ...
                codeVerifier: 'verifier-123',
                state: 'state-123',
                authorizationUrl: 'https://openrouter.ai/auth?state=state-123&code_challenge=challenge-123',
            },
        });
    ...
    it('returns 404 for non-callback paths', async () => {
        const listener = startOAuthCallbackListener('http://localhost:3102/openrouter/callback', 5000, 'state-123');
        await listener.ready;
    ...
    it('rejects with error when OpenRouter returns an error parameter', async () => {
        const listener = startOAuthCallbackListener('http://localhost:3103/openrouter/callback', 5000, 'state-123');
        await listener.ready;
        const codePromise = listener.waitForCode.catch((err) => err);
        await new Promise((resolve, reject) => {
            const req = request('http://localhost:3103/openrouter/callback?error=access_denied&state=state-123', (res) => {
                expect(res.statusCode).toBe(400);
                res.resume();
                res.on('end', resolve);
            });
    ...
    it('rejects with missing code error', async () => {
        const listener = startOAuthCallbackListener('http://localhost:3104/openrouter/callback', 5000, 'state-123');
        await listener.ready;
        const codePromise = listener.waitForCode.catch((err) => err);
        await new Promise((resolve, reject) => {
            const req = request('http://localhost:3104/openrouter/callback?state=state-123', (res) => {
                expect(res.statusCode).toBe(400);
                res.resume();
                res.on('end', resolve);
            });
    ...
    it('retries ports when address is in use', async () => {
        const blockingListener = startOAuthCallbackListener('http://localhost:3150/openrouter/callback', 10000, 'block-state');
        await blockingListener.ready;
        try {
            const retried = await startOAuthCallbackListenerWithRetry('http://localhost:3150/openrouter/callback', 5000, 'retry-state', 5);
    ...
    it('throws non-http protocol error', () => {
        expect(() => startOAuthCallbackListener('https://localhost:3000/callback', 5000, 'state-123')).toThrow('Only http localhost callback URLs are currently supported.');
    });
});
