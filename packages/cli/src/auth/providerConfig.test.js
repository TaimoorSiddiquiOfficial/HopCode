...
import { buildInstallPlan, buildProviderTemplate, computeModelListVersion, getDefaultModelIds, resolveBaseUrl, shouldShowStep, providerMatchesCredentials, } from './providerConfig.js';
...
describe('shouldShowStep', () => {
    ...
    it('hides apiKey step for oauth providers', () => {
        expect(shouldShowStep(makeConfig({ authMethod: 'input' }), 'apiKey')).toBe(true);
        expect(shouldShowStep(makeConfig({ authMethod: 'oauth' }), 'apiKey')).toBe(false);
    });
...
describe('providerMatchesCredentials', () => {
    ...
    it('returns false for function-typed envKey', () => {
        ...
        expect(providerMatchesCredentials(config, 'https://api.test.com/v1', 'DYNAMIC')).toBe(false);
    });
});
