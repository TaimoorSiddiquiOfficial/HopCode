...
import { isSubpath } from '@hoptrendy/hopcode-core';
...
function resolveKeytermsFiles(settings) {
    ...
    const hopcodeDir = path.dirname(workspacePath);
    const workspaceRoot = path.dirname(hopcodeDir);
    ...
    return [
        {
            filePath: path.join(hopcodeDir, DEFAULT_KEYTERMS_FILENAME),
            workspaceRoot,
