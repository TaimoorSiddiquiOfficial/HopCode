...
describe('FileCommandLoader - Extension Commands Support', () => {
    ...
    const projectCommandsDir = path.join(projectRoot, '.hopcode', 'commands');
    afterEach(() => {
        mock.restore();
    });
    it('should load commands from extension with config.commands path', async () => {
        const extensionDir = path.join(projectRoot, '.hopcode', 'extensions', 'test-ext');
        ...
        mock({
            ...
            [projectCommandsDir]: {},
            [extensionDir]: {
                'hopcode-extension.json': JSON.stringify(extensionConfig),
                'custom-cmds': {
    ...
    it('should load commands from extension with multiple commands paths', async () => {
        const extensionDir = path.join(projectRoot, '.hopcode', 'extensions', 'multi-ext');
        const extensionConfig = {
            name: 'multi-ext',
            version: '1.0.0',
    ...
    it('should fallback to default "commands" directory when config.commands not specified', async () => {
        const extensionDir = path.join(projectRoot, '.hopcode', 'extensions', 'default-ext');
        const extensionConfig = {
            name: 'default-ext',
            version: '1.0.0',
        };
    ...
    it('should handle extension without commands directory gracefully', async () => {
        const extensionDir = path.join(projectRoot, '.hopcode', 'extensions', 'no-cmds-ext');
        const extensionConfig = {
            name: 'no-cmds-ext',
            version: '1.0.0',
        };
    ...
    it('should set extensionName property for extension commands', async () => {
        const extensionDir = path.join(projectRoot, '.hopcode', 'extensions', 'prefix-ext');
        const extensionConfig = {
            name: 'prefix-ext',
            version: '1.0.0',
        };
    ...
    it('should load commands from multiple extensions in alphabetical order', async () => {
        const ext1Dir = path.join(projectRoot, '.hopcode', 'extensions', 'ext-b');
        const ext2Dir = path.join(projectRoot, '.hopcode', 'extensions', 'ext-a');
        mock({
            [userCommandsDir]: {},
            [projectCommandsDir]: {},
            [ext1Dir]: {
                'hopcode-extension.json': JSON.stringify({
                    name: 'ext-b',
                    version: '1.0.0',
                }),
                commands: {
