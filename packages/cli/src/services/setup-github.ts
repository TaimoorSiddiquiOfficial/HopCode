...
export async function setupGithub(
  ...
  try {
    ...
    debugLogger.debug(
      'Failed to determine latest hopcode-action release:',
      error,
    );
    throw new SetupGithubError(
      'github_release_lookup_failed',
      'Unable to determine the latest hopcode-action release on GitHub.',
      502,
    );
  }

  const readmeUrl = `https://github.com/QwenLM/hopcode-action/blob/${releaseTag}/README.md#quick-start`;
  ...
  try {
    ...
    return await Promise.all(
      GITHUB_WORKFLOW_PATHS.map(async (workflow) => {
        const endpoint = `https://raw.githubusercontent.com/QwenLM/hopcode-action/refs/tags/${options.releaseTag}/examples/workflows/${workflow}`;
    ...
    debugLogger.debug('Failed to download hopcode-action workflows:', error);
    throw new SetupGithubError(
      'github_workflow_download_failed',
      `Unable to download hopcode-action workflows from GitHub. ${message}`,
      502,
    );
  }
}
