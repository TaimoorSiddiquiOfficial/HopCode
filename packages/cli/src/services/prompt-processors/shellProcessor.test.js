...
describe('ShellProcessor', () => {
    ...
    it('should process a single valid shell injection if allowed', async () => {
        ...
        const result = await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('git status', expect.any(Object), context.session.sessionShellAllowlist, expect.any(String));
    ...
    it('should NOT throw ConfirmationRequiredError if a command is not allowed but approval mode is IZN', async () => {
        ...
        mockConfig.getApprovalMode.mockReturnValue(ApprovalMode.IZN);
        mockShellExecute.mockReturnValue({
            result: Promise.resolve({ ...SUCCESS_RESULT, output: 'deleted' }),
        });
        const result = await processor.process(prompt, context);
    ...
    it('should execute all commands if they are on the session allowlist', async () => {
        ...
        const result = await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('cmd1', expect.any(Object), context.session.sessionShellAllowlist, expect.any(String));
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('cmd2', expect.any(Object), context.session.sessionShellAllowlist, expect.any(String));
        expect(mockShellExecute).toHaveBeenCalledTimes(2);
    ...
    it('should trim whitespace from the command inside the injection before interpolation', async () => {
        ...
        await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith(expectedCommand, expect.any(Object), context.session.sessionShellAllowlist, expect.any(String));
    ...
    describe('Context-Aware Argument Interpolation ({{args}})', () => {
        ...
        it('should perform security checks on the final, resolved (escaped) command', async () => {
            ...
            await expect(processor.process(prompt, context)).rejects.toThrow(ConfirmationRequiredError);
            expect(mockCheckCommandPermissions).toHaveBeenCalledWith(expectedResolvedCommand, expect.any(Object), context.session.sessionShellAllowlist, expect.any(String));
        });
