 ...
 */
import { ApprovalMode, checkCommandPermissions, escapeShellArg, getShellConfiguration, ShellExecutionService, flatMapTextParts, checkArgumentSafety, } from '@hoptrendy/hopcode-core';
...
export class ShellProcessor {
    ...
    async processString(prompt, context) {
        ...
        for (const injection of resolvedInjections) {
            ...
            const { allAllowed, disallowedCommands, blockReason, isHardDenial } = await checkCommandPermissions(command, config, sessionShellAllowlist, shellConfig.shell);
