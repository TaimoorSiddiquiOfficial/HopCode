...
export const MISSING_TOOL_RESULT_MESSAGE = 'Tool result missing from saved history; the previous run likely ended ' +
...
export class HistoryReplayer {
    ...
    messageEmitter;
    toolCallEmitter;
    pendingReplayToolCalls = new Map();
    constructor(ctx) {
        this.ctx = ctx;
    ...
    async replayRecord(record) {
        ...
        try {
            switch (record.type) {
                case 'user':
                    ...
                    if (record.subtype === 'mid_turn_user_message') {
                        ...
                        if (displayText) {
                            ...
                            await this.replayContent(record.message, 'user', record.timestamp, record.uuid);
                        }
                        break;
                    }
                    if (record.message) {
                ...
                case 'assistant':
                    if (record.message) {
                        await this.replayContent(record.message, 'assistant', record.timestamp, record.uuid);
                    }
     ...
     */
    async replayContent(content, role, timestamp, recordId) {
