...
export async function collectHistoryReplayUpdates({ sessionId, config, records, gaps, cumulativeUsage, logger, supersedeUnrestorableGoal, }) {
    const updates = [];
    try {
        await new HistoryReplayer(replayContext(sessionId, updates, cumulativeUsage, config), { supersedeUnrestorableGoal }).replay(records, gaps);
    }
    catch (error) {
...
export async function replayTranscriptRecordPage({ sessionId, page, config, encodeCursor, logger, }) {
