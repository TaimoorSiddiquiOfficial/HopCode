 ...
 */
import { AgentEventType, ToolConfirmationOutcome, createDebugLogger, } from '@hoptrendy/hopcode-core';
import { z } from 'zod';
...
export class SubAgentTracker {
    ...
    createUsageMetadataHandler(abortSignal) {
        return (...args) => {
            ...
            if (abortSignal.aborted)
                return;
            this.messageEmitter.emitUsageMetadata(event.usage, '', event.durationMs, this.subagentMeta);
        };
    }
    /**
    ...
    createStreamTextHandler(abortSignal) {
        return (...args) => {
            ...
            void this.messageEmitter.emitMessage(event.text, 'assistant', event.thought ?? false, undefined, this.subagentMeta);
        };
    }
}
