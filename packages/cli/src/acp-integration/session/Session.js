...
import { AuthType, ApprovalMode, CompressionStatus, detectLoopSentinel, detectAutonomousSentinel, LoopTickResolver, parsePositiveIntegerEnv, convertToFunctionErrorResponse, convertToFunctionResponse, createDuplicateProviderToolCallResponse, findRepeatedDuplicateProviderToolCall, markDuplicateProviderToolCallResponseSent, createDebugLogger, DiscoveredMCPTool, StreamEventType, ToolConfirmationOutcome, generatePromptSuggestion, logPromptSuggestion, logToolCall, logUserPrompt, PromptSuggestionEvent, getErrorStatus, UserPromptEvent, readManyFiles, getSpecificMimeType, clampInlineMediaPart, Storage, Kind, ToolNames, ToolErrorType, fireNotificationHook, firePermissionRequestHook, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, buildContextUsage, injectPermissionRulesIfMissing, NotificationType, persistPermissionOutcome, createHookOutput, generateToolUseId, MessageBusType, MessageDisplayDispatcher, getPlanModeSystemReminder, getArenaSystemReminder, getStartupContextLength, isSystemReminderContent, buildSessionRecoveryPlanFromApiHistory, TURN_INTERRUPTION_HISTORY_TAIL_COUNT, evaluatePermissionFlow, getEffectivePermissionForConfirmation, needsConfirmation, isPlanModeBlocked, abortGoalForStopHookCap, formatStopHookBlockingCapWarning, applyAutoModeDecision, evaluateAutoMode, getAutoModePermissionDeniedReason, isApproveOutcome, isDenialFallbackReason, MAX_TRANSCRIPT_MESSAGES, formatDenialStateLog, recordAllow, recordFallbackApprove, shouldFallback, shouldClassifyAllShellForAutoMode, shouldForceAutoModeReviewForAllow, shouldFirePermissionDeniedForAutoMode, shouldRunAutoModeForCall, extractDaemonTraceContext, withInteractionSpan, startToolSpan, endToolSpan, runInToolSpanContext, startToolExecutionSpan, endToolExecutionSpan, isShellProgressData, logConversationFinishedEvent, ConversationFinishedEvent, logLoopDetected, LoopDetectedEvent, LoopType, acquireSleepInhibitor, refreshMemoryAfterManagedWrite, clearGoalTerminalObserver, setGoalTerminalObserver, sessionIdContext, dedupeToolCallsById, getProviderToolCallId, DEFAULT_TOKEN_LIMIT, hasImageParts, normalizeParts, runVisionBridge, shouldRunVisionBridge, formatVisionBridgeNotice, formatFullTurnVisionNotice, getFullTurnVisionModelSelector, splitImageParts, approxBase64Bytes, runWithRuntimeContentGenerator, } from '@hoptrendy/hopcode-core';
...
import { isTerminalGoalStatusKind, MessageType, } from '../../ui/types.js';
import { extractAtPathCommands } from '../../ui/hooks/atCommandProcessor.js';
import { goalTerminalEventToHistoryItem, recordGoalStatusItem, } from '../../ui/utils/restoreGoal.js';
...
import { buildPermissionRequestContent, interactionMetaFields, toPermissionOptions, } from './permissionUtils.js';
...
function recordDaemonToolCalls(config, promptId, loopState, count) {
    ...
    // equivalent.
    if (loopState.totalToolCalls <= config.getMaxToolCallsPerTurn())
        return false;
...
export function isExistingFile(resolved, fileExists = existsSync, statFile = statSync) {
    try {
...
export class Session {
    ...
    #notificationContinuesTodoStopGuardWorkChain(item) {
        ...
        if (item.kind === 'agent')
            return !baseline.agents.has(item.taskId);
        if (item.kind === 'shell')
            return !baseline.shells.has(item.taskId);
    ...
    #hasRelevantTodoStopGuardBackgroundInput() {
        ...
        const baseline = this.todoStopGuardBackgroundBaseline;
        const agents = this.config.getBackgroundTaskRegistry?.()?.getAll?.() ?? [];
        if (agents.some((task) => !baseline.agents.has(task.id) &&
            task.isBackgrounded &&
            (task.status === 'running' ||
                task.status === 'paused' ||
        ...
        const monitors = this.config.getMonitorRegistry?.()?.getAll?.() ?? [];
        if (monitors.some((task) => !baseline.monitors.has(task.id) && task.status === 'running')) {
            return true;
        }
    ...
    installGoalTerminalObserver() {
        setGoalTerminalObserver(this.sessionId, (event) => {
            void this.messageEmitter.emitGoalTerminal(event).catch((error) => {
                debugLogger.warn(`Failed to emit goal terminal update: ${this.#formatError(error)}`);
            });
    ...
    emitGoalStatus(status) {
        void this.messageEmitter.emitGoalStatus(status).catch((error) => {
            debugLogger.warn(`Failed to emit goal status update: ${this.#formatError(error)}`);
        });
        ...
        void (async () => {
            try {
                ...
                if (ac.signal.aborted)
                    return;
                if (r.suggestion) {
                    await this.client.extNotification('hopcode/notify/session/prompt-suggestion', {
                        v: 1,
                        sessionId: this.sessionId,
    ...
    async #executePrompt(params, pendingSend) {
        ...
        return sessionIdContext.run(this.config.getSessionId(), () => this.#executePromptInner(params, pendingSend));
    }
    async #executePromptInner(params, pendingSend) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            return await withInteractionSpan(this.config, {
                ...
                if (isContinue) {
                    ...
                    parts = await this.#processSlashCommandResult(slashCommandResult, params.prompt, pendingSend.signal, onFullTurnModel);
                    ...
                    parts = await this.#resolvePrompt(params.prompt, pendingSend.signal, { promptLast: true, onFullTurnModel });
                }
                ...
                try {
                    while (nextMessage !== null) {
                        ...
                        if (functionCalls.length > 0) {
                            const toolRun = await this.#runWithFullTurnModel(fullTurnModelOverride, () => this.runToolCalls(pendingSend.signal, promptId, functionCalls, toolLoopState));
                            if (toolRun.stopAfterPermissionCancel) {
                                this.todoStopGuard.suspend();
                                await this.#preserveStoppedToolRun(toolRun, pendingSend.signal);
                                return { stopReason: 'end_turn' };
                            }
                            const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, onFullTurnModel);
                    ...
                    return await this.#handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, true, fullTurnModelOverride);
                }
                finally {
    ...
    async #handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, allowExternalHooks = true, modelOverride) {
        ...
        while (true) {
            ...
            const guardDecision = queuedPromptArrivedDuringStopHook
                ? null
                : this.todoStopGuard.decide(this.todoStopGuard.needsStopInspection
    ...
    async #runStopContinuation(pendingSend, streamPromptId, toolPromptId, parts, skipCompression, options = {}) {
        ...
        while (nextMessage !== null) {
                ...
                const sendResult = await this.#sendMessageStreamWithAutoCompression(promptIdForSend, nextMessage.parts ?? [], pendingSend.signal, {
                    ...
                    beforeSend: guardForThisSend ||
                        ...
                        ? async ({ compressionFailed }) => {
                            ...
                            const selectedMessage = guardForThisSend || !externalParts
                                ? (nextMessage?.parts ?? [])
                                : externalParts;
                            messageForPreservation = {
                                role: 'user',
            ...
            if (functionCalls.length > 0) {
                const toolRun = await this.#runWithFullTurnModel(options.getModelOverride?.(), () => this.runToolCalls(pendingSend.signal, toolPromptId, functionCalls, toolLoopState));
                ...
                const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, options.onFullTurnModel);
                nextMessage = nextAfterTools.message;
    ...
    async #sendMessageStreamWithAutoCompression(promptId, message, abortSignal, options = {}) {
        ...
        const responseStream = await this.#getCurrentChat().sendMessageStream(options.getModelOverride?.() ??
    ...
    async #drainMidTurnInput(abortSignal, options = {}) {
        ...
        if (this.midTurnDrainUnavailable) {
            return {
                parts: await this.#buildMidTurnParts(recovered, abortSignal, options.onFullTurnModel),
        ...
        try {
            ...
            return {
                parts: await this.#buildMidTurnParts([...recovered, ...parseMidTurnDrainResponse(response)], abortSignal, options.onFullTurnModel),
    ...
    async #startCronSchedulerIfNeeded() {
        ...
        if ((this.todoStopGuard.blocksUnrelatedAutomaticTurns ||
            ...
            const unrelatedIndices = this.cronQueue
                .map((queued, index) => this.#cronContinuesTodoStopGuardWorkChain(queued) ? -1 : index)
            ...
            if (unrelatedIndices.length >= MAX_DEFERRED_UNRELATED_CRON_QUEUE) {
                ...
                debugLogger.warn(`Cron queue overflow while automatic work is deferred: evicting task=${evicted?.taskId ?? 'unknown'}`);
            }
        }
    ...
    async #executeCronPrompt(item) {
        ...
        return sessionIdContext.run(this.config.getSessionId(), () => this.#executeCronPromptInner(item));
    }
    async #executeCronPromptInner(item) {
        ...
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            this.#prepareTodoStopGuardForAutomaticTurn(this.#cronContinuesTodoStopGuardWorkChain(item));
                ...
                try {
                    ...
                    while (nextMessage !== null) {
                        ...
                        if (functionCalls.length > 0) {
                            ...
                            const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, ac.signal);
                            nextMessage = nextAfterTools.message;
                            if (toolRun.loopDetected) {
                                this.todoStopGuard.suspend();
                                await this.#preserveStoppedToolRun(toolRun, ac.signal);
                    ...
                    if (this.todoStopGuard.needsStopInspection) {
                        const guardStop = await this.#handleStopHookLoop(ac, promptId, false, undefined, false);
    ...
    async #drainNotificationQueue() {
        ...
        try {
            while (this.notificationQueue.length > 0) {
                ...
                if (!item)
                    break;
                await sessionIdContext.run(this.config.getSessionId(), () => this.#executeBackgroundNotificationPromptInner(item));
            }
        }
        finally {
            this.notificationProcessing = false;
    ...
    async #executeBackgroundNotificationPromptInner(item) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            this.#prepareTodoStopGuardForAutomaticTurn(this.#notificationContinuesTodoStopGuardWorkChain(item));
    ...
    async runToolCalls(abortSignal, promptId, functionCalls, toolLoopState) {
        ...
        const recordSkippedToolCall = async (fc, message = PERMISSION_CANCEL_SKIP_MESSAGE, emitStart = true) => {
        ...
        if (recordDaemonToolCalls(this.config, promptId, toolLoopState, dedupedFunctionCalls.length)) {
            return {
                parts: await Promise.all(dedupedFunctionCalls.map((fc) => recordSkippedToolCall(fc, LOOP_DETECTED_SKIP_MESSAGE, false))),
        ...
        const pushDuplicateBatch = (request) => {
        ...
        const appendSkippedAfter = async (parts, fc, message = PERMISSION_CANCEL_SKIP_MESSAGE) => {
        ...
        const runBounded = async (calls, runAbortSignal, onStopAfterPermissionCancel, onStopAfterLoopDetected, shouldSkipUnstarted) => {
            let maxConcurrency = parsePositiveIntegerEnv(process.env['HOPCODE_CODE_MAX_TOOL_CONCURRENCY'], 10);
            maxConcurrency = toolLoopState
                ? Math.min(maxConcurrency, DAEMON_INVALID_TOOL_PARAMS_THRESHOLD)
                : maxConcurrency;
            ...
            const fillPermissionSkippedFrom = async (startIndex) => {
                for (let i = startIndex; i < calls.length; i++) {
                    if (results[i])
                        continue;
                    results[i] = {
            ...
            if (toolLoopState &&
                ...
                for (let i = 0; i < startIndex; i++) {
                    ...
                    const r = await this.runTool(runAbortSignal, promptId, calls[i], onStopAfterPermissionCancel, toolLoopState, recordSkippedToolCall);
                    results[i] = r;
                    if (r.loopDetected) {
            ...
            for (let i = startIndex; i < calls.length; i++) {
                ...
                const p = this.runTool(runAbortSignal, promptId, calls[idx], onStopAfterPermissionCancel, toolLoopState, recordSkippedToolCall)
                    .then((r) => {
                    results[idx] = r;
                })
                    .finally(() => {
        ...
        try {
            for (const batch of batches) {
                ...
                if (batch.concurrent && batch.calls.length > 1) {
                    ...
                    let results;
                    try {
                        results = await runBounded(batch.calls, batchAbortController.signal, stopBatchAfterPermissionCancel, () => batchAbortController.abort('loop_detected'), () => batchStopAfterPermissionCancel);
                    }
                    finally {
                    ...
                    if (shouldStopForLoop) {
                        await appendSkippedAfter(parts, batch.calls[batch.calls.length - 1], LOOP_DETECTED_SKIP_MESSAGE);
                        return {
                            parts,
                            stopAfterPermissionCancel: false,
                            loopDetected: true,
                    ...
                    for (const fc of batch.calls) {
                        const r = await this.runTool(abortSignal, promptId, fc, undefined, toolLoopState, recordSkippedToolCall);
                        parts.push(...r.parts);
                        collectMemoryWriteCandidates(r);
                        if (r.loopDetected) {
    ...
    async runTool(abortSignal, promptId, fc, onStopAfterPermissionCancel, toolLoopState, recordSkippedToolCall) {
        ...
        if (!tool) {
            return earlyErrorResponse(new Error(`Tool "${toolName}" not found in registry.`), toolName, { recordInvalidToolParams: true });
        }
        ...
        try {
            return await runInToolSpanContext(toolSpan, async () => {
                ...
                try {
                    ...
                    const flowResult = await evaluatePermissionFlow(this.config, invocation, toolName, toolParams);
                    ...
                    if (finalPermission === 'allow' && forceAutoReviewForAllow) {
                        debugLogger.info(`Auto mode: L4 allow overridden by protected-write guard for ${toolName}`);
                    }
                    let autoModeAllowed = finalPermission === 'allow' && !forceAutoReviewForAllow;
                    ...
                    if (!autoModeAllowed &&
                        ...
                        await fireSessionPermissionDeniedForAutoMode(this.config, decision, outcome, toolName, toolParams, callId, abortSignal);
                        switch (outcome.kind) {
                            case 'approved':
                                autoModeAllowed = true;
                                break;
                            case 'blocked':
                                debugLogger.warn(`Auto mode blocked (${outcome.reason}): tool=${toolName}, ` +
                    ...
                    if (!autoModeAllowed &&
                        needsConfirmation(confirmationPermission, approvalMode, toolName, requiresUserInteraction)) {
                        confirmationDetails =
                            await invocation.getConfirmationDetails(abortSignal);
                        // Centralised rule injection (for display and persistence)
                        injectPermissionRulesIfMissing(confirmationDetails, pmCtx);
                        if (isPlanModeBlocked(isPlanMode, isExitPlanModeTool, isAskUserQuestionTool, confirmationDetails, isEnterPlanModeTool)) {
                        ...
                        if (hooksEnabled && messageBus) {
                            const hookResult = await firePermissionRequestHook(messageBus, toolName, args, String(approvalMode));
                        ...
                        if (!requiresUserInteraction &&
                            ...
                            const mappedKind = this.toolCallEmitter.mapToolKind(tool.kind, toolName);
                            if (hooksEnabled && messageBus) {
                                this.fireNotificationHookWithTerminalSequence(messageBus, `HopCode needs your permission to use ${toolName}`, NotificationType.PermissionPrompt, 'Permission needed');
                            }
                            const params = {
                                sessionId: this.sessionId,
                            ...
                            const stopAfterPermissionCancel = () => {
                                onStopAfterPermissionCancel?.();
                                return earlyErrorResponse(new Error(`Tool "${toolName}" was canceled by the user.`), toolName, { stopAfterPermissionCancel: true });
                            };
                            let output;
                            let outcome;
                            try {
                                output = (await this.client.requestPermission(params));
                                outcome =
                                ...
                                return earlyErrorResponse(new Error(permissionFailureMessage), toolName, { stopAfterPermissionCancel: true });
                            }
                    ...
                    if (hooksEnabledForTool && messageBusForTool) {
                        const preHookResult = await firePreToolUseHook(messageBusForTool, toolName, args, toolUseId, permissionMode, activeToolAbortSignal, callId);
                    ...
                    const heartbeatSpanAttributes = () => heartbeatCount > 0
                        ...
                        : undefined;
                    try {
                        const sleepInhibitorHandle = acquireSleepInhibitor(this.config, `HopCode is executing tool ${toolName}`);
                        try {
                    ...
                    if (hooksEnabledForTool &&
                        ...
                        const postHookResult = await firePostToolUseHook(messageBusForTool, toolName, args, toolResponse, toolUseId, permissionMode, activeToolAbortSignal, callId);
                        ...
                        const failureHookResult = await firePostToolUseFailureHook(messageBusForTool, toolUseId, toolName, args, toolResult.error?.message ?? 'Tool execution was cancelled', isInterrupt, permissionMode, activeToolAbortSignal, callId);
                    ...
                    if (hooksEnabledForError && messageBusForError) {
                        const failureHookResult = await firePostToolUseFailureHook(messageBusForError, toolUseId, toolName, args, error.message, isInterrupt, String(approvalMode), activeToolAbortSignal, callId);
                        // Log additional context if provided
                        if (failureHookResult.additionalContext) {
                            debugLogger.debug(`PostToolUseFailure hook additional context for ${toolName}: ${failureHookResult.additionalContext}`);
                        }
                    ...
                    const loopDetected = !activeToolAbortSignal.aborted &&
                        !toolBuildSucceeded &&
                        recordDaemonInvalidToolParams(this.config, promptId, toolLoopState, toolName, error);
    ...
    options = {}) {
        ...
        const textPathSpecsToRead = new Set();
        ...
        if (pathSpecsToRead.length === 0 &&
            ...
            return this.#applyBridgeConversionsIfNeeded(parts, abortSignal, options.onFullTurnModel);
        }
        if (pathSpecsToRead.length === 0 && embeddedContext.length === 0) {
            return this.#applyBridgeConversionsIfNeeded([...parts, ...extensionParts, ...mcpServerParts], abortSignal, options.onFullTurnModel);
        }
    ...
    fireNotificationHookWithTerminalSequence(messageBus, message, notificationType, title) {
        void fireNotificationHook(messageBus, message, notificationType, title)
            ...
            if (!hookResult.terminalSequence)
                return;
            return this.client.extNotification('hopcode/notify/session/terminal-sequence', {
                v: 1,
                sessionId: this.sessionId,
