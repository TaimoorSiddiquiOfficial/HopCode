 ...
 */
import { APPROVAL_MODE_INFO, APPROVAL_MODES, AuthType, BTW_MAX_INPUT_LENGTH, buildBtwCacheSafeParams, buildBtwPrompt, ALL_PROVIDERS, applyProviderInstallPlan, buildInstallPlan, clearCachedCredentialFile, createDebugLogger, generateSessionRecap, findProviderById, getAllGeminiMdFilenames, getAutoMemoryRoot, getUserAutoMemoryRoot, getDefaultBaseUrlForProtocol, getDefaultModelIds, getScopedEnvContents, HopCodeOAuth2Event, HopCodeOAuth2Events, resolveBaseUrl, MCP_BUDGET_WARN_FRACTION, MCPServerConfig, runForkedAgent, SessionService, SESSION_TITLE_MAX_LENGTH, Storage, tokenLimit, getMCPDiscoveryState, getMCPServerStatus, MCPDiscoveryState, MCPServerStatus, McpTransportPool, POOLED_TRANSPORTS_DEFAULT, findExistingProviderModels, ExtensionManager, ExtensionSettingScope, HookEventName, updateSetting, SessionEndReason, WorkspaceMcpBudget, DiscoveredMCPTool, restoreWorktreeContext, uiTelemetryService, McpBudgetWouldExceedError, McpServerSpawnFailedError, InvalidMcpConfigError, isGatedMcpScope, MCPOAuthProvider, MCPOAuthTokenStorage, InvalidSessionTranscriptCursorError, SESSION_TRANSCRIPT_MAX_LIMIT, SESSION_TRANSCRIPT_MAX_PAGE_BYTES, SessionTranscriptReader, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, encodeSessionTranscriptCursor, subagentGenerator, redactUrlCredentials, computeUniqueBranchTitle, getActiveGoal, unregisterGoalHook, ToolNames, FORK_SUBAGENT_TYPE, runManagedAutoMemoryDream, runManagedRememberByAgent, matchesAnyServerPattern, mcpServerRequiresOAuth, IMAGE_CAPABILITY, registerAcpEventLoopLagGauge, SESSION_ARTIFACT_PERSISTENCE_VERSION, normalizeEventPayload, normalizeSnapshotPayload, startEventLoopLagMonitor, refreshMemoryInstruction, extractDaemonTraceContext, withDaemonSpan, ApprovalMode, } from '@hoptrendy/hopcode-core';
...
import { z } from 'zod';
import { buildDisabledSkillNamesProvider, loadCliConfig, } from '../config/config.js';
import { createWorkspaceMemoryExtractionErrorLogger, shouldSuppressRememberErrorDetails, workspaceMemoryFailureCode, workspaceMemoryFailureDiagnostics, } from '../serve/workspace-remember-errors.js';
...
import { ACP_PREFLIGHT_KINDS, STATUS_SCHEMA_VERSION, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, mapDomainErrorToErrorKind, IDLE_HOOK_EVENTS, } from '@hoptrendy/acp-bridge/status';
import { parseSessionSource } from '@hoptrendy/acp-bridge';
import { CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, CLIENT_MCP_OVER_WS_CONFIG_FLAG, LOAD_REPLAY_BULK_MODE, LOAD_REPLAY_META_KEY, LOAD_REPLAY_MODE_META_KEY, LOAD_REPLAY_PAGE_SIZE_META_KEY, LOAD_REPLAY_VERSION, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, } from '@hoptrendy/acp-bridge/bridgeTypes';
...
import { collectGoalStatusItemsFromRecords, restoreGoalFromHistory, } from '../ui/utils/restoreGoal.js';
...
const HOPCODE_CORE_SETTING_KEYS = Object.keys(HOPCODE_CORE_SETTING_DEFINITIONS);
const HOPCODE_HOOK_EVENTS = Object.values(HookEventName);
const DEFAULT_HOPCODE_MEMORY_SETTINGS = {
...
const HOPCODE_MEMORY_SETTING_KEYS = [
...
function isHookEvent(value) {
    return (typeof value === 'string' &&
        HOPCODE_HOOK_EVENTS.includes(value));
}
...
async function resolveQwenMemoryPaths(params) {
    const fallbackFilename = getAllGeminiMdFilenames()[0] ?? 'HOPCODE.md';
    const userMemoryFile = await resolvePreferredMemoryFile(Storage.getGlobalHopCodeDir(), fallbackFilename);
...
export async function runAcpAgent(config, settings, argv) {
    ...
    const drainPoolBeforeExit = async (label) => {
        if (!agentInstance)
            return;
        try {
...
/**
 * Parse `HOPCODE_SERVE_MCP_POOL_DRAIN_MS` env var. Default 30000ms.
...
function parsePoolDrainMs(envValue) {
    ...
    if (!/^\d+$/.test(trimmed)) {
        process.stderr.write(`hopcode serve: HOPCODE_SERVE_MCP_POOL_DRAIN_MS=${JSON.stringify(envValue)} ` +
...
export function createWorkspaceMcpBudget(onEvent) {
    ...
    if (rawBudget !== undefined && rawBudget !== '') {
        ...
        if (/^\d+$/.test(trimmed) && Number.isSafeInteger(parsed) && parsed > 0) {
            ...
            process.stderr.write(`hopcode serve: ignoring invalid HOPCODE_SERVE_MCP_CLIENT_BUDGET=` +
...
class QwenAgent {
        ...
        return this.enqueueWorkspaceMcpDiscovery('reload', async () => {
            ...
            for (const config of liveConfigs) {
                try {
                    ...
                    const mcpServers = assembleMcpServers(settings.merged.mcpServers, cwd, config.getTopTierMcpServers());
                    const gating = recomputeMcpGating(settings, mcpServers, cwd, config.getCliAllowedMcpServerNames(), config.getApprovalMode() === ApprovalMode.YOLO);
    ...
    async closeStoredSession(sessionId, opts) {
        ...
        const flushRecording = async () => {
            try {
                ...
                debugLogger.debug(`Session ${sessionId} chat recording flush during close failed: ${err instanceof Error ? err.message : String(err)}`);
                return err;
            }
        };
        if (requireFlush) {
        ...
        try {
            ...
            debugLogger.debug(`Session ${sessionId} tool registry stop during close failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    ...
    discardStoredSessionIfCurrent(sessionId, session) {
        if (this.sessions.get(sessionId) !== session) {
            return;
        }
        const logCleanupFailure = (action, err) => {
            debugLogger.debug(`Session ${sessionId} ${action} during failed restore cleanup failed: ${err instanceof Error ? err.message : String(err)}`);
        };
        try {
            session.dispose();
        }
    ...
    constructor(config, settings, argv, connection) {
        ...
        if (process.env['QWEN_SERVE_NO_MCP_POOL'] === '1') {
            ...
            this.mcpPool = new McpTransportPool(this.config, {
                ...
                drainDelayMs: parsePoolDrainMs(process.env['HOPCODE_SERVE_MCP_POOL_DRAIN_MS']),
    ...
    async newSession(params) {
        ...
        return await withDaemonSpan('qwen-code.daemon.session_start', { 'qwen-code.daemon.operation': 'acp_session_new' }, async (span) => {
            ...
            this.settings = settings;
            const config = await profiler.time('config_setup', () => this.newSessionConfig(cwd, mcpServers, settings));
            await profiler.time('auth', () => this.ensureAuthenticated(config));
            profiler.timeSync('file_system_setup', () => this.setupFileSystem(config));
            const session = await profiler.time('session_register', () => this.createAndStoreSession(config, settings));
    ...
    async loadSession(params) {
        ...
        const exists = await runWithAcpRuntimeOutputDir(settings, params.cwd, async () => {
        ...
        params.mcpServers ?? [], settings, params.sessionId, true);
        ...
        const replayPageSize = bulkReplay
            ...
            : undefined;
        const session = await this.createAndStoreSession(config, settings, sessionData, bulkReplay
        ...
        if (bulkReplay) {
            try {
                ...
                if (records) {
                    ...
                    const replayPage = selectRecentHistoryRecords(records, replayPageSize);
    ...
    async unstable_resumeSession(params) {
        ...
        this.settings = settings;
        const config = await this.newSessionConfig(params.cwd, params.mcpServers ?? [], settings, params.sessionId, true);
        await this.ensureAuthenticated(config);
        this.setupFileSystem(config);
        const session = await this.createAndStoreSession(config, settings, config.getResumedSessionData(), { replayHistory: false });
        await this.#restoreWorktreeOnResume(config, session);
        this.#restoreGoalOnResume(config, session);
        const modesData = this.buildModesData(config);
        const availableModels = this.buildAvailableModels(config);
    ...
    #restoreGoalOnResume(config, session) {
        try {
            ...
            if (!records?.length)
                return;
            const restored = restoreGoalFromHistory(collectGoalStatusItemsFromRecords(records), config);
            if (restored.restored) {
                debugLogger.info(`ACP goal restored sessionId=${config.getSessionId()} condition=${restored.condition}`);
            }
                ...
                writeStderrLineSafe(`qwen: not restoring the active goal for session ${config.getSessionId()} (${restored.blockedBy}).`);
            }
        }
        catch (error) {
            ...
            writeStderrLineSafe(`qwen: goal restore failed for session ${config.getSessionId()}: ${error}`);
        }
        finally {
    ...
    async buildWorkspaceMcpStatus(config) {
        try {
            ...
            try {
                ...
                process.stderr.write(`hopcode serve: pool snapshot for workspace MCP status failed: ` +
                    `${err instanceof Error ? err.message : String(err)}\n`);
            }
            ...
            if (wsBudget !== undefined) {
                ...
                try {
                    ...
                    process.stderr.write(`hopcode serve: getMcpClientAccounting failed: ` +
                        `${err instanceof Error ? err.message : String(err)}\n`);
                }
            }
    ...
    async installSkillFromUrl(request) {
        ...
        const skillsBaseDir = path.join(Storage.getGlobalHopCodeDir(), 'skills');
    ...
    async deleteGlobalSkill(request) {
        ...
        const globalDir = path.resolve(Storage.getGlobalHopCodeDir());
    ...
    async readManagedSkillFile(slug, scope, skillManager, cwd) {
        if (scope === 'global') {
            const hopcodeSkillDir = resolveManagedSkillDir(path.join(Storage.getGlobalHopCodeDir(), 'skills'), slug);
    ...
    async extMethod(method, params) {
        ...
        switch (method) {
            ...
            case 'hopcode/settings/setMemory': {
                ...
                const settings = loadSettings(cwd);
                for (const key of HOPCODE_MEMORY_SETTING_KEYS) {
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart: {
                ...
                try {
                    ...
                    if (mergedDisabled !== undefined && !Array.isArray(mergedDisabled)) {
                        process.stderr.write(`hopcode serve: MCP restart for ${JSON.stringify(serverName)}: ` +
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGoalClear: {
                ...
                debugLogger.info(`sessionGoalClear sessionId=${sessionId} cleared=${!!cleared} condition=${cleared?.condition ?? '(none)'}`);
                return {
                    cleared: !!cleared,
                    condition: cleared?.condition,
                };
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeAdd: {
                ...
                const runtimeClientId = typeof originatorClientId === 'string' &&
                ...
                try {
                    ...
                    const { trust: _trust, authProviderType: _auth, includeTools: _inc, excludeTools: _exc, cwd: _cwd, env: _env, oauth: _oauth, headers: _headers, type: _type, 
                    ...
                    [CLIENT_MCP_OVER_WS_CONFIG_FLAG]: clientMcpOverWs, ...safeConfig } = config;
                    ...
                    const result = await manager.addRuntimeMcpServer(name, safeConfig, runtimeClientId);
                    ...
                    await Promise.all(this.getActiveSessions().map(async (session) => {
                        ...
                        try {
                            await sessionManager.addRuntimeMcpServer(name, safeConfig, runtimeClientId);
                        }
                        catch (sessionErr) {
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeRemove: {
                ...
                const result = await manager.removeRuntimeMcpServer(name, runtimeClientId);
                ...
                await Promise.all(this.getActiveSessions().map(async (session) => {
                    ...
                    if (!sessionManager)
                        return;
                    try {
                        await sessionManager.removeRuntimeMcpServer(name, runtimeClientId);
                    }
                    catch (sessionErr) {
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionRewind: {
                ...
                try {
                    ...
                    const sessionData = await runWithAcpRuntimeOutputDir(this.settings, cwd, async () => {
                        const sessionService = new SessionService(cwd);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionBranch: {
                ...
                return await runWithAcpRuntimeOutputDir(this.settings, cwd, async () => {
                    const sessionService = new SessionService(cwd);
                    ...
                    try {
                        ...
                        sessionService.removeSession(newSessionId).catch((rmErr) => {
                            process.stderr.write(`hopcode serve: failed to clean up orphan session ${newSessionId}: ${rmErr instanceof Error ? rmErr.message : rmErr}\n`);
                        });
                        throw err;
                    }
            ...
            case 'hopcode/settings/setCoreValue': {
                ...
                if (typeof key !== 'string' ||
                    !HOPCODE_CORE_SETTING_KEYS.includes(key)) {
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh: {
                ...
                const results = await Promise.allSettled(sessions.map((session) => session.refreshSkillsFromSettings()));
                for (let i = 0; i < results.length; i++) {
                    if (results[i].status === 'rejected') {
                        const reason = results[i].reason;
    ...
    async newSessionConfig(cwd, mcpServers, settings, sessionId, resume, initializeOptions = {}) {
        ...
        for (const server of mcpServers) {
            ...
            if (stdioServer) {
                ...
                sessionMcpServers[stdioServer.name] = new MCPServerConfig(stdioServer.command, stdioServer.args, env, cwd);
                continue;
            }
