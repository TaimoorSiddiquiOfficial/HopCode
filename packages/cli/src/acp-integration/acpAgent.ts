...
interface HopCodePermissionSettings {
...
type HopCodeSettingsScope = 'user' | 'workspace';
type HopCodeSettingValue = string | number | boolean | string[] | undefined;
type HopCodeMcpTransport = 'stdio' | 'http' | 'sse';
...
const DEFAULT_HOPCODE_MEMORY_SETTINGS: HopCodeMemorySettings = {
...
const HOPCODE_MEMORY_SETTING_KEYS = [
  ...
  'enableAutoSkill',
] as const satisfies ReadonlyArray<keyof HopCodeMemorySettings>;

function normalizeHopCodeMemorySettings(value: unknown): HopCodeMemorySettings {
...
function getNestedSettingValue(
  ...
  if (
    ...
    return current as HopCodeSettingValue;
  }
  return undefined;
}
...
async function resolveHopCodeMemoryPaths(params: {
  cwd: string;
  projectRoot: string;
...
/**
 * Parse `HOPCODE_SERVE_MCP_POOL_TRANSPORTS` env var. Comma-separated list
...
function parsePoolDrainMs(envValue: string | undefined): number {
  ...
  if (!/^\d+$/.test(trimmed)) {
    process.stderr.write(
      `hopcode serve: HOPCODE_SERVE_MCP_POOL_DRAIN_MS=${JSON.stringify(envValue)} ` +
...
class HopCodeAgent implements Agent {
  ...
  constructor(
    ...
    // roll back without rebuilding. `runHopCodeServe.ts` sets this when
  ...
  private async buildWorkspaceMcpStatus(
    ...
    try {
      ...
      try {
        ...
        process.stderr.write(
          `hopcode serve: pool snapshot for workspace MCP status failed: ` +
            `${err instanceof Error ? err.message : String(err)}\n`,
        );
      }
  ...
  private async readManagedSkillFile(
    ...
    cwd?: string,
  ): Promise<HopCodeManagedSkillFile> {
    if (scope === 'global') {
  ...
  private async findProjectSkillFileFromCwd(
    ...
    cwd: string,
    skillManager: NonNullable<ReturnType<Config['getSkillManager']>>,
  ): Promise<HopCodeManagedSkillFile | undefined> {
  ...
  async extMethod(
    ...
    switch (method) {
      ...
      case 'hopcode/settings/getMemory': {
        ...
        return {
          settings: normalizeHopCodeMemorySettings(settings.merged.memory),
        };
      }
      ...
      case 'hopcode/settings/getMemoryPaths': {
        const projectRoot =
          typeof params['projectRoot'] === 'string'
            ...
            : cwd;
        return {
          paths: await resolveHopCodeMemoryPaths({ cwd, projectRoot }),
        };
      }
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart: {
        ...
        try {
          ...
          if (mergedDisabled !== undefined && !Array.isArray(mergedDisabled)) {
            process.stderr.write(
              `hopcode serve: MCP restart for ${JSON.stringify(serverName)}: ` +
          ...
          async () => {
            ...
            try {
              ...
              sessionService.removeSession(newSessionId).catch((rmErr) => {
                process.stderr.write(
                  `hopcode serve: failed to clean up orphan session ${newSessionId}: ${rmErr instanceof Error ? rmErr.message : rmErr}\n`,
                );
              });
              throw err;
            }
