...
vi.mock('@hoptrendy/hopcode-core', () => ({
    ...
    findProviderById: vi.fn((id) => {
        ...
        if (id === 'custom-openai-compatible') {
            return {
                ...
                baseUrl: undefined,
                envKey: (protocol, baseUrl) => `HOPCODE_CUSTOM_API_KEY_${protocol}_${baseUrl.replace(/[^A-Za-z0-9]/g, '_')}`,
                models: undefined,
                modelsEditable: true,
                modelNamePrefix: '',
                uiGroup: 'third-party',
                ownsModel: (model) => typeof model.envKey === 'string' &&
                    model.envKey.startsWith('HOPCODE_CUSTOM_API_KEY_'),
            };
        }
        return undefined;
    }),
    ...
    getAutoMemoryRoot: vi.fn((projectRoot) => `${projectRoot}/.hopcode/memory`),
    HopCodeOAuth2Event: {},
...
vi.mock('../utils/languageUtils.js', () => ({
    ...
    writeOutputLanguageAndRegisterPath: vi.fn((_value, config) => {
        ...
        if (!p) {
            config?.setOutputLanguageFilePath('/mock/.hopcode/output-language.md');
        }
    }),
...
import { runAcpAgent, toStdioServer, toSseServer, toHttpServer, normalizeCoreSettingValue, extractFilesFromTarGz, fetchAllowedGitHub, createWorkspaceMcpBudget, deliverClientMcpMessage, } from './acpAgent.js';
import { gzipSync } from 'node:zlib';
import { SessionEndReason, MCPServerConfig, SessionService, MCPDiscoveryState, MCPServerStatus, getMCPDiscoveryState, getMCPServerStatus, tokenLimit, McpBudgetWouldExceedError, buildInstallPlan, applyProviderInstallPlan, Storage, SessionTranscriptReader, InvalidSessionTranscriptCursorError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, SessionTranscriptPageTooLargeError, encodeSessionTranscriptCursor, unregisterGoalHook, getActiveGoal, registerGoalHook, startEventLoopLagMonitor, registerAcpEventLoopLagGauge, SESSION_ARTIFACT_PERSISTENCE_VERSION, mcpServerRequiresOAuth, APPROVAL_MODES, } from '@hoptrendy/hopcode-core';
...
import { SERVE_STATUS_EXT_METHODS, SERVE_CONTROL_EXT_METHODS, } from '@hoptrendy/acp-bridge/status';
...
import { CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, } from '@hoptrendy/acp-bridge/bridgeTypes';
...
describe('runAcpAgent shutdown cleanup', () => {
    ...
    it('registers and disposes the event loop monitor on normal connection close', async () => {
        ...
        const agentPromise = runAcpAgent(mockConfig, mockSettings, mockArgv);
        await vi.waitFor(() => {
            expect(registerAcpEventLoopLagGauge).toHaveBeenCalledWith(expect.any(Function));
        });
...
describe('QwenAgent MCP SSE/HTTP support', () => {
    ...
    it('does not return discontinued qwen-oauth as the only ACP auth option', async () => {
        ...
        vi.mocked(innerConfig.refreshAuth).mockRejectedValue(new Error('hopcode-oauth token expired'));
        vi.mocked(loadSettings).mockReturnValue(makeSessionSettings());
        vi.mocked(loadCliConfig).mockResolvedValue(innerConfig);
    ...
    async function expectAcpLocalReadRoots(sessionId, expectedLocalReadRoots) {
    ...
    function makeCoreSettings(outputLanguage = 'English') {
        ...
        return {
            ...
            workspace: {
                path: '/work/.hopcode/settings.json',
    ...
    it('releases a Todo Stop Guard that yielded to a cancelled FIFO prompt', async () => {
        ...
        const { agent, agentPromise } = await bootAcpAgent();
        await agent.newSession({ cwd: '/tmp', mcpServers: [] });
        await expect(agent.extMethod(TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, { sessionId })).resolves.toEqual({ released: true });
        expect(lastSessionMock?.releaseTodoStopGuardQueuedPromptWait).toHaveBeenCalledOnce();
        mockConnectionState.resolve();
        await agentPromise;
    });
    ...
    it('extMethod preflight surfaces SkillError as parse_error errorKind', async () => {
        const skillError = new (await import('@hoptrendy/hopcode-core')).SkillError('bad frontmatter', 'PARSE_ERROR');
        mockConfig = {
            ...mockConfig,
            getTargetDir: vi.fn().mockReturnValue('/work/status'),
            getMcpServers: vi.fn().mockReturnValue({}),
    ...
    it('reports a null goal state when nothing is active', async () => {
        ...
        await agent.newSession({ cwd: '/tmp', mcpServers: [] });
        await expect(agent.extMethod(SERVE_CONTROL_EXT_METHODS.sessionGoalGet, { sessionId })).resolves.toEqual({ active: null });
        mockConnectionState.resolve();
        await agentPromise;
    });
    ...
    it('qwen/settings extension methods read and update user memory settings', async () => {
        ...
        await expect(agent.extMethod('qwen/settings/getMemoryPaths', {
            ...
            paths: {
                userMemoryFile: path.join('/tmp/qwen-global-test', 'HOPCODE.md'),
                projectMemoryFile: path.join('/tmp/qwen-memory-cwd-test', 'HOPCODE.md'),
                autoMemoryDir: '/tmp/qwen-memory-root-test/.hopcode/memory',
            },
        });
    ...
    it('qwen/providers/connect reuses the custom apiKey for the requested baseUrl only', async () => {
        const customEnvKey = (protocol, baseUrl) => `HOPCODE_CUSTOM_API_KEY_${protocol}_${baseUrl.replace(/[^A-Za-z0-9]/g, '_')}`;
    ...
    it('qwen/skills setEnabled resolves user and project skill files through ACP', async () => {
        ...
        const projectSkill = await writeSkill(tempProject, '.hopcode/skills', 'project-course');
        const refreshCache = vi.fn().mockResolvedValue(undefined);
    ...
    it('qwen/skills setEnabled resolves project skills from the ext method cwd', async () => {
        ...
        try {
            ...
            await expect(fs.readFile(skillFile, 'utf8')).resolves.toContain('disable-model-invocation: true');
            expect(loadSkillsFromDir).toHaveBeenCalledWith(path.join(tempProject, '.hopcode', 'skills'), 'project');
    ...
    it('newSession wires Config.setMcpBudgetEventCallback BEFORE initialize() (codex fix #2)', async () => {
        ...
        expect(extNotification).toHaveBeenCalledWith('hopcode/notify/session/mcp-budget-event', {
            v: 1,
            sessionId,
            ...warningEvent,
        });
        ...
        expect(extNotification).toHaveBeenLastCalledWith('hopcode/notify/session/mcp-budget-event', {
            v: 1,
            sessionId,
            ...refusedEvent,
        });
...
describe('QwenAgent loadSession / unstable_resumeSession', () => {
     ...
     */
    function goalRecord(...outputHistoryItems) {
        return {
            uuid: 'goal-rec',
            parentUuid: null,
    ...
    function allowGoalRestore(innerConfig) {
    ...
    it('unstable_resumeSession also re-registers the goal hook', async () => {
        ...
        expect(registerGoalHook).toHaveBeenCalledWith(expect.objectContaining({
            condition: 'keep going',
            initialIterations: 0,
        }));
        mockConnectionState.resolve();
    ...
    it('loadSession reinstalls the goal terminal observer after a restore', async () => {
        ...
        const installedAt = lastSessionMock.installGoalTerminalObserver.mock.invocationCallOrder.at(-1);
    ...
    it('loadSession can return bulk replay updates without streaming history', async () => {
        ...
        expect(mockHistoryReplay.mock.invocationCallOrder[0]).toBeLessThan(lastSessionMock.installRewriter.mock.invocationCallOrder[0]);
...
describe('normalizeCoreSettingValue', () => {
    ...
    it('accepts an allowed enum value and rejects an unknown one', () => {
        expect(normalizeCoreSettingValue('tools.approvalMode', 'auto')).toBe('auto');
...
describe('sessionLanguage multi-session propagation', () => {
    ...
    it('propagates language write and refresh to all sessions with varying paths', async () => {
        ...
        expect(updateOutputLanguageFile).toHaveBeenCalledWith('zh', '/proj-b/.hopcode/output-language.md');
    ...
    it('still refreshes sessions when a file write fails', async () => {
        ...
        const cfgFail = makeConfig({
            ...
            getOutputLanguageFilePath: vi
                .fn()
                .mockReturnValue('/readonly/.hopcode/output-language.md'),
        });
        ...
        vi.mocked(updateOutputLanguageFile).mockImplementation((_value, path) => {
            if (path === '/readonly/.hopcode/output-language.md') {
    ...
    it('refreshes busy skill sessions and reports per-session failures', async () => {
        ...
        await agent.newSession({ cwd: '/skills', mcpServers: [] });
        await expect(agent.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh, {})).resolves.toEqual({ sessionsRefreshed: 1, sessionsFailed: 1 });
    ...
    it('refreshes extension state without a duplicate direct skill refresh', async () => {
        ...
        expect(extensionManager.refreshTools.mock.invocationCallOrder[0]).toBeLessThan(refreshSystemInstruction.mock.invocationCallOrder[0]);
...
describe('createWorkspaceMcpBudget — env parsing', () => {
    const KEY = 'HOPCODE_SERVE_MCP_CLIENT_BUDGET';
    const MODE = 'HOPCODE_SERVE_MCP_BUDGET_MODE';
    const onEvent = vi.fn();
    afterEach(() => {
        delete process.env[KEY];
