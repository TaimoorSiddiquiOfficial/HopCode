...
import { readWorktreeSessionMarker, SessionService, Storage, writeRuntimeStatus, writeWorktreeSessionMarker, } from '@hoptrendy/hopcode-core';
const exec = promisify(execFile);
async function makeTempRepo() {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-startup-test-'));
...
describe('setupStartupWorktree', () => {
    ...
    it('creates a worktree with an auto-generated slug for bare --worktree', async () => {
        ...
        if (res.ok) {
            ...
            expect(res.context.worktreePath).toContain(path.join('.hopcode', 'worktrees', res.context.slug));
    ...
    it('creates a worktree with an explicit slug', async () => {
        ...
        if (res.ok) {
            ...
            expect(res.context.worktreePath).toBe(path.join(tempRepo, '.hopcode', 'worktrees', 'my-feature'));
        }
    });
    ...
    it('rejects full GitHub PR URLs when origin remote is missing', async () => {
        tempRepo = await makeTempRepo();
        process.chdir(tempRepo);
        const res = await setupStartupWorktree('https://github.com/TaimoorSiddiquiOfficial/HopCode/pull/4174');
        expect(res).not.toBeNull();
        expect(res.ok).toBe(false);
        if (!res.ok) {
    ...
    it('creates a pr-<N> worktree from FETCH_HEAD when fetch succeeds (local fake remote)', async () => {
        ...
        const upstream = await fs.mkdtemp(path.join(os.tmpdir(), 'hopcode-wt-pr-upstream-'));
        ...
        try {
            ...
            if (res.ok) {
                ...
                expect(res.context.worktreePath).toBe(path.join(tempRepo, '.hopcode', 'worktrees', 'pr-42'));
    ...
    it('refuses to re-attach when an existing dir occupies the slot on a different branch', async () => {
        ...
        const slotPath = path.join(tempRepo, '.hopcode', 'worktrees', 'plain-dir-conflict');
...
describe('persistStartupWorktreeSidecar', () => {
    ...
    it('finds an active owner under a repo-subdir relative runtime dir', async () => {
        ...
        Storage.setRuntimeBaseDir('.hopcode', setup.context.worktreePath);
        await persistStartupWorktreeSidecar(makeConfig(setup.context.worktreePath, 'new-session'), { ...setup.context, wasReattached: true });
        expect(await readWorktreeSessionMarker(setup.context.worktreePath)).toBe('old-session');
    });
});
