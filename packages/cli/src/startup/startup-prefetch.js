 ...
 */
import { createDebugLogger, IdeClient, initializeTelemetry, } from '@hoptrendy/hopcode-core';
...
export function startPostRenderPrefetches(config, settings, options = {}) {
    ...
    if (settings.merged.general?.enableAutoUpdate !== false &&
        ...
        runDeferredTask('update_check', async () => {
            const [{ checkForUpdatesDetailed }, { handleAutoUpdate }, { getInstallationInfo }, { updateEventEmitter }, { t },] = await Promise.all([
            ...
            const updateFailedMessage = t('Failed to check for updates. Please check your network or registry configuration.');
            try {
