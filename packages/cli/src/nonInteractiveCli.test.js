 ...
 */
import { executeToolCall, ToolErrorType, shutdownTelemetry, GeminiEventType, Kind, OutputFormat, uiTelemetryService, FatalInputError, ApprovalMode, SendMessageType, } from '@hoptrendy/hopcode-core';
import { runNonInteractive, skipHeadlessLoopSentinel, } from './nonInteractiveCli.js';
...
describe('runNonInteractive', () => {
    ...
    it('should stop repeated duplicate provider tool-call responses from drain items', async () => {
        ...
        const duplicateToolCallEvent = {
            type: GeminiEventType.ToolCallRequest,
            value: {
                callId: 'tool-drain__HOPCODE_dup_2',
    ...
    it('should ignore duplicate provider tool-call ids already present in chat history', async () => {
        ...
        expect(duplicateParts[0].functionResponse?.id).toBe('tool-history__hopcode_dup_2');
    ...
    it('should handle command that requires confirmation by returning early', async () => {
        ...
        expect(processStderrSpy).toHaveBeenCalledWith('Shell command confirmation is not supported in non-interactive mode. Use Izn mode or pre-approve commands.\n');
    });
    ...
    it('should execute only the first duplicate tool call id in stream-json format', async () => {
        ...
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
            callId: 'dup_id_0001',
            args: { file_path: 'a.ts' },
    ...
    describe('--resume with active worktree (Phase C)', () => {
        it('injects a <system-reminder> block into the user prompt when sidecar names a live worktree', async () => {
            ...
            const worktreeDir = path.join(realTmpDir, '.hopcode', 'worktrees', 'worktree-real');
