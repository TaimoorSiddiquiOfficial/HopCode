...
describe('Keypress Debug', () => {
    it('stdin reference check - same object?', async () => {
        let stdinFromHook = null;
        const Comp = () => {
            const { stdin } = useStdin();
            stdinFromHook = stdin;
            return React.createElement(Text, null, 'test');
        };
        const { stdin, unmount } = renderWithProviders(React.createElement(Comp, null));
    ...
    it('keypress listener count after direct on() call', async () => {
        const { stdin, unmount } = renderWithProviders(React.createElement(Text, null, 'test'));
        await new Promise((r) => setTimeout(r, 50));
...
//# sourceMappingURL=keypress-debug.test.js.map