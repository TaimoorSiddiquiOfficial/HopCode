...
vi.mock('@hoptrendy/acp-bridge/spawnChannel', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@hoptrendy/acp-bridge/spawnChannel')>();
  return {
    ...actual,
...
describe('workspace skill settings persistence', () => {
  ...
  afterEach(async () => {
    ...
    if (hopcodeHome) fs.rmSync(hopcodeHome, { recursive: true, force: true });
  ...
  it('canonicalizes, deduplicates, preserves orphans, serializes updates, and enforces user locks', async () => {
    ...
    fs.writeFileSync(
      path.join(hopcodeHome, 'settings.json'),
...
describe('runHopCodeServe runtime startup failures', () => {
  ...
  it.each([
    ...
    'resolves HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS=%s to %s',
    (envValue, expected) => {
  ...
  it('starts deferred runtime for webhook routes without bearer auth', async () => {
    ...
    try {
      ...
      if (previoushopcodeHome === undefined) {
...
describe('runHopCodeServe startup observability', () => {
      ...
      expect(stdoutWrites).toEqual(
        expect.arrayContaining([
          expect.stringMatching(
            /^hopcode serve listening on http:\/\/127\.0\.0\.1:\d+ \(mode=http-bridge, workspace=/,
          ),
        ]),
      );
  ...
  it('uses boot runtimeOutputDir for daemon logs', async () => {
    ...
    try {
      ...
      expect(stderrWrites.join('')).toContain(
        `hopcode serve: daemon log → ${expectedDaemonDir}`,
      );
