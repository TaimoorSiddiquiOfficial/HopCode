 ...
 */
import { getGitWorkingTreeStatus, resolveBranchName, watchRepoBranch, } from '@hoptrendy/hopcode-core';
export class WorkspaceGitState {
    ...
    async getStatus(workspaceCwd, bridge) {
        ...
        const status = await getGitWorkingTreeStatus(workspaceCwd).catch(() => null);
        if (!status) {
