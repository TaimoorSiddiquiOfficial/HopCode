 ...
 */
import { ApprovalMode, APPROVAL_MODES, createDebugLogger, ModelsConfig, tokenLimit, } from '@hoptrendy/hopcode-core';
...
function buildWorkspaceProvidersStatus(workspaceCwd, acpChannelLive, options) {
    try {
        const loaded = loadSettings(workspaceCwd, options.env ? { skipLoadEnvironment: true } : true);
