...
export function allowOriginCors(patterns) {
    ...
    const allowedHeaders = 'Authorization, Content-Type, X-HopCode-Client-Id, Last-Event-ID';
...
export function createMutationGate(deps) {
    ...
    const strictDenier = (_req, res) => {
        ...
        // different boot error. Configuring a token via `HOPCODE_SERVER_TOKEN`
        ...
        res.status(401).json({
            error: 'This route requires the daemon to be configured with a bearer ' +
                'token. Set HOPCODE_SERVER_TOKEN or pass --token to enable bearer ' +
                'auth.',
            code: 'token_required',
        });
    };
