...
import { createLazyBridgeProxy, extractContextFilename, formatChannelWorkerDaemonUrl, InvalidPolicyConfigError, createDisabledChannelWorkerSupervisor, resolveRuntimeStartupTimeoutMs, runHopCodeServe, validatePolicyConfig, waitForRuntimeStartingForShutdown, } from './run-hopcode-serve.js';
...
vi.mock('@hoptrendy/acp-bridge/spawnChannel', async (importOriginal) => {
...
describe('workspace skill settings persistence', () => {
    ...
    it('canonicalizes, deduplicates, preserves orphans, serializes updates, and enforces user locks', async () => {
        ...
        fs.writeFileSync(path.join(qwenHome, 'settings.json'), JSON.stringify({ skills: { disabled: ['locked-skill'] } }));
 ...
 * `integration-tests/cli/hopcode-serve-routes.test.ts`.
 */
 ...
 * during `runHopCodeServe` boot + shutdown. Uses a fake bridge to avoid
 * spawning real `hopcode --acp` child processes.
 */
describe('runHopCodeServe daemon logger wiring', () => {
    let tmpDir;
    afterEach(() => {
        if (tmpDir) {
    ...
    it('creates a daemon log file at boot and flushes on shutdown', async () => {
        ...
        // Minimal fake bridge satisfying the shape runHopCodeServe expects.
        const fakeBridge = {
            spawnOrAttach: vi.fn(),
            shutdown: vi.fn().mockResolvedValue(undefined),
            killAllSync: vi.fn(),
...
describe('runHopCodeServe telemetry validation', () => {
    let tmpDir;
    const originalSensitiveSpanAttributeMaxLengthEnv = process.env['HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH'];
    afterEach(() => {
        vi.restoreAllMocks();
        if (originalSensitiveSpanAttributeMaxLengthEnv === undefined) {
            delete process.env['HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH'];
        }
        else {
            process.env['HOPCODE_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH'] =
...
describe('runHopCodeServe pre-listen bridge option validation', () => {
    let tmpDir;
    afterEach(() => {
        vi.restoreAllMocks();
        if (tmpDir) {
...
describe('runHopCodeServe runtime startup failures', () => {
    let tmpDir;
    afterEach(() => {
        vi.restoreAllMocks();
        if (tmpDir) {
    ...
    it('rebuilds runtime env from the immutable daemon base after workspace reload', async () => {
        ...
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((...args) => {
            ...
            const isReload = typeof options === 'object' && options?.skipLoadEnvironment === true;
            return {
                merged: {
                    env: {
    ...
    ])('resolves HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS=%s to %s', (envValue, expected) => {
        const originalEnv = process.env['HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS'];
        try {
            if (envValue === undefined) {
                delete process.env['HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS'];
            }
            else {
                process.env['HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS'] = envValue;
            }
            ...
            if (originalEnv === undefined) {
                ...
                process.env['HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS'] = originalEnv;
            }
        }
    });
...
describe('runHopCodeServe Web Shell signals on RunHandle', () => {
    let tmpDir;
    afterEach(() => {
        if (tmpDir) {
            fs.rmSync(tmpDir, { recursive: true, force: true });
...
describe('runHopCodeServe startup observability', () => {
    let tmpDir;
    afterEach(() => {
        vi.restoreAllMocks();
        if (tmpDir) {
    ...
    it('keeps the stdout listening contract and exposes startup timing on stderr and status', async () => {
        ...
        try {
            ...
            expect(await readStartup(handle)).toMatchObject({
                ...
                runHopCodeServeToListenMs: expect.any(Number),
    ...
    it('uses boot runtimeOutputDir for daemon logs', async () => {
        ...
        try {
            ...
            const expectedDaemonDir = path.join(boundWorkspace, '.hopcode-runtime', 'debug', 'daemon');
            expect(stderrWrites.join('')).toContain(`hopcode serve: daemon log → ${expectedDaemonDir}`);
    ...
    it('preserves Storage runtime base dir for default exported callers', async () => {
        ...
        fs.writeFileSync(path.join(tmpDir, '.hopcode', 'settings.json'), JSON.stringify({
...
//# sourceMappingURL=run-hopcode-serve.test.js.map