...
import { Storage, WorkspaceMemoryFileTooLargeError, WorkspaceMemoryWriteTimeoutError, getAllGeminiMdFilenames, writeWorkspaceContextFile, } from '@hoptrendy/hopcode-core';
import { writeStderrLine } from '../utils/stdioHelpers.js';
import { isServeDebugMode } from './debug-mode.js';
import { createIdleWorkspaceMemoryStatus, STATUS_SCHEMA_VERSION, } from '@hoptrendy/acp-bridge/status';
...
export function mountWorkspaceMemoryRoutes(app, deps) {
    app.get('/workspace/memory', async (_req, res) => {
        try {
            ...
            writeStderrLine(`hopcode serve: GET /workspace/memory failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
