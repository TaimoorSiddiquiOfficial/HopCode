  ...
  return (res: Response): void => {
    ...
    res.sendFile(
      ...
      (err) => {
        ...
        writeStderrLine(
          `hopcode serve: Web Shell index send failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        if (!res.headersSent) {
...
export function mountWebShellAssets(
  ...
  app.use('/assets', (req: Request, res: Response) => {
    ...
    if (isServeDebugMode()) {
      writeStderrLine(
        `hopcode serve: Web Shell asset not found: ${req.originalUrl}`,
      );
    }
...
export function mountWebShellSpaFallback(
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    if (isServeDebugMode()) {
      writeStderrLine(
        `hopcode serve: Web Shell SPA fallback served for ${req.method} ${req.originalUrl}`,
      );
    }
    sendIndex(res);
  });
