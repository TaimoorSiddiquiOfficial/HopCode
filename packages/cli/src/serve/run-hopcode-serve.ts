...
export function validatePolicyConfig(
  ...
  if (
    ...
    throw new InvalidPolicyConfigError(
      `hopcode serve: invalid policy.permissionStrategy ` +
  ...
  if (
    ...
    throw new InvalidPolicyConfigError(
      `hopcode serve: invalid policy.consensusQuorum ` +
  ...
  if (consensusQuorum !== undefined && permissionStrategy !== 'consensus') {
    onWarning(
      'hopcode serve: policy.consensusQuorum is set but ' +
...
type CoreRuntime = typeof import('@hoptrendy/hopcode-core');
...
function currentServeFeaturesForrunHopCodeServe(
  opts: ServeOptions,
...
function createBootstrapCapabilities(input: {
  ...
  return {
    ...
    features: currentServeFeaturesForrunHopCodeServe(
      input.opts,
 ...
 *   2. `HOPCODE_SERVER_TOKEN` env var
 *
...
export async function runHopCodeServe(
  ...
  const promptDeadlineMs =
    ...
    parseDeadlineEnv(
      HOPCODE_SERVE_PROMPT_DEADLINE_MS_ENV,
      process.env[HOPCODE_SERVE_PROMPT_DEADLINE_MS_ENV],
    );
  ...
  if (opts.allowOrigins && opts.allowOrigins.length > 0) {
    ...
    writeStderrLine(
      `hopcode serve: --allow-origin: ${opts.allowOrigins.join(', ')}` +
        (parsed.allowAny
  ...
  if (opts.allowPrivateAuthBaseUrl) {
    writeStderrLine(
      'hopcode serve: --allow-private-auth-base-url enabled; ' +
  ...
  try {
    ...
    writeStderrLine(
      `hopcode serve: could not read settings for context.fileName / ` +
  ...
  const childEnvOverrides: Record<string, string | undefined> = {
    HOPCODE_SERVE_MCP_CLIENT_BUDGET:
  ...
  if (opts.serveWebShell !== false) {
    if (!webShellDir) {
      writeStderrLine(
        'hopcode serve: Web Shell assets not found; serving API only. ' +
      ...
      if (!isLoopbackBind(opts.hostname)) {
        writeStderrLine(
          'hopcode serve: Web Shell UI is served WITHOUT auth on a non-loopback ' +
        ...
        if (!opts.allowOrigins || opts.allowOrigins.length === 0) {
          writeStderrLine(
            'hopcode serve: without --allow-origin the Web Shell is read-only on a ' +
  ...
  const buildRuntime = async (): Promise<{
    ...
    try {
      ...
      writeStderrLine(
        `hopcode serve: could not read full settings for runtime startup ` +
    ...
    daemonTelemetry.metrics = {
      sessionLifecycle(action) {
        ...
        core.emitDaemonLog(
          `Session ${action}.`,
          {
            'hopcode.workspace.hash': daemonWorkspaceHash,
          },
          {
  ...
  return await new Promise<RunHandle>((resolve, reject) => {
    const server = app.listen(opts.port, listenHostname, () => {
      ...
      startup.runHopCodeServeToListenMs = Math.round(
      ...
      writeStderrLine(
        `hopcode serve: startup timing: processToListenMs=${startup.processToListenMs} ` +
          `runHopCodeServeToListenMs=${startup.runHopCodeServeToListenMs}`,
      );
      if (!token) {
        writeStderrLine(
          `hopcode serve: bearer auth disabled (loopback default). Set ${HOPCODE_SERVER_TOKEN_ENV} to enable.`,
        );
      ...
      const startBridgePreheat = (bridge: AcpSessionBridge): void => {
        ...
        bridge
          ...
          .catch((err) => {
            ...
            startup.preheat.error = message;
            writeStderrLine(
              `hopcode serve: ACP preheat failed, will retry on first session: ${message}`,
            );
          });
      };
          ...
          closePromise = new Promise<void>((res, rej) => {
            ...
            const finish = (err?: Error | null) => {
              ...
              void (
                ...
                .catch((telemetryErr) => {
                  writeStderrLine(
                    `hopcode serve: telemetry shutdown error: ${
