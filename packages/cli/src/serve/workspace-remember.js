...
import { createWorkspaceMemoryExtractionErrorLogger, shouldSuppressRememberErrorDetails, workspaceMemoryFailureCode, workspaceMemoryFailureDiagnostics, } from './workspace-remember-errors.js';
import { MAX_REMEMBER_CONTENT_BYTES } from './workspace-memory-remember-constants.js';
import { formatWorkspaceMemoryDreamSummary, formatWorkspaceMemoryForgetSummary, } from './workspace-memory-summaries.js';
...
async function validateManagedMemoryAvailable(deps, res, kind) {
    try {
...
export function mountWorkspaceMemoryRememberRoutes(app, deps) {
    app.post('/workspace/memory/remember', deps.mutate({ strict: true }), async (req, res) => {
    ...
    app.get('/workspace/memory/remember/:taskId', deps.mutate({ strict: true }), (req, res) => {
    ...
    app.post('/workspace/memory/forget', deps.mutate({ strict: true }), async (req, res) => {
        const body = deps.safeBody(req);
        ...
        if (originatorClientId === null)
            return;
        if (!(await validateManagedMemoryAvailable(deps, res, 'forget')))
            return;
        try {
    ...
    app.get('/workspace/memory/forget/:taskId', deps.mutate({ strict: true }), (req, res) => {
        const requesterClientId = validateOriginatorClientId(deps, req, res);
        if (requesterClientId === null)
            return;
    ...
    app.post('/workspace/memory/dream', deps.mutate({ strict: true }), async (req, res) => {
        const originatorClientId = validateOriginatorClientId(deps, req, res);
        if (originatorClientId === null)
            return;
        if (!(await validateManagedMemoryAvailable(deps, res, 'dream')))
            return;
        try {
    ...
    app.get('/workspace/memory/dream/:taskId', deps.mutate({ strict: true }), (req, res) => {
        const requesterClientId = validateOriginatorClientId(deps, req, res);
        if (requesterClientId === null)
            return;
