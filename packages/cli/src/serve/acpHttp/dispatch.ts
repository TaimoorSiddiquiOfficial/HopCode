...
const HOPCODE_VENDOR_METHODS: readonly string[] = [
  ...
  `${HOPCODE_METHOD_NS}session/shell`,
...
export class AcpDispatcher {
  ...
  private killOrphanSession(sessionId: string): void {
    void this.bridge
      ...
      .catch((err) =>
        writeStderrLine(
          `hopcode serve: /acp orphan killSession(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`,
        ),
      );
  }
  ...
  private async configOptionsFor(
    ...
    try {
      ...
      writeStderrLine(
        `hopcode serve: /acp configOptionsFor(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`,
      );
      return undefined;
    }
  }
  ...
  cancelAbandonedPermission(
    ...
    try {
      ...
      writeStderrLine(
        `hopcode serve: /acp cancelAbandonedPermission(${logSafe(req.sessionId)}) failed: ${logSafe(msg)}`,
      );
      return false;
    }
  }
      ...
      agentCapabilities: {
        ...
        _meta: {
          [HOPCODE_META_KEY]: {
            connectionId,
            workspaceCwd: this.boundWorkspace,
            methods: [...HOPCODE_VENDOR_METHODS],
          },
        },
      },
    };
  ...
  private requireOwned(
    ...
    if (id === undefined) {
      ...
      writeStderrLine(
        `hopcode serve: /acp notification for unowned session ${logSafe(sessionId)} (dropped)`,
      );
      return false;
    }
    conn.sendConn(
  ...
  async handle(
    ...
    if (isResponse(msg)) {
      try {
        ...
        writeStderrLine(
          `hopcode serve: /acp response handling error: ${logSafe(errMsg(err))}`,
        );
      }
      return;
    }
    ...
    try {
      switch (method) {
        ...
        case 'session/resume': {
          ...
          if (conn.destroyed || closeRaced) {
            ...
            void cleanup.catch((err) =>
              writeStderrLine(
                `hopcode serve: /acp orphan ${restored.attached ? 'detach' : 'kill'}(${logSafe(sessionId)}) teardown-race: ${logSafe(errMsg(err))}`,
              ),
            );
        ...
        case `${HOPCODE_METHOD_NS}workspace/memory/write`: {
          ...
          writeStderrLine(
            `hopcode serve: /acp workspace/memory/write scope=${scope} mode=${mode} client=${conn.clientId?.slice(0, 8)} bytes=${Buffer.byteLength(content, 'utf8')}`,
          );
        ...
        case `${HOPCODE_METHOD_NS}sessions/delete`: {
            ...
            ids.map(async (sid) => {
              try {
                ...
                if (
                  ...
                  writeStderrLine(
                    `hopcode serve: /acp sessions/delete closeSession(${sid.slice(0, 8)}) failed: ${msg}`,
                  );
          ...
          for (const e of removeResult.errors) {
            writeStderrLine(
              `hopcode serve: /acp sessions/delete removeSessions(${e.sessionId.slice(0, 8)}) failed: ${e.error.message}`,
            );
          }
          this.replyConn(conn, id, {
      ...
      writeStderrLine(
        `hopcode serve: /acp dispatch error (${logSafe(method)}): ${logSafe(errMsg(err))}`,
      );
      if (id !== undefined) {
    ...
    switch (event.type) {
      ...
      case 'permission_request': {
        ...
        if (!binding?.stream || binding.stream.isClosed) {
          ...
          if (!cancelled) {
            writeStderrLine(
              `hopcode serve: /acp permission cancel FAILED for ${logSafe(sessionId)} (mediator stuck; no automatic recovery)`,
            );
          }
          return;
        }
  ...
  private async handlePrompt(
    ...
    try {
      ...
      if (id !== undefined) {
        ...
        writeStderrLine(
          `hopcode serve: /acp prompt error (${logSafe(sessionId)}, notification): ${logSafe(errMsg(err))}`,
        );
      }
    } finally {
  ...
  private replySession(
    ...
    if (conn.sessions.has(sessionId)) {
      ...
      // session teardown.
      writeStderrLine(
        `hopcode serve: /acp replySession(${logSafe(sessionId)}) binding gone mid-flight, ` +
