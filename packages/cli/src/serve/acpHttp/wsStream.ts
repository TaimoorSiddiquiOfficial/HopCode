...
export class WsStream implements TransportStream {
  ...
  send(message: unknown): Promise<void> {
    ...
    this.writeChain = next.catch((err: unknown) => {
      if (!this._closed) {
        writeStderrLine(
          `hopcode serve: /acp WS write failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        this.close();
      }
    });
  ...
  close(): void {
    ...
    try {
      ...
      writeStderrLine(
        `hopcode serve: /acp WS onClose threw: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
}
