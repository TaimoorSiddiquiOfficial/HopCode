...
function createSendIndex(webShellDir, frameAncestors = []) {
    ...
    return (res) => {
        ...
        res.sendFile(indexPath, { cacheControl: false, dotfiles: 'allow' }, (err) => {
            ...
            writeStderrLine(`hopcode serve: Web Shell index send failed: ${err instanceof Error ? err.message : String(err)}`);
            if (!res.headersSent) {
...
export function mountWebShellAssets(app, webShellDir, frameAncestors = []) {
    ...
    app.use('/assets', (req, res) => {
        ...
        if (isServeDebugMode()) {
            writeStderrLine(`hopcode serve: Web Shell asset not found: ${req.originalUrl}`);
        }
...
export function mountWebShellSpaFallback(app, webShellDir, frameAncestors = []) {
    ...
    app.use((req, res, next) => {
        ...
        if (isServeDebugMode()) {
            writeStderrLine(`hopcode serve: Web Shell SPA fallback served for ${req.method} ${req.originalUrl}`);
        }
        sendIndex(res);
    });
}
