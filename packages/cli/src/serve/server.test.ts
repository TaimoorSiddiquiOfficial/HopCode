...
describe('createServeApp', () => {
  ...
  describe('Web Shell static serving', () => {
    ...
    const INDEX_HTML =
      '<!doctype html><html><head><title>HopCode Web terminal</title>' +
  ...
  describe('GET /workspace/channel/observed-contacts', () => {
    it('is assembled behind bearer authentication', async () => {
      ...
      const hopcodeHome = await fsp.mkdtemp(
  ...
  describe('read-only status routes', () => {
    ...
    it('returns workspace skills from the bridge and providers from daemon-local settings', async () => {
      const tempHome = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'hopcode-serve-providers-'),
      );
      const previoushopcodeHome = process.env['HOPCODE_HOME'];
  ...
  describe('GET /workspace/:id/sessions', () => {
    ...
    beforeEach(async () => {
      ...
      runtimeDir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'hopcode-serve-sessions-'),
      );
  ...
  describe('POST /sessions/delete', () => {
    ...
    beforeEach(async () => {
      previousRuntimeDir = process.env['HOPCODE_RUNTIME_DIR'];
      runtimeDir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'hopcode-serve-batch-delete-'),
      );
      process.env['HOPCODE_RUNTIME_DIR'] = runtimeDir;
      wsDir = realpathSync(runtimeDir);
    });
    ...
    it('isolates webhook configs across workspace sources', async () => {
      ...
      try {
        ...
        for (const [workspace, channel] of [
          ...
          const hopcodeDir = path.join(workspace, '.hopcode');
          await fsp.mkdir(hopcodeDir);
          await fsp.writeFile(
...
describe('runHopCodeServe', () => {
  ...
  it.each([
    ...
    'rejects invalid HOPCODE_SERVE_PROMPT_DEADLINE_MS env var (%s) at boot (#4514 T2.9)',
    async (_label, value) => {
...
describe('createServeApp ServeAppDeps.fsFactory wiring (#4175 PR 18)', () => {
  ...
  it('passes custom ignore files through resolveBridgeFsFactory', async () => {
    ...
    const tmp = await fsp.mkdtemp(
      path.join(os.tmpdir(), 'hopcode-serve-fs-ignore-'),
    );
    try {
