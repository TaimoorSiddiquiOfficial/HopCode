...
import { AGENT_CONTEXT_FILENAME, DEFAULT_CONTEXT_FILENAME, setGeminiMdFilename, Storage, } from '@hoptrendy/hopcode-core';
...
describe('workspace memory routes', () => {
    ...
    describe('POST /workspace/memory', () => {
        it('appends to workspace HOPCODE.md and emits memory_changed', async () => {
            ...
            const written = await fs.readFile(path.join(workspace, 'HOPCODE.md'), 'utf8');
        ...
        it('writes to the global ~/.hopcode directory when scope=global', async () => {
            ...
            const written = await fs.readFile(path.join(globalDir, 'HOPCODE.md'), 'utf8');
        ...
        it('omits errorMessage + filePath in 500/413 responses unless HOPCODE_SERVE_DEBUG is on', async () => {
            ...
            try {
                ...
                if (prevDebug === undefined)
                    ...
                    process.env['HOPCODE_SERVE_DEBUG'] = prevDebug;
