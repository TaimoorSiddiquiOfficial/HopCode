...
async function makeHarness(opts) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `hopcode-wfs-${randomBytes(4).toString('hex')}-`));
...
describe('WorkspaceFileSystem - factory', () => {
    it('canonicalizes the workspace once at factory build', async () => {
        const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `hopcode-wfs-canon-${randomBytes(4).toString('hex')}-`));
        try {
