...
describe('shouldIgnore', () => {
    ...
    let workspace;
    beforeEach(async () => {
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `hopcode-policy-${randomBytes(4).toString('hex')}-`));
...
describe('detectBinary', () => {
    ...
    let workspace;
    beforeEach(async () => {
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `hopcode-binary-${randomBytes(4).toString('hex')}-`));
        const wsDir = path.join(scratch, 'ws');
        await fsp.mkdir(wsDir, { recursive: true });
        workspace = canonicalizeWorkspace(wsDir);
    });
