  ...
  return {
    ...
    async getWorkspaceEnvStatus(_ctx: WorkspaceRequestContext) {
      ...
      try {
        ...
        writeStderrLine(
          `hopcode serve: getEnvStatus failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        return createIdleEnvStatus(boundWorkspace, acpChannelLive);
      }
    },
    ...
    async getWorkspacePreflightStatus(_ctx: WorkspaceRequestContext) {
      ...
      if (statusProvider) {
        try {
          ...
          writeStderrLine(
            `hopcode serve: getDaemonPreflightCells failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      ...
      if (persistSettings) {
        ...
        for (const write of writes) {
          try {
            ...
            writeStderrLine(
              `hopcode serve: workspace voice partial persist error (workspace=${boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${
    ...
    async restartMcpServer(
      ...
      if ('entries' in result) {
        ...
        if (!Array.isArray(result.entries)) {
          writeStderrLine(
            `hopcode serve: pool restart response carried 'entries' field ` +
        ...
        for (const entry of entries) {
          if (
            ...
            writeStderrLine(
              `hopcode serve: skipping malformed pool restart entry ` +
    ...
    async reload(ctx: WorkspaceRequestContext) {
      if (deps.reloadDaemonEnv) {
        try {
          ...
          writeStderrLine(
            `hopcode serve: daemon reload failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
    ...
    async refreshExtensionsForAllSessions() {
      try {
        ...
        writeStderrLine(
          `hopcode serve: refreshExtensionsForAllSessions failed: ${err instanceof Error ? err.message : String(err)}`,
        );
