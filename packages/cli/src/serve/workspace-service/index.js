...
export function createDaemonWorkspaceService(deps) {
    const { boundWorkspace, contextFilename, statusProvider, workspaceProvidersStatusProvider, workspaceSkillsStatusProvider, isChannelLive, persistDisabledTools, persistDisabledSkills, persistSetting, persistSettings, skillInstallEnv, voiceEnv, voiceSettingsScope, preheatAcpChild: preheatAcpChildOnBridge, queryWorkspaceStatus, invokeWorkspaceCommand, refreshExtensionsForAllSessions: refreshExtensionsForAllSessionsOnBridge, publishWorkspaceEvent, } = deps;
    ...
    const getWorkspaceSkillsStatus = async () => {
        let status;
        try {
            ...
            writeStderrLine(`hopcode serve: getWorkspaceSkillsStatus query failed: ${err instanceof Error ? err.message : String(err)}`);
        ...
        if (workspaceSkillsStatusProvider) {
            try {
                ...
                writeStderrLine(`hopcode serve: getWorkspaceSkillsStatus local provider failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        return status;
    };
    const refreshWorkspaceSkillsAfterMutation = async () => {
        ...
        if (!(isChannelLive?.() ?? false))
            return;
        try {
            await invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh, { cwd: boundWorkspace });
        }
        catch (err) {
            if (!(err instanceof SessionNotFoundError) &&
                !(err instanceof BridgeChannelClosedError)) {
                writeStderrLine(`hopcode serve: workspace skill refresh after mutation failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    };
    // -- Facade --
    return {
        ...
        async preheatAcpChild(_ctx, opts) {
            ...
            if (!inFlightAcpPreheat) {
                ...
                void inFlightAcpPreheat.catch((err) => {
                    writeStderrLine(`hopcode serve: ACP preheat failed: ${err instanceof Error ? err.message : String(err)}`);
                });
            }
            try {
                await withTimeout(inFlightAcpPreheat, opts?.timeoutMs ?? 5_000, 'ACP preheat');
            }
            catch (err) {
                if (err instanceof TimeoutError) {
                    inFlightAcpPreheat = undefined;
                    writeStderrLine(`hopcode serve: ACP preheat timed out after ${opts?.timeoutMs ?? 5_000}ms`);
                }
        ...
        async getWorkspaceEnvStatus(_ctx) {
            ...
            try {
                ...
                writeStderrLine(`hopcode serve: getEnvStatus failed: ${err instanceof Error ? err.message : String(err)}`);
                return createIdleEnvStatus(boundWorkspace, acpChannelLive);
            }
        },
        async getWorkspacePreflightStatus(_ctx) {
            ...
            if (statusProvider) {
                try {
                    ...
                    writeStderrLine(`hopcode serve: getDaemonPreflightCells failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        ...
        async getWorkspaceTrustStatus(_ctx) {
            return getWorkspaceTrustStatus(loadSettings(boundWorkspace).merged, boundWorkspace);
        },
        ...
        async getWorkspaceVoiceStatus(_ctx) {
            return buildWorkspaceVoiceStatus(boundWorkspace, loadSettings(boundWorkspace, voiceEnv ? { skipLoadEnvironment: true } : true));
        },
        // -- Mutations --
        ...
        async setWorkspaceVoiceSettings(ctx, request) {
            ...
            const settings = loadSettings(boundWorkspace, voiceEnv ? { skipLoadEnvironment: true } : true);
            validateWorkspaceVoiceState(settings, request, { env: voiceEnv });
            const workspaceTrusted = getWorkspaceTrustStatus(settings.merged, boundWorkspace).effective
                .state === 'trusted';
            ...
            if (persistSettings) {
                ...
                for (const write of writes) {
                    try {
                        await persistSetting(boundWorkspace, write.scope, write.key, write.value);
                    }
                    catch (err) {
                        writeStderrLine(`hopcode serve: workspace voice partial persist error (workspace=${boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
        ...
        async setWorkspaceSkillEnabled(ctx, requestedSkillName, enabled) {
            ...
            const disabledNames = new Set((Array.isArray(disabled) ? disabled : [])
            ...
            const persisted = await persistDisabledSkills(boundWorkspace, skill.name, enabled);
            const channelLive = isChannelLive?.() ?? false;
                ...
                if (channelLive) {
                    try {
                        ...
                        if (err instanceof SessionNotFoundError ||
                            ...
                            activation = 'partial';
                            sessionsFailed = 1;
                            writeStderrLine(`hopcode serve: workspace skill refresh failed: ${err instanceof Error ? err.message : String(err)}`);
                        }
                    }
                }
                publishWorkspaceEvent({
        ...
        async installWorkspaceSkill(_ctx, request) {
            const result = await installWorkspaceSkill(boundWorkspace, request, skillInstallEnv?.['GH_TOKEN'] ?? skillInstallEnv?.['GITHUB_TOKEN']);
        ...
        async deleteWorkspaceSkill(_ctx, requestedSkillName, scope) {
            ...
            const result = await deleteWorkspaceSkill(boundWorkspace, scope, skill.name, skill.installedPath);
            await refreshWorkspaceSkillsAfterMutation();
            return result;
        },
        ...
        async restartMcpServer(ctx, serverName, opts) {
            ...
            if ('entries' in result) {
                ...
                if (!Array.isArray(result.entries)) {
                    writeStderrLine(`hopcode serve: pool restart response carried 'entries' field ` +
                ...
                for (const entry of entries) {
                    if (typeof entry !== 'object' ||
                        ...
                        writeStderrLine(`hopcode serve: skipping malformed pool restart entry ` +
        ...
        async reload(ctx) {
            if (deps.reloadDaemonEnv) {
                try {
                    ...
                    writeStderrLine(`hopcode serve: daemon reload failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            ...
            try {
                ...
                if (err instanceof SessionNotFoundError) {
                    ...
                    writeStderrLine(`hopcode serve: reload failed: ${childError}`);
                }
            }
            publishWorkspaceEvent({
        ...
        async refreshExtensionsForAllSessions() {
            try {
                ...
                writeStderrLine(`hopcode serve: refreshExtensionsForAllSessions failed: ${err instanceof Error ? err.message : String(err)}`);
