...
import { DEFAULT_COMPACTED_REPLAY_MAX_BYTES, normalizeCompactedReplayMaxBytes, } from '@hoptrendy/acp-bridge/replayWindowLimits';
import { getDeviceFlowRegistry } from './auth/device-flow.js';
import { loadServeFastPathSettings, preResolveServeFastPathHomeEnvOverrides, } from './fast-path-settings.js';
...
import { allowOriginMode, listenerMaxConnections, parseDaemonStatusDetail, positiveFiniteOrNull, } from './daemon-status.js';
...
const HOPCODE_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV = 'HOPCODE_SERVE_WRITER_IDLE_TIMEOUT_MS';
...
const RUNTIME_STARTUP_TIMEOUT_ENV = 'HOPCODE_SERVE_RUNTIME_STARTUP_TIMEOUT_MS';
...
function parseDeadlineEnv(envName, raw) {
    ...
    // `HOPCODE_SERVE_PROMPT_DEADLINE_MS=" "` as "not set" would let a
...
export function validatePolicyConfig(policyConfig = {}, onWarning = writeStderrLine) {
    ...
    if (permissionStrategy !== undefined &&
        ...
        throw new InvalidPolicyConfigError(`hopcode serve: invalid policy.permissionStrategy ` +
    ...
    if (consensusQuorum !== undefined &&
        (typeof consensusQuorum !== 'number' ||
            ...
            consensusQuorum < 1)) {
        throw new InvalidPolicyConfigError(`hopcode serve: invalid policy.consensusQuorum ` +
    ...
    if (consensusQuorum !== undefined && permissionStrategy !== 'consensus') {
        onWarning('hopcode serve: policy.consensusQuorum is set but ' +
...
function loadCoreRuntime() {
    coreRuntimePromise ??= import('@hoptrendy/hopcode-core');
...
async function resolveDaemonLogBaseDirForRun(input) {
    ...
    if (input.deps.bootSettings === undefined) {
        ...
        if (core.Storage.getRuntimeBaseDir() !== core.Storage.getGlobalHopCodeDir()) {
...
async function loadServeRuntimeModules() {
    const [serverModule, bridgeModule, spawnChannelModule, workspaceModule, workspaceTypesModule, daemonStatusProviderModule, workspaceProvidersStatusModule, workspaceSkillsStatusModule, totalSessionAdmissionModule, workspaceRegistryModule,] = await Promise.all([
        import('./server.js'),
        import('@hoptrendy/acp-bridge/bridge'),
        import('@hoptrendy/acp-bridge/spawnChannel'),
...
function currentServeFeaturesForrunHopCodeServe(opts, sessionShellCommandEnabled, sessionArtifactsPersistenceAvailable, env) {
...
function createBootstrapCapabilities(input) {
    return {
        ...
        ...(input.HopCodeVersion
            ? { HopCodeVersion: input.HopCodeVersion }
            : {}),
        mode: input.opts.mode,
        features: currentServeFeaturesForrunHopCodeServe(input.opts, input.sessionShellCommandEnabled, input.sessionArtifactsPersistenceAvailable, input.env),
        modelServices: [],
...
function createBootstrapServeApp(input) {
    const { opts, getPort, boundWorkspace, startup, daemonLog, HopCodeVersion, sessionShellCommandEnabled, sessionArtifactsPersistenceAvailable, permissionPolicy, multiWorkspaceCapabilitiesRequireRuntime, getRuntimeError, getChannelWorkerSnapshot, getChannelWorkerSnapshots, onHealthServed, } = input;
    const app = express();
    ...
    app.get(BOOTSTRAP_DAEMON_STATUS_PATH, (req, res) => {
        ...
        const response = {
            ...
            capabilities: {
                protocolVersions: getServeProtocolVersions(),
                features: currentServeFeaturesForrunHopCodeServe(opts, sessionShellCommandEnabled, sessionArtifactsPersistenceAvailable, process.env),
            },
            runtime: {
...
export async function runHopCodeServe(optsIn, deps = {}) {
    let daemonLog;
    let owner = 'startup';
    try {
...
async function runHopCodeServeImpl(optsIn, deps, loggerLifecycle) {
    ...
    // Trim both sources. Common gotcha: `export HOPCODE_SERVER_TOKEN=$(cat
    ...
    const rawToken = optsIn.token ?? process.env[HOPCODE_SERVER_TOKEN_ENV];
    ...
    if (optsIn.enableSessionShell === true && token === undefined) {
        writeStderrLine(`hopcode serve: --enable-session-shell ignored because no bearer token ` +
    ...
    const promptDeadlineMs = optsIn.promptDeadlineMs ??
        parseDeadlineEnv(HOPCODE_SERVE_PROMPT_DEADLINE_MS_ENV, process.env[HOPCODE_SERVE_PROMPT_DEADLINE_MS_ENV]);
    const writerIdleTimeoutMs = optsIn.writerIdleTimeoutMs ??
        parseDeadlineEnv(HOPCODE_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV, process.env[HOPCODE_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV]);
    ...
    if (!isLoopbackBind(opts.hostname) && !token) {
        throw new Error(`Refusing to bind ${opts.hostname}:${opts.port} without a bearer token. ` +
            `Set ${HOPCODE_SERVER_TOKEN_ENV} or pass --token, or rebind to loopback ` +
    ...
    if (opts.requireAuth && !token) {
        throw new Error(`Refusing to start with --require-auth set but no bearer token ` +
            `configured. Set ${HOPCODE_SERVER_TOKEN_ENV} or pass --token, or omit ` +
    ...
    if (opts.allowOrigins && opts.allowOrigins.length > 0) {
        ...
        if (parsed.allowAny && !token) {
            throw new Error(`Refusing to start with --allow-origin '*' but no bearer token ` +
                ...
                `${HOPCODE_SERVER_TOKEN_ENV} or pass --token, or list specific ` +
                `origins instead of '*'.`);
        }
        writeStderrLine(`hopcode serve: --allow-origin: ${opts.allowOrigins.join(', ')}` +
            (parsed.allowAny
    ...
    if (opts.allowPrivateAuthBaseUrl) {
        writeStderrLine('hopcode serve: --allow-private-auth-base-url enabled; ' +
    ...
    const validateAndCanonicalizeWorkspace = (workspace) => {
    ...
    try {
        ...
        writeStderrLine(`hopcode serve: could not read settings for context.fileName / ` +
    ...
    writeStderrLine(`hopcode serve: daemon log → ${daemonLog.getLogPath() || '(disabled)'}`);
    ...
    // (`HOPCODE_SERVE_NO_MCP_POOL=1`) in its own env, infer
    // `mcpPoolActive: false` so the capabilities envelope drops the
    // `mcp_workspace_pool` + `mcp_pool_restart` tags.
    const inheritedNoPool = process.env['HOPCODE_SERVE_NO_MCP_POOL'] === '1';
    if (opts.mcpPoolActive === undefined && inheritedNoPool) {
        opts.mcpPoolActive = false;
    }
    const childEnvOverrides = {
        HOPCODE_SERVE_MCP_CLIENT_BUDGET: opts.mcpClientBudget !== undefined
            ? String(opts.mcpClientBudget)
            : undefined,
        HOPCODE_SERVE_MCP_BUDGET_MODE: opts.mcpBudgetMode,
    };
    ...
    if (opts.serveWebShell !== false) {
        if (!webShellDir) {
            writeStderrLine('hopcode serve: Web Shell assets not found; serving API only. ' +
            ...
            writeStderrLine(`hopcode serve: Web Shell UI served from ${webShellDir}`);
            if (!isLoopbackBind(opts.hostname)) {
                writeStderrLine('hopcode serve: Web Shell UI is served WITHOUT auth on a non-loopback ' +
                ...
                if (!opts.allowOrigins || opts.allowOrigins.length === 0) {
                    writeStderrLine('hopcode serve: without --allow-origin the Web Shell is read-only on a ' +
    ...
    const buildRuntime = async () => {
        const [runtime, core, settingsRuntime, resolvedCliVersion] = await Promise.all([
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: could not read full settings for runtime startup ` +
                `(${err instanceof Error ? err.message : String(err)}); falling back to defaults.`);
        }
        const trustedWorkspace = deps.trustedWorkspace ??
            (runtimeBootSettings
                ? settingsRuntime.trustedFolders.getWorkspaceTrustStatus(runtimeBootSettings.merged, boundWorkspace).effective.state === 'trusted'
                : true);
        ...
        const runtimeEnvSnapshot = runtimeBootSettings
            ? settingsRuntime.environment.buildRuntimeEnvironment(runtimeBootSettings.merged, boundWorkspace, daemonRuntimeBaseEnv)
            : {
        ...
        core.initializeTelemetry(createDaemonTelemetryRuntimeConfig(daemonTelemetrySettings, resolvedCliVersion, `daemon:${process.pid}`, {
        ...
        const boundWorkspaces = runtime.resolveBoundWorkspacesFromIdeEnv(boundWorkspace, undefined, (workspace, index) => {
            ...
            if (!runtimeBootSettings) {
                daemonLog.warn('excluding secondary workspace root because trust settings are unavailable', { workspace });
                return false;
            }
            const trustedSecondary = settingsRuntime.trustedFolders.getWorkspaceTrustStatus(runtimeBootSettings.merged, workspace).effective.state === 'trusted';
            if (!trustedSecondary) {
        ...
        const persistDisabledToolsFn = (workspace, toolName, enabled) => withSettingsLock(workspace, async () => {
            ...
            if (enabled)
                ...
                next.add(toolName);
            fresh.setValue(WORKSPACE_SETTING_SCOPE, 'tools.disabled', [...next].sort());
        });
        const persistDisabledSkillsFn = (workspace, skillName, enabled) => withSettingsLock(workspace, async () => {
            const fresh = settingsRuntime.settings.loadSettings(workspace);
            const normalizedName = skillName.trim().toLowerCase();
            const disabledNames = (value) => Array.isArray(value)
            ...
            for (const [scope, names] of lockedScopes) {
                if (disabledNames(names).some((name) => name.trim().toLowerCase() === normalizedName)) {
                    throw new runtime.WorkspaceSkillNotToggleableError(skillName, 'locked', scope);
                }
            }
            const workspaceDisabled = disabledNames(fresh.workspace.settings.skills?.disabled);
            const next = [];
            let found = false;
            let changed = false;
            ...
            fresh.setValue(WORKSPACE_SETTING_SCOPE, 'skills.disabled', next.length > 0 ? next : undefined);
            return { changed: true, disabled: next };
        });
        const persistSettingFn = (workspace, scope, key, value) => withSettingsLock(workspace, async () => {
            const fresh = settingsRuntime.settings.loadSettings(workspace);
            fresh.setValue(scope, key, value);
            return fresh;
        });
        const persistSettingsFn = (workspace, writes) => withSettingsLock(workspace, async () => {
            const fresh = settingsRuntime.settings.loadSettings(workspace);
            ...
            try {
                ...
                writeStderrLine(`hopcode serve: ${message}`);
        ...
        const workspaceService = runtime.createDaemonWorkspaceService({
            ...
            reloadDaemonEnv: (workspace) => withSettingsLock(workspace, async () => {
                const fresh = settingsRuntime.settings.loadSettings(workspace, {
                    skipLoadEnvironment: true,
                });
                const result = settingsRuntime.settings.reloadEnvironment(fresh.merged, workspace);
                let refreshedRuntimeEnv;
                let fallbackReason;
                try {
                    refreshedRuntimeEnv =
                        settingsRuntime.environment.buildRuntimeEnvironment(fresh.merged, workspace, daemonRuntimeBaseEnv);
                }
                catch (err) {
        ...
        const createRuntimeEnvMetadata = (workspace, settings) => {
            const snapshot = settings
                ? settingsRuntime.environment.buildRuntimeEnvironment(settings.merged, workspace, daemonRuntimeBaseEnv)
                : {
                    effectiveEnv: { ...daemonRuntimeBaseEnv },
                    overlayKeys: Object.freeze([]),
                    envFilePaths: Object.freeze([]),
            ...
            const effectiveEnv = { ...snapshot.effectiveEnv };
            const metadata = {
                mode: 'runtime-overlay',
        ...
        const subSessionStoppersByWorkspace = new Map();
        ...
        for (const workspaceInput of workspaceInputs.slice(1)) {
            ...
            const secondaryTrusted = secondarySettings
                ? settingsRuntime.trustedFolders.getWorkspaceTrustStatus(secondarySettings.merged, workspaceInput.cwd).effective.state === 'trusted'
                : false;
            if (!secondaryTrusted) {
            ...
            const secondaryEnv = createRuntimeEnvMetadata(workspaceInput.cwd, secondarySettings);
            ...
            subSessionStoppersByWorkspace.set(workspaceInput.cwd, secondarySubSessionLauncher.stop);
            const secondaryWorkspaceService = runtime.createDaemonWorkspaceService({
                ...
                reloadDaemonEnv: (workspace) => withSettingsLock(workspace, async () => {
                    ...
                    const result = settingsRuntime.settings.reloadEnvironment(fresh.merged, workspace);
                    try {
                        const refreshedRuntimeEnv = settingsRuntime.environment.buildRuntimeEnvironment(fresh.merged, workspace, daemonRuntimeBaseEnv);
                        logRuntimeEnvFileReadFailures(workspace, refreshedRuntimeEnv);
        ...
        const createDynamicWorkspaceRuntime = async (cwd) => {
            let wsSettings;
            try {
            ...
            const trusted = wsSettings
                ? settingsRuntime.trustedFolders.getWorkspaceTrustStatus(wsSettings.merged, cwd).effective.state === 'trusted'
                : false;
        ...
        const workspaceRuntimeRemoval = {
            ...
            getActivity(runtimeToDrain) {
                return {
                    ...
                    channelWorkers: channelWorkerManager?.workspaceActivity(runtimeToDrain.workspaceCwd) ?? 0,
            ...
            disposeRuntime(runtimeToDrain, reason = 'workspace_removed') {
                ...
                const cleanup = (async () => {
                    ...
                    const stopSubSessions = subSessionStoppersByWorkspace.get(runtimeToDrain.workspaceCwd);
                    try {
                        stopSubSessions?.();
                    }
                    catch {
        ...
        const app = runtime.createServeApp(opts, () => actualPort, {
            ...
            webShellDir,
            boundWorkspace,
            HopCodeVersion: resolvedCliVersion,
            startup,
            ...
            installAuthProvider: (req) => withSettingsLock(boundWorkspace, async () => {
                ...
                core.emitDaemonLog('Auth provider installed.', {
                    'hopcode.daemon.auth.provider_id': provider.id,
                    'hopcode.daemon.auth.auth_type': plan.authType,
                });
    ...
    const resolveChannelWorkspaceGroupsAtListen = () => {
        ...
        if (workspaceInputs.length === 1) {
            ...
            channelWebhookEnvByWorkspace.set(workspace.cwd, channelValidationSettingsRuntime.environment.buildRuntimeEnvironment(settings.merged, workspace.cwd, daemonRuntimeBaseEnv).effectiveEnv);
            return undefined;
        }
        const workspaces = workspaceInputs.map((workspace, index) => {
            ...
            channelWebhookEnvByWorkspace.set(workspace.cwd, channelValidationSettingsRuntime.environment.buildRuntimeEnvironment(settings.merged, workspace.cwd, daemonRuntimeBaseEnv).effectiveEnv);
            const trusted = index === 0 && deps.trustedWorkspace !== undefined
                ? deps.trustedWorkspace
                : channelValidationSettingsRuntime.trustedFolders.getWorkspaceTrustStatus(settings.merged, workspace.cwd).effective.state === 'trusted';
            return {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const onListening = () => {
            ...
            startup.runHopCodeServeToListenMs = Math.round(performance.now() - runStartedAt);
            ...
            writeStderrLine(`hopcode serve: bound to workspace ${JSON.stringify(boundWorkspace)}`);
            writeStderrLine(`hopcode serve: startup timing: processToListenMs=${startup.processToListenMs} ` +
            ...
            if (!token) {
                ...
                writeStderrLine('hopcode serve: --require-auth enabled (bearer token mandatory ' +
            ...
            const failRuntimeStartup = async (err, bridgeForCleanup) => {
                ...
                writeStderrLine(`hopcode serve: runtime startup failed: ${message}`);
            ...
            const startBridgePreheat = (bridge) => {
                ...
                bridge
                    ...
                    startup.preheat.durationMs = Math.round(performance.now() - preheatStartedAt);
                    startup.preheat.error = message;
                    writeStderrLine(`hopcode serve: ACP preheat failed, will retry on first session: ${message}`);
                });
            };
            const startRuntime = () => {
                if (runtimeStarting)
            ...
            const scheduleRuntimeStartFallback = () => {
                if (shuttingDown || runtimeStarting || runtimeStartFallbackTimer)
                    return;
                daemonLog.info(`deferred runtime: scheduling fallback start in ${HOPCODE_FAST_PATH_RUNTIME_START_FALLBACK_MS}ms`);
            ...
            startRuntimeAfterHealth = () => {
                ...
                clearRuntimeStartFallbackTimer();
                daemonLog.info(`deferred runtime: health served, scheduling start in ${HOPCODE_FAST_PATH_RUNTIME_START_AFTER_HEALTH_MS}ms`);
            ...
            const onSignal = async (signal) => {
                if (shuttingDown) {
                    ...
                    //
                    // Synchronously SIGKILL every live `hopcode --acp`
                    ...
                    closePromise = new Promise((res, rej) => {
                        ...
                        const finish = (err) => {
                            ...
                            void (coreRuntimePromise
                                ...
                                : Promise.resolve())
                                .catch((telemetryErr) => {
                                writeStderrLine(`hopcode serve: telemetry shutdown error: ${telemetryErr instanceof Error
                                ...
                                if (loggerPublished || loggerSignalOwned) {
                                    writeDaemonLifecycleBestEffort(() => daemonLog.info('daemon stopped'));
...
//# sourceMappingURL=run-hopcode-serve.js.map