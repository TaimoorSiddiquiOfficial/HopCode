...
function useTemphopcodeHome(): string {
  ...
  process.env['HOPCODE_CODE_SYSTEM_SETTINGS_PATH'] = join(
    temphopcodeHome,
    'system-settings.json',
  );
  process.env['HOPCODE_CODE_SYSTEM_DEFAULTS_PATH'] = join(
    temphopcodeHome,
...
describe('CLI entry import boundary', () => {
  ...
  it('keeps runHopCodeServe from statically loading the full server and ACP runtime', () => {
    ...
    expect(runServeSource).not.toMatch(
      /from ['"]@hopcode\/acp-bridge\/spawnChannel['"]/,
    );
...
describe('serve fast path argument parsing', () => {
  ...
  it('prints a breadcrumb when settings bootstrap falls back to the full CLI', async () => {
    ...
    expect(stderrWrites.join('')).toContain(
      'hopcode serve: fast-path bootstrap failed, falling back to full startup:',
    );
  });
...
describe('serve fast path environment bootstrap', () => {
  ...
  it('closes the listener and exits when runtime startup fails after listen', async () => {
    ...
    expect(stderrWrites.join('')).toContain(
      'hopcode serve: runtime startup failed after listener was ready: runtime boom',
    );
  ...
  it('validates rate limit env after settings bootstrap enables rate limiting', async () => {
    ...
    mkdirSync(join(tempWorkspace, '.hopcode'));
    writeFileSync(
      join(tempWorkspace, '.hopcode', 'settings.json'),
      JSON.stringify({
        env: {
          HOPCODE_SERVE_RATE_LIMIT: '1',
  ...
  it('exits when runHopCodeServe fails after settings bootstrap succeeds', async () => {
    useTemphopcodeHome();
    vi.spyOn(runHopCodeServeModule, 'runHopCodeServe').mockRejectedValue(
  ...
  it('loads HOPCODE_SERVER_TOKEN from the workspace .env before the daemon starts', async () => {
    ...
    mkdirSync(join(tempWorkspace, '.hopcode'));
    writeFileSync(
      join(tempWorkspace, '.hopcode', '.env'),
      'HOPCODE_SERVER_TOKEN=from-workspace-env\n',
    );
    process.chdir(tempWorkspace);
  ...
  it('loads .env from --workspace even when launched from another directory', async () => {
    ...
    writeFileSync(
      join(tempWorkspace, '.hopcode', '.env'),
      'HOPCODE_SERVER_TOKEN=from-explicit-workspace-env\n',
    );
    process.chdir(tempLaunchCwd);

    await bootstrapServeFastPathEnvironment(tempWorkspace);

    expect(process.env['HOPCODE_SERVER_TOKEN']).toBe(
  ...
  it('loads home .env after workspace .env for daemon boot-time keys', async () => {
    ...
    writeFileSync(
      ...
      'HOPCODE_SERVE_RATE_LIMIT_PROMPT=123\n',
    );
    writeFileSync(
  ...
  it('applies legacy excludedProjectEnvVars before loading workspace .env', async () => {
    ...
    writeFileSync(
      join(hopcodeHome, 'settings.json'),
      JSON.stringify({ excludedProjectEnvVars: ['HOPCODE_SERVER_TOKEN'] }),
    );
    tempWorkspace = realpathSync(
  ...
  it('pre-resolves home env overrides in the same order as the full loader', () => {
    ...
    writeFileSync(
      join(tempLaunchCwd, '.hopcode', '.env'),
      `HOPCODE_HOME=${temphopcodeHome}\n`,
    );
    writeFileSync(
      join(tempLaunchCwd, '.env'),
      'HOPCODE_RUNTIME_DIR=from-home-env\n',
    );
    writeFileSync(
      join(temphopcodeHome, '.env'),
      [
    ...
    expect(process.env['HOPCODE_CODE_MCP_APPROVALS_PATH']).toBe(
      'from-discovered-home',
    );
    expect(process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH']).toBe(
  ...
  it('still pre-resolves missing home-scoped keys when HOPCODE_HOME and runtime are already set', () => {
    ...
    writeFileSync(
      join(hopcodeHome, '.env'),
      'HOPCODE_CODE_TRUSTED_FOLDERS_PATH=from-existing-home\n',
    );
  ...
  it('uses trusted-folders path from home .env before loading workspace env', async () => {
    ...
    writeFileSync(
      join(hopcodeHome, '.env'),
      `HOPCODE_CODE_TRUSTED_FOLDERS_PATH=${customTrustedFoldersPath}\n`,
    );
    writeFileSync(
      join(hopcodeHome, 'settings.json'),
    ...
    writeFileSync(
      join(tempWorkspace, '.env'),
      'HOPCODE_SERVER_TOKEN=from-untrusted-workspace-env\n',
    );
  ...
  it('uses legacy folderTrust before loading workspace env', async () => {
    ...
    process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH'] = join(
      hopcodeHome,
      'trustedFolders.json',
    );
    writeFileSync(
  ...
  it('checks trust against the canonical explicit workspace path', async () => {
    ...
    writeFileSync(
      join(tempWorkspace, '.env'),
      'HOPCODE_SERVER_TOKEN=from-symlinked-untrusted-workspace-env\n',
    );
    process.chdir(tempLaunchCwd);
