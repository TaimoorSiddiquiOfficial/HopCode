...
export interface RunHopCodeServeDeps {
   ...
   * `runHopCodeServe` constructs one using `boundWorkspace`,
...
export async function runHopCodeServe(
  ...
  // daemons (`runHopCodeServe()` × 2 in the same process) would race
  ...
  //
  // Also: `runHopCodeServe` is exported and other validations
  ...
  // these for fast-fail UX, but `runHopCodeServe` is the source of
  // truth.
