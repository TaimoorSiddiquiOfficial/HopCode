...
describe('rateLimit', () => {
    ...
    describe('tier resolution', () => {
        ...
        it('exempts GET /demo', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'GET', path: '/demo' }), mockRes(), next);
            }
            expect(next).toHaveBeenCalledTimes(5);
        });
    ...
    describe('key extraction', () => {
        it('uses client-id on loopback', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'X-HopCode-Client-Id' ? 'my-client' : undefined),
            });
        ...
        it('combines IP and client-id on non-loopback', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'X-HopCode-Client-Id' ? 'web-1' : undefined),
                socket: { remoteAddress: '10.0.0.5' },
            });
        ...
        it('rejects invalid client-id', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'X-HopCode-Client-Id' ? 'bad id with spaces' : undefined),
            });
            expect(extractor(req)).toBe('anonymous');
        });
    });
