...
describe('createWorkspaceProvidersStatusProvider', () => {
    ...
    const originalSystemSettings = process.env['HOPCODE_CODE_SYSTEM_SETTINGS_PATH'];
    const originalSystemDefaults = process.env['HOPCODE_CODE_SYSTEM_DEFAULTS_PATH'];
    beforeEach(async () => {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'providers-status-'));
        workspace = path.join(tmpDir, 'workspace');
        hopcodeHome = path.join(tmpDir, 'hopcode-home');
        await fs.mkdir(workspace, { recursive: true });
        await fs.mkdir(hopcodeHome, { recursive: true });
        process.env['HOPCODE_HOME'] = hopcodeHome;
        process.env['HOPCODE_RUNTIME_DIR'] = path.join(tmpDir, 'runtime');
        process.env['HOPCODE_CODE_SYSTEM_SETTINGS_PATH'] = path.join(tmpDir, 'system-settings.json');
        process.env['HOPCODE_CODE_SYSTEM_DEFAULTS_PATH'] = path.join(tmpDir, 'system-defaults.json');
    ...
    afterEach(async () => {
        restoreEnv('HOPCODE_HOME', originalhopcodeHome);
        restoreEnv('HOPCODE_RUNTIME_DIR', originalQwenRuntimeDir);
        restoreEnv('HOPCODE_CODE_SYSTEM_SETTINGS_PATH', originalSystemSettings);
        restoreEnv('HOPCODE_CODE_SYSTEM_DEFAULTS_PATH', originalSystemDefaults);
        resetHomeEnvBootstrapForTesting();
    ...
    async function writeUserSettings(settings) {
        await fs.writeFile(path.join(hopcodeHome, 'settings.json'), JSON.stringify(settings), 'utf8');
    }
});
