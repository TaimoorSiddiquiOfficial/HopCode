...
export function createVoiceWsConnectionHandler(
  ...
  return (ws: WebSocket) => {
    if (activeSessions >= MAX_CONCURRENT_VOICE_SESSIONS) {
      writeStderrLine(
        `hopcode serve: voice websocket rejected; activeSessions=${activeSessions}`,
      );
      try {
        ws.send(
          JSON.stringify({
    ...
    activeSessions++;
    writeStderrLine(
      `hopcode serve: voice websocket accepted; activeSessions=${activeSessions}`,
    );
    let released = false;
    const releaseSlot = () => {
      if (!released) {
        ...
        activeSessions--;
        writeStderrLine(
          `hopcode serve: voice websocket slot released; activeSessions=${activeSessions}`,
        );
      }
    };
    ...
    function fail(message: string): void {
      ...
      writeStderrLine(`hopcode serve: voice websocket failed: ${message}`);
    ...
    async function finalize(): Promise<void> {
      ...
      writeStderrLine('hopcode serve: voice websocket finalized successfully');
      cleanup();
      try {
        ws.close(1000, 'done');
      } catch {
    ...
    async function handleMessage(
      ...
      switch (control.type) {
        case 'start':
          writeStderrLine('hopcode serve: voice websocket start received');
          await ensureStarted();
          return;
        case 'stop':
          writeStderrLine('hopcode serve: voice websocket stop received');
          await finalize();
          return;
        default:
          return;
    ...
    ws.on('message', (data: RawData, isBinary: boolean) => {
      ...
      if (!isBinary) {
        ...
        if (control?.type === 'abort') {
          writeStderrLine('hopcode serve: voice websocket abort received');
          cleanup();
          try {
