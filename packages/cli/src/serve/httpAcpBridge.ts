...
// `@hoptrendy/hopcode-acp-bridge/bridgeTypes` in #4175 PR 22b. Imported AND
...
import type {
  ...
  HttpAcpBridge,
} from '@hoptrendy/hopcode-acp-bridge/bridgeTypes';
export type {
  BridgeSpawnRequest,
  BridgeSession,
  BridgeRestoreSessionRequest,
...
// Bridge errors lifted to `@hoptrendy/hopcode-acp-bridge/bridgeErrors` in
// #4175 PR 22b. `MAX_WORKSPACE_PATH_LENGTH` lifted to
// `@hoptrendy/hopcode-acp-bridge/workspacePaths` in the same slice.
...
} from '@hoptrendy/hopcode-acp-bridge/bridgeErrors';
import { MAX_WORKSPACE_PATH_LENGTH } from '@hoptrendy/hopcode-acp-bridge/workspacePaths';
export {
  SessionNotFoundError,
  RestoreInProgressError,
  InvalidSessionScopeError,
...
// `@hoptrendy/hopcode-acp-bridge` in #4175 PR 22a so `channels/base/AcpBridge.ts`
...
import type {
  ...
  AcpChannelExitInfo,
  ChannelFactory,
} from '@hoptrendy/hopcode-acp-bridge';
...
} from '@hoptrendy/hopcode-acp-bridge/bridgeOptions';
...
// `@hoptrendy/hopcode-acp-bridge/bridgeErrors` in #4175 PR 22b — see
...
// `@hoptrendy/hopcode-acp-bridge/bridgeErrors` in #4175 PR 22b — see the
...
class BridgeClient implements Client {
  ...
  private bufferEarlyEvent(
    ...
    if (
      ...
      writeStderrLine(
        `hopcode serve: dropping mcp guardrail extNotification ` +
...
export function createHttpAcpBridge(opts: BridgeOptions): HttpAcpBridge {
  ...
  /**
   * Get-or-create the daemon's single `hopcode --acp` channel (#3803 §02).
  ...
  return {
    ...
    async closeSession(sessionId, context) {
      ...
      if (ci && ci.sessionIds.size === 0 && ci.pendingRestoreIds.size === 0) {
        ...
        await ci.channel.kill().catch((err) => {
          writeStderrLine(
            `hopcode serve: closeSession channel kill failed for session ` +
    ...
    publishWorkspaceEvent(event) {
      ...
      // HOPCODE_SERVE_DEBUG would let a true regression succeed at the
    ...
    async getWorkspaceEnvStatus() {
      ...
      try {
        ...
        writeStderrLine(
          `hopcode serve: statusProvider.getEnvStatus failed; ` +
    ...
    async getWorkspacePreflightStatus() {
      ...
      if (!opts.statusProvider) {
        ...
        try {
          ...
          writeStderrLine(
            `hopcode serve: statusProvider.getDaemonPreflightCells failed; ` +
    ...
    async setSessionApprovalMode(sessionId, mode, opts, context) {
      // #4175 Wave 4 PR 17. Forwards through `hopcode/control/session/
      ...
      // #4282 fold-in 4 (hopcode-latest C1): validate the persist contract
      ...
      // #4282 fold-in 4 (hopcode-latest S2): when the change is persisted to
    ...
    async initWorkspace(initOpts, originatorClientId) {
      // #4175 Wave 4 PR 17. Mechanical scaffold of an empty `HOPCODE.md`
...
export const defaultSpawnChannelFactory: ChannelFactory = async (
  ...
  // Resolution order:
  //   1. `HOPCODE_CLI_ENTRY` env override — escape hatch for non-standard
  ...
  // `ANTHROPIC_API_KEY` / `HOPCODE_*` / `DASHSCOPE_API_KEY` / a custom
  ...
  // `HOPCODE_SERVER_TOKEN` in overrides — defense in depth).
