 ...
 */
import { cacheHopCodeCredentials, generatePKCEPair, isDeviceAuthorizationSuccess, isDeviceTokenPending, isDeviceTokenSuccess, HopCodeOAuth2Client, HopCodeOAuthPollError, } from '@hoptrendy/hopcode-core';
 ...
 * audit. The raw `err.message` from `HopCodeOAuth2Client` can embed the
 ...
 * a URL.
 */
export class HopCodeOAuthDeviceFlowProvider {
    providerId = 'hopcode-oauth';
    client;
    constructor(client) {
        this.client = client ?? new HopCodeOAuth2Client();
    }
    async start(opts) {
        ...
        try {
            ...
            writeStderrLine(`[serve] hopcode device-flow start failed (raw): ${truncateForStderr(detail)}`);
            throw new UpstreamDeviceFlowError('HopCode IdP device authorization request failed');
        }
        ...
        if (!isDeviceAuthorizationSuccess(auth)) {
            ...
            writeStderrLine(truncateForStderr(`[serve] hopcode device-flow start error envelope (raw): error=${errorData?.error ?? 'unknown'} description=${errorData?.error_description ?? '(none)'}`));
            throw new UpstreamDeviceFlowError('HopCode IdP rejected the device authorization request');
        }
        return {
    ...
    async poll(state, opts) {
        ...
        try {
            ...
            const errorKind = err instanceof HopCodeOAuthPollError
            ...
            if (!aborted) {
                let safeDetail;
                if (err instanceof HopCodeOAuthPollError) {
                ...
                writeStderrLine(`[serve] hopcode device-flow poll failed (errorKind=${errorKind}): ${truncateForStderr(safeDetail)}`);
            }
            return {
                kind: 'error',
                errorKind,
        ...
        if (isDeviceTokenSuccess(response)) {
            ...
            return {
                ...
                // `cacheHopCodeCredentials({signal})` which forwards to
                ...
                async persist(persistOpts) {
                    // Order matters: write to disk FIRST. If `cacheHopCodeCredentials`
                    ...
                    // The HopCode IdP token response doesn't carry an
 ...
 * `HopCodeOAuthPollError.oauthError`) to the registry's
...
//# sourceMappingURL=hopcode-device-flow-provider.js.map