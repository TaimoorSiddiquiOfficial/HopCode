...
import { HopCodeOAuthPollError, } from '@hoptrendy/hopcode-core';
...
describe('HopCodeOAuthDeviceFlowProvider.poll() â€” stderr audit branches', () => {
    ...
    it('logs only err.name + message length on generic Error (raw message is suppressed)', async () => {
        ...
        const longMessage = 'HTTP 502 from HopCode IdP: <html><body>Forbidden â€” request body: device_code=device-code-secret-AAAA1111&code_verifier=pkce-verifier-secret-BBBB2222</body></html>';
        const provider = new HopCodeOAuthDeviceFlowProvider(fakeClient({
            pollDeviceToken: async () => {
    ...
    it('sanitizes control characters in attacker-controlled err.name on the non-OAuth path (round-4 #4)', async () => {
        // PR #4291 follow-up review (hopcode-latest, round-4 #4):
    ...
    it('sanitizes Unicode bidi ISOLATE controls U+2066–U+2069 (CVE-2021-42574 Trojan Source) (round-6 #5)', async () => {
        // Round-6 review (hopcode-latest, #5): the round-5 regex covered
...
//# sourceMappingURL=hopcode-device-flow-provider.test.js.map