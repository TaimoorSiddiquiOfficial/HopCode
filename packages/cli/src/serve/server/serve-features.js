...
export function createServeFeatures(deps) {
    const { opts, boundWorkspace, persistSettingAvailable, sessionArtifactsPersistenceAvailable, sessionGenerationAvailable, reloadAvailable, channelReloadAvailable, channelControlAvailable, sessionShellCommandEnabled, multiWorkspaceSessionsEnabled, persistentWorkspaceRegistrationAvailable, workspaceRuntimeRemovalAvailable, } = deps;
    ...
    const getCachedVoiceTranscriptionAvailable = () => {
        cachedVoiceTranscriptionAvailable ??=
            isWorkspaceVoiceTranscriptionAvailable(boundWorkspace, env, deps.env !== undefined);
        return cachedVoiceTranscriptionAvailable;
    };
    return {
        languageCodes: SERVE_LANGUAGE_CODES,
        invalidateServeFeaturesCache,
        currentServeFeatures: () => getAdvertisedServeFeatures(undefined, {
...
function isWorkspaceVoiceTranscriptionAvailable(boundWorkspace, env, skipLoadEnvironment) {
    try {
        return hasConfiguredBatchVoiceTranscriptionModel(loadSettings(boundWorkspace, { skipLoadEnvironment }), { env });
    }
    catch (err) {
        writeStderrLine(`hopcode serve: workspace voice transcription capability check failed: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
}
