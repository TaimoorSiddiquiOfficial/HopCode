 ...
 */
import { emitDaemonLog, InvalidSessionTranscriptCursorError, recordDaemonBridgeError, recordDaemonError, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, TrustGateError, } from '@hoptrendy/hopcode-core';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { BranchWhilePromptActiveError, CancelSentinelCollisionError, CdWhilePromptActiveError, InvalidClientIdError, InvalidPermissionOptionError, InvalidRewindTargetError, InvalidSessionMetadataError, InvalidSessionScopeError, McpServerNotFoundError, McpServerRestartFailedError, PermissionForbiddenError, PermissionPolicyNotImplementedError, PromptQueueFullError, RestoreInProgressError, SessionArtifactAuthorizationError, SessionArchivedError, SessionArchivingError, SessionBusyError, SessionConflictError, SessionLimitExceededError, SessionNotArchivedError, SessionNotFoundError, SessionShellClientRequiredError, SessionShellDisabledError, WorkspaceInitConflictError, WorkspaceInitPathEscapeError, WorkspaceInitRaceError, WorkspaceInitSymlinkError, WorkspaceMismatchError, WorkspaceDrainingError, TotalSessionLimitExceededError, } from '../acp-session-bridge.js';
import { WorkspaceSkillNotFoundError, WorkspaceSkillNotToggleableError, } from '../workspace-service/types.js';
...
export function sendBridgeError(res, err, ctx, daemonLog) {
    ...
    if (err instanceof WorkspaceSkillNotToggleableError) {
        res.status(409).json({
            error: err.message,
            code: err.reason === 'inactive_extension'
    ...
    if (daemonLog) {
        ...
        writeStderrLine(`hopcode serve: bridge error${ctxStr}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
    }
