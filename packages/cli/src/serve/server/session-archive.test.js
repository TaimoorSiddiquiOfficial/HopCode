...
import { SessionService, Storage, readCronTasks, updateCronTasks, } from '@hoptrendy/hopcode-core';
...
import { archiveDaemonSessions, assertSessionArchived, assertSessionLoadable, deleteDaemonSessions, SessionArchiveCoordinator, unarchiveDaemonSessions, } from './session-archive.js';
...
function writeSessionFile(workspaceDir, sessionId, state, recordCwd = workspaceDir) {
...
function sessionPath(workspaceDir, sessionId, state) {
    const chatsDir = path.join(new Storage(workspaceDir).getProjectDir(), 'chats');
