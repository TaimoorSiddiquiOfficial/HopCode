...
export async function deleteDaemonSessions(params) {
    ...
    await removeTasksForSessions(service.getProjectRoot(), removed).catch((err) => {
        logSessionArchiveWarning(`removeTasksForSessions failed for [${removed.join(', ')}]: ${err instanceof Error ? err.message : String(err)}`);
    });
    return { removed, notFound, errors: [...closeErrors, ...removeErrors] };
}
export async function assertSessionLoadable(workspaceCwd, sessionId) {
    const location = await new SessionService(workspaceCwd).getSessionLocation(sessionId);
...
export async function assertSessionArchived(workspaceCwd, sessionId) {
    const location = await new SessionService(workspaceCwd).getSessionLocation(sessionId);
    if (location === 'active') {
...
async function classifySessionLocations(service, sessionIds) {
    const result = {
        active: [],
        archived: [],
        notFound: [],
    ...
    const locationResults = await Promise.allSettled(sessionIds.map(async (sessionId) => ({
        sessionId,
...
export async function archiveDaemonSessions(params) {
    ...
    if (activeIds.length > 0) {
        await coordinator.runExclusiveMany(activeIds, async () => {
            ...
            const closeResults = await Promise.allSettled(closableIds.map(async (sessionId) => {
                try {
    ...
    await disableTasksForSessions(service.getProjectRoot(), archived).catch((err) => {
        logSessionArchiveWarning(`disableTasksForSessions failed for [${archived.join(', ')}]: ${err instanceof Error ? err.message : String(err)} — bound tasks may keep firing until reconciled`);
    });
...
export async function unarchiveDaemonSessions(params) {
    ...
    try {
        ...
        logSessionArchiveWarning(`enableTasksForSessions failed for [${resumeSessionIds.join(', ')}]: ${err instanceof Error ? err.message : String(err)}`);
