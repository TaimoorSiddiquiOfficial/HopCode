...
export const INVALID_PERMISSION_OUTCOME_ERROR = '`outcome` must be `{ outcome: "cancelled" }` or `{ outcome: "selected", optionId: string }`';
...
export function parseMaxQueuedQuery(raw, res) {
    ...
    if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
        ...
        // `sendBridgeError`.
        writeStderrLine(`hopcode serve: rejected ?maxQueued ${safeLogValue(raw)} ` +
...
export function parseLastEventId(raw) {
    ...
    if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
        ...
        if (typeof raw === 'string' && raw.length > 0) {
            writeStderrLine(`hopcode serve: rejected Last-Event-ID ${safeLogValue(raw)} ` +
                `(not a decimal integer)`);
        }
        return undefined;
    }
