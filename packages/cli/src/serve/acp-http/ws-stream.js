...
export class WsStream {
    ...
    constructor(ws, onClose, onHeartbeat) {
        ...
        ws.on('error', (err) => {
            writeStderrLine(`hopcode serve: /acp WS error: ${err instanceof Error ? err.message : String(err)}`);
            this.close();
        });
        let alive = true;
        ws.on('pong', () => {
    ...
    send(message, _id) {
        ...
        this.writeChain = next.catch((err) => {
            if (!this._closed) {
                writeStderrLine(`hopcode serve: /acp WS write failed: ${err instanceof Error ? err.message : String(err)}`);
                this.close();
            }
        });
    ...
    close() {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: /acp WS onClose threw: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
}
