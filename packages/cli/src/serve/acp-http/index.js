...
import { isPortableAbsolutePath, resolveManagedWorkspaceRuntimeFromParam, resolveManagedWorkspaceRuntimeByPathSelector, } from '../workspace-route-runtime.js';
...
function isActiveDrainCorrelation(registry, conn, message) {
    ...
    const params = message.params !== null && typeof message.params === 'object'
        ? message.params
        : undefined;
    return (typeof params?.sessionId === 'string' && conn.ownsSession(params.sessionId));
}
...
export function mountAcpHttp(app, bridge, opts) {
    ...
    (sessionId, clientId) => {
        void bridge.detachClient(sessionId, clientId).catch((err) => {
            writeStderrLine(`hopcode serve: /acp detachClient(${sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }, opts.maxConnections);
    ...
    const dispatcher = new AcpDispatcher(bridge, opts.boundWorkspace, primaryEnv, opts.workspace, opts.workspaceRememberLane, opts.fsFactory, opts.deviceFlowRegistry, opts.sessionShellCommandEnabled === true, registry, opts.archiveCoordinator ?? new SessionArchiveCoordinator());
    ...
    const handleAcpPost = async (mount, req, res) => {
        ...
        if (opts.checkRate && isRequest(message)) {
            ...
            if (!WS_EXEMPT_METHODS.has(m)) {
                const tier = m === 'session/prompt' || m === '_hopcode/session/prompt'
                    ? 'prompt'
    ...
    const createSecondaryAcpMount = (rt) => {
        ...
        const secondaryDispatcher = new AcpDispatcher(rt.bridge, rt.workspaceCwd, runtimeEffectiveEnv(rt, daemonEnv), rt.workspaceService, workspaceRememberLane, rt.routeFileSystemFactory, 
        ...
        opts.deviceFlowRegistry, opts.sessionShellCommandEnabled === true, secondaryRegistry, opts.archiveCoordinator ?? new SessionArchiveCoordinator());
    ...
    const resolvePluralMount = (req, res) => {
        ...
        const rt = resolveManagedWorkspaceRuntimeFromParam(workspaceRegistry, req, res);
        if (!rt)
            return null;
        if (!rt.primary && !rt.trusted) {
            res.status(403).json({
    ...
    function setupWebSocket(httpServer) {
        ...
        upgradeListener = (req, socket, head) => {
            ...
            const logReject = (reason) => {
                writeStderrLine(`hopcode serve: WebSocket upgrade rejected (${reason}) from ${rawAddr}`);
            };
