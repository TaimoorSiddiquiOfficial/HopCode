...
export class AcpConnection {
  ...
  destroy(): void {
    ...
    for (const binding of this.sessions.values()) {
      try {
        ...
        writeStderrLine(
          `hopcode serve: /acp teardownBinding(${logSafe(binding.sessionId)}) failed during destroy: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
    this.sessions.clear();
  ...
  private abandonPendingForSession(
    ...
    for (const [id, req] of this.pending) {
      ...
      if (!cancelled) {
        writeStderrLine(
          `hopcode serve: /acp MEDIATOR STUCK: abandonPendingForSession(${logSafe(sessionId)}) cancel failed for ${logSafe(req.bridgeRequestId)}`,
        );
      }
    }
  }
...
function pushCapped(buf: unknown[], frame: unknown, label = 'stream'): void {
  if (buf.length >= MAX_BUFFERED_FRAMES) {
    buf.shift();
    writeStderrLine(
      `hopcode serve: /acp pre-attach buffer full (${label}), dropped oldest frame`,
    );
  }
  buf.push(frame);
}
...
export class ConnectionRegistry {
  ...
  private sweep(): void {
    ...
    for (const [id, conn] of this.byId) {
      ...
      writeStderrLine(
        `hopcode serve: /acp reaping idle connection ${id} ` +
