...
function makeBridge() {
    return {
        spawnOrAttach: vi.fn(async (req) => ({
            sessionId: req.workspaceCwd === '/ws-b' ? 'secondary-session' : 'primary-session',
...
async function writeStoredSession(sessionId, cwd) {
...
async function withRuntimeDir(fn) {
    ...
    const runtimeDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-workspace-qualified-acp-'));
