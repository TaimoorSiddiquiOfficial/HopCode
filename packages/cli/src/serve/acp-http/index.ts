...
export function mountAcpHttp(
  ...
  const enabled = opts.enabled ?? process.env['HOPCODE_SERVE_ACP_HTTP'] !== '0';
  ...
  app.post(path, async (req: Request, res: Response) => {
    ...
    if (!parsed.ok) {
      writeStderrLine(
        `hopcode serve: /acp malformed request from ${req.socket?.remoteAddress}: ${parsed.error.error.message}`,
      );
    ...
    if (isRequest(message) && message.method === 'initialize') {
      ...
      if (!conn) {
        ...
        writeStderrLine(
          `hopcode serve: /acp connection cap reached (max=${registry.connectionCap}), rejecting initialize`,
        );
      ...
      writeStderrLine(
        `hopcode serve: /acp connection established ${conn.connectionId.slice(0, 8)} ` +
    ...
    await dispatcher
      ...
      .catch((err: unknown) => {
        writeStderrLine(
          `hopcode serve: /acp handle error: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
      });
  ...
  app.get(path, (req: Request, res: Response) => {
    ...
    if (!sessionId) {
      ...
      const stream = new SseStream(
        ...
        () => {
          writeStderrLine(
            `hopcode serve: /acp connection stream closed (${connId.slice(0, 8)})`,
          );
          ...
          const reapConnIfDead = () => {
            if (
              ...
              writeStderrLine(
                `hopcode serve: /acp reaping connection ${connId.slice(0, 8)} (conn stream gone, no live session stream)`,
              );
    ...
    void dispatcher
      ...
      .then(onPumpSettled, (err: unknown) => {
        writeStderrLine(
          `hopcode serve: /acp event pump error (${sessionId}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        onPumpSettled();
  ...
  app.delete(path, (req: Request, res: Response) => {
    ...
    if (existed) {
      writeStderrLine(
        `hopcode serve: /acp connection deleted ${connectionId.slice(0, 8)} (remaining=${registry.size})`,
      );
    }
    res.status(202).end();
  });
    ...
    upgradeListener = (req: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      wss!.handleUpgrade(req, socket, head, (ws: WebSocket) => {
        ...
        const initTimer = setTimeout(() => {
          if (!initialized) {
            writeStderrLine(
              `hopcode serve: /acp WS initialize timeout (30s) from ${rawAddr}`,
            );
        ...
        ws.on('error', (err) => {
          writeStderrLine(
            `hopcode serve: /acp WS error: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
        ...
        ws.on('message', (rawData: Buffer | string) => {
          messageQueue = messageQueue
            ...
            .catch((err) => {
              writeStderrLine(
                `hopcode serve: /acp WS message handler error: ${err instanceof Error ? err.message : String(err)}`,
              );
            });
        });
        ...
        async function handleWsMessage(
          ...
          if (!initialized) {
            ...
            const stream = new WsStream(
              ...
              () => {
                writeStderrLine(
                  `hopcode serve: /acp WS closed (${conn.connectionId.slice(0, 8)})`,
                );
            ...
            connRef = conn;
            writeStderrLine(
              `hopcode serve: /acp WS established ${conn.connectionId.slice(0, 8)} (loopback=${fromLoopback}, active=${registry.size})`,
            );
              ...
              if (
                ...
                void dispatcher
                  ...
                  .then(cleanupSession, (err: unknown) => {
                    writeStderrLine(
                      `hopcode serve: /acp WS pump error (${sid}): ${err instanceof Error ? err.message : String(err)}`,
                    );
                    cleanupSession();
                  });
              }
          ...
          const dispatchP = dispatcher
            ...
            .catch((err: unknown) => {
              writeStderrLine(
                `hopcode serve: /acp WS handle error: ${err instanceof Error ? err.message : String(err)}`,
              );
            });
