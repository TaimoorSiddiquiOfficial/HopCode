...
import { APPROVAL_MODES, BTW_MAX_INPUT_LENGTH, GROUP_COLOR_OPTIONS, SessionService, SessionOrganizationError, BuiltinAgentRegistry, IMAGE_CAPABILITY, SubagentError, WorkspaceMemoryFileTooLargeError, WorkspaceMemoryWriteTimeoutError, writeWorkspaceContextFile, } from '@hoptrendy/hopcode-core';
...
import { CancelSentinelCollisionError, InvalidPermissionOptionError, PermissionForbiddenError, PermissionPolicyNotImplementedError, SessionArchivingError, SessionShellClientRequiredError, SessionShellDisabledError, WorkspaceMismatchError, } from '@hoptrendy/acp-bridge/bridgeErrors';
import { SessionArtifactAuthorizationError, SessionArtifactValidationError, } from '@hoptrendy/acp-bridge/sessionArtifacts';
...
import { MAX_READ_BYTES, } from '../fs/index.js';
...
import { InvalidCursorError, listWorkspaceSessionsForResponse, } from '../server.js';
import { createSessionOrganizationService } from '../session-organization-helpers.js';
import { assertSessionLoadable, logSessionArchiveWarning, SessionArchiveCoordinator, unarchiveDaemonSessions, } from '../server/session-archive.js';
import { WorkspacePermissionRulesSessionRequiredError, WorkspaceSettingsPartialPersistError, } from '../workspace-service/types.js';
import { HOPCODE_META_KEY, HOPCODE_METHOD_NS, RPC, error, isNotification, isObject, isRequest, isResponse, logSafe, notification, request, success, } from './json-rpc.js';
...
import { resolveSetupGithubProxy, createSetupGithubFileOps, setupGithubEventData, sanitizeSetupGithubMessage, sanitizeSetupGithubResult, } from '../routes/workspace-setup-github.js';
...
const ALL_HOPCODE_VENDOR_METHODS = [
    ...
    // Wave 1: files
    `${HOPCODE_METHOD_NS}file/read`,
    `${HOPCODE_METHOD_NS}file/read_bytes`,
    `${HOPCODE_METHOD_NS}file/stat`,
    `${HOPCODE_METHOD_NS}file/list`,
    `${HOPCODE_METHOD_NS}file/glob`,
    `${HOPCODE_METHOD_NS}file/write`,
    `${HOPCODE_METHOD_NS}file/edit`,
    // Wave 1: auth
    `${HOPCODE_METHOD_NS}workspace/auth/status`,
    `${HOPCODE_METHOD_NS}workspace/auth/device_flow/start`,
    `${HOPCODE_METHOD_NS}workspace/auth/device_flow/get`,
    `${HOPCODE_METHOD_NS}workspace/auth/device_flow/cancel`,
    // Wave 1: remaining workspace
    `${HOPCODE_METHOD_NS}workspace/tools`,
    `${HOPCODE_METHOD_NS}workspace/mcp/tools`,
    `${HOPCODE_METHOD_NS}workspace/mcp/servers/add`,
    `${HOPCODE_METHOD_NS}workspace/mcp/servers/remove`,
    `${HOPCODE_METHOD_NS}sessions/delete`,
    // Wave 2: agents
    `${HOPCODE_METHOD_NS}workspace/agents/list`,
    `${HOPCODE_METHOD_NS}workspace/agents/get`,
    `${HOPCODE_METHOD_NS}workspace/agents/create`,
    `${HOPCODE_METHOD_NS}workspace/agents/update`,
    `${HOPCODE_METHOD_NS}workspace/agents/delete`,
];
function advertisedQwenVendorMethods(sessionShellCommandEnabled) {
    return ALL_HOPCODE_VENDOR_METHODS.filter((method) => sessionShellCommandEnabled || method !== SESSION_SHELL_METHOD);
}
/**
...
function parsePermissionResponse(params) {
    ...
    if (answers !== undefined) {
        if (isObject(answers) &&
            ...
            writeStderrLine('hopcode serve: /acp session/permission dropping invalid `answers` (expected an object map of string values)');
        }
    }
...
export class AcpDispatcher {
    ...
    archiveCoordinator;
    agentManager;
    constructor(bridge, boundWorkspace, env, workspace, _workspaceRememberLane, fsFactory, deviceFlowRegistry, sessionShellCommandEnabled = false, registry, archiveCoordinator = new SessionArchiveCoordinator()) {
        this.bridge = bridge;
    ...
    killOrphanSession(sessionId, removePersistedSession = false) {
        void this.bridge
            .killSession(sessionId, { requireZeroAttaches: true })
            .then(async (killed) => {
            if (killed && removePersistedSession) {
                await new SessionService(this.boundWorkspace).removeSession(sessionId);
            }
        })
            .catch((err) => writeStderrLine(`hopcode serve: /acp orphan killSession(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`));
    }
    /**
     ...
     */
    async configOptionsFor(sessionId) {
        try {
            const ctx = (await this.bridge.getSessionContextStatus(sessionId));
            const co = ctx?.state?.configOptions;
            return Array.isArray(co) ? co : undefined;
        }
        catch (err) {
            writeStderrLine(`hopcode serve: /acp configOptionsFor(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`);
            return undefined;
        }
    }
    /**
    ...
    cancelAbandonedPermission(req, clientId) {
        try {
            ...
            if (/not found|unknown session/i.test(msg))
                return true;
            writeStderrLine(`hopcode serve: /acp cancelAbandonedPermission(${logSafe(req.sessionId)}) failed: ${logSafe(msg)}`);
            return false;
        }
    }
    /**
    ...
    buildInitializeResult(connectionId, requestedVersion) {
        ...
        return {
            ...
            agentCapabilities: {
                ...
                // feature-detect before calling `_hopcode/…` methods.
                _meta: {
                    [HOPCODE_META_KEY]: {
                        connectionId,
                        workspaceCwd: this.boundWorkspace,
    ...
    requireOwned(conn, sessionId, id) {
        ...
        if (id === undefined) {
            ...
            writeStderrLine(`hopcode serve: /acp notification for unowned session ${logSafe(sessionId)} (dropped)`);
            return false;
        }
    ...
    async handle(conn, msg, sessionHeader, reqLoopback) {
        ...
        if (isResponse(msg)) {
            try {
                ...
                writeStderrLine(`hopcode serve: /acp response handling error: ${logSafe(errMsg(err))}`);
            }
            return;
        }
        ...
        try {
            switch (method) {
                ...
                case 'session/resume': {
                    ...
                    const restored = await this.archiveCoordinator.runSharedMany([sessionId], async () => {
                        ...
                        const metadata = await new SessionService(cwd).readCreationMetadata(sessionId);
                    ...
                    if (conn.destroyed || closeRaced) {
                        ...
                        void cleanup.catch((err) => writeStderrLine(`hopcode serve: /acp orphan ${restored.attached ? 'detach' : 'kill'}(${logSafe(sessionId)}) teardown-race: ${logSafe(errMsg(err))}`));
                ...
                case 'session/list': {
                    ...
                    const result = await listWorkspaceSessionsForResponse(this.bridge, workspaceCwd, {
                        cursor,
                        size: metaSize,
                        archiveState,
                        view,
                ...
                case 'session/close': {
                    ...
                    const closeSession = async () => {
                        ...
                        try {
                            ...
                            writeStderrLine(`hopcode serve: /acp session/close local teardown failed (${logSafe(sessionId)}): ${logSafe(errMsg(teardownErr))}`);
                        }
                        finally {
                ...
                case 'session/permission': {
                    ...
                    if (!requestId) {
                        ...
                        writeStderrLine(`hopcode serve: /acp session/permission rejected: \`requestId\` is required`);
                        if (id !== undefined) {
                    ...
                    try {
                        ...
                        if (err instanceof AcpParamError) {
                            writeStderrLine(`hopcode serve: /acp session/permission invalid params (requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                    ...
                    if (sessionIdParam &&
                        ...
                        writeStderrLine(`hopcode serve: /acp session/permission vote rejected: requestId ${logSafe(requestId)} does not belong to session ${logSafe(sessionIdParam)}`);
                        if (id !== undefined) {
                    ...
                    if (this.registry && !pendingRef) {
                        writeStderrLine(`hopcode serve: /acp session/permission vote dropped: no pending request for requestId ${logSafe(requestId)}`);
                        if (id !== undefined) {
                    ...
                    // this connection).
                    if (!conn.ownsSession(sessionId)) {
                        writeStderrLine(`hopcode serve: /acp session/permission vote rejected: session ${logSafe(sessionId)} not owned by this connection (requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                    ...
                    try {
                        ...
                        if (err instanceof InvalidPermissionOptionError) {
                            writeStderrLine(`hopcode serve: /acp session/permission invalid option (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 400,
                        ...
                        if (err instanceof PermissionForbiddenError) {
                            writeStderrLine(`hopcode serve: /acp session/permission forbidden (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 403,
                        ...
                        if (err instanceof PermissionPolicyNotImplementedError) {
                            ...
                            writeStderrLine(`hopcode serve: /acp session/permission policy not implemented (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                        ...
                        if (err instanceof CancelSentinelCollisionError) {
                            ...
                            writeStderrLine(`hopcode serve: /acp session/permission cancel-sentinel collision (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INTERNAL_ERROR, err.message, {
                                    httpStatus: 500,
                        ...
                        // error for the wire.
                        writeStderrLine(`hopcode serve: /acp session/permission vote failed (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(errMsg(err))}`);
                    ...
                    if (!accepted) {
                        ...
                        writeStderrLine(`hopcode serve: /acp session/permission vote not accepted by bridge (${logSafe(sessionId)}, requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                    ...
                    writeStderrLine(`hopcode serve: /acp session/permission vote accepted (${logSafe(sessionId)}, requestId ${logSafe(requestId)}, connection ${logSafe(conn.connectionId.slice(0, 8))})`);
                    this.replyConn(conn, id, {});
                    return;
                }
                ...
                case 'session/set_config_option': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (configId === 'model') {
                            ...
                            if (id !== undefined) {
                                this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, `Unknown configId: ${configId}`));
                            }
                            return;
                        }
                ...
                case `${HOPCODE_METHOD_NS}session/context`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                ...
                case `${HOPCODE_METHOD_NS}session/supported_commands`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                ...
                case `${HOPCODE_METHOD_NS}session/update_metadata`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        const metadata = isObject(params['metadata'])
                            ? params['metadata']
                            : {};
                        const result = this.bridge.updateSessionMetadata(sessionId, metadata, this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                ...
                case `${HOPCODE_METHOD_NS}workspace/mcp`:
                    this.replyConn(conn, id, await this.workspace.getWorkspaceMcpStatus(this.wsCtx(conn, method)));
                    return;
                case `${HOPCODE_METHOD_NS}workspace/skills`:
                    this.replyConn(conn, id, await this.workspace.getWorkspaceSkillsStatus(this.wsCtx(conn, method)));
                    return;
                case `${HOPCODE_METHOD_NS}workspace/providers`:
                    this.replyConn(conn, id, await this.workspace.getWorkspaceProvidersStatus(this.wsCtx(conn, method)));
                    return;
                case `${HOPCODE_METHOD_NS}workspace/env`:
                    this.replyConn(conn, id, await this.workspace.getWorkspaceEnvStatus(this.wsCtx(conn, method)));
                    return;
                case `${HOPCODE_METHOD_NS}workspace/preflight`:
                    this.replyConn(conn, id, await this.workspace.getWorkspacePreflightStatus(this.wsCtx(conn, method)));
                    return;
                case `${HOPCODE_METHOD_NS}workspace/init`: {
                ...
                case `${HOPCODE_METHOD_NS}workspace/setup-github`: {
                    ...
                    try {
                        const result = await setupGithub({
                            ...
                            fileOps: createSetupGithubFileOps(this.fsFactory, `ACP ${method}`, conn.clientId),
                        });
                ...
                case `${HOPCODE_METHOD_NS}workspace/restart_mcp_server`: {
                ...
                case `${HOPCODE_METHOD_NS}session/shell`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        writeStderrLine(`hopcode serve: /acp session/shell session=${logSessionId} client=${logClientId} cmd=${logCommand}`);
                ...
                case `${HOPCODE_METHOD_NS}session/context_usage`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                ...
                case `${HOPCODE_METHOD_NS}session/tasks`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                ...
                case `${HOPCODE_METHOD_NS}workspace/memory/write`: {
                    ...
                    writeStderrLine(`hopcode serve: /acp workspace/memory/write scope=${scope} mode=${mode} client=${conn.clientId?.slice(0, 8)} bytes=${Buffer.byteLength(content, 'utf8')}`);
                ...
                case `${HOPCODE_METHOD_NS}file/read`: {
                ...
                case `${HOPCODE_METHOD_NS}file/read_bytes`: {
                    if (!this.fsFactory) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'File system not configured'));
                        return;
                ...
                case `${HOPCODE_METHOD_NS}file/stat`: {
                    if (!this.fsFactory) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'File system not configured'));
                        return;
                ...
                case `${HOPCODE_METHOD_NS}file/list`: {
                    if (!this.fsFactory) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'File system not configured'));
                        return;
                ...
                case `${HOPCODE_METHOD_NS}file/glob`: {
                ...
                case `${HOPCODE_METHOD_NS}file/write`: {
                    const p = String(params['path'] ?? '');
                    if (!p) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`path` required'));
                ...
                case `${HOPCODE_METHOD_NS}file/edit`: {
                    const p = String(params['path'] ?? '');
                    if (!p) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`path` required'));
                ...
                case `${HOPCODE_METHOD_NS}workspace/auth/status`: {
                ...
                case `${HOPCODE_METHOD_NS}workspace/auth/device_flow/start`: {
                    if (!this.deviceFlowRegistry) {
                        if (id !== undefined)
                ...
                case `${HOPCODE_METHOD_NS}workspace/auth/device_flow/get`: {
                    if (!this.deviceFlowRegistry) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'Device flow not configured'));
                        return;
                ...
                case `${HOPCODE_METHOD_NS}workspace/auth/device_flow/cancel`: {
                    if (!this.deviceFlowRegistry) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'Device flow not configured'));
                        return;
                ...
                case `${HOPCODE_METHOD_NS}workspace/tools`: {
                    const result = await this.bridge.getWorkspaceToolsStatus();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${HOPCODE_METHOD_NS}workspace/mcp/tools`: {
                    const serverName = String(params['serverName'] ?? '');
                    if (!serverName) {
                        if (id !== undefined)
                ...
                case `${HOPCODE_METHOD_NS}workspace/mcp/servers/add`: {
                ...
                case `${HOPCODE_METHOD_NS}workspace/mcp/servers/remove`: {
                    const name = String(params['name'] ?? '');
                    if (!name || name.length > MAX_NAME_LENGTH) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`name\` required, max ${MAX_NAME_LENGTH} chars`));
                ...
                case `${HOPCODE_METHOD_NS}sessions/delete`: {
                    const sessionIds = params['sessionIds'];
                    if (!Array.isArray(sessionIds) ||
                        sessionIds.length === 0 ||
                        sessionIds.length > 100 ||
                        !sessionIds.every((s) => typeof s === 'string')) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`sessionIds` must be non-empty string array (max 100)'));
                        return;
                    }
                    ...
                    await Promise.allSettled(ids.map(async (sid) => {
                        try {
                            ...
                            if (err instanceof Error &&
                                ...
                                writeStderrLine(`hopcode serve: /acp sessions/delete closeSession(${safeSessionId}) failed: ${safeMessage}`);
                    ...
                    for (const e of removeResult.errors) {
                        ...
                        writeStderrLine(`hopcode serve: /acp sessions/delete removeSessions(${safeSessionId}) failed: ${safeMessage}`);
                    }
                    this.replyConn(conn, id, {
                ...
                case `${HOPCODE_METHOD_NS}workspace/agents/list`: {
                ...
                case `${HOPCODE_METHOD_NS}workspace/agents/get`: {
                ...
                case `${HOPCODE_METHOD_NS}workspace/agents/create`: {
                    const scope = params['scope'];
                ...
                case `${HOPCODE_METHOD_NS}workspace/agents/update`: {
                    const agentType = String(params['agentType'] ?? '');
                    if (!agentType) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`agentType` required'));
                    ...
                    await this.agentManager.updateSubagent(agentType, updates, existing.level);
                ...
                case `${HOPCODE_METHOD_NS}workspace/agents/delete`: {
                    const agentType = String(params['agentType'] ?? '');
                    if (!agentType) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`agentType` required'));
            ...
            writeStderrLine(`hopcode serve: /acp dispatch error (${logSafe(method)}): ${logSafe(errMsg(err))}`);
            if (id !== undefined) {
                const { code, message, data } = toRpcError(err);
     ...
     */
    async pumpSessionEvents(conn, sessionId, signal, lastEventId) {
        try {
            ...
            if (conn.hasInitialReplayPending(sessionId)) {
                ...
                if (snapshot) {
                    ...
                    writeStderrLine(`hopcode serve: /acp initial replay skipped (no snapshot) session=${logSafe(sessionId)}`);
            ...
            for await (const event of iterable) {
                ...
                if (event.type === 'replay_complete') {
                    ...
                    const replayedCount = event.data
                        ?.replayedCount;
                    writeStderrLine(`hopcode serve: /acp replay complete (${logSafe(sessionId)}) ` +
            ...
            if (!signal.aborted) {
                ...
                conn.flushBufferedSessionFrames(sessionId);
                conn.sendSession(sessionId, notification(`${HOPCODE_METHOD_NS}notify`, {
                    kind: 'stream_error',
                    error: errMsg(err),
                }));
            }
            ...
            case 'permission_request': {
                ...
                if (!binding?.stream || binding.stream.isClosed) {
                    ...
                    if (binding?.graceTimer) {
                        writeStderrLine(`hopcode serve: /acp permission cancel during reconnect grace ` +
                    ...
                    if (!cancelled) {
                        writeStderrLine(`hopcode serve: /acp permission cancel FAILED for ${logSafe(sessionId)} (mediator stuck; no automatic recovery)`);
                    }
                    return;
                }
    ...
    resolveClientResponse(conn, msg, fromLoopback) {
        // Our outbound request ids are strings (`_HOPCODE_perm_N`); a client echoes
        ...
        if (pendingConn !== conn && !conn.ownsSession(pending.sessionId)) {
            ...
            writeStderrLine(`hopcode serve: /acp permission vote dropped: responding connection ${logSafe(conn.connectionId.slice(0, 8))} does not own session ${logSafe(pending.sessionId)} (requestId ${logSafe(pending.bridgeRequestId)})`);
            return;
        }
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: /acp permission vote failed (${logSafe(pending.sessionId)}): ${logSafe(errMsg(err))}`);
    ...
    async handlePrompt(conn, sessionId, id, params, fromLoopback) {
        ...
        try {
            ...
            if (id !== undefined) {
                ...
                writeStderrLine(`hopcode serve: /acp prompt error (${logSafe(sessionId)}, notification): ${logSafe(errMsg(err))}`);
            }
        }
        finally {
    ...
    replySession(conn, sessionId, id, result, errorFrame) {
        ...
        if (conn.sessions.has(sessionId)) {
            ...
            try {
                ...
                anchorId = undefined;
                writeStderrLine(`hopcode serve: /acp replySession(${logSafe(sessionId)}) ` +
            ...
            // session teardown.
            writeStderrLine(`hopcode serve: /acp replySession(${logSafe(sessionId)}) binding gone mid-flight, ` +
