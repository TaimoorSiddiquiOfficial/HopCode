 ...
 *
 * Unlike the REST `/session/:id/events` stream (hopcode event envelopes), the
...
export class SseStream {
    ...
    close() {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: /acp SSE onClose threw: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    writeRaw(chunk) {
        ...
        this.writeChain = next.catch((err) => {
            if (!this.closed) {
                writeStderrLine(`hopcode serve: /acp SSE write failed, closing stream: ${err instanceof Error ? err.message : String(err)}`);
                this.close();
            }
            return undefined;
        });
