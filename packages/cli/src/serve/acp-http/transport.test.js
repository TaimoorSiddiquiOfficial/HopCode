...
import { CancelSentinelCollisionError, InvalidClientIdError, InvalidPermissionOptionError, PermissionForbiddenError, PermissionPolicyNotImplementedError, PromptQueueFullError, SessionLimitExceededError, SessionShellClientRequiredError, SessionShellDisabledError, TotalSessionLimitExceededError, } from '@hoptrendy/acp-bridge/bridgeErrors';
...
import { WorkspaceRememberTaskLane, mountWorkspaceMemoryRememberRoutes, } from '../workspace-remember.js';
...
const fakeWorkspace = {
    ...
    async getWorkspacePermissionsStatus() {
        return {
            v: 1,
            user: { path: '/home/.hopcode/settings.json', rules: emptyRules() },
            workspace: { path: '/ws/.hopcode/settings.json', rules: emptyRules() },
            merged: emptyRules(),
            isTrusted: true,
        };
    },
    ...
    async initWorkspace() {
        return { path: '/ws/HOPCODE.md', action: 'created' };
    },
...
describe('ACP Streamable HTTP transport (over the wire)', () => {
    ...
    beforeEach(async () => {
        ...
        setupGithubMocks.setupGithub.mockResolvedValue({
            ...
            gitRepoRoot: '/ws',
            releaseTag: 'v1.2.3',
            readmeUrl: 'https://github.com/QwenLM/hopcode-action',
            workflows: [],
    ...
    async function withRuntimeDir(fn) {
        ...
        const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-acp-archive-'));
    ...
    async function writeStoredSession(sessionId, state = 'active', parentSessionId, sourceType, sourceId) {
        const chatsDir = path.join(new Storage('/ws').getProjectDir(), 'chats', ...(state === 'archived' ? ['archive'] : []));
    ...
    it('initialize omits _hopcode/session/shell by default', async () => {
        ...
        const result = body['result'];
        expect(result.agentCapabilities._meta.hopcode.methods).not.toContain('_hopcode/session/shell');
    });
    it('initialize advertises _hopcode/session/shell when enabled', async () => {
        await restartServer({ sessionShellCommandEnabled: true });
        const { body } = await initializeRaw();
        const result = body['result'];
        expect(result.agentCapabilities._meta.hopcode.methods).toContain('_hopcode/session/shell');
        expect(result.agentCapabilities._meta.hopcode.methods).not.toContain('_qwen/session/rewind');
    });
    it('initialize advertises _hopcode/session/lsp', async () => {
        const { body } = await initializeRaw();
        const result = body['result'];
        expect(result.agentCapabilities._meta.hopcode.methods).toContain('_hopcode/session/lsp');
    });
    ...
    it('permission request round-trips agent→client→agent', async () => {
        ...
        try {
            ...
            expect(reqFrame.params._meta['hopcode'].requestId).toBe('perm-1');
    ...
    it('session/set_config_option (mode) routes to setSessionApprovalMode', async () => {
        ...
        await post(connId, {
            ...
            id: 10,
            method: 'session/set_config_option',
            params: { sessionId: 'sess-1', configId: 'mode', value: 'izn' },
        });
        await got;
        expect(bridge.lastApprovalMode).toBe('izn');
    });
    it('_hopcode/workspace/mcp introspection reaches the bridge', async () => {
        const connId = await initialize();
        const connStream = await openStream(connId);
        const got = takeFrames(connStream, 1);
        await new Promise((r) => setTimeout(r, 50));
    ...
    it.each(['session/load', 'session/resume'])('%s rejects archived sessions', async (method) => {
        ...
        try {
            const chatsDir = path.join(new Storage('/ws').getProjectDir(), 'chats', 'archive');
            await fs.mkdir(chatsDir, { recursive: true });
    ...
    it('_hopcode/* introspection methods reach the bridge (conn-routed)', async () => {
        const connId = await initialize();
        await newSession(connId);
        const connStream = await openStream(connId);
    ...
    it('_hopcode/workspace/set_tool_enabled + restart_mcp_server validate name', async () => {
        const connId = await initialize();
        const connStream = await openStream(connId);
    ...
    // ── Wave 1+2: new _hopcode/* method tests ──────────────────────────
    ...
    describe('session extension methods', () => {
        it('_hopcode/session/recap returns recap', async () => {
            const connId = await initialize();
        ...
        it('_hopcode/session/btw validates question length', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/session/btw returns answer', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/session/context_usage returns usage', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/session/tasks returns tasks', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/session/lsp returns status', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
    ...
    describe('workspace methods', () => {
        it('_hopcode/workspace/tools returns tools', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/mcp/tools returns tools', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/mcp/servers/add rejects missing name', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/mcp/servers/remove rejects missing name', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/sessions/delete rejects non-array', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/sessions/delete rejects >100 ids', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
        ...
        it('_hopcode/sessions/delete sanitizes stderr remove errors', async () => {
            ...
            try {
                ...
                const deleteLog = stdioMocks.writeStderrLine.mock.calls
                    .map(([line]) => line)
                    .find((line) => line.includes('sessions/delete'));
                expect(deleteLog).toContain('removeSession(sess FAK) failed: remove FAILED  [31m');
                expect(deleteLog).not.toContain('\n');
                expect(deleteLog).not.toContain('\r');
                expect(deleteLog).not.toContain('\x1b');
                expect(deleteLog).not.toContain(lineSep);
    ...
    describe('auth methods', () => {
        it('_hopcode/workspace/auth/status returns empty when no registry', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/auth/device_flow/start rejects without registry', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
    ...
    describe('memory methods', () => {
        it('_hopcode/workspace/memory/write rejects non-string content', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/memory/write rejects invalid scope', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/workspace/memory/write rejects invalid mode', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
    ...
    describe('file methods', () => {
        it('_hopcode/file/read rejects without fsFactory (503-equivalent)', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/file/read rejects missing path', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/file/write rejects missing content', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/file/edit rejects missing oldText/newText', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/file/glob rejects missing pattern', async () => {
            const connId = await initialize();
            const streamRes = openStream(connId);
            await new Promise((r) => setTimeout(r, 30));
            await post(connId, {
        ...
        it('_hopcode/file/glob honors a valid maxResults limit', async () => {
        ...
        it('_hopcode/file/glob defaults maxResults when omitted', async () => {
            const glob = vi.fn(async () => []);
            await restartServer({ fsFactory: makeGlobFsFactory(glob) });
            const connId = await initialize();
            const streamRes = openStream(connId);
        ...
        it.each([0, -1, 1.5, 50_001, '2', null])('_hopcode/file/glob rejects invalid maxResults (%s)', async (maxResults) => {
            const glob = vi.fn(async () => []);
            await restartServer({ fsFactory: makeGlobFsFactory(glob) });
            const connId = await initialize();
            const streamRes = openStream(connId);
...
describe('ACP WebSocket transport security', () => {
    ...
    it('does not register chrome-devtools MCP without an explicit CDP MCP command', async () => {
        ...
        expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: set QWEN_CDP_MCP_COMMAND to enable browser automation MCP (no adapter is bundled)');
        ws.close();
    ...
    it('removes chrome-devtools MCP when settings already define it', async () => {
        ...
        expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: chrome-devtools runtime MCP skipped because settings already define it');
        ws.close();
        await new Promise((resolve) => ws.once('close', () => resolve()));
    });
    it('retries chrome-devtools MCP registration after a skipped result', async () => {
        ...
        await vi.waitFor(() => expect(bridge.runtimeMcpAdds).toHaveLength(1));
        expect(bridge.runtimeMcpRemoves).toHaveLength(0);
        expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: chrome-devtools runtime MCP skipped: budget_exceeded');
        bridge.runtimeMcpAddResult = {};
        const second = await wsConnect();
        await initializeCdpBridge(second, 2);
    ...
    it('retries chrome-devtools MCP registration after add failure', async () => {
        ...
        await initializeCdpBridge(first, 1);
        await vi.waitFor(() => {
            expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: failed to add chrome-devtools runtime MCP: add failed');
        });
    ...
    it('stops retrying chrome-devtools MCP registration after ACP channel retry exhaustion', async () => {
        ...
        await initializeCdpBridge(ws);
        await vi.waitFor(() => {
            expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: failed to add chrome-devtools runtime MCP: no channel');
        }, { timeout: 7_000 });
    ...
    it('skips chrome-devtools MCP registration when /cdp requires auth', async () => {
        ...
        await initializeCdpBridge(ws);
        expect(bridge.runtimeMcpAdds).toHaveLength(0);
        expect(stdioMocks.writeStderrLine).toHaveBeenCalledWith('hopcode serve: chrome-devtools runtime MCP skipped because /cdp requires bearer auth');
        ws.close();
        await new Promise((resolve) => ws.once('close', () => resolve()));
    });
    ...
    it('classifies _hopcode/workspace/trust as a WS read method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/trust/request as a WS mutation method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/permissions as a WS read method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/permissions/set as a WS mutation method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/voice as a WS read method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/voice/set as a WS mutation method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
    ...
    it('classifies _hopcode/workspace/setup-github as a WS mutation method', async () => {
        const tiers = [];
        await startServer({
            checkRate: (_key, tier) => {
                tiers.push(tier);
