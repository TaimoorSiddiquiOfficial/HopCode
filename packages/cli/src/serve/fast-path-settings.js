...
import { DEFAULT_EXCLUDED_ENV_VARS, HOME_ENV_BOOTSTRAP_KEYS, PROJECT_ENV_HARDCODED_EXCLUSIONS, } from '../config/shared-env-keys.js';
import { getGlobalhopcodeDirLite, getSystemDefaultsPath, getSystemSettingsPath, SETTINGS_DIRECTORY_NAME, } from '../config/storage-paths-lite.js';
import { getPathComparisonVariants, isWithinRoot, } from '../config/path-comparison.js';
...
function getTrustedFoldersPathFastPath() {
    return (process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH'] ??
        path.join(getGlobalhopcodeDirLite(), 'trustedFolders.json'));
}
function getUserLevelEnvPathsFastPath() {
    const homeDir = os.homedir();
    const globalhopcodeDir = getGlobalhopcodeDirLite();
    return new Set([
...
export function preResolveServeFastPathHomeEnvOverrides() {
    ...
    const initialhopcodeDir = getGlobalhopcodeDirLite();
    readHomeEnvIntoFastPath(path.join(initialhopcodeDir, '.env'));
    if (!initialhopcodeHome) {
        readHomeEnvIntoFastPath(path.join(path.dirname(initialhopcodeDir), '.env'));
    }
    const discoveredhopcodeHome = process.env['HOPCODE_HOME'];
    if (discoveredhopcodeHome && discoveredhopcodeHome !== initialhopcodeHome) {
        const discoveredDir = getGlobalhopcodeDirLite();
...
function findEnvFilesFastPath(settings, startDir, userLevelPaths = getUserLevelEnvPathsFastPath()) {
    ...
    const globalhopcodeDir = getGlobalhopcodeDirLite();
    const legacyhopcodeDir = path.normalize(path.join(homeDir, SETTINGS_DIRECTORY_NAME));
...
export function loadServeFastPathSettings(workspaceDir) {
    ...
    const user = readSettingsSummary(path.join(getGlobalhopcodeDirLite(), 'settings.json'));
