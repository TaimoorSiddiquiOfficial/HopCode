...
export function createHealthDemoRoutes(deps) {
    const { opts, getPort, workspaceRegistry, getActiveSseCount, getRateLimiter, } = deps;
    ...
    const demoHandler = (_req, res) => {
        try {
            ...
            writeStderrLine(`hopcode serve: /demo render failed: ${err instanceof Error ? err.message : String(err)}`);
...
//# sourceMappingURL=health-demo.js.map