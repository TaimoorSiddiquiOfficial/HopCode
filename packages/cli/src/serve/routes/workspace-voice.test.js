...
async function makeHarness(opts = {}) {
    ...
    process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH'] = path.join(home, TRUSTED_FOLDERS_FILENAME);
...
async function writeWorkspaceVoiceEnabled(h, enabled) {
    await writeJson(path.join(h.workspace, '.hopcode', 'settings.json'), {
...
describe('workspace voice routes', () => {
    ...
    it('POST updates voice settings only after validating the resulting state', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'voiceModel', 'qwen3-asr-flash');
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.mode', 'hold');
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.language', 'english');
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', true);
    });
    it('POST writes initial trusted workspace voice settings to user scope', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.mode', 'tap');
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', true);
    });
    it('POST writes trusted workspace voice settings to workspace scope after workspace opt-in', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.Workspace, 'general.voice.mode', 'tap');
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.Workspace, 'general.voice.enabled', true);
    });
    ...
    it('POST rejects enabling voice when the selected model config is unusable', async () => {
                ...
                openai: [
                    {
                        ...
                        label: 'Qwen ASR',
                        baseUrl: 'https://dashscope.example/compatible-mode/v1',
                        envKey: 'HOPCODE_VOICE_TEST_MISSING_KEY',
                    },
                ],
            },
            voiceModel: 'qwen3-asr-flash',
        ...
        expect(missingKey.body.error).toContain('requires HOPCODE_VOICE_TEST_MISSING_KEY');
        await writeJson(path.join(h.home, 'settings.json'), {
            modelProviders: {
                openai: [
                    {
    ...
    it('POST allows disabling voice without a selected model', async () => {
        const res = await request(h.app)
            ...
            .send({ enabled: false });
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', false);
    });
