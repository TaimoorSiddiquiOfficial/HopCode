...
export function registerWorkspacePermissionsRoutes(
  ...
  app.get('/workspace/permissions', (_req: Request, res: Response) => {
    try {
      ...
      writeStderrLine(
        `hopcode serve: GET /workspace/permissions error: ${
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        writeStderrLine(
          `hopcode serve: POST /workspace/permissions ACP error (key=${key}, scope=${permissionScope}, workspace=${boundWorkspace}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        res.status(500).json({
