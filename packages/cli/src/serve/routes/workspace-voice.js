...
import { requireTrustedWorkspaceRuntime, resolveManagedWorkspaceRuntimeFromParam, resolveWorkspaceRuntimeFromParam, } from '../workspace-route-runtime.js';
...
function broadcastVoiceWrite(deps, write, clientId) {
    try {
        ...
        writeStderrLine(`hopcode serve: POST /workspace/voice broadcast error (key=${write.key}, scope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
    }
}
...
async function persistVoiceUpdate(deps, settings, update, clientId, workspaceTrusted) {
    ...
    if (deps.persistSettings) {
        ...
        for (const write of writes) {
            try {
                ...
                writeStderrLine(`hopcode serve: POST /workspace/voice partial persist error (workspace=${deps.boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
...
function loadVoiceSettings(deps) {
    return loadSettings(deps.boundWorkspace, deps.env ? { skipLoadEnvironment: true } : true);
}
...
async function handleVoiceUpdate(req, res, deps, route) {
    ...
    if (clientId === null)
        return;
    try {
        const workspaceTrusted = getWorkspaceTrustStatus(settings.merged, deps.boundWorkspace).effective
            .state === 'trusted';
