...
import { FsError, MAX_READ_BYTES, canonicalizeWorkspace, isFsError, } from '../fs/index.js';
import { getWorkspaceRouteContext, resolveWorkspaceRuntimeFromParam, setWorkspaceRouteContext, } from '../workspace-route-runtime.js';
/**
...
export function sendFsError(res, err, route) {
    ...
    writeStderrLine(`hopcode serve: ${route} unexpected error: ${err instanceof Error ? err.message : String(err)}`);
    res.status(500).json({
...
function requireStringQuery(res, raw, name, route) {
    if (typeof raw !== 'string' || raw.length === 0) {
        ...
        writeStderrLine(`hopcode serve: ${route} rejected request missing required ?${name}`);
        return null;
    }
    return raw;
}
