...
function broadcastVoiceWrite(
  ...
  try {
    ...
    writeStderrLine(
      `hopcode serve: POST /workspace/voice broadcast error (key=${write.key}, scope=${voiceSettingsScopeToWire(write.scope)}): ${
  ...
  if (deps.persistSettings) {
    ...
    for (const write of writes) {
      try {
        ...
        writeStderrLine(
          `hopcode serve: POST /workspace/voice partial persist error (workspace=${deps.boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
...
export function registerWorkspaceVoiceRoutes(
  ...
  app.get('/workspace/voice', (_req: Request, res: Response) => {
    try {
      ...
      writeStderrLine(
        `hopcode serve: GET /workspace/voice error: ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
      res.status(500).json({
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        writeStderrLine(
          `hopcode serve: POST /workspace/voice persist error (workspace=${deps.boundWorkspace}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        res.status(500).json({
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        writeStderrLine(
          `hopcode serve: POST /workspace/voice/transcribe error (workspace=${deps.boundWorkspace}): ${
            message
          }`,
        );
        res.status(502).json({
