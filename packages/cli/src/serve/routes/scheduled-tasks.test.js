...
import { SessionService, Storage, getCronFilePath, } from '@hoptrendy/hopcode-core';
...
describe('workspace-qualified scheduled-tasks routes', () => {
    ...
    it('writes to the targeted workspace’s own cron file on disk', async () => {
        await request(h.app)
            .post(qualified(h.secondary.workspaceId))
            .send({ cron: '0 9 * * *', prompt: 'p' });
        const onDisk = JSON.parse(await fsp.readFile(getCronFilePath(h.secondary.workspaceCwd), 'utf-8'));
        expect(onDisk).toHaveLength(1);
        // The primary's file was never created.
        await expect(fsp.readFile(getCronFilePath(h.primary.workspaceCwd), 'utf-8')).rejects.toThrow();
    });
