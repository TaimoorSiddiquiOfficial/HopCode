...
export function registerWorkspaceManagementRoutes(app, deps) {
    const { workspaceRegistry, mutate, safeBody, createWorkspaceRuntime, workspaceRegistrationStore, getAcpHandle, runtimeRemoval, } = deps;
    ...
    app.post('/workspaces', mutate({ strict: true }), async (req, res) => {
        ...
        if (existingRuntime && persist && !existingRuntime.primary) {
            ...
            const nested = [
                ...
                ...[...inFlight].flatMap(([cwd, operation]) => operation === 'addition' || operation === 'promotion' ? [cwd] : []),
            ].some((boundCwd) => boundCwd !== canonical &&
            ...
            try {
                ...
                const alreadyPersisted = snapshot.workspaces.some((stored) => existingRuntime.registrationIds?.includes(workspaceRegistrationId(stored)) === true ||
        ...
        const projectedWorkspaceCwds = new Set(workspaceRegistry.listManaged().map((runtime) => runtime.workspaceCwd));
        for (const [cwd, operation] of inFlight) {
            if (operation === 'addition')
                projectedWorkspaceCwds.add(cwd);
        }
