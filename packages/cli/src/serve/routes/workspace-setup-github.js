...
export function createSetupGithubFileOps(factory, route, originatorClientId) {
...
async function ensureDirectoryWithoutSymlink(root, segments) {
...
export function resolveSetupGithubProxy(boundWorkspace, env) {
    const settings = loadSettings(boundWorkspace, { skipLoadEnvironment: true });
    const trustState = getWorkspaceTrustStatus(settingsForSetupGithubTrust(settings), boundWorkspace).effective.state;
    const settingsProxy = trustState === 'trusted'
