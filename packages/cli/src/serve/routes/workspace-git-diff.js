 ...
 */
import { fetchGitDiff, fetchGitDiffHunksForFile, } from '@hoptrendy/hopcode-core';
...
async function handleDiffList(res, workspaceCwd, sendBridgeError, route) {
    try {
...
async function handleDiffFile(req, res, workspaceCwd, sendBridgeError, route) {
...
export function registerWorkspaceGitDiffRoutes(app, deps) {
    app.get('/workspace/git/diff', (_req, res) => {
        void handleDiffList(res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/diff');
    });
    app.get('/workspace/git/diff/file', (req, res) => {
        void handleDiffFile(req, res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/diff/file');
    });
}
...
export function registerWorkspaceQualifiedGitDiffRoutes(app, deps) {
    app.get('/workspaces/:workspace/git/diff', (req, res) => {
        ...
        if (!runtime)
            return;
        void handleDiffList(res, runtime.workspaceCwd, deps.sendBridgeError, 'GET /workspaces/:workspace/git/diff');
    });
    app.get('/workspaces/:workspace/git/diff/file', (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        void handleDiffFile(req, res, runtime.workspaceCwd, deps.sendBridgeError, 'GET /workspaces/:workspace/git/diff/file');
    });
}
