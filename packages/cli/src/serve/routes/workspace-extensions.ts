    ...
    void enqueueExtensionInstall(async () => {
      try {
        ...
        try {
          ...
          writeStderrLine(
            `hopcode serve: extensions ${operation}: refreshed ${result.refreshed} session(s), ${result.failed} failed`,
          );
        } catch (refreshErr) {
          ...
          try {
            ...
            writeStderrLine(
              `hopcode serve: extensions ${operation}: failed to broadcast refresh failure: ${
          ...
          writeStderrLine(
            `hopcode serve: extensions ${operation}: mutation succeeded but refresh failed: ${message}`,
          );
        }
      } catch (err) {
        const message = redactUrlCredentials(
        ...
        try {
          ...
          writeStderrLine(
            `hopcode serve: extensions ${operation}: failed to broadcast failure: ${
              broadcastErr instanceof Error
                ? redactUrlCredentials(broadcastErr.message)
                : String(broadcastErr)
            }`,
        ...
        try {
          writeStderrLine(
            `hopcode serve: extensions ${operation}: background task failed: ${message}`,
          );
        } catch {
      ...
      try {
        writeStderrLine(
          `hopcode serve: extensions ${operation}: queued task failed: ${message}`,
        );
      } catch {
