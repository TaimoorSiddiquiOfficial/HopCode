...
import { parseInstallSource, redactUrlCredentials, SettingScope, } from '@hoptrendy/hopcode-core';
            ...
            try {
                ...
                results.forEach((result, index) => {
                    if (result.status === 'fulfilled') {
                        ...
                        writeStderrLine(`hopcode serve: extension generation reconciliation failed for workspace ${runtimes[index].workspaceId}: ${redactUrlCredentials(result.reason instanceof Error
                ...
                writeStderrLine(`hopcode serve: extension generation reconciliation failed: ${redactUrlCredentials(error instanceof Error ? error.message : String(error))}`);
            }
            finally {
                reconciling = false;
            }
    ...
    if (workspaceRegistry) {
        ...
        app.post('/workspaces/:workspace/extensions/refresh', mutate({ strict: true }), (req, res) => {
            ...
            if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
                return;
            const manager = primaryController.createExtensionManager(runtime.workspaceCwd, true);
            sendOperation(req, res, 'POST /workspaces/:workspace/extensions/refresh', manager, 'refresh', {}, async () => ({ status: 'refreshed' }), { refreshRuntimes: [runtime] });
        });
    }
}
