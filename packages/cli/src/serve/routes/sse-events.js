...
import { SubscriberLimitExceededError, } from '@hoptrendy/acp-bridge/eventBus';
import { errorMessage, } from '../server/error-response.js';
import { parseLastEventId, parseMaxQueuedQuery, } from '../server/request-helpers.js';
    ...
    app.get('/session/:id/events', (req, res) => {
        ...
        try {
            ...
            if (err instanceof SubscriberLimitExceededError) {
                writeStderrLine(`hopcode serve: subscriber limit reached for session ${sessionId} (limit=${err.limit}); rejecting new SSE client with 429`);
        ...
        if (daemonLog) {
            ...
            const sseClientId = req.headers['x-qwen-client-id'];
        ...
        if (trackWriterIdle) {
            ...
            idleTimer = setInterval(() => {
                ...
                try {
                    writeStderrLine(`hopcode serve: evicting SSE client (session ${sessionId}) — ` +
        ...
        res.on('error', (err) => {
            ...
            writeStderrLine(`hopcode serve: SSE socket error (session ${sessionId}): ${err.message}`);
            cleanup();
        });
        void (async () => {
            try {
                while (true) {
                    ...
                    if (next.value.type === 'state_resync_required') {
                        ...
                        const gap = typeof data.earliestAvailableId === 'number' &&
                            ...
                            : undefined;
                        writeStderrLine(`hopcode serve: SSE ring eviction detected (session ${sessionId}): ` +
                ...
                if (!res.writableEnded) {
                    ...
                    writeStderrLine(`hopcode serve: bridge iterator error (session ${sessionId}): ` +
                        `${errorMessage(err)}` +
                        (errorKind ? ` [${errorKind}]` : ''));
