...
import { APPROVAL_MODES, BTW_MAX_INPUT_LENGTH, GROUP_COLOR_OPTIONS, SessionService, SessionOrganizationError, SESSION_TRANSCRIPT_MAX_LIMIT, SESSION_TRANSCRIPT_MAX_PAGE_BYTES, SessionTranscriptPageTooLargeError, SessionTranscriptCursorCodec, SessionTranscriptReader, SessionTranscriptSnapshotUnavailableError, addDaemonRequestAttribute, runWithoutDebugLogSession, } from '@hoptrendy/hopcode-core';
...
import { PromptDeadlineExceededError, resolvePromptDeadlineMs, } from '../server/prompt-deadline.js';
...
import { InvalidCursorError, getWorkspaceSessionInfoForResponse, listLiveWorkspaceSessionsForResponse, listWorkspaceSessionsForResponse, parseSessionPageSizeQuery, } from '../server/session-list.js';
import { archiveDaemonSessions, assertSessionArchived, assertSessionLoadable, deleteDaemonSessions, logSessionArchiveWarning, unarchiveDaemonSessions, } from '../server/session-archive.js';
...
import { resolveWorkspaceRuntimeFromParam, sendUntrustedWorkspaceResponse, } from '../workspace-route-runtime.js';
...
function writeGenerationSseChunk(res, chunk) {
    if (res.destroyed) {
...
export function registerSessionRoutes(app, deps) {
    const { boundWorkspace, bridge, workspaceRegistry, archiveCoordinator, mutate, sendBridgeError, daemonLog, promptDeadlineMs, sessionShellCommandEnabled, } = deps;
    ...
    const hasActivePersistedSessions = async (workspaceCwd) => {
        try {
    ...
    const enterRestoreOwner = (res, route, sessionId, runtime) => {
        const existing = inFlightRestoreOwners.get(sessionId);
        if (existing && existing.workspaceCwd !== runtime.workspaceCwd) {
            const existingRuntime = workspaceRegistry.getByWorkspaceCwd(existing.workspaceCwd) ??
                workspaceRegistry.primary;
            sendSessionWorkspaceConflict(res, route, sessionId, runtime, existingRuntime);
            return undefined;
        }
        if (existing) {
            existing.count += 1;
        ...
        return () => {
            ...
            if (!current || current.workspaceCwd !== runtime.workspaceCwd)
                return;
            current.count -= 1;
            if (current.count <= 0) {
    ...
    const resolveTranscriptSessionRuntime = async (res, route, sessionId, hasCursor) => {
        const activeInRuntime = async (runtime) => {
            const location = await assertSessionLoadable(runtime.workspaceCwd, sessionId);
    ...
    const resolveWorkspaceParam = (req, res) => {
    ...
    const restoreSessionHandler = (action) => async (req, res) => {
        ...
        if (resolvedRuntime === undefined)
            return;
        const { runtime, workspaceCwd } = resolvedRuntime;
        const releaseRestoreOwner = enterRestoreOwner(res, route, sessionId, runtime);
        if (!releaseRestoreOwner)
            return;
        const approvalMode = parseOptionalApprovalMode(body, res);
        if (approvalMode === null) {
            releaseRestoreOwner();
        ...
        try {
            const session = await archiveCoordinator.runSharedMany([sessionId], async () => {
                ...
                const metadata = await new SessionService(workspaceCwd).readCreationMetadata(sessionId);
                return action === 'load'
    ...
    app.post('/session/:id/branch', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/branch', async (req, res, sessionId) => {
        const body = safeBody(req);
    ...
    app.post('/session/:id/fork', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/fork', async (req, res, sessionId) => {
        const body = safeBody(req);
        ...
        if (clientId === null)
            return;
        const result = await bridge.launchSessionForkAgent(sessionId, directive, clientId !== undefined ? { clientId } : undefined);
        res.status(202).json(result);
    }));
    app.post('/session/:id/cd', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/cd', async (req, res, sessionId) => {
        const body = safeBody(req);
        ...
        if (clientId === null)
            return;
        const result = await bridge.changeSessionCwd(sessionId, { path: targetPath }, clientId !== undefined ? { clientId } : undefined);
    ...
    app.get('/session/:id/context', withOwnerReadSession('GET /session/:id/context', async (_req, res, sessionId, runtime) => {
        res
            .status(200)
    ...
    app.get('/session/:id/supported-commands', withOwnerReadSession('GET /session/:id/supported-commands', async (_req, res, sessionId, runtime) => {
        res
            .status(200)
    ...
    app.post('/session/:id/continue', mutate({ strict: true }), withOwnerMutableSession('POST /session/:id/continue', async (req, res, sessionId, runtime) => {
        ...
        res.status(200).json(await runtime.bridge.continueSession(sessionId, {
            ...(clientId !== undefined ? { clientId } : {}),
            promptId,
        }));
    }));
    ...
    app.post('/session/:id/heartbeat', mutate(), withOwnerMutableSession('POST /session/:id/heartbeat', (req, res, sessionId, runtime) => {
        const clientId = parseClientIdHeader(req, res);
        if (clientId === null)
            return;
    ...
    app.post('/session/:id/detach', mutate(), withOwnerMutableSession('POST /session/:id/detach', async (req, res, sessionId, runtime) => {
        const clientId = parseClientIdHeader(req, res);
        if (clientId === null)
            return;
    ...
    app.delete('/session/:id', async (req, res) => {
        ...
        try {
            ...
            await archiveCoordinator.runExclusiveMany([sessionId], async () => runtime.bridge.closeSession(sessionId, clientId !== undefined ? { clientId } : undefined));
            res.status(204).end();
        }
        catch (err) {
            sendBridgeError(res, err, {
    ...
    app.patch('/session/:id/metadata', mutate({ strict: true }), withOwnerMutableSession('PATCH /session/:id/metadata', (req, res, sessionId, runtime) => {
        ...
        const displayName = typeof rawDisplayName === 'string'
            ...
            : undefined;
        const effective = runtime.bridge.updateSessionMetadata(sessionId, { displayName }, clientId !== undefined ? { clientId } : undefined);
    ...
    const handleSessionOrganizationUpdate = async (req, res, target) => {
        ...
        try {
            await archiveCoordinator.runSharedMany([sessionId], async () => {
                ...
                const organization = await createSessionOrganizationService(target.workspaceCwd).updateSessionOrganization(sessionId, {
    ...
    app.patch('/workspace/:id/session-groups/:groupId', mutate(), async (req, res) => {
        ...
        const body = safeBody(req);
        try {
            const group = await createSessionOrganizationService(key).updateGroup((req.params['groupId'] ?? ''), {
    ...
    app.delete('/workspace/:id/session-groups/:groupId', mutate(), async (req, res) => {
        ...
        if (key === null)
            return;
        try {
            const deleted = await createSessionOrganizationService(key).deleteGroup((req.params['groupId'] ?? ''));
    ...
    app.post('/workspaces/:workspace/session-groups', mutate(), async (req, res) => {
        ...
        const body = safeBody(req);
        try {
            const group = await createSessionOrganizationService(runtime.workspaceCwd).createGroup({
                name: body['name'],
                color: body['color'],
            });
            res.status(201).json({ group });
    ...
    app.patch('/workspaces/:workspace/session-groups/:groupId', mutate(), async (req, res) => {
        ...
        const body = safeBody(req);
        try {
            const group = await createSessionOrganizationService(runtime.workspaceCwd).updateGroup((req.params['groupId'] ?? ''), {
                ...(Object.prototype.hasOwnProperty.call(body, 'name')
                    ? { name: body['name'] }
                    : {}),
                ...(Object.prototype.hasOwnProperty.call(body, 'color')
    ...
    app.delete('/workspaces/:workspace/session-groups/:groupId', mutate(), async (req, res) => {
        ...
        if (!runtime)
            return;
        try {
            const deleted = await createSessionOrganizationService(runtime.workspaceCwd).deleteGroup((req.params['groupId'] ?? ''));
            res.status(200).json({ deleted });
        }
        catch (err) {
            if (sendSessionOrganizationError(res, err))
    ...
    app.post('/session/:id/mid-turn-message', mutate(), withOwnerMutableSession('POST /session/:id/mid-turn-message', (req, res, sessionId, runtime) => {
        ...
        if (clientId === null)
            return;
        const result = runtime.bridge.enqueueMidTurnMessage(sessionId, trimmed, clientId !== undefined ? { clientId } : undefined);
        res.status(200).json(result);
    }));
