...
function buildSettingsResponse(boundWorkspace, keys) {
    ...
    for (const key of keys) {
        ...
        if (!def)
            continue;
        const effective = getNestedProperty(loaded.merged, key);
...
function prepareSettingWrite(workspace, scope, key, value, mcpServerMutation) {
    if (key !== 'mcpServers') {
...
export function registerWorkspaceSettingsRoutes(app, deps) {
    ...
    app.get('/workspace/settings', (_req, res) => {
        try {
            ...
            writeStderrLine(`hopcode serve: GET /workspace/settings error: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/settings', mutate({ strict: true }), async (req, res) => {
        ...
        try {
            const persist = async () => {
                const prepared = prepareSettingWrite(boundWorkspace, settingScope, key, value, mcpServerMutation);
            ...
            writeStderrLine(`hopcode serve: POST /workspace/settings persist error (key=${key}, scope=${scope}, workspace=${boundWorkspace}): ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: POST /workspace/settings broadcast error (key=${key}, scope=${scope}): ${err instanceof Error ? err.message : String(err)}`);
        }
        res.status(200).json({
            key,
            scope,
...
export function registerWorkspaceQualifiedSettingsRoutes(app, deps) {
    ...
    app.post('/workspaces/:workspace/settings', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            const persist = async () => {
                const prepared = prepareSettingWrite(runtime.workspaceCwd, settingScope, key, value, mcpServerMutation);
                publicValue = prepared.publicValue;
                await deps.persistSetting(runtime.workspaceCwd, settingScope, key, prepared.persistedValue);
            };
            if (mcpServerMutation) {
