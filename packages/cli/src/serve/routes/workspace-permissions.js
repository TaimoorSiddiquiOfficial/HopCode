...
export function registerWorkspacePermissionsRoutes(app, deps) {
    ...
    app.get('/workspace/permissions', (_req, res) => {
        try {
            ...
            writeStderrLine(`hopcode serve: GET /workspace/permissions error: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/permissions', mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: POST /workspace/permissions ACP error (key=${key}, scope=${permissionScope}, workspace=${boundWorkspace}): ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
