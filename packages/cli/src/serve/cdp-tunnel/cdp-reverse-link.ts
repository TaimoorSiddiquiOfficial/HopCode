...
export class CdpReverseLink {
  ...
  readonly forwardToTab = async (
    ...
    return new Promise<unknown>((resolve, reject) => {
      ...
      try {
        this.log?.(
          `hopcode serve: /cdp forwarded command id=${id} method=${method} to extension`,
        );
  ...
  private handleAttached(frame: CdpAttachedFrame): void {
    ...
    if (!attach || attach.id !== frame.id) {
      this.log?.(
        `hopcode serve: /cdp dropped unexpected cdp_attached (id=${String(frame.id)})`,
      );
      return;
    }
    this.pendingAttach = undefined;
  ...
  private armProgressLog(
    ...
    const timer = setTimeout(() => {
      this.log?.(
        `hopcode serve: /cdp still waiting for command id=${id} method=${method} after ${delay}ms`,
      );
    }, delay);
    timer.unref?.();
    return timer;
