...
import { APPROVAL_MODES, BuiltinAgentRegistry, SubagentError, SubagentErrorCode, SubagentManager, } from '@hoptrendy/hopcode-core';
...
export function mountWorkspaceAgentsRoutes(app, deps) {
    ...
    app.get('/workspace/agents', async (_req, res) => {
        try {
            ...
            writeStderrLine(`hopcode serve: GET /workspace/agents failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/agents', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: POST /workspace/agents failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/agents/generate', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: POST /workspace/agents/generate failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.get('/workspace/agents/:agentType', async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: GET /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/agents/:agentType', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: POST /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.delete('/workspace/agents/:agentType', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`hopcode serve: DELETE /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
        ...
        if (remaining.length > 0) {
            writeStderrLine(`hopcode serve: DELETE /workspace/agents/${safeLogValue(agentType)} partial — ` +
...
export function createDaemonSubagentManager(boundWorkspace, safeMode = false) {
    ...
    const guarded = new Proxy(stub, {
        get(target, prop) {
            ...
            if (prop === 'then')
                return undefined;
            throw new Error(`hopcode serve workspace agents: SubagentManager touched Config.` +
        ...
        has(target, prop) {
            ...
            if (prop === 'then')
                return false;
            throw new Error(`hopcode serve workspace agents: SubagentManager probed Config.` +
