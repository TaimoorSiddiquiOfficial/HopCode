...
describe('qwen mcp approve / reject', () => {
    ...
    beforeEach(() => {
        ...
        process.env['HOPCODE_CODE_MCP_APPROVALS_PATH'] = path.join(dir, MCP_APPROVALS_FILENAME);
    ...
    const writeWorkspaceSettings = (servers) => {
        ...
        fs.writeFileSync(path.join(hopcodeDir, 'settings.json'), JSON.stringify({ mcpServers: servers }));
    };
    /** Read the persisted approval status straight off disk (scope-agnostic). */
    const persistedStatus = (name) => {
        const raw = fs.readFileSync(process.env['HOPCODE_CODE_MCP_APPROVALS_PATH'], 'utf-8');
