...
async function setProjectServerStatus(name, status, all) {
    ...
    if (names.length === 0) {
        writeStdoutLine('No approval-requiring MCP servers found (looked in .mcp.json and .hopcode/settings.json).');
        return;
    }
...
export const approveCommand = {
    ...
    describe: 'Approve a gated MCP server (.mcp.json or workspace .hopcode/settings.json)',
    builder: (yargs) => yargs
...
export const rejectCommand = {
    command: 'reject [name]',
    describe: 'Reject a gated MCP server (.mcp.json or workspace .hopcode/settings.json)',
    builder: (yargs) => yargs
