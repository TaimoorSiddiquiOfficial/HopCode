...
import { DEFAULT_TRUNCATE_TOOL_OUTPUT_THRESHOLD } from '@hoptrendy/hopcode-core';
 ...
 */
const CANONICAL_LGTM_RE = /^No issues found\.?\s*LGTM!?\s*(?:✅\s*)?(?:_— [^\n]{0,200} via Qwen Code \/review_\s*)?$/i;
/**
...
export function buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews) {
    ...
    if (openRoots.length > 0) {
        ...
        for (const c of openRoots) {
            parts.push(`- \`${c.path ?? '?'}\`:${c.line ?? '?'} by @${c.user?.login ?? '?'}: ${snippetWithRef(c.body, 240, pullCommentRef(c.id, ctx))}`);
        }
        parts.push('');
    }
