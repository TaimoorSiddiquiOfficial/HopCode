...
import { readRecordedPrompts, wasDeliveredVerbatim, briefPath, } from './prompt-record.js';
...
export function coverageFromTranscripts(planPath, env = process.env) {
        ...
        if (chunk !== null) {
            ...
            if (b === undefined) {
                ...
                if (!nothingBuiltAtAll && !superseded(rec, chunk)) {
                    rewrittenPrompts.push(`${name} — ran on a prompt the run wrote itself (none was built for ` +
                ...
                rewrittenThisRecord = true;
                if (!superseded(rec, chunk)) {
                    rewrittenPrompts.push(`${name} — launched with a prompt that is not the one the CLI built`);
                }
            }
        }
    ...
    if (nobodyBuiltAnything) {
        ...
        missingRoles.push(`every dimension — none of the ${roster.length} required agents is on ` +
    ...
    const openedOfReq = buildable.map((req, i) => candidatesOf[i].filter((r) => openedBrief(r, req.key)));
    ...
    for (const req of roster) {
        ...
        if (b === undefined) {
            if (!nobodyBuiltAnything) {
                missingRoles.push(`${roleLabel(req)} — no record shows its brief reaching an agent, so ` +
        ...
        if (!opened) {
            unreadBriefs.push(`${roleLabel(req)} — never opened its brief (${brief}), so it reviewed ` +
