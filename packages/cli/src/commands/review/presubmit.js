...
function classifyExistingComments(hopcodeComments, repliedToIds, newFindingKeys, commitSha) {
...
async function runPresubmit(args) {
    ...
    const hopcodeComments = allComments.filter((c) => /via (?:HopCode|HopCode) \/review/.test(c.body ?? ''));
    ...
    const buckets = classifyExistingComments(hopcodeComments, repliedToIds, newFindingKeys, commitSha);
    // --- Downgrade decisions ----------------------------------------------
