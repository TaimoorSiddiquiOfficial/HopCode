...
import { mkdirSync, writeFileSync, readFileSync, rmSync, lstatSync, existsSync, } from 'node:fs';
...
export function classifyProbeRun(exitCode, stdout, probes, stderr = '') {
    ...
    return probes.map((file) => {
        ...
        const result = byFile.find((r) => (r.name ?? '').endsWith(`/${file}`) || r.name === file);
 ...
 * says why.
 */
export function probeCreateFailureDetail(err, sweepStderr) {
    const sweepErr = sweepStderr.trim();
    return (`probe worktree could not be created: ${err instanceof Error ? err.message : String(err)}` +
...
async function runTestEfficacy(args) {
    ...
    if (probes.length > 0 && revert.length > 0) {
        ...
        try {
            ...
            for (const file of probes) {
                results.push({ file, verdict: 'inconclusive', detail });
            }
        }
        if (created) {
            try {
                ...
                if (r.error)
                    throw r.error;
                if (r.signal) {
                    throw new Error(`runner killed by ${r.signal}${r.signal === 'SIGTERM' ? ' (probe timed out after 300s)' : ''}`);
                }
                results.push(...classifyProbeRun(r.status ?? 1, `${r.stdout ?? ''}`, probes, `${r.stderr ?? ''}`));
            }
            catch (e) {
    ...
    writeStdoutLine(`Wrote test-efficacy report to ${out} (${unreachable.length} unreachable, ${results.length} probed, ${findings.length} finding(s))`);
...
export const testEfficacyCommand = {
    ...
    describe: "Check whether the diff's new tests actually gate its new behaviour (unreachable + revert probe)",
    builder: (yargs) => yargs
        .positional('report', {
        type: 'string',
        demandOption: true,
