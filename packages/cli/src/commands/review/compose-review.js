...
import { CRITICAL_PREFIX, SUGGESTION_PREFIX, countInlineFindings, unmarkedComments, } from './lib/inline-counts.js';
...
export function composeReview(input) {
    ...
    if (!input.planPath) {
        unreviewed.push('coverage — no plan was given, so this run cannot show that any of the ' +
            'diff was read');
    }
    else {
        try {
            ...
            for (const label of cov.idleAgents) {
                unreviewed.push(`${label} — the agent made no tool call: it read nothing`);
            }
            ...
            for (const label of cov.unopenedAgents) {
                unreviewed.push(`${label} — pointed at diff lines it never opened: it made tool calls, ` +
            ...
            unreviewed.push(`coverage — ${why}, so this run cannot show that any of the diff was read`);
        }
        ...
        try {
            ...
            unreviewed.push(`verification — could not check that Step 4 and Step 5 ran ` +
                `(${err.message})`);
        }
    }
    ...
    if (missingReceipts.length > 0) {
        ...
        notReviewedParts.push(`Not reviewed: ${missingReceipts
            .map((id) => `chunk ${id}`)
            .join(', ')} — no agent reported covering these; nobody read them.`);
    }
    if (uncoverable.length > 0) {
        notReviewedParts.push(`Not reviewed: ${uncoverable.join(', ')} — a line there exceeds the read limit.`);
    }
    ...
    if (whiffedDimensions.length > 0) {
        notReviewedParts.push(`Not reviewed: ${whiffedDimensions.join(', ')} — the agent returned no evidence of its walk twice.`);
    }
    ...
    if (downgraded && downgradedFrom) {
        ...
        clauses.push(`⚠️ Downgraded from ${downgradedFrom} to Comment${reasons ? `: ${reasons}` : ''}.`);
    }
    ...
    if (suggestionsInline > 0)
        clauses.push('Suggestions are inline.');
    if (suggestionsDiscarded > 0) {
        clauses.push(`${suggestionsDiscarded} Suggestion-level finding(s) could not be anchored to the diff; see the terminal output.`);
    }
