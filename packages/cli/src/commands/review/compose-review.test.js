...
import { composeReview, composeReviewCommand, verdictLine, } from './compose-review.js';
 ...
 */
function coveredPlan(step45Keys = ['verify', 'reverse-audit']) {
...
describe('composeReview — 422 recovery (round-7 Critical #1 & round-6: verdict never upgrades)', () => {
    it('all Suggestions discarded on resubmit stays COMMENT, never APPROVE (round-6: Suggestion-only flipped to LGTM)', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('2 Suggestion-level finding(s) could not be anchored to the diff; see the terminal output.');
...
describe('composeReviewCommand handler (the CLI glue)', () => {
    ...
    it('accepts the review-payload shape too — the same file submit takes', () => {
        ...
        try {
            ...
            expect(JSON.parse(readFileSync(outPath, 'utf8')).event).toBe('REQUEST_CHANGES');
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it.each([
    ...
    ])('refuses a state JSON carrying %s — counts are counted, not typed', (_, extra) => {
        ...
        try {
            ...
            expect(() => composeReviewCommand.handler({
                input: inputPath,
                comments: commentsPath,
    ...
    ])('refuses %s — omission is the failure mode, not a default', (_, commentsPath, pattern) => {
