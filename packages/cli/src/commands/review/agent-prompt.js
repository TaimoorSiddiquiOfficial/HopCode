    ...
    if (role === '7') {
        ...
        if (typeof base === 'string' && base && pr !== undefined && opts.planPath) {
            ...
            parts.push('', '**Then run the test-efficacy probe.** A green suite says the tests pass. It does ' +
                ...
                'different claims:', '', '```bash', `"\${QWEN_CODE_CLI:-qwen}" review test-efficacy ${resolve(opts.planPath)} \\`, `  --worktree ${typeof wt === 'string' ? resolve(wt) : '<worktree>'} \\`, `  --base ${base} \\`, `  --out ${resolve(dirname(opts.planPath), `qwen-review-pr-${pr}-efficacy.json`)}`, '```', '', 'Read its `findings[]`. `kind: "unreachable"` is a test the project\'s test command ' +
 ...
 */
export function findingsSection(role, content) {
...
function runAgentPrompt(args) {
    ...
    if (args.roster) {
        ...
        if (!hasFindings && BRIEFS[role]?.acceptsFindings) {
            bad(`--role ${role} needs --findings <file>: it is launched with a findings ` +
        ...
        if (hasFindings && !BRIEFS[role]?.acceptsFindings) {
            ...
            bad(`--findings folds a findings list into the prompt, only for a role that ` +
        ...
        const findingRoles = Object.keys(BRIEFS).filter((r) => BRIEFS[r].acceptsFindings);
        bad(`--findings folds a findings list into a ` +
    ...
    const printed = findingsContent !== undefined && args.role
        ? foldFindings(args.role, findingsContent, prompt)
        : prompt;
...
export const agentPromptCommand = {
    ...
    builder: (yargs) => yargs
        ...
        .option('findings', {
        type: 'string',
        describe: 'Path to a file of findings to fold into a --role verify (the shard it ' +
