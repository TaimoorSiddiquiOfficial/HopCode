...
import { affectedWorkspaces, buildSetFor, hasUnmodeledWorkspaceGlob, readRootPackage, readWorkspaceGlobs, readWorkspacePackages, } from './lib/workspaces.js';
...
export function unresolvedWorkspaceDeps(output, packages) {
    ...
    const found = new Set();
    // `error TS2307: Cannot find module '@hoptrendy/webui' or its corresponding
...
export function runBuildTest(args) {
    ...
    if (affected.length === 0) {
        return {
            ...
            ok: true,
            timedOut: [],
            note: `The diff changes ${changed.length} file(s), none of them inside a workspace ` +
...
export const buildTestCommand = {
    command: 'build-test',
    describe: 'Build and test the workspaces the diff changes (and what they compile ' +
