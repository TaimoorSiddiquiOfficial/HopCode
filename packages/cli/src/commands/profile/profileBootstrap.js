 ...
 */
export async function applyActiveProfile(settings, argv) {
    ...
    if (!profile)
        return;
    // Write auth type
    settings.setValue(SettingScope.User, 'security.auth.selectedType', profile.provider);
    // Write model
    ...
    if (profile.apiKey && profile.envKey) {
        settings.setValue(SettingScope.User, `env.${profile.envKey}`, profile.apiKey);
    ...
    const modelProvidersKey = providerLower === 'anthropic'
        ? 'USE_ANTHROPIC'
        : providerLower === 'gemini' || providerLower === 'vertex'
            ? 'USE_GEMINI'
            : 'USE_OPENAI';
    const existingProviders = settings.merged.modelProviders ??
        {};
...
//# sourceMappingURL=profileBootstrap.js.map