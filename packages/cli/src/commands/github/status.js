...
async function handleGithubStatus() {
    ...
    if (!remote) {
        writeStderrLine(t('Not in a GitHub repository. Navigate to a git repo first.'));
        process.exit(1);
    }
    const { owner, repo: repoName } = remote;
    const branch = getCurrentBranch();
    writeStdoutLine(t('\n  ── GitHub Status ─────────────────────────────────────'));
    writeStdoutLine(t('  Repo:    {{owner}}/{{repo}}', { owner, repo: repoName }));
    ...
    writeStdoutLine(t('  Stars:   {{stars}}  ·  Open Issues: {{issues}}', {
    ...
    if (openPRs.length === 0) {
        ...
        for (const pr of openPRs.slice(0, 8)) {
            ...
            writeStdoutLine(t('       {{head}} → {{base}}  by @{{user}}', {
                head: pr.head.ref,
                base: pr.base.ref,
                user: pr.user.login,
            }));
    ...
    for (const commit of recentCommits) {
        ...
        writeStdoutLine(t('  {{sha}}  {{msg}}  ({{author}})', { sha, msg, author }));
    }
    writeStdoutLine('');
}
export const githubStatusCommand = {
    command: 'status',
