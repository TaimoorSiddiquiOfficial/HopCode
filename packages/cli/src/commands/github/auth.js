 ...
 */
import { saveGitHubToken, clearGitHubToken, loadGitHubToken, } from '../../utils/githubTokenStore.js';
 ...
 */
const HOPCODE_GITHUB_APP_CLIENT_ID = process.env['HOPCODE_GITHUB_CLIENT_ID'] ?? 'Iv23livRiRBTa9cyBnk1';
...
async function requestDeviceCode() {
    ...
    if (!resp.ok) {
        throw new Error(`Failed to start device flow: ${resp.status} ${resp.statusText}`);
    }
        ...
        if ('error' in data) {
            switch (data.error) {
                ...
                case 'slow_down':
                    ...
                    break;
                case 'expired_token':
                    throw new Error('Device code expired. Please run: hopcode github auth');
                case 'access_denied':
...
// ── Subcommand handlers ────────────────────────────────────────────────────────
async function handleGithubAuth(options) {
    if (options.logout) {
        clearGitHubToken();
    ...
    if (existing?.accessToken) {
        try {
            ...
            writeStdoutLine(t('✓ Already authenticated as: {{login}} ({{name}})', {
                login: user.login,
                name: user.name ?? user.login,
            }));
            writeStdoutLine(t('  To re-authenticate, run: hopcode github auth --logout && hopcode github auth'));
            return;
        }
        catch {
    ...
    writeStdoutLine(t('  ┌─────────────────────────────────────────┐'));
    writeStdoutLine(t('  │  Open:  {{url}}', {
        url: deviceData.verification_uri.padEnd(38) + '│',
    }));
    writeStdoutLine(t('  │  Code:  {{code}}', { code: deviceData.user_code.padEnd(38) + '│' }));
    writeStdoutLine(t('  └─────────────────────────────────────────┘'));
    ...
    writeStdoutLine(t('  You can now use: hopcode github pr, hopcode github issues, hopcode github commit\n'));
    void token; // used indirectly via saveGitHubToken
}
// ── Command export ─────────────────────────────────────────────────────────────
export const githubAuthCommand = {
    command: 'auth',
    describe: t('Authenticate with GitHub using OAuth Device Flow'),
    builder: (yargs) => yargs.option('logout', {
        type: 'boolean',
    ...
    handler: async (argv) => {
        try {
            await handleGithubAuth({ logout: argv['logout'] });
        }
        catch (err) {
