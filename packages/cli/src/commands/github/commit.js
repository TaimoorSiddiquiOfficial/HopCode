 ...
 */
async function generateCommitMessage(diff, files) {
    ...
    try {
        ...
        const { stdout } = await execAsync('hopcode', ['--non-interactive', '--output-format', 'text', prompt], { timeout: 30000 });
    ...
    const type = ext === 'md' ? 'docs' : ext === 'test' || ext === 'spec' ? 'test' : 'chore';
...
async function handleCommit(argv) {
    ...
    if (!files.length) {
        writeStderrLine(t('No staged changes. Stage files first with: git add <files>'));
        writeStderrLine(t('Or use: hopcode github commit --all  (stages all tracked changes)'));
        process.exit(1);
    }
...
// ── Command export ─────────────────────────────────────────────────────────────
export const githubCommitCommand = {
    command: 'commit',
