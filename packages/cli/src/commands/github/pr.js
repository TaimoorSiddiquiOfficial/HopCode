...
// ── AI PR generation ──────────────────────────────────────────────────────────
async function generatePRContent(diff, branch) {
    ...
    try {
        const { stdout } = await execFileAsync('hopcode', ['--non-interactive', '--output-format', 'text', prompt], { timeout: 45000 });
...
const createCommand = {
    ...
    handler: async (argv) => {
        ...
        const base = argv['base'] ?? getDefaultBranch();
        if (!head || head === base) {
            writeStderrLine(t('Cannot create PR: current branch is {{head}}. Switch to a feature branch first.', { head }));
            process.exit(1);
        }
        ...
        writeStdoutLine(t('  Draft: {{draft}}', { draft: argv['draft'] ? 'yes' : 'no' }));
        writeStdoutLine('');
...
const listCommand = {
    ...
    handler: async (argv) => {
        ...
        for (const pr of prs) {
            ...
            writeStdoutLine(t('  #{{num}}  {{title}}{{draft}}', {
                num: String(pr.number),
                title: pr.title.slice(0, 65),
                draft,
            }));
            writeStdoutLine(t('       {{head}} → {{base}}  @{{user}}  {{url}}', {
                head: pr.head.ref,
                base: pr.base.ref,
                user: pr.user.login,
                url: pr.html_url,
...
const reviewCommand = {
    ...
    builder: (yargs) => yargs.positional('number', { type: 'number', description: t('PR number') }),
    handler: async (argv) => {
        const { owner, repo } = requireRepo();
        ...
        try {
            const { stdout } = await execFileAsync('hopcode', ['--non-interactive', '--output-format', 'text', prompt], { timeout: 60000 });
            writeStdoutLine(stdout);
        }
        catch {
            writeStderrLine(t('AI review failed. Make sure hopcode is configured with an AI provider.'));
        }
    },
};
// ── Parent command ─────────────────────────────────────────────────────────────
export const githubPrCommand = {
    command: 'pr',
