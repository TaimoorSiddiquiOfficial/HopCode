...
async function handleDashboard(port, open) {
    const serverPath = findDashboardServer();
    if (!serverPath) {
        process.stderr.write('❌  Dashboard server not found.\n' +
            '   Build it first:\n' +
    ...
    process.stdout.write(`🐇  Starting HopCode Dashboard on http://localhost:${port} …\n`);
