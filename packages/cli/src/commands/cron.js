...
const addCommand = {
    ...
    handler: (argv) => {
        if (!validateCron(argv.cron)) {
            process.stderr.write(`Error: Invalid cron expression "${argv.cron}". Expected 5 fields: min hour day month weekday\n` +
        ...
        process.stdout.write(`✓ Created ${kind} job [${job.id}]\n` +
            `  Schedule: ${argv.cron}\n` +
            `  Prompt:   ${argv.prompt}\n`);
    },
};
...
const listCommand = {
    ...
    handler: () => {
        ...
        if (store.jobs.length === 0) {
            ...
            process.stdout.write('Add one with: hopcode cron add "0 9 * * MON" "your prompt"\n');
            return;
        }
        ...
        for (const job of store.jobs) {
            ...
            process.stdout.write(`[${job.id}] ${job.cronExpr}${label}\n` +
                `  Type:    ${kind}\n` +
                `  Prompt:  ${job.prompt}\n` +
...
export const cronCommand = {
    ...
    builder: (yargs) => yargs
        ...
        .command(showCommand)
        .demandCommand(1, 'You need at least one subcommand. Try: hopcode cron list')
        .version(false),
    handler: () => { },
};
