...
async function* streamJsonlMatches(filePath, queryTokens, sinceMs, modelFilter) {
...
function findAllJsonlFiles(root) {
    const files = [];
...
export const searchCommand = {
    ...
    handler: async (argv) => {
        ...
        if (argv.since) {
            ...
            if (isNaN(d.getTime())) {
                process.stderr.write(`Error: invalid --since date "${argv.since}". Use YYYY-MM-DD.\n`);
                process.exit(1);
            }
            sinceMs = d.getTime();
        }
        ...
        outer: for (const file of files) {
            for await (const match of streamJsonlMatches(file, queryTokens, sinceMs, argv.model)) {
                results.push(match);
        ...
        process.stdout.write(`Found ${results.length} match(es) for "${query}":\n\n`);
        for (const m of results) {
            ...
            process.stdout.write(`─── session:${session} · turn ${m.turnIndex} · ${m.role}${model} · ${ts}\n`);
            process.stdout.write(`    ${truncate(m.text)}\n\n`);
        }
        if (results.length >= limit) {
            process.stdout.write(`(showing first ${limit} results — use --limit to see more)\n`);
        }
    },
};
