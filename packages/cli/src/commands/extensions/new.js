...
import { isNodeError, resolveBundleDir } from '@hoptrendy/hopcode-core';
...
async function handleNew(args) {
    try {
        if (args.template) {
            ...
            await writeFile(join(args.path, 'hopcode-extension.json'), JSON.stringify(manifest, null, 2));
            writeStdoutLine(`Successfully created new extension at ${args.path}.`);
        }
        writeStdoutLine(`You can install this using "hopcode extensions link ${args.path}" to test it out.`);
    }
    catch (error) {
