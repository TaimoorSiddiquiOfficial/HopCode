 ...
 * extensionConfig.
 */
export function extensionConsentString(extensionConfig, commands = [], skills = [], subagents = [], originSource = 'HopCode') {
    const output = [];
    if (originSource !== 'HopCode') {
        output.push(t('You are installing an extension from {{originSource}}. Some features may not work perfectly with HopCode.', { originSource }));
    }
    ...
    if (extensionConfig.contextFileName) {
        ...
        output.push(t('This extension will append info to your HOPCODE.md context using {{fileName}}', { fileName }));
    }
    if (skills.length > 0) {
...
export const requestConsentOrFail = async (requestConsent, options) => {
    if (!options)
        return;
    const { extensionConfig, originSource = 'HopCode', commands = [], skills = [], subagents = [], previousExtensionConfig, previousCommands = [], previousSkills = [], previousSubagents = [], } = options;
