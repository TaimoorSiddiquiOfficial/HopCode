...
const mockrunHopCodeServe = vi.hoisted(() => vi.fn());
...
vi.mock('../serve/run-hopcode-serve.js', () => ({
    runHopCodeServe: mockrunHopCodeServe,
}));
...
describe('serve rate limit env parsing', () => {
    ...
    it.each([
        ['HOPCODE_SERVE_RATE_LIMIT_PROMPT', '0x10'],
        ['HOPCODE_SERVE_RATE_LIMIT_MUTATION', '1e3'],
        ['HOPCODE_SERVE_RATE_LIMIT_READ', '2.5'],
        ['HOPCODE_SERVE_RATE_LIMIT_WINDOW_MS', '0x3e8'],
    ...
    it('passes decimal env values to runHopCodeServe', async () => {
        process.env['HOPCODE_SERVE_RATE_LIMIT_PROMPT'] = '11';
        process.env['HOPCODE_SERVE_RATE_LIMIT_MUTATION'] = ' 31 ';
        process.env['HOPCODE_SERVE_RATE_LIMIT_READ'] = '121';
        process.env['HOPCODE_SERVE_RATE_LIMIT_WINDOW_MS'] = '60000';
        mockrunHopCodeServe.mockResolvedValueOnce({
            url: 'http://127.0.0.1:4170/',
            webShellMounted: false,
        });
        await startServeHandler();
        expect(mockrunHopCodeServe).toHaveBeenCalledWith(expect.objectContaining({
            rateLimit: true,
            rateLimitPrompt: 11,
    ...
    it('passes --max-total-sessions to runHopCodeServe', async () => {
        ...
        expect(mockrunHopCodeServe).toHaveBeenCalledWith(expect.objectContaining({ maxTotalSessions: 42 }));
    });
...
describe('maybeOpenWebShellBrowser', () => {
    ...
    it('skips --open when the runtime failed to mount', async () => {
        ...
        expect(mockOpenBrowserSecurely).not.toHaveBeenCalled();
        expect(stderrWrites.join('')).toContain('hopcode serve: Web Shell runtime not ready; skipping --open: runtime boom');
    });
...
describe('serve startup import boundary', () => {
    it('reaches listening through the dev entrypoint without loading interactive Ink internals first', async () => {
        ...
        const hopcodeHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-import-boundary-home-')));
        ...
        try {
            const reachedListening = await new Promise((resolve, reject) => {
                ...
                child.stdout.on('data', (chunk) => {
                    ...
                    if (stdout.includes('hopcode serve listening on')) {
            ...
            await removeTempDir(hopcodeHome);
        }
    }, 20_000);
});
