...
import { fetchOpenAICompatibleModels, fetchOpenRouterModels, } from './discovery.js';
...
function flattenToOptions(categories) {
    return categories.flatMap((cat) => cat.models.map((m) => ({
        value: m.id,
        label: m.label,
...
function printCategories(categories, currentModel) {
...
// ── Handler ─────────────────────────────────────────────────────────────────
async function handleModelCommand(argv) {
    ...
    if (info.providerId === 'ollama-local' ||
        ...
        if (providerDef?.liveModels && providerDef.baseUrl) {
            ...
            if (info.providerId === 'openrouter') {
                ...
                categories = await fetchOpenAICompatibleModels(providerDef.baseUrl, apiKey);
            }
            if (categories) {
                process.stdout.write(' ✓\n');
            }
            else {
                process.stdout.write(t(' (could not reach provider, showing catalog)\n'));
            }
        }
    }
    ...
    if (!categories) {
        ...
        if (!catalog) {
            writeStderrLine(t('No model catalog available for provider: {{id}}', {
                id: info.providerId,
            }));
            process.exit(1);
        }
    ...
    if (!options.length) {
        writeStderrLine(t('No models found for provider: {{id}}', { id: info.providerId }));
        process.exit(1);
    }
    const selector = new InteractiveSelector(options, t('Select a model (↑↓ navigate, Enter select, Esc cancel):'));
    ...
    const existingProviders = settings.merged?.modelProviders?.[providerKey] ?? [];
    if (existingProviders.length > 0) {
        existingProviders[0]['id'] = chosen;
        settings.setValue(scope, `modelProviders.${providerKey}`, existingProviders);
    }
    writeStdoutLine(t('\n  ✓ Model switched to: {{model}}\n', { model: chosen }));
    writeStdoutLine(t('  Restart HopCode for the model change to take effect in a running session.\n'));
}
// ── Command definition ───────────────────────────────────────────────────────
export const modelCommand = {
    command: 'model',
    describe: t('Select or list available models for the active provider'),
    builder: (yargs) => yargs.option('list', {
        alias: 'l',
        type: 'boolean',
