 ...
 */
function buildModelDescription(tag, isCloud) {
    const parts = [];
    if (!isCloud) {
 ...
 */
function ollamaHttpError(status, isCloud) {
 ...
 */
export async function fetchOllamaModels(baseUrl, apiKey, timeoutMs = 4000) {
    ...
    try {
        const categories = await fetchOpenAICompatibleModels(baseUrl, apiKey, timeoutMs);
        if (categories)
            return categories;
    }
    catch (err) {
