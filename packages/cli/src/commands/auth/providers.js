 ...
 */
import { AuthType, getErrorMessage, } from '@hoptrendy/hopcode-core';
...
import { PROVIDER_REGISTRY, getProvider, detectActiveProvider, } from './registry.js';
 ...
 */
export async function promptForApiKey(providerLabel) {
    return promptForSecretInput(t('Enter your {{provider}} API key: ', { provider: providerLabel }));
}
/**
 * Run a quick connectivity check against an Ollama endpoint.
 * Returns a human-readable status line, never throws.
 */
async function checkOllamaConnectivity(baseUrl, apiKey) {
 ...
 */
export async function handleApiKeyAuth(providerId, options = {}, overrideBaseUrl) {
    ...
    try {
        ...
        const config = await loadCliConfig(settings.merged, minimalArgv, process.cwd(), [], {
        ...
        if (!apiKey) {
            if (provider.requiresApiKey) {
                if (providerId === 'ollama-cloud') {
                    writeStdoutLine(t('  Get your API key at: https://ollama.com → Account → API Keys'));
                }
        ...
        writeStdoutLine(t('Configuring {{provider}} authentication...', {
        ...
        if (modelProvidersKey) {
            const existingConfigs = (settings.merged.modelProviders ??
                {})[modelProvidersKey] ?? [];
            // Remove any existing entry with the same envKey+baseUrl to keep settings clean
            const filteredConfigs = existingConfigs.filter((c) => !(c.envKey === provider.envKey && c.baseUrl === effectiveBaseUrl));
            const updatedConfigs = [newModelConfig, ...filteredConfigs];
            settings.setValue(authTypeScope, `modelProviders.${modelProvidersKey}`, updatedConfigs);
            // Set model.name to the default model
            settings.setValue(authTypeScope, 'model.name', provider.defaultModel);
        }
        // 4. Set selectedType
        settings.setValue(authTypeScope, 'security.auth.selectedType', provider.authType);
        // 5. Refresh auth to validate the configuration
        await config.refreshAuth(provider.authType);
        writeStdoutLine(t('✓ {{provider}} authentication configured successfully!', {
            provider: provider.label,
        }));
        ...
        if (providerId.startsWith('ollama')) {
            ...
            writeStdoutLine(t('  Ollama running at {{url}} — start Ollama first with `ollama serve`', { url: provider.baseUrl ?? 'http://localhost:11434' }));
        }
        writeStdoutLine(t('  Default model: {{model}}', { model: provider.defaultModel }));
        writeStdoutLine(t('  Switch model anytime with: hopcode -m <model-id>'));
        process.exit(0);
    }
    catch (error) {
        writeStderrLine(t('Failed to configure {{provider}}: {{error}}', {
            provider: provider.label,
 ...
 */
export async function handleOllamaLocalAuth(options = {}) {
    const { host } = options;
