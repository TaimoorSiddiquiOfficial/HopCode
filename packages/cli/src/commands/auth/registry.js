...
export const PROVIDER_REGISTRY = [
    ...
    },
    {
        id: 'ollama-cloud',
        label: 'Ollama Cloud',
        description: 'Llama, DeepSeek, Qwen & more on Ollama Cloud · Requires OLLAMA_CLOUD_API_KEY',
    ...
    },
    {
        id: 'nvidia-nim',
        label: 'NVIDIA NIM',
        description: 'NVIDIA-hosted Llama, Mistral & more · Requires NVIDIA_API_KEY',
    ...
    // ── Local providers ───────────────────────────────────────────────────────
    {
        ...
        description: 'Local models via LM Studio at localhost:1234 · No API key needed',
 ...
 */
export function detectActiveProvider(modelProvidersEntry) {
    if (!modelProvidersEntry || modelProvidersEntry.length === 0)
        return undefined;
    const first = modelProvidersEntry[0];
    if (!first)
        return undefined;
    if (isCodingPlanConfig(first.baseUrl, first.envKey))
        return undefined;
    return PROVIDER_REGISTRY.find((p) => p.envKey === first.envKey && p.baseUrl === first.baseUrl);
}
