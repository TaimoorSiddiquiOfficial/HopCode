...
function findSessionFiles(limit = 20) {
...
function findSessionById(sessionId) {
    ...
    const projectDirs = fs
        ...
        .filter((d) => d.isDirectory());
    for (const pd of projectDirs) {
        const filePath = path.join(projectsDir, pd.name, 'chats', `${sessionId}.jsonl`);
...
function readSession(filePath, maxRecords) {
    ...
    for (const rec of records) {
        switch (rec.type) {
            ...
            case 'assistant': {
                const text = extractText(rec.message);
                if (text.trim())
                    lines.push(`ASSISTANT: ${text.trim().slice(0, 800)}`);
                break;
            }
            case 'tool_result':
...
function buildLearningPrompt(sessionId, transcript) {
...
// Main handler
// ──────────────────────────────────────────────────────────────
async function handleLearn(sessionIdArg, list, dryRun, maxTurns) {
    // --list mode
    if (list) {
    ...
    if (sessionIdArg) {
        ...
        if (all.length === 0) {
            process.stderr.write('No sessions found. Run hopcode first to create a session.\n');
            process.exit(1);
        }
        sessionFile = all[0];
        process.stdout.write(`📖  Analysing most recent session: ${sessionFile.sessionId}\n`);
    }
    // Read and format session
    const records = readSession(sessionFile.filePath, maxTurns);
    if (records.length < 3) {
        process.stderr.write(`Session ${sessionFile.sessionId} is too short (${records.length} records). ` +
    ...
    process.stdout.write(`🧠  Distilling skill from session (${records.length} records)…\n\n`);
