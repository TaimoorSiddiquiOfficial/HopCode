...
async function readSkillsFromDir(dir, level) {
...
const listCommand = {
    ...
    handler: async () => {
        ...
        if (skills.length === 0) {
            ...
            writeStdoutLine(t('  Bundled skills (batch, loop, qc-helper, review, spec-driven, git-workflow,\n' +
                '  codebase-map, changelog, mcp-builder) are available as /slash commands.\n'));
            writeStdoutLine(t('  Add a skill: hopcode skills add <github-url | local-path>\n'));
            return;
        }
        ...
        writeStdoutLine('');
        writeStdoutLine(t('  Bundled skills are available as /slash commands (use /skills to list them).\n'));
    },
};
const showCommand = {
    command: 'show <name>',
    ...
    handler: async (argv) => {
        ...
        if (!skill) {
            writeStderrLine(t(`Skill "{{name}}" not found in user or project skills.\n` +
                `  Run: hopcode skills list`, { name: argv.name }));
            process.exit(1);
        }
...
const addCommand = {
    command: 'add <source>',
    describe: t('Install a skill from a GitHub URL or local path to ~/.hopcode/skills/<name>/'),
    ...
    handler: async (argv) => {
        ...
        if (isGithubUrl(source)) {
            ...
            const rawUrl = source.includes('SKILL.md')
                ? toRawUrl(source)
                : toRawUrl(source.endsWith('/') ? `${source}SKILL.md` : `${source}/SKILL.md`);
            writeStdoutLine(t(`\n  Fetching skill from: ${rawUrl}\n`));
            try {
                const response = await fetch(rawUrl);
                if (!response.ok) {
                    writeStderrLine(t(`Failed to fetch skill: HTTP ${response.status} ${response.statusText}`));
                    process.exit(1);
                }
                content = await response.text();
            }
            catch (err) {
                writeStderrLine(t(`Network error: ${err instanceof Error ? err.message : String(err)}`));
                process.exit(1);
            }
        }
        else {
            ...
            const isDir = fsSync.existsSync(localPath) &&
        ...
        if (!content.includes('---')) {
            writeStderrLine(t('Invalid SKILL.md: missing YAML frontmatter (expected --- delimiters).'));
            process.exit(1);
        }
        ...
        writeStdoutLine(t('  Restart HopCode for the skill to appear as a /slash command.\n'));
    },
};
const removeCommand = {
    command: 'remove <name>',
    ...
    handler: async (argv) => {
        ...
        if (!fsSync.existsSync(targetDir)) {
            writeStderrLine(t(`Skill "${argv.name}" not found in user skills (${userSkillsDir()}).\n` +
