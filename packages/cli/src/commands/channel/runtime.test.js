...
import { daemonObservedContactsPath, daemonSessionRoutesPath, parseConfiguredChannels, registerPermissionRelay, registerSessionCleanup, sessionsPath, } from './runtime.js';
...
it('isolates daemon route stores by workspace hash', () => {
    expect(daemonSessionRoutesPath('/workspace')).toBe('/tmp/qwen/channels/daemon/workspace-hash/routes.json');
    expect(daemonSessionRoutesPath('/other')).toBe('/tmp/qwen/channels/daemon/other-hash/routes.json');
    expect(daemonSessionRoutesPath('/workspace')).not.toBe(sessionsPath());
});
it('isolates observed contact stores beside daemon routes', () => {
    expect(daemonObservedContactsPath('/workspace')).toBe('/tmp/qwen/channels/daemon/workspace-hash/observed-contacts.json');
    expect(daemonObservedContactsPath('/other')).toBe('/tmp/qwen/channels/daemon/other-hash/observed-contacts.json');
