...
import { addChannelMemoryEntries, clearChannelMemory, getChannelMemoryRevision, listChannelMemoryEntries, readChannelMemory, removeChannelMemoryEntries, updateChannelMemoryEntry, } from '@hoptrendy/hopcode-core';
import { loadSettings } from '../../config/settings.js';
import { DaemonChannelBridge, sanitizeLogText, SessionRouter, } from '@hoptrendy/channel-base';
import { normalizeServeChannelSelection } from '../../serve/channel-selection.js';
import { CHANNEL_DAEMON_WORKER_SENTINEL, CHANNEL_WORKER_HEARTBEAT_INTERVAL_MS, QWEN_DAEMON_TOKEN_ENV, QWEN_DAEMON_URL_ENV, QWEN_DAEMON_WORKSPACE_ENV, HOPCODE_SERVER_TOKEN_ENV, } from '../../serve/channel-worker-env.js';
...
import { createChannel, daemonObservedContactsPath, daemonSessionRoutesPath, loadChannelsConfig, loadChannelsFromExtensions, parseConfiguredChannels, registerPermissionRelay, registerSessionCleanup, registerToolCallDispatch, selectFirstModel, } from './runtime.js';
...
async function loadDaemonSdk() {
    return (await import('@hoptrendy/sdk/daemon'));
}
    ...
    handler: async (argv) => {
        ...
        try {
            ...
            const handle = await runChannelDaemonWorker({
                ...
                reportStartup: (message) => reportStartupToSupervisor(message, startupAbortController.signal),
                sendReady: (ready) => {
                ...
                if (shuttingDown) {
                    ...
                    try {
                        if (activeWebhookTasks.size > 0) {
                            writeStderrLine(`[Channel] shutdown: draining ${activeWebhookTasks.size} webhook task(s)...`);
                            await Promise.race([
                                Promise.allSettled(activeWebhookTasks.values()),
                                new Promise((resolve) => {
                                    const timer = setTimeout(resolve, WEBHOOK_TASK_SHUTDOWN_DRAIN_MS);
                                    timer.unref();
                                }),
                            ]);
                        }
