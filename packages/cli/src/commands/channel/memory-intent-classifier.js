...
export class BridgeChannelMemoryIntentClassifier {
    ...
    async classifyChannelMemoryIntent(text, entries = []) {
        ...
        try {
            const response = await bridge.prompt(sessionId, `${CLASSIFIER_PROMPT}${JSON.stringify(text)}${buildMemoryManifest(entries)}`, {});
            try {
            ...
            try {
                ...
                process.stderr.write(`[classifier] cancelSession failed: ${sanitizeLogText(error instanceof Error ? error.message : String(error), 200)}\n`);
            }
        }
    }
}
