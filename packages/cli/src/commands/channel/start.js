import { addChannelMemoryEntries, clearChannelMemory, getChannelMemoryRevision, listChannelMemoryEntries, nextFireTime, parseCron, readChannelMemory, removeChannelMemoryEntries, updateChannelMemoryEntry, } from '@hoptrendy/hopcode-core';
import { loadSettings } from '../../config/settings.js';
import { writeStderrLine, writeStdoutLine } from '../../utils/stdioHelpers.js';
import { AcpBridge, ChannelLoopScheduler, ChannelLoopStore, SessionRouter, } from '@hoptrendy/channel-base';
...
import { createChannel, channelLoopPath, loadChannelsConfig, loadChannelsFromExtensions, parseConfiguredChannels, registerPermissionRelay, registerSessionCleanup, registerToolCallDispatch, selectFirstModel, sessionsPath, } from './runtime.js';
...
async function startSingle(name, proxy, cronEnabled) {
    ...
    try {
        ...
        writeStderrLine(`[Channel] Another channel service started concurrently. Error: ${err instanceof Error ? err.message : String(err)}`);
            ...
            try {
                ...
                for (const [name, channel] of connectedChannels) {
                    try {
                        ...
                        writeStderrLine(`[Channel] "${name}" failed to reconnect: ${err instanceof Error ? err.message : String(err)}`);
