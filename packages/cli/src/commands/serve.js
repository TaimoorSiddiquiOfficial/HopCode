...
import { ApprovalMode, MCP_BUDGET_WARN_FRACTION, openBrowserSecurely, parsePositiveIntegerEnv, shouldLaunchBrowser, } from '@hoptrendy/hopcode-core';
...
export async function maybeOpenWebShellBrowser(handle, open) {
    ...
    try {
        ...
        writeStderrLine(`hopcode serve: Web Shell runtime not ready; skipping --open: ${runtimeErr instanceof Error ? runtimeErr.message : String(runtimeErr)}`);
        return;
    }
    try {
        ...
        if (handle.resolvedToken) {
            ...
            writeStderrLine('hopcode serve: --open passes the token in the browser launch command ' +
                '(visible via `ps` / /proc); on a multi-user host open the URL manually instead.');
        }
        await openBrowserSecurely(target.toString());
    }
    catch (browserErr) {
        writeStderrLine(`hopcode serve: failed to open browser: ${browserErr instanceof Error ? browserErr.message : String(browserErr)}`);
    }
}
    ...
    handler: async (argv) => {
        ...
        if (mcpClientBudget !== undefined) {
            if (!Number.isFinite(mcpClientBudget) ||
                ...
                mcpClientBudget <= 0) {
                writeStderrLine('hopcode serve: --mcp-client-budget must be a positive integer.');
                process.exit(1);
            }
        }
        if (mcpBudgetMode === 'enforce' && mcpClientBudget === undefined) {
            writeStderrLine('hopcode serve: --mcp-budget-mode=enforce requires --mcp-client-budget=N.');
            process.exit(1);
        }
        ...
        if (maxPendingPromptsPerSession !== Number.POSITIVE_INFINITY &&
            ...
            writeStderrLine('hopcode serve: --max-pending-prompts-per-session must be a non-negative integer (0 / Infinity = unlimited).');
            process.exit(1);
        }
        if (mcpClientBudget !== undefined) {
        ...
        const rateLimit = argv['rate-limit'] ??
            (process.env['HOPCODE_SERVE_RATE_LIMIT'] === '1' ||
                process.env['HOPCODE_SERVE_RATE_LIMIT'] === 'true');
        let rateLimitPrompt;
        ...
        if (rateLimit) {
            ...
            rateLimitWindowMs =
                argv['rate-limit-window-ms'] ??
                    envInt('HOPCODE_SERVE_RATE_LIMIT_WINDOW_MS');
            for (const [name, value] of [
                ...
                if (value !== undefined &&
                    ...
                    writeStderrLine(`hopcode serve: ${name} must be a positive integer.`);
                    process.exit(1);
                }
            }
            if (rateLimitWindowMs !== undefined &&
                ...
                writeStderrLine('hopcode serve: --rate-limit-window-ms must be an integer >= 1000.');
                process.exit(1);
            }
        }
        ...
        const { runHopCodeServe } = await import('../serve/run-hopcode-serve.js');
        try {
