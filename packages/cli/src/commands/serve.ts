...
export async function maybeOpenWebShellBrowser(
  ...
  try {
    ...
    writeStderrLine(
      `hopcode serve: Web Shell runtime not ready; skipping --open: ${
  ...
  try {
    ...
    if (handle.resolvedToken) {
      ...
      writeStderrLine(
        'hopcode serve: --open passes the token in the browser launch command ' +
    ...
    writeStderrLine(
      `hopcode serve: failed to open browser: ${browserErr instanceof Error ? browserErr.message : String(browserErr)}`,
    );
  }
}
  ...
  handler: async (argv) => {
    ...
    if (rateLimit) {
      ...
      if (
        ...
        writeStderrLine(
          'hopcode serve: --rate-limit-window-ms must be an integer >= 1000.',
        );
        process.exit(1);
      }
    }
