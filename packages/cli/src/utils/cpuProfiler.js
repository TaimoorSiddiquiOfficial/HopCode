 ...
 *
 * Three trigger modes:
 * 1. Environment variable: HOPCODE_CODE_CPU_PROFILE=1 — records from process start to exit
 ...
 *
 * Output: ~/.hopcode/cpu-profiles/hopcode-cpu-<pid>-<timestamp>.cpuprofile
...
export function initCpuProfiler() {
    ...
    const enabled = process.env['HOPCODE_CODE_CPU_PROFILE'] === '1';
    if (!enabled)
        return;
...
export async function stopCpuProfile(options) {
    ...
    try {
        ...
        const filePath = path.join(outputDir, `hopcode-cpu-${process.pid}-${formatTimestamp(now)}.cpuprofile`);
        try {
...
function defaultOutputDir() {
    return path.join(os.homedir(), '.hopcode', 'cpu-profiles');
}
...
function cleanupOldProfiles(outputDir, maxProfiles) {
    ...
    try {
        profiles = fs
            ...
            .filter((name) => name.startsWith('hopcode-cpu-') && name.endsWith('.cpuprofile'))
