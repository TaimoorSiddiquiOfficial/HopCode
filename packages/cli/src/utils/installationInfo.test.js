...
import { formatUpdateInstructions, getInstallationInfo, PackageManager, resolveUpdateCommand, } from './installationInfo.js';
...
describe('getInstallationInfo', () => {
    ...
    it('should detect Homebrew installation via execSync', () => {
        ...
        const info = getInstallationInfo(projectRoot, false);
        expect(mockedExecSync).toHaveBeenCalledWith('brew list -1 | grep -q "^hopcode$"', { stdio: 'ignore' });
    ...
    it('should detect global yarn installation', () => {
        ...
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('yarn global add @hoptrendy/hopcode@latest');
        expect(info.updateMessage).toContain('Attempting to automatically update');
        // isAutoUpdateEnabled = false -> "Please run..."
        const infoDisabled = getInstallationInfo(projectRoot, false);
...
describe('resolveUpdateCommand', () => {
    it('replaces @latest with the pinned stable version', () => {
        expect(resolveUpdateCommand('npm i -g @hoptrendy/hopcode@latest', '1.2.3')).toBe('npm i -g @hoptrendy/hopcode@1.2.3');
    });
    it('replaces @latest with @nightly for nightly versions', () => {
        expect(resolveUpdateCommand('npm i -g @hoptrendy/hopcode@latest', '1.2.3-nightly.20250101')).toBe('npm i -g @hoptrendy/hopcode@nightly');
    });
});
describe('formatUpdateInstructions', () => {
    ...
    it('resolves @latest in updateMessage-only guidance for nightly versions', () => {
        expect(formatUpdateInstructions({
            packageManager: PackageManager.NPM,
            isGlobal: true,
            updateMessage: 'Update requires sudo. Please run: sudo npm i -g @hoptrendy/hopcode@latest',
