...
describe('standalone-update', () => {
    let tempDir;
    beforeEach(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hopcode-update-test-'));
    });
    afterEach(() => {
    ...
    describe('rollbackStandaloneUpdate', () => {
        ...
        it('succeeds even with minimal manifest in .old', () => {
            ...
            fs.mkdirSync(standaloneDir);
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@hoptrendy/hopcode', version: '0.17.0' }));
    ...
    describe('ensureBinWrapper', () => {
        ...
        it.skipIf(process.platform === 'win32')('creates a Unix shell wrapper script', () => {
            ...
            const wrapperPath = path.join(tempDir, '.local', 'bin', 'hopcode');
        ...
        it('creates a Windows cmd wrapper', () => {
            ...
            const wrapperPath = path.join(tempDir, '.local', 'bin', 'hopcode.cmd');
            expect(fs.existsSync(wrapperPath)).toBe(true);
            const content = fs.readFileSync(wrapperPath, 'utf-8');
    ...
    describe('performStandaloneUpdate', () => {
        ...
        it('rejects directory without manifest as non-managed install', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('not a HopCode standalone install');
        });
    ...
    describe('isSafeTarEntryPath', () => {
        ...
        it('rejects parent-directory segments and absolute paths', () => {
            expect(isSafeTarEntryPath('../hopcode/manifest.json')).toBe(false);
            expect(isSafeTarEntryPath('hopcode/../manifest.json')).toBe(false);
            expect(isSafeTarEntryPath('hopcode\\..\\manifest.json')).toBe(false);
            expect(isSafeTarEntryPath('/tmp/hopcode/manifest.json')).toBe(false);
            expect(isSafeTarEntryPath('C:\\tmp\\hopcode\\manifest.json')).toBe(false);
    ...
    describe('rollbackStandaloneUpdate — concurrent lock protection', () => {
        ...
        it('proceeds when lock has dead PID', () => {
            ...
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@hoptrendy/hopcode', version: '0.17.0' }));
            fs.writeFileSync(path.join(oldDir, 'manifest.json'), JSON.stringify({ name: '@hoptrendy/hopcode', version: '0.16.0' }));
            fs.writeFileSync(lockPath, '999999999');
            const result = rollbackStandaloneUpdate(standaloneDir);
            expect(result.ok).toBe(true);
        });
    ...
    describe.skipIf(process.platform === 'win32')('ensurePathInShellRc', () => {
        ...
        it('skips if legacy marker already in rc file', () => {
            const binDir = path.join(tempDir, 'bin');
            const zshrc = path.join(tempDir, '.zshrc');
            fs.writeFileSync(zshrc, `# Added by HopCode standalone installer\nexport PATH="${binDir}:$PATH"\n`);
            const origShell = process.env['SHELL'];
            const origHome = process.env['HOME'];
            process.env['SHELL'] = '/bin/zsh';
            process.env['HOME'] = tempDir;
