 ...
 */
import { AuthType, MODEL_GENERATION_CONFIG_FIELDS, normalizeReasoningEffort, REASONING_EFFORT_TIERS, resolveModelConfig, resolveProviderProtocol, stripRuntimeSnapshotPrefix, } from '@hoptrendy/hopcode-core';
...
const AUTH_ENV_MODEL_VARS = {
    [AuthType.USE_OPENAI]: ['OPENAI_MODEL', 'HOPCODE_MODEL'],
...
function buildSkippedProviderWarnings(modelProviders, providerProtocol) {
    ...
    for (const [providerId, models] of Object.entries(modelProviders)) {
        ...
        if (protocol === AuthType.HOPCODE_OAUTH &&
            providerId !== AuthType.HOPCODE_OAUTH) {
            warnings.push(`Warning: modelProviders provider "${providerId}" maps to "hopcode-oauth" via providerProtocol, but hopcode-oauth uses hard-coded models only; its ${models.length} model(s) are ignored.`);
            continue;
        }
 ...
 * - USE_OPENAI: OPENAI_MODEL, HOPCODE_MODEL
