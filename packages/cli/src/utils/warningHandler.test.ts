...
const ENV_KEYS = ['NODE_ENV', 'DEBUG', 'HOPCODE_DEBUG'] as const;
...
describe('initializeWarningHandler', () => {
  ...
  it('keeps warnings visible when HOPCODE_DEBUG is set', () => {
    process.env['HOPCODE_DEBUG'] = '1';
    initializeWarningHandler();
    emit(
      makeWarning(
        'MaxListenersExceededWarning',
  ...
  it('a child process with the handler installed does not print suppressed AbortSignal warnings to stderr', async () => {
    ...
    try {
      const script = `
        ...
        delete process.env.DEBUG; delete process.env.HOPCODE_DEBUG;
