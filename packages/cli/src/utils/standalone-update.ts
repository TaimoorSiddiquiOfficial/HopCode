...
const OSS_BASE =
  'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/releases/hopcode';
const GITHUB_BASE = 'https://github.com/QwenLM/hopcode/releases/download';
...
async function verifyChecksum(
  ...
  // key and publishes SHA256SUMS.sig, set HOPCODE_REQUIRE_SIGNATURE=1 to enforce.
  // Until then, verification is best-effort (passes when .sig exists, warns when not).
  const requireSig = process.env['HOPCODE_REQUIRE_SIGNATURE'] === '1';
  ...
  if (sigResponse) {
    ...
    throw new Error(
      'SHA256SUMS.sig not found and HOPCODE_REQUIRE_SIGNATURE=1 is set',
    );
  } else {
    debugLogger.warn(
      'SHA256SUMS.sig not available — update integrity relies on SHA256 checksum only. ' +
        'Set HOPCODE_REQUIRE_SIGNATURE=1 to enforce signature verification.',
    );
  }
...
/**
 * Ensures ~/.local/bin/hopcode exists and points to the standalone install.
...
export function ensureBinWrapper(standaloneDir: string, target: string): void {
  ...
  try {
    ...
    if (target.startsWith('win')) {
      ...
      const wrapperPath = path.join(binDir, 'hopcode.cmd');
      if (!fs.existsSync(wrapperPath)) {
        const content = `@echo off\r\ncall "${standaloneDir}\\bin\\hopcode.cmd" %*\r\n`;
      ...
      if (!fs.existsSync(wrapperPath)) {
        const content = `#!/bin/sh\nexec "${standaloneDir}/bin/hopcode" "$@"\n`;
 ...
 * Mirrors the logic in install-hopcode-standalone.sh maybe_update_shell_path.
 */
export function ensurePathInShellRc(binDir: string): void {
  ...
  if (shell.endsWith('/zsh')) {
    ...
    // Match install-hopcode-standalone.sh's maybe_update_shell_path logic.
  ...
  try {
    ...
    const marker = '# Added by HopCode standalone installer';
...
export async function performStandaloneUpdate(
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    // Directory exists but has no manifest — not a managed HopCode install.
    // Refuse to overwrite to avoid data loss.
    throw new Error(
      `${standaloneDir} exists but is not a HopCode standalone install. Remove it manually to proceed.`,
    );
  } else {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hopcode-update-'));
  let extractDir: string;
  let updateResult: 'done' | 'deferred' | undefined;
  try {
    extractDir = fs.mkdtempSync(path.join(parentDir, '.hopcode-update-'));
  } catch (err) {
  ...
  try {
    ...
    const newInstallDir = path.join(extractDir, 'hopcode');
    if (!fs.existsSync(path.join(newInstallDir, 'manifest.json'))) {
      throw new Error(
        'Extracted archive does not contain expected hopcode directory',
      );
    }
    ...
    if (fs.existsSync(oldDir)) {
      if (isFirstTimeMigration) {
        ...
        try {
          ...
          fs.writeFileSync(
            path.join(oldDir, '.hopcode-rollback-info.json'),
