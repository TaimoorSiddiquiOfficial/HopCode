...
import { startBackgroundHousekeeping, _resetForTesting, _needsCatchUpForTesting, _runHousekeepingForTesting, _runPassForTesting, _FILE_HISTORY_MARKER_FOR_TESTING, _getSubagentMarkerPathForTesting, } from './scheduler.js';
...
describe('_runHousekeepingForTesting', () => {
    ...
    it('whitelists the current session via lazy getSessionId()', async () => {
        ...
        expect(fs.existsSync(path.join(hopcodeHome, _FILE_HISTORY_MARKER_FOR_TESTING))).toBe(true);
    });
    it('sweeps old subagent transcripts under <projectDir>/subagents, protecting the current session', async () => {
        const projectDir = path.join(hopcodeHome, 'project');
        ...
        expect(fs.existsSync(_getSubagentMarkerPathForTesting(hopcodeHome, projectDir))).toBe(true);
