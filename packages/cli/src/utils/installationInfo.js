...
const STANDALONE_UNIX_INSTALLER = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-hopcode-standalone.sh';
const STANDALONE_WINDOWS_INSTALLER = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-hopcode-standalone.ps1';
    ...
    try {
        ...
        if (process.platform === 'darwin') {
            try {
                ...
                childProcess.execSync('brew list -1 | grep -q "^hopcode$"', {
                    stdio: 'ignore',
                });
                return {
        ...
        if (!npmPrefixWritable) {
            ...
            return {
                ...
                isGlobal: true,
                updateMessage: 'Update requires sudo. Please run: sudo npm install -g @hoptrendy/hopcode@latest',
            };
        }
...
function isStandaloneInstallDir(installDir) {
    try {
        ...
        return (fs.existsSync(hopcodeBin) &&
