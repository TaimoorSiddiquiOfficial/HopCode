...
import { AuthType, resolveModelConfig, } from '@hoptrendy/hopcode-core';
...
describe('modelConfigUtils', () => {
    ...
    describe('resolveCliGenerationConfig', () => {
        ...
        beforeEach(() => {
            ...
            delete process.env['HOPCODE_MODEL'];
        ...
        it('warns when a custom provider maps to hopcode-oauth', () => {
            const settings = makeMockSettings({
                model: { name: 'some-model' },
                modelProviders: {
                    idealab: [{ id: 'qwen3.7-max' }],
                ...
                providerProtocol: {
                    idealab: AuthType.HOPCODE_OAUTH,
                },
            });
            vi.mocked(resolveModelConfig).mockReturnValue({
                config: { model: 'some-model', apiKey: '', baseUrl: '' },
            ...
            expect(result.warnings).toEqual(expect.arrayContaining([
                expect.stringContaining('modelProviders provider "idealab" maps to "hopcode-oauth"'),
            ]));
        });
        ...
        // Edge Case 3: OPENAI_MODEL over HOPCODE_MODEL when both are set and settings.model.name is not set
        it('Edge Case 3: OPENAI_MODEL should win over HOPCODE_MODEL when both set', () => {
            const argv = {};
        ...
        describe('[Regression] model precedence', () => {
            ...
            it('[Regression] HOPCODE_MODEL as fallback when OPENAI_MODEL not set', () => {
                ...
                vi.mocked(resolveModelConfig).mockImplementation(() => ({
                    ...
                    sources: { model: { kind: 'env', envKey: 'HOPCODE_MODEL' } },
                    warnings: [],
                }));
                resolveCliGenerationConfig({
                    argv: {},
