...
describe('memoryDiagnostics', () => {
    ...
    it('surfaces high heap pressure with actionable recommendations', () => {
        ...
        expect(report).toContain('restart HopCode to recover memory');
    ...
    it('writes heap snapshots to a diagnostics directory with stable filenames', () => {
        const outputDir = path.join(os.tmpdir(), 'hopcode-memory-diagnostics-test');
        ...
        expect(writtenPath).toBe(path.join(outputDir, `hopcode-heap-${process.pid}-2026-05-15T12-00-00-000Z.heapsnapshot`));
    });
    it('refuses heap snapshots when estimated heap dump would leave little free disk', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-disk-${process.pid}`);
    ...
    it('rate-limits repeated heap snapshot writes in the same directory', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-rate-limit-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        try {
    ...
    it('keeps only the newest heap snapshots after writing', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const oldSnapshot = path.join(outputDir, `hopcode-heap-${process.pid}-2026-05-15T11-00-00-000Z.heapsnapshot`);
        const newerSnapshot = path.join(outputDir, `hopcode-heap-${process.pid}-2026-05-15T11-30-00-000Z.heapsnapshot`);
    ...
    it('orders heap snapshot cleanup by modification time across process ids', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-mtime-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const newerLowPidSnapshot = path.join(outputDir, 'hopcode-heap-9-2026-05-15T12-30-00-000Z.heapsnapshot');
        const olderHighPidSnapshot = path.join(outputDir, 'hopcode-heap-12345-2026-05-15T12-00-00-000Z.heapsnapshot');
    ...
    it('uses filename timestamps to break equal-mtime cleanup ties', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-equal-mtime-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const olderSnapshot = path.join(outputDir, 'hopcode-heap-1-2026-05-15T11-00-00-000Z.heapsnapshot');
        const newerSnapshot = path.join(outputDir, 'hopcode-heap-1-2026-05-15T12-00-00-000Z.heapsnapshot');
    ...
    it('keeps successful heap snapshot writes when cleanup fails', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-cleanup-fail-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const brokenSymlink = path.join(outputDir, 'hopcode-heap-999-2026-05-15T11-00-00-000Z.heapsnapshot');
    ...
    it('removes partial heap snapshot files after a failed write', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-partial-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        let partialPath = '';
        try {
            expect(() => writeMemoryHeapSnapshot({
    ...
    it('creates heap snapshot directories and files with private permissions', () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-private-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        const writtenPath = writeMemoryHeapSnapshot({
            outputDir,
            now: new Date('2026-05-15T12:00:00.000Z'),
    ...
    it('refuses heap snapshot writes when free disk space cannot be read', async () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-statfs-fallback-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        vi.resetModules();
    ...
    it('falls back to process heap total when V8 heap statistics are unavailable for snapshot sizing', async () => {
        const outputDir = path.join(os.tmpdir(), `hopcode-memory-diagnostics-estimate-fallback-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        vi.resetModules();
