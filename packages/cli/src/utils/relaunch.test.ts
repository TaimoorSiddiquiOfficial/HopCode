  ...
  describe('when HOPCODE_NO_RELAUNCH is not set', () => {
    ...
    it.each([
      ...
      async (_name, marker, expectedElectron) => {
        process.argv = ['/usr/bin/node', '/app/cli.js'];
        if (marker) {
          process.env['HOPCODE_CODE_SCRUB_ELECTRON_RUN_AS_NODE'] = marker;
        } else {
        ...
        for (const call of mockedSpawn.mock.calls) {
          ...
          expect(env?.['HOPCODE_CODE_SCRUB_ELECTRON_RUN_AS_NODE']).toBe(marker);
          expect(env?.['HOPCODE_CODE_NO_RELAUNCH']).toBe('true');
        }
