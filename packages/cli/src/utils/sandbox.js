...
export async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = []) {
    ...
    if (process.env['BUILD_SANDBOX']) {
        if (!gcPath.includes('hopcode/packages/')) {
            throw new FatalSandboxError('Cannot build sandbox using installed HopCode binary; ' +
    ...
    if (isIntegrationTest) {
        containerName = `hopcode-integration-test-${randomBytes(4).toString('hex')}`;
    ...
    if (process.env['HOPCODE_CODE_MCP_APPROVALS_PATH']) {
        args.push('--env', `HOPCODE_CODE_MCP_APPROVALS_PATH=${getContainerPath(process.env['HOPCODE_CODE_MCP_APPROVALS_PATH'])}`);
    }
