 ...
 */
import { getInstallationInfo, resolveUpdateCommand, } from './installationInfo.js';
...
const UPDATE_SUCCESS_MESSAGE = 'Update successful! Please restart HopCode to use the new version. ' +
...
export function handleAutoUpdate(info, settings, projectRoot, spawnFn = spawnWrapper) {
    ...
    // enableAutoUpdate is checked in interactive.tsx before calling this function,
    ...
    updateProcess.on('close', (code) => {
        if (code === 0) {
            ...
            debugLogger.warn(`Automatic update command failed: ${updateCommand}; stderr: ${errorOutput.trim()}`);
            updateEventEmitter.emit('update-failed', {
