...
describe('resolveSeatbeltProfileFile', () => {
    ...
    it('keeps custom seatbelt profiles under project settings', () => {
        const bundleDir = path.resolve(path.sep, 'tmp', 'qwen', 'lib');
        const chunkUrl = pathToFileURL(path.join(bundleDir, 'chunks', 'sandbox-AAAA.js')).toString();
        expect(resolveSeatbeltProfileFile('project-profile', chunkUrl)).toBe(path.join(HOPCODE_DIR, 'sandbox-macos-project-profile.sb'));
    });
...
describe('parseSandboxImageName', () => {
    it('uses the image basename and tag for container names', () => {
        expect(parseSandboxImageName('ghcr.io/hopcode/hopcode:0.18.3')).toBe('hopcode-0.18.3');
    });
    it('handles registry ports without treating them as tags', () => {
        expect(parseSandboxImageName('localhost:5000/team/hopcode-sandbox:dev')).toBe('hopcode-sandbox-dev');
    });
    it('handles registry ports when the image is untagged', () => {
        expect(parseSandboxImageName('localhost:5000/team/hopcode-sandbox')).toBe('hopcode-sandbox');
    });
    it('drops digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/hopcode-sandbox@sha256:abcdef')).toBe('hopcode-sandbox');
    });
    it('keeps tags when dropping digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/hopcode-sandbox:dev@sha256:abcdef')).toBe('hopcode-sandbox-dev');
    });
});
