...
const OSS_BASE = 'https://hopcode-assets.oss-cn-hangzhou.aliyuncs.com/releases/hopcode';
const GITHUB_BASE = 'https://github.com/TaimoorSiddiquiOfficial/HopCode/releases/download';
...
async function verifyChecksum(actualHash, filename, versionPath) {
    ...
    if (sigResponse) {
        ...
        throw new Error('SHA256SUMS.sig not found and HOPCODE_REQUIRE_SIGNATURE=1 is set');
    }
    else {
...
function atomicReplace(standaloneDir, newDir, lockPath) {
    ...
    if (os.platform() === 'win32') {
        ...
        const scriptPath = path.join(path.dirname(standaloneDir), 'hopcode-update.bat');
...
export async function performStandaloneUpdate(standaloneDir, newVersion) {
    ...
    if (fs.existsSync(manifestPath)) {
        ...
        throw new Error(`${standaloneDir} exists but is not a HopCode standalone install. Remove it manually to proceed.`);
    }
    else {
    ...
    try {
        ...
        if (!fs.existsSync(path.join(newInstallDir, 'manifest.json'))) {
            throw new Error('Extracted archive does not contain expected hopcode directory');
        }
        ...
        if (fs.existsSync(oldDir)) {
            if (isFirstTimeMigration) {
                ...
                try {
                    ...
                    fs.writeFileSync(path.join(oldDir, '.hopcode-rollback-info.json'), JSON.stringify(rollbackInfo, null, 2));
                }
                catch {
