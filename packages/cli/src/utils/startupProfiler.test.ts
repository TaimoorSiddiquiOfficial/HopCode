...
describe('startupProfiler', () => {
  ...
  beforeEach(() => {
    ...
    vi.restoreAllMocks();
    saveEnv(
      'HOPCODE_PROFILE_STARTUP',
      'HOPCODE_PROFILE_STARTUP_OUTER',
      'HOPCODE_PROFILE_STARTUP_NO_HEAP',
      'SANDBOX',
    );
    delete process.env['HOPCODE_PROFILE_STARTUP'];
    delete process.env['HOPCODE_PROFILE_STARTUP_OUTER'];
    delete process.env['HOPCODE_PROFILE_STARTUP_NO_HEAP'];
  ...
  function enableProfiler() {
    process.env['HOPCODE_PROFILE_STARTUP'] = '1';
  ...
  describe('when outside sandbox (SANDBOX not set)', () => {
    it('should not enable profiler even with HOPCODE_PROFILE_STARTUP=1', () => {
      process.env['HOPCODE_PROFILE_STARTUP'] = '1';
      delete process.env['SANDBOX'];
  ...
  describe('when enabled (HOPCODE_PROFILE_STARTUP=1 + SANDBOX)', () => {
    beforeEach(() => {
      enableProfiler();
    });
    ...
    it('omits heap snapshots when HOPCODE_PROFILE_STARTUP_NO_HEAP=1', () => {
      process.env['HOPCODE_PROFILE_STARTUP_NO_HEAP'] = '1';
      initStartupProfiler();
      profileCheckpoint('phase_a');
      const report = getStartupReport()!;
  ...
  describe('outer-process opt-in (HOPCODE_PROFILE_STARTUP_OUTER=1)', () => {
    ...
    it('collects outside sandbox when OUTER=1 and writes outer-prefixed file', () => {
      process.env['HOPCODE_PROFILE_STARTUP'] = '1';
      process.env['HOPCODE_PROFILE_STARTUP_OUTER'] = '1';
      delete process.env['SANDBOX'];
