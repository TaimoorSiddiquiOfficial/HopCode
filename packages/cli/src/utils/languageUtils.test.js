...
describe('languageUtils', () => {
    ...
    describe('writeOutputLanguageFile', () => {
        ...
        it('should include machine-readable marker', () => {
            writeOutputLanguageFile('Chinese');
            const writtenContent = vi.mocked(fs.writeFileSync).mock.calls[0][1];
            expect(writtenContent).toContain('<!-- hopcode:llm-output-language: Chinese -->');
        });
        it('should sanitize language for marker (remove dangerous characters)', () => {
            ...
            expect(writtenContent).toContain('<!-- hopcode:llm-output-language: TestLanguage -->');
        });
        ...
        it('should write to custom targetPath when provided', () => {
            writeOutputLanguageFile('Korean', '/proj/.hopcode/output-language.md');
            expect(fs.mkdirSync).toHaveBeenCalledWith('/proj/.hopcode', {
                recursive: true,
            });
            expect(fs.writeFileSync).toHaveBeenCalledWith('/proj/.hopcode/output-language.md', expect.stringContaining('Korean'), 'utf-8');
        });
...
<!-- hopcode:llm-output-language: French -->
`);
            initializeLlmOutputLanguage();
        ...
        it('writes to config-bound path when available', () => {
            const config = {
                getOutputLanguageFilePath: vi
                    .fn()
                    .mockReturnValue('/proj/.hopcode/output-language.md'),
                setOutputLanguageFilePath: vi.fn(),
            };
            writeOutputLanguageAndRegisterPath('Chinese', config);
            expect(fs.writeFileSync).toHaveBeenCalledWith('/proj/.hopcode/output-language.md', expect.stringContaining('Chinese'), 'utf-8');
