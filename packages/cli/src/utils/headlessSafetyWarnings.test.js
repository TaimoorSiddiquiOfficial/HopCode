...
import { HEADLESS_IZN_NO_SANDBOX_WARNING, getHeadlessIznSafetyWarning, } from './headlessSafetyWarnings.js';
...
describe('getHeadlessIznSafetyWarning', () => {
    it('warns when approval mode is IZN and no sandbox is configured', () => {
        ...
        expect(getHeadlessIznSafetyWarning(cfg, {})).toBe(HEADLESS_IZN_NO_SANDBOX_WARNING);
    });
    ...
    it('does not warn when SANDBOX env is set to the value the sandbox transport actually writes', () => {
        const cfg = makeConfig(ApprovalMode.IZN, undefined);
        // macOS seatbelt
        expect(getHeadlessIznSafetyWarning(cfg, { SANDBOX: 'sandbox-exec' })).toBeNull();
        // Docker / Podman container name
        expect(getHeadlessIznSafetyWarning(cfg, { SANDBOX: 'hopcode-sandbox' })).toBeNull();
    ...
    it('warns when SANDBOX env is unset or empty string', () => {
        const cfg = makeConfig(ApprovalMode.IZN, undefined);
        expect(getHeadlessIznSafetyWarning(cfg, {})).toBe(HEADLESS_IZN_NO_SANDBOX_WARNING);
        expect(getHeadlessIznSafetyWarning(cfg, { SANDBOX: '' })).toBe(HEADLESS_IZN_NO_SANDBOX_WARNING);
    });
    it('respects the explicit suppression env var when set to 1 or true', () => {
        const cfg = makeConfig(ApprovalMode.IZN, undefined);
        expect(getHeadlessIznSafetyWarning(cfg, {
