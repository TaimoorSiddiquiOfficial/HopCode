...
import { Logger, recordSkillInvocation, uiTelemetryService, createDebugLogger, } from '@hoptrendy/hopcode-core';
...
function handleCommandResult(result, outputHistoryItems) {
    switch (result.type) {
        ...
        case 'message':
            return {
                type: 'message',
                messageType: result.messageType === 'warning'
                    ? 'info'
                    : result.messageType,
                content: result.content,
                ...(outputHistoryItems?.length ? { outputHistoryItems } : {}),
        ...
        case 'confirm_shell_commands':
            return {
                type: 'unsupported',
                reason: 'Shell command confirmation is not supported in non-interactive mode. Use Izn mode or pre-approve commands.',
                originalType: 'confirm_shell_commands',
            };
        case 'startImmediateSubagent':
            return {
                type: 'unsupported',
                reason: 'Immediate subagent launch is not supported in non-interactive mode.',
