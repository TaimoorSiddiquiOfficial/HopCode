            ...
            if (addItem) {
                if (config.getReasoningEffort() !== effort) {
                    addItem({
                        ...
                        text: t('Reasoning effort set to {{tier}}, but thinking is currently disabled — it will take effect when thinking is re-enabled.', { tier: effort }),
                    }, Date.now());
                }
                else {
                    addItem({
                        type: MessageType.INFO,
                        text: t('Reasoning effort: {{tier}} (requested; the effective tier depends on the active provider/model).', { tier: effort }),
                    }, Date.now());
                }
            }
        }
