...
import { fireNotificationHook, NotificationType, } from '@hoptrendy/hopcode-core';
import { sendNotification } from '../../services/notificationService.js';
export const LONG_TASK_NOTIFICATION_THRESHOLD_SECONDS = 20;
const NOTIFICATION_TITLE = 'HopCode';
    ...
    useEffect(() => {
        if (streamingState === StreamingState.WaitingForConfirmation &&
            ...
            const message = awaitingToolName
                ? `HopCode needs your permission to use ${awaitingToolName}`
        ...
        if (streamingState === StreamingState.Idle) {
            ...
            if (config && !idleNotificationSentRef.current) {
                ...
                if (hooksEnabled && messageBus) {
                    fireNotificationHook(messageBus, 'HopCode is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input')
                        .then((hookResult) => {
