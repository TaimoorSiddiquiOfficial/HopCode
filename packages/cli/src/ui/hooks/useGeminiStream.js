...
terminalWidthRef, midTurnRestoreRef) => {
        ...
        if (typeof query === 'string') {
            ...
            if (slashCommandResult) {
                switch (slashCommandResult.type) {
                    ...
                    default: {
                        throw new Error(`Unhandled slash command result type: ${String(slashCommandResult.type)}`);
                    }
                }
            }
    ...
    const handleContentEvent = useCallback((eventValue, currentGeminiMessageBuffer, userMessageTimestamp) => {
        ...
        let nextPendingType = pendingHistoryItemRef.current?.type;
        ...
        const flushBufferedStreamEvents = () => {
            ...
            while (bufferedEvents.length > 0) {
                ...
                if (nextEvent.kind === 'content') {
                    ...
                    geminiMessageBuffer = handleContentEvent(contentParts.join(''), geminiMessageBuffer, userMessageTimestamp);
                    continue;
                }
        ...
        return {
            ...
            accept: () => {
                for (const { message, parts, sideEffects } of resolvedForRecording) {
                    ...
                    addItem({ type: MessageType.NOTIFICATION, text: message }, Date.now());
                }
            },
        };
    ...
    const resolveDrainedSteerMessages = useCallback(async (messages, signal) => {
        try {
            ...
            if (!resolved || resolved.parts.length === 0)
                return undefined;
            let settled = false;
            return {
        ...
        return promptIdContext.run(prompt_id, async () => {
            ...
            try {
                ...
                if (immediateDuplicateToolResponses) {
                    immediateDuplicateToolResponsesRef.current = null;
                    await submitQuery(immediateDuplicateToolResponses.responseParts, SendMessageType.ToolResult, immediateDuplicateToolResponses.promptId);
                }
    ...
    const handleApprovalModeChange = useCallback(async (newApprovalMode) => {
        ...
        if (newApprovalMode === ApprovalMode.IZN ||
    ...
    const handleCompletedTools = useCallback(async (completedToolCallsFromScheduler) => {
        ...
        const geminiTools = completedAndReadyToSubmitTools.filter((t) => !t.request.isClientInitiated &&
            !historyCallIdsWithResponse.has(t.request.callId));
        const didRefreshManagedMemory = await refreshMemoryAfterManagedWrite(config, completedAndReadyToSubmitTools.map((toolCall) => ({
        ...
        const pendingDuplicateResponseParts = readyDuplicateBatches.flatMap((batch) => batch.responseParts);
