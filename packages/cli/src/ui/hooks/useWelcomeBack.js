...
import { clearWelcomeBackState, getProjectSummaryInfo, getWelcomeBackState, saveWelcomeBackRestartChoice, } from '@hoptrendy/hopcode-core';
...
export function useWelcomeBack(config, submitQuery, buffer, settings) {
    ...
    const handleWelcomeBackSelection = useCallback((choice) => {
        ...
        if (choice === 'continue' && welcomeBackInfo?.content) {
            ...
            const contextMessage = `@.hopcode/PROJECT_SUMMARY.md, Based on our previous conversation,Let's continue?`;
