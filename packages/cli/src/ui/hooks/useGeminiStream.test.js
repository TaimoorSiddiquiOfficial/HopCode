...
import { ApprovalMode, AUTONOMOUS_SENTINEL_DYNAMIC, AuthType, GeminiEventType as ServerGeminiEventType, SendMessageType, ToolErrorType, ToolConfirmationOutcome, } from '@hoptrendy/hopcode-core';
...
describe('useGeminiStream', () => {
    ...
    const renderTestHook = (initialToolCalls = [], geminiClient, availableTerminalHeightRef) => {
        ...
        const { result, rerender } = renderHook((props) => {
            ...
            return useGeminiStream(props.client, props.history, props.addItem, props.config, true, props.loadedSettings, props.onDebugMessage, props.handleSlashCommand, props.shellModeActive, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, undefined, // midTurnDrainRef
            undefined, // logger
    ...
    it('records mid-turn queued user messages after tool results accept them', async () => {
        ...
        const queuedPromptAddItemIndex = mockAddItem.mock.calls.findIndex(([item]) => item.type === MessageType.NOTIFICATION && item.text === queuedPrompt);
        ...
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.NOTIFICATION, text: queuedPrompt }, expect.any(Number));
    ...
    it('provides queued steer input to core at the next sampling boundary', async () => {
        ...
        expect(mockAddItem).not.toHaveBeenCalledWith({ type: MessageType.NOTIFICATION, text: steeredPrompt }, expect.any(Number));
        steerInput?.accept();
        expect(recordMidTurnUserMessage).toHaveBeenCalledWith([{ text: steeredPrompt }], steeredPrompt);
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.NOTIFICATION, text: steeredPrompt }, expect.any(Number));
    });
    ...
    it('skips mid-turn @ fallback side effects when cancelled during resolution', async () => {
        ...
        expect(resolveSignal?.aborted).toBe(true);
        expect(recordMidTurnUserMessage).not.toHaveBeenCalled();
        expect(mockAddItem).not.toHaveBeenCalledWith({ type: MessageType.NOTIFICATION, text: queuedPrompt }, expect.any(Number));
        expect(mockSendMessageStream).not.toHaveBeenCalled();
    });
    ...
    describe('Slash Command Handling', () => {
        ...
        describe('inline model override', () => {
            ...
            const allowInlineModel = (modelId = 'inline-model') => {
                ...
                mockConfig.getContentGeneratorConfig = vi.fn(() => ({ authType: AuthType.HOPCODE_OAUTH }));
                mockConfig.getAvailableModelsForAuthType = vi.fn(() => [{ id: modelId, authType: AuthType.HOPCODE_OAUTH }]);
            };
