...
describe('handleAtCommand', () => {
    ...
    describe('hopcode-ignore filtering', () => {
        it('should skip hopcode-ignored files in @ commands', async () => {
            await createTestFile(path.join(testRootDir, '.hopcodeignore'), 'build/output.js');
            const hopcodeIgnoredFile = await createTestFile(path.join(testRootDir, 'build', 'output.js'), 'console.log("Hello");');
            ...
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Path ${hopcodeIgnoredFile} is hopcode-ignored and will be skipped.`);
        ...
        it('should skip files ignored by .agentignore in @ commands', async () => {
            ...
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Path ${agentIgnoredFile} is hopcode-ignored and will be skipped.`);
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Ignored 1 files:\nhopcode-ignored: ${agentIgnoredFile}`);
        });
    });
    ...
    it('should handle mixed hopcode-ignored and valid files', async () => {
        await createTestFile(path.join(testRootDir, '.hopcodeignore'), 'dist/bundle.js');
        const validFile = await createTestFile(path.join(testRootDir, 'src', 'main.ts'), '// Main application entry');
        const hopcodeIgnoredFile = await createTestFile(path.join(testRootDir, 'dist', 'bundle.js'), 'console.log("bundle");');
