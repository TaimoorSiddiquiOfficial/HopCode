            ...
            if (shouldWriteHeapSnapshot) {
                ...
                try {
                    ...
                    if (isHighHeapPressure(latestDiagnostics)) {
                        throw new Error(t('Heap snapshot skipped: V8 heap pressure is already high, and writing a synchronous heap snapshot could make the process unresponsive or trigger OOM. Restart HopCode first if it is unstable, or retry before memory pressure reaches the warning threshold.'));
                    }
    ...
    if (!stopResult.ok) {
        ...
        if (alreadyStopped) {
            const infoMsg = process.platform === 'win32'
                ? t('CPU profile was stopped externally. Check ~/.hopcode/cpu-profiles/ for the output.')
                : t('CPU profile was stopped externally (e.g., via SIGUSR1). Check ~/.hopcode/cpu-profiles/ for the output.');
            if (executionMode === 'interactive') {
...
function rollbackDoctorAction(context) {
    ...
    if (process.platform === 'win32') {
        const winMsg = t('Rollback on Windows requires manual intervention. Rename hopcode.old to hopcode in your installation directory.');
        if (context.executionMode === 'interactive') {
