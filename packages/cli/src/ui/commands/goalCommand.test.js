...
import { __resetActiveGoalStoreForTests, clearActiveGoal, getActiveGoal, notifyGoalTerminal, } from '@hoptrendy/hopcode-core';
...
describe('goalCommand', () => {
    ...
    it.each(['interactive', 'non_interactive', 'acp'])('accepts conditions longer than 4,000 characters in %s mode', async (executionMode) => {
        const ctx = createMockCommandContext({
            executionMode,
            services: { config: makeConfig() },
        });
        ...
        expect(ctx.ui.addItem.mock.calls[0][0]).toMatchObject({
            type: 'goal_status',
            kind: 'set',
            condition,
        });
