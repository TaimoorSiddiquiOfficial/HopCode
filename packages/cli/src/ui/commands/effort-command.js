...
import { normalizeReasoningEffort, REASONING_EFFORT_TIERS, } from '@hoptrendy/hopcode-core';
    ...
    action: async (context, actionArgs) => {
        ...
        if (config.getReasoningEffort() !== tier) {
            return {
                type: 'message',
                messageType: 'info',
                content: t('Reasoning effort set to {{tier}}, but thinking is currently disabled — it will take effect when thinking is re-enabled.', { tier }),
            };
        }
        ...
        return {
            type: 'message',
            messageType: 'info',
            content: t('Reasoning effort: {{tier}} (requested; the effective tier depends on the active provider/model).', { tier }),
        };
    },
};
