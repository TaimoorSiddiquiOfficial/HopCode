 ...
 */
import { listWorkflowSnapshots } from '@hoptrendy/hopcode-core';
...
export const workflowsCommand = {
    name: 'workflows',
    get description() {
        return t('List active and completed workflow runs (text dump — interactive dialog opens via the footer pill)');
    },
    get argumentHint() {
        return t('[runId]');
    },
    ...
    action: async (context, args) => {
        ...
        if (context.executionMode === 'interactive') {
            lines.push(t('Tip: use `/workflows <runId>` for the per-run detail view (name, description, phase tree, recent logs).'), '');
        }
        lines.push(`Workflow runs (${running.length + terminal.length} total · ${running.length} running)`, '');
        if (running.length > 0) {
            lines.push('Active');
