...
describe('modelCommand', () => {
    ...
    it('should have the correct name and description', () => {
        ...
        expect(modelCommand.description).toBe('Switch the model for this session (--fast for suggestion model, --voice for voice transcription model, --vision for the vision bridge model, --project to persist to project settings, --global to persist to user settings, [model-id] to switch immediately, or [model-id] [prompt] to run a one-off prompt on another model; the inline prompt is sent verbatim without @file expansion).');
    });
    ...
    it('should switch the main model directly in interactive mode when args are provided', async () => {
        ...
        expect(switchModel).toHaveBeenCalledWith(AuthType.HOPCODE_OAUTH, 'hopcode-max', undefined);
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'model.name', 'hopcode-max');
    ...
    it('should not persist the model when direct model validation fails', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Model 'missing-model' is not available for auth type 'hopcode-oauth'.\n" +
    ...
    it('should not persist the model when direct model switching fails after validation', async () => {
        ...
        await expect(modelCommand.action(mockContext, 'hopcode-max')).rejects.toThrow('Refresh failed');
        expect(switchModel).toHaveBeenCalledWith(AuthType.HOPCODE_OAUTH, 'hopcode-max', undefined);
        expect(setValue).not.toHaveBeenCalled();
    });
        ...
        it('should return current fast model without triggering dialog for --fast no args', async () => {
            mockContext = createMockCommandContext({
                ...
                services: {
                    ...
                    settings: {
                        merged: { fastModel: 'hopcode-turbo' },
                    },
                },
            });
