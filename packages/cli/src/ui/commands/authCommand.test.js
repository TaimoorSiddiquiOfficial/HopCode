...
describe('authCommand', () => {
    ...
    it.each(['non_interactive', 'acp'])('should return an info message in %s mode', (executionMode) => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Authentication configuration is only available in interactive mode. To configure authentication, run HopCode interactively and use /auth, or set environment variables: OPENAI_API_KEY, OPENAI_BASE_URL, OPENAI_MODEL.',
        });
    });
