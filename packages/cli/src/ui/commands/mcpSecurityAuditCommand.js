 ...
 */
export const mcpSecurityAuditCommand = {
 ...
 */
async function performMcpSecurityAudit(context) {
    const findings = {
        critical: [],
        high: [],
        medium: [],
 ...
 * Find MCP configuration files
 */
async function findMcpConfigFiles(_context) {
    const configFiles = [];
    const possiblePaths = [
        '.hopcode/mcp.json',
        'mcp.json',
 ...
 */
async function findMcpServerFiles(_context) {
    const serverFiles = [];
 ...
 */
async function auditMcpConfigFile(configPath, findings) {
    try {
 ...
 */
async function auditMcpServerFile(serverPath, findings) {
    try {
 ...
 * Check for hardcoded secrets
 */
function auditForSecrets(filePath, content, findings) {
 ...
 */
function auditForUnpinnedVersions(filePath, config, findings) {
 ...
 * Check for insecure command execution
 */
function auditForInsecureCommands(filePath, config, findings) {
    if (!config['mcpServers']) {
        return;
    }
    for (const [serverName, serverConfig] of Object.entries(config['mcpServers'])) {
        ...
        for (const dangerous of dangerousCommands) {
            if (typeof cfg['command'] === 'string' &&
                cfg['command'].includes(dangerous)) {
 ...
 * Check for missing input validation
 */
function auditForMissingValidation(filePath, config, findings) {
    if (!config['mcpServers']) {
        return;
    }
    for (const [serverName, serverConfig] of Object.entries(config['mcpServers'])) {
        ...
        if (cfg['env'] && typeof cfg['env'] === 'object') {
            ...
            const hasSecrets = Object.keys(envObj).some((k) => k.toLowerCase().includes('key') || k.toLowerCase().includes('secret'));
 ...
 */
function auditForPermissions(filePath, config, findings) {
    if (!config['mcpServers']) {
        return;
    }
    for (const [serverName, serverConfig] of Object.entries(config['mcpServers'])) {
 ...
 */
function auditForDangerousPatterns(filePath, content, lines, findings) {
 ...
 */
function auditForInputValidation(filePath, content, findings) {
    // Check if tool handlers validate input
    const hasToolHandlers = /tools\.setToolHandler|server\.tool/g.test(content);
    const hasInputValidation = /z\.object|yup\.object|joi\.object|validateInput|assertInput/g.test(content);
 ...
 * Check for insecure temp file usage
 */
function auditForInsecureTempFiles(filePath, content, findings) {
...
function generateSecurityReport(findings) {
    const total = findings.critical.length +
...
//# sourceMappingURL=mcpSecurityAuditCommand.js.map