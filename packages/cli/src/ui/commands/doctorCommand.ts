  ...
  if (!stopResult.ok) {
    ...
    if (alreadyStopped) {
      const infoMsg =
        process.platform === 'win32'
          ? t(
              'CPU profile was stopped externally. Check ~/.hopcode/cpu-profiles/ for the output.',
            )
          : t(
              'CPU profile was stopped externally (e.g., via SIGUSR1). Check ~/.hopcode/cpu-profiles/ for the output.',
            );
      if (executionMode === 'interactive') {
