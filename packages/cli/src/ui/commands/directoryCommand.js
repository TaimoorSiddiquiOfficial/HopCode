...
import { expandHomeDir, loadServerHierarchicalMemory, ConditionalRulesRegistry, } from '@hoptrendy/hopcode-core';
                    ...
                    if (added.length > 0) {
                        try {
                            if (config.isSafeMode()) {
                                ...
                                messages.push(t('Successfully added HOPCODE.md files from the following directories if there are:\n- {{directories}}', { directories: added.join('\n- ') }));
                            }
                        }
                        catch (error) {
