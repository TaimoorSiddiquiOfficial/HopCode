...
import { listWorkflowRuns, listJobsForRun, getJobLogs, rerunFailedJobs, cancelRun, dispatchWorkflow, listWorkflows, workflowRunIcon, formatRunDuration, } from '../../utils/githubApi.js';
...
async function* logsStream(owner, repo, branch) {
    yield { messageType: 'info', content: '🔍 Fetching latest failed run…' };
    const runs = await listWorkflowRuns(owner, repo, branch, 5);
    const failedRun = runs.find((r) => r.status === 'completed' && r.conclusion === 'failure');
    if (!failedRun) {
        yield {
            messageType: 'info',
...
export const ciCommand = {
    name: 'ci',
    get description() {
        return t('Manage GitHub Actions CI: list runs, view logs, rerun, cancel, dispatch');
    },
    ...
    action: async (context) => {
        ...
        if (!token) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('GitHub token not configured. Set GITHUB_TOKEN env var or run: hopcode github auth'),
            };
        }
        // Detect repo context
        let repoInfo;
        ...
        const subCmd = (context.invocation?.args?.trim() ?? '').split(/\s+/)[0]?.toLowerCase() ??
            '';
        // ── /ci workflows ──────────────────────────────────────────────────────
        if (subCmd === 'workflows') {
            return {
...
//# sourceMappingURL=ciCommand.js.map