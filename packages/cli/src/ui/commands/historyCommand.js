...
export const historyCommand = {
    ...
    subCommands: [
        {
            ...
            action(context, args) {
                ...
                if (!query)
                    return makeError('Usage: /history search <query>');
        ...
        },
        {
            ...
            action(context) {
                ...
                if (info) {
                    return makeInfo(`Showing items ${info.windowStart}–${info.windowEnd} of ${info.total}.` +
        ...
        },
        {
            ...
            action(context) {
                const info = context.ui.windowInfo;
                if (!info)
                    return makeInfo('History info is not available.');
                const lines = [
                    `History window:`,
        ...
        },
        {
            ...
            action(context, args) {
                ...
                if (isNaN(n) || n < 0) {
                    return makeError('Usage: /history jump <n>  (where n is a non-negative integer)');
                }
                const jumpFn = context.ui.jumpToSearchResult;
                if (!jumpFn)
                    return makeError('Jump is not available in this context.');
                jumpFn(n);
                const info = context.ui.windowInfo;
                if (info) {
                    return makeInfo(`Jumped to index ${n}. Showing items ${info.windowStart}–${info.windowEnd} of ${info.total}.`);
                }
                return undefined;
            },
        },
    ...
    action(context, args) {
        ...
        if (!sub) {
            ...
            return makeInfo(`History management${totalStr}\n\n` +
...
export const searchCommand = {
    ...
    action(context, args) {
        const query = args.trim();
        if (!query)
            return makeError('Usage: /search <query>');
        const searchFn = context.ui.searchHistory;
        if (!searchFn)
            return makeError('Search is not available in this context.');
        const results = searchFn(query);
