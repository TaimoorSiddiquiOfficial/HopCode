...
import { AuthType, isImageCapable, parseVisionModelSetting, resolveModelId, } from '@hoptrendy/hopcode-core';
...
export const modelCommand = {
    ...
    get description() {
        return t('Switch the model for this session (--fast for suggestion model, --voice for voice transcription model, --vision for the vision bridge model, --project to persist to project settings, --global to persist to user settings, [model-id] to switch immediately, or [model-id] [prompt] to run a one-off prompt on another model; the inline prompt is sent verbatim without @file expansion).');
    },
    argumentHint: '[--fast|--voice|--vision] [--project|--global] [<model-id>] | <model-id> <prompt>',
    ...
    action: async (context, actionArgs) => {
        ...
        if (isFastModelCommand) {
            ...
            const availableModels = (selector.authType
                ...
                : config.getAllConfiguredModels()).filter((m) => !m.voiceOnly);
        ...
        if (isVisionModelCommand) {
            ...
            const availableModels = (selector.authType
                ? config.getAvailableModelsForAuthType(selector.authType)
                : config.getAllConfiguredModels()).filter((m) => !m.fastOnly && !m.voiceOnly);
