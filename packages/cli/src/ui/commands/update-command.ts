...
export const updateCommand: SlashCommand = {
  name: 'update',
  get description() {
    return t('Check for HopCode updates and install if available');
  },
  ...
  action: async (context) => {
    ...
    if (updateCheck.status === 'up-to-date') {
      const msg = t('HopCode {{version}} is up to date!', {
      ...
      if (customSandboxImage) {
        return {
          ...
          content: `${info.message}\n${t(
            'This session uses the custom sandbox image {{image}}. Update that image and restart HopCode.',
