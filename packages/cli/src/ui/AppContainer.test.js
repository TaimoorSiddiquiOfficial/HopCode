...
import { AppContainer, dedupeNewestFirst, getSpeculativeToolResult, getNextRenderMode, isInputActiveForState, isRenderModeToggleKey, mergeStartupWarnings, shouldAutoOpenSkillReview, shouldDrainMessageQueue, } from './AppContainer.js';
...
describe('AppContainer State Management', () => {
    ...
    describe('Terminal Title Update Feature', () => {
        ...
        it('should register for recorded session titles and format them in the terminal title', async () => {
            ...
            expect(formatSessionWindowTitle(null, 'my-project')).toBe('HopCode - my-project');
    ...
    describe('Transcript (Ctrl+O) integration', () => {
        ...
        const renderApp = () => render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
        ...
        it('swallows arbitrary keys while open and keeps the transcript open', () => {
            ...
            act(() => handleKeypress(ctrlO));
            expect(lastFrame()).toContain(TRANSCRIPT_MARKER);
            expect(() => act(() => handleKeypress(makeKey({ name: 'x', sequence: 'x' })))).not.toThrow();
            expect(lastFrame()).toContain(TRANSCRIPT_MARKER);
        });
