...
function makeSettings(workspaceDir, opts = {}) {
    ...
    return {
        isTrusted,
        workspace: {
            path: path.join(workspaceDir, '.hopcode', 'settings.json'),
...
describe('buildVoiceKeyterms', () => {
    ...
    it('does not include project- or branch-derived terms (no metadata sent)', () => {
        const terms = buildVoiceKeyterms();
        expect(terms).not.toContain('hopcode');
    ...
    describe('custom keyterms file', () => {
        ...
        beforeEach(() => {
            ...
            hopcodeDir = path.join(workspaceDir, '.hopcode');
        ...
        it('auto-loads .hopcode/voice-keyterms.txt and merges with the globals', () => {
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), 'Kubernetes\nGraphQL\n');
        ...
        it('ignores blank lines and whole-line "#" comments', () => {
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), '# project terms\n\n  Kubernetes # container orchestration  \nIssue #42\nPR #5817\nC#\nF#\n   # indented comment\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('honors an explicit absolute keytermsFile over auto-discovery', () => {
            ...
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), 'Auto\n');
        ...
        it('dedupes case-insensitively and keeps the global casing', () => {
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), 'typescript\nKubernetes\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
            expect(terms).toContain('TypeScript');
        ...
        it('caps by term count for a file of many short terms', () => {
            ...
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), many);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('caps by total byte length for a file of few long terms', () => {
            ...
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), long);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('caps non-ASCII keyterms by UTF-8 byte length', () => {
            ...
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), cjkTerms);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('skips over a term that exceeds the remaining byte budget', () => {
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), `${'x'.repeat(2200)}\nShortTerm\n`);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
            expect(terms).toContain('ShortTerm');
            expect(terms.join(' ').length).toBeLessThanOrEqual(2000);
        });
        it('does not read a keyterms file in an untrusted workspace', () => {
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), 'ShouldNotLoad\n');
        ...
        it('does not follow a symlinked keyterms file (no secret exfiltration)', () => {
            ...
            fs.symlinkSync(secret, path.join(hopcodeDir, 'voice-keyterms.txt'));
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('does not read a hard-linked keyterms file', () => {
            ...
            fs.linkSync(secret, path.join(hopcodeDir, 'voice-keyterms.txt'));
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('does not load the default file through a symlinked .hopcode directory', () => {
            const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'voice-keyterms-outside-'));
            fs.rmSync(hopcodeDir, { recursive: true, force: true });
            fs.writeFileSync(path.join(outsideDir, 'voice-keyterms.txt'), 'SECRETKEYMATERIAL\n');
            fs.symlinkSync(outsideDir, hopcodeDir);
            try {
                const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
                expect(terms).not.toContain('SECRETKEYMATERIAL');
                expect(terms).toContain('TypeScript'); // globals only
        ...
        it('ignores a keyterms file larger than the size cap', () => {
            ...
            fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), huge);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
