...
describe('createNativeAudioRecorder', () => {
    ...
    it('explains mirror registry installs when the native package is missing', async () => {
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw new Error("Cannot find package '@hoptrendy/audio-capture' imported from /qwen/dist/cli.js");
            },
        });
    ...
    it.each(['ERR_MODULE_NOT_FOUND', 'MODULE_NOT_FOUND'])('explains mirror registry installs for %s native package errors', async (code) => {
        const error = Object.assign(new Error('missing @hoptrendy/audio-capture dependency'), { code });
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw error;
            },
    ...
    it('does not rewrite wrapped native addon load failures as missing packages', async () => {
        const loadError = new Error("Native audio capture addon could not be loaded. Reinstall @hoptrendy/audio-capture, or use the SoX fallback. (Cannot find module 'node-gyp-build')");
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw loadError;
            },
    ...
    it('does not explain native start failures as missing packages', async () => {
        const startError = new Error("Cannot find package '@hoptrendy/audio-capture' while starting");
        const backend = {
            startRecording: vi.fn(() => {
                throw startError;
            }),
