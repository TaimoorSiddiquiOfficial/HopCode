...
function createSettings(env = {}, apiKey) {
    return {
...
describe('voice-transcriber', () => {
    ...
    it('keeps realtime model ids on their matching streaming transport', () => {
        ...
        expect(qwenStreamConfig).toEqual({
            ...
            baseUrl: 'https://dashscope.example/v1',
            apiKey: 'sk-test',
            keytermsContext: expect.stringContaining('hopcode'),
        });
    ...
    it('threads a custom keyterms file term into the realtime keytermsContext', () => {
        ...
        const hopcodeDir = path.join(workspaceDir, '.hopcode');
        fs.mkdirSync(hopcodeDir, { recursive: true });
        fs.writeFileSync(path.join(hopcodeDir, 'voice-keyterms.txt'), 'Paraformer\n');
        try {
            const settings = {
                isTrusted: true,
                workspace: { path: path.join(hopcodeDir, 'settings.json') },
                merged: {
            ...
            expect(streamConfig.keytermsContext).toContain('hopcode'); // globals too
        }
        finally {
    ...
    it('threads a custom keyterms file term into the batch keytermsContext', async () => {
        ...
        try {
            ...
            expect(sys.content[0].text).toContain('hopcode'); // globals too
        }
        finally {
            fs.rmSync(workspaceDir, { recursive: true, force: true });
        }
    ...
    it('does not include project path metadata in voice keyterms', () => {
        ...
        expect(streamConfig.keytermsContext).toContain('hopcode');
        ...
        const config = createConfig([
            {
                ...
                label: 'Qwen ASR',
                authType: AuthType.USE_OPENAI,
                baseUrl: 'https://user:secret@dashscope.aliyuncs.com/compatible-mode/v1',
            },
        ]);
        expect(resolveVoiceTranscriptionConfig({
            config,
