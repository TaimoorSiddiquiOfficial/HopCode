...
import { HopCodeLight } from './hopcode-light.js';
import { HopCodeDark } from './hopcode-dark.js';
...
export const DEFAULT_THEME = HopCodeDark;
...
class ThemeManager {
    ...
    resolveAutoTheme() {
        ...
        return detected === 'light' ? HopCodeLight : HopCodeDark;
    }
    /**
    ...
    async resolveAutoThemeAsync() {
        ...
        this.activeTheme = detected === 'light' ? HopCodeLight : HopCodeDark;
    ...
    getAvailableThemes() {
        ...
        const hopcodeThemes = builtInThemes.filter((theme) => theme.name === HopCodeLight.name || theme.name === HopCodeDark.name);
        // Separate built-in themes
        const otherBuiltInThemes = builtInThemes.filter((theme) => theme.name !== HopCodeLight.name && theme.name !== HopCodeDark.name);
