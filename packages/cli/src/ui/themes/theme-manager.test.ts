...
describe('ThemeManager', () => {
  ...
  describe('auto theme detection', () => {
    it('should select HopCode Dark when terminal is detected as dark', () => {
      ...
      expect(themeManager.getActiveTheme().name).toBe('HopCode Dark');
    });

    it('should select HopCode Light when terminal is detected as light', () => {
      vi.mocked(detectModule.detectTerminalTheme).mockReturnValue('light');
      const result = themeManager.setActiveTheme(AUTO_THEME_NAME);
      expect(result).toBe(true);
      expect(themeManager.getActiveTheme().name).toBe('HopCode Light');
    });
    ...
    it('should resolve async auto theme with HopCode Light for light', async () => {
    ...
    it('should resolve async auto theme with HopCode Dark for dark', async () => {
      vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue(
        'dark',
      );
      await themeManager.resolveAutoThemeAsync();
