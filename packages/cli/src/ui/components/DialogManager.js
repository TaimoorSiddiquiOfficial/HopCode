...
export const DialogManager = ({ addItem, terminalWidth, }) => {
    ...
    if (uiState.isModelDialogOpen) {
        return (_jsx(ModelDialog, { onClose: uiActions.closeModelDialog, isFastModelMode: uiState.isFastModelMode, isVoiceModelMode: uiState.isVoiceModelMode, isVisionModelMode: uiState.isVisionModelMode, persistScope: uiState.modelDialogPersistScope, availableTerminalHeight: listDialogHeight }));
    }
    ...
    if (uiState.auth.isAuthenticating) {
        ...
        if (uiState.auth.pendingAuthType === AuthType.HOPCODE_OAUTH) {
            return (_jsx(HopCodeOAuthProgress, { deviceAuth: uiState.auth.hopCodeAuthState.deviceAuth || undefined, authStatus: uiState.auth.hopCodeAuthState.authStatus, authMessage: uiState.auth.hopCodeAuthState.authMessage, onTimeout: () => {
                    uiActions.auth.onAuthError('HopCode OAuth authentication timed out.');
                    uiActions.auth.cancelAuthentication();
                    uiActions.auth.setAuthState(AuthState.Updating);
                }, onCancel: () => {
                    uiActions.auth.cancelAuthentication();
