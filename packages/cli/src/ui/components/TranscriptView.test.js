...
describe('<TranscriptView />', () => {
    ...
    it('renders thinking blocks expanded (fullDetail) — full text, not a summary', () => {
        ...
        expect(frame).toContain('a private reasoning step that is normally collapsed');
    ...
    it('enters and exits the alternate screen by default (useAlternateScreen defaults to true)', () => {
        setTTY(true);
        const { unmount } = renderWithProviders(_jsx(TranscriptView, { items: items }));
    ...
    it('renders frozen pending items carrying negative ids without key collisions', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(TranscriptView, { items: withPending, useAlternateScreen: false }));
        const frame = lastFrame();
...
//# sourceMappingURL=TranscriptView.test.js.map