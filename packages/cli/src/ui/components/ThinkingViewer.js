...
const WHEEL_LINES = 3;
export const ThinkingViewer = ({ data, onClose, useAlternateScreen = true, }) => {
    ...
    const lines = useMemo(() => wrapToVisualLines(data.text, contentWidth), [data.text, contentWidth]);
    ...
    const { schedule: scheduleWheelFlush } = useFrameCoalescedFlush(useCallback(() => {
    ...
    }, [onClose, scrollBy, contentHeight, maxScroll]), { isActive: true });
    ...
    const title = data.durationMs != null
    ...
    return (_jsx(AlternateScreen, { disabled: !useAlternateScreen, children: _jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.text.secondary, paddingX: 1, height: rows, children: [_jsxs(Box, { children: [_jsxs(Text, { color: theme.text.accent, bold: true, children: [THINKING_ICON, title] }), _jsx(Text, { dimColor: true, children: scrollIndicator })] }), _jsx(Box, { flexDirection: "column", flexGrow: 1, children: visibleLines.map((line, i) => (_jsx(Text, { dimColor: true, wrap: "truncate-end", children: line || ' ' }, i))) }), _jsx(Box, { justifyContent: "center", children: _jsxs(Text, { dimColor: true, italic: true, children: ["ESC ", t('to close'), " ", '  ', "\u2191\u2193 ", t('to scroll'), " ", '  ', "PgUp/PgDn", ' ', "Ctrl+Home/End"] }) })] }) }));
};
//# sourceMappingURL=ThinkingViewer.js.map