...
import { getAllGeminiMdFilenames, Storage, getAutoMemoryRoot, getAutoMemoryProjectStateDir, getUserAutoMemoryRoot, AUTO_MEMORY_INDEX_FILENAME, } from '@hoptrendy/hopcode-core';
...
export function MemoryDialog({ onClose }) {
    ...
    const globalMemoryPath = useMemo(() => path.join(Storage.getGlobalHopCodeDir(), getAllGeminiMdFilenames()[0] ?? 'HOPCODE.md'), []);
    const projectMemoryPath = useMemo(() => path.join(config.getWorkingDir(), getAllGeminiMdFilenames()[0] ?? 'HOPCODE.md'), [config]);
    ...
    const resolveTargetPath = useCallback(async (item) => {
        ...
        switch (item.value) {
            case 'project':
                return resolvePreferredMemoryFile(config.getWorkingDir(), getAllGeminiMdFilenames()[0] ?? 'HOPCODE.md');
            case 'global':
                return resolvePreferredMemoryFile(Storage.getGlobalHopCodeDir(), getAllGeminiMdFilenames()[0] ?? 'HOPCODE.md');
            default: {
                const _exhaustive = item.value;
                return _exhaustive;
            }
