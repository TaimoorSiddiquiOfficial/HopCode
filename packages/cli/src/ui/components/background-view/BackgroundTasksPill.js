...
export function hasPendingApproval(entries) {
    return entries.some((e) => e.kind === 'agent' && (e.pendingApprovals?.length ?? 0) > 0);
}
/**
...
export const BackgroundTasksPill = () => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Text, { color: theme.text.secondary, children: " \u00B7 " }), _jsx(Text, { inverse: pillFocused, children: label }), needsApproval && (_jsx(Text, { color: theme.status.warning, children: ` ⚠ ${t('needs approval')}` }))] }));
};
