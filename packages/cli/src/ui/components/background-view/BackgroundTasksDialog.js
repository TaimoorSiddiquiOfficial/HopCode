...
import { buildBackgroundEntryLabel, MAX_RECENT_ACTIVITIES, } from '@hoptrendy/hopcode-core';
...
const WorkflowDetailBody = ({ entry, maxHeight, maxWidth }) => {
    ...
    const hasError = Boolean(entry.error);
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [terminal && (_jsx(Text, { color: terminal.color, children: `${terminal.icon} ${statusVerb(entry.status)} · ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' · ') })] }), entry.meta?.description && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { wrap: "wrap", children: entry.meta.description }) })] })), _jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Phases') }) }), entry.phases.length === 0 ? (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: t('(no phase recorded yet)') }) })) : (_jsxs(Fragment, { children: [phaseOverflow > 0 && (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: `+${phaseOverflow} ${t('more above')}` }) })), visiblePhases.map((phaseTitle, i) => {
                        const isCurrent = entry.status === 'running' &&
...
export const BackgroundTasksDialog = ({ availableTerminalHeight, terminalWidth, }) => {
    ...
    const selectedApproval = selectedEntry?.kind === 'agent'
        ? selectedEntry.pendingApprovals?.[0]
        : undefined;
    const approvalActive = isDetailMode && Boolean(selectedApproval);
    const approvalUsesQuestionDialog = selectedApproval?.confirmationDetails.type === 'ask_user_question';
    ...
    const approvalConfirmationDetails = selectedApproval && selectedAgentIdForActivity
            ...
            // an object spread.
            {
                ...
                onConfirm: async (outcome, payload) => {
                    await config
                        ...
                        .resolvePendingApproval(selectedAgentIdForActivity, selectedApproval.callId, outcome, payload);
                },
            }
        : undefined;
    useEffect(() => {
    ...
    // be saved to `.hopcode/workflows/<name>.js` from the detail view.
    ...
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.border.default, marginTop: 1, paddingX: 1, children: [dialogMode === 'list' && (_jsx(Box, { paddingX: 1, children: _jsx(Text, { bold: true, color: theme.text.accent, children: t('Background tasks') }) })), _jsxs(Box, { marginTop: dialogMode === 'list' ? 1 : 0, flexDirection: "column", children: [dialogMode === 'list' ? (_jsx(ListBody, { entries: entries, selectedIndex: selectedIndex, maxRows: listMaxRows })) : selectedEntry ? (_jsx(DetailBody, { entry: selectedEntry, 
                        ...
                        maxHeight: approvalActive
                            ...
                            : detailContentHeight, maxWidth: detailContentWidth })) : (_jsx(Box, { paddingX: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('No entry to show.') }) })), approvalActive && approvalConfirmationDetails && (_jsxs(Box, { flexDirection: "column", marginTop: 1, paddingX: 1, children: [_jsx(Text, { bold: true, color: theme.status.warning, children: t('Background agent needs approval') }), _jsx(ToolConfirmationMessage, { confirmationDetails: approvalConfirmationDetails, config: config, isFocused: approvalActive, contentWidth: detailContentWidth - 2, availableTerminalHeight: Math.max(6, Math.floor(detailContentHeight / 2)), compactMode: true })] }))] }), saveActive && workflowSaveTarget && config ? (_jsx(WorkflowSaveOverlay, { script: workflowSaveTarget.script, initialName: workflowSaveTarget.meta?.name ?? '', config: config, isActive: saveActive, onClose: () => setSaveActive(false) })) : (_jsx(Box, { marginTop: 1, paddingX: 1, children: _jsx(Text, { color: theme.text.secondary, children: hints.join(' \u00B7 ') }) }))] }));
};
