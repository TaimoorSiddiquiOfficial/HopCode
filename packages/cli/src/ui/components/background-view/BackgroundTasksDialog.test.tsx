...
function setup(initial: readonly DialogEntry[]): Harness {
  ...
  // snapshot ΓÇö the dialog now re-reads agent entries via `.get()` to pick up
...
describe('BackgroundTasksDialog', () => {
  ...
  it('routes monitor cancel via monitorRegistry.cancel(monitorId)', () => {
    // Pin the monitor-cancel branch in `cancelSelected` ΓÇö flipping it to
    ...
    // Agent registry's cancel must NOT be called for a monitor entry ΓÇö
  ...
  it('keeps detail mode when an already-terminal entry is opened (no spurious fallback)', () => {
    ...
    // The auto-fallback ref must only trigger on a running ΓåÆ terminal
  ...
  describe('MonitorDetailBody render branches', () => {
    ...
    it('uses singular "1 event" / plural "N events"', () => {
      ...
      // Guard against false positive ΓÇö substring "1 event" also matches "1 events".
    ...
    it('renders Error block for failed status', () => {
      ...
      // The auto-stop label must not appear on a `failed` entry ΓÇö the
