...
export const Footer = () => {
    ...
    const { lines: statusLineLines, useThemeColors, respectUserColors, hideContextIndicator, } = useStatusLine();
    ...
    const leftBottomContent = uiState.ctrlCPressedOnce ? (_jsx(Text, { color: theme.status.warning, children: t('Press Ctrl+C again to exit.') })) : uiState.ctrlDPressedOnce ? (_jsx(Text, { color: theme.status.warning, children: t('Press Ctrl+D again to exit.') })) : uiState.showEscapePrompt ? (_jsx(Text, { color: theme.text.secondary, children: t('Press Esc again to clear.') })) : uiState.rewindEscPending ? (_jsx(Text, { color: theme.text.secondary, children: t('Press Esc again to rewind conversation.') })) : vimEnabled && vimMode === 'INSERT' ? (_jsx(Text, { color: theme.text.secondary, children: "-- INSERT --" })) : vimEnabled && vimMode === 'NORMAL' ? (_jsx(Text, { color: theme.text.secondary, children: "-- NORMAL --" })) : uiState.shellModeActive ? (_jsx(ShellModeIndicator, {})) : configInitMessage ? (_jsxs(Text, { color: theme.text.secondary, children: [_jsx(GeminiSpinner, {}), " ", configInitMessage] })) : uiState.startupIdeConnectionStatus.state === 'connecting' ? (_jsxs(Text, { color: theme.text.secondary, children: [_jsx(GeminiSpinner, {}), " ", t('IDE connecting... context may be unavailable')] })) : uiState.startupIdeConnectionStatus.state === 'failed' ? (_jsx(Text, { color: theme.status.warning, children: t('IDE connection unavailable: {{message}}', {
        ...
        }) })) : uiState.streamingState === StreamingState.Responding ? (_jsx(Text, { color: theme.text.secondary, children: t('Enter to steer · Ctrl+Q to queue') })) : showAutoAcceptIndicator !== undefined ? (_jsx(AutoAcceptIndicator, { approvalMode: showAutoAcceptIndicator })) : suppressHint ? null : (_jsx(Text, { color: theme.text.secondary, children: t('? for shortcuts') }));
    const rightItems = [];
    if (sandboxInfo) {
        rightItems.push({
            key: 'sandbox',
    ...
    return (_jsxs(Box, { flexDirection: isNarrow ? 'column' : 'row', justifyContent: isNarrow ? 'flex-start' : 'space-between', width: "100%", paddingX: 2, gap: isNarrow ? 0 : 1, children: [_jsxs(Box, { flexDirection: "column", flexGrow: 1, flexShrink: isNarrow ? 0 : 1, minWidth: 0, children: [statusLineLines.length > 0 &&
                        ...
                        !uiState.ctrlCPressedOnce &&
                        !uiState.ctrlDPressedOnce && (_jsx(Text, { color: theme.text.accent, wrap: "truncate", children: `▷ ${t('workflow active')}` })), _jsxs(Box, { flexDirection: "row", flexShrink: 1, children: [_jsx(Text, { wrap: "truncate", children: leftBottomContent }), _jsx(BackgroundTasksPill, {}), _jsx(MCPHealthPill, {}), !uiState.isSkillReviewDialogOpen &&
                                (uiState.skillReviewPending?.skills.length ?? 0) > 0 && (_jsx(Text, { color: theme.status.warning, children: ` ⚠ ${t('{{count}} skill(s) pending review', {
