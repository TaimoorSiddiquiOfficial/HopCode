...
export function SkillsManagerDialog({ settings, config, addItem, onClose, reloadCommands, setInputBuffer, availableTerminalHeight, }) {
    ...
    const initialWorkspaceDisabled = useMemo(() => new Set(normalizeNames(namesFromScope(settings, SettingScope.Workspace))), [settings]);
    ...
    const persistChanges = useCallback(async () => {
        if (!settings.isTrusted) {
            addItem({
                ...
                text: t('Workspace is untrusted; workspace settings are ignored by the merged config. Run /trust first to persist skills changes here, or edit ~/.hopcode/settings.json directly to manage skills at user scope.'),
            }, Date.now());
            return 'untrusted';
        }
        ...
        const previousStrings = previousWorkspace.filter((n) => typeof n === 'string');
        ...
        // extension, deleted .hopcode/skills/ directory). Without this, opening
        ...
        const unchanged = prevNormalized.length === nextNormalized.length &&
            prevNormalized.every((n, i) => n === nextNormalized[i]);
        if (unchanged)
            return 'ok';
        try {
            settings.setValue(SettingScope.Workspace, 'skills.disabled', nextDisabled.length > 0 ? nextDisabled : undefined);
        }
        catch (e) {
            addItem({
                type: MessageType.ERROR,
