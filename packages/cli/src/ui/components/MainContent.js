...
export const MainContent = () => {
    ...
    const renderVirtualItem = useCallback(({ item }) => {
        ...
        if (isPending) {
            ...
            return (_jsx(VirtualHistoryItem, { terminalWidth: terminalWidth, mainAreaWidth: mainAreaWidth, availableTerminalHeight: ps.constrainHeight ? ps.availableTerminalHeight : undefined, item: { ...item, id: 0 }, isPending: true, isFocused: !ps.isEditorDialogOpen, activeShellPtyId: ps.activePtyId, embeddedShellFocused: ps.embeddedShellFocused, commands: uiState.slashCommands, sourceCopyIndexOffsets: sourceCopyIndexOffsets }));
        }
        return (_jsx(VirtualHistoryItem, { terminalWidth: terminalWidth, mainAreaWidth: mainAreaWidth, availableTerminalHeight: staticAreaMaxItemHeight, availableTerminalHeightGemini: MAX_GEMINI_MESSAGE_LINES, item: item, isPending: false, commands: uiState.slashCommands, sourceCopyIndexOffsets: sourceCopyIndexOffsets, thoughtHeadId: thoughtHeadIdByItemRef.current.get(item) }));
    }, [
        version,
        terminalWidth,
        mainAreaWidth,
    ...
    if (useVirtualScroll) {
        ...
        return (_jsxs(OverflowProvider, { children: [_jsx(ScrollableList, { ref: scrollRef, hasFocus: !uiState.dialogsVisible, data: allVirtualItems, renderItem: renderVirtualItem, estimatedItemHeight: virtualEstimatedItemHeight, keyExtractor: virtualKeyExtractor, initialScrollIndex: allVirtualItems.length <= 1 ? 0 : SCROLL_TO_ITEM_END, isStaticItem: virtualIsStaticItem, containerHeight: scrollContainerHeight, showScrollbar: showScrollbar }), _jsx(TextSelectionController, { isActive: !uiState.dialogsVisible, getViewportRect: () => scrollRef.current?.getViewportRect() ?? null, getScrollState: () => scrollRef.current?.getScrollState() ?? {
                        scrollTop: 0,
                        scrollHeight: 0,
                        innerHeight: 0,
