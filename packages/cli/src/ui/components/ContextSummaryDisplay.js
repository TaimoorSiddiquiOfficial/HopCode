...
export const ContextSummaryDisplay = ({ contextMdFileCount, contextFileNames, mcpServers, blockedMcpServers, showToolDescriptions, ideContext, }) => {
    ...
    const geminiMdText = (() => {
        ...
        return contextMdFileCount === 1
            ? t('{{count}} {{name}} file', {
                count: String(contextMdFileCount),
                name,
            })
            : t('{{count}} {{name}} files', {
                count: String(contextMdFileCount),
