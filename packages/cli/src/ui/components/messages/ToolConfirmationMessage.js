...
import { IdeClient, ToolConfirmationOutcome, buildHumanReadableRuleLabel, } from '@hoptrendy/hopcode-core';
...
export const ToolConfirmationMessage = ({ confirmationDetails, config, isFocused = true, availableTerminalHeight, contentWidth, compactMode = false, }) => {
    ...
    if (confirmationDetails.type === 'edit') {
        ...
        const wrappedWarningRows = warnings.reduce((sum, w) => sum + Math.max(Math.ceil((w.length + warningPrefixLen) / safeWidth), 1), 0);
