...
vi.mock('../HopCodeRespondingSpinner.js', () => ({
    HopCodeRespondingSpinner: ({ nonRespondingDisplay, }) => {
...
describe('<ToolMessage />', () => {
    ...
    describe('fullDetail (§4.9 transcript) data-source switch', () => {
        ...
        it('preserves TAB and LF in detailedDisplay (structural whitespace)', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "table.txt", resultDisplay: "Read 1 file", detailedDisplay: 'colA\tcolB\nrow2A\trow2B', fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame() ?? '';
