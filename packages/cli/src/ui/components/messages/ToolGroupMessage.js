...
export const ToolGroupMessage = ({ toolCalls, availableTerminalHeight, contentWidth, isFocused = true, isPending = false, activeShellPtyId, embeddedShellFocused, memoryWriteCount, memoryReadCount, isUserInitiated, fullDetail = false, }) => {
    ...
    if (allMemOpsComplete) {
        ...
        return (_jsxs(Box, { flexDirection: "column", width: contentWidth, children: [readCount > 0 && (_jsx(Box, { paddingLeft: 1, children: _jsxs(Text, { dimColor: true, children: ['● ', "Recalled ", readCount, " ", readCount === 1 ? 'memory' : 'memories'] }) })), writeCount > 0 && (_jsx(Box, { paddingLeft: 1, children: _jsxs(Text, { dimColor: true, children: ['● ', "Wrote ", writeCount, " ", writeCount === 1 ? 'memory' : 'memories'] }) }))] }));
    }
    ...
    const collapsibleTools = forceExpandAll
        ? []
        : inlineToolCalls.filter((t) => isCollapsibleTool(t.name) && t.status !== ToolCallStatus.Canceled);
    ...
    const memoryBadge = hasMemoryBadge ? (_jsx(Box, { paddingLeft: 1, children: _jsxs(Text, { dimColor: true, children: ['● ', [
                    (memoryReadCount ?? 0) > 0 &&
