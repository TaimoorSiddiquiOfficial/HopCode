...
import { formatVisionBridgeNoticeDisplay, isVisionBridgeNoticeDisplay, } from '@hoptrendy/hopcode-core';
...
export const ToolMessage = ({ name, description, resultDisplay, detailedDisplay, status, availableTerminalHeight, contentWidth, emphasis = 'medium', renderOutputAsMarkdown = true, activeShellPtyId, embeddedShellFocused, ptyId, config, forceShowResult, fullDetail, isFocused, isPending, executionStartTime, }) => {
    ...
    const visionBridgeNoticeText = visionBridgeNoticeDisplay
        ? sanitizeTerminalText(formatVisionBridgeNoticeDisplay(visionBridgeNoticeDisplay))
        : undefined;
...
const ToolInfo = ({ name, description, status, emphasis, }) => {
    ...
    return (_jsx(Box, { flexGrow: 1, children: _jsxs(Text, { wrap: "wrap", strikethrough: status === ToolCallStatus.Canceled, children: [_jsx(Text, { color: nameColor, bold: true, children: localizeToolDisplayName(name) }), ' ', _jsx(Text, { color: theme.text.secondary, children: description })] }) }));
};
