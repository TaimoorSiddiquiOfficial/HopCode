...
import { AgentStatus, AgentEventType, getGitBranch, } from '@hoptrendy/hopcode-core';
...
export const AgentChatContent = ({ core, interactiveAgent, instanceKey, modelName, }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Static, { items: [
                ...
                ], children: (item) => item }, `agent-${instanceKey}-${historyRemountKey}`), pendingItems.map((item) => (_jsx(HistoryItemDisplay, { item: item, isPending: true, terminalWidth: terminalWidth, mainAreaWidth: contentWidth, availableTerminalHeight: constrainHeight ? availableTerminalHeight : undefined, isFocused: !readonly, activeShellPtyId: activePtyId, embeddedShellFocused: embeddedShellFocused }, item.id))), isRunning && (_jsx(Box, { marginX: 2, marginTop: 1, children: _jsx(HopCodeRespondingSpinner, {}) }))] }));
};
