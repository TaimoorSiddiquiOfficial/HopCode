...
import { AuthType, ModelSlashCommandEvent, logModelSlashCommand, MAINLINE_CODER_MODEL, isImageCapable, parseVisionModelSetting, resolveModelId, } from '@hoptrendy/hopcode-core';
...
function handleModelSwitchSuccess({ settings, uiState, after, effectiveAuthType, effectiveModelId, effectiveBaseUrl, isRuntime, persistScope, }) {
...
export function ModelDialog({ onClose, isFastModelMode, isVoiceModelMode, isVisionModelMode, persistScope, availableTerminalHeight, }) {
    ...
    const activeRuntimeSnapshot = isFastModelMode || isVoiceModelMode || isVisionModelMode
    ...
    const handleSelect = useCallback(async (selected) => {
        ...
        const isQwenOAuthSelection = selected.startsWith(`${AuthType.HOPCODE_OAUTH}::`) ||
            (selected.startsWith('$runtime|') &&
                selected.split('|')[1] === AuthType.HOPCODE_OAUTH);
        const isRuntimeOAuthSelection = selected.startsWith(`$runtime|${AuthType.HOPCODE_OAUTH}|`);
