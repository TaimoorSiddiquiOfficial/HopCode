...
import { redactUrlCredentials, } from '@hoptrendy/hopcode-core';
...
export const PluginDetailView = ({ extension, scope, isFavorite, hasUpdateAvailable, isFocused, showFavorite = true, onAction, }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", gap: 1, children: [_jsxs(Box, { flexDirection: "column", children: [_jsx(InfoRow, { label: t('Name:'), children: ext.name }), _jsx(InfoRow, { label: t('Version:'), children: stripUnsafeCharacters(ext.version ?? '') }), _jsx(InfoRow, { label: t('Scope:'), children: scope }), _jsxs(InfoRow, { label: t('Status:'), children: [_jsx(Text, { color: isActive ? theme.status.success : theme.text.secondary, children: isActive ? t('active') : t('disabled') }), isFavorite ? _jsx(Text, { color: theme.status.warning, children: " \u2605" }) : null] }), ext.installMetadata && (_jsx(InfoRow, { label: t('Source:'), children: redactUrlCredentials(ext.installMetadata.source) })), _jsx(InfoRow, { label: t('Components:'), children: componentSummary(ext) })] }), _jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.text.secondary, children: t('Actions') }), _jsx(RadioButtonSelect, { items: actions, isFocused: isFocused, showNumbers: false, onSelect: onAction })] })] }));
};
