...
import { SettingScope, MCPServerStatus, getMCPServerStatus, removeMCPServerStatus, addMCPStatusChangeListener, removeMCPStatusChangeListener, mcpServerRequiresOAuth, MCPOAuthTokenStorage, createDebugLogger, matchesAnyServerPattern, } from '@hoptrendy/hopcode-core';
...
export const InstalledTab = ({ config, isActive, onLockChange, onStatus, extensionsUpdateState, reloadSignal, }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [hasAbove ? (_jsx(Text, { color: theme.text.secondary, children: t('↑ more above') })) : null, visibleRows.map((row, i) => {
                ...
                return (_jsxs(Box, { children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: marker }) }), _jsxs(Box, { flexGrow: 1, children: [_jsxs(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: [isChild ? '  └ ' : '', item.name] }), item.isFavorite ? (_jsx(Text, { color: theme.status.warning, children: " \u2605" })) : null] }), _jsxs(Text, { color: isSelected ? theme.text.accent : theme.text.secondary, children: [kindBadge, ' '] }), _jsxs(Text, { color: isSelected ? theme.text.accent : statusColor, children: ["(", statusLabel, ")"] })] }, item.key));
