...
import { parseInstallSource, redactUrlCredentials, createDebugLogger, isExtensionCommittedWithWarningsError, } from '@hoptrendy/hopcode-core';
    ...
    const renderRow = (index, label, rightText, isAction = false) => {
        ...
        const labelColor = isSelected
            ...
            : isAction
                ? theme.text.link
                : theme.text.primary;
        return (_jsxs(Box, { children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: isSelected ? '●' : ' ' }) }), _jsx(Box, { flexGrow: 1, children: _jsx(Text, { color: labelColor, children: label }) }), rightText ? (_jsx(Text, { color: theme.text.secondary, children: rightText })) : null] }, `row-${index}`));
    };
