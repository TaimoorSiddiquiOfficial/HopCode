...
export function SettingsDialog({ settings, onSelect, onRestartRequest, availableTerminalHeight, width, config, }) {
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", padding: 1, width: "100%", children: [_jsx(ConfigTabBar, { activeTab: activeTab, focused: focusZone === 'tabs' }), _jsx(Box, { height: 1 }), activeTab !== 'settings' ? (_jsx(Box, { flexDirection: "column", flexGrow: 1, children: activeTab === 'status' ? (systemInfo ? (_jsx(AboutBox, { ...systemInfo, width: width ? width - 4 : undefined })) : statusError ? (_jsx(Text, { color: theme.status.error, children: t('Failed to load status. Press r to retry.') })) : (_jsx(Text, { color: theme.text.secondary, children: t('Loading status…') }))) : (_jsx(StatsDialog
                ...
                , { 
                    ...
                    width: width ? width - 4 : undefined, availableHeight: availableTerminalHeight != null
                        ...
                        : t('(Use Enter to apply scope, Tab to go back)') }) }), showRestartPrompt && (_jsx(Text, { color: theme.status.warning, children: t('To see changes, HopCode must be restarted. Press r to exit and apply changes now.') })), activeTab === 'settings' &&
                mode === 'settings' &&
                focusZone === 'list' &&
