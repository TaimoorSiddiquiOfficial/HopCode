...
export const computeSessionStats = (metrics) => {
    ...
    const estimatedCostUsd = Object.entries(models).reduce((total, [modelName, modelMetrics]) => total +
        estimateModelCost(modelName, modelMetrics.tokens.prompt, modelMetrics.tokens.candidates), 0);
    return {
        totalApiTime,
        totalToolTime,
        agentActiveTime,
