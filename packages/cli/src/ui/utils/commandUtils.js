            ...
            try {
                ...
                try {
                    ...
                    if (xclipNotFound && xselNotFound) {
                        ...
                        if (!writeOsc52(text)) {
                            throw new Error('Clipboard unavailable: xclip/xsel not found and OSC 52 requires a TTY. Try running inside a terminal emulator.');
                        }
                        return;
                    }
                    ...
                    if (writeOsc52(text))
                        return;
                    throw new Error(`Clipboard unavailable: xclip/xsel failed ("${primaryMsg}", "${fallbackMsg}") and OSC 52 requires a TTY. Try running inside a terminal emulator.`);
                }
            }
            return;
        default:
