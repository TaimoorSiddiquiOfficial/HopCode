...
function unavailableImageRenderingDisabled() {
    return {
        kind: 'unavailable',
        reason: 'Mermaid image rendering is disabled via HOPCODE_DISABLE_MERMAID_IMAGES.',
    };
}
/**
...
export function renderMermaidImageSync({ source, contentWidth, availableTerminalHeight, env = process.env, }) {
    ...
    if (imageRendering !== '1' &&
        ...
        return {
            kind: 'unavailable',
            reason: 'Mermaid image rendering is disabled by default. Set HOPCODE_MERMAID_IMAGE_RENDERING=1 to enable external renderers.',
            showReason: false,
        };
    }
    ...
    if (!mmdc) {
        return {
            kind: 'unavailable',
            reason: 'Mermaid CLI (mmdc) was not found. Install @mermaid-js/mermaid-cli, set HOPCODE_MERMAID_MMD_CLI, or set HOPCODE_MERMAID_ALLOW_NPX=1.',
        };
    }
...
function findExecutable(command, env) {
    ...
    const allowLocalRenderers = env['HOPCODE_MERMAID_ALLOW_LOCAL_RENDERERS'] === '1';
...
async function renderPngWithMmdcAsync(source, mmdc, env, signal) {
    const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'hopcode-mermaid-'));
    const inputPath = path.join(tempDir, 'diagram.mmd');
    const outputPath = path.join(tempDir, 'diagram.png');
    const renderWidth = getMermaidRenderWidth(env);
    try {
...
function getMermaidCellAspectRatio(env) {
    const configuredAspectRatio = Number(env['HOPCODE_MERMAID_CELL_ASPECT_RATIO']);
