...
export function terminalSupportsSynchronizedOutput(env = process.env) {
    const disableSynced = env['HOPCODE_DISABLE_SYNCHRONIZED_OUTPUT'] ??
        env['HOPCODE_DISABLE_SYNCHRONIZED_OUTPUT'];
    const synced = env['HOPCODE_SYNCHRONIZED_OUTPUT'] ?? env['HOPCODE_SYNCHRONIZED_OUTPUT'];
    const forceSynced = env['HOPCODE_FORCE_SYNCHRONIZED_OUTPUT'] ??
