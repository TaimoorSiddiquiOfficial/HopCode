...
function makeSettings(opts) {
    ...
    return {
        ...
        user: opts.userUi
            ? file({ ui: opts.userUi }, opts.userPath ?? '/home/u/.hopcode/settings.json')
            : empty,
        workspace: opts.workspaceUi
            ? file({ ui: opts.workspaceUi }, opts.workspacePath ?? '/repo/.hopcode/settings.json')
            : empty,
...
describe('resolveCustomBanner', () => {
    ...
    it('rejects {path} mixed with tier keys (mutually exclusive object branches)', () => {
        const out = resolveCustomBanner(makeSettings({
            ...
            userPath: '/home/u/.hopcode/settings.json',
        }));
