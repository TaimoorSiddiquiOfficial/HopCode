...
export class HistoryVault {
    ...
    truncateFromId(itemId) {
        const idx = this._items.findIndex((i) => i.id === itemId);
        if (idx === -1)
            return;
        this._items = this._items.slice(0, idx);
        this._windowStart = Math.min(this._windowStart, Math.max(0, this._items.length - WINDOW_SIZE));
        this._contextNoteCache.clear();
    }
    /** Clear everything. */
    clear() {
    ...
    jumpToIndex(globalIndex) {
        ...
        const liveEdge = Math.max(0, this._items.length - WINDOW_SIZE);
        this._windowStart = Math.max(0, Math.min(liveEdge, clamped - Math.floor(WINDOW_SIZE / 2)));
    }
    // ─── Queries ──────────────────────────────────────────────────────────────
    getWindowInfo() {
     ...
     */
    getWindowItems() {
        const slice = this._items.slice(this._windowStart, this._windowStart + WINDOW_SIZE);
     ...
     */
    search(query, maxResults = 200) {
        const trimmed = query.trim();
        if (!trimmed)
            return [];
        const lower = trimmed.toLowerCase();
        const results = [];
        for (let i = 0; i < this._items.length && results.length < maxResults; i++) {
            ...
            const matchExcerpt = (start > 0 ? '…' : '') + raw + (end < text.length ? '…' : '');
            results.push({ item, globalIndex: i, matchExcerpt });
        }
        return results;
    }
    // ─── Private helpers ──────────────────────────────────────────────────────
    _getOrBuildContextNote() {
    ...
    _buildContextNote() {
        ...
        const excerpts = [];
        for (let i = olderItems.length - 1; i >= 0 && excerpts.length < MAX_CONTEXT_EXCERPTS; i--) {
            const item = olderItems[i];
            if (!CONTEXT_WORTHY_TYPES.has(item.type))
                continue;
            const text = getSearchableText(item);
            if (!text || text.length < MIN_EXCERPT_LEN)
                continue;
            const truncated = text.slice(0, MAX_EXCERPT_LEN);
            excerpts.unshift(`[${item.type}] ${truncated}`);
        }
        const summaryText = excerpts.length > 0
...
export function getSearchableText(item) {
    ...
    if (typeof asAny['summary'] === 'string' && asAny['summary']) {
        return asAny['summary'];
    }
    return '';
}
//# sourceMappingURL=HistoryVault.js.map