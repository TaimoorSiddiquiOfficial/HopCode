...
import { OPENROUTER_DEFAULT_MODEL, fetchOpenRouterModels, isOpenRouterConfig, mergeOpenRouterConfigs, } from '../../auth/providers/oauth/openrouterOAuth.js';
import { PROVIDER_REGISTRY, getProvider, } from '../../commands/auth/registry.js';
 ...
 */
export const MANAGE_MODELS_SOURCES = PROVIDER_REGISTRY.filter((p) => p.liveModels && p.authType === AuthType.USE_OPENAI).map((p) => p.id);
...
export async function fetchManageModelsCatalog(source) {
    ...
    const categories = await fetchOpenAICompatibleModels(provider.baseUrl, apiKey);
    if (!categories || categories.length === 0) {
        throw new Error(`Could not fetch models for ${provider.label}. Check your API key and connection.`);
    }
    const models = categories.flatMap((cat) => cat.models.map((m) => ({
        id: m.id,
        name: m.label || m.id,
...
export function getEnabledModelIdsForSource(source, settings) {
    ...
    if (!provider)
        return [];
    return openaiConfigs
        .filter((config) => config.envKey === provider.envKey &&
...
export async function saveManageModelsSelection(params) {
    ...
    switch (source) {
        ...
        default: {
            ...
            const selectedWithMeta = selectedModels.map((m) => ({
                ...m,
                envKey: provider.envKey || undefined,
                baseUrl: provider.baseUrl,
            }));
            // Keep all existing configs that don't belong to this provider
            const nonProviderConfigs = existingOpenAIConfigs.filter((c) => !(c.envKey === provider.envKey && c.baseUrl === provider.baseUrl));
