...
export const AppContainer = (props) => {
    ...
    const [transcriptFreeze, setTranscriptFreeze] = useState(null);
    ...
    useEffect(() => {
        (async () => {
            ...
            if (resumedSessionData) {
                ...
                const userTurnCount = resumedSessionData.conversation.messages.filter((m) => m.type === 'user' && m.subtype !== 'mid_turn_user_message').length;
                if (userTurnCount > 0) {
    ...
    useEffect(() => {
        ...
        restorePromptStash(promptStashTargetDir, buffer.text, (text) => buffer.setText(text));
    ...
    const { isModelDialogOpen, isFastModelMode, isVoiceModelMode, isVisionModelMode, modelDialogPersistScope, openModelDialog, closeModelDialog, } = useModelCommand();
    ...
    const { messageQueue, addMessage, popAllMessages, restoreMessages, drainQueue, popNextSegment, } = useMessageQueue();
    ...
    const handleFinalSubmit = useCallback((submittedValue, options) => {
        ...
        if (['/quit', '/exit', 'exit', 'quit', ':q', ':q!', ':wq', ':wq!'].includes(submittedValue.trim())) {
    ...
    const openTranscript = useCallback(() => {
        setTranscriptFreeze({
            ...
            committedItems: historyForTranscriptRef.current.filter(isHistoryItemVisibleAfterRestore),
    ...
    const needsBlockingInput = dialogsVisible || streamingState === StreamingState.WaitingForConfirmation;
    useEffect(() => {
    ...
    return (_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(ConfigContext.Provider, { value: config, children: _jsx(AppContext.Provider, { value: {
                        ...
                        startupWarnings,
                    }, children: _jsx(ThoughtExpandedProvider, { value: thoughtExpandedValue, children: _jsx(RenderModeProvider, { value: renderModeValue, children: _jsx(TerminalOutputProvider, { value: writeRaw, children: _jsx(ShellFocusContext.Provider, { value: isFocused, children: transcriptFreeze ? (_jsx(StreamingContext.Provider, { value: streamingState, children: _jsx(TranscriptView, { items: transcriptItems, useAlternateScreen: !useTerminalBuffer }) })) : (_jsx(App, {})) }) }) }) }) }) }) }) }));
};
