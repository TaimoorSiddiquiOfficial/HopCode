...
export async function runNonInteractive(config, settings, input, prompt_id, options = {}) {
    return promptIdContext.run(prompt_id, async () => {
            ...
            if (manager) {
                ...
                if (options.controlService) {
                    ...
                    approvalListener = (event) => {
                        ...
                        const reason = `Auto-cancelling tool ${event.toolName} requested by ` +
        ...
        try {
            ...
            else if (!options.continueInterrupted &&
            ...
            const processToolCallBatch = async (batchRequests, setModelOverride) => {
                ...
                const duplicatePendingResponses = [];
                ...
                const launchToolCall = async (requestInfo) => {
                    ...
                    const response = await executeToolCall(config, requestInfo, abortController.signal, {
                        outputUpdateHandler,
            ...
            while (true) {
                ...
                if (toolCallRequests.length > 0) {
                    ...
                    const { responseParts: toolResponseParts, repeatedDuplicateProviderToolCall, } = await processToolCallBatch(toolCallRequests, (override) => {
                        if (!inlineModelOverrideActive) {
