...
import { FatalConfigError, getErrorMessage, Storage, createDebugLogger, stripRuntimeSnapshotPrefix, } from '@hoptrendy/hopcode-core';
 ...
 */
function detecthopcodeHomeRedirectWithoutMigration(activeUserSettingsPath) {
    ...
    return (`HOPCODE_HOME points to "${activehopcodeDir}" but no settings.json was found there. ` +
...
export function loadSettings(workspaceDir = process.cwd(), consumeCorruptionEnvVars = true) {
    ...
    const hopcodeHomeRedirectWarning = detecthopcodeHomeRedirectWithoutMigration(userSettingsPath);
    let systemSettings = {};
    ...
    const isTrusted = isWorkspaceTrusted(initialTrustCheckSettings, undefined, realWorkspaceDir).isTrusted ?? true;
