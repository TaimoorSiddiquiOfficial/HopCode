...
import { atomicWriteFileSync, FatalConfigError, ideContextStore, } from '@hoptrendy/hopcode-core';
...
describe('Trusted Folders Loading', () => {
    ...
    it('should use HOPCODE_CODE_TRUSTED_FOLDERS_PATH env var if set', () => {
        const customPath = '/custom/path/to/trusted_folders.json';
        process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH'] = customPath;
  ...
  "/keep/path": "TRUST_FOLDER"
    ...
    it('saveTrustedFolders should fall back to a clean rewrite when preserving comments fails during parse', () => {
        ...
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('Falling back to clean rewrite for trusted folders'));
    });
  ...
  "/existing/path": "TRUST_FOLDER"
        ...
        const actualCommentJson = await vi.importActual('comment-json');
        mockFsExistsSync.mockImplementation((p) => p === userPath || p === dirPath);
        fs.readFileSync.mockImplementation((p) => {
            if (p === userPath)
                return originalContent;
        ...
        parseSpy
            .mockImplementationOnce((...args) => actualCommentJson.parse(...args))
        ...
        expect(atomicWriteFileSync).toHaveBeenCalledTimes(1);
        expect(vi.mocked(atomicWriteFileSync).mock.calls[0]?.[1]).toBe(`{\n  "/new/path": "TRUST_FOLDER"\n}`);
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('invalid preserved output'));
    });
    it('saveTrustedFolders should fall back to a clean rewrite when the existing file is a top-level array', () => {
        ...
        expect(atomicWriteFileSync).toHaveBeenCalledTimes(1);
        expect(vi.mocked(atomicWriteFileSync).mock.calls[0]?.[1]).toBe(`{\n  "/new/path": "TRUST_FOLDER"\n}`);
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('trusted folders file is not a JSON object'));
    });
    it.each(['"hello"', '42', 'true', 'null'])('saveTrustedFolders should fall back to a clean rewrite when the existing file is a top-level primitive: %s', (existingContent) => {
        const userPath = getTrustedFoldersPath();
        const dirPath = path.dirname(userPath);
        mockFsExistsSync.mockImplementation((p) => p === userPath || p === dirPath);
        fs.readFileSync.mockImplementation((p) => {
