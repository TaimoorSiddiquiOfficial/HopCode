...
function makeSettingsFile(overrides = {}) {
    return {
        settings: {},
        originalSettings: {},
        path: '/home/user/.hopcode/settings.json',
        rawJson: '{}',
        ...overrides,
    };
}
function makeLoadedSettings(overrides = {}) {
    ...
    const workspace = makeSettingsFile({
        path: '/project/.hopcode/settings.json',
        ...overrides.workspace,
    });
    return {
        user,
...
describe('SettingsWatcher', () => {
    ...
    describe('lifecycle', () => {
        it('should create chokidar watchers for user and workspace directories', () => {
            ...
            expect(mockWatch).toHaveBeenCalledWith('/home/user/.hopcode', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
            expect(mockWatch).toHaveBeenCalledWith('/project/.hopcode', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
        });
    ...
    describe('path filtering', () => {
        it('should trigger refresh only for settings.json basename', async () => {
            ...
            fireAllEvent(0, 'change', '/home/user/.hopcode/settings.json');
        ...
        it('should ignore .tmp files', async () => {
            ...
            const listener = vi.fn();
            watcher.addChangeListener(listener);
            fireAllEvent(0, 'change', '/home/user/.hopcode/settings.json.tmp');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
        ...
        it('should ignore .orig files', async () => {
            ...
            const listener = vi.fn();
            watcher.addChangeListener(listener);
            fireAllEvent(0, 'change', '/home/user/.hopcode/settings.json.orig');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            expect(settings.reloadScopeFromDisk).not.toHaveBeenCalled();
        });
        it('should ignore unrelated files in the same directory', async () => {
            ...
            const listener = vi.fn();
            watcher.addChangeListener(listener);
            fireAllEvent(0, 'change', '/home/user/.hopcode/other-file.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            expect(settings.reloadScopeFromDisk).not.toHaveBeenCalled();
        });
    });
    describe('debouncing', () => {
        ...
        it('should batch changes from different scopes in one debounce window', async () => {
            ...
            fireAllEvent(0, 'change', '/home/user/.hopcode/settings.json');
            fireAllEvent(1, 'change', '/project/.hopcode/settings.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
    ...
    describe('change type classification', () => {
        it('should report created when file appears', async () => {
            ...
            fireAllEvent(userIdx, 'add', '/home/user/.hopcode/settings.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            const events = listener.mock.calls[0][0];
        ...
        it('should report deleted when file disappears', async () => {
            ...
            fireAllEvent(0, 'unlink', '/home/user/.hopcode/settings.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            const events = listener.mock.calls[0][0];
    ...
    describe('watcher creation failure', () => {
        it('should continue with remaining scopes when chokidar throws', () => {
            ...
            expect(mockWatchers[0].dir).toBe('/project/.hopcode');
        });
    ...
    describe('lazy directory watching', () => {
        ...
        it('bootstrap ignored predicate allows only the .hopcode entry', () => {
            const workspaceOnly = makeLoadedSettings({
                workspaceSettingsActive: false,
                user: { path: '/home/user/.hopcode/settings.json' },
            });
            const w = new SettingsWatcher(workspaceOnly);
            mockExistsSync.mockReturnValue(false);
            w.startWatching();
            ...
            expect(ignored('/home/user/.hopcode')).toBe(false);
        ...
        it('should promote to a target watcher when .hopcode appears', async () => {
            const workspaceOnly = makeLoadedSettings({
                workspaceSettingsActive: false,
            });
            const w = new SettingsWatcher(workspaceOnly);
            ...
            // `.hopcode` is created.
            fireAllEvent(0, 'addDir', '/home/user/.hopcode');
            await flush();
            // Bootstrap closed, target watcher opened on `.hopcode`.
            expect(mockWatchers[0].instance.close).toHaveBeenCalled();
            expect(mockWatchers).toHaveLength(2);
            expect(mockWatchers[1].dir).toBe('/home/user/.hopcode');
            // A settings.json already inside `.hopcode` is picked up via a refresh.
        ...
        it('should promote immediately when .hopcode appears during the TOCTOU window', async () => {
            const workspaceOnly = makeLoadedSettings({
                workspaceSettingsActive: false,
            });
            const w = new SettingsWatcher(workspaceOnly);
            ...
            w.startWatching();
            await flush();
            // Bootstrap on parent, then immediate promote to `.hopcode` without an event.
            expect(mockWatchers).toHaveLength(2);
            expect(mockWatchers[0].dir).toBe('/home/user');
            expect(mockWatchers[1].dir).toBe('/home/user/.hopcode');
            w.stopWatching();
        });
        it('should demote back to bootstrap when .hopcode is removed', async () => {
            const workspaceOnly = makeLoadedSettings({
                workspaceSettingsActive: false,
            });
            const w = new SettingsWatcher(workspaceOnly);
            ...
            w.startWatching();
            expect(mockWatchers).toHaveLength(1);
            expect(mockWatchers[0].dir).toBe('/home/user/.hopcode');
            // `.hopcode` directory itself is removed.
            fireAllEvent(0, 'unlinkDir', '/home/user/.hopcode');
            await flush();
            ...
            fireAllEvent(1, 'addDir', '/home/user/.hopcode');
            await flush();
            expect(mockWatchers).toHaveLength(3);
            expect(mockWatchers[2].dir).toBe('/home/user/.hopcode');
            w.stopWatching();
        });
        it('should not double-promote from a stale bootstrap callback', async () => {
            ...
            await flush();
            expect(mockWatchers).toHaveLength(2);
            const targetWatchers = mockWatchers.filter((m) => m.dir === '/home/user/.hopcode');
    ...
    describe('path resolution', () => {
        it('should use resolved paths from LoadedSettings (supports HOPCODE_HOME redirect)', () => {
            const customSettings = makeLoadedSettings({
                user: { path: '/custom/hopcode-home/settings.json' },
            });
            const w = new SettingsWatcher(customSettings);
            w.startWatching();
            expect(mockWatch).toHaveBeenCalledWith('/custom/hopcode-home', expect.any(Object));
            w.stopWatching();
        });
    });
