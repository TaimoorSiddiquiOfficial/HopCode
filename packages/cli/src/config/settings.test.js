...
describe('Settings Loading and Merging', () => {
    ...
    describe('loadSettings', () => {
        ...
        it('should resolve ${VAR} in settings from home-level .env file (#4466)', () => {
            const homeHopCodeEnvPath = path.join(path.dirname(USER_SETTINGS_PATH), '.env');
            const userSettingsContent = {
                mcpServers: {
                    myServer: {
                        headers: {
            ...
            mockFsExistsSync.mockImplementation((p) => p === USER_SETTINGS_PATH || p === homeHopCodeEnvPath);
            fs.readFileSync.mockImplementation((p) => {
                if (p === USER_SETTINGS_PATH)
                    return JSON.stringify(userSettingsContent);
        ...
        it('should prefer ~/.hopcode/.env over ~/.env for the same key (first-write-wins) (#4466)', () => {
            const hopCodeEnvPath = path.join(path.dirname(USER_SETTINGS_PATH), '.env');
            const homeEnvPath = path.join(path.dirname(path.dirname(USER_SETTINGS_PATH)), '.env');
            const userSettingsContent = {
                mcpServers: {
                    myServer: {
            ...
            mockFsExistsSync.mockImplementation((p) => p === USER_SETTINGS_PATH || p === hopCodeEnvPath || p === homeEnvPath);
            fs.readFileSync.mockImplementation((p) => {
                if (p === USER_SETTINGS_PATH)
                    return JSON.stringify(userSettingsContent);
                if (p === hopCodeEnvPath)
                    return 'PRECEDENCE_TOKEN=from_hopcode_dir';
                if (p === homeEnvPath)
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            const mcpServers = settings.merged.mcpServers;
            expect(mcpServers?.['myServer']?.headers?.['Authorization']).toBe('Bearer from_hopcode_dir');
            delete process.env['PRECEDENCE_TOKEN'];
        });
        it('should succeed with unresolved placeholder when .env read throws (#4466)', () => {
            ...
            mockFsExistsSync.mockImplementation((p) => p === USER_SETTINGS_PATH || p === hopCodeEnvPath);
            fs.readFileSync.mockImplementation((p) => {
                if (p === USER_SETTINGS_PATH)
                    return JSON.stringify(userSettingsContent);
                if (p === hopCodeEnvPath)
    ...
    describe('loadEnvironment', () => {
        function setup({ isFolderTrustEnabled = true, isWorkspaceTrustedValue = true, }) {
            ...
            const geminiEnvPath = path.resolve(path.join(HOPCODE_DIR, '.env'));
            vi.mocked(isWorkspaceTrusted).mockReturnValue({
        ...
        it('uses user .hopcode/.env as fallback when the project .env lacks an API key', () => {
            ...
            expect(process.env['OPENCODE_GO_API_KEY']).toEqual('from_user_HOPCODE_env');
            cwdSpy.mockRestore();
        });
        it('lets the project .env win over user .hopcode/.env when both define the API key', () => {
            delete process.env['OPENCODE_GO_API_KEY'];
            const cwdSpy = vi
                .spyOn(process, 'cwd')
                .mockReturnValue(MOCK_WORKSPACE_DIR);
        ...
        it('still loads user .hopcode/.env fallback when the workspace is untrusted', () => {
            delete process.env['OPENCODE_GO_API_KEY'];
            delete process.env['PROJECT_ENV_VAR'];
            const cwdSpy = vi
                .spyOn(process, 'cwd')
        ...
        describe('settings.env field', () => {
            ...
            it('should never set HOPCODE_HOME or HOPCODE_RUNTIME_DIR from settings.env', () => {
                ...
                const userSettingsContent = {
                    env: {
                        HOPCODE_HOME: '/redirected/by/settings',
                        HOPCODE_RUNTIME_DIR: '/redirected/runtime',
                        HARMLESS_VAR: 'ok',
                    },
                };
                mockFsExistsSync.mockImplementation((p) => [USER_SETTINGS_PATH].includes(p.toString()));
            ...
            it('should let .env override settings.env when process.env has an empty string', () => {
                ...
                const geminiEnvPath = path.join(RESOLVED_MOCK_WORKSPACE_DIR, HOPCODE_DIR, '.env');
                const userSettingsContent = {
                    env: {
        ...
        describe('HOPCODE_HOME custom directory', () => {
            ...
            beforeEach(() => {
                ...
                delete process.env['HOPCODE_HOME_TEST_VAR'];
            });
            afterEach(() => {
            ...
            it('does not exclude DEBUG/DEBUG_MODE from .env in a HOPCODE_HOME dir not named .hopcode', () => {
                ...
                // because the .env lives inside the user-level HOPCODE_HOME directory.
            ...
            it('still honors HOPCODE_HOME from a user-level .env (~/.hopcode/.env)', () => {
                delete process.env['HOPCODE_HOME'];
                const cwdSpy = vi
                    .spyOn(process, 'cwd')
                ...
                mockFsExistsSync.mockImplementation((p) => [USER_SETTINGS_PATH, userHopcodeEnvPath].includes(p.toString()));
                fs.readFileSync.mockImplementation((p) => {
                    if (p === USER_SETTINGS_PATH)
                        return JSON.stringify({});
            ...
            it('does not exclude DEBUG/DEBUG_MODE from a workspace .hopcode/.env', () => {
                const cwdSpy = vi
                    .spyOn(process, 'cwd')
                    .mockReturnValue(MOCK_WORKSPACE_DIR);
                const workspaceHopcodeEnvPath = path.join(MOCK_WORKSPACE_DIR, HOPCODE_DIR, '.env');
                vi.mocked(isWorkspaceTrusted).mockReturnValue({
                    isTrusted: true,
                    source: 'file',
                });
                mockFsExistsSync.mockImplementation((p) => [USER_SETTINGS_PATH, workspaceHopcodeEnvPath].includes(p.toString()));
                fs.readFileSync.mockImplementation((p) => {
                    if (p === USER_SETTINGS_PATH)
                        return JSON.stringify({});
            ...
            it('still blocks HOPCODE_HOME from a workspace .hopcode/.env', () => {
                delete process.env['HOPCODE_HOME'];
                delete process.env['HOPCODE_RUNTIME_DIR'];
                const cwdSpy = vi
                    .spyOn(process, 'cwd')
            ...
            it('redirects user settings path when HOPCODE_HOME is set in ~/.hopcode/.env', () => {
                ...
                mockFsExistsSync.mockImplementation((p) => [userHopcodeEnvPath, customSettingsPath].includes(p.toString()));
                fs.readFileSync.mockImplementation((p) => {
                    if (p === userHopcodeEnvPath)
                        return 'HOPCODE_HOME=/tmp/from-user-env';
                    if (p === customSettingsPath)
                ...
                const loaded = loadSettings(MOCK_WORKSPACE_DIR);
                // The pre-pass propagates HOPCODE_HOME from ~/.hopcode/.env into
            ...
            it('warns when HOPCODE_HOME redirects but the legacy ~/.hopcode still has settings', () => {
                ...
                process.env['HOPCODE_HOME'] = customHome;
                const legacySettings = path.join('/mock/home/user', HOPCODE_DIR, 'settings.json');
                ...
                mockFsExistsSync.mockImplementation((p) => {
                    const s = p.toString();
                    // Legacy settings exist, active HOPCODE_HOME settings do not.
                    if (s === legacySettings)
                        return true;
                ...
                fs.readFileSync.mockImplementation(() => '{}');
                const loaded = loadSettings(MOCK_WORKSPACE_DIR);
                const warningMatch = loaded.migrationWarnings.find((w) => w.includes('HOPCODE_HOME points to'));
            ...
            it('prefers HOPCODE_HOME/.env over ~/.env at the home-dir step', () => {
                ...
                fs.readFileSync.mockImplementation((p) => {
                    ...
                    if (p === homeEnvPath)
                        return 'HOPCODE_HOME_TEST_VAR=fromHomeDir';
                    return '{}';
                });
                loadEnvironment(loadSettings(MOCK_WORKSPACE_DIR).merged);
            ...
            it('falls back to legacy ~/.hopcode/.env for non-routing keys when <HOPCODE_HOME>/.env is absent', () => {
                // User keeps OPENAI_API_KEY in ~/.hopcode/.env and adds HOPCODE_HOME to the
                ...
                mockFsExistsSync.mockImplementation((p) => [USER_SETTINGS_PATH, customSettingsPath, userHopcodeEnvPath].includes(p.toString()));
                fs.readFileSync.mockImplementation((p) => {
                    if (p === USER_SETTINGS_PATH)
                        return JSON.stringify({});
                    if (p === customSettingsPath)
    ...
    describe('reloadEnvironment', () => {
        ...
        it('uses user .hopcode/.env as fallback when the project .env lacks an API key', () => {
            ...
            const userQwenEnvPath = path.normalize(path.join('/mock/home/user', HOPCODE_DIR, '.env'));
