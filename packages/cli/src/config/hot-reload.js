...
import { createDebugLogger, ApprovalMode, getMCPServerStatus, } from '@hoptrendy/hopcode-core';
...
export function registerMcpHotReload(watcher, settings, config, topTierMcpServers) {
    ...
    return watcher.addChangeListener(async (events) => {
        ...
        const nextGating = recomputeMcpGating(settings, next, cwd, config.getCliAllowedMcpServerNames(), isYolo);
