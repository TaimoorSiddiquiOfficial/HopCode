...
export class SettingsWatcher {
    ...
    startWatching() {
        ...
        for (const { scope, settingsPath } of this.getScopePaths()) {
            ...
            // Watch `.hopcode` directly when it already exists; otherwise bootstrap on
            // the parent and promote once `.hopcode` appears. We never create the
        ...
        try {
            const watcher = watchFs(dir, {
                ...
                if (this.watchGeneration.get(scope) !== gen)
                    return;
                // The `.hopcode` directory itself was removed — demote so we can catch
    ...
    watchParentForDir(scope, settingsPath) {
        ...
        // Close the TOCTOU gap: `.hopcode` may have been created between the
    ...
    async promoteScope(scope, settingsPath) {
        ...
        if (!this.started)
            return;
        this.watchTargetDir(scope, settingsPath);
        // Pick up a settings.json that already exists inside the new `.hopcode`.
        this.scheduleRefresh(scope);
    }
