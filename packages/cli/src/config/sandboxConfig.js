...
function getSandboxCommand(sandbox) {
    ...
    const environmentConfiguredSandbox = process.env['HOPCODE_SANDBOX']?.toLowerCase().trim() ?? '';
    sandbox =
    ...
    if (typeof sandbox === 'string' && sandbox) {
        ...
        throw new FatalSandboxError(`Missing sandbox command '${sandbox}' (from HOPCODE_SANDBOX)`);
    }
    ...
    if (sandbox === true) {
        throw new FatalSandboxError('HOPCODE_SANDBOX is true but failed to determine command for sandbox; ' +
