...
import { ToolNames, DEFAULT_HOPCODE_MODEL, OutputFormat, NativeLspService, Storage, } from '@hoptrendy/hopcode-core';
...
describe('parseArguments', () => {
    ...
    it('should throw an error when both --izn and --approval-mode are used together', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Cannot use both --izn (-z) and --approval-mode together. Use --approval-mode=izn instead.'));
        mockExit.mockRestore();
    });
    ...
    it('should accept --json-schema with no -p / positional when stdin is piped', async () => {
        // `echo "..." | hopcode --json-schema ...` — input arrives via the
...
describe('loadCliConfig', () => {
    ...
    it('should reset context file names to HOPCODE.md (plus legacy HOPCODE.md) and AGENTS.md by default', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {};
    ...
    it('should reset context filenames to defaults when context.fileName is not configured', async () => {
        ...
        const argv = await parseArguments();
        const settings = {};
        const defaultContextFiles = ['HOPCODE.md', 'AGENTS.md'];
        const getAllSpy = vi
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
        ...
        const settings = {};
        await expect(loadCliConfig(settings, invalidArgv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, auto, izn');
    });
});
...
describe('loadCliConfig model selection', () => {
    ...
    it('uses the default HopCode model if nothing is set', async () => {
        process.argv = ['node', 'script.js']; // No model set.
        const argv = await parseArguments();
        const config = await loadCliConfig({
        // No model set.
        }, argv, undefined, []);
        expect(config.getModel()).toBe(DEFAULT_HOPCODE_MODEL);
    });
...
describe('loadCliConfig approval mode', () => {
    ...
    it('should throw when approval mode in settings is invalid', async () => {
        ...
        await expect(loadCliConfig(settings, argv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, auto, izn');
    });
...
describe('Telemetry configuration via environment variables', () => {
    ...
    it("should treat HOPCODE_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES='1' as true", async () => {
        vi.stubEnv('HOPCODE_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES', '1');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({}, argv, undefined, []);
        expect(config.getTelemetryIncludeSensitiveSpanAttributes()).toBe(true);
    });
    it("should treat HOPCODE_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES='false' as false", async () => {
        vi.stubEnv('HOPCODE_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES', 'false');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
...
describe('loadCliConfig runtimeOutputDir', () => {
    const originalArgv = process.argv;
    const originalRuntimeEnv = process.env['HOPCODE_RUNTIME_DIR'];
    beforeEach(() => {
        process.argv = ['node', 'script.js'];
    ...
    afterEach(() => {
        ...
        if (originalRuntimeEnv !== undefined) {
            process.env['HOPCODE_RUNTIME_DIR'] = originalRuntimeEnv;
        }
        else {
            delete process.env['HOPCODE_RUNTIME_DIR'];
        }
    ...
    it('should let HOPCODE_RUNTIME_DIR env var take priority over settings', async () => {
