...
describe('settings/trustedFolders path getters are lazy', () => {
    ...
    afterEach(() => {
        ...
        if (originalTrustedPath === undefined)
            ...
            process.env['HOPCODE_CODE_TRUSTED_FOLDERS_PATH'] = originalTrustedPath;
    });
    it('getUserSettingsPath() reflects HOPCODE_HOME set after module load', () => {
        ...
        expect(getUserSettingsPath()).toBe(path.join('/tmp/hopcode-lazy-test', 'settings.json'));
    });
    ...
    it('getTrustedFoldersPath() reflects HOPCODE_HOME set after module load', () => {
        expect(getTrustedFoldersPath()).toBe(path.join(homedir(), '.hopcode', 'trustedFolders.json'));
        process.env['HOPCODE_HOME'] = '/tmp/hopcode-lazy-test';
        expect(getTrustedFoldersPath()).toBe(path.join('/tmp/hopcode-lazy-test', 'trustedFolders.json'));
    });
});
