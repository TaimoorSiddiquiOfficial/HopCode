 ...
 */
import { ApprovalMode, AuthType, Config, DEFAULT_HOPCODE_EMBEDDING_MODEL, FileDiscoveryService, getAllGeminiMdFilenames, loadServerHierarchicalMemory, MAX_SUBAGENT_DEPTH_LIMIT, setGeminiMdFilename as setServerGeminiMdFilename, resolveTelemetrySettings, FatalConfigError, Storage, InputFormat, OutputFormat, SessionService, ideContextStore, ToolNames, NativeLspClient, createDebugLogger, NativeLspService, isBareMode, isSafeModeEnv, isToolEnabled, isTlsVerificationDisabled, SchemaValidator, } from '@hoptrendy/hopcode-core';
...
export async function parseArguments() {
    ...
    if (rawArgv.length > 0 &&
        (rawArgv[0].endsWith('/dist/hopcode-cli/cli.js') ||
    ...
    const yargsInstance = yargs(rawArgv)
        ...
        .command('$0 [query..]', 'Launch HopCode CLI', (yargsInstance) => yargsInstance
        .positional('query', {
        ...
        alias: 'z',
        type: 'boolean',
        description: 'Automatically accept all actions (aka IZN mode, see https://www.youtube.com/watch?v=xvFZjo5PgG0 for more details)?',
        default: false,
    })
        .option('IZN', {
        type: 'boolean',
        ...
        description: 'Set the approval mode: plan (plan only), default (prompt for approval), auto-edit (auto-approve edit tools), auto (LLM classifier auto-approves safe actions, blocks risky ones), izn (auto-approve all tools)',
    })
        .option('acp', {
        type: 'boolean',
        ...
        .option('worktree', {
        type: 'string',
        description: 'Start the session inside a git worktree at <repoRoot>/.hopcode/worktrees/<slug>/. ' +
...
settingsWatcher) {
    ...
    // Pass cwd so that relative paths like ".hopcode" resolve per-project.
    ...
    const projectOutputLanguagePath = path.join(projectStorage.getHopCodeDir(), 'output-language.md');
    const globalOutputLanguagePath = path.join(Storage.getGlobalHopCodeDir(), 'output-language.md');
    ...
    if (argv.continue || argv.resume) {
        ...
        if (argv.resume) {
            ...
            if (!sessionData) {
                const message = `No saved session found with ID ${argv.resume}. Run \`hopcode --resume\` without an ID to choose from existing sessions.`;
    ...
    const webSearchConfig = buildWebSearchConfig(argv, settings, selectedAuthType);
    const configParams = {
        sessionId,
        sessionData,
        ...
        powerShellConfig: settings.security?.powershell
            ? {
                ...
                mode: settings.security.powershell.mode ??
                    'ask',
