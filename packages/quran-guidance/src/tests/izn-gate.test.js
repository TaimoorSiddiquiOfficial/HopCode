...
describe('checkIznGate', () => {
    ...
    it('provides verification steps when blocked', () => {
        ...
        if (!result.allowed) {
            ...
            expect(result.requiredConfirmation).toContain('Self-Verification Required');
            // Intent questions probe the user's goal
            expect(result.intentQuestions.some((q) => q.toLowerCase().includes('goal'))).toBe(true);
...
describe('reportIznScope', () => {
    ...
    it('includes intent verification guidance in destructive report', () => {
        ...
        expect(result.context).toContain('If intent was clarified before this action');
        expect(result.context).toContain('confirm the result matches what was agreed');
    });
...
describe('Izn progressive escalation', () => {
    const blockHistory = (cat, count) => Array.from({ length: count }, () => ({
        category: cat,
    }));
    ...
    it('reportIznScope for generic tool does not escalate', () => {
        const result = reportIznScope({ toolName: 'read_file' }, blockHistory('file_deletion', 10));
        expect(result.context).toContain('Izn scope');
        expect(result.context).not.toContain('WARNING');
        expect(result.context).not.toContain('REFUSAL');
    });
});
//# sourceMappingURL=izn-gate.test.js.map