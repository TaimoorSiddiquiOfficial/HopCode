...
function mockInitResponse() {
    return new Response('data: {"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"quran-mcp","version":"1.0"}}}\n\n', {
        status: 200,
        headers: {
...
function mockToolResponse(text) {
    // JSON-escape the text to avoid breaking the SSE payload structure
    const escaped = JSON.stringify(text);
    return new Response(`data: {"jsonrpc":"2.0","id":2,"result":{"content":[{"type":"text","text":${escaped}}]}}\n\n`, {
        status: 200,
        headers: {
            'Content-Type': 'text/event-stream',
            'Mcp-Session-Id': 'test-session-1',
...
describe('createQuranMcpClient', () => {
    ...
    describe('session initialization', () => {
        it('establishes an MCP session on first tool call', async () => {
            ...
            fetchSpy.mockResolvedValueOnce(mockInitResponse());
            // Second call: notifications/initialized
            fetchSpy.mockResolvedValueOnce(new Response(null, { status: 200 }));
            // Third call: tool call
            fetchSpy.mockResolvedValueOnce(mockToolResponse('بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ'));
    ...
    describe('caching', () => {
        it('returns cached result on second call without re-fetching', async () => {
            ...
            fetchSpy.mockResolvedValueOnce(mockInitResponse());
            fetchSpy.mockResolvedValueOnce(new Response(null, { status: 200 }));
            fetchSpy.mockResolvedValueOnce(mockToolResponse('قُلْ هُوَ اللَّهُ أَحَدٌ'));
            ...
            fetchSpy.mockResolvedValueOnce(mockToolResponse('Say, "He is Allah, [Who is] One"'));
    ...
    describe('searchQuran', () => {
        it('passes query and translation to MCP tool', async () => {
            ...
            fetchSpy.mockResolvedValueOnce(mockInitResponse());
            fetchSpy.mockResolvedValueOnce(new Response(null, { status: 200 }));
            fetchSpy.mockResolvedValueOnce(new Response('data: {"jsonrpc":"2.0","id":3,"result":{"content":[{"type":"text","text":"results here"}]}}\n\n', {
                status: 200,
                headers: {
                    'Content-Type': 'text/event-stream',
                    'Mcp-Session-Id': 'test-session-1',
...
//# sourceMappingURL=quran-mcp-client.test.js.map