...
const VALID_RESPONSE_PATTERNS = new Set([
...
const VALID_DESTRUCTIVE_CATEGORIES = new Set([
    'file_deletion',
    'force_push',
    'database_drop',
    'database_truncate',
...
describe('Data validation — ayahGuidance', () => {
    it('every useWhen value is a valid AgentSituation', () => {
        for (const entry of ayahGuidance) {
            for (const situation of entry.agentStrategy.useWhen) {
                expect(VALID_SITUATIONS.has(situation), `"${situation}" in ayah "${entry.ref}" is not a valid AgentSituation`).toBe(true);
            }
        }
    });
    it('every angle is a valid QuranicAngle', () => {
        for (const entry of ayahGuidance) {
            for (const angle of entry.angles) {
                expect(VALID_ANGLES.has(angle), `"${angle}" in ayah "${entry.ref}" is not a valid QuranicAngle`).toBe(true);
            }
        }
    });
    ...
    it('do, avoid, and tone arrays are non-empty', () => {
        for (const entry of ayahGuidance) {
            expect(entry.agentStrategy.do.length, `"${entry.ref}" has empty "do" array`).toBeGreaterThan(0);
            expect(entry.agentStrategy.avoid.length, `"${entry.ref}" has empty "avoid" array`).toBeGreaterThan(0);
            expect(entry.agentStrategy.tone.length, `"${entry.ref}" has empty "tone" array`).toBeGreaterThan(0);
        }
    });
});
describe('Data validation — situationAngleMap', () => {
    it('every key is a valid AgentSituation', () => {
        for (const key of Object.keys(situationAngleMap)) {
            expect(VALID_SITUATIONS.has(key), `"${key}" is not a valid AgentSituation`).toBe(true);
        }
    });
    it('every angle in every mapping is a valid QuranicAngle', () => {
        for (const [situation, angles] of Object.entries(situationAngleMap)) {
            for (const angle of angles) {
                expect(VALID_ANGLES.has(angle), `"${angle}" in situation "${situation}" is not a valid QuranicAngle`).toBe(true);
            }
        }
    });
    it('covers all AgentSituation values', () => {
        ...
        for (const situation of VALID_SITUATIONS) {
            expect(mappedSituations.has(situation), `AgentSituation "${situation}" is missing from situationAngleMap`).toBe(true);
        }
    });
});
describe('Data validation — responsePatterns', () => {
    it('every key is a valid response pattern', () => {
        for (const key of Object.keys(responsePatterns)) {
            expect(VALID_RESPONSE_PATTERNS.has(key), `"${key}" is not a valid response pattern key`).toBe(true);
        }
    });
});
describe('Data validation — iznBehaviorRules', () => {
    it('every category is a valid DestructiveActionCategory', () => {
        for (const rule of iznBehaviorRules) {
            expect(VALID_DESTRUCTIVE_CATEGORIES.has(rule.category), `"${rule.category}" is not a valid DestructiveActionCategory`).toBe(true);
        }
    });
    ...
    it('covers all DestructiveActionCategory values', () => {
        ...
        for (const category of VALID_DESTRUCTIVE_CATEGORIES) {
            expect(ruleCategories.has(category), `DestructiveActionCategory "${category}" has no corresponding iznBehaviorRule`).toBe(true);
        }
    });
...
//# sourceMappingURL=data-validation.test.js.map