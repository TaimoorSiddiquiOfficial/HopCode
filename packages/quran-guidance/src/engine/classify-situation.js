...
function classifySituationInternal(input) {
    ...
    // ── 2. Privacy risk ─────────────────────────────────────────────
    if (matchesAny(text, ['private', 'personal data', 'user data', 'sensitive'])) {
        return {
    ...
    // ── 8. User frustration ─────────────────────────────────────────
    if (matchesAny(text, ['frustrated', 'annoying', 'useless', 'waste of time'])) {
        return {
    ...
    if (matchPhrase(text, 'build everything') ||
...
function matchesAny(text, keywords) {
    return keywords.some((kw) => kw.includes(' ') ? matchPhrase(text, kw) : hasWord(text, kw));
}
// ── Signal helpers ──────────────────────────────────────────────────
function hasBugSignals(text) {
    return (matchesAny(text, ['bug', 'error', 'fail', 'crash']) ||
...
function hasArchDecisionSignals(text) {
    return (matchPhrase(text, 'should i use') ||
...
function hasPlanningSignals(text) {
    return matchesAny(text, ['plan', 'architecture', 'design']);
}
function resolveMissingContextConfidence(msgLen, text) {
    if (msgLen < 15)
        return 0.85;
    if (matchPhrase(text, 'not sure') || hasWord(text, 'idk'))
        return 0.8;
    if (msgLen < 40)
        return 0.7;
    return 0.65;
}
//# sourceMappingURL=classify-situation.js.map