...
function computeEscalationLevel(matchedCategories, blockHistory) {
    ...
    if (consecutive >= REFUSAL_THRESHOLD)
        return IZN_ESCALATION_LEVELS.REFUSAL;
    if (consecutive >= WARNING_THRESHOLD)
        return IZN_ESCALATION_LEVELS.WARNING;
    if (consecutive >= 1)
        return IZN_ESCALATION_LEVELS.CAUTION;
    return null;
}
/**
 ...
 */
export function checkIznGate(input, blockHistory) {
    const textToCheck = buildTextToCheck(input);
    const matchedCategories = matchCategories(textToCheck, input.toolName);
    if (matchedCategories.length === 0) {
        return { allowed: true };
    }
    const escalationLevel = computeEscalationLevel(matchedCategories, blockHistory);
    ...
    return {
        allowed: false,
        reason,
        requiredConfirmation: buildConfirmationText(uniqueAnalysisPlan, escalationLevel),
 ...
 */
export function reportIznScope(input, blockHistory) {
    const textToCheck = buildTextToCheck(input);
    const matchedCategories = matchCategories(textToCheck, input.toolName);
    const escalationLevel = computeEscalationLevel(matchedCategories, blockHistory);
    if (matchedCategories.length === 0) {
    ...
    const escalationHeader = escalationLevel === IZN_ESCALATION_LEVELS.REFUSAL
        ? '⚠️ IZN REFUSAL '
    ...
    if (uniqueRevertConditions.length > 0) {
        lines.push('', 'Revert conditions (check if any apply):', ...uniqueRevertConditions.map((c) => `- ${c}`));
    }
    if (escalationLevel === IZN_ESCALATION_LEVELS.WARNING) {
        lines.push('', '⚠️ Repeated same-category action — regression risk elevated.', '    Confirm this specific action is still within the agreed scope.');
    }
    if (escalationLevel === IZN_ESCALATION_LEVELS.REFUSAL) {
        lines.push('', '⛔ This action type has been blocked repeatedly.', '    The risk of unintended damage now outweighs the stated intent.', '    If this action is genuinely necessary, re-explain the full intent', '    and break the work into smaller, verified steps.');
    }
    ...
    return [
        ...
        ...(input.toolArgs
            ? Object.entries(input.toolArgs)
                .filter((entry) => entry[0] !== 'description' && typeof entry[1] === 'string')
                .map(([, value]) => value)
            : []),
    ].join(' ');
}
 ...
 */
function matchCategories(textToCheck, toolName) {
...
function buildEscalatedReason(matchedCategories, escalationLevel) {
...
function buildConfirmationText(analysisPlan, escalationLevel) {
    const prefix = escalationLevel === IZN_ESCALATION_LEVELS.REFUSAL
...
//# sourceMappingURL=izn-gate.js.map