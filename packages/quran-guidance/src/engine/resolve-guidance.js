 ...
 */
export function resolveGuidance(analysis, iznModeActive = false) {
    let matches = ayahGuidance.filter((entry) => {
        const situationMatch = entry.agentStrategy.useWhen.includes(analysis.situation);
        const angleMatch = entry.angles.some((angle) => analysis.requiredAngles.includes(angle));
        return situationMatch || angleMatch;
    });
    // Izn overlay: if in Izn mode, also match izn-specific guidance
    if (iznModeActive) {
        const iznMatches = ayahGuidance.filter((entry) => entry.agentStrategy.useWhen.includes('izn_mode_active'));
        matches = [...matches, ...iznMatches];
    }
    // Take up to 5 best matches
    const selected = matches.slice(0, 5);
    let appliedAngles = Array.from(new Set(selected.flatMap((entry) => entry.angles)));
    ...
    const doActions = Array.from(new Set(selected.flatMap((entry) => entry.agentStrategy.do)));
    const avoidActions = Array.from(new Set(selected.flatMap((entry) => entry.agentStrategy.avoid)));
    const tone = Array.from(new Set(selected.flatMap((entry) => entry.agentStrategy.tone)));
    return {
...
function chooseResponsePattern(situation, iznModeActive) {
    if (iznModeActive) {
...
//# sourceMappingURL=resolve-guidance.js.map