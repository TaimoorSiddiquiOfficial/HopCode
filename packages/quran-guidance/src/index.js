...
export async function getQuranGuidedBehaviorWithMCP(input) {
    ...
    const enriched = input.mcpClient
        ? await (await import('./mcp/enrich-guidance.js')).enrichGuidanceWithMCP(decision, input.mcpClient, input.translation)
        : { decision, ayahTexts: new Map() };
    const behaviorPrompt = composeAgentBehavior(enriched.decision, enriched.ayahTexts);
    return {
        analysis,
