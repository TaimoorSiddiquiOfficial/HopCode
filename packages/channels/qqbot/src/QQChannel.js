 ...
 */
import { ChannelBase, SessionRouter, getGlobalHopCodeDir, sanitizeSenderName, sanitizePromptText, sanitizeLogText, } from '@hoptrendy/channel-base';
    ...
    async connect() {
        ...
        if (!this.config.instructions) {
            ...
            if (this.qqConfig.allowMention !== false) {
                parts.push('', '## @提及格式', '', '消息内容中的 <@OPENID> 标签代表群成员的 QQ 标识。', '当其他群成员 @你（机器人）时，消息内容中会出现 <@你的BotOPENID> 标签，这代表该消息是 @给你的。机器人自己的 OPENID 将在连接建立后告知。', '你可以在回复中使用 <@OPENID> 格式来 @提及特定的群成员。', '例如：回复 "<@ABC123DEF456> 你好" 会在群里 @该成员。');
            }
