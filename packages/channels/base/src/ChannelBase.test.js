...
describe('ChannelBase', () => {
    ...
    describe('dispatch modes', () => {
        it('collect: buffers messages and coalesces into one followup prompt', async () => {
            ...
            const p2 = ch.handleInbound(envelope({ text: 'second', messageId: 'msg-2' }));
            const p3 = ch.handleInbound(envelope({ text: 'third', messageId: 'msg-3' }));
    ...
    describe('loop prompts', () => {
        ...
        describe('runWebhookTask', () => {
            ...
            it('runs an unattended prompt and proactively sends the final response', async () => {
                ...
                expect(bridge.prompt).toHaveBeenCalledTimes(1);
                expect(bridge.prompt).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('[External event "ci_failed" from github-ci]'), {});
        ...
        it('drains collected messages after a loop prompt completes', async () => {
            ...
            expect(bridge.prompt).toHaveBeenLastCalledWith(expect.any(String), 'while loop runs', expect.any(Object));
        });
