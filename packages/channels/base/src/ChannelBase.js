...
import { sanitizeSenderName, sanitizeQuotedText, sanitizePromptText, sanitizePromptPath, sanitizeLogText, truncateCodePoints, PROMPT_UNSAFE_INVISIBLES, } from './sanitize.js';
import { ChannelLoopSkippedError } from './ChannelLoopScheduler.js';
import { buildChannelWebhookPrompt, resolveChannelWebhookTarget, } from './ChannelWebhookTask.js';
import { parseChannelMemoryIntent, } from './channel-memory-intent.js';
import { createChannelMemoryRecallIndex, selectRelevantChannelMemory, selectRelevantChannelMemoryFromIndex, } from './channel-memory-recall.js';
/**
...
export class ChannelBase {
    ...
    constructor(name, config, bridge, options) {
        ...
        this.groupHistory = new GroupHistoryStore(options?.groupHistoryPath ??
            join(getGlobalHopCodeDir(), 'channels', `${encodeURIComponent(name)}-group-history.jsonl`));
        ...
        const pairingStore = config.senderPolicy === 'pairing'
    ...
    registerCancelCommand(name = 'cancel') {
        this.registerCommand(name, async (envelope) => {
            ...
            await this.sendMessage(envelope.chatId, cancelSucceeded
    ...
    async handlePermissionResponseCommand(envelope, args, decision) {
        ...
        if (lookup.kind === 'none') {
            await this.sendMessage(envelope.chatId, lookup.explicit
        ...
        if (!response) {
            await this.sendMessage(envelope.chatId, decision === 'approve-always'
        ...
        this.removePendingPermission(pending.requestId);
        await this.sendMessage(envelope.chatId, accepted
            ? decision === 'approve'
                ? 'Permission approved.'
    ...
    registerSharedCommands() {
        ...
        this.registerCommand('help', async (envelope) => {
            ...
            if (agentCommands.length > 0) {
                lines.push('', 'Agent commands (forwarded to HopCode):');
    ...
    isStoredLoopTargetAuthorized(target, senderName) {
        ...
        return (this.groupGate.check(envelope).allowed &&
    ...
    async handleChannelMemoryIntent(envelope, intent) {
        ...
        if (intent.kind === 'update_confirm' || intent.kind === 'remove_confirm') {
            ...
            if (!pending) {
                await this.sendMessage(envelope.chatId, intent.kind === 'update_confirm'
            ...
            try {
                ...
                await this.sendMessage(envelope.chatId, message === 'Channel memory entry changed'
        ...
        if (intent.kind === 'remember') {
            ...
            if (result.added.length > 0) {
                ...
                await this.sendMessage(envelope.chatId, result.duplicateIds.length > 0
        ...
        if (intent.kind === 'list' || intent.kind === 'inspect') {
            ...
            if (intent.kind === 'inspect') {
                ...
                await this.sendMessage(envelope.chatId, entry
    ...
    async processInbound(envelope) {
        ...
        const current = prev.then(async () => {
            ...
            if (!recognizedSlashCommand &&
                ...
                try {
                    const relevantEntries = await this.selectRelevantChannelMemory(envelope, memoryTarget, recallRead);
