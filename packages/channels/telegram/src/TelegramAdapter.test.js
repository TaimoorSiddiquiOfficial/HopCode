...
function installFakeBot(channel) {
    const bot = {
        token: 'token',
        api: {
            getMe: vi.fn().mockResolvedValue({ id: 123, username: 'HOPCODE_bot' }),
...
describe('TelegramChannel', () => {
    ...
    it('handles /start locally', async () => {
        ...
        expect(bot.api.sendMessage).toHaveBeenCalledWith('chat-1', expect.stringContaining('HopCode Telegram bot'), { parse_mode: 'HTML' });
    });
    it('only treats addressed Telegram bot commands as mentions in groups', () => {
        const channel = createChannel();
        installFakeBot(channel);
        channel.botUsername = 'HOPCODE_bot';
        ...
        const addressedCommand = channel.buildTestEnvelope({
            from: { id: 1, first_name: 'User' },
            chat: { id: 2, type: 'group' },
        }, '/cancel@HOPCODE_bot', [{ type: 'bot_command', offset: 0, length: 16 }]);
    ...
    it('does not let bare bot commands pass mention-gated groups', async () => {
        ...
        await channel.handleInbound(channel.buildTestEnvelope(groupMessage, '/cancel@HOPCODE_bot', [
