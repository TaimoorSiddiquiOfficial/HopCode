...
export class FeishuChannel extends ChannelBase {
        ...
        try {
            ...
            if (!resp.ok) {
                if (resp.status === 401)
                    this.tokenCache = undefined;
                const detail = await resp.text().catch(() => '');
                process.stderr.write(`[Feishu:${this.name}] createStreamingCard failed: HTTP ${resp.status} ${detail}\n`);
                return { messageId: '', success: false };
            }
            const data = (await resp.json());
    ...
    async updateCard(messageId, text, finished = false, inboundMsgId, statusLabel) {
        ...
        if (!FEISHU_ID_RE.test(messageId))
            return false;
        try {
    ...
    async onResponseComplete(chatId, fullText, sessionId) {
        ...
        if (displayText.length + completedSuffix.length > MAX_FINAL_CARD_CHARS) {
            ...
            const maxBody = MAX_FINAL_CARD_CHARS -
                prefix.length -
                suffix.length -
    ...
    onPromptStart(chatId, sessionId, messageId) {
        ...
        if (inboundMsgId) {
            ...
            if (!this.cardSessions.has(inboundMsgId)) {
                ...
                this.cardSessions.set(inboundMsgId, cardState);
                this.createStreamingCard(chatId, placeholderText, undefined, inboundMsgId)
                    .then((result) => {
                    ...
                    if (cardState.stopped || this.stoppedMessages.has(inboundMsgId)) {
                        ...
                        if (cardState.abandoned) {
                            if (result.success) {
                                this.deleteCard(result.messageId).catch((err) => {
                                    process.stderr.write(`[Feishu:${this.name}] ORPHANED CARD: failed to delete abandoned card msg=${result.messageId}: ${err instanceof Error ? err.message : err}\n`);
                                });
                            }
                        ...
                        if (result.success) {
                            ...
                            const prefix = cardState.atPrefix ||
                                this.msgToSenderName.get(inboundMsgId) ||
                                '';
                            this.updateCard(result.messageId, prefix, true, inboundMsgId, this.stopLabelFor(cardState.terminalStatus, cardState.userStopped ?? false)).catch(() => { });
                        }
                        cardState.creating = false;
                        this.cleanupCard(inboundMsgId);
                        return;
                    ...
                    .catch((err) => {
                    process.stderr.write(`[Feishu:${this.name}] Processing card error: ${err}\n`);
                    cardState.creating = false;
                    this.cleanupCard(inboundMsgId);
                });
            }
