...
import { ChannelBase, isTerminalTaskLifecycleType, sanitizeLogText, sanitizeSenderName, } from '@hoptrendy/channel-base';
...
function splitTextChunks(text, firstChunkLimit) {
    if (!text)
        return [text];
    const chunks = [];
    let offset = 0;
...
export class DingtalkChannel extends ChannelBase {
    ...
    async sendTextReply(chatId, text, atUserId) {
        const webhook = this.webhooks.get(chatId);
        if (!webhook)
            return;
        const mentionPrefix = atUserId ? `@${atUserId}\n\n` : '';
        const chunks = splitTextChunks(text, TEXT_MESSAGE_LIMIT - mentionPrefix.length);
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (!resp.ok) {
                const detail = await resp.text().catch(() => '');
                process.stderr.write(`[DingTalk:${this.name}] sendTextReply failed: HTTP ${resp.status} ${detail}\n`);
            }
        }
    }
