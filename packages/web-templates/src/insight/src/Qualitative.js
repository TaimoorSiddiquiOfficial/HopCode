...
export function InteractionStyle({ qualitative, insights, }) {
    ...
    if (!interactionStyle)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("h2", { id: "section-usage", className: "text-xl font-semibold text-slate-900 mt-8 mb-4", children: "How You Use HopCode" }), _jsxs("div", { className: "narrative", children: [_jsx("p", { children: _jsx(MarkdownText, { children: interactionStyle.narrative }) }), interactionStyle.key_pattern && (_jsxs("div", { className: "key-insight", children: [_jsx("strong", { children: "Key pattern:" }), ' ', _jsx(MarkdownText, { children: interactionStyle.key_pattern })] }))] }), _jsx(DashboardCards, { insights: insights }), _jsx(HeatmapSection, { heatmap: insights.heatmap })] }));
}
...
function HopCodeMdAdditionsSection({ additions, }) {
...
export function Improvements({ qualitative, }) {
    ...
    if (!improvements)
        return null;
    const hasFeatureSuggestions = (Array.isArray(improvements.HOPCODE_md_additions) &&
