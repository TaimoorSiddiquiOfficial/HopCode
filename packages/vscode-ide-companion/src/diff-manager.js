 ...
 */
import { IdeDiffAcceptedNotificationSchema, IdeDiffClosedNotificationSchema, } from '@hoptrendy/hopcode-core';
...
export class DiffManager {
    ...
    async showDiff(filePath, a, b) {
        ...
        // Left side: old content using hopcode-diff scheme
        ...
        // Right side: new content using hopcode-diff scheme
        ...
        await vscode.commands.executeCommand('setContext', 'hopcode.diff.isVisible', true);
    ...
    async onActiveEditorChange(editor) {
        ...
        await vscode.commands.executeCommand('setContext', 'hopcode.diff.isVisible', isVisible);
    }
    ...
    async closeDiffEditor(rightDocUri) {
        const diffInfo = this.diffDocuments.get(rightDocUri.toString());
        await vscode.commands.executeCommand('setContext', 'hopcode.diff.isVisible', false);
        if (diffInfo) {
    ...
    /** Close all open hopcode-diff editors */
    async closeAll() {
