...
import { applyProviderInstallPlanToFile, snapshotSettingsForRollback, restoreSettingsSnapshot, writeCodingPlanConfig, readHopCodeSettingsForVSCode, clearPersistedAuth, } from '../../services/settingsWriter.js';
...
const AUTH_RELATED_HOPCODE_SETTINGS = [
    'hopcode.provider',
    'hopcode.apiKey',
    'hopcode.codingPlanRegion',
];
export function resolveHopCodeCliEntryPath(extensionUri, extensionMode) {
...
export class WebViewProvider {
    ...
    constructor(context, extensionUri) {
        ...
        const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(async (e) => {
            const authSettingsChanged = AUTH_RELATED_HOPCODE_SETTINGS.some((setting) => e.affectsConfiguration(setting));
     ...
     */
    async syncVSCodeSettingsToHopCodeConfig() {
     ...
     */
    async syncHopCodeConfigToVSCodeSettings() {
        try {
            ...
            const updates = [];
            if (config.get('provider', 'coding-plan') !==
            ...
            if (config.get('codingPlanRegion', 'china') !==
                ...
                updates.push(config.update('codingPlanRegion', hopcodeSettings.codingPlanRegion, target));
            }
            if (updates.length === 0) {
            ...
            logger.error('[WebViewProvider] Failed to sync hopcode config to VSCode settings:', error);
        }
    }
    /**
     * Attempt to restore authentication state and initialize connection.
    ...
    async doInitializeAgentConnection(options) {
        ...
        const run = async () => {
            ...
            const cliEntry = resolveHopCodeCliEntryPath(this.extensionUri, this.context.extensionMode);
            try {
     ...
     * the HopCode panel (or sidebar view) when clicked.
     */
    notifyUser(message) {
        void vscode.window
            .showInformationMessage(`HopCode: ${message}`, 'Show')
            .then((action) => {
            if (action === 'Show') {
                const panel = this.panelManager.getPanel();
                if (panel) {
                    ...
                    void vscode.commands.executeCommand('hopcode.focusChat');
                }
            }
        });
    ...
    /**
     * Whether the user can currently see the HopCode panel.
    ...
    /** Whether the hopcode.notifications setting is enabled. */
