...
const { mockConfigChangeHandlers, availableCommandsCallbackRef, mockCreateImagePathResolver, mockConfigGet, mockConfigUpdate, mockGetGlobalTempDir, mockGetPanel, mockMessageHandlerInstances, mockOnDidChangeConfiguration, mockOnDidChangeActiveTextEditor, mockOnDidChangeTextEditorSelection, mockOpenExternal, mockReadHopCodeSettingsForVSCode, mockWriteCodingPlanConfig, mockWriteModelProvidersConfig, mockClearPersistedAuth, mockApplyProviderInstallPlanToFile, mockSnapshotSettingsForRollback, mockRestoreSettingsSnapshot, slashCommandNotificationCallbackRef, endTurnCallbackRef, streamChunkCallbackRef, permissionRequestCallbackRef, askUserQuestionCallbackRef, mockShowInformationMessage, mockWindowState, mockHopCodeAgentManagerInstances, mockClipboardWriteText, } = vi.hoisted(() => ({
    ...
    mockReadHopCodeSettingsForVSCode: vi.fn(() => null),
...
import { WebViewProvider, resolveHopCodeCliEntryPath, } from './WebViewProvider.js';
...
describe('resolveHopCodeCliEntryPath', () => {
    it('uses the source dev entry when the extension runs in development mode', () => {
        ...
        try {
            ...
            expect(resolveHopCodeCliEntryPath({ fsPath: extensionRoot }, 2)).toBe(devEntry);
        }
        finally {
    ...
    it('uses the bundled CLI outside development mode', () => {
        expect(resolveHopCodeCliEntryPath({ fsPath: '/extension-root' }, undefined)).toBe('/extension-root/dist/hopcode-cli/cli.js');
    });
});
/**
...
describe('WebViewProvider settings sync', () => {
    ...
    it('does not report success for api-key settings without interactive auth data', async () => {
        ...
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        const synced = await provider.syncVSCodeSettingsToHopCodeConfig();
    ...
    it('only syncs non-secret VS Code settings from ~/.hopcode/settings.json', async () => {
        ...
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        await provider.syncHopCodeConfigToVSCodeSettings();
    ...
    it('ignores non-auth hopcode setting changes', async () => {
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        const syncSpy = vi
            .spyOn(provider, 'syncVSCodeSettingsToHopCodeConfig')
    ...
    it('reacts to auth-related hopcode setting changes', async () => {
        ...
        const configChangeHandler = mockConfigChangeHandlers.at(-1);
        expect(configChangeHandler).toBeDefined();
        await configChangeHandler?.(createConfigChangeEvent('hopcode', 'hopcode.apiKey'));
    ...
    it('clears persisted credentials and disconnects when apiKey is emptied', async () => {
        ...
        vi.spyOn(provider, 'syncVSCodeSettingsToHopCodeConfig').mockResolvedValue(false);
    ...
    it('does not de-auth when non-apiKey auth settings change on an api-key provider', async () => {
        ...
        await configChangeHandler?.(createConfigChangeEvent('hopcode', 'hopcode.codingPlanRegion'));
...
describe('Notification & dot indicator', () => {
    ...
    it('shows orange dot and notification when a long task completes while panel is not active', async () => {
        ...
        expect(mockShowInformationMessage).toHaveBeenCalledWith('HopCode: Waiting for your input.', 'Show');
    });
    ...
    it('shows blue dot and notification for permission requests when panel is not active', async () => {
        ...
        expect(mockShowInformationMessage).toHaveBeenCalledWith('HopCode: Needs your permission to use Bash.', 'Show');
    });
    ...
    it('does not show idle notification for background notification turns', async () => {
        ...
        expect(mockShowInformationMessage).not.toHaveBeenCalledWith('HopCode: Waiting for your input.', 'Show');
    });
