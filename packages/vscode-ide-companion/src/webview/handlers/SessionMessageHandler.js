...
export class SessionMessageHandler extends BaseMessageHandler {
    ...
    async handle(message) {
        ...
        switch (message.type) {
            ...
            case 'switchHopCodeSession':
                await this.handleSwitchHopCodeSession(data?.sessionId || '');
                break;
            case 'getHopCodeSessions':
                await this.handleGetHopCodeSessions(data?.cursor ?? undefined, data?.size ?? undefined);
                break;
            case 'resumeSession':
                await this.handleResumeSession(data?.sessionId || '');
                break;
            case 'deleteHopCodeSession':
                await this.handleDeleteHopCodeSession(data?.sessionId || '');
                break;
            case 'renameHopCodeSession':
                await this.handleRenameHopCodeSession(data?.sessionId || '', data?.title || '');
                break;
            case 'openNewChatTab':
    ...
    async promptAuth(message) {
        ...
        if (result === 'Configure') {
            if (this.authHandler) {
                ...
                await vscode.commands.executeCommand('hopcode.auth');
            }
            return true;
        }
        return false;
    ...
    async handleSessionExport(format) {
    ...
    async handleSendMessage(text, context, fileContext, attachments, editTargetTurnIndex) {
        ...
        if (editTargetTurnIndex !== undefined) {
            ...
            if (!this.agentManager.isConnected) {
                await this.promptAuth('You need to configure your provider to use HopCode.');
                return;
            }
            if (!this.agentManager.currentSessionId) {
                try {
                    ...
                    if (this.shouldPromptAuth(createErr)) {
                        await this.promptAuth('Your session has expired or is invalid. Please configure your provider to continue using HopCode.');
                        return;
                    }
     ...
     */
    async handleSwitchHopCodeSession(sessionId) {
        try {
     ...
     */
    async handleGetHopCodeSessions(cursor, size) {
        try {
     ...
     */
    async handleDeleteHopCodeSession(sessionId) {
        try {
     ...
     */
    async handleRenameHopCodeSession(sessionId, title) {
        try {
