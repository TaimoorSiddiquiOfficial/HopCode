...
import { ALL_PROVIDERS, ALIBABA_PROVIDERS, AuthType, THIRD_PARTY_PROVIDERS, shouldShowStep, resolveBaseUrl, getDefaultBaseUrlForProtocol, getDefaultModelIds, } from '@hoptrendy/hopcode-core';
/**
...
export class AuthMessageHandler extends BaseMessageHandler {
    ...
    async handleAuthInteractive() {
        try {
            ...
            const selectedId = await this.pick(items, 'HopCode: Select Provider', 'Choose how to connect');
            if (!selectedId) {
                return;
            }
    ...
    async runProviderSetupFlow(provider) {
        const flowTitle = provider.uiLabels?.flowTitle ?? `HopCode: ${provider.label}`;
