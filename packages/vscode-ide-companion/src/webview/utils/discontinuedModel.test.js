...
import { DISCONTINUED_MESSAGES, isDiscontinuedModel, parseAcpModelId, HOPCODE_OAUTH_AUTH_TYPE, RUNTIME_PREFIX, } from './discontinuedModel.js';
...
describe('parseAcpModelId', () => {
    ...
    it('marks runtime snapshots and still strips the trailing wrapper', () => {
        expect(parseAcpModelId('$runtime|hopcode-oauth|qwen3-coder-plus(hopcode-oauth)')).toEqual({
...
describe('isDiscontinuedModel', () => {
    ...
    it('does NOT flag a runtime HopCode OAuth snapshot as discontinued', () => {
        expect(isDiscontinuedModel('$runtime|hopcode-oauth|qwen3-coder-plus(hopcode-oauth)')).toBe(false);
    });
