...
describe('HopCodeConnectionHandler', () => {
    ...
    beforeEach(async () => {
        ...
        handler = new HopCodeConnectionHandler();
        mockConnection = {
    ...
    describe('connect retry logic', () => {
        ...
        it('retries connect on spawn failure and succeeds on second attempt', async () => {
            mockConnection.connect
                .mockRejectedValueOnce(new Error('HopCode ACP process failed to start (exit code: null, signal: SIGTERM)'))
        ...
        it('does not retry when startup was cancelled by disconnect', async () => {
            const cancelledError = new AcpConnectionDisconnectError('HopCode ACP process startup was cancelled by disconnect');
            mockConnection.connect.mockRejectedValue(cancelledError);
            await expect(handler.connect(mockConnection, '/workspace', '/path/to/cli.js')).rejects.toThrow(cancelledError);
            expect(mockConnection.connect).toHaveBeenCalledTimes(1);
        });
        it('throws after exhausting all connect retry attempts (3 attempts)', async () => {
            const spawnError = new Error('HopCode ACP process failed to start (exit code: null, signal: SIGTERM)');
...
//# sourceMappingURL=hopcodeConnectionHandler.test.js.map