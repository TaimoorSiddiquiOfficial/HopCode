...
import { applyProviderInstallPlanToFile, clearPersistedAuth, readHopCodeSettingsForVSCode, restoreSettingsSnapshot, snapshotSettingsForRollback, writeCodingPlanConfig, writeModelProvidersConfig, } from './settingsWriter.js';
describe('settingsWriter', () => {
    let tempDir;
    let settingsPath;
    beforeEach(() => {
        vi.clearAllMocks();
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hopcode-vscode-settings-'));
        ...
        it('wipes preset, custom, and subscription-plan env keys without touching unrelated env', () => {
            ...
            const initial = {
                env: {
                    ...
                    HOPCODE_CUSTOM_API_KEY_OPENAI_HTTPS_API_FOO_COM_ABC123DEF456: 'sk-custom-1',
                    HOPCODE_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_BAR_COM_DEAD0BEEF000: 'sk-custom-2',
