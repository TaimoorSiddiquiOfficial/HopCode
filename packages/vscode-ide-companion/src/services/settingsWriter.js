...
import { ALL_PROVIDERS, AuthType, CUSTOM_API_KEY_ENV_PREFIX, Storage, applyProviderInstallPlan, resolveMetadataKey, stripRuntimeSnapshotPrefix, } from '@hoptrendy/hopcode-core';
...
// ---------------------------------------------------------------------------
/**
 * Mode for ~/.hopcode/settings.json — owner-only read/write. The unified install
 ...
 * so that `JSON.parse` can handle ~/.hopcode/settings.json files that
...
/**
 * Read ~/.hopcode/settings.json. Returns {} if the file is missing.
...
function readSettings() {
    ...
    try {
        ...
        throw new Error(`Cannot parse ~/.hopcode/settings.json (${message}). ` +
...
/**
 * Write ~/.hopcode/settings.json atomically (temp file + rename), creating the
...
// ---------------------------------------------------------------------------
/**
 * Create a ProviderSettingsAdapter backed by ~/.hopcode/settings.json.
 ...
 */
export function readHopCodeSettingsForVSCode() {
    let settings;
    try {
