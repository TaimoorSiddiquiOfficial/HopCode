...
export function isAcpConnectionDisconnectError(error) {
    return (error instanceof AcpConnectionDisconnectError ||
        (typeof error === 'object' &&
            error !== null &&
            error.name === 'AcpConnectionDisconnectError'));
}
/**
...
export class AcpConnection {
    ...
    async connect(cliEntryPath, workingDir = process.cwd(), extraArgs = []) {
        ...
        if (!fs.existsSync(cliEntryPath)) {
            throw new Error(`Bundled HopCode CLI entry not found at ${cliEntryPath}. The extension may not have been packaged correctly.`);
        }
    ...
    async setupChildProcessHandlers() {
        ...
        this.child.on('exit', (code, signal) => {
            ...
            rejectOnExit?.(this.disconnectRequested
        ...
        if (!this.child || this.child.killed) {
            ...
            if (this.disconnectRequested) {
                throw new AcpConnectionDisconnectError(`HopCode ACP process startup was cancelled by disconnect (exit code: ${code}, signal: ${signal})${stderrSuffix}`);
            }
            throw new Error(`HopCode ACP process failed to start (exit code: ${code}, signal: ${signal})${stderrSuffix}`);
        }
    ...
    handleExtNotification(method, params) {
        ...
        else if (method === '_hopcode/end_turn') {
