# HopCode Agent Server Extension for Zed

A [Zed](https://zed.dev) extension that integrates [HopCode](https://github.com/TaimoorSiddiquiOfficial/HopCode) as an AI agent server using the [Agent Client Protocol (ACP)](https://agentclientprotocol.com).
...
3. Search for "HopCode"
4. Click "Install"
5. Switch to the **Agent Server** tab and ensure HopCode is enabled

Alternatively, you can install from the command line:

```bash
zed --install-extension hopcode
```

## Usage

1. Open the Agent Panel in Zed (`cmd-shift-a` on macOS or `ctrl-shift-a` on Linux/Windows)
2. Select "HopCode" from the agent list
...
- Node.js >= 20 (for running HopCode agent server)

## Configuration

### Environment Variables

When running as an agent server, HopCode will:

- Inherit environment variables from Zed
- Read/create `~/.hopcode` directory for runtime settings
- Use existing model and authentication settings in `~/.hopcode/settings.json` (except for initial login)
...
- [HopCode Issues](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues)
...
- [Report Issues](https://github.com/TaimoorSiddiquiOfficial/HopCode/issues)
...
HopCode is an AI-powered coding assistant that helps developers write better code faster. It provides intelligent code completion, refactoring suggestions, bug detection, and natural language code generation supporting dozens of AI providers.
