...
export class HopCodeSessionManager {
    ...
    // ---------------------------------------------------------------------------
    async spawnAgent(session, request) {
    ...
    // ---------------------------------------------------------------------------
    translateAndEmit(session, event) {
        ...
        switch (type) {
            ...
            case 'content_block_delta': {
                const delta = asEvent(event).delta;
            ...
            case 'assistant': {
                const msg = asEvent(event).message;
                if (msg?.content) {
            ...
            case 'tool_result': {
                const toolResultEvent = asEvent(event);
            ...
            case 'control_request': {
                const controlEvent = asEvent(event);
                if (controlEvent.subtype === 'can_use_tool') {
                    const data = controlEvent.data ?? event;
                    session.writeCallback({
                        permissionRequest: {
            ...
            case 'result': {
                const resultEvent = asEvent(event);
            ...
            case 'system': {
                const systemEvent = asEvent(event);
    ...
    // Message handlers
    // ---------------------------------------------------------------------------
    async handleUserPrompt(session, prompt) {
    ...
    async handleCancelRound(session) {
    ...
    async handleShutdown(session, shutdown) {
