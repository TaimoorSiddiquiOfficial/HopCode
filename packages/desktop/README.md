...
HopCode is a desktop and headless agent workspace. It provides multi-session chat, source connections, skills, file previews, automations, and permission modes in a local-first application.

## Backend

This fork is Qwen-only:

- Agent sessions run through HopCode over ACP.
- The app does not store third-party LLM API keys.
- The built-in LLM connection is `hopcode`.
- Legacy multi-provider backends and package/runtime wiring have been removed.

## HopCode CLI Runtime

The desktop app talks to the HopCode CLI over ACP. Treat the CLI as a
runtime artifact, not as desktop source code. A packaged app must bundle a
known CLI build so users can launch it without installing `hopcode` separately.
...
| Published npm package       | You want a specific published CLI version for dev, CI, or release builds. | `HOPCODE_CODE_VERSION=0.15.12-acp.0 bun run dev`                        |
| Local npm tarball           | You need to verify the exact package contents before publishing.          | `HOPCODE_CODE_TARBALL=/path/to/hopcode-0.15.12-acp.0.tgz bun run dev` |
| Local hopcode checkout    | You are changing ACP or other CLI behavior while testing desktop.         | `HOPCODE_CODE_ROOT=/path/to/hopcode bun run dev`                      |
| Explicit CLI entry          | You need to point at a specific CLI file.                                 | `HOPCODE_CODE_CLI=/path/to/hopcode/scripts/dev.js bun run dev`        |

`electron:dev` uses local overrides first. If no override is set and this
repository is not inside the hopcode monorepo, it vendors the default version
from `HopCodeRuntime.version` in `package.json` and points Electron at the
vendored CLI automatically.

If you are preparing a package without publishing it, create the tarball from
the HopCode repository and point desktop at it:
...
cd /path/to/desktop
HOPCODE_CODE_TARBALL=/path/to/hopcode/hopcode-0.15.12-acp.0.tgz bun run dist:mac
```

Distribution builds run `electron:vendor:qwen` automatically. Set
`HOPCODE_CODE_VERSION` or `HOPCODE_CODE_TARBALL` when you want the packaged app to use
...
`HopCodeRuntime.version` from `package.json`.

Development runtime resolution checks sources in this order:

```text
HOPCODE_CODE_CLI / HOPCODE_CODE_ROOT / HOPCODE_CODE_PATH
HOPCODE_CODE_TARBALL
HOPCODE_CODE_VERSION
...
HOPCODE_CODE_TARBALL
HOPCODE_CODE_VERSION
HOPCODE_CODE_ROOT / HOPCODE_CODE_PATH
local monorepo checkout
HopCodeRuntime.version from package.json
```
...
| Platform | Artifact                                                                 |
| -------- | ------------------------------------------------------------------------ |
| macOS    | `hopcode-Desktop-{arm64,x64}.dmg`, `hopcode-Desktop-{arm64,x64}.zip` |
| Windows  | `hopcode-Desktop-x64.exe`                                              |
| Linux    | `hopcode-Desktop-x64.AppImage`                                         |

### What the Build Does

Each `electron:dist:*` command runs three stages:

1. **`electron:vendor:qwen`** — vendors a HopCode CLI runtime into `vendor/hopcode/`. Set `HOPCODE_CODE_VERSION` to download a published npm version, or `HOPCODE_CODE_TARBALL` to use a local `npm pack` tarball. If neither is set in this monorepo, it builds from the local checkout.
...
The `run` command spawns a headless server, creates a temporary session, streams the response, and exits. Provider flags are accepted only for compatibility; the backend remains HopCode.
...
- HopCode model discovery through ACP
