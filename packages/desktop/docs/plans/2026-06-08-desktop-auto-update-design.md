...
Enable stable-channel desktop auto-updates for packaged OpenWork and HopCode desktop builds using public GitHub Releases and `electron-updater`.
...
- Generate builder config for `openwork` and `hopcode`, verifying each brand emits the right `publish` config.
