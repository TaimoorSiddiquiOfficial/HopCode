...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="hopcode-Desktop-x64.AppImage"
...
if [ -z "$filename" ]; then
    filename="hopcode-Desktop-${arch}.${ext}"
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if is_app_running; then
        ...
        if is_app_running; then
            ...
            pkill -9 -f "HopCode Desktop.app" 2>/dev/null || true
            pkill -9 -f "HopCode.app" 2>/dev/null || true
    ...
    if [ -d "$INSTALL_DIR/HopCode.app" ]; then
        info "Removing previous HopCode installation..."
        rm -rf "$INSTALL_DIR/HopCode.app"
    fi
    ...
    echo ""
    printf "%b\n" "  HopCode Desktop has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'HopCode Desktop'${NC}"
    echo ""
    ...
    WRAPPER_PATH="$INSTALL_DIR/hopcode"
    APPIMAGE_INSTALL_PATH="$APP_DIR/hopcode-Desktop-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "hopcode.*AppImage" >/dev/null 2>&1; then
        info "Stopping HopCode Desktop..."
        pkill -f "hopcode.*AppImage" 2>/dev/null || true
        sleep 2
    fi
    ...
    [ -f "$APP_DIR/hopcode-x64.AppImage" ] && rm -f "$APP_DIR/hopcode-x64.AppImage"
...
#!/bin/bash
# HopCode Desktop launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.craft-agent/app/hopcode-Desktop-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@hopcode"
ELECTRON_CACHE_ALT="$HOME/.cache/@hopcode"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: HopCode Desktop not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/hopcode-x64.AppImage"
