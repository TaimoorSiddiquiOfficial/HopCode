...
function prepareSourcePackage(sourceRoot) {
    ...
    if (!existsSync(join(distDir, 'cli.js'))) {
        throw new Error(`Prepared HopCode runtime is missing ${join(distDir, 'cli.js')}`);
    }
    return distDir;
}
...
function verifyVendoredCli() {
    ...
    if (!cliPath) {
        throw new Error(`Vendored HopCode CLI not found. Checked: ${candidates.join(', ')}`);
    }
...
async function main() {
    ...
    const sourceRoot = (process.env.HOPCODE_CODE_ROOT || process.env.HOPCODE_CODE_PATH)?.trim();
    if (tarball) {
        ...
        if (monorepoRoot) {
            ...
            if (!defaultVersion) {
                throw new Error('No HopCode runtime configured. Set HOPCODE_CODE_TARBALL, HOPCODE_CODE_VERSION, HOPCODE_CODE_ROOT, or HopCodeRuntime.version.');
            }
...
await main();
//# sourceMappingURL=vendor-hopcode.js.map