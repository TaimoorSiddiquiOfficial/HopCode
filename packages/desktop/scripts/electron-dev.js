...
const HOPCODE_VENDOR_DIR = join(ELECTRON_DIR, 'vendor', 'hopcode');
const HOPCODE_VENDOR_CLI_CANDIDATES = [
    join(HOPCODE_VENDOR_DIR, 'dist', 'cli.js'),
    join(HOPCODE_VENDOR_DIR, 'cli.js'),
    join(HOPCODE_VENDOR_DIR, 'packages', 'cli', 'dist', 'index.js'),
];
function firstExistingPath(candidates) {
    return candidates.find((candidate) => existsSync(candidate));
}
function readDefaultHopCodeVersion() {
    try {
...
async function ensureQwenRuntimeForDev() {
    if (process.env.HOPCODE_CODE_CLI ||
        process.env.HOPCODE_CODE_ROOT ||
        process.env.HOPCODE_CODE_PATH) {
        console.log('🧭 Using HopCode runtime override from environment');
        return;
    }
    const requestedExternalArtifact = Boolean(process.env.HOPCODE_CODE_TARBALL || process.env.HOPCODE_CODE_VERSION);
    ...
    const vendoredCli = firstExistingPath(HOPCODE_VENDOR_CLI_CANDIDATES);
    if (vendoredCli && !requestedExternalArtifact) {
        process.env.HOPCODE_CODE_CLI = vendoredCli;
        console.log(`🧭 Using vendored HopCode CLI: ${vendoredCli}`);
        return;
    }
    const env = { ...process.env };
    if (!env.HOPCODE_CODE_TARBALL && !env.HOPCODE_CODE_VERSION) {
        const defaultVersion = readDefaultHopCodeVersion();
        if (!defaultVersion) {
            throw new Error('No HopCode CLI runtime configured. Set HOPCODE_CODE_VERSION, HOPCODE_CODE_TARBALL, HOPCODE_CODE_ROOT, or HopCodeRuntime.version in package.json.');
        }
        env.HOPCODE_CODE_VERSION = defaultVersion;
    }
    ...
    const proc = spawn({
        cmd: ['bun', 'run', 'scripts/vendor-hopcode.ts'],
        cwd: ROOT_DIR,
        stdout: 'inherit',
        stderr: 'inherit',
        env,
    ...
    if (exitCode !== 0) {
        throw new Error(`Failed to vendor HopCode CLI from ${sourceLabel}`);
    }
    const resolvedCli = firstExistingPath(HOPCODE_VENDOR_CLI_CANDIDATES);
    if (!resolvedCli) {
        throw new Error(`Vendored HopCode CLI not found in ${HOPCODE_VENDOR_DIR}`);
    }
    process.env.HOPCODE_CODE_CLI = resolvedCli;
    console.log(`🧭 Using vendored HopCode CLI: ${resolvedCli}`);
}
...
function getElectronEnv() {
    ...
    return {
        ...
        CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || 'HopCode Desktop',
