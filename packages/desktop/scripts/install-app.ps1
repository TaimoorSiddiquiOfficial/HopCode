# HopCode Desktop Windows Installer
...
$VERSIONS_URL = ""
$DOWNLOAD_DIR = "$env:TEMP\hopcode-install"
$APP_NAME = "HopCode Desktop"
...
if (-not $filename) {
    $filename = "hopcode-Desktop-$arch.exe"
}
...
$process = Get-Process -Name "HopCode Desktop", "HopCode" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing HopCode Desktop..."
...
Write-Info "Adding 'hopcode' command to PATH..."

$binDir = "$env:LOCALAPPDATA\HopCode\bin"
$cmdFile = "$binDir\hopcode.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\HopCode Desktop\HopCode Desktop.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'hopcode' command)"
} else {
    Write-Success "Command 'hopcode' is ready"
}
...
Write-Host ""
Write-Host "  HopCode Desktop has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: hopcode (restart terminal first)"
Write-Host ""
}
