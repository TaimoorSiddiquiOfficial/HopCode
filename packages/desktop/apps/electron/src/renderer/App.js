...
export default function App() {
    ...
    const hopcodeModelRefreshInFlightRef = useRef(null);
    const hopcodeModelRefreshAttemptedRef = useRef(new Set());
    const hopcodeModelRefreshCompletedRef = useRef(new Set());
    ...
    const refreshLlmConnections = useCallback(async () => {
        ...
        const visibleConnections = connections.map(connection => {
            ...
            if (connection.models?.length) {
                hopcodeModelRefreshCompletedRef.current.add(connection.slug);
            }
            return hopcodeModelRefreshCompletedRef.current.has(connection.slug)
                ? connection
        ...
        const hopcodeConnectionToRefresh = connections.find(connection => connection.providerType === 'hopcode'
            && !connection.models?.length
            && !hopcodeModelRefreshAttemptedRef.current.has(connection.slug));
        if (hopcodeConnectionToRefresh && !hopcodeModelRefreshInFlightRef.current) {
            hopcodeModelRefreshAttemptedRef.current.add(hopcodeConnectionToRefresh.slug);
            hopcodeModelRefreshInFlightRef.current = (async () => {
                try {
                    ...
                    hopcodeModelRefreshCompletedRef.current.add(hopcodeConnectionToRefresh.slug);
    ...
    const handleOpenUrlInBuiltInBrowser = useCallback((url) => {
        const open = async () => {
            ...
            const hostPattern = /^(localhost|\d{1,3}(?:\.\d{1,3}){3}|[\w-]+(?:\.[\w-]+)+)(?::\d+)?(?:\/|$)/i;
            ...
            try {
                ...
                if (instanceId) {
                    console.warn('[App] Failed to finish opening URL in built-in browser:', error);
                    return;
                }
                console.warn('[App] Failed to open URL in built-in browser, falling back to default browser:', error);
                handleOpenUrlExternal(url);
            }
        };
        void open();
