...
describe('normalizehopcodeSettingsSnapshot', () => {
    it('adds empty extension settings for older core settings snapshots', () => {
        const snapshot = {
            user: {
                path: '/Users/me/.hopcode/settings.json',
                values: {},
                mcpServers: [],
                hooks: [],
            },
            workspace: {
                path: '/repo/.hopcode/settings.json',
                values: {},
                mcpServers: [],
                hooks: [],
            },
        ...
        expect(normalizehopcodeSettingsSnapshot(snapshot)?.merged.extensions).toEqual([]);
    });
    it('normalizes missing extension child arrays', () => {
        ...
        expect(normalizehopcodeSettingsSnapshot(snapshot)?.merged.extensions[0]).toEqual({
            id: 'extension-a',
            name: 'Extension A',
            version: undefined,
            isActive: undefined,
    ...
    it('accepts the ACP shape with top-level extensions and isTrusted', () => {
        ...
        const normalized = normalizehopcodeSettingsSnapshot(snapshot);
...
//# sourceMappingURL=hopcode-settings-snapshot.test.js.map