...
function ExpandableNameCell({ row }) {
    ...
    return (_jsxs("div", { className: "flex items-center gap-1.5 p-1.5 pl-2.5", 
        ...
        style: { paddingLeft: `${row.depth * 16 + 10}px` }, children: [canExpand ? (_jsx("button", { type: "button", onClick: (e) => {
                    e.stopPropagation();
                    row.toggleExpanded();
                }, className: "p-0.5 rounded hover:bg-foreground/5 transition-colors", children: _jsx(ChevronRight, { className: cn('w-3 h-3 text-muted-foreground transition-transform duration-150', isExpanded && 'rotate-90') }) })) : (_jsx("span", { className: "w-4" })), _jsx("span", { className: "text-sm truncate", children: row.original.name })] }));
}
