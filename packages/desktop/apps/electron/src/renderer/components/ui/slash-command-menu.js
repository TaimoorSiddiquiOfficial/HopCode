...
const FALLBACK_HOPCODE_COMMANDS = [
...
export function isHopCodeSlashCommandId(commandId) {
...
export function createQwenSlashSections({ availableCommands = EMPTY_AVAILABLE_COMMANDS, availableSkills = EMPTY_AVAILABLE_SKILLS, enabled = true, }) {
    ...
    for (const command of sourceCommands) {
        ...
        if (!name || HIDDEN_HOPCODE_SLASH_NAMES.has(name) || seenNames.has(name))
            continue;
        seenNames.add(name);
        const inputHint = getCommandInputHint(command);
        commandItems.push({
            id: `${HOPCODE_COMMAND_ID_PREFIX}${name}`,
            label: `/${name}`,
    ...
    for (const skill of availableSkills) {
        ...
        seenNames.add(name);
        skillItems.push({
            id: `${HOPCODE_SKILL_ID_PREFIX}${name}`,
            label: `/${name}`,
