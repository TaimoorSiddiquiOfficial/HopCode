...
function CrossfadeAvatar({ src, alt, fallback, className, fallbackClassName, imageClassName, }) {
    ...
    return (_jsxs("div", { className: cn("relative flex shrink-0 overflow-hidden", className), children: [_jsx("div", { className: cn("absolute inset-0 flex items-center justify-center transition-opacity duration-200", isLoaded ? "opacity-0" : "opacity-100", fallbackClassName), children: fallback }), src && (isSvg ? (_jsx("div", { className: cn("w-full h-full transition-opacity duration-200", isLoaded ? "opacity-100" : "opacity-0", imageClassName), style: {
                    backgroundImage: `url("${src}")`,
                    backgroundSize: 'contain',
                    backgroundPosition: 'center',
                    backgroundRepeat: 'no-repeat',
                }, role: "img", "aria-label": alt, children: _jsx("img", { ref: imgCallbackRef, src: src, alt: "", onLoad: () => setIsLoaded(true), style: { display: 'none' } }) })) : (_jsx("img", { ref: imgCallbackRef, src: src, alt: alt, onLoad: () => setIsLoaded(true), className: cn("h-full w-full object-cover transition-opacity duration-200", isLoaded ? "opacity-100" : "opacity-0", imageClassName) }))), !src && (_jsx("div", { className: cn("flex h-full w-full items-center justify-center", fallbackClassName), children: fallback }))] }));
}
