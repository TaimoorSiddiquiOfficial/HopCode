...
describe('createQwenSlashSections', () => {
  it('uses provider-advertised commands when available', () => {
    ...
    const hopcodeSection = sections.find(section => section.id === 'qwen-commands')

    expect(hopcodeSection?.label).toBe('Commands')
    expect(hopcodeSection?.labelKey).toBe('commands.title')
    expect(hopcodeSection?.items.map(item => item.label)).toEqual(['/status', '/compress'])
    expect(hopcodeSection?.items[1]).toMatchObject({
      id: 'qwen:compress',
