...
const FALLBACK_HOPCODE_COMMANDS: AvailableSlashCommand[] = [
  ...
  { name: 'docs', description: 'Open HopCode documentation' },
...
export function isHopCodeSlashCommandId(commandId: SlashCommandId): commandId is HopCodeSlashCommandId | HopCodeSkillCommandId {
  return commandId.startsWith(HOPCODE_COMMAND_ID_PREFIX) || commandId.startsWith(HOPCODE_SKILL_ID_PREFIX)
}
...
export function createQwenSlashSections({
  ...
  const sourceCommands = availableCommands.length > 0 ? availableCommands : FALLBACK_HOPCODE_COMMANDS
  const seenNames = new Set<string>()
  const commandItems: SlashCommand[] = []

  for (const command of sourceCommands) {
    const name = normalizeQwenSlashName(command.name)
    if (!name || HIDDEN_HOPCODE_SLASH_NAMES.has(name) || seenNames.has(name)) continue
    seenNames.add(name)

    const inputHint = getCommandInputHint(command)
    commandItems.push({
      id: `${HOPCODE_COMMAND_ID_PREFIX}${name}` as HopCodeSlashCommandId,
      label: `/${name}`,
      description: command.description?.trim() || inputHint || 'HopCode command',
  ...
  for (const skill of availableSkills) {
    ...
    seenNames.add(name)
    skillItems.push({
      id: `${HOPCODE_SKILL_ID_PREFIX}${name}` as HopCodeSkillCommandId,
      label: `/${name}`,
  ...
  const hopcodeItems = [...commandItems, ...skillItems]
  const sections: SlashSection[] = []
  if (hopcodeItems.length > 0) {
    sections.push({
      id: 'qwen-commands',
      label: 'Commands',
