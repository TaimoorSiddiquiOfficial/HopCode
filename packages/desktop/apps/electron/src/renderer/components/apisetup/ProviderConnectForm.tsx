...
function defaultProtocol(provider: HopCodeProviderSummary): string {
  return provider.protocolOptions[0] || provider.protocol;
}

function defaultBaseUrl(provider: HopCodeProviderSummary): string {
...
function initialModelIds(provider: HopCodeProviderSummary): string[] {
...
export function ProviderConnectForm({
  ...
  const [catalog, setCatalog] = useState<HopCodeProviderCatalog | null>(null);
  ...
  const loadCatalog = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const result = await window.electronAPI.listHopCodeProviders();
      setCatalog(result);
    } catch (error) {
      setLoadError(
  ...
  const providersByGroup = useMemo(() => {
    const groups: Record<ProviderGroup, HopCodeProviderSummary[]> = {
      alibaba: [],
      'third-party': [],
      custom: [],
    };
  ...
  const selectProvider = useCallback((provider: HopCodeProviderSummary) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const params: HopCodeProviderConnectParams = {
      ...
      const result = await window.electronAPI.connectHopCodeProvider(params);
      if (!result.success) {
