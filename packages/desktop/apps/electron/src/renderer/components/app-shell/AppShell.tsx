...
function AppShellContent({
  ...
  const [hopcodeCapabilityCache, sethopcodeCapabilityCache] = React.useState<
    Record<string, HopCodeCapabilitySnapshot>
  >({})
  ...
  const shouldUseQwenAcpSkills =
    activeEffectiveConnection?.providerType === 'hopcode'
  const activehopcodeSessionId =
    shouldUseQwenAcpSkills
      ? activeSessionBelongsToActiveWorkspace
        ? (session.selected ?? null)
        : (firstActiveWorkspaceSessionMeta?.id ?? null)
      : null
  const activehopcodeCapabilityCacheKey = gethopcodeCapabilityCacheKey(
    activeWorkspaceId,
    activeSkillsWorkingDirectory,
    activeEffectiveConnectionSlug,
  )
  ...
  const getHopCodeCapabilitySnapshot = React.useCallback(
    (
      workspaceId?: string | null,
      workingDirectory?: string | null,
      connectionSlug?: string | null,
    ) => {
      const key = gethopcodeCapabilityCacheKey(
        workspaceId,
        workingDirectory,
        connectionSlug,
      )
      if (key && hopcodeCapabilityCache[key]) return hopcodeCapabilityCache[key]
      ...
      for (const fallbackDirectory of fallbackDirectories) {
        const fallbackKey = gethopcodeCapabilityCacheKey(
          workspaceId,
          fallbackDirectory,
          connectionSlug,
        )
    ...
    async (options?: { force?: boolean }) => {
      ...
      try {
        const loaded = await window.electronAPI.getSkills(
          workspaceId,
          activeSkillsWorkingDirectory,
          activehopcodeSessionId ?? undefined,
        )
                ...
                {isSkillMarketplaceNavigation(navState) &&
                  activeWorkspaceId && (
                    <SkillMarketplacePanel
                      ...
                      activeSessionId={activehopcodeSessionId}
                      selectedSkillId={
