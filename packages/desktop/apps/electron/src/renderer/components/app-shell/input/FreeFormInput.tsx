...
export function FreeFormInput({
  ...
  const hopcodeConnection = React.useMemo(
    () =>
      llmConnections.find((c) => c.providerType === 'hopcode') ??
      llmConnections[0],
    [llmConnections],
  );
  ...
  const hopcodeAcpMentionSkills = React.useMemo<LoadedSkill[]>(
    () =>
  ...
  const hopcodeCommandRefreshKeyRef = React.useRef<string | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (hopcodeCommandRefreshKeyRef.current === refreshKey) return;
    ...
    api
      ...
      .catch((error) => {
        api.debugLog?.(
          '[HOPCODE] Failed to refresh slash commands:',
          String(error),
        );
      });
  }, [
                        ...
                        {availableModels.map((model) => {
                          ...
                          return (
                            <StyledDropdownMenuItem
                              key={modelId}
                              onSelect={() =>
                                onModelChange(modelId, hopcodeConnection?.slug)
                              }
