...
import { gethopcodeCapabilityCacheKey, getWorkspaceSkillsCacheKey, providerSkillsFromQwenCapabilities, hopcodeCapabilitiesFromSkills, } from '@/lib/hopcode-capability-cache';
...
function FilterMenuRow({ icon, label, accessory, iconClassName, iconStyle, noIconContainer, }) {
    return (_jsxs(_Fragment, { children: [noIconContainer ? (_jsx("span", { style: iconStyle, children: React.isValidElement(icon)
...
function AppShellContent({ contextValue, defaultLayout = [20, 32, 48], defaultCollapsed = false, menuNewChatTrigger, isFocusedMode = false, isSessionListLoading = false, onProjectSessionSnapshotsReadyChange, }) {
    ...
    const [hopcodeCapabilityCache, sethopcodeCapabilityCache] = React.useState({});
    ...
    const shouldUseQwenAcpSkills = activeEffectiveConnection?.providerType === 'hopcode';
    const activehopcodeSessionId = shouldUseQwenAcpSkills
        ? activeSessionBelongsToActiveWorkspace
            ? (session.selected ?? null)
            : (firstActiveWorkspaceSessionMeta?.id ?? null)
        : null;
    const activehopcodeCapabilityCacheKey = gethopcodeCapabilityCacheKey(activeWorkspaceId, activeSkillsWorkingDirectory, activeEffectiveConnectionSlug);
    const activeHopCodeCapabilitySnapshot = activehopcodeCapabilityCacheKey
        ? hopcodeCapabilityCache[activehopcodeCapabilityCacheKey]
        : undefined;
    ...
    const getHopCodeCapabilitySnapshot = React.useCallback((workspaceId, workingDirectory, connectionSlug) => {
        const key = gethopcodeCapabilityCacheKey(workspaceId, workingDirectory, connectionSlug);
        if (key && hopcodeCapabilityCache[key])
            return hopcodeCapabilityCache[key];
        ...
        for (const fallbackDirectory of fallbackDirectories) {
            const fallbackKey = gethopcodeCapabilityCacheKey(workspaceId, fallbackDirectory, connectionSlug);
            if (fallbackKey && hopcodeCapabilityCache[fallbackKey]) {
                return hopcodeCapabilityCache[fallbackKey];
            }
        }
        return undefined;
    }, [
    ...
    React.useEffect(() => {
        if (!shouldUseQwenAcpSkills ||
            !activehopcodeCapabilityCacheKey ||
            !selectedSession)
            return;
        ...
        if (availableCommands.length === 0 &&
            ...
            return;
        sethopcodeCapabilityCache((prev) => {
            const current = prev[activehopcodeCapabilityCacheKey];
    ...
    }, [activehopcodeCapabilityCacheKey, selectedSession, shouldUseQwenAcpSkills]);
    ...
    React.useEffect(() => {
        const cleanup = window.electronAPI.onSkillsChanged((workspaceId, updatedSkills) => {
            ...
            sethopcodeCapabilityCache((prev) => {
                ...
                return {
                    [activehopcodeCapabilityCacheKey]: {
                        ...hopcodeCapabilitiesFromSkills(safeSkills),
    ...
    const reloadSkills = React.useCallback(async (options) => {
        ...
        if (shouldUseQwenAcpSkills &&
            !options?.force &&
            activeHopCodeCapabilitySnapshot) {
            setSkillsByScopeKey((prev) => ({
                ...prev,
                [scopeKey]: {
                    skills: activeHopCodeCapabilitySnapshot.skills,
                    status: 'ready',
                    requestId,
                },
            }));
        ...
        try {
            const loaded = await window.electronAPI.getSkills(workspaceId, activeSkillsWorkingDirectory, activehopcodeSessionId ?? undefined);
            if (shouldUseQwenAcpSkills && activehopcodeCapabilityCacheKey) {
                sethopcodeCapabilityCache((prev) => {
                    const current = prev[activehopcodeCapabilityCacheKey];
                    const skills = loaded || [];
                    ...
                    return {
                        ...prev,
                        [activehopcodeCapabilityCacheKey]: {
                            ...hopcodeCapabilitiesFromSkills(skills),
                            availableCommands: current?.availableCommands ?? [],
                        },
                    };
                });
    ...
    const handleDeleteSkill = useCallback(async (skillSlug) => {
        if (!activeWorkspace)
            return;
        try {
            await window.electronAPI.deleteSkill(activeWorkspace.id, skillSlug, activeSkillsWorkingDirectory, activehopcodeSessionId ?? undefined);
    ...
    const handleSetSkillEnabled = useCallback(async (skill, enabled) => {
        ...
        try {
            await window.electronAPI.setSkillEnabled(activeWorkspace.id, skill.slug, enabled, activeSkillsWorkingDirectory, activehopcodeSessionId ?? undefined, scope);
            await reloadSkills({ force: true });
        }
        catch (error) {
    ...
    return (_jsxs(AppShellProvider, { value: appShellContextValue, children: [_jsx(TopBar, { workspaces: workspaces, activeWorkspaceId: activeWorkspaceId, onSelectWorkspace: onSelectWorkspace, workspaceUnreadMap: workspaceUnreadMap, onWorkspaceCreated: () => onRefreshWorkspaces?.(), onWorkspaceRemoved: () => onRefreshWorkspaces?.(), onNewChat: () => handleNewChat(), onNewWindow: () => window.electronAPI.menuNewWindow(), onOpenSettings: onOpenSettings, onOpenSettingsSubpage: handleSettingsClick, onOpenKeyboardShortcuts: onOpenKeyboardShortcuts, onShowAbout: BRAND.creditsEntries.length > 0
                    ...
                    gap: PANEL_GAP,
                            ...
                            isSessionNavigatorCollapsed ? null : (_jsxs("div", { style: { width: isAutoCompact ? '100%' : sessionListWidth }, className: "h-full flex flex-col min-w-0 relative z-panel", children: [_jsx(PanelHeader, { title: isSidebarVisible ? listTitle : undefined, compensateForStoplight: !isSidebarVisible, badge: automationFilter?.automationType === 'scheduled' ? (_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("span", { className: "text-muted-foreground/50 cursor-default flex items-center titlebar-no-drag", children: _jsx(Info, { className: "h-3 w-3" }) }) }), _jsx(TooltipContent, { side: "bottom", className: "max-w-[220px]", children: "Scheduling requires your machine to be running. It can be locked, but must be powered on." })] })) : undefined, actions: _jsxs(_Fragment, { children: [isSessionsNavigation(navState) && (_jsxs(DropdownMenu, { onOpenChange: (open) => {
                                                    ...
                                                    : 'add-source', activeWorkspace.rootPath) })), isAutomationsNavigation(navState) && activeWorkspace && (_jsx(EditPopover, { trigger: _jsx(HeaderIconButton, { icon: _jsx(Plus, { className: "h-4 w-4" }), tooltip: t('sidebarMenu.addAutomation') }), ...getEditConfig('automation-config', activeWorkspace.rootPath) }))] }) }), isSourcesNavigation(navState) && (_jsx(SourcesListPanel, { sources: sources, sourceFilter: sourceFilter, workspaceRootPath: activeWorkspace?.rootPath, onDeleteSource: handleDeleteSource, onSourceClick: handleSourceSelect, selectedSourceSlug: isSourcesNavigation(navState) && navState.details
                                        ? navState.details.sourceSlug
                                        : null, localMcpEnabled: localMcpEnabled })), isSkillsNavigation(navState) && activeWorkspaceId && (_jsx(SkillsListPanel, { skills: skills, workspaceId: activeWorkspaceId, workspaceRootPath: activeWorkspace?.rootPath, isLoading: skillsLoading, onSkillClick: handleSkillSelect, onDeleteSkill: handleDeleteSkill, onSetSkillEnabled: handleSetSkillEnabled, selectedSkillSlug: isSkillsNavigation(navState) &&
                                        navState.details?.type === 'skill'
                                        ? navState.details.skillSlug
                                        : null })), isSkillMarketplaceNavigation(navState) &&
                                    activeWorkspaceId && (_jsx(SkillMarketplacePanel, { workspaceId: activeWorkspaceId, workingDirectory: activeSkillsWorkingDirectory, activeSessionId: activehopcodeSessionId, selectedSkillId: navState.details?.type === 'marketplaceSkill'
                                        ? navState.details.skillId
                                        : null, onSkillSelect: handleMarketplaceSkillSelect, onInstalled: reloadSkills, installingSkillIds: installingMarketplaceSkillIds, onInstallStart: handleMarketplaceSkillInstallStart, onInstallFinish: handleMarketplaceSkillInstallFinish })), isAutomationsNavigation(navState) && (_jsx(AutomationsListPanel, { automations: automations, automationFilter: automationFilter
                                        ? {
                                        ...
                                        : null, workspaceRootPath: activeWorkspace?.rootPath })), isSettingsNavigation(navState) && (_jsx(SettingsNavigator, { selectedSubpage: navState.subpage, onSelectSubpage: (subpage) => handleSettingsClick(subpage) })), isSessionsNavigation(navState) && (_jsx(_Fragment, { children: _jsx(SessionList, { items: searchActive
                                            ? workspaceSessionMetas
