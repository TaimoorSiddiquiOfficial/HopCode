...
export default function DraftChatPage() {
  ...
  const HopCodeCapabilitySnapshot = getHopCodeCapabilitySnapshot?.(
    activeWorkspaceId,
    workingDirectory,
    effectiveConnectionSlug,
  );
  const fallbackHopCodeCapabilitySnapshot = React.useMemo(
    () =>
      effectiveConnection?.providerType === 'hopcode'
        ? hopcodeCapabilitiesFromSkills(skills ?? [])
        : undefined,
    [effectiveConnection?.providerType, skills],
  );
  const draftHopCodeCapabilitySnapshot =
    HopCodeCapabilitySnapshot ?? fallbackHopCodeCapabilitySnapshot;
      ...
      ...(draftHopCodeCapabilitySnapshot
        ? {
            ...
            availableSkillDetails:
              draftHopCodeCapabilitySnapshot.availableSkillDetails,
          }
        : {}),
    }),
    [
