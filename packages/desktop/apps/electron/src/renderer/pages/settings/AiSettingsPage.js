...
export default function AiSettingsPage() {
    ...
    const hopcodeConnection = useMemo(() => llmConnections.find((connection) => connection.providerType === 'hopcode') ??
        llmConnections[0], [llmConnections]);
    const modelOptions = useMemo(() => getModelOptionsForConnection(hopcodeConnection).map((option) => ({
        ...option,
        description: option.descriptionKey
            ? t(option.descriptionKey)
            : option.description,
    })), [hopcodeConnection, t]);
    const defaultModel = hopcodeConnection?.defaultModel || modelOptions[0]?.value || '';
    const modelCount = modelOptions.length;
    const providerConnectionLabel = hopcodeConnection?.providerType === 'hopcode'
        ? t('settings.ai.providerConnectionName')
        : hopcodeConnection?.name || t('settings.ai.providerConnectionName');
    const handleDefaultModelChange = useCallback(async (model) => {
        if (!window.electronAPI || !hopcodeConnection)
            return;
    ...
    }, [hopcodeConnection, refreshLlmConnections]);
