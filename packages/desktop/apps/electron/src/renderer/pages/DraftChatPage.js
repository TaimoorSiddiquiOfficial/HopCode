...
export default function DraftChatPage() {
    ...
    const { activeWorkspaceId, workspaces, llmConnections, workspaceDefaultLlmConnection, onOptimisticDefaultModelChange, onCreateSession, onSendMessage, onOpenFile, onOpenUrl, enabledSources, skills, getHopCodeCapabilitySnapshot, labels, enabledModes, globalPermissionMode, onSessionOptionsChange, leadingAction, isCompactMode, } = useAppShellContext();
    ...
    const fallbackHopCodeCapabilitySnapshot = React.useMemo(() => effectiveConnection?.providerType === 'hopcode'
        ? hopcodeCapabilitiesFromSkills(skills ?? [])
        : undefined, [effectiveConnection?.providerType, skills]);
    const draftHopCodeCapabilitySnapshot = HopCodeCapabilitySnapshot ?? fallbackHopCodeCapabilitySnapshot;
    ...
    const draftSession = React.useMemo(() => ({
        ...
        ...(draftHopCodeCapabilitySnapshot
            ? {
                availableCommands: draftHopCodeCapabilitySnapshot.availableCommands,
                availableSkills: draftHopCodeCapabilitySnapshot.availableSkills ?? [],
                availableSkillDetails: draftHopCodeCapabilitySnapshot.availableSkillDetails,
            }
            : {}),
    }), [
        activeWorkspaceId,
