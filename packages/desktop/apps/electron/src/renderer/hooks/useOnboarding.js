...
export function useOnboarding({ onComplete, initialSetupNeeds, initialStep = 'provider-select', initialApiSetupMethod, onDismiss, onConfigSaved, editingSlug = null, existingSlugs = new Set(), }) {
    ...
    const savehopcodeConnection = useCallback(async (connectionSlugOverride) => {
        setState(s => ({ ...s, completionStatus: 'saving' }));
        const setup = apiSetupMethodToConnectionSetup('HOPCODE_code', {}, connectionSlugOverride ?? editingSlug, existingSlugs);
        ...
        if (!result.success) {
            setState(s => ({
                ...
                completionStatus: 'saving',
                errorMessage: result.error || 'Failed to save HopCode configuration',
            }));
            return false;
        }
    ...
    const handleSubmitCredential = useCallback(async (_data) => {
        setState(s => ({ ...s, apiSetupMethod: 'HOPCODE_code', credentialStatus: 'validating', errorMessage: undefined }));
        try {
            ...
            if (!testResult.success) {
                setState(s => ({
                    ...s,
                    credentialStatus: 'error',
                    errorMessage: testResult.error || 'HopCode connection test failed',
                }));
                return;
            }
            const saved = await savehopcodeConnection();
            ...
            setState(s => ({
                ...s,
                credentialStatus: 'error',
                errorMessage: error instanceof Error ? error.message : 'HopCode validation failed',
            }));
        }
    }, [savehopcodeConnection]);
    const handleSelectProvider = useCallback((_choice) => {
        setState(s => ({
            ...s,
            apiSetupMethod: 'HOPCODE_code',
            step: 'credentials',
            credentialStatus: 'idle',
