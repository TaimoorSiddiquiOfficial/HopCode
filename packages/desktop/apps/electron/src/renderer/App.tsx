...
export default function App() {
  ...
  const hopcodeModelRefreshInFlightRef = useRef<Promise<void> | null>(null)
  const hopcodeModelRefreshAttemptedRef = useRef<Set<string>>(new Set())
  const hopcodeModelRefreshCompletedRef = useRef<Set<string>>(new Set())
  ...
  const refreshLlmConnections = useCallback(async () => {
    ...
    const visibleConnections = connections.map(connection => {
      if (connection.providerType !== 'hopcode') return connection
    ...
    const hopcodeConnectionToRefresh = connections.find(connection =>
