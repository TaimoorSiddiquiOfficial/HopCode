...
export function parseArgs(argv: string[]): CliArgs {
  ...
  if (!provider) provider = process.env.LLM_PROVIDER ?? 'hopcode'
...
function getProviderDisplayName(provider: string): string {
  return provider === 'hopcode' ? 'HopCode' : provider.charAt(0).toUpperCase() + provider.slice(1)
}
...
async function setupLlmConnection(
  ...
  const provider = 'hopcode'
  const connectionSlug = 'hopcode'
  ...
  process.stderr.write('LLM connection configured: HopCode\n')
...
async function cmdRun(args: CliArgs): Promise<void> {
  ...
  try {
    ...
    // Auto-setup the HopCode connection when no connection exists yet.
  ...
  return [
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        const provider = ctx.provider || 'hopcode'
        ...
        return '0 found -> created HopCode connection'
      },
    },
    {
      name: 'sources:get',
...
function printHelp(): void {
  process.stdout.write(`craft-cli — Terminal client for HopCode server
...
LLM Configuration (for 'run' command):
  ...
  --api-key <key>        Ignored for HopCode; kept for script compatibility
  --base-url <url>       Ignored for HopCode; kept for script compatibility
...
Examples:
  ...
  craft-cli run --provider hopcode --model qwen3-coder "Summarize this repo"
