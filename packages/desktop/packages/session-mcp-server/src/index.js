...
async function connectDocsUpstream() {
    try {
        ...
        docsClient = client;
        console.error(`HopCode Docs proxy connected: ${docsTools.length} tools`);
    }
    catch (err) {
        console.error(`HopCode Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
        docsClient = null;
        docsTools = [];
    }
}
...
async function callDocsUpstream(name, args) {
    if (!docsClient) {
        return (0, session_tools_core_1.errorResponse)(`HopCode Docs server is not connected. Tool '${name}' unavailable.`);
    }
    try {
