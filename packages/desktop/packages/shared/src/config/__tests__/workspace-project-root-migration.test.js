...
function setupProjectDir(name = 'hopcode') {
...
describe('project-root workspace storage', () => {
    it('stores new workspaces for existing projects under managed config storage', () => {
        ...
        const output = runEval(configDir, `const ws = addWorkspace({ name: 'hopcode', rootPath: ${JSON.stringify(projectDir)} }); console.log(JSON.stringify(ws));`);
    ...
    it('migrates legacy external project workspaces without writing more project files', () => {
        ...
        writeFileSync(join(configDir, 'config.json'), JSON.stringify({
            workspaces: [{ id: 'ws-1', name: 'hopcode', slug: 'hopcode', rootPath: projectDir, createdAt: 1 }],
        ...
        expect(workspace.rootPath).toBe(join(configDir, 'workspaces', 'hopcode'));
        expect(existsSync(join(projectDir, '.agents-plugin', 'plugin.json'))).toBe(false);
        const managedRoot = join(configDir, 'workspaces', 'hopcode');
