...
import { HOPCODE_CODE_CONNECTION_SLUG, type LlmProviderType } from './llm-connections.ts';
...
export interface StoredConfig {
  ...
  // Pet companion
  selectedPetId?: string;  // ID of the selected pet companion (default: 'hopcode')
...
export const DEFAULT_CONVERSATION_WORKSPACE_NAME = 'hopcode';
export const DEFAULT_CONVERSATION_WORKSPACE_KIND = 'conversation' as const;
export const DEFAULT_CONVERSATION_WORKSPACE_PATH_ENV = 'HOPCODE_DEFAULT_WORKSPACE_DIR';
...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for HopCode',
  defaults: {
...
/**
 * Get the selected pet companion id. Defaults to 'hopcode' if not set.
 */
export function getSelectedPetId(): string {
  const config = loadStoredConfig();
  return config?.selectedPetId ?? 'hopcode';
}
...
function stripQwenRuntimeModelFields(connection: LlmConnection): void {
  if (connection.providerType !== 'hopcode') return;
...
function createHopCodeConnection(existing?: Partial<LlmConnection>): LlmConnection {
  return {
...
function normalizeToHopCodeOnly(target: StoredConfig): boolean {
  const previousConnections = target.llmConnections ?? [];
  const existingQwen = previousConnections.find(c => c.slug === HOPCODE_CODE_CONNECTION_SLUG)
    ?? previousConnections.find(c => c.providerType === 'hopcode');
  const hopcodeConnection = createHopCodeConnection(existingQwen);
  const removedConnectionSlugs = previousConnections
    .filter(c => c.slug !== HOPCODE_CODE_CONNECTION_SLUG && c.providerType !== 'hopcode')
    .map(c => c.slug);

  let changed = previousConnections.length !== 1
    || JSON.stringify(previousConnections[0] ?? null) !== JSON.stringify(hopcodeConnection)
    || target.defaultLlmConnection !== HOPCODE_CODE_CONNECTION_SLUG;

  target.llmConnections = [hopcodeConnection];
  target.defaultLlmConnection = HOPCODE_CODE_CONNECTION_SLUG;
...
/**
 * Collapse any legacy provider configuration to the single built-in HopCode backend.
...
export function migrateLegacyLlmConnectionsConfig(): void {
  ...
  if (normalizeToHopCodeOnly(config)) {
    needsSave = true;
  }
 ...
 *
 * HopCode uses the local backend runtime, so there are no app-managed
