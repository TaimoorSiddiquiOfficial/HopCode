...
export const GLOBAL_HOPCODE_SKILLS_DIR = join(homedir(), '.hopcode', 'skills');
...
export const PROJECT_HOPCODE_SKILLS_DIR = '.hopcode/skills';
/**
...
export function loadAllSkills(workspaceRoot, projectRoot) {
    ...
    const skillsBySlug = new Map();
    // 1. Global skills (lowest priority): ~/.hopcode/skills/ then ~/.agents/skills/
    for (const skill of loadSkillsFromDirs([GLOBAL_HOPCODE_SKILLS_DIR, GLOBAL_AGENT_SKILLS_DIR], 'global')) {
        skillsBySlug.set(skill.slug, skill);
    }
    ...
    // 3. Project skills (highest priority): {projectRoot}/.hopcode/skills/ then .agents/skills/
    if (projectRoot) {
        const projectSkillsDirs = [
            join(projectRoot, PROJECT_HOPCODE_SKILLS_DIR),
...
export function loadSkillBySlug(workspaceRoot, slug, projectRoot) {
    ...
    return (loadSkillFromDir(GLOBAL_HOPCODE_SKILLS_DIR, slug, 'global') ??
