 ...
 * 1. Global skills: ~/.hopcode/skills/ and ~/.agents/skills/ (lowest priority)
 * 2. Workspace skills: {workspaceRoot}/skills/ (medium priority)
 * 3. Project skills: {projectRoot}/.hopcode/skills/ and .agents/skills/ (highest priority)
 *
 * Uses real temp directories to test actual filesystem operations.
 *
 * Note: The global skills directories (~/.hopcode/skills/ and ~/.agents/skills/) are module-level constants
...
describe('loadAllSkills', () => {
  ...
  it('should load hopcode project skills without requiring a Craft workspace skills directory', () => {
    ...
    createSkill(hopcodeProjDir, `${TEST_PREFIX}HOPCODE_proj`, {
      ...
      description: 'From .hopcode skills',
    });

    const skills = loadAllSkills(workspaceRoot, projectRoot);
    const skill = skills.find(s => s.slug === `${TEST_PREFIX}HOPCODE_proj`);

    expect(skills.length).toBe(baselineGlobal.size + 1);
    expect(skill).toBeDefined();
    expect(skill!.source).toBe('project');
    expect(skill!.path).toBe(join(hopcodeProjDir, `${TEST_PREFIX}HOPCODE_proj`));
  });
  ...
  it('should prefer .hopcode project skills over .agents project skills with the same slug', () => {
    ...
    createSkill(hopcodeProjDir, `${TEST_PREFIX}provider_dup`, {
      name: 'Qwen Provider Skill',
      description: 'From .hopcode',
    });
    ...
    expect(skill!.path).toBe(join(hopcodeProjDir, `${TEST_PREFIX}provider_dup`));
  });
