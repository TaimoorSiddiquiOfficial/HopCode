...
export type SessionCommand = {
    ...
    type: 'getHopCodePermissionSettings';
} | {
...
export interface HopCodePermissionSettings {
...
export type HopCodeSettingValue = string | number | boolean | string[] | undefined;
export type HopCodeMcpTransport = 'stdio' | 'http' | 'sse';
export type HopCodeCoreSettingKey = 'model.name' | 'fastModel' | 'general.outputLanguage' | 'general.language' | 'tools.approvalMode' | 'general.vimMode' | 'general.enableAutoUpdate' | 'general.showSessionRecap' | 'general.sessionRecapAwayThresholdMinutes' | 'general.terminalBell' | 'general.gitCoAuthor.commit' | 'general.gitCoAuthor.pr' | 'general.defaultFileEncoding' | 'context.fileFiltering.respectGitIgnore' | 'context.fileFiltering.respectHopcodeIgnore' | 'context.fileFiltering.enableFuzzySearch' | 'memory.enableManagedAutoMemory' | 'memory.enableManagedAutoDream' | 'memory.enableAutoSkill' | 'memory.autoSkillConfirm' | 'disableAllHooks';
...
export interface HopCodeMcpServerEntry {
    name: string;
    scope: HopCodeSettingsScope | 'extension';
    server: HopCodeMcpServerConfig;
}
export type HopCodeHookEvent = 'UserPromptSubmit' | 'Stop' | 'Notification' | 'PreToolUse' | 'PostToolUse' | 'PostToolUseFailure' | 'SessionStart' | 'SessionEnd' | 'PreCompact' | 'SubagentStart' | 'SubagentStop' | 'PermissionRequest';
export interface HopCodeHookConfig {
    type: 'command' | 'http';
    command?: string;
    url?: string;
    headers?: Record<string, string>;
...
export interface HopCodeHookDefinition {
    matcher?: string;
    sequential?: boolean;
    hooks: HopCodeHookConfig[];
}
export interface HopCodeHookEntry {
    event: HopCodeHookEvent;
    index: number;
    scope: HopCodeSettingsScope | 'extension';
    hook: HopCodeHookDefinition;
}
export interface HopCodeExtensionSettingDefinition {
    type: 'string' | 'number' | 'boolean';
    default?: HopCodeSettingValue;
    description?: string;
    sensitive?: boolean;
}
export interface HopCodeExtensionSettingsEntry {
    ...
    skills: string[];
    mcpServers: string[];
    settings: Array<HopCodeExtensionSettingDefinition & {
        name: string;
        envVar: string;
        userValue?: HopCodeSettingValue;
        workspaceValue?: HopCodeSettingValue;
        effectiveValue?: HopCodeSettingValue;
        effectiveScope?: HopCodeSettingsScope;
        hasUserValue: boolean;
        hasWorkspaceValue: boolean;
    }>;
}
export interface HopCodeCoreSettingsScopeState {
    path: string;
    values: Partial<Record<HopCodeCoreSettingKey, HopCodeSettingValue>>;
    mcpServers: HopCodeMcpServerEntry[];
    hooks: HopCodeHookEntry[];
}
export interface HopCodeCoreSettingsSnapshot {
    user: HopCodeCoreSettingsScopeState;
    workspace: HopCodeCoreSettingsScopeState;
    merged: {
        values: Partial<Record<HopCodeCoreSettingKey, HopCodeSettingValue>>;
        mcpServers: HopCodeMcpServerEntry[];
        hooks: HopCodeHookEntry[];
        extensions: HopCodeExtensionSettingsEntry[];
    };
...
export interface HopCodeProviderBaseUrlOption {
    id: string;
    label: string;
    url: string;
...
export interface HopCodeProviderModelSpec {
    id: string;
    contextWindowSize?: number;
    enableThinking?: boolean;
    description?: string;
}
export interface HopCodeProviderSummary {
    id: string;
    label: string;
    description: string;
    protocol: string;
    protocolOptions: string[];
    baseUrl?: string | HopCodeProviderBaseUrlOption[];
    baseUrlPlaceholder?: string;
    defaultModelIds: string[];
    models: HopCodeProviderModelSpec[];
    ...
    existingConfig?: {
        ...
        apiKey?: string;
        modelIds?: string[];
        advancedConfig?: HopCodeProviderAdvancedConfig;
    };
}
export interface HopCodeProviderCatalog {
    providers: HopCodeProviderSummary[];
}
export interface HopCodeProviderAdvancedConfig {
    enableThinking?: boolean;
    multimodal?: {
        image?: boolean;
        video?: boolean;
...
export interface HopCodeProviderConnectParams {
    providerId: string;
    protocol?: string;
    baseUrl?: string;
    apiKey: string;
...
export interface HopCodeProviderConnectResult {
    success: boolean;
    error?: string;
    providerId?: string;
...
export interface HopCodeSkillInstallRequest {
    id: string;
    slug: string;
    name: string;
    description?: string;
...
export interface HopCodeSkillInstallResult {
    id?: string;
    slug?: string;
    installed?: boolean;
    installedPath?: string;
...
export interface HopCodeSkillDeleteRequest {
    slug: string;
    scope?: 'global';
}
export interface HopCodeSkillDeleteResult {
    slug?: string;
    deleted?: boolean;
    message?: string;
}
export interface HopCodeSkillSetEnabledRequest {
    slug: string;
    enabled: boolean;
    scope?: 'global' | 'project';
}
export interface HopCodeSkillSetEnabledResult {
    slug?: string;
    enabled?: boolean;
    installedPath?: string;
    message?: string;
