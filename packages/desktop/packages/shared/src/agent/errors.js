...
export function parseError(error, providerContext) {
    ...
    const providerInfo = providerContext
        ? getProviderMetadata(providerContext.providerType ?? 'hopcode') ?? undefined
        : undefined;
    ...
    if (code === 'model_no_tool_support') {
        ...
        if (modelMatch?.[1]) {
            return {
                code,
                ...definition,
                message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for HopCode. Please choose a different model with tool support in Settings.`,
                originalError: errorMessage,
                providerInfo,
            };
        }
