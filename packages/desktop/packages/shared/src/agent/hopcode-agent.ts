...
import type {
  ...
  HopCodePermissionSettings,
  HopCodeProviderCatalog,
  HopCodeProviderConnectParams,
  HopCodeProviderConnectResult,
  HopCodeSettingValue,
  HopCodeSettingsScope,
  HopCodeSkillDeleteRequest,
  HopCodeSkillDeleteResult,
  HopCodeSkillInstallRequest,
  HopCodeSkillInstallResult,
  HopCodeSkillSetEnabledRequest,
  HopCodeSkillSetEnabledResult,
...
class SharedHopCodeAcpProcess {
  ...
  private subscribers = new Set<HopCodeAcpSubscriber>();
  private sessionOwners = new Map<string, HopCodeAcpSubscriber>();
  private refCount = 0;
  ...
  constructor(private readonly options: HopCodeAcpProcessOptions) {
    this.commandDescription = `${options.command} ${options.args.join(' ')}`;
  }

  async acquire(subscriber: HopCodeAcpSubscriber): Promise<HopCodeAcpLease> {
    this.refCount += 1;
    if (this.idleTimer) {
  ...
  private releaseSubscriber(subscriber: HopCodeAcpSubscriber): void {
...
function hopcodeSettingsCwd(hostRuntime: BackendHostRuntimeContext): string {
  return hostRuntime.appRootPath || homedir() || process.cwd();
}

function hopcodeAcpWithTimeout<T>(
  promise: Promise<T>,
  method: string,
  timeoutMs: number,
): Promise<T> {
...
async function callHopCodeSettingsAcpMethod(
  ...
  if (!hopcodeCliPath) {
    throw new Error(
      'HopCode CLI not found. Build the current hopcode checkout with npm run build && npm run bundle, or set HOPCODE_CODE_CLI to a dist/cli.js path.',
    );
  }
...
export async function setHopCodeMemorySettingsViaAcp(
  options: HopCodeSettingsAcpOptions,
...
export async function getHopCodePermissionSettingsViaAcp(
  options: HopCodeSettingsAcpOptions,
...
export async function getHopCodeMemoryPathsViaAcp(
...
export async function listHopCodeProvidersViaAcp(
  options: HopCodeSettingsAcpOptions,
...
function normalizeHopCodeProviderCatalog(
  response: JsonRecord,
): HopCodeProviderCatalog {
  return {
    providers: Array.isArray(response.providers)
      ? (response.providers as HopCodeProviderCatalog['providers'])
      : [],
  };
}

export async function connectHopCodeProviderViaAcp(
  options: HopCodeSettingsAcpOptions,
...
function normalizeHopCodeProviderConnectResult(
  response: JsonRecord,
): HopCodeProviderConnectResult {
...
function hopcodeSkillNameFromTextElement(
...
function hopcodeTranscriptPlaceholderFromSourceElement(
...
function buildHopCodeTranscriptTextElements(
  ...
  for (const sourceElement of sourceElements ?? []) {
    const placeholder =
      hopcodeTranscriptPlaceholderFromSourceElement(sourceElement);
...
function hopcodeFunctionResponseText(response: JsonRecord): string | undefined {
...
function hopcodeInitializeTimeoutMs(): number {
  const raw =
    process.env.HOPCODE_ACP_INITIALIZE_TIMEOUT_MS ||
...
export class HopCodeAgent extends BaseAgent {
  ...
  private acpLease: HopCodeAcpLease | null = null;
  private connection: ClientSideConnection | null = null;
  ...
  async getPermissionSettings(): Promise<HopCodePermissionSettings> {
    await this.ensureProcess();
    const result = await this.callAcp(
      'hopcode/permissions/getSettings',
      (connection) =>
        connection.extMethod('hopcode/permissions/getSettings', {
          cwd: this.resolvedCwd(),
        }),
      10_000,
    );
    return result as unknown as HopCodePermissionSettings;
  }
  ...
  async setPermissionRules(
    ...
    const result = await this.callAcp(
      'hopcode/permissions/setRules',
      (connection) =>
        connection.extMethod('hopcode/permissions/setRules', {
          cwd: this.resolvedCwd(),
          scope,
          ruleType,
          rules,
  ...
  async getCoreSettings(): Promise<HopCodeCoreSettingsSnapshot> {
    await this.ensureProcess();
    const result = await this.callAcp(
      'hopcode/settings/getCore',
      (connection) =>
        connection.extMethod('hopcode/settings/getCore', {
          cwd: this.resolvedCwd(),
        }),
      10_000,
    );
    return result as unknown as HopCodeCoreSettingsSnapshot;
  }

  async listProviders(): Promise<HopCodeProviderCatalog> {
    await this.ensureProcess();
    const result = await this.callAcp(
      'hopcode/providers/list',
      (connection) =>
        connection.extMethod('hopcode/providers/list', {
          cwd: this.resolvedCwd(),
        }),
      10_000,
    );
  ...
  async connectProvider(
    ...
    const result = await this.callAcp(
      'hopcode/providers/connect',
      (connection) =>
        connection.extMethod('hopcode/providers/connect', {
          cwd: this.resolvedCwd(),
          ...params,
  ...
  async setCoreSetting(
    ...
    const result = await this.callAcp(
      'hopcode/settings/setCoreValue',
      (connection) =>
        connection.extMethod('hopcode/settings/setCoreValue', {
          cwd: this.resolvedCwd(),
          scope,
          key,
          value,
  ...
  async setMcpServer(
    ...
    const result = await this.callAcp(
      'hopcode/settings/setMcpServer',
      (connection) =>
        connection.extMethod('hopcode/settings/setMcpServer', {
          cwd: this.resolvedCwd(),
          scope,
          name,
          server,
  ...
  async removeMcpServer(
    ...
    const result = await this.callAcp(
      'hopcode/settings/removeMcpServer',
      (connection) =>
        connection.extMethod('hopcode/settings/removeMcpServer', {
          cwd: this.resolvedCwd(),
          scope,
          name,
        }),
  ...
  async setHook(
    ...
    const result = await this.callAcp(
      'hopcode/settings/setHook',
      (connection) =>
        connection.extMethod('hopcode/settings/setHook', {
          cwd: this.resolvedCwd(),
          scope,
          event,
          index,
  ...
  async removeHook(
    ...
    const result = await this.callAcp(
      'hopcode/settings/removeHook',
      (connection) =>
        connection.extMethod('hopcode/settings/removeHook', {
          cwd: this.resolvedCwd(),
          scope,
          event,
          index,
  ...
  async setExtensionSetting(
    ...
    const result = await this.callAcp(
      'hopcode/settings/setExtensionSetting',
      (connection) =>
        connection.extMethod('hopcode/settings/setExtensionSetting', {
          cwd: this.resolvedCwd(),
          extensionId,
          settingKey,
          scope,
  ...
  ): Promise<HopCodeSkillInstallResult> {
    await this.ensureProcess();
    this.invalidateAvailableCommandsSnapshot('skill install');
    const result = await this.callAcp(
      'hopcode/skills/install',
      (connection) =>
        connection.extMethod('hopcode/skills/install', {
          cwd: this.resolvedCwd(),
          skill: {
            ...request,
  ...
  ): Promise<HopCodeSkillDeleteResult> {
    await this.ensureProcess();
    this.invalidateAvailableCommandsSnapshot('skill delete');
    const result = await this.callAcp(
      'hopcode/skills/delete',
      (connection) =>
        connection.extMethod('hopcode/skills/delete', {
          cwd: this.resolvedCwd(),
          skill: {
            ...request,
            scope: request.scope ?? 'global',
  ...
  ): Promise<HopCodeSkillSetEnabledResult> {
    await this.ensureProcess();
    this.invalidateAvailableCommandsSnapshot('skill enabled state change');
    const result = await this.callAcp(
      'hopcode/skills/setEnabled',
      (connection) =>
        connection.extMethod('hopcode/skills/setEnabled', {
          cwd: this.resolvedCwd(),
          skill: {
            ...request,
            scope: request.scope ?? 'global',
  ...
  async loadSessionMessages(
    ...
    try {
      const response = toRecord(
        await this.callAcp(
          'ext/hopcode/session/loadUpdates',
          (connection) =>
            connection.extMethod('hopcode/session/loadUpdates', {
              sessionId,
              cwd,
            }),
          30_000,
  ...
  private async forwardModel(
    model: string,
    sessionId = this.hopcodeSessionId,
  ...
  private handleSessionUpdate(params: unknown): void {
    ...
    if (sessionId !== this.hopcodeSessionId || !this._isProcessing) return;
  ...
  private extractHopCodeTranscriptToolResult(record: JsonRecord):
    ...
    for (const part of parts) {
      ...
      const text =
        asString(result.resultDisplay) || hopcodeFunctionResponseText(response);
