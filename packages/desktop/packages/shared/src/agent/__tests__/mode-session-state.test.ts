...
describe('mode transition session_state context', () => {
  it('includes explicit transition metadata after a mode change', () => {
    ...
    expect(diagnostics.transitionDisplay).toBe('Plan mode -> IZN');
    ...
    expect(stateBlock).toContain('modeTransition: Plan mode -> IZN');
    ...
    expect(stateBlock).toContain('modeChangeSummary: Last mode change by user at 2026-03-02T08:00:00.000Z (Plan mode -> IZN, modeVersion=2)');
