...
async function readNextQueuedEvent(agent: HopCodeAgent): Promise<unknown> {
  const queue = (agent as unknown as HopCodeModelInternals).eventQueue;
...
describe('HopCodeAgent model metadata', () => {
  ...
  it('uses ACP-reported context and thinking metadata without a hardcoded context fallback', () => {
    ...
    (agent as unknown as HopCodeModelInternals).recordSessionModels({
      models: {
  ...
  it('uses ACP total tokens for context usage without double-counting cached tokens', () => {
    ...
    const usage = (agent as unknown as HopCodeModelInternals).extractUsage({
      _meta: {
        usage: {
          inputTokens: 100,
          outputTokens: 50,
  ...
  it('applies configured session models through ACP session/set_model', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'qwen-cwd-'));
    const agent = createAgent(cwd, () => {});
    const internals = agent as unknown as HopCodeModelInternals;
  ...
  it('extracts latest replay usage for loaded hopcode native history', () => {
    ...
    const tokenUsage = (
      agent as unknown as HopCodeModelInternals
