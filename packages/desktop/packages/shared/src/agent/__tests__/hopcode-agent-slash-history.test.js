...
function writeHopCodeTranscript(runtimeRoot, cwd, sessionId, records) {
...
function readHopCodeTranscript(runtimeRoot, cwd, sessionId) {
    const projectId = resolve(cwd).replace(/[^a-zA-Z0-9]/g, '-');
...
describe('HopCodeAgent slash command history', () => {
    const tempRoots = [];
    afterEach(() => {
    ...
    it('sends slash commands as raw ACP prompts', () => {
        const blocks = HopCodeAgent.prototype.buildPromptBlocks('  /context  ');
        expect(blocks).toEqual([{ type: 'text', text: '/context' }]);
    });
    it('starts HopCode ACP with the desktop channel', () => {
        const command = HopCodeAgent.prototype.buildSpawnCommand('/opt/hopcode/dist/cli.js', '/usr/local/bin/node');
        expect(command).toEqual({
            command: '/usr/local/bin/node',
            args: ['/opt/hopcode/dist/cli.js', '--acp', '--channel=desktop'],
        });
    });
    it('does not prepend Craft context to HopCode prompts while disabled', () => {
        const blocks = HopCodeAgent.prototype.buildPromptBlocks('hello');
    ...
    it('logs attachments skipped while building prompt blocks', () => {
        ...
        expect(blocks).toEqual([{ type: 'text', text: 'hello' }]);
        expect(debugMessages).toContain('[HopCodeAgent] Skipping attachment empty.bin while building prompt blocks: no readable content');
        agent.destroy();
    });
    ...
    it('writes slash command text elements into the HopCode transcript user record', () => {
        ...
        const agent = createAgent(cwd);
        agent.persistHopCodeTranscriptTextElements(sessionId, cwd, [
            {
                type: 'slash_command',
    ...
    it('loads text elements back from the HopCode transcript', () => {
        ...
        const agent = createAgent(cwd);
        const messages = agent.applyHopCodeTranscriptTextElements([
            {
                id: 'message-1',
                role: 'user',
                content: '@qc-helper',
    ...
    it('buffers ACP available command updates until the HopCode Session id is recorded', async () => {
        ...
        internals.flushPendingAvailableCommandsUpdate('hopcode-session');
    ...
    it('preserves ACP available command updates emitted during suppressed session load', async () => {
        ...
        internals.hopcodeSessionId = 'hopcode-session';
        internals._isProcessing = true;
        internals.suppressedSessionUpdates.add('hopcode-session');
        internals.handleSessionUpdate({
            sessionId: 'hopcode-session',
            update: {
                sessionUpdate: 'available_commands_update',
        ...
        expect(internals.eventQueue.hasPending).toBe(false);
        internals.suppressedSessionUpdates.delete('hopcode-session');
        internals.flushPendingAvailableCommandsUpdate('hopcode-session');
        const event = await readNextQueuedEvent(agent);
        agent.destroy();
        expect(event).toEqual({
    ...
    it('invalidates cached available commands after installing a skill', async () => {
        ...
        internals.callAcp = async (method, execute) => {
            calledMethods.push(method);
            if (method === 'hopcode/skills/install') {
                return execute({
                    extMethod: async () => ({
                        slug: 'pptx',
                        installed: true,
        ...
        const snapshot = await agent.refreshAvailableCommands();
        agent.destroy();
        expect(calledMethods).toEqual(['hopcode/skills/install', 'session/load']);
    ...
    it('deduplicates concurrent ACP session setup during command refresh', async () => {
        ...
        expect(capturedSessionIds).toEqual(['hopcode-session']);
    ...
    it('returns available commands captured while loading HopCode history', async () => {
        ...
        const result = await agent.loadSessionMessages('hopcode-session', { cwd });
        agent.destroy();
    ...
    it('loads HopCode history updates through ACP extension before session/load fallback', async () => {
        ...
        internals.callAcp = async (method, execute) => {
            ...
            return execute({
                extMethod: async (extMethod, params) => {
                    expect(extMethod).toBe('hopcode/session/loadUpdates');
                    expect(params).toEqual({ sessionId: 'hopcode-session', cwd });
                    return {
                        updates: [
                            {
                                sessionUpdate: 'user_message_chunk',
        ...
        const result = await agent.loadSessionMessages('hopcode-session', { cwd });
        agent.destroy();
        expect(calledMethods).toEqual(['ext/hopcode/session/loadUpdates']);
...
//# sourceMappingURL=hopcode-agent-slash-history.test.js.map