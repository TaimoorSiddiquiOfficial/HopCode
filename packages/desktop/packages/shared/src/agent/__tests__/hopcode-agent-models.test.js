...
import { HopCodeAgent, extractHopCodeParentToolUseId, formatHopCodeAcpErrorMessage, resolveHopCodeParentToolUseId, } from '../hopcode-agent.ts';
...
describe('HopCodeAgent ACP error formatting', () => {
    it('includes ACP internal error details in user-visible messages', () => {
        const error = Object.assign(new Error('Internal error'), {
            data: { details: '401 Unauthorized' },
        });
        expect(formatHopCodeAcpErrorMessage(error)).toBe('Internal error: 401 Unauthorized');
    });
    it('uses nested provider messages from ACP internal error data', () => {
        ...
        expect(formatHopCodeAcpErrorMessage(error)).toBe('Internal error: Model access denied');
    });
});
describe('HopCodeAgent model metadata', () => {
    it('extracts subagent parent metadata from HopCode ACP updates', () => {
        expect(extractHopCodeParentToolUseId({
            _meta: {
    ...
    it('falls back to the only active parent tool without self-referencing', () => {
        expect(resolveHopCodeParentToolUseId({
            update: {},
    ...
    it('applies configured session models through ACP session/set_model', async () => {
        ...
        await internals.applySessionSettings('hopcode-session-1');
...
//# sourceMappingURL=hopcode-agent-models.test.js.map