...
function generateMismatchSuggestion(command, matchedPrefix, failedToken, pattern) {
    ...
    if (commandsWithSubcommands.includes(firstWord || '') &&
        ...
        return `The pattern expects a subcommand after \`${firstWord}\`, but found flag \`${failedToken}\`. ` +
            `Run from the target directory or switch to Ask before edits, Edit automatically, or IZN.`;
    }
...
export function formatBashRejectionMessage(reason, config) {
    const modeSwitchHint = `Switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to run it.`;
...
export function shouldAllowToolInMode(toolName, toolInput, mode, options) {
    ...
    if (toolName === 'Bash') {
        ...
        if (typeof command === 'string') {
            ...
            if (likelyWriteAttempt && (options?.plansFolderPath || options?.dataFolderPath)) {
                ...
                if (targetPath) {
                    ...
                    lines.push(``, `Allowed paths in Plan mode:`, ...[plansHint, dataHint].filter(Boolean).map(p => `• ${p}`), `• Or ask the user to switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to enable writes anywhere`);
                    return {
                        allowed: false,
                        reason: lines.join('\n'),
                    };
        ...
        return {
            allowed: false,
            reason: `Bash command is missing or invalid. Switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to run it.`,
        };
    }
    ...
    if (toolName.startsWith('mcp__')) {
        ...
        if (toolName.startsWith('mcp__session__')) {
            ...
            return {
                allowed: false,
                reason: `Session configuration changes are blocked in ${config.displayName}. Switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to create, update, or delete sources and agents.`
            };
        }
        ...
        if (toolName.includes('__api_')) {
            ...
            return {
                allowed: false,
                reason: `API ${method} ${path ?? ''} is blocked in ${config.displayName}. Switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to make changes.`
            };
        }
        ...
        return {
            allowed: false,
            reason: `MCP write operations are blocked in ${config.displayName}. Switch to Ask before edits, Edit automatically, or IZN (${config.shortcutHint}) to make changes.`
        };
    }
...
function getBlockReasonWithConfig(toolName, config) {
    ...
    if (toolName === 'Bash') {
        return `Bash commands are blocked in ${displayName}. Switch to Ask before edits, Edit automatically, or IZN (${shortcut}) to run commands.`;
    }
    if (toolName === 'Write' || toolName === 'Edit' || toolName === 'MultiEdit') {
        return `File modifications are blocked in ${displayName}. Switch to Ask before edits, Edit automatically, or IZN (${shortcut}) to make changes.`;
    }
    if (toolName.startsWith('mcp__')) {
        return `MCP write operations are blocked in ${displayName}. Switch to Ask before edits, Edit automatically, or IZN (${shortcut}) to make changes.`;
    }
    if (toolName.startsWith('api_')) {
        return `API mutations are blocked in ${displayName}. Switch to Ask before edits, Edit automatically, or IZN (${shortcut}) to make changes.`;
    }
    return `${toolName} is blocked in ${displayName}. Switch to Ask before edits, Edit automatically, or IZN (${shortcut}) to use this tool.`;
}
/**
