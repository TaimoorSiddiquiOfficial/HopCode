...
const originalHopCodeCli = process.env.HOPCODE_CODE_CLI;
const originalHopCodePath = process.env.HOPCODE_CODE_PATH;
const originalHopCodeRoot = process.env.HOPCODE_CODE_ROOT;
...
describe('resolveBackendRuntimePaths', () => {
    beforeEach(() => {
        ...
        delete process.env.HOPCODE_CODE_PATH;
        delete process.env.HOPCODE_CODE_ROOT;
    });
    afterEach(() => {
        if (originalHopCodeCli === undefined) {
            delete process.env.HOPCODE_CODE_CLI;
        }
        else {
            process.env.HOPCODE_CODE_CLI = originalHopCodeCli;
        }
        if (originalHopCodePath === undefined) {
            delete process.env.HOPCODE_CODE_PATH;
        }
        else {
            process.env.HOPCODE_CODE_PATH = originalHopCodePath;
        }
        if (originalHopCodeRoot === undefined) {
            delete process.env.HOPCODE_CODE_ROOT;
        }
        else {
            process.env.HOPCODE_CODE_ROOT = originalHopCodeRoot;
        }
    });
    it('prefers the current checkout Qwen CLI source entry in dev mode', () => {
        ...
        try {
            ...
            expect(resolved.hopcodeCliPath).toBe(join(root, 'scripts', 'dev.js'));
        }
        finally {
    ...
    it('falls back to the current checkout Qwen CLI bundle when source dev entry is absent', () => {
        ...
        try {
            ...
            expect(resolved.hopcodeCliPath).toBe(join(root, 'dist', 'cli.js'));
        }
        finally {
            rmSync(root, { recursive: true, force: true });
        }
    ...
    it('does not silently fall back to an unrelated Qwen CLI from a source checkout', () => {
        ...
        try {
            ...
            expect(resolved.hopcodeCliPath).toBeUndefined();
        }
        finally {
    ...
    it('resolves a vendored desktop CLI in standalone dev checkouts', () => {
        const root = mkdtempSync(join(tmpdir(), 'craft-runtime-'));
        const originalCwd = process.cwd();
        const cliPath = join(root, 'apps', 'electron', 'vendor', 'hopcode', 'dist', 'cli.js');
        mkdirSync(join(root, 'apps', 'electron', 'vendor', 'hopcode', 'dist'), {
            recursive: true,
        });
        ...
        try {
            ...
            expect(resolved.hopcodeCliPath).toBe(cliPath);
        }
        finally {
            process.chdir(originalCwd);
            rmSync(root, { recursive: true, force: true });
