...
function isBlockedResolvedIp(address) {
...
export async function assertVoiceBaseUrlNetworkAllowed(baseUrl, model, lookupHost) {
    ...
    if (isIP(host) !== 0) {
        if (isPrivateNetworkIp(host)) {
            throw new Error(`Voice model '${model}': baseUrl is a private-network address.`);
        }
        return;
    }
    let result;
    ...
    if (records.some((record) => isBlockedResolvedIp(record.address))) {
        throw new Error(`Voice model '${model}' resolved to a private-network address.`);
    }
}
