...
const NO_CREDENTIALS_ERROR = 'Voice dictation needs Qwen credentials. Sign in to HopCode (or set a DashScope API key), then try again.';
/**
...
async function readQwenJsonFromDisk(file) {
    try {
        return JSON.parse(await readFile(join(getQwenConfigDir(), file), 'utf-8'));
    }
    catch {
        return undefined;
    }
...
async function fromQwenSettings(deps) {
    const settings = await deps.readQwenJson('settings.json');
    if (!settings)
        return undefined;
    ...
    for (const provider of providers) {
        if (provider.baseUrl && isDashscopeCompatible(provider.baseUrl)) {
            ...
            if (apiKey)
                return { baseUrl: normalizeBaseUrl(provider.baseUrl), apiKey };
        }
    }
    return undefined;
}
...
export async function resolveDesktopVoiceConfig(deps = {}) {
    ...
    const creds = (await fromOAuth(resolvedDeps)) ??
