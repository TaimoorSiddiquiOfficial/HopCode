...
describe('assertVoiceBaseUrlNetworkAllowed', () => {
    it('rejects private IP-literal hosts without DNS lookup', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed('https://10.0.0.1:443', 'm', lookup)).rejects.toThrow(/private-network/);
        await expect(assertVoiceBaseUrlNetworkAllowed('https://169.254.169.254', 'm', lookup)).rejects.toThrow(/private-network/);
        expect(called).toBe(false);
    });
    it('rejects a hostname that resolves to a private address', async () => {
        await expect(assertVoiceBaseUrlNetworkAllowed('https://evil.example', 'm', async () => [
            { address: '10.1.2.3' },
    ...
    it('allows a hostname that resolves to a public address', async () => {
        await expect(assertVoiceBaseUrlNetworkAllowed('https://api.example', 'm', async () => [
    ...
    it('rejects when the DNS lookup fails (cannot verify network safety)', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed('https://voice.example', 'm', lookup)).rejects.toThrow("Voice model 'm': DNS lookup failed for voice.example. Cannot verify network safety.");
    });
});
