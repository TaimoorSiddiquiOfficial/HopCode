...
import {
  ...
  type HopCodePermissionSettings,
  type HopCodeCoreSettingKey,
  type HopCodeCoreSettingsSnapshot,
  type HopCodeHookDefinition,
  type HopCodeHookEvent,
  type HopCodeMcpServerConfig,
  type HopCodeProviderCatalog,
  type HopCodeProviderConnectParams,
  type HopCodeProviderConnectResult,
  type HopCodeSettingValue,
  type HopCodeSettingsScope,
  type HopCodeSkillDeleteRequest,
  type HopCodeSkillDeleteResult,
  type HopCodeSkillInstallRequest,
  type HopCodeSkillInstallResult,
  type HopCodeSkillSetEnabledRequest,
  type HopCodeSkillSetEnabledResult,
  RPC_CHANNELS,
  generateMessageId,
...
type hopcodeSettingsAgent = AgentBackend & {
...
type HopCodeProvidersAgent = AgentBackend & {
...
function canManagehopcodeSettings(
  agent: AgentBackend,
): agent is hopcodeSettingsAgent {
  const candidate = agent as Partial<hopcodeSettingsAgent>
  return (
...
function canManageHopCodeProviders(
  agent: AgentBackend,
): agent is HopCodeProvidersAgent {
  const candidate = agent as Partial<HopCodeProvidersAgent>
  return (
...
function hopcodeCanonicalLocalVisualMessages(messages: Message[]): Message[] {
...
function mergehopcodeCanonicalLocalVisualMessages(
  messages: Message[],
  localMessages: Message[],
): Message[] {
  const overlays = hopcodeCanonicalLocalVisualMessages(localMessages)
...
function isQwenCanonicalManagedSession(
  ...
  return (
    ...
    (managed.llmConnection === HOPCODE_CODE_CONNECTION_SLUG ||
...
export class SessionManager implements ISessionManager {
  ...
  private updatehopcodeConnectionDefault(slug: string, model: string): void {
    const connection = getLlmConnection(slug)
    if (!connection || connection.providerType !== 'hopcode') return
  ...
  private inferLegacyQwenCanonicalAttachmentOverlays(
    ...
    if (hopcodeCanonicalLocalVisualMessages(managed.messages).length > 0) {
      return []
    }
  ...
  private async upsertExternalListedSession(args: {
    ...
    if (managed) {
      ...
      if (connectionSlug === HOPCODE_CODE_CONNECTION_SLUG) {
    ...
    const shouldPersistInspectedMessages =
      connectionSlug !== HOPCODE_CODE_CONNECTION_SLUG
    const createdAt =
  ...
  private persistSession(managed: ManagedSession): void {
    try {
      ...
      const persistableMessages = usesQwenCanonicalMessages
        ? hopcodeCanonicalLocalVisualMessages(managed.messages)
  ...
  async createSession(
    ...
    const targetProviderType =
      targetBackendContext.connection?.providerType ?? 'hopcode'
    ...
    if (options?.branchFromSessionId || options?.branchFromMessageId) {
      ...
      const sourceProviderType =
        sourceBackendContext.connection?.providerType ?? 'hopcode'
  ...
  private async createAgentForManagedSession(
    ...
    if (!managed.agent) {
      ...
      if (connection) {
        ...
        sessionLog.warn(
          `No LLM connection found for session ${managed.id}, using HopCode fallback`,
        )
      }
          ...
          onAvailableModelsUpdate:
            connection?.providerType === 'hopcode'
              ? (models, currentModelId) =>
                  this.updatehopcodeConnectionModels(
                    connection.slug,
                    models,
                    currentModelId,
                  )
  ...
  async updateSessionModel(
    ...
    if (sessionConn?.providerType === 'hopcode' && persistedModel) {
      this.updatehopcodeConnectionDefault(sessionConn.slug, persistedModel)
    }
    ...
    if (managed.agent) {
      ...
      if (effectiveModel) {
        ...
        if (sessionConn?.providerType === 'hopcode') {
          const agent = managed.agent
          void (async () => {
            try {
              await agent.refreshAvailableCommands?.()
              await this.refreshhopcodeConnectionDefault(
                sessionConn.slug,
  ...
  private async refreshhopcodeConnectionDefault(
    slug: string,
    reason: string,
  ): Promise<void> {
    try {
  ...
  private async persistQwenApprovalMode(
    ...
    for (const managed of candidates) {
      const agent = managed.agent
      if (!agent || !canManagehopcodeSettings(agent)) continue
      try {
  ...
  async listSessionHopCodeProviders(
    sessionId: string,
  ...
  async connectSessionHopCodeProvider(
    sessionId: string,
    params: HopCodeProviderConnectParams,
  ): Promise<HopCodeProviderConnectResult> {
    const managed = this.sessions.get(sessionId)
  ...
  private async refreshDraftAvailableCommands(
    ...
    if (backendContext.provider !== 'hopcode') {
      return {
        success: false,
