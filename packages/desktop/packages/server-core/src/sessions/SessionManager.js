...
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, HOPCODE_CODE_CONNECTION_SLUG, } from '@craft-agent/shared/config';
...
function canManagehopcodeSettings(agent) {
    const candidate = agent;
...
function canManageHopCodeProviders(agent) {
    const candidate = agent;
...
function hopcodeCanonicalLocalVisualMessages(messages) {
...
function mergehopcodeCanonicalLocalVisualMessages(messages, localMessages) {
...
export class SessionManager {
    ...
    updatehopcodeConnectionModels(slug, models, currentModelId) {
    ...
    updatehopcodeConnectionDefault(slug, model) {
        const connection = getLlmConnection(slug);
        if (!connection || connection.providerType !== 'hopcode')
            return;
        try {
            const previous = getModelRefreshService().getRuntimeModelState(slug);
    ...
    async upsertExternalListedSession(args) {
        ...
        const shouldPersistInspectedMessages = connectionSlug !== HOPCODE_CODE_CONNECTION_SLUG;
        ...
        const storedSession = (connectionSlug === HOPCODE_CODE_CONNECTION_SLUG
            ? {
    ...
    async createSession(workspaceId, options) {
        ...
        const targetProviderType = targetBackendContext.connection?.providerType ?? 'hopcode';
        ...
        if (options?.branchFromSessionId || options?.branchFromMessageId) {
            ...
            const sourceProviderType = sourceBackendContext.connection?.providerType ?? 'hopcode';
    ...
    async createAgentForManagedSession(managed) {
        if (!managed.agent) {
            ...
            if (connection) {
                ...
                sessionLog.warn(`No LLM connection found for session ${managed.id}, using HopCode fallback`);
            }
            ...
            managed.agent = createBackendFromResolvedContext({
                ...
                coreConfig: {
                    ...
                    onAvailableModelsUpdate: connection?.providerType === 'hopcode'
                        ? (models, currentModelId) => this.updatehopcodeConnectionModels(connection.slug, models, currentModelId)
                        : undefined,
                    getRecoveryMessages,
    ...
    async persistDraftModelSelection(workspaceId, model, connection) {
        ...
        try {
            ...
            if (sessionConnection?.slug) {
                await this.refreshhopcodeConnectionDefault(sessionConnection.slug, 'draft model selection');
            }
    ...
    async refreshhopcodeConnectionDefault(slug, reason) {
        try {
    ...
    async persistQwenApprovalMode(mode, preferredSessionId) {
        ...
        for (const managed of candidates) {
            const agent = managed.agent;
            if (!agent || !canManagehopcodeSettings(agent))
                continue;
            try {
    ...
    async listSessionHopCodeProviders(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async connectSessionHopCodeProvider(sessionId, params) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
