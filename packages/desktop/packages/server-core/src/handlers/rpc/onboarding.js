...
import { getHopCodeWorkspacePreflightViaAcp, listHopCodeProvidersViaAcp, } from '@craft-agent/shared/agent';
...
async function getHopCodeSetupNeeds(deps) {
    if (isSetupDeferred()) {
    ...
    try {
        ...
        const catalog = await listHopCodeProvidersViaAcp({
            hostRuntime: buildBackendHostRuntimeContext(deps.platform),
        });
        ...
        deps.platform.logger?.warn(`HopCode setup preflight failed; continuing to main UI: ${message}`);
        return completeSetupNeeds();
    }
}
