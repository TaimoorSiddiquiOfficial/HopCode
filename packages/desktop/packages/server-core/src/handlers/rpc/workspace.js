...
async function ensureWorktreesGitignored(repoRoot) {
    const hopcodeDir = join(repoRoot, '.hopcode');
    await mkdir(hopcodeDir, { recursive: true });
    await writeFile(join(hopcodeDir, '.gitignore'), `# Auto-generated by hopcode.\n${WORKTREES_DIR}/\n`, { encoding: 'utf8', flag: 'wx' }).catch((error) => {
...
export function registerWorkspaceCoreHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.workspaces.CREATE_PERMANENT_WORKTREE, async (_ctx, workspaceId, branchNameInput) => {
        ...
        if (/[\\/]\.hopcode[\\/]worktrees[\\/]/.test(sourceDir)) {
        ...
        const worktreesDir = join(repoRoot, '.hopcode', WORKTREES_DIR);
        ...
        await ensureWorktreesGitignored(repoRoot).catch((error) => {
            deps.platform.logger.warn(`Failed to update .hopcode/.gitignore for worktrees: ${error}`);
        });
