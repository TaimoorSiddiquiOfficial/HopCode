...
import { HOPCODE_CODE_CONNECTION_SLUG, addLlmConnection, deleteLlmConnection, getDefaultLlmConnection, getLlmConnection, getLlmConnections, getWorkspaceByNameOrId, setDefaultLlmConnection, touchLlmConnection, updateLlmConnection, } from '@craft-agent/shared/config';
...
import { connectHopCodeProviderViaAcp, listHopCodeProvidersViaAcp, } from '@craft-agent/shared/agent';
...
export const HANDLED_CHANNELS = [
    ...
    RPC_CHANNELS.settings.CONNECT_HOPCODE_PROVIDER,
];
function ensurehopcodeConnection(slug = HOPCODE_CODE_CONNECTION_SLUG) {
    return (getLlmConnection(slug) ?? createBuiltInConnection(HOPCODE_CODE_CONNECTION_SLUG));
}
...
export function registerLlmConnectionsHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.settings.LIST_HOPCODE_PROVIDERS, async (ctx) => listHopCodeProvidersViaAcp(await getQwenWorkspaceAcpOptions(deps, ctx)));
    server.handle(RPC_CHANNELS.settings.CONNECT_HOPCODE_PROVIDER, async (ctx, params, _sessionId) => {
        try {
            const scopedParams = {
                ...params,
                scope: 'user',
            };
            const result = await connectHopCodeProviderViaAcp(await getQwenWorkspaceAcpOptions(deps, ctx), scopedParams);
            if (!result.success)
                return result;
            const existing = ensurehopcodeConnection();
            const hopcodeConnection = {
                ...existing,
                slug: HOPCODE_CODE_CONNECTION_SLUG,
                name: BRAND.selfReferName,
                providerType: 'hopcode',
                authType: 'none',
                defaultModel: result.modelId || existing.defaultModel,
            };
            if (getLlmConnection(hopcodeConnection.slug)) {
                updateLlmConnection(hopcodeConnection.slug, hopcodeConnection);
            }
            else {
                addLlmConnection(hopcodeConnection);
            }
            setDefaultLlmConnection(hopcodeConnection.slug);
            try {
    ...
    server.handle(RPC_CHANNELS.settings.SETUP_LLM_CONNECTION, async (_ctx, setup) => {
        try {
            const connection = ensurehopcodeConnection(setup.slug);
            const hopcodeConnection = {
                ...connection,
                slug: HOPCODE_CODE_CONNECTION_SLUG,
                name: BRAND.selfReferName,
    ...
    server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params) => {
        ...
        try {
            ...
            deps.platform.logger?.info(`[testhopcodeConnectionSetup] Elapsed: ${elapsed}ms, success=${result.success}`);
            ...
            deps.platform.logger?.info(`[testhopcodeConnectionSetup] Elapsed: ${elapsed}ms, threw: ${msg.slice(0, 1000)}`);
    ...
    server.handle(RPC_CHANNELS.llmConnections.SAVE, async (_ctx, connection) => {
        try {
            const hopcodeConnection = {
                ...connection,
                slug: connection.slug || HOPCODE_CODE_CONNECTION_SLUG,
                name: connection.name || BRAND.selfReferName,
                providerType: 'hopcode',
                authType: 'none',
            };
            const existing = getLlmConnection(hopcodeConnection.slug);
