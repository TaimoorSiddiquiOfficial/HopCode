...
import { getHopCodeCoreSettingsViaAcp, getHopCodeMemorySettingsViaAcp, getHopCodePermissionSettingsViaAcp, removeHopCodeHookViaAcp, removeHopCodeMcpServerViaAcp, setHopCodeMemorySettingsViaAcp, setHopCodeCoreSettingViaAcp, setHopCodeExtensionSettingViaAcp, setHopCodeHookViaAcp, setHopCodeMcpServerViaAcp, setHopCodePermissionRulesViaAcp, getHopCodeSettingsPathViaAcp, getHopCodeMemoryPathsViaAcp, } from '@craft-agent/shared/agent';
...
export const HANDLED_CHANNELS = [
    ...
    RPC_CHANNELS.settings.GET_HOPCODE_CORE_SETTINGS,
    RPC_CHANNELS.settings.SET_HOPCODE_CORE_SETTING,
    RPC_CHANNELS.settings.SET_HOPCODE_MCP_SERVER,
    RPC_CHANNELS.settings.REMOVE_HOPCODE_MCP_SERVER,
    RPC_CHANNELS.settings.SET_HOPCODE_HOOK,
    RPC_CHANNELS.settings.REMOVE_HOPCODE_HOOK,
    RPC_CHANNELS.settings.SET_HOPCODE_EXTENSION_SETTING,
    RPC_CHANNELS.settings.GET_HOPCODE_PERMISSION_SETTINGS,
    RPC_CHANNELS.settings.SET_HOPCODE_PERMISSION_RULES,
...
async function getHopCodeWorkspaceAcpOptions(deps, ctx, workspaceId) {
...
function mapPermissionModeToHopCodeApprovalMode(mode) {
    switch (mode) {
        case 'allow-all':
            return 'izn';
        case 'safe':
...
function permissionModeFromHopCodeCoreSettings(snapshot) {
...
export function registerSettingsHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.settings.GET_GLOBAL_PERMISSION_MODE, async (ctx) => {
        const snapshot = await getHopCodeCoreSettingsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx));
    ...
    server.handle(RPC_CHANNELS.settings.SET_GLOBAL_PERMISSION_MODE, async (ctx, mode) => {
        ...
        const snapshot = await setHopCodeCoreSettingViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), 'user', 'tools.approvalMode', mapPermissionModeToHopCodeApprovalMode(parsed));
        await deps.sessionManager.applyGlobalPermissionMode(permissionModeFromHopCodeCoreSettings(snapshot));
        return { success: true };
    });
    // ============================================================
    ...
    server.handle(RPC_CHANNELS.settings.GET_HOPCODE_CORE_SETTINGS, async (ctx) => getHopCodeCoreSettingsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx)));
    server.handle(RPC_CHANNELS.settings.SET_HOPCODE_CORE_SETTING, async (ctx, scope, key, value) => {
        const snapshot = await setHopCodeCoreSettingViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, key, value);
        if (key === 'tools.approvalMode') {
            await deps.sessionManager.applyGlobalPermissionMode(permissionModeFromHopCodeCoreSettings(snapshot));
        }
        return snapshot;
    });
    server.handle(RPC_CHANNELS.settings.SET_HOPCODE_MCP_SERVER, async (ctx, scope, name, mcpServer) => setHopCodeMcpServerViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, name, mcpServer));
    server.handle(RPC_CHANNELS.settings.REMOVE_HOPCODE_MCP_SERVER, async (ctx, scope, name) => removeHopCodeMcpServerViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, name));
    server.handle(RPC_CHANNELS.settings.SET_HOPCODE_HOOK, async (ctx, scope, event, index, hook) => setHopCodeHookViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, event, index, hook));
    server.handle(RPC_CHANNELS.settings.REMOVE_HOPCODE_HOOK, async (ctx, scope, event, index) => removeHopCodeHookViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, event, index));
    server.handle(RPC_CHANNELS.settings.SET_HOPCODE_EXTENSION_SETTING, async (ctx, extensionId, settingKey, scope, value) => setHopCodeExtensionSettingViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), extensionId, settingKey, scope, value));
    server.handle(RPC_CHANNELS.settings.GET_HOPCODE_PERMISSION_SETTINGS, async (ctx) => getHopCodePermissionSettingsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx)));
    server.handle(RPC_CHANNELS.settings.SET_HOPCODE_PERMISSION_RULES, async (ctx, scope, ruleType, rules) => setHopCodePermissionRulesViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx), scope, ruleType, rules));
    // ============================================================
    ...
    // Memory Settings
    // ============================================================
    server.handle(RPC_CHANNELS.memory.GET_SETTINGS, async (ctx, workspaceId) => getHopCodeMemorySettingsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx, workspaceId)));
    server.handle(RPC_CHANNELS.memory.SET_SETTINGS, async (ctx, updates, workspaceId) => setHopCodeMemorySettingsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx, workspaceId), updates));
    server.handle(RPC_CHANNELS.memory.GET_SETTINGS_PATH, async () => getHopCodeSettingsPathViaAcp({
        hostRuntime: buildBackendHostRuntimeContext(deps.platform),
    }));
    server.handle(RPC_CHANNELS.memory.GET_PATHS, async (ctx, workspaceId) => getHopCodeMemoryPathsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx, workspaceId)));
    server.handle(RPC_CHANNELS.memory.OPEN_PATH, async (ctx, target, workspaceId) => {
        ...
        const paths = await getHopCodeMemoryPathsViaAcp(await getHopCodeWorkspaceAcpOptions(deps, ctx, workspaceId));
