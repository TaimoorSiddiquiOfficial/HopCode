  ...
  server.handle(RPC_CHANNELS.gitbash.CHECK, async () => {
    ...
    if (persistedPath) {
      if (await isUsableGitBashPath(persistedPath)) {
        process.env.HOPCODE_CODE_GIT_BASH_PATH = persistedPath.trim()
    ...
    for (const bashPath of commonPaths) {
      if (await isUsableGitBashPath(bashPath)) {
        process.env.HOPCODE_CODE_GIT_BASH_PATH = bashPath
    ...
    try {
      ...
      if (
        ...
        process.env.HOPCODE_CODE_GIT_BASH_PATH = firstPath
