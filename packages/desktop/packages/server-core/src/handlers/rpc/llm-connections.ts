...
function ensurehopcodeConnection(slug = HOPCODE_CODE_CONNECTION_SLUG): LlmConnection {
  return (
    getLlmConnection(slug) ?? createBuiltInConnection(HOPCODE_CODE_CONNECTION_SLUG)
  );
}
...
export function registerLlmConnectionsHandlers(
  ...
  server.handle(
    ...
    async (
      ...
      try {
        const scopedParams: HopCodeProviderConnectParams = {
          ...params,
          scope: 'user',
        };
        ...
        const hopcodeConnection: LlmConnection = {
          ...existing,
        ...
        try {
          ...
          deps.platform.logger?.warn(
            `hopcode model refresh after provider connect failed: ${err instanceof Error ? err.message : err}`,
          );
        }
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          deps.platform.logger?.warn(
            `hopcode model refresh after setup failed: ${err instanceof Error ? err.message : err}`,
          );
        }
    ...
    > => {
      ...
      try {
        ...
        deps.platform.logger?.info(
          `[testhopcodeConnectionSetup] Elapsed: ${elapsed}ms, success=${result.success}`,
        );
        ...
        deps.platform.logger?.info(
          `[testhopcodeConnectionSetup] Elapsed: ${elapsed}ms, threw: ${msg.slice(0, 1000)}`,
        );
      ...
      try {
        ...
        if (result.shouldRefreshModels) {
          getModelRefreshService()
            ...
            .catch((err) => {
              deps.platform.logger?.warn(
                `hopcode model refresh failed during validation: ${err instanceof Error ? err.message : err}`,
              );
            });
        }
        return { success: true };
