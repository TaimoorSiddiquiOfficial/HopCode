...
import { DaemonSessionProvider, useDaemonActions, useDaemonConnection, useDaemonSessionNotices, useDaemonPendingPermissions, useDaemonPromptStatus, useDaemonStreamingState, useDaemonTranscriptBlocks, useDaemonTranscriptHistory, useDaemonTranscriptState, useDaemonTranscriptStore, useDaemonWorkspaceEventSignals, } from './DaemonSessionProvider.js';
...
const sdkMocks = vi.hoisted(() => {
    ...
    return {
        ...
        reset() {
            ...
            readWorkspaceFile.mockResolvedValue({ path: 'HOPCODE.md', text: '' });
...
describe('DaemonSessionProvider', () => {
        ...
        const session = createMockSession({
            events: async function* workspaceEvents() {
                yield {
                    ...
                    data: {
                        scope: 'workspace',
                        filePath: '/mock-workspace/HOPCODE.md',
                        mode: 'append',
                        bytesWritten: 12,
                    },
                };
    ...
    it('reloads restored active prompts after epoch reset', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-epoch-active', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(promptStatus).toBe('streaming');
    });
    ...
    it('recovers internally when the daemon requests a state resync', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-resync', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(connection).toMatchObject({
            status: 'connected',
            sessionId: 'session-resync',
        });
    ...
    it('loads controlled sessionId changes', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-b', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(connection).toMatchObject({ sessionId: 'session-b' });
        expect(blocks).toMatchObject([
            { kind: 'assistant', text: 'new transcript' },
        ]);
    ...
    it('loads controlled sessionId on mount without creating a session', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-a', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(sdkMocks.MockDaemonSessionClient.createOrAttach).not.toHaveBeenCalled();
        expect(connection).toMatchObject({ sessionId: 'session-a' });
    });
    ...
    it('does not clear a deferred session created after an empty controlled render', async () => {
        sdkMocks.sessions.push(createMockSession({ sessionId: 'created-session' }), createMockSession({ sessionId: 'created-session' }));
        let actions;
        let connection;
        function Harness() {
            actions = useDaemonActions();
    ...
    it('reloads the session snapshot after ring-evicted resync', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-ring-evicted', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(awaitingResync).toBe(false);
        expect(blocks).toEqual(expect.arrayContaining([
            expect.objectContaining({
                kind: 'assistant',
    ...
    it('reloads after epoch reset instead of consuming same-stream session_closed', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), 'session-epoch-closed-tail', { workspaceCwd: '/mock-workspace' }, expect.any(String));
        expect(connection?.status).toBe('connected');
    ...
    it('keeps history pagination disabled when the server does not advertise it', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), session.sessionId, { workspaceCwd: '/mock-workspace' }, expect.any(String));
    ...
    it('prepends an older transcript page from the first replay record', async () => {
        ...
        expect(sdkMocks.MockDaemonSessionClient.load).toHaveBeenCalledWith(expect.anything(), session.sessionId, { workspaceCwd: '/mock-workspace', historyPageSize: 25 }, expect.any(String));
