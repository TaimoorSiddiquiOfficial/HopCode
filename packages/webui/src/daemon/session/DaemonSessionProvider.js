...
import { DaemonClient, DaemonHttpError, DaemonSessionClient, createDaemonTranscriptStore, extractServerTimestamp, matchTurnEvent, normalizeDaemonEvent, } from '@hoptrendy/sdk/daemon';
...
import { getCurrentMode, getSessionDisplayName, getReplayTokenUsage, getTokenCountFromUsage, mapProviderStatus, mapSessionContextModels, mapSupportedCommands, mapWorkspaceSkills, updateConnectionFromDaemonEvent, } from './mappers.js';
...
import { parseSidechannelFollowupSuggestion, publishSidechannelFollowupSuggestion, } from '../followupSidechannel.js';
...
function prependTranscriptHistory(store, events, maxBlocks) {
...
export function DaemonSessionProvider(props) {
    const { baseUrl, token, workspaceCwd, sessionId, clientId, createSessionRequest, maxQueued = 1024, maxBlocks = DEFAULT_MAX_BLOCKS, historyPageSize, suppressOwnUserEcho = true, includeRawEvent = false, autoConnect = true, autoReconnect = true, restartEventStreamOnPrompt = false, reconnectDelayMs = 1_000, maxReconnectDelayMs = 10_000, heartbeatIntervalMs = 30_000, heartbeatFailureThreshold = 3, loadWarnings, children, } = props;
    ...
    const store = useMemo(() => createDaemonTranscriptStore({ maxBlocks }), [maxBlocks]);
    const sessionRef = useRef(undefined);
    const transcriptHistoryRef = useRef({ hasMore: false, loading: false, capacityReached: false });
    ...
    const skipNextCleanupDetachSessionIdRef = useRef(undefined);
    ...
    const clientIdRef = useRef(undefined);
    ...
    useEffect(() => {
                ...
                try {
                    ...
                    if (!session) {
                        ...
                        if ((shouldDeferInitialSessionCreation ||
                            ...
                            if (acpStatusResult.status === 'fulfilled' &&
                        ...
                        const restoreMethod = restoreSessionId && restoreMode === 'resume'
                    ...
                    const historyHasMore = Array.isArray(capabilities?.features) &&
                    ...
                    if (replayInjected) {
                        ...
                        for (const replayEvent of replayEvents) {
                            try {
                                const replayUiEvents = normalizeAndFilterEvent(replayEvent, activeSession.clientId, replayOpts, setConnection, { updateConnection: false });
                                allUiEvents.push(...filterDaemonUiEventsForTranscript(replayEvent, replayUiEvents, addNotice, dismissNotice));
                                if (replayEvent.type === 'turn_complete') {
                                    const stopReason = replayEvent.data
                                        ?.stopReason ?? 'end_turn';
                                    allUiEvents.push(assistantDoneFromTurnEvent(replayEvent, stopReason));
                                }
                                else if (replayEvent.type === 'turn_error') {
                                    allUiEvents.push(assistantDoneFromTurnEvent(replayEvent, 'error'));
                                }
                            }
                            catch (error) {
                    ...
                    if (loadWarningTexts.length > 0) {
                        store.dispatch(loadWarningTexts.map((text) => ({
                            type: 'status',
                            text,
                        })));
                    }
                    ...
                    for await (const event of activeSession.events({
                        ...
                        try {
                            ...
                            const normalizedUiEvents = normalizeAndFilterEvent(event, activeSession.clientId, eventOptionsRef.current, setConnection);
        ...
        return () => {
            ...
            setPromptStatus('idle');
            clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
            const keepSessionForNextEffect = session?.sessionId === skipNextCleanupDetachSessionIdRef.current;
            const isUnmounting = !mountedRef.current;
            if (pendingSessionLoadRef.current &&
                (!keepSessionForNextEffect || isUnmounting)) {
                clearTimeout(pendingSessionLoadRef.current.timeout);
                pendingSessionLoadRef.current.reject(new DOMException('Session load interrupted by cleanup', 'AbortError'));
                pendingSessionLoadRef.current = undefined;
            }
    ...
    const loadMoreTranscript = useCallback(async () => {
        ...
        try {
            ...
            if (page.partial || page.replayError) {
                ...
                throw new Error(page.replayError ?? 'Earlier session history was only partially read');
            }
            const replayOpts = {
                ...eventOptionsRef.current,
                suppressOwnUserEcho: false,
            ...
            for (const replayEvent of page.events) {
                try {
                    uiEvents.push(...filterDaemonUiEventsForTranscript(replayEvent, normalizeAndFilterEvent(replayEvent, activeSession.clientId, replayOpts, setConnection, { updateConnection: false }), addNotice, dismissNotice));
                }
                catch (error) {
                    const message = error instanceof Error ? error.message : String(error);
                    addNotice({
...
function filterDaemonUiEventsForTranscript(sourceEvent, events, addNotice, dismissNotice) {
    if (sourceEvent.type === 'session_snapshot' &&
...
export function useDaemonStreamingState() {
    ...
    return useMemo(() => selectDaemonStreamingState(blocks, promptStatus), [blocks, promptStatus]);
}
