...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@hopcode/webui` package
...
```typescript
// @hopcode/webui/src/context/PlatformContext.ts
...
```javascript
// @hopcode/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @hopcode/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@hopcode/webui';
// or import { FileIcon } from '@hopcode/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@hopcode/webui`
...
│     └── Edit files in @hopcode/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@hopcode/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@hopcode/webui`
