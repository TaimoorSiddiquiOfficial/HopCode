...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@hoptrendy/webui` package
...
```typescript
// @hoptrendy/webui/src/context/PlatformContext.ts
...
```javascript
// @hoptrendy/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @hoptrendy/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@hoptrendy/webui';
// or import { FileIcon } from '@hoptrendy/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@hoptrendy/webui`
...
│     └── Edit files in @hoptrendy/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@hoptrendy/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@hoptrendy/webui`
