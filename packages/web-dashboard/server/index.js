...
function getHopcodeDir() {
    const env = process.env['HOPCODE_RUNTIME_DIR'] ?? process.env['HOPCODE_RUNTIME_DIR'];
    if (env)
        return env;
...
async function readJSONLHead(filePath, maxLines = 5) {
    const records = [];
...
function extractPrompt(records) {
...
async function buildSessionMeta(filePath, projectDir, mtime) {
    try {
...
app.get('/api/sessions', async (req, res) => {
    try {
        ...
        for (let i = 0; i < files.length; i += CONCURRENCY) {
            ...
            const results = await Promise.all(batch.map((f) => buildSessionMeta(f.filePath, f.projectDir, f.mtime)));
        ...
        const filtered = search
            ? allMeta.filter((s) => s.prompt.toLowerCase().includes(search) ||
...
app.get('/api/sessions/:sessionId', async (req, res) => {
    try {
        ...
        function resolveSafePath(projectName, session) {
            ...
            const resolved = path.resolve(projectsDir, projectName, 'chats', `${session}.jsonl`);
        ...
        if (!filePath && fs.existsSync(projectsDir)) {
            ...
            for (const entry of fs.readdirSync(projectsDir, {
                ...
                if (!entry.isDirectory())
                    continue;
                const candidate = path.join(projectsDir, entry.name, 'chats', `${sessionId}.jsonl`);
...
app.get('/api/stats', async (_req, res) => {
    try {
        ...
        for (const { filePath } of files.slice(0, 200)) {
            ...
            try {
                const records = await readJSONLAll(filePath);
                totalMessages += records.filter((r) => r.type === 'user' || r.type === 'assistant').length;
                for (const r of records) {
...
app.get('*', (_req, res) => {
    ...
    if (fs.existsSync(indexPath)) {
        ...
        res.status(200).send(`<html><body style="font-family:monospace;padding:2rem">
...
app.listen(PORT, () => {
    ...
    console.log(`   Override: HOPCODE_RUNTIME_DIR=... or HOPCODE_DASHBOARD_PORT=...\n`);
});
