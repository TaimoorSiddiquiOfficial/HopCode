...
const { mockConnection, mockSessionActions, mockWorkspace, mockWorkspaceActions, mockMcp, mockStore, mockFollowup, testState, sidebarTokens, rawEnqueuePrompt, editorClear, editorCommit, editorFocus, editorInsertText, settingsReload, } = vi.hoisted(() => {
    const connection = {
        status: 'connected',
        sessionId: 'session-1',
        clientId: 'client-1',
...
vi.mock('./components/ChatEditor', async () => {
    ...
    return {
        ChatEditor: React.forwardRef(function ChatEditor(props, ref) {
            ...
            React.useImperativeHandle(ref, () => ({
                ...
                submit: () => {
                    props.onSubmit(testState.prompt, testState.promptImages, editorCommit, testState.inputAnnotations
...
vi.mock('./components/MessageList', async () => {
    ...
    return {
        MessageList: React.forwardRef(function MessageList(props, ref) {
            ...
            return React.createElement('div', { 'data-testid': 'messages' }, React.createElement(InteractionBlockerProbe), props.showRetryHint
...
vi.mock('./components/messages/SettingsMessage', async () => {
    const React = await import('react');
    return {
        SettingsMessage: (props) => React.createElement('div', { 'data-testid': 'settings-message' }, React.createElement('button', {
...
vi.mock('./components/dialogs/DialogShell', async () => {
    const React = await import('react');
    return {
        DialogShell: (props) => React.createElement('div', { 'data-testid': 'dialog-shell' }, props.children),
    };
});
...
vi.doMock('./components/SplitView', async () => {
    ...
    return {
        SplitView: (props) => {
            ...
            React.createElement('span', { 'data-testid': 'split-initial' }, (props.sessionIds ?? []).join(',')), 
            ...
            React.createElement('button', {
                ...
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [artifact], paneActions),
            }, 'artifact'), React.createElement('button', {
                'data-testid': 'split-report-updated-artifact',
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [updatedArtifact], paneActions),
            }, 'updated artifact'), React.createElement('button', {
                'data-testid': 'split-clear-artifacts',
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [], paneActions),
...
function renderApp(props = {}) {
    ...
    const doRender = (nextProps = props) => {
        act(() => {
            root.render(_jsx(App, { sidebar: { enabled: true }, ...nextProps }));
        });
    };
    doRender(props);
...
describe('App manual-run orchestration (scheduled tasks)', () => {
    ...
    it('rejects a bound run when the session switch times out', async () => {
        ...
        expect(err?.message).toMatch(/Timed out switching/);
    });
