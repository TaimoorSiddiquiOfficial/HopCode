...
vi.mock('./ChatPane', () => ({
    ChatPane: (props) => {
        ...
        return (_jsxs("div", { "data-testid": "chat-pane", "data-pane-workspace": props.workspaceCwd, "data-maximized": props.isMaximized ? 'true' : 'false', "data-pane-restart-sse": props.restartSseOnPrompt ? 'true' : 'false', children: [_jsx("span", { "data-testid": "pane-title", children: props.title }), props.onToggleMaximize && (_jsx("button", { "data-testid": "pane-maximize", onClick: props.onToggleMaximize, children: "max" })), props.onClose && (_jsx("button", { "data-testid": "pane-close", onClick: props.onClose, children: "x" }))] }));
    },
}));
...
beforeEach(() => {
    ...
    workspaceClient = {
        listWorkspaceSessions: vi.fn(async (cwd) => otherWorkspaceSessions[cwd] ?? []),
        workspaceByCwd: vi.fn((cwd) => ({
            listWorkspaceSessions: vi.fn(async () => otherWorkspaceSessions[cwd] ?? []),
        })),
    };
    reloadMock = vi.fn();
});
...
describe('SplitView', () => {
    ...
    it('reloads the picker list when the parent bumps the reload token', () => {
        ...
        expect(reloadMock).not.toHaveBeenCalled();
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: ['s1'], sessionListReloadToken: 1 }) })));
        expect(reloadMock).toHaveBeenCalledTimes(1);
    });
    ...
    it('offers other trusted workspaces’ sessions in the picker, tagged by workspace', async () => {
        ...
        expect(options.some((o) => o.includes('Beta') && o.includes('wsB'))).toBe(true);
    ...
    it('re-queries other workspaces when the picker opens', async () => {
        ...
        expect(workspaceClient.listWorkspaceSessions.mock.calls.length).toBeGreaterThan(before);
    });
});
