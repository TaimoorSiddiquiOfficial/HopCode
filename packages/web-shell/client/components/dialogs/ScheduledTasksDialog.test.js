...
async function mount(tasks, opts = {}) {
    ...
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ScheduledTasksDialog, { onRunPrompt: opts.onRunPrompt ?? vi.fn(), onCreateViaChat: vi.fn(), onOpenSession: opts.onOpenSession, onError: opts.onError ?? vi.fn() }) }));
    });
    await flush();
}
...
describe('ScheduledTasksDialog editing', () => {
    ...
    it('inserts extension, skill, and MCP references into the created prompt', async () => {
        ...
        await flush();
        expect(document.querySelector('[role="textbox"]')?.textContent).toContain('alibabacloud-compute-suite');
...
describe('ScheduledTasksDialog multi-workspace', () => {
    ...
    it('creates a task in the workspace chosen in the picker', async () => {
        ...
        expect(Array.from(wsSelect.querySelectorAll('option')).map((o) => o.textContent)).toEqual(['main', 'other']);
