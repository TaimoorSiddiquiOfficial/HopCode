...
function DiffFileRow({ workspaceCwd, file, }) {
    const { t } = useI18n();
...
export function GitDiffDialog({ workspaceCwd, onClose, }) {
    ...
    if (loading) {
        ...
        body = (_jsxs("div", { className: styles.fileList, children: [diff.files.map((file) => (_jsx(DiffFileRow
                ...
                , { workspaceCwd: workspaceCwd, file: file }, `${workspaceCwd}:${file.path}`))), diff.hiddenCount > 0 && (_jsx("div", { className: styles.hiddenNote, children: t('gitDiff.hidden', { count: diff.hiddenCount }) }))] }));
    }
    return (_jsx(DialogShell, { title: t('gitDiff.title'), subtitle: subtitle, size: "xl", allowFullscreen: true, onClose: onClose, children: body }));
}
