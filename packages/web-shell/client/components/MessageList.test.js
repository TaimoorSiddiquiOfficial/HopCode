...
import { attachTurnOutputs, applyTurnCollapse, findDisplayItemIndex, findTurnIdForIndex, getSessionTimelineEntries, getSessionTimelineRangeForIndexes, getSessionTimelineSignature, getTurnTimelineNode, getDisplayItemVirtualKey, getTurnIdByDisplayIndex, groupParallelAgents, shouldUseVirtualScroll, VIRTUAL_SCROLL_THRESHOLD, } from './MessageList';
...
function makeBackgroundAgentToolGroup(id) {
    return {
        id,
        role: 'tool_group',
        tools: [
...
describe('getSessionTimelineEntries', () => {
    ...
    it('cleans markdown markers from timeline details', () => {
        const [entry] = getSessionTimelineEntries([
            ...
            {
                ...makeAssistantMessage('a1'),
                content: '**agent-reproduce-align** – 对齐测试技能\n\n**用途：** 在 [HopCode](https://example.com) 中运行参考代码，中文*强调*·*范围*—*引用*「_下划线_，保留 snake_case。',
            },
        ]);
        expect(entry?.label).toBe('介绍下 `agent-reproduce-align`');
        expect(entry?.detail).toBe('agent-reproduce-align – 对齐测试技能 用途： 在 HopCode 中运行参考代码，中文强调·范围—引用「下划线，保留 snake_case。');
    });
...
function rowIds(items) {
    return items.flatMap((item) => {
        if (item.type === 'turn_content' && item.collapsed)
            return [];
        return item.type === 'message' ? item.message.id : item.key;
    });
}
function flattenedRowIds(items) {
    return items.flatMap((item) => {
        if (item.type === 'turn_content')
            return flattenedRowIds(item.items);
        return item.type === 'message' ? item.message.id : item.key;
    });
}
