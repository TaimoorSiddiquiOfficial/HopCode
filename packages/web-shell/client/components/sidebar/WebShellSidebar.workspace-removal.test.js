...
const { connection, workspace, workspaceActions, active, archived, useSessions, listWorkspaceSessions, archiveSessionsData, unarchiveSessionsData, exportArchivedSession, } = vi.hoisted(() => {
    const makeSessions = () => ({
        sessions: [],
        loading: false,
        error: null,
...
describe('WebShellSidebar workspace removal', () => {
    it('scopes pinned and archived sessions to a locked secondary workspace', async () => {
        ...
        expect(useSessions.mock.calls.every(([options]) => !Object.hasOwn(options ?? {}, 'sourceType'))).toBe(true);
        expect(listSecondarySessions.mock.calls.every(([options]) => !Object.hasOwn(options ?? {}, 'sourceType'))).toBe(true);
    });
...
describe('WebShellSidebar non-primary archive', () => {
    it('archives a trusted secondary session and reconciles every catalog', async () => {
        ...
        const secondaryRow = Array.from(container.querySelectorAll('[role="button"]')).find((row) => row.textContent?.includes('Secondary active'));
        expect(secondaryRow?.querySelector('button[aria-label="More actions"]')).toBeNull();
        await act(async () => {
            click(archiveButton);
