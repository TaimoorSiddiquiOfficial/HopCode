...
function renderSection(overrides = {}) {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WorkspaceSection, { workspace: overrides.workspace ?? trustedWorkspace, client: overrides.client ?? makeClient(), reloadToken: overrides.reloadToken ?? 0, untrustedLabel: "Untrusted", readOnlyLabel: "Read-only", trustToOpenLabel: "Trust to open", noSessionsLabel: "No sessions", loadErrorLabel: "Load failed", organizationEnabled: false, ungroupedLabel: "Ungrouped", formatTime: () => '', renderSession: (session) => (_jsx("div", { children: session.displayName }, session.sessionId)), onOpenGitDiff: overrides.onOpenGitDiff }) }));
    });
}
