...
function clampSidebarVisualWidth(width) {
    return Math.min(SIDEBAR_MAX_WIDTH, Math.max(SIDEBAR_DRAG_VISUAL_MIN_WIDTH, width));
}
...
export function WebShellSidebar({ collapsed, onCollapsedChange, onOpenSettings, onOpenPlugins, onOpenDaemonStatus, onOpenScheduledTasks, onOpenGoals, onOpenSessions, canOpenSessionsOverview, onOpenSplitView, canOpenSplitView, onNewSession, onLoadSession, onSelectCurrentSession, onError, theme, onThemeChange, mobileOpen, sessionListReloadToken, selectedWorkspaceCwd, onSelectWorkspace, onOpenGitDiff, workspaces: providedWorkspaces, lockedWorkspaceCwd, lockedWorkspace: lockedWorkspaceOptions, branding, footer, }) {
    ...
    const { sessions, loading, error, data: sessionsPage, reload, deleteSession, exportSession, archiveSession, } = useSessions({
        autoLoad: true,
    ...
    const { sessions: primaryPinnedSessions, reload: reloadPinnedSessions } = useSessions({
    ...
    const { sessions: archivedSessions, loading: archivedLoading, error: archivedError, reload: reloadArchived, deleteSession: deleteArchivedSession, unarchiveSession, } = useSessions({
        autoLoad: true,
    ...
    const [secondaryArchivedLoading, setSecondaryArchivedLoading] = useState(false);
    const [secondaryArchivedError, setSecondaryArchivedError] = useState(false);
    const [secondaryArchivedReloadToken, setSecondaryArchivedReloadToken] = useState(0);
    ...
    const previousRunningRef = useRef(null);
    ...
    const getSessionWorkspaceActions = useCallback((session) => {
        const sessionWorkspace = displayedWorkspaces.find((entry) => entry.cwd === session.workspaceCwd);
    ...
    const isPrimaryWorkspaceCwd = useCallback((workspaceCwd) => !workspaceCwd ||
        displayedWorkspaces.find((entry) => entry.cwd === workspaceCwd)
            ?.primary !== false, [displayedWorkspaces]);
    const getIdentityForSession = useCallback((session) => getSessionIdentity(session.sessionId, session.workspaceCwd || primaryWorkspaceCwd), [primaryWorkspaceCwd]);
    ...
    const canMutateSessionArchive = useCallback((session) => {
        const sessionWorkspace = displayedWorkspaces.find((entry) => entry.cwd === session.workspaceCwd);
        if (sessionWorkspace?.trusted === false)
            return false;
    ...
    const getArchivedExportWorkspaceCwd = useCallback((session) => {
        ...
        const sessionWorkspace = displayedWorkspaces.find((entry) => entry.cwd === workspaceCwd);
    ...
    useEffect(() => {
        ...
        const secondaryWorkspaces = displayedWorkspaces.filter((entry) => !entry.primary && entry.trusted);
        let cancelled = false;
        void Promise.allSettled(secondaryWorkspaces.map(async (entry) => {
            ...
            if (cancelled)
                return;
            setSecondaryPinnedSessions(results.flatMap((result) => result.status === 'fulfilled' ? result.value : []));
        });
        return () => {
            cancelled = true;
        };
    ...
    useEffect(() => {
        ...
        void Promise.allSettled(secondaryWorkspaces.map(async (entry) => {
            ...
            setSecondaryArchivedSessions(results.flatMap((result) => result.status === 'fulfilled' ? result.value : []));
            setSecondaryArchivedError(failures.length > 0);
            for (const failure of failures) {
                console.warn('[WebShellSidebar] archived session load failed:', failure.reason);
            }
        })
            .finally(() => {
            if (!cancelled)
    ...
    useEffect(() => {
        if (!projectExpanded && !hasRunningSession)
            return;
        const pollInterval = hasRunningSession && !error
    ...
    useEffect(() => {
        if (sessionListReloadToken !== undefined &&
    ...
    useEffect(() => {
        const runningBySessionId = new Map(sessions.map((session) => [
    ...
    const handleAddWorkspace = useCallback(async (cwd, persist) => {
    ...
    const handleSuggestWorkspacePaths = useCallback((prefix) => workspaceActions.suggestWorkspacePaths(prefix), [workspaceActions]);
    ...
    const saveRename = useCallback(() => {
        ...
        if (busySessionIdsRef.current.has(getSessionIdentity(sessionId, connection.workspaceCwd))) {
            return;
        }
            ...
            try {
                ...
                if (groupEditor.mode === 'create') {
                    if (groupEditor.targetSession) {
                        try {
                            await groupActions.updateSessionOrganization(groupEditor.targetSession.sessionId, 
    ...
    const confirmDeleteGroup = useCallback(() => {
        if (!deleteGroupCandidate)
            return;
        setGroupBusy(true);
        const groupActions = !isPrimaryWorkspaceCwd(deleteGroupCandidate.workspaceCwd)
    ...
    const handleUnarchive = useCallback((session) => {
        const sessionId = session.sessionId;
        const sessionIdentity = getIdentityForSession(session);
        if (!canMutateSessionArchive(session))
            return;
        if (busySessionIdsRef.current.has(sessionIdentity))
            return;
        setSessionBusy(sessionId, true, session.workspaceCwd);
    ...
    const openGroupMenuFromAnchor = useCallback(async (anchorEl, session) => {
        let groupCount = 0;
        try {
            const catalog = await getSessionWorkspaceActions(session).listSessionGroups();
            setMenuGroups(catalog.groups);
            setColorOptions(catalog.colorOptions);
    ...
    const renderSessionRow = useCallback((session, options = {}) => {
        ...
        const details = (_jsxs("div", { className: styles.tooltipContent, children: [_jsx("div", { className: styles.tooltipTitle, children: label }), _jsxs("div", { className: styles.tooltipTags, children: [session.hasActivePrompt && (_jsx("span", { className: cx(styles.tooltipTag, styles.tooltipTagRunning), children: t('sidebar.running') })), completedUnread && (_jsx("span", { className: cx(styles.tooltipTag, styles.tooltipTagNew), children: t('sidebar.completedUnread') })), _jsx("span", { className: styles.tooltipTag, children: t('sidebar.clients', { count: session.clientCount ?? 0 }) })] }), _jsx("div", { className: styles.tooltipMeta, children: session.sessionId })] }));
        if (isArchived) {
            return (_jsxs("div", { className: cx(styles.sessionRow, styles.archivedRow, busy && styles.busySession), children: [_jsx("span", { className: styles.sessionText, children: label }), _jsxs("div", { className: styles.sessionMetaSlot, children: [_jsx("span", { className: styles.sessionTime, children: time }), _jsx("div", { className: styles.sessionActions, onClick: (event) => event.stopPropagation(), onKeyDown: (event) => event.stopPropagation(), children: _jsxs(DropdownMenu, { children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { className: styles.sessionActionButton, type: "button", "aria-label": t('sidebar.moreActions'), title: t('sidebar.moreActions'), children: _jsx(EllipsisVerticalIcon, {}) }) }), _jsx(DropdownMenuContent, { align: "end", className: "w-auto min-w-40", onPointerDownOutside: () => {
                                                ...
                                                event.preventDefault();
                                            }, children: _jsxs(DropdownMenuGroup, { children: [_jsxs(DropdownMenuSub, { children: [_jsxs(DropdownMenuSubTrigger, { children: [_jsx(InfoIcon, {}), t('sidebar.details')] }), _jsx(DropdownMenuSubContent, { className: "min-w-64 p-3", children: details })] }), getArchivedExportWorkspaceCwd(session) && (_jsxs(DropdownMenuItem, { disabled: exporting, onSelect: () => handleExportSession(session), children: [_jsx(DownloadIcon, {}), t('sidebar.export')] })), canMutateSessionArchive(session) && (_jsxs(DropdownMenuItem, { onSelect: () => handleUnarchive(session), children: [_jsx(ArchiveRestoreIcon, {}), t('sidebar.unarchive')] })), _jsxs(DropdownMenuItem, { variant: "destructive", onSelect: () => handleDeleteSession(session), children: [_jsx(Trash2Icon, {}), t('sidebar.delete')] })] }) })] }) })] })] }, sessionIdentity));
        }
        ...
        return (_jsx("div", { className: cx(styles.sessionRow, isCurrent && styles.currentSession, session.isPinned && styles.pinnedSession, session.hasActivePrompt && styles.runningSession, busy && styles.busySession), role: "button", tabIndex: 0, "aria-current": isCurrent ? 'page' : undefined, onClick: () => handleLoadSession(session.sessionId, session.workspaceCwd), onDoubleClick: () => {
                if (!readOnly && isCurrent && !collapsed)
                    startRename(session);
            }, onKeyDown: (event) => {
                if (event.key === 'Enter') {
                    handleLoadSession(session.sessionId, session.workspaceCwd);
                }
            }, children: !collapsed && (_jsxs(_Fragment, { children: [_jsx("span", { className: styles.sessionStatusSlot, children: completedUnread ? (_jsx("span", { className: styles.sessionStatusDot, "aria-hidden": "true" })) : null }), isEditing && !readOnly ? (_jsx("form", { className: styles.renameForm, onClick: (event) => event.stopPropagation(), onSubmit: (event) => {
                            event.preventDefault();
                            saveRename();
                            ...
                            } }) })) : (_jsxs(_Fragment, { children: [_jsx("span", { className: styles.sessionText, children: label }), _jsxs("div", { className: styles.sessionMetaSlot, children: [attentionLabel && (_jsx("span", { className: cx(styles.sessionAttention, needsUserInput && styles.sessionAttentionUserInput), "aria-label": attentionLabel, children: attentionLabel })), session.hasActivePrompt ? (_jsx("span", { className: styles.sessionLoading, "aria-label": t('sidebar.running') })) : !attentionLabel ? (_jsx("span", { className: styles.sessionTime, children: time })) : null, readOnly && canMutateSessionArchive(session) && (_jsx("div", { className: styles.sessionActions, onClick: (event) => event.stopPropagation(), onKeyDown: (event) => event.stopPropagation(), children: _jsx("button", { className: styles.sessionActionButton, type: "button", disabled: busy || isCurrent, "aria-label": t('sidebar.archive'), title: isCurrent
                                                ? t('sidebar.archiveCurrentDisabled')
                                                : t('sidebar.archive'), onClick: () => handleArchive(session), children: _jsx(ArchiveIcon, {}) }) })), !readOnly && (_jsxs("div", { className: styles.sessionActions, onClick: (event) => event.stopPropagation(), onKeyDown: (event) => event.stopPropagation(), children: [organizationEnabled && (_jsx("button", { className: cx(styles.sessionActionButton, session.isPinned &&
                                                    styles.activeSessionActionButton), type: "button", disabled: busy, "aria-label": session.isPinned
                                                    ? t('sidebar.unpin')
                                                    : t('sidebar.pin'), title: session.isPinned
                                                    ? t('sidebar.unpin')
                                                    : t('sidebar.pin'), onClick: () => handleTogglePin(session), children: _jsx(PinIcon, {}) })), canMutateSessionArchive(session) && (_jsx("button", { className: styles.sessionActionButton, type: "button", disabled: busy || isCurrent, "aria-label": t('sidebar.archive'), title: isCurrent
                                                    ? t('sidebar.archiveCurrentDisabled')
                                                    : t('sidebar.archive'), onClick: () => handleArchive(session), children: _jsx(ArchiveIcon, {}) })), _jsxs(DropdownMenu, { children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { className: styles.sessionActionButton, type: "button", "aria-label": t('sidebar.moreActions'), title: t('sidebar.moreActions'), children: _jsx(EllipsisVerticalIcon, {}) }) }), _jsx(DropdownMenuContent, { align: "end", className: "w-auto min-w-40", onPointerDownOutside: () => {
                                                            ...
                                                            sessionMenuPointerDismissRef.current = false;
                                                            event.preventDefault();
                                                        }, children: _jsxs(DropdownMenuGroup, { children: [_jsxs(DropdownMenuSub, { children: [_jsxs(DropdownMenuSubTrigger, { children: [_jsx(InfoIcon, {}), t('sidebar.details')] }), _jsx(DropdownMenuSubContent, { className: "min-w-64 p-3", children: details })] }), _jsxs(DropdownMenuItem, { disabled: !isCurrent, title: !isCurrent
                                                                        ? t('sidebar.renameCurrentOnly')
                                                                        : undefined, onSelect: () => handleRenameFromMenu(session), children: [_jsx(PencilIcon, {}), t('sidebar.rename')] }), organizationEnabled && (_jsxs(DropdownMenuItem, { disabled: busy, onSelect: (event) => openGroupMenuFromAnchor(event.currentTarget, session), children: [_jsx(FolderInputIcon, {}), t('sidebar.sessionGroup')] })), canExportSessions && (_jsxs(DropdownMenuItem, { disabled: exporting, onSelect: () => handleExportSession(session), children: [_jsx(DownloadIcon, {}), t('sidebar.export')] })), _jsxs(DropdownMenuItem, { variant: "destructive", disabled: isCurrent, title: isCurrent
                                                                        ? t('sidebar.currentDeleteDisabled')
                                                                        : undefined, onSelect: () => handleDeleteSession(session), children: [_jsx(Trash2Icon, {}), t('sidebar.delete')] })] }) })] })] }))] })] }))] })) }, sessionIdentity));
    }, [
        busySessionIds,
        canMutateSessionArchive,
        canExportSessions,
    ...
    const body = useMemo(() => {
        ...
        return sessionSections.map((section) => {
            ...
            return (_jsx(SessionGroupSection, { id: section.id, label: section.label, count: section.sessions.length, color: section.color, expanded: expanded, onToggle: () => toggleSessionSection(section.id), onRename: section.kind === 'group' && group
                    ? () => handleRenameGroup(group)
                    : undefined, onDelete: section.kind === 'group' && group
    ...
    const archivedSection = useMemo(() => {
        if (!sessionArchiveEnabled || collapsed || searchQuery.trim())
            return null;
        const header = (_jsxs("button", { type: "button", className: styles.archivedHeader, "aria-expanded": archivedExpanded, onClick: () => setArchivedExpanded((expanded) => !expanded), children: [_jsx("span", { className: styles.archivedTitle, style: { flex: '0 1 auto' }, children: t('sidebar.archivedTitle') }), _jsx("span", { className: styles.archivedChevron, "aria-hidden": "true", children: archivedExpanded ? _jsx(ChevronDownIcon, {}) : _jsx(ChevronRightIcon, {}) }), archivedExpanded && allArchivedSessions.length > 0 && (_jsx("span", { className: styles.archivedCount, children: allArchivedSessions.length }))] }));
        if (!archivedExpanded) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("aside", { className: cx(styles.sidebar, collapsed && styles.collapsed, isResizing && styles.resizing, mobileOpen && styles.mobileOpen), "aria-label": t('sidebar.label'), style: sidebarStyle, children: [groupMenu && (_jsxs("div", { ref: groupMenuRef, className: styles.groupMenu, role: "menu", "aria-label": t('sidebar.sessionGroup'), style: { top: groupMenu.top, left: groupMenu.left }, onClick: (event) => event.stopPropagation(), onKeyDown: handleGroupMenuKeyDown, onMouseDown: (event) => event.stopPropagation(), children: [_jsxs("button", { className: cx(styles.groupMenuItem, groupMenuUngroupedSelected && styles.groupMenuItemActive), type: "button", role: "menuitemradio", "aria-checked": groupMenuUngroupedSelected, onClick: () => assignSessionGroup(groupMenu.session, null), children: [_jsx("span", { className: styles.groupMenuEmptyDot }), _jsx("span", { className: styles.groupMenuName, children: t('sidebar.groupUngrouped') }), groupMenuUngroupedSelected && (_jsx("span", { className: styles.groupMenuCheck, children: "\u2713" }))] }), menuColorOptions.map((color) => {
                            ...
                            if (!groupBusy)
                                setDeleteGroupCandidate(null);
                                        ...
                                        name: deleteGroupCandidateLabel,
                                    }) }), _jsxs("div", { className: styles.confirmActions, children: [_jsx("button", { className: styles.secondaryButton, type: "button", disabled: groupBusy, onClick: () => setDeleteGroupCandidate(null), children: t('common.cancel') }), _jsx("button", { className: styles.dangerButton, type: "button", disabled: groupBusy, onClick: confirmDeleteGroup, children: t('sidebar.groupDelete') })] })] }) })), shouldRenderBrand && (_jsx("div", { className: styles.topRow, children: branding?.render ? (branding.render()) : (_jsxs(_Fragment, { children: [_jsx("span", { className: styles.brandLogo, "aria-hidden": "true", children: _jsx(IconQwenLogo, {}) }), !collapsed && (_jsx("span", { className: styles.brandName, children: "HopCode" }))] })) })), _jsxs("div", { className: styles.primaryNav, children: [_jsxs("button", { className: styles.newChatButton, type: "button", title: t('sidebar.newTask'), "aria-label": t('sidebar.newTask'), disabled: newSessionDisabled, onClick: () => handleNewSession(), children: [_jsx("span", { className: styles.navIcon, children: _jsx(SquarePenIcon, { size: 16, strokeWidth: 1.2 }) }), !collapsed && _jsx("span", { children: t('sidebar.newTask') })] }), _jsxs("button", { className: styles.pluginButton, type: "button", title: t('sidebar.plugins'), "aria-label": t('sidebar.plugins'), onClick: onOpenPlugins, children: [_jsx("span", { className: styles.navIcon, children: _jsx(BlocksIcon, { size: 16, strokeWidth: 1.2 }) }), !collapsed && _jsx("span", { children: t('sidebar.plugins') })] }), footerItems.has('scheduledTasks') && (_jsxs("button", { className: styles.pluginButton, type: "button", title: t('sidebar.scheduledTasks'), "aria-label": t('sidebar.scheduledTasks'), onClick: onOpenScheduledTasks, children: [_jsx("span", { className: styles.navIcon, children: _jsx(CalendarClockIcon, { size: 16, strokeWidth: 1.2 }) }), !collapsed && _jsx("span", { children: t('sidebar.scheduledTasks') })] })), footerItems.has('goals') && (_jsxs("button", { className: styles.pluginButton, type: "button", title: t('sidebar.goals'), "aria-label": t('sidebar.goals'), onClick: onOpenGoals, children: [_jsx("span", { className: styles.navIcon, children: _jsx(TargetIcon, { size: 16, strokeWidth: 1.2 }) }), !collapsed && _jsx("span", { children: t('sidebar.goals') })] }))] }), _jsx("div", { className: styles.body, children: _jsxs("div", { className: styles.sessionList, children: [!collapsed && pinnedSessions.length > 0 && (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.projectsHeader, children: _jsxs("button", { className: styles.projectsHeaderToggle, type: "button", "aria-expanded": pinnedExpanded, onClick: () => setPinnedExpanded((expanded) => !expanded), children: [_jsx("span", { children: t('sidebar.pinnedSessions') }), _jsx(IconChevron, { expanded: pinnedExpanded })] }) }), pinnedExpanded && (_jsx("div", { className: styles.pinnedSessionList, children: pinnedSessions.map((session) => renderSessionRow(session)) }))] })), !collapsed && (_jsxs("div", { className: styles.projectsHeader, children: [_jsxs("button", { className: styles.projectsHeaderToggle, type: "button", "aria-expanded": projectsExpanded, onClick: () => setProjectsExpanded((expanded) => !expanded), children: [_jsx("span", { children: t('sidebar.project') }), _jsx(IconChevron, { expanded: projectsExpanded })] }), _jsxs("div", { className: styles.projectsHeaderActions, children: [_jsx("button", { className: styles.projectsHeaderAction, type: "button", title: t('sidebar.search'), "aria-label": t('sidebar.search'), onClick: () => {
                                                        setSearchOpen((open) => {
                                                            if (open)
                                                                setSearchQuery('');
                                                            return !open;
                                                        ...
                                                        setProjectsExpanded(true);
                                                    }, children: _jsx(SearchIcon, {}) }), !lockedWorkspaceCwd && (_jsx("button", { className: styles.projectsHeaderAction, type: "button", title: t('sidebar.addWorkspace'), "aria-label": t('sidebar.addWorkspace'), onClick: () => {
                                                        setShowAddWorkspaceDialog(true);
                                                    }, children: _jsx(PlusIcon, {}) }))] })] })), searchOpen && !collapsed && (_jsxs("div", { className: styles.projectSearch, children: [_jsx(SearchIcon, { "aria-hidden": "true" }), _jsx(Input, { value: searchQuery, placeholder: t('sidebar.searchPlaceholder'), "aria-label": t('sidebar.search'), autoFocus: true, onChange: (event) => setSearchQuery(event.target.value), onKeyDown: (event) => {
                                                if (event.key === 'Escape') {
                                                    setSearchQuery('');
                                                    setSearchOpen(false);
                                                }
                                            } })] })), (collapsed || projectsExpanded) && (_jsx(_Fragment, { children: !collapsed && (_jsx("div", { className: styles.workspacePicker, children: _jsx("div", { className: styles.workspaceList, children: displayedWorkspaces.map((ws) => (_jsxs(Fragment, { children: [_jsx(WorkspaceSection, { workspace: ws, renderHeader: lockedWorkspaceCwd &&
                                                            lockedWorkspaceOptions?.render
                                                            ? (expanded) => lockedWorkspaceOptions.render?.(ws, {
                                                                expanded,
                                                            })
                                                            : undefined, client: workspace.client, reloadToken: workspaceSessionsReloadToken, untrustedLabel: t('sidebar.workspaceUntrusted'), readOnlyLabel: t('sidebar.workspaceReadOnly'), trustToOpenLabel: t('sidebar.workspaceTrustToOpen'), noSessionsLabel: t('sidebar.noSessions'), loadErrorLabel: t('sidebar.loadFailed'), organizationEnabled: organizationEnabled, ungroupedLabel: t('sidebar.groupUngrouped'), onRenameGroup: handleRenameGroup, onDeleteGroup: handleDeleteGroup, renameGroupLabel: t('sidebar.groupRename'), deleteGroupLabel: t('sidebar.groupDelete'), groupActionsDisabled: groupBusy, excludePinned: true, onOpenGitDiff: onOpenGitDiff, formatTime: (iso) => formatRelativeTime(iso, t), searchQuery: searchQuery, expanded: ws.primary ? projectExpanded : undefined, autoExpandKey: autoExpandWorkspace?.id === ws.id
                                                            ...
                                                            ...session,
                                                            workspaceCwd: ws.cwd,
                                                        }, { readOnly: !ws.primary }), headerActions: (visible) => {
                                                            ...
                                                            return (_jsxs("div", { className: styles.workspaceHeaderActions, style: {
                                                                    visibility: visible ? 'visible' : 'hidden',
                                                                }, children: [ws.trusted && (_jsxs(_Fragment, { children: [_jsx("button", { className: styles.workspaceHeaderAction, type: "button", "aria-label": t('sidebar.groupCreate'), onClick: (event) => {
                                                                                    event.preventDefault();
                                                                                    ...
                                                                                    event.preventDefault();
                                                                                    event.stopPropagation();
                                                                                    handleNewSession(ws.primary ? undefined : ws.cwd);
                                                                                        ...
                                                                                        workspaceRemovalCandidate?.id ===
                                                                                            ws.id, children: _jsx(EllipsisVerticalIcon, { size: 16, strokeWidth: 1.2 }) }) }), _jsx(DropdownMenuContent, { align: "end", children: _jsxs(DropdownMenuItem, { variant: "destructive", "aria-label": `${t('sidebar.removeWorkspace')}: ${ws.cwd}`, onSelect: () => requestWorkspaceRemoval(ws), children: [_jsx(Trash2Icon, {}), t('sidebar.removeWorkspace')] }) })] }))] }));
                                                        } }), ws.primary &&
