...
export const MessageItem = memo(function MessageItem({ message, pendingApproval, onShowContextDetail, workspaceCwd, isLatest = false, showRetryHint = false, onRetryClick, onBranchSession, showAssistantActions = false, showAssistantBranch = false, isLocateFlashing = false, assistantTurnFooterInfo, generateContent, }) {
    const { t } = useI18n();
    const body = (() => {
        switch (message.role) {
            case 'user':
                return (_jsx(UserMessage, { content: message.content, images: message.images, inputAnnotations: message.inputAnnotations, isLocateFlashing: isLocateFlashing }));
            case 'assistant':
    ...
    return (_jsx(MessageTimestamp, { timestamp: message.timestamp, chatMode: message.role === 'user', copyText: message.role === 'user' ? message.content : undefined, copyTitle: t('common.copy'), children: selectableSafeBody }));
