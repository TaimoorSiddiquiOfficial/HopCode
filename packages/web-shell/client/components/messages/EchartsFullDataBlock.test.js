...
import { ECHARTS_FULLDATA_SANITIZER_KEY_OVERLAP, EchartsFullDataBlock, createEchartsFullDataRenderer, } from './EchartsFullDataBlock';
...
async function render(node) {
    return (await mount(_jsx(I18nProvider, { language: "en", children: node })))
        .container;
}
...
async function renderEchartsMarkdown({ code, fenceLanguage = 'echarts-fulldata', loadEcharts, resolveDataRef, language = 'en', source = 'assistant', }) {
    return render(_jsx(I18nProvider, { language: language, children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: {
                    markdown: {
                ...
                }, children: _jsx(Markdown, { content: `\`\`\`${fenceLanguage}\n${code}\n\`\`\``, source: source }) }) }) }));
}
function getDataRows(container) {
    return Array.from(container.querySelectorAll('tbody tr')).map((row) => Array.from(row.querySelectorAll('td'))
        .slice(1)
...
describe('EchartsFullDataBlock', () => {
    ...
    it('does not render echarts blocks from thinking markdown', async () => {
        ...
        expect(container.querySelector('[data-testid="echarts-fulldata-rendered"]')).toBeNull();
    ...
    it('renders echarts blocks with case-insensitive fence language matching', async () => {
        ...
        expect(container.querySelector('[data-testid="echarts-fulldata-rendered"]')).not.toBeNull();
    ...
    it('renders chart/data icon switching from dataset-backed options', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => runtime }));
        await flushChart();
        expect(runtime.init).toHaveBeenCalledOnce();
        ...
        const renderedOption = setOption.mock.calls[0]?.[0];
        expect(renderedOption).toEqual(expect.not.objectContaining({ title: expect.anything() }));
        expect(renderedOption).toEqual(expect.objectContaining({
            backgroundColor: '#0d0d0d',
            color: expect.arrayContaining(['#8AA0FF', '#60CCC5']),
            grid: expect.objectContaining({ containLabel: true }),
        }));
        expect(renderedOption?.xAxis).toEqual(expect.objectContaining({
        ...
        expect(container.querySelector('section[aria-label="Weekly orders"]')).not.toBeNull();
        ...
        const headerTexts = Array.from(container.querySelectorAll('th')).map((cell) => cell.textContent?.trim() ?? '');
        expect(headerTexts[1]).toContain('day');
        expect(headerTexts[2]).toContain('orders');
        expect(container.textContent).toContain('Quick copy');
        expect(container.querySelector('button[aria-label="Sort by orders"]')).not.toBeNull();
        ...
        await act(async () => {
            container
                .querySelector('button[aria-label="Sort by orders"]')
                ?.click();
        });
        await act(async () => {
            container
                .querySelector('button[aria-label="Sort by orders, ascending"]')
                ?.click();
        });
        expect(getDataRows(container)).toEqual([
            ['Tue', '200'],
    ...
    it('uses item tooltip trigger for item-only series', async () => {
        ...
        await render(_jsx(EchartsFullDataBlock, { option: {
                dataset: {
            ...
            }, theme: "dark", loadEcharts: () => runtime }));
        await flushChart();
        const renderedOption = setOption.mock.calls[0]?.[0];
    ...
    it('rejects inline envelope row length mismatches', async () => {
        ...
        expect(container.textContent).toContain('Chart envelope data.source row 2 has 1 cells; expected 2.');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it('rejects inline envelope cells that cannot be rendered safely', async () => {
        ...
        expect(container.textContent).toContain('Chart envelope data.source row 1 cell 2 must be a string, number, boolean, or null.');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it.each([
        {
        ...
        },
        {
            ...
            error: 'Chart envelope data.dimensions contains an unsafe dimension name.',
        },
        {
    ...
    ])('rejects invalid full-data envelopes: $name', async ({ envelope, error }) => {
        ...
        expect(container.querySelector('pre code')).toBeNull();
        expect(console.warn).toHaveBeenCalledWith('[web-shell] echarts-fulldata parse failed: %s (code length=%d)', error, code.length);
    });
    it('rejects legacy array-row dimension mismatches', async () => {
        ...
        expect(container.textContent).toContain('Chart data row 2 has 1 cells; expected 2.');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it('rejects legacy dataset cells that cannot be rendered safely', async () => {
        ...
        expect(container.textContent).toContain('Chart data row 1 cell 2 must be a string, number, boolean, or null.');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it('rejects unsafe legacy dataset dimensions', async () => {
        ...
        expect(container.textContent).toContain('Chart envelope data.dimensions contains an unsafe dimension name.');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it('resolves ref envelope data through the host resolver', async () => {
        ...
        await flushChart();
        expect(resolveDataRef).toHaveBeenCalledWith('artifact://chart-data/orders', {
            format: 'csv',
            dimensions: ['day', 'orders'],
        });
        expect(setOption.mock.calls[0]?.[0]).toEqual(expect.objectContaining({
    ...
    it('does not refetch resolved ref data when streaming toggles with unchanged code', async () => {
        ...
        const tree = (isStreaming) => (_jsx(I18nProvider, { language: "en", children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock: renderer } }, children: _jsx(Markdown, { content: content, source: "assistant", isStreaming: isStreaming }) }) }) }));
    ...
    it('reports ref resolver rejections inside the chart card', async () => {
        ...
        await flushChart();
        expect(container.textContent).toContain('Chart data reference could not be resolved.');
        expect(container.textContent).not.toContain('missing artifact');
        expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata data-ref resolution failed:', 'artifact://chart-data/missing', expect.any(Error));
        expect(container.querySelector('pre code')).toBeNull();
    ...
    it('reports non-error ref resolver rejections with the generic message', async () => {
        ...
        expect(container.textContent).toContain('Chart data reference could not be resolved.');
        expect(container.textContent).not.toContain('missing artifact');
        expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata data-ref resolution failed:', 'artifact://chart-data/missing', 'missing artifact');
        expect(container.querySelector('pre code')).toBeNull();
    });
    it('reports invalid ref resolver data with a resolver-specific diagnostic', async () => {
        ...
        await flushChart();
        expect(container.textContent).toContain('Chart data resolver returned an invalid dataset.');
        expect(container.textContent).not.toContain('Chart envelope data.dimensions must be a string array.');
        expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata resolver returned invalid dataset:', 'artifact://chart-data/invalid', null);
        expect(setOption).not.toHaveBeenCalled();
    });
    it('reports ref resolver timeouts inside the chart card', async () => {
        ...
        try {
            ...
            expect(container.textContent).toContain('Chart data reference could not be resolved.');
            expect(container.textContent).not.toContain('Data reference resolution timed out.');
            expect(console.warn).toHaveBeenCalledWith('[web-shell] echarts-fulldata data-ref resolution timed out after %dms (ref=%s, format=%s)', 30_000, 'artifact://chart-data/slow', 'json');
            expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata data-ref resolution failed:', 'artifact://chart-data/slow', expect.any(Error));
            expect(container.querySelector('pre code')).toBeNull();
            expect(setOption).not.toHaveBeenCalled();
        }
        finally {
    ...
    it('ignores stale ref resolutions after the chart code changes', async () => {
        let resolveFirst = () => { };
        let resolveSecond = () => { };
        const resolveDataRef = vi.fn((ref) => new Promise((resolve) => {
        ...
        const contentFor = (ref) => `\`\`\`echarts-fulldata\n${JSON.stringify({
            version: 1,
            data: {
                kind: 'ref',
                ref,
        ...
        const tree = (ref) => (_jsx(I18nProvider, { language: "en", children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock: renderer } }, children: _jsx(Markdown, { content: contentFor(ref), source: "assistant" }) }) }) }));
    ...
    it('does not re-resolve ref data when the resolver prop identity changes', async () => {
        ...
        const tree = (nonce) => (_jsx(I18nProvider, { language: "en", children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: {
                        markdown: {
                            renderCodeBlock: createEchartsFullDataRenderer({
                                loadEcharts: () => runtime,
    ...
    it('reports missing and unsupported ref resolver states without showing raw code', async () => {
        ...
        expect(missingResolver.textContent).toContain('Chart data reference resolver is unavailable.');
    ...
    it('validates ref envelopes before calling the host resolver', async () => {
        ...
        const invalidRefs = [
            ...
            },
            {
                ...
                message: 'Chart envelope data.ref path must use non-empty normalized segments and cannot contain ".", "..", or drive-qualified segments.',
            },
            {
            ...
            },
            {
                ...
                message: 'Chart envelope data.ref path must not include query, hash, or backslash characters.',
            },
            {
                ref: 'artifact://chart-data/%252e%252e/orders',
                message: 'Chart envelope data.ref path must not include query, hash, whitespace, control, backslash, or double-encoded percent characters.',
            },
            {
                ref: 'artifact://chart-data/\u0000orders',
                message: 'Chart envelope data.ref must not contain whitespace or control characters.',
            },
        ];
    ...
    it('rejects resolved ref data over the dataset limits', async () => {
        ...
        await flushChart();
        expect(container.textContent).toContain('Chart data has too many rows. Maximum supported rows: 2000.');
        expect(setOption).not.toHaveBeenCalled();
    });
    it('normalizes {c} label templates for object-row datasets', async () => {
        ...
        await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => runtime }));
        await flushChart();
        const renderedOption = setOption.mock.calls[0]?.[0];
        expect(renderedOption.series?.[0]?.label?.formatter).toBe('{@convRate|.2f}%');
        expect(renderedOption.series?.[1]?.label?.formatter).toBe('¥{@aov}');
        expect(renderedOption.series?.[2]?.label?.formatter).toBe('Cost {@cost$|.1f}');
    ...
    it('keeps the parsed option stable across parent rerenders with unchanged code', async () => {
        ...
        const content = `\`\`\`echarts-fulldata\n${code}\n\`\`\``;
        const tree = (nonce) => (_jsx("div", { "data-nonce": nonce, children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: {
                        markdown: {
    ...
    it('reinitializes the chart when the theme changes', async () => {
        ...
        const tree = (theme) => (_jsx(I18nProvider, { language: "en", children: _jsx(EchartsFullDataBlock, { option: option, theme: theme, loadEcharts: () => runtime }) }));
        ...
        expect(cloneCount).toBe(1);
        expect(runtime.init).toHaveBeenCalledTimes(2);
        expect(runtime.init).toHaveBeenNthCalledWith(1, expect.any(HTMLElement), 'dark');
        expect(runtime.init).toHaveBeenNthCalledWith(2, expect.any(HTMLElement), 'light');
        expect(dispose).toHaveBeenCalledOnce();
        expect(setOption).toHaveBeenCalledTimes(2);
        expect(setOption.mock.calls[0]?.[0]).toEqual(expect.objectContaining({ backgroundColor: '#0d0d0d' }));
        expect(setOption.mock.calls[1]?.[0]).toEqual(expect.objectContaining({ backgroundColor: '#ffffff' }));
    });
    it('shows loading while the chart runtime loader is pending', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => runtimePromise }));
    ...
    it('reports chart runtime loader timeouts instead of spinning forever', async () => {
        ...
        try {
            const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => new Promise(() => { }) }));
            expect(container.querySelector('[role="status"]')).not.toBeNull();
            await act(async () => {
                vi.advanceTimersByTime(30_000);
                await Promise.resolve();
            ...
            expect(console.warn).toHaveBeenCalledWith('[web-shell] echarts-fulldata runtime load timed out after %dms', 30_000);
            expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata render failed:', expect.any(Error));
            expect(container.querySelector('[role="status"]')).toBeNull();
        }
        finally {
            vi.useRealTimers();
    ...
    it('falls back to window resize listener when ResizeObserver is unavailable', async () => {
        ...
        const tree = (nextOption) => (_jsx(I18nProvider, { language: "en", children: _jsx(EchartsFullDataBlock, { option: nextOption, theme: "dark", loadEcharts: () => runtime }) }));
        try {
    ...
    it('reports synchronous loader failures inside the chart card', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => {
    ...
    it('reports async loader rejections inside the chart card', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark", loadEcharts: () => Promise.reject(new Error('async loader failed')) }));
        await flushChart();
    ...
    it('reports chart option sanitization failures inside the chart card', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: {
                dataset: {
                    dimensions: ['value'],
                    source: [[BigInt(1)]],
                },
        ...
        expect(setOption).not.toHaveBeenCalled();
        expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata render failed:', expect.any(TypeError));
    });
    it('recovers from chart errors after option changes', async () => {
        ...
        const { container, root } = await mount(_jsx(I18nProvider, { language: "en", children: _jsx(EchartsFullDataBlock, { option: baseOption, theme: "dark", loadEcharts: () => runtime }) }));
        await flushChart();
        expect(container.textContent).toContain('setOption failed');
        expect(container.querySelector('[data-testid="echarts-fulldata-chart"]')).not.toBeNull();
        await act(async () => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(EchartsFullDataBlock, { option: nextOption, theme: "dark", loadEcharts: () => runtime }) }));
        });
        await flushChart();
    ...
    it('clears stale chart errors while ref data is pending after option changes', async () => {
        ...
        let resolveRef = () => { };
        const resolveDataRef = vi.fn(() => new Promise((resolve) => {
            resolveRef = resolve;
        }));
        const setOption = vi
            .fn()
        ...
        const tree = (content) => (_jsx(I18nProvider, { language: "en", children: _jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock: renderer } }, children: _jsx(Markdown, { content: content, source: "assistant" }) }) }) }));
        ...
        expect(resolveDataRef).toHaveBeenCalledOnce();
        expect(container.textContent).not.toContain('setOption failed');
        expect(container.querySelector('[role="status"]')?.getAttribute('aria-label')).toBe('Rendering chart');
        await act(async () => {
            resolveRef({
                dimensions: ['day', 'orders'],
                source: [['Tue', 200]],
    ...
    it('sanitizes unsafe chart option fields before calling ECharts', async () => {
        ...
        expect(Object.getPrototypeOf(renderedOption.dataset?.[0]?.source?.[0])).toBe(Object.prototype);
        expect(Object.prototype.hasOwnProperty.call(renderedOption.dataset?.[0]?.source?.[0], '__proto__')).toBe(false);
        ...
        expect(renderedOption.tooltip).toEqual(expect.objectContaining({
            enterable: false,
            renderMode: 'richText',
        }));
        ...
        expect(renderedOption.series?.[0]?.markArea?.data?.[0]?.[0]?.xAxis).toBe('Mon');
        expect(renderedOption.series?.[0]?.markArea?.itemStyle?.color).toBeUndefined();
        expect(renderedOption.series?.[0]?.markLine?.data?.[0]?.yAxis).toBe(150);
        expect(renderedOption.series?.[0]?.markLine?.data?.[0]?.label?.formatter).toBe('Goal <Q4 2024>');
        expect(renderedOption.series?.[0]?.markLine?.data?.[0]?.lineStyle?.color).toBeUndefined();
        expect(renderedOption.series?.[0]?.name).toBeUndefined();
        expect(renderedOption.series?.[0]?.polluted).toBeUndefined();
        expect(Object.getPrototypeOf(renderedOption.series?.[0])).toBe(Object.prototype);
        ...
        expect(renderedOption.series?.[0]?.tooltip).toEqual(expect.objectContaining({
            appendToBody: false,
            confine: true,
            enterable: false,
            renderMode: 'richText',
        ...
        expect(renderedOption.series?.[1]?.label?.formatter).toBe('Results <Q4 2024>');
    });
    it('shows an error when sanitization strips every renderable chart entry', async () => {
        ...
        await flushChart();
        expect(container.textContent).toContain('Sanitized chart has no series or dataset; option keys may have been stripped.');
        expect(consoleError).toHaveBeenCalledWith('[web-shell] echarts-fulldata render failed:', expect.any(Error));
    });
    ...
    it('uses the default title when a single title object has empty text', async () => {
        ...
        const container = await render(_jsx(EchartsFullDataBlock, { option: option, theme: "dark" }));
    ...
    it('localizes chart chrome strings', async () => {
        ...
        const { container } = await mount(_jsx(I18nProvider, { language: "zh-CN", children: _jsx(EchartsFullDataBlock, { option: option, theme: "dark" }) }));
        await flushChart();
        expect(container
            .querySelector('[data-testid="echarts-fulldata-rendered"]')
            ?.getAttribute('aria-label')).toBe('图表加载中');
        expect(container.textContent).toContain('图表运行时不可用。');
        expect(container.querySelector('button[aria-label="显示图表"]')).not.toBeNull();
        expect(container.querySelector('button[aria-label="显示数据"]')).not.toBeNull();
    ...
    it('rejects chart data over the cell limit', async () => {
        ...
        expect(container.textContent).toContain('Chart data has too many cells. Maximum supported cells: 40000.');
    });
    it('rejects chart data over the series limit', async () => {
        ...
        expect(container.textContent).toContain('Chart data has too many series. Maximum supported series: 100.');
    });
    ...
    it('rejects chart code over the size limit before parsing', async () => {
        ...
        expect(container.textContent).toContain('Chart data is too large. Maximum supported size: 500000 characters.');
    });
    it('rejects chart data without dataset source rows', async () => {
        ...
        expect(container.textContent).toContain('Chart data must include dataset.source.');
    });
    it('rejects syntactically valid JSON that is not an option object', async () => {
        const container = await render(_jsx(ThemeProvider, { value: "dark", children: _jsx(WebShellCustomizationProvider, { value: {
                    markdown: {
                        renderCodeBlock: createEchartsFullDataRenderer(),
                    },
                }, children: _jsx(Markdown, { content: '```echarts-fulldata\nnull\n```', source: "assistant" }) }) }));
        expect(container.textContent).toContain('Chart data must be a JSON object.');
        expect(container.querySelector('pre code')).toBeNull();
    });
...
//# sourceMappingURL=EchartsFullDataBlock.test.js.map