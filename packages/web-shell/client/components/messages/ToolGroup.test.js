...
const { buildUnifiedDiff, extractDiff, fencedCodeBlock, formatSingleToolSummary, formatToolGroupSummary, getActiveTool, getRawFileDiff, getToolHeaderKind, hasActiveTool, hasExpandableContent, isActiveToolStatus, isWebFetchToolName, languageForPath, shouldAutoExpand, ToolGroup, ToolLine, } = await import('./ToolGroup');
...
function renderToolLine(tool, props = {}) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ToolLine, { tool: tool, ...props }) }));
    });
...
const t = (key, values) => {
    if (key === 'toolGroup.running') {
        return `Running ${values?.name ?? 'tool'}${values?.duration ? ` ${values.duration}` : ''}${Number(values?.count ?? 0) > 1 ? ` · ${values?.count ?? 0} tools` : ''}`;
    }
