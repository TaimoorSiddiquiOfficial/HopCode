...
function rerender(keyboardActive, req = request) {
    act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(AskUserQuestion, { request: req, onConfirm: onConfirm, keyboardActive: keyboardActive }) })));
}
...
function submitButton() {
    return (Array.from(container.querySelectorAll('button')).find((b) => b.textContent === 'Submit' || b.textContent === '提交') ?? null);
}
...
describe('AskUserQuestion accessibility', () => {
    ...
    it('ignores option shortcuts when focus is on an action button, not an option', () => {
        ...
        act(() => submit.dispatchEvent(new KeyboardEvent('keydown', {
            key: 'Escape',
            bubbles: true,
            cancelable: true,
        })));
