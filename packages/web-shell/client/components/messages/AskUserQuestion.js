...
export function AskUserQuestion({ request, onConfirm, variant = 'inline', keyboardActive = true, }) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        setAnswers(firstQuestion && !firstQuestion.multiSelect && firstQuestion.options[0]
            ...
            : {});
        setCustomInputs({});
        setSelectedMulti(firstQuestion?.multiSelect && firstQuestion.options[0]
            ? { 0: [firstQuestion.options[0].label] }
            : {});
    ...
    const handleKeyDown = useCallback((e) => {
        ...
        if (!e.target.closest('[data-web-shell-ask-option]')) {
            return;
        }
        if (!current)
            return;
    ...
    useEffect(() => {
        ...
        if (!keyboardActive)
            return;
        if (wasActive && request.id === prevRequestId)
            return;
        const idx = selectedIdxRef.current ?? 0;
        if (idx === optionCountRef.current)
            customRef.current?.focus();
        else
            optionRefs.current[idx]?.focus();
    ...
    const selectQuestion = (nextIdx) => {
        ...
        if (question.multiSelect) {
            setSelectedMulti((prev) => (prev[nextIdx] || []).length > 0 || customInputs[nextIdx]
                ? prev
                : { ...prev, [nextIdx]: [question.options[0].label] });
            return;
        }
        setAnswers((prev) => prev[nextIdx] || customInputs[nextIdx]
            ? prev
            : { ...prev, [nextIdx]: question.options[0].label });
    };
    ...
    return (_jsxs("div", { className: `${styles.question} ${variant === 'floating' ? styles.floating : ''} ${collapsed ? styles.collapsed : ''}`, "data-web-shell-ask-panel": true, role: "alertdialog", "aria-label": localizeToolDisplayName('ask_user_question', t), "aria-labelledby": collapsed ? undefined : `${headingId} ${questionTextId}`, onKeyDown: handleKeyDown, children: [_jsxs("div", { className: styles.titleLine, children: [_jsx("span", { className: styles.icon, "aria-hidden": "true", children: "?" }), _jsx("span", { className: styles.toolName, id: headingId, children: localizeToolDisplayName('ask_user_question', t) }), _jsx("span", { className: styles.toolDesc, children: t('askUser.progress', {
                            current: displayIdx + 1,
                            total: questions.length,
                        }) }), _jsxs("div", { className: styles.topRight, children: [_jsx("div", { className: styles.tabs, children: questions.map((_, i) => (_jsx("span", { className: `${styles.tab} ${i === currentIdx ? styles.tabActive : ''}`, "aria-hidden": "true" }, i))) }), _jsx("button", { type: "button", className: styles.collapseButton, onClick: () => setCollapsed((c) => !c), "aria-expanded": !collapsed, "aria-label": collapsed ? t('common.expand') : t('common.collapse'), title: collapsed ? t('common.expand') : t('common.collapse'), children: _jsx("svg", { viewBox: "0 0 16 16", className: `${styles.collapseIcon} ${collapsed ? styles.collapseIconCollapsed : ''}`, children: _jsx("path", { d: "M4 6l4 4 4-4", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5" }) }) })] })] }), !collapsed && (_jsxs(_Fragment, { children: [current ? (_jsxs(_Fragment, { children: [_jsxs("p", { className: styles.text, id: questionTextId, children: [current.question, isMulti && (_jsxs("span", { className: styles.multiHint, children: [' ', "(", t('askUser.multiHint'), ")"] }))] }), _jsx("p", { className: styles.description, children: t('askUser.selectAnswer') }), _jsxs("div", { className: styles.options, role: isMulti ? 'group' : 'radiogroup', "aria-labelledby": questionTextId, children: [current.options.map((opt, i) => {
                                        ...
                                        return (_jsxs("button", { type: "button", ref: (el) => {
                                            ...
                                            }, className: `${styles.option} ${isActive ? styles.optionActive : ''} ${isSelected ? styles.optionSelected : ''}`, "data-web-shell-ask-option": true, tabIndex: isActive ? 0 : -1, role: isMulti ? undefined : 'radio', "aria-checked": isMulti ? undefined : isSelected, "aria-pressed": isMulti ? isSelected : undefined, "aria-keyshortcuts": i < 9 ? String(i + 1) : undefined, onClick: () => chooseOption(i), onFocus: () => setSelectedIdx(i), children: [_jsx("span", { className: styles.pointer, "aria-hidden": "true", children: isActive ? '›' : ' ' }), _jsx("span", { className: styles.optionNum, "aria-hidden": "true", children: i + 1 }), _jsxs("span", { className: styles.optionContent, children: [_jsx("span", { className: styles.optionLabel, children: opt.label }), opt.description && (_jsx("span", { className: styles.optionDesc, children: opt.description }))] })] }, opt.label));
                                    }), (() => {
                                        const isCustomActive = selectedIdx === current.options.length;
                                        const hasCustomValue = !!customInputs[currentIdx];
                                        return (_jsxs("div", { className: `${styles.option} ${isCustomActive ? styles.optionActive : ''} ${hasCustomValue ? styles.optionSelected : ''}`, 
                                            ...
                                            onClick: () => chooseOption(current.options.length), children: [_jsx("span", { className: styles.pointer, "aria-hidden": "true", children: isCustomActive ? '›' : ' ' }), _jsx("span", { className: styles.editIcon, "aria-hidden": "true", children: _jsx("svg", { viewBox: "0 0 16 16", children: _jsx("path", { d: "M3.2 10.9 4 7.8 10.8 1l3.2 3.2-6.8 6.8-3 .8zM10 1.8l3.2 3.2M3 14h10", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }) }) }), customFocused ? (_jsx("input", { type: "text", className: styles.customInput, placeholder: t('askUser.typePlaceholder'), value: customInputs[currentIdx] || '', "aria-label": t('askUser.typePlaceholder'), onChange: (e) => setCustomInputs({
                                                        ...customInputs,
                                                    ...
                                                    // option choice.
                                                    onClick: (e) => e.stopPropagation(), onFocus: () => setSelectedIdx(current.options.length), onBlur: () => setCustomFocused(false), autoFocus: true })) : (_jsx("button", { type: "button", ref: customRef, className: `${styles.customTrigger} ${customInputs[currentIdx]
                                                        ? ''
                                                        : styles.optionPlaceholder}`, "data-web-shell-ask-option": true, tabIndex: isCustomActive ? 0 : -1, role: isMulti ? undefined : 'radio', "aria-checked": isMulti ? undefined : hasCustomValue, "aria-pressed": isMulti ? hasCustomValue : undefined, "aria-keyshortcuts": current.options.length < 9
                                                        ? String(current.options.length + 1)
                                                        : undefined, onFocus: () => setSelectedIdx(current.options.length), children: customInputs[currentIdx] ||
                                                        t('askUser.typePlaceholder') }))] }));
                                    })()] })] })) : null, _jsxs("div", { className: styles.actions, children: [_jsx("button", { type: "button", className: styles.ignoreButton, onClick: handleCancel, children: t('askUser.ignore') }), questions.length > 1 && (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: styles.button, disabled: currentIdx <= 0, onClick: handlePrevious, children: t('common.previous') }), _jsx("button", { type: "button", className: styles.button, disabled: currentIdx >= questions.length - 1, onClick: handleNext, children: t('common.next') })] })), _jsx("button", { type: "button", className: `${styles.button} ${styles.submitButton}`, disabled: !canSubmit, onClick: handleSubmit, children: t('askUser.submit') })] })] }))] }));
}
