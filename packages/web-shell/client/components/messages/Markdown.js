...
function CodeBlock({ className, children, isStreaming, }) {
    ...
    useEffect(() => {
        ...
        const warmHtml = highlightToHtmlSync(code, resolvedLang, shikiTheme, persist);
        if (warmHtml !== null) {
            setHtml(warmHtml);
            return;
        }
        ...
        getCodeHighlighter(resolvedLang)
            ...
            if (cancelled)
                return;
            const cold = highlightToHtmlSync(code, resolvedLang, shikiTheme, persist);
            if (cold !== null)
                setHtml(cold);
        })
            .catch((err) => {
    ...
    return (_jsxs("div", { className: styles.codeBlock, children: [_jsxs("div", { className: styles.codeBlockHeader, children: [_jsx("span", { className: styles.codeBlockLang, children: label }), _jsx("button", { className: styles.codeBlockCopy, onClick: handleCopy, children: copied ? t('code.copied') : t('code.copy') })] }), html !== null ? (_jsx("div", { className: styles.codeBlockContent, dangerouslySetInnerHTML: { __html: html } })) : (_jsx("pre", { className: `${styles.codeBlockContent} ${styles.codeBlockPlain}`, children: _jsx("code", { children: code }) }))] }));
}
...
function MarkdownCode({ className, children, }) {
    ...
    if (isBlock) {
        return (_jsx(MarkdownFencedCode, { className: className, isStreaming: isStreaming, children: children }));
    }
    return _jsx(InlineCode, { children: children });
}
function MarkdownFencedCode({ className, children, isStreaming, }) {
...
export const Markdown = memo(function Markdown({ content, source, isStreaming, tableMode, }) {
    ...
    const renderedContent = content && source && sourceMarkdown?.transformMarkdown
    ...
    const rehypePlugins = sourceMarkdown?.rehypePlugins
        ...
        : [rehypeKatex];
    return (_jsx("div", { className: source !== 'thinking' ? styles.content : undefined, "data-markdown-source": source, children: _jsx(IsStreamingContext.Provider, { value: !!isStreaming, children: _jsx(MarkdownSourceContext.Provider, { value: source, children: _jsx(ReactMarkdown, { remarkPlugins: remarkPlugins, rehypePlugins: rehypePlugins, components: renderedComponents, urlTransform: markdownUrlTransform, children: renderedContent }) }) }) }));
});
