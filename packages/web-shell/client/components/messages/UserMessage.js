...
import { getComposerTagIconUrl, getComposerTagViewModel, isBuiltinComposerTagIconUrl, splitComposerTagContentByAnnotations, } from '../../utils/composerTag';
...
export const UserMessage = memo(function UserMessage({ content, images, inputAnnotations, isLocateFlashing = false, }) {
    ...
    const renderedContent = useMemo(() => {
        ...
        return parts.map((part, index) => {
            if (part.type === 'text')
                return part.text;
            return (_jsx(UserMessageTag, { tag: part.tag, composerTagIcons: composerTagIcons, renderComposerTag: renderComposerTag, renderComposerTagTooltip: renderComposerTagTooltip, onComposerTagClick: onComposerTagClick }, `${part.tag.id}-${index}`));
        });
    }, [
        content,
        images,
    ...
    return (_jsx("div", { className: styles.chatMessageRow, children: _jsxs("div", { className: `${styles.chatBubble}${isLocateFlashing ? ` ${flashStyles.flash}` : ''}`, children: [_jsxs("div", { ref: contentRef, className: `${styles.chatContent} ${heightOverflowing && !expanded ? styles.chatContentCollapsed : ''}`, children: [images && images.length > 0 && (_jsx("div", { className: styles.chatImages, children: images.map((img, index) => {
                                const src = img.data.startsWith('data:')
                                    ? img.data
                                    : `data:${img.mimeType};base64,${img.data}`;
                                if (!isSafeImageSrc(src))
                                    return null;
                                return (_jsx("img", { src: src, alt: t('user.uploadedImage', { index: index + 1 }), className: styles.chatImageThumb, onLoad: measureOverflow }, index));
                            }) })), renderedContent] }), heightOverflowing && (_jsxs("button", { type: "button", className: styles.toggleButton, onClick: () => setExpanded((value) => !value), children: [_jsx("span", { children: expanded ? t('userMessage.showLess') : t('userMessage.showMore') }), _jsx("svg", { className: `${styles.toggleIcon} ${expanded ? styles.toggleIconExpanded : ''}`, viewBox: "0 0 16 16", "aria-hidden": "true", children: _jsx("path", { d: "m4 6 4 4 4-4", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }) })] }))] }) }));
});
function getTagText(tag) {
    return getComposerTagDisplay(tag);
}
function UserMessageTag({ tag, composerTagIcons, renderComposerTag, renderComposerTagTooltip, onComposerTagClick, title, preserveCustomKindLabel = false, }) {
