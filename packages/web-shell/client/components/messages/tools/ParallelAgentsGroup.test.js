...
const { computeAgentsTimeline, ParallelAgentsGroup } = await import('./ParallelAgentsGroup');
...
describe('computeAgentsTimeline', () => {
    it('returns null for a single agent or missing start times', () => {
        expect(computeAgentsTimeline([agent({ startTime: 0, endTime: 5_000 })], 10_000)).toBeNull();
        expect(computeAgentsTimeline([
