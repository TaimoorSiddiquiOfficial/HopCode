...
vi.mock('./MessageList', () => ({
    MessageList: (props) => (_jsx("div", { "data-testid": "pane-messages", "data-approval": props.pendingApproval ? 'yes' : 'no', children: props.messages.length })),
}));
...
function render(props = {}) {
    ...
    act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ChatPane, { ...props }) })));
}
function testid(id) {
...
describe('ChatPane', () => {
    ...
    it('reports loaded pane artifacts to the outer panel owner', async () => {
        ...
        expect(onPaneArtifactsChange).toHaveBeenLastCalledWith('sess-1', [artifact], expect.any(Object));
    });
    ...
    it('forwards composer annotations with a queued prompt', () => {
        ...
        expect(enqueuePrompt).toHaveBeenCalledWith('queue @.husky/', undefined, undefined, inputAnnotations);
        expect(sendPrompt).not.toHaveBeenCalled();
    });
    ...
    it('reports an idle prompt failure to the pane error handler', async () => {
        ...
        expect(commit).not.toHaveBeenCalled();
        expect(clearFollowup).not.toHaveBeenCalled();
        expect(onError).toHaveBeenCalledWith(expect.any(Error), 'Failed to send prompt');
    });
