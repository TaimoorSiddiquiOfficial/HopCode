...
import { useConnection, useStatusReport, } from '@hoptrendy/webui/daemon-react-sdk';
import { useI18n } from '../i18n';
import { formatRelativeTime } from '../utils/formatRelativeTime';
import { buildSplitUrl, MAX_SPLIT_PANES } from '../utils/splitUrl';
import { hasMultipleWorkspaces, isNonPrimaryWorkspaceSession, mergeSessionsById, workspaceBasename, } from '../utils/workspace';
...
function SessionOverviewPanelInner({ onOpenSession, onOpenSplit, includeOtherWorkspaces, workspaceCwd, }) {
    ...
    useEffect(() => {
        const timer = window.setInterval(() => {
            if (document.hidden || listInFlight.current)
                return;
