...
function extension(updateState) {
    return {
        kind: 'extension',
        id: 'demo',
        name: 'demo',
...
function renderPage() {
    root?.render(_jsx(I18nProvider, { language: "en", children: _jsx(ExtensionsManagerPage, { onClose: vi.fn() }) }));
}
async function mount(extensions = [], activeOperations = []) {
...
describe('ExtensionsManagerPage', () => {
    it('reports recovery failures and clears the error after retry', async () => {
        ...
        try {
            await mount();
            expect(document.body.textContent).toContain('Could not recover operations');
            ...
            expect(document.body.textContent).not.toContain('Could not recover operations');
    ...
    it('keeps the add dialog open until the install request is accepted', async () => {
        let acceptInstall;
        actions.installExtension.mockImplementation(() => new Promise((resolve) => {
    ...
    it('closes a failed interaction and resumes polling the install', async () => {
        ...
        actions.respondToExtensionInteraction.mockRejectedValue(new Error('Interaction expired'));
        await mount();
        await startInstall();
        changeInput(document.querySelector('input[aria-label="API key"]'), 'secret');
        act(() => {
            document
        ...
        await flush();
        expect(actions.respondToExtensionInteraction).toHaveBeenCalledWith('op-1', 'interaction-1', { value: 'secret' }, 'client-1');
        expect(actions.extensionOperationStatus).toHaveBeenCalledTimes(2);
        expect(document.body.textContent).toContain('Interaction expired');
        expect(document.querySelector('input[aria-label="Second API key"]')?.value).toBe('');
    });
    it('submits a marketplace plugin selection while installing', async () => {
        ...
        click(buttonIncluding('Install'));
        await flush();
        expect(actions.respondToExtensionInteraction).toHaveBeenCalledWith('op-marketplace', 'interaction-marketplace', { pluginName: 'example-plugin' }, 'client-1');
        expect(actions.extensionOperationStatus).toHaveBeenCalledTimes(2);
    });
    it('keeps polling while an interaction is waiting', async () => {
        ...
        try {
            ...
            await flush();
            expect(actions.extensionOperationStatus).toHaveBeenCalledTimes(2);
            expect(document.body.textContent).toContain('Extension interaction timed out');
            expect(buttonIncluding('Add Extension')?.disabled).toBe(false);
        }
        finally {
            vi.useRealTimers();
    ...
    it('stops tracking an operation that is no longer on the daemon', async () => {
        ...
        actions.extensionOperationStatus.mockRejectedValue(new DaemonHttpError(404, {}, 'Operation not found'));
        await mount();
        await startInstall();
    ...
    it('updates an extension without a session client id', async () => {
        ...
        await flush();
        pointerDown(document.querySelector('button[aria-label="Extension actions"]') ??
            undefined);
        await flush();
        click(elementIncluding('[data-slot="dropdown-menu-item"]', 'Update Extension'));
        await flush();
        ...
        await flush();
        expect(actions.respondToExtensionInteraction).toHaveBeenCalledWith('op-update', 'interaction-update', { value: '' }, undefined);
        expect(actions.extensionOperationStatus).toHaveBeenCalledTimes(2);
        expect(document.body.textContent).not.toContain('Wait for the session to connect');
    });
    it('keeps uninstall progress on the detail page and returns silently to the list', async () => {
        vi.useFakeTimers();
        let acceptUninstall;
        actions.uninstallExtension.mockImplementation(() => new Promise((resolve) => {
        ...
        try {
            ...
            pointerDown(document.querySelector('button[aria-label="Extension actions"]') ??
                undefined);
            await flush();
            click(elementIncluding('[data-slot="dropdown-menu-item"]', 'Uninstall Extension'));
            await flush();
            click(buttonIncluding('Uninstall Extension'));
            await flush();
            expect(document.querySelector('h1')?.textContent).toContain('Demo');
            expect(document.body.textContent).toContain('Uninstalling extension "demo"');
            expect(document.querySelector('button[aria-label="Extension actions"]')?.disabled).toBe(true);
            ...
            await flush();
            expect(document.querySelector('h1')?.textContent).toContain('Manage Extensions');
            expect(document.body.textContent).not.toContain('Extension "demo" uninstalled.');
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('shows disable progress and disables detail actions', async () => {
        ...
        pointerDown(document.querySelector('button[aria-label="Extension actions"]') ??
            undefined);
        await flush();
        click(elementIncluding('[data-slot="dropdown-menu-item"]', 'Disable Extension'));
        await flush();
    ...
    it('shows the extension description only once on the detail page', async () => {
        ...
        click(document.querySelector('[data-slot="card"]') ?? undefined);
        await flush();
        expect(document.body.textContent?.match(/A single extension description/g)).toHaveLength(1);
    });
    it('keeps only the status beside the card title without a footer', async () => {
        await mount([extension()]);
        const card = document.querySelector('[data-slot="card"]');
        const titleRow = card?.querySelector('[data-slot="card-title"]')?.parentElement;
        ...
        expect(card
            ...
            ?.classList.contains('truncate')).toBe(true);
    });
...
//# sourceMappingURL=ExtensionsManagerPage.test.js.map