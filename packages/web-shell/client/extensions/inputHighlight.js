...
function buildDecorations(view, getCommands, getLanguage) {
    ...
    for (let i = 1; i <= doc.lines; i++) {
        ...
        const argumentHint = getSlashCommandArgumentHint(text, getCommands(), getLanguage());
        if (argumentHint) {
    ...
    return ViewPlugin.fromClass(class {
        ...
        update(update) {
            if (update.docChanged || update.viewportChanged) {
                this.decorations = buildDecorations(update.view, getCommands, getLanguage);
            }
        }
    }, {
