...
function createFileProvider(getActions, getCurrentDir, getCache, label, description) {
    return {
        id: FILE_PROVIDER_ID,
        label,
        description,
...
export function useAtMentionMenu({ viewRef, disabledRef, shellModeRef, workspaceActionsRef, workspaceKey, builtinProviders, providers = EMPTY_PROVIDERS, createInlineTagEffect, }) {
    const { t } = useI18n();
    ...
    const allProviders = useMemo(() => {
        const builtinAtProviders = [
            createFileProvider(() => workspaceActionsRef.current, () => fileDirectoryRef.current, () => builtinCacheRef.current, t('at.category.files'), t('at.category.files.description')),
        ...
        Promise.resolve()
            ...
            setMenu((prev) => {
                if (!prev || prev.level !== 'items')
                    return prev;
                const maxItems = providerId === FILE_PROVIDER_ID && query.length === 0
