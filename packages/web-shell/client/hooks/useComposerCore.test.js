...
import { buildComposerPrompt, buildComposerPromptWithInlineTagPlacements, createLargePastePlaceholder, expandLargePastePlaceholders, getComposerTagDisplay, getComposerTagLabel, getComposerTagValue, getFollowupCompletion, isLargePaste, normalizePastedText, prunePendingPastes, replaceInlineTagPlacements, serializeComposerTag, } from './useComposerCore';
...
describe('large paste placeholders', () => {
    it('creates stable placeholders and increments duplicate labels', () => {
        ...
        const second = createLargePastePlaceholder(pending, first.nextPasteId, 'def');
    ...
    it('expands longer placeholder names before shorter prefixes', () => {
        ...
        expect(expandLargePastePlaceholders(pending, '[Pasted Content 3 chars] #2\n[Pasted Content 3 chars]')).toBe('second\nfirst');
    });
...
describe('composer tag serialization', () => {
    ...
    it('replaces inline tags before expanding large paste placeholders', () => {
        const pending = new Map();
        const paste = createLargePastePlaceholder(pending, 1, 'expanded pasted content that is longer than the placeholder');
        ...
        expect(expandLargePastePlaceholders(pending, withInlineTags)).toBe('expanded pasted content that is longer than the placeholder explain <table />');
    });
});
