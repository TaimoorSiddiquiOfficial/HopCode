 ...
 */
export function useOtherWorkspaceSessions(enabled = true) {
    ...
    const targetsKey = enabled
        ? (workspace.capabilities?.workspaces ?? [])
            ...
            .join('\n')
        : '';
    const [sessions, setSessions] = useState(EMPTY);
    ...
    const fetchSessions = useCallback(async () => {
        ...
        if (cwds.length === 0)
            return EMPTY;
        const settled = await Promise.allSettled(cwds.map((cwd) => 
        ...
        settled.forEach((result, index) => {
            if (result.status === 'fulfilled') {
                ...
                console.warn(`[useOtherWorkspaceSessions] failed to list sessions for ${cwds[index]}:`, result.reason);
            }
        });
