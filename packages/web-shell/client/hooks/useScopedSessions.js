...
export function useScopedSessions(workspaceCwd, options = {}) {
    const { autoLoad = false, enabled = true, pageSize, archiveState, view, group, } = options;
    ...
    const { reload: reloadPrimary, deleteSession: deletePrimarySession, deleteSessions: deletePrimarySessions, } = primary;
    ...
    useEffect(() => {
        ...
        if (autoLoad && enabled)
            void reloadScoped().catch(() => undefined);
        return () => {
            requestSequence.current += 1;
        };
    }, [autoLoad, enabled, reloadScoped, workspaceCwd]);
    const reload = useCallback(() => (workspaceCwd ? reloadScoped() : reloadPrimary()), [reloadPrimary, reloadScoped, workspaceCwd]);
    const deleteSessions = useCallback(async (sessionIds) => {
        if (!workspaceCwd)
            return deletePrimarySessions(sessionIds);
    ...
    const deleteSession = useCallback(async (sessionId) => {
        if (!workspaceCwd)
            return deletePrimarySession(sessionId);
