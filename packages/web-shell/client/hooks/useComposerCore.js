...
import { useOptionalWorkspace, } from '@hoptrendy/webui/daemon-react-sdk';
...
import { createInputAnnotationsFromComposerTags, getComposerTagIconUrl, getComposerTagSerialized, isBuiltinComposerTagIconUrl, } from '../utils/composerTag';
...
export function createLargePastePlaceholder(pendingPastes, nextPasteId, pasted) {
...
export function prunePendingPastes(pendingPastes, docText) {
...
export function expandLargePastePlaceholders(pendingPastes, text) {
    if (pendingPastes.size === 0)
        return text;
    const placeholders = [...pendingPastes.keys()].sort((a, b) => b.length - a.length);
    const pattern = new RegExp(placeholders.map(escapeRegExp).join('|'), 'g');
    return text.replace(pattern, (placeholderText) => pendingPastes.get(placeholderText) ?? placeholderText);
}
...
export function useComposerCore(options) {
    ...
    const pendingPastesRef = useRef(new Map());
    ...
    const refreshSlashMenuForView = useCallback((view, preferredIndex) => {
        ...
        const result = getSlashCommandCompletionResult(view.state.doc.toString(), selection.head, commandsRef.current, skillsRef.current, languageRef.current, (key) => tRef.current(key), slashCommandCategoryOrderRef.current ?? DEFAULT_COMMAND_CATEGORY_ORDER);
        if (!result) {
            setSlashMenu(null);
            return;
        }
    ...
    useEffect(() => {
        ...
        const submitText = (view, textOverride, tagsOverride) => {
            const inlineTags = tagsOverride === undefined ? getInlineComposerTagPlacements(view) : [];
            ...
            if (!rawText && tags.length === 0 && inlineTags.length === 0)
                return true;
            const textWithInlineTags = tagsOverride === undefined
                ? replaceInlineTagPlacements(rawText, normalizedInlineTags)
                : rawText;
            const text = expandLargePastePlaceholders(pendingPastesRef.current, textWithInlineTags);
            const prompt = buildComposerPrompt(text, tags);
            const images = pastedImagesRef.current;
            const isShellMode = shellModeRef.current;
            const promptText = isShellMode ? `!${prompt}` : prompt;
            const inputAnnotations = createInputAnnotationsFromComposerTags(promptText, [...tags, ...normalizedInlineTags.map((placement) => placement.tag)]);
            let committed = false;
        ...
        const composerUpdateListener = EditorView.updateListener.of((update) => {
            if (update.docChanged && pendingPastesRef.current.size > 0) {
                const nextPasteId = prunePendingPastes(pendingPastesRef.current, update.state.doc.toString());
                if (nextPasteId !== null) {
            ...
            extensions: [
                ...
                EditorView.domEventHandlers({
                    ...
                    paste(event) {
                        ...
                        const { placeholderText: pt, nextPasteId } = createLargePastePlaceholder(pendingPastesRef.current, nextPasteIdRef.current, pasted);
                        nextPasteIdRef.current = nextPasteId;
                        const selection = view.state.selection.main;
                        view.dispatch({
                            changes: {
        ...
        setHasContent(initialText.length > 0 ||
            composerTagsRef.current.length > 0 ||
            pastedImagesRef.current.length > 0);
        return () => {
    ...
    useEffect(() => {
        ...
        const followupSuggestion = !disabled && followupState?.isVisible && followupState.suggestion
            ? followupState.suggestion
            : null;
        const nextPlaceholder = followupSuggestion ??
    ...
    const hasInput = useCallback(() => {
        return ((viewRef.current?.state.doc.toString().trim().length ?? 0) > 0 ||
            composerTagsRef.current.length > 0 ||
            pastedImagesRef.current.length > 0);
    }, []);
    ...
    const submitSearchMatch = useCallback((match) => {
        ...
        const images = pastedImagesRef.current;
        const isShellMode = shellModeRef.current;
        const accepted = onSubmitRef.current(isShellMode ? `!${text}` : text, images.length > 0 ? [...images] : undefined);
        if (accepted === false) {
