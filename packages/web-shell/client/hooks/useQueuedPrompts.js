...
import { consumePendingPromptEvents, getPendingPromptEvents, getPendingPromptVersion, subscribePendingPromptEvents, subscribePendingPromptVersion, useDaemonMidTurnInjected, } from '@hoptrendy/webui/daemon-react-sdk';
...
export function useQueuedPrompts({ connected, sessionId, clientId, streamingState, sessionActions, store, editorRef, reportError, notifySuccess, t, }) {
    ...
    const queuedTexts = useMemo(() => queuedPrompts.map((prompt) => prompt.text), [queuedPrompts]);
    useEffect(() => {
    ...
    const restoreTextToEditor = useCallback((text, images, targetSessionId) => {
            ...
            if (event.type === 'pending_prompt_started') {
                ...
                if (shouldAppendLocalUserMessage &&
                    ...
                    if (text) {
                        ...
                        store.appendLocalUserMessage(text, toStoreImages(prompt?.images), prompt?.inputAnnotations?.length
                ...
                if (callback) {
                    ...
                    if (!completedPromptIdsRef.current.has(promptId)) {
                        ...
                        while (completedPromptIdOrderRef.current.length >
    ...
    const enqueuePrompt = useCallback((text, images, onComplete, inputAnnotations) => {
        ...
        sessionActions
            ...
            if (latestSessionIdRef.current !== targetSessionId) {
                sessionActions
                    ...
                    .catch((error) => {
                    console.warn('[useQueuedPrompts] cleanup removePendingPrompt failed after session change', { targetSessionId, promptId: result.promptId, error });
                });
                return;
            }
            if (latestStreamingStateRef.current === 'idle') {
                if (!displayedServerPromptIdsRef.current.has(result.promptId)) {
                    ...
                    store.appendLocalUserMessage(trimmed, toStoreImages(queuedImages), queuedAnnotations?.length
                        ? { inputAnnotations: queuedAnnotations }
                        : undefined);
                }
    ...
    const insertQueuedPrompt = useCallback(async (id) => {
        const prompt = queuedPromptsRef.current.find((item) => item.id === id);
        if (!prompt || (prompt.images?.length ?? 0) > 0)
            return;
        if (prompt.serverState === 'submitting' ||
            prompt.isEditing ||
            prompt.isRemoving ||
        ...
        if (prompt.serverPromptId &&
            !(await removeServerPromptForAction(prompt, { isRemoving: true }, t('queue.insertFailed')))) {
            return;
        }
        ...
        if (!result.accepted) {
            ...
            reportError(new Error('Queued message was not accepted for insertion'), t('queue.insertFailed'));
            return;
        }
        finishRemovedPrompt();
        if (!prompt.serverPromptId) {
    ...
    const clearQueuedPrompts = useCallback(() => {
        ...
        const submittingPrompts = queuedPromptsRef.current.filter((prompt) => prompt.serverState === 'submitting');
        const clearablePrompts = queuedPromptsRef.current.filter((prompt) => prompt.serverState !== 'submitting');
        ...
        const clearIds = new Set(queuedPromptsRef.current.map((prompt) => prompt.id));
        ...
        void (async () => {
            ...
            if (latestSessionIdRef.current !== clearSessionId)
                return;
    ...
    useEffect(() => {
        if (!sessionId || midTurnInjectedBatches.length === 0)
            return;
        const next = removeInjectedFromQueue(queuedPromptsRef.current, midTurnInjectedBatches, sessionId, clientId);
        if (next) {
            queuedPromptsRef.current = next;
            setQueuedPrompts(next);
        }
        consumeMidTurnInjected(midTurnInjectedBatches.filter((b) => b.sessionId === sessionId));
