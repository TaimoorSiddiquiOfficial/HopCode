...
export function useBackgroundTasks(taskActivityKey, connected, refreshTrigger = 0) {
    ...
    useEffect(() => {
        if (!connected || !taskActivityKey)
            return;
        emptyPollsRef.current = 0;
        setPollingActive(true);
    }, [connected, taskActivityKey]);
    useEffect(() => {
        if (!connected || refreshTrigger === 0)
            return;
        emptyPollsRef.current = 0;
        setPollingActive(true);
