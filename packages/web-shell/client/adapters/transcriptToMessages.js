...
function isIgnoredWebShellStatus(text) {
    return (text.startsWith('language_changed (unrecognized daemon event):') ||
...
function isBackgroundNotificationAssistantBlock(block) {
    const extended = block;
    const meta = extended.meta;
    return (meta?.['source'] === 'background_notification' &&
        meta['hopcodeDiscreteMessage'] === true &&
        meta['backgroundTask'] !== undefined);
}
function normalizeAssistantTextBlock(block) {
    if (isBackgroundNotificationAssistantBlock(block))
        return null;
    if (!block.text && !block.usage)
        return null;
    return block;
}
...
export function transcriptBlocksToDaemonMessages(blocks, options = {}) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        switch (block.kind) {
            ...
            case 'assistant': {
                const textBlock = normalizeAssistantTextBlock(block);
                if (!textBlock)
                    break;
