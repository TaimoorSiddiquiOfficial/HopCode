...
import { DAEMON_APPROVAL_MODES, useActions, useConnection, useDaemonFollowupSuggestion, useSettings, useProviders, useSessionNotices, useStreamingState, useTranscriptBlocks, useTranscriptHistory, useTranscriptStore, useWorkspace, useWorkspaceActions, useWorkspaceEventSignals, } from '@hoptrendy/webui/daemon-react-sdk';
...
import { AgentsMessage, } from './components/messages/AgentsMessage';
...
import { computeTodoDetails, computeTodoTimeline, getFloatingTodos, todoDetailSignature, todoTimelineSignature, } from './utils/todos';
...
function isBackgroundShellToolCall(tool) {
...
function getBackgroundTaskActivityKey(messages) {
    const parts = [];
    for (const message of messages) {
        if (message.role !== 'tool_group')
            continue;
        for (const tool of message.tools) {
            if (isBackgroundSubAgentToolCall(tool) ||
...
export function App({ onSessionIdChange, onSessionCreated, theme: providedTheme, onThemeChange, language: providedLanguage, onLanguageChange, className: externalClassName, style: externalStyle, onConnectionChange, onStreamingStateChange, onError, onBugReport, hiddenSlashCommands, slashCommandCategoryOrder, builtinAtProviders, atProviders, composerTagIcons, renderToolHeaderExtra, renderWelcomeHeader, renderWelcomeFooter, mobileWelcomeFooterMiddle = false, parseUserMessageContent, renderUserMessageContent, renderComposerTag, renderComposerTagTooltip, onComposerTagClick, renderAssistantTurnFooter, renderComposerToolbarStart, renderComposerToolbarEnd, renderComposerToolbarRight, renderComposerHeader, renderFooter, bottomStatusItems, chatMaxWidth, sidebar, splitSessionIds: externalSplitSessionIds, onSplitSessionIdsChange, onRightPanelOpen, messageTurnOutputs, shellRef, composerToolbarActions, composerPlaceholders, compactThinking = false, collapseCompletedTurns = true, markdownTableMode = 'basic', virtualScrollThreshold, markdown, loadingPhrases, onTranscriptChange, onToast, composerRef, onComposerReady, composerInput, composerInputVersion, onSessionChange, onSubmitBefore, restartSseOnPrompt, lockedWorkspaceCwd, lockedWorkspaceCapability, } = {}) {
    ...
    const [selectedWorkspaceCwd, setSelectedWorkspaceCwd] = useState(undefined);
    ...
    useEffect(() => {
        ...
        const fetchStatus = () => {
            void workspace.client
                ...
                .then((git) => {
                if (!cancelled)
                    setSelectedWorkspaceGitStatus(git);
            })
                .catch(() => {
                if (!cancelled)
                    setSelectedWorkspaceGitStatus(undefined);
    ...
    useEffect(() => {
        ...
        const paneArtifactIds = new Set(artifactPanelTabs
        ...
        setArtifactPanelExtraArtifacts((previous) => {
            const next = previous.filter((artifact) => !artifactIds.has(artifact.id) || paneArtifactIds.has(artifact.id));
    ...
    const handlePaneArtifactsChange = useCallback((paneSessionId, paneArtifacts, paneWorkspaceActions) => {
        setPaneArtifactSnapshots((current) => {
            const previous = current.get(paneSessionId);
            const unchanged = previous?.workspaceActions === paneWorkspaceActions &&
        ...
        setArtifactPanelTabs((tabs) => {
            ...
            const next = tabs.map((tab) => {
                ...
                if (tab.previewContent !== undefined)
                    changed = true;
                if (tab.workspaceActions)
                    return updated;
                changed = true;
                return updated;
            });
    ...
    const openReviewPanel = useCallback((changes, selectedPath) => {
        const reviewTab = {
            id: 'review',
            kind: 'review',
    ...
    const openScheduledTaskPanel = useCallback((task, tabWorkspaceActions) => {
        const tab = {
    ...
    const rawPendingApproval = useMemo(() => extractPendingPermission(messageBlocks), [messageBlocks]);
    ...
    const floatingTodos = useStableArray(floatingTodosState.todos, (t) => `${t.id}:${t.status}:${t.content}`);
    ...
    const backgroundTaskActivityKey = useMemo(() => getBackgroundTaskActivityKey(messages), [messages]);
    const [backgroundTasksRefreshTrigger, setBackgroundTasksRefreshTrigger] = useState(0);
    const backgroundTasks = useBackgroundTasks(backgroundTaskActivityKey, connection.status === 'connected', backgroundTasksRefreshTrigger);
    ...
    const composerTextDebounceRef = useRef(null);
    ...
    const [diffWorkspaceCwd, setDiffWorkspaceCwd] = useState(undefined);
    ...
    const [agentsDialogMode, setAgentsDialogMode] = useState(null);
    ...
    const delayedReloadTimerRef = useRef(null);
    useEffect(() => () => {
    ...
    const sendPrompt = useCallback(async (text, images, opts) => {
        ...
        const result = await sessionActions.sendPrompt(text, promptOptions);
        return result;
    ...
    const { queuedPrompts, queuedTexts, enqueuePrompt: rawEnqueuePrompt, removeQueuedPrompt, insertQueuedPrompt, editQueuedPrompt, editLastQueuedPrompt, clearQueuedPrompts, } = useQueuedPrompts({
        connected,
    ...
    const currentVoiceModel = (() => {
        const value = readScopedModelSetting(workspaceSettings, modelSettingScope, 'voiceModel');
    ...
    const handleSetMode = useCallback((modeId) => {
        ...
        sessionActions
            ...
            if (!approval)
                return;
            const shouldAutoApprove = modeId === 'izn' ||
    ...
    useEffect(() => {
        ...
        if (prev !== 'idle' && streamingState === 'idle') {
            ...
            if (!sessionId || sessionId !== streamingSessionIdRef.current)
                return;
    ...
    useEffect(() => {
        ...
        const activeWorkspace = workspaces.find((entry) => entry.cwd === connection.workspaceCwd);
        ...
        lastNotifiedWorkspaceIdRef.current = workspaceId;
        lastNotifiedWorkspaceCwdRef.current = connection.workspaceCwd;
        onSessionIdChange?.(connection.sessionId, workspaceId, connection.workspaceCwd);
    }, [
    ...
    const showContextUsage = useCallback((commandText, detail) => {
        ...
        if (!requireActiveSessionForLocalCommand())
            return;
        if (echoOrDeferLocalCommand(commandText))
            return;
        sessionActions
    ...
    const { suggestion: newSessionSuggestion, dismiss: dismissNewSessionSuggestion, suppress: suppressNewSessionSuggestion, } = useNewSessionSuggestion({
    ...
    const loadSidebarSession = useCallback(async (sessionId, workspaceCwd) => {
        ...
        try {
            ...
            setSidebarSwitchingSessionId((current) => current === sessionId ? null : current);
            throw error;
        }
    ...
    const handleOpenSessionFromOverview = useCallback((sessionId) => {
        setMainView('chat');
        void loadSidebarSession(sessionId).catch((error) => {
    ...
    const handleSubmit = useCallback((text, images, commitComposerAccepted, metadata) => {
        ...
        if (text.startsWith('/')) {
            ...
            if (match) {
                ...
                if (cmd === 'model') {
                    ...
                    if (modelArg.startsWith('--voice ')) {
                        ...
                        setWorkspaceSetting('workspace', 'voiceModel', voiceModelId).catch((error) => reportError(error, t('model.setVoice')));
                        return true;
                    }
                    if (modelArg === '--vision') {
                ...
                if (cmd === 'skills') {
                    ...
                    if (!skillArg || skillArg === 'detail' || skillArg === 'details') {
                        ...
                        return submitPromptFromEditor(text, images, 'Failed to send /skills command', { inputAnnotations: metadata?.inputAnnotations });
                    }
                    return true;
                }
                if (cmd === 'tools') {
                ...
                if (cmd === 'resume') {
                    ...
                    if (sessionId) {
                        ...
                        closePanel();
                        sessionActions.loadSession(sessionId).catch((error) => {
                ...
                if (cmd === 'bug') {
                    ...
                    Promise.all([
                        ...
                        .then(([preflight, env]) => {
                        const sys = collectSystemInfo(preflight, env);
                        const hopCodeVersion = connectionRef.current.capabilities?.HopCodeVersion || '';
                        const sysInfo = {};
                        if (hopCodeVersion)
                            sysInfo.cliVersion = hopCodeVersion;
                        if (sys.nodeVersion)
            ...
            if (!requireActiveSessionForLocalCommand())
                return false;
            sessionActions.sendShellCommand(cmd).catch((error) => {
    ...
    const handleEditorSubmit = useCallback((text, images, commitComposerAccepted, metadata) => {
        const accepted = handleSubmit(text, images, commitComposerAccepted, metadata);
        if (accepted !== false) {
            resumeChatBottomFollow('smooth');
        }
        return accepted;
    ...
    const handleRetry = useCallback(() => {
        if (showRetryHintRef.current &&
            ...
            setShowRetryHint(false);
            sendPrompt(lastSubmittedPromptRef.current, lastSubmittedImagesRef.current, {
                optimisticUserMessage: false,
                retry: true,
            }).catch((error) => reportError(error, 'Failed to retry prompt'));
        }
        else {
    ...
    const handleVoiceModelSelect = useCallback((modelId) => {
        ...
        setWorkspaceSetting(modelSettingScope, 'voiceModel', bareModelId).catch((error) => reportError(error, t('model.setVoice')));
    ...
    const commands = useMemo(() => {
        ...
        const refreshedSkillCommands = loadedSkillsReady
            ...
            : [];
        return localizeBuiltinDescriptions(mergeCommands(retainedCommands, refreshedSkillCommands, getLocalCommands(t)), t)
    ...
    const welcomeHeaderProps = useMemo(() => ({
        version: connection.capabilities?.HopCodeVersion || '',
        cwd: connection.workspaceCwd || '',
        currentModel,
        currentMode,
        hideTips: hideTipsSetting?.values.effective === true,
    }), [
        connection.capabilities?.HopCodeVersion,
        connection.workspaceCwd,
        currentModel,
        currentMode,
        hideTipsSetting?.values.effective,
    ]);
    const welcomeHeader = useMemo(() => renderWelcomeHeader ? (renderWelcomeHeader(welcomeHeaderProps)) : (_jsx(WelcomeHeader, { ...welcomeHeaderProps })), [renderWelcomeHeader, welcomeHeaderProps]);
    ...
    return (_jsx(ThemeProvider, { value: selectedTheme, children: _jsx(I18nProvider, { language: selectedLanguage, children: _jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsxs("div", { ref: appRootRef, className: appClassName, style: appStyle, "data-web-shell-root": true, "data-web-shell-shadcn": true, lang: selectedLanguage, children: [!onToast && _jsx(ToastHost, { toasts: toasts, onDismiss: dismissToast }), showResumeDialog && (_jsx(DialogShell, { title: t('resume.title'), size: "lg", onClose: () => setShowResumeDialog(false), children: _jsx(ResumeDialog, { workspaceCwd: lockedWorkspaceCwd, onSelect: (sessionId) => {
                                    closeMobileDrawer();
                                    closePanel();
                                    sessionActions
                                    ...
                                    setModelDialogMode(null);
                                } }) })), showApprovalModeDialog && (_jsx(DialogShell, { title: t('mode.select'), size: "sm", onClose: () => setShowApprovalModeDialog(false), children: _jsx(ApprovalModeDialog, { currentMode: currentMode, onSelect: (modeId) => {
                                    handleSetMode(modeId);
                                    setShowApprovalModeDialog(false);
                                } }) })), showToolsDialog && (_jsx(DialogShell, { title: t('tools.title'), size: "lg", onClose: () => setShowToolsDialog(false), children: _jsx(ToolsDialog, {}) })), diffWorkspaceCwd && (_jsx(GitDiffDialog, { workspaceCwd: diffWorkspaceCwd, onClose: () => setDiffWorkspaceCwd(undefined) })), tasksDialogMessage && (_jsx(DialogShell, { title: t('tasks.title'), size: "lg", onClose: () => setTasksDialogMessage(null), children: _jsx(TasksStatusMessage, { message: tasksDialogMessage, embedded: true, manageActiveEvent: false, onClose: () => setTasksDialogMessage(null) }) })), agentsDialogMode && (_jsx(DialogShell, { title: agentsDialogMode === 'manage'
                                ? t('agent.manage')
                                : agentsDialogMode === 'menu'
                                    ? t('agents.title')
                                    : t('agent.create'), size: "lg", onClose: () => setAgentsDialogMode(null), children: _jsx(AgentsMessage, { mode: agentsDialogMode, embedded: true, onMessage: (text) => store.dispatch([{ type: 'status', text }]), onClose: () => setAgentsDialogMode(null) }) })), showMemoryDialog && (_jsx(DialogShell, { title: t('memory.menu'), size: "lg", onClose: () => setShowMemoryDialog(false), children: _jsx(MemoryMessage, { refreshSignal: memoryRefreshSignal, addSignal: memoryAddSignal, addScope: memoryAddScope, onMessage: (text, type = 'status') => {
                                    ...
                                    pushToast('error', t('release.failed', { reason }));
                                }, onClose: () => setShowReleaseDialog(false) }) })), showRewindDialog && (_jsx(DialogShell, { title: t('rewind.title'), subtitle: t('rewind.subtitle'), size: "lg", onClose: () => setShowRewindDialog(false), children: _jsx(RewindDialog, { blocks: blocks, loadSnapshots: loadRewindSnapshots, rewind: rewindConversationOnly, onError: handleRewindError, onClose: () => setShowRewindDialog(false) }) })), _jsxs("div", { className: styles.appShell, children: [sidebarOptions.enabled && (_jsxs("div", { "data-sidebar-shell": "", ...(mobileDrawerOpen
                                        ? { role: 'dialog', 'aria-modal': 'true' }
                                        : {}), "aria-label": t('sidebar.label'), className: [
                                        styles.mobileDrawer,
                                        ...
                                        .join(' '), children: [_jsx("div", { className: styles.mobileBackdrop, onClick: closeMobileDrawer, "aria-hidden": "true" }), _jsx(WebShellSidebar, { collapsed: (sidebarCollapsed ||
                                                ...
                                                setMainView('chat');
                                                closePanel();
                                            }, onError: reportError, mobileOpen: mobileDrawerOpen, sessionListReloadToken: sessionListReloadToken, selectedWorkspaceCwd: selectedWorkspaceCwd, onSelectWorkspace: setSelectedWorkspaceCwd, onOpenGitDiff: setDiffWorkspaceCwd, workspaces: workspaces, lockedWorkspaceCwd: lockedWorkspaceCwd, lockedWorkspace: sidebarOptions.lockedWorkspace, branding: sidebarOptions.branding, footer: sidebarOptions.footer })] })), _jsxs("div", { ref: chatPaneRef, className: [
                                        styles.chatPane,
                                        ...
                                        .filter(Boolean)
                                        .join(' '), children: [sidebarOptions.enabled &&
                                            ...
                                            mainView === 'chat' && (_jsx("button", { type: "button", className: [
                                                ...
                                                : activePanel === 'status'
                                                    ...
                                                    activePanel !== 'plugins' && (_jsxs("div", { className: styles.panelHeader, children: [_jsx("button", { ref: panelBackRef, type: "button", className: styles.panelBack, "data-testid": "panel-back", onClick: closePanel, "aria-label": t('common.back'), title: t('common.back'), children: _jsx("svg", { viewBox: "0 0 24 24", "aria-hidden": "true", children: _jsx("path", { d: "M15 5l-7 7 7 7", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" }) }) }), _jsx("div", { className: styles.panelTitle, children: activePanel === 'settings'
                                                                ...
                                                                : activePanel === 'status'
                                                                    ? t('daemon.title')
                                                                    : t('sessionsOverview.title') })] })), _jsx("div", { className: styles.panelBody, children: activePanel === 'settings' ? (_jsx(SettingsMessage, { settingsState: workspaceSettingsState, embedded: true, onLanguageChange: handleSettingsLanguageChange, onThemeChange: handleThemeChange, chatWidthMode: chatWidthMode, onChatWidthModeChange: handleChatWidthModeChange, modelManagement: {
                                                            ...
                                                            closePanel();
                                                        } })) : activePanel === 'skills' ? (_jsx(SkillsManagerPage, { onClose: closePanel, onUseSkill: handleUseSkill })) : activePanel === 'plugins' ? (_jsx(PluginManagerPage, { mcpMessage: mcpDialogMessage, loadMcpMessage: async () => {
                                                            try {
                                                        ...
                                                        workspaces: lockedWorkspaceCwd
                                                            ...
                                                            sessionListReloadToken: sessionListReloadToken, includeOtherWorkspaces: !lockedWorkspaceCwd, workspaceCwd: lockedWorkspaceCwd, 
                                                            // Back returns to the Session Overview (the hub the split
                                                            // is launched from), not the single-session chat.
                                                            onExit: handleSplitExit, onError: reportError, onRightPanelOpen: handleTurnOutputOpen, onPaneArtifactsChange: handlePaneArtifactsChange, messageTurnOutputs: messageTurnOutputs, restartSseOnPrompt: restartSseOnPrompt }) }) })] })), _jsxs("div", { className: [
                                                styles.chatViewWrap,
                                                hasMobileComposerBottom
                                                ...
                                                .join(' '), "aria-hidden": activePanel || mainView !== 'chat' ? true : undefined, children: [showMissingSessionState && (_jsxs("div", { className: styles.missingSessionState, children: [_jsx("div", { className: styles.missingSessionMessage, children: t('session.missing') }), _jsx("button", { type: "button", className: styles.missingSessionButton, disabled: isCreatingMissingSession, onClick: handleMissingSessionNewSession, children: t('session.new') })] })), _jsx("div", { className: showMissingSessionState
                                                        ...
                                                        : styles.chatSubtree, children: _jsxs(WebShellCustomizationProvider, { value: customization, children: [_jsx(CompactModeContext.Provider, { value: compactMode, children: _jsx(TodoContextsProvider, { timeline: todoTimeline, details: todoDetails, children: _jsx(InteractionBlockContext.Provider, { value: registerInteractionBlocker, children: (() => {
                                                                            ...
                                                                            const messageList = (_jsx(MessageList, { ref: messageListRef, messages: displayMessages, pendingApproval: pendingToolApproval, onShowContextDetail: handleShowContextDetail, loadingTranscript: connection.loadingTranscript, catchingUp: connection.catchingUp, hasOlderHistory: transcriptHistory.hasMore, loadingOlderHistory: transcriptHistory.loading, historyCapacityReached: transcriptHistory.capacityReached, onLoadOlderHistory: transcriptHistory.loadMore, isResponding: streamingState !== 'idle', activeTurnStartedAt: activeTurnStartedAt, workspaceCwd: connection.workspaceCwd || '', hideSessionTimeline: effectiveChatWidthMode === 'wide', showRetryHint: showRetryHint, onRetryClick: handleRetry, onBranchSession: handleBranchCurrentSession, bottomOverlayInset: bottomPanelInset, welcomeHeader: isChatEmptyState ? welcomeHeader : undefined, centerWelcomeHeader: showMobileWelcomeFooterMiddle || undefined, tailContent: undefined, tailKey: undefined, onCanScrollToBottomChange: handleCanScrollToBottomChange, virtualScrollThreshold: virtualScrollThreshold, turnFileChanges: visibleTurnOutputKinds.has('file')
                                                                                    ...
                                                                                    : undefined, onTurnOutputOpen: handleTurnOutputOpen, onReviewChanges: openReviewPanel, onOpenArtifact: openArtifactPanel, onOpenScheduledTask: openScheduledTaskPanel, generateContent: connection.capabilities?.features.includes('session_generation')
                                                                    ...
                                                                    : styles.footer, children: [canScrollMessageListToBottom && (_jsx("div", { className: showBottomPanels
                                                                            ...
                                                                            : styles.scrollToBottomLayer, children: _jsx("button", { type: "button", className: styles.scrollToBottomButton, "aria-label": t('chat.scrollToBottom'), onClick: () => resumeChatBottomFollow('smooth'), children: _jsx("svg", { className: styles.scrollToBottomIcon, viewBox: "0 0 24 24", "aria-hidden": "true", children: _jsx("path", { d: "M12 5v13M6.5 12.5 12 18l5.5-5.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }) }) }) })), showBottomPanels && (_jsx("div", { ref: bottomPanelsRef, className: styles.bottomPanels, children: _jsx(TodoPanel, { todos: showFloatingTodos ? floatingTodos : [], statusItems: floatingBottomStatusItems }) })), pendingToolApproval && mainView === 'chat' && (_jsx("div", { "data-testid": "approval-overlay", className: styles.approvalOverlay, children: _jsx(ToolApproval, { request: pendingToolApproval, onConfirm: handleConfirm, variant: "floating", keyboardActive: toolApprovalOverlayVisible }) })), pendingAskUserApproval && mainView === 'chat' && (_jsx("div", { "data-testid": "approval-overlay", className: styles.approvalOverlay, children: _jsx(AskUserQuestion, { request: pendingAskUserApproval, onConfirm: handleConfirm, variant: "floating", keyboardActive: askUserOverlayVisible }) })), _jsxs("div", { className: styles.composer, children: [streamingState !== 'idle' ? (_jsx(StreamingStatus, { startedAt: activeTurnStartedAt })) : newSessionSuggestion ? (_jsxs("div", { className: styles.composerActionTip, role: "status", "data-testid": "new-session-suggestion", children: [_jsx("span", { className: styles.composerActionTipIcon, "aria-hidden": "true", children: "\u2726" }), _jsx("span", { className: styles.composerActionTipText, children: t('editor.newSessionSuggestionTitle') }), _jsx("div", { className: styles.composerActionTipActions, children: _jsx("button", { type: "button", className: `${styles.composerActionTipButton} ${styles.composerActionTipButtonPrimary}`, "data-testid": "new-session-suggestion-start", onMouseDown: (event) => event.preventDefault(), onClick: handleAcceptNewSessionSuggestion, children: t('editor.newSessionSuggestionStart') }) })] })) : escapeHintVisible && streamingState === 'idle' ? (_jsx("div", { className: styles.escClearStatus, role: "status", children: t('editor.escClearHint') })) : null, _jsx(QueuedPromptDisplay, { prompts: queuedPrompts, t: t, onDelete: removeQueuedPrompt, onInsert: insertQueuedPrompt, onEdit: editQueuedPrompt }), CustomComposerHeader && (_jsx("div", { className: styles.composerHeader, children: _jsx(CustomComposerHeader, { disabled: isDisabled, isRunning: streamingState !== 'idle', currentMode: currentMode, currentModel: currentModel, sessionName: sessionDisplayName }) })), _jsx(ChatEditor, { ref: setEditorHandle, onSubmit: handleEditorSubmit, onInputTextChange: handleComposerTextChange, onCycleMode: handleCycleMode, onToggleShortcuts: handleToggleShortcuts, onCancel: handleCancel, isRunning: streamingState !== 'idle', isPreparing: isPreparingPrompt || isStartingNewSessionSuggestion, cancelArmed: cancelArmed, disabled: isDisabled || isStartingNewSessionSuggestion, commands: commands, skills: loadedSkills, slashCommandCategoryOrder: slashCommandCategoryOrder, builtinAtProviders: builtinAtProviders, atProviders: atProviders, composerTagIcons: composerTagIcons, queuedMessages: queuedTexts, onFocusFooter: handleFocusTaskPill, onPopQueuedMessages: editLastQueuedPrompt, onClearQueuedMessages: clearQueuedPrompts, currentMode: currentMode, currentModel: currentModel, gitBranch: connection.sessionId
                                                                                    ? connection.gitBranch
                                                                                    : (selectedWorkspaceGitStatus?.branch ?? undefined), gitStatus: selectedWorkspaceGitStatus, onOpenGitDiff: gitDiffWorkspaceCwd
                                                                                    ? () => setDiffWorkspaceCwd(gitDiffWorkspaceCwd)
                                                                                    : undefined, chatWidthMode: chatWidthMode, showChatWidthToggle: !isChatEmptyState, chatWidthToggleMin: chatWidthToggleMin, visibleToolbarActions: composerToolbarActions, availableModels: availableModels, onSelectMode: handleSetMode, onSelectModel: handleModelSelect, workspaces: !lockedWorkspaceCwd && workspaces.length > 1
                                                                                    ...
                                                                                    : undefined, selectedWorkspaceCwd: connection.sessionId
                                                                                    ? workspaces.find((entry) => entry.cwd === connection.workspaceCwd)?.primary
                                                                                        ? undefined
                                                                                        : connection.workspaceCwd
                                                                                    : selectedWorkspaceCwd, workspaceSelectionDisabled: Boolean(connection.sessionId), atWorkspaceCwd: lockedWorkspaceCwd ??
                                                                                    (connection.sessionId
                                                                                        ? connection.workspaceCwd
                                                                                        : (selectedWorkspaceCwd ??
                                                                                            workspaces.find((entry) => entry.primary)?.cwd)), onSelectWorkspace: (cwd) => {
                                                                                    selectedWorkspaceCwdRef.current = cwd;
                                                                                    setSelectedWorkspaceCwd(cwd);
                                                                                }, onChatWidthModeChange: handleChatWidthModeChange, sessionName: sessionDisplayName, dialogOpen: interactionBlocked || approvalOverlayActive, followupState: followupState, onAcceptFollowup: onAcceptFollowup, onDismissFollowup: onDismissFollowup, composerInput: composerInput, composerInputVersion: composerInputVersion, placeholderText: composerPlaceholderText })] }), CustomFooter ? (hasMobileComposerBottom ? (_jsx("div", { className: styles.customFooter, children: _jsx(CustomFooter, { connected: connected, mode: currentMode, model: currentModel, streamingState: streamingState, contextUsageRatio: (connection.contextWindow ?? 0) > 0
                                                                            ...
                                                                            .filter(Boolean)
                                                                            .join(' '), children: welcomeFooter }))] })] }) })] })] }), artifactPanelOpen && (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.artifactResizeHandle, role: "separator", "aria-orientation": "vertical", "aria-valuemin": MIN_ARTIFACT_PANEL_WIDTH, "aria-valuemax": getMaxArtifactPanelWidth(), "aria-valuenow": artifactPanelWidth, onPointerDown: handleArtifactPanelResizeStart }), _jsx(ArtifactPanel, { artifacts: artifactPanelArtifacts, tabs: artifactPanelTabs, activeTabId: activeArtifactPanelTabId, reviewChanges: reviewChanges, selectedReviewPath: selectedReviewPath, panelWidth: artifactPanelWidth, workspaceCwd: connection.workspaceCwd || '', loading: artifactsLoading, error: artifactsError, onSelectTab: setActiveArtifactPanelTabId, onCloseTab: closeArtifactPanelTab, onClose: closeArtifactPanel })] }))] })] }) }) }) }));
}
