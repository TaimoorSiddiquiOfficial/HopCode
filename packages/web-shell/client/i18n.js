...
const EN = {
    ...
    'agent.create.method.hopcode': 'Generate with HopCode',
    'agent.create.method.hopcode.recommended': 'Generate with HopCode (Recommended)',
    'agent.create.method.hopcode.desc': 'LLM generates name, prompt, and description',
    ...
    'agent.create.project.cli': 'Project-level (.hopcode/agents/)',
    ...
    'agent.create.hopcodeHint': 'Describe what this subagent should do and when it should be used. (Be comprehensive for best results)',
    'agent.create.hopcodePlaceholder': '> e.g., Expert code reviewer that reviews code based on best practices...',
    ...
    'agent.create.user.cli': 'User-level (~/.hopcode/agents/)',
    ...
    'queue.insertTip': 'Insert this message into the current session; it takes effect on the next model call.',
    'queue.inserted': 'Inserted. It takes effect on the next model call.',
    'queue.insertCommandDisabled': "Commands can't be inserted into the running turn; they run after it finishes.",
    ...
    'queue.shellBlocked': "Shell commands can't be queued while a turn is running.",
    ...
    'help.intro': 'HopCode understands your codebase, makes edits with your permission, and executes commands right from your terminal.',
    ...
    'local.docs': 'Open the full HopCode documentation',
    ...
    'local.hooks': 'Manage HopCode hooks',
    'local.importConfig': 'Import MCP servers from Claude configs',
    'local.init': 'Analyze the project and create a HOPCODE.md file',
    ...
    'skilldesc.extensionCreator': 'Create, test, and customize HopCode extensions',
    ...
    'skilldesc.qcHelper': 'Answer questions about using HopCode',
    ...
    'skilldesc.stuck': 'Diagnose frozen or slow HopCode sessions',
    ...
    'skilldesc.autofix': 'Implement approved issues or address PR review feedback',
    ...
    'skilldesc.desktopPet': 'Create a pixel-art desktop pet for HopCode',
    ...
    'skilldesc.e2eTesting': 'Run end-to-end tests of the HopCode CLI',
    ...
    'skilldesc.qwenCodeClaw': 'Use HopCode as a code-understanding agent',
    ...
    'skilldesc.triage': 'Triage and review HopCode issues and PRs',
    ...
    'extensions.manage.installDescription': 'Enter a GitHub, Git, or npm extension source.',
    ...
    'mode.desc.izn': 'Automatically approve all tools',
    ...
    'mode.izn': 'IZN mode',
    'mode.izn.desc': 'Automatically approve all operations',
    'plan.title': 'Plan',
    ...
    'toolGroup.running': (v) => `Running ${v?.name ?? 'tool'}${v?.duration ? ` ${v.duration}` : ''}${Number(v?.count ?? 0) > 1 ? ` · ${v?.count ?? 0} tools` : ''}`,
...
const ZH = {
    ...
    'agent.create.method.hopcode': '使用 HopCode 生成',
    'agent.create.method.hopcode.recommended': '使用 HopCode 生成（推荐）',
    'agent.create.method.hopcode.desc': 'LLM 生成名称、提示词和描述',
    'agent.create.name': '名称',
    'agent.create.namePlaceholder': 'my-agent',
    'agent.create.nameHelp': '输入此子智能体清晰且唯一的名称。',
    'agent.create.project': '项目',
    'agent.create.project.cli': '项目级 (.hopcode/agents/)',
    ...
    'agent.create.hopcodeHint': '描述此子智能体应该做什么以及何时使用它。（为了获得最佳效果，请全面描述）',
    'agent.create.hopcodePlaceholder': '> 例如：专业的代码审查员，根据最佳实践审查代码...',
    ...
    'agent.create.user.cli': '用户级 (~/.hopcode/agents/)',
    ...
    'help.intro': 'HopCode 可以理解代码库，在你的许可下修改文件，并直接执行命令。',
    ...
    'local.hooks': '管理 HopCode 钩子',
    'local.importConfig': '从 Claude 配置导入 MCP 服务器',
    'local.init': '分析项目并创建 HOPCODE.md 文件',
    ...
    'skilldesc.qwenCodeClaw': '将 HopCode 用作代码理解智能体',
    ...
    'skilldesc.triage': '把关和审查 HopCode 的 issue 与 PR',
    ...
    'toolGroup.running': (v) => `正在执行 ${v?.name ?? '工具'}${v?.duration ? ` ${v.duration}` : ''}${Number(v?.count ?? 0) > 1 ? ` · 共 ${v?.count ?? 0} 个工具` : ''}`,
    ...
    'settings.description.general.cleanupPeriodDays': '~/.hopcode/file-history/ 中用于 /rewind 的会话备份保留天数。后台清理最多每天运行一次。设为 0 表示最小保留（约 1 小时），仍会保护最近一小时触碰过的会话和当前活动会话。',
    'settings.label.general.gitCoAuthor.commit': '归因：commit',
    'settings.description.general.gitCoAuthor.commit': '通过 HopCode 创建 commit 时，添加 Co-authored-by trailer，并写入逐文件 AI 归因 git note。关闭后两者都会跳过。',
    'settings.label.general.gitCoAuthor.pr': '归因：PR',
    'settings.description.general.gitCoAuthor.pr': '运行 gh pr create 时，在 PR 描述中追加 HopCode 归因行。',
    'settings.label.general.language': '语言：界面',
    'settings.description.general.language': '用户界面的语言。使用 auto 可根据系统设置自动检测；也可以在 ~/.hopcode/locales/ 中放置 JS 语言文件来使用自定义语言代码。',
    ...
    'settings.description.general.preventSystemSleep': '当 HopCode 正在流式生成模型回复或执行工具时防止系统睡眠。空闲输入状态和权限确认状态不会阻止睡眠。',
    ...
    'settings.description.ui.enableUserFeedback': '对话结束后显示可选反馈对话框，帮助改进 hopcode 表现。',
    ...
    'settings.label.context.fileFiltering.respecthopcodeignore': '遵守 .hopcodeignore',
    'settings.description.context.fileFiltering.respecthopcodeignore': '搜索时遵守 .hopcodeignore 文件。',
    ...
    'settings.description.experimental.enableCronTools': '启用会话内 cron/loop 工具（实验性）。启用后，模型可以用 cron_create、cron_list 和 cron_delete 创建周期性提示。也可通过 HOPCODE_CODE_ENABLE_CRON=1 环境变量启用。',
