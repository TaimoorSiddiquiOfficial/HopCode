...
const daemonProxy = {
    target: process.env['HOPCODE_DAEMON_URL'] ?? 'http://127.0.0.1:4170',
    changeOrigin: true,
    bypass: (req) => {
...
export default defineConfig(({ command }) => ({
    ...
    resolve: {
        alias: {
            ...
            ...(command === 'serve'
                ? {
                    '@hoptrendy/webui/daemon-react-sdk': resolve(__dirname, '../webui/src/daemon-react-sdk.ts'),
                    '@hoptrendy/webui': resolve(__dirname, '../webui/src/index.ts'),
                    '@hoptrendy/sdk/daemon': resolve(__dirname, '../sdk-typescript/src/daemon/index.ts'),
                    '@hoptrendy/sdk': resolve(__dirname, '../sdk-typescript/src/index.ts'),
                }
                : {}),
        },
