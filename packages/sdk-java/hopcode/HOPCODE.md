...
- Working Directory: /Users/weigeng/repos/HopCode-code/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: HopCodecode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `HopCodeCodeCli` class with simple static methods for basic usage
...
- `HopCodeCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `HopCodeCodeCliTest` demonstrates how to use the SDK to make simple queries to the HopCode CLI.
...
- **Session creation**: Use `HopCodeCodeCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "HopCode_code_cli-pool-{number}"
...
The SDK includes several example files in `src/test/java/com/alibaba/HopCode/code/cli/example/` that demonstrate different aspects of the API:
