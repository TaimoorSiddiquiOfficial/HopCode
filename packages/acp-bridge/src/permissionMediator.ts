 ...
 * `X-HopCode-Client-Id` header — a legitimate path for the local TUI
 ...
 * `hopcode serve` provides a ring-backed publisher; embedded callers and
...
export interface MediatorDeps {
   ...
   * has NOT yet hit any session route (no `X-HopCode-Client-Id` known
...
export class MultiClientPermissionMediator implements PermissionMediator {
    ...
    return new Promise<PermissionResolution>((resolve) => {
      ...
      if (timeoutMs > 0) {
        pending.timer = setTimeout(() => {
          ...
          try {
            process.stderr.write(
              `hopcode serve: permission ${record.requestId} ` +
  ...
  private writeForbiddenStderr(
    ...
    try {
      ...
      process.stderr.write(
        `hopcode serve: permission ${pending.requestId} ` +
