 ...
 *
 *   - `HOPCODE_SERVER_TOKEN` (the daemon's own bearer token) leaking into
 ...
 *   - `HOPCODE_SIMPLE` leaking from the daemon/IDE process into
...
import { createSpawnChannelFactory, createStderrForwarder, getAcpMemoryArgs, scrubChildEnv, } from './spawnChannel.js';
...
describe('createSpawnChannelFactory env policy', () => {
    ...
    afterEach(() => {
        ...
        if (originalSimple === undefined) {
            ...
            process.env['HOPCODE_SIMPLE'] = originalSimple;
        }
    ...
    it('scrubs daemon-only env vars from the spawned ACP child', async () => {
        ...
        await factory('/tmp/project', {
            HOPCODE_SIMPLE: '1',
            HOPCODE_SERVER_TOKEN: 'override-secret',
        });
...
const SCRUBBED = new Set(['HOPCODE_SERVER_TOKEN', 'HOPCODE_SIMPLE']);
describe('scrubChildEnv (defaultSpawnChannelFactory env policy)', () => {
    ...
    it('strips HOPCODE_SERVER_TOKEN from the child env', () => {
        const source = { HOPCODE_SERVER_TOKEN: 'super-secret', PATH: '/usr/bin' };
        const result = scrubChildEnv(source, SCRUBBED);
        expect(result).not.toHaveProperty('HOPCODE_SERVER_TOKEN');
        expect(result['PATH']).toBe('/usr/bin');
    });
    it('strips HOPCODE_SIMPLE from the child env', () => {
        const source = { HOPCODE_SIMPLE: '1', PATH: '/usr/bin' };
        const result = scrubChildEnv(source, SCRUBBED);
        expect(result).not.toHaveProperty('HOPCODE_SIMPLE');
        expect(result['PATH']).toBe('/usr/bin');
    });
    ...
    it('overrides CANNOT re-introduce a scrubbed key (defense in depth)', () => {
        const source = { PATH: '/usr/bin' };
        const result = scrubChildEnv(source, SCRUBBED, {
            HOPCODE_SERVER_TOKEN: 'sneaky-attempt-via-override',
        });
        expect(result).not.toHaveProperty('HOPCODE_SERVER_TOKEN');
    });
    it('overrides CANNOT re-introduce HOPCODE_SIMPLE', () => {
        const source = { PATH: '/usr/bin' };
        const result = scrubChildEnv(source, SCRUBBED, {
            HOPCODE_SIMPLE: '1',
        });
    ...
    it('overrides CANNOT undo the scrub by setting undefined for a scrubbed key', () => {
        ...
        const source = { HOPCODE_SERVER_TOKEN: 'secret', PATH: '/usr/bin' };
        const result = scrubChildEnv(source, SCRUBBED, {
            HOPCODE_SERVER_TOKEN: undefined,
        });
        expect(result).not.toHaveProperty('HOPCODE_SERVER_TOKEN');
    });
    it('overrides are applied AFTER scrub — the denylist always wins', () => {
        ...
        const source = { HOPCODE_SERVER_TOKEN: 'from-process-env' };
        const result = scrubChildEnv(source, SCRUBBED, {
            HOPCODE_SERVER_TOKEN: 'from-override',
        });
        expect(result).not.toHaveProperty('HOPCODE_SERVER_TOKEN');
    });
