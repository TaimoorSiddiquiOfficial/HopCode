...
import { InvalidClientIdError, InvalidPermissionOptionError, InvalidSessionMetadataError, InvalidSessionScopeError, NOT_CURRENTLY_GENERATING_CANCEL_MESSAGE, PromptQueueFullError, RestoreInProgressError, SessionShellClientRequiredError, SessionShellDisabledError, SessionBusyError, SessionNotFoundError, TotalSessionLimitExceededError, WorkspaceMismatchError, } from './bridgeErrors.js';
...
import { BridgeChannelClosedError, BridgeTimeoutError, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, } from './status.js';
...
import { ApprovalMode, SESSION_ARTIFACT_PERSISTENCE_VERSION, ShellExecutionService, stableSessionArtifactId, ToolNames, } from '@hoptrendy/hopcode-core';
...
import { MID_TURN_QUEUE_DRAIN_METHOD, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, } from './bridgeTypes.js';
function deferred() {
    let resolve;
    let reject;
...
describe('createAcpSessionBridge', () => {
    ...
    it('forwards childEnvOverrides to the channelFactory at spawn time (#4247 R6 line 216)', async () => {
        ...
        const bridge1 = makeBridge({
            ...
            childEnvOverrides: {
                HOPCODE_SERVE_MCP_CLIENT_BUDGET: '5',
                HOPCODE_SERVE_MCP_BUDGET_MODE: 'enforce',
            },
        });
        const bridge2 = makeBridge({
            channelFactory: factory,
            childEnvOverrides: {
                HOPCODE_SERVE_MCP_CLIENT_BUDGET: '20',
                HOPCODE_SERVE_MCP_BUDGET_MODE: 'warn',
            },
        });
            ...
            channelFactory: async () => {
                const h = makeChannel({
                    extMethodImpl: (method, params) => {
                        ...
                        if (method === 'hopcode/status/session/tasks') {
                            return {
                                v: 1,
                                sessionId: params['sessionId'],
    ...
    it('rejects oversized response-mode load replay payloads', async () => {
        ...
        await expect(bridge.loadSession({
            ...
            workspaceCwd: WS_A,
            historyReplay: 'response',
        })).rejects.toThrow('hopcode.session.loadReplay updates exceed limit (10001 > 10000)');
        await bridge.shutdown();
    });
    ...
    describe('sendPrompt', () => {
        ...
        it('echoes user_message_chunk to ALL session subscribers (cross-client sync)', async () => {
            ...
            }, undefined, { clientId: session.clientId, promptId: 'prompt-echo' });
    ...
    describe('pendingPromptList', () => {
        ...
        it('publishes pending_prompt_added only for queued prompts', async () => {
            ...
            expect(addedEvents[0].data.text).toBe('queued behind first');
    ...
    describe('permission flow', () => {
        ...
        it('returns false (not InvalidClientIdError) when session exists but requestId is unknown and clientId is unregistered (#4335 / 3271978329 / 3272493792 / 3273077272)', async () => {
            ...
            // supply an arbitrary `X-HopCode-Client-Id`, and distinguish
        ...
        it('rejects cancel sentinel injection via {selected,"__cancelled__"} (#4335 / 3271420267)', async () => {
            // wenshao/hopcode-latest review #4335 (3271420267) — the most
        ...
        it('returns false uniformly for unknown permission votes regardless of clientId registration (#4335 / 3272493777)', async () => {
            ...
            // with various `X-HopCode-Client-Id` headers could distinguish
    ...
    describe('BridgeClient file proxy (Stage 1: same-host trust)', () => {
        ...
        it('writeTextFile writes to local fs', async () => {
            ...
            const tmp = path.join(os.tmpdir(), `hopcode-bridge-write-${randomBytes(8).toString('hex')}.txt`);
            try {
        ...
        it('writeTextFile leaves no .tmp turd in the target directory (BSA0D)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'hopcode-bridge-atomic-'));
        ...
        it('readTextFile rejects files past the size cap (BSA0E)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const sparse = path.join(os.tmpdir(), `hopcode-bridge-sparse-${randomBytes(8).toString('hex')}.bin`);
        ...
        it('readTextFile rejects non-regular files even when size=0 (BX8YO)', async () => {
            ...
            if (process.platform === 'win32')
                return;
            const { bridge, conn } = await setupForFs();
            const fifoPath = path.join(os.tmpdir(), `hopcode-bridge-fifo-${randomBytes(8).toString('hex')}`);
        ...
        it('writeTextFile preserves symlinks (BX8Yw)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'hopcode-bridge-symlink-'));
        ...
        it('writeTextFile preserves dangling symlinks (BfFvO)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'hopcode-bridge-dangling-'));
        ...
        it('readTextFile returns full content by default', async () => {
            const { bridge, conn } = await setupForFs();
            const tmp = path.join(os.tmpdir(), `hopcode-bridge-read-${randomBytes(8).toString('hex')}.txt`);
        ...
        it('readTextFile slices via line/limit (ACP 1-based line)', async () => {
            const { bridge, conn } = await setupForFs();
            const tmp = path.join(os.tmpdir(), `hopcode-bridge-slice-${randomBytes(8).toString('hex')}.txt`);
    ...
    describe('setSessionApprovalMode (#4175 Wave 4 PR 17)', () => {
        ...
        it('throws BEFORE the ACP roundtrip when persist:true but no callback wired', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await expect(bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: true }, undefined)).rejects.toThrow(/persistApprovalMode/);
            expect(getCalls().some((c) => c.method === 'hopcode/control/session/approval_mode')).toBe(false);
            await bridge.shutdown();
        });
        it('persist:false bypasses the guard regardless of callback wiring', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const res = await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: false }, undefined);
        ...
        it('serializes concurrent approval-mode changes through the per-session queue (A3)', async () => {
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await Promise.all([
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: false }, undefined),
        ...
        it('serializes persist + publish too, not just the extMethod (A3, persist:true)', async () => {
            ...
            const { factory } = approvalModeFactoryWithCallTracker();
            // persist for 'izn' is SLOWER than for 'default' — if persist ran
            // outside the queue, 'default' would publish before 'izn'.
            const persistDelay = { IZN: 30, default: 1 };
            const bridge = makeBridge({
                channelFactory: factory,
            ...
            await Promise.all([
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: true }, undefined),
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.DEFAULT, { persist: true }, undefined),
            ]);
            await new Promise((r) => setTimeout(r, 20));
            abort.abort();
            await collecting;
            // In call order despite IZN's slower persist — persist+publish are
            // serialized inside the queue, so default can't overtake IZN.
            expect(published).toEqual(['izn', 'default']);
            await bridge.shutdown();
        });
        it('a failed approval-mode change does not poison the queue (A3 tail-swallow)', async () => {
            ...
            await expect(bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: false }, undefined)).rejects.toThrow();
        ...
        it('broadcasts approval_mode_changed to peer sessions when persisted (#4282 fold-in 4 S2)', async () => {
            ...
            await bridge.setSessionApprovalMode(a.sessionId, ApprovalMode.IZN, { persist: true }, undefined);
        ...
        it('does NOT broadcast to peers when persisted is false', async () => {
            ...
            const itB = bridge
                .subscribeEvents(b.sessionId, { signal: aborts[1].signal })[Symbol.asyncIterator]();
            await bridge.setSessionApprovalMode(a.sessionId, ApprovalMode.IZN, { persist: false }, undefined);
            const aFirst = await itA.next();
            expect(aFirst.value?.type).toBe('approval_mode_changed');
    ...
    describe('generateSessionRecap (#4175 follow-up)', () => {
        function recapFactory(respond) {
            return async () => {
                ...
                const agent = new FakeAgent({
                    extMethodImpl: (method, params) => {
                        if (method === 'hopcode/control/session/recap') {
    ...
    describe('addRuntimeMcpServer (T2.8 #4514)', () => {
         ...
         * configurable response for `hopcode/control/workspace/mcp/runtime-add`.
         */
        function runtimeAddFactory(respond) {
            return async () => {
                ...
                const agent = new FakeAgent({
                    extMethodImpl: (method, params) => {
                        if (method === 'hopcode/control/workspace/mcp/runtime-add') {
                            return Promise.resolve(respond(params));
                        }
                        return Promise.resolve({});
                    },
    ...
    describe('removeRuntimeMcpServer (T2.8 #4514)', () => {
        /**
         * Build a channel factory whose ACP `extMethod` handler returns a
         * configurable response for `hopcode/control/workspace/mcp/runtime-remove`.
         */
        function runtimeRemoveFactory(respond) {
            return async () => {
                ...
                const agent = new FakeAgent({
                    extMethodImpl: (method, params) => {
                        if (method === 'hopcode/control/workspace/mcp/runtime-remove') {
                            return Promise.resolve(respond(params));
                        }
                        return Promise.resolve({});
                    },
    ...
    describe('extNotification — MCP budget events (PR 14b)', () => {
        ...
        it('drops unknown extNotification methods, kinds, and missing sessionIds silently', async () => {
            ...
            void capturedConn.extNotification('hopcode/notify/session/mcp-budget-event', { kind: 'budget_warning' });
            // Unknown kind — drop.
            void capturedConn.extNotification('hopcode/notify/session/mcp-budget-event', { sessionId: session.sessionId, kind: 'mystery_kind' });
    ...
    describe('extNotification — in-session model update (A1, #4511)', () => {
        ...
        it('suppresses current_model_update while a bridge model roundtrip is in flight', async () => {
            ...
            const modelChange = bridge
                .setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'hopcode-max' }, undefined)
                .catch(() => { });
            await new Promise((r) => setTimeout(r, 10));
            ...
            expect(collected[0]?.type).toBe('model_switched');
            expect((collected[0]?.data).modelId).toBe('hopcode-max');
            abort.abort();
            await bridge.shutdown();
        });
    ...
    describe('extNotification — followup_suggestion', () => {
        it('publishes followup_suggestion when the child fires a prompt-suggestion notification', async () => {
            ...
            void capturedConn.extNotification('hopcode/notify/session/prompt-suggestion', {
                v: 1,
                sessionId: session.sessionId,
        ...
        it('drops malformed prompt-suggestion payloads', async () => {
            ...
            void capturedConn.extNotification('hopcode/notify/session/prompt-suggestion', { v: 1, sessionId: session.sessionId, suggestion: '' });
            void capturedConn.extNotification('hopcode/notify/session/prompt-suggestion', { v: 1, sessionId: session.sessionId, promptId: 'p1' });
            void capturedConn.extNotification('hopcode/notify/session/prompt-suggestion', { v: 1 });
            void capturedConn.extNotification('hopcode/notify/session/prompt-suggestion', {
                v: 1,
                sessionId: session.sessionId,
                suggestion: 123,
    ...
    describe('extNotification — session title update', () => {
        ...
        it('rebroadcasts a child title-update as session_metadata_updated', async () => {
            ...
            void capturedConn.extNotification('hopcode/notify/session/title-update', {
                v: 1,
                sessionId: session.sessionId,
...
// receives writeServeDebugLine output when HOPCODE_SERVE_DEBUG=1.
// ============================================================
describe('onDiagnosticLine', () => {
    const originalDebug = process.env['HOPCODE_SERVE_DEBUG'];
    afterEach(() => {
        if (originalDebug === undefined)
            delete process.env['HOPCODE_SERVE_DEBUG'];
        else
            process.env['HOPCODE_SERVE_DEBUG'] = originalDebug;
    });
    it('receives writeServeDebugLine output when HOPCODE_SERVE_DEBUG=1', async () => {
        ...
        expect(captured.some((e) => e.line.includes('hopcode serve debug: '))).toBe(true);
    ...
    it('does not invoke callback when HOPCODE_SERVE_DEBUG is off', async () => {
        delete process.env['HOPCODE_SERVE_DEBUG'];
        const captured = [];
        let capturedConn;
        const factory = async () => {
        ...
        const next = await it2.next();
        const payload = next.value.data;
        // Same cross-session vote — but HOPCODE_SERVE_DEBUG is off.
...
describe('createHttpAcpBridge — side-channel state layer (#4511)', () => {
    describe('publish helpers cache + generation', () => {
        it('publishModelSwitched updates cache and publishes model_switched', async () => {
            ...
            await bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'hopcode-max' }, undefined);
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
            expect(next.value?.type).toBe('model_switched');
            expect((next.value?.data).modelId).toBe('hopcode-max');
            abort.abort();
            await bridge.shutdown();
        });
        ...
        it('publishApprovalModeChanged publishes approval_mode_changed on setSessionApprovalMode', async () => {
            ...
            expect((next.value?.data).next).toBe(ApprovalMode.IZN);
            abort.abort();
            await bridge.shutdown();
        });
        it('publishApprovalModeChanged stamps the active promptId', async () => {
            ...
            await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: false });
    ...
    describe('extNotification — in-session mode update (A2)', () => {
        it('promotes current_mode_update to approval_mode_changed when no bridge roundtrip is in flight', async () => {
            ...
            void capturedConn.extNotification('hopcode/notify/session/mode-update', {
                v: 1,
                sessionId: session.sessionId,
        ...
        it('suppresses current_mode_update while a bridge approval-mode roundtrip is in flight', async () => {
                ...
                const fakeAgent = new FakeAgent({
                    extMethodImpl: (method) => {
                        if (method.includes('approval_mode')) {
                            return new Promise((res) => {
                                releaseMode = () => res({ previous: 'default', current: 'izn' });
                            });
                        }
                        return {};
                    },
            ...
            const modeChange = bridge
                .setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, {
                persist: false,
            })
                .catch(() => { });
            await new Promise((r) => setTimeout(r, 10));
            ...
            // Only the bridge's own approval_mode_changed (IZN) — the suppressed
    ...
    describe('A5 — session snapshot on attach', () => {
        it('yields session_snapshot after replay_complete when snapshot=true', async () => {
            ...
            expect(snap.data.currentModelId).toBe('hopcode-turbo');
            abort.abort();
            await bridge.shutdown();
        });
        ...
        it('yields session_snapshot up front on a fresh connection (no Last-Event-ID)', async () => {
            ...
            expect((first.value?.data).currentModelId).toBe('hopcode-turbo');
            abort.abort();
            await bridge.shutdown();
        });
    ...
    describe('§2.2 — post-roundtrip reconciliation', () => {
        ...
        it('does NOT publish a corrective event when agent state matches cache', async () => {
            ...
            await bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'hopcode-plus' }, undefined);
            const switches = [];
            for await (const e of iter) {
                if (e.type === 'model_switched') {
                    switches.push(e.data.modelId);
                    if (switches.includes('hopcode-plus'))
                        break;
                }
            }
        ...
        it('swallows a failed status read without crashing or masking the original change', async () => {
            ...
            await expect(bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'hopcode-max' }, undefined)).resolves.toBeDefined();
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
        ...
        it('does NOT reconcile when the model roundtrip itself fails', async () => {
            ...
            await expect(bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'hopcode-max' }, undefined)).rejects.toThrow();
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
        ...
        it('re-runs reconcile when a newer change publishes during the status read (generation rerun)', async () => {
            ...
            for await (const e of iter) {
                if (e.type === 'model_switched') {
                    switches.push(e.data.modelId);
                    if (switches.includes('hopcode-turbo'))
                        break;
                }
            }
        ...
        it('publishes a corrective approval_mode_changed when the agent mode drifted from cache', async () => {
            // approvalMode analog of the model drift test. The bridge sets IZN,
            ...
            await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.IZN, { persist: false }, undefined);
            const nexts = [];
            for await (const e of iter) {
                if (e.type === 'approval_mode_changed') {
        ...
        it('drops unknown agent-returned approval mode without publishing a corrective event', async () => {
                ...
                const fakeAgent = new FakeAgent({
                    extMethodImpl: (method, params) => {
                        ...
                        if (method === 'hopcode/status/session/context') {
                            ...
                            return Promise.resolve({
                                state: { modes: { currentModeId: 'super-IZN' } },
                            });
                        }
                        return Promise.resolve({});
                    },
            ...
            // 'approvalMode'). The status read returns 'super-IZN' which
            ...
            // for the unknown 'super-IZN' value from the agent.
            expect(modeEvents).toEqual(['izn']);
            abort.abort();
            await collecting;
            await bridge.shutdown();
        });
        it('syncs peer session cache on persisted approval-mode change (snapshot reflects new mode)', async () => {
                ...
                const fakeAgent = new FakeAgent({
                    extMethodImpl: (method, params) => {
                        ...
                        if (method === 'hopcode/status/session/context') {
                            ...
                            // persist, the agent is on 'izn' — return it so reconcile
                            // sees no drift and does not emit a corrective.
                            return Promise.resolve({
                                state: { modes: { currentModeId: 'izn' } },
                            });
                        }
                        return Promise.resolve({});
                    },
            ...
            await bridge.setSessionApprovalMode(sessionA.sessionId, ApprovalMode.IZN, { persist: true }, undefined);
...
describe('createAcpSessionBridge — mid-turn message queue (enqueueMidTurnMessage)', () => {
    ...
    it('accepts while a turn is in flight, then rejects again once it settles', async () => {
        ...
        await new Promise((r) => setTimeout(r, 10));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'also check tests')).toEqual({ accepted: true });
        ...
        release();
        await promptPromise;
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'next time')).toEqual({ accepted: false });
        await bridge.shutdown();
    });
    ...
    it('clears undrained messages at settle — not re-drained on the next turn', async () => {
        ...
        const send = (text) => bridge
            ...
            .catch(() => { });
        const drain = () => handle.agentConnection.extMethod('craft/drainMidTurnQueue', {
            sessionId: session.sessionId,
        });
    ...
    it('stamps the drained injection frame with the originator client id', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'hi', {
            clientId: session.clientId,
    ...
    it('trims the message before queuing (drain returns the trimmed text)', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, '   hello   ')).toEqual({ accepted: true });
        const drained = await handle.agentConnection.extMethod('craft/drainMidTurnQueue', { sessionId: session.sessionId });
        expect(drained).toEqual({
            messages: ['hello'],
            hasQueuedPrompt: false,
