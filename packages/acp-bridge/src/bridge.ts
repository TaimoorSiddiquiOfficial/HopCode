 ...
 *   - At most one `hopcode --acp` child per bridge. Secondary daemon routing
...
export function createAcpSessionBridge(opts: BridgeOptions): AcpSessionBridge {
  ...
  const releaseFreshSessionReservation = (
    ...
    try {
      ...
      opts.onDiagnosticLine?.(
        `hopcode serve: fresh session admission release failed: ${
  ...
  const emitSessionLifecycle = (event: BridgeSessionLifecycleEvent): void => {
    try {
      ...
      const message = `hopcode serve: session lifecycle callback failed: ${
        err instanceof Error ? err.message : String(err)
      }`;
  ...
  async function killChannelWithLog(
    ...
    await ci.channel.kill().catch((err) => {
      writeStderrLine(
        `hopcode serve: channel kill failed${context ? ` (${context})` : ''}: ${String(err)}`,
      );
    });
  }
  ...
  async function startIdleTimer(
    ...
    idleTimer = setTimeout(() => {
      ...
      if (hasNoChannelWork(ci)) {
        writeStderrLine(
          `hopcode serve: idle timeout (${timeoutMs}ms) expired, killing channel`,
        );
  ...
  function startSessionReaper(): void {
    ...
    writeStderrLine(
      `hopcode serve: session reaper started ` +
    ...
    sessionReaper = setInterval(() => {
      ...
      for (const [id, entry] of byId) {
        ...
        writeStderrLine(
          `hopcode serve: reaping idle session ${JSON.stringify(id)} ` +
        ...
        void closeSessionImpl(id, undefined, { reason: 'idle_timeout' }).catch(
          (err) => {
            writeStderrLine(
              `hopcode serve: session reaper failed to close ` +
  ...
  const rollbackAttachRegistration = async (
    ...
    if (
      ...
      await closeSessionImpl(entry.sessionId, undefined, {
        ...
        writeStderrLine(
          `hopcode serve: close-on-attach-rollback failed for ` +
    ...
    try {
      ...
      if (entry.parentSessionId) {
        ...
        if (parentSessionPersisted === false) {
          ...
          writeStderrLine(
            `hopcode serve: parentSessionId for ${entry.sessionId} was not persisted ` +
      ...
      if (entry.sourceType) {
        try {
          ...
          writeStderrLine(
            `hopcode serve: source metadata for ${entry.sessionId} was not persisted ` +
  ...
  const notifyAgentSessionClose = async (
    ...
    try {
      ...
      writeStderrLine(
        `hopcode serve: ${label} ACP session close notification failed ` +
  ...
  const broadcastWorkspaceEvent = (
    ...
    if (eligible > 0 && successCount === 0 && !shuttingDown) {
      writeStderrLine(
        `hopcode serve: broadcastWorkspaceEvent type=${envelope.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`,
      );
    }
  };
  ...
  async function closeSessionImpl(
    ...
    writeStderrLine(
      `hopcode serve: closing session ${JSON.stringify(sessionId)}` +
        ` (reason: ${reason})` +
        (originatorClientId
    ...
    if (!ci) {
      writeStderrLine(
        `hopcode serve: closeSession channelInfoForEntry returned undefined ` +
    ...
    try {
      await telemetry.withSpan(
        ...
        {
          'hopcode.daemon.bridge.operation':
            'session.close.cancel_active_prompt',
          'session.id': sessionId,
        },
  ...
  const bridgeApi: AcpSessionBridge = {
    ...
    sendPrompt(sessionId, req, signal, context) {
      opts.onDiagnosticLine?.(
        `hopcode serve: bridge sendPrompt for session=${sessionId}`,
        'info',
      );
        ...
        pendingAbort.signal.addEventListener(
          ...
          () => {
            ...
            void entry.connection
              ...
              .catch((error) => {
                writeStderrLine(
                  `hopcode serve: Todo Stop Guard queued-prompt release failed for ` +
                  ...
                  .finally(() => {
                    ...
                    if (
                      ...
                      void closeSessionImpl(sessionId, undefined, {
                        ...
                        writeStderrLine(
                          `hopcode serve: deferred close-on-prompt-complete failed for ` +
    ...
    async cancelSession(sessionId, req, context) {
      opts.onDiagnosticLine?.(
        `hopcode serve: bridge cancelSession for session=${sessionId}`,
        'info',
      );
      const entry = byId.get(sessionId);
      if (!entry) throw new SessionNotFoundError(sessionId);
    ...
    respondToPermission(requestId, response, context) {
      ...
      if (sessionId === undefined || !byId.has(sessionId)) {
        ...
        writeStderrLine(
          `hopcode serve: legacy permission vote ${JSON.stringify(requestId)} ` +
    ...
    respondToSessionPermission(sessionId, requestId, response, context) {
      ...
      if (actualSessionId === undefined) {
        writeStderrLine(
          `hopcode serve: rejected permission vote ${JSON.stringify(requestId)} ` +
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              writeStderrLine(
                `hopcode serve: branchSession cleanup of ${result.newSessionId} failed: ${cleanupErr instanceof Error ? cleanupErr.message : cleanupErr}`,
              );
            }
            throw restoreErr;
          }
    ...
    async changeSessionCwd(
      ...
      writeStderrLine(
        `hopcode serve: session ${sessionId} cwd changed: ` +
    ...
    updateSessionMetadata(sessionId, metadata, context) {
      ...
      if (metadata.displayName !== undefined) {
        ...
        if (entry.displayName !== nextDisplayName) {
          ...
          writeStderrLine(
            `hopcode serve: updated session metadata ${JSON.stringify(sessionId)} ` +
            ...
            entry.connection
              ...
              .then((res: unknown) => {
                ...
                if (r && r.persisted === false) {
                  writeStderrLine(
                    `hopcode serve: displayName for ${sessionId} was not persisted`,
                  );
                }
              })
              .catch((err: unknown) => {
                writeStderrLine(
                  `hopcode serve: failed to persist displayName for ${sessionId}: ${
                    err instanceof Error ? err.message : String(err)
                  }`,
                );
              });
    ...
    publishWorkspaceEvent(event) {
      ...
      if (sessions.length > 0 && successCount === 0 && !shuttingDown) {
        writeStderrLine(
          `hopcode serve: publishWorkspaceEvent type=${event.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`,
        );
      }
    },
    ...
    async generateSessionRecap(sessionId, _context) {
      ...
      if (!info || info.isDying) throw new SessionNotFoundError(sessionId);
      opts.onDiagnosticLine?.(
        `hopcode serve: bridge generateSessionRecap dispatching ext-method for session=${sessionId}`,
        'info',
      );
      const response = (await Promise.race([
        withTimeout(
      ...
      opts.onDiagnosticLine?.(
        `hopcode serve: bridge generateSessionRecap completed for session=${sessionId} recap=${response.recap ? `len=${response.recap.length}` : 'null'}`,
        'info',
      );
      return {
        sessionId: entry.sessionId,
    ...
    async launchSessionForkAgent(sessionId, directive, context) {
      ...
      return entry.promptQueue.then(async () => {
        ...
        opts.onDiagnosticLine?.(
          `hopcode serve: launchSessionForkAgent requested for session=${sessionId}`,
          'info',
        );
        ...
        try {
          ...
          opts.onDiagnosticLine?.(
            `hopcode serve: launchSessionForkAgent failed for session=${sessionId}: ${
        ...
        opts.onDiagnosticLine?.(
          `hopcode serve: launchSessionForkAgent completed for session=${sessionId} launched=${result.launched}`,
          'info',
        );
        return result;
      });
    ...
    async executeShellCommand(
      ...
      opts.onDiagnosticLine?.(
        `hopcode serve: bridge executeShellCommand for session=${sessionId}`,
        'info',
      );
    ...
    async killSession(sessionId, opts) {
      ...
      if (!ci) {
        ...
        writeStderrLine(
          `hopcode serve: killSession channelInfoForEntry returned undefined ` +
            `for session ${JSON.stringify(sessionId)} — channel cleanup skipped (entry's channel already torn down)`,
        );
      }
      if (ci && ci.channel === entry.channel) {
    ...
    async detachClient(sessionId, clientId) {
      ...
      if (
        ...
        await closeSessionImpl(sessionId, undefined, {
          ...
          writeStderrLine(
            `hopcode serve: close-on-last-detach failed for ` +
              `${JSON.stringify(sessionId)}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
          );
        });
      }
