 ...
 * daemon or IDE environment leak it into per-session `hopcode --acp`
 ...
 *      `{ HOPCODE_SERVER_TOKEN: 'x' }` in overrides can't smuggle the
...
function killChild(child) {
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            if (!resolved) {
                process.stderr.write(`hopcode serve: killChild hard deadline (${KILL_HARD_DEADLINE_MS}ms) ` +
