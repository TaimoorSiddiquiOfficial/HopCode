...
export class BridgeClient {
    ...
    async requestPermission(params) {
        ...
        if (entry.pendingPermissionIds.size >= this.maxPendingPerSession) {
            writeStderrLine(`hopcode serve: session ${entry.sessionId} exceeded ` +
        ...
        if (drained.length > 0) {
            ...
            for (const item of drained) {
                ...
                if (group)
                    ...
                    byOriginator.set(item.originatorClientId, [item.text]);
            }
    ...
    async extNotification(method, params) {
        if (method === 'hopcode/notify/session/generation/event') {
        ...
        if (method === 'hopcode/notify/session/model-update') {
            this.handleInSessionModelUpdate(params);
            return;
        }
        if (method === 'hopcode/notify/session/mode-update') {
        ...
        if (method === 'hopcode/notify/session/recording-degraded') {
            const sessionId = params['sessionId'];
            if (params['v'] !== 1 ||
                typeof sessionId !== 'string' ||
                sessionId.length === 0 ||
        ...
        if (method === 'hopcode/notify/session/terminal-sequence') {
            const sessionId = params['sessionId'];
            if (typeof sessionId !== 'string')
                return;
        ...
        if (method !== 'hopcode/notify/session/mcp-budget-event')
            return;
        const sessionId = params['sessionId'];
        if (typeof sessionId !== 'string')
            return;
