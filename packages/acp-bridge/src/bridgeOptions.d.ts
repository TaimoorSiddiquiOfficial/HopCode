 ...
 * `cli/src/serve/`.
 */
import type { ApprovalMode, DaemonBridgeTelemetryMetrics } from '@hoptrendy/hopcode-core';
...
export interface BridgeOptions {
     ...
     * wraps its `WorkspaceFileSystem` and a `runHopCodeServe` wiring
     * patch so production `hopcode serve` writes pick up its TOCTOU +
     ...
     * time; `runHopCodeServe` reads it from `settings.policy.
     ...
     * audit consumer.
     *
     * **In production** (`hopcode serve`), `runHopCodeServe.ts` allocates a
 ...
 * `JSONRPCMessage`.
 */
export type ClientMcpMessageSender = (serverName: string) => ((payload: unknown) => Promise<unknown>) | undefined;
