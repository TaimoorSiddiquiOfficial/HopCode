...
import { DAEMON_TRACEPARENT_META_KEY, DAEMON_TRACESTATE_META_KEY, SESSION_ARTIFACT_PERSISTENCE_VERSION, TrustGateError, normalizeSnapshotPayload, ShellExecutionService, } from '@hoptrendy/hopcode-core';
import { EventBus, DEFAULT_RING_SIZE, EVENT_SCHEMA_VERSION, } from './eventBus.js';
import { normalizeCompactedReplayMaxBytes, TurnBoundaryCompactionEngine, } from './compactionEngine.js';
import { BridgeChannelClosedError, BridgeTimeoutError, createIdleWorkspaceExtensionsStatus, createIdleWorkspaceHooksStatus, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, STATUS_SCHEMA_VERSION, } from './status.js';
...
InvalidPermissionOptionError, InvalidSessionMetadataError, isNotCurrentlyGeneratingCancelError, SessionBusyError, InvalidRewindTargetError, } from './bridgeErrors.js';
import { canonicalizeWorkspace } from './workspacePaths.js';
import { parseSessionSource } from './session-source.js';
import { CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, LOAD_REPLAY_BULK_MODE, LOAD_REPLAY_META_KEY, LOAD_REPLAY_MODE_META_KEY, LOAD_REPLAY_PAGE_SIZE_META_KEY, LOAD_REPLAY_VERSION, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, } from './bridgeTypes.js';
...
function getCanonicalModelId(response, fallback) {
    if (!isRecord(response) || !isRecord(response['_meta']))
        return fallback;
    const modelSwitch = response['_meta']['hopcodeModelSwitch'];
...
function extractLoadReplayResponse(state) {
    ...
    if (replayError !== undefined && typeof replayError !== 'string') {
        throw new Error(`Invalid hopcode.session.loadReplay replayError ` +
    ...
    if (hasMore !== undefined && typeof hasMore !== 'boolean') {
        throw new Error(`Invalid hopcode.session.loadReplay hasMore ` +
    ...
    if (invalidUpdateIndex !== -1) {
        ...
        const discriminator = isRecord(invalidUpdate)
            ...
            : undefined;
        throw new Error(`Invalid hopcode.session.loadReplay update at index ${invalidUpdateIndex} ` +
...
function broadcastPromptCancelledOnce(entry, sessionId, promptId, originatorClientId, reason) {
    if (entry.cancelBroadcast) {
        writeStderrLine(`broadcastPromptCancelledOnce: suppressed duplicate cancel for session ${sessionId} (latch already set)`);
        return;
    }
...
export function createAcpSessionBridge(opts) {
    ...
    const reserveFreshSession = (context) => opts.freshSessionAdmission?.(context);
    ...
    async function killChannelWithLog(ci, context) {
        ...
        await ci.channel.kill().catch((err) => {
            writeStderrLine(`hopcode serve: channel kill failed${context ? ` (${context})` : ''}: ${String(err)}`);
        });
    }
    ...
    async function startIdleTimer(ci, context) {
        ...
        idleTimer = setTimeout(() => {
            ...
            if (hasNoChannelWork(ci)) {
                writeStderrLine(`hopcode serve: idle timeout (${timeoutMs}ms) expired, killing channel`);
    ...
    async function reapPendingEmptyChannel(ci) {
        if (!ci.emptyReapPending || !hasNoChannelWork(ci))
            return;
    ...
    function startSessionReaper() {
        if (sessionReapIntervalMs <= 0 || sessionIdleTimeoutMs <= 0) {
            writeStderrLine('hopcode serve: session reaper disabled');
            return;
        }
        writeStderrLine(`hopcode serve: session reaper started ` +
        ...
        sessionReaper = setInterval(() => {
            ...
            for (const [id, entry] of byId) {
                ...
                if (idle < sessionIdleTimeoutMs)
                    continue;
                writeStderrLine(`hopcode serve: reaping idle session ${JSON.stringify(id)} ` +
                    `(idle for ${Math.round(idle / 1000)}s, ` +
                    `threshold ${Math.round(sessionIdleTimeoutMs / 1000)}s)`);
                void closeSessionImpl(id, undefined, { reason: 'idle_timeout' }).catch((err) => {
                    writeStderrLine(`hopcode serve: session reaper failed to close ` +
    ...
    const rollbackAttachRegistration = async (entry, clientId, attachCountDelta = 1) => {
        ...
        else if (entry.clientIds.size === 0 &&
            entry.events.subscriberCount === 0 &&
            !entry.promptActive) {
    ...
    async function ensureChannel() {
        ...
        const promise = (async () => {
            ...
            if (aliveChannels.size > 2) {
                writeStderrLine(`hopcode serve: WARNING aliveChannels.size=${aliveChannels.size} ` +
            ...
            void channel.exited.then((exitInfo) => {
                ...
                if (!shuttingDown) {
                    telemetry.event('channel.exited', {
                        'hopcode.daemon.channel.exit_code': exitInfo?.exitCode ?? -1,
                        'hopcode.daemon.channel.session_count': sessions.length,
                        ...(exitInfo?.signalCode
                            ? { 'hopcode.daemon.channel.signal': exitInfo.signalCode }
                            : {}),
                    });
                    writeStderrLine(`hopcode serve: channel exited (code=${exitInfo?.exitCode ?? 'none'}, signal=${exitInfo?.signalCode ?? 'none'}, ${sessions.length} session(s) torn down)`);
                }
            ...
            try {
                await telemetry.withSpan('channel.initialize', {
                    ...
                    const response = await withTimeout(connection.initialize({
        ...
        try {
            try {
                newSessionResp = await telemetry.withSpan('session.new', {
                    ...
                    const response = await withTimeout(ci.connection.newSession(telemetry.injectPromptContext({
                        cwd: boundWorkspace,
                        mcpServers: [],
            ...
            const entry = createSessionEntry(ci, newSessionResp.sessionId, boundWorkspace, undefined, { parentSessionId, sourceType, sourceId });
    ...
    const requestSessionStatus = async (sessionId, method, params = {}) => {
    ...
    const notifyAgentSessionClose = async (entry, ci, label, opts) => {
        if (!ci || ci.channel !== entry.channel) {
            if (opts?.throwOnFailure === true) {
                writeStderrLine(`hopcode serve: ${label} ACP session close channel unavailable ` +
        ...
        try {
            await Promise.race([
                withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionClose, {
                    sessionId: entry.sessionId,
                    ...(opts?.requireFlush === true ? { requireFlush: true } : {}),
                }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionClose),
                getTransportClosedReject(entry),
            ]);
        }
        catch (err) {
            writeStderrLine(`hopcode serve: ${label} ACP session close notification failed ` +
    ...
    const broadcastWorkspaceEvent = (envelope, skipSessionId) => {
        ...
        for (const entry of sessions) {
            ...
            try {
                ...
                if (published === undefined) {
                    failureCount += 1;
                    teeServeDebugLine(`broadcastWorkspaceEvent: publish on session ${entry.sessionId} no-op (bus closed)`);
                }
                else {
                    successCount += 1;
                }
                ...
                if (shuttingDown) {
                    ...
                    writeStderrLine(`hopcode serve: ${detail}`);
                }
            }
        }
        ...
        if (eligible > 0 && successCount === 0 && !shuttingDown) {
            writeStderrLine(`hopcode serve: broadcastWorkspaceEvent type=${envelope.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`);
        }
    };
    const createSessionEventBus = () => new EventBus(eventRingSize, undefined, new TurnBoundaryCompactionEngine({
    ...
    const createSessionEntry = (ci, sessionId, workspaceCwd, events = createSessionEventBus(), options = {}) => {
        const entry = {
            sessionId,
            workspaceCwd,
    ...
    async function restoreSession(action, req, options = {}) {
        ...
        const promise = (async () => {
            ...
            if (racedEntry) {
                ...
                if (req.approvalMode) {
                    try {
                        await applyApprovalMode(racedEntry, req.approvalMode, false, clientId);
                    }
                    catch (err) {
            ...
            if (replayUpdates.length > 0) {
                ...
                if (req.historyPageSize !== undefined &&
                    entry.events
                        .snapshotReplay()
    ...
    async function closeSessionImpl(sessionId, context, closeOpts) {
        ...
        writeStderrLine(`hopcode serve: closing session ${JSON.stringify(sessionId)}` +
            ` (reason: ${reason})` +
            (originatorClientId
                ? ` by client ${JSON.stringify(originatorClientId)}`
                : ''));
        telemetry.event('session.close', {
            'hopcode.daemon.bridge.operation': 'session.close',
        ...
        if (!ci) {
            writeStderrLine(`hopcode serve: closeSession channelInfoForEntry returned undefined ` +
        ...
        try {
            await telemetry.withSpan('session.close.cancel_active_prompt', {
                'hopcode.daemon.bridge.operation': 'session.close.cancel_active_prompt',
                'session.id': sessionId,
    ...
    const bridgeApi = {
        ...
        async spawnOrAttach(req) {
            ...
            if (effectiveScope === 'single') {
                ...
                if (inFlight) {
                    ...
                    if (req.approvalMode) {
                        await applyApprovalModeForAttach(attachedEntry, req.approvalMode, clientId);
                    }
                    return {
                        ...session,
                        attached: true,
            ...
            const promise = doSpawn(req.modelServiceId, effectiveScope, req.approvalMode, req.clientId, releaseAdmissionOnce, req.parentSessionId, source.sourceType, source.sourceId);
        ...
        sendPrompt(sessionId, req, signal, context) {
            opts.onDiagnosticLine?.(`hopcode serve: bridge sendPrompt for session=${sessionId}`, 'info');
            ...
            if (isQueued) {
                pendingAbort.signal.addEventListener('abort', () => {
                    if (pendingEntry.state !== 'queued')
                        return;
                    if (!entry.todoStopGuardAwaitingQueuedPrompt)
                        return;
            ...
            const result = entry.promptQueue.then(() => telemetry.runWithContext(capturedContext, async () => {
                ...
                try {
                    return await telemetry.withSpan('prompt.dispatch', {
                        'hopcode.daemon.bridge.operation': 'prompt.dispatch',
                        'session.id': sessionId,
                        ...
                        try {
                            ...
                            if (!isRetry && !isContinue) {
                                echoPromptToSessionBus(entry, promptRequest, pendingEntry.promptId, originatorClientId);
                            }
                        }
                        catch (echoErr) {
                        ...
                        const promptPromise = entry.connection
                            ...
                            if (entry.clientIds.size === 0 &&
                                ...
                                void closeSessionImpl(sessionId, undefined, {
                                    ...
                                    writeStderrLine(`hopcode serve: deferred close-on-prompt-complete failed for ` +
                        ...
                        void racedPromise
                            ...
                            entry.connection.cancel({ sessionId }).catch((err) => {
                                writeStderrLine(`[pending-prompt] cancel forward failed after prompt abort session=${sessionId}: ${extractErrorMessage(err)}`);
                            });
                        })
                            .catch(() => { });
            ...
            result.then((promptResult) => {
                broadcastTurnComplete(entry, sessionId, promptResult, pendingEntry.promptId, originatorClientId);
            }, (err) => {
                if (err instanceof DOMException && err.name === 'AbortError')
                    return;
                broadcastTurnError(entry, sessionId, err, pendingEntry.promptId, originatorClientId);
            });
            ...
            result
                ...
                if (entry.pendingPromptCount === 0 &&
                    ...
                    writeStderrLine(`[mid-turn] session=${entry.sessionId} dropped ${entry.midTurnMessageQueue.length} undrained message(s) at idle; browser resends next turn`);
                    entry.midTurnMessageQueue.length = 0;
                }
            })
                .catch(() => { });
        ...
        async cancelSession(sessionId, req, context) {
            opts.onDiagnosticLine?.(`hopcode serve: bridge cancelSession for session=${sessionId}`, 'info');
            const entry = byId.get(sessionId);
            if (!entry)
                throw new SessionNotFoundError(sessionId);
            ...
            broadcastPromptCancelledOnce(entry, sessionId, entry.activePromptId, cancelOriginatorClientId);
            ...
            await telemetry.withSpan('session.cancel', {
                'hopcode.daemon.bridge.operation': 'session.cancel',
                'session.id': sessionId,
            }, async () => {
                try {
        ...
        respondToPermission(requestId, response, context) {
            ...
            if (sessionId === undefined || !byId.has(sessionId)) {
                ...
                writeStderrLine(`hopcode serve: legacy permission vote ${JSON.stringify(requestId)} ` +
        ...
        respondToSessionPermission(sessionId, requestId, response, context) {
            ...
            if (actualSessionId === undefined) {
                writeStderrLine(`hopcode serve: rejected permission vote ${JSON.stringify(requestId)} ` +
        ...
        async branchSession(sessionId, req, context) {
            ...
            const branchResult = entry.promptQueue.then(async () => {
                ...
                try {
                    ...
                    const result = (await withTimeout(ci.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionBranch, {
                        sessionId,
                        cwd: boundWorkspace,
                        name: req.name,
                    }), initTimeoutMs, 'branchSession'));
        ...
        async changeSessionCwd(sessionId, req, context) {
            ...
            const cdPromise = entry.promptQueue.then(async () => {
                ...
                const ci = await ensureChannel();
                const raw = await ci.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionCd, {
                    sessionId,
                    path: req.path,
                });
                const extResult = raw;
            ...
            writeStderrLine(`hopcode serve: session ${sessionId} cwd changed: ` +
        ...
        updateSessionMetadata(sessionId, metadata, context) {
            ...
            if (metadata.displayName !== undefined) {
                ...
                if (entry.displayName !== nextDisplayName) {
                    ...
                    writeStderrLine(`hopcode serve: updated session metadata ${JSON.stringify(sessionId)} ` +
                    ...
                    if (nextDisplayName) {
                        entry.connection
                            ...
                            .catch((err) => {
                            writeStderrLine(`hopcode serve: failed to persist displayName for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`);
                        });
                    }
                    try {
                        entry.events.publish({
        ...
        publishWorkspaceEvent(event) {
            ...
            for (const entry of sessions) {
                try {
                    ...
                    if (published === undefined) {
                        failureCount += 1;
                        teeServeDebugLine(`publishWorkspaceEvent: publish on session ${entry.sessionId} no-op (bus closed)`);
                    }
                    else {
                        successCount += 1;
                    }
            ...
            if (sessions.length > 0 && successCount === 0 && !shuttingDown) {
                writeStderrLine(`hopcode serve: publishWorkspaceEvent type=${event.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`);
            }
        },
        knownClientIds() {
            ...
            const results = await Promise.all(sessions.map(async (entry) => {
                ...
                try {
                    ...
                    if (!inFlight || inFlight.connection !== entry.connection) {
                        const promise = (async () => {
                            await entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh, { sessionId: entry.sessionId });
                        })();
                    ...
                    await Promise.race([
                        withTimeout(inFlight.promise, 30_000, SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh),
                        getTransportClosedReject(entry),
                    ]);
        ...
        async generateSessionRecap(sessionId, _context) {
            ...
            if (!info || info.isDying)
                throw new SessionNotFoundError(sessionId);
            opts.onDiagnosticLine?.(`hopcode serve: bridge generateSessionRecap dispatching ext-method for session=${sessionId}`, 'info');
            const response = (await Promise.race([
            ...
            opts.onDiagnosticLine?.(`hopcode serve: bridge generateSessionRecap completed for session=${sessionId} recap=${response.recap ? `len=${response.recap.length}` : 'null'}`, 'info');
            return {
                sessionId: entry.sessionId,
        ...
        enqueueMidTurnMessage(sessionId, message, context) {
            ...
            if (trimmed.length === 0 || entry.pendingPromptCount === 0) {
                ...
                writeStderrLine(`[mid-turn] session=${entry.sessionId} rejected: ${trimmed.length === 0 ? 'empty message' : 'session idle'}; browser keeps it for next turn`);
            ...
            if (entry.midTurnMessageQueue.length >= MAX_MID_TURN_QUEUE_DEPTH) {
                writeStderrLine(`[mid-turn] session=${entry.sessionId} rejected: queue full (depth ${entry.midTurnMessageQueue.length} >= ${MAX_MID_TURN_QUEUE_DEPTH}); browser keeps it for next turn`);
                return { accepted: false };
            }
        ...
        async launchSessionForkAgent(sessionId, directive, context) {
            ...
            return entry.promptQueue.then(async () => {
                ...
                opts.onDiagnosticLine?.(`hopcode serve: launchSessionForkAgent requested for session=${sessionId}`, 'info');
                let response;
                try {
                    ...
                    opts.onDiagnosticLine?.(`hopcode serve: launchSessionForkAgent failed for session=${sessionId}: ${error instanceof Error ? error.message : String(error)}`, 'warn');
                    throw error;
                }
                const result = {
                    sessionId: entry.sessionId,
                ...
                opts.onDiagnosticLine?.(`hopcode serve: launchSessionForkAgent completed for session=${sessionId} launched=${result.launched}`, 'info');
                return result;
            });
        },
        async executeShellCommand(sessionId, command, signal, context) {
            opts.onDiagnosticLine?.(`hopcode serve: bridge executeShellCommand for session=${sessionId}`, 'info');
            ...
            try {
                ...
                const historyOutput = output.length > MAX_SHELL_OUTPUT_FOR_HISTORY
                    ...
                    : output;
                try {
                    await entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionShellHistory, { sessionId, command, output: historyOutput, exitCode });
                }
                catch (err) {
        ...
        async reloadWorkspaceMcp() {
            ...
            try {
                const result = (await Promise.race([
                    withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpReload, { cwd: boundWorkspace }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.workspaceMcpReload),
                    getChannelClosedReject(info),
                ]));
                if (result.accepted) {
                    beginWorkspaceMcpDiscovery(info);
        ...
        async killSession(sessionId, opts) {
            ...
            if (!ci) {
                ...
                writeStderrLine(`hopcode serve: killSession channelInfoForEntry returned undefined ` +
                    `for session ${JSON.stringify(sessionId)} — channel cleanup skipped (entry's channel already torn down)`);
            }
            if (ci && ci.channel === entry.channel) {
                ci.sessionIds.delete(sessionId);
        ...
        async detachClient(sessionId, clientId) {
            ...
            if (entry.spawnOwnerWantedKill &&
                ...
                await closeSessionImpl(sessionId, undefined, {
                    ...
                    writeStderrLine(`hopcode serve: close-on-last-detach failed for ` +
                        `${JSON.stringify(sessionId)}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
                });
            }
        },
        ...
        async preheat() {
            if (shuttingDown)
                return;
            await telemetry.withSpan('channel.preheat', { 'hopcode.daemon.bridge.operation': 'channel.preheat' }, async () => {
                const ci = await ensureChannel();
