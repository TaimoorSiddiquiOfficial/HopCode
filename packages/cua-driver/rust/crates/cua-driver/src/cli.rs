...
pub fn parse_command() -> Command {
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("                                  identity (com.HopCode.cua-driver). If no daemon is running it reports");
        ...
        println!("                                  to com.HopCode.cua-driver (not your terminal), wait for the grant, then");
...
pub fn run_call(
    ...
    // (LaunchServices' "responsible" process), not to com.HopCode.cua-driver —
...
/// process*, so the ONLY process that can read `com.HopCode.cua-driver`'s real
...
fn run_permissions_status(json: bool) {
    ...
    // Only a listening daemon can answer for com.HopCode.cua-driver. A failed/!ok
    ...
    let Some(structured) = daemon_status else {
        ...
        if json {
            let payload = serde_json::json!({
                ...
                "reason": "no Qwen Cua Driver daemon is running under the driver's own identity \
                           (com.HopCode.cua-driver), so its real TCC status can't be read from this \
        ...
        println!(
            "No Qwen Cua Driver daemon is running under the driver's own identity (com.HopCode.cua-driver), \
...
/// com.HopCode.cua-driver, wait (user-paced) for the daemon to come up — its socket
...
///   - TCC DB rows for com.HopCode.cua-driver (sqlite3, best-effort)
...
fn diagnose_tcc_db_section() -> String {
    ...
    let sql = "SELECT service, client, client_type, auth_value, auth_reason, \
               hex(csreq) AS csreq_hex FROM access WHERE client='com.HopCode.cua-driver';";

    let mut lines = vec!["## tcc database rows for com.HopCode.cua-driver".to_owned()];
