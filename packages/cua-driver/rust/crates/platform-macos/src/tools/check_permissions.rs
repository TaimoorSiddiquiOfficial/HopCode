...
///   - the **Qwen Cua Driver daemon** (`com.HopCode.cua-driver`) when this process is
...
///     standalone check can read `true` while `tccutil … com.HopCode.cua-driver`
///     reports no record.
    ...
    let (attribution, note) = if is_driver_daemon {
        (
            ...
            "These booleans reflect the Qwen Cua Driver daemon's own TCC identity \
             (com.HopCode.cua-driver) because this process is its own responsible \
             process.",
        )
    } else {
        (
            ...
            "These booleans reflect the TCC identity of the app that launched \
             ...
             (com.HopCode.cua-driver). A standalone check can read `true` here while \
             `tccutil … com.HopCode.cua-driver` reports no record. To grant for the \
        ...
        if is_caller {
            summary.push_str(
                "\nℹ️  Status reflects the launching app's TCC identity, not the Qwen Cua Driver \
                 daemon (com.HopCode.cua-driver). See `source` for details.",
            );
        }
