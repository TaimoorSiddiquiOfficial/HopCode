...
_CUA_INSTALL_COMMON_URL="https://raw.githubusercontent.com/QwenLM/hopcode/main/packages/cua-driver/scripts/_install-common.sh"
...
# a stable bundle id (com.HopCode.cua-driver) to attribute the daemon to.
...
# Identical to the Swift driver's QwenCuaDriver.app + com.HopCode.cua-driver
...
# /Applications/QwenCuaDriver.app is shared (bundle id com.HopCode.cua-driver) and the
...
case "$OS-$ARCH_RAW" in
    ...
    *)
        ...
        err "  Windows users: install via install.ps1 (irm https://raw.githubusercontent.com/QwenLM/hopcode/main/packages/cua-driver/scripts/install.ps1 | iex)."
        exit 1
        ;;
esac
...
#   `com.HopCode.cua-driver` via `open -n -g -a QwenCuaDriver`. The
...
# bin into the bundle so `~/.local/bin/hopcode-cua-driver` resolves into
...
# install (`/Applications/QwenCuaDriver.app`, `com.HopCode.cua-driver`), so an
...
if [[ "$OS" == "Darwin" && -n "$SRC_APP" && -d "$SRC_APP" ]]; then
    ...
    # /Applications/QwenCuaDriver.app with bundle id `com.HopCode.cua-driver` —
    ...
    if [[ -e "$APP_DEST" ]]; then
        ...
        if [[ "$PREV_BUNDLE_ID" == "com.HopCode.cua-driver" ]] && [[ -n "$PREV_BUNDLE_VERSION" ]]; then
...
if [[ "$NO_MODIFY_PATH" != "1" ]] && [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    ...
    if [[ -n "$SHELL_RC" ]]; then
        {
            printf '\n# Added by cua-driver-rs installer — see https://github.com/QwenLM/hopcode\n'
...
if [[ "${REPLACED_SWIFT:-0}" == "1" ]]; then
    ...
    echo "(com.HopCode.cua-driver) — which is preserved — so they transfer to the new"
...
HINTS_URL="https://raw.githubusercontent.com/QwenLM/hopcode/main/packages/cua-driver/scripts/post-install-hints.txt"
