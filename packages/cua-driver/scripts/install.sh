...
RUST_INSTALLER_URL="https://raw.githubusercontent.com/QwenLM/hopcode/main/packages/cua-driver/scripts/_install-rust.sh"
    ...
    if [[ "$NO_MODIFY_PATH" == "1" ]]; then
        ...
        if [[ -n "$RC_FILE" ]]; then
            ...
            if [[ -f "$RC_FILE" ]] && grep -qF "$BIN_DIR" "$RC_FILE"; then
                ...
                {
                    printf '\n# Added by cua-driver installer — see https://github.com/QwenLM/hopcode\n'
                    printf '%s\n' "$EXPORT_LINE"
                } >> "$RC_FILE"
