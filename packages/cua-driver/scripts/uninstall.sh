...
#   - ~/.local/bin/hopcode-cua-driver symlink (+ legacy /usr/local/bin/hopcode-cua-driver)
...
#   Linux:
#     - ~/.local/bin/hopcode-cua-driver symlink (only when it resolves to a
...
#     - ~/.local/bin/hopcode-cua-driver symlink (only when it resolves into
...
# symlink use the same bundle id (com.HopCode.cua-driver) as the Swift driver,
...
# com.HopCode.cua-driver is shared with the retired Swift driver, and keeping
...
# Automation for com.HopCode.cua-driver. macOS-only; no-op elsewhere.
maybe_reset_tcc() {
    ...
    log "revoking TCC grants for com.HopCode.cua-driver (--reset-tcc)"
    log "  note: com.HopCode.cua-driver is shared with the retired Swift driver;"
    log "  this clears grants for both. The next launch will re-prompt."
    for SVC in Accessibility ScreenCapture AppleEvents; do
        if tccutil reset "$SVC" com.HopCode.cua-driver >/dev/null 2>&1; then
            log "  reset $SVC"
        else
...
# ----------------------------------------------------------------------
if [[ "$USE_RUST_BACKEND" == "1" ]]; then
    USER_BIN_LINK="$HOME/.local/bin/hopcode-cua-driver"
    # Canonical bundle path for this vendored fork (distinct bundle id
    # `com.HopCode.cua-driver` so it coexists with any upstream trycua
    ...
    # is shared with the Swift driver (same bundle id `com.HopCode.cua-driver`),
    ...
    # --- CLI symlink ---
    # Only remove ~/.local/bin/hopcode-cua-driver when it resolves into a
    ...
    # lives (same bundle id `com.HopCode.cua-driver`), so we only remove
...
--reset-tcc — if you want a clean re-install flow:
  ...
  tccutil reset Accessibility com.HopCode.cua-driver
  tccutil reset ScreenCapture com.HopCode.cua-driver
FINALUNMSG
        fi
    else
        cat << 'FINALUNMSG'
...
USER_BIN_LINK="$HOME/.local/bin/hopcode-cua-driver"
SYSTEM_BIN_LINK="/usr/local/bin/hopcode-cua-driver"
APP_BUNDLE="/Applications/QwenCuaDriver.app"
