...
#   $HOME/.local/bin/hopcode-cua-driver -> .../QwenCuaDriver.app/Contents/MacOS/qwen-cua-driver
#
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "                is attributed to com.HopCode.cua-driver (not your terminal),"
...
# identifier (com.HopCode.cua-driver), not the bare executable path. A loose
        ...
        case "$NEW_IDENTITY" in
            cert:*)
                if [ "$NEW_IDENTITY" != "$OLD_IDENTITY" ]; then
                    tccutil reset Accessibility com.HopCode.cua-driver >/dev/null 2>&1 || true
                    tccutil reset ScreenCapture com.HopCode.cua-driver >/dev/null 2>&1 || true
...
# that actually runs carries the com.HopCode.cua-driver identity (TCC keys
...
if [ "$INSTALL_AUTOSTART" != true ]; then
    ...
    if [ "$OS" = "Darwin" ]; then
        ...
        echo "  A launchd-started daemon is attributed to com.HopCode.cua-driver (not your terminal),"
